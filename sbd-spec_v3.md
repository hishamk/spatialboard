...
Normative spec for the SBD serialization format, version 3. Unlike `sbd-spec.md` / `sbd-spec_v2.md` (which were product/PoC specs), this document covers **only the interchange format**. The reference implementation is `src/serialization/sbd-parser.ts` + `sbd-serializer.ts`; the executable examples live in `src/serialization/__tests__/sbd-roundtrip.test.ts`.
