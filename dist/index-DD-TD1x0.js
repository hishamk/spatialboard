var Ya = Object.defineProperty;
var Ga = (t, e, o) => e in t ? Ya(t, e, { enumerable: !0, configurable: !0, writable: !0, value: o }) : t[e] = o;
var St = (t, e, o) => Ga(t, typeof e != "symbol" ? e + "" : e, o);
import { BlockNoteSchema as ja, defaultBlockSpecs as Va, BlockNoteEditor as Ka } from "@blocknote/core";
import { jsxs as v, jsx as u, Fragment as ut } from "react/jsx-runtime";
import qa, { memo as ce, useRef as at, useState as tt, useEffect as mt, useCallback as it, Component as Za, useMemo as Kt, useLayoutEffect as bi, useContext as Er, createContext as xi, Suspense as Ua, lazy as Qa } from "react";
import { useCreateBlockNote as Ja } from "@blocknote/react";
import { BlockNoteView as $a } from "@blocknote/mantine";
import "@blocknote/mantine/style.css";
import { createPortal as Je, flushSync as _a } from "react-dom";
const tl = "useandom-26T198340PX75pxJACKVERYMINDBUSHWOLF_GQZbfghjklqvwyzrict";
let wt = (t = 21) => {
  let e = "", o = crypto.getRandomValues(new Uint8Array(t |= 0));
  for (; t--; )
    e += tl[o[t] & 63];
  return e;
};
const el = {
  id: "kanban",
  label: "Kanban Board",
  nodes: [
    // Column frames
    { id: "kf1", type: "frame", x: 0, y: 0, w: 300, h: 500, z: 0, data: { label: "To Do", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "kf2", type: "frame", x: 330, y: 0, w: 300, h: 500, z: 0, data: { label: "In Progress", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    { id: "kf3", type: "frame", x: 660, y: 0, w: 300, h: 500, z: 0, data: { label: "Done", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    // To Do stickies
    { id: "ks1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 1", color: "#fecaca", fontSize: 14 } },
    { id: "ks2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Task 2", color: "#fecaca", fontSize: 14 } },
    { id: "ks3", type: "sticky", x: 30, y: 240, w: 240, h: 80, z: 1, data: { text: "Task 3", color: "#fecaca", fontSize: 14 } },
    // In Progress stickies
    { id: "ks4", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 4", color: "#fed7aa", fontSize: 14 } },
    { id: "ks5", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Task 5", color: "#fed7aa", fontSize: 14 } },
    // Done stickies
    { id: "ks6", type: "sticky", x: 690, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 6", color: "#bbf7d0", fontSize: 14 } }
  ]
}, ol = {
  id: "mind-map",
  label: "Mind Map",
  nodes: [
    // Central topic
    { id: "c0", type: "shape", x: 250, y: 200, w: 200, h: 80, z: 0, data: { shape: "ellipse", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "Main Topic" } },
    // Branch topics
    { id: "c1", type: "shape", x: 0, y: 0, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#ec4899", strokeWidth: 2, roughness: 0, fill: "#fdf2f8", fillStyle: "solid", label: "Idea 1" } },
    { id: "c2", type: "shape", x: 540, y: 0, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Idea 2" } },
    { id: "c3", type: "shape", x: 0, y: 400, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Idea 3" } },
    { id: "c4", type: "shape", x: 540, y: 400, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Idea 4" } },
    // Edges from center to branches
    { id: "e1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c1", style: "solid", color: "#ec4899", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c2", style: "solid", color: "#f59e0b", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c3", style: "solid", color: "#22c55e", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c4", style: "solid", color: "#3b82f6", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } }
  ]
}, rl = {
  id: "flowchart",
  label: "Flowchart",
  nodes: [
    // Start (ellipse)
    { id: "fc0", type: "shape", x: 200, y: 0, w: 160, h: 60, z: 0, data: { shape: "ellipse", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Start" } },
    // Process 1
    { id: "fc1", type: "shape", x: 200, y: 120, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Process" } },
    // Decision (diamond)
    { id: "fc2", type: "shape", x: 180, y: 250, w: 200, h: 100, z: 0, data: { shape: "diamond", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Decision?" } },
    // Yes branch
    { id: "fc3", type: "shape", x: 200, y: 420, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Action A" } },
    // No branch
    { id: "fc4", type: "shape", x: 480, y: 270, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Action B" } },
    // End
    { id: "fc5", type: "shape", x: 200, y: 540, w: 160, h: 60, z: 0, data: { shape: "ellipse", stroke: "#ef4444", strokeWidth: 2, roughness: 0, fill: "#fef2f2", fillStyle: "solid", label: "End" } },
    // Edges
    { id: "fe1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc0", toId: "fc1", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc1", toId: "fc2", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc2", toId: "fc3", style: "solid", color: "#22c55e", strokeWidth: 2, arrowHead: "filled", label: "Yes", sourceHandle: "bottom" } },
    { id: "fe4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc2", toId: "fc4", style: "solid", color: "#ef4444", strokeWidth: 2, arrowHead: "filled", label: "No", sourceHandle: "right" } },
    { id: "fe5", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc3", toId: "fc5", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe6", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc4", toId: "fc5", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } }
  ]
}, nl = {
  id: "swot",
  label: "SWOT Analysis",
  nodes: [
    // Quadrant frames
    { id: "sf1", type: "frame", x: 0, y: 0, w: 300, h: 300, z: 0, data: { label: "Strengths", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf2", type: "frame", x: 330, y: 0, w: 300, h: 300, z: 0, data: { label: "Weaknesses", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf3", type: "frame", x: 0, y: 330, w: 300, h: 300, z: 0, data: { label: "Opportunities", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf4", type: "frame", x: 330, y: 330, w: 300, h: 300, z: 0, data: { label: "Threats", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "ss1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Add strength...", color: "#bbf7d0", fontSize: 14 } },
    { id: "ss2", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Add weakness...", color: "#fecaca", fontSize: 14 } },
    { id: "ss3", type: "sticky", x: 30, y: 370, w: 240, h: 80, z: 1, data: { text: "Add opportunity...", color: "#bfdbfe", fontSize: 14 } },
    { id: "ss4", type: "sticky", x: 360, y: 370, w: 240, h: 80, z: 1, data: { text: "Add threat...", color: "#fed7aa", fontSize: 14 } }
  ]
}, sl = {
  id: "retrospective",
  label: "Retrospective",
  nodes: [
    // Column frames
    { id: "rf1", type: "frame", x: 0, y: 0, w: 300, h: 500, z: 0, data: { label: "What went well", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "rf2", type: "frame", x: 330, y: 0, w: 300, h: 500, z: 0, data: { label: "What didn't go well", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "rf3", type: "frame", x: 660, y: 0, w: 300, h: 500, z: 0, data: { label: "Action items", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "rs1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Good thing 1", color: "#bbf7d0", fontSize: 14 } },
    { id: "rs2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Good thing 2", color: "#bbf7d0", fontSize: 14 } },
    { id: "rs3", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Issue 1", color: "#fecaca", fontSize: 14 } },
    { id: "rs4", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Issue 2", color: "#fecaca", fontSize: 14 } },
    { id: "rs5", type: "sticky", x: 690, y: 40, w: 240, h: 80, z: 1, data: { text: "Action 1", color: "#bfdbfe", fontSize: 14 } },
    { id: "rs6", type: "sticky", x: 690, y: 140, w: 240, h: 80, z: 1, data: { text: "Action 2", color: "#bfdbfe", fontSize: 14 } }
  ]
}, il = {
  id: "weekly-planner",
  label: "Weekly Planner",
  nodes: [
    // Day frames
    { id: "wf1", type: "frame", x: 0, y: 0, w: 170, h: 400, z: 0, data: { label: "Monday", backgroundColor: "#6366f115", borderColor: "#6366f1", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf2", type: "frame", x: 180, y: 0, w: 170, h: 400, z: 0, data: { label: "Tuesday", backgroundColor: "#8b5cf615", borderColor: "#8b5cf6", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf3", type: "frame", x: 360, y: 0, w: 170, h: 400, z: 0, data: { label: "Wednesday", backgroundColor: "#ec489915", borderColor: "#ec4899", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf4", type: "frame", x: 540, y: 0, w: 170, h: 400, z: 0, data: { label: "Thursday", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf5", type: "frame", x: 720, y: 0, w: 170, h: 400, z: 0, data: { label: "Friday", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf6", type: "frame", x: 900, y: 0, w: 170, h: 400, z: 0, data: { label: "Saturday", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf7", type: "frame", x: 1080, y: 0, w: 170, h: 400, z: 0, data: { label: "Sunday", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 1, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "ws1", type: "sticky", x: 10, y: 40, w: 150, h: 60, z: 1, data: { text: "Task", color: "#e0e7ff", fontSize: 12 } },
    { id: "ws2", type: "sticky", x: 190, y: 40, w: 150, h: 60, z: 1, data: { text: "Task", color: "#ede9fe", fontSize: 12 } }
  ]
}, al = {
  id: "pros-cons",
  label: "Pros & Cons",
  nodes: [
    // Column frames
    { id: "pf1", type: "frame", x: 0, y: 0, w: 300, h: 450, z: 0, data: { label: "Pros", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "pf2", type: "frame", x: 330, y: 0, w: 300, h: 450, z: 0, data: { label: "Cons", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "ps1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Pro 1", color: "#bbf7d0", fontSize: 14 } },
    { id: "ps2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Pro 2", color: "#bbf7d0", fontSize: 14 } },
    { id: "ps3", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Con 1", color: "#fecaca", fontSize: 14 } },
    { id: "ps4", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Con 2", color: "#fecaca", fontSize: 14 } }
  ]
}, ll = {
  id: "eisenhower",
  label: "Eisenhower Matrix",
  nodes: [
    // Quadrant frames
    { id: "ef1", type: "frame", x: 0, y: 0, w: 300, h: 300, z: 0, data: { label: "Do First (Urgent + Important)", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef2", type: "frame", x: 330, y: 0, w: 300, h: 300, z: 0, data: { label: "Schedule (Important)", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef3", type: "frame", x: 0, y: 330, w: 300, h: 300, z: 0, data: { label: "Delegate (Urgent)", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef4", type: "frame", x: 330, y: 330, w: 300, h: 300, z: 0, data: { label: "Eliminate", backgroundColor: "#6b728015", borderColor: "#6b7280", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "es1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Critical task...", color: "#fecaca", fontSize: 14 } },
    { id: "es2", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Plan for later...", color: "#bfdbfe", fontSize: 14 } },
    { id: "es3", type: "sticky", x: 30, y: 370, w: 240, h: 80, z: 1, data: { text: "Hand off...", color: "#fed7aa", fontSize: 14 } },
    { id: "es4", type: "sticky", x: 360, y: 370, w: 240, h: 80, z: 1, data: { text: "Drop this...", color: "#e5e7eb", fontSize: 14 } }
  ]
}, cl = {
  id: "org-chart",
  label: "Org Chart",
  nodes: [
    // Top level
    { id: "oc0", type: "shape", x: 250, y: 0, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "CEO" } },
    // Second level
    { id: "oc1", type: "shape", x: 0, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Engineering" } },
    { id: "oc2", type: "shape", x: 250, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Product" } },
    { id: "oc3", type: "shape", x: 500, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Marketing" } },
    // Third level
    { id: "oc4", type: "shape", x: -60, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Frontend" } },
    { id: "oc5", type: "shape", x: 80, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Backend" } },
    { id: "oc6", type: "shape", x: 200, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Design" } },
    { id: "oc7", type: "shape", x: 360, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Research" } },
    { id: "oc8", type: "shape", x: 450, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Content" } },
    { id: "oc9", type: "shape", x: 590, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Growth" } },
    // Edges - top to second
    { id: "oe1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc1", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc2", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc3", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    // Edges - second to third
    { id: "oe4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc1", toId: "oc4", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe5", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc1", toId: "oc5", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe6", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc2", toId: "oc6", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe7", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc2", toId: "oc7", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe8", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc3", toId: "oc8", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe9", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc3", toId: "oc9", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } }
  ]
}, dl = {
  id: "timeline",
  label: "Project Timeline",
  nodes: [
    // Phase shapes
    { id: "tl0", type: "shape", x: 0, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "Discovery" } },
    { id: "tl1", type: "shape", x: 230, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Design" } },
    { id: "tl2", type: "shape", x: 460, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Development" } },
    { id: "tl3", type: "shape", x: 690, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Testing" } },
    { id: "tl4", type: "shape", x: 920, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#ec4899", strokeWidth: 2, roughness: 0, fill: "#fdf2f8", fillStyle: "solid", label: "Launch" } },
    // Milestone diamonds below
    { id: "tl5", type: "shape", x: 185, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#6366f1", fillStyle: "solid" } },
    { id: "tl6", type: "shape", x: 415, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#3b82f6", fillStyle: "solid" } },
    { id: "tl7", type: "shape", x: 645, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#22c55e", fillStyle: "solid" } },
    { id: "tl8", type: "shape", x: 875, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#f59e0b", fillStyle: "solid" } },
    // Phase arrows
    { id: "te1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl0", toId: "tl1", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl1", toId: "tl2", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl2", toId: "tl3", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl3", toId: "tl4", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    // Milestone labels
    { id: "tt1", type: "text", x: 155, y: 170, w: 100, h: "auto", z: 1, data: { text: "Kickoff", fontSize: 12, fontFamily: "sans-serif", color: "#6366f1", align: "center" } },
    { id: "tt2", type: "text", x: 385, y: 170, w: 100, h: "auto", z: 1, data: { text: "Specs Done", fontSize: 12, fontFamily: "sans-serif", color: "#3b82f6", align: "center" } },
    { id: "tt3", type: "text", x: 615, y: 170, w: 100, h: "auto", z: 1, data: { text: "Code Freeze", fontSize: 12, fontFamily: "sans-serif", color: "#22c55e", align: "center" } },
    { id: "tt4", type: "text", x: 845, y: 170, w: 100, h: "auto", z: 1, data: { text: "Sign-off", fontSize: 12, fontFamily: "sans-serif", color: "#f59e0b", align: "center" } }
  ]
}, hl = {
  id: "bmc",
  label: "Business Model Canvas",
  nodes: [
    // Top row: 5 sections
    { id: "bf1", type: "frame", x: 0, y: 0, w: 220, h: 320, z: 0, data: { label: "Key Partners", backgroundColor: "#8b5cf615", borderColor: "#8b5cf6", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf2", type: "frame", x: 230, y: 0, w: 220, h: 150, z: 0, data: { label: "Key Activities", backgroundColor: "#6366f115", borderColor: "#6366f1", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf3", type: "frame", x: 230, y: 160, w: 220, h: 160, z: 0, data: { label: "Key Resources", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf4", type: "frame", x: 460, y: 0, w: 220, h: 320, z: 0, data: { label: "Value Propositions", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf5", type: "frame", x: 690, y: 0, w: 220, h: 150, z: 0, data: { label: "Customer Relationships", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf6", type: "frame", x: 690, y: 160, w: 220, h: 160, z: 0, data: { label: "Channels", backgroundColor: "#ec489915", borderColor: "#ec4899", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf7", type: "frame", x: 920, y: 0, w: 220, h: 320, z: 0, data: { label: "Customer Segments", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 1, borderStyle: "solid" } },
    // Bottom row: 2 wide sections
    { id: "bf8", type: "frame", x: 0, y: 340, w: 565, h: 160, z: 0, data: { label: "Cost Structure", backgroundColor: "#6b728015", borderColor: "#6b7280", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf9", type: "frame", x: 575, y: 340, w: 565, h: 160, z: 0, data: { label: "Revenue Streams", backgroundColor: "#14b8a615", borderColor: "#14b8a6", borderWidth: 1, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "bs1", type: "sticky", x: 15, y: 40, w: 190, h: 60, z: 1, data: { text: "Partner...", color: "#ede9fe", fontSize: 12 } },
    { id: "bs2", type: "sticky", x: 475, y: 40, w: 190, h: 60, z: 1, data: { text: "Value prop...", color: "#bbf7d0", fontSize: 12 } },
    { id: "bs3", type: "sticky", x: 935, y: 40, w: 190, h: 60, z: 1, data: { text: "Segment...", color: "#fecaca", fontSize: 12 } }
  ]
}, ul = {
  id: "storyboard",
  label: "Storyboard",
  nodes: [
    // Row 1
    { id: "sb1", type: "frame", x: 0, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 1", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb2", type: "frame", x: 320, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 2", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb3", type: "frame", x: 640, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 3", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    // Row 2
    { id: "sb4", type: "frame", x: 0, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 4", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb5", type: "frame", x: 320, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 5", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb6", type: "frame", x: 640, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 6", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    // Caption text below each frame
    { id: "st1", type: "text", x: 0, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st2", type: "text", x: 320, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st3", type: "text", x: 640, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st4", type: "text", x: 0, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st5", type: "text", x: 320, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st6", type: "text", x: 640, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } }
  ]
}, wi = [
  el,
  ol,
  rl,
  nl,
  sl,
  il,
  al,
  ll,
  cl,
  dl,
  hl,
  ul
];
class fl {
  constructor() {
    St(this, "undoStack", []);
    St(this, "redoStack", []);
    St(this, "maxSize", 50);
  }
  pushSnapshot(e, o) {
    const r = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (r.groupParent = Array.from(o.entries()));
    const n = JSON.stringify(r);
    this.undoStack.push(n), this.undoStack.length > this.maxSize && this.undoStack.shift(), this.redoStack = [];
  }
  undo(e, o) {
    if (this.undoStack.length === 0) return null;
    const r = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (r.groupParent = Array.from(o.entries())), this.redoStack.push(JSON.stringify(r));
    const n = JSON.parse(this.undoStack.pop());
    return {
      nodes: new Map(n.nodes),
      groupParent: new Map(n.groupParent ?? [])
    };
  }
  redo(e, o) {
    if (this.redoStack.length === 0) return null;
    const r = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (r.groupParent = Array.from(o.entries())), this.undoStack.push(JSON.stringify(r));
    const n = JSON.parse(this.redoStack.pop());
    return {
      nodes: new Map(n.nodes),
      groupParent: new Map(n.groupParent ?? [])
    };
  }
  clear() {
    this.undoStack = [], this.redoStack = [];
  }
  canUndo() {
    return this.undoStack.length > 0;
  }
  canRedo() {
    return this.redoStack.length > 0;
  }
}
const ki = 4, pl = 8, yl = 24;
function uo(t, e, o, r) {
  if (!t.rotation) return [e, o];
  const n = t.x + t.w / 2, s = t.y + r / 2, i = -t.rotation * Math.PI / 180, l = Math.cos(i), a = Math.sin(i), c = e - n, h = o - s;
  return [n + c * l - h * a, s + c * a + h * l];
}
function Cr(t, e) {
  return t.h !== "auto" ? t.h : (e == null ? void 0 : e[t.id]) ?? 100;
}
function ml(t, e, o, r = 1, n, s) {
  const i = Array.from(t.values()).filter((c) => c.type !== "edge").sort((c, h) => h.z - c.z);
  let l = null, a = null;
  for (const c of i)
    if (c.type === "draw") {
      if (Wn(c, e, o, r))
        return c;
    } else if (c.type === "shape") {
      if (Wr(c, e, o, r)) return c;
      if (!a && c.data.label) {
        const h = c.h === "auto" ? 100 : c.h, [f, d] = uo(c, e, o, h), p = Mi(c, h);
        p && f >= p.lx && f <= p.rx && d >= p.ly && d <= p.ry && (a = c);
      }
    } else if (s && s.has(c.type)) {
      const h = Cr(c, n);
      vi(c, e, o, r, h) && (l || (l = c));
    } else {
      const h = Cr(c, n), f = ki / r, [d, p] = uo(c, e, o, h);
      d >= c.x - f && d <= c.x + c.w + f && p >= c.y - f && p <= c.y + h + f && (a || (a = c));
    }
  return a ?? l;
}
function vi(t, e, o, r, n) {
  const s = n ?? (t.h === "auto" ? 100 : t.h), [i, l] = uo(t, e, o, s), a = pl / r;
  if (t.data.label && i >= t.x && i <= t.x + t.w && l >= t.y - yl && l <= t.y)
    return !0;
  if (i < t.x - a || i > t.x + t.w + a || l < t.y - a || l > t.y + s + a)
    return !1;
  const h = Math.abs(i - t.x), f = Math.abs(i - (t.x + t.w)), d = Math.abs(l - t.y), p = Math.abs(l - (t.y + s)), y = i >= t.x - a && i <= t.x + t.w + a;
  return l >= t.y - a && l <= t.y + s + a && (h <= a || f <= a) || y && (d <= a || p <= a);
}
function Si(t, e, o, r, n, s) {
  const i = n - o, l = s - r, a = i * i + l * l;
  if (a === 0) return (t - o) ** 2 + (e - r) ** 2;
  const c = Math.max(0, Math.min(1, ((t - o) * i + (e - r) * l) / a)), h = o + c * i, f = r + c * l;
  return (t - h) ** 2 + (e - f) ** 2;
}
function Mi(t, e) {
  const o = t.data;
  if (!o.label) return null;
  const r = o.labelFontSize ?? 14, n = r * 1.3, s = r * 0.55, l = t.w - 12 * 2, a = o.label.split(`
`);
  let c = 0;
  for (const y of a) {
    const m = y.length * s;
    c += Math.max(1, Math.ceil(m / Math.max(l, 1)));
  }
  const h = c * n, f = Math.min(l, Math.max(...a.map((y) => y.length)) * s), d = t.x + t.w / 2, p = t.y + e / 2;
  return {
    lx: d - f / 2 - 4,
    ly: p - h / 2 - 4,
    rx: d + f / 2 + 4,
    ry: p + h / 2 + 4
  };
}
function Wr(t, e, o, r, n) {
  const s = t.h === "auto" ? 100 : t.h, [i, l] = uo(t, e, o, s), a = t.data, h = (a.strokeWidth ?? 2) / 2 / r, f = !!a.fill || !!n;
  switch (a.shape) {
    case "rect": {
      if (f)
        return i >= t.x - h && i <= t.x + t.w + h && l >= t.y - h && l <= t.y + s + h;
      const d = Math.abs(i - t.x), p = Math.abs(i - (t.x + t.w)), y = Math.abs(l - t.y), m = Math.abs(l - (t.y + s)), g = i >= t.x - h && i <= t.x + t.w + h;
      return l >= t.y - h && l <= t.y + s + h && (d <= h || p <= h) || g && (y <= h || m <= h);
    }
    case "ellipse": {
      const d = t.x + t.w / 2, p = t.y + s / 2, y = t.w / 2, m = s / 2;
      if (y === 0 || m === 0) return !1;
      const g = (i - d) / y, b = (l - p) / m, x = g * g + b * b;
      if (f) {
        const z = ((y + h) / y) ** 2;
        return x <= z;
      }
      const C = h / Math.min(y, m);
      return Math.abs(Math.sqrt(x) - 1) <= C;
    }
    case "diamond": {
      const d = t.x + t.w / 2, p = t.y + s / 2, y = t.w / 2, m = s / 2;
      if (y === 0 || m === 0) return !1;
      const g = Math.abs(i - d) / y, b = Math.abs(l - p) / m, x = g + b;
      if (f) {
        const z = h / Math.min(y, m);
        return x <= 1 + z;
      }
      const C = h / Math.min(y, m);
      return Math.abs(x - 1) <= C;
    }
    case "line":
    case "arrow": {
      const d = a.startPoint ?? [0, 0], p = a.endPoint ?? [t.w, s], y = t.x + d[0], m = t.y + d[1], g = t.x + p[0], b = t.y + p[1];
      return Si(i, l, y, m, g, b) <= h * h;
    }
    default:
      return i >= t.x - h && i <= t.x + t.w + h && l >= t.y - h && l <= t.y + s + h;
  }
}
function gl(t, e, o) {
  let r = !1;
  for (let n = 0, s = o.length - 1; n < o.length; s = n++) {
    const i = o[n][0], l = o[n][1], a = o[s][0], c = o[s][1];
    l > e != c > e && t < (a - i) * (e - l) / (c - l) + i && (r = !r);
  }
  return r;
}
function Wn(t, e, o, r) {
  const s = t.data.strokeWidth / 2 / r, i = s * s, l = t.h === "auto" ? 100 : t.h, [a, c] = uo(t, e, o, l);
  if (a < t.x - s || a > t.x + t.w + s || c < t.y - s || c > t.y + l + s)
    return !1;
  const h = t.data.points;
  if (!h || h.length === 0) return !1;
  const f = a - t.x, d = c - t.y;
  if (h.length === 1) {
    const p = f - h[0][0], y = d - h[0][1];
    return p * p + y * y <= i;
  }
  if (t.data.fill && h.length >= 3 && gl(f, d, h))
    return !0;
  for (let p = 0; p < h.length - 1; p++)
    if (Si(f, d, h[p][0], h[p][1], h[p + 1][0], h[p + 1][1]) <= i)
      return !0;
  return !1;
}
function bl(t, e, o, r = 1, n, s) {
  const i = Array.from(t.values()).filter((c) => c.type !== "edge").sort((c, h) => h.z - c.z), l = [], a = [];
  for (const c of i)
    if (c.type === "draw")
      Wn(c, e, o, r) && l.push(c);
    else if (c.type === "shape") {
      if (Wr(c, e, o, r))
        l.push(c);
      else if (c.data.label) {
        const h = c.h === "auto" ? 100 : c.h, [f, d] = uo(c, e, o, h), p = Mi(c, h);
        p && f >= p.lx && f <= p.rx && d >= p.ly && d <= p.ry && a.push(c);
      }
    } else if (s && s.has(c.type)) {
      const h = Cr(c, n);
      vi(c, e, o, r, h) && a.push(c);
    } else {
      const h = Cr(c, n), f = ki / r, [d, p] = uo(c, e, o, h);
      d >= c.x - f && d <= c.x + c.w + f && p >= c.y - f && p <= c.y + h + f && a.push(c);
    }
  return [...l, ...a];
}
function fr(t, e) {
  if (!t.rotation) return { x: t.x, y: t.y, w: t.w, h: e };
  const o = t.x + t.w / 2, r = t.y + e / 2, n = t.w / 2, s = e / 2, i = t.rotation * Math.PI / 180, l = Math.abs(Math.cos(i)), a = Math.abs(Math.sin(i)), c = n * l + s * a, h = n * a + s * l;
  return {
    x: o - c,
    y: r - h,
    w: c * 2,
    h: h * 2
  };
}
const ze = class ze {
  constructor(e, o = 0, r) {
    // Increased depth for potentially large boards
    St(this, "level");
    St(this, "bounds");
    St(this, "objects");
    St(this, "nodes");
    /** Shared across all levels — maps node ID → measured height for auto-height nodes */
    St(this, "heightMap");
    this.bounds = e, this.level = o, this.objects = [], this.nodes = [], this.heightMap = r ?? /* @__PURE__ */ new Map();
  }
  /** Resolve the effective height for a node (uses measured height for auto-height nodes) */
  resolveH(e) {
    return typeof e.h == "number" ? e.h : this.heightMap.get(e.id) ?? 100;
  }
  /** Store a measured height for an auto-height node */
  setMeasuredHeight(e, o) {
    this.heightMap.set(e, o);
  }
  // Clear the quadtree
  clear() {
    this.objects = [];
    for (let e = 0; e < this.nodes.length; e++)
      this.nodes[e].clear();
    this.nodes = [];
  }
  // Split the node into 4 subnodes
  split() {
    const e = this.bounds.w / 2, o = this.bounds.h / 2, r = this.bounds.x, n = this.bounds.y;
    this.nodes[0] = new ze({ x: r + e, y: n, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[1] = new ze({ x: r, y: n, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[2] = new ze({ x: r, y: n + o, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[3] = new ze({ x: r + e, y: n + o, w: e, h: o }, this.level + 1, this.heightMap);
  }
  // Determine which quadrant the object belongs to
  getIndex(e) {
    let o = -1;
    const r = this.bounds.x + this.bounds.w / 2, n = this.bounds.y + this.bounds.h / 2, s = e.y < n && e.y + e.h < n, i = e.y > n;
    return e.x < r && e.x + e.w < r ? s ? o = 1 : i && (o = 2) : e.x > r && (s ? o = 0 : i && (o = 3)), o;
  }
  // Insert the object into the quadtree
  insert(e, o) {
    const r = o ?? this.resolveH(e);
    o != null && e.h === "auto" && this.heightMap.set(e.id, o);
    const n = fr(e, r);
    if (this.nodes.length) {
      const s = this.getIndex(n);
      if (s !== -1) {
        this.nodes[s].insert(e, r);
        return;
      }
    }
    if (this.objects.push(e), this.objects.length > ze.MAX_OBJECTS && this.level < ze.MAX_LEVELS) {
      this.nodes.length || this.split();
      let s = 0;
      for (; s < this.objects.length; ) {
        const i = this.objects[s], l = this.resolveH(i), a = fr(i, l), c = this.getIndex(a);
        c !== -1 ? (this.nodes[c].insert(i, l), this.objects.splice(s, 1)) : s++;
      }
    }
  }
  // Remove an object. Requires the node (with its coordinates) to find it efficiently.
  remove(e) {
    const o = this.objects.findIndex((r) => r.id === e.id);
    if (o !== -1)
      return this.objects.splice(o, 1), !0;
    if (this.nodes.length) {
      const r = this.resolveH(e), n = this.getIndex(fr(e, r));
      if (n !== -1 && this.nodes[n].remove(e))
        return !0;
      for (let s = 0; s < this.nodes.length; s++)
        if (s !== n && this.nodes[s].remove(e)) return !0;
    }
    return !1;
  }
  // Return all objects that could collide with the given rect
  retrieve(e, o) {
    const r = this.getIndex(o);
    for (const n of this.objects) {
      const s = this.resolveH(n), i = fr(n, s);
      i.x < o.x + o.w && i.x + i.w > o.x && i.y < o.y + o.h && i.y + i.h > o.y && e.push(n);
    }
    if (this.nodes.length)
      if (r !== -1)
        this.nodes[r].retrieve(e, o);
      else
        for (const n of this.nodes)
          n.bounds.x < o.x + o.w && n.bounds.x + n.bounds.w > o.x && n.bounds.y < o.y + o.h && n.bounds.y + n.bounds.h > o.y && n.retrieve(e, o);
    return e;
  }
};
// Max number of objects per node before splitting
St(ze, "MAX_OBJECTS", 10), // Max depth of the tree
St(ze, "MAX_LEVELS", 8);
let gn = ze;
function Lo(t, e, o) {
  return Math.min(Math.max(t, e), o);
}
function Eo(t, e, o) {
  return {
    x: (e - t.x) / t.zoom,
    y: (o - t.y) / t.zoom
  };
}
function xl(t, e, o) {
  return {
    x: e * t.zoom + t.x,
    y: o * t.zoom + t.y
  };
}
function wl(t, e, o, r) {
  const n = e > 0 ? 0.95 : 1.05, s = Lo(t.zoom * n, 0.1, 5), i = Eo(t, o, r);
  return {
    x: o - i.x * s,
    y: r - i.y * s,
    zoom: s
  };
}
function kl(t, e, o, r) {
  const n = Lo(t.zoom * e, 0.1, 5), s = Eo(t, o, r);
  return {
    x: o - s.x * n,
    y: r - s.y * n,
    zoom: n
  };
}
const Fn = ja.create({
  blockSpecs: {
    ...Va
    // Future custom blocks:
    // callout: CalloutBlock,
    // aiPrompt: AIPromptBlock,
  }
});
let Qr = null;
function Bn() {
  return Qr || (Qr = Ka.create({ schema: Fn })), Qr;
}
async function vl(t) {
  return await Bn().blocksToMarkdownLossy(t);
}
async function Nn(t) {
  return await Bn().tryParseMarkdownToBlocks(t);
}
function Ci(t) {
  return Bn().tryParseHTMLToBlocks(t);
}
function Sl(t, e, o) {
  const [r, n] = t, [s, i] = e, [l, a] = o, c = l - s, h = a - i, f = c * c + h * h;
  if (f === 0)
    return (r - s) ** 2 + (n - i) ** 2;
  let d = ((r - s) * c + (n - i) * h) / f;
  d = Math.max(0, Math.min(1, d));
  const p = s + d * c, y = i + d * h;
  return (r - p) ** 2 + (n - y) ** 2;
}
function bn(t, e = 1) {
  if (t.length <= 2) return t;
  let o = 0, r = 0;
  const n = t[0], s = t[t.length - 1];
  for (let a = 1; a < t.length - 1; a++) {
    const c = Sl(t[a], n, s);
    c > o && (o = c, r = a);
  }
  if (o <= e)
    return [n, s];
  const i = bn(t.slice(0, r + 1), e), l = bn(t.slice(r), e);
  return [...i.slice(0, -1), ...l];
}
async function Ml(t, e) {
  const o = [], r = ['canvas_w="2000"', 'canvas_h="1500"', 'grid="20"', 'snap="false"'];
  if (e != null && e.background && e.background !== "dot-grid" && r.push(`background="${e.background}"`), e != null && e.originView) {
    const d = e.originView;
    r.push(`originView="${d.x},${d.y},${d.zoom}"`);
  }
  o.push(`<!--@meta ${r.join(" ")} -->`), o.push("");
  const n = t.filter((d) => d.type === "frame").sort((d, p) => d.z - p.z || d.y - p.y || d.x - p.x);
  for (const d of n) {
    const p = d.h === "auto" ? "auto" : Math.round(d.h), y = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${p}"`,
      `z="${d.z}"`
    ];
    d.data.label && y.push(`label="${d.data.label.replace(/"/g, "&quot;")}"`), d.data.backgroundColor && y.push(`backgroundColor="${d.data.backgroundColor}"`), d.data.borderColor && y.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && y.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && y.push(`borderStyle="${d.data.borderStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && y.push(`opacity="${d.data.opacity}"`), d.data.slideOrder != null && y.push(`slideOrder="${d.data.slideOrder}"`), d.data.transition && d.data.transition !== "pan" && y.push(`transition="${d.data.transition}"`), d.data.transitionDuration != null && y.push(`transitionDuration="${d.data.transitionDuration}"`), d.rotation && y.push(`rotation="${d.rotation}"`), d.locked && y.push('locked="true"'), d.groupId && y.push(`group="${d.groupId}"`), o.push(`<!--@frame ${y.join(" ")} -->`), o.push("");
  }
  const s = t.filter((d) => d.type === "content").sort((d, p) => d.z - p.z || d.y - p.y || d.x - p.x);
  for (const d of s) {
    const p = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${d.h}"`,
      `z="${d.z}"`
    ];
    d.rotation && p.push(`rotation="${d.rotation}"`), d.locked && p.push('locked="true"'), d.groupId && p.push(`group="${d.groupId}"`), d.data.borderColor && p.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && p.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && p.push(`borderStyle="${d.data.borderStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && p.push(`opacity="${d.data.opacity}"`), o.push(`<!--@block ${p.join(" ")} -->`);
    const y = d.data.blocks.length > 0 ? await vl(d.data.blocks) : "";
    o.push(y), o.push("");
  }
  const i = t.filter((d) => d.type === "draw");
  for (const d of i) {
    const p = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `z="${d.z}"`,
      `tool="${d.data.tool}"`,
      `color="${d.data.color}"`,
      `width="${d.data.strokeWidth}"`
    ];
    d.data.opacity !== void 0 && d.data.opacity !== 1 && p.push(`opacity="${d.data.opacity}"`), d.data.fill && p.push(`fill="${d.data.fill}"`), d.data.fillStyle && d.data.fillStyle !== "hachure" && p.push(`fillStyle="${d.data.fillStyle}"`), d.rotation && p.push(`rotation="${d.rotation}"`), d.locked && p.push('locked="true"'), d.groupId && p.push(`group="${d.groupId}"`), o.push(`<!--@draw ${p.join(" ")} -->`);
    const m = bn([...d.data.points], 1).map(
      ([g, b, x]) => `${(g + d.x).toFixed(1)},${(b + d.y).toFixed(1)},${x.toFixed(2)}`
    ).join(" ");
    o.push(m), o.push("");
  }
  const l = t.filter((d) => d.type === "shape");
  for (const d of l) {
    const p = d.h === "auto" ? "auto" : Math.round(d.h), y = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${p}"`,
      `z="${d.z}"`,
      'tool="shape"',
      `shape="${d.data.shape}"`,
      `color="${d.data.stroke}"`,
      `stroke="${d.data.strokeWidth}"`,
      `roughness="${d.data.roughness}"`
    ];
    d.data.fill && y.push(`fill="${d.data.fill}"`), d.data.fillStyle && d.data.fillStyle !== "hachure" && y.push(`fillStyle="${d.data.fillStyle}"`), d.data.strokeStyle && d.data.strokeStyle !== "solid" && y.push(`strokeStyle="${d.data.strokeStyle}"`), d.data.edgeStyle && d.data.edgeStyle !== "sharp" && y.push(`edgeStyle="${d.data.edgeStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && y.push(`opacity="${d.data.opacity}"`), d.data.startPoint && y.push(`startPt="${d.data.startPoint[0].toFixed(1)},${d.data.startPoint[1].toFixed(1)}"`), d.data.endPoint && y.push(`endPt="${d.data.endPoint[0].toFixed(1)},${d.data.endPoint[1].toFixed(1)}"`), d.data.label && y.push(`label="${d.data.label.replace(/"/g, "&quot;")}"`), d.data.labelFontSize && y.push(`labelFontSize="${d.data.labelFontSize}"`), d.data.labelFontFamily && d.data.labelFontFamily !== "Excalifont" && y.push(`labelFontFamily="${d.data.labelFontFamily}"`), d.data.labelAlign && d.data.labelAlign !== "center" && y.push(`labelAlign="${d.data.labelAlign}"`), d.rotation && y.push(`rotation="${d.rotation}"`), d.locked && y.push('locked="true"'), d.groupId && y.push(`group="${d.groupId}"`), o.push(`<!--@draw ${y.join(" ")} -->`), o.push("");
  }
  const a = t.filter((d) => d.type === "text");
  for (const d of a) {
    const p = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `z="${d.z}"`,
      `fontSize="${d.data.fontSize}"`,
      `fontFamily="${d.data.fontFamily}"`,
      `color="${d.data.color}"`,
      `align="${d.data.align}"`
    ];
    d.data.opacity !== void 0 && d.data.opacity !== 1 && p.push(`opacity="${d.data.opacity}"`), d.rotation && p.push(`rotation="${d.rotation}"`), d.locked && p.push('locked="true"'), d.groupId && p.push(`group="${d.groupId}"`), o.push(`<!--@text ${p.join(" ")} -->`), o.push(d.data.text), o.push("");
  }
  const c = t.filter((d) => d.type === "image");
  for (const d of c) {
    const p = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${Math.round(d.h)}"`,
      `z="${d.z}"`,
      `src="${d.data.src.replace(/"/g, "&quot;")}"`
    ];
    d.rotation && p.push(`rotation="${d.rotation}"`), d.locked && p.push('locked="true"'), d.groupId && p.push(`group="${d.groupId}"`), d.data.alt && p.push(`alt="${d.data.alt.replace(/"/g, "&quot;")}"`), d.data.opacity != null && d.data.opacity !== 1 && p.push(`opacity="${d.data.opacity}"`), d.data.borderColor && p.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && p.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && p.push(`borderStyle="${d.data.borderStyle}"`), o.push(`<!--@image ${p.join(" ")} -->`), o.push("");
  }
  const h = t.filter((d) => d.type === "edge");
  for (const d of h) {
    const p = [
      `id="${d.id}"`,
      `from="${d.data.fromId}"`,
      `to="${d.data.toId}"`,
      `style="${d.data.style}"`,
      `color="${d.data.color}"`
    ];
    d.data.label && p.push(`label="${d.data.label}"`), d.data.strokeWidth && d.data.strokeWidth !== 1 && p.push(`strokeWidth="${d.data.strokeWidth}"`), d.data.arrowHead && d.data.arrowHead !== "none" && p.push(`arrowHead="${d.data.arrowHead}"`), d.data.arrowTail && d.data.arrowTail !== "none" && p.push(`arrowTail="${d.data.arrowTail}"`), d.data.arrowHeadSize && p.push(`arrowHeadSize="${d.data.arrowHeadSize}"`), d.data.arrowTailSize && p.push(`arrowTailSize="${d.data.arrowTailSize}"`), d.data.edgeType && d.data.edgeType !== "bezier" && p.push(`edgeType="${d.data.edgeType}"`), d.data.animated && p.push('animated="true"'), d.data.animatedDirection && d.data.animatedDirection !== "forward" && p.push(`animatedDirection="${d.data.animatedDirection}"`), d.data.sourceHandle && p.push(`sourceHandle="${d.data.sourceHandle}"`), d.data.targetHandle && p.push(`targetHandle="${d.data.targetHandle}"`), d.data.midpointOffset != null && d.data.midpointOffset !== 0.5 && p.push(`midpointOffset="${d.data.midpointOffset}"`), d.data.curveOffset && (d.data.curveOffset[0] !== 0 || d.data.curveOffset[1] !== 0) && p.push(`curveOffset="${d.data.curveOffset[0]},${d.data.curveOffset[1]}"`), d.locked && p.push('locked="true"'), d.groupId && p.push(`group="${d.groupId}"`), o.push(`<!--@edge ${p.join(" ")} -->`), o.push("");
  }
  const f = t.filter((d) => d.type === "sticky");
  for (const d of f) {
    const p = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${d.h}"`,
      `z="${d.z}"`,
      `color="${d.data.color}"`
    ];
    d.data.fontSize && d.data.fontSize !== 16 && p.push(`fontSize="${d.data.fontSize}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && p.push(`opacity="${d.data.opacity}"`), d.rotation && p.push(`rotation="${d.rotation}"`), d.locked && p.push('locked="true"'), d.groupId && p.push(`group="${d.groupId}"`), o.push(`<!--@sticky ${p.join(" ")} -->`), o.push(d.data.text), o.push("");
  }
  return o.join(`
`);
}
const zi = "data:font/woff2;base64,", Ke = "Excalifont", vr = [
  // Bundled — hand-drawn default
  { key: "Excalifont", label: "Excalifont", category: "hand" },
  // System
  { key: "sans-serif", label: "Sans (system)", category: "system" },
  { key: "serif", label: "Serif (system)", category: "system" },
  { key: "monospace", label: "Mono (system)", category: "system" },
  // Google — Professional
  { key: "Inter", label: "Inter", category: "sans" },
  { key: "Roboto", label: "Roboto", category: "sans" },
  { key: "Open Sans", label: "Open Sans", category: "sans" },
  { key: "Lora", label: "Lora", category: "serif" },
  { key: "Playfair Display", label: "Playfair Display", category: "serif" },
  { key: "Merriweather", label: "Merriweather", category: "serif" },
  // Google — Mono
  { key: "JetBrains Mono", label: "JetBrains Mono", category: "mono" },
  { key: "Fira Code", label: "Fira Code", category: "mono" },
  { key: "Source Code Pro", label: "Source Code Pro", category: "mono" },
  // Google — Handwritten / Casual
  { key: "Caveat", label: "Caveat", category: "hand" },
  { key: "Shadows Into Light", label: "Shadows Into Light", category: "hand" },
  { key: "Dancing Script", label: "Dancing Script", category: "hand" },
  { key: "Amatic SC", label: "Amatic SC", category: "hand" },
  // Google — Display / Funky
  { key: "Pacifico", label: "Pacifico", category: "display" },
  { key: "Lobster", label: "Lobster", category: "display" },
  { key: "Permanent Marker", label: "Permanent Marker", category: "display" },
  { key: "Bangers", label: "Bangers", category: "display" },
  { key: "Righteous", label: "Righteous", category: "display" },
  { key: "Satisfy", label: "Satisfy", category: "display" },
  { key: "Kaushan Script", label: "Kaushan Script", category: "display" },
  { key: "Fredericka the Great", label: "Fredericka the Great", category: "display" },
  { key: "Comfortaa", label: "Comfortaa", category: "display" }
], Ii = /* @__PURE__ */ new Set(["sans-serif", "serif", "monospace"]), Cl = /* @__PURE__ */ new Set(["Excalifont"]), zl = /* @__PURE__ */ new Set([...Ii, ...Cl]);
function Il(t) {
  switch (t) {
    case "serif":
    case "display":
      return "H";
    case "mono":
      return "</>";
    case "hand":
      return "✏";
    default:
      return "A";
  }
}
function qe(t) {
  return Ii.has(t) ? t : `'${t}', sans-serif`;
}
let xs = !1;
function Tl(t = document) {
  if (xs) return;
  xs = !0;
  const e = t.createElement("style");
  e.textContent = `
@font-face {
  font-family: 'Excalifont';
  src: url('${zi}') format('woff2');
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}`, t.head.appendChild(e);
  const o = vr.filter((n) => !zl.has(n.key)).map((n) => "family=" + n.key.replace(/ /g, "+")).join("&"), r = t.createElement("link");
  r.rel = "stylesheet", r.href = `https://fonts.googleapis.com/css2?${o}&display=swap`, t.head.appendChild(r);
}
function He(t) {
  const e = {}, o = /(\w+)="([^"]*)"/g;
  let r;
  for (; (r = o.exec(t)) !== null; )
    e[r[1]] = r[2];
  return e;
}
const Pl = {
  default: "dot-grid",
  "cutting-board": "blueprint",
  // Removed grid-as-paper types → nearest color equivalent
  "graph-paper": "plain-white",
  "college-ruled": "plain-white",
  isometric: "plain-white"
};
async function Rl(t) {
  var s, i;
  const e = [], o = {}, r = t.split(`
`);
  let n = 0;
  for (; n < r.length; ) {
    const l = r[n].trim();
    if (l.startsWith("<!--@meta")) {
      const a = He(l);
      if (a.background) {
        const c = Pl[a.background] ?? a.background;
        o.background = c;
      }
      if (a.originView) {
        const c = a.originView.split(",").map(Number);
        c.length === 3 && c.every((h) => !isNaN(h)) && (o.originView = { x: c[0], y: c[1], zoom: c[2] });
      }
      n++;
      continue;
    }
    if (l.startsWith("<!--@frame")) {
      const a = He(l);
      for (n++; n < r.length && r[n].trim() === ""; ) n++;
      e.push({
        id: a.id || wt(10),
        type: "frame",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "400"),
        h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          label: ((s = a.label) == null ? void 0 : s.replace(/&quot;/g, '"')) || void 0,
          backgroundColor: a.backgroundColor || void 0,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0,
          slideOrder: a.slideOrder ? parseInt(a.slideOrder, 10) : void 0,
          transition: a.transition || void 0,
          transitionDuration: a.transitionDuration ? parseInt(a.transitionDuration, 10) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@block")) {
      const a = He(l);
      n++;
      const c = [];
      for (; n < r.length && !r[n].trim().startsWith("<!--@"); )
        c.push(r[n]), n++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      const h = c.join(`
`), f = h.trim().length > 0 ? await Nn(h) : [];
      e.push({
        id: a.id || wt(10),
        type: "content",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "300"),
        h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
        z: parseInt(a.z || "1"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          blocks: f,
          markdown: h,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@draw")) {
      const a = He(l);
      if (n++, a.tool === "shape")
        for (e.push({
          id: a.id || wt(10),
          type: "shape",
          x: parseFloat(a.x || "0"),
          y: parseFloat(a.y || "0"),
          w: parseFloat(a.w || "100"),
          h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
          z: parseInt(a.z || "0"),
          rotation: a.rotation ? parseFloat(a.rotation) : void 0,
          locked: a.locked === "true" || void 0,
          groupId: a.group || void 0,
          data: {
            shape: a.shape || "rect",
            stroke: a.color || "#1e1e2e",
            fill: a.fill || void 0,
            fillStyle: a.fillStyle || void 0,
            strokeWidth: parseFloat(a.stroke || "2"),
            strokeStyle: a.strokeStyle || void 0,
            edgeStyle: a.edgeStyle || void 0,
            roughness: parseFloat(a.roughness || "1"),
            opacity: a.opacity ? parseFloat(a.opacity) : void 0,
            startPoint: a.startPt ? a.startPt.split(",").map(Number) : void 0,
            endPoint: a.endPt ? a.endPt.split(",").map(Number) : void 0,
            label: ((i = a.label) == null ? void 0 : i.replace(/&quot;/g, '"')) || void 0,
            labelFontSize: a.labelFontSize ? parseFloat(a.labelFontSize) : void 0,
            labelFontFamily: a.labelFontFamily || void 0,
            labelAlign: a.labelAlign || void 0
          }
        }); n < r.length && r[n].trim() === ""; ) n++;
      else {
        let c = "";
        n < r.length && !r[n].trim().startsWith("<!--@") && (c = r[n].trim(), n++);
        const h = c ? c.split(" ").filter(Boolean).map((g) => {
          const b = g.split(",").map(Number);
          return [
            b[0] || 0,
            b[1] || 0,
            b[2] || 0.5
          ];
        }) : [];
        let f = 1 / 0, d = 1 / 0, p = -1 / 0, y = -1 / 0;
        for (const [g, b] of h)
          g < f && (f = g), b < d && (d = b), g > p && (p = g), b > y && (y = b);
        isFinite(f) || (f = parseFloat(a.x || "0"), d = parseFloat(a.y || "0"), p = f, y = d);
        const m = h.map(
          ([g, b, x]) => [g - f, b - d, x]
        );
        for (e.push({
          id: a.id || wt(10),
          type: "draw",
          x: f,
          y: d,
          w: p - f,
          h: y - d,
          z: parseInt(a.z || "0"),
          rotation: a.rotation ? parseFloat(a.rotation) : void 0,
          locked: a.locked === "true" || void 0,
          groupId: a.group || void 0,
          data: {
            tool: a.tool || "pen",
            points: m,
            color: a.color || "#1e1e2e",
            strokeWidth: parseFloat(a.width || "2"),
            opacity: a.opacity ? parseFloat(a.opacity) : void 0,
            fill: a.fill || void 0,
            fillStyle: a.fillStyle || void 0
          }
        }); n < r.length && r[n].trim() === ""; ) n++;
      }
      continue;
    }
    if (l.startsWith("<!--@image")) {
      const a = He(l);
      n++, e.push({
        id: a.id || wt(10),
        type: "image",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: parseFloat(a.h || "150"),
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          src: a.src || "",
          alt: a.alt,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@edge")) {
      const a = He(l);
      for (n++, e.push({
        id: a.id || wt(10),
        type: "edge",
        x: 0,
        y: 0,
        w: 0,
        h: 0,
        z: 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          fromId: a.from || "",
          toId: a.to || "",
          label: a.label,
          style: a.style || "solid",
          color: a.color || "#666",
          strokeWidth: a.strokeWidth ? parseFloat(a.strokeWidth) : 1,
          arrowHead: a.arrowHead || void 0,
          arrowTail: a.arrowTail || void 0,
          arrowHeadSize: a.arrowHeadSize ? parseFloat(a.arrowHeadSize) : void 0,
          arrowTailSize: a.arrowTailSize ? parseFloat(a.arrowTailSize) : void 0,
          edgeType: a.edgeType || void 0,
          animated: a.animated === "true" || void 0,
          animatedDirection: a.animatedDirection || void 0,
          sourceHandle: a.sourceHandle || void 0,
          targetHandle: a.targetHandle || void 0,
          midpointOffset: a.midpointOffset ? parseFloat(a.midpointOffset) : void 0,
          curveOffset: a.curveOffset ? a.curveOffset.split(",").map(Number) : void 0
        }
      }); n < r.length && r[n].trim() === ""; ) n++;
      continue;
    }
    if (l.startsWith("<!--@text")) {
      const a = He(l);
      n++;
      const c = [];
      for (; n < r.length && !r[n].trim().startsWith("<!--@"); )
        c.push(r[n]), n++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      e.push({
        id: a.id || wt(10),
        type: "text",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: "auto",
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          text: c.join(`
`),
          fontSize: parseFloat(a.fontSize || "20"),
          fontFamily: a.fontFamily || Ke,
          color: a.color || "#1e1e2e",
          align: a.align || "left",
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@sticky")) {
      const a = He(l);
      n++;
      const c = [];
      for (; n < r.length && !r[n].trim().startsWith("<!--@"); )
        c.push(r[n]), n++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      e.push({
        id: a.id || wt(10),
        type: "sticky",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: parseFloat(a.h || "150"),
        z: parseInt(a.z || "1"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          text: c.join(`
`),
          color: a.color || "#FEF3C7",
          fontSize: a.fontSize ? parseFloat(a.fontSize) : void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    n++;
  }
  return { nodes: e, meta: o };
}
function fo(t, e) {
  return t.h !== "auto" ? t.h : (e == null ? void 0 : e[t.id]) ?? 100;
}
const Al = 14;
function er(t, e, o, r, n) {
  const s = e.find((d) => d.id === o);
  if (!s) return null;
  const i = fo(t, n), l = Al / r, a = e.filter((d) => d.direction === s.direction), c = a.indexOf(s);
  if (c < 0) return null;
  const h = t.y + i / (a.length + 1) * (c + 1), f = s.direction === "input" ? t.x - l : t.x + t.w + l;
  if (t.rotation) {
    const d = t.x + t.w / 2, p = t.y + i / 2, y = t.rotation * Math.PI / 180, m = Math.cos(y), g = Math.sin(y), b = f - d, x = h - p;
    return { x: d + b * m - x * g, y: p + b * g + x * m };
  }
  return { x: f, y: h };
}
function ws(t, e, o, r, n, s, i, l) {
  const a = i - n, c = l - s;
  if (a === 0 && c === 0) return { x: n, y: s, side: "right" };
  let h = 1 / 0, f = n, d = s, p = "right";
  if (a !== 0) {
    const y = (t + o - n) / a;
    if (y > 0 && y < h) {
      const m = s + y * c;
      m >= e && m <= e + r && (h = y, f = t + o, d = m, p = "right");
    }
  }
  if (a !== 0) {
    const y = (t - n) / a;
    if (y > 0 && y < h) {
      const m = s + y * c;
      m >= e && m <= e + r && (h = y, f = t, d = m, p = "left");
    }
  }
  if (c !== 0) {
    const y = (e + r - s) / c;
    if (y > 0 && y < h) {
      const m = n + y * a;
      m >= t && m <= t + o && (h = y, f = m, d = e + r, p = "bottom");
    }
  }
  if (c !== 0) {
    const y = (e - s) / c;
    if (y > 0 && y < h) {
      const m = n + y * a;
      m >= t && m <= t + o && (h = y, f = m, d = e, p = "top");
    }
  }
  return { x: f, y: d, side: p };
}
function po(t, e, o, r, n) {
  const s = Math.cos(n), i = Math.sin(n), l = t - o, a = e - r;
  return [o + l * s - a * i, r + l * i + a * s];
}
function Ll(t, e, o, r) {
  const n = t.x + t.w / 2, s = t.y + e / 2;
  if (!t.rotation)
    return ws(t.x, t.y, t.w, e, n, s, o, r);
  const i = -t.rotation * Math.PI / 180, [l, a] = po(o, r, n, s, i), c = ws(t.x, t.y, t.w, e, n, s, l, a), [h, f] = po(c.x, c.y, n, s, -i);
  return { x: h, y: f, side: c.side };
}
function Ti(t, e, o, r) {
  return Math.abs(t) / o >= Math.abs(e) / r ? t >= 0 ? "right" : "left" : e >= 0 ? "bottom" : "top";
}
function Dl(t, e, o, r) {
  const n = t.x + t.w / 2, s = t.y + e / 2, i = t.w / 2, l = e / 2, a = t.rotation ? -t.rotation * Math.PI / 180 : 0, [c, h] = t.rotation ? po(o, r, n, s, a) : [o, r], f = c - n, d = h - s;
  if (f === 0 && d === 0)
    return { x: n + i, y: s, side: "right" };
  const p = 1 / Math.sqrt((f / i) ** 2 + (d / l) ** 2);
  let y = n + f * p, m = s + d * p;
  const g = Ti(f, d, i, l);
  return t.rotation && ([y, m] = po(y, m, n, s, -a)), { x: y, y: m, side: g };
}
function El(t, e, o, r) {
  const n = t.x + t.w / 2, s = t.y + e / 2, i = t.w / 2, l = e / 2, a = t.rotation ? -t.rotation * Math.PI / 180 : 0, [c, h] = t.rotation ? po(o, r, n, s, a) : [o, r], f = c - n, d = h - s;
  if (f === 0 && d === 0)
    return { x: n + i, y: s, side: "right" };
  const p = 1 / (Math.abs(f) / i + Math.abs(d) / l);
  let y = n + f * p, m = s + d * p;
  const g = Ti(f, d, i, l);
  return t.rotation && ([y, m] = po(y, m, n, s, -a)), { x: y, y: m, side: g };
}
function xn(t, e, o, r) {
  var n;
  if (t.type === "shape") {
    const s = (n = t.data) == null ? void 0 : n.shape;
    if (s === "ellipse") return Dl(t, e, o, r);
    if (s === "diamond") return El(t, e, o, r);
  }
  return Ll(t, e, o, r);
}
function wn(t, e, o, r) {
  const n = xn(t, e, o, r);
  return { x: n.x, y: n.y };
}
function kn(t, e, o) {
  const r = t.x + t.w / 2, n = t.y + e / 2;
  let s, i;
  switch (o) {
    case "top":
      s = r, i = t.y;
      break;
    case "bottom":
      s = r, i = t.y + e;
      break;
    case "left":
      s = t.x, i = n;
      break;
    case "right":
      s = t.x + t.w, i = n;
      break;
  }
  if (!t.rotation) return { x: s, y: i };
  const l = t.rotation * Math.PI / 180, [a, c] = po(s, i, r, n, l);
  return { x: a, y: c };
}
function Bo(t) {
  switch (t) {
    case "top":
      return { dx: 0, dy: -1 };
    case "bottom":
      return { dx: 0, dy: 1 };
    case "left":
      return { dx: -1, dy: 0 };
    case "right":
      return { dx: 1, dy: 0 };
  }
}
function ks(t) {
  var o;
  if (t.type !== "shape") return !1;
  const e = (o = t.data) == null ? void 0 : o.shape;
  return e === "ellipse" || e === "diamond";
}
function je(t, e, o = "bezier", r, n, s, i, l, a, c) {
  const h = fo(t, r), f = fo(e, r), d = t.x + t.w / 2, p = t.y + h / 2, y = e.x + e.w / 2, m = e.y + f / 2;
  let g, b, x, C;
  if (a)
    g = a.x, b = a.y, x = n ?? "right";
  else if (n) {
    const W = kn(t, h, n);
    g = W.x, b = W.y, x = n;
  } else {
    const W = xn(t, h, y, m);
    if (g = W.x, b = W.y, x = W.side, ks(t)) {
      const K = Math.hypot(y - d, m - p);
      K > 0 && (C = { dx: (y - d) / K, dy: (m - p) / K });
    }
  }
  let z, S, F, D;
  if (c)
    z = c.x, S = c.y, F = s ?? "left";
  else if (s) {
    const W = kn(e, f, s);
    z = W.x, S = W.y, F = s;
  } else {
    const W = xn(e, f, d, p);
    if (z = W.x, S = W.y, F = W.side, ks(e)) {
      const K = Math.hypot(d - y, p - m);
      K > 0 && (D = { dx: (d - y) / K, dy: (p - m) / K });
    }
  }
  switch (o) {
    case "straight":
      return Wl(g, b, z, S, x, F);
    case "bezier":
      return Fl(g, b, z, S, x, F, l, C, D);
    case "smoothstep":
      return Bl(g, b, z, S, x, F, i);
    case "step":
      return Nl(g, b, z, S, x, F, i);
  }
}
function Wl(t, e, o, r, n, s) {
  const i = Math.min(t, o), l = Math.min(e, r), a = Math.abs(o - t), c = Math.abs(r - e);
  return {
    path: `M${t},${e} L${o},${r}`,
    labelX: (t + o) / 2,
    labelY: (e + r) / 2,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: Math.atan2(r - e, o - t),
    tailAngle: Math.atan2(e - r, t - o),
    sourceSide: n,
    targetSide: s,
    bounds: { x: i, y: l, w: a, h: c }
  };
}
function Fl(t, e, o, r, n, s, i, l, a) {
  const c = Math.hypot(o - t, r - e), h = Math.min(c * 0.5, Math.max(50, c * 0.25)), f = l ?? Bo(n), d = a ?? Bo(s), p = i ? i[0] * (4 / 3) : 0, y = i ? i[1] * (4 / 3) : 0, m = t + f.dx * h + p, g = e + f.dy * h + y, b = o + d.dx * h + p, x = r + d.dy * h + y, C = 0.125 * t + 0.375 * m + 0.375 * b + 0.125 * o, z = 0.125 * e + 0.375 * g + 0.375 * x + 0.125 * r, S = Math.atan2(r - x, o - b), F = Math.atan2(e - g, t - m), D = {
    x: C,
    y: z,
    axis: "xy",
    min: 0,
    max: 0
  }, W = Math.min(t, o, m, b), K = Math.min(e, r, g, x), Z = Math.max(t, o, m, b), dt = Math.max(e, r, g, x);
  return {
    path: `M${t},${e} C${m},${g} ${b},${x} ${o},${r}`,
    labelX: C,
    labelY: z,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: S,
    tailAngle: F,
    sourceSide: n,
    targetSide: s,
    kinkHandle: D,
    bounds: { x: W, y: K, w: Z - W, h: dt - K }
  };
}
function Bl(t, e, o, r, n, s, i) {
  const { points: c, kinkHandle: h } = Hn(t, e, o, r, n, s, 20, i), f = Hl(c, 8), d = Math.floor(c.length / 2), p = (c[d - 1][0] + c[d][0]) / 2, y = (c[d - 1][1] + c[d][1]) / 2, m = c[c.length - 1], g = c[c.length - 2], b = Math.atan2(m[1] - g[1], m[0] - g[0]), x = c[0], C = c[1], z = Math.atan2(x[1] - C[1], x[0] - C[0]);
  let S = 1 / 0, F = 1 / 0, D = -1 / 0, W = -1 / 0;
  for (const [K, Z] of c)
    K < S && (S = K), Z < F && (F = Z), K > D && (D = K), Z > W && (W = Z);
  return {
    path: f,
    labelX: p,
    labelY: y,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: b,
    tailAngle: z,
    sourceSide: n,
    targetSide: s,
    kinkHandle: h,
    bounds: { x: S, y: F, w: D - S, h: W - F }
  };
}
function Nl(t, e, o, r, n, s, i) {
  const { points: a, kinkHandle: c } = Hn(t, e, o, r, n, s, 20, i), h = [`M${a[0][0]},${a[0][1]}`];
  for (let W = 1; W < a.length; W++)
    h.push(`L${a[W][0]},${a[W][1]}`);
  const f = Math.floor(a.length / 2), d = (a[f - 1][0] + a[f][0]) / 2, p = (a[f - 1][1] + a[f][1]) / 2, y = a[a.length - 1], m = a[a.length - 2], g = Math.atan2(y[1] - m[1], y[0] - m[0]), b = a[0], x = a[1], C = Math.atan2(b[1] - x[1], b[0] - x[0]);
  let z = 1 / 0, S = 1 / 0, F = -1 / 0, D = -1 / 0;
  for (const [W, K] of a)
    W < z && (z = W), K < S && (S = K), W > F && (F = W), K > D && (D = K);
  return {
    path: h.join(" "),
    labelX: d,
    labelY: p,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: g,
    tailAngle: C,
    sourceSide: n,
    targetSide: s,
    kinkHandle: c,
    bounds: { x: z, y: S, w: F - z, h: D - S }
  };
}
function Hn(t, e, o, r, n, s, i, l) {
  const a = Bo(n), c = Bo(s), h = t + a.dx * i, f = e + a.dy * i, d = o + c.dx * i, p = r + c.dy * i, y = n === "left" || n === "right", m = s === "left" || s === "right", g = [[t, e], [h, f]], b = l ?? 0.5;
  let x;
  if (y && m) {
    const C = h + (d - h) * b;
    g.push([C, f], [C, p]);
    const z = Math.min(h, d), S = Math.max(h, d);
    x = { x: C, y: (f + p) / 2, axis: "x", min: z, max: S };
  } else if (!y && !m) {
    const C = f + (p - f) * b;
    g.push([h, C], [d, C]);
    const z = Math.min(f, p), S = Math.max(f, p);
    x = { x: (h + d) / 2, y: C, axis: "y", min: z, max: S };
  } else y && !m ? g.push([d, f]) : g.push([h, p]);
  return g.push([d, p], [o, r]), { points: g, kinkHandle: x };
}
function Hl(t, e) {
  if (t.length < 2) return "";
  if (t.length === 2) return `M${t[0][0]},${t[0][1]} L${t[1][0]},${t[1][1]}`;
  const o = [`M${t[0][0]},${t[0][1]}`];
  for (let n = 1; n < t.length - 1; n++) {
    const s = t[n - 1], i = t[n], l = t[n + 1], a = i[0] - s[0], c = i[1] - s[1], h = l[0] - i[0], f = l[1] - i[1], d = Math.hypot(a, c), p = Math.hypot(h, f);
    if (d === 0 || p === 0) {
      o.push(`L${i[0]},${i[1]}`);
      continue;
    }
    const y = Math.min(e, d / 2, p / 2), m = i[0] - a / d * y, g = i[1] - c / d * y, b = i[0] + h / p * y, x = i[1] + f / p * y;
    o.push(`L${m},${g}`), o.push(`Q${i[0]},${i[1]} ${b},${x}`);
  }
  const r = t[t.length - 1];
  return o.push(`L${r[0]},${r[1]}`), o.join(" ");
}
function Ol(t, e, o, r, n, s, i, l, a) {
  const c = 1 - a, h = c * c, f = h * c, d = a * a, p = d * a;
  return [
    f * t + 3 * h * a * o + 3 * c * d * n + p * i,
    f * e + 3 * h * a * r + 3 * c * d * s + p * l
  ];
}
function Xl(t, e, o, r, n, s, i, l, a, c, h = 24) {
  let f = 1 / 0, d = o, p = r;
  for (let y = 1; y <= h; y++) {
    const m = y / h, [g, b] = Ol(o, r, n, s, i, l, a, c, m), x = On(t, e, d, p, g, b);
    x < f && (f = x), d = g, p = b;
  }
  return f;
}
function Yl(t, e, o) {
  let r = 1 / 0;
  for (let n = 1; n < o.length; n++) {
    const s = On(t, e, o[n - 1][0], o[n - 1][1], o[n][0], o[n][1]);
    s < r && (r = s);
  }
  return r;
}
function Pi(t, e, o, r, n, s, i, l) {
  const a = n.data.edgeType || "bezier", c = je(
    o,
    r,
    a,
    s,
    n.data.sourceHandle,
    n.data.targetHandle,
    n.data.midpointOffset,
    n.data.curveOffset,
    i,
    l
  ), { x1: h, y1: f, x2: d, y2: p } = c;
  if (a === "straight")
    return On(t, e, h, f, d, p);
  if (a === "bezier") {
    const g = Math.hypot(d - h, p - f), b = Math.min(g * 0.5, Math.max(50, g * 0.25)), x = Bo(c.sourceSide), C = Bo(c.targetSide), z = n.data.curveOffset ? n.data.curveOffset[0] * (4 / 3) : 0, S = n.data.curveOffset ? n.data.curveOffset[1] * (4 / 3) : 0, F = h + x.dx * b + z, D = f + x.dy * b + S, W = d + C.dx * b + z, K = p + C.dy * b + S;
    return Xl(t, e, h, f, F, D, W, K, d, p);
  }
  const y = 20, { points: m } = Hn(h, f, d, p, c.sourceSide, c.targetSide, y, n.data.midpointOffset);
  return Yl(t, e, m);
}
function Jr(t, e, o) {
  const r = fo(t, o), n = fo(e, o), s = t.x + t.w / 2, i = t.y + r / 2, l = e.x + e.w / 2, a = e.y + n / 2, c = wn(t, r, l, a), h = wn(e, n, s, i);
  return { x1: c.x, y1: c.y, x2: h.x, y2: h.y };
}
function $r(t, e, o, r) {
  const n = fo(t, r);
  return wn(t, n, e, o);
}
function On(t, e, o, r, n, s) {
  const i = n - o, l = s - r, a = i * i + l * l;
  if (a === 0) return Math.hypot(t - o, e - r);
  const c = Math.max(0, Math.min(1, ((t - o) * i + (e - r) * l) / a)), h = o + c * i, f = r + c * l;
  return Math.hypot(t - h, e - f);
}
function Gl(t, e, o, r, n) {
  const s = (l, a) => l >= n.x && l <= n.x + n.w && a >= n.y && a <= n.y + n.h;
  if (s(t, e) || s(o, r)) return !0;
  const i = [
    [n.x, n.y, n.x + n.w, n.y],
    // top
    [n.x, n.y + n.h, n.x + n.w, n.y + n.h],
    // bottom
    [n.x, n.y, n.x, n.y + n.h],
    // left
    [n.x + n.w, n.y, n.x + n.w, n.y + n.h]
    // right
  ];
  for (const [l, a, c, h] of i)
    if (jl(t, e, o, r, l, a, c, h)) return !0;
  return !1;
}
function jl(t, e, o, r, n, s, i, l) {
  const a = o - t, c = r - e, h = i - n, f = l - s, d = a * f - c * h;
  if (Math.abs(d) < 1e-10) return !1;
  const p = n - t, y = s - e, m = (p * f - y * h) / d, g = (p * c - y * a) / d;
  return m >= 0 && m <= 1 && g >= 0 && g <= 1;
}
function Do(t, e, o, r) {
  const n = Math.cos(o), s = Math.sin(o), i = -s, l = n, a = r / 2, c = t + n * a, h = e + s * a, f = t - n * a, d = e - s * a, p = r * 0.4;
  return `M${f + i * p},${d + l * p} L${c},${h} L${f - i * p},${d - l * p}`;
}
function zr(t, e, o, r) {
  const n = Math.cos(o), s = Math.sin(o), i = -s, l = n, a = r / 2, c = t + n * a, h = e + s * a, f = t - n * a, d = e - s * a, p = r * 0.4;
  return `M${c},${h} L${f + i * p},${d + l * p} L${f - i * p},${d - l * p} Z`;
}
function Wo(t, e) {
  const o = fo(t, e);
  return ["top", "right", "bottom", "left"].map((n) => {
    const s = kn(t, o, n);
    return { side: n, x: s.x, y: s.y };
  });
}
function pr(t, e, o, r) {
  const n = Wo(t, r);
  let s = n[0], i = 1 / 0;
  for (const l of n) {
    const a = Math.hypot(l.x - e, l.y - o);
    a < i && (i = a, s = l);
  }
  return s.side;
}
function Vl(t, e, o, r, n, s) {
  const i = 8 / r, l = [];
  for (const a of t.values()) {
    if (a.type !== "edge") continue;
    const c = a, h = t.get(c.data.fromId), f = t.get(c.data.toId);
    if (!h || !f) continue;
    const d = s == null ? void 0 : s(c, h, f);
    Pi(e, o, h, f, c, n, d == null ? void 0 : d.sourcePortPos, d == null ? void 0 : d.targetPortPos) < i && l.push(a);
  }
  return l;
}
function Kl(t, e, o, r, n, s) {
  const i = 8 / r;
  let l = null, a = i;
  for (const c of t.values()) {
    if (c.type !== "edge") continue;
    const h = c, f = t.get(h.data.fromId), d = t.get(h.data.toId);
    if (!f || !d) continue;
    const p = s == null ? void 0 : s(h, f, d), y = Pi(e, o, f, d, h, n, p == null ? void 0 : p.sourcePortPos, p == null ? void 0 : p.targetPortPos);
    y < a && (a = y, l = c);
  }
  return l;
}
function ql(t, e, o) {
  const r = t.x, n = t.x + t.w / 2, s = t.x + t.w, i = t.y, l = t.y + t.h / 2, a = t.y + t.h, c = [r, n, s], h = [i, l, a];
  let f = 1 / 0, d = 1 / 0;
  const p = [];
  for (const m of e) {
    const g = m.x, b = m.x + m.w / 2, x = m.x + m.w, C = m.y, z = m.y + m.h / 2, S = m.y + m.h, F = [g, b, x], D = [C, z, S];
    for (const W of c)
      for (const K of F) {
        const Z = K - W;
        Math.abs(Z) <= o && (Math.abs(Z) < Math.abs(f) && (f = Z), p.push({
          axis: "x",
          position: K,
          start: Math.min(t.y, t.y + t.h, m.y, m.y + m.h),
          end: Math.max(t.y, t.y + t.h, m.y, m.y + m.h)
        }));
      }
    for (const W of h)
      for (const K of D) {
        const Z = K - W;
        Math.abs(Z) <= o && (Math.abs(Z) < Math.abs(d) && (d = Z), p.push({
          axis: "y",
          position: K,
          start: Math.min(t.x, t.x + t.w, m.x, m.x + m.w),
          end: Math.max(t.x, t.x + t.w, m.x, m.x + m.w)
        }));
      }
  }
  const y = /* @__PURE__ */ new Map();
  for (const m of p) {
    const g = `${m.axis}:${m.position.toFixed(1)}`, b = y.get(g);
    b ? (b.start = Math.min(b.start, m.start), b.end = Math.max(b.end, m.end)) : y.set(g, { ...m });
  }
  return {
    guides: Array.from(y.values()),
    snapDx: Math.abs(f) <= o ? f : 0,
    snapDy: Math.abs(d) <= o ? d : 0
  };
}
class Zl {
  constructor() {
    St(this, "nodes", /* @__PURE__ */ new Map());
    St(this, "viewport", { x: 0, y: 0, zoom: 1 });
    St(this, "selection", /* @__PURE__ */ new Set());
    St(this, "activeGroupId", null);
    St(this, "groupRotations", /* @__PURE__ */ new Map());
    /** Maps child groupId → parent groupId for nested groups. */
    St(this, "groupParent", /* @__PURE__ */ new Map());
    /** Reverse index: parent groupId → set of child groupIds. Maintained alongside groupParent. */
    St(this, "groupChildren", /* @__PURE__ */ new Map());
    St(this, "mode", "select");
    St(this, "activeTool", {
      tool: "pen",
      color: "#1e1e2e",
      width: 2.5,
      shapeType: "rect",
      strokeStyle: "solid",
      roughness: 1,
      opacity: 1
    });
    St(this, "containerOffset", { x: 0, y: 0 });
    /** DOM element that hosts the canvas — used to derive the correct window in pop-out scenarios. */
    St(this, "_container", null);
    St(this, "snapToGrid", !1);
    St(this, "smartGuides", !0);
    St(this, "lassoSelect", !1);
    St(this, "presentationMode", !1);
    St(this, "presentationSlides", []);
    St(this, "presentationIndex", 0);
    St(this, "_presentationAnimId", null);
    /** Transition overlay state — consumed by PresentationOverlay for visual effects. */
    St(this, "_transitionOverlay", null);
    St(this, "gridSize", 20);
    St(this, "boardBackground", "dot-grid");
    /** Saved "origin" viewport position restored on next load. */
    St(this, "originView", null);
    /** Current alignment guides (set during drag). */
    St(this, "alignGuides", []);
    /** Container dimensions for viewport bounds computation. */
    St(this, "_containerWidth", 2e3);
    St(this, "_containerHeight", 1500);
    St(this, "history", new fl());
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    St(this, "listeners", {});
    St(this, "_suppressEvents", !1);
    St(this, "_collabMode", !1);
    St(this, "clipboard", []);
    St(this, "pasteCount", 0);
    St(this, "nextZValue", 1);
    St(this, "_minZ", 0);
    St(this, "quadTree", new gn({ x: -1e5, y: -1e5, w: 2e5, h: 2e5 }));
    St(this, "adjacency", /* @__PURE__ */ new Map());
    /** Explicit frame→children tracking. Only nodes intentionally placed inside a frame are tracked. */
    St(this, "frameChildren", /* @__PURE__ */ new Map());
    /** Node types that act as containers (frame-like behavior). "frame" is always included. */
    St(this, "_containerTypes", /* @__PURE__ */ new Set(["frame"]));
    St(this, "registry");
    /** Measured heights for auto-height nodes (canvas-coordinate units). */
    St(this, "_measuredHeights", {});
  }
  get transitionOverlay() {
    return this._transitionOverlay;
  }
  /** Set the node type registry for lifecycle hooks. */
  setRegistry(e) {
    this.registry = e;
  }
  /** Enable collaborative mode. Disables local snapshot history. */
  setCollabMode(e) {
    this._collabMode = e, e && this.history.clear();
  }
  /** Whether the engine is in collaborative mode. */
  get isCollabMode() {
    return this._collabMode;
  }
  /** Register a node type as a container (frame-like behavior). */
  registerContainerType(e) {
    this._containerTypes.add(e);
  }
  /** Check whether a node type behaves as a container. */
  isContainerType(e) {
    return this._containerTypes.has(e);
  }
  /** The set of container type strings (read-only). */
  get containerTypes() {
    return this._containerTypes;
  }
  /** Update the measured height for an auto-height node. */
  setMeasuredHeight(e, o) {
    this._measuredHeights[e] = o;
  }
  /** Get the resolved height for a node (measured or explicit). */
  resolveHeight(e) {
    return e.h !== "auto" ? e.h : this._measuredHeights[e.id] ?? 100;
  }
  /** Get all measured heights (for canvas rendering). */
  get measuredHeights() {
    return this._measuredHeights;
  }
  // --- Edge helpers (data-flow) ---
  /** Get all edge nodes connected to a given node. */
  getEdgesForNode(e) {
    const o = this.adjacency.get(e);
    if (!o) return [];
    const r = [];
    for (const n of o) {
      const s = this.nodes.get(n);
      s && s.type === "edge" && r.push(s);
    }
    return r;
  }
  /** Get all edge nodes in the board. */
  getAllEdges() {
    const e = [];
    for (const o of this.nodes.values())
      o.type === "edge" && e.push(o);
    return e;
  }
  /** Set the container element (used by SpatialCanvas on mount). */
  setContainer(e) {
    this._container = e;
  }
  /** Get the window object for the container (supports pop-out windows). */
  getWindow() {
    var e;
    return ((e = this._container) == null ? void 0 : e.ownerDocument.defaultView) ?? window;
  }
  // --- Event Emitter ---
  on(e, o) {
    this.listeners[e] || (this.listeners[e] = /* @__PURE__ */ new Set()), this.listeners[e].add(o);
  }
  off(e, o) {
    var r;
    (r = this.listeners[e]) == null || r.delete(o);
  }
  emit(e, ...o) {
    var r;
    this._suppressEvents || (r = this.listeners[e]) == null || r.forEach((n) => n(...o));
  }
  /** Request entering image crop mode (handled by the canvas component). */
  requestImageCrop(e) {
    this.emit("image:cropRequest", e);
  }
  // --- Grid Snapping ---
  toggleSnapToGrid() {
    this.snapToGrid = !this.snapToGrid;
  }
  toggleSmartGuides() {
    this.smartGuides = !this.smartGuides;
  }
  toggleLassoSelect() {
    this.lassoSelect = !this.lassoSelect, this.emit("lassoToggle");
  }
  // ── Presentation mode ─────────────────────────────────────────
  enterPresentation() {
    const e = [];
    for (const a of this.nodes.values())
      if (a.type === "frame") {
        const c = a.data;
        e.push({ id: a.id, x: a.x, y: a.y, order: c.slideOrder });
      }
    if (e.length === 0) return;
    const o = e.filter((a) => a.order != null).sort((a, c) => a.order - c.order), r = e.filter((a) => a.order == null), n = 100;
    r.sort((a, c) => a.y - c.y);
    const s = [];
    for (const a of r) {
      const c = s[s.length - 1];
      c && Math.abs(a.y - c[0].y) < n ? c.push(a) : s.push([a]);
    }
    const i = s.flatMap((a) => a.sort((c, h) => c.x - h.x)), l = [...o, ...i];
    this.presentationSlides = l.map((a) => a.id), this.presentationIndex = 0, this.presentationMode = !0, this.selection.size > 0 && (this.selection.clear(), this.emit("selection")), this.emit("presentation"), this.presentationGoTo(0);
  }
  exitPresentation() {
    this._presentationAnimId != null && (cancelAnimationFrame(this._presentationAnimId), this._presentationAnimId = null), this._transitionOverlay = null, this.presentationMode = !1, this.presentationSlides = [], this.presentationIndex = 0, this.emit("presentation");
  }
  presentationNext() {
    this.presentationIndex < this.presentationSlides.length - 1 && this.presentationGoTo(this.presentationIndex + 1);
  }
  presentationPrev() {
    this.presentationIndex > 0 && this.presentationGoTo(this.presentationIndex - 1);
  }
  presentationGoTo(e) {
    if (e < 0 || e >= this.presentationSlides.length) return;
    const o = this.presentationSlides[e], r = this.nodes.get(o);
    if (!r) {
      this.exitPresentation();
      return;
    }
    const n = this.presentationIndex;
    this.presentationIndex = e, this.emit("presentation"), this._presentationAnimId != null && (cancelAnimationFrame(this._presentationAnimId), this._presentationAnimId = null), this._transitionOverlay = null;
    const s = this._computeSlideViewport(r), i = r.data, l = i.transition ?? "pan", a = i.transitionDuration, c = e >= n ? 1 : -1;
    switch (l) {
      case "none":
        this._transitionNone(s);
        break;
      case "fade":
        this._transitionFade(s, a);
        break;
      case "dissolve":
        this._transitionDissolve(s, a);
        break;
      case "zoom":
        this._transitionZoom(s, a);
        break;
      case "fold":
        this._transitionFold(s, a);
        break;
      case "cube":
        this._transitionCube(s, a, c);
        break;
      case "pan":
      default:
        this._transitionPan(s, a);
        break;
    }
  }
  _computeSlideViewport(e) {
    const o = this.resolveHeight(e), r = 40, n = e.x - r, s = e.y - r, i = e.w + r * 2, l = o + r * 2, a = this._containerWidth, c = this._containerHeight, h = Lo(Math.min(a / i, c / l), 0.1, 5);
    return {
      x: (a - i * h) / 2 - n * h,
      y: (c - l * h) / 2 - s * h,
      zoom: h
    };
  }
  /** Pan transition: smooth viewport interpolation (default). */
  _transitionPan(e, o) {
    const r = o ?? 400, n = performance.now(), s = { ...this.viewport }, i = (l) => {
      const a = Math.min((l - n) / r, 1), c = 1 - Math.pow(1 - a, 3);
      this.viewport.x = s.x + (e.x - s.x) * c, this.viewport.y = s.y + (e.y - s.y) * c, this.viewport.zoom = s.zoom + (e.zoom - s.zoom) * c, this.emit("viewport"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : this._presentationAnimId = null;
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** None transition: instant viewport snap. */
  _transitionNone(e) {
    this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport");
  }
  /** Fade transition: fade to black, snap viewport, fade from black. */
  _transitionFade(e, o) {
    const r = (o ?? 500) / 2, n = performance.now(), s = (i) => {
      const l = Math.min((i - n) / r, 1);
      if (this._transitionOverlay = { type: "fade", phase: "out", progress: l }, this.emit("presentation"), l < 1)
        this._presentationAnimId = requestAnimationFrame(s);
      else {
        this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport");
        const a = performance.now(), c = (h) => {
          const f = Math.min((h - a) / r, 1);
          this._transitionOverlay = { type: "fade", phase: "in", progress: f }, this.emit("presentation"), f < 1 ? this._presentationAnimId = requestAnimationFrame(c) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
        };
        this._presentationAnimId = requestAnimationFrame(c);
      }
    };
    this._presentationAnimId = requestAnimationFrame(s);
  }
  /** Dissolve transition: quick overlay fade, snap viewport at midpoint. */
  _transitionDissolve(e, o) {
    const r = o ?? 400, n = performance.now();
    let s = !1;
    const i = (l) => {
      const a = Math.min((l - n) / r, 1);
      a < 0.5 ? this._transitionOverlay = { type: "dissolve", phase: "out", progress: a * 2 } : (s || (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), s = !0), this._transitionOverlay = { type: "dissolve", phase: "in", progress: (a - 0.5) * 2 }), this.emit("presentation"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** Zoom transition: zoom out from current, zoom into target. */
  _transitionZoom(e, o) {
    const r = o ?? 600, n = performance.now(), s = { ...this.viewport }, i = Math.max(0.1, Math.min(s.zoom, e.zoom) * 0.35), l = (s.x + e.x) / 2, a = (s.y + e.y) / 2, c = (h) => {
      const f = Math.min((h - n) / r, 1);
      if (f < 0.5) {
        const d = f * 2, p = 1 - Math.pow(1 - d, 3);
        this.viewport.x = s.x + (l - s.x) * p, this.viewport.y = s.y + (a - s.y) * p, this.viewport.zoom = s.zoom + (i - s.zoom) * p;
      } else {
        const d = (f - 0.5) * 2, p = 1 - Math.pow(1 - d, 3);
        this.viewport.x = l + (e.x - l) * p, this.viewport.y = a + (e.y - a) * p, this.viewport.zoom = i + (e.zoom - i) * p;
      }
      this.emit("viewport"), f < 1 ? this._presentationAnimId = requestAnimationFrame(c) : this._presentationAnimId = null;
    };
    this._presentationAnimId = requestAnimationFrame(c);
  }
  /** Fold transition: two halves fold shut like a book, snap viewport, unfold to reveal. */
  _transitionFold(e, o) {
    const r = o ?? 700, n = performance.now();
    let s = !1;
    const i = (l) => {
      const a = Math.min((l - n) / r, 1);
      a < 0.5 ? this._transitionOverlay = { type: "fold", phase: "out", progress: a * 2 } : (s || (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), s = !0), this._transitionOverlay = { type: "fold", phase: "in", progress: (a - 0.5) * 2 }), this.emit("presentation"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** Cube transition: zoom out → 3D rotate → zoom in, snap viewport at midpoint. */
  _transitionCube(e, o, r = 1) {
    const n = o ?? 1200, s = performance.now();
    let i = !1;
    const l = (a) => {
      const c = Math.min((a - s) / n, 1);
      c >= 0.5 && !i && (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), i = !0), this._transitionOverlay = {
        type: "cube",
        phase: c < 0.5 ? "out" : "in",
        progress: c < 0.5 ? c * 2 : (c - 0.5) * 2,
        direction: r,
        t: c
      }, this.emit("presentation"), c < 1 ? this._presentationAnimId = requestAnimationFrame(l) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(l);
  }
  snap(e, o) {
    return this.snapToGrid ? {
      x: Math.round(e / this.gridSize) * this.gridSize,
      y: Math.round(o / this.gridSize) * this.gridSize
    } : { x: e, y: o };
  }
  /** Update the container dimensions (called from canvas resize observer). */
  setContainerSize(e, o) {
    const r = this._containerWidth, n = this._containerHeight;
    this._containerWidth = e, this._containerHeight = o, this.presentationMode && this.presentationSlides.length > 0 ? this.presentationGoTo(this.presentationIndex) : r > 0 && n > 0 && (this.viewport.x += (e - r) / 2, this.viewport.y += (o - n) / 2, this.emit("viewport"));
  }
  /**
   * Compute smart guide alignment + grid snap for a drag operation.
   * Sets `this.alignGuides` and emits `guides` event.
   * Returns the adjusted delta to apply.
   */
  computeDragSnap(e, o, r, n, s) {
    const i = this.snapToGrid && !s, l = this.smartGuides && !s;
    let a = r, c = n, h = [];
    const f = o instanceof Set ? o : new Set(o);
    if (l) {
      let d = 1 / 0, p = 1 / 0, y = -1 / 0, m = -1 / 0;
      for (const W of e) {
        const K = this.getNode(W.id);
        if (!K) continue;
        const Z = W.x + r, dt = W.y + n, X = this.resolveHeight(K);
        d = Math.min(d, Z), p = Math.min(p, dt), y = Math.max(y, Z + K.w), m = Math.max(m, dt + X);
      }
      const g = { x: d, y: p, w: y - d, h: m - p }, b = -this.viewport.x / this.viewport.zoom, x = -this.viewport.y / this.viewport.zoom, C = this._containerWidth / this.viewport.zoom, z = this._containerHeight / this.viewport.zoom, S = [], F = this.quadTree.retrieve([], { x: b, y: x, w: C, h: z });
      for (const W of F) {
        if (W.type === "edge" || f.has(W.id)) continue;
        const K = this.resolveHeight(W);
        S.push({ x: W.x, y: W.y, w: W.w, h: K });
      }
      const D = ql(g, S, 5);
      if (h = D.guides, i) {
        const W = e[0].x + r, K = e[0].y + n, Z = this.snap(W, K), dt = Z.x - W, X = Z.y - K, rt = D.snapDx !== 0 && Math.abs(D.snapDx) <= Math.abs(dt), J = D.snapDy !== 0 && Math.abs(D.snapDy) <= Math.abs(X);
        a = r + (rt ? D.snapDx : dt), c = n + (J ? D.snapDy : X), rt || (h = h.filter((U) => U.axis !== "x")), J || (h = h.filter((U) => U.axis !== "y"));
      } else
        a = r + D.snapDx, c = n + D.snapDy;
    } else if (i) {
      const d = this.snap(e[0].x + r, e[0].y + n);
      a = d.x - e[0].x, c = d.y - e[0].y;
    }
    return this.alignGuides = h, this.emit("guides"), { finalDx: a, finalDy: c };
  }
  /** Clear alignment guides (call on drag end). */
  clearAlignGuides() {
    this.alignGuides.length !== 0 && (this.alignGuides = [], this.emit("guides"));
  }
  // --- Board Background ---
  setBoardBackground(e) {
    this.boardBackground !== e && (this.boardBackground = e, this.emit("background"));
  }
  // --- Viewport ---
  pan(e, o) {
    this.viewport.x += e, this.viewport.y += o, this.emit("viewport");
  }
  zoomByWheel(e, o, r) {
    this.viewport = wl(
      this.viewport,
      e,
      o - this.containerOffset.x,
      r - this.containerOffset.y
    ), this.emit("viewport");
  }
  zoomByFactor(e, o, r) {
    this.viewport = kl(
      this.viewport,
      e,
      o - this.containerOffset.x,
      r - this.containerOffset.y
    ), this.emit("viewport");
  }
  zoomTo(e, o) {
    const r = Lo(e, 0.1, 5);
    if (o) {
      const n = o.x - this.containerOffset.x, s = o.y - this.containerOffset.y, i = Eo(this.viewport, n, s);
      this.viewport = {
        x: n - i.x * r,
        y: s - i.y * r,
        zoom: r
      };
    } else
      this.viewport.zoom = r;
    this.emit("viewport");
  }
  zoomIn() {
    this.zoomTo(this.viewport.zoom * 1.2);
  }
  zoomOut() {
    this.zoomTo(this.viewport.zoom / 1.2);
  }
  /** Zoom and pan to center a node for editing (e.g. after double-click on placeholder) */
  zoomToNode(e, o = 1) {
    const r = this.nodes.get(e);
    if (!r) return;
    const n = r.h === "auto" ? 100 : r.h, s = r.x + r.w / 2, i = r.y + n / 2, l = this.getWindow(), a = l.innerWidth, c = l.innerHeight, h = Lo(o, 0.2, 5);
    this.viewport = {
      x: a / 2 - s * h,
      y: c / 2 - i * h,
      zoom: h
    }, this.emit("viewport");
  }
  fitToContent() {
    if (this.nodes.size === 0) return;
    let e = 1 / 0, o = 1 / 0, r = -1 / 0, n = -1 / 0;
    for (const f of this.nodes.values()) {
      const d = f.h === "auto" ? 100 : f.h;
      f.x < e && (e = f.x), f.y < o && (o = f.y), f.x + f.w > r && (r = f.x + f.w), f.y + d > n && (n = f.y + d);
    }
    const s = 50;
    e -= s, o -= s, r += s, n += s;
    const i = r - e, l = n - o, a = this._containerWidth, c = this._containerHeight, h = Lo(
      Math.min(a / i, c / l),
      0.1,
      5
    );
    this.viewport = {
      x: (a - i * h) / 2 - e * h,
      y: (c - l * h) / 2 - o * h,
      zoom: h
    }, this.emit("viewport");
  }
  /** Save the current viewport as the origin view (restored on next load). */
  setOriginView() {
    this.originView = { ...this.viewport }, this.emit("background");
  }
  /** Clear the saved origin view. */
  clearOriginView() {
    this.originView = null, this.emit("background");
  }
  /** Jump to the saved origin view, or fit-to-content if none is saved. */
  goToOriginView() {
    this.originView ? (this.viewport = { ...this.originView }, this.emit("viewport")) : this.fitToContent();
  }
  screenToCanvas(e, o) {
    return Eo(
      this.viewport,
      e - this.containerOffset.x,
      o - this.containerOffset.y
    );
  }
  canvasToScreen(e, o) {
    return xl(this.viewport, e, o);
  }
  // --- Node CRUD ---
  addNode(e) {
    var o, r, n;
    if (this.history.pushSnapshot(this.nodes, this.groupParent), this.nodes.set(e.id, e), this.quadTree.insert(e), e.z < this._minZ && (this._minZ = e.z), e.type === "edge") {
      const s = e, { fromId: i, toId: l } = s.data;
      this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.has(l) || this.adjacency.set(l, /* @__PURE__ */ new Set()), this.adjacency.get(i).add(e.id), this.adjacency.get(l).add(e.id);
    }
    e.type !== "edge" && this.updateFrameMembership([e.id]), (n = (r = (o = this.registry) == null ? void 0 : o.get(e.type)) == null ? void 0 : r.onCreate) == null || n.call(r, e, this), this.emit("node:create", e), this.emit("change"), this.emit("history");
  }
  addNodes(e) {
    if (e.length === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    for (const r of e)
      if (this.nodes.set(r.id, r), this.quadTree.insert(r), r.type === "edge") {
        const n = r, { fromId: s, toId: i } = n.data;
        this.adjacency.has(s) || this.adjacency.set(s, /* @__PURE__ */ new Set()), this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.get(s).add(r.id), this.adjacency.get(i).add(r.id);
      }
    const o = e.filter((r) => r.type !== "edge").map((r) => r.id);
    o.length > 0 && this.updateFrameMembership(o), this.emit("change"), this.emit("history");
  }
  updateNode(e, o) {
    var s, i, l, a, c, h, f, d, p;
    const r = this.nodes.get(e);
    if (!r) return;
    const n = { ...r, ...o };
    if (o.data && typeof o.data == "object" && r.data && typeof r.data == "object" && (n.data = {
      ...r.data,
      ...o.data
    }), this.nodes.set(e, n), (r.x !== n.x || r.y !== n.y || r.w !== n.w || r.h !== n.h || (r.rotation ?? 0) !== (n.rotation ?? 0)) && (this.quadTree.remove(r), this.quadTree.insert(n), this.updateConnectedEdges(e)), r.x !== n.x || r.y !== n.y) {
      const y = n.x - r.x, m = n.y - r.y;
      (l = (i = (s = this.registry) == null ? void 0 : s.get(n.type)) == null ? void 0 : i.onMove) == null || l.call(i, n, y, m, this), this.emit("node:move", n, y, m);
    }
    if (r.w !== n.w || r.h !== n.h) {
      const y = r.w !== 0 ? n.w / r.w : 1, m = r.h === "auto" ? 0 : r.h, g = n.h === "auto" ? 0 : n.h, b = m !== 0 ? g / m : 1;
      this.emit("node:resize", n, y, b);
    }
    (r.rotation ?? 0) !== (n.rotation ?? 0) && ((h = (c = (a = this.registry) == null ? void 0 : a.get(n.type)) == null ? void 0 : c.onRotate) == null || h.call(c, n, n.rotation ?? 0, this), this.emit("node:rotate", n, n.rotation ?? 0)), o.data && r.data !== n.data && ((p = (d = (f = this.registry) == null ? void 0 : f.get(n.type)) == null ? void 0 : d.onDataChange) == null || p.call(d, n, r.data, n.data, this), this.emit("node:data", n, r.data, n.data)), this.emit("change");
  }
  /**
   * Batch update multiple nodes with a single change emit.
   * Use during drag/resize to avoid N re-renders per frame.
   */
  updateMany(e) {
    let o = !1;
    for (const { id: r, patch: n } of e) {
      const s = this.nodes.get(r);
      if (!s) continue;
      const i = { ...s, ...n };
      n.data && typeof n.data == "object" && s.data && typeof s.data == "object" && (i.data = {
        ...s.data,
        ...n.data
      }), this.nodes.set(r, i), (s.x !== i.x || s.y !== i.y || s.w !== i.w || s.h !== i.h || (s.rotation ?? 0) !== (i.rotation ?? 0)) && (this.quadTree.remove(s), this.quadTree.insert(i), this.updateConnectedEdges(r)), o = !0;
    }
    o && this.emit("change");
  }
  updateConnectedEdges(e) {
    const o = this.adjacency.get(e);
    if (o)
      for (const r of o) {
        const n = this.nodes.get(r);
        if (!n || n.type !== "edge") continue;
        const s = n, i = this.nodes.get(s.data.fromId), l = this.nodes.get(s.data.toId);
        if (i && l) {
          const a = je(
            i,
            l,
            s.data.edgeType,
            void 0,
            s.data.sourceHandle,
            s.data.targetHandle,
            s.data.midpointOffset,
            s.data.curveOffset
          ), c = { ...s, ...a.bounds };
          this.nodes.set(r, c), this.quadTree.remove(s), this.quadTree.insert(c);
        }
      }
  }
  updateNodeWithHistory(e, o) {
    this.history.pushSnapshot(this.nodes, this.groupParent), this.updateNode(e, o), this.emit("history");
  }
  /** Update multiple nodes in a single undo step. */
  batchUpdateWithHistory(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const { id: o, patch: r } of e)
        this.updateNode(o, r);
      this.emit("history");
    }
  }
  deleteNode(e) {
    var r, n, s, i, l;
    if (!this.nodes.has(e) || (r = this.nodes.get(e)) != null && r.locked) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = this.nodes.get(e);
    o && ((i = (s = (n = this.registry) == null ? void 0 : n.get(o.type)) == null ? void 0 : s.onDelete) == null || i.call(s, o, this), this.emit("node:delete", o), this.quadTree.remove(o)), this.nodes.delete(e), this.selection.delete(e), this.adjacency.delete(e), this.frameChildren.delete(e);
    for (const a of this.frameChildren.values()) a.delete(e);
    for (const [a, c] of this.nodes)
      if (c.type === "edge") {
        const h = c.data;
        if (h.fromId === e || h.toId === e) {
          const f = this.nodes.get(a);
          f && this.quadTree.remove(f), this.nodes.delete(a), this.selection.delete(a);
          const d = h.fromId === e ? h.toId : h.fromId;
          (l = this.adjacency.get(d)) == null || l.delete(a);
        }
      }
    this.emit("change"), this.emit("selection"), this.emit("history");
  }
  getNode(e) {
    return this.nodes.get(e);
  }
  getAllNodes() {
    return Array.from(this.nodes.values());
  }
  /** Returns a read-only iterable of all nodes (no copy). */
  iterNodes() {
    return this.nodes.values();
  }
  getNodesByType(e) {
    const o = [];
    for (const r of this.nodes.values())
      r.type === e && o.push(r);
    return o;
  }
  /** Returns all non-edge nodes fully contained within a frame's bounds (including nested frames). */
  getNodesInsideFrame(e) {
    const o = this.nodes.get(e);
    if (!o || !this._containerTypes.has(o.type)) return [];
    const r = this.resolveHeight(o), n = [];
    for (const s of this.nodes.values()) {
      if (s.id === e || s.type === "edge") continue;
      const i = this.resolveHeight(s);
      s.x >= o.x && s.y >= o.y && s.x + s.w <= o.x + o.w && s.y + i <= o.y + r && n.push(s);
    }
    return n;
  }
  /** Returns tracked frame children (nodes explicitly added to the frame). */
  getFrameChildren(e) {
    const o = this.frameChildren.get(e);
    if (!o) return [];
    const r = [];
    for (const n of o) {
      const s = this.nodes.get(n);
      s && r.push(s);
    }
    return r;
  }
  /** Returns IDs of all descendants of a frame (children, grandchildren, etc.). */
  getFrameDescendantIds(e) {
    const o = /* @__PURE__ */ new Set(), r = (n) => {
      const s = this.frameChildren.get(n);
      if (s)
        for (const i of s) {
          if (o.has(i)) continue;
          o.add(i);
          const l = this.nodes.get(i);
          l && this._containerTypes.has(l.type) && r(i);
        }
    };
    return r(e), o;
  }
  /** Rebuild frameChildren from spatial containment. Called on load/undo/redo.
   *  Each node is assigned only to its smallest containing frame. */
  rebuildFrameChildren() {
    this.frameChildren.clear();
    const e = [];
    for (const r of this.nodes.values()) {
      if (!this._containerTypes.has(r.type)) continue;
      const n = this.resolveHeight(r);
      e.push({ node: r, area: r.w * n });
    }
    e.sort((r, n) => r.area - n.area);
    const o = /* @__PURE__ */ new Set();
    for (const { node: r } of e) {
      const s = this.getNodesInsideFrame(r.id).filter((i) => !o.has(i.id));
      if (s.length > 0) {
        const i = /* @__PURE__ */ new Set();
        for (const l of s)
          i.add(l.id), o.add(l.id);
        this.frameChildren.set(r.id, i);
      }
    }
  }
  /** After nodes are moved, update which frames they belong to.
   *  Each node is assigned only to its smallest containing frame.
   *  Frames can be nested inside other frames (but not inside themselves or their descendants). */
  updateFrameMembership(e) {
    for (const o of e) {
      const r = this.nodes.get(o);
      if (!r || r.type === "edge") continue;
      const n = this.resolveHeight(r);
      for (const [c, h] of this.frameChildren) {
        if (!h.has(o)) continue;
        const f = this.nodes.get(c);
        if (!f) {
          h.delete(o);
          continue;
        }
        const d = this.resolveHeight(f);
        r.x >= f.x && r.y >= f.y && r.x + r.w <= f.x + f.w && r.y + n <= f.y + d || h.delete(o);
      }
      let s;
      this._containerTypes.has(r.type) && (s = this.getFrameDescendantIds(o));
      let i = null, l = 1 / 0;
      const a = this.quadTree.retrieve([], { x: r.x, y: r.y, w: r.w, h: n });
      for (const c of a) {
        if (!this._containerTypes.has(c.type) || c.id === o || s != null && s.has(c.id)) continue;
        const h = this.resolveHeight(c);
        if (r.x >= c.x && r.y >= c.y && r.x + r.w <= c.x + c.w && r.y + n <= c.y + h) {
          const d = c.w * h;
          d < l && (l = d, i = c);
        }
      }
      for (const [, c] of this.frameChildren)
        c.delete(o);
      i && (this.frameChildren.has(i.id) || this.frameChildren.set(i.id, /* @__PURE__ */ new Set()), this.frameChildren.get(i.id).add(o));
    }
  }
  /** Sync frame children after resize: remove nodes no longer inside, add newly contained nodes. */
  syncFrameChildrenAfterResize(e) {
    const o = this.nodes.get(e);
    if (!o || !this._containerTypes.has(o.type)) return;
    const r = this.getNodesInsideFrame(e);
    r.length > 0 ? this.frameChildren.set(e, new Set(r.map((n) => n.id))) : this.frameChildren.delete(e);
  }
  /** Adopt all existing nodes that are spatially inside a newly created frame. */
  adoptNodesIntoNewFrame(e) {
    const o = this.getNodesInsideFrame(e);
    if (o.length > 0) {
      const r = /* @__PURE__ */ new Set();
      for (const n of o) r.add(n.id);
      this.frameChildren.set(e, r);
    }
  }
  nextZ() {
    return this.nextZValue++;
  }
  setNextZ(e) {
    this.nextZValue = e;
  }
  // --- Z-ordering ---
  bringToFront(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.nodes.get(o);
        r && !r.locked && this.nodes.set(o, { ...r, z: this.nextZValue++ });
      }
      this.emit("change"), this.emit("history");
    }
  }
  sendToBack(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (let o = e.length - 1; o >= 0; o--) {
        const r = this.nodes.get(e[o]);
        r && !r.locked && this.nodes.set(e[o], { ...r, z: --this._minZ });
      }
      this.emit("change"), this.emit("history");
    }
  }
  /** AABB overlap test between two nodes. */
  _nodesOverlap(e, o) {
    const r = this.resolveHeight(e), n = this.resolveHeight(o);
    return e.x < o.x + o.w && e.x + e.w > o.x && e.y < o.y + n && e.y + r > o.y;
  }
  bringForward(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.nodes.get(o);
        if (!r || r.locked) continue;
        const n = r.type === "edge", s = [];
        for (const h of this.nodes.values())
          h.id !== o && (n ? h.type === "edge" : h.type !== "edge") && h.z >= r.z && this._nodesOverlap(r, h) && s.push(h);
        if (s.length === 0) continue;
        s.sort((h, f) => h.z - f.z);
        const i = s[0], l = this.nodes.get(i.id), a = r.z, c = l.z;
        a === c ? this.nodes.set(o, { ...r, z: c + 1 }) : (this.nodes.set(o, { ...r, z: c }), this.nodes.set(i.id, { ...l, z: a }));
      }
      this.emit("change"), this.emit("history");
    }
  }
  sendBackward(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.nodes.get(o);
        if (!r || r.locked) continue;
        const n = r.type === "edge", s = [];
        for (const h of this.nodes.values())
          h.id !== o && (n ? h.type === "edge" : h.type !== "edge") && h.z <= r.z && this._nodesOverlap(r, h) && s.push(h);
        if (s.length === 0) continue;
        s.sort((h, f) => f.z - h.z);
        const i = s[0], l = this.nodes.get(i.id), a = r.z, c = l.z;
        a === c ? this.nodes.set(o, { ...r, z: c - 1 }) : (this.nodes.set(o, { ...r, z: c }), this.nodes.set(i.id, { ...l, z: a }));
      }
      this.emit("change"), this.emit("history");
    }
  }
  /** Update the QuadTree bounds for an auto-height node when its measured height changes. */
  updateMeasuredHeight(e, o) {
    const r = this.nodes.get(e);
    !r || r.h !== "auto" || (this._measuredHeights[e] = o, this.quadTree.remove(r), this.quadTree.insert(r, o));
  }
  // --- Spatial Queries ---
  hitTest(e, o, r) {
    const s = this.quadTree.retrieve([], {
      x: e - 50,
      y: o - 50,
      w: 100,
      h: 100
    }), i = /* @__PURE__ */ new Map();
    for (const l of s) i.set(l.id, l);
    return ml(i, e, o, this.viewport.zoom, r, this._containerTypes);
  }
  /** Returns all nodes at a point, sorted highest-z first */
  hitTestAll(e, o, r) {
    const s = this.quadTree.retrieve([], {
      x: e - 50,
      y: o - 50,
      w: 100,
      h: 100
    }), i = /* @__PURE__ */ new Map();
    for (const l of s) i.set(l.id, l);
    return bl(i, e, o, this.viewport.zoom, r, this._containerTypes);
  }
  getNodesInRect(e) {
    return this.quadTree.retrieve([], e);
  }
  // --- Selection ---
  /** Expand selection to include all group siblings, walking up the group
   *  hierarchy until the active group (or root) is reached. */
  expandSelectionToGroups() {
    const e = /* @__PURE__ */ new Set();
    for (const n of this.selection) {
      const s = this.nodes.get(n);
      if (!(s != null && s.groupId) || this.activeGroupId && s.groupId === this.activeGroupId) continue;
      let i = s.groupId;
      for (; ; ) {
        const l = this.groupParent.get(i);
        if (!l || this.activeGroupId && l === this.activeGroupId) break;
        i = l;
      }
      e.add(i);
    }
    if (e.size === 0) return;
    const o = new Set(e), r = (n) => {
      const s = this.groupChildren.get(n);
      if (s)
        for (const i of s)
          o.has(i) || (o.add(i), r(i));
    };
    for (const n of e)
      r(n);
    for (const n of this.nodes.values())
      n.groupId && o.has(n.groupId) && this.selection.add(n.id);
  }
  select(e) {
    var o, r, n, s, i, l;
    for (const a of this.selection) {
      const c = this.nodes.get(a);
      c && ((n = (r = (o = this.registry) == null ? void 0 : o.get(c.type)) == null ? void 0 : r.onDeselect) == null || n.call(r, c, this), this.emit("node:deselect", c));
    }
    this.selection.clear(), this.selection.add(e), this.expandSelectionToGroups();
    for (const a of this.selection) {
      const c = this.nodes.get(a);
      c && ((l = (i = (s = this.registry) == null ? void 0 : s.get(c.type)) == null ? void 0 : i.onSelect) == null || l.call(i, c, this), this.emit("node:select", c));
    }
    this.emit("selection");
  }
  toggleSelect(e) {
    const o = this.nodes.get(e);
    if (this.selection.has(e))
      if (o != null && o.groupId)
        for (const r of this.nodes.values())
          r.groupId === o.groupId && this.selection.delete(r.id);
      else
        this.selection.delete(e);
    else
      this.selection.add(e), this.expandSelectionToGroups();
    this.emit("selection");
  }
  selectMultiple(e) {
    this.selection = new Set(e), this.expandSelectionToGroups(), this.emit("selection");
  }
  deselectAll() {
    var e, o, r;
    if (!(this.selection.size === 0 && !this.activeGroupId)) {
      for (const n of this.selection) {
        const s = this.nodes.get(n);
        s && ((r = (o = (e = this.registry) == null ? void 0 : e.get(s.type)) == null ? void 0 : o.onDeselect) == null || r.call(o, s, this), this.emit("node:deselect", s));
      }
      this.selection.clear(), this.activeGroupId && (this.activeGroupId = null, this.emit("group:exit")), this.emit("selection");
    }
  }
  deleteSelected() {
    var r, n, s;
    if (this.selection.size === 0) return;
    const e = new Set(
      Array.from(this.selection).filter((i) => {
        var l;
        return !((l = this.nodes.get(i)) != null && l.locked);
      })
    );
    if (e.size === 0) return;
    this.activeGroupId && this.getGroupMembers(this.activeGroupId).filter((l) => !e.has(l.id)).length === 0 && (this.activeGroupId = null, this.emit("group:exit")), this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = e;
    for (const i of e) {
      const l = this.nodes.get(i);
      l && ((s = (n = (r = this.registry) == null ? void 0 : r.get(l.type)) == null ? void 0 : n.onDelete) == null || s.call(n, l, this), this.emit("node:delete", l), this.quadTree.remove(l), this.nodes.delete(i));
    }
    for (const [i, l] of this.nodes)
      if (l.type === "edge") {
        const a = l.data;
        if (o.has(a.fromId) || o.has(a.toId)) {
          const c = this.nodes.get(i);
          c && this.quadTree.remove(c), this.nodes.delete(i);
        }
      }
    this.cleanupEmptyGroups();
    for (const i of e) this.selection.delete(i);
    this.emit("change"), this.emit("selection"), this.emit("history");
  }
  /** Remove groupParent entries for groups that no longer have any members. */
  cleanupEmptyGroups() {
    const e = /* @__PURE__ */ new Set();
    for (const o of this.nodes.values())
      o.groupId && e.add(o.groupId);
    for (const [o] of this.groupParent)
      e.has(o) || this.unlinkGroupParent(o);
  }
  /** Set a groupParent entry and keep groupChildren in sync. */
  linkGroupParent(e, o) {
    var s;
    const r = this.groupParent.get(e);
    r && ((s = this.groupChildren.get(r)) == null || s.delete(e)), this.groupParent.set(e, o);
    let n = this.groupChildren.get(o);
    n || (n = /* @__PURE__ */ new Set(), this.groupChildren.set(o, n)), n.add(e);
  }
  /** Remove a groupParent entry and keep groupChildren in sync. */
  unlinkGroupParent(e) {
    const o = this.groupParent.get(e);
    if (o) {
      const r = this.groupChildren.get(o);
      r && (r.delete(e), r.size === 0 && this.groupChildren.delete(o));
    }
    this.groupParent.delete(e);
  }
  /** Rebuild the groupChildren reverse index from groupParent. */
  rebuildGroupChildren() {
    this.groupChildren.clear();
    for (const [e, o] of this.groupParent) {
      let r = this.groupChildren.get(o);
      r || (r = /* @__PURE__ */ new Set(), this.groupChildren.set(o, r)), r.add(e);
    }
  }
  deleteNodes(e) {
    var r, n, s;
    if (e.length === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = new Set(e);
    for (const i of e) {
      const l = this.nodes.get(i);
      if (l) {
        (s = (n = (r = this.registry) == null ? void 0 : r.get(l.type)) == null ? void 0 : n.onDelete) == null || s.call(n, l, this), this.emit("node:delete", l), this.quadTree.remove(l), this.nodes.delete(i), this.frameChildren.delete(i);
        for (const a of this.frameChildren.values()) a.delete(i);
      }
    }
    for (const [i, l] of this.nodes)
      if (l.type === "edge") {
        const a = l.data;
        if (o.has(a.fromId) || o.has(a.toId)) {
          const c = this.nodes.get(i);
          c && this.quadTree.remove(c), this.nodes.delete(i);
        }
      }
    this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  // --- Flip ---
  flipSelected(e) {
    var o;
    if (this.selection.size !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const r of this.selection) {
        const n = this.nodes.get(r);
        if (!n) continue;
        this.quadTree.remove(n);
        let s = null;
        const i = (o = this.registry) == null ? void 0 : o.get(n.type);
        if (i != null && i.onFlip) {
          const l = i.onFlip(n, e, this);
          l && Object.keys(l).length > 0 && (s = {
            ...n,
            data: { ...n.data, ...l }
          });
        } else if (n.type === "draw") {
          const l = n;
          if (e === "h") {
            const a = l.data.points.map(
              ([c, h, f]) => [l.w - c, h, f]
            );
            s = { ...l, data: { ...l.data, points: a } };
          } else {
            const a = l.h === "auto" ? 0 : l.h, c = l.data.points.map(
              ([h, f, d]) => [h, a - f, d]
            );
            s = { ...l, data: { ...l.data, points: c } };
          }
        } else if (n.type === "shape") {
          const l = n;
          if (l.data.shape === "arrow" || l.data.shape === "line")
            if (l.data.startPoint && l.data.endPoint)
              if (e === "h") {
                const a = [l.w - l.data.startPoint[0], l.data.startPoint[1]], c = [l.w - l.data.endPoint[0], l.data.endPoint[1]];
                s = { ...l, data: { ...l.data, startPoint: a, endPoint: c } };
              } else {
                const a = l.h === "auto" ? 0 : l.h, c = [l.data.startPoint[0], a - l.data.startPoint[1]], h = [l.data.endPoint[0], a - l.data.endPoint[1]];
                s = { ...l, data: { ...l.data, startPoint: c, endPoint: h } };
              }
            else
              s = e === "h" ? { ...l, rotation: -(l.rotation || 0) + 180 } : { ...l, rotation: -(l.rotation || 0) };
        } else if (n.type === "image") {
          const l = n;
          s = e === "h" ? { ...l, data: { ...l.data, flipH: !l.data.flipH } } : { ...l, data: { ...l.data, flipV: !l.data.flipV } };
        }
        s ? (this.nodes.set(r, s), this.quadTree.insert(s), this.emit("node:flip", s, e)) : this.quadTree.insert(n);
      }
      this.emit("change"), this.emit("history");
    }
  }
  flipSelectedHorizontal() {
    this.flipSelected("h");
  }
  flipSelectedVertical() {
    this.flipSelected("v");
  }
  // --- Grouping ---
  groupSelected() {
    if (this.selection.size < 2 || this.activeGroupId) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const e = wt(10), o = /* @__PURE__ */ new Set();
    for (const r of this.selection) {
      const n = this.nodes.get(r);
      if (n != null && n.groupId) {
        let s = n.groupId;
        for (; this.groupParent.has(s); ) s = this.groupParent.get(s);
        o.add(s);
      }
    }
    if (o.size > 0) {
      for (const r of o)
        this.linkGroupParent(r, e);
      for (const r of this.selection) {
        const n = this.nodes.get(r);
        n && !n.groupId && this.nodes.set(r, { ...n, groupId: e });
      }
    } else
      for (const r of this.selection) {
        const n = this.nodes.get(r);
        n && this.nodes.set(r, { ...n, groupId: e });
      }
    this.emit("change"), this.emit("history");
  }
  ungroupSelected() {
    if (this.selection.size === 0) return;
    const e = /* @__PURE__ */ new Set();
    for (const o of this.selection) {
      const r = this.nodes.get(o);
      if (r != null && r.groupId) {
        let n = r.groupId;
        for (; this.groupParent.has(n); ) {
          const s = this.groupParent.get(n);
          if (s === this.activeGroupId) break;
          n = s;
        }
        e.add(n);
      }
    }
    if (e.size !== 0) {
      this.activeGroupId && e.has(this.activeGroupId) && (this.activeGroupId = null, this.emit("group:exit")), this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.groupParent.get(o);
        for (const s of this.nodes.values())
          if (s.groupId === o)
            if (r)
              this.nodes.set(s.id, { ...s, groupId: r });
            else {
              const { groupId: i, ...l } = s;
              this.nodes.set(s.id, l);
            }
        const n = this.groupChildren.get(o);
        if (n)
          for (const s of [...n])
            r ? this.linkGroupParent(s, r) : this.unlinkGroupParent(s);
        this.unlinkGroupParent(o), this.groupChildren.delete(o), this.groupRotations.delete(o);
      }
      this.emit("change"), this.emit("history");
    }
  }
  selectionHasGroup() {
    var e;
    for (const o of this.selection)
      if ((e = this.nodes.get(o)) != null && e.groupId) return !0;
    return !1;
  }
  /** Returns the outermost groupId if all selected nodes belong to the same group tree, else undefined. */
  selectionGroupId() {
    if (this.selection.size < 2) return;
    let e;
    for (const o of this.selection) {
      const r = this.nodes.get(o);
      if (!(r != null && r.groupId)) return;
      let n = r.groupId;
      for (; this.groupParent.has(n); ) n = this.groupParent.get(n);
      if (!e) e = n;
      else if (n !== e) return;
    }
    return e;
  }
  /** True if all selected nodes belong to exactly one group (possibly nested). */
  selectionIsSingleGroup() {
    if (this.selection.size < 2) return !1;
    let e;
    for (const o of this.selection) {
      const r = this.nodes.get(o);
      if (!(r != null && r.groupId)) return !1;
      let n = r.groupId;
      for (; this.groupParent.has(n); ) n = this.groupParent.get(n);
      if (!e) e = n;
      else if (n !== e) return !1;
    }
    return !0;
  }
  getGroupMembers(e) {
    const o = [];
    for (const r of this.nodes.values())
      r.groupId === e && o.push(r);
    return o;
  }
  /** Enter a group for drill-down selection of individual children. */
  enterGroup(e) {
    this.activeGroupId !== e && (this.activeGroupId = e, this.emit("group:enter", e));
  }
  /** Fully exit all group levels and deselect. */
  exitAllGroups() {
    this.activeGroupId && (this.activeGroupId = null, this.emit("group:exit"));
  }
  /** Exit the active group — go up one level for nested groups, or exit entirely. */
  exitGroup() {
    if (!this.activeGroupId) return;
    const e = this.activeGroupId, o = this.groupParent.get(e);
    o ? (this.activeGroupId = o, this.emit("group:enter", o)) : (this.activeGroupId = null, this.emit("group:exit"));
    const r = this.getGroupMembers(e);
    r.length > 0 && (this.selection = /* @__PURE__ */ new Set([r[0].id]), this.expandSelectionToGroups(), this.emit("selection"));
  }
  /** Check if a node belongs to the currently active (entered) group or any of its descendants. */
  isNodeInActiveGroup(e) {
    if (!this.activeGroupId) return !1;
    const o = this.nodes.get(e);
    if (!(o != null && o.groupId)) return !1;
    let r = o.groupId;
    for (; r; ) {
      if (r === this.activeGroupId) return !0;
      r = this.groupParent.get(r);
    }
    return !1;
  }
  /** Get the outermost group of a node (stopping at activeGroupId boundary). */
  getNodeOutermostGroup(e) {
    const o = this.nodes.get(e);
    if (!(o != null && o.groupId)) return;
    let r = o.groupId;
    for (; ; ) {
      const n = this.groupParent.get(r);
      if (!n || this.activeGroupId && n === this.activeGroupId) break;
      r = n;
    }
    return r;
  }
  /** Get all nodes that are descendants of a group (direct + nested sub-groups). */
  getAllGroupDescendantNodes(e) {
    const o = /* @__PURE__ */ new Set([e]), r = (s) => {
      const i = this.groupChildren.get(s);
      if (i)
        for (const l of i)
          o.has(l) || (o.add(l), r(l));
    };
    r(e);
    const n = [];
    for (const s of this.nodes.values())
      s.groupId && o.has(s.groupId) && n.push(s);
    return n;
  }
  duplicateSelected() {
    if (this.selection.size === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const e = 20, o = /* @__PURE__ */ new Map(), r = [];
    for (const s of this.selection) {
      const i = this.nodes.get(s);
      if (!i) continue;
      const l = wt();
      o.set(i.id, l), r.push({
        ...JSON.parse(JSON.stringify(i)),
        id: l,
        x: i.x + e,
        y: i.y + e,
        z: this.nextZValue++,
        locked: void 0
      });
    }
    for (const s of r)
      if (s.type === "edge" && s.data) {
        const i = s.data;
        o.has(i.fromId) && (i.fromId = o.get(i.fromId)), o.has(i.toId) && (i.toId = o.get(i.toId));
      }
    const n = /* @__PURE__ */ new Map();
    for (const s of r)
      s.groupId && (n.has(s.groupId) || n.set(s.groupId, wt(10)), s.groupId = n.get(s.groupId));
    for (const [s, i] of this.groupParent)
      n.has(s) && n.has(i) && this.linkGroupParent(n.get(s), n.get(i));
    this.addNodes(r), this.selection = new Set(r.map((s) => s.id)), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  // --- Mode ---
  // --- Clipboard ---
  copySelected() {
    this.selection.size !== 0 && (this.clipboard = Array.from(this.selection).map((e) => {
      const o = this.nodes.get(e);
      return JSON.parse(JSON.stringify(o));
    }), this.pasteCount = 0);
  }
  cutSelected() {
    this.copySelected(), this.deleteSelected();
  }
  /**
   * Paste clipboard contents centered at a canvas position.
   * If no position given, uses viewport center.
   */
  pasteClipboard(e, o) {
    if (this.clipboard.length === 0) return;
    this.pasteCount++;
    let r = 1 / 0, n = 1 / 0, s = -1 / 0, i = -1 / 0;
    for (const b of this.clipboard) {
      const x = b.h === "auto" ? 100 : b.h;
      b.x < r && (r = b.x), b.y < n && (n = b.y), b.x + b.w > s && (s = b.x + b.w), b.y + x > i && (i = b.y + x);
    }
    const l = (r + s) / 2, a = (n + i) / 2;
    let c, h;
    if (e !== void 0 && o !== void 0)
      c = e, h = o;
    else {
      const b = this.getWindow(), x = b.innerWidth / 2, C = b.innerHeight / 2, z = Eo(this.viewport, x, C);
      c = z.x, h = z.y;
    }
    const f = this.pasteCount * 20, d = c - l + f, p = h - a + f, y = /* @__PURE__ */ new Map(), m = this.clipboard.map((b) => {
      const x = wt();
      return y.set(b.id, x), {
        ...structuredClone(b),
        id: x,
        x: b.x + d,
        y: b.y + p,
        z: this.nextZValue++,
        locked: void 0
      };
    });
    for (const b of m)
      if (b.type === "edge" && b.data) {
        const x = b.data;
        y.has(x.fromId) && (x.fromId = y.get(x.fromId)), y.has(x.toId) && (x.toId = y.get(x.toId));
      }
    const g = /* @__PURE__ */ new Map();
    for (const b of m)
      b.groupId && (g.has(b.groupId) || g.set(b.groupId, wt(10)), b.groupId = g.get(b.groupId));
    for (const [b, x] of this.groupParent)
      g.has(b) && g.has(x) && this.linkGroupParent(g.get(b), g.get(x));
    this.addNodes(m), this.selectMultiple(m.map((b) => b.id));
  }
  /**
   * Insert a pre-built template centered at (cx, cy) in canvas coordinates.
   */
  applyTemplate(e, o, r) {
    const n = wi.find((p) => p.id === e);
    if (!n) return;
    const s = structuredClone(n.nodes), i = /* @__PURE__ */ new Map();
    for (const p of s) {
      const y = wt(10);
      i.set(p.id, y), p.id = y;
    }
    for (const p of s) {
      if (p.type === "edge" && p.data) {
        const y = p.data;
        i.has(y.fromId) && (y.fromId = i.get(y.fromId)), i.has(y.toId) && (y.toId = i.get(y.toId));
      }
      p.groupId && i.has(p.groupId) && (p.groupId = i.get(p.groupId));
    }
    let l = 1 / 0, a = 1 / 0, c = -1 / 0, h = -1 / 0;
    for (const p of s) {
      if (p.type === "edge") continue;
      const y = p.h === "auto" ? 100 : p.h;
      l = Math.min(l, p.x), a = Math.min(a, p.y), c = Math.max(c, p.x + p.w), h = Math.max(h, p.y + y);
    }
    const f = o - (l + c) / 2, d = r - (a + h) / 2;
    for (const p of s)
      p.type !== "edge" && (p.x += f, p.y += d), p.z = this.nextZValue++;
    this.addNodes(s), this.selectMultiple(s.map((p) => p.id));
  }
  hasClipboard() {
    return this.clipboard.length > 0;
  }
  getClipboardNodes() {
    return this.clipboard.map((e) => structuredClone(e));
  }
  setClipboard(e) {
    this.clipboard = e.map((o) => structuredClone(o)), this.pasteCount = 0;
  }
  // --- Mode ---
  setMode(e) {
    this.mode !== e && (this.mode = e, this.selection.size > 0 && (this.selection.clear(), this.emit("selection")), this.emit("mode"));
  }
  // --- History ---
  pushHistorySnapshot() {
    this.history.pushSnapshot(this.nodes, this.groupParent), this.emit("history");
  }
  /*private*/
  rebuildQuadTree() {
    this.quadTree.clear(), this.adjacency.clear();
    let e = 0, o = 0;
    for (const r of this.nodes.values())
      if (this.quadTree.insert(r), r.z < e && (e = r.z), r.z > o && (o = r.z), r.type === "edge") {
        const n = r, { fromId: s, toId: i } = n.data;
        this.adjacency.has(s) || this.adjacency.set(s, /* @__PURE__ */ new Set()), this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.get(s).add(r.id), this.adjacency.get(i).add(r.id);
      }
    this._minZ = e, this.nextZValue = o + 1;
  }
  undo() {
    const e = this.history.undo(this.nodes, this.groupParent);
    e && (this.nodes = e.nodes, this.groupParent = e.groupParent, this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history"));
  }
  redo() {
    const e = this.history.redo(this.nodes, this.groupParent);
    e && (this.nodes = e.nodes, this.groupParent = e.groupParent, this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history"));
  }
  canUndo() {
    return this.history.canUndo();
  }
  canRedo() {
    return this.history.canRedo();
  }
  // --- Remote Collaboration ---
  /** Add a remote node without emitting events or pushing history. */
  addRemoteNode(e) {
    if (this._suppressEvents = !0, this.nodes.set(e.id, e), this.quadTree.insert(e), e.type === "edge") {
      const o = e, { fromId: r, toId: n } = o.data;
      this.adjacency.has(r) || this.adjacency.set(r, /* @__PURE__ */ new Set()), this.adjacency.has(n) || this.adjacency.set(n, /* @__PURE__ */ new Set()), this.adjacency.get(r).add(e.id), this.adjacency.get(n).add(e.id);
    }
    e.z >= this.nextZValue && (this.nextZValue = e.z + 1), e.z < this._minZ && (this._minZ = e.z), this._suppressEvents = !1;
  }
  /** Delete a remote node without emitting events or pushing history. */
  deleteRemoteNode(e) {
    var r;
    this._suppressEvents = !0;
    const o = this.nodes.get(e);
    if (o) {
      this.quadTree.remove(o), this.nodes.delete(e), this.selection.delete(e), this.adjacency.delete(e), this.frameChildren.delete(e);
      for (const n of this.frameChildren.values()) n.delete(e);
      for (const [n, s] of this.nodes)
        if (s.type === "edge") {
          const i = s.data;
          if (i.fromId === e || i.toId === e) {
            const l = this.nodes.get(n);
            l && this.quadTree.remove(l), this.nodes.delete(n), this.selection.delete(n);
            const a = i.fromId === e ? i.toId : i.fromId;
            (r = this.adjacency.get(a)) == null || r.delete(n);
          }
        }
    }
    this._suppressEvents = !1;
  }
  /** Apply a remote node update without emitting events or pushing history. */
  applyRemoteNodeUpdate(e, o) {
    this._suppressEvents = !0;
    const r = this.nodes.get(e);
    if (r) {
      const n = { ...r, ...o };
      o.data && typeof o.data == "object" && r.data && typeof r.data == "object" && (n.data = {
        ...r.data,
        ...o.data
      }), this.nodes.set(e, n), (r.x !== n.x || r.y !== n.y || r.w !== n.w || r.h !== n.h) && (this.quadTree.remove(r), this.quadTree.insert(n), this.updateConnectedEdges(e)), n.z >= this.nextZValue && (this.nextZValue = n.z + 1);
    }
    this._suppressEvents = !1;
  }
  /** Trigger a re-render without pushing history. Used after remote updates. */
  notifyChange() {
    var e;
    (e = this.listeners.change) == null || e.forEach((o) => o());
  }
  /** Emit draw progress for collab live stroke preview. */
  notifyDrawProgress(e) {
    this.emit("draw:progress", e);
  }
  /** Emit draw end when a stroke is completed or cancelled. */
  notifyDrawEnd() {
    this.emit("draw:end");
  }
  /** Emit shape progress for collab live shape preview. */
  notifyShapeProgress(e) {
    this.emit("shape:progress", e);
  }
  /** Emit shape end when shape creation is completed or cancelled. */
  notifyShapeEnd() {
    this.emit("shape:end");
  }
  /** Emit laser pointer progress for collab trail preview. */
  notifyLaserProgress(e) {
    this.emit("laser:progress", e);
  }
  /** Emit laser pointer end when trail has fully faded. */
  notifyLaserEnd() {
    this.emit("laser:end");
  }
  // --- Serialization ---
  async toSBD() {
    return Ml(this.getAllNodes(), {
      background: this.boardBackground,
      originView: this.originView ?? void 0
    });
  }
  async fromSBD(e) {
    this.history.clear(), this.nodes.clear(), this.groupParent.clear(), this.groupChildren.clear();
    const { nodes: o, meta: r } = await Rl(e);
    r.background && (this.boardBackground = r.background, this.emit("background")), r.originView ? this.originView = r.originView : this.originView = null;
    let n = 0, s = 0;
    for (const i of o)
      this.nodes.set(i.id, i), i.z > n && (n = i.z), i.z < s && (s = i.z);
    this.rebuildQuadTree(), this.rebuildFrameChildren(), this.nextZValue = n + 1, this._minZ = s, this.selection.clear(), this.goToOriginView(), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  toJSON() {
    const e = {
      nodes: Array.from(this.nodes.entries()),
      viewport: this.viewport
    };
    return this.groupParent.size > 0 && (e.groupParent = Array.from(this.groupParent.entries())), e;
  }
  fromJSON(e) {
    this.history.clear(), this.nodes = new Map(e.nodes), this.groupParent = new Map(e.groupParent ?? []), this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), e.viewport && (this.viewport = e.viewport), this.selection.clear(), this.emit("change"), this.emit("viewport"), this.emit("selection"), this.emit("history");
  }
}
class Ul {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor(e) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    St(this, "types", /* @__PURE__ */ new Map());
    if (e)
      for (const o of e) this.register(o);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  register(e) {
    this.types.set(e.type, e);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  get(e) {
    return this.types.get(e);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getAll() {
    return Array.from(this.types.values());
  }
  /** Returns type strings for nodes that render in the DOM layer (not SVG-only). */
  getDOMTypes() {
    const e = /* @__PURE__ */ new Set();
    for (const o of this.types.values())
      o.isSVGOnly || e.add(o.type);
    return e;
  }
  has(e) {
    return this.types.has(e);
  }
}
const vs = ["n", "ne", "e", "se", "s", "sw", "w", "nw"], Ql = {
  n: "ns-resize",
  ne: "nesw-resize",
  e: "ew-resize",
  se: "nwse-resize",
  s: "ns-resize",
  sw: "nesw-resize",
  w: "ew-resize",
  nw: "nwse-resize"
};
function Fr(t, e) {
  const o = vs.indexOf(t);
  if (o === -1) return "default";
  const r = (e % 360 + 360) % 360, n = Math.round(r / 45) % 8, s = (o + n) % 8;
  return Ql[vs[s]];
}
class Jl extends Za {
  constructor() {
    super(...arguments);
    St(this, "state", { hasError: !1 });
  }
  static getDerivedStateFromError() {
    return { hasError: !0 };
  }
  componentDidCatch(o, r) {
    console.error("[ContentBlock] Editor mount failed, showing markdown fallback:", o, r);
  }
  render() {
    return this.state.hasError ? this.props.fallback : this.props.children;
  }
}
function Ss({ markdown: t }) {
  return /* @__PURE__ */ u(
    "div",
    {
      className: "sb-markdown-fallback",
      style: {
        padding: "8px 12px",
        fontSize: 14,
        lineHeight: 1.6,
        whiteSpace: "pre-wrap",
        wordBreak: "break-word",
        color: "#374151",
        opacity: 0.85
      },
      children: t || " "
    }
  );
}
const $l = 0, _l = [
  { pos: "nw", top: 0, left: 0 },
  { pos: "n", top: 0, left: "50%" },
  { pos: "ne", top: 0, left: "100%" },
  { pos: "e", top: "50%", left: "100%" },
  { pos: "se", top: "100%", left: "100%" },
  { pos: "s", top: "100%", left: "50%" },
  { pos: "sw", top: "100%", left: 0 },
  { pos: "w", top: "50%", left: 0 }
];
function tc(t) {
  return t.type !== "paragraph" ? !1 : !t.content || t.content.length === 0 ? !0 : t.content.every(
    (e) => e.type === "text" && (!e.text || e.text === "")
  );
}
function ec({
  node: t,
  isSelected: e,
  multiSelected: o,
  engine: r,
  schema: n,
  interactive: s,
  zoom: i,
  onMeasuredHeight: l,
  autoEdit: a
}) {
  const c = at(null), h = at(a === !0), f = at(!1), d = at(!1), p = at(!1), y = at(!1), m = at(t.data.blocks), [g, b] = tt(!1), [x, C] = tt(!1), z = at(null), S = Ja({ schema: n }), F = at(
    t.data.blocks.length > 0 ? t.data.blocks : null
  );
  mt(() => {
    const R = F.current;
    if (!R) return;
    F.current = null;
    const O = requestAnimationFrame(() => {
      try {
        S.replaceBlocks(S.document, R);
        return;
      } catch {
      }
      try {
        const _ = S.blocksToHTMLLossy(R);
        S._tiptapEditor.commands.setContent(_);
        return;
      } catch {
      }
      console.warn("[ContentBlock] All block rendering strategies failed, using markdown fallback"), C(!0);
    });
    return () => cancelAnimationFrame(O);
  }, [S]), mt(() => {
    (!e || o) && b(!1);
  }, [e, o]), mt(() => {
    h.current && (h.current = !1, f.current = !0, b(!0));
  }, [S]), mt(() => {
    if (!g || !f.current && !z.current) return;
    const R = z.current;
    z.current = null, f.current = !1;
    const O = requestAnimationFrame(() => {
      if (S.focus(), R)
        try {
          const _ = S._tiptapEditor, V = _.view.posAtCoords({ left: R.x, top: R.y });
          V && _.commands.setTextSelection(V.pos);
        } catch {
        }
    });
    return () => cancelAnimationFrame(O);
  }, [g, S]);
  const D = it(() => {
    if (d.current || p.current) return;
    const R = r.getNode(t.id), O = S.document;
    m.current = O, r.updateNode(t.id, {
      data: { ...R == null ? void 0 : R.data, blocks: O }
    });
  }, [S, r, t.id]);
  mt(() => {
    if (!S) return;
    const R = () => {
      var nt, ot;
      if (d.current || p.current || y.current) return;
      const G = S.document.length, V = r.getNode(t.id), Y = ((ot = (nt = V == null ? void 0 : V.data) == null ? void 0 : nt.blocks) == null ? void 0 : ot.length) ?? 0;
      if (G < Y) return;
      const $ = setTimeout(D, 100);
      return () => clearTimeout($);
    };
    let O;
    const _ = S.onChange(() => {
      O == null || O(), O = R();
    });
    return () => {
      _ == null || _(), O == null || O();
    };
  }, [S, D]), mt(() => {
    const R = c.current;
    if (!R) return;
    const O = (_) => {
      const G = _.relatedTarget;
      G && R.contains(G) || D();
    };
    return R.addEventListener("focusout", O), () => R.removeEventListener("focusout", O);
  }, [D]), mt(() => {
    if (g || t.data.blocks === m.current) return;
    const R = JSON.stringify(t.data.blocks), O = JSON.stringify(m.current);
    if (R !== O) {
      if (t.data.blocks.length > 0 && S.document.length > 0) {
        y.current = !0;
        try {
          S.replaceBlocks(S.document, t.data.blocks);
        } catch {
          try {
            const _ = S.blocksToHTMLLossy(t.data.blocks);
            S._tiptapEditor.commands.setContent(_);
          } catch {
          }
        }
        y.current = !1;
      }
      m.current = t.data.blocks;
    }
  }, [t.data.blocks, g, S]), mt(() => {
    if (t.h !== "auto" || !l) return;
    const R = c.current;
    if (!R) return;
    const O = () => {
      const G = R.offsetHeight;
      G > 0 && l(t.id, G);
    };
    O();
    const _ = new ResizeObserver(O);
    return _.observe(R), () => _.disconnect();
  }, [t.id, t.h, l]);
  const W = it(() => {
    const R = r.getNode(t.id);
    if (!R || R.h === "auto" || !S || !c.current)
      return;
    const O = R.h - $l, _ = c.current.querySelector(".bn-editor");
    if (!_) return;
    const G = S.document;
    if (G.length === 0) return;
    let V = 0;
    for (let ot = G.length - 1; ot >= 1 && tc(G[ot]); ot--)
      V++;
    const Y = _.scrollHeight, $ = G.length > 0 ? Y / G.length : 36;
    if (d.current = !0, Y < O) {
      const ot = O - Y, yt = Math.max(0, Math.floor(ot / $));
      if (yt > 0) {
        const ft = G[G.length - 1];
        S.insertBlocks(
          Array.from({ length: yt }, () => ({
            type: "paragraph",
            content: []
          })),
          ft,
          "after"
        );
      }
    } else if (Y > O && V > 0) {
      const ot = Y - O, yt = Math.min(V, Math.ceil(ot / $));
      if (yt > 0) {
        const ft = G.slice(G.length - yt);
        S.removeBlocks(ft);
      }
    }
    const nt = r.getNode(t.id);
    nt && r.updateNode(t.id, {
      data: { ...nt.data, blocks: S.document }
    }), d.current = !1;
  }, [S, r, t.id]), K = at(W);
  K.current = W, mt(() => {
    if (t.h === "auto") return;
    const R = setTimeout(() => K.current(), 60);
    return () => clearTimeout(R);
  }, []);
  const Z = it(
    (R) => {
      const O = R.currentTarget.ownerDocument;
      if (R.altKey) return;
      if (!r.selection.has(t.id) && r.selection.size > 0) {
        const { x: gt, y: ht } = r.screenToCanvas(R.clientX, R.clientY);
        for (const Dt of r.selection) {
          const It = r.getNode(Dt);
          if (!It) continue;
          const Rt = It.h === "auto" ? 100 : It.h;
          if (gt >= It.x && gt <= It.x + It.w && ht >= It.y && ht <= It.y + Rt)
            return;
        }
      }
      R.stopPropagation(), R.preventDefault(), R.currentTarget.setPointerCapture(R.pointerId), R.shiftKey ? r.toggleSelect(t.id) : r.selection.has(t.id) || r.select(t.id);
      const _ = R.clientX, G = R.clientY, V = Array.from(r.selection), Y = V.map((gt) => {
        const ht = r.getNode(gt);
        return { id: gt, x: ht.x, y: ht.y };
      });
      let $ = !1, nt = null, ot = _, yt = G, ft = !1;
      const et = () => {
        nt = null;
        const gt = (ot - _) / r.viewport.zoom, ht = (yt - G) / r.viewport.zoom, { finalDx: Dt, finalDy: It } = r.computeDragSnap(
          Y,
          V,
          gt,
          ht,
          ft
        ), Rt = Y.map((Qt) => ({
          id: Qt.id,
          patch: { x: Qt.x + Dt, y: Qt.y + It }
        }));
        r.updateMany(Rt);
      }, Pt = (gt) => {
        const ht = (gt.clientX - _) / r.viewport.zoom, Dt = (gt.clientY - G) / r.viewport.zoom;
        if (!$)
          if (Math.abs(ht) > 2 || Math.abs(Dt) > 2)
            $ = !0, p.current = !0, r.pushHistorySnapshot();
          else
            return;
        ot = gt.clientX, yt = gt.clientY, ft = gt.metaKey || gt.ctrlKey, nt === null && (nt = requestAnimationFrame(et));
      }, zt = () => {
        p.current = !1, nt !== null && (cancelAnimationFrame(nt), et()), r.clearAlignGuides(), O.removeEventListener("pointermove", Pt), O.removeEventListener("pointerup", zt);
      };
      O.addEventListener("pointermove", Pt), O.addEventListener("pointerup", zt);
    },
    [r, t.id]
  ), dt = it(
    (R) => {
      var et;
      const O = R.currentTarget.ownerDocument;
      R.stopPropagation(), R.preventDefault();
      const _ = t.h === "auto" ? (((et = c.current) == null ? void 0 : et.getBoundingClientRect().height) ?? 60) / r.viewport.zoom : t.h, G = t.x + t.w / 2, V = t.y + _ / 2, Y = t.rotation || 0, { x: $, y: nt } = r.screenToCanvas(
        R.clientX,
        R.clientY
      ), ot = Math.atan2(nt - V, $ - G);
      r.pushHistorySnapshot();
      const yt = (Pt) => {
        const { x: zt, y: gt } = r.screenToCanvas(Pt.clientX, Pt.clientY), ht = Math.atan2(gt - V, zt - G);
        let Dt = Y + (ht - ot) * (180 / Math.PI);
        (Pt.shiftKey || r.snapToGrid) && !(Pt.metaKey || Pt.ctrlKey) && (Dt = Math.round(Dt / 15) * 15), r.updateNode(t.id, { rotation: Dt });
      }, ft = () => {
        O.removeEventListener("pointermove", yt), O.removeEventListener("pointerup", ft);
      };
      O.addEventListener("pointermove", yt), O.addEventListener("pointerup", ft);
    },
    [r, t.id, t.x, t.y, t.w, t.h, t.rotation]
  ), X = it(
    (R, O) => {
      var et;
      const _ = O.currentTarget.ownerDocument;
      O.stopPropagation(), O.preventDefault();
      const G = O.clientX, V = O.clientY, Y = t.x, $ = t.y, nt = t.w, ot = t.h === "auto" ? (((et = c.current) == null ? void 0 : et.getBoundingClientRect().height) ?? 60) / r.viewport.zoom : t.h;
      r.pushHistorySnapshot();
      const yt = (Pt) => {
        const zt = (Pt.clientX - G) / r.viewport.zoom, gt = (Pt.clientY - V) / r.viewport.zoom;
        let ht = Y, Dt = $, It = nt, Rt = ot;
        if ((R === "nw" || R === "w" || R === "sw") && (ht = Y + zt, It = nt - zt), (R === "ne" || R === "e" || R === "se") && (It = nt + zt), (R === "nw" || R === "n" || R === "ne") && (Dt = $ + gt, Rt = ot - gt), (R === "sw" || R === "s" || R === "se") && (Rt = ot + gt), r.snapToGrid && !(Pt.metaKey || Pt.ctrlKey)) {
          const Qt = r.gridSize, Jt = (qt) => Math.round(qt / Qt) * Qt;
          (R === "nw" || R === "w" || R === "sw") && (ht = Jt(ht), It = Y + nt - ht), (R === "ne" || R === "e" || R === "se") && (It = Jt(ht + It) - ht), (R === "nw" || R === "n" || R === "ne") && (Dt = Jt(Dt), Rt = $ + ot - Dt), (R === "sw" || R === "s" || R === "se") && (Rt = Jt(Dt + Rt) - Dt);
        }
        It < 100 && (It = 100, (R === "nw" || R === "w" || R === "sw") && (ht = Y + nt - 100)), Rt < 60 && (Rt = 60, (R === "nw" || R === "n" || R === "ne") && (Dt = $ + ot - 60)), r.updateNode(t.id, { x: ht, y: Dt, w: It, h: Rt });
      }, ft = () => {
        _.removeEventListener("pointermove", yt), _.removeEventListener("pointerup", ft), requestAnimationFrame(() => K.current());
      };
      _.addEventListener("pointermove", yt), _.addEventListener("pointerup", ft);
    },
    [r, t.id, t.x, t.y, t.w, t.h]
  ), rt = it(
    (R) => {
      if (!R.altKey) {
        if (g) {
          R.stopPropagation();
          return;
        }
        if (e) {
          Z(R);
          return;
        }
        Z(R);
      }
    },
    [g, e, Z, r, t.id]
  ), J = it(
    (R) => {
      if (R.stopPropagation(), !g) {
        if (t.groupId) {
          const O = [];
          let _ = t.groupId;
          for (; _; )
            O.push(_), _ = r.groupParent.get(_);
          if (!r.activeGroupId) {
            r.enterGroup(O[O.length - 1]), r.select(t.id);
            return;
          }
          const G = O.indexOf(r.activeGroupId);
          if (G > 0) {
            r.enterGroup(O[G - 1]), r.select(t.id);
            return;
          }
        }
        r.select(t.id), z.current = { x: R.clientX, y: R.clientY }, b(!0);
      }
    },
    [g, r, t.id, t.groupId, S]
  ), U = e && !o;
  return /* @__PURE__ */ v(
    "div",
    {
      ref: c,
      "data-node-id": t.id,
      className: s ? void 0 : "sb-block-inert",
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        zIndex: t.z,
        height: t.h === "auto" ? void 0 : t.h,
        minHeight: 20,
        border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : "none",
        boxSizing: t.data.borderColor ? "border-box" : void 0,
        outline: e ? `${1.5 / i}px dashed #3b82f6` : "none",
        outlineOffset: t.data.borderColor ? 2 / i : 0,
        borderRadius: t.data.edgeStyle === "round" ? 12 : 0,
        background: "transparent",
        boxShadow: "none",
        overflow: "visible",
        pointerEvents: s ? "auto" : "none",
        display: "flex",
        flexDirection: "column",
        opacity: t.data.opacity ?? 1,
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0
      },
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            onDoubleClick: J,
            style: {
              overflow: "hidden",
              height: "100%",
              display: "flex",
              flexDirection: "column",
              borderRadius: t.data.borderColor || e ? 7 : 0
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                className: "sb-editor-wrap",
                onPointerDown: rt,
                onKeyDown: g ? (R) => {
                  R.key === "Escape" && (R.stopPropagation(), b(!1));
                } : void 0,
                style: g ? { cursor: "text", userSelect: "text" } : { cursor: "move", userSelect: "none" },
                children: x ? /* @__PURE__ */ u(Ss, { markdown: t.data.markdown ?? "" }) : /* @__PURE__ */ u(Jl, { fallback: /* @__PURE__ */ u(Ss, { markdown: t.data.markdown ?? "" }), children: /* @__PURE__ */ u(
                  $a,
                  {
                    editor: S,
                    theme: "light",
                    editable: s && g
                  }
                ) })
              }
            )
          }
        ),
        U && _l.map(({ pos: R, top: O, left: _ }) => {
          const G = 8 / i;
          return /* @__PURE__ */ u(
            "div",
            {
              onPointerDown: (V) => X(R, V),
              style: {
                position: "absolute",
                top: O,
                left: _,
                width: G,
                height: G,
                transform: "translate(-50%, -50%)",
                background: "white",
                border: `${1.5 / i}px solid #3b82f6`,
                cursor: Fr(R, t.rotation || 0),
                zIndex: 10,
                pointerEvents: "auto"
              }
            },
            R
          );
        }),
        U && (() => {
          const R = 25 / i, O = 10 / i;
          return /* @__PURE__ */ v(ut, { children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  position: "absolute",
                  top: -R,
                  left: "50%",
                  width: 1.5 / i,
                  height: R,
                  transform: "translateX(-50%)",
                  background: "#3b82f6",
                  pointerEvents: "none"
                }
              }
            ),
            /* @__PURE__ */ u(
              "div",
              {
                onPointerDown: dt,
                style: {
                  position: "absolute",
                  top: -(R + O / 2),
                  left: "50%",
                  width: O,
                  height: O,
                  transform: "translateX(-50%) rotate(45deg)",
                  borderRadius: 1.5 / i,
                  background: "white",
                  border: `${1.5 / i}px solid #3b82f6`,
                  cursor: "grab",
                  zIndex: 10,
                  pointerEvents: "auto"
                }
              }
            )
          ] });
        })()
      ]
    }
  );
}
const Ri = ce(ec);
function oc(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Ri,
    {
      node: e,
      isSelected: t.isSelected,
      multiSelected: t.multiSelected,
      engine: t.engine,
      schema: Fn,
      interactive: t.interactive,
      zoom: t.zoom,
      onMeasuredHeight: t.callbacks.onMeasuredHeight
    }
  );
}
const rc = {
  type: "content",
  component: oc,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.markdown || null
}, { PI: nc } = Math, or = nc + 1e-4, Ms = 0.5, Cs = [1, 1];
function zs(t, e, o, r = (n) => n) {
  return t * r(0.5 - e * (0.5 - o));
}
const { min: _r } = Math;
function Ai(t, e, o) {
  let r = _r(1, e / o);
  return _r(1, t + (_r(1, 1 - r) - t) * (r * 0.275));
}
function sc(t) {
  return [-t[0], -t[1]];
}
function Te(t, e) {
  return [t[0] + e[0], t[1] + e[1]];
}
function Is(t, e, o) {
  return t[0] = e[0] + o[0], t[1] = e[1] + o[1], t;
}
function Ve(t, e) {
  return [t[0] - e[0], t[1] - e[1]];
}
function vn(t, e, o) {
  return t[0] = e[0] - o[0], t[1] = e[1] - o[1], t;
}
function Ge(t, e) {
  return [t[0] * e, t[1] * e];
}
function tn(t, e, o) {
  return t[0] = e[0] * o, t[1] = e[1] * o, t;
}
function ic(t, e) {
  return [t[0] / e, t[1] / e];
}
function Li(t) {
  return [t[1], -t[0]];
}
function en(t, e) {
  let o = e[0];
  return t[0] = e[1], t[1] = -o, t;
}
function Ts(t, e) {
  return t[0] * e[0] + t[1] * e[1];
}
function ac(t, e) {
  return t[0] === e[0] && t[1] === e[1];
}
function lc(t) {
  return Math.hypot(t[0], t[1]);
}
function Ps(t, e) {
  let o = t[0] - e[0], r = t[1] - e[1];
  return o * o + r * r;
}
function Di(t) {
  return ic(t, lc(t));
}
function cc(t, e) {
  return Math.hypot(t[1] - e[1], t[0] - e[0]);
}
function Xn(t, e, o) {
  let r = Math.sin(o), n = Math.cos(o), s = t[0] - e[0], i = t[1] - e[1], l = s * n - i * r, a = s * r + i * n;
  return [l + e[0], a + e[1]];
}
function Rs(t, e, o, r) {
  let n = Math.sin(r), s = Math.cos(r), i = e[0] - o[0], l = e[1] - o[1], a = i * s - l * n, c = i * n + l * s;
  return t[0] = a + o[0], t[1] = c + o[1], t;
}
function As(t, e, o) {
  return Te(t, Ge(Ve(e, t), o));
}
function dc(t, e, o, r) {
  let n = o[0] - e[0], s = o[1] - e[1];
  return t[0] = e[0] + n * r, t[1] = e[1] + s * r, t;
}
function Ei(t, e, o) {
  return Te(t, Ge(e, o));
}
const ie = [0, 0], Oe = [0, 0], Xe = [0, 0];
function hc(t, e) {
  let o = Ei(t, Di(Li(Ve(t, Te(t, [1, 1])))), -e), r = [], n = 1 / 13;
  for (let s = n; s <= 1; s += n) r.push(Xn(o, t, or * 2 * s));
  return r;
}
function uc(t, e, o) {
  let r = [], n = 1 / o;
  for (let s = n; s <= 1; s += n) r.push(Xn(e, t, or * s));
  return r;
}
function fc(t, e, o) {
  let r = Ve(e, o), n = Ge(r, 0.5), s = Ge(r, 0.51);
  return [Ve(t, n), Ve(t, s), Te(t, s), Te(t, n)];
}
function pc(t, e, o, r) {
  let n = [], s = Ei(t, e, o), i = 1 / r;
  for (let l = i; l < 1; l += i) n.push(Xn(s, t, or * 3 * l));
  return n;
}
function yc(t, e, o) {
  return [Te(t, Ge(e, o)), Te(t, Ge(e, o * 0.99)), Ve(t, Ge(e, o * 0.99)), Ve(t, Ge(e, o))];
}
function Ls(t, e, o) {
  return t === !1 || t === void 0 ? 0 : t === !0 ? Math.max(e, o) : t;
}
function mc(t, e, o) {
  return t.slice(0, 10).reduce((r, n) => {
    let s = n.pressure;
    return e && (s = Ai(r, n.distance, o)), (r + s) / 2;
  }, t[0].pressure);
}
function gc(t, e = {}) {
  let { size: o = 16, smoothing: r = 0.5, thinning: n = 0.5, simulatePressure: s = !0, easing: i = (O) => O, start: l = {}, end: a = {}, last: c = !1 } = e, { cap: h = !0, easing: f = (O) => O * (2 - O) } = l, { cap: d = !0, easing: p = (O) => --O * O * O + 1 } = a;
  if (t.length === 0 || o <= 0) return [];
  let y = t[t.length - 1].runningLength, m = Ls(l.taper, o, y), g = Ls(a.taper, o, y), b = (o * r) ** 2, x = [], C = [], z = mc(t, s, o), S = zs(o, n, t[t.length - 1].pressure, i), F, D = t[0].vector, W = t[0].point, K = W, Z = W, dt = K, X = !1;
  for (let O = 0; O < t.length; O++) {
    let { pressure: _ } = t[O], { point: G, vector: V, distance: Y, runningLength: $ } = t[O], nt = O === t.length - 1;
    if (!nt && y - $ < 3) continue;
    n ? (s && (_ = Ai(z, Y, o)), S = zs(o, n, _, i)) : S = o / 2, F === void 0 && (F = S);
    let ot = $ < m ? f($ / m) : 1, yt = y - $ < g ? p((y - $) / g) : 1;
    S = Math.max(0.01, S * Math.min(ot, yt));
    let ft = (nt ? t[O] : t[O + 1]).vector, et = nt ? 1 : Ts(V, ft), Pt = Ts(V, D) < 0 && !X, zt = et !== null && et < 0;
    if (Pt || zt) {
      en(ie, D), tn(ie, ie, S);
      for (let gt = 0; gt <= 1; gt += 0.07692307692307693) vn(Oe, G, ie), Rs(Oe, Oe, G, or * gt), Z = [Oe[0], Oe[1]], x.push(Z), Is(Xe, G, ie), Rs(Xe, Xe, G, or * -gt), dt = [Xe[0], Xe[1]], C.push(dt);
      W = Z, K = dt, zt && (X = !0);
      continue;
    }
    if (X = !1, nt) {
      en(ie, V), tn(ie, ie, S), x.push(Ve(G, ie)), C.push(Te(G, ie));
      continue;
    }
    dc(ie, ft, V, et), en(ie, ie), tn(ie, ie, S), vn(Oe, G, ie), Z = [Oe[0], Oe[1]], (O <= 1 || Ps(W, Z) > b) && (x.push(Z), W = Z), Is(Xe, G, ie), dt = [Xe[0], Xe[1]], (O <= 1 || Ps(K, dt) > b) && (C.push(dt), K = dt), z = _, D = V;
  }
  let rt = [t[0].point[0], t[0].point[1]], J = t.length > 1 ? [t[t.length - 1].point[0], t[t.length - 1].point[1]] : Te(t[0].point, [1, 1]), U = [], R = [];
  if (t.length === 1) {
    if (!(m || g) || c) return hc(rt, F || S);
  } else {
    m || g && t.length === 1 || (h ? U.push(...uc(rt, C[0], 13)) : U.push(...fc(rt, x[0], C[0])));
    let O = Li(sc(t[t.length - 1].vector));
    g || m && t.length === 1 ? R.push(J) : d ? R.push(...pc(J, O, S, 29)) : R.push(...yc(J, O, S));
  }
  return x.concat(R, C.reverse(), U);
}
const Ds = [0, 0];
function Es(t) {
  return t != null && t >= 0;
}
function bc(t, e = {}) {
  var d;
  let { streamline: o = 0.5, size: r = 16, last: n = !1 } = e;
  if (t.length === 0) return [];
  let s = 0.15 + (1 - o) * 0.85, i = Array.isArray(t[0]) ? t : t.map(({ x: p, y, pressure: m = Ms }) => [p, y, m]);
  if (i.length === 2) {
    let p = i[1];
    i = i.slice(0, -1);
    for (let y = 1; y < 5; y++) i.push(As(i[0], p, y / 4));
  }
  i.length === 1 && (i = [...i, [...Te(i[0], Cs), ...i[0].slice(2)]]);
  let l = [{ point: [i[0][0], i[0][1]], pressure: Es(i[0][2]) ? i[0][2] : 0.25, vector: [...Cs], distance: 0, runningLength: 0 }], a = !1, c = 0, h = l[0], f = i.length - 1;
  for (let p = 1; p < i.length; p++) {
    let y = n && p === f ? [i[p][0], i[p][1]] : As(h.point, i[p], s);
    if (ac(h.point, y)) continue;
    let m = cc(y, h.point);
    if (c += m, p < f && !a) {
      if (c < r) continue;
      a = !0;
    }
    vn(Ds, h.point, y), h = { point: y, pressure: Es(i[p][2]) ? i[p][2] : Ms, vector: Di(Ds), distance: m, runningLength: c }, l.push(h);
  }
  return l[0].vector = ((d = l[1]) == null ? void 0 : d.vector) || [0, 0], l;
}
function xc(t, e = {}) {
  return gc(bc(t, e), e);
}
var wc = xc;
function Yn(t, e = {}) {
  const o = wc(t, {
    size: e.size || 4,
    thinning: e.thinning ?? 0.5,
    smoothing: e.smoothing ?? 0.5,
    streamline: e.streamline ?? 0.5
  });
  return kc(o);
}
function kc(t) {
  if (!t.length) return "";
  const e = [], [o, r] = t[0];
  e.push("M", o, r);
  for (let n = 0; n < t.length; n++) {
    const [s, i] = t[n], [l, a] = t[(n + 1) % t.length];
    e.push("Q", s, i, (s + l) / 2, (i + a) / 2);
  }
  return e.push("Z"), e.join(" ");
}
function Wi(t, e = 0.5) {
  if (t.length < 2) return t;
  const o = 0.15 + (1 - e) * 0.85, r = [[t[0][0], t[0][1]]];
  for (let n = 1; n < t.length; n++) {
    const s = r[n - 1];
    r.push([
      s[0] + (t[n][0] - s[0]) * o,
      s[1] + (t[n][1] - s[1]) * o
    ]);
  }
  return r;
}
function vc(t, e = 0.5) {
  if (t.length < 2) return "";
  const o = Wi(t, e), r = o.length, n = [];
  n.push("M", o[0][0], o[0][1]);
  for (let s = 0; s < r; s++) {
    const [i, l] = o[s], [a, c] = o[(s + 1) % r];
    n.push("Q", i, l, (i + a) / 2, (l + c) / 2);
  }
  return n.push("Z"), n.join(" ");
}
function Sc(t, e, o, r) {
  const n = e[0] - t[0], s = e[1] - t[1], i = r[0] - o[0], l = r[1] - o[1], a = n * l - s * i;
  if (Math.abs(a) < 1e-10) return null;
  const c = ((o[0] - t[0]) * l - (o[1] - t[1]) * i) / a, h = ((o[0] - t[0]) * s - (o[1] - t[1]) * n) / a;
  return c <= 0 || c >= 1 || h <= 0 || h >= 1 ? null : [t[0] + c * n, t[1] + c * s];
}
function Mc(t) {
  if (t.length < 2) return "";
  let e = `M ${t[0][0]},${t[0][1]}`;
  for (let o = 1; o < t.length; o++)
    e += ` L ${t[o][0]},${t[o][1]}`;
  return e + " Z";
}
function Ws(t) {
  let e = 0;
  for (let o = 0, r = t.length - 1; o < t.length; r = o++)
    e += (t[r][0] + t[o][0]) * (t[r][1] - t[o][1]);
  return Math.abs(e) / 2;
}
function Cc(t) {
  if (t.length < 4) return [];
  const e = t.length, o = [];
  for (let i = 0; i < e - 1; i++)
    for (let l = i + 2; l < e - 1; l++) {
      const a = Sc(
        t[i],
        t[i + 1],
        t[l],
        t[l + 1]
      );
      if (!a) continue;
      const c = [a];
      for (let h = i + 1; h <= l; h++)
        c.push(t[h]);
      Ws(c) < 100 || o.push({
        pathD: Mc(c),
        points: c.map((h) => [h[0], h[1]])
      });
    }
  if (o.length === 0) return [];
  const r = o.map((i) => Ws(i.points)), s = Math.max(...r) * 0.05;
  return o.filter((i, l) => r[l] >= s);
}
function on(t, e, o) {
  if (t && t.length) {
    const [r, n] = e, s = Math.PI / 180 * o, i = Math.cos(s), l = Math.sin(s);
    for (const a of t) {
      const [c, h] = a;
      a[0] = (c - r) * i - (h - n) * l + r, a[1] = (c - r) * l + (h - n) * i + n;
    }
  }
}
function zc(t, e) {
  return t[0] === e[0] && t[1] === e[1];
}
function Ic(t, e, o, r = 1) {
  const n = o, s = Math.max(e, 0.1), i = t[0] && t[0][0] && typeof t[0][0] == "number" ? [t] : t, l = [0, 0];
  if (n) for (const c of i) on(c, l, n);
  const a = function(c, h, f) {
    const d = [];
    for (const x of c) {
      const C = [...x];
      zc(C[0], C[C.length - 1]) || C.push([C[0][0], C[0][1]]), C.length > 2 && d.push(C);
    }
    const p = [];
    h = Math.max(h, 0.1);
    const y = [];
    for (const x of d) for (let C = 0; C < x.length - 1; C++) {
      const z = x[C], S = x[C + 1];
      if (z[1] !== S[1]) {
        const F = Math.min(z[1], S[1]);
        y.push({ ymin: F, ymax: Math.max(z[1], S[1]), x: F === z[1] ? z[0] : S[0], islope: (S[0] - z[0]) / (S[1] - z[1]) });
      }
    }
    if (y.sort((x, C) => x.ymin < C.ymin ? -1 : x.ymin > C.ymin ? 1 : x.x < C.x ? -1 : x.x > C.x ? 1 : x.ymax === C.ymax ? 0 : (x.ymax - C.ymax) / Math.abs(x.ymax - C.ymax)), !y.length) return p;
    let m = [], g = y[0].ymin, b = 0;
    for (; m.length || y.length; ) {
      if (y.length) {
        let x = -1;
        for (let C = 0; C < y.length && !(y[C].ymin > g); C++) x = C;
        y.splice(0, x + 1).forEach((C) => {
          m.push({ s: g, edge: C });
        });
      }
      if (m = m.filter((x) => !(x.edge.ymax <= g)), m.sort((x, C) => x.edge.x === C.edge.x ? 0 : (x.edge.x - C.edge.x) / Math.abs(x.edge.x - C.edge.x)), (f !== 1 || b % h == 0) && m.length > 1) for (let x = 0; x < m.length; x += 2) {
        const C = x + 1;
        if (C >= m.length) break;
        const z = m[x].edge, S = m[C].edge;
        p.push([[Math.round(z.x), g], [Math.round(S.x), g]]);
      }
      g += f, m.forEach((x) => {
        x.edge.x = x.edge.x + f * x.edge.islope;
      }), b++;
    }
    return p;
  }(i, s, r);
  if (n) {
    for (const c of i) on(c, l, -n);
    (function(c, h, f) {
      const d = [];
      c.forEach((p) => d.push(...p)), on(d, h, f);
    })(a, l, -n);
  }
  return a;
}
function rr(t, e) {
  var o;
  const r = e.hachureAngle + 90;
  let n = e.hachureGap;
  n < 0 && (n = 4 * e.strokeWidth), n = Math.round(Math.max(n, 0.1));
  let s = 1;
  return e.roughness >= 1 && (((o = e.randomizer) === null || o === void 0 ? void 0 : o.next()) || Math.random()) > 0.7 && (s = n), Ic(t, n, r, s || 1);
}
class Gn {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    return this._fillPolygons(e, o);
  }
  _fillPolygons(e, o) {
    const r = rr(e, o);
    return { type: "fillSketch", ops: this.renderLines(r, o) };
  }
  renderLines(e, o) {
    const r = [];
    for (const n of e) r.push(...this.helper.doubleLineOps(n[0][0], n[0][1], n[1][0], n[1][1], o));
    return r;
  }
}
function Br(t) {
  const e = t[0], o = t[1];
  return Math.sqrt(Math.pow(e[0] - o[0], 2) + Math.pow(e[1] - o[1], 2));
}
class Tc extends Gn {
  fillPolygons(e, o) {
    let r = o.hachureGap;
    r < 0 && (r = 4 * o.strokeWidth), r = Math.max(r, 0.1);
    const n = rr(e, Object.assign({}, o, { hachureGap: r })), s = Math.PI / 180 * o.hachureAngle, i = [], l = 0.5 * r * Math.cos(s), a = 0.5 * r * Math.sin(s);
    for (const [c, h] of n) Br([c, h]) && i.push([[c[0] - l, c[1] + a], [...h]], [[c[0] + l, c[1] - a], [...h]]);
    return { type: "fillSketch", ops: this.renderLines(i, o) };
  }
}
class Pc extends Gn {
  fillPolygons(e, o) {
    const r = this._fillPolygons(e, o), n = Object.assign({}, o, { hachureAngle: o.hachureAngle + 90 }), s = this._fillPolygons(e, n);
    return r.ops = r.ops.concat(s.ops), r;
  }
}
class Rc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const r = rr(e, o = Object.assign({}, o, { hachureAngle: 0 }));
    return this.dotsOnLines(r, o);
  }
  dotsOnLines(e, o) {
    const r = [];
    let n = o.hachureGap;
    n < 0 && (n = 4 * o.strokeWidth), n = Math.max(n, 0.1);
    let s = o.fillWeight;
    s < 0 && (s = o.strokeWidth / 2);
    const i = n / 4;
    for (const l of e) {
      const a = Br(l), c = a / n, h = Math.ceil(c) - 1, f = a - h * n, d = (l[0][0] + l[1][0]) / 2 - n / 4, p = Math.min(l[0][1], l[1][1]);
      for (let y = 0; y < h; y++) {
        const m = p + f + y * n, g = d - i + 2 * Math.random() * i, b = m - i + 2 * Math.random() * i, x = this.helper.ellipse(g, b, s, s, o);
        r.push(...x.ops);
      }
    }
    return { type: "fillSketch", ops: r };
  }
}
class Ac {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const r = rr(e, o);
    return { type: "fillSketch", ops: this.dashedLine(r, o) };
  }
  dashedLine(e, o) {
    const r = o.dashOffset < 0 ? o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap : o.dashOffset, n = o.dashGap < 0 ? o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap : o.dashGap, s = [];
    return e.forEach((i) => {
      const l = Br(i), a = Math.floor(l / (r + n)), c = (l + n - a * (r + n)) / 2;
      let h = i[0], f = i[1];
      h[0] > f[0] && (h = i[1], f = i[0]);
      const d = Math.atan((f[1] - h[1]) / (f[0] - h[0]));
      for (let p = 0; p < a; p++) {
        const y = p * (r + n), m = y + r, g = [h[0] + y * Math.cos(d) + c * Math.cos(d), h[1] + y * Math.sin(d) + c * Math.sin(d)], b = [h[0] + m * Math.cos(d) + c * Math.cos(d), h[1] + m * Math.sin(d) + c * Math.sin(d)];
        s.push(...this.helper.doubleLineOps(g[0], g[1], b[0], b[1], o));
      }
    }), s;
  }
}
class Lc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const r = o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap, n = o.zigzagOffset < 0 ? r : o.zigzagOffset, s = rr(e, o = Object.assign({}, o, { hachureGap: r + n }));
    return { type: "fillSketch", ops: this.zigzagLines(s, n, o) };
  }
  zigzagLines(e, o, r) {
    const n = [];
    return e.forEach((s) => {
      const i = Br(s), l = Math.round(i / (2 * o));
      let a = s[0], c = s[1];
      a[0] > c[0] && (a = s[1], c = s[0]);
      const h = Math.atan((c[1] - a[1]) / (c[0] - a[0]));
      for (let f = 0; f < l; f++) {
        const d = 2 * f * o, p = 2 * (f + 1) * o, y = Math.sqrt(2 * Math.pow(o, 2)), m = [a[0] + d * Math.cos(h), a[1] + d * Math.sin(h)], g = [a[0] + p * Math.cos(h), a[1] + p * Math.sin(h)], b = [m[0] + y * Math.cos(h + Math.PI / 4), m[1] + y * Math.sin(h + Math.PI / 4)];
        n.push(...this.helper.doubleLineOps(m[0], m[1], b[0], b[1], r), ...this.helper.doubleLineOps(b[0], b[1], g[0], g[1], r));
      }
    }), n;
  }
}
const de = {};
class Dc {
  constructor(e) {
    this.seed = e;
  }
  next() {
    return this.seed ? (2 ** 31 - 1 & (this.seed = Math.imul(48271, this.seed))) / 2 ** 31 : Math.random();
  }
}
const Ec = 0, rn = 1, Fs = 2, yr = { A: 7, a: 7, C: 6, c: 6, H: 1, h: 1, L: 2, l: 2, M: 2, m: 2, Q: 4, q: 4, S: 4, s: 4, T: 2, t: 2, V: 1, v: 1, Z: 0, z: 0 };
function nn(t, e) {
  return t.type === e;
}
function jn(t) {
  const e = [], o = function(i) {
    const l = new Array();
    for (; i !== ""; ) if (i.match(/^([ \t\r\n,]+)/)) i = i.substr(RegExp.$1.length);
    else if (i.match(/^([aAcChHlLmMqQsStTvVzZ])/)) l[l.length] = { type: Ec, text: RegExp.$1 }, i = i.substr(RegExp.$1.length);
    else {
      if (!i.match(/^(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)/)) return [];
      l[l.length] = { type: rn, text: `${parseFloat(RegExp.$1)}` }, i = i.substr(RegExp.$1.length);
    }
    return l[l.length] = { type: Fs, text: "" }, l;
  }(t);
  let r = "BOD", n = 0, s = o[n];
  for (; !nn(s, Fs); ) {
    let i = 0;
    const l = [];
    if (r === "BOD") {
      if (s.text !== "M" && s.text !== "m") return jn("M0,0" + t);
      n++, i = yr[s.text], r = s.text;
    } else nn(s, rn) ? i = yr[r] : (n++, i = yr[s.text], r = s.text);
    if (!(n + i < o.length)) throw new Error("Path data ended short");
    for (let a = n; a < n + i; a++) {
      const c = o[a];
      if (!nn(c, rn)) throw new Error("Param not a number: " + r + "," + c.text);
      l[l.length] = +c.text;
    }
    if (typeof yr[r] != "number") throw new Error("Bad segment: " + r);
    {
      const a = { key: r, data: l };
      e.push(a), n += i, s = o[n], r === "M" && (r = "L"), r === "m" && (r = "l");
    }
  }
  return e;
}
function Fi(t) {
  let e = 0, o = 0, r = 0, n = 0;
  const s = [];
  for (const { key: i, data: l } of t) switch (i) {
    case "M":
      s.push({ key: "M", data: [...l] }), [e, o] = l, [r, n] = l;
      break;
    case "m":
      e += l[0], o += l[1], s.push({ key: "M", data: [e, o] }), r = e, n = o;
      break;
    case "L":
      s.push({ key: "L", data: [...l] }), [e, o] = l;
      break;
    case "l":
      e += l[0], o += l[1], s.push({ key: "L", data: [e, o] });
      break;
    case "C":
      s.push({ key: "C", data: [...l] }), e = l[4], o = l[5];
      break;
    case "c": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "C", data: a }), e = a[4], o = a[5];
      break;
    }
    case "Q":
      s.push({ key: "Q", data: [...l] }), e = l[2], o = l[3];
      break;
    case "q": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "Q", data: a }), e = a[2], o = a[3];
      break;
    }
    case "A":
      s.push({ key: "A", data: [...l] }), e = l[5], o = l[6];
      break;
    case "a":
      e += l[5], o += l[6], s.push({ key: "A", data: [l[0], l[1], l[2], l[3], l[4], e, o] });
      break;
    case "H":
      s.push({ key: "H", data: [...l] }), e = l[0];
      break;
    case "h":
      e += l[0], s.push({ key: "H", data: [e] });
      break;
    case "V":
      s.push({ key: "V", data: [...l] }), o = l[0];
      break;
    case "v":
      o += l[0], s.push({ key: "V", data: [o] });
      break;
    case "S":
      s.push({ key: "S", data: [...l] }), e = l[2], o = l[3];
      break;
    case "s": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "S", data: a }), e = a[2], o = a[3];
      break;
    }
    case "T":
      s.push({ key: "T", data: [...l] }), e = l[0], o = l[1];
      break;
    case "t":
      e += l[0], o += l[1], s.push({ key: "T", data: [e, o] });
      break;
    case "Z":
    case "z":
      s.push({ key: "Z", data: [] }), e = r, o = n;
  }
  return s;
}
function Bi(t) {
  const e = [];
  let o = "", r = 0, n = 0, s = 0, i = 0, l = 0, a = 0;
  for (const { key: c, data: h } of t) {
    switch (c) {
      case "M":
        e.push({ key: "M", data: [...h] }), [r, n] = h, [s, i] = h;
        break;
      case "C":
        e.push({ key: "C", data: [...h] }), r = h[4], n = h[5], l = h[2], a = h[3];
        break;
      case "L":
        e.push({ key: "L", data: [...h] }), [r, n] = h;
        break;
      case "H":
        r = h[0], e.push({ key: "L", data: [r, n] });
        break;
      case "V":
        n = h[0], e.push({ key: "L", data: [r, n] });
        break;
      case "S": {
        let f = 0, d = 0;
        o === "C" || o === "S" ? (f = r + (r - l), d = n + (n - a)) : (f = r, d = n), e.push({ key: "C", data: [f, d, ...h] }), l = h[0], a = h[1], r = h[2], n = h[3];
        break;
      }
      case "T": {
        const [f, d] = h;
        let p = 0, y = 0;
        o === "Q" || o === "T" ? (p = r + (r - l), y = n + (n - a)) : (p = r, y = n);
        const m = r + 2 * (p - r) / 3, g = n + 2 * (y - n) / 3, b = f + 2 * (p - f) / 3, x = d + 2 * (y - d) / 3;
        e.push({ key: "C", data: [m, g, b, x, f, d] }), l = p, a = y, r = f, n = d;
        break;
      }
      case "Q": {
        const [f, d, p, y] = h, m = r + 2 * (f - r) / 3, g = n + 2 * (d - n) / 3, b = p + 2 * (f - p) / 3, x = y + 2 * (d - y) / 3;
        e.push({ key: "C", data: [m, g, b, x, p, y] }), l = f, a = d, r = p, n = y;
        break;
      }
      case "A": {
        const f = Math.abs(h[0]), d = Math.abs(h[1]), p = h[2], y = h[3], m = h[4], g = h[5], b = h[6];
        f === 0 || d === 0 ? (e.push({ key: "C", data: [r, n, g, b, g, b] }), r = g, n = b) : (r !== g || n !== b) && (Ni(r, n, g, b, f, d, p, y, m).forEach(function(x) {
          e.push({ key: "C", data: x });
        }), r = g, n = b);
        break;
      }
      case "Z":
        e.push({ key: "Z", data: [] }), r = s, n = i;
    }
    o = c;
  }
  return e;
}
function Zo(t, e, o) {
  return [t * Math.cos(o) - e * Math.sin(o), t * Math.sin(o) + e * Math.cos(o)];
}
function Ni(t, e, o, r, n, s, i, l, a, c) {
  const h = (f = i, Math.PI * f / 180);
  var f;
  let d = [], p = 0, y = 0, m = 0, g = 0;
  if (c) [p, y, m, g] = c;
  else {
    [t, e] = Zo(t, e, -h), [o, r] = Zo(o, r, -h);
    const rt = (t - o) / 2, J = (e - r) / 2;
    let U = rt * rt / (n * n) + J * J / (s * s);
    U > 1 && (U = Math.sqrt(U), n *= U, s *= U);
    const R = n * n, O = s * s, _ = R * O - R * J * J - O * rt * rt, G = R * J * J + O * rt * rt, V = (l === a ? -1 : 1) * Math.sqrt(Math.abs(_ / G));
    m = V * n * J / s + (t + o) / 2, g = V * -s * rt / n + (e + r) / 2, p = Math.asin(parseFloat(((e - g) / s).toFixed(9))), y = Math.asin(parseFloat(((r - g) / s).toFixed(9))), t < m && (p = Math.PI - p), o < m && (y = Math.PI - y), p < 0 && (p = 2 * Math.PI + p), y < 0 && (y = 2 * Math.PI + y), a && p > y && (p -= 2 * Math.PI), !a && y > p && (y -= 2 * Math.PI);
  }
  let b = y - p;
  if (Math.abs(b) > 120 * Math.PI / 180) {
    const rt = y, J = o, U = r;
    y = a && y > p ? p + 120 * Math.PI / 180 * 1 : p + 120 * Math.PI / 180 * -1, d = Ni(o = m + n * Math.cos(y), r = g + s * Math.sin(y), J, U, n, s, i, 0, a, [y, rt, m, g]);
  }
  b = y - p;
  const x = Math.cos(p), C = Math.sin(p), z = Math.cos(y), S = Math.sin(y), F = Math.tan(b / 4), D = 4 / 3 * n * F, W = 4 / 3 * s * F, K = [t, e], Z = [t + D * C, e - W * x], dt = [o + D * S, r - W * z], X = [o, r];
  if (Z[0] = 2 * K[0] - Z[0], Z[1] = 2 * K[1] - Z[1], c) return [Z, dt, X].concat(d);
  {
    d = [Z, dt, X].concat(d);
    const rt = [];
    for (let J = 0; J < d.length; J += 3) {
      const U = Zo(d[J][0], d[J][1], h), R = Zo(d[J + 1][0], d[J + 1][1], h), O = Zo(d[J + 2][0], d[J + 2][1], h);
      rt.push([U[0], U[1], R[0], R[1], O[0], O[1]]);
    }
    return rt;
  }
}
const Wc = { randOffset: function(t, e) {
  return At(t, e);
}, randOffsetWithRange: function(t, e, o) {
  return Ir(t, e, o);
}, ellipse: function(t, e, o, r, n) {
  const s = Oi(o, r, n);
  return Sn(t, e, n, s).opset;
}, doubleLineOps: function(t, e, o, r, n) {
  return Ze(t, e, o, r, n, !0);
} };
function Hi(t, e, o, r, n) {
  return { type: "path", ops: Ze(t, e, o, r, n) };
}
function Sr(t, e, o) {
  const r = (t || []).length;
  if (r > 2) {
    const n = [];
    for (let s = 0; s < r - 1; s++) n.push(...Ze(t[s][0], t[s][1], t[s + 1][0], t[s + 1][1], o));
    return e && n.push(...Ze(t[r - 1][0], t[r - 1][1], t[0][0], t[0][1], o)), { type: "path", ops: n };
  }
  return r === 2 ? Hi(t[0][0], t[0][1], t[1][0], t[1][1], o) : { type: "path", ops: [] };
}
function Fc(t, e, o, r, n) {
  return function(s, i) {
    return Sr(s, !0, i);
  }([[t, e], [t + o, e], [t + o, e + r], [t, e + r]], n);
}
function Bs(t, e) {
  if (t.length) {
    const o = typeof t[0][0] == "number" ? [t] : t, r = mr(o[0], 1 * (1 + 0.2 * e.roughness), e), n = e.disableMultiStroke ? [] : mr(o[0], 1.5 * (1 + 0.22 * e.roughness), Os(e));
    for (let s = 1; s < o.length; s++) {
      const i = o[s];
      if (i.length) {
        const l = mr(i, 1 * (1 + 0.2 * e.roughness), e), a = e.disableMultiStroke ? [] : mr(i, 1.5 * (1 + 0.22 * e.roughness), Os(e));
        for (const c of l) c.op !== "move" && r.push(c);
        for (const c of a) c.op !== "move" && n.push(c);
      }
    }
    return { type: "path", ops: r.concat(n) };
  }
  return { type: "path", ops: [] };
}
function Oi(t, e, o) {
  const r = Math.sqrt(2 * Math.PI * Math.sqrt((Math.pow(t / 2, 2) + Math.pow(e / 2, 2)) / 2)), n = Math.ceil(Math.max(o.curveStepCount, o.curveStepCount / Math.sqrt(200) * r)), s = 2 * Math.PI / n;
  let i = Math.abs(t / 2), l = Math.abs(e / 2);
  const a = 1 - o.curveFitting;
  return i += At(i * a, o), l += At(l * a, o), { increment: s, rx: i, ry: l };
}
function Sn(t, e, o, r) {
  const [n, s] = Xs(r.increment, t, e, r.rx, r.ry, 1, r.increment * Ir(0.1, Ir(0.4, 1, o), o), o);
  let i = Tr(n, null, o);
  if (!o.disableMultiStroke && o.roughness !== 0) {
    const [l] = Xs(r.increment, t, e, r.rx, r.ry, 1.5, 0, o), a = Tr(l, null, o);
    i = i.concat(a);
  }
  return { estimatedPoints: s, opset: { type: "path", ops: i } };
}
function Ns(t, e, o, r, n, s, i, l, a) {
  const c = t, h = e;
  let f = Math.abs(o / 2), d = Math.abs(r / 2);
  f += At(0.01 * f, a), d += At(0.01 * d, a);
  let p = n, y = s;
  for (; p < 0; ) p += 2 * Math.PI, y += 2 * Math.PI;
  y - p > 2 * Math.PI && (p = 0, y = 2 * Math.PI);
  const m = 2 * Math.PI / a.curveStepCount, g = Math.min(m / 2, (y - p) / 2), b = Ys(g, c, h, f, d, p, y, 1, a);
  if (!a.disableMultiStroke) {
    const x = Ys(g, c, h, f, d, p, y, 1.5, a);
    b.push(...x);
  }
  return i && (l ? b.push(...Ze(c, h, c + f * Math.cos(p), h + d * Math.sin(p), a), ...Ze(c, h, c + f * Math.cos(y), h + d * Math.sin(y), a)) : b.push({ op: "lineTo", data: [c, h] }, { op: "lineTo", data: [c + f * Math.cos(p), h + d * Math.sin(p)] })), { type: "path", ops: b };
}
function Hs(t, e) {
  const o = Bi(Fi(jn(t))), r = [];
  let n = [0, 0], s = [0, 0];
  for (const { key: i, data: l } of o) switch (i) {
    case "M":
      s = [l[0], l[1]], n = [l[0], l[1]];
      break;
    case "L":
      r.push(...Ze(s[0], s[1], l[0], l[1], e)), s = [l[0], l[1]];
      break;
    case "C": {
      const [a, c, h, f, d, p] = l;
      r.push(...Bc(a, c, h, f, d, p, s, e)), s = [d, p];
      break;
    }
    case "Z":
      r.push(...Ze(s[0], s[1], n[0], n[1], e)), s = [n[0], n[1]];
  }
  return { type: "path", ops: r };
}
function sn(t, e) {
  const o = [];
  for (const r of t) if (r.length) {
    const n = e.maxRandomnessOffset || 0, s = r.length;
    if (s > 2) {
      o.push({ op: "move", data: [r[0][0] + At(n, e), r[0][1] + At(n, e)] });
      for (let i = 1; i < s; i++) o.push({ op: "lineTo", data: [r[i][0] + At(n, e), r[i][1] + At(n, e)] });
    }
  }
  return { type: "fillPath", ops: o };
}
function Mo(t, e) {
  return function(o, r) {
    let n = o.fillStyle || "hachure";
    if (!de[n]) switch (n) {
      case "zigzag":
        de[n] || (de[n] = new Tc(r));
        break;
      case "cross-hatch":
        de[n] || (de[n] = new Pc(r));
        break;
      case "dots":
        de[n] || (de[n] = new Rc(r));
        break;
      case "dashed":
        de[n] || (de[n] = new Ac(r));
        break;
      case "zigzag-line":
        de[n] || (de[n] = new Lc(r));
        break;
      default:
        n = "hachure", de[n] || (de[n] = new Gn(r));
    }
    return de[n];
  }(e, Wc).fillPolygons(t, e);
}
function Os(t) {
  const e = Object.assign({}, t);
  return e.randomizer = void 0, t.seed && (e.seed = t.seed + 1), e;
}
function Xi(t) {
  return t.randomizer || (t.randomizer = new Dc(t.seed || 0)), t.randomizer.next();
}
function Ir(t, e, o, r = 1) {
  return o.roughness * r * (Xi(o) * (e - t) + t);
}
function At(t, e, o = 1) {
  return Ir(-t, t, e, o);
}
function Ze(t, e, o, r, n, s = !1) {
  const i = s ? n.disableMultiStrokeFill : n.disableMultiStroke, l = Mn(t, e, o, r, n, !0, !1);
  if (i) return l;
  const a = Mn(t, e, o, r, n, !0, !0);
  return l.concat(a);
}
function Mn(t, e, o, r, n, s, i) {
  const l = Math.pow(t - o, 2) + Math.pow(e - r, 2), a = Math.sqrt(l);
  let c = 1;
  c = a < 200 ? 1 : a > 500 ? 0.4 : -16668e-7 * a + 1.233334;
  let h = n.maxRandomnessOffset || 0;
  h * h * 100 > l && (h = a / 10);
  const f = h / 2, d = 0.2 + 0.2 * Xi(n);
  let p = n.bowing * n.maxRandomnessOffset * (r - e) / 200, y = n.bowing * n.maxRandomnessOffset * (t - o) / 200;
  p = At(p, n, c), y = At(y, n, c);
  const m = [], g = () => At(f, n, c), b = () => At(h, n, c), x = n.preserveVertices;
  return i ? m.push({ op: "move", data: [t + (x ? 0 : g()), e + (x ? 0 : g())] }) : m.push({ op: "move", data: [t + (x ? 0 : At(h, n, c)), e + (x ? 0 : At(h, n, c))] }), i ? m.push({ op: "bcurveTo", data: [p + t + (o - t) * d + g(), y + e + (r - e) * d + g(), p + t + 2 * (o - t) * d + g(), y + e + 2 * (r - e) * d + g(), o + (x ? 0 : g()), r + (x ? 0 : g())] }) : m.push({ op: "bcurveTo", data: [p + t + (o - t) * d + b(), y + e + (r - e) * d + b(), p + t + 2 * (o - t) * d + b(), y + e + 2 * (r - e) * d + b(), o + (x ? 0 : b()), r + (x ? 0 : b())] }), m;
}
function mr(t, e, o) {
  if (!t.length) return [];
  const r = [];
  r.push([t[0][0] + At(e, o), t[0][1] + At(e, o)]), r.push([t[0][0] + At(e, o), t[0][1] + At(e, o)]);
  for (let n = 1; n < t.length; n++) r.push([t[n][0] + At(e, o), t[n][1] + At(e, o)]), n === t.length - 1 && r.push([t[n][0] + At(e, o), t[n][1] + At(e, o)]);
  return Tr(r, null, o);
}
function Tr(t, e, o) {
  const r = t.length, n = [];
  if (r > 3) {
    const s = [], i = 1 - o.curveTightness;
    n.push({ op: "move", data: [t[1][0], t[1][1]] });
    for (let l = 1; l + 2 < r; l++) {
      const a = t[l];
      s[0] = [a[0], a[1]], s[1] = [a[0] + (i * t[l + 1][0] - i * t[l - 1][0]) / 6, a[1] + (i * t[l + 1][1] - i * t[l - 1][1]) / 6], s[2] = [t[l + 1][0] + (i * t[l][0] - i * t[l + 2][0]) / 6, t[l + 1][1] + (i * t[l][1] - i * t[l + 2][1]) / 6], s[3] = [t[l + 1][0], t[l + 1][1]], n.push({ op: "bcurveTo", data: [s[1][0], s[1][1], s[2][0], s[2][1], s[3][0], s[3][1]] });
    }
  } else r === 3 ? (n.push({ op: "move", data: [t[1][0], t[1][1]] }), n.push({ op: "bcurveTo", data: [t[1][0], t[1][1], t[2][0], t[2][1], t[2][0], t[2][1]] })) : r === 2 && n.push(...Mn(t[0][0], t[0][1], t[1][0], t[1][1], o, !0, !0));
  return n;
}
function Xs(t, e, o, r, n, s, i, l) {
  const a = [], c = [];
  if (l.roughness === 0) {
    t /= 4, c.push([e + r * Math.cos(-t), o + n * Math.sin(-t)]);
    for (let h = 0; h <= 2 * Math.PI; h += t) {
      const f = [e + r * Math.cos(h), o + n * Math.sin(h)];
      a.push(f), c.push(f);
    }
    c.push([e + r * Math.cos(0), o + n * Math.sin(0)]), c.push([e + r * Math.cos(t), o + n * Math.sin(t)]);
  } else {
    const h = At(0.5, l) - Math.PI / 2;
    c.push([At(s, l) + e + 0.9 * r * Math.cos(h - t), At(s, l) + o + 0.9 * n * Math.sin(h - t)]);
    const f = 2 * Math.PI + h - 0.01;
    for (let d = h; d < f; d += t) {
      const p = [At(s, l) + e + r * Math.cos(d), At(s, l) + o + n * Math.sin(d)];
      a.push(p), c.push(p);
    }
    c.push([At(s, l) + e + r * Math.cos(h + 2 * Math.PI + 0.5 * i), At(s, l) + o + n * Math.sin(h + 2 * Math.PI + 0.5 * i)]), c.push([At(s, l) + e + 0.98 * r * Math.cos(h + i), At(s, l) + o + 0.98 * n * Math.sin(h + i)]), c.push([At(s, l) + e + 0.9 * r * Math.cos(h + 0.5 * i), At(s, l) + o + 0.9 * n * Math.sin(h + 0.5 * i)]);
  }
  return [c, a];
}
function Ys(t, e, o, r, n, s, i, l, a) {
  const c = s + At(0.1, a), h = [];
  h.push([At(l, a) + e + 0.9 * r * Math.cos(c - t), At(l, a) + o + 0.9 * n * Math.sin(c - t)]);
  for (let f = c; f <= i; f += t) h.push([At(l, a) + e + r * Math.cos(f), At(l, a) + o + n * Math.sin(f)]);
  return h.push([e + r * Math.cos(i), o + n * Math.sin(i)]), h.push([e + r * Math.cos(i), o + n * Math.sin(i)]), Tr(h, null, a);
}
function Bc(t, e, o, r, n, s, i, l) {
  const a = [], c = [l.maxRandomnessOffset || 1, (l.maxRandomnessOffset || 1) + 0.3];
  let h = [0, 0];
  const f = l.disableMultiStroke ? 1 : 2, d = l.preserveVertices;
  for (let p = 0; p < f; p++) p === 0 ? a.push({ op: "move", data: [i[0], i[1]] }) : a.push({ op: "move", data: [i[0] + (d ? 0 : At(c[0], l)), i[1] + (d ? 0 : At(c[0], l))] }), h = d ? [n, s] : [n + At(c[p], l), s + At(c[p], l)], a.push({ op: "bcurveTo", data: [t + At(c[p], l), e + At(c[p], l), o + At(c[p], l), r + At(c[p], l), h[0], h[1]] });
  return a;
}
function Uo(t) {
  return [...t];
}
function Gs(t, e = 0) {
  const o = t.length;
  if (o < 3) throw new Error("A curve must have at least three points.");
  const r = [];
  if (o === 3) r.push(Uo(t[0]), Uo(t[1]), Uo(t[2]), Uo(t[2]));
  else {
    const n = [];
    n.push(t[0], t[0]);
    for (let l = 1; l < t.length; l++) n.push(t[l]), l === t.length - 1 && n.push(t[l]);
    const s = [], i = 1 - e;
    r.push(Uo(n[0]));
    for (let l = 1; l + 2 < n.length; l++) {
      const a = n[l];
      s[0] = [a[0], a[1]], s[1] = [a[0] + (i * n[l + 1][0] - i * n[l - 1][0]) / 6, a[1] + (i * n[l + 1][1] - i * n[l - 1][1]) / 6], s[2] = [n[l + 1][0] + (i * n[l][0] - i * n[l + 2][0]) / 6, n[l + 1][1] + (i * n[l][1] - i * n[l + 2][1]) / 6], s[3] = [n[l + 1][0], n[l + 1][1]], r.push(s[1], s[2], s[3]);
    }
  }
  return r;
}
function Mr(t, e) {
  return Math.pow(t[0] - e[0], 2) + Math.pow(t[1] - e[1], 2);
}
function Nc(t, e, o) {
  const r = Mr(e, o);
  if (r === 0) return Mr(t, e);
  let n = ((t[0] - e[0]) * (o[0] - e[0]) + (t[1] - e[1]) * (o[1] - e[1])) / r;
  return n = Math.max(0, Math.min(1, n)), Mr(t, co(e, o, n));
}
function co(t, e, o) {
  return [t[0] + (e[0] - t[0]) * o, t[1] + (e[1] - t[1]) * o];
}
function Cn(t, e, o, r) {
  const n = r || [];
  if (function(l, a) {
    const c = l[a + 0], h = l[a + 1], f = l[a + 2], d = l[a + 3];
    let p = 3 * h[0] - 2 * c[0] - d[0];
    p *= p;
    let y = 3 * h[1] - 2 * c[1] - d[1];
    y *= y;
    let m = 3 * f[0] - 2 * d[0] - c[0];
    m *= m;
    let g = 3 * f[1] - 2 * d[1] - c[1];
    return g *= g, p < m && (p = m), y < g && (y = g), p + y;
  }(t, e) < o) {
    const l = t[e + 0];
    n.length ? (s = n[n.length - 1], i = l, Math.sqrt(Mr(s, i)) > 1 && n.push(l)) : n.push(l), n.push(t[e + 3]);
  } else {
    const a = t[e + 0], c = t[e + 1], h = t[e + 2], f = t[e + 3], d = co(a, c, 0.5), p = co(c, h, 0.5), y = co(h, f, 0.5), m = co(d, p, 0.5), g = co(p, y, 0.5), b = co(m, g, 0.5);
    Cn([a, d, m, b], 0, o, n), Cn([b, g, y, f], 0, o, n);
  }
  var s, i;
  return n;
}
function Hc(t, e) {
  return Pr(t, 0, t.length, e);
}
function Pr(t, e, o, r, n) {
  const s = n || [], i = t[e], l = t[o - 1];
  let a = 0, c = 1;
  for (let h = e + 1; h < o - 1; ++h) {
    const f = Nc(t[h], i, l);
    f > a && (a = f, c = h);
  }
  return Math.sqrt(a) > r ? (Pr(t, e, c + 1, r, s), Pr(t, c, o, r, s)) : (s.length || s.push(i), s.push(l)), s;
}
function an(t, e = 0.15, o) {
  const r = [], n = (t.length - 1) / 3;
  for (let s = 0; s < n; s++)
    Cn(t, 3 * s, e, r);
  return o && o > 0 ? Pr(r, 0, r.length, o) : r;
}
const pe = "none";
class Rr {
  constructor(e) {
    this.defaultOptions = { maxRandomnessOffset: 2, roughness: 1, bowing: 1, stroke: "#000", strokeWidth: 1, curveTightness: 0, curveFitting: 0.95, curveStepCount: 9, fillStyle: "hachure", fillWeight: -1, hachureAngle: -41, hachureGap: -1, dashOffset: -1, dashGap: -1, zigzagOffset: -1, seed: 0, disableMultiStroke: !1, disableMultiStrokeFill: !1, preserveVertices: !1, fillShapeRoughnessGain: 0.8 }, this.config = e || {}, this.config.options && (this.defaultOptions = this._o(this.config.options));
  }
  static newSeed() {
    return Math.floor(Math.random() * 2 ** 31);
  }
  _o(e) {
    return e ? Object.assign({}, this.defaultOptions, e) : this.defaultOptions;
  }
  _d(e, o, r) {
    return { shape: e, sets: o || [], options: r || this.defaultOptions };
  }
  line(e, o, r, n, s) {
    const i = this._o(s);
    return this._d("line", [Hi(e, o, r, n, i)], i);
  }
  rectangle(e, o, r, n, s) {
    const i = this._o(s), l = [], a = Fc(e, o, r, n, i);
    if (i.fill) {
      const c = [[e, o], [e + r, o], [e + r, o + n], [e, o + n]];
      i.fillStyle === "solid" ? l.push(sn([c], i)) : l.push(Mo([c], i));
    }
    return i.stroke !== pe && l.push(a), this._d("rectangle", l, i);
  }
  ellipse(e, o, r, n, s) {
    const i = this._o(s), l = [], a = Oi(r, n, i), c = Sn(e, o, i, a);
    if (i.fill) if (i.fillStyle === "solid") {
      const h = Sn(e, o, i, a).opset;
      h.type = "fillPath", l.push(h);
    } else l.push(Mo([c.estimatedPoints], i));
    return i.stroke !== pe && l.push(c.opset), this._d("ellipse", l, i);
  }
  circle(e, o, r, n) {
    const s = this.ellipse(e, o, r, r, n);
    return s.shape = "circle", s;
  }
  linearPath(e, o) {
    const r = this._o(o);
    return this._d("linearPath", [Sr(e, !1, r)], r);
  }
  arc(e, o, r, n, s, i, l = !1, a) {
    const c = this._o(a), h = [], f = Ns(e, o, r, n, s, i, l, !0, c);
    if (l && c.fill) if (c.fillStyle === "solid") {
      const d = Object.assign({}, c);
      d.disableMultiStroke = !0;
      const p = Ns(e, o, r, n, s, i, !0, !1, d);
      p.type = "fillPath", h.push(p);
    } else h.push(function(d, p, y, m, g, b, x) {
      const C = d, z = p;
      let S = Math.abs(y / 2), F = Math.abs(m / 2);
      S += At(0.01 * S, x), F += At(0.01 * F, x);
      let D = g, W = b;
      for (; D < 0; ) D += 2 * Math.PI, W += 2 * Math.PI;
      W - D > 2 * Math.PI && (D = 0, W = 2 * Math.PI);
      const K = (W - D) / x.curveStepCount, Z = [];
      for (let dt = D; dt <= W; dt += K) Z.push([C + S * Math.cos(dt), z + F * Math.sin(dt)]);
      return Z.push([C + S * Math.cos(W), z + F * Math.sin(W)]), Z.push([C, z]), Mo([Z], x);
    }(e, o, r, n, s, i, c));
    return c.stroke !== pe && h.push(f), this._d("arc", h, c);
  }
  curve(e, o) {
    const r = this._o(o), n = [], s = Bs(e, r);
    if (r.fill && r.fill !== pe) if (r.fillStyle === "solid") {
      const i = Bs(e, Object.assign(Object.assign({}, r), { disableMultiStroke: !0, roughness: r.roughness ? r.roughness + r.fillShapeRoughnessGain : 0 }));
      n.push({ type: "fillPath", ops: this._mergedShape(i.ops) });
    } else {
      const i = [], l = e;
      if (l.length) {
        const a = typeof l[0][0] == "number" ? [l] : l;
        for (const c of a) c.length < 3 ? i.push(...c) : c.length === 3 ? i.push(...an(Gs([c[0], c[0], c[1], c[2]]), 10, (1 + r.roughness) / 2)) : i.push(...an(Gs(c), 10, (1 + r.roughness) / 2));
      }
      i.length && n.push(Mo([i], r));
    }
    return r.stroke !== pe && n.push(s), this._d("curve", n, r);
  }
  polygon(e, o) {
    const r = this._o(o), n = [], s = Sr(e, !0, r);
    return r.fill && (r.fillStyle === "solid" ? n.push(sn([e], r)) : n.push(Mo([e], r))), r.stroke !== pe && n.push(s), this._d("polygon", n, r);
  }
  path(e, o) {
    const r = this._o(o), n = [];
    if (!e) return this._d("path", n, r);
    e = (e || "").replace(/\n/g, " ").replace(/(-\s)/g, "-").replace("/(ss)/g", " ");
    const s = r.fill && r.fill !== "transparent" && r.fill !== pe, i = r.stroke !== pe, l = !!(r.simplification && r.simplification < 1), a = function(h, f, d) {
      const p = Bi(Fi(jn(h))), y = [];
      let m = [], g = [0, 0], b = [];
      const x = () => {
        b.length >= 4 && m.push(...an(b, f)), b = [];
      }, C = () => {
        x(), m.length && (y.push(m), m = []);
      };
      for (const { key: S, data: F } of p) switch (S) {
        case "M":
          C(), g = [F[0], F[1]], m.push(g);
          break;
        case "L":
          x(), m.push([F[0], F[1]]);
          break;
        case "C":
          if (!b.length) {
            const D = m.length ? m[m.length - 1] : g;
            b.push([D[0], D[1]]);
          }
          b.push([F[0], F[1]]), b.push([F[2], F[3]]), b.push([F[4], F[5]]);
          break;
        case "Z":
          x(), m.push([g[0], g[1]]);
      }
      if (C(), !d) return y;
      const z = [];
      for (const S of y) {
        const F = Hc(S, d);
        F.length && z.push(F);
      }
      return z;
    }(e, 1, l ? 4 - 4 * (r.simplification || 1) : (1 + r.roughness) / 2), c = Hs(e, r);
    if (s) if (r.fillStyle === "solid") if (a.length === 1) {
      const h = Hs(e, Object.assign(Object.assign({}, r), { disableMultiStroke: !0, roughness: r.roughness ? r.roughness + r.fillShapeRoughnessGain : 0 }));
      n.push({ type: "fillPath", ops: this._mergedShape(h.ops) });
    } else n.push(sn(a, r));
    else n.push(Mo(a, r));
    return i && (l ? a.forEach((h) => {
      n.push(Sr(h, !1, r));
    }) : n.push(c)), this._d("path", n, r);
  }
  opsToPath(e, o) {
    let r = "";
    for (const n of e.ops) {
      const s = typeof o == "number" && o >= 0 ? n.data.map((i) => +i.toFixed(o)) : n.data;
      switch (n.op) {
        case "move":
          r += `M${s[0]} ${s[1]} `;
          break;
        case "bcurveTo":
          r += `C${s[0]} ${s[1]}, ${s[2]} ${s[3]}, ${s[4]} ${s[5]} `;
          break;
        case "lineTo":
          r += `L${s[0]} ${s[1]} `;
      }
    }
    return r.trim();
  }
  toPaths(e) {
    const o = e.sets || [], r = e.options || this.defaultOptions, n = [];
    for (const s of o) {
      let i = null;
      switch (s.type) {
        case "path":
          i = { d: this.opsToPath(s), stroke: r.stroke, strokeWidth: r.strokeWidth, fill: pe };
          break;
        case "fillPath":
          i = { d: this.opsToPath(s), stroke: pe, strokeWidth: 0, fill: r.fill || pe };
          break;
        case "fillSketch":
          i = this.fillSketch(s, r);
      }
      i && n.push(i);
    }
    return n;
  }
  fillSketch(e, o) {
    let r = o.fillWeight;
    return r < 0 && (r = o.strokeWidth / 2), { d: this.opsToPath(e), stroke: o.fill || pe, strokeWidth: r, fill: pe };
  }
  _mergedShape(e) {
    return e.filter((o, r) => r === 0 || o.op !== "move");
  }
}
class Oc {
  constructor(e, o) {
    this.canvas = e, this.ctx = this.canvas.getContext("2d"), this.gen = new Rr(o);
  }
  draw(e) {
    const o = e.sets || [], r = e.options || this.getDefaultOptions(), n = this.ctx, s = e.options.fixedDecimalPlaceDigits;
    for (const i of o) switch (i.type) {
      case "path":
        n.save(), n.strokeStyle = r.stroke === "none" ? "transparent" : r.stroke, n.lineWidth = r.strokeWidth, r.strokeLineDash && n.setLineDash(r.strokeLineDash), r.strokeLineDashOffset && (n.lineDashOffset = r.strokeLineDashOffset), this._drawToContext(n, i, s), n.restore();
        break;
      case "fillPath": {
        n.save(), n.fillStyle = r.fill || "";
        const l = e.shape === "curve" || e.shape === "polygon" || e.shape === "path" ? "evenodd" : "nonzero";
        this._drawToContext(n, i, s, l), n.restore();
        break;
      }
      case "fillSketch":
        this.fillSketch(n, i, r);
    }
  }
  fillSketch(e, o, r) {
    let n = r.fillWeight;
    n < 0 && (n = r.strokeWidth / 2), e.save(), r.fillLineDash && e.setLineDash(r.fillLineDash), r.fillLineDashOffset && (e.lineDashOffset = r.fillLineDashOffset), e.strokeStyle = r.fill || "", e.lineWidth = n, this._drawToContext(e, o, r.fixedDecimalPlaceDigits), e.restore();
  }
  _drawToContext(e, o, r, n = "nonzero") {
    e.beginPath();
    for (const s of o.ops) {
      const i = typeof r == "number" && r >= 0 ? s.data.map((l) => +l.toFixed(r)) : s.data;
      switch (s.op) {
        case "move":
          e.moveTo(i[0], i[1]);
          break;
        case "bcurveTo":
          e.bezierCurveTo(i[0], i[1], i[2], i[3], i[4], i[5]);
          break;
        case "lineTo":
          e.lineTo(i[0], i[1]);
      }
    }
    o.type === "fillPath" ? e.fill(n) : e.stroke();
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  line(e, o, r, n, s) {
    const i = this.gen.line(e, o, r, n, s);
    return this.draw(i), i;
  }
  rectangle(e, o, r, n, s) {
    const i = this.gen.rectangle(e, o, r, n, s);
    return this.draw(i), i;
  }
  ellipse(e, o, r, n, s) {
    const i = this.gen.ellipse(e, o, r, n, s);
    return this.draw(i), i;
  }
  circle(e, o, r, n) {
    const s = this.gen.circle(e, o, r, n);
    return this.draw(s), s;
  }
  linearPath(e, o) {
    const r = this.gen.linearPath(e, o);
    return this.draw(r), r;
  }
  polygon(e, o) {
    const r = this.gen.polygon(e, o);
    return this.draw(r), r;
  }
  arc(e, o, r, n, s, i, l = !1, a) {
    const c = this.gen.arc(e, o, r, n, s, i, l, a);
    return this.draw(c), c;
  }
  curve(e, o) {
    const r = this.gen.curve(e, o);
    return this.draw(r), r;
  }
  path(e, o) {
    const r = this.gen.path(e, o);
    return this.draw(r), r;
  }
}
const gr = "http://www.w3.org/2000/svg";
class Xc {
  constructor(e, o) {
    this.svg = e, this.gen = new Rr(o);
  }
  draw(e) {
    const o = e.sets || [], r = e.options || this.getDefaultOptions(), n = this.svg.ownerDocument || window.document, s = n.createElementNS(gr, "g"), i = e.options.fixedDecimalPlaceDigits;
    for (const l of o) {
      let a = null;
      switch (l.type) {
        case "path":
          a = n.createElementNS(gr, "path"), a.setAttribute("d", this.opsToPath(l, i)), a.setAttribute("stroke", r.stroke), a.setAttribute("stroke-width", r.strokeWidth + ""), a.setAttribute("fill", "none"), r.strokeLineDash && a.setAttribute("stroke-dasharray", r.strokeLineDash.join(" ").trim()), r.strokeLineDashOffset && a.setAttribute("stroke-dashoffset", `${r.strokeLineDashOffset}`);
          break;
        case "fillPath":
          a = n.createElementNS(gr, "path"), a.setAttribute("d", this.opsToPath(l, i)), a.setAttribute("stroke", "none"), a.setAttribute("stroke-width", "0"), a.setAttribute("fill", r.fill || ""), e.shape !== "curve" && e.shape !== "polygon" || a.setAttribute("fill-rule", "evenodd");
          break;
        case "fillSketch":
          a = this.fillSketch(n, l, r);
      }
      a && s.appendChild(a);
    }
    return s;
  }
  fillSketch(e, o, r) {
    let n = r.fillWeight;
    n < 0 && (n = r.strokeWidth / 2);
    const s = e.createElementNS(gr, "path");
    return s.setAttribute("d", this.opsToPath(o, r.fixedDecimalPlaceDigits)), s.setAttribute("stroke", r.fill || ""), s.setAttribute("stroke-width", n + ""), s.setAttribute("fill", "none"), r.fillLineDash && s.setAttribute("stroke-dasharray", r.fillLineDash.join(" ").trim()), r.fillLineDashOffset && s.setAttribute("stroke-dashoffset", `${r.fillLineDashOffset}`), s;
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  opsToPath(e, o) {
    return this.gen.opsToPath(e, o);
  }
  line(e, o, r, n, s) {
    const i = this.gen.line(e, o, r, n, s);
    return this.draw(i);
  }
  rectangle(e, o, r, n, s) {
    const i = this.gen.rectangle(e, o, r, n, s);
    return this.draw(i);
  }
  ellipse(e, o, r, n, s) {
    const i = this.gen.ellipse(e, o, r, n, s);
    return this.draw(i);
  }
  circle(e, o, r, n) {
    const s = this.gen.circle(e, o, r, n);
    return this.draw(s);
  }
  linearPath(e, o) {
    const r = this.gen.linearPath(e, o);
    return this.draw(r);
  }
  polygon(e, o) {
    const r = this.gen.polygon(e, o);
    return this.draw(r);
  }
  arc(e, o, r, n, s, i, l = !1, a) {
    const c = this.gen.arc(e, o, r, n, s, i, l, a);
    return this.draw(c);
  }
  curve(e, o) {
    const r = this.gen.curve(e, o);
    return this.draw(r);
  }
  path(e, o) {
    const r = this.gen.path(e, o);
    return this.draw(r);
  }
}
var Yc = { canvas: (t, e) => new Oc(t, e), svg: (t, e) => new Xc(t, e), generator: (t) => new Rr(t), newSeed: () => Rr.newSeed() };
const Fe = Yc.generator();
function Gc(t) {
  let e = 0;
  for (let o = 0; o < t.length; o++) {
    const r = t.charCodeAt(o);
    e = (e << 5) - e + r, e |= 0;
  }
  return Math.abs(e);
}
function Ue(t) {
  return {
    stroke: t.stroke,
    fill: t.fill || "none",
    fillStyle: t.fill ? t.fillStyle || "hachure" : void 0,
    roughness: t.roughness,
    strokeWidth: t.strokeWidth,
    strokeLineDash: t.strokeLineDash,
    seed: t.seed ? Gc(t.seed) : void 0,
    fillWeight: t.strokeWidth / 2,
    hachureGap: Math.max(t.strokeWidth * 4, 4)
  };
}
function Qe(t) {
  var r;
  const e = t.options, o = (r = e == null ? void 0 : e.strokeLineDash) != null && r.length ? e.strokeLineDash.join(" ") : void 0;
  return Fe.toPaths(t).map((n) => ({
    d: n.d,
    stroke: n.stroke,
    strokeWidth: n.strokeWidth,
    fill: n.fill,
    // Apply dash only to stroke paths, not fill paths
    strokeDasharray: n.stroke !== "none" && n.strokeWidth > 0 ? o : void 0
  }));
}
function Oo(t, e) {
  return Math.min(t, e) * 0.25;
}
function jc(t, e, o, r, n) {
  const s = Math.min(n, o / 2, r / 2);
  return [
    `M${t + s},${e}`,
    `L${t + o - s},${e}`,
    `A${s},${s} 0 0 1 ${t + o},${e + s}`,
    `L${t + o},${e + r - s}`,
    `A${s},${s} 0 0 1 ${t + o - s},${e + r}`,
    `L${t + s},${e + r}`,
    `A${s},${s} 0 0 1 ${t},${e + r - s}`,
    `L${t},${e + s}`,
    `A${s},${s} 0 0 1 ${t + s},${e}`,
    "Z"
  ].join(" ");
}
function Ar(t, e, o, r, n, s) {
  if (s) {
    const i = Oo(o, r);
    return Qe(Fe.path(jc(t, e, o, r, i), Ue(n)));
  }
  return Qe(Fe.rectangle(t, e, o, r, Ue(n)));
}
function Vn(t, e, o, r, n) {
  return Qe(Fe.ellipse(t, e, o, r, Ue(n)));
}
function Vc(t, e, o, r, n) {
  const s = t + o / 2, i = e + r / 2, l = [s, e], a = [t + o, i], c = [s, e + r], h = [t, i], f = Math.hypot(o / 2, r / 2), d = Math.min(n, f / 2) / f, p = (F, D, W) => [
    F[0] + W * (D[0] - F[0]),
    F[1] + W * (D[1] - F[1])
  ], y = p(h, l, 1 - d), m = p(l, a, d), g = p(l, a, 1 - d), b = p(a, c, d), x = p(a, c, 1 - d), C = p(c, h, d), z = p(c, h, 1 - d), S = p(h, l, d);
  return [
    `M${m[0]},${m[1]}`,
    `L${g[0]},${g[1]}`,
    `Q${a[0]},${a[1]} ${b[0]},${b[1]}`,
    `L${x[0]},${x[1]}`,
    `Q${c[0]},${c[1]} ${C[0]},${C[1]}`,
    `L${z[0]},${z[1]}`,
    `Q${h[0]},${h[1]} ${S[0]},${S[1]}`,
    `L${y[0]},${y[1]}`,
    `Q${l[0]},${l[1]} ${m[0]},${m[1]}`,
    "Z"
  ].join(" ");
}
function Kn(t, e, o, r, n, s) {
  if (s) {
    const l = Oo(o, r);
    return Qe(Fe.path(Vc(t, e, o, r, l), Ue(n)));
  }
  const i = [
    [t + o / 2, e],
    [t + o, e + r / 2],
    [t + o / 2, e + r],
    [t, e + r / 2]
  ];
  return Qe(Fe.polygon(i, Ue(n)));
}
function Fo(t, e, o, r, n) {
  return Qe(Fe.line(t, e, o, r, Ue(n)));
}
function qn(t, e, o, r, n) {
  const s = Fo(t, e, o, r, n), i = Math.atan2(r - e, o - t), l = Math.max(12, n.strokeWidth * 4), a = Math.PI / 6, c = o - l * Math.cos(i - a), h = r - l * Math.sin(i - a), f = o - l * Math.cos(i + a), d = r - l * Math.sin(i + a), p = Fo(o, r, c, h, n), y = Fo(o, r, f, d, n);
  return [...s, ...p, ...y];
}
function js(t, e) {
  const o = {
    ...Ue(e),
    stroke: "none"
  };
  return Qe(Fe.polygon(t, o));
}
function ln(t, e) {
  return Qe(Fe.path(t, Ue(e)));
}
function $e(t) {
  if (t === "dashed") return [8, 4];
  if (t === "dotted") return [2, 2];
}
function Kc(t) {
  const e = t.replace("#", ""), o = parseInt(e.substring(0, 2), 16) || 0, r = parseInt(e.substring(2, 4), 16) || 0, n = parseInt(e.substring(4, 6), 16) || 0;
  return (0.299 * o + 0.587 * r + 0.114 * n) / 255 > 0.5 ? "#1e1e2e" : "#ffffff";
}
function qc({ node: t, editingLabel: e }) {
  if (t.type === "draw") {
    const o = t;
    return o.data.tool === "vector" ? /* @__PURE__ */ u(Uc, { node: o }) : /* @__PURE__ */ u(Zc, { node: o });
  }
  return /* @__PURE__ */ u(Qc, { node: t, editingLabel: e });
}
const Lr = ce(qc), Zc = ce(function({ node: e }) {
  const o = e.data.strokeStyle === "dashed" || e.data.strokeStyle === "dotted", r = $e(e.data.strokeStyle), n = Kt(
    () => o ? null : Yn(e.data.points, { size: e.data.strokeWidth }),
    [e.data.points, e.data.strokeWidth, o]
  ), s = Kt(() => {
    const h = e.data.points;
    if (!h || h.length === 0) return "";
    if (h.length === 1) return `M${h[0][0]},${h[0][1]}L${h[0][0]},${h[0][1]}`;
    const f = [`M${h[0][0]},${h[0][1]}`];
    for (let d = 1; d < h.length; d++)
      f.push(`L${h[d][0]},${h[d][1]}`);
    return f.join("");
  }, [e.data.points]), i = Kt(() => {
    if (!o) return null;
    const h = e.data.points;
    if (h.length < 2) return "";
    const f = ["M", h[0][0], h[0][1]];
    for (let p = 1; p < h.length; p++) {
      const [y, m] = h[p], [g, b] = h[p - 1];
      f.push("Q", g, b, (g + y) / 2, (b + m) / 2);
    }
    const d = h[h.length - 1];
    return f.push("L", d[0], d[1]), f.join(" ");
  }, [e.data.points, o]), l = Kt(() => {
    if (!e.data.fill || e.data.points.length < 3) return null;
    const h = e.data.points.map((z) => [z[0], z[1]]), f = Wi(h), d = f[0], p = f[f.length - 1], y = Math.hypot(d[0] - p[0], d[1] - p[1]);
    let m = 0;
    for (let z = 1; z < f.length; z++)
      m += Math.hypot(f[z][0] - f[z - 1][0], f[z][1] - f[z - 1][1]);
    const g = m >= 1 && y <= Math.max(e.data.strokeWidth * 4, 20) && y <= m * 0.1, b = e.data.fillStyle || "solid";
    if (g) {
      const z = vc(f, 0);
      return b === "solid" ? { kind: "solid", d: z, fill: e.data.fill } : { kind: "rough", paths: js(f, {
        stroke: "none",
        fill: e.data.fill,
        fillStyle: b,
        roughness: 1,
        strokeWidth: e.data.strokeWidth
      }) };
    }
    const x = Cc(f);
    if (x.length === 0) return null;
    if (b === "solid")
      return {
        kind: "solid",
        d: "",
        fill: e.data.fill,
        regions: x
      };
    const C = [];
    for (const { points: z } of x)
      z.length >= 3 && C.push(
        ...js(z, {
          stroke: "none",
          fill: e.data.fill,
          fillStyle: b,
          roughness: 1,
          strokeWidth: e.data.strokeWidth
        })
      );
    return { kind: "rough", paths: C, regions: x };
  }, [e.data.fill, e.data.fillStyle, e.data.points, e.data.strokeWidth]), a = e.h === "auto" ? 0 : e.h, c = e.data.strokeWidth * 4;
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x - c,
        top: e.y - c,
        width: e.w + c * 2,
        height: a + c * 2,
        zIndex: e.z,
        pointerEvents: "none",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "svg",
        {
          width: e.w + c * 2,
          height: a + c * 2,
          style: { overflow: "visible" },
          children: /* @__PURE__ */ v("g", { transform: `translate(${c}, ${c})`, opacity: e.data.opacity ?? 1, children: [
            (l == null ? void 0 : l.kind) === "solid" && (l.regions ? l.regions.map((h, f) => /* @__PURE__ */ u(
              "path",
              {
                d: h.pathD,
                fill: l.fill,
                stroke: "none"
              },
              f
            )) : /* @__PURE__ */ u("path", { d: l.d, fill: l.fill, stroke: "none" })),
            (l == null ? void 0 : l.kind) === "rough" && l.paths.map((h, f) => /* @__PURE__ */ u(
              "path",
              {
                d: h.d,
                stroke: h.stroke,
                strokeWidth: h.strokeWidth,
                fill: h.fill,
                strokeLinecap: "round",
                strokeLinejoin: "round"
              },
              f
            )),
            o ? /* @__PURE__ */ u(
              "path",
              {
                d: i,
                fill: "none",
                stroke: e.data.color,
                strokeWidth: e.data.strokeWidth,
                strokeDasharray: r == null ? void 0 : r.map((h) => h * Math.max(e.data.strokeWidth, 1)).join(" "),
                strokeLinecap: "round",
                strokeLinejoin: "round"
              }
            ) : /* @__PURE__ */ u(
              "path",
              {
                d: n,
                fill: e.data.color
              }
            ),
            s && /* @__PURE__ */ u(
              "path",
              {
                d: s,
                fill: "none",
                stroke: "transparent",
                strokeWidth: e.data.strokeWidth,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                pointerEvents: "stroke"
              }
            )
          ] })
        }
      )
    }
  );
}), Uc = ce(function({ node: e }) {
  const o = e.h === "auto" ? 0 : e.h, r = e.data.strokeWidth * 2, n = Kt(() => {
    const l = e.data.points;
    if (!l || l.length === 0) return "";
    const a = [`M${l[0][0]},${l[0][1]}`];
    for (let c = 1; c < l.length; c++)
      a.push(`L${l[c][0]},${l[c][1]}`);
    return a.push("Z"), a.join("");
  }, [e.data.points]), s = $e(e.data.strokeStyle), i = s == null ? void 0 : s.map((l) => l * Math.max(e.data.strokeWidth, 1)).join(" ");
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x - r,
        top: e.y - r,
        width: e.w + r * 2,
        height: o + r * 2,
        zIndex: e.z,
        pointerEvents: "none",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "svg",
        {
          width: e.w + r * 2,
          height: o + r * 2,
          style: { overflow: "visible" },
          children: /* @__PURE__ */ v("g", { transform: `translate(${r}, ${r})`, opacity: e.data.opacity ?? 1, children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: n,
                fill: e.data.fill || "none",
                stroke: e.data.color,
                strokeWidth: e.data.strokeWidth,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                strokeDasharray: i
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: n,
                fill: e.data.fill ? "transparent" : "none",
                stroke: "transparent",
                strokeWidth: Math.max(e.data.strokeWidth, 8),
                pointerEvents: e.data.fill ? "painted" : "stroke"
              }
            )
          ] })
        }
      )
    }
  );
}), Qc = ce(function({ node: e, editingLabel: o }) {
  var g, b, x, C;
  const r = e.h === "auto" ? 100 : e.h, n = e.data.strokeWidth * 2, s = $e(e.data.strokeStyle), i = ((g = e.data.startPoint) == null ? void 0 : g[0]) ?? 0, l = ((b = e.data.startPoint) == null ? void 0 : b[1]) ?? r / 2, a = ((x = e.data.endPoint) == null ? void 0 : x[0]) ?? e.w, c = ((C = e.data.endPoint) == null ? void 0 : C[1]) ?? r / 2, h = Kt(() => {
    if (e.data.roughness === 0) return null;
    const z = {
      stroke: e.data.stroke,
      fill: e.data.fill,
      fillStyle: e.data.fillStyle,
      roughness: e.data.roughness,
      strokeWidth: e.data.strokeWidth,
      strokeLineDash: s,
      seed: e.id
    }, S = e.data.edgeStyle === "round";
    switch (e.data.shape) {
      case "rect":
        return Ar(0, 0, e.w, r, z, S);
      case "ellipse":
        return Vn(e.w / 2, r / 2, e.w, r, z);
      case "diamond":
        return Kn(0, 0, e.w, r, z, S);
      case "line":
        return Fo(i, l, a, c, z);
      case "arrow":
        return qn(i, l, a, c, z);
      default:
        return null;
    }
  }, [e, s, i, l, a, c, r]), f = e.data.fill && e.data.fillStyle === "solid" && e.data.roughness > 0, d = e.data.opacity ?? 1, p = e.data.shape === "line" || e.data.shape === "arrow", y = e.data.label, m = e.data.labelFontSize ?? 14;
  return /* @__PURE__ */ v(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x,
        top: e.y,
        width: e.w,
        height: r,
        zIndex: e.z,
        pointerEvents: "none",
        overflow: "visible",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: [
        /* @__PURE__ */ u(
          "svg",
          {
            width: e.w + n * 2,
            height: r + n * 2,
            style: { overflow: "visible", marginLeft: -n, marginTop: -n },
            children: /* @__PURE__ */ v("g", { transform: `translate(${n}, ${n})`, opacity: d, children: [
              f && /* @__PURE__ */ u(
                _c,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: r,
                  fill: e.data.fill,
                  rounded: e.data.edgeStyle === "round"
                }
              ),
              h ? h.map((z, S) => f && z.fill && z.fill !== "none" ? null : /* @__PURE__ */ u(
                "path",
                {
                  d: z.d,
                  stroke: z.stroke,
                  strokeWidth: z.strokeWidth,
                  fill: z.fill,
                  strokeDasharray: z.strokeDasharray,
                  strokeLinecap: "round",
                  strokeLinejoin: "round"
                },
                S
              )) : /* @__PURE__ */ u(
                Jc,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: r,
                  x1: i,
                  y1: l,
                  x2: a,
                  y2: c,
                  stroke: e.data.stroke,
                  fill: e.data.fill,
                  strokeWidth: e.data.strokeWidth,
                  dashArray: s,
                  rounded: e.data.edgeStyle === "round"
                }
              ),
              /* @__PURE__ */ u(
                $c,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: r,
                  x1: i,
                  y1: l,
                  x2: a,
                  y2: c,
                  hasFill: !!e.data.fill,
                  strokeWidth: e.data.strokeWidth,
                  rounded: e.data.edgeStyle === "round"
                }
              )
            ] })
          }
        ),
        !p && y && !o && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              pointerEvents: "none",
              padding: "4px 8px"
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                style: {
                  textAlign: e.data.labelAlign ?? "center",
                  fontFamily: qe(e.data.labelFontFamily ?? Ke),
                  fontSize: m,
                  color: e.data.fill && e.data.fillStyle === "solid" ? Kc(e.data.fill) : e.data.stroke,
                  lineHeight: 1.3,
                  wordBreak: "break-word",
                  whiteSpace: "pre-wrap",
                  width: "100%"
                },
                children: y
              }
            )
          }
        )
      ]
    }
  );
});
function Zn(t, e) {
  const o = Oo(t, e), r = t / 2, n = e / 2, s = [r, 0], i = [t, n], l = [r, e], a = [0, n], c = Math.hypot(t / 2, e / 2), h = Math.min(o, c / 2) / c, f = (z, S, F) => [
    z[0] + F * (S[0] - z[0]),
    z[1] + F * (S[1] - z[1])
  ], d = f(s, i, h), p = f(s, i, 1 - h), y = f(i, l, h), m = f(i, l, 1 - h), g = f(l, a, h), b = f(l, a, 1 - h), x = f(a, s, h), C = f(a, s, 1 - h);
  return [
    `M${d[0]},${d[1]}`,
    `L${p[0]},${p[1]}`,
    `Q${i[0]},${i[1]} ${y[0]},${y[1]}`,
    `L${m[0]},${m[1]}`,
    `Q${l[0]},${l[1]} ${g[0]},${g[1]}`,
    `L${b[0]},${b[1]}`,
    `Q${a[0]},${a[1]} ${x[0]},${x[1]}`,
    `L${C[0]},${C[1]}`,
    `Q${s[0]},${s[1]} ${d[0]},${d[1]}`,
    "Z"
  ].join(" ");
}
function Jc({
  shape: t,
  w: e,
  h: o,
  x1: r,
  y1: n,
  x2: s,
  y2: i,
  stroke: l,
  fill: a,
  strokeWidth: c,
  dashArray: h,
  rounded: f
}) {
  const d = h == null ? void 0 : h.join(",");
  switch (t) {
    case "rect": {
      const p = f ? Oo(e, o) : 0;
      return /* @__PURE__ */ u(
        "rect",
        {
          x: 0,
          y: 0,
          width: e,
          height: o,
          rx: p || void 0,
          ry: p || void 0,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    }
    case "ellipse":
      return /* @__PURE__ */ u(
        "ellipse",
        {
          cx: e / 2,
          cy: o / 2,
          rx: e / 2,
          ry: o / 2,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "diamond":
      return f ? /* @__PURE__ */ u(
        "path",
        {
          d: Zn(e, o),
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "line":
      return /* @__PURE__ */ u(
        "line",
        {
          x1: r,
          y1: n,
          x2: s,
          y2: i,
          stroke: l,
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "arrow": {
      const p = Math.atan2(i - n, s - r), y = Math.max(12, c * 4), m = Math.PI / 6, g = s - y * Math.cos(p - m), b = i - y * Math.sin(p - m), x = s - y * Math.cos(p + m), C = i - y * Math.sin(p + m);
      return /* @__PURE__ */ v(ut, { children: [
        /* @__PURE__ */ u(
          "line",
          {
            x1: r,
            y1: n,
            x2: s,
            y2: i,
            stroke: l,
            strokeWidth: c,
            strokeDasharray: d
          }
        ),
        /* @__PURE__ */ u(
          "polyline",
          {
            points: `${g},${b} ${s},${i} ${x},${C}`,
            stroke: l,
            strokeWidth: c,
            fill: "none"
          }
        )
      ] });
    }
    default:
      return null;
  }
}
function $c({
  shape: t,
  w: e,
  h: o,
  x1: r,
  y1: n,
  x2: s,
  y2: i,
  hasFill: l,
  strokeWidth: a,
  rounded: c
}) {
  const h = l ? "painted" : "stroke", f = l ? "transparent" : "none";
  switch (t) {
    case "rect": {
      const d = c ? Oo(e, o) : 0;
      return /* @__PURE__ */ u(
        "rect",
        {
          x: 0,
          y: 0,
          width: e,
          height: o,
          rx: d || void 0,
          ry: d || void 0,
          fill: f,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    }
    case "ellipse":
      return /* @__PURE__ */ u(
        "ellipse",
        {
          cx: e / 2,
          cy: o / 2,
          rx: e / 2,
          ry: o / 2,
          fill: f,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    case "diamond":
      return c ? /* @__PURE__ */ u(
        "path",
        {
          d: Zn(e, o),
          fill: f,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          fill: f,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    case "line":
    case "arrow":
      return /* @__PURE__ */ u(
        "line",
        {
          x1: r,
          y1: n,
          x2: s,
          y2: i,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: "stroke"
        }
      );
    default:
      return null;
  }
}
function _c({
  shape: t,
  w: e,
  h: o,
  fill: r,
  rounded: n
}) {
  switch (t) {
    case "rect": {
      const s = n ? Oo(e, o) : 0;
      return /* @__PURE__ */ u("rect", { x: 0, y: 0, width: e, height: o, rx: s || void 0, ry: s || void 0, fill: r, stroke: "none" });
    }
    case "ellipse":
      return /* @__PURE__ */ u("ellipse", { cx: e / 2, cy: o / 2, rx: e / 2, ry: o / 2, fill: r, stroke: "none" });
    case "diamond":
      return n ? /* @__PURE__ */ u(
        "path",
        {
          d: Zn(e, o),
          fill: r,
          stroke: "none"
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          fill: r,
          stroke: "none"
        }
      );
    default:
      return null;
  }
}
const td = ce(function(e) {
  return /* @__PURE__ */ u(Lr, { node: e.node });
}), ed = {
  type: "draw",
  component: td,
  handlesOwnLayout: !0,
  hitTest: (t, e, o, r) => Wn(t, e, o, r),
  getHitPadding: (t) => {
    const e = t.data;
    return Math.max(20, e.strokeWidth * 4);
  },
  onResize: (t, e, o) => ({
    points: t.data.points.map(
      ([n, s, i]) => [n * e, s * o, i]
    )
  }),
  onFlip: (t, e) => {
    const o = t.data;
    if (e === "h")
      return {
        points: o.points.map(
          ([n, s, i]) => [t.w - n, s, i]
        )
      };
    const r = t.h === "auto" ? 0 : t.h;
    return {
      points: o.points.map(
        ([n, s, i]) => [n, r - s, i]
      )
    };
  },
  getClipboardText: () => null
};
function od({
  node: t
}) {
  const e = t.h === "auto" ? t.type === "draw" ? 0 : 100 : t.h, o = t.type === "draw" && e === 0 ? 24 : e, r = t.type === "draw" ? t.data.strokeWidth * 4 : t.data.strokeWidth * 2;
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: t.x - r,
        top: t.y - r,
        width: t.w + r * 2,
        height: o + r * 2,
        zIndex: t.z,
        pointerEvents: "none",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        border: `1px solid ${t.type === "draw" ? t.data.color : t.data.stroke}`,
        borderRadius: 4,
        opacity: 0.6
      }
    }
  );
}
const Yi = ce(od), rd = ce(function(e) {
  const o = e.node, r = o.h === "auto" ? 100 : o.h, n = o.w * e.zoom, s = r * e.zoom;
  return Math.min(n, s) < 2 ? /* @__PURE__ */ u(Yi, { node: o }) : /* @__PURE__ */ u(Lr, { node: o, editingLabel: e.editing });
}), nd = {
  type: "shape",
  component: rd,
  handlesOwnLayout: !0,
  hitTest: (t, e, o, r) => Wr(t, e, o, r),
  onResize: (t, e, o) => {
    const r = t.data, n = {};
    return r.startPoint && (n.startPoint = [r.startPoint[0] * e, r.startPoint[1] * o]), r.endPoint && (n.endPoint = [r.endPoint[0] * e, r.endPoint[1] * o]), Object.keys(n).length > 0 ? n : null;
  },
  onFlip: (t, e) => {
    const o = t.data;
    if (o.shape !== "arrow" && o.shape !== "line")
      return {};
    if (e === "h")
      return o.startPoint && o.endPoint ? {
        startPoint: [t.w - o.startPoint[0], o.startPoint[1]],
        endPoint: [t.w - o.endPoint[0], o.endPoint[1]]
      } : {};
    const r = t.h === "auto" ? 0 : t.h;
    return o.startPoint && o.endPoint ? {
      startPoint: [o.startPoint[0], r - o.startPoint[1]],
      endPoint: [o.endPoint[0], r - o.endPoint[1]]
    } : {};
  },
  getClipboardText: (t) => t.data.label || null
};
function sd(t) {
  return null;
}
const id = {
  type: "edge",
  component: sd,
  isSVGOnly: !0,
  handlesOwnLayout: !0,
  getClipboardText: () => null
}, br = 0.05, ad = [
  { pos: "nw", edges: ["left", "top"], cx: 0, cy: 0, cursor: "nwse-resize" },
  { pos: "n", edges: ["top"], cx: 0.5, cy: 0, cursor: "ns-resize" },
  { pos: "ne", edges: ["right", "top"], cx: 1, cy: 0, cursor: "nesw-resize" },
  { pos: "e", edges: ["right"], cx: 1, cy: 0.5, cursor: "ew-resize" },
  { pos: "se", edges: ["right", "bottom"], cx: 1, cy: 1, cursor: "nwse-resize" },
  { pos: "s", edges: ["bottom"], cx: 0.5, cy: 1, cursor: "ns-resize" },
  { pos: "sw", edges: ["left", "bottom"], cx: 0, cy: 1, cursor: "nesw-resize" },
  { pos: "w", edges: ["left"], cx: 0, cy: 0.5, cursor: "ew-resize" }
];
function ld({
  node: t,
  isSelected: e,
  engine: o,
  interactive: r,
  zoom: n,
  onResizeHandleDown: s,
  cropping: i,
  onCropStart: l,
  onCropEnd: a
}) {
  const c = t.h, h = t.data.crop, f = at(!1);
  f.current = !!i;
  const d = at(null), [p, y] = tt(null), m = it(() => {
    d.current && d.current.naturalWidth > 0 && y({ w: d.current.naturalWidth, h: d.current.naturalHeight });
  }, []);
  mt(() => {
    d.current && d.current.naturalWidth > 0 && y({ w: d.current.naturalWidth, h: d.current.naturalHeight });
  }, [t.data.src]);
  const [g, b] = tt({ x: 0, y: 0, w: 1, h: 1 });
  mt(() => {
    i && (b(h ?? { x: 0, y: 0, w: 1, h: 1 }), !p && d.current && d.current.naturalWidth > 0 && y({ w: d.current.naturalWidth, h: d.current.naturalHeight }));
  }, [i]);
  const x = Kt(() => {
    if (!p) return null;
    const G = p.w / p.h, V = t.w / c;
    let Y, $;
    return G > V ? (Y = t.w, $ = t.w / G) : ($ = c, Y = c * G), { x: (t.w - Y) / 2, y: (c - $) / 2, w: Y, h: $ };
  }, [p, t.w, c]), C = it(() => {
    const G = g.x < 1e-3 && g.y < 1e-3 && g.w > 0.999 && g.h > 0.999;
    o.updateNodeWithHistory(t.id, {
      data: {
        ...t.data,
        crop: G ? void 0 : { x: g.x, y: g.y, w: g.w, h: g.h }
      }
    }), a == null || a();
  }, [o, t, g, a]), z = it(() => {
    a == null || a();
  }, [a]);
  mt(() => {
    if (!i) return;
    const G = (V) => {
      V.key === "Enter" ? (C(), V.preventDefault(), V.stopPropagation()) : V.key === "Escape" && (z(), V.preventDefault(), V.stopPropagation());
    };
    return document.addEventListener("keydown", G, !0), () => document.removeEventListener("keydown", G, !0);
  }, [i, C, z]);
  const S = it(
    (G, V) => {
      if (V.stopPropagation(), V.preventDefault(), !x) return;
      const Y = V.currentTarget.ownerDocument, $ = V.clientX, nt = V.clientY, ot = { ...g }, yt = (et) => {
        const Pt = (et.clientX - $) / n / x.w, zt = (et.clientY - nt) / n / x.h, gt = { ...ot }, ht = ot.x + ot.w, Dt = ot.y + ot.h;
        if (G.includes("left")) {
          const It = Math.max(0, Math.min(ht - br, ot.x + Pt));
          gt.x = It, gt.w = ht - It;
        }
        if (G.includes("right") && (gt.w = Math.max(
          br,
          Math.min(1 - ot.x, ot.w + Pt)
        )), G.includes("top")) {
          const It = Math.max(0, Math.min(Dt - br, ot.y + zt));
          gt.y = It, gt.h = Dt - It;
        }
        G.includes("bottom") && (gt.h = Math.max(
          br,
          Math.min(1 - ot.y, ot.h + zt)
        )), b(gt);
      }, ft = () => {
        Y.removeEventListener("pointermove", yt), Y.removeEventListener("pointerup", ft);
      };
      Y.addEventListener("pointermove", yt), Y.addEventListener("pointerup", ft);
    },
    [g, x, n]
  ), F = it(
    (G) => {
      if (G.stopPropagation(), G.preventDefault(), !x) return;
      const V = G.currentTarget.ownerDocument, Y = G.clientX, $ = G.clientY, nt = { ...g }, ot = (ft) => {
        const et = (ft.clientX - Y) / n / x.w, Pt = (ft.clientY - $) / n / x.h;
        b({
          ...nt,
          x: Math.max(0, Math.min(1 - nt.w, nt.x + et)),
          y: Math.max(0, Math.min(1 - nt.h, nt.y + Pt))
        });
      }, yt = () => {
        V.removeEventListener("pointermove", ot), V.removeEventListener("pointerup", yt);
      };
      V.addEventListener("pointermove", ot), V.addEventListener("pointerup", yt);
    },
    [g, x, n]
  ), D = it(
    (G) => {
      if (f.current) return;
      const V = G.currentTarget.ownerDocument;
      if (G.altKey) return;
      if (!o.selection.has(t.id) && o.selection.size > 0) {
        const { x: It, y: Rt } = o.screenToCanvas(
          G.clientX,
          G.clientY
        );
        for (const Qt of o.selection) {
          const Jt = o.getNode(Qt);
          if (!Jt) continue;
          const qt = Jt.h === "auto" ? 100 : Jt.h;
          if (It >= Jt.x && It <= Jt.x + Jt.w && Rt >= Jt.y && Rt <= Jt.y + qt)
            return;
        }
      }
      G.stopPropagation(), G.preventDefault(), G.shiftKey ? o.toggleSelect(t.id) : o.selection.has(t.id) || o.select(t.id);
      const Y = G.clientX, $ = G.clientY, nt = Array.from(o.selection), ot = nt.map((It) => {
        const Rt = o.getNode(It);
        return { id: It, x: Rt.x, y: Rt.y };
      });
      let yt = !1, ft = null, et = Y, Pt = $, zt = !1;
      const gt = () => {
        ft = null;
        const It = (et - Y) / o.viewport.zoom, Rt = (Pt - $) / o.viewport.zoom, { finalDx: Qt, finalDy: Jt } = o.computeDragSnap(
          ot,
          nt,
          It,
          Rt,
          zt
        ), qt = ot.map(($t) => ({
          id: $t.id,
          patch: { x: $t.x + Qt, y: $t.y + Jt }
        }));
        o.updateMany(qt);
      }, ht = (It) => {
        const Rt = (It.clientX - Y) / o.viewport.zoom, Qt = (It.clientY - $) / o.viewport.zoom;
        if (!yt)
          if (Math.abs(Rt) > 2 || Math.abs(Qt) > 2)
            yt = !0, o.pushHistorySnapshot();
          else
            return;
        et = It.clientX, Pt = It.clientY, zt = It.metaKey || It.ctrlKey, ft === null && (ft = requestAnimationFrame(gt));
      }, Dt = () => {
        ft !== null && (cancelAnimationFrame(ft), gt()), o.clearAlignGuides(), V.removeEventListener("pointermove", ht), V.removeEventListener("pointerup", Dt);
      };
      V.addEventListener("pointermove", ht), V.addEventListener("pointerup", Dt);
    },
    [o, t.id]
  ), W = [
    { pos: "nw", cx: 0, cy: 0 },
    { pos: "n", cx: 0.5, cy: 0 },
    { pos: "ne", cx: 1, cy: 0 },
    { pos: "e", cx: 1, cy: 0.5 },
    { pos: "se", cx: 1, cy: 1 },
    { pos: "s", cx: 0.5, cy: 1 },
    { pos: "sw", cx: 0, cy: 1 },
    { pos: "w", cx: 0, cy: 0.5 }
  ], K = 8 / n, Z = K / 2, dt = 25 / n, X = e && s && !i, rt = it(
    (G) => {
      const V = G.currentTarget.ownerDocument;
      G.stopPropagation(), G.preventDefault();
      const Y = t.x + t.w / 2, $ = t.y + c / 2, nt = t.rotation || 0, { x: ot, y: yt } = o.screenToCanvas(
        G.clientX,
        G.clientY
      ), ft = Math.atan2(yt - $, ot - Y);
      o.pushHistorySnapshot();
      const et = (zt) => {
        const { x: gt, y: ht } = o.screenToCanvas(
          zt.clientX,
          zt.clientY
        ), Dt = Math.atan2(ht - $, gt - Y);
        let It = nt + (Dt - ft) * (180 / Math.PI);
        (zt.shiftKey || o.snapToGrid) && !(zt.metaKey || zt.ctrlKey) && (It = Math.round(It / 15) * 15), o.updateNode(t.id, { rotation: It });
      }, Pt = () => {
        V.removeEventListener("pointermove", et), V.removeEventListener("pointerup", Pt);
      };
      V.addEventListener("pointermove", et), V.addEventListener("pointerup", Pt);
    },
    [o, t.id, t.x, t.y, t.w, c, t.rotation]
  ), J = i && x ? {
    left: x.x + g.x * x.w,
    top: x.y + g.y * x.h,
    width: g.w * x.w,
    height: g.h * x.h
  } : null, U = i ? void 0 : [t.data.flipH ? "scaleX(-1)" : "", t.data.flipV ? "scaleY(-1)" : ""].filter(Boolean).join(" ") || void 0, R = {
    width: "100%",
    height: "100%",
    objectFit: "contain",
    display: "block",
    pointerEvents: "none",
    opacity: t.data.opacity ?? 1,
    transform: U
  };
  if (!i && h) {
    const G = h.y * 100, V = (1 - h.x - h.w) * 100, Y = (1 - h.y - h.h) * 100, $ = h.x * 100;
    R.objectViewBox = `inset(${G}% ${V}% ${Y}% ${$}%)`;
  }
  const O = 8 / n, _ = O / 2;
  return /* @__PURE__ */ v(
    "div",
    {
      onPointerDown: D,
      onDoubleClick: !i && r ? (G) => {
        G.stopPropagation(), l == null || l();
      } : void 0,
      style: {
        position: "absolute",
        left: t.x + t.w / 2,
        top: t.y + c / 2,
        width: t.w,
        height: c,
        marginLeft: -t.w / 2,
        marginTop: -c / 2,
        zIndex: t.z,
        border: e ? "2px dashed #3b82f6" : "none",
        borderRadius: 6,
        overflow: "visible",
        pointerEvents: r ? "auto" : "none",
        cursor: i ? "default" : "move",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        boxSizing: "border-box"
      },
      children: [
        /* @__PURE__ */ v(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              overflow: "hidden",
              borderRadius: 4,
              border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : "none",
              boxSizing: "border-box"
            },
            children: [
              /* @__PURE__ */ u(
                "img",
                {
                  ref: d,
                  src: t.data.src,
                  alt: t.data.alt ?? "",
                  onLoad: m,
                  style: R,
                  draggable: !1
                }
              ),
              i && J && /* @__PURE__ */ u(
                "div",
                {
                  onPointerDown: F,
                  style: {
                    position: "absolute",
                    left: J.left,
                    top: J.top,
                    width: J.width,
                    height: J.height,
                    boxShadow: `0 0 0 ${Math.max(t.w, c) * 2}px rgba(0,0,0,0.45)`,
                    border: `${1.5 / n}px dashed rgba(255,255,255,0.8)`,
                    boxSizing: "border-box",
                    cursor: "move",
                    zIndex: 10
                  }
                }
              )
            ]
          }
        ),
        i && J && ad.map(({ pos: G, edges: V, cx: Y, cy: $, cursor: nt }) => /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: (ot) => S(V, ot),
            style: {
              position: "absolute",
              left: J.left + Y * J.width - _,
              top: J.top + $ * J.height - _,
              width: O,
              height: O,
              background: "white",
              border: `${1.5 / n}px solid #3b82f6`,
              borderRadius: 2,
              cursor: nt,
              zIndex: 11
            }
          },
          G
        )),
        e && !i && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                left: "50%",
                top: -dt,
                width: 1,
                height: dt,
                background: "#3b82f6",
                marginLeft: -0.5,
                pointerEvents: "none"
              }
            }
          ),
          /* @__PURE__ */ u(
            "div",
            {
              onPointerDown: rt,
              style: {
                position: "absolute",
                left: "50%",
                top: -(dt + K / 2),
                width: K,
                height: K,
                marginLeft: -K / 2,
                borderRadius: "50%",
                background: "white",
                border: "1.5px solid #3b82f6",
                cursor: "grab"
              }
            }
          )
        ] }),
        X && W.map(({ pos: G, cx: V, cy: Y }) => /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: ($) => {
              $.stopPropagation(), s == null || s(t.id, G, $);
            },
            style: {
              position: "absolute",
              left: `calc(${V * 100}% - ${Z}px)`,
              top: `calc(${Y * 100}% - ${Z}px)`,
              width: K,
              height: K,
              background: "white",
              border: "1.5px solid #3b82f6",
              borderRadius: 2,
              cursor: Fr(G, t.rotation || 0)
            }
          },
          G
        ))
      ]
    }
  );
}
const Gi = ce(ld);
function cd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Gi,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      onResizeHandleDown: t.callbacks.onResizeHandleDown,
      cropping: t.editing,
      onCropStart: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditStart) == null ? void 0 : r.call(o, e.id);
      },
      onCropEnd: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      }
    }
  );
}
const dd = {
  type: "image",
  component: cd,
  handlesOwnLayout: !0,
  onFlip: (t, e) => {
    const o = t.data;
    return e === "h" ? { flipH: !o.flipH } : { flipV: !o.flipV };
  },
  getClipboardText: (t) => t.data.src || null
};
function hd({
  node: t,
  engine: e,
  editing: o,
  editClickPos: r,
  onStopEdit: n,
  onMeasuredHeight: s
}) {
  const i = at(null), [l, a] = tt(t.data.text), c = at(!1), h = at(t.data.text), f = at(null), d = at(e);
  d.current = e;
  const p = at(t);
  p.current = t, mt(() => {
    o || a(t.data.text);
  }, [t.data.text]), bi(() => {
    var S, F;
    if (o && i.current) {
      i.current.innerText = t.data.text, i.current.focus();
      const D = i.current.ownerDocument;
      let W = !1;
      if (r) {
        const K = D.caretRangeFromPoint(r.clientX, r.clientY);
        if (K && i.current.contains(K.startContainer)) {
          const Z = (S = D.defaultView) == null ? void 0 : S.getSelection();
          Z == null || Z.removeAllRanges(), Z == null || Z.addRange(K), W = !0;
        }
      }
      if (!W) {
        const K = D.createRange(), Z = (F = D.defaultView) == null ? void 0 : F.getSelection();
        i.current.childNodes.length > 0 && (K.selectNodeContents(i.current), K.collapse(!1)), Z == null || Z.removeAllRanges(), Z == null || Z.addRange(K);
      }
      h.current = t.data.text, c.current = !1;
    }
  }, [o]), mt(() => {
    if (o)
      return () => {
        if (c.current) return;
        c.current = !0;
        const S = h.current, F = e.getNode(t.id);
        if (F && F.type === "text") {
          const D = F.data;
          S !== D.text && e.updateNodeWithHistory(t.id, {
            data: { ...D, text: S }
          });
        }
      };
  }, [o, e, t.id]), mt(() => {
    if (!i.current || !s) return;
    const S = new ResizeObserver(() => {
      var D;
      const F = ((D = i.current) == null ? void 0 : D.offsetHeight) ?? 0;
      F > 0 && s(t.id, F);
    });
    return S.observe(i.current), () => S.disconnect();
  }, [t.id, s, o]);
  const y = it(() => {
    var F;
    if (c.current) return;
    c.current = !0, f.current && (clearTimeout(f.current), f.current = null);
    const S = ((F = i.current) == null ? void 0 : F.innerText) ?? "";
    a(S), h.current = S, S !== t.data.text && e.updateNodeWithHistory(t.id, {
      data: { ...t.data, text: S }
    }), n();
  }, [e, t, n]), m = it(
    (S) => {
      var F;
      S.key === "Escape" && (S.preventDefault(), y(), (F = i.current) == null || F.blur()), S.stopPropagation();
    },
    [y]
  ), g = it(() => {
    y();
  }, [y]), b = it(() => {
    if (i.current) {
      const S = i.current.innerText;
      a(S), h.current = S, f.current && clearTimeout(f.current), f.current = setTimeout(() => {
        const F = p.current;
        S !== F.data.text && d.current.updateNode(F.id, {
          data: { ...F.data, text: S }
        });
      }, 0);
    }
  }, []), x = t.h === "auto" ? void 0 : t.h, C = t.data.opacity ?? 1, z = {
    fontFamily: qe(t.data.fontFamily),
    fontSize: t.data.fontSize,
    color: t.data.color,
    textAlign: t.data.align,
    opacity: C,
    lineHeight: 1,
    outline: "none",
    whiteSpace: "pre-wrap",
    wordBreak: "break-word",
    border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : void 0,
    boxSizing: t.data.borderColor ? "border-box" : void 0,
    borderRadius: t.data.borderColor ? 4 : void 0,
    padding: t.data.borderColor ? 6 : void 0
  };
  return /* @__PURE__ */ u(
    "div",
    {
      "data-node-id": t.id,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: x,
        zIndex: t.z,
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        pointerEvents: o ? "auto" : "none"
      },
      children: o ? /* @__PURE__ */ u(
        "div",
        {
          ref: i,
          contentEditable: !0,
          suppressContentEditableWarning: !0,
          onKeyDown: m,
          onBlur: g,
          onInput: b,
          onPointerDown: (S) => S.stopPropagation(),
          style: { ...z, minHeight: t.data.fontSize, cursor: "text" }
        }
      ) : /* @__PURE__ */ u("div", { ref: i, style: z, children: l || " " })
    }
  );
}
const ji = ce(hd);
function ud(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    ji,
    {
      node: e,
      engine: t.engine,
      editing: t.editing,
      editClickPos: t.editClickPos,
      onStopEdit: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      },
      onMeasuredHeight: t.callbacks.onMeasuredHeight
    }
  );
}
const fd = {
  type: "text",
  component: ud,
  handlesOwnLayout: !0,
  onResize: (t, e, o) => ({ fontSize: t.data.fontSize * e }),
  getClipboardText: (t) => t.data.text || null
};
function pd(t) {
  const e = t.node, o = e.h === "auto" ? 100 : e.h, r = it(
    (s) => {
      var l, a;
      const i = s.currentTarget.value.trim();
      t.engine.updateNodeWithHistory(e.id, {
        data: { ...e.data, label: i || void 0 }
      }), (a = (l = t.callbacks).onEditEnd) == null || a.call(l);
    },
    [e.id, e.data, t.engine, t.callbacks]
  ), n = it(
    (s) => {
      (s.key === "Enter" || s.key === "Escape") && s.currentTarget.blur(), s.stopPropagation();
    },
    []
  );
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x,
        top: e.y,
        width: e.w,
        height: o,
        zIndex: e.z,
        background: e.data.backgroundColor || "rgba(0,0,0,0.02)",
        border: `${e.data.borderWidth || 1}px ${e.data.borderStyle || "dashed"} ${e.data.borderColor || "#ccc"}`,
        boxSizing: "border-box",
        borderRadius: 8,
        opacity: e.data.opacity ?? 1,
        pointerEvents: "none",
        overflow: "visible",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: t.editing ? /* @__PURE__ */ u(
        "input",
        {
          autoFocus: !0,
          defaultValue: e.data.label ?? "",
          placeholder: "Frame label...",
          onBlur: r,
          onKeyDown: n,
          onPointerDown: (s) => s.stopPropagation(),
          style: {
            position: "absolute",
            top: -24,
            left: 0,
            fontSize: 12,
            color: e.data.borderColor || "#999",
            fontWeight: 500,
            background: "rgba(255,255,255,0.95)",
            border: "1px solid #3b82f6",
            borderRadius: 4,
            padding: "1px 4px",
            outline: "none",
            pointerEvents: "auto",
            minWidth: 80
          }
        }
      ) : e.data.label ? /* @__PURE__ */ u(
        "div",
        {
          onDoubleClick: (s) => {
            var i, l;
            s.stopPropagation(), t.engine.select(e.id), (l = (i = t.callbacks).onEditStart) == null || l.call(i, e.id);
          },
          style: {
            position: "absolute",
            top: -20,
            left: 4,
            fontSize: 12,
            color: e.data.borderColor || "#999",
            fontWeight: 500,
            whiteSpace: "nowrap",
            userSelect: "none",
            pointerEvents: "auto",
            cursor: "default"
          },
          children: e.data.label
        }
      ) : null
    }
  );
}
const yd = {
  type: "frame",
  component: pd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.label || null
}, md = 100;
function gd({
  node: t,
  isSelected: e,
  engine: o,
  interactive: r,
  zoom: n,
  editing: s,
  onEditStart: i,
  onEditEnd: l
}) {
  const a = at(null), c = at(null), h = at(""), f = at(null), d = at(null), p = at(t);
  p.current = t;
  const y = at(o);
  y.current = o, mt(() => {
    var z;
    if (s && c.current) {
      const S = c.current;
      S.innerText = t.data.text || "", h.current = t.data.text || "", S.focus();
      const F = S.ownerDocument, D = (z = F.defaultView) == null ? void 0 : z.getSelection(), W = f.current;
      f.current = null;
      let K = !1;
      if (W && D && F.caretRangeFromPoint) {
        const Z = F.caretRangeFromPoint(W.x, W.y);
        Z && S.contains(Z.startContainer) && (D.removeAllRanges(), D.addRange(Z), K = !0);
      }
      if (!K && D) {
        const Z = F.createRange();
        S.childNodes.length > 0 && (Z.selectNodeContents(S), Z.collapse(!1)), D.removeAllRanges(), D.addRange(Z);
      }
    }
  }, [s]), mt(() => {
    if (s)
      return () => {
        const z = p.current, S = h.current;
        S !== z.data.text && y.current.updateNodeWithHistory(z.id, {
          data: { ...z.data, text: S }
        });
      };
  }, [s]);
  const m = it(() => {
    d.current && (clearTimeout(d.current), d.current = null), c.current && (h.current = c.current.innerText), l();
  }, [l]), g = it(
    (z) => {
      const S = z.currentTarget.ownerDocument;
      if (z.altKey) return;
      if (!o.selection.has(t.id) && o.selection.size > 0) {
        const { x: _, y: G } = o.screenToCanvas(z.clientX, z.clientY);
        for (const V of o.selection) {
          const Y = o.getNode(V);
          if (!Y) continue;
          const $ = Y.h === "auto" ? 100 : Y.h;
          if (_ >= Y.x && _ <= Y.x + Y.w && G >= Y.y && G <= Y.y + $)
            return;
        }
      }
      if (z.stopPropagation(), s) return;
      z.currentTarget.setPointerCapture(z.pointerId), z.shiftKey ? o.toggleSelect(t.id) : o.selection.has(t.id) || o.select(t.id);
      const F = z.clientX, D = z.clientY, W = Array.from(o.selection), K = [];
      for (const _ of W) {
        const G = o.getNode(_);
        G && K.push({ id: _, x: G.x, y: G.y });
      }
      if (K.length === 0) return;
      let Z = !1, dt = null, X = F, rt = D, J = !1;
      const U = () => {
        dt = null;
        const _ = (X - F) / o.viewport.zoom, G = (rt - D) / o.viewport.zoom, { finalDx: V, finalDy: Y } = o.computeDragSnap(
          K,
          W,
          _,
          G,
          J
        ), $ = K.map((nt) => ({
          id: nt.id,
          patch: { x: nt.x + V, y: nt.y + Y }
        }));
        o.updateMany($);
      }, R = (_) => {
        const G = (_.clientX - F) / o.viewport.zoom, V = (_.clientY - D) / o.viewport.zoom;
        if (!Z)
          if (Math.abs(G) > 2 || Math.abs(V) > 2)
            Z = !0, o.pushHistorySnapshot();
          else
            return;
        X = _.clientX, rt = _.clientY, J = _.metaKey || _.ctrlKey, dt === null && (dt = requestAnimationFrame(U));
      }, O = () => {
        dt !== null && (cancelAnimationFrame(dt), U()), o.clearAlignGuides(), S.removeEventListener("pointermove", R), S.removeEventListener("pointerup", O);
      };
      S.addEventListener("pointermove", R), S.addEventListener("pointerup", O);
    },
    [o, t.id, s]
  ), b = it(
    (z) => {
      if (r) {
        if (z.stopPropagation(), t.groupId) {
          const S = [];
          let F = t.groupId;
          for (; F; )
            S.push(F), F = o.groupParent.get(F);
          if (!o.activeGroupId) {
            o.enterGroup(S[S.length - 1]), o.select(t.id);
            return;
          }
          const D = S.indexOf(o.activeGroupId);
          if (D > 0) {
            o.enterGroup(S[D - 1]), o.select(t.id);
            return;
          }
        }
        s || (f.current = { x: z.clientX, y: z.clientY }, o.select(t.id), i(t.id));
      }
    },
    [r, s, o, t.id, t.groupId, i]
  ), x = t.data.fontSize ?? 16, C = t.h === "auto" ? md : t.h;
  return /* @__PURE__ */ u(
    "div",
    {
      ref: a,
      "data-node-id": t.id,
      className: r ? void 0 : "sb-block-inert",
      onPointerDown: r ? g : void 0,
      onDoubleClick: b,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: C,
        zIndex: t.z,
        background: t.data.color,
        borderRadius: t.data.edgeStyle === "round" ? 12 : 2,
        boxShadow: "0 2px 8px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.08)",
        opacity: t.data.opacity ?? 1,
        pointerEvents: r ? "auto" : "none",
        outline: "none",
        overflow: "hidden",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "div",
        {
          style: {
            padding: 12,
            height: "100%",
            overflow: "auto",
            touchAction: s ? "pan-y" : "none",
            cursor: s ? "text" : "move",
            userSelect: s ? "text" : "none"
          },
          children: s ? /* @__PURE__ */ u(
            "div",
            {
              ref: c,
              contentEditable: !0,
              suppressContentEditableWarning: !0,
              onBlur: m,
              onInput: () => {
                c.current && (h.current = c.current.innerText, d.current && clearTimeout(d.current), d.current = setTimeout(() => {
                  const z = p.current, S = h.current;
                  S !== z.data.text && y.current.updateNode(z.id, {
                    data: { ...z.data, text: S }
                  });
                }, 0));
              },
              onKeyDown: (z) => {
                z.key === "Escape" && (z.stopPropagation(), m()), z.stopPropagation();
              },
              onPointerDown: (z) => z.stopPropagation(),
              style: {
                fontSize: x,
                fontFamily: qe(Ke),
                lineHeight: 1.5,
                color: "#1e1e2e",
                wordWrap: "break-word",
                whiteSpace: "pre-wrap",
                outline: "none",
                minHeight: "100%"
              }
            }
          ) : /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: x,
                fontFamily: qe(Ke),
                lineHeight: 1.5,
                color: "#1e1e2e",
                wordWrap: "break-word",
                whiteSpace: "pre-wrap"
              },
              children: t.data.text || ""
            }
          )
        }
      )
    }
  );
}
const Vi = ce(gd);
function bd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Vi,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      editing: t.editing,
      onEditStart: (o) => {
        var r, n;
        return (n = (r = t.callbacks).onEditStart) == null ? void 0 : n.call(r, o);
      },
      onEditEnd: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      }
    }
  );
}
const xd = {
  type: "sticky",
  component: bd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.text || null
}, Ki = /(?:youtube\.com\/(?:watch\?.*v=|embed\/|shorts\/|live\/)|youtu\.be\/)([\w-]{11})/;
function wd(t) {
  const e = t.match(Ki);
  return e ? e[1] : null;
}
function kd(t) {
  return Ki.test(t);
}
function vd(t) {
  return `https://www.youtube.com/embed/${t}`;
}
function Sd(t) {
  return `https://img.youtube.com/vi/${t}/hqdefault.jpg`;
}
function Md({
  node: t,
  isSelected: e,
  engine: o,
  interactive: r,
  zoom: n,
  editing: s,
  onResizeHandleDown: i,
  onEditStart: l
}) {
  const a = t.h, { data: c } = t, h = (y) => {
    if (r && s) {
      y.stopPropagation();
      return;
    }
  }, f = c.borderColor ? `${c.borderWidth ?? 1}px ${c.borderStyle ?? "solid"} ${c.borderColor}` : "none", d = Math.max(6, 8 / n), p = [
    { key: "nw", x: "0%", y: "0%", cursor: "nwse-resize" },
    { key: "ne", x: "100%", y: "0%", cursor: "nesw-resize" },
    { key: "se", x: "100%", y: "100%", cursor: "nwse-resize" },
    { key: "sw", x: "0%", y: "100%", cursor: "nesw-resize" },
    { key: "n", x: "50%", y: "0%", cursor: "ns-resize" },
    { key: "s", x: "50%", y: "100%", cursor: "ns-resize" },
    { key: "e", x: "100%", y: "50%", cursor: "ew-resize" },
    { key: "w", x: "0%", y: "50%", cursor: "ew-resize" }
  ];
  return /* @__PURE__ */ v(
    "div",
    {
      onPointerDown: h,
      onDoubleClick: !s && r ? (y) => {
        y.stopPropagation(), l == null || l();
      } : void 0,
      style: {
        position: "absolute",
        left: t.x + t.w / 2,
        top: t.y + a / 2,
        width: t.w,
        height: a,
        marginLeft: -t.w / 2,
        marginTop: -a / 2,
        zIndex: t.z,
        border: e ? "2px dashed #3b82f6" : "none",
        borderRadius: 6,
        overflow: "visible",
        pointerEvents: r ? "auto" : "none",
        cursor: s ? "default" : "move",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        boxSizing: "border-box"
      },
      children: [
        /* @__PURE__ */ v(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              overflow: "hidden",
              borderRadius: 4,
              border: f,
              boxSizing: "border-box",
              opacity: c.opacity ?? 1
            },
            children: [
              /* @__PURE__ */ u(
                "iframe",
                {
                  src: vd(c.videoId),
                  title: "YouTube video",
                  allow: "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture",
                  allowFullScreen: !0,
                  style: {
                    width: "100%",
                    height: "100%",
                    border: "none",
                    display: "block",
                    pointerEvents: s ? "auto" : "none"
                  }
                }
              ),
              !s && /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    position: "absolute",
                    inset: 0,
                    cursor: "move"
                  }
                }
              )
            ]
          }
        ),
        e && r && !s && p.map((y) => /* @__PURE__ */ u(
          "div",
          {
            "data-handle": y.key,
            onPointerDown: (m) => {
              m.stopPropagation(), i == null || i(t.id, y.key, m);
            },
            style: {
              position: "absolute",
              left: y.x,
              top: y.y,
              width: d,
              height: d,
              marginLeft: -d / 2,
              marginTop: -d / 2,
              background: "#fff",
              border: "1px solid #3b82f6",
              borderRadius: 2,
              cursor: y.cursor,
              zIndex: 1
            }
          },
          y.key
        ))
      ]
    }
  );
}
const Cd = ce(Md);
function zd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Cd,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      editing: t.editing,
      onResizeHandleDown: t.callbacks.onResizeHandleDown,
      onEditStart: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditStart) == null ? void 0 : r.call(o, e.id);
      },
      onEditEnd: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      }
    }
  );
}
const Id = {
  type: "youtube",
  component: zd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.url || null
}, Td = [
  rc,
  ed,
  nd,
  id,
  dd,
  fd,
  yd,
  xd,
  Id
];
function io(t, e) {
  return `${t}:${e}`;
}
class Pd {
  constructor(e, o) {
    St(this, "spatial");
    St(this, "registry");
    /** Current resolved port values. */
    St(this, "values", /* @__PURE__ */ new Map());
    /** Node IDs that need recomputation. */
    St(this, "dirty", /* @__PURE__ */ new Set());
    /** Whether a microtask flush is already scheduled. */
    St(this, "scheduled", !1);
    /** Generation counter for canceling stale async results. */
    St(this, "generation", 0);
    /** Change subscribers. */
    St(this, "listeners", /* @__PURE__ */ new Set());
    /** Node IDs that are part of a cycle (updated after each topoSort). */
    St(this, "_cycleNodeIds", /* @__PURE__ */ new Set());
    this.spatial = e, this.registry = o;
  }
  // ── Public API ─────────────────────────────────────────────
  /** Node IDs that are part of a dependency cycle (read-only). */
  get cycleNodeIds() {
    return this._cycleNodeIds;
  }
  /** Subscribe to port value changes. Returns unsubscribe function. */
  onChange(e) {
    return this.listeners.add(e), () => this.listeners.delete(e);
  }
  /** Get current value of a specific port. */
  getPortValue(e, o) {
    return this.values.get(io(e, o)) ?? null;
  }
  /** Get all input values for a node, resolved from connected edges. */
  getInputs(e) {
    var s;
    const o = this.registry.get(
      ((s = this.spatial.nodes.get(e)) == null ? void 0 : s.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const r = {}, n = o.ports.filter((i) => i.direction === "input");
    for (const i of n) {
      const l = this.spatial.getEdgesForNode(e);
      let a = !1;
      for (const c of l) {
        const h = c.data;
        if (h.toId === e && h.targetPort === i.id) {
          const f = this.values.get(
            io(h.fromId, h.sourcePort ?? "")
          );
          r[i.id] = f ?? i.defaultValue ?? null, a = !0;
          break;
        }
      }
      a || (r[i.id] = i.defaultValue ?? null);
    }
    return r;
  }
  /** Get all output values for a node. */
  getOutputs(e) {
    var n;
    const o = this.registry.get(
      ((n = this.spatial.nodes.get(e)) == null ? void 0 : n.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const r = {};
    for (const s of o.ports)
      s.direction === "output" && (r[s.id] = this.values.get(io(e, s.id)) ?? null);
    return r;
  }
  /** Get all port values (inputs + outputs) for a node. */
  getAllPortValues(e) {
    var n;
    const o = this.registry.get(
      ((n = this.spatial.nodes.get(e)) == null ? void 0 : n.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const r = {};
    for (const s of o.ports)
      if (s.direction === "input") {
        const i = this.spatial.getEdgesForNode(e);
        let l = !1;
        for (const a of i) {
          const c = a.data;
          if (c.toId === e && c.targetPort === s.id) {
            r[s.id] = this.values.get(io(c.fromId, c.sourcePort ?? "")) ?? s.defaultValue ?? null, l = !0;
            break;
          }
        }
        l || (r[s.id] = s.defaultValue ?? null);
      } else
        r[s.id] = this.values.get(io(e, s.id)) ?? null;
    return r;
  }
  /** Mark a node as dirty and schedule recomputation. */
  markDirty(e) {
    this.dirty.add(e), this.scheduleFlush();
  }
  /** Wire up SpatialEngine event listeners. Returns cleanup function. */
  connect() {
    const e = (n) => {
      const s = this.registry.get(n.type);
      s != null && s.ports && this.markDirty(n.id);
    }, o = (n) => {
      if (n.type === "edge") {
        const s = n.data;
        s.targetPort && this.markDirty(s.toId);
      } else {
        const s = this.registry.get(n.type);
        s != null && s.ports && s.compute && this.markDirty(n.id);
      }
    }, r = (n) => {
      if (n.type === "edge") {
        const s = n.data;
        s.targetPort && this.markDirty(s.toId);
      } else {
        const s = this.registry.get(n.type);
        if (s != null && s.ports) {
          for (const i of s.ports)
            this.values.delete(io(n.id, i.id));
          this.markDownstream(n.id);
        }
      }
    };
    return this.spatial.on("node:data", e), this.spatial.on("node:create", o), this.spatial.on("node:delete", r), this.initializeAll(), () => {
      this.spatial.off("node:data", e), this.spatial.off("node:create", o), this.spatial.off("node:delete", r);
    };
  }
  /** Dispose and clean up. */
  dispose() {
    this.values.clear(), this.dirty.clear(), this.listeners.clear(), this.scheduled = !1;
  }
  // ── Private implementation ─────────────────────────────────
  /** Initialize all nodes with ports. */
  initializeAll() {
    for (const e of this.spatial.nodes.values()) {
      const o = this.registry.get(e.type);
      o != null && o.ports && o.compute && this.dirty.add(e.id);
    }
    this.dirty.size > 0 && this.scheduleFlush();
  }
  /** Schedule a microtask flush if not already scheduled. */
  scheduleFlush() {
    this.scheduled || (this.scheduled = !0, queueMicrotask(() => {
      this.scheduled = !1, this.flush();
    }));
  }
  /** Mark all downstream nodes (nodes that depend on outputs of nodeId) as dirty. */
  markDownstream(e) {
    const o = this.spatial.getEdgesForNode(e);
    for (const r of o) {
      const n = r.data;
      n.fromId === e && n.targetPort && this.dirty.add(n.toId);
    }
  }
  /** Topological sort of dirty nodes + their downstream dependents. */
  topoSort() {
    const e = /* @__PURE__ */ new Set();
    for (const y of this.spatial.nodes.values()) {
      const m = this.registry.get(y.type);
      m != null && m.ports && m.compute && e.add(y.id);
    }
    if (e.size === 0) {
      const y = this._cycleNodeIds.size > 0;
      return y && (this._cycleNodeIds = /* @__PURE__ */ new Set()), { sorted: [], cyclesChanged: y };
    }
    const o = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
    for (const y of e)
      o.set(y, /* @__PURE__ */ new Set()), r.set(y, 0);
    const n = this.spatial.getAllEdges();
    for (const y of n) {
      const m = y.data;
      m.sourcePort && m.targetPort && e.has(m.fromId) && e.has(m.toId) && (o.get(m.fromId).add(m.toId), r.set(m.toId, (r.get(m.toId) ?? 0) + 1));
    }
    const s = new Set(this.dirty), i = /* @__PURE__ */ new Set(), l = (y) => {
      if (i.has(y)) return;
      i.add(y);
      const m = o.get(y);
      if (m)
        for (const g of m)
          s.add(g), l(g);
    };
    for (const y of [...this.dirty])
      l(y);
    const a = /* @__PURE__ */ new Map();
    for (const y of s)
      a.set(y, 0);
    for (const y of n) {
      const m = y.data;
      m.sourcePort && m.targetPort && s.has(m.fromId) && s.has(m.toId) && a.set(
        m.toId,
        (a.get(m.toId) ?? 0) + 1
      );
    }
    const c = [];
    for (const [y, m] of a)
      m === 0 && c.push(y);
    const h = [];
    for (; c.length > 0; ) {
      const y = c.shift();
      h.push(y);
      const m = o.get(y);
      if (m)
        for (const g of m) {
          if (!s.has(g)) continue;
          const b = (a.get(g) ?? 1) - 1;
          a.set(g, b), b === 0 && c.push(g);
        }
    }
    const f = new Set(h), d = /* @__PURE__ */ new Set();
    for (const y of s)
      f.has(y) || d.add(y);
    let p = !1;
    return (d.size !== this._cycleNodeIds.size || [...d].some((y) => !this._cycleNodeIds.has(y))) && (this._cycleNodeIds = d, p = !0), { sorted: h, cyclesChanged: p };
  }
  /** Full graph recompute of dirty nodes. */
  flush() {
    if (this.dirty.size === 0) return;
    const { sorted: e, cyclesChanged: o } = this.topoSort();
    this.dirty.clear();
    let r = !1;
    for (const n of e)
      this.executeNode(n) && (r = !0);
    (r || o) && this.notifyListeners();
  }
  /** Execute a single node's compute function. Returns true if outputs changed. */
  executeNode(e) {
    const o = this.spatial.nodes.get(e);
    if (!o) return !1;
    const r = this.registry.get(o.type);
    if (!(r != null && r.compute) || !r.ports) return !1;
    const n = this.getInputs(e), s = r.compute(n, o.data);
    if (s instanceof Promise) {
      const i = ++this.generation;
      return s.then((l) => {
        if (i !== this.generation) return;
        this.applyOutputs(e, r.ports, l) && (this.markDownstream(e), this.notifyListeners(), this.dirty.size > 0 && this.scheduleFlush());
      }), !1;
    }
    return this.applyOutputs(e, r.ports, s);
  }
  /** Apply computed outputs to the values map. Returns true if any value changed. */
  applyOutputs(e, o, r) {
    let n = !1;
    for (const s of o) {
      if (s.direction !== "output") continue;
      const i = io(e, s.id), l = r[s.id] ?? null, a = this.values.get(i) ?? null;
      Rd(a, l) || (this.values.set(i, l), n = !0);
    }
    return n && this.markDownstream(e), n;
  }
  /** Notify all change listeners. */
  notifyListeners() {
    for (const e of this.listeners)
      e();
  }
}
function Rd(t, e) {
  if (t === e) return !0;
  if (t == null || e == null || typeof t != typeof e) return !1;
  if (typeof t == "object" && typeof e == "object") {
    const o = Object.keys(t), r = Object.keys(e);
    if (o.length !== r.length) return !1;
    for (const n of o)
      if (t[n] !== e[n])
        return !1;
    return !0;
  }
  return !1;
}
const $o = [
  // Light
  { key: "plain-white", label: "White", group: "light", canvasBg: "#ffffff", swatchColor: "#ffffff" },
  { key: "dot-grid", label: "Cream", group: "light", canvasBg: "#f8f7f5", swatchColor: "#f8f7f5" },
  { key: "engineering", label: "Warm", group: "light", canvasBg: "#fdf6e3", swatchColor: "#fdf6e3" },
  // Dark
  { key: "blueprint", label: "Blueprint", group: "dark", canvasBg: "#1e3a5f", swatchColor: "#1e3a5f" },
  { key: "dark-grid", label: "Night", group: "dark", canvasBg: "#1a1a2e", swatchColor: "#1a1a2e" },
  // Textured
  { key: "japanese-stationery", label: "Japanese Stationery", group: "textured", canvasBg: "#f5f0e8", swatchColor: "#f5f0e8" },
  { key: "kraft", label: "Kraft Paper", group: "textured", canvasBg: "#d4b896", swatchColor: "#d4b896" }
];
function nr(t) {
  return $o.find((e) => e.key === t) ?? $o[1];
}
function Ad() {
  return {
    staticDefs: /* @__PURE__ */ v("filter", { id: "paper-texture", x: "-20%", y: "-20%", width: "140%", height: "140%", children: [
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.08", numOctaves: 4, seed: 12, stitchTiles: "stitch", result: "noise" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "noise", type: "saturate", values: "0", result: "bump" }),
      /* @__PURE__ */ u("feDiffuseLighting", { in: "bump", lightingColor: "#f7f4ee", surfaceScale: "1.2", diffuseConstant: "1", result: "lit", children: /* @__PURE__ */ u("feDistantLight", { azimuth: "225", elevation: "50" }) }),
      /* @__PURE__ */ u("feComposite", { in: "lit", in2: "bump", operator: "in", result: "lit-masked" }),
      /* @__PURE__ */ u("feFlood", { floodColor: "#f5f0e8", result: "base" }),
      /* @__PURE__ */ u("feBlend", { in: "base", in2: "lit-masked", mode: "overlay", result: "paper" }),
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.6", numOctaves: 3, seed: 7, stitchTiles: "stitch", result: "grain" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "grain", type: "saturate", values: "0", result: "grain-gray" }),
      /* @__PURE__ */ v("feComponentTransfer", { in: "grain-gray", result: "grain-subtle", children: [
        /* @__PURE__ */ u("feFuncR", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncG", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncB", { type: "linear", slope: "0.06", intercept: "0.47" })
      ] }),
      /* @__PURE__ */ u("feBlend", { in: "paper", in2: "grain-subtle", mode: "overlay", result: "paper-final" })
    ] }),
    staticLayers: [
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "#f5f0e8", filter: "url(#paper-texture)" }, "texture")
    ]
  };
}
function Ld() {
  return {
    staticDefs: /* @__PURE__ */ v("filter", { id: "kraft-texture", x: "-20%", y: "-20%", width: "140%", height: "140%", children: [
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.04", numOctaves: 5, seed: 42, stitchTiles: "stitch", result: "noise" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "noise", type: "saturate", values: "0", result: "bump" }),
      /* @__PURE__ */ u("feDiffuseLighting", { in: "bump", lightingColor: "#e0c9a6", surfaceScale: "1.4", diffuseConstant: "0.95", result: "lit", children: /* @__PURE__ */ u("feDistantLight", { azimuth: "200", elevation: "50" }) }),
      /* @__PURE__ */ u("feComposite", { in: "lit", in2: "bump", operator: "in", result: "lit-masked" }),
      /* @__PURE__ */ u("feFlood", { floodColor: "#d4b896", result: "base" }),
      /* @__PURE__ */ u("feBlend", { in: "base", in2: "lit-masked", mode: "overlay", result: "kraft" }),
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.35", numOctaves: 2, seed: 99, stitchTiles: "stitch", result: "fiber" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "fiber", type: "saturate", values: "0", result: "fiber-gray" }),
      /* @__PURE__ */ v("feComponentTransfer", { in: "fiber-gray", result: "fiber-subtle", children: [
        /* @__PURE__ */ u("feFuncR", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncG", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncB", { type: "linear", slope: "0.06", intercept: "0.47" })
      ] }),
      /* @__PURE__ */ u("feBlend", { in: "kraft", in2: "fiber-subtle", mode: "overlay", result: "kraft-final" })
    ] }),
    staticLayers: [
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "#d4b896", filter: "url(#kraft-texture)" }, "texture")
    ]
  };
}
const cn = {
  "japanese-stationery": Ad,
  kraft: Ld
};
function Dd(t) {
  var e;
  return ((e = cn[t]) == null ? void 0 : e.call(cn)) ?? {};
}
const qi = {
  position: "absolute",
  inset: 0,
  width: "100%",
  height: "100%",
  pointerEvents: "none"
}, Ed = {
  ...qi,
  willChange: "transform"
}, Wd = ce(function({
  background: e
}) {
  const o = nr(e), { staticDefs: r, staticLayers: n } = Dd(e);
  return /* @__PURE__ */ v("svg", { style: Ed, children: [
    r && /* @__PURE__ */ u("defs", { children: r }),
    /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: o.canvasBg }),
    n
  ] });
});
function Fd({
  viewport: t,
  gridSize: e = 20,
  background: o = "dot-grid",
  gridVisible: r = !0
}) {
  const n = e * t.zoom, s = t.x % n, i = t.y % n, a = nr(o).group === "dark" ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.15)";
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(Wd, { background: o }),
    r && /* @__PURE__ */ v("svg", { style: qi, children: [
      /* @__PURE__ */ u("defs", { children: /* @__PURE__ */ u(
        "pattern",
        {
          id: "grid-dots",
          x: s,
          y: i,
          width: n,
          height: n,
          patternUnits: "userSpaceOnUse",
          children: /* @__PURE__ */ u("circle", { cx: n / 2, cy: n / 2, r: 1.5, fill: a })
        }
      ) }),
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "url(#grid-dots)" })
    ] })
  ] });
}
const Zi = "sb-excalib-index", Un = "sb-excalib-";
function Nr() {
  try {
    const t = localStorage.getItem(Zi);
    return t ? JSON.parse(t) : [];
  } catch {
    return [];
  }
}
function Ui(t) {
  localStorage.setItem(Zi, JSON.stringify(t));
}
function Bd(t) {
  try {
    const e = localStorage.getItem(Un + t);
    return e ? Qn(JSON.parse(e)) : null;
  } catch {
    return null;
  }
}
function Qn(t) {
  if (t.libraryItems)
    return t;
  const o = (t.library ?? []).map((r, n) => ({
    id: wt(10),
    name: `Item ${n + 1}`,
    status: "published",
    created: Date.now(),
    elements: r
  }));
  return {
    type: "excalidrawlib",
    version: 2,
    source: t.source,
    libraryItems: o
  };
}
function Qi() {
  return Nr();
}
function Jn(t) {
  const e = Bd(t);
  return (e == null ? void 0 : e.libraryItems) ?? [];
}
function $n(t, e) {
  const o = Qn(t), r = wt(10), n = o.libraryItems.map((l) => l.name || "Untitled"), s = {
    id: r,
    name: (e == null ? void 0 : e.name) || "Imported Library",
    source: (e == null ? void 0 : e.source) || "local-import",
    installedAt: Date.now(),
    itemCount: o.libraryItems.length,
    itemNames: n
  };
  localStorage.setItem(Un + r, JSON.stringify(o));
  const i = Nr();
  return i.push(s), Ui(i), s;
}
function Nd(t) {
  localStorage.removeItem(Un + t);
  const e = Nr().filter((o) => o.id !== t);
  Ui(e);
}
function Hd(t) {
  if (!t.trim()) return [];
  const e = t.toLowerCase(), o = [], r = Nr();
  for (const n of r) {
    if (!n.itemNames.some((l) => l.toLowerCase().includes(e)) && !n.name.toLowerCase().includes(e)) continue;
    const i = Jn(n.id);
    for (const l of i)
      ((l.name || "").toLowerCase().includes(e) || n.name.toLowerCase().includes(e)) && o.push({ library: n, item: l });
  }
  return o;
}
async function Od(t, e) {
  const o = await fetch(t);
  if (!o.ok) throw new Error(`Failed to fetch library: ${o.status}`);
  const r = await o.json();
  if (r.type !== "excalidrawlib")
    throw new Error("Invalid file: not an Excalidraw library");
  const n = Qn(r);
  return $n(n, { name: e, source: t });
}
const zn = {
  toolbarBg: "#1e1e2e",
  panelBg: "#1e1e2e",
  panelShadow: "0 4px 24px rgba(0,0,0,0.4)",
  panelBorderRadius: 12,
  controlBg: "#2a2a3e",
  controlBgActive: "#3b82f6",
  controlBorderRadius: 6,
  text: "white",
  textMuted: "#999",
  textSecondary: "#888",
  textFaint: "#666",
  textDisabled: "#555",
  border: "#333",
  separator: "#444",
  swatchBorderActive: "white",
  error: "#e74c3c",
  accentColor: "#3b82f6"
}, Ji = xi(zn);
function Ut() {
  return Er(Ji);
}
function sr(t) {
  const e = Math.round(t) / 100;
  return e < 1 ? e : void 0;
}
function yo(t) {
  if (t)
    return t * (180 / Math.PI);
}
function $i(t) {
  if (!(!t || t === "transparent"))
    return t;
}
function _i(t) {
  if (t === "solid") return "solid";
  if (t === "cross-hatch") return "cross-hatch";
  if (t === "hachure") return "hachure";
}
function ta(t) {
  if (t === "dashed") return "dashed";
  if (t === "dotted") return "dotted";
}
function ea(t) {
  switch (t) {
    case 2:
      return "sans-serif";
    case 3:
      return "monospace";
    default:
      return "Excalifont";
  }
}
function oa(t) {
  return t === "right" ? "right" : t === "center" ? "center" : "left";
}
function Xd(t) {
  if (t.roundness || t.strokeSharpness === "round") return "round";
}
function dn(t, e) {
  return {
    id: wt(10),
    type: "shape",
    x: t.x,
    y: t.y,
    w: t.width,
    h: t.height,
    z: 0,
    rotation: yo(t.angle),
    locked: t.locked || void 0,
    data: {
      shape: e,
      stroke: t.strokeColor || "#1e1e2e",
      fill: $i(t.backgroundColor),
      fillStyle: _i(t.fillStyle),
      strokeWidth: t.strokeWidth || 2,
      strokeStyle: ta(t.strokeStyle),
      roughness: Math.min(t.roughness ?? 1, 2),
      opacity: sr(t.opacity ?? 100),
      edgeStyle: e === "rect" || e === "diamond" ? Xd(t) : void 0
    }
  };
}
function Vs(t, e) {
  const o = t.points ?? [[0, 0]];
  if (o.length < 2) return [];
  const r = {
    stroke: t.strokeColor || "#1e1e2e",
    fill: void 0,
    fillStyle: void 0,
    strokeWidth: t.strokeWidth || 2,
    strokeStyle: ta(t.strokeStyle),
    roughness: Math.min(t.roughness ?? 1, 2),
    opacity: sr(t.opacity ?? 100)
  };
  if (o.length === 2) {
    const [l, a] = o, c = Math.min(l[0], a[0]), h = Math.min(l[1], a[1]), f = Math.max(l[0], a[0]), d = Math.max(l[1], a[1]), p = Math.max(f - c, 1), y = Math.max(d - h, 1);
    return [
      {
        id: wt(10),
        type: "shape",
        x: t.x + c,
        y: t.y + h,
        w: p,
        h: y,
        z: 0,
        rotation: yo(t.angle),
        locked: t.locked || void 0,
        data: {
          ...r,
          shape: e ? "arrow" : "line",
          startPoint: [l[0] - c, l[1] - h],
          endPoint: [a[0] - c, a[1] - h]
        }
      }
    ];
  }
  if (t.backgroundColor && t.backgroundColor !== "transparent") {
    const l = Yd(t);
    if (l) return [l];
  }
  const s = wt(10), i = [];
  for (let l = 0; l < o.length - 1; l++) {
    const a = o[l], c = o[l + 1], h = Math.min(a[0], c[0]), f = Math.min(a[1], c[1]), d = Math.max(a[0], c[0]), p = Math.max(a[1], c[1]), y = Math.max(d - h, 1), m = Math.max(p - f, 1), g = l === o.length - 2;
    i.push({
      id: wt(10),
      type: "shape",
      x: t.x + h,
      y: t.y + f,
      w: y,
      h: m,
      z: 0,
      rotation: yo(t.angle),
      locked: t.locked || void 0,
      groupId: s,
      data: {
        ...r,
        shape: e && g ? "arrow" : "line",
        startPoint: [a[0] - h, a[1] - f],
        endPoint: [c[0] - h, c[1] - f]
      }
    });
  }
  return i;
}
function Yd(t) {
  const e = t.points ?? [];
  if (e.length < 3) return null;
  let o = 1 / 0, r = 1 / 0, n = -1 / 0, s = -1 / 0;
  for (const [l, a] of e)
    l < o && (o = l), a < r && (r = a), l > n && (n = l), a > s && (s = a);
  if (!isFinite(o)) return null;
  const i = e.map(([l, a]) => [
    l - o,
    a - r,
    0.5
  ]);
  return {
    id: wt(10),
    type: "draw",
    x: t.x + o,
    y: t.y + r,
    w: Math.max(n - o, 1),
    h: Math.max(s - r, 1),
    z: 0,
    rotation: yo(t.angle),
    locked: t.locked || void 0,
    data: {
      tool: "vector",
      points: i,
      color: t.strokeColor || "#1e1e2e",
      strokeWidth: t.strokeWidth || 2,
      opacity: sr(t.opacity ?? 100),
      fill: $i(t.backgroundColor),
      fillStyle: _i(t.fillStyle)
    }
  };
}
function Gd(t) {
  const e = t.points;
  if (!e || e.length === 0) return null;
  const o = t.pressures, r = t.simulatePressure !== !1, n = e.map((h, f) => {
    const d = !r && o && f < o.length ? o[f] : 0.5;
    return [h[0], h[1], d];
  });
  let s = 1 / 0, i = 1 / 0, l = -1 / 0, a = -1 / 0;
  for (const [h, f] of n)
    h < s && (s = h), f < i && (i = f), h > l && (l = h), f > a && (a = f);
  isFinite(s) || (s = 0, i = 0, l = 0, a = 0);
  const c = n.map(
    ([h, f, d]) => [h - s, f - i, d]
  );
  return {
    id: wt(10),
    type: "draw",
    x: t.x + s,
    y: t.y + i,
    w: Math.max(l - s, 1),
    h: Math.max(a - i, 1),
    z: 0,
    rotation: yo(t.angle),
    locked: t.locked || void 0,
    data: {
      tool: "pen",
      points: c,
      color: t.strokeColor || "#1e1e2e",
      strokeWidth: t.strokeWidth || 2,
      opacity: sr(t.opacity ?? 100)
    }
  };
}
function jd(t) {
  return {
    id: wt(10),
    type: "text",
    x: t.x,
    y: t.y,
    w: Math.ceil((t.width || 200) * 1.2),
    h: "auto",
    z: 0,
    rotation: yo(t.angle),
    locked: t.locked || void 0,
    data: {
      text: t.originalText || t.text || "",
      fontSize: t.fontSize || 20,
      fontFamily: ea(t.fontFamily),
      color: t.strokeColor || "#1e1e2e",
      align: oa(t.textAlign),
      opacity: sr(t.opacity ?? 100)
    }
  };
}
function Vd(t) {
  return {
    id: wt(10),
    type: "frame",
    x: t.x,
    y: t.y,
    w: t.width || 400,
    h: t.height || 300,
    z: 0,
    rotation: yo(t.angle),
    locked: t.locked || void 0,
    data: {
      label: t.name || void 0
    }
  };
}
function ra(t) {
  return Kd(t.elements);
}
function Kd(t) {
  const e = [], o = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map();
  for (const s of t)
    s.isDeleted || s.type === "text" && s.containerId && n.set(s.containerId, s);
  for (const s of t) {
    if (s.isDeleted || s.type === "text" && s.containerId) continue;
    let i = [];
    switch (s.type) {
      case "rectangle":
        i = [dn(s, "rect")];
        break;
      case "ellipse":
        i = [dn(s, "ellipse")];
        break;
      case "diamond":
        i = [dn(s, "diamond")];
        break;
      case "arrow":
        i = Vs(s, !0);
        break;
      case "line":
        i = Vs(s, !1);
        break;
      case "freedraw": {
        const l = Gd(s);
        l && (i = [l]);
        break;
      }
      case "text":
        i = [jd(s)];
        break;
      case "frame":
      case "magicframe":
        i = [Vd(s)];
        break;
      case "image":
        continue;
      default:
        continue;
    }
    i.length > 0 && o.set(s.id, i[0].id), e.push(...i);
  }
  for (const [s, i] of n) {
    const l = o.get(s);
    if (!l) continue;
    const a = e.find((h) => h.id === l);
    if (!a || a.type !== "shape") continue;
    const c = a.data;
    c.label = i.originalText || i.text || "", c.labelFontSize = i.fontSize || 20, c.labelFontFamily = ea(i.fontFamily), c.labelAlign = oa(i.textAlign);
  }
  return qd(t, e, o, r), Zd(e), { nodes: e, groupParent: r };
}
function qd(t, e, o, r) {
  var s;
  const n = /* @__PURE__ */ new Map();
  for (const i of t) {
    if (i.isDeleted || !((s = i.groupIds) != null && s.length)) continue;
    for (let a = 0; a < i.groupIds.length - 1; a++) {
      const c = i.groupIds[a], h = i.groupIds[a + 1];
      n.has(c) || n.set(c, h);
    }
    const l = o.get(i.id);
    if (l) {
      const a = e.find((c) => c.id === l);
      a && (a.groupId = i.groupIds[0]);
    }
  }
  for (const [i, l] of n)
    r.set(i, l);
}
function Zd(t) {
  if (t.length === 0) return;
  let e = 1 / 0, o = 1 / 0;
  for (const r of t)
    r.x < e && (e = r.x), r.y < o && (o = r.y);
  if (isFinite(e))
    for (const r of t)
      r.x -= e, r.y -= o;
}
function _n(t, e = 60) {
  if (t.length === 0)
    return `<svg width="${e}" height="${e}" xmlns="http://www.w3.org/2000/svg"/>`;
  let o = 1 / 0, r = 1 / 0, n = -1 / 0, s = -1 / 0;
  for (const f of t) {
    const d = f.h === "auto" ? 40 : f.h;
    o = Math.min(o, f.x), r = Math.min(r, f.y), n = Math.max(n, f.x + f.w), s = Math.max(s, f.y + d);
  }
  const i = n - o || 1, l = s - r || 1, a = 4, c = `${o - a} ${r - a} ${i + a * 2} ${l + a * 2}`, h = [];
  for (const f of t)
    switch (f.type) {
      case "shape":
        h.push(Ud(f));
        break;
      case "draw":
        h.push(Qd(f));
        break;
      case "text":
        h.push(Jd(f));
        break;
    }
  return `<svg width="${e}" height="${e}" viewBox="${c}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">${h.join("")}</svg>`;
}
function na(t) {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function Ud(t) {
  var d, p, y, m;
  const e = t.data, o = t.h === "auto" ? 100 : t.h, r = {
    stroke: e.stroke,
    fill: e.fill,
    fillStyle: e.fillStyle,
    roughness: Math.min(e.roughness, 1),
    // cap roughness for speed
    strokeWidth: e.strokeWidth,
    strokeLineDash: $e(e.strokeStyle),
    seed: t.id
  }, n = ((d = e.startPoint) == null ? void 0 : d[0]) ?? 0, s = ((p = e.startPoint) == null ? void 0 : p[1]) ?? o / 2, i = ((y = e.endPoint) == null ? void 0 : y[0]) ?? t.w, l = ((m = e.endPoint) == null ? void 0 : m[1]) ?? o / 2;
  let a;
  switch (e.shape) {
    case "rect":
      a = Ar(t.x, t.y, t.w, o, r, e.edgeStyle === "round");
      break;
    case "ellipse":
      a = Vn(t.x + t.w / 2, t.y + o / 2, t.w, o, r);
      break;
    case "diamond":
      a = Kn(t.x, t.y, t.w, o, r, e.edgeStyle === "round");
      break;
    case "line":
      a = Fo(t.x + n, t.y + s, t.x + i, t.y + l, r);
      break;
    case "arrow":
      a = qn(t.x + n, t.y + s, t.x + i, t.y + l, r);
      break;
    default:
      return "";
  }
  const c = e.opacity ?? 1, h = c < 1 ? `<g opacity="${c}">` : "<g>", f = a.map(
    (g) => `<path d="${na(g.d)}" fill="${g.fill || "none"}" stroke="${g.stroke}" stroke-width="${g.strokeWidth}"${g.strokeDasharray ? ` stroke-dasharray="${g.strokeDasharray}"` : ""} stroke-linecap="round" stroke-linejoin="round"/>`
  );
  return `${h}${f.join("")}</g>`;
}
function Qd(t) {
  const e = t.data;
  if (!e.points.length) return "";
  const o = e.points.map(([s, i]) => `${(t.x + s).toFixed(1)},${(t.y + i).toFixed(1)}`).join(" "), r = e.opacity ?? 1, n = e.tool === "vector" && e.fill ? e.fill : "none";
  return `<polygon points="${o}" fill="${n}" stroke="${e.color}" stroke-width="${e.strokeWidth}" stroke-linecap="round" stroke-linejoin="round"${r < 1 ? ` opacity="${r}"` : ""}/>`;
}
function Jd(t) {
  const e = t.data, o = Math.max(e.fontSize, 8), r = e.opacity ?? 1, n = e.text.split(`
`)[0] || "";
  return `<text x="${t.x}" y="${t.y + o}" fill="${e.color}" font-size="${o}" font-family="sans-serif"${r < 1 ? ` opacity="${r}"` : ""}>${na(n)}</text>`;
}
const sa = "sb-personal-library";
function ts() {
  try {
    const t = localStorage.getItem(sa);
    return t ? JSON.parse(t) : [];
  } catch {
    return [];
  }
}
function ia(t) {
  localStorage.setItem(sa, JSON.stringify(t));
}
function aa() {
  return ts();
}
function $d(t, e, o) {
  const r = structuredClone(e);
  if (r.length > 0) {
    let a = 1 / 0, c = 1 / 0;
    for (const h of r)
      h.x < a && (a = h.x), h.y < c && (c = h.y);
    if (isFinite(a))
      for (const h of r)
        h.x -= a, h.y -= c;
  }
  const n = new Set(
    r.map((a) => a.groupId).filter(Boolean)
  ), s = [];
  for (const [a, c] of o)
    n.has(a) && s.push([a, c]);
  const i = {
    id: wt(10),
    name: t.trim() || "Untitled",
    nodes: r,
    groupParent: s,
    createdAt: Date.now()
  }, l = ts();
  return l.unshift(i), ia(l), i;
}
function _d(t) {
  const e = ts().filter((o) => o.id !== t);
  ia(e);
}
function la(t, e, o, r) {
  const { nodes: n, groupParent: s } = ra(e);
  if (n.length === 0) return;
  const i = structuredClone(n), l = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map();
  for (const x of i) {
    const C = wt(10);
    l.set(x.id, C), x.id = C;
  }
  for (const x of i)
    x.groupId && (a.has(x.groupId) || a.set(x.groupId, wt(10)), x.groupId = a.get(x.groupId));
  let c = 1 / 0, h = 1 / 0, f = -1 / 0, d = -1 / 0;
  for (const x of i) {
    const C = x.h === "auto" ? 100 : x.h;
    c = Math.min(c, x.x), h = Math.min(h, x.y), f = Math.max(f, x.x + x.w), d = Math.max(d, x.y + C);
  }
  const p = o ?? window.innerWidth / 2, y = r ?? window.innerHeight / 2, m = t.screenToCanvas(p, y), g = m.x - (c + f) / 2, b = m.y - (h + d) / 2;
  for (const x of i)
    x.x += g, x.y += b, x.z = t.nextZ();
  t.addNodes(i);
  for (const [x, C] of s) {
    const z = a.get(x) ?? x, S = a.get(C) ?? C;
    t.groupParent.set(z, S);
  }
  t.selectMultiple(i.map((x) => x.id));
}
const In = "application/x-spatialboard-library-item", Tn = "application/x-spatialboard-personal-item";
function ca(t, e, o, r) {
  if (e.nodes.length === 0) return;
  const n = structuredClone(e.nodes), s = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Map();
  for (const g of n) {
    const b = wt(10);
    s.set(g.id, b), g.id = b;
  }
  for (const g of n)
    g.groupId && (i.has(g.groupId) || i.set(g.groupId, wt(10)), g.groupId = i.get(g.groupId));
  for (const g of n)
    if (g.type === "edge") {
      const b = g.data;
      b.fromId && s.has(b.fromId) && (b.fromId = s.get(b.fromId)), b.toId && s.has(b.toId) && (b.toId = s.get(b.toId));
    }
  let l = 1 / 0, a = 1 / 0, c = -1 / 0, h = -1 / 0;
  for (const g of n) {
    const b = g.h === "auto" ? 100 : g.h;
    l = Math.min(l, g.x), a = Math.min(a, g.y), c = Math.max(c, g.x + g.w), h = Math.max(h, g.y + b);
  }
  const f = o ?? window.innerWidth / 2, d = r ?? window.innerHeight / 2, p = t.screenToCanvas(f, d), y = p.x - (l + c) / 2, m = p.y - (a + h) / 2;
  for (const g of n)
    g.x += y, g.y += m, g.z = t.nextZ();
  t.addNodes(n);
  for (const [g, b] of e.groupParent) {
    const x = i.get(g) ?? g, C = i.get(b) ?? b;
    t.groupParent.set(x, C);
  }
  t.selectMultiple(n.map((g) => g.id));
}
const No = /* @__PURE__ */ new Map();
function th({ item: t }) {
  const e = Kt(() => {
    const o = No.get(t.id);
    if (o) return o;
    const { nodes: r } = ra(t), n = _n(r, 56);
    return No.set(t.id, n), n;
  }, [t.id]);
  return /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: e } });
}
function da({
  item: t,
  libId: e,
  onClick: o,
  theme: r
}) {
  const n = it(
    (s) => {
      s.dataTransfer.setData(
        In,
        JSON.stringify({ libraryId: e, itemId: t.id })
      ), s.dataTransfer.effectAllowed = "copy";
    },
    [e, t.id]
  );
  return /* @__PURE__ */ u(
    "button",
    {
      title: t.name || "Untitled",
      onClick: o,
      draggable: !0,
      onDragStart: n,
      style: {
        border: `1px solid ${r.border}`,
        borderRadius: 4,
        background: r.controlBg,
        cursor: "grab",
        padding: 2,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        aspectRatio: "1"
      },
      children: /* @__PURE__ */ u(th, { item: t })
    }
  );
}
function eh({ nodes: t }) {
  const e = Kt(() => {
    const o = "personal-" + t.map((s) => s.id).join(","), r = No.get(o);
    if (r) return r;
    const n = _n(t, 56);
    return No.set(o, n), n;
  }, [t]);
  return /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: e } });
}
function ha({
  item: t,
  onClick: e,
  onRemove: o,
  theme: r
}) {
  const [n, s] = tt(!1), i = it(
    (l) => {
      l.dataTransfer.setData(
        Tn,
        JSON.stringify({ itemId: t.id })
      ), l.dataTransfer.effectAllowed = "copy";
    },
    [t.id]
  );
  return /* @__PURE__ */ v(
    "div",
    {
      style: { position: "relative", aspectRatio: "1" },
      onMouseEnter: () => s(!0),
      onMouseLeave: () => s(!1),
      children: [
        /* @__PURE__ */ u(
          "button",
          {
            title: t.name,
            onClick: e,
            draggable: !0,
            onDragStart: i,
            style: {
              border: `1px solid ${r.border}`,
              borderRadius: 4,
              background: r.controlBg,
              cursor: "grab",
              padding: 2,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              width: "100%",
              height: "100%"
            },
            children: /* @__PURE__ */ u(eh, { nodes: t.nodes })
          }
        ),
        n && /* @__PURE__ */ u(
          "button",
          {
            title: "Remove from Personal Library",
            onClick: (l) => {
              l.stopPropagation(), o();
            },
            style: {
              position: "absolute",
              top: -4,
              right: -4,
              width: 16,
              height: 16,
              borderRadius: "50%",
              border: "none",
              background: "#ef4444",
              color: "#fff",
              fontSize: 10,
              lineHeight: "16px",
              textAlign: "center",
              cursor: "pointer",
              padding: 0,
              display: "flex",
              alignItems: "center",
              justifyContent: "center"
            },
            children: "×"
          }
        )
      ]
    }
  );
}
function oh({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: r,
  onBrowseDirectory: n
}) {
  const s = Ut(), i = at(null), l = at(null), [a, c] = tt([]), [h, f] = tt([]), [d, p] = tt(""), [y, m] = tt(/* @__PURE__ */ new Set()), g = it(() => {
    c(Qi()), f(aa());
  }, []);
  mt(() => {
    e && g();
  }, [e, g]), mt(() => {
    if (!e) return;
    const W = (K) => {
      i.current && !i.current.contains(K.target) && o();
    };
    return document.addEventListener("pointerdown", W), () => document.removeEventListener("pointerdown", W);
  }, [e, o]);
  const b = it(
    (W) => {
      var dt;
      const K = (dt = W.target.files) == null ? void 0 : dt[0];
      if (!K) return;
      const Z = new FileReader();
      Z.onload = () => {
        try {
          const X = JSON.parse(Z.result);
          if (X.type !== "excalidrawlib") {
            console.warn("Not an Excalidraw library file");
            return;
          }
          const rt = K.name.replace(/\.excalidrawlib$/, "");
          $n(X, { name: rt }), g();
        } catch (X) {
          console.error("Failed to parse library file:", X);
        }
      }, Z.readAsText(K), W.target.value = "";
    },
    [g]
  ), x = it(
    (W) => {
      Nd(W), No.clear(), g();
    },
    [g]
  ), C = it(
    (W) => {
      la(t, W);
    },
    [t]
  ), z = it(
    (W) => {
      ca(t, W);
    },
    [t]
  ), S = it(
    (W) => {
      _d(W), No.clear(), g();
    },
    [g]
  ), F = it((W) => {
    m((K) => {
      const Z = new Set(K);
      return Z.has(W) ? Z.delete(W) : Z.add(W), Z;
    });
  }, []), D = Kt(() => {
    if (!d.trim()) return null;
    const W = d.toLowerCase(), K = Hd(d), Z = h.filter(
      (dt) => dt.name.toLowerCase().includes(W)
    );
    return { excalidraw: K, personal: Z };
  }, [d, h]);
  return !e || !r ? null : Je(
    /* @__PURE__ */ v(
      "div",
      {
        ref: i,
        style: {
          position: "fixed",
          left: r.right + 8,
          top: r.top,
          background: s.panelBg,
          border: `1px solid ${s.border}`,
          borderRadius: s.panelBorderRadius,
          padding: 0,
          zIndex: 99999,
          boxShadow: s.panelShadow,
          width: 280,
          maxHeight: `calc(100vh - ${r.top + 20}px)`,
          display: "flex",
          flexDirection: "column",
          overflow: "hidden"
        },
        onPointerDown: (W) => W.stopPropagation(),
        children: [
          /* @__PURE__ */ v("div", { style: { padding: "10px 12px 6px", flexShrink: 0 }, children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 11,
                  fontWeight: 600,
                  color: s.text,
                  marginBottom: 8
                },
                children: "Libraries"
              }
            ),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                placeholder: "Search library...",
                value: d,
                onChange: (W) => p(W.target.value),
                style: {
                  width: "100%",
                  padding: "5px 8px",
                  border: `1px solid ${s.border}`,
                  borderRadius: s.controlBorderRadius,
                  background: s.controlBg,
                  color: s.text,
                  fontSize: 11,
                  outline: "none",
                  boxSizing: "border-box"
                }
              }
            )
          ] }),
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                flex: 1,
                overflowY: "auto",
                overflowX: "hidden",
                padding: "4px 12px"
              },
              children: D !== null ? D.excalidraw.length === 0 && D.personal.length === 0 ? /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    color: s.textDisabled,
                    fontSize: 11,
                    textAlign: "center",
                    padding: 20
                  },
                  children: "No matching items"
                }
              ) : /* @__PURE__ */ v(
                "div",
                {
                  style: {
                    display: "grid",
                    gridTemplateColumns: "repeat(4, 1fr)",
                    gap: 4
                  },
                  children: [
                    D.personal.map((W) => /* @__PURE__ */ u(
                      ha,
                      {
                        item: W,
                        onClick: () => z(W),
                        onRemove: () => S(W.id),
                        theme: s
                      },
                      W.id
                    )),
                    D.excalidraw.map(({ library: W, item: K }) => /* @__PURE__ */ u(
                      da,
                      {
                        item: K,
                        libId: W.id,
                        onClick: () => C(K),
                        theme: s
                      },
                      K.id
                    ))
                  ]
                }
              ) : /* @__PURE__ */ v(ut, { children: [
                h.length > 0 && /* @__PURE__ */ u(
                  nh,
                  {
                    items: h,
                    onPlace: z,
                    onRemove: S,
                    theme: s
                  }
                ),
                a.length === 0 && h.length === 0 ? /* @__PURE__ */ v(
                  "div",
                  {
                    style: {
                      color: s.textDisabled,
                      fontSize: 11,
                      textAlign: "center",
                      padding: "20px 10px"
                    },
                    children: [
                      "No libraries installed.",
                      /* @__PURE__ */ u("br", {}),
                      "Import an .excalidrawlib file",
                      /* @__PURE__ */ u("br", {}),
                      "or browse the community directory."
                    ]
                  }
                ) : a.map((W) => {
                  const K = y.has(W.id);
                  return /* @__PURE__ */ u(
                    rh,
                    {
                      lib: W,
                      expanded: K,
                      onToggle: () => F(W.id),
                      onPlace: C,
                      onUninstall: () => x(W.id),
                      theme: s
                    },
                    W.id
                  );
                })
              ] })
            }
          ),
          /* @__PURE__ */ v(
            "div",
            {
              style: {
                borderTop: `1px solid ${s.border}`,
                padding: "8px 12px",
                display: "flex",
                gap: 6,
                flexShrink: 0
              },
              children: [
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: () => {
                      var W;
                      return (W = l.current) == null ? void 0 : W.click();
                    },
                    style: {
                      flex: 1,
                      padding: "5px 8px",
                      border: `1px solid ${s.border}`,
                      borderRadius: s.controlBorderRadius,
                      background: s.controlBg,
                      color: s.text,
                      cursor: "pointer",
                      fontSize: 10,
                      fontWeight: 500
                    },
                    children: "Import file"
                  }
                ),
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: n,
                    style: {
                      flex: 1,
                      padding: "5px 8px",
                      border: "none",
                      borderRadius: s.controlBorderRadius,
                      background: s.accentColor,
                      color: "#fff",
                      cursor: "pointer",
                      fontSize: 10,
                      fontWeight: 500
                    },
                    children: "Browse libraries"
                  }
                )
              ]
            }
          ),
          /* @__PURE__ */ u(
            "input",
            {
              ref: l,
              type: "file",
              accept: ".excalidrawlib,.json",
              style: { display: "none" },
              onChange: b
            }
          )
        ]
      }
    ),
    document.body
  );
}
function rh({
  lib: t,
  expanded: e,
  onToggle: o,
  onPlace: r,
  onUninstall: n,
  theme: s
}) {
  const [i, l] = tt(null);
  return mt(() => {
    e && i === null && l(Jn(t.id));
  }, [e, i, t.id]), /* @__PURE__ */ v("div", { style: { marginBottom: 4 }, children: [
    /* @__PURE__ */ v(
      "div",
      {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
          padding: "4px 0",
          cursor: "pointer",
          userSelect: "none"
        },
        onClick: o,
        children: [
          /* @__PURE__ */ u(
            "svg",
            {
              width: 12,
              height: 12,
              viewBox: "0 0 12 12",
              fill: "none",
              style: {
                transform: e ? "rotate(90deg)" : "rotate(0deg)",
                transition: "transform 0.15s"
              },
              children: /* @__PURE__ */ u(
                "path",
                {
                  d: "M4 2l4 4-4 4",
                  stroke: s.textMuted,
                  strokeWidth: 1.5,
                  strokeLinecap: "round"
                }
              )
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flex: 1,
                fontSize: 10,
                fontWeight: 600,
                color: s.textMuted,
                textTransform: "uppercase",
                letterSpacing: "0.03em",
                overflow: "hidden",
                textOverflow: "ellipsis",
                whiteSpace: "nowrap"
              },
              children: t.name
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                fontSize: 9,
                color: s.textDisabled
              },
              children: t.itemCount
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Uninstall library",
              onClick: (a) => {
                a.stopPropagation(), n();
              },
              style: {
                border: "none",
                background: "transparent",
                color: s.textDisabled,
                cursor: "pointer",
                padding: "2px 4px",
                fontSize: 12,
                lineHeight: 1
              },
              children: "×"
            }
          )
        ]
      }
    ),
    e && i && /* @__PURE__ */ u(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: "repeat(4, 1fr)",
          gap: 4,
          padding: "2px 0 6px"
        },
        children: i.map((a) => /* @__PURE__ */ u(
          da,
          {
            item: a,
            libId: t.id,
            onClick: () => r(a),
            theme: s
          },
          a.id
        ))
      }
    )
  ] });
}
function nh({
  items: t,
  onPlace: e,
  onRemove: o,
  theme: r
}) {
  const [n, s] = tt(!0);
  return /* @__PURE__ */ v("div", { style: { marginBottom: 4 }, children: [
    /* @__PURE__ */ v(
      "div",
      {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
          padding: "4px 0",
          cursor: "pointer",
          userSelect: "none"
        },
        onClick: () => s((i) => !i),
        children: [
          /* @__PURE__ */ u(
            "svg",
            {
              width: 12,
              height: 12,
              viewBox: "0 0 12 12",
              fill: "none",
              style: {
                transform: n ? "rotate(90deg)" : "rotate(0deg)",
                transition: "transform 0.15s"
              },
              children: /* @__PURE__ */ u(
                "path",
                {
                  d: "M4 2l4 4-4 4",
                  stroke: r.textMuted,
                  strokeWidth: 1.5,
                  strokeLinecap: "round"
                }
              )
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flex: 1,
                fontSize: 10,
                fontWeight: 600,
                color: r.textMuted,
                textTransform: "uppercase",
                letterSpacing: "0.03em"
              },
              children: "Personal"
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                fontSize: 9,
                color: r.textDisabled
              },
              children: t.length
            }
          )
        ]
      }
    ),
    n && /* @__PURE__ */ u(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: "repeat(4, 1fr)",
          gap: 4,
          padding: "2px 0 6px"
        },
        children: t.map((i) => /* @__PURE__ */ u(
          ha,
          {
            item: i,
            onClick: () => e(i),
            onRemove: () => o(i.id),
            theme: r
          },
          i.id
        ))
      }
    )
  ] });
}
async function sh(t, e, o = 1, r = 20, n) {
  const s = `${t}/search?q=${encodeURIComponent(e)}&page=${o}&per_page=${r}`;
  return (await fetch(s, { signal: n, credentials: "include" })).json();
}
async function Ks(t, e = 1, o = 20, r) {
  const n = `${t}/trending?page=${e}&per_page=${o}`;
  return (await fetch(n, { signal: r, credentials: "include" })).json();
}
const Pn = "application/x-spatialboard-gif-item";
function ua(t, e, o, r) {
  const n = e.file.hd.gif, s = 400, i = 300;
  let l = n.width, a = n.height;
  const c = Math.min(1, s / l, i / a);
  l = Math.round(l * c), a = Math.round(a * c);
  const h = o ?? window.innerWidth / 2, f = r ?? window.innerHeight / 2, d = t.screenToCanvas(h, f), p = {
    id: wt(10),
    type: "image",
    x: d.x - l / 2,
    y: d.y - a / 2,
    w: l,
    h: a,
    z: t.nextZ(),
    data: { src: n.url }
  };
  t.addNode(p), t.select(p.id);
}
function ih({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: r,
  baseUrl: n
}) {
  const s = Ut(), i = at(null), l = at(null), [a, c] = tt(""), [h, f] = tt([]), [d, p] = tt(!1), [y, m] = tt(1), [g, b] = tt(!1), x = at();
  mt(() => {
    if (!e) return;
    const D = (W) => {
      i.current && !i.current.contains(W.target) && o();
    };
    return document.addEventListener("pointerdown", D), () => document.removeEventListener("pointerdown", D);
  }, [e, o]), mt(() => {
    if (!e || a.trim()) return;
    const D = new AbortController();
    return p(!0), Ks(n, 1, 30, D.signal).then((W) => {
      f(W.data.data.filter((K) => K.type !== "ad")), m(1), b(W.data.has_next);
    }).catch(() => {
    }).finally(() => p(!1)), () => D.abort();
  }, [e, n, a]);
  const C = it(
    (D, W, K) => {
      if (!D.trim()) return;
      const Z = new AbortController();
      return p(!0), sh(n, D, W, 30, Z.signal).then((dt) => {
        const X = dt.data.data.filter((rt) => rt.type !== "ad");
        f((rt) => K ? [...rt, ...X] : X), m(W), b(dt.data.has_next);
      }).catch(() => {
      }).finally(() => p(!1)), Z;
    },
    [n]
  ), z = it(
    (D) => {
      if (c(D), x.current && clearTimeout(x.current), !D.trim()) {
        f([]), m(1), b(!1);
        return;
      }
      x.current = setTimeout(() => {
        C(D, 1, !1);
      }, 350);
    },
    [C]
  ), S = it(() => {
    const D = l.current;
    !D || d || !g || D.scrollTop + D.clientHeight >= D.scrollHeight - 100 && (a.trim() ? C(a, y + 1, !0) : (p(!0), Ks(n, y + 1, 30).then((W) => {
      const K = W.data.data.filter((Z) => Z.type !== "ad");
      f((Z) => [...Z, ...K]), m(y + 1), b(W.data.has_next);
    }).catch(() => {
    }).finally(() => p(!1))));
  }, [d, g, a, y, C, n]), F = it(
    (D) => {
      ua(t, D);
    },
    [t]
  );
  return !e || !r ? null : Je(
    /* @__PURE__ */ v(
      "div",
      {
        ref: i,
        style: {
          position: "fixed",
          left: r.right + 8,
          top: r.top,
          background: s.panelBg,
          border: `1px solid ${s.border}`,
          borderRadius: s.panelBorderRadius,
          padding: 0,
          zIndex: 99999,
          boxShadow: s.panelShadow,
          width: 300,
          maxHeight: `calc(100vh - ${r.top + 20}px)`,
          display: "flex",
          flexDirection: "column",
          overflow: "hidden"
        },
        onPointerDown: (D) => D.stopPropagation(),
        children: [
          /* @__PURE__ */ v("div", { style: { padding: "10px 12px 6px", flexShrink: 0 }, children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 11,
                  fontWeight: 600,
                  color: s.text,
                  marginBottom: 8
                },
                children: "GIFs"
              }
            ),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                placeholder: "Search KLIPY",
                value: a,
                onChange: (D) => z(D.target.value),
                style: {
                  width: "100%",
                  padding: "5px 8px",
                  border: `1px solid ${s.border}`,
                  borderRadius: s.controlBorderRadius,
                  background: s.controlBg,
                  color: s.text,
                  fontSize: 11,
                  outline: "none",
                  boxSizing: "border-box"
                }
              }
            )
          ] }),
          /* @__PURE__ */ v(
            "div",
            {
              ref: l,
              onScroll: S,
              style: {
                flex: 1,
                overflowY: "auto",
                overflowX: "hidden",
                padding: "4px 12px",
                minHeight: 200
              },
              children: [
                h.length === 0 && !d ? /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      color: s.textDisabled,
                      fontSize: 11,
                      textAlign: "center",
                      padding: 20
                    },
                    children: a.trim() ? "No results" : "Loading..."
                  }
                ) : /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      display: "grid",
                      gridTemplateColumns: "repeat(3, 1fr)",
                      gap: 4
                    },
                    children: h.map((D) => /* @__PURE__ */ u(
                      ah,
                      {
                        item: D,
                        onClick: () => F(D),
                        engine: t,
                        theme: s
                      },
                      D.id
                    ))
                  }
                ),
                d && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      color: s.textMuted,
                      fontSize: 10,
                      textAlign: "center",
                      padding: 12
                    },
                    children: "Loading..."
                  }
                )
              ]
            }
          ),
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                borderTop: `1px solid ${s.border}`,
                padding: "6px 12px",
                fontSize: 9,
                color: s.textMuted,
                textAlign: "center",
                flexShrink: 0
              },
              children: "Powered by KLIPY"
            }
          )
        ]
      }
    ),
    document.body
  );
}
function ah({
  item: t,
  onClick: e,
  engine: o,
  theme: r
}) {
  const n = t.file.sm.webp, s = n.width / n.height, i = it(
    (l) => {
      l.dataTransfer.setData(Pn, JSON.stringify(t)), l.dataTransfer.effectAllowed = "copy";
    },
    [t]
  );
  return /* @__PURE__ */ u(
    "button",
    {
      title: t.title,
      onClick: e,
      draggable: !0,
      onDragStart: i,
      style: {
        border: `1px solid ${r.border}`,
        borderRadius: r.controlBorderRadius,
        background: r.controlBg,
        cursor: "grab",
        padding: 0,
        overflow: "hidden",
        aspectRatio: s > 1.5 ? "16/9" : s < 0.7 ? "3/4" : "1"
      },
      children: /* @__PURE__ */ u(
        "img",
        {
          src: n.url,
          alt: t.title,
          loading: "lazy",
          style: {
            width: "100%",
            height: "100%",
            objectFit: "cover",
            display: "block"
          }
        }
      )
    }
  );
}
function lh({
  nodes: t,
  onSave: e,
  onCancel: o
}) {
  const [r, n] = tt(""), s = at(null), i = at(null);
  mt(() => {
    var f;
    (f = s.current) == null || f.focus();
  }, []);
  const l = Kt(() => _n(t, 56), [t]), a = it(() => {
    e(r.trim() || "Untitled");
  }, [r, e]), c = it(
    (f) => {
      f.key === "Enter" ? (f.preventDefault(), a()) : f.key === "Escape" && (f.preventDefault(), o());
    },
    [a, o]
  ), h = it(
    (f) => {
      i.current && !i.current.contains(f.target) && o();
    },
    [o]
  );
  return Je(
    /* @__PURE__ */ u(
      "div",
      {
        onClick: h,
        style: {
          position: "fixed",
          inset: 0,
          zIndex: 99999,
          background: "rgba(0,0,0,0.4)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center"
        },
        children: /* @__PURE__ */ v(
          "div",
          {
            ref: i,
            onPointerDown: (f) => f.stopPropagation(),
            style: {
              background: "#1e1e2e",
              borderRadius: 8,
              border: "1px solid #333",
              boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
              padding: 16,
              width: 280,
              color: "#e0e0e0",
              fontSize: 13,
              fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
            },
            children: [
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    fontSize: 13,
                    fontWeight: 600,
                    marginBottom: 12
                  },
                  children: "Add to Personal Library"
                }
              ),
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    display: "flex",
                    justifyContent: "center",
                    marginBottom: 12,
                    padding: 8,
                    background: "rgba(255,255,255,0.05)",
                    borderRadius: 6,
                    border: "1px solid #333"
                  },
                  children: /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: l } })
                }
              ),
              /* @__PURE__ */ u(
                "input",
                {
                  ref: s,
                  type: "text",
                  value: r,
                  onChange: (f) => n(f.target.value),
                  onKeyDown: c,
                  placeholder: "Item name",
                  style: {
                    width: "100%",
                    padding: "6px 8px",
                    border: "1px solid #333",
                    borderRadius: 4,
                    background: "rgba(255,255,255,0.06)",
                    color: "#e0e0e0",
                    fontSize: 13,
                    outline: "none",
                    boxSizing: "border-box",
                    marginBottom: 12
                  }
                }
              ),
              /* @__PURE__ */ v("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end" }, children: [
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: o,
                    style: {
                      padding: "5px 12px",
                      border: "1px solid #333",
                      borderRadius: 4,
                      background: "transparent",
                      color: "#e0e0e0",
                      cursor: "pointer",
                      fontSize: 12
                    },
                    children: "Cancel"
                  }
                ),
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: a,
                    style: {
                      padding: "5px 12px",
                      border: "none",
                      borderRadius: 4,
                      background: "#6366f1",
                      color: "#fff",
                      cursor: "pointer",
                      fontSize: 12,
                      fontWeight: 500
                    },
                    children: "Save"
                  }
                )
              ] })
            ]
          }
        )
      }
    ),
    document.body
  );
}
function Rn(t) {
  const e = t.trim();
  if (!e.includes("<svg")) return null;
  const o = e.match(/<svg[\s\S]*?<\/svg>/i);
  return o ? o[0] : null;
}
function ch(t) {
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(t)}`;
}
function fa(t, e, o, r) {
  return new Promise((n) => {
    const s = ch(t), i = new Image();
    i.onload = () => {
      let c = i.naturalWidth || 200, h = i.naturalHeight || 200;
      if (c <= 1 || h <= 1) {
        const f = t.match(/viewBox=["']([^"']+)["']/i);
        if (f) {
          const d = f[1].trim().split(/[\s,]+/).map(Number);
          d.length === 4 && d[2] > 0 && d[3] > 0 && (c = d[2], h = d[3]);
        }
      }
      if (c > 400 || h > 400) {
        const f = Math.min(400 / c, 400 / h);
        c = Math.round(c * f), h = Math.round(h * f);
      }
      n({
        id: wt(10),
        type: "image",
        x: e,
        y: o,
        w: c,
        h,
        z: r,
        data: { src: s }
      });
    }, i.onerror = () => n(null), i.src = s;
  });
}
async function dh(t, e, o, r) {
  const { x: n, y: s } = t.screenToCanvas(o, r), i = await fa(e, n, s, t.nextZ());
  i && (t.addNode(i), t.select(i.id));
}
const qs = {
  number: "#3b82f6",
  // blue
  string: "#10b981",
  // green
  boolean: "#f59e0b",
  // amber
  object: "#8b5cf6",
  // purple
  any: "#6b7280",
  // gray
  signal: "#ef4444"
  // red
}, hh = ce(function({
  node: e,
  zoom: o,
  showHandles: r = !0,
  measuredHeights: n,
  onHandlePointerDown: s,
  onRotateStart: i
}) {
  const l = e.h === "auto" ? (n == null ? void 0 : n[e.id]) ?? 100 : e.h, a = e.rotation || 0, c = e.x + e.w / 2, h = e.y + l / 2, f = 8 / o, d = f / 2, p = 25 / o, y = !!e.locked, m = [
    { pos: "nw", cx: e.x, cy: e.y },
    { pos: "n", cx: e.x + e.w / 2, cy: e.y },
    { pos: "ne", cx: e.x + e.w, cy: e.y },
    { pos: "e", cx: e.x + e.w, cy: e.y + l / 2 },
    { pos: "se", cx: e.x + e.w, cy: e.y + l },
    { pos: "s", cx: e.x + e.w / 2, cy: e.y + l },
    { pos: "sw", cx: e.x, cy: e.y + l },
    { pos: "w", cx: e.x, cy: e.y + l / 2 }
  ];
  return /* @__PURE__ */ v("g", { transform: `rotate(${a}, ${c}, ${h})`, children: [
    /* @__PURE__ */ u(
      "rect",
      {
        x: e.x,
        y: e.y,
        width: e.w,
        height: l,
        fill: "none",
        stroke: y ? "#f59e0b" : "#3b82f6",
        strokeWidth: 1.5 / o,
        strokeDasharray: `${4 / o} ${3 / o}`
      }
    ),
    y && (() => {
      const g = 16 / o, b = e.x + e.w - g - 4 / o, x = e.y - g - 4 / o;
      return /* @__PURE__ */ v("g", { transform: `translate(${b}, ${x})`, children: [
        /* @__PURE__ */ u(
          "rect",
          {
            x: 0,
            y: 0,
            width: g,
            height: g,
            rx: 3 / o,
            fill: "#f59e0b"
          }
        ),
        /* @__PURE__ */ v("g", { transform: `scale(${g / 24})`, children: [
          /* @__PURE__ */ u("rect", { x: "6", y: "11", width: "12", height: "9", rx: "1", fill: "white" }),
          /* @__PURE__ */ u("path", { d: "M8 11V7a4 4 0 0 1 8 0v4", fill: "none", stroke: "white", strokeWidth: "2", strokeLinecap: "round" })
        ] })
      ] });
    })(),
    r && !y && m.map(({ pos: g, cx: b, cy: x }) => /* @__PURE__ */ u(
      "rect",
      {
        x: b - d,
        y: x - d,
        width: f,
        height: f,
        fill: "white",
        stroke: "#3b82f6",
        strokeWidth: 1.5 / o,
        style: {
          cursor: Fr(g, a),
          pointerEvents: "auto"
        },
        onPointerDown: (C) => {
          C.stopPropagation(), s == null || s(e.id, g, C);
        }
      },
      g
    )),
    r && !y && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u(
        "line",
        {
          x1: e.x + e.w / 2,
          y1: e.y,
          x2: e.x + e.w / 2,
          y2: e.y - p,
          stroke: "#3b82f6",
          strokeWidth: 1.5 / o
        }
      ),
      /* @__PURE__ */ u(
        "rect",
        {
          x: e.x + e.w / 2 - d,
          y: e.y - p - d,
          width: f,
          height: f,
          rx: 1.5 / o,
          transform: `rotate(45, ${e.x + e.w / 2}, ${e.y - p})`,
          fill: "white",
          stroke: "#3b82f6",
          strokeWidth: 1.5 / o,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (g) => {
            g.stopPropagation(), i == null || i(e.id, g);
          }
        }
      )
    ] })
  ] });
}), uh = ce(function({
  edge: e,
  fromNode: o,
  toNode: r,
  viewport: n,
  selection: s,
  measuredHeights: i,
  registry: l,
  onEdgeEndpointDown: a,
  onKinkHandleDown: c,
  edgeReconnect: h,
  eraserMarkedIds: f,
  cycleNodeIds: d
}) {
  const p = e.data.edgeType || "bezier";
  let y, m;
  if (l && e.data.sourcePort) {
    const ht = l.get(o.type);
    ht != null && ht.ports && (y = er(o, ht.ports, e.data.sourcePort, n.zoom, i) ?? void 0);
  }
  if (l && e.data.targetPort) {
    const ht = l.get(r.type);
    ht != null && ht.ports && (m = er(r, ht.ports, e.data.targetPort, n.zoom, i) ?? void 0);
  }
  const g = je(
    o,
    r,
    p,
    i,
    e.data.sourceHandle,
    e.data.targetHandle,
    e.data.midpointOffset,
    e.data.curveOffset,
    y,
    m
  ), { path: b, x1: x, y1: C, x2: z, y2: S, labelX: F, labelY: D, arrowAngle: W, tailAngle: K, kinkHandle: Z } = g, dt = s.has(e.id), X = e.data.strokeWidth, rt = e.data.style === "dashed" ? `${8 * X},${4 * X}` : e.data.style === "dotted" ? `${2 * X},${3 * X}` : void 0, J = Math.max(8, X * 3), U = e.data.arrowHeadSize ?? J, R = e.data.arrowTailSize ?? J, O = e.data.animated, _ = f == null ? void 0 : f.has(e.id), G = (h == null ? void 0 : h.edgeId) === e.id, V = !!(d && d.size > 0 && e.data.sourcePort && e.data.targetPort && d.has(e.data.fromId) && d.has(e.data.toId)), Y = V ? "#ef4444" : e.data.color, $ = e.data.roughness ?? 0, nt = Kt(() => $ <= 0 ? null : {
    stroke: Y,
    roughness: $,
    strokeWidth: X,
    strokeLineDash: e.data.style === "dashed" ? [8, 4] : e.data.style === "dotted" ? [2, 2] : void 0,
    seed: e.id
  }, [Y, $, X, e.data.style, e.id]);
  let ot = null, yt = null, ft = null;
  nt && (ot = ln(b, nt), e.data.arrowHead === "arrow" && (yt = ln(Do(z, S, W, U), { ...nt, strokeLineDash: void 0 })), e.data.arrowTail === "arrow" && (ft = ln(Do(x, C, K, R), { ...nt, strokeLineDash: void 0 })));
  const et = Kt(
    () => ({ animation: "edge-cycle-pulse 1.5s ease-in-out infinite" }),
    []
  ), Pt = Kt(() => {
    if (!O) return;
    const ht = e.data.animatedDirection === "reverse" ? "edge-flow-reverse" : e.data.animatedDirection === "both" ? "edge-flow-both" : "edge-flow", Dt = e.data.animatedDirection === "both" ? "2s" : "1s";
    return { animation: `${ht} ${Dt} linear infinite` };
  }, [O, e.data.animatedDirection]), zt = Kt(
    () => ({ animation: "edge-cycle-pulse 1.5s ease-in-out infinite, edge-flow 1s linear infinite" }),
    []
  ), gt = Kt(
    () => _ ? { filter: "saturate(0)" } : void 0,
    [_]
  );
  return /* @__PURE__ */ v("g", { opacity: G ? 0.15 : _ ? 0.25 : void 0, style: gt, children: [
    V && /* @__PURE__ */ u(
      "path",
      {
        d: b,
        stroke: "#ef4444",
        strokeWidth: X + 6 / n.zoom,
        strokeLinecap: "round",
        fill: "none",
        opacity: 0.25,
        style: et
      }
    ),
    dt && /* @__PURE__ */ u(
      "path",
      {
        d: b,
        stroke: "#3b82f6",
        strokeWidth: X + 6 / n.zoom,
        strokeLinecap: "round",
        fill: "none",
        opacity: 0.3
      }
    ),
    ot ? ot.map((ht, Dt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeDasharray: ht.strokeDasharray,
        strokeLinecap: "round",
        fill: ht.fill ?? "none",
        style: O ? Pt : void 0
      },
      Dt
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: b,
        stroke: Y,
        strokeWidth: X,
        strokeDasharray: O ? "12,8" : V ? `${6 * X},${4 * X}` : rt,
        strokeLinecap: "round",
        fill: "none",
        style: V ? zt : Pt
      }
    ),
    e.data.arrowHead === "arrow" && (yt ? yt.map((ht, Dt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeLinecap: "round",
        strokeLinejoin: "round",
        fill: ht.fill ?? "none"
      },
      `ah${Dt}`
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: Do(z, S, W, U),
        fill: "none",
        stroke: Y,
        strokeWidth: X,
        strokeLinecap: "round",
        strokeLinejoin: "round"
      }
    )),
    e.data.arrowHead === "filled" && /* @__PURE__ */ u(
      "path",
      {
        d: zr(z, S, W, U),
        fill: Y,
        stroke: "none"
      }
    ),
    e.data.arrowHead === "dot" && /* @__PURE__ */ u(
      "circle",
      {
        cx: z,
        cy: S,
        r: U * 0.25,
        fill: Y
      }
    ),
    e.data.arrowTail === "arrow" && (ft ? ft.map((ht, Dt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeLinecap: "round",
        strokeLinejoin: "round",
        fill: ht.fill ?? "none"
      },
      `at${Dt}`
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: Do(x, C, K, R),
        fill: "none",
        stroke: Y,
        strokeWidth: X,
        strokeLinecap: "round",
        strokeLinejoin: "round"
      }
    )),
    e.data.arrowTail === "filled" && /* @__PURE__ */ u(
      "path",
      {
        d: zr(x, C, K, R),
        fill: Y,
        stroke: "none"
      }
    ),
    e.data.arrowTail === "dot" && /* @__PURE__ */ u(
      "circle",
      {
        cx: x,
        cy: C,
        r: R * 0.25,
        fill: Y
      }
    ),
    e.data.label && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u(
        "rect",
        {
          x: F - (e.data.label.length * 3.5 + 6) / n.zoom,
          y: D - 8 / n.zoom,
          width: (e.data.label.length * 7 + 12) / n.zoom,
          height: 16 / n.zoom,
          fill: "white",
          rx: 4 / n.zoom,
          opacity: 0.9
        }
      ),
      /* @__PURE__ */ u(
        "text",
        {
          x: F,
          y: D + 4 / n.zoom,
          fill: Y,
          fontSize: 12 / n.zoom,
          textAnchor: "middle",
          style: { pointerEvents: "none" },
          children: e.data.label
        }
      )
    ] }),
    dt && !G && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u(
        "circle",
        {
          cx: x,
          cy: C,
          r: 5 / n.zoom,
          fill: "#3b82f6",
          stroke: "white",
          strokeWidth: 1.5 / n.zoom,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (ht) => {
            ht.stopPropagation(), a == null || a(e.id, "source", ht);
          }
        }
      ),
      /* @__PURE__ */ u(
        "circle",
        {
          cx: z,
          cy: S,
          r: 5 / n.zoom,
          fill: "#3b82f6",
          stroke: "white",
          strokeWidth: 1.5 / n.zoom,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (ht) => {
            ht.stopPropagation(), a == null || a(e.id, "target", ht);
          }
        }
      )
    ] }),
    dt && !G && Z && /* @__PURE__ */ u(
      "circle",
      {
        cx: Z.x,
        cy: Z.y,
        r: 5 / n.zoom,
        fill: "white",
        stroke: "#3b82f6",
        strokeWidth: 1.5 / n.zoom,
        style: {
          cursor: Z.axis === "xy" ? "move" : Z.axis === "x" ? "ew-resize" : "ns-resize",
          pointerEvents: "auto"
        },
        onPointerDown: (ht) => {
          ht.stopPropagation(), c == null || c(e.id, Z.axis, Z.min, Z.max, ht);
        }
      }
    )
  ] });
});
function fh({
  nodes: t,
  viewport: e,
  selection: o,
  measuredHeights: r,
  activeStroke: n,
  shapePreview: s,
  shapePreviewStyle: i,
  onResizeHandleDown: l,
  onRotateStart: a,
  onConnectionHandleDown: c,
  onEdgeEndpointDown: h,
  onKinkHandleDown: f,
  edgePreview: d,
  edgeReconnect: p,
  eraserMarkedIds: y,
  eraserTrail: m,
  laserTrail: g,
  mode: b,
  hoveredNodeId: x,
  registry: C,
  onPortHandleDown: z,
  cycleNodeIds: S,
  containerTypes: F,
  alignGuides: D
}) {
  const W = `translate(${e.x}, ${e.y}) scale(${e.zoom})`, K = t.filter(
    (X) => X.type !== "edge" && X.type !== "content" && X.type !== "image"
  ), Z = t.filter((X) => X.type === "edge").sort((X, rt) => X.z - rt.z), dt = Kt(() => new Map(t.map((X) => [X.id, X])), [t]);
  return /* @__PURE__ */ u(
    "svg",
    {
      "data-sb-overlay": !0,
      style: {
        position: "absolute",
        inset: 0,
        width: "100%",
        height: "100%",
        overflow: "visible",
        pointerEvents: "none"
      },
      children: /* @__PURE__ */ v("g", { transform: W, children: [
        Z.map((X) => {
          const rt = dt.get(X.data.fromId), J = dt.get(X.data.toId);
          return !rt || !J ? null : /* @__PURE__ */ u(
            uh,
            {
              edge: X,
              fromNode: rt,
              toNode: J,
              viewport: e,
              selection: o,
              measuredHeights: r,
              registry: C,
              onEdgeEndpointDown: h,
              onKinkHandleDown: f,
              edgeReconnect: p,
              eraserMarkedIds: y,
              cycleNodeIds: S
            },
            X.id
          );
        }),
        (() => {
          var G, V;
          const X = !!d || !!p, rt = (d == null ? void 0 : d.cursorX) ?? (p == null ? void 0 : p.cursorX) ?? 0, J = (d == null ? void 0 : d.cursorY) ?? (p == null ? void 0 : p.cursorY) ?? 0, U = (d == null ? void 0 : d.fromNode.id) ?? (p == null ? void 0 : p.anchorNodeId) ?? null;
          let R = null, O = null;
          const _ = /* @__PURE__ */ new Set();
          if (X) {
            let Y = 1 / 0, $ = !1;
            const nt = 50 / e.zoom;
            for (const ot of t) {
              if (ot.type === "edge" || ot.id === U || (V = (G = C == null ? void 0 : C.get(ot.type)) == null ? void 0 : G.ports) != null && V.length) continue;
              const yt = ot.h === "auto" ? (r == null ? void 0 : r[ot.id]) ?? 100 : ot.h, ft = ot.w * 0.2, et = yt * 0.2;
              rt >= ot.x - ft && rt <= ot.x + ot.w + ft && J >= ot.y - et && J <= ot.y + yt + et && _.add(ot.id);
              const Pt = Wo(ot, r), zt = F ? F.has(ot.type) : ot.type === "frame";
              for (const gt of Pt) {
                const ht = Math.hypot(gt.x - rt, gt.y - J);
                ht >= nt || zt && !$ && R || (!zt && $ || ht < Y) && (Y = ht, $ = zt, R = ot.id, O = gt.side);
              }
            }
          }
          return t.filter((Y) => {
            var $, nt;
            return Y.type === "edge" || (nt = ($ = C == null ? void 0 : C.get(Y.type)) == null ? void 0 : $.ports) != null && nt.length ? !1 : o.size <= 1 && o.has(Y.id) || X && (Y.id === U || _.has(Y.id));
          }).map((Y) => {
            const $ = Wo(Y, r), nt = 4 / e.zoom, ot = 26 / e.zoom, yt = Y.rotation || 0, ft = Y.h === "auto" ? (r == null ? void 0 : r[Y.id]) ?? 100 : Y.h, et = Y.x + Y.w / 2, Pt = Y.y + ft / 2, zt = d && d.fromNode.id === Y.id || p && p.anchorNodeId === Y.id, gt = o.has(Y.id) && !X;
            return /* @__PURE__ */ u("g", { transform: yt ? `rotate(${yt}, ${et}, ${Pt})` : void 0, children: $.map(({ side: ht }) => {
              const Dt = {
                top: [Y.x + Y.w / 2, Y.y],
                bottom: [Y.x + Y.w / 2, Y.y + ft],
                left: [Y.x, Y.y + ft / 2],
                right: [Y.x + Y.w, Y.y + ft / 2]
              }, [It, Rt] = Dt[ht], Qt = ht === "top" && o.has(Y.id) ? 42 / e.zoom : ot;
              let Jt = It, qt = Rt;
              switch (ht) {
                case "top":
                  qt = Rt - Qt;
                  break;
                case "bottom":
                  qt = Rt + Qt;
                  break;
                case "left":
                  Jt = It - Qt;
                  break;
                case "right":
                  Jt = It + Qt;
                  break;
              }
              const $t = X && R === Y.id && O === ht;
              return /* @__PURE__ */ u(
                "circle",
                {
                  cx: Jt,
                  cy: qt,
                  r: $t ? 5 / e.zoom : nt,
                  fill: zt || $t ? "#3b82f6" : "white",
                  stroke: $t ? "white" : X && !zt ? "#3b82f6" : "#94a3b8",
                  strokeWidth: 1.5 / e.zoom,
                  opacity: $t || X && !zt ? 1 : 0.8,
                  style: {
                    cursor: gt ? "crosshair" : "default",
                    pointerEvents: gt ? "auto" : "none"
                  },
                  onPointerDown: gt ? (he) => {
                    he.stopPropagation(), c == null || c(Y.id, ht, he);
                  } : void 0
                },
                `ch-${Y.id}-${ht}`
              );
            }) }, `conn-${Y.id}`);
          });
        })(),
        C && (() => {
          var G;
          const X = !!d || !!p, rt = (d == null ? void 0 : d.cursorX) ?? (p == null ? void 0 : p.cursorX) ?? 0, J = (d == null ? void 0 : d.cursorY) ?? (p == null ? void 0 : p.cursorY) ?? 0, U = (d == null ? void 0 : d.fromNode.id) ?? null, R = (d == null ? void 0 : d.sourceDirection) === "output" ? "input" : (d == null ? void 0 : d.sourceDirection) === "input" ? "output" : null;
          let O = null, _ = null;
          if (X && R) {
            let V = 40 / e.zoom;
            for (const Y of t) {
              if (Y.type === "edge" || Y.id === U) continue;
              const $ = C.get(Y.type);
              if (!((G = $ == null ? void 0 : $.ports) != null && G.length)) continue;
              const nt = Y.h === "auto" ? (r == null ? void 0 : r[Y.id]) ?? 100 : Y.h, ot = 14 / e.zoom, yt = $.ports.filter((ft) => ft.direction === R);
              for (let ft = 0; ft < yt.length; ft++) {
                const et = yt[ft], Pt = Y.y + nt / (yt.length + 1) * (ft + 1), zt = et.direction === "input" ? Y.x - ot : Y.x + Y.w + ot, gt = Math.hypot(zt - rt, Pt - J);
                gt < V && (V = gt, O = Y.id, _ = et.id);
              }
            }
          }
          return t.filter((V) => {
            var $;
            if (V.type === "edge") return !1;
            const Y = C.get(V.type);
            return !!(($ = Y == null ? void 0 : Y.ports) != null && $.length);
          }).map((V) => {
            const $ = C.get(V.type).ports, nt = V.h === "auto" ? (r == null ? void 0 : r[V.id]) ?? 100 : V.h, ot = V.rotation || 0, yt = V.x + V.w / 2, ft = V.y + nt / 2, et = 6 / e.zoom, Pt = 14 / e.zoom, zt = $.filter((Rt) => Rt.direction === "input"), gt = $.filter((Rt) => Rt.direction === "output"), ht = !X, Dt = (Rt, Qt, Jt, qt) => {
              const $t = V.y + nt / (Jt.length + 1) * (Qt + 1), he = qt === "input" ? V.x - Pt : V.x + V.w + Pt, Re = qs[Rt.dataType] || qs.any, _t = O === V.id && _ === Rt.id, Ae = _t ? 8 / e.zoom : et, to = qt === "input" ? V.x : V.x + V.w, bo = qt === "input" ? he - et - 4 / e.zoom : he + et + 4 / e.zoom;
              return /* @__PURE__ */ v("g", { children: [
                /* @__PURE__ */ u(
                  "line",
                  {
                    x1: he,
                    y1: $t,
                    x2: to,
                    y2: $t,
                    stroke: Re,
                    strokeWidth: 1.5 / e.zoom,
                    opacity: 0.4,
                    style: { pointerEvents: "none" }
                  }
                ),
                _t && /* @__PURE__ */ u(
                  "circle",
                  {
                    cx: he,
                    cy: $t,
                    r: 12 / e.zoom,
                    fill: "none",
                    stroke: "white",
                    strokeWidth: 1.5 / e.zoom,
                    opacity: 0.3,
                    style: { pointerEvents: "none" }
                  }
                ),
                /* @__PURE__ */ u(
                  "circle",
                  {
                    cx: he,
                    cy: $t,
                    r: Ae,
                    fill: _t ? "white" : Re,
                    stroke: _t ? Re : "#1a1a2e",
                    strokeWidth: 2 / e.zoom,
                    style: {
                      cursor: ht ? "crosshair" : "default",
                      pointerEvents: ht ? "auto" : "none",
                      transition: "r 0.1s, fill 0.1s"
                    },
                    onPointerDown: ht ? (ue) => {
                      ue.stopPropagation(), z == null || z(V.id, Rt.id, qt, ue);
                    } : void 0
                  }
                ),
                (() => {
                  const ue = Rt.label || Rt.id, ve = 9 / e.zoom, eo = 5 / e.zoom, oo = 2.5 / e.zoom, fe = ue.length * ve * 0.62 + eo * 2, Se = ve + oo * 2, ge = qt === "input" ? bo - fe : bo, Be = $t - Se / 2, k = Se / 2, st = _t ? Re : "#1a1a2e", Yt = _t ? Re : "#2a2a40", ne = _t ? "#fff" : "#94a3b8";
                  return /* @__PURE__ */ v("g", { style: { pointerEvents: "none" }, children: [
                    /* @__PURE__ */ u(
                      "rect",
                      {
                        x: ge,
                        y: Be,
                        width: fe,
                        height: Se,
                        rx: k,
                        ry: k,
                        fill: st,
                        fillOpacity: _t ? 0.9 : 0.85,
                        stroke: Yt,
                        strokeWidth: 1 / e.zoom
                      }
                    ),
                    /* @__PURE__ */ u(
                      "text",
                      {
                        x: ge + fe / 2,
                        y: $t + ve * 0.35,
                        fill: ne,
                        fontSize: ve,
                        fontWeight: 600,
                        fontFamily: "'Inter', system-ui, sans-serif",
                        textAnchor: "middle",
                        style: { userSelect: "none" },
                        children: ue
                      }
                    )
                  ] });
                })()
              ] }, `port-${V.id}-${Rt.id}`);
            }, It = S == null ? void 0 : S.has(V.id);
            return /* @__PURE__ */ v("g", { transform: ot ? `rotate(${ot}, ${yt}, ${ft})` : void 0, children: [
              zt.map((Rt, Qt) => Dt(Rt, Qt, zt, "input")),
              gt.map((Rt, Qt) => Dt(Rt, Qt, gt, "output")),
              It && (() => {
                const Rt = 10 / e.zoom, Qt = V.x + V.w + Rt * 0.3, Jt = V.y - Rt * 0.3;
                return /* @__PURE__ */ v("g", { style: { pointerEvents: "none" }, children: [
                  /* @__PURE__ */ u(
                    "circle",
                    {
                      cx: Qt,
                      cy: Jt,
                      r: Rt,
                      fill: "#ef4444",
                      stroke: "#1a1a2e",
                      strokeWidth: 2 / e.zoom
                    }
                  ),
                  /* @__PURE__ */ u(
                    "text",
                    {
                      x: Qt,
                      y: Jt + 4 / e.zoom,
                      fill: "white",
                      fontSize: 12 / e.zoom,
                      fontWeight: 800,
                      textAnchor: "middle",
                      style: { pointerEvents: "none", userSelect: "none" },
                      children: "!"
                    }
                  )
                ] });
              })()
            ] }, `ports-${V.id}`);
          });
        })(),
        d && (() => {
          let X, rt;
          if (d.sourcePort && C) {
            const J = d.fromNode, U = C.get(J.type), R = U != null && U.ports ? er(J, U.ports, d.sourcePort, e.zoom, r) : null;
            if (R)
              X = R.x, rt = R.y;
            else {
              const O = $r(J, d.cursorX, d.cursorY, r);
              X = O.x, rt = O.y;
            }
          } else if (d.sourceHandle) {
            const J = d.fromNode, U = J.h === "auto" ? (r == null ? void 0 : r[J.id]) ?? 100 : J.h, R = {
              top: [J.x + J.w / 2, J.y],
              bottom: [J.x + J.w / 2, J.y + U],
              left: [J.x, J.y + U / 2],
              right: [J.x + J.w, J.y + U / 2]
            }, O = d.sourceHandle, _ = O === "top" ? 42 / e.zoom : 26 / e.zoom, [G, V] = R[O];
            let Y = G, $ = V;
            switch (O) {
              case "top":
                $ = V - _;
                break;
              case "bottom":
                $ = V + _;
                break;
              case "left":
                Y = G - _;
                break;
              case "right":
                Y = G + _;
                break;
            }
            if (J.rotation) {
              const nt = J.x + J.w / 2, ot = J.y + U / 2, yt = J.rotation * Math.PI / 180, ft = Math.cos(yt), et = Math.sin(yt), Pt = Y - nt, zt = $ - ot;
              X = nt + Pt * ft - zt * et, rt = ot + Pt * et + zt * ft;
            } else
              X = Y, rt = $;
          } else {
            const J = $r(d.fromNode, d.cursorX, d.cursorY, r);
            X = J.x, rt = J.y;
          }
          return /* @__PURE__ */ u(
            "line",
            {
              x1: X,
              y1: rt,
              x2: d.cursorX,
              y2: d.cursorY,
              stroke: "#3b82f6",
              strokeWidth: 2 / e.zoom,
              strokeDasharray: `${4 / e.zoom}`,
              strokeLinecap: "round"
            }
          );
        })(),
        p && (() => {
          const X = dt.get(p.anchorNodeId);
          if (!X) return null;
          let rt, J;
          if (p.anchorHandle) {
            const U = X.h === "auto" ? (r == null ? void 0 : r[X.id]) ?? 100 : X.h, R = {
              top: [X.x + X.w / 2, X.y],
              bottom: [X.x + X.w / 2, X.y + U],
              left: [X.x, X.y + U / 2],
              right: [X.x + X.w, X.y + U / 2]
            }, O = p.anchorHandle, _ = O === "top" ? 42 / e.zoom : 26 / e.zoom, [G, V] = R[O];
            let Y = G, $ = V;
            switch (O) {
              case "top":
                $ = V - _;
                break;
              case "bottom":
                $ = V + _;
                break;
              case "left":
                Y = G - _;
                break;
              case "right":
                Y = G + _;
                break;
            }
            if (X.rotation) {
              const nt = X.x + X.w / 2, ot = X.y + U / 2, yt = X.rotation * Math.PI / 180, ft = Math.cos(yt), et = Math.sin(yt), Pt = Y - nt, zt = $ - ot;
              rt = nt + Pt * ft - zt * et, J = ot + Pt * et + zt * ft;
            } else
              rt = Y, J = $;
          } else {
            const U = $r(X, p.cursorX, p.cursorY, r);
            rt = U.x, J = U.y;
          }
          return /* @__PURE__ */ u(
            "line",
            {
              x1: rt,
              y1: J,
              x2: p.cursorX,
              y2: p.cursorY,
              stroke: "#3b82f6",
              strokeWidth: 2 / e.zoom,
              strokeDasharray: `${4 / e.zoom}`,
              strokeLinecap: "round"
            }
          );
        })(),
        o.size === 1 && K.filter((X) => o.has(X.id)).map((X) => /* @__PURE__ */ u(
          hh,
          {
            node: X,
            zoom: e.zoom,
            showHandles: o.size === 1,
            measuredHeights: r,
            onHandlePointerDown: l,
            onRotateStart: a
          },
          `sel-${X.id}`
        )),
        n && n.points.length > 1 && (() => {
          if (n.strokeStyle === "dashed" || n.strokeStyle === "dotted") {
            const rt = n.points, J = ["M", rt[0][0], rt[0][1]];
            for (let O = 1; O < rt.length; O++) {
              const [_, G] = rt[O], [V, Y] = rt[O - 1];
              J.push("Q", V, Y, (V + _) / 2, (Y + G) / 2);
            }
            const U = rt[rt.length - 1];
            J.push("L", U[0], U[1]);
            const R = $e(n.strokeStyle);
            return /* @__PURE__ */ u(
              "path",
              {
                d: J.join(" "),
                fill: "none",
                stroke: n.color,
                strokeWidth: n.width,
                strokeDasharray: R == null ? void 0 : R.map((O) => O * Math.max(n.width, 1)).join(" "),
                strokeLinecap: "round",
                strokeLinejoin: "round"
              }
            );
          }
          return /* @__PURE__ */ u(
            "path",
            {
              d: Yn(n.points, {
                size: n.width
              }),
              fill: n.color
            }
          );
        })(),
        s && i && (() => {
          const X = Math.min(s.startX, s.endX), rt = Math.min(s.startY, s.endY), J = Math.abs(s.endX - s.startX), U = Math.abs(s.endY - s.startY);
          if (J < 2 && U < 2) return null;
          const R = i, O = R.shapeType || "rect";
          if (O === "ellipse")
            return /* @__PURE__ */ u(
              "ellipse",
              {
                cx: X + J / 2,
                cy: rt + U / 2,
                rx: J / 2,
                ry: U / 2,
                stroke: R.stroke,
                strokeWidth: R.strokeWidth,
                fill: "none",
                strokeDasharray: "4"
              }
            );
          if (O === "diamond")
            return /* @__PURE__ */ u(
              "polygon",
              {
                points: `${X + J / 2},${rt} ${X + J},${rt + U / 2} ${X + J / 2},${rt + U} ${X},${rt + U / 2}`,
                stroke: R.stroke,
                strokeWidth: R.strokeWidth,
                fill: "none",
                strokeDasharray: "4"
              }
            );
          if (O === "line" || O === "arrow") {
            const _ = s.startX, G = s.startY, V = s.endX, Y = s.endY;
            return /* @__PURE__ */ v(ut, { children: [
              /* @__PURE__ */ u(
                "line",
                {
                  x1: _,
                  y1: G,
                  x2: V,
                  y2: Y,
                  stroke: R.stroke,
                  strokeWidth: R.strokeWidth,
                  strokeDasharray: "4"
                }
              ),
              O === "arrow" && (() => {
                const $ = Math.atan2(Y - G, V - _), nt = Math.max(12, R.strokeWidth * 4), ot = Math.PI / 6, yt = V - nt * Math.cos($ - ot), ft = Y - nt * Math.sin($ - ot), et = V - nt * Math.cos($ + ot), Pt = Y - nt * Math.sin($ + ot);
                return /* @__PURE__ */ u(
                  "polyline",
                  {
                    points: `${yt},${ft} ${V},${Y} ${et},${Pt}`,
                    stroke: R.stroke,
                    strokeWidth: R.strokeWidth,
                    fill: "none",
                    strokeDasharray: "4"
                  }
                );
              })()
            ] });
          }
          return /* @__PURE__ */ u(
            "rect",
            {
              x: X,
              y: rt,
              width: J,
              height: U,
              stroke: R.stroke,
              strokeWidth: R.strokeWidth,
              fill: "none",
              strokeDasharray: "4"
            }
          );
        })(),
        m && m.length > 1 && (() => {
          const X = performance.now(), rt = 400, J = 6 / e.zoom, U = [`M${m[0][0]},${m[0][1]}`];
          if (m.length === 2)
            U.push(`L${m[1][0]},${m[1][1]}`);
          else {
            for (let nt = 0; nt < m.length - 1; nt++) {
              const ot = (m[nt][0] + m[nt + 1][0]) / 2, yt = (m[nt][1] + m[nt + 1][1]) / 2;
              U.push(`Q${m[nt][0]},${m[nt][1]},${ot},${yt}`);
            }
            const $ = m[m.length - 1];
            U.push(`L${$[0]},${$[1]}`);
          }
          const R = U.join(" "), O = (X - m[m.length - 1][2]) / rt, _ = (X - m[0][2]) / rt, G = Math.max(0, 0.85 * (1 - O)), V = Math.max(0, 0.85 * (1 - _)), Y = (G + V) / 2;
          return Y <= 0 ? null : /* @__PURE__ */ v(ut, { children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: R,
                fill: "none",
                stroke: "#9ca3af",
                strokeWidth: J * 3,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: Y * 0.35
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: R,
                fill: "none",
                stroke: "#d1d5db",
                strokeWidth: J,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: Y
              }
            )
          ] });
        })(),
        g && g.length > 1 && (() => {
          const X = performance.now(), rt = 1560, J = 6 / e.zoom, U = [];
          let R = !1, O = !1;
          for (let yt = 0; yt < g.length; yt++) {
            const ft = g[yt];
            if (isNaN(ft[0])) {
              R = !1, O = !1;
              continue;
            }
            if (!R)
              U.push(`M${ft[0]},${ft[1]}`), R = !0, O = !0;
            else if (O) {
              const et = yt + 1 < g.length && !isNaN(g[yt + 1][0]) ? g[yt + 1] : null;
              if (et) {
                const Pt = (ft[0] + et[0]) / 2, zt = (ft[1] + et[1]) / 2;
                U.push(`Q${ft[0]},${ft[1]},${Pt},${zt}`);
              } else
                U.push(`L${ft[0]},${ft[1]}`);
            }
          }
          if (U.length === 0) return null;
          const _ = U.join(" "), G = g.filter((yt) => !isNaN(yt[0]));
          if (G.length === 0) return null;
          const V = (X - G[G.length - 1][2]) / rt, Y = (X - G[0][2]) / rt, $ = Math.max(0, 0.85 * (1 - V)), nt = Math.max(0, 0.85 * (1 - Y)), ot = ($ + nt) / 2;
          return ot <= 0 ? null : /* @__PURE__ */ v(ut, { children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: _,
                fill: "none",
                stroke: "#ef4444",
                strokeWidth: J * 3,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: ot * 0.35
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: _,
                fill: "none",
                stroke: "#ff6b6b",
                strokeWidth: J,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: ot
              }
            )
          ] });
        })(),
        D && D.length > 0 && D.map((X, rt) => /* @__PURE__ */ u(
          "line",
          {
            x1: X.axis === "x" ? X.position : X.start,
            y1: X.axis === "x" ? X.start : X.position,
            x2: X.axis === "x" ? X.position : X.end,
            y2: X.axis === "x" ? X.end : X.position,
            stroke: "#f472b6",
            strokeWidth: 1 / e.zoom,
            strokeDasharray: `${3 / e.zoom} ${2 / e.zoom}`,
            opacity: 0.8
          },
          `guide-${rt}`
        ))
      ] })
    }
  );
}
function ph({
  x: t,
  y: e,
  sections: o,
  onClose: r
}) {
  const n = at(null);
  mt(() => {
    var y;
    const f = (m) => {
      n.current && !n.current.contains(m.target) && r();
    }, d = (m) => {
      m.key === "Escape" && r();
    }, p = ((y = n.current) == null ? void 0 : y.ownerDocument) ?? document;
    return p.addEventListener("pointerdown", f, !0), p.addEventListener("keydown", d), () => {
      p.removeEventListener("pointerdown", f, !0), p.removeEventListener("keydown", d);
    };
  }, [r]), mt(() => {
    const f = n.current;
    if (!f) return;
    const d = f.getBoundingClientRect(), p = f.ownerDocument.defaultView ?? window;
    let y = t, m = e;
    d.right > p.innerWidth && (y = t - d.width), d.bottom > p.innerHeight && (m = e - d.height), y = Math.max(0, y), m = Math.max(0, m), f.style.left = `${y}px`, f.style.top = `${m}px`;
  }, [t, e]);
  const s = it(
    (f) => {
      f.disabled || (f.action(), r());
    },
    [r]
  ), i = navigator.platform.includes("Mac"), l = i ? "⌘" : "Ctrl+", a = i ? "⌥" : "Alt+", c = i ? "⇧" : "Shift+", h = (f) => f.replace("Mod+", l).replace("Alt+", a).replace("Shift+", c);
  return /* @__PURE__ */ u(
    "div",
    {
      "data-sb-context-menu": !0,
      ref: n,
      onPointerDown: (f) => f.stopPropagation(),
      onContextMenu: (f) => f.preventDefault(),
      style: {
        position: "fixed",
        left: t,
        top: e,
        zIndex: 9999,
        minWidth: 200,
        background: "#1e1e2e",
        borderRadius: 8,
        border: "1px solid #333",
        boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
        padding: "4px 0",
        color: "#e0e0e0",
        fontSize: 13,
        fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
        userSelect: "none"
      },
      children: o.map((f, d) => /* @__PURE__ */ v("div", { children: [
        d > 0 && /* @__PURE__ */ u(
          "div",
          {
            style: {
              height: 1,
              background: "#333",
              margin: "4px 0"
            }
          }
        ),
        f.items.map((p, y) => /* @__PURE__ */ v(
          "div",
          {
            onClick: () => s(p),
            style: {
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              padding: "6px 16px",
              cursor: p.disabled ? "default" : "pointer",
              opacity: p.disabled ? 0.4 : 1,
              color: p.danger ? "#f87171" : "#e0e0e0",
              transition: "background 0.1s"
            },
            onMouseEnter: (m) => {
              p.disabled || (m.currentTarget.style.background = "rgba(255,255,255,0.08)");
            },
            onMouseLeave: (m) => {
              m.currentTarget.style.background = "transparent";
            },
            children: [
              /* @__PURE__ */ v("span", { children: [
                p.checked !== void 0 && /* @__PURE__ */ u("span", { style: { display: "inline-block", width: 16, marginRight: 4 }, children: p.checked ? "✓" : "" }),
                p.label
              ] }),
              p.shortcut && /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    marginLeft: 32,
                    fontSize: 12,
                    color: "#888"
                  },
                  children: h(p.shortcut)
                }
              )
            ]
          },
          y
        ))
      ] }, d))
    }
  );
}
const pa = "sbd-clipboard", yh = "sbd-nodes:";
function ya(t) {
  const e = JSON.stringify(t), o = new TextEncoder().encode(e);
  let r = "";
  for (let n = 0; n < o.length; n++) r += String.fromCharCode(o[n]);
  return btoa(r);
}
function Zs(t) {
  try {
    const e = atob(t), o = new Uint8Array(e.length);
    for (let n = 0; n < e.length; n++) o[n] = e.charCodeAt(n);
    const r = new TextDecoder().decode(o);
    return JSON.parse(r);
  } catch {
    return null;
  }
}
function ma(t) {
  const e = t.match(/data-sbd-nodes="([A-Za-z0-9+/=]+)"/);
  if (e) return Zs(e[1]);
  const o = t.match(
    new RegExp(`<!--${yh}([A-Za-z0-9+/=]+)-->`)
  );
  return o ? Zs(o[1]) : null;
}
function xr(t) {
  return !!t.closest(".bn-editor") || t.isContentEditable || t.tagName === "INPUT" || t.tagName === "TEXTAREA";
}
function ga(t) {
  return t.map((e) => {
    var n;
    const o = (e.content || []).filter((s) => s.type === "text").map((s) => s.text ?? "").join(""), r = (n = e.children) != null && n.length ? `
` + ga(e.children) : "";
    return o + r;
  }).filter(Boolean).join(`
`);
}
function mh(t) {
  var o;
  const e = [];
  for (const r of t)
    switch (r.type) {
      case "content": {
        const n = r.data;
        (o = n.blocks) != null && o.length ? e.push(ga(n.blocks)) : n.markdown && e.push(n.markdown);
        break;
      }
      case "image": {
        const n = r.data;
        n.src.startsWith("http") ? e.push(n.src) : e.push(n.alt || "[Image]");
        break;
      }
      case "shape": {
        const n = r.data;
        n.label && e.push(n.label);
        break;
      }
      case "text": {
        const n = r.data;
        n.text && e.push(n.text);
        break;
      }
      case "sticky": {
        const n = r.data;
        n.text && e.push(n.text);
        break;
      }
      case "draw":
        break;
      case "edge": {
        const n = r.data;
        n.label && e.push(n.label);
        break;
      }
    }
  return e.join(`

`);
}
function Us(t, e) {
  const o = mh(e), r = o.split(`
`).filter(Boolean).map((s) => `<p>${s}</p>`).join(""), n = ya(e);
  return t.setData(
    "text/html",
    `<!--${pa}--><div data-sbd-nodes="${n}">${r || "<p></p>"}</div>`
  ), t.setData("text/plain", o), o;
}
function gh(t, e) {
  let o = (e == null ? void 0 : e.ownerDocument) ?? document, r = o.defaultView ?? window, n = r.innerWidth / 2, s = r.innerHeight / 2, i = null;
  const l = (m) => {
    n = m.clientX, s = m.clientY;
  }, a = (m) => {
    xr(m.target) || t.selection.size !== 0 && (m.preventDefault(), t.copySelected(), i = Us(
      m.clipboardData,
      t.getClipboardNodes()
    ));
  }, c = (m) => {
    xr(m.target) || t.selection.size !== 0 && (m.preventDefault(), t.copySelected(), i = Us(
      m.clipboardData,
      t.getClipboardNodes()
    ), t.deleteSelected());
  }, h = async (m) => {
    var W, K, Z;
    if (xr(m.target)) return;
    const { x: g, y: b } = t.screenToCanvas(n, s), x = ((W = m.clipboardData) == null ? void 0 : W.getData("text/html")) || "", C = ((K = m.clipboardData) == null ? void 0 : K.getData("text/plain")) || "";
    if (x.includes(pa) || x.includes("data-sbd-nodes=") || i !== null && C === i) {
      if (i !== null && C === i && t.hasClipboard()) {
        m.preventDefault(), t.pasteClipboard(g, b);
        return;
      }
      const X = ma(x);
      if (X) {
        m.preventDefault(), t.setClipboard(X), t.pasteClipboard(g, b);
        return;
      }
    }
    const S = (Z = m.clipboardData) == null ? void 0 : Z.items;
    if (S) {
      for (const dt of Array.from(S))
        if (dt.type.startsWith("image/")) {
          m.preventDefault();
          const X = dt.getAsFile();
          if (!X) continue;
          const rt = new FileReader();
          rt.onload = () => {
            const J = rt.result, U = new Image();
            U.onload = () => {
              const R = t.screenToCanvas(n, s), O = 400, _ = 300, G = U.naturalWidth / U.naturalHeight, V = Math.min(U.naturalWidth, O), Y = Math.min(U.naturalHeight, _), $ = G >= 1 ? V : Y * G, nt = G >= 1 ? V / G : Y;
              let ot = J;
              if (x) {
                const ft = x.match(/<img[^>]+src=["']([^"']+)["']/i);
                ft && /\.(gif|webp|apng)(\?|#|$)/i.test(ft[1]) && (ot = ft[1].replace(/&amp;/g, "&"));
              }
              const yt = {
                id: wt(10),
                type: "image",
                x: R.x,
                y: R.y,
                w: $,
                h: nt,
                z: t.nextZ(),
                data: { src: ot }
              };
              t.addNode(yt), t.select(yt.id);
            }, U.src = J;
          }, rt.readAsDataURL(X);
          return;
        }
    }
    const F = Rn(C) ?? Rn(x);
    if (F) {
      m.preventDefault();
      const dt = t.screenToCanvas(n, s), X = await fa(
        F,
        dt.x,
        dt.y,
        t.nextZ()
      );
      X && (t.addNode(X), t.select(X.id));
      return;
    }
    if (kd(C)) {
      const dt = wd(C);
      if (dt) {
        m.preventDefault();
        const X = {
          id: wt(10),
          type: "youtube",
          x: g,
          y: b,
          w: 560,
          h: 315,
          z: t.nextZ(),
          data: { videoId: dt, url: C.trim() }
        };
        t.addNode(X), t.select(X.id);
        return;
      }
    }
    const D = x.replace(/^<meta[^>]*>/i, "").replace(/<!--StartFragment-->|<!--EndFragment-->/g, "").trim();
    if (D)
      try {
        const dt = Ci(D);
        if (dt.length > 0) {
          m.preventDefault();
          const X = {
            id: wt(10),
            type: "content",
            x: g,
            y: b,
            w: 300,
            h: "auto",
            z: t.nextZ(),
            data: { blocks: dt, markdown: C, borderColor: "#1e1e2e" }
          };
          t.addNode(X), t.select(X.id);
          return;
        }
      } catch {
      }
    if (C.trim()) {
      m.preventDefault();
      const dt = await Nn(C), X = {
        id: wt(10),
        type: "content",
        x: g,
        y: b,
        w: 300,
        h: "auto",
        z: t.nextZ(),
        data: { blocks: dt, markdown: C, borderColor: "#1e1e2e" }
      };
      t.addNode(X), t.select(X.id);
      return;
    }
    t.hasClipboard() && (m.preventDefault(), t.pasteClipboard(g, b));
  }, f = (m) => {
    const g = m.target;
    if (xr(g)) return;
    if (t.presentationMode) {
      if (m.key === "ArrowRight" || m.key === " ") {
        m.preventDefault(), t.presentationNext();
        return;
      }
      if (m.key === "ArrowLeft") {
        m.preventDefault(), t.presentationPrev();
        return;
      }
      if (m.key === "Escape") {
        m.preventDefault(), t.exitPresentation();
        return;
      }
      return;
    }
    const b = m.ctrlKey || m.metaKey;
    if (b && m.key === "c") {
      t.copySelected();
      return;
    }
    if (b && m.key === "x") {
      t.copySelected();
      return;
    }
    if (b && m.key === "d") {
      m.preventDefault(), t.duplicateSelected();
      return;
    }
    if (b && m.key === "g") {
      m.preventDefault(), m.shiftKey ? t.ungroupSelected() : t.groupSelected();
      return;
    }
    if (m.shiftKey && !b && m.key === "H") {
      m.preventDefault(), t.flipSelectedHorizontal();
      return;
    }
    if (m.shiftKey && !b && m.key === "V") {
      m.preventDefault(), t.flipSelectedVertical();
      return;
    }
    if (b && m.key === "]") {
      m.preventDefault();
      const x = Array.from(t.selection);
      m.altKey ? t.bringToFront(x) : t.bringForward(x);
      return;
    }
    if (b && m.key === "[") {
      m.preventDefault();
      const x = Array.from(t.selection);
      m.altKey ? t.sendToBack(x) : t.sendBackward(x);
      return;
    }
    if (!b && !m.altKey && !m.shiftKey) {
      if (m.key === "s") {
        t.setMode("select");
        return;
      }
      if (m.key === "p") {
        t.setMode("hand");
        return;
      }
      if (m.key === "d") {
        t.setMode("draw");
        return;
      }
      if (m.key === "g") {
        t.setMode("shape");
        return;
      }
      if (m.key === "t") {
        t.setMode("text");
        return;
      }
      if (m.key === "b") {
        t.setMode("note");
        return;
      }
      if (m.key === "y") {
        t.setMode("sticky");
        return;
      }
      if (m.key === "f") {
        t.setMode("frame");
        return;
      }
      if (m.key === "e") {
        t.setMode("erase");
        return;
      }
      if (m.key === "l") {
        t.toggleLassoSelect();
        return;
      }
      if (m.key === "z") {
        t.setMode("laser");
        return;
      }
    }
    if (m.key === "Delete" || m.key === "Backspace") {
      t.deleteSelected();
      return;
    }
    if (b && m.key === "z") {
      m.preventDefault(), m.shiftKey ? t.redo() : t.undo();
      return;
    }
    if (b && m.key === "a") {
      m.preventDefault(), t.selectMultiple(t.getAllNodes().map((x) => x.id));
      return;
    }
    if (m.key === "Escape") {
      if (t.activeGroupId) {
        t.exitGroup();
        return;
      }
      t.deselectAll(), t.setMode("select");
      return;
    }
    if (b && (m.key === "=" || m.key === "+")) {
      m.preventDefault(), t.zoomIn();
      return;
    }
    if (b && m.key === "-") {
      m.preventDefault(), t.zoomOut();
      return;
    }
    if (b && m.key === "0") {
      m.preventDefault(), t.fitToContent();
      return;
    }
  };
  function d(m, g) {
    m.addEventListener("pointermove", l), m.addEventListener("copy", a), m.addEventListener("cut", c), m.addEventListener("paste", h), g.addEventListener("keydown", f);
  }
  function p(m, g) {
    m.removeEventListener("pointermove", l), m.removeEventListener("copy", a), m.removeEventListener("cut", c), m.removeEventListener("paste", h), g.removeEventListener("keydown", f);
  }
  d(o, r);
  const y = setInterval(() => {
    if (!e) return;
    const m = e.ownerDocument;
    m !== o && (p(o, r), o = m, r = m.defaultView ?? window, n = r.innerWidth / 2, s = r.innerHeight / 2, d(o, r));
  }, 500);
  return () => {
    clearInterval(y), p(o, r);
  };
}
async function Qs(t, e) {
  const o = t.getAllNodes();
  if (o.length === 0) return;
  const r = t.measuredHeights, n = bh(o, r, t), s = e.padding ?? 40, i = e.background !== !1, l = e.format === "png", a = n.w + s * 2, c = n.h + s * 2, h = n.x - s, f = n.y - s, d = await ba(o, t, r, h, f, l), p = i ? nr(t.boardBackground).canvasBg : "transparent", y = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${a}" height="${c}" viewBox="0 0 ${a} ${c}">`,
    `<rect width="${a}" height="${c}" fill="${p}"/>`,
    ...d,
    "</svg>"
  ].join(`
`);
  if (e.format === "svg")
    Js(new Blob([y], { type: "image/svg+xml" }), "board.svg");
  else {
    const m = e.scale ?? 4, g = await Ah(y, a, c, m);
    Js(g, "board.png");
  }
}
function bh(t, e, o) {
  let r = 1 / 0, n = 1 / 0, s = -1 / 0, i = -1 / 0;
  for (const a of t) {
    if (a.type === "edge") continue;
    const c = o.resolveHeight(a);
    r = Math.min(r, a.x), n = Math.min(n, a.y), s = Math.max(s, a.x + a.w), i = Math.max(i, a.y + c);
  }
  const l = new Map(t.map((a) => [a.id, a]));
  for (const a of t) {
    if (a.type !== "edge") continue;
    const c = a, h = l.get(c.data.fromId), f = l.get(c.data.toId);
    if (!h || !f) continue;
    const d = je(
      h,
      f,
      c.data.edgeType,
      e,
      c.data.sourceHandle,
      c.data.targetHandle,
      c.data.midpointOffset,
      c.data.curveOffset
    );
    r = Math.min(r, d.bounds.x), n = Math.min(n, d.bounds.y), s = Math.max(s, d.bounds.x + d.bounds.w), i = Math.max(i, d.bounds.y + d.bounds.h);
  }
  return isFinite(r) ? { x: r, y: n, w: s - r, h: i - n } : { x: 0, y: 0, w: 100, h: 100 };
}
async function ba(t, e, o, r, n, s) {
  const i = new Map(t.map((c) => [c.id, c])), l = [...t].sort((c, h) => c.z - h.z), a = [];
  for (const c of l) {
    const h = c.x - r, f = c.y - n, d = e.resolveHeight(c);
    switch (c.type) {
      case "frame":
        a.push(xh(c, h, f, d));
        break;
      case "content":
        a.push(wh(c, h, f, c.w, d));
        break;
      case "draw":
        a.push(kh(c, r, n));
        break;
      case "shape":
        a.push(Sh(c, h, f, c.w, d));
        break;
      case "text":
        a.push(Mh(c, h, f, c.w, d));
        break;
      case "sticky":
        a.push(Ch(c, h, f, c.w, d));
        break;
      case "image":
        a.push(await zh(c, h, f, c.w, d, s));
        break;
      case "youtube":
        a.push(await Ih(c, h, f, c.w, d, s));
        break;
      case "edge": {
        const p = c, y = i.get(p.data.fromId), m = i.get(p.data.toId);
        y && m && a.push(Ph(p, y, m, o, r, n));
        break;
      }
    }
  }
  return a;
}
function _e(t, e, o, r, n, s, i) {
  const l = [];
  if (s) {
    const a = e + r / 2, c = o + n / 2;
    l.push(`transform="rotate(${s}, ${a}, ${c})"`);
  }
  return i !== void 0 && i !== 1 && l.push(`opacity="${i}"`), `<g ${l.join(" ")}>${t}</g>`;
}
function xh(t, e, o, r) {
  const n = t.data, s = n.backgroundColor || "rgba(0,0,0,0.02)", i = n.borderColor || "#d1d5db", l = n.borderWidth ?? 1, a = Hr(n.borderStyle, l), c = n.label ? Ho(n.label) : "";
  let h = `<rect x="${e}" y="${o}" width="${t.w}" height="${r}" rx="4" fill="${s}" stroke="${i}" stroke-width="${l}"` + (a ? ` stroke-dasharray="${a}"` : "") + "/>";
  return c && (h += `<text x="${e + 8}" y="${o - 6}" font-size="12" fill="#6b7280" font-family="sans-serif">${c}</text>`), _e(h, e, o, t.w, r, t.rotation, n.opacity);
}
function wh(t, e, o, r, n) {
  var f;
  const s = t.data, i = ((f = s.markdown) == null ? void 0 : f.trim()) || "", l = s.borderColor, a = s.borderWidth ?? 0, c = Hr(s.borderStyle, a);
  let h = "";
  return l && a > 0 ? h += `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="4" fill="white" stroke="${l}" stroke-width="${a}"` + (c ? ` stroke-dasharray="${c}"` : "") + "/>" : h += `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="4" fill="white"/>`, i && (h += es(i, e + 12, o + 20, r - 24, 14, 1.6, "#374151", "left", "sans-serif")), _e(h, e, o, r, n, t.rotation, s.opacity);
}
function kh(t, e, o) {
  const r = t.data, n = r.points.map(
    ([l, a, c]) => [l + t.x - e, a + t.y - o, c]
  );
  if (n.length === 0) return "";
  if (r.tool === "vector")
    return vh(n, r, t);
  const s = $e(r.strokeStyle);
  let i = "";
  if (r.fill) {
    const l = n.map(([a, c]) => [a, c]);
    if (l.length > 2) {
      const a = l.map((c, h) => `${h === 0 ? "M" : "L"}${c[0]},${c[1]}`).join(" ") + " Z";
      i += `<path d="${a}" fill="${r.fill}" fill-opacity="0.4" stroke="none"/>`;
    }
  }
  if (s) {
    const l = n.map((c, h) => `${h === 0 ? "M" : "L"}${c[0].toFixed(1)},${c[1].toFixed(1)}`).join(" "), a = s.map((c) => c * Math.max(r.strokeWidth, 1)).join(" ");
    i += `<path d="${l}" fill="none" stroke="${r.color}" stroke-width="${r.strokeWidth}" stroke-dasharray="${a}" stroke-linecap="round" stroke-linejoin="round"/>`;
  } else {
    const l = Yn(n, { size: r.strokeWidth });
    l && (i += `<path d="${l}" fill="${r.color}" stroke="none"/>`);
  }
  return r.opacity !== void 0 && r.opacity !== 1 ? `<g opacity="${r.opacity}">${i}</g>` : i;
}
function vh(t, e, o) {
  const r = t.map((a, c) => `${c === 0 ? "M" : "L"}${a[0].toFixed(2)},${a[1].toFixed(2)}`).join(" ") + " Z", n = $e(e.strokeStyle), s = n ? ` stroke-dasharray="${n.map((a) => a * Math.max(e.strokeWidth, 1)).join(" ")}"` : "", i = `<path d="${r}" fill="${e.fill || "none"}" stroke="${e.color}" stroke-width="${e.strokeWidth}" stroke-linecap="round" stroke-linejoin="round"${s}/>`, l = o.h === "auto" ? 0 : o.h;
  return _e(i, o.x, o.y, o.w, l, o.rotation, e.opacity);
}
function Sh(t, e, o, r, n) {
  const s = t.data, i = {
    stroke: s.stroke,
    fill: s.fill,
    fillStyle: s.fillStyle,
    roughness: s.roughness,
    strokeWidth: s.strokeWidth,
    strokeLineDash: $e(s.strokeStyle),
    seed: t.id
  };
  let l;
  const a = s.edgeStyle === "round";
  switch (s.shape) {
    case "rect":
      l = Ar(e, o, r, n, i, a);
      break;
    case "ellipse":
      l = Vn(e + r / 2, o + n / 2, r, n, i);
      break;
    case "diamond":
      l = Kn(e, o, r, n, i, a);
      break;
    case "line": {
      const h = s.startPoint ?? [0, 0], f = s.endPoint ?? [r, n];
      l = Fo(e + h[0], o + h[1], e + f[0], o + f[1], i);
      break;
    }
    case "arrow": {
      const h = s.startPoint ?? [0, 0], f = s.endPoint ?? [r, n];
      l = qn(e + h[0], o + h[1], e + f[0], o + f[1], i);
      break;
    }
    default:
      l = Ar(e, o, r, n, i);
  }
  const c = l.map(
    (h) => `<path d="${h.d}" fill="${h.fill || "none"}" stroke="${h.stroke}" stroke-width="${h.strokeWidth}"` + (h.strokeDasharray ? ` stroke-dasharray="${h.strokeDasharray}"` : "") + "/>"
  ).join(`
`);
  return _e(c, e, o, r, n, t.rotation, s.opacity);
}
function Mh(t, e, o, r, n) {
  const s = t.data, i = n || s.text.split(`
`).length * s.fontSize * 1, l = qe(s.fontFamily), a = !!s.borderColor, c = a ? 6 : 0;
  let h = "";
  if (a) {
    const d = s.borderWidth ?? 1, p = Hr(s.borderStyle, d);
    h += `<rect x="${e}" y="${o}" width="${r}" height="${i}" rx="4" fill="none" stroke="${s.borderColor}" stroke-width="${d}"` + (p ? ` stroke-dasharray="${p}"` : "") + "/>";
  }
  const f = s.align === "center" ? e + r / 2 : s.align === "right" ? e + r - c : e + c;
  return h += es(
    s.text,
    f,
    o + c + s.fontSize,
    r - c * 2,
    s.fontSize,
    1,
    s.color,
    s.align,
    l
  ), _e(h, e, o, r, i, t.rotation, s.opacity);
}
function Ch(t, e, o, r, n) {
  const s = t.data, i = s.fontSize ?? 16, l = `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="2" fill="${s.color}"/>` + es(s.text, e + 12, o + 12 + i, r - 24, i, 1.4, "#1e1e2e", "left", "sans-serif");
  return _e(l, e, o, r, n, t.rotation, s.opacity);
}
async function zh(t, e, o, r, n, s) {
  const i = t.data;
  let l = i.src;
  if (s && l && !l.startsWith("data:"))
    try {
      l = await Dr(l);
    } catch {
    }
  const a = i.borderColor, c = i.borderWidth ?? 0, h = Hr(i.borderStyle, c);
  let f = `<image href="${Ho(l)}" x="${e}" y="${o}" width="${r}" height="${n}" preserveAspectRatio="xMidYMid slice"/>`;
  return a && c > 0 && (f += `<rect x="${e}" y="${o}" width="${r}" height="${n}" fill="none" stroke="${a}" stroke-width="${c}"` + (h ? ` stroke-dasharray="${h}"` : "") + "/>"), _e(f, e, o, r, n, t.rotation, i.opacity);
}
async function Ih(t, e, o, r, n, s) {
  const i = t.data;
  let l = Sd(i.videoId);
  if (s)
    try {
      l = await Dr(l);
    } catch {
    }
  let a = `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="4" fill="#1a1a1a"/><image href="${Ho(l)}" x="${e}" y="${o}" width="${r}" height="${n}" preserveAspectRatio="xMidYMid slice"/>`;
  const c = e + r / 2, h = o + n / 2, f = Math.min(r, n) * 0.12;
  return a += `<circle cx="${c}" cy="${h}" r="${f}" fill="rgba(0,0,0,0.6)"/><path d="${Th(c, h, f * 0.5)}" fill="white"/>`, _e(a, e, o, r, n, t.rotation, i.opacity);
}
function Th(t, e, o) {
  const r = o * 0.15, n = t - o * 0.7 + r, s = e - o, i = t + o + r, l = e, a = n, c = e + o;
  return `M${n},${s} L${i},${l} L${a},${c} Z`;
}
function Ph(t, e, o, r, n, s) {
  const i = t.data, l = je(
    e,
    o,
    i.edgeType,
    r,
    i.sourceHandle,
    i.targetHandle,
    i.midpointOffset,
    i.curveOffset
  ), a = `translate(${-n}, ${-s})`, c = i.style === "dashed" ? "8 4" : i.style === "dotted" ? "2 3" : void 0, h = i.strokeWidth;
  let f = `<path d="${l.path}" fill="none" stroke="${i.color}" stroke-width="${h}"` + (c ? ` stroke-dasharray="${c}"` : "") + ' stroke-linecap="round" stroke-linejoin="round"/>';
  const d = i.arrowHeadSize ?? Math.max(8, h * 3), p = i.arrowTailSize ?? Math.max(8, h * 3);
  if (i.arrowHead && i.arrowHead !== "none") {
    if (i.arrowHead === "arrow")
      f += `<path d="${Do(l.x2, l.y2, l.arrowAngle, d)}" fill="none" stroke="${i.color}" stroke-width="${h}" stroke-linecap="round" stroke-linejoin="round"/>`;
    else if (i.arrowHead === "filled")
      f += `<path d="${zr(l.x2, l.y2, l.arrowAngle, d)}" fill="${i.color}" stroke="none"/>`;
    else if (i.arrowHead === "dot") {
      const y = d / 3;
      f += `<circle cx="${l.x2}" cy="${l.y2}" r="${y}" fill="${i.color}"/>`;
    }
  }
  if (i.arrowTail && i.arrowTail !== "none") {
    if (i.arrowTail === "arrow")
      f += `<path d="${Do(l.x1, l.y1, l.tailAngle, p)}" fill="none" stroke="${i.color}" stroke-width="${h}" stroke-linecap="round" stroke-linejoin="round"/>`;
    else if (i.arrowTail === "filled")
      f += `<path d="${zr(l.x1, l.y1, l.tailAngle, p)}" fill="${i.color}" stroke="none"/>`;
    else if (i.arrowTail === "dot") {
      const y = p / 3;
      f += `<circle cx="${l.x1}" cy="${l.y1}" r="${y}" fill="${i.color}"/>`;
    }
  }
  return i.label && (f += `<text x="${l.labelX}" y="${l.labelY}" font-size="12" fill="${i.color}" text-anchor="middle" dominant-baseline="central" font-family="sans-serif">${Ho(i.label)}</text>`), `<g transform="${a}">${f}</g>`;
}
function es(t, e, o, r, n, s, i, l, a) {
  if (!t) return "";
  const c = l === "center" ? "middle" : l === "right" ? "end" : "start", h = Rh(t, r, n), f = n * s, d = h.map(
    (p, y) => `<tspan x="${e}" dy="${y === 0 ? 0 : f}">${Ho(p)}</tspan>`
  ).join("");
  return `<text x="${e}" y="${o}" font-size="${n}" fill="${i}" font-family="${Ho(a)}" text-anchor="${c}">${d}</text>`;
}
function Rh(t, e, o) {
  const r = o * 0.55, n = Math.max(1, Math.floor(e / r)), s = [];
  for (const i of t.split(`
`)) {
    if (!i.trim()) {
      s.push("");
      continue;
    }
    const l = i.split(/\s+/);
    let a = "";
    for (const c of l) {
      const h = a ? a + " " + c : c;
      h.length > n && a ? (s.push(a), a = c) : a = h;
    }
    a && s.push(a);
  }
  return s;
}
function Hr(t, e) {
  const o = e ?? 1;
  if (t === "dashed") return `${8 * o} ${4 * o}`;
  if (t === "dotted") return `${2 * o} ${2 * o}`;
}
function Ho(t) {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
}
async function Dr(t) {
  const o = await (await fetch(t)).blob();
  return new Promise((r, n) => {
    const s = new FileReader();
    s.onloadend = () => r(s.result), s.onerror = n, s.readAsDataURL(o);
  });
}
function Ah(t, e, o, r) {
  return new Promise((n, s) => {
    const i = new Image(), l = new Blob([t], { type: "image/svg+xml;charset=utf-8" }), a = URL.createObjectURL(l);
    i.onload = () => {
      const c = document.createElement("canvas");
      c.width = e * r, c.height = o * r;
      const h = c.getContext("2d");
      h.scale(r, r), h.drawImage(i, 0, 0, e, o), URL.revokeObjectURL(a), c.toBlob((f) => {
        f ? n(f) : s(new Error("Canvas toBlob failed"));
      }, "image/png");
    }, i.onerror = () => {
      URL.revokeObjectURL(a), s(new Error("Failed to load SVG as image"));
    }, i.src = a;
  });
}
const Lh = /* @__PURE__ */ new Set(["sans-serif", "serif", "monospace"]), Co = /* @__PURE__ */ new Map(), Dh = 12;
function Eh(t) {
  const e = /* @__PURE__ */ new Set();
  for (const o of t)
    if (o.type === "text") {
      const r = o.data.fontFamily;
      r && !Lh.has(r) && e.add(r);
    }
  return [...e];
}
async function Wh(t) {
  if (t.length === 0) return "";
  const e = [];
  for (const o of t) {
    if (Co.has(o)) {
      e.push(Co.get(o));
      continue;
    }
    try {
      let r;
      if (o === "Excalifont")
        r = await Dr(zi);
      else {
        const l = (await (await fetch(
          `https://fonts.googleapis.com/css2?family=${encodeURIComponent(o)}&display=swap`
        )).text()).match(/url\((https:\/\/[^)]+\.woff2)\)/);
        if (!l) continue;
        r = await Dr(l[1]);
      }
      const n = `@font-face { font-family: '${o}'; src: url('${r}') format('woff2'); }`;
      if (Co.size >= Dh) {
        const s = Co.keys().next().value;
        s !== void 0 && Co.delete(s);
      }
      Co.set(o, n), e.push(n);
    } catch {
    }
  }
  return e.join(`
`);
}
async function Fh(t, e) {
  const o = t.getNode(e);
  if (!o || o.type !== "frame") return "";
  const r = t.resolveHeight(o), n = 0, s = o.w + n * 2, i = r + n * 2, l = o.x - n, a = o.y - n, c = [o], h = /* @__PURE__ */ new Set([e]), f = (x) => {
    h.has(x.id) || x.type === "edge" || (h.add(x.id), c.push(x));
  };
  for (const x of t.getNodesInRect({ x: o.x, y: o.y, w: o.w, h: r }))
    f(x);
  for (const x of t.getFrameChildren(e))
    f(x);
  for (const x of t.getAllNodes())
    if (x.type === "edge") {
      const C = x;
      h.has(C.data.fromId) && h.has(C.data.toId) && c.push(x);
    }
  const d = t.measuredHeights, p = await ba(c, t, d, l, a, !0), y = Eh(c), m = await Wh(y), g = nr(t.boardBackground).canvasBg, b = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${i}" viewBox="0 0 ${s} ${i}">`,
    m ? `<defs><style>${m}</style></defs>` : "",
    `<rect width="${s}" height="${i}" fill="${g}"/>`,
    ...p,
    "</svg>"
  ].join(`
`);
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(b)}`;
}
function Js(t, e) {
  const o = URL.createObjectURL(t), r = document.createElement("a");
  r.href = o, r.download = e, document.body.appendChild(r), r.click(), document.body.removeChild(r), URL.revokeObjectURL(o);
}
const $s = [
  { key: "iphone-se", label: "iPhone SE", w: 375, h: 667, group: "phone" },
  { key: "iphone-xr", label: "iPhone XR", w: 414, h: 896, group: "phone" },
  { key: "iphone-12-pro", label: "iPhone 12 Pro", w: 390, h: 844, group: "phone" },
  { key: "iphone-14-pro-max", label: "iPhone 14 Pro Max", w: 430, h: 932, group: "phone" },
  { key: "iphone-16-pro", label: "iPhone 16 Pro", w: 402, h: 874, group: "phone" },
  { key: "pixel-7", label: "Pixel 7", w: 412, h: 915, group: "phone" },
  { key: "galaxy-s8", label: "Galaxy S8+", w: 360, h: 740, group: "phone" },
  { key: "galaxy-s20-ultra", label: "Galaxy S20 Ultra", w: 412, h: 915, group: "phone" },
  { key: "galaxy-z-fold-5", label: "Galaxy Z Fold 5", w: 344, h: 882, group: "phone" },
  { key: "galaxy-a51", label: "Galaxy A51/71", w: 412, h: 914, group: "phone" }
], _s = [
  { key: "ipad-mini", label: "iPad Mini", w: 768, h: 1024, group: "tablet" },
  { key: "ipad-air", label: "iPad Air", w: 820, h: 1180, group: "tablet" },
  { key: "ipad-pro", label: "iPad Pro", w: 1024, h: 1366, group: "tablet" },
  { key: "surface-pro-7", label: "Surface Pro 7", w: 912, h: 1368, group: "tablet" },
  { key: "surface-duo", label: "Surface Duo", w: 540, h: 720, group: "tablet" },
  { key: "zenbook-fold", label: "Asus Zenbook Fold", w: 853, h: 1280, group: "tablet" }
];
function ti(t, e) {
  return t.map((o) => ({
    key: `${o.key}-landscape`,
    label: `${o.label} ↔`,
    w: o.h,
    h: o.w,
    group: e
  }));
}
const xa = [
  ...$s,
  ...ti($s, "phone-landscape"),
  ..._s,
  ...ti(_s, "tablet-landscape"),
  // Other devices (already landscape-oriented)
  { key: "nest-hub", label: "Nest Hub", w: 1024, h: 600, group: "other" },
  { key: "nest-hub-max", label: "Nest Hub Max", w: 1280, h: 800, group: "other" },
  // Standard ratios
  { key: "16-9", label: "16:9", w: 1920, h: 1080, group: "standard" },
  { key: "9-16", label: "9:16", w: 1080, h: 1920, group: "standard" },
  { key: "4-3", label: "4:3", w: 1024, h: 768, group: "standard" },
  { key: "3-4", label: "3:4", w: 768, h: 1024, group: "standard" },
  { key: "1-1", label: "1:1", w: 1e3, h: 1e3, group: "standard" },
  { key: "21-9", label: "21:9", w: 2560, h: 1080, group: "standard" }
], Bh = new Map(xa.map((t) => [t.key, t]));
function An(t) {
  return Bh.get(t);
}
function wa(t) {
  return t.w / t.h;
}
const Nh = {
  phone: "Phones",
  "phone-landscape": "Phones (Landscape)",
  tablet: "Tablets",
  "tablet-landscape": "Tablets (Landscape)",
  other: "Devices",
  standard: "Standard"
};
function Hh() {
  const t = /* @__PURE__ */ new Map();
  for (const e of xa) {
    const o = t.get(e.group);
    o ? o.push(e) : t.set(e.group, [e]);
  }
  return Array.from(t.entries()).map(([e, o]) => ({
    label: Nh[e] ?? e,
    presets: o
  }));
}
function Oh(t) {
  const e = t.replace("#", ""), o = parseInt(e.substring(0, 2), 16) || 0, r = parseInt(e.substring(2, 4), 16) || 0, n = parseInt(e.substring(4, 6), 16) || 0;
  return (0.299 * o + 0.587 * r + 0.114 * n) / 255 > 0.5 ? "#1e1e2e" : "#ffffff";
}
function hn(t, e, o) {
  let r = !1;
  for (let n = 0, s = o.length - 1; n < o.length; s = n++) {
    const [i, l] = o[n], [a, c] = o[s];
    l > e != c > e && t < (a - i) * (e - l) / (c - l) + i && (r = !r);
  }
  return r;
}
async function Xh(t, e, o) {
  try {
    const r = await navigator.clipboard.read();
    let n = null;
    for (const i of r)
      if (i.types.includes("text/html")) {
        const l = await (await i.getType("text/html")).text();
        if (l.includes("sbd-clipboard") || l.includes("data-sbd-nodes=")) {
          const a = ma(l);
          if (a) {
            t.setClipboard(a), t.pasteClipboard(e, o);
            return;
          }
          if (t.hasClipboard()) {
            t.pasteClipboard(e, o);
            return;
          }
        }
        n = l;
      }
    for (const i of r) {
      const l = i.types.find((a) => a.startsWith("image/"));
      if (l) {
        const a = await i.getType(l), c = await new Promise((x) => {
          const C = new FileReader();
          C.onload = () => x(C.result), C.readAsDataURL(a);
        }), h = new Image();
        await new Promise((x) => {
          h.onload = () => x(), h.src = c;
        });
        const f = h.naturalWidth / h.naturalHeight, d = Math.min(h.naturalWidth, 400), p = Math.min(h.naturalHeight, 300), y = f >= 1 ? d : p * f, m = f >= 1 ? d / f : p;
        let g = c;
        if (n) {
          const x = n.match(/<img[^>]+src=["']([^"']+)["']/i);
          x && /\.(gif|webp|apng)(\?|#|$)/i.test(x[1]) && (g = x[1].replace(/&amp;/g, "&"));
        }
        const b = {
          id: wt(10),
          type: "image",
          x: e,
          y: o,
          w: y,
          h: m,
          z: t.nextZ(),
          data: { src: g }
        };
        t.addNode(b), t.select(b.id);
        return;
      }
    }
    const s = await navigator.clipboard.readText();
    if (n) {
      const i = n.replace(/^<meta[^>]*>/i, "").replace(/<!--StartFragment-->|<!--EndFragment-->/g, "").trim();
      try {
        const l = Ci(i);
        if (l.length > 0) {
          const a = {
            id: wt(10),
            type: "content",
            x: e,
            y: o,
            w: 300,
            h: "auto",
            z: t.nextZ(),
            data: { blocks: l, markdown: s || "", borderColor: "#1e1e2e" }
          };
          t.addNode(a), t.select(a.id);
          return;
        }
      } catch {
      }
    }
    if (s != null && s.trim()) {
      const i = await Nn(s), l = {
        id: wt(10),
        type: "content",
        x: e,
        y: o,
        w: 300,
        h: "auto",
        z: t.nextZ(),
        data: { blocks: i, markdown: s, borderColor: "#1e1e2e" }
      };
      t.addNode(l), t.select(l.id);
      return;
    }
  } catch {
  }
  t.pasteClipboard(e, o);
}
async function ei(t) {
  const e = t.getClipboardNodes();
  if (e.length === 0) return;
  const o = [];
  for (const l of e)
    if (l.type === "content") {
      const a = l.data;
      a.markdown && o.push(a.markdown);
    } else if (l.type === "text") {
      const a = l.data;
      a.text && o.push(a.text);
    } else if (l.type === "image") {
      const a = l.data;
      o.push(a.src.startsWith("http") ? a.src : a.alt || "[Image]");
    } else if (l.type === "shape") {
      const a = l.data;
      a.label && o.push(a.label);
    } else if (l.type === "sticky") {
      const a = l.data;
      a.text && o.push(a.text);
    } else if (l.type === "edge") {
      const a = l.data;
      a.label && o.push(a.label);
    }
  const r = o.join(`

`), n = r.split(`
`).filter(Boolean).map((l) => `<p>${l}</p>`).join(""), i = `<!--sbd-clipboard--><div data-sbd-nodes="${ya(e)}">${n || "<p></p>"}</div>`;
  try {
    await navigator.clipboard.write([
      new ClipboardItem({
        "text/plain": new Blob([r], { type: "text/plain" }),
        "text/html": new Blob([i], { type: "text/html" })
      })
    ]);
  } catch {
    try {
      await navigator.clipboard.writeText(r);
    } catch {
    }
  }
}
function wr(t) {
  switch (t) {
    case "select":
      return "default";
    case "text":
      return "text";
    case "note":
      return "text";
    case "sticky":
      return "crosshair";
    case "draw":
      return "crosshair";
    case "shape":
      return "crosshair";
    case "edge":
      return "crosshair";
    case "frame":
      return "crosshair";
    case "erase":
      return `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='20' height='20'><circle cx='10' cy='10' r='9' fill='none' stroke='%239ca3af' stroke-width='1.5'/></svg>") ${20 / 2} ${20 / 2}, crosshair`;
    case "laser":
      return `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16'><circle cx='8' cy='8' r='3' fill='%23ef4444'/><circle cx='8' cy='8' r='7' fill='none' stroke='%23ef4444' stroke-width='1' opacity='0.4'/></svg>") ${16 / 2} ${16 / 2}, crosshair`;
    case "hand":
      return "grab";
    default:
      return "default";
  }
}
function oi(t, e) {
  const o = e.x - t.x, r = e.y - t.y;
  return { dist: Math.sqrt(o * o + r * r), mx: (t.x + e.x) / 2, my: (t.y + e.y) / 2 };
}
const zo = `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' fill='none' stroke='%23e0e0e0' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'><path d='M12 3a7 7 0 0 1 4.5 12.4l-2 2'/><path d='M14.5 15.4q.5 1.5.5 2.6c0 2-1.5 3-3 3s-2-1-2-2c0-.8.5-1.5 1-2'/><circle cx='12' cy='3' r='1.5' fill='%23e0e0e0' stroke='none'/></svg>") 12 3, crosshair`;
function Yh({
  node: t,
  isInteractive: e,
  measuredH: o,
  onMeasuredHeight: r,
  observeElement: n,
  unobserveElement: s,
  isContainer: i,
  children: l
}) {
  const a = at(null);
  mt(() => {
    if (t.h !== "auto") return;
    const f = a.current;
    if (!f) return;
    const d = f.offsetHeight;
    return d > 0 && r(t.id, d), n(f, () => {
      const p = f.offsetHeight;
      p > 0 && r(t.id, p);
    }), () => s(f);
  }, [t.id, t.h, r, n, s]);
  const c = t.h === "auto" ? o ?? "auto" : t.h, h = Kt(() => ({
    position: "absolute",
    left: t.x,
    top: t.y,
    width: t.w,
    height: c,
    zIndex: t.z,
    pointerEvents: i ? "none" : e ? "auto" : "none",
    transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
    transformOrigin: "center center"
  }), [t.x, t.y, t.w, c, t.z, t.rotation, i, e]);
  return /* @__PURE__ */ u(
    "div",
    {
      ref: a,
      "data-node-id": t.id,
      className: e ? void 0 : "sb-block-inert",
      style: h,
      children: l
    }
  );
}
function Gh({
  node: t,
  engine: e,
  onDone: o
}) {
  const r = at(null), n = at(t.data.label ?? ""), s = at(t);
  s.current = t;
  const i = at(t.data.label ?? "");
  mt(() => () => {
    const h = s.current, f = n.current.trim();
    if (f !== i.current) {
      const p = { data: { ...h.data, label: f || void 0 } }, y = r.current;
      if (y && f) {
        const g = h.h === "auto" ? 100 : h.h, b = y.scrollHeight + 24;
        b > g && (p.h = b);
      }
      e.updateNodeWithHistory(h.id, p);
    }
  }, []);
  const l = t.h === "auto" ? 100 : t.h, a = t.data.labelFontSize ?? 14, c = t.data.fill && t.data.fillStyle === "solid" ? Oh(t.data.fill) : t.data.stroke;
  return /* @__PURE__ */ u(
    "div",
    {
      "data-node-id": t.id,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: l,
        zIndex: 999998,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        pointerEvents: "auto",
        padding: "8px 12px",
        boxSizing: "border-box"
      },
      children: /* @__PURE__ */ u(
        "textarea",
        {
          ref: r,
          autoFocus: !0,
          defaultValue: t.data.label ?? "",
          placeholder: "",
          rows: 1,
          onBlur: () => o(),
          onKeyDown: (h) => {
            h.key === "Escape" && h.currentTarget.blur(), h.stopPropagation();
          },
          onInput: (h) => {
            const f = h.currentTarget;
            n.current = f.value;
            const d = s.current;
            e.updateNode(d.id, {
              data: { ...d.data, label: f.value || void 0 }
            }), f.style.height = "auto", f.style.height = f.scrollHeight + "px";
            const y = f.scrollHeight + 24;
            y > l && e.updateNode(t.id, { h: y });
          },
          onPointerDown: (h) => h.stopPropagation(),
          style: {
            textAlign: t.data.labelAlign ?? "center",
            fontSize: a,
            fontFamily: qe(t.data.labelFontFamily ?? Ke),
            color: c,
            background: "transparent",
            border: "none",
            outline: "none",
            resize: "none",
            overflow: "hidden",
            width: "100%",
            padding: 0,
            margin: 0,
            lineHeight: 1.3,
            wordBreak: "break-word",
            whiteSpace: "pre-wrap"
          }
        }
      )
    }
  );
}
function jh({
  engine: t,
  schema: e,
  registry: o,
  dataFlow: r
}) {
  const n = at(null), s = () => {
    var w;
    return ((w = n.current) == null ? void 0 : w.ownerDocument) ?? document;
  }, [i, l] = tt({ w: 0, h: 0 }), [a, c] = tt({ ...t.viewport }), [h, f] = tt(t.getAllNodes()), [d, p] = tt(
    new Set(t.selection)
  ), [y, m] = tt(t.mode), [g, b] = tt(t.activeGroupId), [x, C] = tt(t.snapToGrid), [z, S] = tt(t.gridSize), [F, D] = tt(t.smartGuides), [W, K] = tt([]), [Z, dt] = tt(t.boardBackground), X = at(!1), rt = at(!1), J = at(/* @__PURE__ */ new Map()), U = at(!1), R = at(!1), O = at(null), _ = at(null);
  mt(() => {
    const w = (I) => {
      var H, M;
      if (I.key === " " && !I.repeat && !X.current) {
        const E = (H = I.target) == null ? void 0 : H.tagName;
        if (E === "INPUT" || E === "TEXTAREA" || (M = I.target) != null && M.isContentEditable) return;
        X.current = !0;
        const T = n.current;
        T && (T.style.cursor = "grab"), I.preventDefault();
      }
    }, A = (I) => {
      if (I.key === " ") {
        X.current = !1, rt.current = !1;
        const H = n.current;
        H && (H.style.cursor = t.lassoSelect ? zo : wr(t.mode));
      }
    };
    return window.addEventListener("keydown", w), window.addEventListener("keyup", A), () => {
      window.removeEventListener("keydown", w), window.removeEventListener("keyup", A);
    };
  }, []), mt(() => {
    const w = (I) => {
      J.current.delete(I.pointerId), I.pointerType === "pen" && (R.current = !1), O.current && (clearTimeout(O.current), O.current = null, _.current = null);
    }, A = s();
    return A.addEventListener("pointerup", w), A.addEventListener("pointercancel", w), () => {
      A.removeEventListener("pointerup", w), A.removeEventListener("pointercancel", w);
    };
  }, []);
  const [G, V] = tt(null), [Y, $] = tt(null), [nt, ot] = tt(null), [yt, ft] = tt(null);
  mt(() => {
    const w = n.current;
    if (!w) return;
    t.setContainer(w);
    const A = () => {
      const H = w.getBoundingClientRect();
      t.containerOffset = { x: H.left, y: H.top };
    };
    A();
    const I = new ResizeObserver((H) => {
      var T;
      const { width: M, height: E } = ((T = H[0]) == null ? void 0 : T.contentRect) ?? { width: 0, height: 0 };
      l((B) => B.w === M && B.h === E ? B : { w: M, h: E }), t.setContainerSize(M, E), A();
    });
    return I.observe(w), () => I.disconnect();
  }, [t]);
  const [et, Pt] = tt({}), zt = it((w, A) => {
    Pt(
      (I) => I[w] === A ? I : { ...I, [w]: A }
    ), t.updateMeasuredHeight(w, A);
  }, [t]), gt = at(null), ht = at(/* @__PURE__ */ new Map());
  function Dt() {
    return gt.current || (gt.current = new ResizeObserver((w) => {
      var A;
      for (const I of w)
        (A = ht.current.get(I.target)) == null || A(I);
    })), gt.current;
  }
  const It = it((w, A) => {
    ht.current.set(w, A), Dt().observe(w);
  }, []), Rt = it((w) => {
    var A;
    ht.current.delete(w), (A = gt.current) == null || A.unobserve(w);
  }, []);
  mt(() => () => {
    var w;
    (w = gt.current) == null || w.disconnect(), gt.current = null, ht.current.clear();
  }, []);
  const Qt = Kt(() => new Set(h.map((w) => w.id)), [h]);
  mt(() => {
    Pt((w) => {
      let A = !1;
      const I = {};
      for (const [H, M] of Object.entries(w))
        Qt.has(H) ? I[H] = M : A = !0;
      return A ? I : w;
    });
  }, [Qt]);
  const Jt = it(
    (w, A, I) => {
      let H, M;
      if (o && w.data.sourcePort) {
        const E = o.get(A.type);
        E != null && E.ports && (H = er(A, E.ports, w.data.sourcePort, a.zoom, et) ?? void 0);
      }
      if (o && w.data.targetPort) {
        const E = o.get(I.type);
        E != null && E.ports && (M = er(I, E.ports, w.data.targetPort, a.zoom, et) ?? void 0);
      }
      return { sourcePortPos: H, targetPortPos: M };
    },
    [o, a.zoom, et]
  );
  it(
    (w) => t.zoomToNode(w),
    [t]
  );
  const qt = it(
    (w, A) => {
      if (!w.rotation)
        return { minX: w.x, minY: w.y, maxX: w.x + w.w, maxY: w.y + A };
      const I = w.x + w.w / 2, H = w.y + A / 2, M = w.rotation * Math.PI / 180, E = Math.cos(M), T = Math.sin(M), B = [
        [w.w / 2, A / 2],
        [-w.w / 2, A / 2],
        [-w.w / 2, -A / 2],
        [w.w / 2, -A / 2]
      ];
      let L = 1 / 0, N = 1 / 0, P = -1 / 0, j = -1 / 0;
      for (const [q, Q] of B) {
        const ct = I + q * E - Q * T, lt = H + q * T + Q * E;
        L = Math.min(L, ct), N = Math.min(N, lt), P = Math.max(P, ct), j = Math.max(j, lt);
      }
      return { minX: L, minY: N, maxX: P, maxY: j };
    },
    []
  ), $t = 8, he = it(
    (w, A) => A.filter((I) => {
      if (I.type === "edge") {
        const E = I.data, T = t.getNode(E.fromId), B = t.getNode(E.toId);
        if (!T || !B) return !1;
        const { x1: L, y1: N, x2: P, y2: j } = Jr(T, B, et);
        return L >= w.x && L <= w.x + w.w && N >= w.y && N <= w.y + w.h && P >= w.x && P <= w.x + w.w && j >= w.y && j <= w.y + w.h;
      }
      const H = I.h === "auto" ? et[I.id] ?? 100 : I.h, M = qt(I, H);
      return M.minX >= w.x && M.maxX <= w.x + w.w && M.minY >= w.y && M.maxY <= w.y + w.h;
    }),
    [qt, et]
  ), Re = it(
    (w, A) => w.length < 3 ? [] : A.filter((I) => {
      if (I.type === "edge") {
        const T = I, B = t.getNode(T.data.fromId), L = t.getNode(T.data.toId);
        if (!B || !L) return !1;
        const { x1: N, y1: P, x2: j, y2: q } = Jr(B, L, et);
        return hn(N, P, w) && hn(j, q, w);
      }
      const H = I.h === "auto" ? et[I.id] ?? 100 : I.h, M = I.x + I.w / 2, E = I.y + H / 2;
      return hn(M, E, w);
    }),
    [t, et]
  ), _t = Kt(() => {
    if (d.size < 2) return null;
    let w = 1 / 0, A = 1 / 0, I = -1 / 0, H = -1 / 0;
    for (const M of d) {
      const E = h.find((L) => L.id === M);
      if (!E || E.type === "edge") continue;
      const T = E.h === "auto" ? et[E.id] ?? 100 : E.h, B = qt(E, T);
      w = Math.min(w, B.minX), A = Math.min(A, B.minY), I = Math.max(I, B.maxX), H = Math.max(H, B.maxY);
    }
    return w === 1 / 0 ? null : {
      x: w - $t,
      y: A - $t,
      w: I - w + $t * 2,
      h: H - A + $t * 2
    };
  }, [d, h, et, qt]), Ae = Kt(() => {
    if (!g) return null;
    const w = t.getAllGroupDescendantNodes(g);
    if (w.length === 0) return null;
    let A = 1 / 0, I = 1 / 0, H = -1 / 0, M = -1 / 0;
    for (const T of w) {
      if (T.type === "edge") continue;
      const B = T.h === "auto" ? et[T.id] ?? 100 : T.h, L = qt(T, B);
      A = Math.min(A, L.minX), I = Math.min(I, L.minY), H = Math.max(H, L.maxX), M = Math.max(M, L.maxY);
    }
    if (A === 1 / 0) return null;
    const E = 8;
    return { x: A - E, y: I - E, w: H - A + E * 2, h: M - I + E * 2 };
  }, [g, h, et, qt, t]), to = Kt(() => {
    const w = h.filter(
      (N) => {
        if (o) {
          const P = o.get(N.type);
          return P && !P.isSVGOnly;
        }
        return N.type === "content" || N.type === "draw" || N.type === "shape" || N.type === "image" || N.type === "text" || N.type === "frame" || N.type === "sticky";
      }
    ), A = a.zoom < 0.5 ? 15 : a.zoom < 1 ? 25 : 30;
    if (i.w <= 0 || i.h <= 0 || w.length < A)
      return null;
    const { zoom: I, x: H, y: M } = a, E = 500, T = {
      x: -H / I - E,
      y: -M / I - E,
      w: i.w / I + E * 2,
      h: i.h / I + E * 2
    }, B = t.getNodesInRect(T), L = /* @__PURE__ */ new Map();
    for (const N of B) {
      const P = t.getNode(N.id);
      P && L.set(N.id, P);
    }
    for (const N of d) {
      const P = t.getNode(N);
      P && L.set(P.id, P);
    }
    for (const N of h) {
      if (N.type !== "edge" || L.has(N.id)) continue;
      const P = N.data, j = t.getNode(P.fromId), q = t.getNode(P.toId);
      if (!j || !q) continue;
      let Q = L.has(P.fromId) || L.has(P.toId);
      if (!Q) {
        const { x1: ct, y1: lt, x2: kt, y2: Mt } = Jr(j, q, et);
        Q = Gl(ct, lt, kt, Mt, T);
      }
      Q && (L.set(N.id, N), L.has(j.id) || L.set(j.id, j), L.has(q.id) || L.set(q.id, q));
    }
    return Array.from(L.values());
  }, [a, i, h, d, t]), bo = Kt(() => to || h, [h, to]);
  mt(() => {
    let w = null;
    const A = () => {
      w === null && (w = requestAnimationFrame(() => {
        w = null, f([...t.getAllNodes()]);
      }));
    };
    let I = null;
    const H = () => {
      I === null && (I = requestAnimationFrame(() => {
        I = null, c({ ...t.viewport });
      }));
    }, M = () => {
      p((j) => {
        const q = new Set(t.selection);
        return j.size !== q.size || [...j].some((Q) => !q.has(Q)) ? (me((Q) => Q && !q.has(Q) ? null : Q), De((Q) => Q && !q.has(Q) ? null : Q), no((Q) => Q && !q.has(Q) ? null : Q), xo((Q) => Q && !q.has(Q) ? null : Q), wo((Q) => Q && !q.has(Q) ? null : Q), Be(null), q) : j;
      });
    }, E = () => {
      m(t.mode), t.mode === "text" && (Ko.current = !1);
    }, T = () => dt(t.boardBackground), B = () => K([...t.alignGuides]);
    t.on("change", A), t.on("viewport", H), t.on("selection", M), t.on("mode", E), t.on("background", T), t.on("guides", B);
    const L = (j) => b(j), N = () => b(null), P = () => {
      const j = n.current;
      j && (j.style.cursor = t.lassoSelect ? zo : wr(t.mode));
    };
    return t.on("group:enter", L), t.on("group:exit", N), t.on("lassoToggle", P), () => {
      w !== null && cancelAnimationFrame(w), I !== null && cancelAnimationFrame(I), t.off("change", A), t.off("viewport", H), t.off("selection", M), t.off("mode", E), t.off("background", T), t.off("guides", B), t.off("group:enter", L), t.off("group:exit", N), t.off("lassoToggle", P);
    };
  }, [t]), mt(() => {
    const w = n.current;
    if (!w) return;
    const A = (I) => {
      if (!I.ctrlKey && !I.metaKey) {
        const M = I.target.closest(".sb-editor-wrap");
        if (M && M.scrollHeight > M.clientHeight) {
          const E = M.scrollTop <= 0 && I.deltaY < 0, T = M.scrollTop + M.clientHeight >= M.scrollHeight && I.deltaY > 0;
          if (!E && !T) return;
        }
      }
      I.preventDefault(), I.ctrlKey || I.metaKey ? t.zoomByWheel(I.deltaY, I.clientX, I.clientY) : t.pan(-I.deltaX, -I.deltaY);
    };
    return w.addEventListener("wheel", A, { passive: !1 }), () => w.removeEventListener("wheel", A);
  }, [t]);
  const [ue, ve] = tt(null), [eo, oo] = tt(null), [fe, Se] = tt(null), [ge, Be] = tt(null), k = at({
    x: 0,
    y: 0,
    index: -1
  }), [st, Yt] = tt(null), [ne, be] = tt(null), Le = at(null), [ro, me] = tt(null), Go = at(null), [lr, De] = tt(null), [ls, no] = tt(null), [jo, xo] = tt(null), [cs, wo] = tt(null), [Sa, ds] = tt(null);
  mt(() => {
    const w = (A) => {
      _a(() => wo(A));
    };
    return t.on("image:cropRequest", w), () => t.off("image:cropRequest", w);
  }, [t]);
  const hs = ro || ls || lr || jo || cs || Sa, Yr = at(null), us = at(null), [Gr, jr] = tt(/* @__PURE__ */ new Set()), so = at(/* @__PURE__ */ new Set()), [fs, Vo] = tt([]), [cr, Vr] = tt(null), Me = at([]), Ne = at(null), [ps, dr] = tt([]), ae = at([]), ko = at(null), Ko = at(!1), ys = it(
    (w, A, I, H = "auto") => {
      const M = wt(10);
      us.current = M, t.addNode({
        id: M,
        type: "content",
        x: w,
        y: A,
        w: I,
        h: H,
        z: t.nextZ(),
        data: { blocks: [], borderColor: "#1e1e2e" }
      });
    },
    [t]
  ), hr = it(
    (w, A, I) => {
      const { x: H, y: M } = t.screenToCanvas(w, A);
      if (I) {
        const N = t.hitTestAll(H, M, et);
        if (N.length > 0) {
          const P = k.current, j = Math.abs(H - P.x) + Math.abs(M - P.y);
          let q = 0;
          j < 5 && (q = (P.index + 1) % N.length), k.current = { x: H, y: M, index: q }, t.select(N[q].id);
        } else
          t.deselectAll();
      } else {
        let N = !1;
        for (const P of t.selection) {
          const j = t.getNode(P);
          if (!j) continue;
          const q = j.h === "auto" ? 100 : j.h;
          if (H >= j.x && H <= j.x + j.w && M >= j.y && M <= j.y + q) {
            N = !0;
            break;
          }
        }
        if (!N && t.selection.size >= 2) {
          let P = 1 / 0, j = 1 / 0, q = -1 / 0, Q = -1 / 0;
          for (const ct of t.selection) {
            const lt = t.getNode(ct);
            if (!lt || lt.type === "edge") continue;
            const kt = lt.h === "auto" ? 100 : lt.h;
            P = Math.min(P, lt.x), j = Math.min(j, lt.y), q = Math.max(q, lt.x + lt.w), Q = Math.max(Q, lt.y + kt);
          }
          P !== 1 / 0 && H >= P && H <= q && M >= j && M <= Q && (N = !0);
        }
        if (!N) {
          const P = t.hitTest(H, M, et);
          P ? t.select(P.id) : t.deselectAll();
        }
      }
      const E = Array.from(t.selection), T = E.length > 0, B = [];
      if (B.push({
        items: [
          {
            label: "Cut",
            shortcut: "Mod+X",
            disabled: !T,
            action: () => {
              t.cutSelected(), ei(t);
            }
          },
          {
            label: "Copy",
            shortcut: "Mod+C",
            disabled: !T,
            action: () => {
              t.copySelected(), ei(t);
            }
          },
          {
            label: "Paste",
            shortcut: "Mod+V",
            disabled: !1,
            action: () => {
              Xh(t, H, M);
            }
          }
        ]
      }), B.push({
        items: [
          {
            label: "Duplicate",
            shortcut: "Mod+D",
            disabled: !T,
            action: () => t.duplicateSelected()
          }
        ]
      }), T && B.push({
        items: [
          {
            label: "Add to Personal Library",
            action: () => {
              const N = E.map((q) => t.getNode(q)).filter((q) => !!q).map((q) => structuredClone(q)), P = new Set(
                N.map((q) => q.groupId).filter(Boolean)
              ), j = /* @__PURE__ */ new Map();
              for (const [q, Q] of t.groupParent)
                P.has(q) && j.set(q, Q);
              Vr({
                nodes: N,
                groupParent: j
              });
            }
          }
        ]
      }), E.length >= 2 || T && t.selectionHasGroup()) {
        const N = [];
        E.length >= 2 && N.push({
          label: "Group selection",
          shortcut: "Mod+G",
          action: () => t.groupSelected()
        }), t.selectionHasGroup() && N.push({
          label: "Ungroup selection",
          shortcut: "Mod+Shift+G",
          action: () => t.ungroupSelected()
        }), B.push({ items: N });
      }
      if (T && E.every((P) => {
        const j = t.getNode(P);
        return j && (j.type === "draw" || j.type === "shape");
      }) && B.push({
        items: [
          {
            label: "Flip horizontal",
            shortcut: "Shift+H",
            action: () => t.flipSelectedHorizontal()
          },
          {
            label: "Flip vertical",
            shortcut: "Shift+V",
            action: () => t.flipSelectedVertical()
          }
        ]
      }), T && B.push({
        items: [
          {
            label: "Bring forward",
            shortcut: "Mod+]",
            action: () => t.bringForward(E)
          },
          {
            label: "Send backward",
            shortcut: "Mod+[",
            action: () => t.sendBackward(E)
          },
          {
            label: "Bring to front",
            shortcut: "Mod+Alt+]",
            action: () => t.bringToFront(E)
          },
          {
            label: "Send to back",
            shortcut: "Mod+Alt+[",
            action: () => t.sendToBack(E)
          }
        ]
      }), T) {
        const N = E.some((q) => {
          var Q;
          return (Q = t.getNode(q)) == null ? void 0 : Q.locked;
        }), P = E.some((q) => {
          var Q;
          return !((Q = t.getNode(q)) != null && Q.locked);
        }), j = [];
        P && j.push({
          label: "Lock",
          action: () => {
            for (const q of E) t.updateNode(q, { locked: !0 });
          }
        }), N && j.push({
          label: "Unlock",
          action: () => {
            for (const q of E) t.updateNode(q, { locked: void 0 });
          }
        }), B.push({ items: j });
      }
      T && B.push({
        items: [
          {
            label: "Delete",
            shortcut: "Delete",
            danger: !0,
            action: () => t.deleteSelected()
          }
        ]
      });
      const L = [10, 20, 40, 80];
      return B.push({
        items: [
          {
            label: "Toggle Grid",
            checked: t.snapToGrid,
            action: () => {
              t.toggleSnapToGrid(), C(t.snapToGrid);
            }
          },
          {
            label: "Smart Guides",
            checked: t.smartGuides,
            action: () => {
              t.toggleSmartGuides(), D(t.smartGuides);
            }
          },
          ...L.map((N) => ({
            label: `${N}px`,
            checked: t.gridSize === N,
            action: () => {
              t.gridSize = N, S(N);
            }
          }))
        ]
      }), B.push({
        items: [
          {
            label: "Export as PNG",
            action: () => Qs(t, { format: "png" })
          },
          {
            label: "Export as SVG",
            action: () => Qs(t, { format: "svg" })
          }
        ]
      }), B;
    },
    [t]
  ), Ma = it(
    (w) => {
      if (w.preventDefault(), t.presentationMode) return;
      const A = hr(w.clientX, w.clientY, w.altKey);
      Se({ x: w.clientX, y: w.clientY, sections: A });
    },
    [t, hr]
  ), qo = it(
    (w, A, I) => {
      const H = wt(10);
      t.addNode({
        id: H,
        type: "text",
        x: w,
        y: A,
        w: I,
        h: "auto",
        z: t.nextZ(),
        data: {
          text: "",
          fontSize: t.activeTool.fontSize ?? 20,
          fontFamily: t.activeTool.fontFamily ?? Ke,
          color: t.activeTool.color,
          align: t.activeTool.textAlign ?? "left",
          opacity: t.activeTool.opacity
        }
      }), t.select(H), Yr.current = H, me(H);
    },
    [t]
  ), Ca = it(
    (w) => {
      if (t.presentationMode) return;
      if (t.mode === "text" && Ko.current) {
        Ko.current = !1, n.current && (n.current.style.cursor = "text"), t.deselectAll();
        const { x: E, y: T } = t.screenToCanvas(w.clientX, w.clientY);
        qo(E, T, 300);
        return;
      }
      if (t.mode !== "select") return;
      const { x: A, y: I } = t.screenToCanvas(w.clientX, w.clientY), H = t.hitTestAll(A, I, et), M = H.find((E) => !t.isContainerType(E.type)) ?? H[0] ?? null;
      if (M != null && M.groupId) {
        const E = [];
        let T = M.groupId;
        for (; T; )
          E.push(T), T = t.groupParent.get(T);
        if (!t.activeGroupId) {
          t.enterGroup(E[E.length - 1]), t.select(M.id);
          return;
        }
        const B = E.indexOf(t.activeGroupId);
        if (B > 0) {
          t.enterGroup(E[B - 1]), t.select(M.id);
          return;
        }
      }
      if (M && M.type === "text") {
        t.select(M.id), Go.current = { clientX: w.clientX, clientY: w.clientY }, me(M.id);
        return;
      }
      if (M && M.type === "sticky") {
        t.select(M.id), no(M.id);
        return;
      }
      if (M && M.type === "frame") {
        t.select(M.id), De(M.id);
        return;
      }
      if (M && M.type === "shape") {
        const E = M.data, T = E.shape === "line" || E.shape === "arrow";
        t.select(M.id), T || xo(M.id);
        return;
      }
      if (M && M.type === "draw") {
        t.select(M.id);
        return;
      }
      if (!M || M.type === "draw") {
        const T = t.getAllNodes().filter((B) => B.type === "shape").sort((B, L) => L.z - B.z).find((B) => !(B.data.shape === "line" || B.data.shape === "arrow") && Wr(B, A, I, t.viewport.zoom, !0));
        if (T) {
          t.select(T.id), xo(T.id);
          return;
        }
      }
      M || (t.deselectAll(), qo(A, I, 300));
    },
    [t, et, qo]
  ), za = it(
    (w) => {
      if (J.current.set(w.pointerId, { x: w.clientX, y: w.clientY }), w.pointerType === "pen" && (R.current = !0), w.pointerType === "touch" && (J.current.size >= 2 || R.current)) {
        U.current = !0, O.current && (clearTimeout(O.current), O.current = null, _.current = null);
        const M = new Map(J.current), E = [...J.current.keys()].find((P) => P !== w.pointerId);
        E !== void 0 && s().dispatchEvent(
          new PointerEvent("pointerup", {
            pointerId: E,
            bubbles: !0,
            clientX: w.clientX,
            clientY: w.clientY
          })
        );
        const T = [...M.values()];
        let B = oi(T[0], T[1] ?? T[0]);
        const L = (P) => {
          if (!M.has(P.pointerId)) return;
          M.set(P.pointerId, { x: P.clientX, y: P.clientY });
          const j = [...M.values()];
          if (j.length < 2) return;
          const q = oi(j[0], j[1]);
          if (t.pan(q.mx - B.mx, q.my - B.my), B.dist > 1) {
            const Q = Math.min(Math.max(q.dist / B.dist, 0.9), 1.1);
            t.zoomByFactor(Q, q.mx, q.my);
          }
          B = q;
        }, N = (P) => {
          J.current.delete(P.pointerId), M.delete(P.pointerId), P.pointerType === "pen" && (R.current = !1), M.size < 2 && !R.current && (U.current = !1, s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", N), s().removeEventListener("pointercancel", N));
        };
        s().addEventListener("pointermove", L), s().addEventListener("pointerup", N), s().addEventListener("pointercancel", N);
        return;
      }
      if (U.current || t.presentationMode && !(w.button === 1 || w.button === 0 && X.current))
        return;
      if (fe && Se(null), w.pointerType === "touch") {
        const M = w.clientX, E = w.clientY, T = w.pointerId;
        _.current = { clientX: M, clientY: E }, O.current = setTimeout(() => {
          if (O.current = null, !_.current || U.current) return;
          const B = hr(M, E, !1);
          Se({ x: M, y: E, sections: B }), s().dispatchEvent(
            new PointerEvent("pointerup", {
              pointerId: T,
              bubbles: !0,
              clientX: M,
              clientY: E
            })
          ), _.current = null;
        }, 500);
      }
      if (w.button === 1 || w.button === 0 && X.current) {
        w.preventDefault(), rt.current = !0;
        const M = t.viewport.x, E = t.viewport.y, T = w.clientX, B = w.clientY, L = n.current;
        L && (L.style.cursor = "grabbing");
        const N = (j) => {
          t.viewport.x = M + (j.clientX - T), t.viewport.y = E + (j.clientY - B), c({ ...t.viewport });
        }, P = () => {
          rt.current = !1, L && (L.style.cursor = X.current ? "grab" : t.lassoSelect ? zo : ""), s().removeEventListener("pointermove", N), s().removeEventListener("pointerup", P);
        };
        s().addEventListener("pointermove", N), s().addEventListener("pointerup", P);
        return;
      }
      const { x: I, y: H } = t.screenToCanvas(w.clientX, w.clientY);
      if (w.pointerType === "touch" && O.current && t.hitTest(I, H, et) && (clearTimeout(O.current), O.current = null, _.current = null), t.mode === "select") {
        if (w.button !== 0) return;
        if (w.altKey) {
          const T = t.hitTestAll(I, H, et);
          if (T.length > 0) {
            const B = k.current, L = Math.abs(I - B.x) + Math.abs(H - B.y);
            let N = 0;
            L < 5 && (N = (B.index + 1) % T.length), k.current = { x: I, y: H, index: N }, t.select(T[N].id);
          }
          return;
        }
        let M = !1;
        !t.lassoSelect && t.selection.size >= 2 && _t && I >= _t.x && I <= _t.x + _t.w && H >= _t.y && H <= _t.y + _t.h && (M = !0);
        let E = null;
        if (!t.lassoSelect) {
          const T = t.hitTestAll(I, H, et);
          E = T.find((B) => t.selection.has(B.id) && !t.isContainerType(B.type)) ?? T.find((B) => !t.isContainerType(B.type)) ?? T[0] ?? null, !E && !M && (E = Kl(t.nodes, I, H, t.viewport.zoom, et, Jt));
        }
        if (E || M) {
          E && (t.activeGroupId && !t.isNodeInActiveGroup(E.id) && t.exitAllGroups(), w.shiftKey ? t.toggleSelect(E.id) : t.selection.has(E.id) || t.select(E.id));
          const T = Array.from(t.selection).filter(
            (vt) => {
              var Ct;
              return !((Ct = t.getNode(vt)) != null && Ct.locked);
            }
          );
          if (T.length === 0) return;
          const B = w.clientX, L = w.clientY, N = /* @__PURE__ */ new Set(), P = /* @__PURE__ */ new Set();
          for (const vt of T) {
            const Ct = t.getNode(vt);
            if (Ct && t.isContainerType(Ct.type)) {
              P.add(vt);
              for (const Ht of t.getFrameDescendantIds(vt))
                t.selection.has(Ht) || N.add(Ht);
            }
          }
          const j = [...T, ...N], q = j.map((vt) => {
            const Ct = t.getNode(vt);
            return { id: vt, x: Ct.x, y: Ct.y };
          }), Q = t.selectionGroupId(), ct = Q ? t.groupRotations.get(Q) : null, lt = ct == null ? void 0 : ct.cx, kt = ct == null ? void 0 : ct.cy;
          Be(null);
          let Mt = !1, xt = null, Tt = B, Xt = L, Ot = !1;
          const pt = new Set(j), Lt = () => {
            xt = null;
            const vt = (Tt - B) / t.viewport.zoom, Ct = (Xt - L) / t.viewport.zoom, { finalDx: Ht, finalDy: jt } = t.computeDragSnap(
              q,
              pt,
              vt,
              Ct,
              Ot
            ), oe = q.map((re) => ({
              id: re.id,
              patch: { x: re.x + Ht, y: re.y + jt }
            }));
            t.updateMany(oe), ct && Q && t.groupRotations.set(Q, {
              angle: ct.angle,
              cx: lt + Ht,
              cy: kt + jt
            });
          }, bt = (vt) => {
            const Ct = (vt.clientX - B) / t.viewport.zoom, Ht = (vt.clientY - L) / t.viewport.zoom;
            if (!Mt)
              if (Math.abs(Ct) > 2 || Math.abs(Ht) > 2)
                Mt = !0, t.pushHistorySnapshot();
              else
                return;
            Tt = vt.clientX, Xt = vt.clientY, Ot = vt.metaKey || vt.ctrlKey, xt === null && (xt = requestAnimationFrame(Lt));
          }, Gt = () => {
            if (xt !== null && (cancelAnimationFrame(xt), Lt()), t.clearAlignGuides(), s().removeEventListener("pointermove", bt), s().removeEventListener("pointerup", Gt), Mt) {
              const vt = T.filter(
                (Ct) => !N.has(Ct)
              );
              vt.length > 0 && t.updateFrameMembership(vt);
            }
          };
          s().addEventListener("pointermove", bt), s().addEventListener("pointerup", Gt);
        } else {
          if (t.activeGroupId) {
            t.exitGroup();
            return;
          }
          w.shiftKey || t.deselectAll();
          const T = new Set(t.selection);
          if (t.lassoSelect) {
            const B = [[I, H]];
            oo([...B]);
            let L = null, N = 0;
            const P = (Q = !1) => {
              L = null;
              const ct = Q || N % 2 === 0;
              if (N++, ct && B.length >= 3) {
                const kt = Re(B, t.getAllNodes()).map((xt) => xt.id), Mt = w.shiftKey ? [.../* @__PURE__ */ new Set([...T, ...kt])] : kt;
                (Mt.length !== t.selection.size || Mt.some((xt) => !t.selection.has(xt))) && t.selectMultiple(Mt);
              }
              oo([...B]);
            }, j = (Q) => {
              const { x: ct, y: lt } = t.screenToCanvas(Q.clientX, Q.clientY);
              B.push([ct, lt]), L === null && (L = requestAnimationFrame(() => P(!1)));
            }, q = () => {
              L !== null && cancelAnimationFrame(L), P(!0), s().removeEventListener("pointermove", j), s().removeEventListener("pointerup", q), oo(null), t.toggleLassoSelect();
            };
            s().addEventListener("pointermove", j), s().addEventListener("pointerup", q);
          } else {
            const B = { startX: I, startY: H, endX: I, endY: H };
            ve(B);
            let L = null, N = 0;
            const P = (Q = !1, ct = !1) => {
              L = null;
              const lt = Math.min(B.startX, B.endX), kt = Math.min(B.startY, B.endY), Mt = Math.abs(B.endX - B.startX), xt = Math.abs(B.endY - B.startY), Tt = ct || Q || N % 2 === 0;
              if (N++, Tt) {
                const Ot = he(
                  { x: lt, y: kt, w: Mt, h: xt },
                  t.getAllNodes()
                ).map((Lt) => Lt.id), pt = w.shiftKey ? [.../* @__PURE__ */ new Set([...T, ...Ot])] : Ot;
                (pt.length !== t.selection.size || pt.some((Lt) => !t.selection.has(Lt))) && t.selectMultiple(pt);
              }
              ve({ ...B });
            }, j = (Q) => {
              const { x: ct, y: lt } = t.screenToCanvas(Q.clientX, Q.clientY);
              B.endX = ct, B.endY = lt, L === null && (L = requestAnimationFrame(() => P(!1)));
            }, q = () => {
              L !== null && cancelAnimationFrame(L), P(!0), s().removeEventListener("pointermove", j), s().removeEventListener("pointerup", q), ve(null);
            };
            s().addEventListener("pointermove", j), s().addEventListener("pointerup", q);
          }
        }
      } else if (t.mode === "text") {
        if (Ko.current) return;
        t.deselectAll();
        const M = I, E = H, T = {
          startX: I,
          startY: H,
          endX: I,
          endY: H
        };
        let B = !1;
        Yt(T);
        const L = (P) => {
          const { x: j, y: q } = t.screenToCanvas(P.clientX, P.clientY);
          T.endX = j, T.endY = q;
          const Q = Math.abs(T.endX - T.startX), ct = Math.abs(T.endY - T.startY);
          (Q > 10 || ct > 10) && (B = !0), Yt({ ...T });
        }, N = () => {
          s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", N), Yt(null);
          const P = B ? Math.max(Math.abs(T.endX - T.startX), 60) : 300, j = B ? Math.min(T.startX, T.endX) : M, q = B ? Math.min(T.startY, T.endY) : E;
          qo(j, q, P), Ko.current = !0, n.current && (n.current.style.cursor = "crosshair");
        };
        s().addEventListener("pointermove", L), s().addEventListener("pointerup", N);
      } else if (t.mode === "note") {
        t.deselectAll();
        const M = I, E = H, T = {
          startX: I,
          startY: H,
          endX: I,
          endY: H
        };
        let B = !1;
        Yt(T);
        const L = (P) => {
          const { x: j, y: q } = t.screenToCanvas(P.clientX, P.clientY);
          T.endX = j, T.endY = q;
          const Q = Math.abs(T.endX - T.startX), ct = Math.abs(T.endY - T.startY);
          (Q > 10 || ct > 10) && (B = !0), Yt({ ...T });
        }, N = () => {
          s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", N), Yt(null);
          const P = B ? Math.max(Math.abs(T.endX - T.startX), 100) : 300, j = B ? Math.max(Math.abs(T.endY - T.startY), 40) : "auto", q = B ? Math.min(T.startX, T.endX) : M, Q = B ? Math.min(T.startY, T.endY) : E;
          ys(q, Q, P, j), t.setMode("select");
        };
        s().addEventListener("pointermove", L), s().addEventListener("pointerup", N);
      } else if (t.mode === "sticky") {
        t.deselectAll();
        const M = I, E = H, T = {
          startX: I,
          startY: H,
          endX: I,
          endY: H
        };
        let B = !1;
        Yt(T);
        const L = (P) => {
          const { x: j, y: q } = t.screenToCanvas(P.clientX, P.clientY);
          T.endX = j, T.endY = q, Math.abs(T.endX - T.startX) > 10 && (B = !0), Yt({ ...T });
        }, N = () => {
          s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", N), Yt(null);
          const P = B ? Math.max(Math.abs(T.endX - T.startX), 100) : 200, j = B ? Math.min(T.startX, T.endX) : M, q = B ? Math.min(T.startY, T.endY) : E, Q = wt(10), ct = B ? Math.max(Math.abs(T.endY - T.startY), 100) : 150;
          t.addNode({
            id: Q,
            type: "sticky",
            x: j,
            y: q,
            w: P,
            h: ct,
            z: t.nextZ(),
            data: { text: "", color: "#FEF3C7" }
          }), t.select(Q), no(Q), t.setMode("select");
        };
        s().addEventListener("pointermove", L), s().addEventListener("pointerup", N);
      } else if (t.mode === "draw") {
        const M = w.pressure || 0.5, E = {
          points: [[I, H, M]],
          color: t.activeTool.color,
          width: t.activeTool.width,
          strokeStyle: t.activeTool.strokeStyle
        };
        V(E), t.notifyDrawProgress(E);
        const T = (L) => {
          const { x: N, y: P } = t.screenToCanvas(L.clientX, L.clientY), j = L.pressure || 0.5;
          E.points.push([N, P, j]), V({ ...E, points: [...E.points] }), t.notifyDrawProgress({ ...E, points: [...E.points] });
        }, B = () => {
          if (s().removeEventListener("pointermove", T), s().removeEventListener("pointerup", B), t.notifyDrawEnd(), E.points.length < 2) {
            V(null);
            return;
          }
          let L = 1 / 0, N = 1 / 0, P = -1 / 0, j = -1 / 0;
          for (const [Q, ct] of E.points)
            Q < L && (L = Q), ct < N && (N = ct), Q > P && (P = Q), ct > j && (j = ct);
          const q = E.points.map(
            ([Q, ct, lt]) => [Q - L, ct - N, lt]
          );
          t.addNode({
            id: wt(10),
            type: "draw",
            x: L,
            y: N,
            w: P - L,
            h: j - N,
            z: t.nextZ(),
            data: {
              tool: "pen",
              points: q,
              color: E.color,
              strokeWidth: E.width,
              opacity: t.activeTool.opacity,
              fill: t.activeTool.fillColor || void 0,
              fillStyle: t.activeTool.fillStyle || void 0,
              strokeStyle: t.activeTool.strokeStyle || void 0
            }
          }), requestAnimationFrame(() => V(null));
        };
        s().addEventListener("pointermove", T), s().addEventListener("pointerup", B);
      } else if (t.mode === "shape") {
        const M = {
          startX: I,
          startY: H,
          endX: I,
          endY: H
        };
        $(M);
        const E = {
          shapeType: t.activeTool.shapeType || "rect",
          stroke: t.activeTool.color,
          strokeWidth: t.activeTool.width
        }, T = (L) => {
          const { x: N, y: P } = t.screenToCanvas(L.clientX, L.clientY);
          M.endX = N, M.endY = P, $({ ...M }), t.notifyShapeProgress({ ...M, ...E });
        }, B = () => {
          s().removeEventListener("pointermove", T), s().removeEventListener("pointerup", B), t.notifyShapeEnd();
          const L = t.activeTool.shapeType || "rect", N = L === "line" || L === "arrow", P = Math.min(M.startX, M.endX);
          let j = Math.min(M.startY, M.endY);
          const q = Math.abs(M.endX - M.startX), Q = Math.abs(M.endY - M.startY);
          let ct;
          if (N) {
            const Mt = t.activeTool.width * 2;
            ct = Math.max(Q, Mt), Q < Mt && (j -= (Mt - Q) / 2);
          } else
            ct = Q;
          if (q < 5 && (N ? q < 5 && Math.abs(M.endY - M.startY) < 5 : ct < 5)) {
            $(null);
            return;
          }
          const lt = {};
          N && (lt.startPoint = [
            M.startX - P,
            M.startY - j
          ], lt.endPoint = [
            M.endX - P,
            M.endY - j
          ]);
          const kt = wt(10);
          t.addNode({
            id: kt,
            type: "shape",
            x: P,
            y: j,
            w: q,
            h: ct,
            z: t.nextZ(),
            data: {
              shape: L,
              stroke: t.activeTool.color,
              fill: t.activeTool.fillColor || void 0,
              fillStyle: t.activeTool.fillStyle,
              strokeWidth: t.activeTool.width,
              strokeStyle: t.activeTool.strokeStyle,
              roughness: t.activeTool.roughness ?? 1,
              opacity: t.activeTool.opacity ?? 1,
              ...lt
            }
          }), $(null), t.setMode("select"), t.select(kt);
        };
        s().addEventListener("pointermove", T), s().addEventListener("pointerup", B);
      } else if (t.mode === "edge") {
        const M = t.hitTest(I, H, et);
        if (!M || M.type === "edge") return;
        ot({ fromNode: M, cursorX: I, cursorY: H });
        const E = (B) => {
          const { x: L, y: N } = t.screenToCanvas(B.clientX, B.clientY);
          ot(
            (P) => P ? { ...P, cursorX: L, cursorY: N } : null
          );
        }, T = (B) => {
          s().removeEventListener("pointermove", E), s().removeEventListener("pointerup", T), ot(null);
          const { x: L, y: N } = t.screenToCanvas(B.clientX, B.clientY);
          let P = t.hitTest(L, N, et);
          if (!P || P.type === "edge" || t.isContainerType(P.type)) {
            const lt = 50 / t.viewport.zoom;
            let kt = 1 / 0, Mt = !1, xt = null;
            for (const Tt of t.getAllNodes()) {
              if (Tt.type === "edge" || Tt.id === M.id) continue;
              const Xt = t.isContainerType(Tt.type), Ot = Wo(Tt, et);
              for (const pt of Ot) {
                const Lt = Math.hypot(pt.x - L, pt.y - N);
                Lt >= lt || Xt && !Mt && xt || (!Xt && Mt || Lt < kt) && (kt = Lt, Mt = Xt, xt = Tt);
              }
            }
            xt && (P = xt);
          }
          if (!P || P.type === "edge" || P.id === M.id || t.getAllNodes().some(
            (lt) => lt.type === "edge" && (lt.data.fromId === M.id && lt.data.toId === P.id || lt.data.fromId === P.id && lt.data.toId === M.id)
          )) return;
          const q = pr(M, I, H, et), Q = pr(P, L, N, et), ct = {
            id: wt(10),
            type: "edge",
            x: 0,
            y: 0,
            w: 0,
            h: 0,
            z: t.nextZ(),
            data: {
              fromId: M.id,
              toId: P.id,
              style: "solid",
              color: t.activeTool.color,
              strokeWidth: 2,
              arrowHead: "arrow",
              arrowTail: "none",
              edgeType: "bezier",
              sourceHandle: q,
              targetHandle: Q
            }
          };
          t.addNode(ct), t.select(ct.id);
        };
        s().addEventListener("pointermove", E), s().addEventListener("pointerup", T);
      } else if (t.mode === "frame") {
        const M = {
          startX: I,
          startY: H,
          endX: I,
          endY: H
        };
        $(M);
        const E = (B) => {
          const { x: L, y: N } = t.screenToCanvas(B.clientX, B.clientY);
          M.endX = L, M.endY = N, $({ ...M });
        }, T = () => {
          s().removeEventListener("pointermove", E), s().removeEventListener("pointerup", T);
          const B = Math.min(M.startX, M.endX), L = Math.min(M.startY, M.endY), N = Math.abs(M.endX - M.startX), P = Math.abs(M.endY - M.startY);
          if (N < 20 || P < 20) {
            $(null);
            return;
          }
          const j = wt(10);
          t.addNode({
            id: j,
            type: "frame",
            x: B,
            y: L,
            w: N,
            h: P,
            z: t.nextZ(),
            data: {
              label: "Frame",
              backgroundColor: "rgba(0, 0, 0, 0.02)",
              borderColor: "#1e1e2e",
              borderWidth: 1,
              borderStyle: "dashed"
            }
          }), t.adoptNodesIntoNewFrame(j), $(null), t.select(j), t.setMode("select");
        };
        s().addEventListener("pointermove", E), s().addEventListener("pointerup", T);
      } else if (t.mode === "erase") {
        if (w.button !== 0) return;
        const M = (lt, kt) => {
          const Mt = t.hitTestAll(lt, kt, et), xt = Vl(
            t.nodes,
            lt,
            kt,
            t.viewport.zoom,
            et,
            Jt
          );
          let Tt = !1;
          for (const Xt of [...Mt, ...xt])
            so.current.has(Xt.id) || (so.current.add(Xt.id), Tt = !0);
          Tt && jr(new Set(so.current));
        }, E = 400;
        so.current = /* @__PURE__ */ new Set();
        const T = performance.now();
        Me.current = [[I, H, T]], Vo([[I, H, T]]), M(I, H);
        let B = I, L = H;
        const N = () => {
          const lt = performance.now(), kt = Me.current.length;
          Me.current = Me.current.filter(
            (Mt) => lt - Mt[2] < E
          ), Me.current.length !== kt && Vo([...Me.current]), Ne.current = requestAnimationFrame(N);
        };
        Ne.current = requestAnimationFrame(N);
        const P = (lt) => {
          const { x: kt, y: Mt } = t.screenToCanvas(lt.clientX, lt.clientY);
          B = kt, L = Mt;
          const xt = performance.now();
          Me.current.push([B, L, xt]), Vo([...Me.current]), M(B, L);
        }, j = () => {
          Ne.current !== null && (cancelAnimationFrame(Ne.current), Ne.current = null), so.current = /* @__PURE__ */ new Set(), jr(/* @__PURE__ */ new Set()), Me.current = [], Vo([]);
        }, q = () => {
          ct();
          const lt = Array.from(so.current);
          j(), lt.length > 0 && t.deleteNodes(lt);
        }, Q = (lt) => {
          lt.key === "Escape" && (ct(), j());
        }, ct = () => {
          s().removeEventListener("pointermove", P), s().removeEventListener("pointerup", q), s().removeEventListener("keydown", Q);
        };
        s().addEventListener("pointermove", P), s().addEventListener("pointerup", q), s().addEventListener("keydown", Q);
      } else if (t.mode === "laser") {
        if (w.button !== 0) return;
        const M = 1560;
        ko.current !== null && (cancelAnimationFrame(ko.current), ko.current = null);
        const E = performance.now();
        ae.current.length > 0 && ae.current.push([NaN, NaN, E]), ae.current.push([I, H, E]), dr([...ae.current]), t.notifyLaserProgress([[I, H]]);
        let T = E;
        const B = () => {
          const P = performance.now(), j = ae.current.length;
          ae.current = ae.current.filter(
            (q) => P - q[2] < M
          ), (ae.current.length !== j || ae.current.length > 0) && dr([...ae.current]), P - T >= 60 && (T = P, ae.current.length > 0 && t.notifyLaserProgress(
            ae.current.map((q) => [q[0], q[1]])
          )), ae.current.length > 0 ? ko.current = requestAnimationFrame(B) : (ko.current = null, dr([]), t.notifyLaserEnd());
        };
        ko.current = requestAnimationFrame(B);
        const L = (P) => {
          const { x: j, y: q } = t.screenToCanvas(P.clientX, P.clientY), Q = performance.now();
          ae.current.push([j, q, Q]), dr([...ae.current]), t.notifyLaserProgress(
            ae.current.map((ct) => [ct[0], ct[1]])
          );
        }, N = () => {
          s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", N);
        };
        s().addEventListener("pointermove", L), s().addEventListener("pointerup", N);
      } else if (t.mode === "hand") {
        if (w.button !== 0) return;
        w.preventDefault();
        const M = t.viewport.x, E = t.viewport.y, T = w.clientX, B = w.clientY, L = n.current;
        L && (L.style.cursor = "grabbing");
        const N = (j) => {
          t.viewport.x = M + (j.clientX - T), t.viewport.y = E + (j.clientY - B), c({ ...t.viewport });
        }, P = () => {
          L && (L.style.cursor = t.lassoSelect ? zo : wr(t.mode)), s().removeEventListener("pointermove", N), s().removeEventListener("pointerup", P);
        };
        s().addEventListener("pointermove", N), s().addEventListener("pointerup", P);
      }
    },
    [
      t,
      ys,
      qo,
      fe,
      hr,
      _t,
      et,
      qt,
      he
    ]
  ), Kr = it(
    (w, A, I) => {
      if (I.preventDefault(), t.presentationMode) return;
      const H = t.getNode(w);
      if (!H || H.locked) return;
      const M = I.clientX, E = I.clientY, T = H.x, B = H.y, L = H.w, N = H.h === "auto", P = N ? et[w] ?? 100 : H.h, j = H.type === "draw" ? H.data.points.map(
        (Mt) => [...Mt]
      ) : null, q = H.type === "shape" ? H.data.startPoint : void 0, Q = H.type === "shape" ? H.data.endPoint : void 0, ct = H.type === "text" ? H.data.fontSize : 0;
      t.pushHistorySnapshot();
      const lt = (Mt) => {
        const xt = (Mt.clientX - M) / t.viewport.zoom, Tt = (Mt.clientY - E) / t.viewport.zoom;
        let Xt = T, Ot = B, pt = L, Lt = P;
        if ((A === "nw" || A === "w" || A === "sw") && (Xt = T + xt, pt = L - xt), (A === "ne" || A === "e" || A === "se") && (pt = L + xt), (A === "nw" || A === "n" || A === "ne") && (Ot = B + Tt, Lt = P - Tt), (A === "sw" || A === "s" || A === "se") && (Lt = P + Tt), t.snapToGrid && !(Mt.metaKey || Mt.ctrlKey)) {
          const vt = t.gridSize, Ct = (Ht) => Math.round(Ht / vt) * vt;
          (A === "nw" || A === "w" || A === "sw") && (Xt = Ct(Xt), pt = T + L - Xt), (A === "ne" || A === "e" || A === "se") && (pt = Ct(Xt + pt) - Xt), (A === "nw" || A === "n" || A === "ne") && (Ot = Ct(Ot), Lt = B + P - Ot), (A === "sw" || A === "s" || A === "se") && (Lt = Ct(Ot + Lt) - Ot);
        }
        const bt = 10;
        if (pt < bt && (pt = bt, (A === "nw" || A === "w" || A === "sw") && (Xt = T + L - bt)), Lt < bt && (Lt = bt, (A === "nw" || A === "n" || A === "ne") && (Ot = B + P - bt)), H.type === "frame") {
          const vt = H.data.devicePreset;
          if (vt) {
            const Ct = An(vt);
            if (Ct) {
              const Ht = wa(Ct);
              if (A === "nw" || A === "ne" || A === "sw" || A === "se" || (A === "e" || A === "w")) {
                const re = Math.round(pt / Ht);
                (A === "nw" || A === "ne") && (Ot = B + P - re), Lt = re;
              } else
                pt = Math.round(Lt * Ht);
            }
          }
        }
        const Gt = {
          x: Xt,
          y: Ot,
          w: pt,
          h: N ? "auto" : Lt
        };
        if (j && H.type === "draw") {
          const vt = L > 0 ? pt / L : 1, Ct = P > 0 ? Lt / P : 1, Ht = j.map(
            ([jt, oe, re]) => [jt * vt, oe * Ct, re]
          );
          Gt.data = { ...H.data, points: Ht };
        }
        if (H.type === "shape" && (q || Q)) {
          const vt = L > 0 ? pt / L : 1, Ct = P > 0 ? Lt / P : 1, Ht = { ...H.data };
          q && (Ht.startPoint = [
            q[0] * vt,
            q[1] * Ct
          ]), Q && (Ht.endPoint = [
            Q[0] * vt,
            Q[1] * Ct
          ]), Gt.data = Ht;
        }
        if (H.type === "text" && ct > 0 && A !== "e" && A !== "w") {
          const vt = A === "n" || A === "s" ? P > 0 ? Lt / P : 1 : L > 0 ? pt / L : 1, Ct = Math.max(8, Math.round(ct * vt));
          Gt.data = { ...H.data, fontSize: Ct };
        }
        t.updateNode(w, Gt);
      }, kt = () => {
        s().removeEventListener("pointermove", lt), s().removeEventListener("pointerup", kt), t.isContainerType(H.type) && t.syncFrameChildrenAfterResize(w);
      };
      s().addEventListener("pointermove", lt), s().addEventListener("pointerup", kt);
    },
    [t, et]
  ), Ia = it(
    (w, A) => {
      A.stopPropagation(), A.preventDefault();
      const I = t.getNode(w);
      if (!I || I.locked) return;
      const H = I.h === "auto" ? et[w] ?? 100 : I.h, M = I.x + I.w / 2, E = I.y + H / 2, T = I.rotation || 0, { x: B, y: L } = t.screenToCanvas(
        A.clientX,
        A.clientY
      ), N = Math.atan2(L - E, B - M);
      t.pushHistorySnapshot();
      const P = (q) => {
        const { x: Q, y: ct } = t.screenToCanvas(q.clientX, q.clientY), lt = Math.atan2(ct - E, Q - M);
        let kt = T + (lt - N) * (180 / Math.PI);
        (q.shiftKey || t.snapToGrid) && !(q.metaKey || q.ctrlKey) && (kt = Math.round(kt / 15) * 15), t.updateNode(w, { rotation: kt });
      }, j = () => {
        s().removeEventListener("pointermove", P), s().removeEventListener("pointerup", j);
      };
      s().addEventListener("pointermove", P), s().addEventListener("pointerup", j);
    },
    [t, et]
  ), ms = it(
    (w, A, I) => {
      I.stopPropagation(), I.preventDefault();
      const H = t.getNode(w);
      if (!H) return;
      const { x: M, y: E } = t.screenToCanvas(I.clientX, I.clientY);
      ot({ fromNode: H, cursorX: M, cursorY: E, sourceHandle: A });
      const T = (L) => {
        const { x: N, y: P } = t.screenToCanvas(L.clientX, L.clientY);
        ot(
          (j) => j ? { ...j, cursorX: N, cursorY: P } : null
        );
      }, B = (L) => {
        s().removeEventListener("pointermove", T), s().removeEventListener("pointerup", B), ot(null);
        const { x: N, y: P } = t.screenToCanvas(L.clientX, L.clientY);
        let j = t.hitTest(N, P, et);
        if (!j || j.type === "edge" || t.isContainerType(j.type)) {
          const lt = 50 / t.viewport.zoom;
          let kt = 1 / 0, Mt = !1, xt = null;
          for (const Tt of t.getAllNodes()) {
            if (Tt.type === "edge" || Tt.id === H.id) continue;
            const Xt = t.isContainerType(Tt.type), Ot = Wo(Tt, et);
            for (const pt of Ot) {
              const Lt = Math.hypot(pt.x - N, pt.y - P);
              Lt >= lt || Xt && !Mt && xt || (!Xt && Mt || Lt < kt) && (kt = Lt, Mt = Xt, xt = Tt);
            }
          }
          xt && (j = xt);
        }
        if (!j || j.type === "edge" || j.id === H.id || t.getAllNodes().some(
          (lt) => lt.type === "edge" && (lt.data.fromId === H.id && lt.data.toId === j.id || lt.data.fromId === j.id && lt.data.toId === H.id)
        )) return;
        const Q = pr(j, N, P, et), ct = {
          id: wt(10),
          type: "edge",
          x: 0,
          y: 0,
          w: 0,
          h: 0,
          z: t.nextZ(),
          data: {
            fromId: H.id,
            toId: j.id,
            style: "solid",
            color: t.activeTool.color,
            strokeWidth: 2,
            arrowHead: "arrow",
            arrowTail: "none",
            edgeType: "bezier",
            sourceHandle: A,
            targetHandle: Q
          }
        };
        t.addNode(ct), t.select(ct.id);
      };
      s().addEventListener("pointermove", T), s().addEventListener("pointerup", B);
    },
    [t, et]
  ), Ta = it(
    (w) => {
      let A = null, I = w === "top" || w === "left" ? 1 / 0 : -1 / 0;
      for (const H of t.selection) {
        const M = t.getNode(H);
        if (!M || M.type === "edge") continue;
        const E = M.h === "auto" ? et[M.id] ?? 100 : M.h;
        let T;
        switch (w) {
          case "top":
            T = M.y;
            break;
          case "bottom":
            T = M.y + E;
            break;
          case "left":
            T = M.x;
            break;
          case "right":
            T = M.x + M.w;
            break;
        }
        (w === "top" || w === "left" ? T < I : T > I) && (I = T, A = H);
      }
      return A;
    },
    [t, et]
  ), Pa = it(
    (w, A, I, H) => {
      var q;
      H.stopPropagation(), H.preventDefault();
      const M = t.getNode(w);
      if (!M || !o) return;
      const E = o.get(M.type), T = (q = E == null ? void 0 : E.ports) == null ? void 0 : q.find((Q) => Q.id === A);
      if (!T) return;
      const B = I === "input" ? "left" : "right", { x: L, y: N } = t.screenToCanvas(H.clientX, H.clientY);
      ot({
        fromNode: M,
        cursorX: L,
        cursorY: N,
        sourceHandle: B,
        sourcePort: A,
        sourceDirection: I
      });
      const P = (Q) => {
        const { x: ct, y: lt } = t.screenToCanvas(Q.clientX, Q.clientY);
        ot(
          (kt) => kt ? { ...kt, cursorX: ct, cursorY: lt } : null
        );
      }, j = (Q) => {
        var Ee;
        s().removeEventListener("pointermove", P), s().removeEventListener("pointerup", j), ot(null);
        const { x: ct, y: lt } = t.screenToCanvas(Q.clientX, Q.clientY), kt = I === "output" ? "input" : "output", Mt = 40 / t.viewport.zoom;
        let xt = null, Tt = null, Xt = 1 / 0;
        for (const ee of t.getAllNodes()) {
          if (ee.type === "edge" || ee.id === M.id) continue;
          const se = o.get(ee.type);
          if (!((Ee = se == null ? void 0 : se.ports) != null && Ee.length)) continue;
          const vo = ee.h === "auto" ? t.measuredHeights[ee.id] ?? 100 : ee.h;
          for (const xe of se.ports) {
            if (xe.direction !== kt || T.dataType !== "any" && xe.dataType !== "any" && T.dataType !== xe.dataType) continue;
            const So = se.ports.filter((Xa) => Xa.direction === xe.direction), Zr = So.indexOf(xe), ur = 14 / t.viewport.zoom, Ha = ee.y + vo / (So.length + 1) * (Zr + 1), Oa = xe.direction === "input" ? ee.x - ur : ee.x + ee.w + ur, Ur = Math.hypot(Oa - ct, Ha - lt);
            Ur < Mt && Ur < Xt && (Xt = Ur, xt = ee, Tt = xe);
          }
        }
        if (!xt || !Tt) return;
        const Ot = Tt.id, pt = I === "output" ? xt.id : M.id, Lt = I === "output" ? Ot : A;
        if (t.getAllNodes().some((ee) => {
          if (ee.type !== "edge") return !1;
          const se = ee.data;
          return se.toId === pt && se.targetPort === Lt;
        })) return;
        const Gt = I === "output" ? M.id : xt.id, vt = I === "output" ? xt.id : M.id, Ct = I === "output" ? A : Ot, Ht = I === "output" ? Ot : A, re = {
          id: wt(10),
          type: "edge",
          x: 0,
          y: 0,
          w: 0,
          h: 0,
          z: t.nextZ(),
          data: {
            fromId: Gt,
            toId: vt,
            style: "solid",
            color: "#6b7280",
            strokeWidth: 2,
            arrowHead: "filled",
            arrowTail: "none",
            edgeType: "bezier",
            sourceHandle: "right",
            targetHandle: "left",
            sourcePort: Ct,
            targetPort: Ht
          }
        };
        t.addNode(re), t.select(re.id);
      };
      s().addEventListener("pointermove", P), s().addEventListener("pointerup", j);
    },
    [t, o, et]
  ), [gs, Ra] = tt(0);
  mt(() => {
    if (r)
      return r.onChange(() => Ra((w) => w + 1));
  }, [r]);
  const Aa = it(
    (w, A, I, H, M) => {
      M.stopPropagation(), M.preventDefault();
      const E = t.getNode(w);
      if (!E || E.type !== "edge") return;
      t.pushHistorySnapshot();
      const T = (L) => {
        const N = t.screenToCanvas(L.clientX, L.clientY), P = t.getNode(w);
        if (!P) return;
        const j = t.getNode(P.data.fromId), q = t.getNode(P.data.toId);
        if (!(!j || !q))
          if (A === "xy") {
            const Q = je(
              j,
              q,
              P.data.edgeType || "bezier",
              et,
              P.data.sourceHandle,
              P.data.targetHandle,
              void 0,
              void 0
              // no offsets → natural midpoint
            );
            if (!Q.kinkHandle) return;
            const ct = N.x - Q.kinkHandle.x, lt = N.y - Q.kinkHandle.y;
            t.updateNode(w, {
              data: { ...P.data, curveOffset: [ct, lt] }
            });
          } else {
            const Q = A === "x" ? N.x : N.y, ct = je(
              j,
              q,
              P.data.edgeType || "bezier",
              et,
              P.data.sourceHandle,
              P.data.targetHandle,
              0.5
              // default to get range
            );
            if (!ct.kinkHandle) return;
            const lt = ct.kinkHandle.min, kt = ct.kinkHandle.max, Mt = kt - lt;
            if (Mt === 0) return;
            const Tt = (Math.max(lt, Math.min(kt, Q)) - lt) / Mt;
            t.updateNode(w, {
              data: { ...P.data, midpointOffset: Tt }
            });
          }
      }, B = () => {
        s().removeEventListener("pointermove", T), s().removeEventListener("pointerup", B);
      };
      s().addEventListener("pointermove", T), s().addEventListener("pointerup", B);
    },
    [t, et]
  ), La = it(
    (w, A, I) => {
      I.stopPropagation(), I.preventDefault();
      const H = t.getNode(w);
      if (!H || H.type !== "edge") return;
      const { fromId: M, toId: E, sourceHandle: T, targetHandle: B } = H.data, L = A === "source" ? E : M, N = A === "source" ? B : T, P = t.getNode(M), j = t.getNode(E);
      if (!P || !j) return;
      const q = je(
        P,
        j,
        H.data.edgeType || "bezier",
        et,
        T,
        B
      ), Q = A === "source" ? { x: q.x1, y: q.y1 } : { x: q.x2, y: q.y2 };
      ft({
        edgeId: w,
        endpoint: A,
        anchorNodeId: L,
        anchorHandle: N,
        cursorX: Q.x,
        cursorY: Q.y
      });
      const ct = (kt) => {
        const { x: Mt, y: xt } = t.screenToCanvas(kt.clientX, kt.clientY);
        ft(
          (Tt) => Tt ? { ...Tt, cursorX: Mt, cursorY: xt } : null
        );
      }, lt = (kt) => {
        s().removeEventListener("pointermove", ct), s().removeEventListener("pointerup", lt), ft(null);
        const { x: Mt, y: xt } = t.screenToCanvas(kt.clientX, kt.clientY);
        let Tt = t.hitTest(Mt, xt, et);
        if (!Tt || Tt.type === "edge" || t.isContainerType(Tt.type)) {
          const vt = 50 / t.viewport.zoom;
          let Ct = 1 / 0, Ht = !1, jt = null;
          for (const oe of t.getAllNodes()) {
            if (oe.type === "edge") continue;
            const re = t.isContainerType(oe.type), Ee = Wo(oe, et);
            for (const ee of Ee) {
              const se = Math.hypot(ee.x - Mt, ee.y - xt);
              se >= vt || re && !Ht && jt || (!re && Ht || se < Ct) && (Ct = se, Ht = re, jt = oe);
            }
          }
          jt && (Tt = jt);
        }
        if (!Tt || Tt.type === "edge") return;
        const Xt = A === "source" ? Tt.id : M, Ot = A === "target" ? Tt.id : E;
        if (Xt === Ot) return;
        const pt = A === "source" ? M : E;
        if (Tt.id === pt || t.getAllNodes().some((vt) => {
          if (vt.type !== "edge" || vt.id === w) return !1;
          const Ct = vt.data;
          return Ct.fromId === Xt && Ct.toId === Ot || Ct.fromId === Ot && Ct.toId === Xt;
        })) return;
        const bt = pr(Tt, Mt, xt, et), Gt = A === "source" ? { fromId: Tt.id, sourceHandle: bt } : { toId: Tt.id, targetHandle: bt };
        t.updateNodeWithHistory(w, { data: Gt });
      };
      s().addEventListener("pointermove", ct), s().addEventListener("pointerup", lt);
    },
    [t, et]
  ), Da = it(
    (w) => {
      if (w.stopPropagation(), w.preventDefault(), t.presentationMode) return;
      const A = Array.from(t.selection).map((bt) => t.getNode(bt)).filter(Boolean);
      if (A.length < 2) return;
      const H = t.selectionIsSingleGroup() ? t.selectionGroupId() ?? null : null, M = H ? t.groupRotations.get(H) : null;
      let E, T;
      if (M)
        E = M.cx, T = M.cy;
      else {
        let bt = 1 / 0, Gt = 1 / 0, vt = -1 / 0, Ct = -1 / 0;
        for (const Ht of A) {
          const jt = Ht.h === "auto" ? et[Ht.id] ?? 100 : Ht.h, oe = qt(Ht, jt);
          bt = Math.min(bt, oe.minX), Gt = Math.min(Gt, oe.minY), vt = Math.max(vt, oe.maxX), Ct = Math.max(Ct, oe.maxY);
        }
        E = (bt + vt) / 2, T = (Gt + Ct) / 2;
      }
      const B = (M == null ? void 0 : M.angle) ?? 0, N = A.filter((bt) => !bt.locked).map((bt) => {
        const Gt = bt.h === "auto" ? et[bt.id] ?? 100 : bt.h;
        return {
          id: bt.id,
          cx: bt.x + bt.w / 2,
          cy: bt.y + Gt / 2,
          w: bt.w,
          h: Gt,
          rotation: bt.rotation || 0
        };
      }), P = -B * Math.PI / 180, j = Math.cos(P), q = Math.sin(P);
      let Q = 1 / 0, ct = 1 / 0, lt = -1 / 0, kt = -1 / 0;
      for (const bt of N) {
        const Gt = bt.cx - E, vt = bt.cy - T, Ct = E + Gt * j - vt * q, Ht = T + Gt * q + vt * j;
        Q = Math.min(Q, Ct - bt.w / 2), ct = Math.min(ct, Ht - bt.h / 2), lt = Math.max(lt, Ct + bt.w / 2), kt = Math.max(kt, Ht + bt.h / 2);
      }
      const Mt = {
        x: Q - $t,
        y: ct - $t,
        w: lt - Q + $t * 2,
        h: kt - ct + $t * 2
      }, { x: xt, y: Tt } = t.screenToCanvas(w.clientX, w.clientY), Xt = Math.atan2(Tt - T, xt - E);
      t.pushHistorySnapshot();
      let Ot = B;
      const pt = (bt) => {
        const { x: Gt, y: vt } = t.screenToCanvas(bt.clientX, bt.clientY);
        let Ht = (Math.atan2(vt - T, Gt - E) - Xt) * (180 / Math.PI);
        (bt.shiftKey || t.snapToGrid) && !(bt.metaKey || bt.ctrlKey) && (Ht = Math.round(Ht / 15) * 15), Ot = B + Ht, Be({ angle: Ot, cx: E, cy: T, bounds: Mt });
        const jt = Ht * Math.PI / 180, oe = Math.cos(jt), re = Math.sin(jt), Ee = N.map((ee) => {
          const se = ee.cx - E, vo = ee.cy - T, xe = E + se * oe - vo * re, So = T + se * re + vo * oe;
          return {
            id: ee.id,
            patch: {
              x: xe - ee.w / 2,
              y: So - ee.h / 2,
              rotation: Ot
            }
          };
        });
        t.updateMany(Ee);
      }, Lt = () => {
        H && t.groupRotations.set(H, { angle: Ot, cx: E, cy: T }), Be({ angle: Ot, cx: E, cy: T, bounds: Mt }), s().removeEventListener("pointermove", pt), s().removeEventListener("pointerup", Lt);
      };
      s().addEventListener("pointermove", pt), s().addEventListener("pointerup", Lt);
    },
    [t, et, qt]
  ), Ea = it(
    (w, A) => {
      if (A.stopPropagation(), A.preventDefault(), t.presentationMode) return;
      const I = Array.from(t.selection).map((pt) => t.getNode(pt)).filter(Boolean);
      if (I.length < 2) return;
      const H = (pt) => pt.h === "auto" ? et[pt.id] ?? 100 : pt.h;
      let M = 1 / 0, E = 1 / 0, T = -1 / 0, B = -1 / 0;
      for (const pt of I) {
        const Lt = H(pt), bt = qt(pt, Lt);
        M = Math.min(M, bt.minX), E = Math.min(E, bt.minY), T = Math.max(T, bt.maxX), B = Math.max(B, bt.maxY);
      }
      const L = { x: M, y: E, w: T - M, h: B - E }, N = L.w || 1, P = L.h || 1, q = I.filter((pt) => !pt.locked).map((pt) => {
        const Lt = H(pt);
        return {
          id: pt.id,
          type: pt.type,
          isAutoH: pt.h === "auto",
          relX: (pt.x - L.x) / N,
          relY: (pt.y - L.y) / P,
          relW: pt.w / N,
          relH: Lt / P,
          origW: pt.w,
          origH: Lt,
          origPoints: pt.type === "draw" ? pt.data.points.map((bt) => [...bt]) : null,
          drawData: pt.type === "draw" ? { ...pt.data } : null
        };
      }), Q = A.clientX, ct = A.clientY;
      t.pushHistorySnapshot();
      let lt = null, kt = Q, Mt = ct, xt = !1;
      const Tt = () => {
        lt = null;
        const pt = (kt - Q) / t.viewport.zoom, Lt = (Mt - ct) / t.viewport.zoom;
        let bt = L.x, Gt = L.y, vt = L.w, Ct = L.h;
        if ((w === "nw" || w === "w" || w === "sw") && (bt = L.x + pt, vt = L.w - pt), (w === "ne" || w === "e" || w === "se") && (vt = L.w + pt), (w === "nw" || w === "n" || w === "ne") && (Gt = L.y + Lt, Ct = L.h - Lt), (w === "sw" || w === "s" || w === "se") && (Ct = L.h + Lt), t.snapToGrid && !xt) {
          const jt = t.gridSize, oe = (re) => Math.round(re / jt) * jt;
          (w === "nw" || w === "w" || w === "sw") && (bt = oe(bt), vt = L.x + L.w - bt), (w === "ne" || w === "e" || w === "se") && (vt = oe(bt + vt) - bt), (w === "nw" || w === "n" || w === "ne") && (Gt = oe(Gt), Ct = L.y + L.h - Gt), (w === "sw" || w === "s" || w === "se") && (Ct = oe(Gt + Ct) - Gt);
        }
        vt < 20 && (vt = 20, (w === "nw" || w === "w" || w === "sw") && (bt = L.x + L.w - 20)), Ct < 20 && (Ct = 20, (w === "nw" || w === "n" || w === "ne") && (Gt = L.y + L.h - 20));
        const Ht = q.map((jt) => {
          const oe = bt + jt.relX * vt, re = Gt + jt.relY * Ct, Ee = jt.relW * vt, ee = jt.relH * Ct, se = {
            x: oe,
            y: re,
            w: Ee,
            h: jt.isAutoH ? "auto" : ee
          };
          if (jt.origPoints && jt.drawData) {
            const vo = jt.origW > 0 ? Ee / jt.origW : 1, xe = jt.origH > 0 ? ee / jt.origH : 1;
            se.data = {
              ...jt.drawData,
              points: jt.origPoints.map(
                ([So, Zr, ur]) => [So * vo, Zr * xe, ur]
              )
            };
          }
          return { id: jt.id, patch: se };
        });
        t.updateMany(Ht);
      }, Xt = (pt) => {
        kt = pt.clientX, Mt = pt.clientY, xt = pt.metaKey || pt.ctrlKey, lt === null && (lt = requestAnimationFrame(Tt));
      }, Ot = () => {
        lt !== null && (cancelAnimationFrame(lt), Tt()), s().removeEventListener("pointermove", Xt), s().removeEventListener("pointerup", Ot);
        for (const pt of I)
          t.isContainerType(pt.type) && t.syncFrameChildrenAfterResize(pt.id);
      };
      s().addEventListener("pointermove", Xt), s().addEventListener("pointerup", Ot);
    },
    [t, et, qt]
  );
  mt(() => {
    n.current && (n.current.style.cursor = t.lassoSelect ? zo : wr(y)), y !== "select" && y !== "edge" && (Le.current = null, be(null)), y !== "erase" && (Ne.current !== null && (cancelAnimationFrame(Ne.current), Ne.current = null), so.current = /* @__PURE__ */ new Set(), jr(/* @__PURE__ */ new Set()), Me.current = [], Vo([]));
  }, [y]);
  const qr = at(null), bs = at(null), Wa = it(
    (w) => {
      if (O.current && w.pointerType === "touch" && _.current) {
        const A = w.clientX - _.current.clientX, I = w.clientY - _.current.clientY;
        Math.sqrt(A * A + I * I) > 8 && (clearTimeout(O.current), O.current = null, _.current = null);
      }
      t.mode !== "select" && t.mode !== "edge" || (bs.current = { clientX: w.clientX, clientY: w.clientY }, qr.current === null && (qr.current = requestAnimationFrame(() => {
        qr.current = null;
        const A = n.current, I = bs.current;
        if (!A || !I) return;
        const { x: H, y: M } = t.screenToCanvas(I.clientX, I.clientY);
        if (t.lassoSelect) {
          A.style.cursor = zo;
          return;
        }
        if (t.mode === "edge") {
          const B = t.hitTest(H, M, et), L = B && B.type !== "edge" ? B.id : null;
          L !== Le.current && (Le.current = L, be(L));
          return;
        }
        if (t.selection.size >= 2 && _t && H >= _t.x && H <= _t.x + _t.w && M >= _t.y && M <= _t.y + _t.h) {
          A.style.cursor = "move";
          return;
        }
        const E = t.hitTest(H, M, et), T = E ? E.id : null;
        if (T !== Le.current && (Le.current = T, be(T)), E) {
          A.style.cursor = "move";
          return;
        }
        A.style.cursor = "default";
      })));
    },
    [t, _t, et, qt]
  ), Fa = it((w) => {
    (w.dataTransfer.types.includes("Files") || w.dataTransfer.types.includes(In) || w.dataTransfer.types.includes(Tn) || w.dataTransfer.types.includes(Pn)) && (w.preventDefault(), w.dataTransfer.dropEffect = "copy");
  }, []), Ba = it(
    (w) => {
      if (w.preventDefault(), t.presentationMode) return;
      const A = w.dataTransfer.getData(Pn);
      if (A) {
        try {
          const L = JSON.parse(A);
          ua(t, L, w.clientX, w.clientY);
        } catch (L) {
          console.error("Failed to place GIF:", L);
        }
        return;
      }
      const I = w.dataTransfer.getData(Tn);
      if (I) {
        try {
          const { itemId: L } = JSON.parse(I), P = aa().find((j) => j.id === L);
          P && ca(t, P, w.clientX, w.clientY);
        } catch (L) {
          console.error("Failed to place personal library item:", L);
        }
        return;
      }
      const H = w.dataTransfer.getData(In);
      if (H) {
        try {
          const { libraryId: L, itemId: N } = JSON.parse(H), j = Jn(L).find((q) => q.id === N);
          j && la(t, j, w.clientX, w.clientY);
        } catch (L) {
          console.error("Failed to place library item:", L);
        }
        return;
      }
      const M = w.dataTransfer.files[0];
      if (!M) return;
      if (M.name.endsWith(".excalidrawlib") || M.name.endsWith(".excalidrawlib.json")) {
        const L = new FileReader();
        L.onload = () => {
          try {
            const N = JSON.parse(L.result);
            if (N.type === "excalidrawlib") {
              const P = M.name.replace(/\.excalidrawlib(\.json)?$/, "");
              $n(N, { name: P });
            }
          } catch (N) {
            console.error("Failed to import library:", N);
          }
        }, L.readAsText(M);
        return;
      }
      if (M.type === "image/svg+xml" || M.name.endsWith(".svg")) {
        const L = new FileReader();
        L.onload = () => {
          const N = L.result, P = Rn(N);
          P && dh(t, P, w.clientX, w.clientY);
        }, L.readAsText(M);
        return;
      }
      if (!M.type.startsWith("image/")) return;
      const { x: E, y: T } = t.screenToCanvas(w.clientX, w.clientY), B = new FileReader();
      B.onload = () => {
        const L = B.result, N = new Image();
        N.onload = () => {
          const P = Math.min(N.naturalWidth, 400), j = Math.min(N.naturalHeight, 300), q = N.naturalWidth / N.naturalHeight, Q = q >= 1 ? P : j * q, ct = q >= 1 ? P / q : j;
          t.addNode({
            id: wt(10),
            type: "image",
            x: E,
            y: T,
            w: Q,
            h: ct,
            z: t.nextZ(),
            data: { src: L }
          });
        }, N.src = L;
      }, B.readAsDataURL(M);
    },
    [t]
  ), Na = `translate(${a.x}px, ${a.y}px) scale(${a.zoom})`;
  return /* @__PURE__ */ v(
    "div",
    {
      ref: n,
      "data-sb-canvas": !0,
      style: {
        width: "100%",
        height: "100%",
        overflow: "hidden",
        position: "relative",
        userSelect: "none",
        touchAction: "none",
        background: nr(Z).canvasBg
      },
      onPointerDown: za,
      onPointerMove: Wa,
      onDoubleClick: Ca,
      onContextMenu: Ma,
      onDragOver: Fa,
      onDrop: Ba,
      children: [
        /* @__PURE__ */ u(Fd, { viewport: a, gridSize: z, background: Z, gridVisible: x }),
        /* @__PURE__ */ v(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              transform: Na,
              transformOrigin: "0 0",
              pointerEvents: "none"
            },
            children: [
              (to || h).filter((w) => {
                if (o) {
                  const A = o.get(w.type);
                  return A && !A.isSVGOnly;
                }
                return w.type === "content" || w.type === "draw" || w.type === "shape" || w.type === "image" || w.type === "text" || w.type === "frame" || w.type === "sticky";
              }).sort((w, A) => w.z - A.z).map((w) => {
                var H;
                const A = Gr.has(w.id);
                let I;
                if (o) {
                  const M = o.get(w.type);
                  if (M) {
                    const E = M.component, T = d.has(w.id), B = y === "select" || y === "text" || y === "note" || y === "sticky", L = /* @__PURE__ */ u(
                      E,
                      {
                        node: w,
                        data: w.data,
                        isSelected: T,
                        multiSelected: d.size > 1 && T && !t.selectionIsSingleGroup(),
                        engine: t,
                        interactive: B,
                        zoom: a.zoom,
                        editing: hs === w.id,
                        editClickPos: hs === w.id ? Go.current : null,
                        callbacks: {
                          onMeasuredHeight: zt,
                          onResizeHandleDown: Kr,
                          onEditStart: (N) => {
                            const P = t.getNode(N);
                            P && (P.type === "text" ? me(N) : P.type === "sticky" ? no(N) : P.type === "frame" ? De(N) : P.type === "shape" ? xo(N) : P.type === "image" ? wo(N) : P.type === "youtube" && ds(N));
                          },
                          onEditEnd: () => {
                            me(null), no(null), De(null), xo(null), wo(null), ds(null);
                          }
                        },
                        portValues: r && ((H = M.ports) != null && H.length) && gs >= 0 ? r.getAllPortValues(w.id) : void 0,
                        updateData: (N) => {
                          t.updateNodeWithHistory(w.id, {
                            data: { ...w.data, ...N }
                          });
                        }
                      },
                      M.handlesOwnLayout ? w.id : void 0
                    );
                    M.handlesOwnLayout ? I = L : I = /* @__PURE__ */ u(
                      Yh,
                      {
                        node: w,
                        isInteractive: B,
                        measuredH: et[w.id],
                        onMeasuredHeight: zt,
                        observeElement: It,
                        unobserveElement: Rt,
                        isContainer: M.isContainer,
                        children: L
                      },
                      w.id
                    );
                  }
                } else if (w.type === "content") {
                  const M = w;
                  I = /* @__PURE__ */ u(
                    Ri,
                    {
                      node: M,
                      isSelected: d.has(w.id),
                      multiSelected: d.size > 1 && d.has(w.id) && !t.selectionIsSingleGroup(),
                      engine: t,
                      schema: e,
                      interactive: y === "select" || y === "text" || y === "note",
                      zoom: a.zoom,
                      onMeasuredHeight: zt,
                      autoEdit: us.current === M.id
                    },
                    w.id
                  );
                } else if (w.type === "text")
                  I = /* @__PURE__ */ u(
                    ji,
                    {
                      node: w,
                      engine: t,
                      editing: ro === w.id,
                      editClickPos: ro === w.id ? Go.current : null,
                      onStopEdit: () => {
                        if (Yr.current === w.id) {
                          Yr.current = null;
                          const M = t.getNode(w.id);
                          if (!M || !M.data.text.trim()) {
                            t.deleteNode(w.id), me(null);
                            return;
                          }
                          t.pushHistorySnapshot();
                        }
                        me(null);
                      },
                      onMeasuredHeight: zt
                    },
                    w.id
                  );
                else if (w.type === "image")
                  I = /* @__PURE__ */ u(
                    Gi,
                    {
                      node: w,
                      isSelected: d.has(w.id),
                      engine: t,
                      interactive: y === "select",
                      zoom: a.zoom,
                      onResizeHandleDown: Kr,
                      cropping: cs === w.id,
                      onCropStart: () => wo(w.id),
                      onCropEnd: () => wo(null)
                    },
                    w.id
                  );
                else if (w.type === "sticky")
                  I = /* @__PURE__ */ u(
                    Vi,
                    {
                      node: w,
                      isSelected: d.has(w.id),
                      engine: t,
                      interactive: y === "select" || y === "sticky",
                      zoom: a.zoom,
                      editing: ls === w.id,
                      onEditStart: no,
                      onEditEnd: () => no(null)
                    },
                    w.id
                  );
                else if (w.type === "frame") {
                  const M = w, E = M.h === "auto" ? 100 : M.h;
                  I = /* @__PURE__ */ u(
                    "div",
                    {
                      style: {
                        position: "absolute",
                        left: M.x,
                        top: M.y,
                        width: M.w,
                        height: E,
                        zIndex: M.z,
                        background: M.data.backgroundColor || "rgba(0,0,0,0.02)",
                        border: `${M.data.borderWidth || 1}px ${M.data.borderStyle || "dashed"} ${M.data.borderColor || "#ccc"}`,
                        boxSizing: "border-box",
                        borderRadius: 8,
                        opacity: M.data.opacity ?? 1,
                        pointerEvents: "none",
                        overflow: "visible",
                        transform: M.rotation ? `rotate(${M.rotation}deg)` : void 0,
                        transformOrigin: "center center"
                      },
                      children: lr === w.id ? /* @__PURE__ */ u(
                        "input",
                        {
                          autoFocus: !0,
                          defaultValue: M.data.label ?? "",
                          placeholder: "Frame label...",
                          onBlur: (T) => {
                            const B = T.currentTarget.value.trim();
                            t.updateNodeWithHistory(w.id, {
                              data: { ...M.data, label: B || void 0 }
                            }), De(null);
                          },
                          onKeyDown: (T) => {
                            (T.key === "Enter" || T.key === "Escape") && T.currentTarget.blur(), T.stopPropagation();
                          },
                          onPointerDown: (T) => T.stopPropagation(),
                          style: {
                            position: "absolute",
                            top: -24,
                            left: 0,
                            fontSize: 12,
                            color: M.data.borderColor || "#999",
                            fontWeight: 500,
                            background: "rgba(255,255,255,0.95)",
                            border: "1px solid #3b82f6",
                            borderRadius: 4,
                            padding: "1px 4px",
                            outline: "none",
                            pointerEvents: "auto",
                            minWidth: 80
                          }
                        }
                      ) : M.data.label ? /* @__PURE__ */ u(
                        "div",
                        {
                          onDoubleClick: (T) => {
                            T.stopPropagation(), t.select(w.id), De(w.id);
                          },
                          style: {
                            position: "absolute",
                            top: -20,
                            left: 4,
                            fontSize: 12,
                            color: M.data.borderColor || "#999",
                            fontWeight: 500,
                            whiteSpace: "nowrap",
                            userSelect: "none",
                            pointerEvents: "auto",
                            cursor: "default"
                          },
                          children: M.data.label
                        }
                      ) : null
                    },
                    w.id
                  );
                } else {
                  const M = w;
                  if (M.type === "draw")
                    I = /* @__PURE__ */ u(Lr, { node: M }, w.id);
                  else {
                    const E = M.h === "auto" ? 100 : M.h, T = M.w * a.zoom, B = E * a.zoom;
                    I = Math.min(T, B) < 2 ? /* @__PURE__ */ u(
                      Yi,
                      {
                        node: M
                      },
                      w.id
                    ) : /* @__PURE__ */ u(Lr, { node: M, editingLabel: jo === w.id }, w.id);
                  }
                }
                return A ? /* @__PURE__ */ u("div", { style: { opacity: 0.25, filter: "saturate(0)" }, children: I }, w.id) : I;
              }),
              jo && (() => {
                const w = t.getNode(jo);
                if (!w || w.type !== "shape") return null;
                const A = w.data;
                return A.shape === "line" || A.shape === "arrow" ? null : /* @__PURE__ */ u(
                  Gh,
                  {
                    node: w,
                    engine: t,
                    onDone: () => xo(null)
                  },
                  jo
                );
              })()
            ]
          }
        ),
        /* @__PURE__ */ u(
          fh,
          {
            nodes: bo,
            viewport: a,
            selection: d,
            measuredHeights: et,
            activeStroke: G,
            shapePreview: Y,
            shapePreviewStyle: Y ? {
              stroke: t.mode === "frame" ? "#1e1e2e" : t.activeTool.color,
              strokeWidth: t.mode === "frame" ? 1 : t.activeTool.width,
              roughness: t.mode === "frame" ? 0 : t.activeTool.roughness ?? 1,
              shapeType: t.mode === "frame" ? "rect" : t.activeTool.shapeType || "rect"
            } : null,
            onResizeHandleDown: Kr,
            onRotateStart: Ia,
            onConnectionHandleDown: ms,
            onEdgeEndpointDown: La,
            onKinkHandleDown: Aa,
            edgePreview: nt,
            edgeReconnect: yt,
            eraserMarkedIds: Gr.size > 0 ? Gr : void 0,
            eraserTrail: fs.length > 1 ? fs : void 0,
            laserTrail: ps.length > 1 ? ps : void 0,
            mode: y,
            hoveredNodeId: ne,
            registry: o,
            onPortHandleDown: Pa,
            cycleNodeIds: r && gs >= 0 ? r.cycleNodeIds : void 0,
            containerTypes: t.containerTypes,
            alignGuides: W
          }
        ),
        _t && (() => {
          const w = t.selectionGroupId(), A = w ? t.groupRotations.get(w) : void 0;
          let I, H, M, E;
          if (ge)
            I = ge.bounds, H = ge.angle, M = ge.cx, E = ge.cy;
          else if (A && A.angle !== 0) {
            const P = -A.angle * Math.PI / 180, j = Math.cos(P), q = Math.sin(P);
            let Q = 1 / 0, ct = 1 / 0, lt = -1 / 0, kt = -1 / 0;
            for (const Mt of t.selection) {
              const xt = t.getNode(Mt);
              if (!xt || xt.type === "edge") continue;
              const Tt = xt.h === "auto" ? et[xt.id] ?? 100 : xt.h, Xt = xt.x + xt.w / 2, Ot = xt.y + Tt / 2, pt = Xt - A.cx, Lt = Ot - A.cy, bt = A.cx + pt * j - Lt * q, Gt = A.cy + pt * q + Lt * j;
              Q = Math.min(Q, bt - xt.w / 2), ct = Math.min(ct, Gt - Tt / 2), lt = Math.max(lt, bt + xt.w / 2), kt = Math.max(kt, Gt + Tt / 2);
            }
            I = {
              x: Q - $t,
              y: ct - $t,
              w: lt - Q + $t * 2,
              h: kt - ct + $t * 2
            }, H = A.angle, M = A.cx, E = A.cy;
          } else
            I = _t, H = 0, M = 0, E = 0;
          const T = 8 / a.zoom, B = T / 2, L = [
            { pos: "nw", cx: I.x, cy: I.y },
            { pos: "n", cx: I.x + I.w / 2, cy: I.y },
            { pos: "ne", cx: I.x + I.w, cy: I.y },
            { pos: "e", cx: I.x + I.w, cy: I.y + I.h / 2 },
            { pos: "se", cx: I.x + I.w, cy: I.y + I.h },
            { pos: "s", cx: I.x + I.w / 2, cy: I.y + I.h },
            { pos: "sw", cx: I.x, cy: I.y + I.h },
            { pos: "w", cx: I.x, cy: I.y + I.h / 2 }
          ], N = H !== 0 ? ` rotate(${H}, ${M}, ${E})` : "";
          return /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ v("g", { transform: N, children: [
                /* @__PURE__ */ u(
                  "rect",
                  {
                    x: I.x,
                    y: I.y,
                    width: I.w,
                    height: I.h,
                    fill: "none",
                    stroke: "#3b82f6",
                    strokeWidth: 1.5 / a.zoom
                  }
                ),
                H === 0 && L.map(({ pos: P, cx: j, cy: q }) => /* @__PURE__ */ u(
                  "rect",
                  {
                    x: j - B,
                    y: q - B,
                    width: T,
                    height: T,
                    fill: "white",
                    stroke: "#3b82f6",
                    strokeWidth: 1.5 / a.zoom,
                    style: { cursor: Fr(P, H), pointerEvents: "auto" },
                    onPointerDown: (Q) => {
                      Q.stopPropagation(), Ea(P, Q);
                    }
                  },
                  P
                )),
                (() => {
                  const P = 25 / a.zoom, j = I.x + I.w / 2, q = I.y;
                  return /* @__PURE__ */ v(ut, { children: [
                    /* @__PURE__ */ u(
                      "line",
                      {
                        x1: j,
                        y1: q,
                        x2: j,
                        y2: q - P,
                        stroke: "#3b82f6",
                        strokeWidth: 1.5 / a.zoom,
                        style: { pointerEvents: "none" }
                      }
                    ),
                    (() => {
                      const Q = 8 / a.zoom, ct = Q / 2;
                      return /* @__PURE__ */ u(
                        "rect",
                        {
                          x: j - ct,
                          y: q - P - ct,
                          width: Q,
                          height: Q,
                          rx: 1.5 / a.zoom,
                          transform: `rotate(45, ${j}, ${q - P})`,
                          fill: "white",
                          stroke: "#3b82f6",
                          strokeWidth: 1.5 / a.zoom,
                          style: { cursor: "grab", pointerEvents: "auto" },
                          onPointerDown: (lt) => Da(lt)
                        }
                      );
                    })()
                  ] });
                })(),
                (() => {
                  const P = 26 / a.zoom, j = 42 / a.zoom, q = 4 / a.zoom;
                  return [
                    { side: "top", cx: I.x + I.w / 2, cy: I.y - j },
                    { side: "right", cx: I.x + I.w + P, cy: I.y + I.h / 2 },
                    { side: "bottom", cx: I.x + I.w / 2, cy: I.y + I.h + P },
                    { side: "left", cx: I.x - P, cy: I.y + I.h / 2 }
                  ].map(({ side: ct, cx: lt, cy: kt }) => /* @__PURE__ */ u(
                    "circle",
                    {
                      cx: lt,
                      cy: kt,
                      r: q,
                      fill: "white",
                      stroke: "#94a3b8",
                      strokeWidth: 1.5 / a.zoom,
                      opacity: 0.8,
                      style: { cursor: "crosshair", pointerEvents: "auto" },
                      onPointerDown: (Mt) => {
                        Mt.stopPropagation();
                        const xt = Ta(ct);
                        xt && ms(xt, ct, Mt);
                      }
                    },
                    `conn-${ct}`
                  ));
                })()
              ] }) })
            }
          );
        })(),
        Ae && /* @__PURE__ */ u(
          "svg",
          {
            "data-sb-overlay": !0,
            style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
            children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ u(
              "rect",
              {
                x: Ae.x,
                y: Ae.y,
                width: Ae.w,
                height: Ae.h,
                fill: "none",
                stroke: "#6366f1",
                strokeWidth: 1.5 / a.zoom,
                strokeDasharray: `${5 / a.zoom} ${3 / a.zoom}`,
                rx: 4 / a.zoom,
                opacity: 0.5
              }
            ) })
          }
        ),
        ue && (() => {
          const w = t.canvasToScreen(ue.startX, ue.startY), A = t.canvasToScreen(ue.endX, ue.endY), I = Math.min(w.x, A.x), H = Math.min(w.y, A.y), M = Math.abs(A.x - w.x), E = Math.abs(A.y - w.y);
          return M < 2 && E < 2 ? null : /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u(
                "rect",
                {
                  x: I,
                  y: H,
                  width: M,
                  height: E,
                  fill: "rgba(59,130,246,0.08)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5,
                  strokeDasharray: "4"
                }
              )
            }
          );
        })(),
        eo && eo.length > 2 && (() => {
          const A = eo.map(([I, H]) => t.canvasToScreen(I, H)).map((I) => `${I.x},${I.y}`).join(" ");
          return /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u(
                "polygon",
                {
                  points: A,
                  fill: "rgba(59,130,246,0.08)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5,
                  strokeDasharray: "4"
                }
              )
            }
          );
        })(),
        st && (() => {
          const w = Math.min(st.startX, st.endX), A = Math.min(st.startY, st.endY), I = Math.abs(st.endX - st.startX), H = Math.abs(st.endY - st.startY);
          return I < 2 && H < 2 ? null : /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ u(
                "rect",
                {
                  x: w,
                  y: A,
                  width: I,
                  height: H,
                  fill: "rgba(59,130,246,0.06)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5 / a.zoom,
                  strokeDasharray: `${4 / a.zoom}`,
                  rx: 8 / a.zoom
                }
              ) })
            }
          );
        })(),
        fe && /* @__PURE__ */ u(
          ph,
          {
            x: fe.x,
            y: fe.y,
            sections: fe.sections,
            onClose: () => Se(null)
          }
        ),
        cr && /* @__PURE__ */ u(
          lh,
          {
            nodes: cr.nodes,
            onSave: (w) => {
              $d(w, cr.nodes, cr.groupParent), Vr(null);
            },
            onCancel: () => Vr(null)
          }
        )
      ]
    }
  );
}
const ir = 52, Jo = 270, Xf = ir + Jo, Vh = [
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], os = [
  { key: "hachure", label: "Hachure" },
  { key: "cross-hatch", label: "Cross-hatch" },
  { key: "solid", label: "Solid" }
], Kh = [
  { key: "solid", label: "Solid", dash: "" },
  { key: "dashed", label: "Dashed", dash: "6,3" },
  { key: "dotted", label: "Dotted", dash: "2,2" }
], rs = [
  { value: 0, label: "Architect" },
  { value: 1, label: "Artist" },
  { value: 2, label: "Cartoonist" }
], qh = [1, 2.5, 5, 10, 20], ka = [14, 20, 28, 36], ns = [
  { key: "left", label: "←" },
  { key: "center", label: "↔" },
  { key: "right", label: "→" }
], Zh = [
  "#FEF3C7",
  "#FCE7F3",
  "#DBEAFE",
  "#D1FAE5",
  "#EDE9FE",
  "#FFEDD5"
], we = [
  { name: "Standard", colors: Vh },
  { name: "Pastel", colors: ["#F8B4B4", "#FDBA74", "#FDE68A", "#86EFAC", "#93C5FD", "#C4B5FD"] },
  { name: "Earth", colors: ["#78350F", "#92400E", "#6B7280", "#065F46", "#1E3A5F", "#7C2D12"] },
  { name: "Neon", colors: ["#FF1493", "#39FF14", "#00FFFF", "#FF6600", "#FFFF00", "#BF00FF"] },
  { name: "Crayon", colors: ["#EF4444", "#F97316", "#EAB308", "#22C55E", "#3B82F6", "#A855F7"] },
  { name: "Mono", colors: ["#000000", "#404040", "#737373", "#A3A3A3", "#D4D4D4", "#FFFFFF"] }
], ss = we, Uh = [
  { name: "Standard", colors: Zh },
  { name: "Bright", colors: ["#FDE047", "#FB923C", "#F87171", "#4ADE80", "#60A5FA", "#C084FC"] },
  { name: "Earth", colors: ["#D6CFC7", "#E8D5B7", "#C4B5A0", "#B8C5A3", "#A3B5C4", "#C7B8A8"] },
  { name: "Cool", colors: ["#BFDBFE", "#A5F3FC", "#C7D2FE", "#DDD6FE", "#BAE6FD", "#E0E7FF"] }
], Et = {
  display: "flex",
  alignItems: "center",
  gap: 4
}, Wt = {
  width: 52,
  fontSize: 10,
  flexShrink: 0
}, Zt = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0,
  flexShrink: 0
}, Qh = {
  fontSize: 9,
  fontWeight: 600,
  textTransform: "uppercase",
  letterSpacing: "0.05em",
  padding: "4px 0 2px",
  marginTop: 4
}, Jh = "https://libraries.excalidraw.com/libraries.json", Ln = "https://libraries.excalidraw.com/libraries";
function $h({
  onClose: t,
  onInstalled: e
}) {
  const o = Ut(), [r, n] = tt([]), [s, i] = tt(!0), [l, a] = tt(null), [c, h] = tt(""), [f, d] = tt(null), [p, y] = tt(/* @__PURE__ */ new Set()), m = it(() => {
    const x = Qi(), C = new Set(x.map((z) => z.source));
    y(C);
  }, []);
  mt(() => {
    let x = !1;
    return (async () => {
      try {
        const C = await fetch(Jh);
        if (!C.ok) throw new Error(`HTTP ${C.status}`);
        const z = await C.json();
        x || (n(z), i(!1));
      } catch (C) {
        x || (a(String(C)), i(!1));
      }
    })(), m(), () => {
      x = !0;
    };
  }, [m]);
  const g = Kt(() => {
    if (!c.trim()) return r;
    const x = c.toLowerCase();
    return r.filter(
      (C) => {
        var z, S;
        return C.name.toLowerCase().includes(x) || ((z = C.description) == null ? void 0 : z.toLowerCase().includes(x)) || ((S = C.itemNames) == null ? void 0 : S.some((F) => F.toLowerCase().includes(x)));
      }
    );
  }, [r, c]), b = it(
    async (x) => {
      d(x.id);
      try {
        const C = `${Ln}/${x.source}`;
        await Od(C, x.name), m(), e();
      } catch (C) {
        console.error("Failed to install library:", C);
      } finally {
        d(null);
      }
    },
    [e, m]
  );
  return Je(
    /* @__PURE__ */ u(
      "div",
      {
        style: {
          position: "fixed",
          inset: 0,
          zIndex: 1e5,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          background: "rgba(0,0,0,0.4)",
          backdropFilter: "blur(4px)"
        },
        onPointerDown: (x) => {
          x.target === x.currentTarget && t();
        },
        children: /* @__PURE__ */ v(
          "div",
          {
            style: {
              width: 620,
              maxWidth: "90vw",
              maxHeight: "80vh",
              background: o.panelBg,
              border: `1px solid ${o.border}`,
              borderRadius: 12,
              boxShadow: "0 8px 32px rgba(0,0,0,0.25)",
              display: "flex",
              flexDirection: "column",
              overflow: "hidden"
            },
            onPointerDown: (x) => x.stopPropagation(),
            children: [
              /* @__PURE__ */ v(
                "div",
                {
                  style: {
                    padding: "16px 20px 12px",
                    borderBottom: `1px solid ${o.border}`,
                    flexShrink: 0
                  },
                  children: [
                    /* @__PURE__ */ v(
                      "div",
                      {
                        style: {
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "center",
                          marginBottom: 12
                        },
                        children: [
                          /* @__PURE__ */ u(
                            "span",
                            {
                              style: {
                                fontSize: 14,
                                fontWeight: 600,
                                color: o.text
                              },
                              children: "Excalidraw Libraries"
                            }
                          ),
                          /* @__PURE__ */ u(
                            "button",
                            {
                              onClick: t,
                              style: {
                                border: "none",
                                background: "transparent",
                                color: o.textMuted,
                                cursor: "pointer",
                                fontSize: 18,
                                lineHeight: 1,
                                padding: "2px 6px"
                              },
                              children: "×"
                            }
                          )
                        ]
                      }
                    ),
                    /* @__PURE__ */ u(
                      "input",
                      {
                        type: "text",
                        placeholder: "Search libraries...",
                        value: c,
                        onChange: (x) => h(x.target.value),
                        autoFocus: !0,
                        style: {
                          width: "100%",
                          padding: "7px 10px",
                          border: `1px solid ${o.border}`,
                          borderRadius: 6,
                          background: o.controlBg,
                          color: o.text,
                          fontSize: 12,
                          outline: "none",
                          boxSizing: "border-box"
                        }
                      }
                    )
                  ]
                }
              ),
              /* @__PURE__ */ v(
                "div",
                {
                  style: {
                    flex: 1,
                    overflowY: "auto",
                    padding: "8px 20px"
                  },
                  children: [
                    s && /* @__PURE__ */ u(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: o.textMuted,
                          fontSize: 12
                        },
                        children: "Loading libraries..."
                      }
                    ),
                    l && /* @__PURE__ */ v(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: "#ef4444",
                          fontSize: 12
                        },
                        children: [
                          "Failed to load directory: ",
                          l
                        ]
                      }
                    ),
                    !s && !l && g.length === 0 && /* @__PURE__ */ u(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: o.textDisabled,
                          fontSize: 12
                        },
                        children: "No libraries match your search."
                      }
                    ),
                    g.map((x, C) => {
                      const z = p.has(
                        `${Ln}/${x.source}`
                      ), S = f === x.id;
                      return /* @__PURE__ */ u(
                        _h,
                        {
                          entry: x,
                          isInstalled: z,
                          isInstalling: S,
                          onInstall: () => b(x),
                          theme: o
                        },
                        x.id || `dir-${C}`
                      );
                    })
                  ]
                }
              ),
              /* @__PURE__ */ v(
                "div",
                {
                  style: {
                    padding: "8px 20px",
                    borderTop: `1px solid ${o.border}`,
                    color: o.textDisabled,
                    fontSize: 10,
                    textAlign: "center",
                    flexShrink: 0
                  },
                  children: [
                    g.length,
                    " libraries • Powered by Excalidraw Libraries"
                  ]
                }
              )
            ]
          }
        )
      }
    ),
    document.body
  );
}
function _h({
  entry: t,
  isInstalled: e,
  isInstalling: o,
  onInstall: r,
  theme: n
}) {
  var i;
  const s = t.preview ? `${Ln}/${t.preview}` : null;
  return /* @__PURE__ */ v(
    "div",
    {
      style: {
        display: "flex",
        gap: 12,
        padding: "10px 0",
        borderBottom: `1px solid ${n.border}`,
        alignItems: "flex-start"
      },
      children: [
        s && /* @__PURE__ */ u(
          "img",
          {
            src: s,
            alt: t.name,
            loading: "lazy",
            style: {
              width: 64,
              height: 64,
              objectFit: "cover",
              borderRadius: 6,
              border: `1px solid ${n.border}`,
              flexShrink: 0,
              background: "#fff"
            }
          }
        ),
        /* @__PURE__ */ v("div", { style: { flex: 1, minWidth: 0 }, children: [
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: 12,
                fontWeight: 600,
                color: n.text,
                marginBottom: 2
              },
              children: t.name
            }
          ),
          ((i = t.authors) == null ? void 0 : i.length) > 0 && /* @__PURE__ */ v(
            "div",
            {
              style: {
                fontSize: 10,
                color: n.textMuted,
                marginBottom: 4
              },
              children: [
                "by ",
                t.authors.map((l) => l.name).join(", ")
              ]
            }
          ),
          t.description && /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: 10,
                color: n.textSecondary,
                lineHeight: 1.4,
                overflow: "hidden",
                display: "-webkit-box",
                WebkitLineClamp: 2,
                WebkitBoxOrient: "vertical"
              },
              children: t.description
            }
          )
        ] }),
        /* @__PURE__ */ u(
          "button",
          {
            onClick: r,
            disabled: e || o,
            style: {
              flexShrink: 0,
              padding: "5px 10px",
              border: e ? `1px solid ${n.border}` : "none",
              borderRadius: 4,
              background: e ? "transparent" : o ? n.controlBgActive : n.accentColor,
              color: e ? n.textMuted : "#fff",
              cursor: e || o ? "default" : "pointer",
              fontSize: 10,
              fontWeight: 500,
              opacity: o ? 0.7 : 1
            },
            children: e ? "Installed" : o ? "Installing..." : "Install"
          }
        )
      ]
    }
  );
}
const tu = /^[A-Za-z][A-Za-z0-9_:-]*$/, ri = /^[A-Za-z][A-Za-z0-9_]*$/;
function eu(t) {
  const e = t.trim();
  return e.startsWith('"') && e.endsWith('"') || e.startsWith("'") && e.endsWith("'") ? e.slice(1, -1).trim() : e;
}
function Ie(t) {
  return eu(t).replace(/<br\s*\/?>/gi, `
`).replace(/\\n/g, `
`);
}
function un(t, e) {
  const o = t.nodes.get(e.key);
  return o ? (o.label === o.key && e.label !== e.key && (o.label = e.label), o.shape === "rect" && e.shape !== "rect" && (o.shape = e.shape), o) : (t.nodes.set(e.key, e), e);
}
function ho(t) {
  const e = t.trim();
  if (!e) return null;
  let o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\(\(([\s\S]+)\)\)$/);
  return o ? { key: o[1], label: Ie(o[2]), shape: "circle" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\(([\s\S]+)\)$/), o ? { key: o[1], label: Ie(o[2]), shape: "round" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\{([\s\S]+)\}$/), o ? { key: o[1], label: Ie(o[2]), shape: "diamond" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\[([\s\S]+)\]$/), o ? { key: o[1], label: Ie(o[2]), shape: "rect" } : tu.test(e) ? { key: e, label: e, shape: "rect" } : null)));
}
function ou(t) {
  let e = t.match(/^(.*?)\s*--\s*\|([^|]+)\|\s*-->\s*(.*?)$/);
  if (e) {
    const o = ho(e[1]), r = ho(e[3]);
    return !o || !r ? null : { from: o, to: r, label: Ie(e[2]) };
  }
  if (e = t.match(/^(.*?)\s*--\s*([^>-][\s\S]*?)\s*-->\s*(.*?)$/), e) {
    const o = ho(e[1]), r = ho(e[3]);
    return !o || !r ? null : { from: o, to: r, label: Ie(e[2]) };
  }
  if (e = t.match(/^(.*?)\s*(?:-->|==>|-\.->|---)\s*(.*?)$/), e) {
    const o = ho(e[1]), r = ho(e[2]);
    return !o || !r ? null : { from: o, to: r };
  }
  return null;
}
function ru(t) {
  const e = t.match(/\b(TB|TD|BT|LR|RL)\b/i);
  if (!e) return "TB";
  const o = e[1].toUpperCase();
  return o === "TD" ? "TB" : o === "TB" || o === "BT" || o === "LR" || o === "RL" ? o : "TB";
}
function nu(t) {
  const e = t.match(/^subgraph(?:\s+([\s\S]+))?$/i);
  if (!e) return null;
  const o = (e[1] ?? "").trim();
  if (!o) return {};
  const r = o.match(/^[A-Za-z][A-Za-z0-9_:-]*\s*\[([\s\S]+)\]$/);
  return r ? { label: Ie(r[1]) } : { label: Ie(o) };
}
function su(t) {
  const o = { direction: "TB", nodes: /* @__PURE__ */ new Map(), edges: [], groups: [] }, r = t.replace(/\r\n/g, `
`).split(`
`).map((a) => a.replace(/%%.*$/, "").trim()).filter(Boolean);
  if (r.length === 0)
    throw new Error("Paste a Mermaid flowchart first.");
  const n = r[0];
  /^(flowchart|graph)\b/i.test(n) && (o.direction = ru(n), r.shift());
  const i = [], l = (a) => {
    for (const c of i) c.nodeKeys.add(a);
  };
  for (const a of r) {
    const c = a.split(";").map((h) => h.trim()).filter(Boolean);
    for (const h of c) {
      const f = nu(h);
      if (f) {
        i.push({ label: f.label, nodeKeys: /* @__PURE__ */ new Set() });
        continue;
      }
      if (/^end\b/i.test(h)) {
        const y = i.pop();
        y && o.groups.push({
          label: y.label,
          nodeKeys: Array.from(y.nodeKeys)
        });
        continue;
      }
      const d = ou(h);
      if (d) {
        const y = un(o, d.from), m = un(o, d.to);
        l(y.key), l(m.key), o.edges.push({ fromKey: y.key, toKey: m.key, label: d.label });
        continue;
      }
      const p = ho(h);
      if (p) {
        const y = un(o, p);
        l(y.key);
      }
    }
  }
  for (; i.length > 0; ) {
    const a = i.pop();
    o.groups.push({
      label: a.label,
      nodeKeys: Array.from(a.nodeKeys)
    });
  }
  if (o.nodes.size === 0)
    throw new Error("Could not parse Mermaid nodes. Try simple flowchart syntax like A-->B.");
  return o;
}
function iu(t) {
  const e = t.indexOf(":");
  if (e < 0) return null;
  const o = t.slice(0, e).trim(), r = t.slice(e + 1).trim();
  if (!o || !r) return null;
  const n = [
    "-->>",
    "->>",
    "--x",
    "-x",
    "-->",
    "->",
    "--)",
    "-)",
    "-.->",
    "==>",
    "---"
  ];
  for (const s of n) {
    const i = o.indexOf(s);
    if (i < 0) continue;
    const l = o.slice(0, i).trim(), a = o.slice(i + s.length).trim();
    if (!(!ri.test(l) || !ri.test(a)))
      return {
        from: l,
        arrow: s,
        to: a,
        label: Ie(r)
      };
  }
  return null;
}
function au(t) {
  const e = t.match(/^Note\s+(left|right|over)\s+of\s+([A-Za-z][A-Za-z0-9_:-]*)\s*:\s*([\s\S]+)$/i);
  return e ? {
    side: e[1].toLowerCase(),
    of: e[2],
    text: Ie(e[3])
  } : null;
}
function lu(t) {
  return t ? /^#[0-9a-f]{3,8}$/i.test(t) || /^(rgb|rgba|hsl|hsla)\(/i.test(t) ? !0 : (/* @__PURE__ */ new Set([
    "red",
    "green",
    "blue",
    "purple",
    "pink",
    "orange",
    "yellow",
    "brown",
    "gray",
    "grey",
    "black",
    "white",
    "teal",
    "cyan",
    "magenta",
    "indigo",
    "violet",
    "gold",
    "silver",
    "maroon",
    "navy",
    "olive",
    "lime",
    "aqua",
    "fuchsia",
    "rebeccapurple"
  ])).has(t.toLowerCase()) : !1;
}
function cu(t) {
  const e = t.match(/^box(?:\s+(.+))?$/i);
  if (!e) return null;
  const o = (e[1] ?? "").trim();
  if (!o) return {};
  const r = o.indexOf(" "), n = r >= 0 ? o.slice(0, r) : o, s = r >= 0 ? o.slice(r + 1).trim() : "";
  return lu(n) ? { color: n, label: s || void 0 } : { label: o };
}
function du(t) {
  const e = t.replace(/\r\n/g, `
`).split(`
`).map((f) => f.replace(/%%.*$/, "").trim()).filter(Boolean);
  if (e.length === 0)
    throw new Error("Paste Mermaid sequenceDiagram text first.");
  if (!/^sequenceDiagram\b/i.test(e[0]))
    throw new Error("Not a Mermaid sequence diagram.");
  const o = /* @__PURE__ */ new Set(), r = [], n = [], s = [], i = [], l = [];
  let a = 0;
  const c = (f) => {
    o.has(f) || (o.add(f), r.push(f));
    for (const d of l) d.participants.add(f);
  };
  for (let f = 1; f < e.length; f++) {
    const d = e[f];
    if (/^autonumber\b/i.test(d)) continue;
    const p = cu(d);
    if (p) {
      l.push({ label: p.label, color: p.color, participants: /* @__PURE__ */ new Set() });
      continue;
    }
    if (/^end\b/i.test(d)) {
      const b = l.pop();
      b && i.push(b);
      continue;
    }
    const y = d.match(/^participant\s+([A-Za-z][A-Za-z0-9_]*)(?:\s+as\s+[\s\S]+)?$/i);
    if (y) {
      c(y[1]);
      continue;
    }
    const m = au(d);
    if (m) {
      c(m.of), s.push({ step: a, note: m });
      continue;
    }
    const g = iu(d);
    if (g) {
      c(g.from), c(g.to), n.push(g), a += 1;
      continue;
    }
  }
  for (; l.length > 0; ) {
    const f = l.pop();
    i.push(f);
  }
  const h = r;
  if (h.length === 0)
    throw new Error("No participants found in sequenceDiagram.");
  if (n.length === 0 && s.length === 0)
    throw new Error("No messages/notes found in sequenceDiagram.");
  return {
    participants: h,
    messages: n,
    notes: s,
    groups: i.map((f) => ({
      label: f.label,
      color: f.color,
      participants: Array.from(f.participants)
    })).filter((f) => f.participants.length > 0)
  };
}
function kr(t) {
  return t === "diamond" ? { w: 200, h: 120 } : t === "circle" ? { w: 140, h: 140 } : { w: 200, h: 96 };
}
function hu(t) {
  const e = Array.from(t.nodes.keys()).sort(), o = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
  for (const c of e)
    o.set(c, 0), r.set(c, []);
  for (const c of t.edges)
    !o.has(c.fromKey) || !o.has(c.toKey) || (r.get(c.fromKey).push(c.toKey), o.set(c.toKey, (o.get(c.toKey) ?? 0) + 1));
  const n = e.filter((c) => (o.get(c) ?? 0) === 0), s = /* @__PURE__ */ new Map();
  for (const c of n) s.set(c, 0);
  const i = [...n];
  for (; i.length > 0; ) {
    const c = i.shift(), h = s.get(c) ?? 0;
    for (const f of r.get(c) ?? []) {
      const d = Math.max(s.get(f) ?? 0, h + 1);
      s.set(f, d), o.set(f, (o.get(f) ?? 0) - 1), (o.get(f) ?? 0) <= 0 && i.push(f);
    }
  }
  let l = 0;
  for (const c of s.values()) l = Math.max(l, c);
  for (const c of e)
    s.has(c) || (l += 1, s.set(c, l));
  const a = /* @__PURE__ */ new Map();
  for (const c of e) {
    const h = s.get(c) ?? 0;
    a.has(h) || a.set(h, []), a.get(h).push(c);
  }
  return Array.from(a.entries()).sort((c, h) => c[0] - h[0]).map(([, c]) => c.sort());
}
function uu(t, e, o, r) {
  const n = du(t), s = [], i = [], l = 180, a = 64, c = 270, h = o - 140, f = h + a + 8, d = 88, p = Math.max(1, n.messages.length), y = f + p * d + 40, m = y + 12, g = m + a, b = /* @__PURE__ */ new Map();
  for (const x of n.groups) {
    const C = x.participants.map((K) => b.get(K)).filter((K) => typeof K == "number");
    if (C.length === 0)
      for (const K of x.participants) {
        const Z = n.participants.indexOf(K);
        Z >= 0 && C.push(e + (Z - (n.participants.length - 1) / 2) * c);
      }
    if (C.length === 0) continue;
    const z = Math.min(...C) - l / 2 - 24, S = Math.max(...C) + l / 2 + 24, F = h - 22, D = g - F + 18, W = {
      id: wt(10),
      type: "shape",
      x: z,
      y: F,
      w: S - z,
      h: D,
      z: r(),
      data: {
        shape: "rect",
        stroke: x.color ? x.color : "#475569",
        strokeWidth: 1.5,
        strokeStyle: "solid",
        roughness: 0,
        fill: x.color ? x.color : "#334155",
        fillStyle: "solid",
        opacity: x.color ? 0.2 : 0.08,
        edgeStyle: "sharp"
      }
    };
    if (s.push(W), i.push(W.id), x.label) {
      const K = {
        id: wt(10),
        type: "text",
        x: z + 10,
        y: F + 8,
        w: Math.max(120, S - z - 20),
        h: "auto",
        z: r(),
        data: {
          text: x.label,
          fontSize: 14,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "left"
        }
      };
      s.push(K);
    }
  }
  for (let x = 0; x < n.participants.length; x++) {
    const C = n.participants[x], z = e + (x - (n.participants.length - 1) / 2) * c;
    b.set(C, z);
    const S = {
      id: wt(10),
      type: "shape",
      x: z - l / 2,
      y: h,
      w: l,
      h: a,
      z: r(),
      data: {
        shape: "rect",
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        edgeStyle: "round",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        label: C,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    s.push(S), i.push(S.id);
    const F = {
      id: wt(10),
      type: "shape",
      x: z - 1,
      y: f,
      w: 2,
      h: y - f,
      z: r(),
      data: {
        shape: "line",
        stroke: "#94a3b8",
        strokeWidth: 2,
        strokeStyle: "dashed",
        roughness: 0,
        startPoint: [1, 0],
        endPoint: [1, y - f]
      }
    };
    s.push(F), i.push(F.id);
    const D = {
      id: wt(10),
      type: "shape",
      x: z - l / 2,
      y: m,
      w: l,
      h: a,
      z: r(),
      data: {
        shape: "rect",
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        edgeStyle: "round",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        label: C,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    s.push(D), i.push(D.id);
  }
  for (let x = 0; x < n.messages.length; x++) {
    const C = n.messages[x], z = f + (x + 1) * d, S = b.get(C.from), F = b.get(C.to);
    if (S == null || F == null) continue;
    const D = Math.min(S, F), W = Math.max(S, F), K = Math.max(W - D, 40), Z = S <= F ? 0 : K, dt = S <= F ? K : 0, X = C.arrow.includes("--") || C.arrow === "-.->", rt = C.arrow.toLowerCase().includes("x"), J = C.arrow.includes(">") || C.arrow.includes(")"), U = {
      id: wt(10),
      type: "shape",
      x: D,
      y: z - 14,
      w: K,
      h: 28,
      z: r(),
      data: {
        shape: "arrow",
        stroke: "#475569",
        strokeWidth: 2,
        strokeStyle: X ? "dashed" : "solid",
        roughness: 0,
        startPoint: [Z, 14],
        endPoint: [dt, 14]
      }
    };
    s.push(U), i.push(U.id), J ? U.data.shape = "arrow" : U.data.shape = "line";
    const R = {
      id: wt(10),
      type: "text",
      x: D,
      y: z - 46,
      w: K,
      h: "auto",
      z: r(),
      data: {
        text: C.label,
        fontSize: 14,
        fontFamily: "sans-serif",
        color: "#0f172a",
        align: "center"
      }
    };
    if (s.push(R), rt) {
      const O = S <= F ? D + K - 14 : D + 8, _ = {
        id: wt(10),
        type: "text",
        x: O,
        y: z - 20,
        w: 20,
        h: "auto",
        z: r(),
        data: {
          text: "×",
          fontSize: 16,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "center"
        }
      };
      s.push(_);
    }
  }
  for (const x of n.notes) {
    const C = f + (x.step + 1) * d, z = b.get(x.note.of);
    if (z == null) continue;
    let S = z;
    x.note.side === "right" && (S += 130), x.note.side === "left" && (S -= 300), x.note.side === "over" && (S -= 110);
    const F = {
      id: wt(10),
      type: "text",
      x: S,
      y: C - 8,
      w: 260,
      h: "auto",
      z: r(),
      data: {
        text: x.note.text,
        fontSize: 13,
        fontFamily: "sans-serif",
        color: "#0f172a",
        align: "left"
      }
    };
    s.push(F);
  }
  return { nodes: s, shapeNodeIds: i };
}
function fu(t, e, o, r) {
  const n = t.trimStart();
  if (/^sequenceDiagram\b/i.test(n))
    return uu(t, e, o, r);
  const s = su(t), i = hu(s), l = Array.from(s.nodes.values()).map((g) => kr(g.shape)), a = l.length > 0 ? Math.max(...l.map((g) => g.h)) : 96, c = Math.max(a + 130, 260), h = /* @__PURE__ */ new Map(), f = i.length;
  for (let g = 0; g < i.length; g++) {
    const b = i[g], x = b.length, C = (g - (f - 1) / 2) * c, z = b.length > 0 ? Math.max(
      ...b.map((F) => {
        const D = s.nodes.get(F);
        return D ? kr(D.shape).w : 200;
      })
    ) : 200, S = Math.max(z + 90, 260);
    for (let F = 0; F < b.length; F++) {
      const D = b[F], W = (F - (x - 1) / 2) * S;
      if (s.direction === "LR" || s.direction === "RL") {
        const K = s.direction === "LR" ? e + C : e - C, Z = o + W;
        h.set(D, { x: K, y: Z });
      } else {
        const K = e + W, Z = s.direction === "TB" ? o + C : o - C;
        h.set(D, { x: K, y: Z });
      }
    }
  }
  const d = /* @__PURE__ */ new Map(), p = [], y = [], m = /* @__PURE__ */ new Map();
  for (const g of s.groups) {
    if (!g.nodeKeys.length) continue;
    const b = g.nodeKeys.map((D) => {
      const W = s.nodes.get(D), K = h.get(D);
      if (!W || !K) return null;
      const Z = kr(W.shape);
      return { x: K.x - Z.w / 2, y: K.y - Z.h / 2, w: Z.w, h: Z.h };
    }).filter((D) => !!D);
    if (!b.length) continue;
    const x = Math.min(...b.map((D) => D.x)) - 30, C = Math.max(...b.map((D) => D.x + D.w)) + 30, z = Math.min(...b.map((D) => D.y)) - 34, S = Math.max(...b.map((D) => D.y + D.h)) + 24, F = {
      id: wt(10),
      type: "shape",
      x,
      y: z,
      w: C - x,
      h: S - z,
      z: r(),
      data: {
        shape: "rect",
        stroke: "#475569",
        strokeWidth: 1.5,
        strokeStyle: "solid",
        roughness: 0,
        fill: "#334155",
        fillStyle: "solid",
        opacity: 0.12,
        edgeStyle: "sharp"
      }
    };
    if (p.push(F), y.push(F.id), g.label) {
      const D = {
        id: wt(10),
        type: "text",
        x: x + 10,
        y: z + 8,
        w: Math.max(120, C - x - 20),
        h: "auto",
        z: r(),
        data: {
          text: g.label,
          fontSize: 14,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "left"
        }
      };
      p.push(D);
    }
  }
  for (const [g, b] of s.nodes) {
    const x = h.get(g) ?? { x: e, y: o }, C = kr(b.shape), z = {
      id: wt(10),
      type: "shape",
      x: x.x - C.w / 2,
      y: x.y - C.h / 2,
      w: C.w,
      h: C.h,
      z: r(),
      data: {
        shape: b.shape === "diamond" ? "diamond" : b.shape === "circle" ? "ellipse" : (b.shape === "round", "rect"),
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        edgeStyle: b.shape === "round" ? "round" : "sharp",
        label: b.label,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    p.push(z), y.push(z.id), d.set(g, z.id), m.set(g, { x: z.x, y: z.y, w: C.w, h: C.h });
  }
  for (const g of s.edges) {
    const b = d.get(g.fromKey), x = d.get(g.toKey);
    if (!b || !x || b === x) continue;
    const C = {
      id: wt(10),
      type: "edge",
      x: 0,
      y: 0,
      w: 0,
      h: 0,
      z: r(),
      data: {
        fromId: b,
        toId: x,
        label: g.label,
        style: "solid",
        color: "#64748b",
        strokeWidth: 2,
        arrowHead: "arrow",
        edgeType: "bezier"
      }
    };
    p.push(C);
  }
  return { nodes: p, shapeNodeIds: y };
}
const ni = `flowchart LR
  A[Idea] --> B{Decision}
  B -- Yes --> C[Ship]
  B -- No --> D[Refine]
  D --> B`;
function pu({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: r
}) {
  const n = Ut(), s = at(null), [i, l] = tt(ni), [a, c] = tt(null), [h, f] = tt(null);
  mt(() => {
    if (!e) return;
    const y = (m) => {
      s.current && !s.current.contains(m.target) && o();
    };
    return document.addEventListener("pointerdown", y), () => document.removeEventListener("pointerdown", y);
  }, [e, o]);
  const d = Kt(
    () => "Supported: flowchart/graph (TB/BT/LR/RL) and sequenceDiagram. Flowchart nodes: A[Text], A{Decision}, A((Start)). Edges: A-->B, A -- label --> B.",
    []
  ), p = it(() => {
    try {
      const y = window.innerWidth / 2, m = window.innerHeight / 2, g = t.screenToCanvas(y, m), { nodes: b, shapeNodeIds: x } = fu(i, g.x, g.y, () => t.nextZ());
      if (b.length === 0)
        throw new Error("No nodes were parsed.");
      t.addNodes(b), x.length > 0 && t.selectMultiple(x), c(null), f(`Inserted ${x.length} nodes and ${b.length - x.length} edges.`);
    } catch (y) {
      f(null), c(y instanceof Error ? y.message : "Failed to parse Mermaid graph.");
    }
  }, [t, i]);
  return !e || !r ? null : Je(
    /* @__PURE__ */ v(
      "div",
      {
        ref: s,
        style: {
          position: "fixed",
          left: r.right + 8,
          top: r.top,
          background: n.panelBg,
          border: `1px solid ${n.border}`,
          borderRadius: n.panelBorderRadius,
          boxShadow: n.panelShadow,
          width: 340,
          maxHeight: `calc(100vh - ${r.top + 20}px)`,
          overflow: "hidden",
          display: "flex",
          flexDirection: "column",
          zIndex: 99999
        },
        onPointerDown: (y) => y.stopPropagation(),
        children: [
          /* @__PURE__ */ v("div", { style: { padding: "10px 12px 8px", borderBottom: `1px solid ${n.border}` }, children: [
            /* @__PURE__ */ u("div", { style: { fontSize: 12, fontWeight: 700, color: n.text }, children: "Mermaid Sketch" }),
            /* @__PURE__ */ u("div", { style: { marginTop: 4, fontSize: 10, color: n.textMuted, lineHeight: 1.45 }, children: d })
          ] }),
          /* @__PURE__ */ v("div", { style: { padding: 12, display: "flex", flexDirection: "column", gap: 8 }, children: [
            /* @__PURE__ */ u(
              "textarea",
              {
                value: i,
                onChange: (y) => l(y.target.value),
                spellCheck: !1,
                style: {
                  width: "100%",
                  minHeight: 180,
                  resize: "vertical",
                  padding: "8px 10px",
                  borderRadius: n.controlBorderRadius,
                  border: `1px solid ${n.border}`,
                  background: n.controlBg,
                  color: n.text,
                  fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace",
                  fontSize: 11,
                  lineHeight: 1.4,
                  boxSizing: "border-box"
                }
              }
            ),
            a && /* @__PURE__ */ u("div", { style: { fontSize: 10, color: "#ef4444" }, children: a }),
            h && /* @__PURE__ */ u("div", { style: { fontSize: 10, color: "#16a34a" }, children: h }),
            /* @__PURE__ */ v("div", { style: { display: "flex", justifyContent: "space-between", gap: 8 }, children: [
              /* @__PURE__ */ u(
                "button",
                {
                  onClick: () => l(ni),
                  style: {
                    border: `1px solid ${n.border}`,
                    background: "transparent",
                    color: n.text,
                    borderRadius: n.controlBorderRadius,
                    padding: "6px 10px",
                    fontSize: 11,
                    cursor: "pointer"
                  },
                  children: "Reset Example"
                }
              ),
              /* @__PURE__ */ u(
                "button",
                {
                  onClick: p,
                  style: {
                    border: `1px solid ${n.border}`,
                    background: n.controlBgActive,
                    color: n.text,
                    borderRadius: n.controlBorderRadius,
                    padding: "6px 10px",
                    fontSize: 11,
                    fontWeight: 600,
                    cursor: "pointer"
                  },
                  children: "Insert Diagram"
                }
              )
            ] })
          ] })
        ]
      }
    ),
    document.body
  );
}
const yu = [
  { key: "select", label: "Select", shortcut: "S", num: "" },
  { key: "hand", label: "Hand", shortcut: "P", num: "" },
  { key: "draw", label: "Draw", shortcut: "D", num: "" },
  { key: "shape", label: "Shape", shortcut: "G", num: "" },
  { key: "text", label: "Text", shortcut: "T", num: "" },
  { key: "note", label: "Note", shortcut: "B", num: "" },
  { key: "sticky", label: "Sticky", shortcut: "Y", num: "" },
  { key: "frame", label: "Frame", shortcut: "F", num: "" },
  { key: "erase", label: "Eraser", shortcut: "E", num: "" },
  { key: "laser", label: "Laser", shortcut: "Z", num: "" }
], mo = {
  border: "none",
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, Ft = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function go({ name: t, size: e = 18 }) {
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "select" && /* @__PURE__ */ u("path", { d: "M6 2v17l4-4.5L13.5 21l2.5-1.5L12.5 13H18z", fill: "currentColor" }),
    t === "draw" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M17 3l4 4L7.5 20.5 2 22l1.5-5.5z", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M15 5l4 4", ...Ft })
    ] }),
    t === "shape" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...Ft }),
    t === "text" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M7 4h10", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M12 4v16", ...Ft })
    ] }),
    t === "note" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 3h16v14l-4 4H4z", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M16 17v4l4-4z", fill: "currentColor", opacity: 0.4 })
    ] }),
    t === "sticky" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "1", fill: "currentColor", opacity: 0.15, ...Ft }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "9", x2: "17", y2: "9", ...Ft, opacity: 0.5 }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "13", x2: "14", y2: "13", ...Ft, opacity: 0.5 })
    ] }),
    t === "frame" && /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2", ...Ft, strokeDasharray: "4,2" }),
    t === "hand" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M8 13V5.5a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M11 5.5v-2a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M14 5.5a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M17 7.5a1.5 1.5 0 0 1 3 0V16a6 6 0 0 1-6 6h-2a6 6 0 0 1-6-6V9.5a1.5 1.5 0 0 1 3 0", ...Ft })
    ] }),
    t === "erase" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 20H9L3 14l9.5-9.5 8 8L16 17", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M12.5 4.5l8 8", ...Ft })
    ] }),
    t === "laser" && /* @__PURE__ */ u("circle", { cx: "12", cy: "12", r: "4", fill: "currentColor", opacity: 0.9 }),
    t === "lasso" && /* @__PURE__ */ u("path", { d: "M18 4c-3 0-5 2-8 5S5 14 4 16c-1 3 1 4 3 4s4-2 6-4 4-4 6-5 3-2 3-4-1-3-3-3z", ...Ft, strokeDasharray: "3,2" }),
    t === "undo" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...Ft, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...Ft, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...Ft, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...Ft, fill: "none" })
    ] }),
    t === "print" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M6 9V3h12v6", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M6 18H4a1 1 0 0 1-1-1v-5a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-2", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "6", y: "14", width: "12", height: "7", rx: "1", ...Ft })
    ] }),
    t === "fit" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...Ft })
    ] }),
    t === "paper" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "4", y: "2", width: "16", height: "20", rx: "1", ...Ft }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "7", x2: "16", y2: "7", ...Ft, opacity: 0.4 }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "11", x2: "16", y2: "11", ...Ft, opacity: 0.4 }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "15", x2: "13", y2: "15", ...Ft, opacity: 0.4 })
    ] }),
    t === "template" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "13", y: "3", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "3", y: "13", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "13", y: "13", width: "8", height: "8", rx: "1", ...Ft })
    ] }),
    t === "library" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M8 7h6", ...Ft, opacity: 0.5 }),
      /* @__PURE__ */ u("path", { d: "M8 11h4", ...Ft, opacity: 0.5 })
    ] }),
    t === "gif" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "4", width: "20", height: "16", rx: "2", ...Ft }),
      /* @__PURE__ */ u("text", { x: "12", y: "14.5", textAnchor: "middle", fontSize: "7", fontWeight: "700", fill: "currentColor", stroke: "none", children: "GIF" })
    ] }),
    t === "mermaid" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "4", width: "18", height: "14", rx: "2", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M6 8l2.6 3 2.1-2 2.2 3 2.2-2.5L18 13", ...Ft }),
      /* @__PURE__ */ u("circle", { cx: "6", cy: "8", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "10.7", cy: "9", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "14.9", cy: "9.5", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "18", cy: "13", r: "1.1", fill: "currentColor", stroke: "none" })
    ] })
  ] });
}
const mu = {
  light: "Light",
  dark: "Dark",
  textured: "Textured"
};
function gu({
  engine: t,
  background: e
}) {
  const o = Ut(), [r, n] = tt(!1), s = at(null), i = at(null);
  mt(() => {
    if (!r) return;
    const c = (h) => {
      i.current && !i.current.contains(h.target) && s.current && !s.current.contains(h.target) && n(!1);
    };
    return document.addEventListener("pointerdown", c), () => document.removeEventListener("pointerdown", c);
  }, [r]);
  const l = $o.find((c) => c.key === e) ?? $o[1], a = r && s.current ? (() => {
    const c = s.current.getBoundingClientRect();
    return Je(
      /* @__PURE__ */ u(
        "div",
        {
          ref: i,
          style: {
            position: "fixed",
            left: c.right + 8,
            top: c.top,
            background: o.panelBg,
            border: `1px solid ${o.border}`,
            borderRadius: o.panelBorderRadius,
            padding: 8,
            zIndex: 99999,
            boxShadow: o.panelShadow,
            width: 180,
            maxHeight: 400,
            overflowY: "auto"
          },
          onPointerDown: (h) => h.stopPropagation(),
          children: ["light", "dark", "textured"].map((h) => {
            const f = $o.filter((d) => d.group === h);
            return f.length === 0 ? null : /* @__PURE__ */ v("div", { style: { marginBottom: 6 }, children: [
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    fontSize: 10,
                    fontWeight: 600,
                    color: o.textMuted,
                    textTransform: "uppercase",
                    letterSpacing: "0.05em",
                    padding: "4px 6px 2px"
                  },
                  children: mu[h]
                }
              ),
              f.map((d) => /* @__PURE__ */ v(
                "button",
                {
                  onClick: () => {
                    t.setBoardBackground(d.key), n(!1);
                  },
                  style: {
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                    width: "100%",
                    padding: "5px 6px",
                    border: "none",
                    borderRadius: o.controlBorderRadius,
                    background: e === d.key ? o.controlBgActive : "transparent",
                    color: o.text,
                    cursor: "pointer",
                    fontSize: 12,
                    textAlign: "left"
                  },
                  children: [
                    /* @__PURE__ */ u(
                      "span",
                      {
                        style: {
                          width: 18,
                          height: 18,
                          borderRadius: 3,
                          background: d.swatchColor,
                          border: `1.5px solid ${o.border}`,
                          flexShrink: 0
                        }
                      }
                    ),
                    d.label
                  ]
                },
                d.key
              ))
            ] }, h);
          })
        }
      ),
      document.body
    );
  })() : null;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ v(
      "button",
      {
        ref: s,
        title: "Paper type",
        onClick: () => n((c) => !c),
        style: {
          ...mo,
          width: 40,
          height: 40,
          borderRadius: o.controlBorderRadius,
          background: r ? o.controlBgActive : "transparent",
          color: o.text,
          position: "relative"
        },
        children: [
          /* @__PURE__ */ u(go, { name: "paper" }),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                position: "absolute",
                bottom: 4,
                right: 4,
                width: 8,
                height: 8,
                borderRadius: "50%",
                background: l.swatchColor,
                border: `1px solid ${o.border}`
              }
            }
          )
        ]
      }
    ),
    a
  ] });
}
function bu({ engine: t }) {
  const e = Ut(), [o, r] = tt(!1), n = at(null), s = at(null);
  mt(() => {
    if (!o) return;
    const l = (a) => {
      s.current && !s.current.contains(a.target) && n.current && !n.current.contains(a.target) && r(!1);
    };
    return document.addEventListener("pointerdown", l), () => document.removeEventListener("pointerdown", l);
  }, [o]);
  const i = o && n.current ? (() => {
    const l = n.current.getBoundingClientRect();
    return Je(
      /* @__PURE__ */ v(
        "div",
        {
          ref: s,
          style: {
            position: "fixed",
            left: l.right + 8,
            top: l.top,
            background: e.panelBg,
            border: `1px solid ${e.border}`,
            borderRadius: 8,
            padding: 8,
            zIndex: 99999,
            boxShadow: "0 4px 16px rgba(0,0,0,0.15)",
            width: 180
          },
          onPointerDown: (a) => a.stopPropagation(),
          children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 10,
                  fontWeight: 600,
                  color: e.textMuted,
                  textTransform: "uppercase",
                  letterSpacing: "0.05em",
                  padding: "4px 6px 2px"
                },
                children: "Templates"
              }
            ),
            wi.map((a) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => {
                  const c = typeof window < "u" ? window : void 0;
                  if (!c) return;
                  const h = c.innerWidth / 2, f = c.innerHeight / 2, d = Eo(t.viewport, h, f);
                  t.applyTemplate(a.id, d.x, d.y), r(!1);
                },
                style: {
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  width: "100%",
                  padding: "6px 6px",
                  border: "none",
                  borderRadius: e.controlBorderRadius,
                  background: "transparent",
                  color: e.text,
                  cursor: "pointer",
                  fontSize: 12,
                  textAlign: "left"
                },
                onMouseEnter: (c) => {
                  c.currentTarget.style.background = e.controlBgActive;
                },
                onMouseLeave: (c) => {
                  c.currentTarget.style.background = "transparent";
                },
                children: a.label
              },
              a.id
            ))
          ]
        }
      ),
      document.body
    );
  })() : null;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: n,
        title: "Templates",
        onClick: () => r((l) => !l),
        style: {
          ...mo,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(go, { name: "template" })
      }
    ),
    i
  ] });
}
function xu({ engine: t }) {
  const e = Ut(), [o, r] = tt(!1), [n, s] = tt(!1), i = at(null), [l, a] = tt(null), c = it(() => {
    r((d) => (!d && i.current && a(i.current.getBoundingClientRect()), !d));
  }, []), h = it(() => r(!1), []), f = it(() => {
    s(!0);
  }, []);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: i,
        title: "Libraries",
        onClick: c,
        style: {
          ...mo,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(go, { name: "library" })
      }
    ),
    /* @__PURE__ */ u(
      oh,
      {
        engine: t,
        open: o,
        onClose: h,
        triggerRect: l,
        onBrowseDirectory: f
      }
    ),
    n && /* @__PURE__ */ u(
      $h,
      {
        onClose: () => s(!1),
        onInstalled: () => {
          r(!1), setTimeout(() => {
            i.current && a(i.current.getBoundingClientRect()), r(!0);
          }, 100);
        }
      }
    )
  ] });
}
function wu({ engine: t, baseUrl: e }) {
  const o = Ut(), [r, n] = tt(!1), s = at(null), [i, l] = tt(null), a = it(() => {
    n((h) => (!h && s.current && l(s.current.getBoundingClientRect()), !h));
  }, []), c = it(() => n(!1), []);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: s,
        title: "GIF Search",
        onClick: a,
        style: {
          ...mo,
          width: 40,
          height: 40,
          borderRadius: o.controlBorderRadius,
          background: r ? o.controlBgActive : "transparent",
          color: o.text
        },
        children: /* @__PURE__ */ u(go, { name: "gif" })
      }
    ),
    /* @__PURE__ */ u(
      ih,
      {
        engine: t,
        open: r,
        onClose: c,
        triggerRect: i,
        baseUrl: e
      }
    )
  ] });
}
function ku({ engine: t }) {
  const e = Ut(), [o, r] = tt(!1), n = at(null), [s, i] = tt(null), l = it(() => {
    r((c) => (!c && n.current && i(n.current.getBoundingClientRect()), !c));
  }, []), a = it(() => r(!1), []);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: n,
        title: "Mermaid Sketch",
        onClick: l,
        style: {
          ...mo,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(go, { name: "mermaid" })
      }
    ),
    /* @__PURE__ */ u(
      pu,
      {
        engine: t,
        open: o,
        onClose: a,
        triggerRect: s
      }
    )
  ] });
}
function vu({ engine: t, gifApiBaseUrl: e }) {
  const o = Ut(), [r, n] = tt(t.mode), [s, i] = tt(t.boardBackground), [l, a] = tt(t.lassoSelect);
  return mt(() => {
    const c = () => n(t.mode), h = () => i(t.boardBackground), f = () => a(t.lassoSelect);
    return t.on("mode", c), t.on("background", h), t.on("lassoToggle", f), () => {
      t.off("mode", c), t.off("background", h), t.off("lassoToggle", f);
    };
  }, [t]), /* @__PURE__ */ v(
    "div",
    {
      "data-sb-toolbar": !0,
      style: {
        width: ir,
        height: "100%",
        flexShrink: 0,
        background: o.toolbarBg,
        borderRight: `1px solid ${o.border}`,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        padding: "8px 0",
        gap: 4
      },
      children: [
        yu.map((c) => {
          const h = r === c.key && !(c.key === "select" && l);
          return /* @__PURE__ */ v(
            "button",
            {
              title: `${c.label} (${c.shortcut}${c.num ? ` / ${c.num}` : ""})`,
              onClick: () => {
                l && (t.toggleLassoSelect(), a(!1)), t.setMode(c.key);
              },
              style: {
                ...mo,
                width: 40,
                height: 40,
                borderRadius: o.controlBorderRadius,
                background: h ? o.controlBgActive : "transparent",
                color: o.text,
                position: "relative"
              },
              children: [
                /* @__PURE__ */ u(go, { name: c.key }),
                /* @__PURE__ */ u(
                  "span",
                  {
                    style: {
                      position: "absolute",
                      bottom: 2,
                      right: 2,
                      fontSize: 8,
                      lineHeight: 1,
                      color: o.textMuted,
                      fontWeight: 500,
                      pointerEvents: "none"
                    },
                    children: c.num || c.shortcut
                  }
                )
              ]
            },
            c.key
          );
        }),
        /* @__PURE__ */ u("div", { style: { width: 28, height: 1, background: o.separator, margin: "8px 0" } }),
        /* @__PURE__ */ v(
          "button",
          {
            title: "Lasso Select (L)",
            onClick: () => {
              l ? (t.toggleLassoSelect(), a(!1)) : (t.setMode("select"), t.lassoSelect || t.toggleLassoSelect(), a(!0));
            },
            style: {
              ...mo,
              width: 40,
              height: 40,
              borderRadius: o.controlBorderRadius,
              background: l ? o.controlBgActive : "transparent",
              color: o.text,
              position: "relative"
            },
            children: [
              /* @__PURE__ */ u(go, { name: "lasso" }),
              /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    position: "absolute",
                    bottom: 2,
                    right: 2,
                    fontSize: 8,
                    lineHeight: 1,
                    color: o.textMuted,
                    fontWeight: 500,
                    pointerEvents: "none"
                  },
                  children: "L"
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ u("div", { style: { width: 28, height: 1, background: o.separator, margin: "8px 0" } }),
        /* @__PURE__ */ u(gu, { engine: t, background: s }),
        /* @__PURE__ */ u(bu, { engine: t }),
        /* @__PURE__ */ u(xu, { engine: t }),
        /* @__PURE__ */ u(ku, { engine: t }),
        e && /* @__PURE__ */ u(wu, { engine: t, baseUrl: e })
      ]
    }
  );
}
const Su = /* @__PURE__ */ new Set(["shape", "draw", "text", "image", "content", "frame", "sticky"]), Mu = /* @__PURE__ */ new Set(["text", "image", "content", "frame"]);
function si(t) {
  return t.data.opacity ?? 1;
}
function Io(t, e) {
  return t.data[e];
}
function Cu(t) {
  const e = {}, o = t.filter((n) => Su.has(n.type));
  if (o.length > 0) {
    const n = si(o[0]), s = o.every((i) => si(i) === n);
    e.opacity = s ? n : "mixed";
  }
  const r = t.filter((n) => Mu.has(n.type));
  if (r.length > 0) {
    const n = Io(r[0], "borderColor"), s = r.every(
      (h) => Io(h, "borderColor") === n
    );
    e.borderColor = s ? n ?? null : "mixed";
    const i = Io(r[0], "borderWidth") ?? 1, l = r.every(
      (h) => (Io(h, "borderWidth") ?? 1) === i
    );
    e.borderWidth = l ? i : "mixed";
    const a = Io(r[0], "borderStyle") ?? "solid", c = r.every(
      (h) => (Io(h, "borderStyle") ?? "solid") === a
    );
    e.borderStyle = c ? a : "mixed";
  }
  return e;
}
function zu(t) {
  const [e, o] = tt(t.mode), [r, n] = tt(new Set(t.selection)), [, s] = tt(0);
  if (mt(() => {
    const h = () => o(t.mode), f = () => {
      n(new Set(t.selection)), s((p) => p + 1);
    }, d = () => s((p) => p + 1);
    return t.on("mode", h), t.on("selection", f), t.on("change", d), () => {
      t.off("mode", h), t.off("selection", f), t.off("change", d);
    };
  }, [t]), r.size === 0)
    return e === "draw" || e === "shape" || e === "text" ? { target: { kind: "tool", mode: e }, commonProps: {} } : { target: { kind: "none" }, commonProps: {} };
  const i = [];
  for (const h of r) {
    const f = t.getNode(h);
    f && i.push(f);
  }
  if (i.length === 0)
    return { target: { kind: "none" }, commonProps: {} };
  if (i.length === 1)
    return { target: { kind: "single", node: i[0] }, commonProps: {} };
  const l = /* @__PURE__ */ new Map();
  for (const h of i) {
    const f = l.get(h.type);
    f ? f.push(h) : l.set(h.type, [h]);
  }
  const a = [];
  for (const [h, f] of l)
    a.push({ type: h, nodes: f });
  const c = Cu(i);
  return {
    target: { kind: "multi", nodes: i, typeGroups: a },
    commonProps: c
  };
}
const Or = xi(null);
function Pe(t, e) {
  const o = Er(Or);
  return it(
    (r) => {
      if (o && o.length > 1) {
        const n = o.map((s) => ({
          id: s.id,
          patch: {
            data: { ...s.data, ...r }
          }
        }));
        t.batchUpdateWithHistory(n);
      } else
        t.updateNodeWithHistory(e.id, {
          data: { ...e.data, ...r }
        });
    },
    [t, e, o]
  );
}
function ke({
  value: t,
  onChange: e,
  mixed: o
}) {
  const r = Ut(), n = o || t === void 0 ? 100 : Math.round(t * 100);
  return /* @__PURE__ */ v("div", { style: Et, children: [
    /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Opacity" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "range",
        min: 0,
        max: 100,
        value: n,
        onChange: (s) => e(parseInt(s.target.value) / 100),
        style: { flex: 1, accentColor: r.accentColor }
      }
    ),
    /* @__PURE__ */ u("span", { style: { width: 28, textAlign: "right", fontSize: 10, color: o ? r.textFaint : r.text }, children: o ? "--" : n })
  ] });
}
const Iu = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
function ye({
  label: t,
  palettes: e,
  value: o,
  onChange: r,
  allowNull: n,
  mixed: s
}) {
  const i = Ut(), [l, a] = tt(""), [c, h] = tt(0), [f, d] = tt(!1), p = at(null), y = e[c] ?? e[0], m = o == null ? void 0 : o.toLowerCase();
  mt(() => {
    if (!f) return;
    const x = (C) => {
      p.current && !p.current.contains(C.target) && d(!1);
    };
    return document.addEventListener("mousedown", x), () => document.removeEventListener("mousedown", x);
  }, [f]);
  const g = () => {
    const x = l.trim();
    if (!x) return;
    const C = x.startsWith("#") ? x : `#${x}`;
    Iu.test(C) && (r(C), a(""));
  }, b = e.some(
    (x) => x.colors.some((C) => C.toLowerCase() === m)
  );
  return /* @__PURE__ */ v("div", { style: { display: "flex", flexDirection: "column", gap: 3 }, children: [
    /* @__PURE__ */ v("div", { style: { ...Et, margin: 0, flexWrap: "wrap", gap: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: i.textMuted }, children: t }),
      n && /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r(null),
          title: "None",
          style: {
            ...Zt,
            width: 20,
            height: 20,
            background: "transparent",
            border: !s && o == null ? `2px solid ${i.swatchBorderActive}` : `2px solid ${i.textDisabled}`,
            borderRadius: "50%",
            position: "relative",
            overflow: "hidden"
          },
          children: /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                width: "140%",
                height: 2,
                background: i.error,
                transform: "rotate(-45deg)",
                top: "50%",
                left: "-20%"
              }
            }
          )
        }
      ),
      y.colors.map((x) => {
        const C = !s && m === x.toLowerCase();
        return /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r(x),
            style: {
              ...Zt,
              width: 20,
              height: 20,
              background: x,
              border: C ? `2px solid ${i.swatchBorderActive}` : "2px solid transparent",
              borderRadius: "50%"
            }
          },
          x
        );
      }),
      o && !b && !s && /* @__PURE__ */ u(
        "div",
        {
          style: {
            width: 20,
            height: 20,
            borderRadius: "50%",
            background: o,
            border: `2px solid ${i.swatchBorderActive}`,
            flexShrink: 0
          }
        }
      ),
      s && /* @__PURE__ */ u("span", { style: { fontSize: 9, color: i.textMuted, fontStyle: "italic" }, children: "Mixed" }),
      e.length > 1 && /* @__PURE__ */ v("div", { ref: p, style: { position: "relative", marginLeft: "auto" }, children: [
        /* @__PURE__ */ v(
          "button",
          {
            onClick: () => d((x) => !x),
            title: "Switch palette",
            style: {
              ...Zt,
              height: 20,
              padding: "0 5px",
              background: i.controlBg,
              color: i.textMuted,
              fontSize: 9,
              borderRadius: i.controlBorderRadius,
              display: "flex",
              alignItems: "center",
              gap: 2
            },
            children: [
              y.name,
              /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: f ? "▲" : "▼" })
            ]
          }
        ),
        f && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              top: "100%",
              right: 0,
              marginTop: 2,
              background: i.panelBg,
              border: `1px solid ${i.border}`,
              borderRadius: i.panelBorderRadius,
              padding: 4,
              zIndex: 100,
              display: "flex",
              flexDirection: "column",
              gap: 2,
              minWidth: 120,
              boxShadow: i.panelShadow
            },
            children: e.map((x, C) => /* @__PURE__ */ v(
              "button",
              {
                onClick: () => {
                  h(C), d(!1);
                },
                style: {
                  ...Zt,
                  height: 28,
                  padding: "0 8px",
                  background: C === c ? i.controlBgActive : "transparent",
                  color: i.text,
                  fontSize: 10,
                  borderRadius: i.controlBorderRadius,
                  display: "flex",
                  alignItems: "center",
                  gap: 6,
                  width: "100%",
                  justifyContent: "flex-start"
                },
                children: [
                  /* @__PURE__ */ u("span", { style: { display: "flex", gap: 2 }, children: x.colors.slice(0, 6).map((z) => /* @__PURE__ */ u(
                    "span",
                    {
                      style: {
                        width: 10,
                        height: 10,
                        borderRadius: "50%",
                        background: z,
                        display: "inline-block"
                      }
                    },
                    z
                  )) }),
                  /* @__PURE__ */ u("span", { children: x.name })
                ]
              },
              x.name
            ))
          }
        )
      ] })
    ] }),
    /* @__PURE__ */ u("div", { style: { display: "flex", alignItems: "center", gap: 4, paddingLeft: 52 }, children: /* @__PURE__ */ u(
      "input",
      {
        type: "text",
        value: l,
        onChange: (x) => a(x.target.value),
        onKeyDown: (x) => {
          x.key === "Enter" && g();
        },
        onBlur: g,
        placeholder: o ?? "#000000",
        style: {
          width: 72,
          height: 20,
          background: i.controlBg,
          border: `1px solid ${i.border}`,
          borderRadius: i.controlBorderRadius,
          color: i.text,
          fontSize: 10,
          fontFamily: "monospace",
          padding: "0 6px",
          outline: "none"
        }
      }
    ) })
  ] });
}
function Xo({
  label: t,
  value: e,
  onChange: o,
  mixed: r
}) {
  const n = Ut();
  return /* @__PURE__ */ v("div", { style: Et, children: [
    /* @__PURE__ */ u("span", { style: { ...Wt, color: n.textMuted }, children: t }),
    Kh.map((s) => /* @__PURE__ */ u(
      "button",
      {
        title: s.label,
        onClick: () => o(s.key),
        style: {
          ...Zt,
          width: 36,
          height: 28,
          background: !r && e === s.key ? n.controlBgActive : n.controlBg,
          borderRadius: n.controlBorderRadius
        },
        children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
          "line",
          {
            x1: 2,
            y1: 6,
            x2: 22,
            y2: 6,
            stroke: n.text,
            strokeWidth: 2,
            strokeDasharray: s.dash
          }
        ) })
      },
      s.key
    ))
  ] });
}
function Yo({
  label: t,
  widths: e = qh,
  value: o,
  onChange: r,
  mixed: n
}) {
  const s = Ut();
  return /* @__PURE__ */ v("div", { style: Et, children: [
    /* @__PURE__ */ u("span", { style: { ...Wt, color: s.textMuted }, children: t }),
    e.map((i) => /* @__PURE__ */ u(
      "button",
      {
        title: `${i}px`,
        onClick: () => r(i),
        style: {
          ...Zt,
          width: 36,
          height: 24,
          background: !n && o === i ? s.controlBgActive : s.controlBg,
          borderRadius: s.controlBorderRadius
        },
        children: /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 20,
              height: Math.max(i, 1),
              background: s.text,
              borderRadius: i / 2
            }
          }
        )
      },
      i
    ))
  ] });
}
function ar({
  borderColor: t,
  borderStyle: e,
  borderWidth: o,
  mixed: r,
  onChange: n
}) {
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      ye,
      {
        label: "Border",
        palettes: we,
        value: t,
        onChange: (s) => n("borderColor", s ?? void 0),
        allowNull: !0,
        mixed: r == null ? void 0 : r.color
      }
    ),
    (t || (r == null ? void 0 : r.color)) && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u(
        Xo,
        {
          label: "Style",
          value: e ?? "solid",
          onChange: (s) => n("borderStyle", s),
          mixed: r == null ? void 0 : r.style
        }
      ),
      /* @__PURE__ */ u(
        Yo,
        {
          label: "Width",
          value: o ?? 1,
          onChange: (s) => n("borderWidth", s),
          mixed: r == null ? void 0 : r.width
        }
      )
    ] })
  ] });
}
function is({ style: t }) {
  const e = Ut();
  return t === "hachure" ? /* @__PURE__ */ v("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: e.text, strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: e.text, strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: e.text, strokeWidth: 1.5 })
  ] }) : t === "cross-hatch" ? /* @__PURE__ */ v("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 2, y1: 2, x2: 8, y2: 14, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 2, x2: 14, y2: 14, stroke: e.text, strokeWidth: 1.2 })
  ] }) : /* @__PURE__ */ u("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: /* @__PURE__ */ u("rect", { x: 2, y: 2, width: 16, height: 12, fill: e.text, rx: 2 }) });
}
const Tu = /* @__PURE__ */ v("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
  /* @__PURE__ */ u("circle", { cx: "11", cy: "11", r: "8" }),
  /* @__PURE__ */ u("path", { d: "m21 21-4.35-4.35" })
] });
function Xr({
  value: t,
  onChange: e,
  fontsInScene: o,
  triggerStyle: r
}) {
  var x, C;
  const n = Ut(), [s, i] = tt(!1), [l, a] = tt(""), c = at(null), h = at(null), [f, d] = tt(null), p = l.trim().toLowerCase(), y = Kt(
    () => o.filter((z) => z.toLowerCase().includes(p)),
    [o, p]
  ), m = Kt(
    () => vr.filter(
      (z) => !o.includes(z.key) && (z.key.toLowerCase().includes(p) || z.label.toLowerCase().includes(p))
    ),
    [o, p]
  );
  mt(() => {
    if (!s || !h.current) return;
    const z = h.current.getBoundingClientRect(), S = 260, F = 16;
    let D = z.left;
    D + S > window.innerWidth - F && (D = window.innerWidth - S - F), D < F && (D = F), d({ top: z.bottom + 4, left: D });
  }, [s]), mt(() => {
    var F;
    if (!s) return;
    const z = (D) => {
      var dt, X;
      const W = D.target;
      if ((dt = c.current) != null && dt.contains(W)) return;
      const Z = (((X = c.current) == null ? void 0 : X.ownerDocument) ?? document).getElementById("font-picker-popover");
      Z != null && Z.contains(W) || i(!1);
    }, S = ((F = c.current) == null ? void 0 : F.ownerDocument) ?? document;
    return S.addEventListener("mousedown", z), () => S.removeEventListener("mousedown", z);
  }, [s]);
  const g = (z) => {
    e(z), i(!1), a("");
  }, b = (z, S) => {
    const F = (S == null ? void 0 : S.label) ?? z, D = S == null ? void 0 : S.category, W = t === z;
    return /* @__PURE__ */ v(
      "button",
      {
        type: "button",
        onClick: () => g(z),
        style: {
          display: "flex",
          alignItems: "center",
          gap: 10,
          width: "100%",
          padding: "8px 12px",
          border: "none",
          background: W ? "rgba(139, 92, 246, 0.15)" : "transparent",
          color: "#1e1e2e",
          cursor: "pointer",
          textAlign: "left",
          fontFamily: qe(z),
          fontSize: 14,
          borderRadius: 6
        },
        onMouseEnter: (K) => {
          W || (K.currentTarget.style.background = "rgba(0,0,0,0.05)");
        },
        onMouseLeave: (K) => {
          W || (K.currentTarget.style.background = "transparent");
        },
        children: [
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                width: 24,
                flexShrink: 0,
                fontSize: 12,
                color: "#64748b",
                fontFamily: "sans-serif"
              },
              children: Il(D)
            }
          ),
          /* @__PURE__ */ u("span", { style: { flex: 1, overflow: "hidden", textOverflow: "ellipsis" }, children: F })
        ]
      },
      z
    );
  };
  return /* @__PURE__ */ v("div", { ref: c, style: { position: "relative", flex: 1, minWidth: 0 }, children: [
    /* @__PURE__ */ v(
      "button",
      {
        ref: h,
        type: "button",
        onClick: () => i((z) => !z),
        style: {
          display: "flex",
          alignItems: "center",
          width: "100%",
          height: 28,
          padding: "0 8px",
          background: n.controlBg,
          color: n.text,
          border: `1px solid ${n.separator}`,
          borderRadius: n.controlBorderRadius,
          fontSize: 11,
          fontFamily: qe(t),
          cursor: "pointer",
          textAlign: "left",
          justifyContent: "space-between",
          ...r
        },
        children: [
          /* @__PURE__ */ u("span", { style: { overflow: "hidden", textOverflow: "ellipsis" }, children: ((x = vr.find((z) => z.key === t)) == null ? void 0 : x.label) ?? t }),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flexShrink: 0,
                opacity: 0.7,
                transform: s ? "rotate(180deg)" : "none",
                transition: "transform 0.15s"
              },
              children: "▼"
            }
          )
        ]
      }
    ),
    s && f && Je(
      /* @__PURE__ */ v(
        "div",
        {
          id: "font-picker-popover",
          style: {
            position: "fixed",
            top: f.top,
            left: f.left,
            width: 260,
            maxHeight: Math.min(320, window.innerHeight - f.top - 16),
            background: "#fff",
            borderRadius: 12,
            boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
            zIndex: 1e4,
            overflow: "hidden",
            display: "flex",
            flexDirection: "column"
          },
          children: [
            /* @__PURE__ */ v(
              "div",
              {
                style: {
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  padding: "10px 12px",
                  borderBottom: "1px solid #eee",
                  background: "#fafafa"
                },
                children: [
                  /* @__PURE__ */ u("span", { style: { color: "#64748b", display: "flex" }, children: Tu }),
                  /* @__PURE__ */ u(
                    "input",
                    {
                      type: "text",
                      placeholder: "Quick search",
                      value: l,
                      onChange: (z) => a(z.target.value),
                      autoFocus: !0,
                      style: {
                        flex: 1,
                        border: "none",
                        background: "transparent",
                        fontSize: 13,
                        outline: "none",
                        color: "#1e1e2e"
                      }
                    }
                  )
                ]
              }
            ),
            /* @__PURE__ */ v("div", { style: { overflowY: "auto", padding: 8, flex: 1 }, children: [
              y.length > 0 && /* @__PURE__ */ v("div", { style: { marginBottom: 12 }, children: [
                /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      fontSize: 10,
                      fontWeight: 600,
                      color: "#64748b",
                      textTransform: "uppercase",
                      letterSpacing: "0.05em",
                      marginBottom: 6,
                      paddingLeft: 4
                    },
                    children: "In this scene"
                  }
                ),
                y.map((z) => b(z, vr.find((S) => S.key === z)))
              ] }),
              /* @__PURE__ */ v("div", { children: [
                /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      fontSize: 10,
                      fontWeight: 600,
                      color: "#64748b",
                      textTransform: "uppercase",
                      letterSpacing: "0.05em",
                      marginBottom: 6,
                      paddingLeft: 4
                    },
                    children: "Available fonts"
                  }
                ),
                m.length > 0 ? m.map((z) => b(z.key, z)) : /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      padding: "12px",
                      fontSize: 12,
                      color: "#94a3b8"
                    },
                    children: l ? "No fonts match your search" : "All fonts are in use"
                  }
                )
              ] })
            ] })
          ]
        }
      ),
      (((C = c.current) == null ? void 0 : C.ownerDocument) ?? document).body
    )
  ] });
}
function as({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round",
    fill: "none"
  };
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "sharp" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", ...o }),
    t === "round" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "4", ...o })
  ] });
}
const Pu = [
  { label: "S", size: 14 },
  { label: "M", size: 20 },
  { label: "L", size: 28 },
  { label: "XL", size: 36 }
], Ru = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
];
function Au({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
function ao(t, e) {
  if (t.length < 2) return !1;
  const o = e(t[0]);
  return !t.every((r) => e(r) === o);
}
function Lu({ engine: t, node: e, fontsInScene: o }) {
  const r = Ut(), n = Pe(t, e), s = Er(Or) ?? [e], { data: i } = e, l = i.fill ?? null, a = i.fillStyle ?? "hachure", c = i.strokeStyle ?? "solid", h = ao(s, (b) => b.data.stroke), f = ao(s, (b) => b.data.fill ?? null), d = ao(s, (b) => b.data.fillStyle ?? "hachure"), p = ao(s, (b) => b.data.strokeStyle ?? "solid"), y = ao(s, (b) => b.data.strokeWidth), m = ao(s, (b) => b.data.roughness), g = ao(s, (b) => b.data.opacity ?? 1);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Shape" }),
      Ru.map((b) => /* @__PURE__ */ u(
        "button",
        {
          title: b.label,
          onClick: () => n({ shape: b.key }),
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: i.shape === b.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(Au, { name: b.key })
        },
        b.key
      ))
    ] }),
    (i.shape === "rect" || i.shape === "diamond") && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((b) => /* @__PURE__ */ u(
        "button",
        {
          title: b.label,
          onClick: () => n({ edgeStyle: b.key === "sharp" ? void 0 : b.key }),
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (i.edgeStyle ?? "sharp") === b.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(as, { name: b.key })
        },
        b.key
      ))
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Label" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: i.label ?? "",
          placeholder: "Add label...",
          onChange: (b) => n({ label: b.target.value || void 0 }),
          style: {
            flex: 1,
            fontSize: 12,
            padding: "4px 6px",
            background: r.controlBg,
            color: r.text,
            border: `1px solid ${r.border}`,
            borderRadius: r.controlBorderRadius,
            outline: "none"
          }
        }
      )
    ] }),
    i.label && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Font" }),
      /* @__PURE__ */ u(
        Xr,
        {
          value: i.labelFontFamily ?? "Excalifont",
          onChange: (b) => n({ labelFontFamily: b === "Excalifont" ? void 0 : b }),
          fontsInScene: o
        }
      )
    ] }),
    i.label && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Size" }),
      Pu.map((b) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => n({ labelFontSize: b.size === 14 ? void 0 : b.size }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: (i.labelFontSize ?? 14) === b.size ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 10,
            fontWeight: 600,
            borderRadius: r.controlBorderRadius
          },
          children: b.label
        },
        b.size
      ))
    ] }),
    i.label && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Align" }),
      ns.map((b) => /* @__PURE__ */ u(
        "button",
        {
          title: b.key,
          onClick: () => n({ labelAlign: b.key === "center" ? void 0 : b.key }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: (i.labelAlign ?? "center") === b.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 12,
            borderRadius: r.controlBorderRadius
          },
          children: b.label
        },
        b.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Stroke",
        palettes: we,
        value: h ? void 0 : i.stroke,
        mixed: h,
        onChange: (b) => n({ stroke: b })
      }
    ),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Fill",
        palettes: ss,
        value: f ? void 0 : l,
        mixed: f,
        onChange: (b) => n({ fill: b ?? void 0 }),
        allowNull: !0
      }
    ),
    l && !f && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Fill pattern" }),
      os.map((b) => /* @__PURE__ */ u(
        "button",
        {
          title: b.label,
          onClick: () => n({ fillStyle: b.key }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: !d && a === b.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(is, { style: b.key })
        },
        b.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Xo,
      {
        label: "Stroke style",
        value: c,
        mixed: p,
        onChange: (b) => n({ strokeStyle: b })
      }
    ),
    /* @__PURE__ */ u(
      Yo,
      {
        label: "Stroke width",
        value: i.strokeWidth,
        mixed: y,
        onChange: (b) => n({ strokeWidth: b })
      }
    ),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Roughness" }),
      rs.map((b) => /* @__PURE__ */ u(
        "button",
        {
          title: b.label,
          onClick: () => n({ roughness: b.value }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 8px",
            background: !m && i.roughness === b.value ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: b.label
        },
        b.value
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: i.opacity ?? 1,
        mixed: g,
        onChange: (b) => n({ opacity: b })
      }
    )
  ] });
}
function To(t, e) {
  if (t.length < 2) return !1;
  const o = e(t[0]);
  return !t.every((r) => e(r) === o);
}
function Du({ engine: t, node: e }) {
  const o = Ut(), r = Pe(t, e), n = Er(Or) ?? [e], { data: s } = e, i = s.fill ?? null, l = s.fillStyle ?? "hachure", a = s.strokeStyle ?? "solid", c = To(n, (m) => m.data.color), h = To(n, (m) => m.data.fill ?? null), f = To(n, (m) => m.data.fillStyle ?? "hachure"), d = To(n, (m) => m.data.strokeStyle ?? "solid"), p = To(n, (m) => m.data.strokeWidth), y = To(n, (m) => m.data.opacity ?? 1);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      ye,
      {
        label: "Stroke",
        palettes: we,
        value: c ? void 0 : s.color,
        mixed: c,
        onChange: (m) => r({ color: m })
      }
    ),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Fill",
        palettes: ss,
        value: h ? void 0 : i,
        mixed: h,
        onChange: (m) => r({ fill: m ?? void 0 }),
        allowNull: !0
      }
    ),
    i && !h && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Fill pattern" }),
      os.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => r({ fillStyle: m.key }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: !f && l === m.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 9,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(is, { style: m.key })
        },
        m.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Xo,
      {
        label: "Stroke style",
        value: a,
        mixed: d,
        onChange: (m) => r({ strokeStyle: m })
      }
    ),
    /* @__PURE__ */ u(
      Yo,
      {
        label: "Stroke width",
        value: s.strokeWidth,
        mixed: p,
        onChange: (m) => r({ strokeWidth: m })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: s.opacity ?? 1,
        mixed: y,
        onChange: (m) => r({ opacity: m })
      }
    )
  ] });
}
function Eu({ engine: t, node: e, fontsInScene: o }) {
  const r = Ut(), n = Pe(t, e), { data: s } = e;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Font" }),
      /* @__PURE__ */ u(
        Xr,
        {
          value: s.fontFamily,
          onChange: (i) => n({ fontFamily: i }),
          fontsInScene: o
        }
      )
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Size" }),
      ka.map((i) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => n({ fontSize: i }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: s.fontSize === i ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 10,
            borderRadius: r.controlBorderRadius
          },
          children: i
        },
        i
      ))
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Align" }),
      ns.map((i) => /* @__PURE__ */ u(
        "button",
        {
          title: i.key,
          onClick: () => n({ align: i.key }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: s.align === i.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 12,
            borderRadius: r.controlBorderRadius
          },
          children: i.label
        },
        i.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Color",
        palettes: we,
        value: s.color,
        onChange: (i) => n({ color: i })
      }
    ),
    /* @__PURE__ */ u(
      ar,
      {
        borderColor: s.borderColor ?? null,
        borderStyle: s.borderStyle,
        borderWidth: s.borderWidth,
        onChange: (i, l) => n({ [i]: l })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: s.opacity ?? 1,
        onChange: (i) => n({ opacity: i })
      }
    )
  ] });
}
function Wu({ engine: t, node: e }) {
  const o = Ut(), r = Pe(t, e), { data: n } = e;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      ye,
      {
        label: "Color",
        palettes: we,
        value: n.color,
        onChange: (s) => r({ color: s })
      }
    ),
    /* @__PURE__ */ u(
      Xo,
      {
        label: "Style",
        value: n.style,
        onChange: (s) => r({ style: s })
      }
    ),
    /* @__PURE__ */ u(
      Yo,
      {
        label: "Width",
        value: n.strokeWidth,
        onChange: (s) => r({ strokeWidth: s })
      }
    ),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Roughness" }),
      rs.map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => r({ roughness: s.value }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 8px",
            background: (n.roughness ?? 0) === s.value ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 9,
            borderRadius: o.controlBorderRadius
          },
          children: s.label
        },
        s.value
      ))
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Head" }),
      ["none", "arrow", "filled", "dot"].map((s) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r({ arrowHead: s }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 6px",
            background: (n.arrowHead ?? "none") === s ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 11,
            borderRadius: o.controlBorderRadius
          },
          children: s === "none" ? "None" : s === "arrow" ? "▷" : s === "filled" ? "▶" : "●"
        },
        s
      ))
    ] }),
    (n.arrowHead ?? "none") !== "none" && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Head size" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "range",
          min: 4,
          max: 40,
          step: 1,
          value: n.arrowHeadSize ?? Math.max(8, n.strokeWidth * 3),
          onChange: (s) => r({ arrowHeadSize: Number(s.target.value) }),
          style: { flex: 1, accentColor: o.accentColor }
        }
      ),
      /* @__PURE__ */ u("span", { style: { color: o.textMuted, fontSize: 11, minWidth: 24, textAlign: "right" }, children: n.arrowHeadSize ?? Math.max(8, n.strokeWidth * 3) })
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Tail" }),
      ["none", "arrow", "filled", "dot"].map((s) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r({ arrowTail: s }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 6px",
            background: (n.arrowTail ?? "none") === s ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 11,
            borderRadius: o.controlBorderRadius
          },
          children: s === "none" ? "None" : s === "arrow" ? "◁" : s === "filled" ? "◀" : "●"
        },
        s
      ))
    ] }),
    (n.arrowTail ?? "none") !== "none" && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Tail size" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "range",
          min: 4,
          max: 40,
          step: 1,
          value: n.arrowTailSize ?? Math.max(8, n.strokeWidth * 3),
          onChange: (s) => r({ arrowTailSize: Number(s.target.value) }),
          style: { flex: 1, accentColor: o.accentColor }
        }
      ),
      /* @__PURE__ */ u("span", { style: { color: o.textMuted, fontSize: 11, minWidth: 24, textAlign: "right" }, children: n.arrowTailSize ?? Math.max(8, n.strokeWidth * 3) })
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Label" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: n.label ?? "",
          onChange: (s) => r({ label: s.target.value || void 0 }),
          placeholder: "Edge label...",
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 11,
            outline: "none"
          }
        }
      )
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Path" }),
      [
        { key: "bezier", label: "Bezier" },
        { key: "straight", label: "Straight" },
        { key: "smoothstep", label: "Smooth" },
        { key: "step", label: "Step" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => r({ edgeType: s.key }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 6px",
            background: (n.edgeType ?? "bezier") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 9,
            borderRadius: o.controlBorderRadius
          },
          children: s.label
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Animate" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r({ animated: !n.animated }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 12px",
            background: n.animated ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 11,
            borderRadius: o.controlBorderRadius
          },
          children: n.animated ? "On" : "Off"
        }
      )
    ] }),
    n.animated && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Direction" }),
      ["forward", "reverse", "both"].map((s) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r({ animatedDirection: s }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 6px",
            background: (n.animatedDirection ?? "forward") === s ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: s === "forward" ? "→" : s === "reverse" ? "←" : "⇆"
        },
        s
      ))
    ] })
  ] });
}
function Fu({ engine: t, node: e }) {
  const o = Ut(), [r, n] = tt("idle"), s = Pe(t, e), { data: i } = e, l = !!i.crop;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      ar,
      {
        borderColor: i.borderColor ?? null,
        borderStyle: i.borderStyle,
        borderWidth: i.borderWidth,
        onChange: (a, c) => s({ [a]: c })
      }
    ),
    /* @__PURE__ */ v("div", { style: { ...Et, marginTop: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Crop" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.requestImageCrop(e.id),
          style: {
            ...Zt,
            height: 28,
            padding: "0 10px",
            background: o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: "Crop"
        }
      ),
      l && /* @__PURE__ */ u(
        "button",
        {
          onClick: () => s({ crop: void 0 }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 10px",
            background: o.controlBg,
            color: o.textMuted,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: "Reset"
        }
      )
    ] }),
    /* @__PURE__ */ v("div", { style: { ...Et, marginTop: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Background" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: async () => {
            if (r !== "loading") {
              n("loading");
              try {
                const { removeBackground: a } = await import("@imgly/background-removal"), h = await (await fetch(i.src)).blob(), f = await a(h), d = new FileReader(), p = await new Promise((y, m) => {
                  d.onload = () => y(d.result), d.onerror = m, d.readAsDataURL(f);
                });
                s({ src: p }), n("idle");
              } catch (a) {
                console.error("Background removal failed:", a), n("error"), setTimeout(() => n("idle"), 3e3);
              }
            }
          },
          disabled: r === "loading",
          style: {
            ...Zt,
            height: 28,
            padding: "0 10px",
            background: r === "error" ? o.error : o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius,
            gap: 4,
            opacity: r === "loading" ? 0.6 : 1
          },
          children: r === "loading" ? "Removing..." : r === "error" ? "Failed" : "Remove BG"
        }
      )
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: i.opacity ?? 1,
        onChange: (a) => s({ opacity: a })
      }
    )
  ] });
}
function Bu({ engine: t, node: e }) {
  const o = Ut(), r = Pe(t, e), { data: n } = e;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      ar,
      {
        borderColor: n.borderColor ?? null,
        borderStyle: n.borderStyle,
        borderWidth: n.borderWidth,
        onChange: (s, i) => r({ [s]: i })
      }
    ),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => r({ edgeStyle: s.key === "sharp" ? void 0 : s.key }),
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (n.edgeStyle ?? "sharp") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(as, { name: s.key })
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: n.opacity ?? 1,
        onChange: (s) => r({ opacity: s })
      }
    )
  ] });
}
const _o = {
  pan: 400,
  fade: 500,
  dissolve: 400,
  zoom: 600,
  fold: 700,
  cube: 1200,
  none: 0
}, Nu = Hh();
function Hu({
  value: t,
  onChange: e,
  theme: o
}) {
  const [r, n] = tt(String(t));
  mt(() => n(String(t)), [t]);
  const s = () => {
    const i = parseInt(r, 10);
    !isNaN(i) && i >= 100 && i <= 5e3 ? e(i) : n(String(t));
  };
  return /* @__PURE__ */ v("div", { style: Et, children: [
    /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Duration" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "number",
        min: 100,
        max: 5e3,
        step: 50,
        value: r,
        onChange: (i) => n(i.target.value),
        onBlur: s,
        onKeyDown: (i) => {
          i.key === "Enter" && s();
        },
        style: {
          width: 64,
          background: o.controlBg,
          color: o.text,
          border: "none",
          borderRadius: o.controlBorderRadius,
          padding: "4px 6px",
          fontSize: 11,
          outline: "none"
        }
      }
    ),
    /* @__PURE__ */ u("span", { style: { fontSize: 10, color: o.textMuted }, children: "ms" })
  ] });
}
function Ou({ engine: t, node: e }) {
  const o = Ut(), r = Pe(t, e), { data: n } = e, s = it(
    (l) => {
      var d;
      if (!l) {
        r({ devicePreset: void 0 });
        return;
      }
      const a = An(l);
      if (!a) return;
      const c = wa(a), h = Math.round(e.w / c), f = { devicePreset: l };
      (!n.label || ((d = An(n.devicePreset ?? "")) == null ? void 0 : d.label) === n.label) && (f.label = a.label), r(f), t.updateNodeWithHistory(e.id, { h });
    },
    [t, e, n.label, n.devicePreset, r]
  ), i = Kt(() => {
    const l = t.getAllNodes().filter((d) => d.type === "frame"), a = l.length, c = /* @__PURE__ */ new Set();
    for (const d of l)
      d.id !== e.id && d.data.slideOrder != null && c.add(d.data.slideOrder);
    const h = [];
    for (let d = 1; d <= a; d++)
      c.has(d) || h.push(d);
    const f = e.data.slideOrder;
    return f != null && !h.includes(f) && (h.push(f), h.sort((d, p) => d - p)), h;
  }, [t, e]);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Label" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: n.label ?? "",
          onChange: (l) => r({ label: l.target.value || void 0 }),
          placeholder: "Frame label...",
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 11,
            outline: "none"
          }
        }
      )
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Device" }),
      /* @__PURE__ */ v(
        "select",
        {
          value: n.devicePreset ?? "",
          onChange: (l) => s(l.target.value),
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "", children: "Freeform" }),
            Nu.map((l) => /* @__PURE__ */ u("optgroup", { label: l.label, children: l.presets.map((a) => /* @__PURE__ */ v("option", { value: a.key, children: [
              a.label,
              " (",
              a.w,
              "×",
              a.h,
              ")"
            ] }, a.key)) }, l.label))
          ]
        }
      )
    ] }),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Background",
        palettes: we,
        value: (() => {
          const l = n.backgroundColor;
          if (!l) return null;
          for (const a of we) {
            const c = a.colors.find((h) => l === `${h}15`);
            if (c) return c;
          }
          return l.length === 9 && l.endsWith("15") ? l.slice(0, 7) : null;
        })(),
        onChange: (l) => r({ backgroundColor: l ? `${l}15` : void 0 }),
        allowNull: !0
      }
    ),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Border",
        palettes: we,
        value: n.borderColor,
        onChange: (l) => r({ borderColor: l })
      }
    ),
    /* @__PURE__ */ u(
      Xo,
      {
        label: "Style",
        value: n.borderStyle ?? "dashed",
        onChange: (l) => r({ borderStyle: l })
      }
    ),
    /* @__PURE__ */ u(
      Yo,
      {
        label: "Width",
        value: n.borderWidth ?? 1,
        onChange: (l) => r({ borderWidth: l })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: n.opacity ?? 1,
        onChange: (l) => r({ opacity: l })
      }
    ),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Slide #" }),
      /* @__PURE__ */ v(
        "select",
        {
          value: n.slideOrder ?? "",
          onChange: (l) => {
            const a = l.target.value;
            r({ slideOrder: a ? parseInt(a, 10) : void 0 });
          },
          style: {
            width: 72,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "", children: "Auto" }),
            i.map((l) => /* @__PURE__ */ u("option", { value: l, children: l }, l))
          ]
        }
      )
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Transition" }),
      /* @__PURE__ */ v(
        "select",
        {
          value: n.transition ?? "pan",
          onChange: (l) => {
            const a = l.target.value;
            r({ transition: a === "pan" ? void 0 : a, transitionDuration: void 0 });
          },
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "pan", children: "Pan" }),
            /* @__PURE__ */ u("option", { value: "fade", children: "Fade to Black" }),
            /* @__PURE__ */ u("option", { value: "dissolve", children: "Dissolve" }),
            /* @__PURE__ */ u("option", { value: "zoom", children: "Zoom" }),
            /* @__PURE__ */ u("option", { value: "fold", children: "Fold" }),
            /* @__PURE__ */ u("option", { value: "cube", children: "Cube" }),
            /* @__PURE__ */ u("option", { value: "none", children: "None (instant)" })
          ]
        }
      )
    ] }),
    (n.transition ?? "pan") !== "none" && /* @__PURE__ */ u(
      Hu,
      {
        value: n.transitionDuration ?? _o[n.transition ?? "pan"],
        onChange: (l) => r({ transitionDuration: l === _o[n.transition ?? "pan"] ? void 0 : l }),
        theme: o
      }
    )
  ] });
}
function Xu({ engine: t, node: e }) {
  const o = Ut(), r = Pe(t, e), { data: n } = e;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      ye,
      {
        label: "Color",
        palettes: Uh,
        value: n.color,
        onChange: (s) => {
          s && r({ color: s });
        }
      }
    ),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Size" }),
      [12, 14, 16, 20, 24].map((s) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r({ fontSize: s }),
          style: {
            ...Zt,
            width: 32,
            height: 24,
            background: (n.fontSize ?? 16) === s ? o.controlBgActive : o.controlBg,
            borderRadius: o.controlBorderRadius,
            fontSize: 10,
            color: o.text
          },
          children: s
        },
        s
      ))
    ] }),
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => r({ edgeStyle: s.key === "sharp" ? void 0 : s.key }),
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (n.edgeStyle ?? "sharp") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(as, { name: s.key })
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: n.opacity ?? 1,
        onChange: (s) => r({ opacity: s })
      }
    )
  ] });
}
function Yu({ engine: t, node: e }) {
  const o = Ut(), r = Pe(t, e), { data: n } = e;
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "URL" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: n.url,
          readOnly: !0,
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.textMuted,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 10,
            outline: "none",
            cursor: "default"
          },
          onClick: (s) => s.target.select()
        }
      )
    ] }),
    /* @__PURE__ */ u(
      ar,
      {
        borderColor: n.borderColor ?? null,
        borderStyle: n.borderStyle,
        borderWidth: n.borderWidth,
        onChange: (s, i) => r({ [s]: i })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: n.opacity ?? 1,
        onChange: (s) => r({ opacity: s })
      }
    )
  ] });
}
function Gu({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
const ju = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
];
function Vu({ engine: t, mode: e, fontsInScene: o }) {
  const r = Ut(), [, n] = tt(0), s = it(() => n((y) => y + 1), []), i = t.activeTool;
  if (e === "text") {
    const y = i.fontFamily ?? Ke, m = i.fontSize ?? 20, g = i.textAlign ?? "left", b = i.color;
    return /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ v("div", { style: Et, children: [
        /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Xr,
          {
            value: y,
            onChange: (x) => {
              i.fontFamily = x, s();
            },
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ v("div", { style: Et, children: [
        /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Size" }),
        ka.map((x) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => {
              i.fontSize = x, s();
            },
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: m === x ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 10,
              borderRadius: r.controlBorderRadius
            },
            children: x
          },
          x
        ))
      ] }),
      /* @__PURE__ */ v("div", { style: Et, children: [
        /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Align" }),
        ns.map((x) => /* @__PURE__ */ u(
          "button",
          {
            title: x.key,
            onClick: () => {
              i.textAlign = x.key, s();
            },
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: g === x.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 12,
              borderRadius: r.controlBorderRadius
            },
            children: x.label
          },
          x.key
        ))
      ] }),
      /* @__PURE__ */ u(
        ye,
        {
          label: "Color",
          palettes: we,
          value: b,
          onChange: (x) => {
            i.color = x, s();
          }
        }
      ),
      /* @__PURE__ */ u(
        ke,
        {
          value: i.opacity ?? 1,
          onChange: (x) => {
            i.opacity = x, s();
          }
        }
      )
    ] });
  }
  const l = e === "shape", a = i.color, c = i.fillColor ?? null, h = i.fillStyle ?? "hachure", f = i.strokeStyle ?? "solid", d = i.width, p = i.roughness ?? 1;
  return /* @__PURE__ */ v(ut, { children: [
    l && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Shape" }),
      ju.map((y) => /* @__PURE__ */ u(
        "button",
        {
          title: y.label,
          onClick: () => {
            i.shapeType = y.key, s();
          },
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (i.shapeType ?? "rect") === y.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(Gu, { name: y.key })
        },
        y.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Stroke",
        palettes: we,
        value: a,
        onChange: (y) => {
          i.color = y, s();
        }
      }
    ),
    /* @__PURE__ */ u(
      ye,
      {
        label: "Fill",
        palettes: ss,
        value: c,
        onChange: (y) => {
          i.fillColor = y ?? void 0, s();
        },
        allowNull: !0
      }
    ),
    c && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Fill pattern" }),
      os.map((y) => /* @__PURE__ */ u(
        "button",
        {
          title: y.label,
          onClick: () => {
            i.fillStyle = y.key, s();
          },
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: h === y.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(is, { style: y.key })
        },
        y.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Xo,
      {
        label: "Stroke style",
        value: f,
        onChange: (y) => {
          i.strokeStyle = y, s();
        }
      }
    ),
    /* @__PURE__ */ u(
      Yo,
      {
        label: "Stroke width",
        value: d,
        onChange: (y) => {
          i.width = y, s();
        }
      }
    ),
    l && /* @__PURE__ */ v("div", { style: Et, children: [
      /* @__PURE__ */ u("span", { style: { ...Wt, color: r.textMuted }, children: "Roughness" }),
      rs.map((y) => /* @__PURE__ */ u(
        "button",
        {
          title: y.label,
          onClick: () => {
            i.roughness = y.value, s();
          },
          style: {
            ...Zt,
            height: 28,
            padding: "0 8px",
            background: p === y.value ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: y.label
        },
        y.value
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: i.opacity ?? 1,
        onChange: (y) => {
          i.opacity = y, s();
        }
      }
    )
  ] });
}
function Ku({ engine: t, node: e, PanelComponent: o }) {
  const r = Pe(t, e);
  return /* @__PURE__ */ u(o, { node: e, data: e.data, engine: t, updateData: r });
}
const Dn = {
  shape: "Shape",
  draw: "Drawing",
  text: "Text",
  edge: "Edge",
  image: "Image",
  content: "Content",
  frame: "Frame",
  sticky: "Sticky Note",
  youtube: "YouTube"
}, qu = /* @__PURE__ */ new Set(["shape", "draw", "text", "image", "content", "frame", "sticky", "youtube"]), Zu = /* @__PURE__ */ new Set(["text", "image", "content", "frame", "youtube"]);
function Uu(t) {
  const e = /* @__PURE__ */ new Set(), o = [];
  for (const r of t.getAllNodes()) {
    let n;
    r.type === "text" ? n = r.data.fontFamily : r.type === "shape" && (n = r.data.labelFontFamily), n && !e.has(n) && (e.add(n), o.push(n));
  }
  return o;
}
function Qu({ label: t }) {
  const e = Ut();
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        fontSize: 10,
        fontWeight: 600,
        color: e.textSecondary,
        padding: "2px 0 6px",
        borderBottom: `1px solid ${e.border}`,
        marginBottom: 2
      },
      children: t
    }
  );
}
function va({
  engine: t,
  node: e,
  registry: o,
  fontsInScene: r
}) {
  switch (e.type) {
    case "shape":
      return /* @__PURE__ */ u(Lu, { engine: t, node: e, fontsInScene: r });
    case "draw":
      return /* @__PURE__ */ u(Du, { engine: t, node: e });
    case "text":
      return /* @__PURE__ */ u(Eu, { engine: t, node: e, fontsInScene: r });
    case "edge":
      return /* @__PURE__ */ u(Wu, { engine: t, node: e });
    case "image":
      return /* @__PURE__ */ u(Fu, { engine: t, node: e });
    case "content":
      return /* @__PURE__ */ u(Bu, { engine: t, node: e });
    case "frame":
      return /* @__PURE__ */ u(Ou, { engine: t, node: e });
    case "sticky":
      return /* @__PURE__ */ u(Xu, { engine: t, node: e });
    case "youtube":
      return /* @__PURE__ */ u(Yu, { engine: t, node: e });
    default: {
      const n = o == null ? void 0 : o.get(e.type);
      return n != null && n.propertiesPanel ? /* @__PURE__ */ u(Ku, { engine: t, node: e, PanelComponent: n.propertiesPanel }) : null;
    }
  }
}
function ii({
  engine: t,
  nodes: e
}) {
  const o = Ut(), r = Math.round(e[0].rotation ?? 0), s = e.every(
    (c) => Math.round(c.rotation ?? 0) === r
  ) ? r : null, [i, l] = tt(null), a = it(
    (c) => {
      l(null);
      const h = parseFloat(c);
      if (isNaN(h)) return;
      const f = Math.max(-360, Math.min(360, h)), d = e.map((p) => ({
        id: p.id,
        patch: { rotation: f }
      }));
      t.batchUpdateWithHistory(d);
    },
    [t, e]
  );
  return /* @__PURE__ */ v("div", { style: Et, children: [
    /* @__PURE__ */ u("span", { style: { ...Wt, color: o.textMuted }, children: "Rotation" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "number",
        min: -360,
        max: 360,
        value: i ?? (s !== null ? String(s) : ""),
        placeholder: s === null ? "Mixed" : void 0,
        onChange: (c) => l(c.target.value),
        onBlur: (c) => a(c.target.value),
        onKeyDown: (c) => {
          c.key === "Enter" && a(c.target.value), c.key === "Escape" && l(null);
        },
        style: {
          width: 52,
          height: 24,
          border: `1px solid ${o.border}`,
          borderRadius: o.controlBorderRadius,
          background: o.controlBg,
          color: o.text,
          fontSize: 10,
          textAlign: "center",
          outline: "none",
          padding: "0 2px"
        }
      }
    ),
    /* @__PURE__ */ u("span", { style: { fontSize: 10, color: o.textMuted }, children: "°" })
  ] });
}
function Ju({
  engine: t,
  nodes: e,
  commonProps: o
}) {
  const r = it(
    (n, s) => {
      const i = n === "opacity" ? qu : Zu, l = e.filter((a) => i.has(a.type)).map((a) => ({
        id: a.id,
        patch: {
          data: { ...a.data, [n]: s }
        }
      }));
      t.batchUpdateWithHistory(l);
    },
    [t, e]
  );
  return /* @__PURE__ */ v(ut, { children: [
    o.opacity !== void 0 && /* @__PURE__ */ u(
      ke,
      {
        value: o.opacity === "mixed" ? void 0 : o.opacity,
        mixed: o.opacity === "mixed",
        onChange: (n) => r("opacity", n)
      }
    ),
    o.borderColor !== void 0 && /* @__PURE__ */ u(
      ar,
      {
        borderColor: o.borderColor === "mixed" ? void 0 : o.borderColor,
        borderStyle: o.borderStyle === "mixed" ? void 0 : o.borderStyle,
        borderWidth: o.borderWidth === "mixed" ? void 0 : o.borderWidth,
        mixed: {
          color: o.borderColor === "mixed",
          style: o.borderStyle === "mixed",
          width: o.borderWidth === "mixed"
        },
        onChange: (n, s) => r(n, s)
      }
    )
  ] });
}
function $u({
  engine: t,
  group: e,
  registry: o,
  fontsInScene: r
}) {
  const n = Dn[e.type] ?? e.type, s = e.nodes.length, i = e.nodes[0], l = Ut();
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ v("div", { style: { ...Qh, color: l.textFaint, borderTop: `1px solid ${l.border}` }, children: [
      n,
      " (",
      s,
      ")"
    ] }),
    /* @__PURE__ */ u(Or.Provider, { value: e.nodes, children: /* @__PURE__ */ u(
      va,
      {
        engine: t,
        node: i,
        registry: o,
        fontsInScene: r
      }
    ) })
  ] });
}
function _u(t) {
  switch (t.kind) {
    case "none":
      return "No selection";
    case "tool":
      return `${t.mode.charAt(0).toUpperCase() + t.mode.slice(1)} tool`;
    case "single":
      return Dn[t.node.type] ?? t.node.type;
    case "multi":
      return t.typeGroups.map(
        (o) => `${o.nodes.length} ${(Dn[o.type] ?? o.type).toLowerCase()}${o.nodes.length > 1 ? "s" : ""}`
      ).join(", ");
  }
}
function ai({
  engine: t,
  registry: e,
  target: o,
  commonProps: r
}) {
  const n = Kt(() => Uu(t), [t, o]), s = _u(o);
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(Qu, { label: s }),
    o.kind === "tool" && /* @__PURE__ */ u(Vu, { engine: t, mode: o.mode, fontsInScene: n }),
    o.kind === "single" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u(
        va,
        {
          engine: t,
          node: o.node,
          registry: e,
          fontsInScene: n
        }
      ),
      /* @__PURE__ */ u(ii, { engine: t, nodes: [o.node] })
    ] }),
    o.kind === "multi" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u(Ju, { engine: t, nodes: o.nodes, commonProps: r }),
      /* @__PURE__ */ u(ii, { engine: t, nodes: o.nodes }),
      o.typeGroups.map((i) => /* @__PURE__ */ u(
        $u,
        {
          engine: t,
          group: i,
          registry: e,
          fontsInScene: n
        },
        i.type
      ))
    ] })
  ] });
}
function tf({ engine: t, registry: e }) {
  const o = Ut(), { target: r, commonProps: n } = zu(t), s = r.kind !== "none", [i, l] = tt(!1), a = at(null), [c, h] = tt(null), f = at(null), [d, p] = tt(!1), y = it(() => {
    var D, W;
    const S = (D = a.current) == null ? void 0 : D.offsetParent;
    if (S) return { width: S.clientWidth, height: S.clientHeight };
    const F = ((W = a.current) == null ? void 0 : W.ownerDocument.defaultView) ?? window;
    return { width: F.innerWidth, height: F.innerHeight };
  }, []), m = it(() => {
    const { width: S } = y();
    return { x: S - Jo - 16, y: 12 };
  }, [y]), g = c ?? m(), b = at(!1);
  bi(() => {
    if (!b.current && a.current && !c) {
      b.current = !0;
      const S = a.current.offsetParent;
      S && h({ x: S.clientWidth - Jo - 16, y: 12 });
    }
  }), mt(() => {
    var D, W;
    const S = ((D = a.current) == null ? void 0 : D.offsetParent) ?? ((W = a.current) == null ? void 0 : W.ownerDocument.body);
    if (!S) return;
    const F = new ResizeObserver((K) => {
      var dt;
      const Z = ((dt = K[0]) == null ? void 0 : dt.contentRect.width) ?? S.clientWidth;
      l(Z < 600);
    });
    return F.observe(S), l(S.clientWidth < 600), () => F.disconnect();
  }, []);
  const x = it(
    (S) => {
      S.stopPropagation(), p(!0);
      const F = c ? c.x : m().x, D = c ? c.y : m().y;
      f.current = {
        startX: S.clientX,
        startY: S.clientY,
        startLeft: F,
        startTop: D
      }, S.currentTarget.setPointerCapture(S.pointerId);
    },
    [c, m]
  ), C = it(
    (S) => {
      if (!f.current) return;
      S.stopPropagation();
      const F = S.clientX - f.current.startX, D = S.clientY - f.current.startY, { width: W, height: K } = y(), Z = Math.max(
        ir,
        Math.min(W - Jo - 8, f.current.startLeft + F)
      ), dt = Math.max(
        8,
        Math.min(K - 100, f.current.startTop + D)
      );
      h({ x: Z, y: dt });
    },
    [y]
  ), z = it(() => {
    f.current = null, p(!1);
  }, []);
  return s ? i ? /* @__PURE__ */ v(
    "div",
    {
      ref: a,
      "data-sb-props-panel": !0,
      onPointerDown: (S) => S.stopPropagation(),
      style: {
        position: "fixed",
        bottom: 0,
        left: 0,
        right: 0,
        height: "45vh",
        minHeight: 200,
        background: o.panelBg,
        borderRadius: "12px 12px 0 0",
        boxShadow: "0 -4px 24px rgba(0,0,0,0.25)",
        zIndex: 200,
        display: "flex",
        flexDirection: "column",
        color: o.text,
        fontSize: 12
      },
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              height: 36,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              flexShrink: 0
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                style: {
                  width: 36,
                  height: 4,
                  borderRadius: 2,
                  background: o.border
                }
              }
            )
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              overflowY: "auto",
              padding: "0 16px 24px",
              flex: 1,
              touchAction: "pan-y"
            },
            children: /* @__PURE__ */ u(
              ai,
              {
                engine: t,
                registry: e,
                target: r,
                commonProps: n
              }
            )
          }
        )
      ]
    }
  ) : /* @__PURE__ */ v(
    "div",
    {
      ref: a,
      "data-sb-props-panel": !0,
      style: {
        position: "absolute",
        left: g.x,
        top: g.y,
        width: Jo,
        background: o.panelBg,
        borderRadius: o.panelBorderRadius,
        padding: "0 0 12px",
        display: "flex",
        flexDirection: "column",
        zIndex: 99,
        color: o.text,
        fontSize: 11,
        maxHeight: "calc(100% - 40px)",
        boxShadow: o.panelShadow
      },
      onPointerDown: (S) => S.stopPropagation(),
      onPointerMove: C,
      onPointerUp: z,
      onPointerCancel: z,
      children: [
        /* @__PURE__ */ v(
          "div",
          {
            onPointerDown: x,
            style: {
              cursor: d ? "grabbing" : "grab",
              padding: "8px 16px",
              userSelect: "none",
              touchAction: "none",
              display: "flex",
              alignItems: "center",
              gap: 6,
              borderBottom: `1px solid ${o.border}`,
              color: o.textMuted,
              fontSize: 10,
              flexShrink: 0
            },
            children: [
              /* @__PURE__ */ u("span", { style: { opacity: 0.6 }, children: "⋮⋮" }),
              /* @__PURE__ */ u("span", { children: "Drag to move" })
            ]
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              overflowY: "auto",
              overflowX: "hidden",
              padding: "8px 12px",
              display: "flex",
              flexDirection: "column",
              gap: 8,
              touchAction: "pan-y"
            },
            children: /* @__PURE__ */ u(
              ai,
              {
                engine: t,
                registry: e,
                target: r,
                commonProps: n
              }
            )
          }
        )
      ]
    }
  ) : null;
}
function ef({ engine: t, registry: e, gifApiBaseUrl: o }) {
  return /* @__PURE__ */ v(ut, { children: [
    /* @__PURE__ */ u(
      "div",
      {
        "data-sb-sidebar": !0,
        style: {
          position: "absolute",
          left: 0,
          top: 0,
          bottom: 0,
          width: ir,
          zIndex: 100
        },
        onPointerDown: (r) => r.stopPropagation(),
        children: /* @__PURE__ */ u(vu, { engine: t, gifApiBaseUrl: o })
      }
    ),
    /* @__PURE__ */ u(tf, { engine: t, registry: e })
  ] });
}
const tr = [0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 3, 4];
function of(t) {
  const e = t.viewport.zoom, o = tr.find((r) => r > e + 1e-3) ?? tr[tr.length - 1];
  t.viewport.zoom = o, t.pan(0, 0);
}
function rf(t) {
  const e = t.viewport.zoom, o = [...tr].reverse().find((r) => r < e - 1e-3) ?? tr[0];
  t.viewport.zoom = o, t.pan(0, 0);
}
const nf = {
  display: "flex",
  alignItems: "center",
  overflow: "hidden"
}, Ce = {
  border: "none",
  background: "transparent",
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, le = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function We({ name: t, size: e = 16 }) {
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "minus" && /* @__PURE__ */ u("path", { d: "M5 12h14", ...le }),
    t === "plus" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M12 5v14", ...le }),
      /* @__PURE__ */ u("path", { d: "M5 12h14", ...le })
    ] }),
    t === "undo" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...le, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...le, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...le, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...le, fill: "none" })
    ] }),
    t === "fit" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...le }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...le })
    ] }),
    t === "play" && /* @__PURE__ */ u("path", { d: "M6 4l14 8-14 8z", fill: "currentColor" }),
    t === "slides" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "6", width: "20", height: "12", rx: "1", ...le }),
      /* @__PURE__ */ u("path", { d: "M6 6V18M18 6V18", ...le }),
      /* @__PURE__ */ u("path", { d: "M2 9h4M2 12h4M2 15h4M18 9h4M18 12h4M18 15h4", ...le })
    ] }),
    t === "home" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M3 12l9-8 9 8", ...le, fill: "none" }),
      /* @__PURE__ */ u("path", { d: "M5 12v7a1 1 0 001 1h12a1 1 0 001-1v-7", ...le, fill: "none" })
    ] }),
    t === "bookmark" && /* @__PURE__ */ u("path", { d: "M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z", ...le, fill: "none" }),
    t === "bookmark-fill" && /* @__PURE__ */ u("path", { d: "M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", fill: "currentColor" })
  ] });
}
function sf({ engine: t, framesPanelOpen: e, onToggleFramesPanel: o }) {
  const r = Ut(), [n, s] = tt(t.viewport.zoom), [i, l] = tt(!1), [a, c] = tt(!1), [h, f] = tt(() => t.originView != null), [d, p] = tt(
    () => t.getAllNodes().filter((x) => x.type === "frame").length
  );
  mt(() => {
    const x = () => s(t.viewport.zoom), C = () => {
      l(t.canUndo()), c(t.canRedo());
    }, z = () => {
      p(t.getAllNodes().filter((S) => S.type === "frame").length), f(t.originView != null);
    };
    return t.on("viewport", x), t.on("history", C), t.on("change", z), t.on("node:create", z), t.on("node:delete", z), () => {
      t.off("viewport", x), t.off("history", C), t.off("change", z), t.off("node:create", z), t.off("node:delete", z);
    };
  }, [t]);
  const y = r.panelBg, m = `1px solid ${r.border}`, g = {
    ...nf,
    borderRadius: r.panelBorderRadius
  }, b = {
    width: 1,
    height: 20,
    background: r.separator,
    flexShrink: 0
  };
  return /* @__PURE__ */ v(
    "div",
    {
      "data-sb-bottombar": !0,
      style: {
        position: "absolute",
        bottom: 12,
        left: "50%",
        transform: "translateX(-50%)",
        display: "flex",
        gap: 8,
        zIndex: 9999,
        pointerEvents: "auto"
      },
      onPointerDown: (x) => x.stopPropagation(),
      children: [
        /* @__PURE__ */ v("div", { style: { ...g, background: y, border: m, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Zoom out",
              onClick: () => rf(t),
              style: { ...Ce, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(We, { name: "minus" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ v(
            "button",
            {
              title: "Reset zoom to 100%",
              onClick: () => {
                t.viewport.zoom = 1, t.pan(0, 0);
              },
              style: {
                ...Ce,
                minWidth: 48,
                height: 32,
                color: r.text,
                fontSize: 11,
                fontWeight: 600,
                fontFamily: "inherit",
                padding: "0 4px"
              },
              children: [
                Math.round(n * 100),
                "%"
              ]
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Zoom in",
              onClick: () => of(t),
              style: { ...Ce, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(We, { name: "plus" })
            }
          )
        ] }),
        /* @__PURE__ */ v("div", { style: { ...g, background: y, border: m, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Fit to content (Ctrl+0)",
              onClick: () => t.fitToContent(),
              style: { ...Ce, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(We, { name: "fit" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: h ? "Clear saved view" : "Save current view as origin",
              onClick: () => {
                h ? (t.clearOriginView(), f(!1)) : (t.setOriginView(), f(!0));
              },
              style: { ...Ce, width: 32, height: 32, color: h ? r.accentColor : r.textFaint },
              children: /* @__PURE__ */ u(We, { name: h ? "bookmark-fill" : "bookmark" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Go to saved view",
              onClick: () => {
                h && t.goToOriginView();
              },
              disabled: !h,
              style: { ...Ce, width: 32, height: 32, color: h ? r.text : r.textFaint },
              children: /* @__PURE__ */ u(We, { name: "home" })
            }
          )
        ] }),
        /* @__PURE__ */ v("div", { style: { ...g, overflow: "visible", background: y, border: m, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Present (frames as slides)",
              onClick: () => t.enterPresentation(),
              style: { ...Ce, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(We, { name: "play" })
            }
          ),
          o && /* @__PURE__ */ v(ut, { children: [
            /* @__PURE__ */ u("div", { style: b }),
            /* @__PURE__ */ v(
              "button",
              {
                title: "Toggle slides panel",
                onClick: o,
                style: {
                  ...Ce,
                  width: 32,
                  height: 32,
                  color: e ? r.text : r.textMuted,
                  position: "relative"
                },
                children: [
                  /* @__PURE__ */ u(We, { name: "slides" }),
                  d > 0 && /* @__PURE__ */ u(
                    "span",
                    {
                      style: {
                        position: "absolute",
                        top: -4,
                        right: -4,
                        minWidth: 14,
                        height: 14,
                        borderRadius: 7,
                        background: r.accentColor,
                        color: "#fff",
                        fontSize: 9,
                        fontWeight: 700,
                        lineHeight: "14px",
                        textAlign: "center",
                        padding: "0 3px",
                        pointerEvents: "none"
                      },
                      children: d
                    }
                  )
                ]
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ v("div", { style: { ...g, background: y, border: m, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Undo (Ctrl+Z)",
              onClick: () => t.undo(),
              disabled: !i,
              style: { ...Ce, width: 32, height: 32, color: i ? r.text : r.textFaint },
              children: /* @__PURE__ */ u(We, { name: "undo" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Redo (Ctrl+Shift+Z)",
              onClick: () => t.redo(),
              disabled: !a,
              style: { ...Ce, width: 32, height: 32, color: a ? r.text : r.textFaint },
              children: /* @__PURE__ */ u(We, { name: "redo" })
            }
          )
        ] })
      ]
    }
  );
}
const li = 240, ci = 6;
function fn(t) {
  const o = t.getAllNodes().filter((h) => h.type === "frame");
  if (o.length === 0) return [];
  const r = o.map((h) => ({
    id: h.id,
    x: h.x,
    y: h.y,
    slideOrder: h.data.slideOrder,
    label: h.data.label || "",
    borderColor: h.data.borderColor,
    transition: h.data.transition,
    transitionDuration: h.data.transitionDuration
  })), n = r.filter((h) => h.slideOrder != null).sort((h, f) => h.slideOrder - f.slideOrder), s = r.filter((h) => h.slideOrder == null), i = 100;
  s.sort((h, f) => h.y - f.y);
  const l = [];
  for (const h of s) {
    const f = l[l.length - 1];
    f && Math.abs(h.y - f[0].y) < i ? f.push(h) : l.push([h]);
  }
  const a = l.flatMap((h) => h.sort((f, d) => f.x - d.x));
  return [...n, ...a].map((h, f) => ({
    id: h.id,
    label: h.label || `Frame ${f + 1}`,
    order: f + 1,
    slideOrder: h.slideOrder,
    borderColor: h.borderColor,
    transition: h.transition,
    transitionDuration: h.transitionDuration
  }));
}
const af = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function lf() {
  return /* @__PURE__ */ u("svg", { width: 14, height: 14, viewBox: "0 0 24 24", fill: "none", children: /* @__PURE__ */ u("path", { d: "M18 6L6 18M6 6l12 12", ...af }) });
}
function cf(t, e, o) {
  const [r, n] = tt("");
  return mt(() => {
    let s = !1;
    return Fh(t, e).then((i) => {
      s || n(i);
    }), () => {
      s = !0;
    };
  }, [t, e, o]), r;
}
function df({ engine: t, frameId: e, tick: o }) {
  const r = cf(t, e, o);
  return r ? /* @__PURE__ */ u(
    "img",
    {
      src: r,
      alt: "",
      draggable: !1,
      style: {
        width: "100%",
        display: "block",
        borderRadius: "4px 4px 0 0",
        pointerEvents: "none"
      }
    }
  ) : /* @__PURE__ */ u(
    "div",
    {
      style: {
        width: "100%",
        aspectRatio: "16/10",
        background: "rgba(128,128,128,0.06)",
        borderRadius: "4px 4px 0 0"
      }
    }
  );
}
const di = [
  { key: "pan", label: "Pan" },
  { key: "fade", label: "Fade" },
  { key: "dissolve", label: "Dissolve" },
  { key: "zoom", label: "Zoom" },
  { key: "fold", label: "Fold" },
  { key: "cube", label: "Cube" },
  { key: "none", label: "Cut" }
];
function hi({ type: t, size: e = 12 }) {
  const o = { stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", fill: "none" };
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 16 16", fill: "none", children: [
    t === "pan" && /* @__PURE__ */ u("path", { d: "M3 8h10M10 5l3 3-3 3", ...o }),
    t === "fade" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "3", width: "5", height: "10", rx: "1", ...o, opacity: 0.4 }),
      /* @__PURE__ */ u("rect", { x: "9", y: "3", width: "5", height: "10", rx: "1", ...o })
    ] }),
    t === "dissolve" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "3", width: "5", height: "10", rx: "1", ...o, strokeDasharray: "2,1" }),
      /* @__PURE__ */ u("rect", { x: "9", y: "3", width: "5", height: "10", rx: "1", ...o })
    ] }),
    t === "zoom" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("circle", { cx: "8", cy: "8", r: "3", ...o }),
      /* @__PURE__ */ u("path", { d: "M5 3L3 1M11 3l2-2M5 13l-2 2M11 13l2 2", ...o })
    ] }),
    t === "fold" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M2 3v10l6-5z", ...o }),
      /* @__PURE__ */ u("path", { d: "M14 3v10l-6-5z", ...o })
    ] }),
    t === "cube" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M3 4h7v8H3z", ...o }),
      /* @__PURE__ */ u("path", { d: "M10 4l3-2v8l-3 2", ...o }),
      /* @__PURE__ */ u("path", { d: "M3 4l3-2h7l-3 2", ...o })
    ] }),
    t === "none" && /* @__PURE__ */ u("path", { d: "M4 4l8 8M12 4l-8 8", ...o })
  ] });
}
const hf = [200, 300, 400, 500, 600, 800, 1e3, 1500, 2e3];
function uf({
  value: t,
  durationMs: e,
  onChange: o,
  onDurationChange: r,
  theme: n
}) {
  var y;
  const [s, i] = tt(!1), [l, a] = tt(!1), c = at(null), h = at(null), f = t !== "none", d = e ?? _o[t];
  mt(() => {
    if (!s && !l) return;
    const m = (g) => {
      s && c.current && !c.current.contains(g.target) && i(!1), l && h.current && !h.current.contains(g.target) && a(!1);
    };
    return document.addEventListener("mousedown", m), () => document.removeEventListener("mousedown", m);
  }, [s, l]);
  const p = {
    border: `1px solid ${n.border}`,
    background: n.panelBg,
    borderRadius: 10,
    height: 20,
    padding: "0 6px",
    display: "flex",
    alignItems: "center",
    gap: 3,
    cursor: "pointer",
    color: n.textMuted,
    fontSize: 9,
    position: "relative",
    zIndex: 1
  };
  return /* @__PURE__ */ v(
    "div",
    {
      style: {
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        height: 22,
        flexShrink: 0,
        position: "relative",
        gap: 4,
        zIndex: s || l ? 50 : void 0
      },
      children: [
        /* @__PURE__ */ u("div", { style: { position: "absolute", left: 0, right: 0, top: "50%", height: 1, background: n.border } }),
        /* @__PURE__ */ v("div", { ref: c, style: { position: "relative", zIndex: 1 }, children: [
          /* @__PURE__ */ v("button", { onClick: () => {
            i((m) => !m), a(!1);
          }, style: p, children: [
            /* @__PURE__ */ u(hi, { type: t }),
            /* @__PURE__ */ u("span", { children: ((y = di.find((m) => m.key === t)) == null ? void 0 : y.label) ?? "Pan" }),
            /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: s ? "▲" : "▼" })
          ] }),
          s && /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                top: "100%",
                left: "50%",
                transform: "translateX(-50%)",
                marginTop: 2,
                background: n.panelBg,
                border: `1px solid ${n.border}`,
                borderRadius: 6,
                padding: 3,
                zIndex: 100,
                display: "flex",
                flexDirection: "column",
                gap: 1,
                minWidth: 100,
                boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
              },
              children: di.map((m) => /* @__PURE__ */ v(
                "button",
                {
                  onClick: () => {
                    o(m.key), i(!1);
                  },
                  style: {
                    border: "none",
                    background: m.key === t ? n.controlBgActive : "transparent",
                    color: n.text,
                    borderRadius: 4,
                    padding: "4px 8px",
                    fontSize: 10,
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    width: "100%"
                  },
                  children: [
                    /* @__PURE__ */ u(hi, { type: m.key }),
                    m.label
                  ]
                },
                m.key
              ))
            }
          )
        ] }),
        f && /* @__PURE__ */ v("div", { ref: h, style: { position: "relative", zIndex: 1 }, children: [
          /* @__PURE__ */ v("button", { onClick: () => {
            a((m) => !m), i(!1);
          }, style: p, children: [
            /* @__PURE__ */ v("span", { children: [
              d,
              "ms"
            ] }),
            /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: l ? "▲" : "▼" })
          ] }),
          l && /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                top: "100%",
                left: "50%",
                transform: "translateX(-50%)",
                marginTop: 2,
                background: n.panelBg,
                border: `1px solid ${n.border}`,
                borderRadius: 6,
                padding: 3,
                zIndex: 100,
                display: "flex",
                flexDirection: "column",
                gap: 1,
                minWidth: 64,
                boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
              },
              children: hf.map((m) => /* @__PURE__ */ v(
                "button",
                {
                  onClick: () => {
                    r(m === _o[t] ? void 0 : m), a(!1);
                  },
                  style: {
                    border: "none",
                    background: m === d ? n.controlBgActive : "transparent",
                    color: n.text,
                    borderRadius: 4,
                    padding: "4px 8px",
                    fontSize: 10,
                    cursor: "pointer",
                    textAlign: "center",
                    width: "100%"
                  },
                  children: [
                    m,
                    "ms",
                    m === _o[t] ? " •" : ""
                  ]
                },
                m
              ))
            }
          )
        ] })
      ]
    }
  );
}
function ff({ engine: t, open: e, onClose: o }) {
  const r = Ut(), [n, s] = tt(() => fn(t)), [i, l] = tt(() => new Set(t.selection)), [a, c] = tt(0), h = at(null), f = at(null), d = at(0), p = at(!1), y = at(n);
  y.current = n;
  const m = at(!1), g = at(!1), [b, x] = tt(null), [C, z] = tt(null), [S, F] = tt(0), D = at([]), W = at(null), K = it(() => {
    if (m.current) return;
    const U = fn(t);
    s(U);
  }, [t]), Z = it(() => {
    l(new Set(t.selection));
  }, [t]), dt = at(null), X = it(() => {
    dt.current && clearTimeout(dt.current), dt.current = setTimeout(() => c((U) => U + 1), 500);
  }, []);
  mt(() => {
    K(), Z();
    const U = setTimeout(() => c((O) => O + 1), 200), R = () => {
      K(), X();
    };
    return t.on("change", R), t.on("node:create", R), t.on("node:delete", R), t.on("node:data", R), t.on("selection", Z), t.on("history", R), () => {
      clearTimeout(U), t.off("change", R), t.off("node:create", R), t.off("node:delete", R), t.off("node:data", R), t.off("selection", Z), t.off("history", R), dt.current && clearTimeout(dt.current);
    };
  }, [t, K, Z, X]), mt(() => {
    if (!W.current) return;
    const U = W.current.querySelectorAll("[data-frame-card]");
    D.current = Array.from(U).map((R) => R.offsetHeight + ci);
  }, [n]);
  const rt = it(
    (U) => {
      t.select(U), t.zoomToNode(U, 0.8);
    },
    [t]
  ), J = it(
    (U, R) => {
      U.preventDefault(), U.stopPropagation(), d.current = U.clientY, h.current = R, f.current = R, p.current = !1;
    },
    []
  );
  return mt(() => {
    const U = (O) => {
      if (h.current === null) return;
      const _ = O.clientY - d.current;
      if (!p.current) {
        if (Math.abs(_) < 4) return;
        p.current = !0, x(h.current), z(h.current);
      }
      F(_);
      const G = D.current, V = h.current;
      let Y = V;
      if (_ > 0) {
        let $ = 0;
        for (let nt = V + 1; nt < y.current.length && ($ += G[nt] || 0, _ > $ - (G[nt] || 0) / 2); nt++)
          Y = nt;
      } else if (_ < 0) {
        let $ = 0;
        for (let nt = V - 1; nt >= 0 && ($ -= G[nt] || 0, _ < $ + (G[nt] || 0) / 2); nt--)
          Y = nt;
      }
      f.current = Y, z(Y);
    }, R = () => {
      const O = h.current, _ = f.current;
      if (O !== null && _ !== null && O !== _) {
        m.current = !0;
        const G = [...y.current], [V] = G.splice(O, 1);
        G.splice(_, 0, V);
        let Y = !0;
        for (let $ = 0; $ < G.length; $++) {
          const nt = G[$], ot = t.getNode(nt.id);
          ot && (Y ? (t.updateNodeWithHistory(nt.id, {
            data: { ...ot.data, slideOrder: $ + 1 }
          }), Y = !1) : t.updateNode(nt.id, {
            data: { ...ot.data, slideOrder: $ + 1 }
          }));
        }
        m.current = !1, g.current = !0, s(fn(t)), c(($) => $ + 1);
      }
      h.current = null, f.current = null, p.current = !1, x(null), z(null), F(0), g.current && requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          g.current = !1;
        });
      });
    };
    return document.addEventListener("pointermove", U), document.addEventListener("pointerup", R), document.addEventListener("pointercancel", R), () => {
      document.removeEventListener("pointermove", U), document.removeEventListener("pointerup", R), document.removeEventListener("pointercancel", R);
    };
  }, [t]), /* @__PURE__ */ v(
    "div",
    {
      "data-sb-frames-panel": !0,
      style: {
        position: "absolute",
        top: 0,
        right: 0,
        bottom: 0,
        width: li,
        background: r.panelBg,
        borderLeft: `1px solid ${r.border}`,
        zIndex: 98,
        display: "flex",
        flexDirection: "column",
        transform: e ? "translateX(0)" : `translateX(${li}px)`,
        transition: "transform 0.2s ease-in-out",
        pointerEvents: e ? "auto" : "none"
      },
      onPointerDown: (U) => U.stopPropagation(),
      children: [
        /* @__PURE__ */ v(
          "div",
          {
            style: {
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              padding: "10px 12px",
              borderBottom: `1px solid ${r.border}`,
              flexShrink: 0
            },
            children: [
              /* @__PURE__ */ v("span", { style: { fontSize: 12, fontWeight: 600, color: r.text, letterSpacing: "0.02em" }, children: [
                "Slides (",
                n.length,
                ")"
              ] }),
              /* @__PURE__ */ u(
                "button",
                {
                  title: "Close slides panel",
                  onClick: o,
                  style: {
                    border: "none",
                    background: "transparent",
                    cursor: "pointer",
                    color: r.textMuted,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    width: 24,
                    height: 24,
                    borderRadius: 4,
                    padding: 0
                  },
                  children: /* @__PURE__ */ u(lf, {})
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ v(
          "div",
          {
            ref: W,
            style: {
              flex: 1,
              overflowY: "auto",
              overflowX: "hidden",
              padding: "8px 12px",
              display: "flex",
              flexDirection: "column",
              gap: ci
            },
            children: [
              n.length === 0 && /* @__PURE__ */ u("div", { style: { padding: "20px 8px", textAlign: "center", color: r.textMuted, fontSize: 11 }, children: "No frames yet. Use the Frame tool (F) to create slides." }),
              n.map((U, R) => {
                const O = i.has(U.id), _ = b === R;
                let G = 0;
                if (_)
                  G = S;
                else if (b !== null && C !== null) {
                  const $ = D.current;
                  b < C ? R > b && R <= C && (G = -($[b] || 0)) : b > C && R >= C && R < b && (G = $[b] || 0);
                }
                const V = ($) => {
                  t.updateNodeWithHistory(U.id, {
                    data: { transition: $ === "pan" ? void 0 : $, transitionDuration: void 0 }
                  });
                }, Y = ($) => {
                  t.updateNodeWithHistory(U.id, {
                    data: { transitionDuration: $ }
                  });
                };
                return /* @__PURE__ */ v(qa.Fragment, { children: [
                  b === null && /* @__PURE__ */ u(
                    uf,
                    {
                      value: U.transition ?? "pan",
                      durationMs: U.transitionDuration,
                      onChange: V,
                      onDurationChange: Y,
                      theme: r
                    }
                  ),
                  /* @__PURE__ */ u(
                    "div",
                    {
                      "data-frame-card": !0,
                      onPointerDown: ($) => J($, R),
                      onDoubleClick: () => rt(U.id),
                      style: {
                        borderRadius: 6,
                        border: O ? `2px solid ${U.borderColor || r.text}` : `1px solid ${r.border}`,
                        background: O ? r.controlBgActive : "transparent",
                        cursor: _ ? "grabbing" : "grab",
                        userSelect: "none",
                        touchAction: "none",
                        transition: _ || g.current ? "none" : "transform 0.15s ease, border-color 0.1s ease",
                        transform: `translateY(${G}px)`,
                        zIndex: _ ? 10 : 1,
                        opacity: _ ? 0.92 : 1,
                        boxShadow: _ ? "0 4px 12px rgba(0,0,0,0.18)" : "none",
                        overflow: "hidden",
                        position: "relative",
                        flexShrink: 0
                      },
                      children: /* @__PURE__ */ u(df, { engine: t, frameId: U.id, tick: a })
                    }
                  )
                ] }, U.id);
              })
            ]
          }
        )
      ]
    }
  );
}
const lo = 50, pn = 30, pf = `
attribute vec2 aUV;
uniform float uLayPos;
uniform float uRadius;
uniform float uSide;

varying float vLight;
varying float vSpec;
varying float vShadow;
varying float vY;

void main() {
  float PI = 3.14159265;
  float d = aUV.x; // 0 = fold edge (center), 1 = far edge
  float y = aUV.y; // -1 to 1

  float x, z, theta;

  // Slight height-based radius modulation for organic curl
  float R = uRadius * (1.0 + 0.04 * sin(y * PI));

  if (d >= uLayPos) {
    // Flat — already laid on screen
    x = d;
    z = 0.0;
    theta = 0.0;
  } else {
    float arc = uLayPos - d;
    theta = arc / R;
    if (theta < PI) {
      // On the curl cylinder
      x = uLayPos - R * sin(theta);
      z = R * (1.0 - cos(theta));
    } else {
      // Back side — past the full curl, extending away
      float back = (theta - PI) * R;
      x = uLayPos + back;
      z = 2.0 * R;
      theta = PI;
    }
  }

  // Mirror for left/right half
  x *= uSide;

  // Perspective
  float eye = 2.5;
  float s = eye / (eye + z);

  // Diffuse lighting (front-facing light)
  float diffuse = 0.2 + 0.8 * max(0.0, cos(theta));
  diffuse *= (1.0 - z * 0.25); // depth darkening

  // Specular highlight on curl crest (~40 degrees)
  float specTarget = 0.7;
  vSpec = pow(max(0.0, 1.0 - abs(theta - specTarget) * 2.5), 6.0) * 0.5;

  // Shadow on flat surface near curl
  if (d >= uLayPos) {
    float distToCurl = d - uLayPos;
    vShadow = 1.0 - 0.4 * exp(-distToCurl * 25.0);
  } else {
    vShadow = 1.0;
  }

  vLight = diffuse;
  vY = y * 0.5 + 0.5; // normalized 0..1

  gl_Position = vec4(x * s * 2.0, y * s, -z * 0.01, 1.0);
}
`, yf = `
precision mediump float;
varying float vLight;
varying float vSpec;
varying float vShadow;
varying float vY;
uniform vec3 uColor;

void main() {
  // Height gradient for subtle sheen (Video Toaster aesthetic)
  float gradient = 0.88 + 0.12 * vY;
  vec3 c = uColor * vLight * gradient * vShadow + vec3(vSpec);
  gl_FragColor = vec4(c, 1.0);
}
`;
function ui(t, e, o) {
  const r = t.createShader(e);
  return r ? (t.shaderSource(r, o), t.compileShader(r), t.getShaderParameter(r, t.COMPILE_STATUS) ? r : (t.deleteShader(r), null)) : null;
}
function mf(t, e, o) {
  const r = ui(t, t.VERTEX_SHADER, e), n = ui(t, t.FRAGMENT_SHADER, o);
  if (!r || !n) return null;
  const s = t.createProgram();
  return t.attachShader(s, r), t.attachShader(s, n), t.linkProgram(s), t.getProgramParameter(s, t.LINK_STATUS) ? s : null;
}
function gf() {
  const t = [], e = [];
  for (let o = 0; o <= pn; o++)
    for (let r = 0; r <= lo; r++)
      t.push(r / lo, o / pn * 2 - 1);
  for (let o = 0; o < pn; o++)
    for (let r = 0; r < lo; r++) {
      const n = o * (lo + 1) + r;
      e.push(n, n + lo + 1, n + 1, n + 1, n + lo + 1, n + lo + 2);
    }
  return { vertices: new Float32Array(t), indices: new Uint16Array(e) };
}
function bf({ phase: t, progress: e }) {
  const o = at(null), r = at(null);
  return mt(() => {
    const n = o.current;
    if (!n) return;
    const s = window.devicePixelRatio || 1;
    n.width = n.clientWidth * s, n.height = n.clientHeight * s;
    const i = n.getContext("webgl", { alpha: !0, premultipliedAlpha: !1, antialias: !0 });
    if (!i) return;
    const l = mf(i, pf, yf);
    if (!l) return;
    i.useProgram(l);
    const { vertices: a, indices: c } = gf(), h = i.createBuffer();
    i.bindBuffer(i.ARRAY_BUFFER, h), i.bufferData(i.ARRAY_BUFFER, a, i.STATIC_DRAW);
    const f = i.createBuffer();
    i.bindBuffer(i.ELEMENT_ARRAY_BUFFER, f), i.bufferData(i.ELEMENT_ARRAY_BUFFER, c, i.STATIC_DRAW);
    const d = i.getAttribLocation(l, "aUV");
    i.enableVertexAttribArray(d), i.vertexAttribPointer(d, 2, i.FLOAT, !1, 0, 0), i.enable(i.DEPTH_TEST), i.clearColor(0, 0, 0, 0);
    const p = (y) => i.getUniformLocation(l, y);
    return r.current = {
      gl: i,
      locs: { uLayPos: p("uLayPos"), uRadius: p("uRadius"), uSide: p("uSide"), uColor: p("uColor") },
      count: c.length
    }, () => {
      i.deleteProgram(l), i.deleteBuffer(h), i.deleteBuffer(f), r.current = null;
    };
  }, []), mt(() => {
    const n = r.current;
    if (!n) return;
    const { gl: s, locs: i, count: l } = n, a = t === "out" ? 1 - Math.pow(1 - e, 3) : Math.pow(e, 3), c = t === "out" ? 1 - a : a, h = 0.07 + 0.16 * c;
    s.viewport(0, 0, s.canvas.width, s.canvas.height), s.clear(s.COLOR_BUFFER_BIT | s.DEPTH_BUFFER_BIT), s.uniform1f(i.uLayPos, c), s.uniform1f(i.uRadius, h), s.uniform1f(i.uSide, 1), s.uniform3f(i.uColor, 0.09, 0.09, 0.17), s.drawElements(s.TRIANGLES, l, s.UNSIGNED_SHORT, 0), s.uniform1f(i.uSide, -1), s.uniform3f(i.uColor, 0.09, 0.09, 0.17), s.drawElements(s.TRIANGLES, l, s.UNSIGNED_SHORT, 0);
  }, [t, e]), /* @__PURE__ */ u(
    "canvas",
    {
      ref: o,
      style: {
        position: "absolute",
        inset: 0,
        width: "100%",
        height: "100%",
        pointerEvents: "none",
        zIndex: 9999
      }
    }
  );
}
const xf = {
  position: "absolute",
  bottom: 0,
  left: 0,
  right: 0,
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  gap: 12,
  padding: "10px 20px",
  background: "rgba(0,0,0,0.7)",
  backdropFilter: "blur(8px)",
  pointerEvents: "auto"
}, yn = {
  border: "none",
  background: "rgba(255,255,255,0.1)",
  color: "white",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  width: 32,
  height: 32,
  padding: 0
}, En = {
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function fi({ dir: t }) {
  return /* @__PURE__ */ v("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", children: [
    t === "left" && /* @__PURE__ */ u("polyline", { points: "15,18 9,12 15,6", ...En }),
    t === "right" && /* @__PURE__ */ u("polyline", { points: "9,6 15,12 9,18", ...En })
  ] });
}
function wf() {
  return /* @__PURE__ */ u("svg", { width: 14, height: 14, viewBox: "0 0 24 24", fill: "none", children: /* @__PURE__ */ u("path", { d: "M18 6L6 18M6 6l12 12", ...En }) });
}
function pi(t) {
  return 1 - Math.pow(1 - t, 3);
}
function yi(t) {
  return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
}
function mi(t, e) {
  let r;
  t <= 0.2 ? r = 1 + (0.55 - 1) * pi(t / 0.2) : t >= 0.8 ? r = 0.55 + (1 - 0.55) * pi((t - 0.8) / 0.2) : r = 0.55;
  let n;
  return t <= 0.1 ? n = 0 : t <= 0.5 ? n = -e * 90 * yi((t - 0.1) / 0.4) : t <= 0.9 ? n = e * 90 * (1 - yi((t - 0.5) / 0.4)) : n = 0, { zoom: r, angle: n };
}
function kf(t, e, o, r) {
  t.style.transform = `perspective(1200px) scale(${o}) rotateY(${r}deg)`, t.style.transformOrigin = "50% 50%", t.style.backfaceVisibility = "hidden", t.style.overflow = "visible", e.style.background = "#0a0a15";
}
function gi(t, e) {
  t.style.transform = "", t.style.transformOrigin = "", t.style.backfaceVisibility = "", t.style.overflow = "", e.style.background = "";
}
function vf({ engine: t }) {
  const [e, o] = tt(t.presentationMode), [r, n] = tt(t.presentationIndex), [s, i] = tt(t.presentationSlides.length), [l, a] = tt(""), [c, h] = tt(t.transitionOverlay), f = at(null), d = at(null);
  if (mt(() => {
    const y = document.querySelector("[data-sb-canvas]");
    f.current = y, d.current = (y == null ? void 0 : y.parentElement) ?? null;
    const m = () => {
      var C;
      if (o(t.presentationMode), n(t.presentationIndex), i(t.presentationSlides.length), h(t.transitionOverlay), t.presentationMode && t.presentationSlides.length > 0) {
        const z = t.presentationSlides[t.presentationIndex], S = t.getNode(z);
        a(((C = S == null ? void 0 : S.data) == null ? void 0 : C.label) || "");
      } else
        a("");
      const g = t.transitionOverlay, b = f.current, x = d.current;
      if (b && x && g && g.type === "cube" && g.t != null) {
        const z = g.direction ?? 1, { zoom: S, angle: F } = mi(g.t, z);
        kf(b, x, S, F);
      } else b && x && gi(b, x);
    };
    return t.on("presentation", m), () => {
      t.off("presentation", m);
      const g = f.current, b = d.current;
      g && b && gi(g, b);
    };
  }, [t]), !e || s === 0) return null;
  const p = c && c.type === "cube" && c.t != null ? (() => {
    const y = c.direction ?? 1, { angle: m } = mi(c.t, y);
    return Math.abs(m) / 90 * 0.4;
  })() : 0;
  return /* @__PURE__ */ v(
    "div",
    {
      "data-sb-presentation": !0,
      style: {
        position: "absolute",
        inset: 0,
        zIndex: 1e4,
        pointerEvents: "none"
      },
      children: [
        c && c.type !== "fold" && c.type !== "cube" && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              backgroundColor: "black",
              opacity: c.phase === "out" ? c.progress : 1 - c.progress,
              pointerEvents: "none",
              zIndex: 9999
            }
          }
        ),
        c && c.type === "fold" && /* @__PURE__ */ u(bf, { phase: c.phase, progress: c.progress }),
        p > 0.01 && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              backgroundColor: "black",
              opacity: p,
              pointerEvents: "none",
              zIndex: 9999
            }
          }
        ),
        /* @__PURE__ */ v("div", { style: xf, onPointerDown: (y) => y.stopPropagation(), children: [
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...yn, position: "absolute", right: 16 },
              title: "Exit presentation (Esc)",
              onClick: () => t.exitPresentation(),
              children: /* @__PURE__ */ u(wf, {})
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...yn, opacity: r <= 0 ? 0.3 : 1 },
              title: "Previous slide (←)",
              onClick: () => t.presentationPrev(),
              disabled: r <= 0,
              children: /* @__PURE__ */ u(fi, { dir: "left" })
            }
          ),
          /* @__PURE__ */ v(
            "span",
            {
              style: {
                color: "white",
                fontSize: 13,
                fontWeight: 500,
                fontFamily: "inherit",
                minWidth: 80,
                textAlign: "center",
                userSelect: "none"
              },
              children: [
                r + 1,
                " / ",
                s,
                l && /* @__PURE__ */ v("span", { style: { opacity: 0.6, marginLeft: 8 }, children: [
                  "— ",
                  l
                ] })
              ]
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...yn, opacity: r >= s - 1 ? 0.3 : 1 },
              title: "Next slide (→)",
              onClick: () => t.presentationNext(),
              disabled: r >= s - 1,
              children: /* @__PURE__ */ u(fi, { dir: "right" })
            }
          )
        ] })
      ]
    }
  );
}
const Sf = Qa(() => import("./DebugPanel-CdjCyWj0.js"));
function Yf({
  nodeTypes: t = Td,
  engine: e,
  keyboardShortcuts: o = !0,
  style: r,
  initialData: n,
  toolbar: s = !0,
  debugPanel: i = !1,
  debugBoards: l,
  theme: a,
  onPresentationChange: c,
  gifApiBaseUrl: h
}) {
  const f = Kt(
    () => e ?? new Zl(),
    [e]
  ), d = Kt(() => new Ul(t), [t]);
  mt(() => Tl(), []), mt(() => {
    f.setRegistry(d);
  }, [f, d]), mt(() => {
    for (const S of t)
      S.isContainer && f.registerContainerType(S.type);
  }, [f, t]);
  const p = at(!1);
  mt(() => {
    n && !p.current && (p.current = !0, f.fromSBD(n));
  }, [f, n]);
  const y = at(null);
  mt(() => {
    if (o)
      return gh(f, y.current);
  }, [f, o]);
  const m = Kt(() => t.some((F) => {
    var D;
    return (D = F.ports) == null ? void 0 : D.length;
  }) ? new Pd(f, d) : null, [f, d, t]);
  mt(() => {
    if (m)
      return m.connect();
  }, [m]);
  const g = Kt(
    () => a ? { ...zn, ...a } : zn,
    [a]
  ), [b, x] = tt(!1), [C, z] = tt(!1);
  return mt(() => {
    const S = () => {
      const F = f.presentationMode;
      x(F), c == null || c(F);
    };
    return f.on("presentation", S), () => f.off("presentation", S);
  }, [f, c]), /* @__PURE__ */ u(Ji.Provider, { value: g, children: /* @__PURE__ */ v(
    "div",
    {
      ref: y,
      style: {
        width: "100%",
        height: "100%",
        position: "relative",
        ...r
      },
      children: [
        s && !b && /* @__PURE__ */ u(ef, { engine: f, registry: d, gifApiBaseUrl: h }),
        i && /* @__PURE__ */ u(Ua, { fallback: null, children: /* @__PURE__ */ u(Sf, { engine: f, extraBoards: l }) }),
        /* @__PURE__ */ v(
          "div",
          {
            style: {
              position: "absolute",
              left: s && !b ? ir : 0,
              top: 0,
              right: 0,
              bottom: 0,
              overflow: "hidden"
            },
            children: [
              /* @__PURE__ */ u(jh, { engine: f, schema: Fn, registry: d, dataFlow: m }),
              !b && /* @__PURE__ */ u(
                sf,
                {
                  engine: f,
                  framesPanelOpen: C,
                  onToggleFramesPanel: () => z((S) => !S)
                }
              ),
              !b && /* @__PURE__ */ u(
                ff,
                {
                  engine: f,
                  open: C,
                  onClose: () => z(!1)
                }
              ),
              /* @__PURE__ */ u(vf, { engine: f })
            ]
          }
        )
      ]
    }
  ) });
}
const Mf = [
  { key: "select", label: "Select", shortcut: "V" },
  { key: "draw", label: "Draw", shortcut: "D" },
  { key: "shape", label: "Shape", shortcut: "S" },
  { key: "text", label: "Text", shortcut: "T" },
  { key: "note", label: "Note", shortcut: "N" },
  { key: "sticky", label: "Sticky", shortcut: "P" },
  { key: "frame", label: "Frame", shortcut: "F" },
  { key: "erase", label: "Eraser", shortcut: "X" }
], Po = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, te = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function Qo({ name: t, size: e = 18 }) {
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "select" && /* @__PURE__ */ u("path", { d: "M6 2v17l4-4.5L13.5 21l2.5-1.5L12.5 13H18z", fill: "currentColor" }),
    t === "draw" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M17 3l4 4L7.5 20.5 2 22l1.5-5.5z", ...te }),
      /* @__PURE__ */ u("path", { d: "M15 5l4 4", ...te })
    ] }),
    t === "shape" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...te }),
    t === "text" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M7 4h10", ...te }),
      /* @__PURE__ */ u("path", { d: "M12 4v16", ...te })
    ] }),
    t === "note" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 3h16v14l-4 4H4z", ...te }),
      /* @__PURE__ */ u("path", { d: "M16 17v4l4-4z", fill: "currentColor", opacity: 0.4 })
    ] }),
    t === "sticky" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "1", fill: "currentColor", opacity: 0.15, ...te }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "9", x2: "17", y2: "9", ...te, opacity: 0.5 }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "13", x2: "14", y2: "13", ...te, opacity: 0.5 })
    ] }),
    t === "frame" && /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2", ...te, strokeDasharray: "4,2" }),
    t === "erase" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 20H9L3 14l9.5-9.5 8 8L16 17", ...te }),
      /* @__PURE__ */ u("path", { d: "M12.5 4.5l8 8", ...te })
    ] }),
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...te }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...te }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...te }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...te }),
    t === "arrow" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...te }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...te, fill: "none" })
    ] }),
    t === "undo" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...te, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...te, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...te, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...te, fill: "none" })
    ] }),
    t === "print" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M6 9V3h12v6", ...te }),
      /* @__PURE__ */ u("path", { d: "M6 18H4a1 1 0 0 1-1-1v-5a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-2", ...te }),
      /* @__PURE__ */ u("rect", { x: "6", y: "14", width: "12", height: "7", rx: "1", ...te })
    ] }),
    t === "fit" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...te }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...te })
    ] })
  ] });
}
function Gf({ engine: t }) {
  const [e, o] = tt(t.mode), [r, n] = tt(!1), [s, i] = tt(!1), [l, a] = tt(t.boardBackground);
  return mt(() => {
    const c = () => o(t.mode), h = () => {
      n(t.canUndo()), i(t.canRedo());
    }, f = () => a(t.boardBackground);
    return t.on("mode", c), t.on("history", h), t.on("background", f), () => {
      t.off("mode", c), t.off("history", h), t.off("background", f);
    };
  }, [t]), /* @__PURE__ */ v(
    "div",
    {
      "data-sb-toolbar": !0,
      style: {
        position: "absolute",
        left: 0,
        top: 0,
        bottom: 0,
        width: 48,
        background: "#1e1e2e",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        padding: "8px 0",
        gap: 4,
        zIndex: 100
      },
      children: [
        Mf.map((c) => /* @__PURE__ */ u(
          "button",
          {
            title: `${c.label} (${c.shortcut})`,
            onClick: () => t.setMode(c.key),
            style: {
              ...Po,
              width: 36,
              height: 36,
              background: e === c.key ? "#3b82f6" : "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u(Qo, { name: c.key })
          },
          c.key
        )),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "8px 0"
            }
          }
        ),
        [
          { key: "dot-grid", color: "#f8f7f5", label: "Dot Grid" },
          { key: "blueprint", color: "#1e3a5f", label: "Blueprint" },
          { key: "japanese-stationery", color: "#f5f0e8", label: "Japanese Stationery" }
        ].map((c) => /* @__PURE__ */ u(
          "button",
          {
            title: c.label,
            onClick: () => t.setBoardBackground(c.key),
            style: {
              ...Po,
              width: 20,
              height: 20,
              background: c.color,
              border: l === c.key ? "2px solid white" : "2px solid transparent",
              borderRadius: 4,
              boxShadow: c.key === "japanese-stationery" ? "inset 0 1px 0 rgba(255,255,255,0.5), inset 0 -1px 0 rgba(160,140,110,0.15)" : void 0
            }
          },
          c.key
        )),
        /* @__PURE__ */ u("div", { style: { flex: 1 } }),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Print (landscape)",
            onClick: () => {
              const c = { ...t.viewport };
              t.fitToContent(), requestAnimationFrame(() => {
                window.print(), t.viewport.x = c.x, t.viewport.y = c.y, t.viewport.zoom = c.zoom, t.pan(0, 0);
              });
            },
            style: {
              ...Po,
              width: 36,
              height: 36,
              background: "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u(Qo, { name: "print" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "4px 0"
            }
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Undo (Ctrl+Z)",
            onClick: () => t.undo(),
            disabled: !r,
            style: {
              ...Po,
              width: 36,
              height: 36,
              background: "transparent",
              color: r ? "white" : "#666"
            },
            children: /* @__PURE__ */ u(Qo, { name: "undo" })
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Redo (Ctrl+Shift+Z)",
            onClick: () => t.redo(),
            disabled: !s,
            style: {
              ...Po,
              width: 36,
              height: 36,
              background: "transparent",
              color: s ? "white" : "#666"
            },
            children: /* @__PURE__ */ u(Qo, { name: "redo" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "4px 0"
            }
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Fit to content (Ctrl+0)",
            onClick: () => t.fitToContent(),
            style: {
              ...Po,
              width: 36,
              height: 36,
              background: "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u(Qo, { name: "fit" })
          }
        )
      ]
    }
  );
}
const Ye = [
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], Cf = [
  null,
  // no fill
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], zf = [
  { key: "hachure", label: "Hachure" },
  { key: "cross-hatch", label: "Cross-hatch" },
  { key: "solid", label: "Solid" }
], Ro = [
  { key: "solid", label: "Solid", dash: "" },
  { key: "dashed", label: "Dashed", dash: "6,3" },
  { key: "dotted", label: "Dotted", dash: "2,2" }
], If = [
  { value: 0, label: "Architect" },
  { value: 1, label: "Artist" },
  { value: 2, label: "Cartoonist" }
], Ao = [1, 2.5, 5, 10, 20], Tf = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
], Pf = [14, 20, 28, 36], Rf = [
  { key: "left", label: "←" },
  { key: "center", label: "↔" },
  { key: "right", label: "→" }
], mn = 300, Bt = {
  display: "flex",
  alignItems: "center",
  gap: 4
}, Nt = {
  width: 64,
  fontSize: 10,
  color: "#999",
  flexShrink: 0
}, Vt = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0,
  flexShrink: 0
};
function jf({
  engine: t,
  registry: e
}) {
  const [o, r] = tt(t.mode), [n, s] = tt(t.selection), [, i] = tt(0), [l, a] = tt(null), c = at(null), h = at(null), [f, d] = tt(!1), p = it(() => {
    var st;
    return { x: (((st = c.current) == null ? void 0 : st.ownerDocument.defaultView) ?? window).innerWidth - mn - 12, y: 12 };
  }, []), y = l ?? p();
  mt(() => {
    const k = () => r(t.mode), st = () => {
      s(new Set(t.selection)), i((ne) => ne + 1);
    }, Yt = () => i((ne) => ne + 1);
    return t.on("mode", k), t.on("selection", st), t.on("change", Yt), () => {
      t.off("mode", k), t.off("selection", st), t.off("change", Yt);
    };
  }, [t]);
  const m = it((k) => {
    k.stopPropagation(), d(!0);
    const st = l ? l.x : p().x, Yt = l ? l.y : p().y;
    h.current = { startX: k.clientX, startY: k.clientY, startLeft: st, startTop: Yt }, k.currentTarget.setPointerCapture(k.pointerId);
  }, [l, p]);
  mt(() => {
    var ne;
    const k = (be) => {
      var De;
      if (!h.current) return;
      const Le = be.clientX - h.current.startX, ro = be.clientY - h.current.startY, me = ((De = c.current) == null ? void 0 : De.ownerDocument.defaultView) ?? window, Go = Math.max(48, Math.min(me.innerWidth - mn - 8, h.current.startLeft + Le)), lr = Math.max(8, Math.min(me.innerHeight - 100, h.current.startTop + ro));
      a({ x: Go, y: lr });
    }, st = () => {
      h.current = null, d(!1);
    }, Yt = ((ne = c.current) == null ? void 0 : ne.ownerDocument) ?? document;
    return Yt.addEventListener("pointermove", k), Yt.addEventListener("pointerup", st), Yt.addEventListener("pointercancel", st), () => {
      Yt.removeEventListener("pointermove", k), Yt.removeEventListener("pointerup", st), Yt.removeEventListener("pointercancel", st);
    };
  }, []);
  const g = (() => {
    if (n.size === 1) {
      const k = Array.from(n)[0], st = t.getNode(k);
      if ((st == null ? void 0 : st.type) === "shape") return { kind: "shape", node: st };
      if ((st == null ? void 0 : st.type) === "draw") return { kind: "draw", node: st };
      if ((st == null ? void 0 : st.type) === "text") return { kind: "text", node: st };
      if ((st == null ? void 0 : st.type) === "edge") return { kind: "edge", node: st };
      if ((st == null ? void 0 : st.type) === "image") return { kind: "image", node: st };
      if ((st == null ? void 0 : st.type) === "content") return { kind: "content", node: st };
      if ((st == null ? void 0 : st.type) === "frame") return { kind: "frame", node: st };
      if ((st == null ? void 0 : st.type) === "sticky") return { kind: "sticky", node: st };
      if (st && e) {
        const Yt = e.get(st.type);
        if (Yt != null && Yt.propertiesPanel)
          return { kind: "custom", node: st, PanelComponent: Yt.propertiesPanel };
      }
    }
    return o === "draw" || o === "shape" || o === "text" ? { kind: "tool" } : null;
  })(), b = it(
    (k) => {
      !g || g.kind !== "shape" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), x = it(
    (k) => {
      !g || g.kind !== "draw" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), C = it(
    (k) => {
      !g || g.kind !== "text" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), z = it(
    (k) => {
      !g || g.kind !== "edge" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), S = it(
    (k) => {
      !g || g.kind !== "image" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), F = it(
    (k) => {
      !g || g.kind !== "content" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), D = it(
    (k) => {
      !g || g.kind !== "frame" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), W = it(
    (k) => {
      !g || g.kind !== "sticky" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), K = it(
    (k) => {
      !g || g.kind !== "custom" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...k }
      });
    },
    [t, g]
  ), [Z, dt] = tt("idle");
  if (!g) return null;
  const X = g.kind === "custom", rt = g.kind === "shape", J = g.kind === "draw", U = g.kind === "text", R = g.kind === "edge", O = g.kind === "image", _ = g.kind === "content", G = g.kind === "frame", V = g.kind === "sticky", Y = g.kind === "tool", $ = Y && o === "shape", nt = Y && o === "text", ot = U ? g.node.data.fontFamily : t.activeTool.fontFamily ?? Ke, yt = U ? g.node.data.fontSize : t.activeTool.fontSize ?? 20, ft = U ? g.node.data.align : t.activeTool.textAlign ?? "left", et = U ? g.node.data.color : t.activeTool.color, Pt = rt ? g.node.data.stroke : J ? g.node.data.color : t.activeTool.color, zt = rt || J ? g.node.data.fill ?? null : t.activeTool.fillColor ?? null, gt = rt || J ? g.node.data.fillStyle ?? "hachure" : t.activeTool.fillStyle ?? "hachure", ht = rt || J ? g.node.data.strokeStyle ?? "solid" : t.activeTool.strokeStyle ?? "solid", Dt = rt || J ? g.node.data.strokeWidth : t.activeTool.width, It = rt ? g.node.data.roughness : t.activeTool.roughness ?? 1, Rt = rt || J || U || O || _ || G || V ? g.node.data.opacity ?? 1 : t.activeTool.opacity ?? 1, Qt = (() => {
    const k = /* @__PURE__ */ new Set(), st = [];
    for (const Yt of t.getAllNodes())
      if (Yt.type === "text") {
        const ne = Yt.data.fontFamily;
        ne && !k.has(ne) && (k.add(ne), st.push(ne));
      }
    return st;
  })(), Jt = !U && !nt && !R && !O && !_ && !G && !V && !X, qt = Jt, $t = Jt, he = rt || $, Re = U || nt, _t = (k) => {
    rt ? b({ stroke: k }) : J ? x({ color: k }) : (t.activeTool.color = k, i((st) => st + 1));
  }, Ae = (k) => {
    rt ? b({ fill: k ?? void 0 }) : J ? x({ fill: k ?? void 0 }) : (t.activeTool.fillColor = k ?? void 0, i((st) => st + 1));
  }, to = (k) => {
    rt ? b({ fillStyle: k }) : J ? x({ fillStyle: k }) : (t.activeTool.fillStyle = k, i((st) => st + 1));
  }, bo = (k) => {
    rt ? b({ strokeStyle: k }) : J ? x({ strokeStyle: k }) : (t.activeTool.strokeStyle = k, i((st) => st + 1));
  }, ue = (k) => {
    rt ? b({ strokeWidth: k }) : J ? x({ strokeWidth: k }) : (t.activeTool.width = k, i((st) => st + 1));
  }, ve = (k) => {
    rt ? b({ roughness: k }) : (t.activeTool.roughness = k, i((st) => st + 1));
  }, eo = (k) => {
    rt ? b({ opacity: k }) : J ? x({ opacity: k }) : U ? C({ opacity: k }) : O ? S({ opacity: k }) : _ ? F({ opacity: k }) : G ? D({ opacity: k }) : V ? W({ opacity: k }) : (t.activeTool.opacity = k, i((st) => st + 1));
  }, oo = (k) => {
    U ? C({ fontFamily: k }) : (t.activeTool.fontFamily = k, i((st) => st + 1));
  }, fe = (k) => {
    U ? C({ fontSize: k }) : (t.activeTool.fontSize = k, i((st) => st + 1));
  }, Se = (k) => {
    U ? C({ align: k }) : (t.activeTool.textAlign = k, i((st) => st + 1));
  }, ge = (k) => {
    U ? C({ color: k }) : (t.activeTool.color = k, i((st) => st + 1));
  }, Be = {
    position: "fixed",
    left: y.x,
    top: y.y,
    width: mn,
    background: "#1e1e2e",
    borderRadius: 12,
    padding: "12px 16px",
    display: "flex",
    flexDirection: "column",
    gap: 10,
    zIndex: 99,
    color: "white",
    fontSize: 11,
    maxHeight: "calc(100vh - 40px)",
    overflowY: "auto",
    boxShadow: "0 4px 24px rgba(0,0,0,0.4)"
  };
  return /* @__PURE__ */ v(
    "div",
    {
      ref: c,
      "data-sb-props-panel": !0,
      style: Be,
      onPointerDown: (k) => k.stopPropagation(),
      children: [
        /* @__PURE__ */ v(
          "div",
          {
            onPointerDown: m,
            style: {
              cursor: f ? "grabbing" : "grab",
              margin: "-12px -16px 8px -16px",
              padding: "8px 16px",
              userSelect: "none",
              display: "flex",
              alignItems: "center",
              gap: 6,
              borderBottom: "1px solid #333",
              color: "#999",
              fontSize: 10
            },
            children: [
              /* @__PURE__ */ u("span", { style: { opacity: 0.6 }, children: "⋮⋮" }),
              /* @__PURE__ */ u("span", { children: "Drag to move" })
            ]
          }
        ),
        Re && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Font" }),
            /* @__PURE__ */ u(
              Xr,
              {
                value: ot,
                onChange: oo,
                fontsInScene: Qt
              }
            )
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Size" }),
            Pf.map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => fe(k),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: yt === k ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: k
              },
              k
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Align" }),
            Rf.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.key,
                onClick: () => Se(k.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: ft === k.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 12,
                  borderRadius: 6
                },
                children: k.label
              },
              k.key
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            Ye.map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => ge(k),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k,
                  border: et === k ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              k
            ))
          ] }),
          U && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...Ye].map((k, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => C({ borderColor: k ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k ?? "transparent",
                  border: (g.node.data.borderColor ?? null) === k ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: 2,
                      height: 24,
                      background: "#e74c3c",
                      transform: "rotate(45deg)"
                    }
                  }
                )
              },
              k ?? "none"
            ))
          ] }),
          U && g.node.data.borderColor && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ro.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => C({ borderStyle: k.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "solid") === k.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: k.dash
                  }
                ) })
              },
              k.key
            ))
          ] }),
          U && g.node.data.borderColor && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Ao.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: `${k}px`,
                onClick: () => C({ borderWidth: k }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(k, 1),
                      background: "white",
                      borderRadius: k / 2
                    }
                  }
                )
              },
              k
            ))
          ] })
        ] }),
        Jt && /* @__PURE__ */ v(ut, { children: [
          $ && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Shape" }),
            Tf.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => {
                  t.activeTool.shapeType = k.key, i((st) => st + 1);
                },
                style: {
                  ...Vt,
                  width: 28,
                  height: 28,
                  background: (t.activeTool.shapeType ?? "rect") === k.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(Af, { name: k.key })
              },
              k.key
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke" }),
            Ye.map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => _t(k),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k,
                  border: Pt === k ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              k
            ))
          ] }),
          qt && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Fill" }),
            Cf.map((k, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => Ae(k),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k ?? "transparent",
                  border: zt === k ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              k ?? "none"
            ))
          ] }),
          qt && zt && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Fill pattern" }),
            zf.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => to(k.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: gt === k.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 9,
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(Lf, { style: k.key })
              },
              k.key
            ))
          ] }),
          $t && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke style" }),
            Ro.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => bo(k.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: ht === k.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: k.dash
                  }
                ) })
              },
              k.key
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke width" }),
            Ao.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: `${k}px`,
                onClick: () => ue(k),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: Dt === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(k, 1),
                      background: "white",
                      borderRadius: k / 2
                    }
                  }
                )
              },
              k
            ))
          ] }),
          he && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Roughness" }),
            If.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => ve(k.value),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: It === k.value ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 9,
                  borderRadius: 6
                },
                children: k.label
              },
              k.value
            ))
          ] })
        ] }),
        R && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            Ye.map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => z({ color: k }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k,
                  border: g.node.data.color === k ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              k
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ro.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => z({ style: k.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: g.node.data.style === k.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: k.dash
                  }
                ) })
              },
              k.key
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Ao.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: `${k}px`,
                onClick: () => z({ strokeWidth: k }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: g.node.data.strokeWidth === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(k, 1),
                      background: "white",
                      borderRadius: k / 2
                    }
                  }
                )
              },
              k
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Head" }),
            ["none", "arrow", "filled", "dot"].map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => z({ arrowHead: k }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.arrowHead ?? "none") === k ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: k === "none" ? "None" : k === "arrow" ? "▷" : k === "filled" ? "▶" : "●"
              },
              k
            ))
          ] }),
          (g.node.data.arrowHead ?? "none") !== "none" && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Head size" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "range",
                min: 4,
                max: 40,
                step: 1,
                value: g.node.data.arrowHeadSize ?? Math.max(8, g.node.data.strokeWidth * 3),
                onChange: (k) => z({ arrowHeadSize: Number(k.target.value) }),
                style: { flex: 1 }
              }
            ),
            /* @__PURE__ */ u("span", { style: { color: "#999", fontSize: 11, minWidth: 24, textAlign: "right" }, children: g.node.data.arrowHeadSize ?? Math.max(8, g.node.data.strokeWidth * 3) })
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Tail" }),
            ["none", "arrow", "filled", "dot"].map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => z({ arrowTail: k }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.arrowTail ?? "none") === k ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: k === "none" ? "None" : k === "arrow" ? "◁" : k === "filled" ? "◀" : "●"
              },
              k
            ))
          ] }),
          (g.node.data.arrowTail ?? "none") !== "none" && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Tail size" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "range",
                min: 4,
                max: 40,
                step: 1,
                value: g.node.data.arrowTailSize ?? Math.max(8, g.node.data.strokeWidth * 3),
                onChange: (k) => z({ arrowTailSize: Number(k.target.value) }),
                style: { flex: 1 }
              }
            ),
            /* @__PURE__ */ u("span", { style: { color: "#999", fontSize: 11, minWidth: 24, textAlign: "right" }, children: g.node.data.arrowTailSize ?? Math.max(8, g.node.data.strokeWidth * 3) })
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Label" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                value: g.node.data.label ?? "",
                onChange: (k) => z({ label: k.target.value || void 0 }),
                placeholder: "Edge label...",
                style: {
                  flex: 1,
                  background: "#2a2a3e",
                  color: "white",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 8px",
                  fontSize: 11,
                  outline: "none"
                }
              }
            )
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Path" }),
            [
              { key: "bezier", label: "Bezier" },
              { key: "straight", label: "Straight" },
              { key: "smoothstep", label: "Smooth" },
              { key: "step", label: "Step" }
            ].map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => z({ edgeType: k.key }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.edgeType ?? "bezier") === k.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: k.label
              },
              k.key
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Animate" }),
            /* @__PURE__ */ u(
              "button",
              {
                onClick: () => z({ animated: !g.node.data.animated }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 12px",
                  background: g.node.data.animated ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: g.node.data.animated ? "On" : "Off"
              }
            )
          ] }),
          g.node.data.animated && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Direction" }),
            ["forward", "reverse", "both"].map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => z({ animatedDirection: k }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.animatedDirection ?? "forward") === k ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: k === "forward" ? "→" : k === "reverse" ? "←" : "⇆"
              },
              k
            ))
          ] })
        ] }),
        O && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...Ye].map((k, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => S({ borderColor: k ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k ?? "transparent",
                  border: (g.node.data.borderColor ?? null) === k ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              k ?? "none"
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ro.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => S({ borderStyle: k.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "solid") === k.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: k.dash
                  }
                ) })
              },
              k.key
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Ao.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: `${k}px`,
                onClick: () => S({ borderWidth: k }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(k, 1),
                      background: "white",
                      borderRadius: k / 2
                    }
                  }
                )
              },
              k
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: { ...Bt, marginTop: 4 }, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Background" }),
            /* @__PURE__ */ u(
              "button",
              {
                onClick: async () => {
                  if (!(Z === "loading" || g.kind !== "image")) {
                    dt("loading");
                    try {
                      const { removeBackground: k } = await import("@imgly/background-removal"), Yt = await (await fetch(g.node.data.src)).blob(), ne = await k(Yt), be = new FileReader(), Le = await new Promise((ro, me) => {
                        be.onload = () => ro(be.result), be.onerror = me, be.readAsDataURL(ne);
                      });
                      S({ src: Le }), dt("idle");
                    } catch (k) {
                      console.error("Background removal failed:", k), dt("error"), setTimeout(() => dt("idle"), 3e3);
                    }
                  }
                },
                disabled: Z === "loading",
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 10px",
                  background: Z === "error" ? "#e74c3c" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6,
                  gap: 4,
                  opacity: Z === "loading" ? 0.6 : 1
                },
                children: Z === "loading" ? "Removing..." : Z === "error" ? "Failed" : "Remove BG"
              }
            )
          ] })
        ] }),
        _ && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...Ye].map((k, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => F({ borderColor: k ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k ?? "transparent",
                  border: (g.node.data.borderColor ?? null) === k ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              k ?? "none"
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ro.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => F({ borderStyle: k.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "solid") === k.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: k.dash
                  }
                ) })
              },
              k.key
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Ao.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: `${k}px`,
                onClick: () => F({ borderWidth: k }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(k, 1),
                      background: "white",
                      borderRadius: k / 2
                    }
                  }
                )
              },
              k
            ))
          ] })
        ] }),
        G && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Label" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                value: g.node.data.label ?? "",
                onChange: (k) => D({ label: k.target.value || void 0 }),
                placeholder: "Frame label...",
                style: {
                  flex: 1,
                  background: "#2a2a3e",
                  color: "white",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 8px",
                  fontSize: 11,
                  outline: "none"
                }
              }
            )
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Background" }),
            [null, ...Ye].map((k, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => D({ backgroundColor: k ? `${k}15` : void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k ?? "transparent",
                  border: (() => {
                    const Yt = g.node.data.backgroundColor;
                    return (k === null ? !Yt : Yt === `${k}15`) ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`;
                  })(),
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              k ?? "none"
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            Ye.map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => D({ borderColor: k }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k,
                  border: g.node.data.borderColor === k ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              k
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ro.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: k.label,
                onClick: () => D({ borderStyle: k.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "dashed") === k.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: k.dash
                  }
                ) })
              },
              k.key
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Ao.map((k) => /* @__PURE__ */ u(
              "button",
              {
                title: `${k}px`,
                onClick: () => D({ borderWidth: k }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(k, 1),
                      background: "white",
                      borderRadius: k / 2
                    }
                  }
                )
              },
              k
            ))
          ] })
        ] }),
        V && /* @__PURE__ */ v(ut, { children: [
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            [
              "#FEF3C7",
              "#FCE7F3",
              "#DBEAFE",
              "#D1FAE5",
              "#EDE9FE",
              "#FFEDD5"
            ].map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => W({ color: k }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: k,
                  border: g.node.data.color === k ? "2px solid #1e1e2e" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              k
            ))
          ] }),
          /* @__PURE__ */ v("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Size" }),
            [12, 14, 16, 20, 24].map((k) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => W({ fontSize: k }),
                style: {
                  ...Vt,
                  width: 32,
                  height: 24,
                  background: (g.node.data.fontSize ?? 16) === k ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6,
                  fontSize: 10,
                  color: "white"
                },
                children: k
              },
              k
            ))
          ] })
        ] }),
        X && (() => {
          const { node: k, PanelComponent: st } = g;
          return /* @__PURE__ */ u(st, { node: k, data: k.data, engine: t, updateData: K });
        })(),
        !R && !X && /* @__PURE__ */ v("div", { style: Bt, children: [
          /* @__PURE__ */ u("span", { style: Nt, children: "Opacity" }),
          /* @__PURE__ */ u(
            "input",
            {
              type: "range",
              min: 0,
              max: 100,
              value: Math.round(Rt * 100),
              onChange: (k) => eo(parseInt(k.target.value) / 100),
              style: { flex: 1, accentColor: "#3b82f6" }
            }
          ),
          /* @__PURE__ */ u("span", { style: { width: 28, textAlign: "right", fontSize: 10 }, children: Math.round(Rt * 100) })
        ] })
      ]
    }
  );
}
function Af({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ v("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ v(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
function Lf({ style: t }) {
  return t === "hachure" ? /* @__PURE__ */ v("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: "white", strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: "white", strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: "white", strokeWidth: 1.5 })
  ] }) : t === "cross-hatch" ? /* @__PURE__ */ v("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 2, y1: 2, x2: 8, y2: 14, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 2, x2: 14, y2: 14, stroke: "white", strokeWidth: 1.2 })
  ] }) : /* @__PURE__ */ u("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: /* @__PURE__ */ u("rect", { x: 2, y: 2, width: 16, height: 12, fill: "white", rx: 2 }) });
}
export {
  Ke as D,
  Ul as N,
  $o as P,
  Xf as S,
  Gf as T,
  zn as a,
  Pd as b,
  jf as c,
  ef as d,
  Yf as e,
  jh as f,
  Zl as g,
  Td as h,
  rc as i,
  ed as j,
  id as k,
  yd as l,
  nr as m,
  wt as n,
  Yn as o,
  dd as p,
  Nn as q,
  Rl as r,
  io as s,
  Ml as t,
  gh as u,
  nd as v,
  xd as w,
  fd as x,
  Ut as y
};
