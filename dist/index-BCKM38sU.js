var Ga = Object.defineProperty;
var ja = (t, e, o) => e in t ? Ga(t, e, { enumerable: !0, configurable: !0, writable: !0, value: o }) : t[e] = o;
var St = (t, e, o) => ja(t, typeof e != "symbol" ? e + "" : e, o);
import { BlockNoteSchema as Va, defaultBlockSpecs as Ka, BlockNoteEditor as qa } from "@blocknote/core";
import { jsxs as k, jsx as u, Fragment as ut } from "react/jsx-runtime";
import Za, { memo as de, useRef as lt, useState as tt, useEffect as mt, useCallback as it, Component as Ua, useMemo as Kt, useLayoutEffect as wi, useContext as Wr, createContext as ki, Suspense as Qa, lazy as Ja } from "react";
import { useCreateBlockNote as $a } from "@blocknote/react";
import { BlockNoteView as _a } from "@blocknote/mantine";
import "@blocknote/mantine/style.css";
import { createPortal as Ne, flushSync as tl } from "react-dom";
const el = "useandom-26T198340PX75pxJACKVERYMINDBUSHWOLF_GQZbfghjklqvwyzrict";
let gt = (t = 21) => {
  let e = "", o = crypto.getRandomValues(new Uint8Array(t |= 0));
  for (; t--; )
    e += el[o[t] & 63];
  return e;
};
const ol = {
  id: "kanban",
  label: "Kanban Board",
  nodes: [
    // Column frames
    { id: "kf1", type: "frame", x: 0, y: 0, w: 300, h: 500, z: 0, data: { label: "To Do", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "kf2", type: "frame", x: 330, y: 0, w: 300, h: 500, z: 0, data: { label: "In Progress", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    { id: "kf3", type: "frame", x: 660, y: 0, w: 300, h: 500, z: 0, data: { label: "Done", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    // To Do stickies
    { id: "ks1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 1", color: "#fecaca", fontSize: 14 } },
    { id: "ks2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Task 2", color: "#fecaca", fontSize: 14 } },
    { id: "ks3", type: "sticky", x: 30, y: 240, w: 240, h: 80, z: 1, data: { text: "Task 3", color: "#fecaca", fontSize: 14 } },
    // In Progress stickies
    { id: "ks4", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 4", color: "#fed7aa", fontSize: 14 } },
    { id: "ks5", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Task 5", color: "#fed7aa", fontSize: 14 } },
    // Done stickies
    { id: "ks6", type: "sticky", x: 690, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 6", color: "#bbf7d0", fontSize: 14 } }
  ]
}, rl = {
  id: "mind-map",
  label: "Mind Map",
  nodes: [
    // Central topic
    { id: "c0", type: "shape", x: 250, y: 200, w: 200, h: 80, z: 0, data: { shape: "ellipse", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "Main Topic" } },
    // Branch topics
    { id: "c1", type: "shape", x: 0, y: 0, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#ec4899", strokeWidth: 2, roughness: 0, fill: "#fdf2f8", fillStyle: "solid", label: "Idea 1" } },
    { id: "c2", type: "shape", x: 540, y: 0, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Idea 2" } },
    { id: "c3", type: "shape", x: 0, y: 400, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Idea 3" } },
    { id: "c4", type: "shape", x: 540, y: 400, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Idea 4" } },
    // Edges from center to branches
    { id: "e1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c1", style: "solid", color: "#ec4899", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c2", style: "solid", color: "#f59e0b", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c3", style: "solid", color: "#22c55e", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c4", style: "solid", color: "#3b82f6", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } }
  ]
}, nl = {
  id: "flowchart",
  label: "Flowchart",
  nodes: [
    // Start (ellipse)
    { id: "fc0", type: "shape", x: 200, y: 0, w: 160, h: 60, z: 0, data: { shape: "ellipse", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Start" } },
    // Process 1
    { id: "fc1", type: "shape", x: 200, y: 120, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Process" } },
    // Decision (diamond)
    { id: "fc2", type: "shape", x: 180, y: 250, w: 200, h: 100, z: 0, data: { shape: "diamond", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Decision?" } },
    // Yes branch
    { id: "fc3", type: "shape", x: 200, y: 420, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Action A" } },
    // No branch
    { id: "fc4", type: "shape", x: 480, y: 270, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Action B" } },
    // End
    { id: "fc5", type: "shape", x: 200, y: 540, w: 160, h: 60, z: 0, data: { shape: "ellipse", stroke: "#ef4444", strokeWidth: 2, roughness: 0, fill: "#fef2f2", fillStyle: "solid", label: "End" } },
    // Edges
    { id: "fe1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc0", toId: "fc1", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc1", toId: "fc2", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc2", toId: "fc3", style: "solid", color: "#22c55e", strokeWidth: 2, arrowHead: "filled", label: "Yes", sourceHandle: "bottom" } },
    { id: "fe4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc2", toId: "fc4", style: "solid", color: "#ef4444", strokeWidth: 2, arrowHead: "filled", label: "No", sourceHandle: "right" } },
    { id: "fe5", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc3", toId: "fc5", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe6", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc4", toId: "fc5", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } }
  ]
}, sl = {
  id: "swot",
  label: "SWOT Analysis",
  nodes: [
    // Quadrant frames
    { id: "sf1", type: "frame", x: 0, y: 0, w: 300, h: 300, z: 0, data: { label: "Strengths", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf2", type: "frame", x: 330, y: 0, w: 300, h: 300, z: 0, data: { label: "Weaknesses", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf3", type: "frame", x: 0, y: 330, w: 300, h: 300, z: 0, data: { label: "Opportunities", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf4", type: "frame", x: 330, y: 330, w: 300, h: 300, z: 0, data: { label: "Threats", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "ss1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Add strength...", color: "#bbf7d0", fontSize: 14 } },
    { id: "ss2", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Add weakness...", color: "#fecaca", fontSize: 14 } },
    { id: "ss3", type: "sticky", x: 30, y: 370, w: 240, h: 80, z: 1, data: { text: "Add opportunity...", color: "#bfdbfe", fontSize: 14 } },
    { id: "ss4", type: "sticky", x: 360, y: 370, w: 240, h: 80, z: 1, data: { text: "Add threat...", color: "#fed7aa", fontSize: 14 } }
  ]
}, il = {
  id: "retrospective",
  label: "Retrospective",
  nodes: [
    // Column frames
    { id: "rf1", type: "frame", x: 0, y: 0, w: 300, h: 500, z: 0, data: { label: "What went well", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "rf2", type: "frame", x: 330, y: 0, w: 300, h: 500, z: 0, data: { label: "What didn't go well", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "rf3", type: "frame", x: 660, y: 0, w: 300, h: 500, z: 0, data: { label: "Action items", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "rs1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Good thing 1", color: "#bbf7d0", fontSize: 14 } },
    { id: "rs2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Good thing 2", color: "#bbf7d0", fontSize: 14 } },
    { id: "rs3", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Issue 1", color: "#fecaca", fontSize: 14 } },
    { id: "rs4", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Issue 2", color: "#fecaca", fontSize: 14 } },
    { id: "rs5", type: "sticky", x: 690, y: 40, w: 240, h: 80, z: 1, data: { text: "Action 1", color: "#bfdbfe", fontSize: 14 } },
    { id: "rs6", type: "sticky", x: 690, y: 140, w: 240, h: 80, z: 1, data: { text: "Action 2", color: "#bfdbfe", fontSize: 14 } }
  ]
}, al = {
  id: "weekly-planner",
  label: "Weekly Planner",
  nodes: [
    // Day frames
    { id: "wf1", type: "frame", x: 0, y: 0, w: 170, h: 400, z: 0, data: { label: "Monday", backgroundColor: "#6366f115", borderColor: "#6366f1", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf2", type: "frame", x: 180, y: 0, w: 170, h: 400, z: 0, data: { label: "Tuesday", backgroundColor: "#8b5cf615", borderColor: "#8b5cf6", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf3", type: "frame", x: 360, y: 0, w: 170, h: 400, z: 0, data: { label: "Wednesday", backgroundColor: "#ec489915", borderColor: "#ec4899", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf4", type: "frame", x: 540, y: 0, w: 170, h: 400, z: 0, data: { label: "Thursday", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf5", type: "frame", x: 720, y: 0, w: 170, h: 400, z: 0, data: { label: "Friday", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf6", type: "frame", x: 900, y: 0, w: 170, h: 400, z: 0, data: { label: "Saturday", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf7", type: "frame", x: 1080, y: 0, w: 170, h: 400, z: 0, data: { label: "Sunday", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 1, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "ws1", type: "sticky", x: 10, y: 40, w: 150, h: 60, z: 1, data: { text: "Task", color: "#e0e7ff", fontSize: 12 } },
    { id: "ws2", type: "sticky", x: 190, y: 40, w: 150, h: 60, z: 1, data: { text: "Task", color: "#ede9fe", fontSize: 12 } }
  ]
}, ll = {
  id: "pros-cons",
  label: "Pros & Cons",
  nodes: [
    // Column frames
    { id: "pf1", type: "frame", x: 0, y: 0, w: 300, h: 450, z: 0, data: { label: "Pros", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "pf2", type: "frame", x: 330, y: 0, w: 300, h: 450, z: 0, data: { label: "Cons", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "ps1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Pro 1", color: "#bbf7d0", fontSize: 14 } },
    { id: "ps2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Pro 2", color: "#bbf7d0", fontSize: 14 } },
    { id: "ps3", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Con 1", color: "#fecaca", fontSize: 14 } },
    { id: "ps4", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Con 2", color: "#fecaca", fontSize: 14 } }
  ]
}, cl = {
  id: "eisenhower",
  label: "Eisenhower Matrix",
  nodes: [
    // Quadrant frames
    { id: "ef1", type: "frame", x: 0, y: 0, w: 300, h: 300, z: 0, data: { label: "Do First (Urgent + Important)", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef2", type: "frame", x: 330, y: 0, w: 300, h: 300, z: 0, data: { label: "Schedule (Important)", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef3", type: "frame", x: 0, y: 330, w: 300, h: 300, z: 0, data: { label: "Delegate (Urgent)", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef4", type: "frame", x: 330, y: 330, w: 300, h: 300, z: 0, data: { label: "Eliminate", backgroundColor: "#6b728015", borderColor: "#6b7280", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "es1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Critical task...", color: "#fecaca", fontSize: 14 } },
    { id: "es2", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Plan for later...", color: "#bfdbfe", fontSize: 14 } },
    { id: "es3", type: "sticky", x: 30, y: 370, w: 240, h: 80, z: 1, data: { text: "Hand off...", color: "#fed7aa", fontSize: 14 } },
    { id: "es4", type: "sticky", x: 360, y: 370, w: 240, h: 80, z: 1, data: { text: "Drop this...", color: "#e5e7eb", fontSize: 14 } }
  ]
}, dl = {
  id: "org-chart",
  label: "Org Chart",
  nodes: [
    // Top level
    { id: "oc0", type: "shape", x: 250, y: 0, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "CEO" } },
    // Second level
    { id: "oc1", type: "shape", x: 0, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Engineering" } },
    { id: "oc2", type: "shape", x: 250, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Product" } },
    { id: "oc3", type: "shape", x: 500, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Marketing" } },
    // Third level
    { id: "oc4", type: "shape", x: -60, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Frontend" } },
    { id: "oc5", type: "shape", x: 80, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Backend" } },
    { id: "oc6", type: "shape", x: 200, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Design" } },
    { id: "oc7", type: "shape", x: 360, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Research" } },
    { id: "oc8", type: "shape", x: 450, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Content" } },
    { id: "oc9", type: "shape", x: 590, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Growth" } },
    // Edges - top to second
    { id: "oe1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc1", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc2", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc3", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    // Edges - second to third
    { id: "oe4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc1", toId: "oc4", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe5", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc1", toId: "oc5", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe6", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc2", toId: "oc6", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe7", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc2", toId: "oc7", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe8", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc3", toId: "oc8", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe9", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc3", toId: "oc9", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } }
  ]
}, hl = {
  id: "timeline",
  label: "Project Timeline",
  nodes: [
    // Phase shapes
    { id: "tl0", type: "shape", x: 0, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "Discovery" } },
    { id: "tl1", type: "shape", x: 230, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Design" } },
    { id: "tl2", type: "shape", x: 460, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Development" } },
    { id: "tl3", type: "shape", x: 690, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Testing" } },
    { id: "tl4", type: "shape", x: 920, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#ec4899", strokeWidth: 2, roughness: 0, fill: "#fdf2f8", fillStyle: "solid", label: "Launch" } },
    // Milestone diamonds below
    { id: "tl5", type: "shape", x: 185, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#6366f1", fillStyle: "solid" } },
    { id: "tl6", type: "shape", x: 415, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#3b82f6", fillStyle: "solid" } },
    { id: "tl7", type: "shape", x: 645, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#22c55e", fillStyle: "solid" } },
    { id: "tl8", type: "shape", x: 875, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#f59e0b", fillStyle: "solid" } },
    // Phase arrows
    { id: "te1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl0", toId: "tl1", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl1", toId: "tl2", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl2", toId: "tl3", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl3", toId: "tl4", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    // Milestone labels
    { id: "tt1", type: "text", x: 155, y: 170, w: 100, h: "auto", z: 1, data: { text: "Kickoff", fontSize: 12, fontFamily: "sans-serif", color: "#6366f1", align: "center" } },
    { id: "tt2", type: "text", x: 385, y: 170, w: 100, h: "auto", z: 1, data: { text: "Specs Done", fontSize: 12, fontFamily: "sans-serif", color: "#3b82f6", align: "center" } },
    { id: "tt3", type: "text", x: 615, y: 170, w: 100, h: "auto", z: 1, data: { text: "Code Freeze", fontSize: 12, fontFamily: "sans-serif", color: "#22c55e", align: "center" } },
    { id: "tt4", type: "text", x: 845, y: 170, w: 100, h: "auto", z: 1, data: { text: "Sign-off", fontSize: 12, fontFamily: "sans-serif", color: "#f59e0b", align: "center" } }
  ]
}, ul = {
  id: "bmc",
  label: "Business Model Canvas",
  nodes: [
    // Top row: 5 sections
    { id: "bf1", type: "frame", x: 0, y: 0, w: 220, h: 320, z: 0, data: { label: "Key Partners", backgroundColor: "#8b5cf615", borderColor: "#8b5cf6", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf2", type: "frame", x: 230, y: 0, w: 220, h: 150, z: 0, data: { label: "Key Activities", backgroundColor: "#6366f115", borderColor: "#6366f1", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf3", type: "frame", x: 230, y: 160, w: 220, h: 160, z: 0, data: { label: "Key Resources", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf4", type: "frame", x: 460, y: 0, w: 220, h: 320, z: 0, data: { label: "Value Propositions", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf5", type: "frame", x: 690, y: 0, w: 220, h: 150, z: 0, data: { label: "Customer Relationships", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf6", type: "frame", x: 690, y: 160, w: 220, h: 160, z: 0, data: { label: "Channels", backgroundColor: "#ec489915", borderColor: "#ec4899", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf7", type: "frame", x: 920, y: 0, w: 220, h: 320, z: 0, data: { label: "Customer Segments", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 1, borderStyle: "solid" } },
    // Bottom row: 2 wide sections
    { id: "bf8", type: "frame", x: 0, y: 340, w: 565, h: 160, z: 0, data: { label: "Cost Structure", backgroundColor: "#6b728015", borderColor: "#6b7280", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf9", type: "frame", x: 575, y: 340, w: 565, h: 160, z: 0, data: { label: "Revenue Streams", backgroundColor: "#14b8a615", borderColor: "#14b8a6", borderWidth: 1, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "bs1", type: "sticky", x: 15, y: 40, w: 190, h: 60, z: 1, data: { text: "Partner...", color: "#ede9fe", fontSize: 12 } },
    { id: "bs2", type: "sticky", x: 475, y: 40, w: 190, h: 60, z: 1, data: { text: "Value prop...", color: "#bbf7d0", fontSize: 12 } },
    { id: "bs3", type: "sticky", x: 935, y: 40, w: 190, h: 60, z: 1, data: { text: "Segment...", color: "#fecaca", fontSize: 12 } }
  ]
}, pl = {
  id: "storyboard",
  label: "Storyboard",
  nodes: [
    // Row 1
    { id: "sb1", type: "frame", x: 0, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 1", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb2", type: "frame", x: 320, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 2", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb3", type: "frame", x: 640, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 3", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    // Row 2
    { id: "sb4", type: "frame", x: 0, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 4", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb5", type: "frame", x: 320, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 5", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb6", type: "frame", x: 640, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 6", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    // Caption text below each frame
    { id: "st1", type: "text", x: 0, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st2", type: "text", x: 320, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st3", type: "text", x: 640, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st4", type: "text", x: 0, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st5", type: "text", x: 320, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st6", type: "text", x: 640, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } }
  ]
}, vi = [
  ol,
  rl,
  nl,
  sl,
  il,
  al,
  ll,
  cl,
  dl,
  hl,
  ul,
  pl
];
class fl {
  constructor() {
    St(this, "undoStack", []);
    St(this, "redoStack", []);
    St(this, "maxSize", 50);
  }
  pushSnapshot(e, o) {
    const r = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (r.groupParent = Array.from(o.entries()));
    const n = JSON.stringify(r);
    this.undoStack.push(n), this.undoStack.length > this.maxSize && this.undoStack.shift(), this.redoStack = [];
  }
  undo(e, o) {
    if (this.undoStack.length === 0) return null;
    const r = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (r.groupParent = Array.from(o.entries())), this.redoStack.push(JSON.stringify(r));
    const n = JSON.parse(this.undoStack.pop());
    return {
      nodes: new Map(n.nodes),
      groupParent: new Map(n.groupParent ?? [])
    };
  }
  redo(e, o) {
    if (this.redoStack.length === 0) return null;
    const r = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (r.groupParent = Array.from(o.entries())), this.undoStack.push(JSON.stringify(r));
    const n = JSON.parse(this.redoStack.pop());
    return {
      nodes: new Map(n.nodes),
      groupParent: new Map(n.groupParent ?? [])
    };
  }
  clear() {
    this.undoStack = [], this.redoStack = [];
  }
  canUndo() {
    return this.undoStack.length > 0;
  }
  canRedo() {
    return this.redoStack.length > 0;
  }
}
const Si = 4, yl = 8, ml = 24;
function po(t, e, o, r) {
  if (!t.rotation) return [e, o];
  const n = t.x + t.w / 2, s = t.y + r / 2, i = -t.rotation * Math.PI / 180, l = Math.cos(i), a = Math.sin(i), c = e - n, h = o - s;
  return [n + c * l - h * a, s + c * a + h * l];
}
function zr(t, e) {
  return t.h !== "auto" ? t.h : (e == null ? void 0 : e[t.id]) ?? 100;
}
function gl(t, e, o, r = 1, n, s) {
  const i = Array.from(t.values()).filter((c) => c.type !== "edge").sort((c, h) => h.z - c.z);
  let l = null, a = null;
  for (const c of i)
    if (c.type === "draw") {
      if (Bn(c, e, o, r))
        return c;
    } else if (c.type === "shape") {
      if (Fr(c, e, o, r)) return c;
      if (!a && c.data.label) {
        const h = c.h === "auto" ? 100 : c.h, [p, d] = po(c, e, o, h), f = zi(c, h);
        f && p >= f.lx && p <= f.rx && d >= f.ly && d <= f.ry && (a = c);
      }
    } else if (s && s.has(c.type)) {
      const h = zr(c, n);
      Mi(c, e, o, r, h) && (l || (l = c));
    } else {
      const h = zr(c, n), p = Si / r, [d, f] = po(c, e, o, h);
      d >= c.x - p && d <= c.x + c.w + p && f >= c.y - p && f <= c.y + h + p && (a || (a = c));
    }
  return a ?? l;
}
function Mi(t, e, o, r, n) {
  const s = n ?? (t.h === "auto" ? 100 : t.h), [i, l] = po(t, e, o, s), a = yl / r;
  if (t.data.label && i >= t.x && i <= t.x + t.w && l >= t.y - ml && l <= t.y)
    return !0;
  if (i < t.x - a || i > t.x + t.w + a || l < t.y - a || l > t.y + s + a)
    return !1;
  const h = Math.abs(i - t.x), p = Math.abs(i - (t.x + t.w)), d = Math.abs(l - t.y), f = Math.abs(l - (t.y + s)), m = i >= t.x - a && i <= t.x + t.w + a;
  return l >= t.y - a && l <= t.y + s + a && (h <= a || p <= a) || m && (d <= a || f <= a);
}
function Ci(t, e, o, r, n, s) {
  const i = n - o, l = s - r, a = i * i + l * l;
  if (a === 0) return (t - o) ** 2 + (e - r) ** 2;
  const c = Math.max(0, Math.min(1, ((t - o) * i + (e - r) * l) / a)), h = o + c * i, p = r + c * l;
  return (t - h) ** 2 + (e - p) ** 2;
}
function zi(t, e) {
  const o = t.data;
  if (!o.label) return null;
  const r = o.labelFontSize ?? 14, n = r * 1.3, s = r * 0.55, l = t.w - 12 * 2, a = o.label.split(`
`);
  let c = 0;
  for (const m of a) {
    const y = m.length * s;
    c += Math.max(1, Math.ceil(y / Math.max(l, 1)));
  }
  const h = c * n, p = Math.min(l, Math.max(...a.map((m) => m.length)) * s), d = t.x + t.w / 2, f = t.y + e / 2;
  return {
    lx: d - p / 2 - 4,
    ly: f - h / 2 - 4,
    rx: d + p / 2 + 4,
    ry: f + h / 2 + 4
  };
}
function Fr(t, e, o, r, n) {
  const s = t.h === "auto" ? 100 : t.h, [i, l] = po(t, e, o, s), a = t.data, h = (a.strokeWidth ?? 2) / 2 / r, p = !!a.fill || !!n;
  switch (a.shape) {
    case "rect": {
      if (p)
        return i >= t.x - h && i <= t.x + t.w + h && l >= t.y - h && l <= t.y + s + h;
      const d = Math.abs(i - t.x), f = Math.abs(i - (t.x + t.w)), m = Math.abs(l - t.y), y = Math.abs(l - (t.y + s)), g = i >= t.x - h && i <= t.x + t.w + h;
      return l >= t.y - h && l <= t.y + s + h && (d <= h || f <= h) || g && (m <= h || y <= h);
    }
    case "ellipse": {
      const d = t.x + t.w / 2, f = t.y + s / 2, m = t.w / 2, y = s / 2;
      if (m === 0 || y === 0) return !1;
      const g = (i - d) / m, b = (l - f) / y, x = g * g + b * b;
      if (p) {
        const I = ((m + h) / m) ** 2;
        return x <= I;
      }
      const C = h / Math.min(m, y);
      return Math.abs(Math.sqrt(x) - 1) <= C;
    }
    case "diamond": {
      const d = t.x + t.w / 2, f = t.y + s / 2, m = t.w / 2, y = s / 2;
      if (m === 0 || y === 0) return !1;
      const g = Math.abs(i - d) / m, b = Math.abs(l - f) / y, x = g + b;
      if (p) {
        const I = h / Math.min(m, y);
        return x <= 1 + I;
      }
      const C = h / Math.min(m, y);
      return Math.abs(x - 1) <= C;
    }
    case "line":
    case "arrow": {
      const d = a.startPoint ?? [0, 0], f = a.endPoint ?? [t.w, s], m = t.x + d[0], y = t.y + d[1], g = t.x + f[0], b = t.y + f[1];
      return Ci(i, l, m, y, g, b) <= h * h;
    }
    default:
      return i >= t.x - h && i <= t.x + t.w + h && l >= t.y - h && l <= t.y + s + h;
  }
}
function bl(t, e, o) {
  let r = !1;
  for (let n = 0, s = o.length - 1; n < o.length; s = n++) {
    const i = o[n][0], l = o[n][1], a = o[s][0], c = o[s][1];
    l > e != c > e && t < (a - i) * (e - l) / (c - l) + i && (r = !r);
  }
  return r;
}
function Bn(t, e, o, r) {
  const s = t.data.strokeWidth / 2 / r, i = s * s, l = t.h === "auto" ? 100 : t.h, [a, c] = po(t, e, o, l);
  if (a < t.x - s || a > t.x + t.w + s || c < t.y - s || c > t.y + l + s)
    return !1;
  const h = t.data.points;
  if (!h || h.length === 0) return !1;
  const p = a - t.x, d = c - t.y;
  if (h.length === 1) {
    const f = p - h[0][0], m = d - h[0][1];
    return f * f + m * m <= i;
  }
  if (t.data.fill && h.length >= 3 && bl(p, d, h))
    return !0;
  for (let f = 0; f < h.length - 1; f++)
    if (Ci(p, d, h[f][0], h[f][1], h[f + 1][0], h[f + 1][1]) <= i)
      return !0;
  return !1;
}
function xl(t, e, o, r = 1, n, s) {
  const i = Array.from(t.values()).filter((c) => c.type !== "edge").sort((c, h) => h.z - c.z), l = [], a = [];
  for (const c of i)
    if (c.type === "draw")
      Bn(c, e, o, r) && l.push(c);
    else if (c.type === "shape") {
      if (Fr(c, e, o, r))
        l.push(c);
      else if (c.data.label) {
        const h = c.h === "auto" ? 100 : c.h, [p, d] = po(c, e, o, h), f = zi(c, h);
        f && p >= f.lx && p <= f.rx && d >= f.ly && d <= f.ry && a.push(c);
      }
    } else if (s && s.has(c.type)) {
      const h = zr(c, n);
      Mi(c, e, o, r, h) && a.push(c);
    } else {
      const h = zr(c, n), p = Si / r, [d, f] = po(c, e, o, h);
      d >= c.x - p && d <= c.x + c.w + p && f >= c.y - p && f <= c.y + h + p && a.push(c);
    }
  return [...l, ...a];
}
function fr(t, e) {
  if (!t.rotation) return { x: t.x, y: t.y, w: t.w, h: e };
  const o = t.x + t.w / 2, r = t.y + e / 2, n = t.w / 2, s = e / 2, i = t.rotation * Math.PI / 180, l = Math.abs(Math.cos(i)), a = Math.abs(Math.sin(i)), c = n * l + s * a, h = n * a + s * l;
  return {
    x: o - c,
    y: r - h,
    w: c * 2,
    h: h * 2
  };
}
const Te = class Te {
  constructor(e, o = 0, r) {
    // Increased depth for potentially large boards
    St(this, "level");
    St(this, "bounds");
    St(this, "objects");
    St(this, "nodes");
    /** Shared across all levels — maps node ID → measured height for auto-height nodes */
    St(this, "heightMap");
    this.bounds = e, this.level = o, this.objects = [], this.nodes = [], this.heightMap = r ?? /* @__PURE__ */ new Map();
  }
  /** Resolve the effective height for a node (uses measured height for auto-height nodes) */
  resolveH(e) {
    return typeof e.h == "number" ? e.h : this.heightMap.get(e.id) ?? 100;
  }
  /** Store a measured height for an auto-height node */
  setMeasuredHeight(e, o) {
    this.heightMap.set(e, o);
  }
  // Clear the quadtree
  clear() {
    this.objects = [];
    for (let e = 0; e < this.nodes.length; e++)
      this.nodes[e].clear();
    this.nodes = [];
  }
  // Split the node into 4 subnodes
  split() {
    const e = this.bounds.w / 2, o = this.bounds.h / 2, r = this.bounds.x, n = this.bounds.y;
    this.nodes[0] = new Te({ x: r + e, y: n, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[1] = new Te({ x: r, y: n, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[2] = new Te({ x: r, y: n + o, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[3] = new Te({ x: r + e, y: n + o, w: e, h: o }, this.level + 1, this.heightMap);
  }
  // Determine which quadrant the object belongs to
  getIndex(e) {
    let o = -1;
    const r = this.bounds.x + this.bounds.w / 2, n = this.bounds.y + this.bounds.h / 2, s = e.y < n && e.y + e.h < n, i = e.y > n;
    return e.x < r && e.x + e.w < r ? s ? o = 1 : i && (o = 2) : e.x > r && (s ? o = 0 : i && (o = 3)), o;
  }
  // Insert the object into the quadtree
  insert(e, o) {
    const r = o ?? this.resolveH(e);
    o != null && e.h === "auto" && this.heightMap.set(e.id, o);
    const n = fr(e, r);
    if (this.nodes.length) {
      const s = this.getIndex(n);
      if (s !== -1) {
        this.nodes[s].insert(e, r);
        return;
      }
    }
    if (this.objects.push(e), this.objects.length > Te.MAX_OBJECTS && this.level < Te.MAX_LEVELS) {
      this.nodes.length || this.split();
      let s = 0;
      for (; s < this.objects.length; ) {
        const i = this.objects[s], l = this.resolveH(i), a = fr(i, l), c = this.getIndex(a);
        c !== -1 ? (this.nodes[c].insert(i, l), this.objects.splice(s, 1)) : s++;
      }
    }
  }
  // Remove an object. Requires the node (with its coordinates) to find it efficiently.
  remove(e) {
    const o = this.objects.findIndex((r) => r.id === e.id);
    if (o !== -1)
      return this.objects.splice(o, 1), !0;
    if (this.nodes.length) {
      const r = this.resolveH(e), n = this.getIndex(fr(e, r));
      if (n !== -1 && this.nodes[n].remove(e))
        return !0;
      for (let s = 0; s < this.nodes.length; s++)
        if (s !== n && this.nodes[s].remove(e)) return !0;
    }
    return !1;
  }
  // Return all objects that could collide with the given rect
  retrieve(e, o) {
    const r = this.getIndex(o);
    for (const n of this.objects) {
      const s = this.resolveH(n), i = fr(n, s);
      i.x < o.x + o.w && i.x + i.w > o.x && i.y < o.y + o.h && i.y + i.h > o.y && e.push(n);
    }
    if (this.nodes.length)
      if (r !== -1)
        this.nodes[r].retrieve(e, o);
      else
        for (const n of this.nodes)
          n.bounds.x < o.x + o.w && n.bounds.x + n.bounds.w > o.x && n.bounds.y < o.y + o.h && n.bounds.y + n.bounds.h > o.y && n.retrieve(e, o);
    return e;
  }
};
// Max number of objects per node before splitting
St(Te, "MAX_OBJECTS", 10), // Max depth of the tree
St(Te, "MAX_LEVELS", 8);
let xn = Te;
function Do(t, e, o) {
  return Math.min(Math.max(t, e), o);
}
function Wo(t, e, o) {
  return {
    x: (e - t.x) / t.zoom,
    y: (o - t.y) / t.zoom
  };
}
function wl(t, e, o) {
  return {
    x: e * t.zoom + t.x,
    y: o * t.zoom + t.y
  };
}
function kl(t, e, o, r) {
  const n = e > 0 ? 0.95 : 1.05, s = Do(t.zoom * n, 0.1, 5), i = Wo(t, o, r);
  return {
    x: o - i.x * s,
    y: r - i.y * s,
    zoom: s
  };
}
function vl(t, e, o, r) {
  const n = Do(t.zoom * e, 0.1, 5), s = Wo(t, o, r);
  return {
    x: o - s.x * n,
    y: r - s.y * n,
    zoom: n
  };
}
const Nn = Va.create({
  blockSpecs: {
    ...Ka
    // Future custom blocks:
    // callout: CalloutBlock,
    // aiPrompt: AIPromptBlock,
  }
});
let Jr = null;
function Hn() {
  return Jr || (Jr = qa.create({ schema: Nn })), Jr;
}
async function Sl(t) {
  return await Hn().blocksToMarkdownLossy(t);
}
async function On(t) {
  return await Hn().tryParseMarkdownToBlocks(t);
}
function Ii(t) {
  return Hn().tryParseHTMLToBlocks(t);
}
function Ml(t, e, o) {
  const [r, n] = t, [s, i] = e, [l, a] = o, c = l - s, h = a - i, p = c * c + h * h;
  if (p === 0)
    return (r - s) ** 2 + (n - i) ** 2;
  let d = ((r - s) * c + (n - i) * h) / p;
  d = Math.max(0, Math.min(1, d));
  const f = s + d * c, m = i + d * h;
  return (r - f) ** 2 + (n - m) ** 2;
}
function wn(t, e = 1) {
  if (t.length <= 2) return t;
  let o = 0, r = 0;
  const n = t[0], s = t[t.length - 1];
  for (let a = 1; a < t.length - 1; a++) {
    const c = Ml(t[a], n, s);
    c > o && (o = c, r = a);
  }
  if (o <= e)
    return [n, s];
  const i = wn(t.slice(0, r + 1), e), l = wn(t.slice(r), e);
  return [...i.slice(0, -1), ...l];
}
async function Cl(t, e) {
  const o = [], r = ['canvas_w="2000"', 'canvas_h="1500"', 'grid="20"', 'snap="false"'];
  if (e != null && e.background && e.background !== "dot-grid" && r.push(`background="${e.background}"`), e != null && e.originView) {
    const d = e.originView;
    r.push(`originView="${d.x},${d.y},${d.zoom}"`);
  }
  o.push(`<!--@meta ${r.join(" ")} -->`), o.push("");
  const n = t.filter((d) => d.type === "frame").sort((d, f) => d.z - f.z || d.y - f.y || d.x - f.x);
  for (const d of n) {
    const f = d.h === "auto" ? "auto" : Math.round(d.h), m = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${f}"`,
      `z="${d.z}"`
    ];
    d.data.label && m.push(`label="${d.data.label.replace(/"/g, "&quot;")}"`), d.data.backgroundColor && m.push(`backgroundColor="${d.data.backgroundColor}"`), d.data.borderColor && m.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && m.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && m.push(`borderStyle="${d.data.borderStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && m.push(`opacity="${d.data.opacity}"`), d.data.slideOrder != null && m.push(`slideOrder="${d.data.slideOrder}"`), d.data.transition && d.data.transition !== "pan" && m.push(`transition="${d.data.transition}"`), d.data.transitionDuration != null && m.push(`transitionDuration="${d.data.transitionDuration}"`), d.rotation && m.push(`rotation="${d.rotation}"`), d.locked && m.push('locked="true"'), d.groupId && m.push(`group="${d.groupId}"`), o.push(`<!--@frame ${m.join(" ")} -->`), o.push("");
  }
  const s = t.filter((d) => d.type === "content").sort((d, f) => d.z - f.z || d.y - f.y || d.x - f.x);
  for (const d of s) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${d.h}"`,
      `z="${d.z}"`
    ];
    d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), d.data.borderColor && f.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && f.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && f.push(`borderStyle="${d.data.borderStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), o.push(`<!--@block ${f.join(" ")} -->`);
    const m = d.data.blocks.length > 0 ? await Sl(d.data.blocks) : "";
    o.push(m), o.push("");
  }
  const i = t.filter((d) => d.type === "draw");
  for (const d of i) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `z="${d.z}"`,
      `tool="${d.data.tool}"`,
      `color="${d.data.color}"`,
      `width="${d.data.strokeWidth}"`
    ];
    d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.data.fill && f.push(`fill="${d.data.fill}"`), d.data.fillStyle && d.data.fillStyle !== "hachure" && f.push(`fillStyle="${d.data.fillStyle}"`), d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@draw ${f.join(" ")} -->`);
    const y = wn([...d.data.points], 1).map(
      ([g, b, x]) => `${(g + d.x).toFixed(1)},${(b + d.y).toFixed(1)},${x.toFixed(2)}`
    ).join(" ");
    o.push(y), o.push("");
  }
  const l = t.filter((d) => d.type === "shape");
  for (const d of l) {
    const f = d.h === "auto" ? "auto" : Math.round(d.h), m = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${f}"`,
      `z="${d.z}"`,
      'tool="shape"',
      `shape="${d.data.shape}"`,
      `color="${d.data.stroke}"`,
      `stroke="${d.data.strokeWidth}"`,
      `roughness="${d.data.roughness}"`
    ];
    d.data.fill && m.push(`fill="${d.data.fill}"`), d.data.fillStyle && d.data.fillStyle !== "hachure" && m.push(`fillStyle="${d.data.fillStyle}"`), d.data.strokeStyle && d.data.strokeStyle !== "solid" && m.push(`strokeStyle="${d.data.strokeStyle}"`), d.data.edgeStyle && d.data.edgeStyle !== "sharp" && m.push(`edgeStyle="${d.data.edgeStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && m.push(`opacity="${d.data.opacity}"`), d.data.startPoint && m.push(`startPt="${d.data.startPoint[0].toFixed(1)},${d.data.startPoint[1].toFixed(1)}"`), d.data.endPoint && m.push(`endPt="${d.data.endPoint[0].toFixed(1)},${d.data.endPoint[1].toFixed(1)}"`), d.data.label && m.push(`label="${d.data.label.replace(/"/g, "&quot;")}"`), d.data.labelFontSize && m.push(`labelFontSize="${d.data.labelFontSize}"`), d.data.labelFontFamily && d.data.labelFontFamily !== "Excalifont" && m.push(`labelFontFamily="${d.data.labelFontFamily}"`), d.data.labelAlign && d.data.labelAlign !== "center" && m.push(`labelAlign="${d.data.labelAlign}"`), d.rotation && m.push(`rotation="${d.rotation}"`), d.locked && m.push('locked="true"'), d.groupId && m.push(`group="${d.groupId}"`), o.push(`<!--@draw ${m.join(" ")} -->`), o.push("");
  }
  const a = t.filter((d) => d.type === "text");
  for (const d of a) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `z="${d.z}"`,
      `fontSize="${d.data.fontSize}"`,
      `fontFamily="${d.data.fontFamily}"`,
      `color="${d.data.color}"`,
      `align="${d.data.align}"`
    ];
    d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@text ${f.join(" ")} -->`), o.push(d.data.text), o.push("");
  }
  const c = t.filter((d) => d.type === "image");
  for (const d of c) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${Math.round(d.h)}"`,
      `z="${d.z}"`,
      `src="${d.data.src.replace(/"/g, "&quot;")}"`
    ];
    d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), d.data.alt && f.push(`alt="${d.data.alt.replace(/"/g, "&quot;")}"`), d.data.opacity != null && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.data.borderColor && f.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && f.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && f.push(`borderStyle="${d.data.borderStyle}"`), o.push(`<!--@image ${f.join(" ")} -->`), o.push("");
  }
  const h = t.filter((d) => d.type === "edge");
  for (const d of h) {
    const f = [
      `id="${d.id}"`,
      `from="${d.data.fromId}"`,
      `to="${d.data.toId}"`,
      `style="${d.data.style}"`,
      `color="${d.data.color}"`
    ];
    d.data.label && f.push(`label="${d.data.label}"`), d.data.strokeWidth && d.data.strokeWidth !== 1 && f.push(`strokeWidth="${d.data.strokeWidth}"`), d.data.arrowHead && d.data.arrowHead !== "none" && f.push(`arrowHead="${d.data.arrowHead}"`), d.data.arrowTail && d.data.arrowTail !== "none" && f.push(`arrowTail="${d.data.arrowTail}"`), d.data.arrowHeadSize && f.push(`arrowHeadSize="${d.data.arrowHeadSize}"`), d.data.arrowTailSize && f.push(`arrowTailSize="${d.data.arrowTailSize}"`), d.data.edgeType && d.data.edgeType !== "bezier" && f.push(`edgeType="${d.data.edgeType}"`), d.data.animated && f.push('animated="true"'), d.data.animatedDirection && d.data.animatedDirection !== "forward" && f.push(`animatedDirection="${d.data.animatedDirection}"`), d.data.sourceHandle && f.push(`sourceHandle="${d.data.sourceHandle}"`), d.data.targetHandle && f.push(`targetHandle="${d.data.targetHandle}"`), d.data.midpointOffset != null && d.data.midpointOffset !== 0.5 && f.push(`midpointOffset="${d.data.midpointOffset}"`), d.data.curveOffset && (d.data.curveOffset[0] !== 0 || d.data.curveOffset[1] !== 0) && f.push(`curveOffset="${d.data.curveOffset[0]},${d.data.curveOffset[1]}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@edge ${f.join(" ")} -->`), o.push("");
  }
  const p = t.filter((d) => d.type === "sticky");
  for (const d of p) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${d.h}"`,
      `z="${d.z}"`,
      `color="${d.data.color}"`
    ];
    d.data.fontSize && d.data.fontSize !== 16 && f.push(`fontSize="${d.data.fontSize}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@sticky ${f.join(" ")} -->`), o.push(d.data.text), o.push("");
  }
  return o.join(`
`);
}
const Ti = "data:font/woff2;base64,", Ze = "Excalifont", Sr = [
  // Bundled — hand-drawn default
  { key: "Excalifont", label: "Excalifont", category: "hand" },
  // System
  { key: "sans-serif", label: "Sans (system)", category: "system" },
  { key: "serif", label: "Serif (system)", category: "system" },
  { key: "monospace", label: "Mono (system)", category: "system" },
  // Google — Professional
  { key: "Inter", label: "Inter", category: "sans" },
  { key: "Roboto", label: "Roboto", category: "sans" },
  { key: "Open Sans", label: "Open Sans", category: "sans" },
  { key: "Lora", label: "Lora", category: "serif" },
  { key: "Playfair Display", label: "Playfair Display", category: "serif" },
  { key: "Merriweather", label: "Merriweather", category: "serif" },
  // Google — Mono
  { key: "JetBrains Mono", label: "JetBrains Mono", category: "mono" },
  { key: "Fira Code", label: "Fira Code", category: "mono" },
  { key: "Source Code Pro", label: "Source Code Pro", category: "mono" },
  // Google — Handwritten / Casual
  { key: "Caveat", label: "Caveat", category: "hand" },
  { key: "Shadows Into Light", label: "Shadows Into Light", category: "hand" },
  { key: "Dancing Script", label: "Dancing Script", category: "hand" },
  { key: "Amatic SC", label: "Amatic SC", category: "hand" },
  // Google — Display / Funky
  { key: "Pacifico", label: "Pacifico", category: "display" },
  { key: "Lobster", label: "Lobster", category: "display" },
  { key: "Permanent Marker", label: "Permanent Marker", category: "display" },
  { key: "Bangers", label: "Bangers", category: "display" },
  { key: "Righteous", label: "Righteous", category: "display" },
  { key: "Satisfy", label: "Satisfy", category: "display" },
  { key: "Kaushan Script", label: "Kaushan Script", category: "display" },
  { key: "Fredericka the Great", label: "Fredericka the Great", category: "display" },
  { key: "Comfortaa", label: "Comfortaa", category: "display" }
], Pi = /* @__PURE__ */ new Set(["sans-serif", "serif", "monospace"]), zl = /* @__PURE__ */ new Set(["Excalifont"]), Il = /* @__PURE__ */ new Set([...Pi, ...zl]);
function Tl(t) {
  switch (t) {
    case "serif":
    case "display":
      return "H";
    case "mono":
      return "</>";
    case "hand":
      return "✏";
    default:
      return "A";
  }
}
function Ue(t) {
  return Pi.has(t) ? t : `'${t}', sans-serif`;
}
let ks = !1;
function Pl(t = document) {
  if (ks) return;
  ks = !0;
  const e = t.createElement("style");
  e.textContent = `
@font-face {
  font-family: 'Excalifont';
  src: url('${Ti}') format('woff2');
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}`, t.head.appendChild(e);
  const o = Sr.filter((n) => !Il.has(n.key)).map((n) => "family=" + n.key.replace(/ /g, "+")).join("&"), r = t.createElement("link");
  r.rel = "stylesheet", r.href = `https://fonts.googleapis.com/css2?${o}&display=swap`, t.head.appendChild(r);
}
function Xe(t) {
  const e = {}, o = /(\w+)="([^"]*)"/g;
  let r;
  for (; (r = o.exec(t)) !== null; )
    e[r[1]] = r[2];
  return e;
}
const Rl = {
  default: "dot-grid",
  "cutting-board": "blueprint",
  // Removed grid-as-paper types → nearest color equivalent
  "graph-paper": "plain-white",
  "college-ruled": "plain-white",
  isometric: "plain-white"
};
async function Al(t) {
  var s, i;
  const e = [], o = {}, r = t.split(`
`);
  let n = 0;
  for (; n < r.length; ) {
    const l = r[n].trim();
    if (l.startsWith("<!--@meta")) {
      const a = Xe(l);
      if (a.background) {
        const c = Rl[a.background] ?? a.background;
        o.background = c;
      }
      if (a.originView) {
        const c = a.originView.split(",").map(Number);
        c.length === 3 && c.every((h) => !isNaN(h)) && (o.originView = { x: c[0], y: c[1], zoom: c[2] });
      }
      n++;
      continue;
    }
    if (l.startsWith("<!--@frame")) {
      const a = Xe(l);
      for (n++; n < r.length && r[n].trim() === ""; ) n++;
      e.push({
        id: a.id || gt(10),
        type: "frame",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "400"),
        h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          label: ((s = a.label) == null ? void 0 : s.replace(/&quot;/g, '"')) || void 0,
          backgroundColor: a.backgroundColor || void 0,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0,
          slideOrder: a.slideOrder ? parseInt(a.slideOrder, 10) : void 0,
          transition: a.transition || void 0,
          transitionDuration: a.transitionDuration ? parseInt(a.transitionDuration, 10) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@block")) {
      const a = Xe(l);
      n++;
      const c = [];
      for (; n < r.length && !r[n].trim().startsWith("<!--@"); )
        c.push(r[n]), n++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      const h = c.join(`
`), p = h.trim().length > 0 ? await On(h) : [];
      e.push({
        id: a.id || gt(10),
        type: "content",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "300"),
        h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
        z: parseInt(a.z || "1"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          blocks: p,
          markdown: h,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@draw")) {
      const a = Xe(l);
      if (n++, a.tool === "shape")
        for (e.push({
          id: a.id || gt(10),
          type: "shape",
          x: parseFloat(a.x || "0"),
          y: parseFloat(a.y || "0"),
          w: parseFloat(a.w || "100"),
          h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
          z: parseInt(a.z || "0"),
          rotation: a.rotation ? parseFloat(a.rotation) : void 0,
          locked: a.locked === "true" || void 0,
          groupId: a.group || void 0,
          data: {
            shape: a.shape || "rect",
            stroke: a.color || "#1e1e2e",
            fill: a.fill || void 0,
            fillStyle: a.fillStyle || void 0,
            strokeWidth: parseFloat(a.stroke || "2"),
            strokeStyle: a.strokeStyle || void 0,
            edgeStyle: a.edgeStyle || void 0,
            roughness: parseFloat(a.roughness || "1"),
            opacity: a.opacity ? parseFloat(a.opacity) : void 0,
            startPoint: a.startPt ? a.startPt.split(",").map(Number) : void 0,
            endPoint: a.endPt ? a.endPt.split(",").map(Number) : void 0,
            label: ((i = a.label) == null ? void 0 : i.replace(/&quot;/g, '"')) || void 0,
            labelFontSize: a.labelFontSize ? parseFloat(a.labelFontSize) : void 0,
            labelFontFamily: a.labelFontFamily || void 0,
            labelAlign: a.labelAlign || void 0
          }
        }); n < r.length && r[n].trim() === ""; ) n++;
      else {
        let c = "";
        n < r.length && !r[n].trim().startsWith("<!--@") && (c = r[n].trim(), n++);
        const h = c ? c.split(" ").filter(Boolean).map((g) => {
          const b = g.split(",").map(Number);
          return [
            b[0] || 0,
            b[1] || 0,
            b[2] || 0.5
          ];
        }) : [];
        let p = 1 / 0, d = 1 / 0, f = -1 / 0, m = -1 / 0;
        for (const [g, b] of h)
          g < p && (p = g), b < d && (d = b), g > f && (f = g), b > m && (m = b);
        isFinite(p) || (p = parseFloat(a.x || "0"), d = parseFloat(a.y || "0"), f = p, m = d);
        const y = h.map(
          ([g, b, x]) => [g - p, b - d, x]
        );
        for (e.push({
          id: a.id || gt(10),
          type: "draw",
          x: p,
          y: d,
          w: f - p,
          h: m - d,
          z: parseInt(a.z || "0"),
          rotation: a.rotation ? parseFloat(a.rotation) : void 0,
          locked: a.locked === "true" || void 0,
          groupId: a.group || void 0,
          data: {
            tool: a.tool || "pen",
            points: y,
            color: a.color || "#1e1e2e",
            strokeWidth: parseFloat(a.width || "2"),
            opacity: a.opacity ? parseFloat(a.opacity) : void 0,
            fill: a.fill || void 0,
            fillStyle: a.fillStyle || void 0
          }
        }); n < r.length && r[n].trim() === ""; ) n++;
      }
      continue;
    }
    if (l.startsWith("<!--@image")) {
      const a = Xe(l);
      n++, e.push({
        id: a.id || gt(10),
        type: "image",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: parseFloat(a.h || "150"),
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          src: a.src || "",
          alt: a.alt,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@edge")) {
      const a = Xe(l);
      for (n++, e.push({
        id: a.id || gt(10),
        type: "edge",
        x: 0,
        y: 0,
        w: 0,
        h: 0,
        z: 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          fromId: a.from || "",
          toId: a.to || "",
          label: a.label,
          style: a.style || "solid",
          color: a.color || "#666",
          strokeWidth: a.strokeWidth ? parseFloat(a.strokeWidth) : 1,
          arrowHead: a.arrowHead || void 0,
          arrowTail: a.arrowTail || void 0,
          arrowHeadSize: a.arrowHeadSize ? parseFloat(a.arrowHeadSize) : void 0,
          arrowTailSize: a.arrowTailSize ? parseFloat(a.arrowTailSize) : void 0,
          edgeType: a.edgeType || void 0,
          animated: a.animated === "true" || void 0,
          animatedDirection: a.animatedDirection || void 0,
          sourceHandle: a.sourceHandle || void 0,
          targetHandle: a.targetHandle || void 0,
          midpointOffset: a.midpointOffset ? parseFloat(a.midpointOffset) : void 0,
          curveOffset: a.curveOffset ? a.curveOffset.split(",").map(Number) : void 0
        }
      }); n < r.length && r[n].trim() === ""; ) n++;
      continue;
    }
    if (l.startsWith("<!--@text")) {
      const a = Xe(l);
      n++;
      const c = [];
      for (; n < r.length && !r[n].trim().startsWith("<!--@"); )
        c.push(r[n]), n++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      e.push({
        id: a.id || gt(10),
        type: "text",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: "auto",
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          text: c.join(`
`),
          fontSize: parseFloat(a.fontSize || "20"),
          fontFamily: a.fontFamily || Ze,
          color: a.color || "#1e1e2e",
          align: a.align || "left",
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@sticky")) {
      const a = Xe(l);
      n++;
      const c = [];
      for (; n < r.length && !r[n].trim().startsWith("<!--@"); )
        c.push(r[n]), n++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      e.push({
        id: a.id || gt(10),
        type: "sticky",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: parseFloat(a.h || "150"),
        z: parseInt(a.z || "1"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          text: c.join(`
`),
          color: a.color || "#FEF3C7",
          fontSize: a.fontSize ? parseFloat(a.fontSize) : void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    n++;
  }
  return { nodes: e, meta: o };
}
function fo(t, e) {
  return t.h !== "auto" ? t.h : (e == null ? void 0 : e[t.id]) ?? 100;
}
const Ll = 14;
function or(t, e, o, r, n) {
  const s = e.find((d) => d.id === o);
  if (!s) return null;
  const i = fo(t, n), l = Ll / r, a = e.filter((d) => d.direction === s.direction), c = a.indexOf(s);
  if (c < 0) return null;
  const h = t.y + i / (a.length + 1) * (c + 1), p = s.direction === "input" ? t.x - l : t.x + t.w + l;
  if (t.rotation) {
    const d = t.x + t.w / 2, f = t.y + i / 2, m = t.rotation * Math.PI / 180, y = Math.cos(m), g = Math.sin(m), b = p - d, x = h - f;
    return { x: d + b * y - x * g, y: f + b * g + x * y };
  }
  return { x: p, y: h };
}
function vs(t, e, o, r, n, s, i, l) {
  const a = i - n, c = l - s;
  if (a === 0 && c === 0) return { x: n, y: s, side: "right" };
  let h = 1 / 0, p = n, d = s, f = "right";
  if (a !== 0) {
    const m = (t + o - n) / a;
    if (m > 0 && m < h) {
      const y = s + m * c;
      y >= e && y <= e + r && (h = m, p = t + o, d = y, f = "right");
    }
  }
  if (a !== 0) {
    const m = (t - n) / a;
    if (m > 0 && m < h) {
      const y = s + m * c;
      y >= e && y <= e + r && (h = m, p = t, d = y, f = "left");
    }
  }
  if (c !== 0) {
    const m = (e + r - s) / c;
    if (m > 0 && m < h) {
      const y = n + m * a;
      y >= t && y <= t + o && (h = m, p = y, d = e + r, f = "bottom");
    }
  }
  if (c !== 0) {
    const m = (e - s) / c;
    if (m > 0 && m < h) {
      const y = n + m * a;
      y >= t && y <= t + o && (h = m, p = y, d = e, f = "top");
    }
  }
  return { x: p, y: d, side: f };
}
function yo(t, e, o, r, n) {
  const s = Math.cos(n), i = Math.sin(n), l = t - o, a = e - r;
  return [o + l * s - a * i, r + l * i + a * s];
}
function Dl(t, e, o, r) {
  const n = t.x + t.w / 2, s = t.y + e / 2;
  if (!t.rotation)
    return vs(t.x, t.y, t.w, e, n, s, o, r);
  const i = -t.rotation * Math.PI / 180, [l, a] = yo(o, r, n, s, i), c = vs(t.x, t.y, t.w, e, n, s, l, a), [h, p] = yo(c.x, c.y, n, s, -i);
  return { x: h, y: p, side: c.side };
}
function Ri(t, e, o, r) {
  return Math.abs(t) / o >= Math.abs(e) / r ? t >= 0 ? "right" : "left" : e >= 0 ? "bottom" : "top";
}
function El(t, e, o, r) {
  const n = t.x + t.w / 2, s = t.y + e / 2, i = t.w / 2, l = e / 2, a = t.rotation ? -t.rotation * Math.PI / 180 : 0, [c, h] = t.rotation ? yo(o, r, n, s, a) : [o, r], p = c - n, d = h - s;
  if (p === 0 && d === 0)
    return { x: n + i, y: s, side: "right" };
  const f = 1 / Math.sqrt((p / i) ** 2 + (d / l) ** 2);
  let m = n + p * f, y = s + d * f;
  const g = Ri(p, d, i, l);
  return t.rotation && ([m, y] = yo(m, y, n, s, -a)), { x: m, y, side: g };
}
function Wl(t, e, o, r) {
  const n = t.x + t.w / 2, s = t.y + e / 2, i = t.w / 2, l = e / 2, a = t.rotation ? -t.rotation * Math.PI / 180 : 0, [c, h] = t.rotation ? yo(o, r, n, s, a) : [o, r], p = c - n, d = h - s;
  if (p === 0 && d === 0)
    return { x: n + i, y: s, side: "right" };
  const f = 1 / (Math.abs(p) / i + Math.abs(d) / l);
  let m = n + p * f, y = s + d * f;
  const g = Ri(p, d, i, l);
  return t.rotation && ([m, y] = yo(m, y, n, s, -a)), { x: m, y, side: g };
}
function kn(t, e, o, r) {
  var n;
  if (t.type === "shape") {
    const s = (n = t.data) == null ? void 0 : n.shape;
    if (s === "ellipse") return El(t, e, o, r);
    if (s === "diamond") return Wl(t, e, o, r);
  }
  return Dl(t, e, o, r);
}
function vn(t, e, o, r) {
  const n = kn(t, e, o, r);
  return { x: n.x, y: n.y };
}
function Sn(t, e, o) {
  const r = t.x + t.w / 2, n = t.y + e / 2;
  let s, i;
  switch (o) {
    case "top":
      s = r, i = t.y;
      break;
    case "bottom":
      s = r, i = t.y + e;
      break;
    case "left":
      s = t.x, i = n;
      break;
    case "right":
      s = t.x + t.w, i = n;
      break;
  }
  if (!t.rotation) return { x: s, y: i };
  const l = t.rotation * Math.PI / 180, [a, c] = yo(s, i, r, n, l);
  return { x: a, y: c };
}
function Ho(t) {
  switch (t) {
    case "top":
      return { dx: 0, dy: -1 };
    case "bottom":
      return { dx: 0, dy: 1 };
    case "left":
      return { dx: -1, dy: 0 };
    case "right":
      return { dx: 1, dy: 0 };
  }
}
function Ss(t) {
  var o;
  if (t.type !== "shape") return !1;
  const e = (o = t.data) == null ? void 0 : o.shape;
  return e === "ellipse" || e === "diamond";
}
function Ke(t, e, o = "bezier", r, n, s, i, l, a, c) {
  const h = fo(t, r), p = fo(e, r), d = t.x + t.w / 2, f = t.y + h / 2, m = e.x + e.w / 2, y = e.y + p / 2;
  let g, b, x, C;
  if (a)
    g = a.x, b = a.y, x = n ?? "right";
  else if (n) {
    const P = Sn(t, h, n);
    g = P.x, b = P.y, x = n;
  } else {
    const P = kn(t, h, m, y);
    if (g = P.x, b = P.y, x = P.side, Ss(t)) {
      const G = Math.hypot(m - d, y - f);
      G > 0 && (C = { dx: (m - d) / G, dy: (y - f) / G });
    }
  }
  let I, M, D, T;
  if (c)
    I = c.x, M = c.y, D = s ?? "left";
  else if (s) {
    const P = Sn(e, p, s);
    I = P.x, M = P.y, D = s;
  } else {
    const P = kn(e, p, d, f);
    if (I = P.x, M = P.y, D = P.side, Ss(e)) {
      const G = Math.hypot(d - m, f - y);
      G > 0 && (T = { dx: (d - m) / G, dy: (f - y) / G });
    }
  }
  switch (o) {
    case "straight":
      return Fl(g, b, I, M, x, D);
    case "bezier":
      return Bl(g, b, I, M, x, D, l, C, T);
    case "smoothstep":
      return Nl(g, b, I, M, x, D, i);
    case "step":
      return Hl(g, b, I, M, x, D, i);
  }
}
function Fl(t, e, o, r, n, s) {
  const i = Math.min(t, o), l = Math.min(e, r), a = Math.abs(o - t), c = Math.abs(r - e);
  return {
    path: `M${t},${e} L${o},${r}`,
    labelX: (t + o) / 2,
    labelY: (e + r) / 2,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: Math.atan2(r - e, o - t),
    tailAngle: Math.atan2(e - r, t - o),
    sourceSide: n,
    targetSide: s,
    bounds: { x: i, y: l, w: a, h: c }
  };
}
function Bl(t, e, o, r, n, s, i, l, a) {
  const c = Math.hypot(o - t, r - e), h = Math.min(c * 0.5, Math.max(50, c * 0.25)), p = l ?? Ho(n), d = a ?? Ho(s), f = i ? i[0] * (4 / 3) : 0, m = i ? i[1] * (4 / 3) : 0, y = t + p.dx * h + f, g = e + p.dy * h + m, b = o + d.dx * h + f, x = r + d.dy * h + m, C = 0.125 * t + 0.375 * y + 0.375 * b + 0.125 * o, I = 0.125 * e + 0.375 * g + 0.375 * x + 0.125 * r, M = Math.atan2(r - x, o - b), D = Math.atan2(e - g, t - y), T = {
    x: C,
    y: I,
    axis: "xy",
    min: 0,
    max: 0
  }, P = Math.min(t, o, y, b), G = Math.min(e, r, g, x), K = Math.max(t, o, y, b), at = Math.max(e, r, g, x);
  return {
    path: `M${t},${e} C${y},${g} ${b},${x} ${o},${r}`,
    labelX: C,
    labelY: I,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: M,
    tailAngle: D,
    sourceSide: n,
    targetSide: s,
    kinkHandle: T,
    bounds: { x: P, y: G, w: K - P, h: at - G }
  };
}
function Nl(t, e, o, r, n, s, i) {
  const { points: c, kinkHandle: h } = Xn(t, e, o, r, n, s, 20, i), p = Ol(c, 8), d = Math.floor(c.length / 2), f = (c[d - 1][0] + c[d][0]) / 2, m = (c[d - 1][1] + c[d][1]) / 2, y = c[c.length - 1], g = c[c.length - 2], b = Math.atan2(y[1] - g[1], y[0] - g[0]), x = c[0], C = c[1], I = Math.atan2(x[1] - C[1], x[0] - C[0]);
  let M = 1 / 0, D = 1 / 0, T = -1 / 0, P = -1 / 0;
  for (const [G, K] of c)
    G < M && (M = G), K < D && (D = K), G > T && (T = G), K > P && (P = K);
  return {
    path: p,
    labelX: f,
    labelY: m,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: b,
    tailAngle: I,
    sourceSide: n,
    targetSide: s,
    kinkHandle: h,
    bounds: { x: M, y: D, w: T - M, h: P - D }
  };
}
function Hl(t, e, o, r, n, s, i) {
  const { points: a, kinkHandle: c } = Xn(t, e, o, r, n, s, 20, i), h = [`M${a[0][0]},${a[0][1]}`];
  for (let P = 1; P < a.length; P++)
    h.push(`L${a[P][0]},${a[P][1]}`);
  const p = Math.floor(a.length / 2), d = (a[p - 1][0] + a[p][0]) / 2, f = (a[p - 1][1] + a[p][1]) / 2, m = a[a.length - 1], y = a[a.length - 2], g = Math.atan2(m[1] - y[1], m[0] - y[0]), b = a[0], x = a[1], C = Math.atan2(b[1] - x[1], b[0] - x[0]);
  let I = 1 / 0, M = 1 / 0, D = -1 / 0, T = -1 / 0;
  for (const [P, G] of a)
    P < I && (I = P), G < M && (M = G), P > D && (D = P), G > T && (T = G);
  return {
    path: h.join(" "),
    labelX: d,
    labelY: f,
    x1: t,
    y1: e,
    x2: o,
    y2: r,
    arrowAngle: g,
    tailAngle: C,
    sourceSide: n,
    targetSide: s,
    kinkHandle: c,
    bounds: { x: I, y: M, w: D - I, h: T - M }
  };
}
function Xn(t, e, o, r, n, s, i, l) {
  const a = Ho(n), c = Ho(s), h = t + a.dx * i, p = e + a.dy * i, d = o + c.dx * i, f = r + c.dy * i, m = n === "left" || n === "right", y = s === "left" || s === "right", g = [[t, e], [h, p]], b = l ?? 0.5;
  let x;
  if (m && y) {
    const C = h + (d - h) * b;
    g.push([C, p], [C, f]);
    const I = Math.min(h, d), M = Math.max(h, d);
    x = { x: C, y: (p + f) / 2, axis: "x", min: I, max: M };
  } else if (!m && !y) {
    const C = p + (f - p) * b;
    g.push([h, C], [d, C]);
    const I = Math.min(p, f), M = Math.max(p, f);
    x = { x: (h + d) / 2, y: C, axis: "y", min: I, max: M };
  } else m && !y ? g.push([d, p]) : g.push([h, f]);
  return g.push([d, f], [o, r]), { points: g, kinkHandle: x };
}
function Ol(t, e) {
  if (t.length < 2) return "";
  if (t.length === 2) return `M${t[0][0]},${t[0][1]} L${t[1][0]},${t[1][1]}`;
  const o = [`M${t[0][0]},${t[0][1]}`];
  for (let n = 1; n < t.length - 1; n++) {
    const s = t[n - 1], i = t[n], l = t[n + 1], a = i[0] - s[0], c = i[1] - s[1], h = l[0] - i[0], p = l[1] - i[1], d = Math.hypot(a, c), f = Math.hypot(h, p);
    if (d === 0 || f === 0) {
      o.push(`L${i[0]},${i[1]}`);
      continue;
    }
    const m = Math.min(e, d / 2, f / 2), y = i[0] - a / d * m, g = i[1] - c / d * m, b = i[0] + h / f * m, x = i[1] + p / f * m;
    o.push(`L${y},${g}`), o.push(`Q${i[0]},${i[1]} ${b},${x}`);
  }
  const r = t[t.length - 1];
  return o.push(`L${r[0]},${r[1]}`), o.join(" ");
}
function Xl(t, e, o, r, n, s, i, l, a) {
  const c = 1 - a, h = c * c, p = h * c, d = a * a, f = d * a;
  return [
    p * t + 3 * h * a * o + 3 * c * d * n + f * i,
    p * e + 3 * h * a * r + 3 * c * d * s + f * l
  ];
}
function Yl(t, e, o, r, n, s, i, l, a, c, h = 24) {
  let p = 1 / 0, d = o, f = r;
  for (let m = 1; m <= h; m++) {
    const y = m / h, [g, b] = Xl(o, r, n, s, i, l, a, c, y), x = Yn(t, e, d, f, g, b);
    x < p && (p = x), d = g, f = b;
  }
  return p;
}
function Gl(t, e, o) {
  let r = 1 / 0;
  for (let n = 1; n < o.length; n++) {
    const s = Yn(t, e, o[n - 1][0], o[n - 1][1], o[n][0], o[n][1]);
    s < r && (r = s);
  }
  return r;
}
function Ai(t, e, o, r, n, s, i, l) {
  const a = n.data.edgeType || "bezier", c = Ke(
    o,
    r,
    a,
    s,
    n.data.sourceHandle,
    n.data.targetHandle,
    n.data.midpointOffset,
    n.data.curveOffset,
    i,
    l
  ), { x1: h, y1: p, x2: d, y2: f } = c;
  if (a === "straight")
    return Yn(t, e, h, p, d, f);
  if (a === "bezier") {
    const g = Math.hypot(d - h, f - p), b = Math.min(g * 0.5, Math.max(50, g * 0.25)), x = Ho(c.sourceSide), C = Ho(c.targetSide), I = n.data.curveOffset ? n.data.curveOffset[0] * (4 / 3) : 0, M = n.data.curveOffset ? n.data.curveOffset[1] * (4 / 3) : 0, D = h + x.dx * b + I, T = p + x.dy * b + M, P = d + C.dx * b + I, G = f + C.dy * b + M;
    return Yl(t, e, h, p, D, T, P, G, d, f);
  }
  const m = 20, { points: y } = Xn(h, p, d, f, c.sourceSide, c.targetSide, m, n.data.midpointOffset);
  return Gl(t, e, y);
}
function $r(t, e, o) {
  const r = fo(t, o), n = fo(e, o), s = t.x + t.w / 2, i = t.y + r / 2, l = e.x + e.w / 2, a = e.y + n / 2, c = vn(t, r, l, a), h = vn(e, n, s, i);
  return { x1: c.x, y1: c.y, x2: h.x, y2: h.y };
}
function _r(t, e, o, r) {
  const n = fo(t, r);
  return vn(t, n, e, o);
}
function Yn(t, e, o, r, n, s) {
  const i = n - o, l = s - r, a = i * i + l * l;
  if (a === 0) return Math.hypot(t - o, e - r);
  const c = Math.max(0, Math.min(1, ((t - o) * i + (e - r) * l) / a)), h = o + c * i, p = r + c * l;
  return Math.hypot(t - h, e - p);
}
function jl(t, e, o, r, n) {
  const s = (l, a) => l >= n.x && l <= n.x + n.w && a >= n.y && a <= n.y + n.h;
  if (s(t, e) || s(o, r)) return !0;
  const i = [
    [n.x, n.y, n.x + n.w, n.y],
    // top
    [n.x, n.y + n.h, n.x + n.w, n.y + n.h],
    // bottom
    [n.x, n.y, n.x, n.y + n.h],
    // left
    [n.x + n.w, n.y, n.x + n.w, n.y + n.h]
    // right
  ];
  for (const [l, a, c, h] of i)
    if (Vl(t, e, o, r, l, a, c, h)) return !0;
  return !1;
}
function Vl(t, e, o, r, n, s, i, l) {
  const a = o - t, c = r - e, h = i - n, p = l - s, d = a * p - c * h;
  if (Math.abs(d) < 1e-10) return !1;
  const f = n - t, m = s - e, y = (f * p - m * h) / d, g = (f * c - m * a) / d;
  return y >= 0 && y <= 1 && g >= 0 && g <= 1;
}
function Eo(t, e, o, r) {
  const n = Math.cos(o), s = Math.sin(o), i = -s, l = n, a = r / 2, c = t + n * a, h = e + s * a, p = t - n * a, d = e - s * a, f = r * 0.4;
  return `M${p + i * f},${d + l * f} L${c},${h} L${p - i * f},${d - l * f}`;
}
function Ir(t, e, o, r) {
  const n = Math.cos(o), s = Math.sin(o), i = -s, l = n, a = r / 2, c = t + n * a, h = e + s * a, p = t - n * a, d = e - s * a, f = r * 0.4;
  return `M${c},${h} L${p + i * f},${d + l * f} L${p - i * f},${d - l * f} Z`;
}
function Fo(t, e) {
  const o = fo(t, e);
  return ["top", "right", "bottom", "left"].map((n) => {
    const s = Sn(t, o, n);
    return { side: n, x: s.x, y: s.y };
  });
}
function yr(t, e, o, r) {
  const n = Fo(t, r);
  let s = n[0], i = 1 / 0;
  for (const l of n) {
    const a = Math.hypot(l.x - e, l.y - o);
    a < i && (i = a, s = l);
  }
  return s.side;
}
function Kl(t, e, o, r, n, s) {
  const i = 8 / r, l = [];
  for (const a of t.values()) {
    if (a.type !== "edge") continue;
    const c = a, h = t.get(c.data.fromId), p = t.get(c.data.toId);
    if (!h || !p) continue;
    const d = s == null ? void 0 : s(c, h, p);
    Ai(e, o, h, p, c, n, d == null ? void 0 : d.sourcePortPos, d == null ? void 0 : d.targetPortPos) < i && l.push(a);
  }
  return l;
}
function ql(t, e, o, r, n, s) {
  const i = 8 / r;
  let l = null, a = i;
  for (const c of t.values()) {
    if (c.type !== "edge") continue;
    const h = c, p = t.get(h.data.fromId), d = t.get(h.data.toId);
    if (!p || !d) continue;
    const f = s == null ? void 0 : s(h, p, d), m = Ai(e, o, p, d, h, n, f == null ? void 0 : f.sourcePortPos, f == null ? void 0 : f.targetPortPos);
    m < a && (a = m, l = c);
  }
  return l;
}
function Zl(t, e, o) {
  const r = t.x, n = t.x + t.w / 2, s = t.x + t.w, i = t.y, l = t.y + t.h / 2, a = t.y + t.h, c = [r, n, s], h = [i, l, a];
  let p = 1 / 0, d = 1 / 0;
  const f = [];
  for (const y of e) {
    const g = y.x, b = y.x + y.w / 2, x = y.x + y.w, C = y.y, I = y.y + y.h / 2, M = y.y + y.h, D = [g, b, x], T = [C, I, M];
    for (const P of c)
      for (const G of D) {
        const K = G - P;
        Math.abs(K) <= o && (Math.abs(K) < Math.abs(p) && (p = K), f.push({
          axis: "x",
          position: G,
          start: Math.min(t.y, t.y + t.h, y.y, y.y + y.h),
          end: Math.max(t.y, t.y + t.h, y.y, y.y + y.h)
        }));
      }
    for (const P of h)
      for (const G of T) {
        const K = G - P;
        Math.abs(K) <= o && (Math.abs(K) < Math.abs(d) && (d = K), f.push({
          axis: "y",
          position: G,
          start: Math.min(t.x, t.x + t.w, y.x, y.x + y.w),
          end: Math.max(t.x, t.x + t.w, y.x, y.x + y.w)
        }));
      }
  }
  const m = /* @__PURE__ */ new Map();
  for (const y of f) {
    const g = `${y.axis}:${y.position.toFixed(1)}`, b = m.get(g);
    b ? (b.start = Math.min(b.start, y.start), b.end = Math.max(b.end, y.end)) : m.set(g, { ...y });
  }
  return {
    guides: Array.from(m.values()),
    snapDx: Math.abs(p) <= o ? p : 0,
    snapDy: Math.abs(d) <= o ? d : 0
  };
}
class Ul {
  constructor() {
    St(this, "nodes", /* @__PURE__ */ new Map());
    St(this, "viewport", { x: 0, y: 0, zoom: 1 });
    St(this, "selection", /* @__PURE__ */ new Set());
    St(this, "activeGroupId", null);
    St(this, "groupRotations", /* @__PURE__ */ new Map());
    /** Maps child groupId → parent groupId for nested groups. */
    St(this, "groupParent", /* @__PURE__ */ new Map());
    /** Reverse index: parent groupId → set of child groupIds. Maintained alongside groupParent. */
    St(this, "groupChildren", /* @__PURE__ */ new Map());
    St(this, "mode", "select");
    St(this, "activeTool", {
      tool: "pen",
      color: "#1e1e2e",
      width: 2.5,
      shapeType: "rect",
      strokeStyle: "solid",
      roughness: 1,
      opacity: 1
    });
    St(this, "containerOffset", { x: 0, y: 0 });
    /** DOM element that hosts the canvas — used to derive the correct window in pop-out scenarios. */
    St(this, "_container", null);
    St(this, "snapToGrid", !1);
    St(this, "smartGuides", !0);
    St(this, "lassoSelect", !1);
    St(this, "presentationMode", !1);
    St(this, "presentationSlides", []);
    St(this, "presentationIndex", 0);
    St(this, "_presentationAnimId", null);
    /** Transition overlay state — consumed by PresentationOverlay for visual effects. */
    St(this, "_transitionOverlay", null);
    St(this, "gridSize", 20);
    St(this, "boardBackground", "dot-grid");
    /** Saved "origin" viewport position restored on next load. */
    St(this, "originView", null);
    /** Current alignment guides (set during drag). */
    St(this, "alignGuides", []);
    /** Container dimensions for viewport bounds computation. */
    St(this, "_containerWidth", 2e3);
    St(this, "_containerHeight", 1500);
    St(this, "history", new fl());
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    St(this, "listeners", {});
    St(this, "_suppressEvents", !1);
    St(this, "_collabMode", !1);
    St(this, "clipboard", []);
    St(this, "pasteCount", 0);
    St(this, "nextZValue", 1);
    St(this, "_minZ", 0);
    St(this, "quadTree", new xn({ x: -1e5, y: -1e5, w: 2e5, h: 2e5 }));
    St(this, "adjacency", /* @__PURE__ */ new Map());
    /** Explicit frame→children tracking. Only nodes intentionally placed inside a frame are tracked. */
    St(this, "frameChildren", /* @__PURE__ */ new Map());
    /** Node types that act as containers (frame-like behavior). "frame" is always included. */
    St(this, "_containerTypes", /* @__PURE__ */ new Set(["frame"]));
    St(this, "registry");
    /** Measured heights for auto-height nodes (canvas-coordinate units). */
    St(this, "_measuredHeights", {});
  }
  get transitionOverlay() {
    return this._transitionOverlay;
  }
  /** Set the node type registry for lifecycle hooks. */
  setRegistry(e) {
    this.registry = e;
  }
  /** Enable collaborative mode. Disables local snapshot history. */
  setCollabMode(e) {
    this._collabMode = e, e && this.history.clear();
  }
  /** Whether the engine is in collaborative mode. */
  get isCollabMode() {
    return this._collabMode;
  }
  /** Register a node type as a container (frame-like behavior). */
  registerContainerType(e) {
    this._containerTypes.add(e);
  }
  /** Check whether a node type behaves as a container. */
  isContainerType(e) {
    return this._containerTypes.has(e);
  }
  /** The set of container type strings (read-only). */
  get containerTypes() {
    return this._containerTypes;
  }
  /** Update the measured height for an auto-height node. */
  setMeasuredHeight(e, o) {
    this._measuredHeights[e] = o;
  }
  /** Get the resolved height for a node (measured or explicit). */
  resolveHeight(e) {
    return e.h !== "auto" ? e.h : this._measuredHeights[e.id] ?? 100;
  }
  /** Get all measured heights (for canvas rendering). */
  get measuredHeights() {
    return this._measuredHeights;
  }
  // --- Edge helpers (data-flow) ---
  /** Get all edge nodes connected to a given node. */
  getEdgesForNode(e) {
    const o = this.adjacency.get(e);
    if (!o) return [];
    const r = [];
    for (const n of o) {
      const s = this.nodes.get(n);
      s && s.type === "edge" && r.push(s);
    }
    return r;
  }
  /** Get all edge nodes in the board. */
  getAllEdges() {
    const e = [];
    for (const o of this.nodes.values())
      o.type === "edge" && e.push(o);
    return e;
  }
  /** Set the container element (used by SpatialCanvas on mount). */
  setContainer(e) {
    this._container = e;
  }
  /** Get the window object for the container (supports pop-out windows). */
  getWindow() {
    var e;
    return ((e = this._container) == null ? void 0 : e.ownerDocument.defaultView) ?? window;
  }
  // --- Event Emitter ---
  on(e, o) {
    this.listeners[e] || (this.listeners[e] = /* @__PURE__ */ new Set()), this.listeners[e].add(o);
  }
  off(e, o) {
    var r;
    (r = this.listeners[e]) == null || r.delete(o);
  }
  emit(e, ...o) {
    var r;
    this._suppressEvents || (r = this.listeners[e]) == null || r.forEach((n) => n(...o));
  }
  /** Request entering image crop mode (handled by the canvas component). */
  requestImageCrop(e) {
    this.emit("image:cropRequest", e);
  }
  // --- Grid Snapping ---
  toggleSnapToGrid() {
    this.snapToGrid = !this.snapToGrid, this.emit("guides");
  }
  toggleSmartGuides() {
    this.smartGuides = !this.smartGuides, this.emit("guides");
  }
  setGridSize(e) {
    const o = Math.max(1, Math.round(e));
    this.gridSize !== o && (this.gridSize = o, this.emit("guides"));
  }
  toggleLassoSelect() {
    this.lassoSelect = !this.lassoSelect, this.emit("lassoToggle");
  }
  // ── Presentation mode ─────────────────────────────────────────
  enterPresentation() {
    const e = [];
    for (const a of this.nodes.values())
      if (a.type === "frame") {
        const c = a.data;
        e.push({ id: a.id, x: a.x, y: a.y, order: c.slideOrder });
      }
    if (e.length === 0) return;
    const o = e.filter((a) => a.order != null).sort((a, c) => a.order - c.order), r = e.filter((a) => a.order == null), n = 100;
    r.sort((a, c) => a.y - c.y);
    const s = [];
    for (const a of r) {
      const c = s[s.length - 1];
      c && Math.abs(a.y - c[0].y) < n ? c.push(a) : s.push([a]);
    }
    const i = s.flatMap((a) => a.sort((c, h) => c.x - h.x)), l = [...o, ...i];
    this.presentationSlides = l.map((a) => a.id), this.presentationIndex = 0, this.presentationMode = !0, this.selection.size > 0 && (this.selection.clear(), this.emit("selection")), this.emit("presentation"), this.presentationGoTo(0);
  }
  exitPresentation() {
    this._presentationAnimId != null && (cancelAnimationFrame(this._presentationAnimId), this._presentationAnimId = null), this._transitionOverlay = null, this.presentationMode = !1, this.presentationSlides = [], this.presentationIndex = 0, this.emit("presentation");
  }
  presentationNext() {
    this.presentationIndex < this.presentationSlides.length - 1 && this.presentationGoTo(this.presentationIndex + 1);
  }
  presentationPrev() {
    this.presentationIndex > 0 && this.presentationGoTo(this.presentationIndex - 1);
  }
  presentationGoTo(e) {
    if (e < 0 || e >= this.presentationSlides.length) return;
    const o = this.presentationSlides[e], r = this.nodes.get(o);
    if (!r) {
      this.exitPresentation();
      return;
    }
    const n = this.presentationIndex;
    this.presentationIndex = e, this.emit("presentation"), this._presentationAnimId != null && (cancelAnimationFrame(this._presentationAnimId), this._presentationAnimId = null), this._transitionOverlay = null;
    const s = this._computeSlideViewport(r), i = r.data, l = i.transition ?? "pan", a = i.transitionDuration, c = e >= n ? 1 : -1;
    switch (l) {
      case "none":
        this._transitionNone(s);
        break;
      case "fade":
        this._transitionFade(s, a);
        break;
      case "dissolve":
        this._transitionDissolve(s, a);
        break;
      case "zoom":
        this._transitionZoom(s, a);
        break;
      case "fold":
        this._transitionFold(s, a);
        break;
      case "cube":
        this._transitionCube(s, a, c);
        break;
      case "pan":
      default:
        this._transitionPan(s, a);
        break;
    }
  }
  _computeSlideViewport(e) {
    const o = this.resolveHeight(e), r = 40, n = e.x - r, s = e.y - r, i = e.w + r * 2, l = o + r * 2, a = this._containerWidth, c = this._containerHeight, h = Do(Math.min(a / i, c / l), 0.1, 5);
    return {
      x: (a - i * h) / 2 - n * h,
      y: (c - l * h) / 2 - s * h,
      zoom: h
    };
  }
  /** Pan transition: smooth viewport interpolation (default). */
  _transitionPan(e, o) {
    const r = o ?? 400, n = performance.now(), s = { ...this.viewport }, i = (l) => {
      const a = Math.min((l - n) / r, 1), c = 1 - Math.pow(1 - a, 3);
      this.viewport.x = s.x + (e.x - s.x) * c, this.viewport.y = s.y + (e.y - s.y) * c, this.viewport.zoom = s.zoom + (e.zoom - s.zoom) * c, this.emit("viewport"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : this._presentationAnimId = null;
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** None transition: instant viewport snap. */
  _transitionNone(e) {
    this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport");
  }
  /** Fade transition: fade to black, snap viewport, fade from black. */
  _transitionFade(e, o) {
    const r = (o ?? 500) / 2, n = performance.now(), s = (i) => {
      const l = Math.min((i - n) / r, 1);
      if (this._transitionOverlay = { type: "fade", phase: "out", progress: l }, this.emit("presentation"), l < 1)
        this._presentationAnimId = requestAnimationFrame(s);
      else {
        this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport");
        const a = performance.now(), c = (h) => {
          const p = Math.min((h - a) / r, 1);
          this._transitionOverlay = { type: "fade", phase: "in", progress: p }, this.emit("presentation"), p < 1 ? this._presentationAnimId = requestAnimationFrame(c) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
        };
        this._presentationAnimId = requestAnimationFrame(c);
      }
    };
    this._presentationAnimId = requestAnimationFrame(s);
  }
  /** Dissolve transition: quick overlay fade, snap viewport at midpoint. */
  _transitionDissolve(e, o) {
    const r = o ?? 400, n = performance.now();
    let s = !1;
    const i = (l) => {
      const a = Math.min((l - n) / r, 1);
      a < 0.5 ? this._transitionOverlay = { type: "dissolve", phase: "out", progress: a * 2 } : (s || (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), s = !0), this._transitionOverlay = { type: "dissolve", phase: "in", progress: (a - 0.5) * 2 }), this.emit("presentation"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** Zoom transition: zoom out from current, zoom into target. */
  _transitionZoom(e, o) {
    const r = o ?? 600, n = performance.now(), s = { ...this.viewport }, i = Math.max(0.1, Math.min(s.zoom, e.zoom) * 0.35), l = (s.x + e.x) / 2, a = (s.y + e.y) / 2, c = (h) => {
      const p = Math.min((h - n) / r, 1);
      if (p < 0.5) {
        const d = p * 2, f = 1 - Math.pow(1 - d, 3);
        this.viewport.x = s.x + (l - s.x) * f, this.viewport.y = s.y + (a - s.y) * f, this.viewport.zoom = s.zoom + (i - s.zoom) * f;
      } else {
        const d = (p - 0.5) * 2, f = 1 - Math.pow(1 - d, 3);
        this.viewport.x = l + (e.x - l) * f, this.viewport.y = a + (e.y - a) * f, this.viewport.zoom = i + (e.zoom - i) * f;
      }
      this.emit("viewport"), p < 1 ? this._presentationAnimId = requestAnimationFrame(c) : this._presentationAnimId = null;
    };
    this._presentationAnimId = requestAnimationFrame(c);
  }
  /** Fold transition: two halves fold shut like a book, snap viewport, unfold to reveal. */
  _transitionFold(e, o) {
    const r = o ?? 700, n = performance.now();
    let s = !1;
    const i = (l) => {
      const a = Math.min((l - n) / r, 1);
      a < 0.5 ? this._transitionOverlay = { type: "fold", phase: "out", progress: a * 2 } : (s || (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), s = !0), this._transitionOverlay = { type: "fold", phase: "in", progress: (a - 0.5) * 2 }), this.emit("presentation"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** Cube transition: zoom out → 3D rotate → zoom in, snap viewport at midpoint. */
  _transitionCube(e, o, r = 1) {
    const n = o ?? 1200, s = performance.now();
    let i = !1;
    const l = (a) => {
      const c = Math.min((a - s) / n, 1);
      c >= 0.5 && !i && (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), i = !0), this._transitionOverlay = {
        type: "cube",
        phase: c < 0.5 ? "out" : "in",
        progress: c < 0.5 ? c * 2 : (c - 0.5) * 2,
        direction: r,
        t: c
      }, this.emit("presentation"), c < 1 ? this._presentationAnimId = requestAnimationFrame(l) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(l);
  }
  snap(e, o) {
    return this.snapToGrid ? {
      x: Math.round(e / this.gridSize) * this.gridSize,
      y: Math.round(o / this.gridSize) * this.gridSize
    } : { x: e, y: o };
  }
  /** Update the container dimensions (called from canvas resize observer). */
  setContainerSize(e, o) {
    const r = this._containerWidth, n = this._containerHeight;
    this._containerWidth = e, this._containerHeight = o, this.presentationMode && this.presentationSlides.length > 0 ? this.presentationGoTo(this.presentationIndex) : r > 0 && n > 0 && (this.viewport.x += (e - r) / 2, this.viewport.y += (o - n) / 2, this.emit("viewport"));
  }
  /**
   * Compute smart guide alignment + grid snap for a drag operation.
   * Sets `this.alignGuides` and emits `guides` event.
   * Returns the adjusted delta to apply.
   */
  computeDragSnap(e, o, r, n, s) {
    const i = this.snapToGrid && !s, l = this.smartGuides && !s;
    let a = r, c = n, h = [];
    const p = o instanceof Set ? o : new Set(o);
    if (l) {
      let d = 1 / 0, f = 1 / 0, m = -1 / 0, y = -1 / 0;
      for (const P of e) {
        const G = this.getNode(P.id);
        if (!G) continue;
        const K = P.x + r, at = P.y + n, B = this.resolveHeight(G);
        d = Math.min(d, K), f = Math.min(f, at), m = Math.max(m, K + G.w), y = Math.max(y, at + B);
      }
      const g = { x: d, y: f, w: m - d, h: y - f }, b = -this.viewport.x / this.viewport.zoom, x = -this.viewport.y / this.viewport.zoom, C = this._containerWidth / this.viewport.zoom, I = this._containerHeight / this.viewport.zoom, M = [], D = this.quadTree.retrieve([], { x: b, y: x, w: C, h: I });
      for (const P of D) {
        if (P.type === "edge" || p.has(P.id)) continue;
        const G = this.resolveHeight(P);
        M.push({ x: P.x, y: P.y, w: P.w, h: G });
      }
      const T = Zl(g, M, 5);
      if (h = T.guides, i) {
        const P = e[0].x + r, G = e[0].y + n, K = this.snap(P, G), at = K.x - P, B = K.y - G, et = T.snapDx !== 0 && Math.abs(T.snapDx) <= Math.abs(at), Z = T.snapDy !== 0 && Math.abs(T.snapDy) <= Math.abs(B);
        a = r + (et ? T.snapDx : at), c = n + (Z ? T.snapDy : B), et || (h = h.filter((Q) => Q.axis !== "x")), Z || (h = h.filter((Q) => Q.axis !== "y"));
      } else
        a = r + T.snapDx, c = n + T.snapDy;
    } else if (i) {
      const d = this.snap(e[0].x + r, e[0].y + n);
      a = d.x - e[0].x, c = d.y - e[0].y;
    }
    return this.alignGuides = h, this.emit("guides"), { finalDx: a, finalDy: c };
  }
  /** Clear alignment guides (call on drag end). */
  clearAlignGuides() {
    this.alignGuides.length !== 0 && (this.alignGuides = [], this.emit("guides"));
  }
  // --- Board Background ---
  setBoardBackground(e) {
    this.boardBackground !== e && (this.boardBackground = e, this.emit("background"));
  }
  // --- Viewport ---
  pan(e, o) {
    this.viewport.x += e, this.viewport.y += o, this.emit("viewport");
  }
  zoomByWheel(e, o, r) {
    this.viewport = kl(
      this.viewport,
      e,
      o - this.containerOffset.x,
      r - this.containerOffset.y
    ), this.emit("viewport");
  }
  zoomByFactor(e, o, r) {
    this.viewport = vl(
      this.viewport,
      e,
      o - this.containerOffset.x,
      r - this.containerOffset.y
    ), this.emit("viewport");
  }
  zoomTo(e, o) {
    const r = Do(e, 0.1, 5);
    if (o) {
      const n = o.x - this.containerOffset.x, s = o.y - this.containerOffset.y, i = Wo(this.viewport, n, s);
      this.viewport = {
        x: n - i.x * r,
        y: s - i.y * r,
        zoom: r
      };
    } else
      this.viewport.zoom = r;
    this.emit("viewport");
  }
  zoomIn() {
    this.zoomTo(this.viewport.zoom * 1.2);
  }
  zoomOut() {
    this.zoomTo(this.viewport.zoom / 1.2);
  }
  /** Zoom and pan to center a node for editing (e.g. after double-click on placeholder) */
  zoomToNode(e, o = 1) {
    const r = this.nodes.get(e);
    if (!r) return;
    const n = r.h === "auto" ? 100 : r.h, s = r.x + r.w / 2, i = r.y + n / 2, l = this.getWindow(), a = l.innerWidth, c = l.innerHeight, h = Do(o, 0.2, 5);
    this.viewport = {
      x: a / 2 - s * h,
      y: c / 2 - i * h,
      zoom: h
    }, this.emit("viewport");
  }
  fitToContent() {
    if (this.nodes.size === 0) return;
    let e = 1 / 0, o = 1 / 0, r = -1 / 0, n = -1 / 0;
    for (const p of this.nodes.values()) {
      const d = p.h === "auto" ? 100 : p.h;
      p.x < e && (e = p.x), p.y < o && (o = p.y), p.x + p.w > r && (r = p.x + p.w), p.y + d > n && (n = p.y + d);
    }
    const s = 50;
    e -= s, o -= s, r += s, n += s;
    const i = r - e, l = n - o, a = this._containerWidth, c = this._containerHeight, h = Do(
      Math.min(a / i, c / l),
      0.1,
      5
    );
    this.viewport = {
      x: (a - i * h) / 2 - e * h,
      y: (c - l * h) / 2 - o * h,
      zoom: h
    }, this.emit("viewport");
  }
  /** Save the current viewport as the origin view (restored on next load). */
  setOriginView() {
    this.originView = { ...this.viewport }, this.emit("background");
  }
  /** Clear the saved origin view. */
  clearOriginView() {
    this.originView = null, this.emit("background");
  }
  /** Jump to the saved origin view, or fit-to-content if none is saved. */
  goToOriginView() {
    this.originView ? (this.viewport = { ...this.originView }, this.emit("viewport")) : this.fitToContent();
  }
  screenToCanvas(e, o) {
    return Wo(
      this.viewport,
      e - this.containerOffset.x,
      o - this.containerOffset.y
    );
  }
  canvasToScreen(e, o) {
    return wl(this.viewport, e, o);
  }
  // --- Node CRUD ---
  addNode(e) {
    var o, r, n;
    if (this.history.pushSnapshot(this.nodes, this.groupParent), this.nodes.set(e.id, e), this.quadTree.insert(e), e.z < this._minZ && (this._minZ = e.z), e.type === "edge") {
      const s = e, { fromId: i, toId: l } = s.data;
      this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.has(l) || this.adjacency.set(l, /* @__PURE__ */ new Set()), this.adjacency.get(i).add(e.id), this.adjacency.get(l).add(e.id);
    }
    e.type !== "edge" && this.updateFrameMembership([e.id]), (n = (r = (o = this.registry) == null ? void 0 : o.get(e.type)) == null ? void 0 : r.onCreate) == null || n.call(r, e, this), this.emit("node:create", e), this.emit("change"), this.emit("history");
  }
  addNodes(e) {
    if (e.length === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    for (const r of e)
      if (this.nodes.set(r.id, r), this.quadTree.insert(r), r.type === "edge") {
        const n = r, { fromId: s, toId: i } = n.data;
        this.adjacency.has(s) || this.adjacency.set(s, /* @__PURE__ */ new Set()), this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.get(s).add(r.id), this.adjacency.get(i).add(r.id);
      }
    const o = e.filter((r) => r.type !== "edge").map((r) => r.id);
    o.length > 0 && this.updateFrameMembership(o), this.emit("change"), this.emit("history");
  }
  updateNode(e, o) {
    var s, i, l, a, c, h, p, d, f;
    const r = this.nodes.get(e);
    if (!r) return;
    const n = { ...r, ...o };
    if (o.data && typeof o.data == "object" && r.data && typeof r.data == "object" && (n.data = {
      ...r.data,
      ...o.data
    }), this.nodes.set(e, n), (r.x !== n.x || r.y !== n.y || r.w !== n.w || r.h !== n.h || (r.rotation ?? 0) !== (n.rotation ?? 0)) && (this.quadTree.remove(r), this.quadTree.insert(n), this.updateConnectedEdges(e)), r.x !== n.x || r.y !== n.y) {
      const m = n.x - r.x, y = n.y - r.y;
      (l = (i = (s = this.registry) == null ? void 0 : s.get(n.type)) == null ? void 0 : i.onMove) == null || l.call(i, n, m, y, this), this.emit("node:move", n, m, y);
    }
    if (r.w !== n.w || r.h !== n.h) {
      const m = r.w !== 0 ? n.w / r.w : 1, y = r.h === "auto" ? 0 : r.h, g = n.h === "auto" ? 0 : n.h, b = y !== 0 ? g / y : 1;
      this.emit("node:resize", n, m, b);
    }
    (r.rotation ?? 0) !== (n.rotation ?? 0) && ((h = (c = (a = this.registry) == null ? void 0 : a.get(n.type)) == null ? void 0 : c.onRotate) == null || h.call(c, n, n.rotation ?? 0, this), this.emit("node:rotate", n, n.rotation ?? 0)), o.data && r.data !== n.data && ((f = (d = (p = this.registry) == null ? void 0 : p.get(n.type)) == null ? void 0 : d.onDataChange) == null || f.call(d, n, r.data, n.data, this), this.emit("node:data", n, r.data, n.data)), this.emit("change");
  }
  /**
   * Batch update multiple nodes with a single change emit.
   * Use during drag/resize to avoid N re-renders per frame.
   */
  updateMany(e) {
    let o = !1;
    for (const { id: r, patch: n } of e) {
      const s = this.nodes.get(r);
      if (!s) continue;
      const i = { ...s, ...n };
      n.data && typeof n.data == "object" && s.data && typeof s.data == "object" && (i.data = {
        ...s.data,
        ...n.data
      }), this.nodes.set(r, i), (s.x !== i.x || s.y !== i.y || s.w !== i.w || s.h !== i.h || (s.rotation ?? 0) !== (i.rotation ?? 0)) && (this.quadTree.remove(s), this.quadTree.insert(i), this.updateConnectedEdges(r)), o = !0;
    }
    o && this.emit("change");
  }
  updateConnectedEdges(e) {
    const o = this.adjacency.get(e);
    if (o)
      for (const r of o) {
        const n = this.nodes.get(r);
        if (!n || n.type !== "edge") continue;
        const s = n, i = this.nodes.get(s.data.fromId), l = this.nodes.get(s.data.toId);
        if (i && l) {
          const a = Ke(
            i,
            l,
            s.data.edgeType,
            void 0,
            s.data.sourceHandle,
            s.data.targetHandle,
            s.data.midpointOffset,
            s.data.curveOffset
          ), c = { ...s, ...a.bounds };
          this.nodes.set(r, c), this.quadTree.remove(s), this.quadTree.insert(c);
        }
      }
  }
  updateNodeWithHistory(e, o) {
    this.history.pushSnapshot(this.nodes, this.groupParent), this.updateNode(e, o), this.emit("history");
  }
  /** Update multiple nodes in a single undo step. */
  batchUpdateWithHistory(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const { id: o, patch: r } of e)
        this.updateNode(o, r);
      this.emit("history");
    }
  }
  deleteNode(e) {
    var r, n, s, i, l;
    if (!this.nodes.has(e) || (r = this.nodes.get(e)) != null && r.locked) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = this.nodes.get(e);
    o && ((i = (s = (n = this.registry) == null ? void 0 : n.get(o.type)) == null ? void 0 : s.onDelete) == null || i.call(s, o, this), this.emit("node:delete", o), this.quadTree.remove(o)), this.nodes.delete(e), this.selection.delete(e), this.adjacency.delete(e), this.frameChildren.delete(e);
    for (const a of this.frameChildren.values()) a.delete(e);
    for (const [a, c] of this.nodes)
      if (c.type === "edge") {
        const h = c.data;
        if (h.fromId === e || h.toId === e) {
          const p = this.nodes.get(a);
          p && this.quadTree.remove(p), this.nodes.delete(a), this.selection.delete(a);
          const d = h.fromId === e ? h.toId : h.fromId;
          (l = this.adjacency.get(d)) == null || l.delete(a);
        }
      }
    this.emit("change"), this.emit("selection"), this.emit("history");
  }
  getNode(e) {
    return this.nodes.get(e);
  }
  getAllNodes() {
    return Array.from(this.nodes.values());
  }
  /** Returns a read-only iterable of all nodes (no copy). */
  iterNodes() {
    return this.nodes.values();
  }
  getNodesByType(e) {
    const o = [];
    for (const r of this.nodes.values())
      r.type === e && o.push(r);
    return o;
  }
  /** Returns all non-edge nodes fully contained within a frame's bounds (including nested frames). */
  getNodesInsideFrame(e) {
    const o = this.nodes.get(e);
    if (!o || !this._containerTypes.has(o.type)) return [];
    const r = this.resolveHeight(o), n = [];
    for (const s of this.nodes.values()) {
      if (s.id === e || s.type === "edge") continue;
      const i = this.resolveHeight(s);
      s.x >= o.x && s.y >= o.y && s.x + s.w <= o.x + o.w && s.y + i <= o.y + r && n.push(s);
    }
    return n;
  }
  /** Returns tracked frame children (nodes explicitly added to the frame). */
  getFrameChildren(e) {
    const o = this.frameChildren.get(e);
    if (!o) return [];
    const r = [];
    for (const n of o) {
      const s = this.nodes.get(n);
      s && r.push(s);
    }
    return r;
  }
  /** Returns IDs of all descendants of a frame (children, grandchildren, etc.). */
  getFrameDescendantIds(e) {
    const o = /* @__PURE__ */ new Set(), r = (n) => {
      const s = this.frameChildren.get(n);
      if (s)
        for (const i of s) {
          if (o.has(i)) continue;
          o.add(i);
          const l = this.nodes.get(i);
          l && this._containerTypes.has(l.type) && r(i);
        }
    };
    return r(e), o;
  }
  /** Rebuild frameChildren from spatial containment. Called on load/undo/redo.
   *  Each node is assigned only to its smallest containing frame. */
  rebuildFrameChildren() {
    this.frameChildren.clear();
    const e = [];
    for (const r of this.nodes.values()) {
      if (!this._containerTypes.has(r.type)) continue;
      const n = this.resolveHeight(r);
      e.push({ node: r, area: r.w * n });
    }
    e.sort((r, n) => r.area - n.area);
    const o = /* @__PURE__ */ new Set();
    for (const { node: r } of e) {
      const s = this.getNodesInsideFrame(r.id).filter((i) => !o.has(i.id));
      if (s.length > 0) {
        const i = /* @__PURE__ */ new Set();
        for (const l of s)
          i.add(l.id), o.add(l.id);
        this.frameChildren.set(r.id, i);
      }
    }
  }
  /** After nodes are moved, update which frames they belong to.
   *  Each node is assigned only to its smallest containing frame.
   *  Frames can be nested inside other frames (but not inside themselves or their descendants). */
  updateFrameMembership(e) {
    for (const o of e) {
      const r = this.nodes.get(o);
      if (!r || r.type === "edge") continue;
      const n = this.resolveHeight(r);
      for (const [c, h] of this.frameChildren) {
        if (!h.has(o)) continue;
        const p = this.nodes.get(c);
        if (!p) {
          h.delete(o);
          continue;
        }
        const d = this.resolveHeight(p);
        r.x >= p.x && r.y >= p.y && r.x + r.w <= p.x + p.w && r.y + n <= p.y + d || h.delete(o);
      }
      let s;
      this._containerTypes.has(r.type) && (s = this.getFrameDescendantIds(o));
      let i = null, l = 1 / 0;
      const a = this.quadTree.retrieve([], { x: r.x, y: r.y, w: r.w, h: n });
      for (const c of a) {
        if (!this._containerTypes.has(c.type) || c.id === o || s != null && s.has(c.id)) continue;
        const h = this.resolveHeight(c);
        if (r.x >= c.x && r.y >= c.y && r.x + r.w <= c.x + c.w && r.y + n <= c.y + h) {
          const d = c.w * h;
          d < l && (l = d, i = c);
        }
      }
      for (const [, c] of this.frameChildren)
        c.delete(o);
      i && (this.frameChildren.has(i.id) || this.frameChildren.set(i.id, /* @__PURE__ */ new Set()), this.frameChildren.get(i.id).add(o));
    }
  }
  /** Sync frame children after resize: remove nodes no longer inside, add newly contained nodes. */
  syncFrameChildrenAfterResize(e) {
    const o = this.nodes.get(e);
    if (!o || !this._containerTypes.has(o.type)) return;
    const r = this.getNodesInsideFrame(e);
    r.length > 0 ? this.frameChildren.set(e, new Set(r.map((n) => n.id))) : this.frameChildren.delete(e);
  }
  /** Adopt all existing nodes that are spatially inside a newly created frame. */
  adoptNodesIntoNewFrame(e) {
    const o = this.getNodesInsideFrame(e);
    if (o.length > 0) {
      const r = /* @__PURE__ */ new Set();
      for (const n of o) r.add(n.id);
      this.frameChildren.set(e, r);
    }
  }
  nextZ() {
    return this.nextZValue++;
  }
  setNextZ(e) {
    this.nextZValue = e;
  }
  // --- Z-ordering ---
  bringToFront(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.nodes.get(o);
        r && !r.locked && this.nodes.set(o, { ...r, z: this.nextZValue++ });
      }
      this.emit("change"), this.emit("history");
    }
  }
  sendToBack(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (let o = e.length - 1; o >= 0; o--) {
        const r = this.nodes.get(e[o]);
        r && !r.locked && this.nodes.set(e[o], { ...r, z: --this._minZ });
      }
      this.emit("change"), this.emit("history");
    }
  }
  /** AABB overlap test between two nodes. */
  _nodesOverlap(e, o) {
    const r = this.resolveHeight(e), n = this.resolveHeight(o);
    return e.x < o.x + o.w && e.x + e.w > o.x && e.y < o.y + n && e.y + r > o.y;
  }
  bringForward(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.nodes.get(o);
        if (!r || r.locked) continue;
        const n = r.type === "edge", s = [];
        for (const h of this.nodes.values())
          h.id !== o && (n ? h.type === "edge" : h.type !== "edge") && h.z >= r.z && this._nodesOverlap(r, h) && s.push(h);
        if (s.length === 0) continue;
        s.sort((h, p) => h.z - p.z);
        const i = s[0], l = this.nodes.get(i.id), a = r.z, c = l.z;
        a === c ? this.nodes.set(o, { ...r, z: c + 1 }) : (this.nodes.set(o, { ...r, z: c }), this.nodes.set(i.id, { ...l, z: a }));
      }
      this.emit("change"), this.emit("history");
    }
  }
  sendBackward(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.nodes.get(o);
        if (!r || r.locked) continue;
        const n = r.type === "edge", s = [];
        for (const h of this.nodes.values())
          h.id !== o && (n ? h.type === "edge" : h.type !== "edge") && h.z <= r.z && this._nodesOverlap(r, h) && s.push(h);
        if (s.length === 0) continue;
        s.sort((h, p) => p.z - h.z);
        const i = s[0], l = this.nodes.get(i.id), a = r.z, c = l.z;
        a === c ? this.nodes.set(o, { ...r, z: c - 1 }) : (this.nodes.set(o, { ...r, z: c }), this.nodes.set(i.id, { ...l, z: a }));
      }
      this.emit("change"), this.emit("history");
    }
  }
  /** Update the QuadTree bounds for an auto-height node when its measured height changes. */
  updateMeasuredHeight(e, o) {
    const r = this.nodes.get(e);
    !r || r.h !== "auto" || (this._measuredHeights[e] = o, this.quadTree.remove(r), this.quadTree.insert(r, o));
  }
  // --- Spatial Queries ---
  hitTest(e, o, r) {
    const s = this.quadTree.retrieve([], {
      x: e - 50,
      y: o - 50,
      w: 100,
      h: 100
    }), i = /* @__PURE__ */ new Map();
    for (const l of s) i.set(l.id, l);
    return gl(i, e, o, this.viewport.zoom, r, this._containerTypes);
  }
  /** Returns all nodes at a point, sorted highest-z first */
  hitTestAll(e, o, r) {
    const s = this.quadTree.retrieve([], {
      x: e - 50,
      y: o - 50,
      w: 100,
      h: 100
    }), i = /* @__PURE__ */ new Map();
    for (const l of s) i.set(l.id, l);
    return xl(i, e, o, this.viewport.zoom, r, this._containerTypes);
  }
  getNodesInRect(e) {
    return this.quadTree.retrieve([], e);
  }
  // --- Selection ---
  /** Expand selection to include all group siblings, walking up the group
   *  hierarchy until the active group (or root) is reached. */
  expandSelectionToGroups() {
    const e = /* @__PURE__ */ new Set();
    for (const n of this.selection) {
      const s = this.nodes.get(n);
      if (!(s != null && s.groupId) || this.activeGroupId && s.groupId === this.activeGroupId) continue;
      let i = s.groupId;
      for (; ; ) {
        const l = this.groupParent.get(i);
        if (!l || this.activeGroupId && l === this.activeGroupId) break;
        i = l;
      }
      e.add(i);
    }
    if (e.size === 0) return;
    const o = new Set(e), r = (n) => {
      const s = this.groupChildren.get(n);
      if (s)
        for (const i of s)
          o.has(i) || (o.add(i), r(i));
    };
    for (const n of e)
      r(n);
    for (const n of this.nodes.values())
      n.groupId && o.has(n.groupId) && this.selection.add(n.id);
  }
  select(e) {
    var o, r, n, s, i, l;
    for (const a of this.selection) {
      const c = this.nodes.get(a);
      c && ((n = (r = (o = this.registry) == null ? void 0 : o.get(c.type)) == null ? void 0 : r.onDeselect) == null || n.call(r, c, this), this.emit("node:deselect", c));
    }
    this.selection.clear(), this.selection.add(e), this.expandSelectionToGroups();
    for (const a of this.selection) {
      const c = this.nodes.get(a);
      c && ((l = (i = (s = this.registry) == null ? void 0 : s.get(c.type)) == null ? void 0 : i.onSelect) == null || l.call(i, c, this), this.emit("node:select", c));
    }
    this.emit("selection");
  }
  toggleSelect(e) {
    const o = this.nodes.get(e);
    if (this.selection.has(e))
      if (o != null && o.groupId)
        for (const r of this.nodes.values())
          r.groupId === o.groupId && this.selection.delete(r.id);
      else
        this.selection.delete(e);
    else
      this.selection.add(e), this.expandSelectionToGroups();
    this.emit("selection");
  }
  selectMultiple(e) {
    this.selection = new Set(e), this.expandSelectionToGroups(), this.emit("selection");
  }
  deselectAll() {
    var e, o, r;
    if (!(this.selection.size === 0 && !this.activeGroupId)) {
      for (const n of this.selection) {
        const s = this.nodes.get(n);
        s && ((r = (o = (e = this.registry) == null ? void 0 : e.get(s.type)) == null ? void 0 : o.onDeselect) == null || r.call(o, s, this), this.emit("node:deselect", s));
      }
      this.selection.clear(), this.activeGroupId && (this.activeGroupId = null, this.emit("group:exit")), this.emit("selection");
    }
  }
  deleteSelected() {
    var r, n, s;
    if (this.selection.size === 0) return;
    const e = new Set(
      Array.from(this.selection).filter((i) => {
        var l;
        return !((l = this.nodes.get(i)) != null && l.locked);
      })
    );
    if (e.size === 0) return;
    this.activeGroupId && this.getGroupMembers(this.activeGroupId).filter((l) => !e.has(l.id)).length === 0 && (this.activeGroupId = null, this.emit("group:exit")), this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = e;
    for (const i of e) {
      const l = this.nodes.get(i);
      l && ((s = (n = (r = this.registry) == null ? void 0 : r.get(l.type)) == null ? void 0 : n.onDelete) == null || s.call(n, l, this), this.emit("node:delete", l), this.quadTree.remove(l), this.nodes.delete(i));
    }
    for (const [i, l] of this.nodes)
      if (l.type === "edge") {
        const a = l.data;
        if (o.has(a.fromId) || o.has(a.toId)) {
          const c = this.nodes.get(i);
          c && this.quadTree.remove(c), this.nodes.delete(i);
        }
      }
    this.cleanupEmptyGroups();
    for (const i of e) this.selection.delete(i);
    this.emit("change"), this.emit("selection"), this.emit("history");
  }
  /** Remove groupParent entries for groups that no longer have any members. */
  cleanupEmptyGroups() {
    const e = /* @__PURE__ */ new Set();
    for (const o of this.nodes.values())
      o.groupId && e.add(o.groupId);
    for (const [o] of this.groupParent)
      e.has(o) || this.unlinkGroupParent(o);
  }
  /** Set a groupParent entry and keep groupChildren in sync. */
  linkGroupParent(e, o) {
    var s;
    const r = this.groupParent.get(e);
    r && ((s = this.groupChildren.get(r)) == null || s.delete(e)), this.groupParent.set(e, o);
    let n = this.groupChildren.get(o);
    n || (n = /* @__PURE__ */ new Set(), this.groupChildren.set(o, n)), n.add(e);
  }
  /** Remove a groupParent entry and keep groupChildren in sync. */
  unlinkGroupParent(e) {
    const o = this.groupParent.get(e);
    if (o) {
      const r = this.groupChildren.get(o);
      r && (r.delete(e), r.size === 0 && this.groupChildren.delete(o));
    }
    this.groupParent.delete(e);
  }
  /** Rebuild the groupChildren reverse index from groupParent. */
  rebuildGroupChildren() {
    this.groupChildren.clear();
    for (const [e, o] of this.groupParent) {
      let r = this.groupChildren.get(o);
      r || (r = /* @__PURE__ */ new Set(), this.groupChildren.set(o, r)), r.add(e);
    }
  }
  deleteNodes(e) {
    var r, n, s;
    if (e.length === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = new Set(e);
    for (const i of e) {
      const l = this.nodes.get(i);
      if (l) {
        (s = (n = (r = this.registry) == null ? void 0 : r.get(l.type)) == null ? void 0 : n.onDelete) == null || s.call(n, l, this), this.emit("node:delete", l), this.quadTree.remove(l), this.nodes.delete(i), this.frameChildren.delete(i);
        for (const a of this.frameChildren.values()) a.delete(i);
      }
    }
    for (const [i, l] of this.nodes)
      if (l.type === "edge") {
        const a = l.data;
        if (o.has(a.fromId) || o.has(a.toId)) {
          const c = this.nodes.get(i);
          c && this.quadTree.remove(c), this.nodes.delete(i);
        }
      }
    this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  // --- Flip ---
  flipSelected(e) {
    var o;
    if (this.selection.size !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const r of this.selection) {
        const n = this.nodes.get(r);
        if (!n) continue;
        this.quadTree.remove(n);
        let s = null;
        const i = (o = this.registry) == null ? void 0 : o.get(n.type);
        if (i != null && i.onFlip) {
          const l = i.onFlip(n, e, this);
          l && Object.keys(l).length > 0 && (s = {
            ...n,
            data: { ...n.data, ...l }
          });
        } else if (n.type === "draw") {
          const l = n;
          if (e === "h") {
            const a = l.data.points.map(
              ([c, h, p]) => [l.w - c, h, p]
            );
            s = { ...l, data: { ...l.data, points: a } };
          } else {
            const a = l.h === "auto" ? 0 : l.h, c = l.data.points.map(
              ([h, p, d]) => [h, a - p, d]
            );
            s = { ...l, data: { ...l.data, points: c } };
          }
        } else if (n.type === "shape") {
          const l = n;
          if (l.data.shape === "arrow" || l.data.shape === "line")
            if (l.data.startPoint && l.data.endPoint)
              if (e === "h") {
                const a = [l.w - l.data.startPoint[0], l.data.startPoint[1]], c = [l.w - l.data.endPoint[0], l.data.endPoint[1]];
                s = { ...l, data: { ...l.data, startPoint: a, endPoint: c } };
              } else {
                const a = l.h === "auto" ? 0 : l.h, c = [l.data.startPoint[0], a - l.data.startPoint[1]], h = [l.data.endPoint[0], a - l.data.endPoint[1]];
                s = { ...l, data: { ...l.data, startPoint: c, endPoint: h } };
              }
            else
              s = e === "h" ? { ...l, rotation: -(l.rotation || 0) + 180 } : { ...l, rotation: -(l.rotation || 0) };
        } else if (n.type === "image") {
          const l = n;
          s = e === "h" ? { ...l, data: { ...l.data, flipH: !l.data.flipH } } : { ...l, data: { ...l.data, flipV: !l.data.flipV } };
        }
        s ? (this.nodes.set(r, s), this.quadTree.insert(s), this.emit("node:flip", s, e)) : this.quadTree.insert(n);
      }
      this.emit("change"), this.emit("history");
    }
  }
  flipSelectedHorizontal() {
    this.flipSelected("h");
  }
  flipSelectedVertical() {
    this.flipSelected("v");
  }
  // --- Grouping ---
  groupSelected() {
    if (this.selection.size < 2 || this.activeGroupId) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const e = gt(10), o = /* @__PURE__ */ new Set();
    for (const r of this.selection) {
      const n = this.nodes.get(r);
      if (n != null && n.groupId) {
        let s = n.groupId;
        for (; this.groupParent.has(s); ) s = this.groupParent.get(s);
        o.add(s);
      }
    }
    if (o.size > 0) {
      for (const r of o)
        this.linkGroupParent(r, e);
      for (const r of this.selection) {
        const n = this.nodes.get(r);
        n && !n.groupId && this.nodes.set(r, { ...n, groupId: e });
      }
    } else
      for (const r of this.selection) {
        const n = this.nodes.get(r);
        n && this.nodes.set(r, { ...n, groupId: e });
      }
    this.emit("change"), this.emit("history");
  }
  ungroupSelected() {
    if (this.selection.size === 0) return;
    const e = /* @__PURE__ */ new Set();
    for (const o of this.selection) {
      const r = this.nodes.get(o);
      if (r != null && r.groupId) {
        let n = r.groupId;
        for (; this.groupParent.has(n); ) {
          const s = this.groupParent.get(n);
          if (s === this.activeGroupId) break;
          n = s;
        }
        e.add(n);
      }
    }
    if (e.size !== 0) {
      this.activeGroupId && e.has(this.activeGroupId) && (this.activeGroupId = null, this.emit("group:exit")), this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const r = this.groupParent.get(o);
        for (const s of this.nodes.values())
          if (s.groupId === o)
            if (r)
              this.nodes.set(s.id, { ...s, groupId: r });
            else {
              const { groupId: i, ...l } = s;
              this.nodes.set(s.id, l);
            }
        const n = this.groupChildren.get(o);
        if (n)
          for (const s of [...n])
            r ? this.linkGroupParent(s, r) : this.unlinkGroupParent(s);
        this.unlinkGroupParent(o), this.groupChildren.delete(o), this.groupRotations.delete(o);
      }
      this.emit("change"), this.emit("history");
    }
  }
  selectionHasGroup() {
    var e;
    for (const o of this.selection)
      if ((e = this.nodes.get(o)) != null && e.groupId) return !0;
    return !1;
  }
  /** Returns the outermost groupId if all selected nodes belong to the same group tree, else undefined. */
  selectionGroupId() {
    if (this.selection.size < 2) return;
    let e;
    for (const o of this.selection) {
      const r = this.nodes.get(o);
      if (!(r != null && r.groupId)) return;
      let n = r.groupId;
      for (; this.groupParent.has(n); ) n = this.groupParent.get(n);
      if (!e) e = n;
      else if (n !== e) return;
    }
    return e;
  }
  /** True if all selected nodes belong to exactly one group (possibly nested). */
  selectionIsSingleGroup() {
    if (this.selection.size < 2) return !1;
    let e;
    for (const o of this.selection) {
      const r = this.nodes.get(o);
      if (!(r != null && r.groupId)) return !1;
      let n = r.groupId;
      for (; this.groupParent.has(n); ) n = this.groupParent.get(n);
      if (!e) e = n;
      else if (n !== e) return !1;
    }
    return !0;
  }
  getGroupMembers(e) {
    const o = [];
    for (const r of this.nodes.values())
      r.groupId === e && o.push(r);
    return o;
  }
  /** Enter a group for drill-down selection of individual children. */
  enterGroup(e) {
    this.activeGroupId !== e && (this.activeGroupId = e, this.emit("group:enter", e));
  }
  /** Fully exit all group levels and deselect. */
  exitAllGroups() {
    this.activeGroupId && (this.activeGroupId = null, this.emit("group:exit"));
  }
  /** Exit the active group — go up one level for nested groups, or exit entirely. */
  exitGroup() {
    if (!this.activeGroupId) return;
    const e = this.activeGroupId, o = this.groupParent.get(e);
    o ? (this.activeGroupId = o, this.emit("group:enter", o)) : (this.activeGroupId = null, this.emit("group:exit"));
    const r = this.getGroupMembers(e);
    r.length > 0 && (this.selection = /* @__PURE__ */ new Set([r[0].id]), this.expandSelectionToGroups(), this.emit("selection"));
  }
  /** Check if a node belongs to the currently active (entered) group or any of its descendants. */
  isNodeInActiveGroup(e) {
    if (!this.activeGroupId) return !1;
    const o = this.nodes.get(e);
    if (!(o != null && o.groupId)) return !1;
    let r = o.groupId;
    for (; r; ) {
      if (r === this.activeGroupId) return !0;
      r = this.groupParent.get(r);
    }
    return !1;
  }
  /** Get the outermost group of a node (stopping at activeGroupId boundary). */
  getNodeOutermostGroup(e) {
    const o = this.nodes.get(e);
    if (!(o != null && o.groupId)) return;
    let r = o.groupId;
    for (; ; ) {
      const n = this.groupParent.get(r);
      if (!n || this.activeGroupId && n === this.activeGroupId) break;
      r = n;
    }
    return r;
  }
  /** Get all nodes that are descendants of a group (direct + nested sub-groups). */
  getAllGroupDescendantNodes(e) {
    const o = /* @__PURE__ */ new Set([e]), r = (s) => {
      const i = this.groupChildren.get(s);
      if (i)
        for (const l of i)
          o.has(l) || (o.add(l), r(l));
    };
    r(e);
    const n = [];
    for (const s of this.nodes.values())
      s.groupId && o.has(s.groupId) && n.push(s);
    return n;
  }
  duplicateSelected() {
    if (this.selection.size === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const e = 20, o = /* @__PURE__ */ new Map(), r = [];
    for (const s of this.selection) {
      const i = this.nodes.get(s);
      if (!i) continue;
      const l = gt();
      o.set(i.id, l), r.push({
        ...JSON.parse(JSON.stringify(i)),
        id: l,
        x: i.x + e,
        y: i.y + e,
        z: this.nextZValue++,
        locked: void 0
      });
    }
    for (const s of r)
      if (s.type === "edge" && s.data) {
        const i = s.data;
        o.has(i.fromId) && (i.fromId = o.get(i.fromId)), o.has(i.toId) && (i.toId = o.get(i.toId));
      }
    const n = /* @__PURE__ */ new Map();
    for (const s of r)
      s.groupId && (n.has(s.groupId) || n.set(s.groupId, gt(10)), s.groupId = n.get(s.groupId));
    for (const [s, i] of this.groupParent)
      n.has(s) && n.has(i) && this.linkGroupParent(n.get(s), n.get(i));
    this.addNodes(r), this.selection = new Set(r.map((s) => s.id)), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  // --- Mode ---
  // --- Clipboard ---
  copySelected() {
    this.selection.size !== 0 && (this.clipboard = Array.from(this.selection).map((e) => {
      const o = this.nodes.get(e);
      return JSON.parse(JSON.stringify(o));
    }), this.pasteCount = 0);
  }
  cutSelected() {
    this.copySelected(), this.deleteSelected();
  }
  /**
   * Paste clipboard contents centered at a canvas position.
   * If no position given, uses viewport center.
   */
  pasteClipboard(e, o) {
    if (this.clipboard.length === 0) return;
    this.pasteCount++;
    let r = 1 / 0, n = 1 / 0, s = -1 / 0, i = -1 / 0;
    for (const b of this.clipboard) {
      const x = b.h === "auto" ? 100 : b.h;
      b.x < r && (r = b.x), b.y < n && (n = b.y), b.x + b.w > s && (s = b.x + b.w), b.y + x > i && (i = b.y + x);
    }
    const l = (r + s) / 2, a = (n + i) / 2;
    let c, h;
    if (e !== void 0 && o !== void 0)
      c = e, h = o;
    else {
      const b = this.getWindow(), x = b.innerWidth / 2, C = b.innerHeight / 2, I = Wo(this.viewport, x, C);
      c = I.x, h = I.y;
    }
    const p = this.pasteCount * 20, d = c - l + p, f = h - a + p, m = /* @__PURE__ */ new Map(), y = this.clipboard.map((b) => {
      const x = gt();
      return m.set(b.id, x), {
        ...structuredClone(b),
        id: x,
        x: b.x + d,
        y: b.y + f,
        z: this.nextZValue++,
        locked: void 0
      };
    });
    for (const b of y)
      if (b.type === "edge" && b.data) {
        const x = b.data;
        m.has(x.fromId) && (x.fromId = m.get(x.fromId)), m.has(x.toId) && (x.toId = m.get(x.toId));
      }
    const g = /* @__PURE__ */ new Map();
    for (const b of y)
      b.groupId && (g.has(b.groupId) || g.set(b.groupId, gt(10)), b.groupId = g.get(b.groupId));
    for (const [b, x] of this.groupParent)
      g.has(b) && g.has(x) && this.linkGroupParent(g.get(b), g.get(x));
    this.addNodes(y), this.selectMultiple(y.map((b) => b.id));
  }
  /**
   * Insert a pre-built template centered at (cx, cy) in canvas coordinates.
   */
  applyTemplate(e, o, r) {
    const n = vi.find((f) => f.id === e);
    if (!n) return;
    const s = structuredClone(n.nodes), i = /* @__PURE__ */ new Map();
    for (const f of s) {
      const m = gt(10);
      i.set(f.id, m), f.id = m;
    }
    for (const f of s) {
      if (f.type === "edge" && f.data) {
        const m = f.data;
        i.has(m.fromId) && (m.fromId = i.get(m.fromId)), i.has(m.toId) && (m.toId = i.get(m.toId));
      }
      f.groupId && i.has(f.groupId) && (f.groupId = i.get(f.groupId));
    }
    let l = 1 / 0, a = 1 / 0, c = -1 / 0, h = -1 / 0;
    for (const f of s) {
      if (f.type === "edge") continue;
      const m = f.h === "auto" ? 100 : f.h;
      l = Math.min(l, f.x), a = Math.min(a, f.y), c = Math.max(c, f.x + f.w), h = Math.max(h, f.y + m);
    }
    const p = o - (l + c) / 2, d = r - (a + h) / 2;
    for (const f of s)
      f.type !== "edge" && (f.x += p, f.y += d), f.z = this.nextZValue++;
    this.addNodes(s), this.selectMultiple(s.map((f) => f.id));
  }
  hasClipboard() {
    return this.clipboard.length > 0;
  }
  getClipboardNodes() {
    return this.clipboard.map((e) => structuredClone(e));
  }
  setClipboard(e) {
    this.clipboard = e.map((o) => structuredClone(o)), this.pasteCount = 0;
  }
  // --- Mode ---
  setMode(e) {
    this.mode !== e && (this.mode = e, this.selection.size > 0 && (this.selection.clear(), this.emit("selection")), this.emit("mode"));
  }
  // --- History ---
  pushHistorySnapshot() {
    this.history.pushSnapshot(this.nodes, this.groupParent), this.emit("history");
  }
  /*private*/
  rebuildQuadTree() {
    this.quadTree.clear(), this.adjacency.clear();
    let e = 0, o = 0;
    for (const r of this.nodes.values())
      if (this.quadTree.insert(r), r.z < e && (e = r.z), r.z > o && (o = r.z), r.type === "edge") {
        const n = r, { fromId: s, toId: i } = n.data;
        this.adjacency.has(s) || this.adjacency.set(s, /* @__PURE__ */ new Set()), this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.get(s).add(r.id), this.adjacency.get(i).add(r.id);
      }
    this._minZ = e, this.nextZValue = o + 1;
  }
  undo() {
    const e = this.history.undo(this.nodes, this.groupParent);
    e && (this.nodes = e.nodes, this.groupParent = e.groupParent, this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history"));
  }
  redo() {
    const e = this.history.redo(this.nodes, this.groupParent);
    e && (this.nodes = e.nodes, this.groupParent = e.groupParent, this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history"));
  }
  canUndo() {
    return this.history.canUndo();
  }
  canRedo() {
    return this.history.canRedo();
  }
  // --- Remote Collaboration ---
  /** Add a remote node without emitting events or pushing history. */
  addRemoteNode(e) {
    if (this._suppressEvents = !0, this.nodes.set(e.id, e), this.quadTree.insert(e), e.type === "edge") {
      const o = e, { fromId: r, toId: n } = o.data;
      this.adjacency.has(r) || this.adjacency.set(r, /* @__PURE__ */ new Set()), this.adjacency.has(n) || this.adjacency.set(n, /* @__PURE__ */ new Set()), this.adjacency.get(r).add(e.id), this.adjacency.get(n).add(e.id);
    }
    e.z >= this.nextZValue && (this.nextZValue = e.z + 1), e.z < this._minZ && (this._minZ = e.z), this._suppressEvents = !1;
  }
  /** Delete a remote node without emitting events or pushing history. */
  deleteRemoteNode(e) {
    var r;
    this._suppressEvents = !0;
    const o = this.nodes.get(e);
    if (o) {
      this.quadTree.remove(o), this.nodes.delete(e), this.selection.delete(e), this.adjacency.delete(e), this.frameChildren.delete(e);
      for (const n of this.frameChildren.values()) n.delete(e);
      for (const [n, s] of this.nodes)
        if (s.type === "edge") {
          const i = s.data;
          if (i.fromId === e || i.toId === e) {
            const l = this.nodes.get(n);
            l && this.quadTree.remove(l), this.nodes.delete(n), this.selection.delete(n);
            const a = i.fromId === e ? i.toId : i.fromId;
            (r = this.adjacency.get(a)) == null || r.delete(n);
          }
        }
    }
    this._suppressEvents = !1;
  }
  /** Apply a remote node update without emitting events or pushing history. */
  applyRemoteNodeUpdate(e, o) {
    this._suppressEvents = !0;
    const r = this.nodes.get(e);
    if (r) {
      const n = { ...r, ...o };
      o.data && typeof o.data == "object" && r.data && typeof r.data == "object" && (n.data = {
        ...r.data,
        ...o.data
      }), this.nodes.set(e, n), (r.x !== n.x || r.y !== n.y || r.w !== n.w || r.h !== n.h) && (this.quadTree.remove(r), this.quadTree.insert(n), this.updateConnectedEdges(e)), n.z >= this.nextZValue && (this.nextZValue = n.z + 1);
    }
    this._suppressEvents = !1;
  }
  /** Trigger a re-render without pushing history. Used after remote updates. */
  notifyChange() {
    var e;
    (e = this.listeners.change) == null || e.forEach((o) => o());
  }
  /** Emit draw progress for collab live stroke preview. */
  notifyDrawProgress(e) {
    this.emit("draw:progress", e);
  }
  /** Emit draw end when a stroke is completed or cancelled. */
  notifyDrawEnd() {
    this.emit("draw:end");
  }
  /** Emit shape progress for collab live shape preview. */
  notifyShapeProgress(e) {
    this.emit("shape:progress", e);
  }
  /** Emit shape end when shape creation is completed or cancelled. */
  notifyShapeEnd() {
    this.emit("shape:end");
  }
  /** Emit laser pointer progress for collab trail preview. */
  notifyLaserProgress(e) {
    this.emit("laser:progress", e);
  }
  /** Emit laser pointer end when trail has fully faded. */
  notifyLaserEnd() {
    this.emit("laser:end");
  }
  // --- Serialization ---
  async toSBD() {
    return Cl(this.getAllNodes(), {
      background: this.boardBackground,
      originView: this.originView ?? void 0
    });
  }
  async fromSBD(e) {
    this.history.clear(), this.nodes.clear(), this.groupParent.clear(), this.groupChildren.clear();
    const { nodes: o, meta: r } = await Al(e);
    r.background && (this.boardBackground = r.background, this.emit("background")), r.originView ? this.originView = r.originView : this.originView = null;
    let n = 0, s = 0;
    for (const i of o)
      this.nodes.set(i.id, i), i.z > n && (n = i.z), i.z < s && (s = i.z);
    this.rebuildQuadTree(), this.rebuildFrameChildren(), this.nextZValue = n + 1, this._minZ = s, this.selection.clear(), this.goToOriginView(), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  toJSON() {
    const e = {
      nodes: Array.from(this.nodes.entries()),
      viewport: this.viewport
    };
    return this.groupParent.size > 0 && (e.groupParent = Array.from(this.groupParent.entries())), e;
  }
  fromJSON(e) {
    this.history.clear(), this.nodes = new Map(e.nodes), this.groupParent = new Map(e.groupParent ?? []), this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), e.viewport && (this.viewport = e.viewport), this.selection.clear(), this.emit("change"), this.emit("viewport"), this.emit("selection"), this.emit("history");
  }
}
class Ql {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor(e) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    St(this, "types", /* @__PURE__ */ new Map());
    if (e)
      for (const o of e) this.register(o);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  register(e) {
    this.types.set(e.type, e);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  get(e) {
    return this.types.get(e);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getAll() {
    return Array.from(this.types.values());
  }
  /** Returns type strings for nodes that render in the DOM layer (not SVG-only). */
  getDOMTypes() {
    const e = /* @__PURE__ */ new Set();
    for (const o of this.types.values())
      o.isSVGOnly || e.add(o.type);
    return e;
  }
  has(e) {
    return this.types.has(e);
  }
}
const Ms = ["n", "ne", "e", "se", "s", "sw", "w", "nw"], Jl = {
  n: "ns-resize",
  ne: "nesw-resize",
  e: "ew-resize",
  se: "nwse-resize",
  s: "ns-resize",
  sw: "nesw-resize",
  w: "ew-resize",
  nw: "nwse-resize"
};
function Br(t, e) {
  const o = Ms.indexOf(t);
  if (o === -1) return "default";
  const r = (e % 360 + 360) % 360, n = Math.round(r / 45) % 8, s = (o + n) % 8;
  return Jl[Ms[s]];
}
class $l extends Ua {
  constructor() {
    super(...arguments);
    St(this, "state", { hasError: !1 });
  }
  static getDerivedStateFromError() {
    return { hasError: !0 };
  }
  componentDidCatch(o, r) {
    console.error("[ContentBlock] Editor mount failed, showing markdown fallback:", o, r);
  }
  render() {
    return this.state.hasError ? this.props.fallback : this.props.children;
  }
}
function Cs({ markdown: t }) {
  return /* @__PURE__ */ u(
    "div",
    {
      className: "sb-markdown-fallback",
      style: {
        padding: "8px 12px",
        fontSize: 14,
        lineHeight: 1.6,
        whiteSpace: "pre-wrap",
        wordBreak: "break-word",
        color: "#374151",
        opacity: 0.85
      },
      children: t || " "
    }
  );
}
const _l = 0, tc = [
  { pos: "nw", top: 0, left: 0 },
  { pos: "n", top: 0, left: "50%" },
  { pos: "ne", top: 0, left: "100%" },
  { pos: "e", top: "50%", left: "100%" },
  { pos: "se", top: "100%", left: "100%" },
  { pos: "s", top: "100%", left: "50%" },
  { pos: "sw", top: "100%", left: 0 },
  { pos: "w", top: "50%", left: 0 }
];
function ec(t) {
  return t.type !== "paragraph" ? !1 : !t.content || t.content.length === 0 ? !0 : t.content.every(
    (e) => e.type === "text" && (!e.text || e.text === "")
  );
}
function oc({
  node: t,
  isSelected: e,
  multiSelected: o,
  engine: r,
  schema: n,
  interactive: s,
  zoom: i,
  onMeasuredHeight: l,
  autoEdit: a
}) {
  const c = lt(null), h = lt(a === !0), p = lt(!1), d = lt(!1), f = lt(!1), m = lt(!1), y = lt(t.data.blocks), [g, b] = tt(!1), [x, C] = tt(!1), I = lt(null), M = $a({ schema: n }), D = lt(
    t.data.blocks.length > 0 ? t.data.blocks : null
  );
  mt(() => {
    const L = D.current;
    if (!L) return;
    D.current = null;
    const X = requestAnimationFrame(() => {
      try {
        M.replaceBlocks(M.document, L);
        return;
      } catch {
      }
      try {
        const _ = M.blocksToHTMLLossy(L);
        M._tiptapEditor.commands.setContent(_);
        return;
      } catch {
      }
      console.warn("[ContentBlock] All block rendering strategies failed, using markdown fallback"), C(!0);
    });
    return () => cancelAnimationFrame(X);
  }, [M]), mt(() => {
    (!e || o) && b(!1);
  }, [e, o]), mt(() => {
    h.current && (h.current = !1, p.current = !0, b(!0));
  }, [M]), mt(() => {
    if (!g || !p.current && !I.current) return;
    const L = I.current;
    I.current = null, p.current = !1;
    const X = requestAnimationFrame(() => {
      if (M.focus(), L)
        try {
          const _ = M._tiptapEditor, q = _.view.posAtCoords({ left: L.x, top: L.y });
          q && _.commands.setTextSelection(q.pos);
        } catch {
        }
    });
    return () => cancelAnimationFrame(X);
  }, [g, M]);
  const T = it(() => {
    if (d.current || f.current) return;
    const L = r.getNode(t.id), X = M.document;
    y.current = X, r.updateNode(t.id, {
      data: { ...L == null ? void 0 : L.data, blocks: X }
    });
  }, [M, r, t.id]);
  mt(() => {
    if (!M) return;
    const L = () => {
      var nt, ot;
      if (d.current || f.current || m.current) return;
      const j = M.document.length, q = r.getNode(t.id), Y = ((ot = (nt = q == null ? void 0 : q.data) == null ? void 0 : nt.blocks) == null ? void 0 : ot.length) ?? 0;
      if (j < Y) return;
      const $ = setTimeout(T, 100);
      return () => clearTimeout($);
    };
    let X;
    const _ = M.onChange(() => {
      X == null || X(), X = L();
    });
    return () => {
      _ == null || _(), X == null || X();
    };
  }, [M, T]), mt(() => {
    const L = c.current;
    if (!L) return;
    const X = (_) => {
      const j = _.relatedTarget;
      j && L.contains(j) || T();
    };
    return L.addEventListener("focusout", X), () => L.removeEventListener("focusout", X);
  }, [T]), mt(() => {
    if (g || t.data.blocks === y.current) return;
    const L = JSON.stringify(t.data.blocks), X = JSON.stringify(y.current);
    if (L !== X) {
      if (t.data.blocks.length > 0 && M.document.length > 0) {
        m.current = !0;
        try {
          M.replaceBlocks(M.document, t.data.blocks);
        } catch {
          try {
            const _ = M.blocksToHTMLLossy(t.data.blocks);
            M._tiptapEditor.commands.setContent(_);
          } catch {
          }
        }
        m.current = !1;
      }
      y.current = t.data.blocks;
    }
  }, [t.data.blocks, g, M]), mt(() => {
    if (t.h !== "auto" || !l) return;
    const L = c.current;
    if (!L) return;
    const X = () => {
      const j = L.offsetHeight;
      j > 0 && l(t.id, j);
    };
    X();
    const _ = new ResizeObserver(X);
    return _.observe(L), () => _.disconnect();
  }, [t.id, t.h, l]);
  const P = it(() => {
    const L = r.getNode(t.id);
    if (!L || L.h === "auto" || !M || !c.current)
      return;
    const X = L.h - _l, _ = c.current.querySelector(".bn-editor");
    if (!_) return;
    const j = M.document;
    if (j.length === 0) return;
    let q = 0;
    for (let ot = j.length - 1; ot >= 1 && ec(j[ot]); ot--)
      q++;
    const Y = _.scrollHeight, $ = j.length > 0 ? Y / j.length : 36;
    if (d.current = !0, Y < X) {
      const ot = X - Y, ft = Math.max(0, Math.floor(ot / $));
      if (ft > 0) {
        const pt = j[j.length - 1];
        M.insertBlocks(
          Array.from({ length: ft }, () => ({
            type: "paragraph",
            content: []
          })),
          pt,
          "after"
        );
      }
    } else if (Y > X && q > 0) {
      const ot = Y - X, ft = Math.min(q, Math.ceil(ot / $));
      if (ft > 0) {
        const pt = j.slice(j.length - ft);
        M.removeBlocks(pt);
      }
    }
    const nt = r.getNode(t.id);
    nt && r.updateNode(t.id, {
      data: { ...nt.data, blocks: M.document }
    }), d.current = !1;
  }, [M, r, t.id]), G = lt(P);
  G.current = P, mt(() => {
    if (t.h === "auto") return;
    const L = setTimeout(() => G.current(), 60);
    return () => clearTimeout(L);
  }, []);
  const K = it(
    (L) => {
      const X = L.currentTarget.ownerDocument;
      if (L.altKey) return;
      if (!r.selection.has(t.id) && r.selection.size > 0) {
        const { x: bt, y: ht } = r.screenToCanvas(L.clientX, L.clientY);
        for (const Wt of r.selection) {
          const Tt = r.getNode(Wt);
          if (!Tt) continue;
          const Rt = Tt.h === "auto" ? 100 : Tt.h;
          if (bt >= Tt.x && bt <= Tt.x + Tt.w && ht >= Tt.y && ht <= Tt.y + Rt)
            return;
        }
      }
      L.stopPropagation(), L.preventDefault(), L.currentTarget.setPointerCapture(L.pointerId), L.shiftKey ? r.toggleSelect(t.id) : r.selection.has(t.id) || r.select(t.id);
      const _ = L.clientX, j = L.clientY, q = Array.from(r.selection), Y = q.map((bt) => {
        const ht = r.getNode(bt);
        return { id: bt, x: ht.x, y: ht.y };
      });
      let $ = !1, nt = null, ot = _, ft = j, pt = !1;
      const rt = () => {
        nt = null;
        const bt = (ot - _) / r.viewport.zoom, ht = (ft - j) / r.viewport.zoom, { finalDx: Wt, finalDy: Tt } = r.computeDragSnap(
          Y,
          q,
          bt,
          ht,
          pt
        ), Rt = Y.map((Qt) => ({
          id: Qt.id,
          patch: { x: Qt.x + Wt, y: Qt.y + Tt }
        }));
        r.updateMany(Rt);
      }, It = (bt) => {
        const ht = (bt.clientX - _) / r.viewport.zoom, Wt = (bt.clientY - j) / r.viewport.zoom;
        if (!$)
          if (Math.abs(ht) > 2 || Math.abs(Wt) > 2)
            $ = !0, f.current = !0, r.pushHistorySnapshot();
          else
            return;
        ot = bt.clientX, ft = bt.clientY, pt = bt.metaKey || bt.ctrlKey, nt === null && (nt = requestAnimationFrame(rt));
      }, zt = () => {
        f.current = !1, nt !== null && (cancelAnimationFrame(nt), rt()), r.clearAlignGuides(), X.removeEventListener("pointermove", It), X.removeEventListener("pointerup", zt);
      };
      X.addEventListener("pointermove", It), X.addEventListener("pointerup", zt);
    },
    [r, t.id]
  ), at = it(
    (L) => {
      var rt;
      const X = L.currentTarget.ownerDocument;
      L.stopPropagation(), L.preventDefault();
      const _ = t.h === "auto" ? (((rt = c.current) == null ? void 0 : rt.getBoundingClientRect().height) ?? 60) / r.viewport.zoom : t.h, j = t.x + t.w / 2, q = t.y + _ / 2, Y = t.rotation || 0, { x: $, y: nt } = r.screenToCanvas(
        L.clientX,
        L.clientY
      ), ot = Math.atan2(nt - q, $ - j);
      r.pushHistorySnapshot();
      const ft = (It) => {
        const { x: zt, y: bt } = r.screenToCanvas(It.clientX, It.clientY), ht = Math.atan2(bt - q, zt - j);
        let Wt = Y + (ht - ot) * (180 / Math.PI);
        (It.shiftKey || r.snapToGrid) && !(It.metaKey || It.ctrlKey) && (Wt = Math.round(Wt / 15) * 15), r.updateNode(t.id, { rotation: Wt });
      }, pt = () => {
        X.removeEventListener("pointermove", ft), X.removeEventListener("pointerup", pt);
      };
      X.addEventListener("pointermove", ft), X.addEventListener("pointerup", pt);
    },
    [r, t.id, t.x, t.y, t.w, t.h, t.rotation]
  ), B = it(
    (L, X) => {
      var rt;
      const _ = X.currentTarget.ownerDocument;
      X.stopPropagation(), X.preventDefault();
      const j = X.clientX, q = X.clientY, Y = t.x, $ = t.y, nt = t.w, ot = t.h === "auto" ? (((rt = c.current) == null ? void 0 : rt.getBoundingClientRect().height) ?? 60) / r.viewport.zoom : t.h;
      r.pushHistorySnapshot();
      const ft = (It) => {
        const zt = (It.clientX - j) / r.viewport.zoom, bt = (It.clientY - q) / r.viewport.zoom;
        let ht = Y, Wt = $, Tt = nt, Rt = ot;
        if ((L === "nw" || L === "w" || L === "sw") && (ht = Y + zt, Tt = nt - zt), (L === "ne" || L === "e" || L === "se") && (Tt = nt + zt), (L === "nw" || L === "n" || L === "ne") && (Wt = $ + bt, Rt = ot - bt), (L === "sw" || L === "s" || L === "se") && (Rt = ot + bt), r.snapToGrid && !(It.metaKey || It.ctrlKey)) {
          const Qt = r.gridSize, Jt = (Zt) => Math.round(Zt / Qt) * Qt;
          (L === "nw" || L === "w" || L === "sw") && (ht = Jt(ht), Tt = Y + nt - ht), (L === "ne" || L === "e" || L === "se") && (Tt = Jt(ht + Tt) - ht), (L === "nw" || L === "n" || L === "ne") && (Wt = Jt(Wt), Rt = $ + ot - Wt), (L === "sw" || L === "s" || L === "se") && (Rt = Jt(Wt + Rt) - Wt);
        }
        Tt < 100 && (Tt = 100, (L === "nw" || L === "w" || L === "sw") && (ht = Y + nt - 100)), Rt < 60 && (Rt = 60, (L === "nw" || L === "n" || L === "ne") && (Wt = $ + ot - 60)), r.updateNode(t.id, { x: ht, y: Wt, w: Tt, h: Rt });
      }, pt = () => {
        _.removeEventListener("pointermove", ft), _.removeEventListener("pointerup", pt), requestAnimationFrame(() => G.current());
      };
      _.addEventListener("pointermove", ft), _.addEventListener("pointerup", pt);
    },
    [r, t.id, t.x, t.y, t.w, t.h]
  ), et = it(
    (L) => {
      if (!L.altKey) {
        if (g) {
          L.stopPropagation();
          return;
        }
        if (e) {
          K(L);
          return;
        }
        K(L);
      }
    },
    [g, e, K, r, t.id]
  ), Z = it(
    (L) => {
      if (L.stopPropagation(), !g) {
        if (t.groupId) {
          const X = [];
          let _ = t.groupId;
          for (; _; )
            X.push(_), _ = r.groupParent.get(_);
          if (!r.activeGroupId) {
            r.enterGroup(X[X.length - 1]), r.select(t.id);
            return;
          }
          const j = X.indexOf(r.activeGroupId);
          if (j > 0) {
            r.enterGroup(X[j - 1]), r.select(t.id);
            return;
          }
        }
        r.select(t.id), I.current = { x: L.clientX, y: L.clientY }, b(!0);
      }
    },
    [g, r, t.id, t.groupId, M]
  ), Q = e && !o;
  return /* @__PURE__ */ k(
    "div",
    {
      ref: c,
      "data-node-id": t.id,
      className: s ? void 0 : "sb-block-inert",
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        zIndex: t.z,
        height: t.h === "auto" ? void 0 : t.h,
        minHeight: 20,
        border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : "none",
        boxSizing: t.data.borderColor ? "border-box" : void 0,
        outline: e ? `${1.5 / i}px dashed #3b82f6` : "none",
        outlineOffset: t.data.borderColor ? 2 / i : 0,
        borderRadius: t.data.edgeStyle === "round" ? 12 : 0,
        background: "transparent",
        boxShadow: "none",
        overflow: "visible",
        pointerEvents: s ? "auto" : "none",
        display: "flex",
        flexDirection: "column",
        opacity: t.data.opacity ?? 1,
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0
      },
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            onDoubleClick: Z,
            style: {
              overflow: "hidden",
              height: "100%",
              display: "flex",
              flexDirection: "column",
              borderRadius: t.data.borderColor || e ? 7 : 0
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                className: "sb-editor-wrap",
                onPointerDown: et,
                onKeyDown: g ? (L) => {
                  L.key === "Escape" && (L.stopPropagation(), b(!1));
                } : void 0,
                style: g ? { cursor: "text", userSelect: "text" } : { cursor: "move", userSelect: "none" },
                children: x ? /* @__PURE__ */ u(Cs, { markdown: t.data.markdown ?? "" }) : /* @__PURE__ */ u($l, { fallback: /* @__PURE__ */ u(Cs, { markdown: t.data.markdown ?? "" }), children: /* @__PURE__ */ u(
                  _a,
                  {
                    editor: M,
                    theme: "light",
                    editable: s && g
                  }
                ) })
              }
            )
          }
        ),
        Q && tc.map(({ pos: L, top: X, left: _ }) => {
          const j = 8 / i;
          return /* @__PURE__ */ u(
            "div",
            {
              onPointerDown: (q) => B(L, q),
              style: {
                position: "absolute",
                top: X,
                left: _,
                width: j,
                height: j,
                transform: "translate(-50%, -50%)",
                background: "white",
                border: `${1.5 / i}px solid #3b82f6`,
                cursor: Br(L, t.rotation || 0),
                zIndex: 10,
                pointerEvents: "auto"
              }
            },
            L
          );
        }),
        Q && (() => {
          const L = 25 / i, X = 10 / i;
          return /* @__PURE__ */ k(ut, { children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  position: "absolute",
                  top: -L,
                  left: "50%",
                  width: 1.5 / i,
                  height: L,
                  transform: "translateX(-50%)",
                  background: "#3b82f6",
                  pointerEvents: "none"
                }
              }
            ),
            /* @__PURE__ */ u(
              "div",
              {
                onPointerDown: at,
                style: {
                  position: "absolute",
                  top: -(L + X / 2),
                  left: "50%",
                  width: X,
                  height: X,
                  transform: "translateX(-50%) rotate(45deg)",
                  borderRadius: 1.5 / i,
                  background: "white",
                  border: `${1.5 / i}px solid #3b82f6`,
                  cursor: "grab",
                  zIndex: 10,
                  pointerEvents: "auto"
                }
              }
            )
          ] });
        })()
      ]
    }
  );
}
const Li = de(oc);
function rc(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Li,
    {
      node: e,
      isSelected: t.isSelected,
      multiSelected: t.multiSelected,
      engine: t.engine,
      schema: Nn,
      interactive: t.interactive,
      zoom: t.zoom,
      onMeasuredHeight: t.callbacks.onMeasuredHeight
    }
  );
}
const nc = {
  type: "content",
  component: rc,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.markdown || null
}, { PI: sc } = Math, rr = sc + 1e-4, zs = 0.5, Is = [1, 1];
function Ts(t, e, o, r = (n) => n) {
  return t * r(0.5 - e * (0.5 - o));
}
const { min: tn } = Math;
function Di(t, e, o) {
  let r = tn(1, e / o);
  return tn(1, t + (tn(1, 1 - r) - t) * (r * 0.275));
}
function ic(t) {
  return [-t[0], -t[1]];
}
function Pe(t, e) {
  return [t[0] + e[0], t[1] + e[1]];
}
function Ps(t, e, o) {
  return t[0] = e[0] + o[0], t[1] = e[1] + o[1], t;
}
function qe(t, e) {
  return [t[0] - e[0], t[1] - e[1]];
}
function Mn(t, e, o) {
  return t[0] = e[0] - o[0], t[1] = e[1] - o[1], t;
}
function Ve(t, e) {
  return [t[0] * e, t[1] * e];
}
function en(t, e, o) {
  return t[0] = e[0] * o, t[1] = e[1] * o, t;
}
function ac(t, e) {
  return [t[0] / e, t[1] / e];
}
function Ei(t) {
  return [t[1], -t[0]];
}
function on(t, e) {
  let o = e[0];
  return t[0] = e[1], t[1] = -o, t;
}
function Rs(t, e) {
  return t[0] * e[0] + t[1] * e[1];
}
function lc(t, e) {
  return t[0] === e[0] && t[1] === e[1];
}
function cc(t) {
  return Math.hypot(t[0], t[1]);
}
function As(t, e) {
  let o = t[0] - e[0], r = t[1] - e[1];
  return o * o + r * r;
}
function Wi(t) {
  return ac(t, cc(t));
}
function dc(t, e) {
  return Math.hypot(t[1] - e[1], t[0] - e[0]);
}
function Gn(t, e, o) {
  let r = Math.sin(o), n = Math.cos(o), s = t[0] - e[0], i = t[1] - e[1], l = s * n - i * r, a = s * r + i * n;
  return [l + e[0], a + e[1]];
}
function Ls(t, e, o, r) {
  let n = Math.sin(r), s = Math.cos(r), i = e[0] - o[0], l = e[1] - o[1], a = i * s - l * n, c = i * n + l * s;
  return t[0] = a + o[0], t[1] = c + o[1], t;
}
function Ds(t, e, o) {
  return Pe(t, Ve(qe(e, t), o));
}
function hc(t, e, o, r) {
  let n = o[0] - e[0], s = o[1] - e[1];
  return t[0] = e[0] + n * r, t[1] = e[1] + s * r, t;
}
function Fi(t, e, o) {
  return Pe(t, Ve(e, o));
}
const ie = [0, 0], Ye = [0, 0], Ge = [0, 0];
function uc(t, e) {
  let o = Fi(t, Wi(Ei(qe(t, Pe(t, [1, 1])))), -e), r = [], n = 1 / 13;
  for (let s = n; s <= 1; s += n) r.push(Gn(o, t, rr * 2 * s));
  return r;
}
function pc(t, e, o) {
  let r = [], n = 1 / o;
  for (let s = n; s <= 1; s += n) r.push(Gn(e, t, rr * s));
  return r;
}
function fc(t, e, o) {
  let r = qe(e, o), n = Ve(r, 0.5), s = Ve(r, 0.51);
  return [qe(t, n), qe(t, s), Pe(t, s), Pe(t, n)];
}
function yc(t, e, o, r) {
  let n = [], s = Fi(t, e, o), i = 1 / r;
  for (let l = i; l < 1; l += i) n.push(Gn(s, t, rr * 3 * l));
  return n;
}
function mc(t, e, o) {
  return [Pe(t, Ve(e, o)), Pe(t, Ve(e, o * 0.99)), qe(t, Ve(e, o * 0.99)), qe(t, Ve(e, o))];
}
function Es(t, e, o) {
  return t === !1 || t === void 0 ? 0 : t === !0 ? Math.max(e, o) : t;
}
function gc(t, e, o) {
  return t.slice(0, 10).reduce((r, n) => {
    let s = n.pressure;
    return e && (s = Di(r, n.distance, o)), (r + s) / 2;
  }, t[0].pressure);
}
function bc(t, e = {}) {
  let { size: o = 16, smoothing: r = 0.5, thinning: n = 0.5, simulatePressure: s = !0, easing: i = (X) => X, start: l = {}, end: a = {}, last: c = !1 } = e, { cap: h = !0, easing: p = (X) => X * (2 - X) } = l, { cap: d = !0, easing: f = (X) => --X * X * X + 1 } = a;
  if (t.length === 0 || o <= 0) return [];
  let m = t[t.length - 1].runningLength, y = Es(l.taper, o, m), g = Es(a.taper, o, m), b = (o * r) ** 2, x = [], C = [], I = gc(t, s, o), M = Ts(o, n, t[t.length - 1].pressure, i), D, T = t[0].vector, P = t[0].point, G = P, K = P, at = G, B = !1;
  for (let X = 0; X < t.length; X++) {
    let { pressure: _ } = t[X], { point: j, vector: q, distance: Y, runningLength: $ } = t[X], nt = X === t.length - 1;
    if (!nt && m - $ < 3) continue;
    n ? (s && (_ = Di(I, Y, o)), M = Ts(o, n, _, i)) : M = o / 2, D === void 0 && (D = M);
    let ot = $ < y ? p($ / y) : 1, ft = m - $ < g ? f((m - $) / g) : 1;
    M = Math.max(0.01, M * Math.min(ot, ft));
    let pt = (nt ? t[X] : t[X + 1]).vector, rt = nt ? 1 : Rs(q, pt), It = Rs(q, T) < 0 && !B, zt = rt !== null && rt < 0;
    if (It || zt) {
      on(ie, T), en(ie, ie, M);
      for (let bt = 0; bt <= 1; bt += 0.07692307692307693) Mn(Ye, j, ie), Ls(Ye, Ye, j, rr * bt), K = [Ye[0], Ye[1]], x.push(K), Ps(Ge, j, ie), Ls(Ge, Ge, j, rr * -bt), at = [Ge[0], Ge[1]], C.push(at);
      P = K, G = at, zt && (B = !0);
      continue;
    }
    if (B = !1, nt) {
      on(ie, q), en(ie, ie, M), x.push(qe(j, ie)), C.push(Pe(j, ie));
      continue;
    }
    hc(ie, pt, q, rt), on(ie, ie), en(ie, ie, M), Mn(Ye, j, ie), K = [Ye[0], Ye[1]], (X <= 1 || As(P, K) > b) && (x.push(K), P = K), Ps(Ge, j, ie), at = [Ge[0], Ge[1]], (X <= 1 || As(G, at) > b) && (C.push(at), G = at), I = _, T = q;
  }
  let et = [t[0].point[0], t[0].point[1]], Z = t.length > 1 ? [t[t.length - 1].point[0], t[t.length - 1].point[1]] : Pe(t[0].point, [1, 1]), Q = [], L = [];
  if (t.length === 1) {
    if (!(y || g) || c) return uc(et, D || M);
  } else {
    y || g && t.length === 1 || (h ? Q.push(...pc(et, C[0], 13)) : Q.push(...fc(et, x[0], C[0])));
    let X = Ei(ic(t[t.length - 1].vector));
    g || y && t.length === 1 ? L.push(Z) : d ? L.push(...yc(Z, X, M, 29)) : L.push(...mc(Z, X, M));
  }
  return x.concat(L, C.reverse(), Q);
}
const Ws = [0, 0];
function Fs(t) {
  return t != null && t >= 0;
}
function xc(t, e = {}) {
  var d;
  let { streamline: o = 0.5, size: r = 16, last: n = !1 } = e;
  if (t.length === 0) return [];
  let s = 0.15 + (1 - o) * 0.85, i = Array.isArray(t[0]) ? t : t.map(({ x: f, y: m, pressure: y = zs }) => [f, m, y]);
  if (i.length === 2) {
    let f = i[1];
    i = i.slice(0, -1);
    for (let m = 1; m < 5; m++) i.push(Ds(i[0], f, m / 4));
  }
  i.length === 1 && (i = [...i, [...Pe(i[0], Is), ...i[0].slice(2)]]);
  let l = [{ point: [i[0][0], i[0][1]], pressure: Fs(i[0][2]) ? i[0][2] : 0.25, vector: [...Is], distance: 0, runningLength: 0 }], a = !1, c = 0, h = l[0], p = i.length - 1;
  for (let f = 1; f < i.length; f++) {
    let m = n && f === p ? [i[f][0], i[f][1]] : Ds(h.point, i[f], s);
    if (lc(h.point, m)) continue;
    let y = dc(m, h.point);
    if (c += y, f < p && !a) {
      if (c < r) continue;
      a = !0;
    }
    Mn(Ws, h.point, m), h = { point: m, pressure: Fs(i[f][2]) ? i[f][2] : zs, vector: Wi(Ws), distance: y, runningLength: c }, l.push(h);
  }
  return l[0].vector = ((d = l[1]) == null ? void 0 : d.vector) || [0, 0], l;
}
function wc(t, e = {}) {
  return bc(xc(t, e), e);
}
var kc = wc;
function jn(t, e = {}) {
  const o = kc(t, {
    size: e.size || 4,
    thinning: e.thinning ?? 0.5,
    smoothing: e.smoothing ?? 0.5,
    streamline: e.streamline ?? 0.5
  });
  return vc(o);
}
function vc(t) {
  if (!t.length) return "";
  const e = [], [o, r] = t[0];
  e.push("M", o, r);
  for (let n = 0; n < t.length; n++) {
    const [s, i] = t[n], [l, a] = t[(n + 1) % t.length];
    e.push("Q", s, i, (s + l) / 2, (i + a) / 2);
  }
  return e.push("Z"), e.join(" ");
}
function Bi(t, e = 0.5) {
  if (t.length < 2) return t;
  const o = 0.15 + (1 - e) * 0.85, r = [[t[0][0], t[0][1]]];
  for (let n = 1; n < t.length; n++) {
    const s = r[n - 1];
    r.push([
      s[0] + (t[n][0] - s[0]) * o,
      s[1] + (t[n][1] - s[1]) * o
    ]);
  }
  return r;
}
function Sc(t, e = 0.5) {
  if (t.length < 2) return "";
  const o = Bi(t, e), r = o.length, n = [];
  n.push("M", o[0][0], o[0][1]);
  for (let s = 0; s < r; s++) {
    const [i, l] = o[s], [a, c] = o[(s + 1) % r];
    n.push("Q", i, l, (i + a) / 2, (l + c) / 2);
  }
  return n.push("Z"), n.join(" ");
}
function Mc(t, e, o, r) {
  const n = e[0] - t[0], s = e[1] - t[1], i = r[0] - o[0], l = r[1] - o[1], a = n * l - s * i;
  if (Math.abs(a) < 1e-10) return null;
  const c = ((o[0] - t[0]) * l - (o[1] - t[1]) * i) / a, h = ((o[0] - t[0]) * s - (o[1] - t[1]) * n) / a;
  return c <= 0 || c >= 1 || h <= 0 || h >= 1 ? null : [t[0] + c * n, t[1] + c * s];
}
function Cc(t) {
  if (t.length < 2) return "";
  let e = `M ${t[0][0]},${t[0][1]}`;
  for (let o = 1; o < t.length; o++)
    e += ` L ${t[o][0]},${t[o][1]}`;
  return e + " Z";
}
function Bs(t) {
  let e = 0;
  for (let o = 0, r = t.length - 1; o < t.length; r = o++)
    e += (t[r][0] + t[o][0]) * (t[r][1] - t[o][1]);
  return Math.abs(e) / 2;
}
function zc(t) {
  if (t.length < 4) return [];
  const e = t.length, o = [];
  for (let i = 0; i < e - 1; i++)
    for (let l = i + 2; l < e - 1; l++) {
      const a = Mc(
        t[i],
        t[i + 1],
        t[l],
        t[l + 1]
      );
      if (!a) continue;
      const c = [a];
      for (let h = i + 1; h <= l; h++)
        c.push(t[h]);
      Bs(c) < 100 || o.push({
        pathD: Cc(c),
        points: c.map((h) => [h[0], h[1]])
      });
    }
  if (o.length === 0) return [];
  const r = o.map((i) => Bs(i.points)), s = Math.max(...r) * 0.05;
  return o.filter((i, l) => r[l] >= s);
}
function rn(t, e, o) {
  if (t && t.length) {
    const [r, n] = e, s = Math.PI / 180 * o, i = Math.cos(s), l = Math.sin(s);
    for (const a of t) {
      const [c, h] = a;
      a[0] = (c - r) * i - (h - n) * l + r, a[1] = (c - r) * l + (h - n) * i + n;
    }
  }
}
function Ic(t, e) {
  return t[0] === e[0] && t[1] === e[1];
}
function Tc(t, e, o, r = 1) {
  const n = o, s = Math.max(e, 0.1), i = t[0] && t[0][0] && typeof t[0][0] == "number" ? [t] : t, l = [0, 0];
  if (n) for (const c of i) rn(c, l, n);
  const a = function(c, h, p) {
    const d = [];
    for (const x of c) {
      const C = [...x];
      Ic(C[0], C[C.length - 1]) || C.push([C[0][0], C[0][1]]), C.length > 2 && d.push(C);
    }
    const f = [];
    h = Math.max(h, 0.1);
    const m = [];
    for (const x of d) for (let C = 0; C < x.length - 1; C++) {
      const I = x[C], M = x[C + 1];
      if (I[1] !== M[1]) {
        const D = Math.min(I[1], M[1]);
        m.push({ ymin: D, ymax: Math.max(I[1], M[1]), x: D === I[1] ? I[0] : M[0], islope: (M[0] - I[0]) / (M[1] - I[1]) });
      }
    }
    if (m.sort((x, C) => x.ymin < C.ymin ? -1 : x.ymin > C.ymin ? 1 : x.x < C.x ? -1 : x.x > C.x ? 1 : x.ymax === C.ymax ? 0 : (x.ymax - C.ymax) / Math.abs(x.ymax - C.ymax)), !m.length) return f;
    let y = [], g = m[0].ymin, b = 0;
    for (; y.length || m.length; ) {
      if (m.length) {
        let x = -1;
        for (let C = 0; C < m.length && !(m[C].ymin > g); C++) x = C;
        m.splice(0, x + 1).forEach((C) => {
          y.push({ s: g, edge: C });
        });
      }
      if (y = y.filter((x) => !(x.edge.ymax <= g)), y.sort((x, C) => x.edge.x === C.edge.x ? 0 : (x.edge.x - C.edge.x) / Math.abs(x.edge.x - C.edge.x)), (p !== 1 || b % h == 0) && y.length > 1) for (let x = 0; x < y.length; x += 2) {
        const C = x + 1;
        if (C >= y.length) break;
        const I = y[x].edge, M = y[C].edge;
        f.push([[Math.round(I.x), g], [Math.round(M.x), g]]);
      }
      g += p, y.forEach((x) => {
        x.edge.x = x.edge.x + p * x.edge.islope;
      }), b++;
    }
    return f;
  }(i, s, r);
  if (n) {
    for (const c of i) rn(c, l, -n);
    (function(c, h, p) {
      const d = [];
      c.forEach((f) => d.push(...f)), rn(d, h, p);
    })(a, l, -n);
  }
  return a;
}
function nr(t, e) {
  var o;
  const r = e.hachureAngle + 90;
  let n = e.hachureGap;
  n < 0 && (n = 4 * e.strokeWidth), n = Math.round(Math.max(n, 0.1));
  let s = 1;
  return e.roughness >= 1 && (((o = e.randomizer) === null || o === void 0 ? void 0 : o.next()) || Math.random()) > 0.7 && (s = n), Tc(t, n, r, s || 1);
}
class Vn {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    return this._fillPolygons(e, o);
  }
  _fillPolygons(e, o) {
    const r = nr(e, o);
    return { type: "fillSketch", ops: this.renderLines(r, o) };
  }
  renderLines(e, o) {
    const r = [];
    for (const n of e) r.push(...this.helper.doubleLineOps(n[0][0], n[0][1], n[1][0], n[1][1], o));
    return r;
  }
}
function Nr(t) {
  const e = t[0], o = t[1];
  return Math.sqrt(Math.pow(e[0] - o[0], 2) + Math.pow(e[1] - o[1], 2));
}
class Pc extends Vn {
  fillPolygons(e, o) {
    let r = o.hachureGap;
    r < 0 && (r = 4 * o.strokeWidth), r = Math.max(r, 0.1);
    const n = nr(e, Object.assign({}, o, { hachureGap: r })), s = Math.PI / 180 * o.hachureAngle, i = [], l = 0.5 * r * Math.cos(s), a = 0.5 * r * Math.sin(s);
    for (const [c, h] of n) Nr([c, h]) && i.push([[c[0] - l, c[1] + a], [...h]], [[c[0] + l, c[1] - a], [...h]]);
    return { type: "fillSketch", ops: this.renderLines(i, o) };
  }
}
class Rc extends Vn {
  fillPolygons(e, o) {
    const r = this._fillPolygons(e, o), n = Object.assign({}, o, { hachureAngle: o.hachureAngle + 90 }), s = this._fillPolygons(e, n);
    return r.ops = r.ops.concat(s.ops), r;
  }
}
class Ac {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const r = nr(e, o = Object.assign({}, o, { hachureAngle: 0 }));
    return this.dotsOnLines(r, o);
  }
  dotsOnLines(e, o) {
    const r = [];
    let n = o.hachureGap;
    n < 0 && (n = 4 * o.strokeWidth), n = Math.max(n, 0.1);
    let s = o.fillWeight;
    s < 0 && (s = o.strokeWidth / 2);
    const i = n / 4;
    for (const l of e) {
      const a = Nr(l), c = a / n, h = Math.ceil(c) - 1, p = a - h * n, d = (l[0][0] + l[1][0]) / 2 - n / 4, f = Math.min(l[0][1], l[1][1]);
      for (let m = 0; m < h; m++) {
        const y = f + p + m * n, g = d - i + 2 * Math.random() * i, b = y - i + 2 * Math.random() * i, x = this.helper.ellipse(g, b, s, s, o);
        r.push(...x.ops);
      }
    }
    return { type: "fillSketch", ops: r };
  }
}
class Lc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const r = nr(e, o);
    return { type: "fillSketch", ops: this.dashedLine(r, o) };
  }
  dashedLine(e, o) {
    const r = o.dashOffset < 0 ? o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap : o.dashOffset, n = o.dashGap < 0 ? o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap : o.dashGap, s = [];
    return e.forEach((i) => {
      const l = Nr(i), a = Math.floor(l / (r + n)), c = (l + n - a * (r + n)) / 2;
      let h = i[0], p = i[1];
      h[0] > p[0] && (h = i[1], p = i[0]);
      const d = Math.atan((p[1] - h[1]) / (p[0] - h[0]));
      for (let f = 0; f < a; f++) {
        const m = f * (r + n), y = m + r, g = [h[0] + m * Math.cos(d) + c * Math.cos(d), h[1] + m * Math.sin(d) + c * Math.sin(d)], b = [h[0] + y * Math.cos(d) + c * Math.cos(d), h[1] + y * Math.sin(d) + c * Math.sin(d)];
        s.push(...this.helper.doubleLineOps(g[0], g[1], b[0], b[1], o));
      }
    }), s;
  }
}
class Dc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const r = o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap, n = o.zigzagOffset < 0 ? r : o.zigzagOffset, s = nr(e, o = Object.assign({}, o, { hachureGap: r + n }));
    return { type: "fillSketch", ops: this.zigzagLines(s, n, o) };
  }
  zigzagLines(e, o, r) {
    const n = [];
    return e.forEach((s) => {
      const i = Nr(s), l = Math.round(i / (2 * o));
      let a = s[0], c = s[1];
      a[0] > c[0] && (a = s[1], c = s[0]);
      const h = Math.atan((c[1] - a[1]) / (c[0] - a[0]));
      for (let p = 0; p < l; p++) {
        const d = 2 * p * o, f = 2 * (p + 1) * o, m = Math.sqrt(2 * Math.pow(o, 2)), y = [a[0] + d * Math.cos(h), a[1] + d * Math.sin(h)], g = [a[0] + f * Math.cos(h), a[1] + f * Math.sin(h)], b = [y[0] + m * Math.cos(h + Math.PI / 4), y[1] + m * Math.sin(h + Math.PI / 4)];
        n.push(...this.helper.doubleLineOps(y[0], y[1], b[0], b[1], r), ...this.helper.doubleLineOps(b[0], b[1], g[0], g[1], r));
      }
    }), n;
  }
}
const ce = {};
class Ec {
  constructor(e) {
    this.seed = e;
  }
  next() {
    return this.seed ? (2 ** 31 - 1 & (this.seed = Math.imul(48271, this.seed))) / 2 ** 31 : Math.random();
  }
}
const Wc = 0, nn = 1, Ns = 2, mr = { A: 7, a: 7, C: 6, c: 6, H: 1, h: 1, L: 2, l: 2, M: 2, m: 2, Q: 4, q: 4, S: 4, s: 4, T: 2, t: 2, V: 1, v: 1, Z: 0, z: 0 };
function sn(t, e) {
  return t.type === e;
}
function Kn(t) {
  const e = [], o = function(i) {
    const l = new Array();
    for (; i !== ""; ) if (i.match(/^([ \t\r\n,]+)/)) i = i.substr(RegExp.$1.length);
    else if (i.match(/^([aAcChHlLmMqQsStTvVzZ])/)) l[l.length] = { type: Wc, text: RegExp.$1 }, i = i.substr(RegExp.$1.length);
    else {
      if (!i.match(/^(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)/)) return [];
      l[l.length] = { type: nn, text: `${parseFloat(RegExp.$1)}` }, i = i.substr(RegExp.$1.length);
    }
    return l[l.length] = { type: Ns, text: "" }, l;
  }(t);
  let r = "BOD", n = 0, s = o[n];
  for (; !sn(s, Ns); ) {
    let i = 0;
    const l = [];
    if (r === "BOD") {
      if (s.text !== "M" && s.text !== "m") return Kn("M0,0" + t);
      n++, i = mr[s.text], r = s.text;
    } else sn(s, nn) ? i = mr[r] : (n++, i = mr[s.text], r = s.text);
    if (!(n + i < o.length)) throw new Error("Path data ended short");
    for (let a = n; a < n + i; a++) {
      const c = o[a];
      if (!sn(c, nn)) throw new Error("Param not a number: " + r + "," + c.text);
      l[l.length] = +c.text;
    }
    if (typeof mr[r] != "number") throw new Error("Bad segment: " + r);
    {
      const a = { key: r, data: l };
      e.push(a), n += i, s = o[n], r === "M" && (r = "L"), r === "m" && (r = "l");
    }
  }
  return e;
}
function Ni(t) {
  let e = 0, o = 0, r = 0, n = 0;
  const s = [];
  for (const { key: i, data: l } of t) switch (i) {
    case "M":
      s.push({ key: "M", data: [...l] }), [e, o] = l, [r, n] = l;
      break;
    case "m":
      e += l[0], o += l[1], s.push({ key: "M", data: [e, o] }), r = e, n = o;
      break;
    case "L":
      s.push({ key: "L", data: [...l] }), [e, o] = l;
      break;
    case "l":
      e += l[0], o += l[1], s.push({ key: "L", data: [e, o] });
      break;
    case "C":
      s.push({ key: "C", data: [...l] }), e = l[4], o = l[5];
      break;
    case "c": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "C", data: a }), e = a[4], o = a[5];
      break;
    }
    case "Q":
      s.push({ key: "Q", data: [...l] }), e = l[2], o = l[3];
      break;
    case "q": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "Q", data: a }), e = a[2], o = a[3];
      break;
    }
    case "A":
      s.push({ key: "A", data: [...l] }), e = l[5], o = l[6];
      break;
    case "a":
      e += l[5], o += l[6], s.push({ key: "A", data: [l[0], l[1], l[2], l[3], l[4], e, o] });
      break;
    case "H":
      s.push({ key: "H", data: [...l] }), e = l[0];
      break;
    case "h":
      e += l[0], s.push({ key: "H", data: [e] });
      break;
    case "V":
      s.push({ key: "V", data: [...l] }), o = l[0];
      break;
    case "v":
      o += l[0], s.push({ key: "V", data: [o] });
      break;
    case "S":
      s.push({ key: "S", data: [...l] }), e = l[2], o = l[3];
      break;
    case "s": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "S", data: a }), e = a[2], o = a[3];
      break;
    }
    case "T":
      s.push({ key: "T", data: [...l] }), e = l[0], o = l[1];
      break;
    case "t":
      e += l[0], o += l[1], s.push({ key: "T", data: [e, o] });
      break;
    case "Z":
    case "z":
      s.push({ key: "Z", data: [] }), e = r, o = n;
  }
  return s;
}
function Hi(t) {
  const e = [];
  let o = "", r = 0, n = 0, s = 0, i = 0, l = 0, a = 0;
  for (const { key: c, data: h } of t) {
    switch (c) {
      case "M":
        e.push({ key: "M", data: [...h] }), [r, n] = h, [s, i] = h;
        break;
      case "C":
        e.push({ key: "C", data: [...h] }), r = h[4], n = h[5], l = h[2], a = h[3];
        break;
      case "L":
        e.push({ key: "L", data: [...h] }), [r, n] = h;
        break;
      case "H":
        r = h[0], e.push({ key: "L", data: [r, n] });
        break;
      case "V":
        n = h[0], e.push({ key: "L", data: [r, n] });
        break;
      case "S": {
        let p = 0, d = 0;
        o === "C" || o === "S" ? (p = r + (r - l), d = n + (n - a)) : (p = r, d = n), e.push({ key: "C", data: [p, d, ...h] }), l = h[0], a = h[1], r = h[2], n = h[3];
        break;
      }
      case "T": {
        const [p, d] = h;
        let f = 0, m = 0;
        o === "Q" || o === "T" ? (f = r + (r - l), m = n + (n - a)) : (f = r, m = n);
        const y = r + 2 * (f - r) / 3, g = n + 2 * (m - n) / 3, b = p + 2 * (f - p) / 3, x = d + 2 * (m - d) / 3;
        e.push({ key: "C", data: [y, g, b, x, p, d] }), l = f, a = m, r = p, n = d;
        break;
      }
      case "Q": {
        const [p, d, f, m] = h, y = r + 2 * (p - r) / 3, g = n + 2 * (d - n) / 3, b = f + 2 * (p - f) / 3, x = m + 2 * (d - m) / 3;
        e.push({ key: "C", data: [y, g, b, x, f, m] }), l = p, a = d, r = f, n = m;
        break;
      }
      case "A": {
        const p = Math.abs(h[0]), d = Math.abs(h[1]), f = h[2], m = h[3], y = h[4], g = h[5], b = h[6];
        p === 0 || d === 0 ? (e.push({ key: "C", data: [r, n, g, b, g, b] }), r = g, n = b) : (r !== g || n !== b) && (Oi(r, n, g, b, p, d, f, m, y).forEach(function(x) {
          e.push({ key: "C", data: x });
        }), r = g, n = b);
        break;
      }
      case "Z":
        e.push({ key: "Z", data: [] }), r = s, n = i;
    }
    o = c;
  }
  return e;
}
function Qo(t, e, o) {
  return [t * Math.cos(o) - e * Math.sin(o), t * Math.sin(o) + e * Math.cos(o)];
}
function Oi(t, e, o, r, n, s, i, l, a, c) {
  const h = (p = i, Math.PI * p / 180);
  var p;
  let d = [], f = 0, m = 0, y = 0, g = 0;
  if (c) [f, m, y, g] = c;
  else {
    [t, e] = Qo(t, e, -h), [o, r] = Qo(o, r, -h);
    const et = (t - o) / 2, Z = (e - r) / 2;
    let Q = et * et / (n * n) + Z * Z / (s * s);
    Q > 1 && (Q = Math.sqrt(Q), n *= Q, s *= Q);
    const L = n * n, X = s * s, _ = L * X - L * Z * Z - X * et * et, j = L * Z * Z + X * et * et, q = (l === a ? -1 : 1) * Math.sqrt(Math.abs(_ / j));
    y = q * n * Z / s + (t + o) / 2, g = q * -s * et / n + (e + r) / 2, f = Math.asin(parseFloat(((e - g) / s).toFixed(9))), m = Math.asin(parseFloat(((r - g) / s).toFixed(9))), t < y && (f = Math.PI - f), o < y && (m = Math.PI - m), f < 0 && (f = 2 * Math.PI + f), m < 0 && (m = 2 * Math.PI + m), a && f > m && (f -= 2 * Math.PI), !a && m > f && (m -= 2 * Math.PI);
  }
  let b = m - f;
  if (Math.abs(b) > 120 * Math.PI / 180) {
    const et = m, Z = o, Q = r;
    m = a && m > f ? f + 120 * Math.PI / 180 * 1 : f + 120 * Math.PI / 180 * -1, d = Oi(o = y + n * Math.cos(m), r = g + s * Math.sin(m), Z, Q, n, s, i, 0, a, [m, et, y, g]);
  }
  b = m - f;
  const x = Math.cos(f), C = Math.sin(f), I = Math.cos(m), M = Math.sin(m), D = Math.tan(b / 4), T = 4 / 3 * n * D, P = 4 / 3 * s * D, G = [t, e], K = [t + T * C, e - P * x], at = [o + T * M, r - P * I], B = [o, r];
  if (K[0] = 2 * G[0] - K[0], K[1] = 2 * G[1] - K[1], c) return [K, at, B].concat(d);
  {
    d = [K, at, B].concat(d);
    const et = [];
    for (let Z = 0; Z < d.length; Z += 3) {
      const Q = Qo(d[Z][0], d[Z][1], h), L = Qo(d[Z + 1][0], d[Z + 1][1], h), X = Qo(d[Z + 2][0], d[Z + 2][1], h);
      et.push([Q[0], Q[1], L[0], L[1], X[0], X[1]]);
    }
    return et;
  }
}
const Fc = { randOffset: function(t, e) {
  return At(t, e);
}, randOffsetWithRange: function(t, e, o) {
  return Tr(t, e, o);
}, ellipse: function(t, e, o, r, n) {
  const s = Yi(o, r, n);
  return Cn(t, e, n, s).opset;
}, doubleLineOps: function(t, e, o, r, n) {
  return Qe(t, e, o, r, n, !0);
} };
function Xi(t, e, o, r, n) {
  return { type: "path", ops: Qe(t, e, o, r, n) };
}
function Mr(t, e, o) {
  const r = (t || []).length;
  if (r > 2) {
    const n = [];
    for (let s = 0; s < r - 1; s++) n.push(...Qe(t[s][0], t[s][1], t[s + 1][0], t[s + 1][1], o));
    return e && n.push(...Qe(t[r - 1][0], t[r - 1][1], t[0][0], t[0][1], o)), { type: "path", ops: n };
  }
  return r === 2 ? Xi(t[0][0], t[0][1], t[1][0], t[1][1], o) : { type: "path", ops: [] };
}
function Bc(t, e, o, r, n) {
  return function(s, i) {
    return Mr(s, !0, i);
  }([[t, e], [t + o, e], [t + o, e + r], [t, e + r]], n);
}
function Hs(t, e) {
  if (t.length) {
    const o = typeof t[0][0] == "number" ? [t] : t, r = gr(o[0], 1 * (1 + 0.2 * e.roughness), e), n = e.disableMultiStroke ? [] : gr(o[0], 1.5 * (1 + 0.22 * e.roughness), Ys(e));
    for (let s = 1; s < o.length; s++) {
      const i = o[s];
      if (i.length) {
        const l = gr(i, 1 * (1 + 0.2 * e.roughness), e), a = e.disableMultiStroke ? [] : gr(i, 1.5 * (1 + 0.22 * e.roughness), Ys(e));
        for (const c of l) c.op !== "move" && r.push(c);
        for (const c of a) c.op !== "move" && n.push(c);
      }
    }
    return { type: "path", ops: r.concat(n) };
  }
  return { type: "path", ops: [] };
}
function Yi(t, e, o) {
  const r = Math.sqrt(2 * Math.PI * Math.sqrt((Math.pow(t / 2, 2) + Math.pow(e / 2, 2)) / 2)), n = Math.ceil(Math.max(o.curveStepCount, o.curveStepCount / Math.sqrt(200) * r)), s = 2 * Math.PI / n;
  let i = Math.abs(t / 2), l = Math.abs(e / 2);
  const a = 1 - o.curveFitting;
  return i += At(i * a, o), l += At(l * a, o), { increment: s, rx: i, ry: l };
}
function Cn(t, e, o, r) {
  const [n, s] = Gs(r.increment, t, e, r.rx, r.ry, 1, r.increment * Tr(0.1, Tr(0.4, 1, o), o), o);
  let i = Pr(n, null, o);
  if (!o.disableMultiStroke && o.roughness !== 0) {
    const [l] = Gs(r.increment, t, e, r.rx, r.ry, 1.5, 0, o), a = Pr(l, null, o);
    i = i.concat(a);
  }
  return { estimatedPoints: s, opset: { type: "path", ops: i } };
}
function Os(t, e, o, r, n, s, i, l, a) {
  const c = t, h = e;
  let p = Math.abs(o / 2), d = Math.abs(r / 2);
  p += At(0.01 * p, a), d += At(0.01 * d, a);
  let f = n, m = s;
  for (; f < 0; ) f += 2 * Math.PI, m += 2 * Math.PI;
  m - f > 2 * Math.PI && (f = 0, m = 2 * Math.PI);
  const y = 2 * Math.PI / a.curveStepCount, g = Math.min(y / 2, (m - f) / 2), b = js(g, c, h, p, d, f, m, 1, a);
  if (!a.disableMultiStroke) {
    const x = js(g, c, h, p, d, f, m, 1.5, a);
    b.push(...x);
  }
  return i && (l ? b.push(...Qe(c, h, c + p * Math.cos(f), h + d * Math.sin(f), a), ...Qe(c, h, c + p * Math.cos(m), h + d * Math.sin(m), a)) : b.push({ op: "lineTo", data: [c, h] }, { op: "lineTo", data: [c + p * Math.cos(f), h + d * Math.sin(f)] })), { type: "path", ops: b };
}
function Xs(t, e) {
  const o = Hi(Ni(Kn(t))), r = [];
  let n = [0, 0], s = [0, 0];
  for (const { key: i, data: l } of o) switch (i) {
    case "M":
      s = [l[0], l[1]], n = [l[0], l[1]];
      break;
    case "L":
      r.push(...Qe(s[0], s[1], l[0], l[1], e)), s = [l[0], l[1]];
      break;
    case "C": {
      const [a, c, h, p, d, f] = l;
      r.push(...Nc(a, c, h, p, d, f, s, e)), s = [d, f];
      break;
    }
    case "Z":
      r.push(...Qe(s[0], s[1], n[0], n[1], e)), s = [n[0], n[1]];
  }
  return { type: "path", ops: r };
}
function an(t, e) {
  const o = [];
  for (const r of t) if (r.length) {
    const n = e.maxRandomnessOffset || 0, s = r.length;
    if (s > 2) {
      o.push({ op: "move", data: [r[0][0] + At(n, e), r[0][1] + At(n, e)] });
      for (let i = 1; i < s; i++) o.push({ op: "lineTo", data: [r[i][0] + At(n, e), r[i][1] + At(n, e)] });
    }
  }
  return { type: "fillPath", ops: o };
}
function Co(t, e) {
  return function(o, r) {
    let n = o.fillStyle || "hachure";
    if (!ce[n]) switch (n) {
      case "zigzag":
        ce[n] || (ce[n] = new Pc(r));
        break;
      case "cross-hatch":
        ce[n] || (ce[n] = new Rc(r));
        break;
      case "dots":
        ce[n] || (ce[n] = new Ac(r));
        break;
      case "dashed":
        ce[n] || (ce[n] = new Lc(r));
        break;
      case "zigzag-line":
        ce[n] || (ce[n] = new Dc(r));
        break;
      default:
        n = "hachure", ce[n] || (ce[n] = new Vn(r));
    }
    return ce[n];
  }(e, Fc).fillPolygons(t, e);
}
function Ys(t) {
  const e = Object.assign({}, t);
  return e.randomizer = void 0, t.seed && (e.seed = t.seed + 1), e;
}
function Gi(t) {
  return t.randomizer || (t.randomizer = new Ec(t.seed || 0)), t.randomizer.next();
}
function Tr(t, e, o, r = 1) {
  return o.roughness * r * (Gi(o) * (e - t) + t);
}
function At(t, e, o = 1) {
  return Tr(-t, t, e, o);
}
function Qe(t, e, o, r, n, s = !1) {
  const i = s ? n.disableMultiStrokeFill : n.disableMultiStroke, l = zn(t, e, o, r, n, !0, !1);
  if (i) return l;
  const a = zn(t, e, o, r, n, !0, !0);
  return l.concat(a);
}
function zn(t, e, o, r, n, s, i) {
  const l = Math.pow(t - o, 2) + Math.pow(e - r, 2), a = Math.sqrt(l);
  let c = 1;
  c = a < 200 ? 1 : a > 500 ? 0.4 : -16668e-7 * a + 1.233334;
  let h = n.maxRandomnessOffset || 0;
  h * h * 100 > l && (h = a / 10);
  const p = h / 2, d = 0.2 + 0.2 * Gi(n);
  let f = n.bowing * n.maxRandomnessOffset * (r - e) / 200, m = n.bowing * n.maxRandomnessOffset * (t - o) / 200;
  f = At(f, n, c), m = At(m, n, c);
  const y = [], g = () => At(p, n, c), b = () => At(h, n, c), x = n.preserveVertices;
  return i ? y.push({ op: "move", data: [t + (x ? 0 : g()), e + (x ? 0 : g())] }) : y.push({ op: "move", data: [t + (x ? 0 : At(h, n, c)), e + (x ? 0 : At(h, n, c))] }), i ? y.push({ op: "bcurveTo", data: [f + t + (o - t) * d + g(), m + e + (r - e) * d + g(), f + t + 2 * (o - t) * d + g(), m + e + 2 * (r - e) * d + g(), o + (x ? 0 : g()), r + (x ? 0 : g())] }) : y.push({ op: "bcurveTo", data: [f + t + (o - t) * d + b(), m + e + (r - e) * d + b(), f + t + 2 * (o - t) * d + b(), m + e + 2 * (r - e) * d + b(), o + (x ? 0 : b()), r + (x ? 0 : b())] }), y;
}
function gr(t, e, o) {
  if (!t.length) return [];
  const r = [];
  r.push([t[0][0] + At(e, o), t[0][1] + At(e, o)]), r.push([t[0][0] + At(e, o), t[0][1] + At(e, o)]);
  for (let n = 1; n < t.length; n++) r.push([t[n][0] + At(e, o), t[n][1] + At(e, o)]), n === t.length - 1 && r.push([t[n][0] + At(e, o), t[n][1] + At(e, o)]);
  return Pr(r, null, o);
}
function Pr(t, e, o) {
  const r = t.length, n = [];
  if (r > 3) {
    const s = [], i = 1 - o.curveTightness;
    n.push({ op: "move", data: [t[1][0], t[1][1]] });
    for (let l = 1; l + 2 < r; l++) {
      const a = t[l];
      s[0] = [a[0], a[1]], s[1] = [a[0] + (i * t[l + 1][0] - i * t[l - 1][0]) / 6, a[1] + (i * t[l + 1][1] - i * t[l - 1][1]) / 6], s[2] = [t[l + 1][0] + (i * t[l][0] - i * t[l + 2][0]) / 6, t[l + 1][1] + (i * t[l][1] - i * t[l + 2][1]) / 6], s[3] = [t[l + 1][0], t[l + 1][1]], n.push({ op: "bcurveTo", data: [s[1][0], s[1][1], s[2][0], s[2][1], s[3][0], s[3][1]] });
    }
  } else r === 3 ? (n.push({ op: "move", data: [t[1][0], t[1][1]] }), n.push({ op: "bcurveTo", data: [t[1][0], t[1][1], t[2][0], t[2][1], t[2][0], t[2][1]] })) : r === 2 && n.push(...zn(t[0][0], t[0][1], t[1][0], t[1][1], o, !0, !0));
  return n;
}
function Gs(t, e, o, r, n, s, i, l) {
  const a = [], c = [];
  if (l.roughness === 0) {
    t /= 4, c.push([e + r * Math.cos(-t), o + n * Math.sin(-t)]);
    for (let h = 0; h <= 2 * Math.PI; h += t) {
      const p = [e + r * Math.cos(h), o + n * Math.sin(h)];
      a.push(p), c.push(p);
    }
    c.push([e + r * Math.cos(0), o + n * Math.sin(0)]), c.push([e + r * Math.cos(t), o + n * Math.sin(t)]);
  } else {
    const h = At(0.5, l) - Math.PI / 2;
    c.push([At(s, l) + e + 0.9 * r * Math.cos(h - t), At(s, l) + o + 0.9 * n * Math.sin(h - t)]);
    const p = 2 * Math.PI + h - 0.01;
    for (let d = h; d < p; d += t) {
      const f = [At(s, l) + e + r * Math.cos(d), At(s, l) + o + n * Math.sin(d)];
      a.push(f), c.push(f);
    }
    c.push([At(s, l) + e + r * Math.cos(h + 2 * Math.PI + 0.5 * i), At(s, l) + o + n * Math.sin(h + 2 * Math.PI + 0.5 * i)]), c.push([At(s, l) + e + 0.98 * r * Math.cos(h + i), At(s, l) + o + 0.98 * n * Math.sin(h + i)]), c.push([At(s, l) + e + 0.9 * r * Math.cos(h + 0.5 * i), At(s, l) + o + 0.9 * n * Math.sin(h + 0.5 * i)]);
  }
  return [c, a];
}
function js(t, e, o, r, n, s, i, l, a) {
  const c = s + At(0.1, a), h = [];
  h.push([At(l, a) + e + 0.9 * r * Math.cos(c - t), At(l, a) + o + 0.9 * n * Math.sin(c - t)]);
  for (let p = c; p <= i; p += t) h.push([At(l, a) + e + r * Math.cos(p), At(l, a) + o + n * Math.sin(p)]);
  return h.push([e + r * Math.cos(i), o + n * Math.sin(i)]), h.push([e + r * Math.cos(i), o + n * Math.sin(i)]), Pr(h, null, a);
}
function Nc(t, e, o, r, n, s, i, l) {
  const a = [], c = [l.maxRandomnessOffset || 1, (l.maxRandomnessOffset || 1) + 0.3];
  let h = [0, 0];
  const p = l.disableMultiStroke ? 1 : 2, d = l.preserveVertices;
  for (let f = 0; f < p; f++) f === 0 ? a.push({ op: "move", data: [i[0], i[1]] }) : a.push({ op: "move", data: [i[0] + (d ? 0 : At(c[0], l)), i[1] + (d ? 0 : At(c[0], l))] }), h = d ? [n, s] : [n + At(c[f], l), s + At(c[f], l)], a.push({ op: "bcurveTo", data: [t + At(c[f], l), e + At(c[f], l), o + At(c[f], l), r + At(c[f], l), h[0], h[1]] });
  return a;
}
function Jo(t) {
  return [...t];
}
function Vs(t, e = 0) {
  const o = t.length;
  if (o < 3) throw new Error("A curve must have at least three points.");
  const r = [];
  if (o === 3) r.push(Jo(t[0]), Jo(t[1]), Jo(t[2]), Jo(t[2]));
  else {
    const n = [];
    n.push(t[0], t[0]);
    for (let l = 1; l < t.length; l++) n.push(t[l]), l === t.length - 1 && n.push(t[l]);
    const s = [], i = 1 - e;
    r.push(Jo(n[0]));
    for (let l = 1; l + 2 < n.length; l++) {
      const a = n[l];
      s[0] = [a[0], a[1]], s[1] = [a[0] + (i * n[l + 1][0] - i * n[l - 1][0]) / 6, a[1] + (i * n[l + 1][1] - i * n[l - 1][1]) / 6], s[2] = [n[l + 1][0] + (i * n[l][0] - i * n[l + 2][0]) / 6, n[l + 1][1] + (i * n[l][1] - i * n[l + 2][1]) / 6], s[3] = [n[l + 1][0], n[l + 1][1]], r.push(s[1], s[2], s[3]);
    }
  }
  return r;
}
function Cr(t, e) {
  return Math.pow(t[0] - e[0], 2) + Math.pow(t[1] - e[1], 2);
}
function Hc(t, e, o) {
  const r = Cr(e, o);
  if (r === 0) return Cr(t, e);
  let n = ((t[0] - e[0]) * (o[0] - e[0]) + (t[1] - e[1]) * (o[1] - e[1])) / r;
  return n = Math.max(0, Math.min(1, n)), Cr(t, ho(e, o, n));
}
function ho(t, e, o) {
  return [t[0] + (e[0] - t[0]) * o, t[1] + (e[1] - t[1]) * o];
}
function In(t, e, o, r) {
  const n = r || [];
  if (function(l, a) {
    const c = l[a + 0], h = l[a + 1], p = l[a + 2], d = l[a + 3];
    let f = 3 * h[0] - 2 * c[0] - d[0];
    f *= f;
    let m = 3 * h[1] - 2 * c[1] - d[1];
    m *= m;
    let y = 3 * p[0] - 2 * d[0] - c[0];
    y *= y;
    let g = 3 * p[1] - 2 * d[1] - c[1];
    return g *= g, f < y && (f = y), m < g && (m = g), f + m;
  }(t, e) < o) {
    const l = t[e + 0];
    n.length ? (s = n[n.length - 1], i = l, Math.sqrt(Cr(s, i)) > 1 && n.push(l)) : n.push(l), n.push(t[e + 3]);
  } else {
    const a = t[e + 0], c = t[e + 1], h = t[e + 2], p = t[e + 3], d = ho(a, c, 0.5), f = ho(c, h, 0.5), m = ho(h, p, 0.5), y = ho(d, f, 0.5), g = ho(f, m, 0.5), b = ho(y, g, 0.5);
    In([a, d, y, b], 0, o, n), In([b, g, m, p], 0, o, n);
  }
  var s, i;
  return n;
}
function Oc(t, e) {
  return Rr(t, 0, t.length, e);
}
function Rr(t, e, o, r, n) {
  const s = n || [], i = t[e], l = t[o - 1];
  let a = 0, c = 1;
  for (let h = e + 1; h < o - 1; ++h) {
    const p = Hc(t[h], i, l);
    p > a && (a = p, c = h);
  }
  return Math.sqrt(a) > r ? (Rr(t, e, c + 1, r, s), Rr(t, c, o, r, s)) : (s.length || s.push(i), s.push(l)), s;
}
function ln(t, e = 0.15, o) {
  const r = [], n = (t.length - 1) / 3;
  for (let s = 0; s < n; s++)
    In(t, 3 * s, e, r);
  return o && o > 0 ? Rr(r, 0, r.length, o) : r;
}
const fe = "none";
class Ar {
  constructor(e) {
    this.defaultOptions = { maxRandomnessOffset: 2, roughness: 1, bowing: 1, stroke: "#000", strokeWidth: 1, curveTightness: 0, curveFitting: 0.95, curveStepCount: 9, fillStyle: "hachure", fillWeight: -1, hachureAngle: -41, hachureGap: -1, dashOffset: -1, dashGap: -1, zigzagOffset: -1, seed: 0, disableMultiStroke: !1, disableMultiStrokeFill: !1, preserveVertices: !1, fillShapeRoughnessGain: 0.8 }, this.config = e || {}, this.config.options && (this.defaultOptions = this._o(this.config.options));
  }
  static newSeed() {
    return Math.floor(Math.random() * 2 ** 31);
  }
  _o(e) {
    return e ? Object.assign({}, this.defaultOptions, e) : this.defaultOptions;
  }
  _d(e, o, r) {
    return { shape: e, sets: o || [], options: r || this.defaultOptions };
  }
  line(e, o, r, n, s) {
    const i = this._o(s);
    return this._d("line", [Xi(e, o, r, n, i)], i);
  }
  rectangle(e, o, r, n, s) {
    const i = this._o(s), l = [], a = Bc(e, o, r, n, i);
    if (i.fill) {
      const c = [[e, o], [e + r, o], [e + r, o + n], [e, o + n]];
      i.fillStyle === "solid" ? l.push(an([c], i)) : l.push(Co([c], i));
    }
    return i.stroke !== fe && l.push(a), this._d("rectangle", l, i);
  }
  ellipse(e, o, r, n, s) {
    const i = this._o(s), l = [], a = Yi(r, n, i), c = Cn(e, o, i, a);
    if (i.fill) if (i.fillStyle === "solid") {
      const h = Cn(e, o, i, a).opset;
      h.type = "fillPath", l.push(h);
    } else l.push(Co([c.estimatedPoints], i));
    return i.stroke !== fe && l.push(c.opset), this._d("ellipse", l, i);
  }
  circle(e, o, r, n) {
    const s = this.ellipse(e, o, r, r, n);
    return s.shape = "circle", s;
  }
  linearPath(e, o) {
    const r = this._o(o);
    return this._d("linearPath", [Mr(e, !1, r)], r);
  }
  arc(e, o, r, n, s, i, l = !1, a) {
    const c = this._o(a), h = [], p = Os(e, o, r, n, s, i, l, !0, c);
    if (l && c.fill) if (c.fillStyle === "solid") {
      const d = Object.assign({}, c);
      d.disableMultiStroke = !0;
      const f = Os(e, o, r, n, s, i, !0, !1, d);
      f.type = "fillPath", h.push(f);
    } else h.push(function(d, f, m, y, g, b, x) {
      const C = d, I = f;
      let M = Math.abs(m / 2), D = Math.abs(y / 2);
      M += At(0.01 * M, x), D += At(0.01 * D, x);
      let T = g, P = b;
      for (; T < 0; ) T += 2 * Math.PI, P += 2 * Math.PI;
      P - T > 2 * Math.PI && (T = 0, P = 2 * Math.PI);
      const G = (P - T) / x.curveStepCount, K = [];
      for (let at = T; at <= P; at += G) K.push([C + M * Math.cos(at), I + D * Math.sin(at)]);
      return K.push([C + M * Math.cos(P), I + D * Math.sin(P)]), K.push([C, I]), Co([K], x);
    }(e, o, r, n, s, i, c));
    return c.stroke !== fe && h.push(p), this._d("arc", h, c);
  }
  curve(e, o) {
    const r = this._o(o), n = [], s = Hs(e, r);
    if (r.fill && r.fill !== fe) if (r.fillStyle === "solid") {
      const i = Hs(e, Object.assign(Object.assign({}, r), { disableMultiStroke: !0, roughness: r.roughness ? r.roughness + r.fillShapeRoughnessGain : 0 }));
      n.push({ type: "fillPath", ops: this._mergedShape(i.ops) });
    } else {
      const i = [], l = e;
      if (l.length) {
        const a = typeof l[0][0] == "number" ? [l] : l;
        for (const c of a) c.length < 3 ? i.push(...c) : c.length === 3 ? i.push(...ln(Vs([c[0], c[0], c[1], c[2]]), 10, (1 + r.roughness) / 2)) : i.push(...ln(Vs(c), 10, (1 + r.roughness) / 2));
      }
      i.length && n.push(Co([i], r));
    }
    return r.stroke !== fe && n.push(s), this._d("curve", n, r);
  }
  polygon(e, o) {
    const r = this._o(o), n = [], s = Mr(e, !0, r);
    return r.fill && (r.fillStyle === "solid" ? n.push(an([e], r)) : n.push(Co([e], r))), r.stroke !== fe && n.push(s), this._d("polygon", n, r);
  }
  path(e, o) {
    const r = this._o(o), n = [];
    if (!e) return this._d("path", n, r);
    e = (e || "").replace(/\n/g, " ").replace(/(-\s)/g, "-").replace("/(ss)/g", " ");
    const s = r.fill && r.fill !== "transparent" && r.fill !== fe, i = r.stroke !== fe, l = !!(r.simplification && r.simplification < 1), a = function(h, p, d) {
      const f = Hi(Ni(Kn(h))), m = [];
      let y = [], g = [0, 0], b = [];
      const x = () => {
        b.length >= 4 && y.push(...ln(b, p)), b = [];
      }, C = () => {
        x(), y.length && (m.push(y), y = []);
      };
      for (const { key: M, data: D } of f) switch (M) {
        case "M":
          C(), g = [D[0], D[1]], y.push(g);
          break;
        case "L":
          x(), y.push([D[0], D[1]]);
          break;
        case "C":
          if (!b.length) {
            const T = y.length ? y[y.length - 1] : g;
            b.push([T[0], T[1]]);
          }
          b.push([D[0], D[1]]), b.push([D[2], D[3]]), b.push([D[4], D[5]]);
          break;
        case "Z":
          x(), y.push([g[0], g[1]]);
      }
      if (C(), !d) return m;
      const I = [];
      for (const M of m) {
        const D = Oc(M, d);
        D.length && I.push(D);
      }
      return I;
    }(e, 1, l ? 4 - 4 * (r.simplification || 1) : (1 + r.roughness) / 2), c = Xs(e, r);
    if (s) if (r.fillStyle === "solid") if (a.length === 1) {
      const h = Xs(e, Object.assign(Object.assign({}, r), { disableMultiStroke: !0, roughness: r.roughness ? r.roughness + r.fillShapeRoughnessGain : 0 }));
      n.push({ type: "fillPath", ops: this._mergedShape(h.ops) });
    } else n.push(an(a, r));
    else n.push(Co(a, r));
    return i && (l ? a.forEach((h) => {
      n.push(Mr(h, !1, r));
    }) : n.push(c)), this._d("path", n, r);
  }
  opsToPath(e, o) {
    let r = "";
    for (const n of e.ops) {
      const s = typeof o == "number" && o >= 0 ? n.data.map((i) => +i.toFixed(o)) : n.data;
      switch (n.op) {
        case "move":
          r += `M${s[0]} ${s[1]} `;
          break;
        case "bcurveTo":
          r += `C${s[0]} ${s[1]}, ${s[2]} ${s[3]}, ${s[4]} ${s[5]} `;
          break;
        case "lineTo":
          r += `L${s[0]} ${s[1]} `;
      }
    }
    return r.trim();
  }
  toPaths(e) {
    const o = e.sets || [], r = e.options || this.defaultOptions, n = [];
    for (const s of o) {
      let i = null;
      switch (s.type) {
        case "path":
          i = { d: this.opsToPath(s), stroke: r.stroke, strokeWidth: r.strokeWidth, fill: fe };
          break;
        case "fillPath":
          i = { d: this.opsToPath(s), stroke: fe, strokeWidth: 0, fill: r.fill || fe };
          break;
        case "fillSketch":
          i = this.fillSketch(s, r);
      }
      i && n.push(i);
    }
    return n;
  }
  fillSketch(e, o) {
    let r = o.fillWeight;
    return r < 0 && (r = o.strokeWidth / 2), { d: this.opsToPath(e), stroke: o.fill || fe, strokeWidth: r, fill: fe };
  }
  _mergedShape(e) {
    return e.filter((o, r) => r === 0 || o.op !== "move");
  }
}
class Xc {
  constructor(e, o) {
    this.canvas = e, this.ctx = this.canvas.getContext("2d"), this.gen = new Ar(o);
  }
  draw(e) {
    const o = e.sets || [], r = e.options || this.getDefaultOptions(), n = this.ctx, s = e.options.fixedDecimalPlaceDigits;
    for (const i of o) switch (i.type) {
      case "path":
        n.save(), n.strokeStyle = r.stroke === "none" ? "transparent" : r.stroke, n.lineWidth = r.strokeWidth, r.strokeLineDash && n.setLineDash(r.strokeLineDash), r.strokeLineDashOffset && (n.lineDashOffset = r.strokeLineDashOffset), this._drawToContext(n, i, s), n.restore();
        break;
      case "fillPath": {
        n.save(), n.fillStyle = r.fill || "";
        const l = e.shape === "curve" || e.shape === "polygon" || e.shape === "path" ? "evenodd" : "nonzero";
        this._drawToContext(n, i, s, l), n.restore();
        break;
      }
      case "fillSketch":
        this.fillSketch(n, i, r);
    }
  }
  fillSketch(e, o, r) {
    let n = r.fillWeight;
    n < 0 && (n = r.strokeWidth / 2), e.save(), r.fillLineDash && e.setLineDash(r.fillLineDash), r.fillLineDashOffset && (e.lineDashOffset = r.fillLineDashOffset), e.strokeStyle = r.fill || "", e.lineWidth = n, this._drawToContext(e, o, r.fixedDecimalPlaceDigits), e.restore();
  }
  _drawToContext(e, o, r, n = "nonzero") {
    e.beginPath();
    for (const s of o.ops) {
      const i = typeof r == "number" && r >= 0 ? s.data.map((l) => +l.toFixed(r)) : s.data;
      switch (s.op) {
        case "move":
          e.moveTo(i[0], i[1]);
          break;
        case "bcurveTo":
          e.bezierCurveTo(i[0], i[1], i[2], i[3], i[4], i[5]);
          break;
        case "lineTo":
          e.lineTo(i[0], i[1]);
      }
    }
    o.type === "fillPath" ? e.fill(n) : e.stroke();
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  line(e, o, r, n, s) {
    const i = this.gen.line(e, o, r, n, s);
    return this.draw(i), i;
  }
  rectangle(e, o, r, n, s) {
    const i = this.gen.rectangle(e, o, r, n, s);
    return this.draw(i), i;
  }
  ellipse(e, o, r, n, s) {
    const i = this.gen.ellipse(e, o, r, n, s);
    return this.draw(i), i;
  }
  circle(e, o, r, n) {
    const s = this.gen.circle(e, o, r, n);
    return this.draw(s), s;
  }
  linearPath(e, o) {
    const r = this.gen.linearPath(e, o);
    return this.draw(r), r;
  }
  polygon(e, o) {
    const r = this.gen.polygon(e, o);
    return this.draw(r), r;
  }
  arc(e, o, r, n, s, i, l = !1, a) {
    const c = this.gen.arc(e, o, r, n, s, i, l, a);
    return this.draw(c), c;
  }
  curve(e, o) {
    const r = this.gen.curve(e, o);
    return this.draw(r), r;
  }
  path(e, o) {
    const r = this.gen.path(e, o);
    return this.draw(r), r;
  }
}
const br = "http://www.w3.org/2000/svg";
class Yc {
  constructor(e, o) {
    this.svg = e, this.gen = new Ar(o);
  }
  draw(e) {
    const o = e.sets || [], r = e.options || this.getDefaultOptions(), n = this.svg.ownerDocument || window.document, s = n.createElementNS(br, "g"), i = e.options.fixedDecimalPlaceDigits;
    for (const l of o) {
      let a = null;
      switch (l.type) {
        case "path":
          a = n.createElementNS(br, "path"), a.setAttribute("d", this.opsToPath(l, i)), a.setAttribute("stroke", r.stroke), a.setAttribute("stroke-width", r.strokeWidth + ""), a.setAttribute("fill", "none"), r.strokeLineDash && a.setAttribute("stroke-dasharray", r.strokeLineDash.join(" ").trim()), r.strokeLineDashOffset && a.setAttribute("stroke-dashoffset", `${r.strokeLineDashOffset}`);
          break;
        case "fillPath":
          a = n.createElementNS(br, "path"), a.setAttribute("d", this.opsToPath(l, i)), a.setAttribute("stroke", "none"), a.setAttribute("stroke-width", "0"), a.setAttribute("fill", r.fill || ""), e.shape !== "curve" && e.shape !== "polygon" || a.setAttribute("fill-rule", "evenodd");
          break;
        case "fillSketch":
          a = this.fillSketch(n, l, r);
      }
      a && s.appendChild(a);
    }
    return s;
  }
  fillSketch(e, o, r) {
    let n = r.fillWeight;
    n < 0 && (n = r.strokeWidth / 2);
    const s = e.createElementNS(br, "path");
    return s.setAttribute("d", this.opsToPath(o, r.fixedDecimalPlaceDigits)), s.setAttribute("stroke", r.fill || ""), s.setAttribute("stroke-width", n + ""), s.setAttribute("fill", "none"), r.fillLineDash && s.setAttribute("stroke-dasharray", r.fillLineDash.join(" ").trim()), r.fillLineDashOffset && s.setAttribute("stroke-dashoffset", `${r.fillLineDashOffset}`), s;
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  opsToPath(e, o) {
    return this.gen.opsToPath(e, o);
  }
  line(e, o, r, n, s) {
    const i = this.gen.line(e, o, r, n, s);
    return this.draw(i);
  }
  rectangle(e, o, r, n, s) {
    const i = this.gen.rectangle(e, o, r, n, s);
    return this.draw(i);
  }
  ellipse(e, o, r, n, s) {
    const i = this.gen.ellipse(e, o, r, n, s);
    return this.draw(i);
  }
  circle(e, o, r, n) {
    const s = this.gen.circle(e, o, r, n);
    return this.draw(s);
  }
  linearPath(e, o) {
    const r = this.gen.linearPath(e, o);
    return this.draw(r);
  }
  polygon(e, o) {
    const r = this.gen.polygon(e, o);
    return this.draw(r);
  }
  arc(e, o, r, n, s, i, l = !1, a) {
    const c = this.gen.arc(e, o, r, n, s, i, l, a);
    return this.draw(c);
  }
  curve(e, o) {
    const r = this.gen.curve(e, o);
    return this.draw(r);
  }
  path(e, o) {
    const r = this.gen.path(e, o);
    return this.draw(r);
  }
}
var Gc = { canvas: (t, e) => new Xc(t, e), svg: (t, e) => new Yc(t, e), generator: (t) => new Ar(t), newSeed: () => Ar.newSeed() };
const Be = Gc.generator();
function jc(t) {
  let e = 0;
  for (let o = 0; o < t.length; o++) {
    const r = t.charCodeAt(o);
    e = (e << 5) - e + r, e |= 0;
  }
  return Math.abs(e);
}
function Je(t) {
  return {
    stroke: t.stroke,
    fill: t.fill || "none",
    fillStyle: t.fill ? t.fillStyle || "hachure" : void 0,
    roughness: t.roughness,
    strokeWidth: t.strokeWidth,
    strokeLineDash: t.strokeLineDash,
    seed: t.seed ? jc(t.seed) : void 0,
    fillWeight: t.strokeWidth / 2,
    hachureGap: Math.max(t.strokeWidth * 4, 4)
  };
}
function $e(t) {
  var r;
  const e = t.options, o = (r = e == null ? void 0 : e.strokeLineDash) != null && r.length ? e.strokeLineDash.join(" ") : void 0;
  return Be.toPaths(t).map((n) => ({
    d: n.d,
    stroke: n.stroke,
    strokeWidth: n.strokeWidth,
    fill: n.fill,
    // Apply dash only to stroke paths, not fill paths
    strokeDasharray: n.stroke !== "none" && n.strokeWidth > 0 ? o : void 0
  }));
}
function Yo(t, e) {
  return Math.min(t, e) * 0.25;
}
function Vc(t, e, o, r, n) {
  const s = Math.min(n, o / 2, r / 2);
  return [
    `M${t + s},${e}`,
    `L${t + o - s},${e}`,
    `A${s},${s} 0 0 1 ${t + o},${e + s}`,
    `L${t + o},${e + r - s}`,
    `A${s},${s} 0 0 1 ${t + o - s},${e + r}`,
    `L${t + s},${e + r}`,
    `A${s},${s} 0 0 1 ${t},${e + r - s}`,
    `L${t},${e + s}`,
    `A${s},${s} 0 0 1 ${t + s},${e}`,
    "Z"
  ].join(" ");
}
function Lr(t, e, o, r, n, s) {
  if (s) {
    const i = Yo(o, r);
    return $e(Be.path(Vc(t, e, o, r, i), Je(n)));
  }
  return $e(Be.rectangle(t, e, o, r, Je(n)));
}
function qn(t, e, o, r, n) {
  return $e(Be.ellipse(t, e, o, r, Je(n)));
}
function Kc(t, e, o, r, n) {
  const s = t + o / 2, i = e + r / 2, l = [s, e], a = [t + o, i], c = [s, e + r], h = [t, i], p = Math.hypot(o / 2, r / 2), d = Math.min(n, p / 2) / p, f = (D, T, P) => [
    D[0] + P * (T[0] - D[0]),
    D[1] + P * (T[1] - D[1])
  ], m = f(h, l, 1 - d), y = f(l, a, d), g = f(l, a, 1 - d), b = f(a, c, d), x = f(a, c, 1 - d), C = f(c, h, d), I = f(c, h, 1 - d), M = f(h, l, d);
  return [
    `M${y[0]},${y[1]}`,
    `L${g[0]},${g[1]}`,
    `Q${a[0]},${a[1]} ${b[0]},${b[1]}`,
    `L${x[0]},${x[1]}`,
    `Q${c[0]},${c[1]} ${C[0]},${C[1]}`,
    `L${I[0]},${I[1]}`,
    `Q${h[0]},${h[1]} ${M[0]},${M[1]}`,
    `L${m[0]},${m[1]}`,
    `Q${l[0]},${l[1]} ${y[0]},${y[1]}`,
    "Z"
  ].join(" ");
}
function Zn(t, e, o, r, n, s) {
  if (s) {
    const l = Yo(o, r);
    return $e(Be.path(Kc(t, e, o, r, l), Je(n)));
  }
  const i = [
    [t + o / 2, e],
    [t + o, e + r / 2],
    [t + o / 2, e + r],
    [t, e + r / 2]
  ];
  return $e(Be.polygon(i, Je(n)));
}
function Bo(t, e, o, r, n) {
  return $e(Be.line(t, e, o, r, Je(n)));
}
function Un(t, e, o, r, n) {
  const s = Bo(t, e, o, r, n), i = Math.atan2(r - e, o - t), l = Math.max(12, n.strokeWidth * 4), a = Math.PI / 6, c = o - l * Math.cos(i - a), h = r - l * Math.sin(i - a), p = o - l * Math.cos(i + a), d = r - l * Math.sin(i + a), f = Bo(o, r, c, h, n), m = Bo(o, r, p, d, n);
  return [...s, ...f, ...m];
}
function Ks(t, e) {
  const o = {
    ...Je(e),
    stroke: "none"
  };
  return $e(Be.polygon(t, o));
}
function cn(t, e) {
  return $e(Be.path(t, Je(e)));
}
function _e(t) {
  if (t === "dashed") return [8, 4];
  if (t === "dotted") return [2, 2];
}
function qc(t) {
  const e = t.replace("#", ""), o = parseInt(e.substring(0, 2), 16) || 0, r = parseInt(e.substring(2, 4), 16) || 0, n = parseInt(e.substring(4, 6), 16) || 0;
  return (0.299 * o + 0.587 * r + 0.114 * n) / 255 > 0.5 ? "#1e1e2e" : "#ffffff";
}
function Zc({ node: t, editingLabel: e }) {
  if (t.type === "draw") {
    const o = t;
    return o.data.tool === "vector" ? /* @__PURE__ */ u(Qc, { node: o }) : /* @__PURE__ */ u(Uc, { node: o });
  }
  return /* @__PURE__ */ u(Jc, { node: t, editingLabel: e });
}
const Dr = de(Zc), Uc = de(function({ node: e }) {
  const o = e.data.strokeStyle === "dashed" || e.data.strokeStyle === "dotted", r = _e(e.data.strokeStyle), n = Kt(
    () => o ? null : jn(e.data.points, { size: e.data.strokeWidth }),
    [e.data.points, e.data.strokeWidth, o]
  ), s = Kt(() => {
    const h = e.data.points;
    if (!h || h.length === 0) return "";
    if (h.length === 1) return `M${h[0][0]},${h[0][1]}L${h[0][0]},${h[0][1]}`;
    const p = [`M${h[0][0]},${h[0][1]}`];
    for (let d = 1; d < h.length; d++)
      p.push(`L${h[d][0]},${h[d][1]}`);
    return p.join("");
  }, [e.data.points]), i = Kt(() => {
    if (!o) return null;
    const h = e.data.points;
    if (h.length < 2) return "";
    const p = ["M", h[0][0], h[0][1]];
    for (let f = 1; f < h.length; f++) {
      const [m, y] = h[f], [g, b] = h[f - 1];
      p.push("Q", g, b, (g + m) / 2, (b + y) / 2);
    }
    const d = h[h.length - 1];
    return p.push("L", d[0], d[1]), p.join(" ");
  }, [e.data.points, o]), l = Kt(() => {
    if (!e.data.fill || e.data.points.length < 3) return null;
    const h = e.data.points.map((I) => [I[0], I[1]]), p = Bi(h), d = p[0], f = p[p.length - 1], m = Math.hypot(d[0] - f[0], d[1] - f[1]);
    let y = 0;
    for (let I = 1; I < p.length; I++)
      y += Math.hypot(p[I][0] - p[I - 1][0], p[I][1] - p[I - 1][1]);
    const g = y >= 1 && m <= Math.max(e.data.strokeWidth * 4, 20) && m <= y * 0.1, b = e.data.fillStyle || "solid";
    if (g) {
      const I = Sc(p, 0);
      return b === "solid" ? { kind: "solid", d: I, fill: e.data.fill } : { kind: "rough", paths: Ks(p, {
        stroke: "none",
        fill: e.data.fill,
        fillStyle: b,
        roughness: 1,
        strokeWidth: e.data.strokeWidth
      }) };
    }
    const x = zc(p);
    if (x.length === 0) return null;
    if (b === "solid")
      return {
        kind: "solid",
        d: "",
        fill: e.data.fill,
        regions: x
      };
    const C = [];
    for (const { points: I } of x)
      I.length >= 3 && C.push(
        ...Ks(I, {
          stroke: "none",
          fill: e.data.fill,
          fillStyle: b,
          roughness: 1,
          strokeWidth: e.data.strokeWidth
        })
      );
    return { kind: "rough", paths: C, regions: x };
  }, [e.data.fill, e.data.fillStyle, e.data.points, e.data.strokeWidth]), a = e.h === "auto" ? 0 : e.h, c = e.data.strokeWidth * 4;
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x - c,
        top: e.y - c,
        width: e.w + c * 2,
        height: a + c * 2,
        zIndex: e.z,
        pointerEvents: "none",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "svg",
        {
          width: e.w + c * 2,
          height: a + c * 2,
          style: { overflow: "visible" },
          children: /* @__PURE__ */ k("g", { transform: `translate(${c}, ${c})`, opacity: e.data.opacity ?? 1, children: [
            (l == null ? void 0 : l.kind) === "solid" && (l.regions ? l.regions.map((h, p) => /* @__PURE__ */ u(
              "path",
              {
                d: h.pathD,
                fill: l.fill,
                stroke: "none"
              },
              p
            )) : /* @__PURE__ */ u("path", { d: l.d, fill: l.fill, stroke: "none" })),
            (l == null ? void 0 : l.kind) === "rough" && l.paths.map((h, p) => /* @__PURE__ */ u(
              "path",
              {
                d: h.d,
                stroke: h.stroke,
                strokeWidth: h.strokeWidth,
                fill: h.fill,
                strokeLinecap: "round",
                strokeLinejoin: "round"
              },
              p
            )),
            o ? /* @__PURE__ */ u(
              "path",
              {
                d: i,
                fill: "none",
                stroke: e.data.color,
                strokeWidth: e.data.strokeWidth,
                strokeDasharray: r == null ? void 0 : r.map((h) => h * Math.max(e.data.strokeWidth, 1)).join(" "),
                strokeLinecap: "round",
                strokeLinejoin: "round"
              }
            ) : /* @__PURE__ */ u(
              "path",
              {
                d: n,
                fill: e.data.color
              }
            ),
            s && /* @__PURE__ */ u(
              "path",
              {
                d: s,
                fill: "none",
                stroke: "transparent",
                strokeWidth: e.data.strokeWidth,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                pointerEvents: "stroke"
              }
            )
          ] })
        }
      )
    }
  );
}), Qc = de(function({ node: e }) {
  const o = e.h === "auto" ? 0 : e.h, r = e.data.strokeWidth * 2, n = Kt(() => {
    const l = e.data.points;
    if (!l || l.length === 0) return "";
    const a = [`M${l[0][0]},${l[0][1]}`];
    for (let c = 1; c < l.length; c++)
      a.push(`L${l[c][0]},${l[c][1]}`);
    return a.push("Z"), a.join("");
  }, [e.data.points]), s = _e(e.data.strokeStyle), i = s == null ? void 0 : s.map((l) => l * Math.max(e.data.strokeWidth, 1)).join(" ");
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x - r,
        top: e.y - r,
        width: e.w + r * 2,
        height: o + r * 2,
        zIndex: e.z,
        pointerEvents: "none",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "svg",
        {
          width: e.w + r * 2,
          height: o + r * 2,
          style: { overflow: "visible" },
          children: /* @__PURE__ */ k("g", { transform: `translate(${r}, ${r})`, opacity: e.data.opacity ?? 1, children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: n,
                fill: e.data.fill || "none",
                stroke: e.data.color,
                strokeWidth: e.data.strokeWidth,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                strokeDasharray: i
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: n,
                fill: e.data.fill ? "transparent" : "none",
                stroke: "transparent",
                strokeWidth: Math.max(e.data.strokeWidth, 8),
                pointerEvents: e.data.fill ? "painted" : "stroke"
              }
            )
          ] })
        }
      )
    }
  );
}), Jc = de(function({ node: e, editingLabel: o }) {
  var g, b, x, C;
  const r = e.h === "auto" ? 100 : e.h, n = e.data.strokeWidth * 2, s = _e(e.data.strokeStyle), i = ((g = e.data.startPoint) == null ? void 0 : g[0]) ?? 0, l = ((b = e.data.startPoint) == null ? void 0 : b[1]) ?? r / 2, a = ((x = e.data.endPoint) == null ? void 0 : x[0]) ?? e.w, c = ((C = e.data.endPoint) == null ? void 0 : C[1]) ?? r / 2, h = Kt(() => {
    if (e.data.roughness === 0) return null;
    const I = {
      stroke: e.data.stroke,
      fill: e.data.fill,
      fillStyle: e.data.fillStyle,
      roughness: e.data.roughness,
      strokeWidth: e.data.strokeWidth,
      strokeLineDash: s,
      seed: e.id
    }, M = e.data.edgeStyle === "round";
    switch (e.data.shape) {
      case "rect":
        return Lr(0, 0, e.w, r, I, M);
      case "ellipse":
        return qn(e.w / 2, r / 2, e.w, r, I);
      case "diamond":
        return Zn(0, 0, e.w, r, I, M);
      case "line":
        return Bo(i, l, a, c, I);
      case "arrow":
        return Un(i, l, a, c, I);
      default:
        return null;
    }
  }, [e, s, i, l, a, c, r]), p = e.data.fill && e.data.fillStyle === "solid" && e.data.roughness > 0, d = e.data.opacity ?? 1, f = e.data.shape === "line" || e.data.shape === "arrow", m = e.data.label, y = e.data.labelFontSize ?? 14;
  return /* @__PURE__ */ k(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x,
        top: e.y,
        width: e.w,
        height: r,
        zIndex: e.z,
        pointerEvents: "none",
        overflow: "visible",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: [
        /* @__PURE__ */ u(
          "svg",
          {
            width: e.w + n * 2,
            height: r + n * 2,
            style: { overflow: "visible", marginLeft: -n, marginTop: -n },
            children: /* @__PURE__ */ k("g", { transform: `translate(${n}, ${n})`, opacity: d, children: [
              p && /* @__PURE__ */ u(
                td,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: r,
                  fill: e.data.fill,
                  rounded: e.data.edgeStyle === "round"
                }
              ),
              h ? h.map((I, M) => p && I.fill && I.fill !== "none" ? null : /* @__PURE__ */ u(
                "path",
                {
                  d: I.d,
                  stroke: I.stroke,
                  strokeWidth: I.strokeWidth,
                  fill: I.fill,
                  strokeDasharray: I.strokeDasharray,
                  strokeLinecap: "round",
                  strokeLinejoin: "round"
                },
                M
              )) : /* @__PURE__ */ u(
                $c,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: r,
                  x1: i,
                  y1: l,
                  x2: a,
                  y2: c,
                  stroke: e.data.stroke,
                  fill: e.data.fill,
                  strokeWidth: e.data.strokeWidth,
                  dashArray: s,
                  rounded: e.data.edgeStyle === "round"
                }
              ),
              /* @__PURE__ */ u(
                _c,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: r,
                  x1: i,
                  y1: l,
                  x2: a,
                  y2: c,
                  hasFill: !!e.data.fill,
                  strokeWidth: e.data.strokeWidth,
                  rounded: e.data.edgeStyle === "round"
                }
              )
            ] })
          }
        ),
        !f && m && !o && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              pointerEvents: "none",
              padding: "4px 8px"
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                style: {
                  textAlign: e.data.labelAlign ?? "center",
                  fontFamily: Ue(e.data.labelFontFamily ?? Ze),
                  fontSize: y,
                  color: e.data.fill && e.data.fillStyle === "solid" ? qc(e.data.fill) : e.data.stroke,
                  lineHeight: 1.3,
                  wordBreak: "break-word",
                  whiteSpace: "pre-wrap",
                  width: "100%"
                },
                children: m
              }
            )
          }
        )
      ]
    }
  );
});
function Qn(t, e) {
  const o = Yo(t, e), r = t / 2, n = e / 2, s = [r, 0], i = [t, n], l = [r, e], a = [0, n], c = Math.hypot(t / 2, e / 2), h = Math.min(o, c / 2) / c, p = (I, M, D) => [
    I[0] + D * (M[0] - I[0]),
    I[1] + D * (M[1] - I[1])
  ], d = p(s, i, h), f = p(s, i, 1 - h), m = p(i, l, h), y = p(i, l, 1 - h), g = p(l, a, h), b = p(l, a, 1 - h), x = p(a, s, h), C = p(a, s, 1 - h);
  return [
    `M${d[0]},${d[1]}`,
    `L${f[0]},${f[1]}`,
    `Q${i[0]},${i[1]} ${m[0]},${m[1]}`,
    `L${y[0]},${y[1]}`,
    `Q${l[0]},${l[1]} ${g[0]},${g[1]}`,
    `L${b[0]},${b[1]}`,
    `Q${a[0]},${a[1]} ${x[0]},${x[1]}`,
    `L${C[0]},${C[1]}`,
    `Q${s[0]},${s[1]} ${d[0]},${d[1]}`,
    "Z"
  ].join(" ");
}
function $c({
  shape: t,
  w: e,
  h: o,
  x1: r,
  y1: n,
  x2: s,
  y2: i,
  stroke: l,
  fill: a,
  strokeWidth: c,
  dashArray: h,
  rounded: p
}) {
  const d = h == null ? void 0 : h.join(",");
  switch (t) {
    case "rect": {
      const f = !!a && a !== "none", m = o <= Math.max(c * 2, 4), y = e <= Math.max(c * 2, 4);
      if (!f && (m || y))
        return m && e >= o ? /* @__PURE__ */ u(
          "line",
          {
            x1: 0,
            y1: o / 2,
            x2: e,
            y2: o / 2,
            stroke: l,
            strokeWidth: Math.max(c, o),
            strokeDasharray: d
          }
        ) : /* @__PURE__ */ u(
          "line",
          {
            x1: e / 2,
            y1: 0,
            x2: e / 2,
            y2: o,
            stroke: l,
            strokeWidth: Math.max(c, e),
            strokeDasharray: d
          }
        );
      const g = p ? Yo(e, o) : 0;
      return /* @__PURE__ */ u(
        "rect",
        {
          x: 0,
          y: 0,
          width: e,
          height: o,
          rx: g || void 0,
          ry: g || void 0,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    }
    case "ellipse":
      return /* @__PURE__ */ u(
        "ellipse",
        {
          cx: e / 2,
          cy: o / 2,
          rx: e / 2,
          ry: o / 2,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "diamond":
      return p ? /* @__PURE__ */ u(
        "path",
        {
          d: Qn(e, o),
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "line":
      return /* @__PURE__ */ u(
        "line",
        {
          x1: r,
          y1: n,
          x2: s,
          y2: i,
          stroke: l,
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "arrow": {
      const f = Math.atan2(i - n, s - r), m = Math.max(12, c * 4), y = Math.PI / 6, g = s - m * Math.cos(f - y), b = i - m * Math.sin(f - y), x = s - m * Math.cos(f + y), C = i - m * Math.sin(f + y);
      return /* @__PURE__ */ k(ut, { children: [
        /* @__PURE__ */ u(
          "line",
          {
            x1: r,
            y1: n,
            x2: s,
            y2: i,
            stroke: l,
            strokeWidth: c,
            strokeDasharray: d
          }
        ),
        /* @__PURE__ */ u(
          "polyline",
          {
            points: `${g},${b} ${s},${i} ${x},${C}`,
            stroke: l,
            strokeWidth: c,
            fill: "none"
          }
        )
      ] });
    }
    default:
      return null;
  }
}
function _c({
  shape: t,
  w: e,
  h: o,
  x1: r,
  y1: n,
  x2: s,
  y2: i,
  hasFill: l,
  strokeWidth: a,
  rounded: c
}) {
  const h = l ? "painted" : "stroke", p = l ? "transparent" : "none";
  switch (t) {
    case "rect": {
      const d = c ? Yo(e, o) : 0;
      return /* @__PURE__ */ u(
        "rect",
        {
          x: 0,
          y: 0,
          width: e,
          height: o,
          rx: d || void 0,
          ry: d || void 0,
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    }
    case "ellipse":
      return /* @__PURE__ */ u(
        "ellipse",
        {
          cx: e / 2,
          cy: o / 2,
          rx: e / 2,
          ry: o / 2,
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    case "diamond":
      return c ? /* @__PURE__ */ u(
        "path",
        {
          d: Qn(e, o),
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    case "line":
    case "arrow":
      return /* @__PURE__ */ u(
        "line",
        {
          x1: r,
          y1: n,
          x2: s,
          y2: i,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: "stroke"
        }
      );
    default:
      return null;
  }
}
function td({
  shape: t,
  w: e,
  h: o,
  fill: r,
  rounded: n
}) {
  switch (t) {
    case "rect": {
      const s = n ? Yo(e, o) : 0;
      return /* @__PURE__ */ u("rect", { x: 0, y: 0, width: e, height: o, rx: s || void 0, ry: s || void 0, fill: r, stroke: "none" });
    }
    case "ellipse":
      return /* @__PURE__ */ u("ellipse", { cx: e / 2, cy: o / 2, rx: e / 2, ry: o / 2, fill: r, stroke: "none" });
    case "diamond":
      return n ? /* @__PURE__ */ u(
        "path",
        {
          d: Qn(e, o),
          fill: r,
          stroke: "none"
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          fill: r,
          stroke: "none"
        }
      );
    default:
      return null;
  }
}
const ed = de(function(e) {
  return /* @__PURE__ */ u(Dr, { node: e.node });
}), od = {
  type: "draw",
  component: ed,
  handlesOwnLayout: !0,
  hitTest: (t, e, o, r) => Bn(t, e, o, r),
  getHitPadding: (t) => {
    const e = t.data;
    return Math.max(20, e.strokeWidth * 4);
  },
  onResize: (t, e, o) => ({
    points: t.data.points.map(
      ([n, s, i]) => [n * e, s * o, i]
    )
  }),
  onFlip: (t, e) => {
    const o = t.data;
    if (e === "h")
      return {
        points: o.points.map(
          ([n, s, i]) => [t.w - n, s, i]
        )
      };
    const r = t.h === "auto" ? 0 : t.h;
    return {
      points: o.points.map(
        ([n, s, i]) => [n, r - s, i]
      )
    };
  },
  getClipboardText: () => null
}, rd = de(function(e) {
  const o = e.node;
  return /* @__PURE__ */ u(Dr, { node: o, editingLabel: e.editing });
}), nd = {
  type: "shape",
  component: rd,
  handlesOwnLayout: !0,
  hitTest: (t, e, o, r) => Fr(t, e, o, r),
  onResize: (t, e, o) => {
    const r = t.data, n = {};
    return r.startPoint && (n.startPoint = [r.startPoint[0] * e, r.startPoint[1] * o]), r.endPoint && (n.endPoint = [r.endPoint[0] * e, r.endPoint[1] * o]), Object.keys(n).length > 0 ? n : null;
  },
  onFlip: (t, e) => {
    const o = t.data;
    if (o.shape !== "arrow" && o.shape !== "line")
      return {};
    if (e === "h")
      return o.startPoint && o.endPoint ? {
        startPoint: [t.w - o.startPoint[0], o.startPoint[1]],
        endPoint: [t.w - o.endPoint[0], o.endPoint[1]]
      } : {};
    const r = t.h === "auto" ? 0 : t.h;
    return o.startPoint && o.endPoint ? {
      startPoint: [o.startPoint[0], r - o.startPoint[1]],
      endPoint: [o.endPoint[0], r - o.endPoint[1]]
    } : {};
  },
  getClipboardText: (t) => t.data.label || null
};
function sd(t) {
  return null;
}
const id = {
  type: "edge",
  component: sd,
  isSVGOnly: !0,
  handlesOwnLayout: !0,
  getClipboardText: () => null
}, xr = 0.05, ad = [
  { pos: "nw", edges: ["left", "top"], cx: 0, cy: 0, cursor: "nwse-resize" },
  { pos: "n", edges: ["top"], cx: 0.5, cy: 0, cursor: "ns-resize" },
  { pos: "ne", edges: ["right", "top"], cx: 1, cy: 0, cursor: "nesw-resize" },
  { pos: "e", edges: ["right"], cx: 1, cy: 0.5, cursor: "ew-resize" },
  { pos: "se", edges: ["right", "bottom"], cx: 1, cy: 1, cursor: "nwse-resize" },
  { pos: "s", edges: ["bottom"], cx: 0.5, cy: 1, cursor: "ns-resize" },
  { pos: "sw", edges: ["left", "bottom"], cx: 0, cy: 1, cursor: "nesw-resize" },
  { pos: "w", edges: ["left"], cx: 0, cy: 0.5, cursor: "ew-resize" }
];
function ld({
  node: t,
  isSelected: e,
  engine: o,
  interactive: r,
  zoom: n,
  onResizeHandleDown: s,
  cropping: i,
  onCropStart: l,
  onCropEnd: a
}) {
  const c = t.h, h = t.data.crop, p = lt(!1);
  p.current = !!i;
  const d = lt(null), [f, m] = tt(null), y = it(() => {
    d.current && d.current.naturalWidth > 0 && m({ w: d.current.naturalWidth, h: d.current.naturalHeight });
  }, []);
  mt(() => {
    d.current && d.current.naturalWidth > 0 && m({ w: d.current.naturalWidth, h: d.current.naturalHeight });
  }, [t.data.src]);
  const [g, b] = tt({ x: 0, y: 0, w: 1, h: 1 });
  mt(() => {
    i && (b(h ?? { x: 0, y: 0, w: 1, h: 1 }), !f && d.current && d.current.naturalWidth > 0 && m({ w: d.current.naturalWidth, h: d.current.naturalHeight }));
  }, [i]);
  const x = Kt(() => {
    if (!f) return null;
    const j = f.w / f.h, q = t.w / c;
    let Y, $;
    return j > q ? (Y = t.w, $ = t.w / j) : ($ = c, Y = c * j), { x: (t.w - Y) / 2, y: (c - $) / 2, w: Y, h: $ };
  }, [f, t.w, c]), C = it(() => {
    const j = g.x < 1e-3 && g.y < 1e-3 && g.w > 0.999 && g.h > 0.999;
    o.updateNodeWithHistory(t.id, {
      data: {
        ...t.data,
        crop: j ? void 0 : { x: g.x, y: g.y, w: g.w, h: g.h }
      }
    }), a == null || a();
  }, [o, t, g, a]), I = it(() => {
    a == null || a();
  }, [a]);
  mt(() => {
    if (!i) return;
    const j = (q) => {
      q.key === "Enter" ? (C(), q.preventDefault(), q.stopPropagation()) : q.key === "Escape" && (I(), q.preventDefault(), q.stopPropagation());
    };
    return document.addEventListener("keydown", j, !0), () => document.removeEventListener("keydown", j, !0);
  }, [i, C, I]);
  const M = it(
    (j, q) => {
      if (q.stopPropagation(), q.preventDefault(), !x) return;
      const Y = q.currentTarget.ownerDocument, $ = q.clientX, nt = q.clientY, ot = { ...g }, ft = (rt) => {
        const It = (rt.clientX - $) / n / x.w, zt = (rt.clientY - nt) / n / x.h, bt = { ...ot }, ht = ot.x + ot.w, Wt = ot.y + ot.h;
        if (j.includes("left")) {
          const Tt = Math.max(0, Math.min(ht - xr, ot.x + It));
          bt.x = Tt, bt.w = ht - Tt;
        }
        if (j.includes("right") && (bt.w = Math.max(
          xr,
          Math.min(1 - ot.x, ot.w + It)
        )), j.includes("top")) {
          const Tt = Math.max(0, Math.min(Wt - xr, ot.y + zt));
          bt.y = Tt, bt.h = Wt - Tt;
        }
        j.includes("bottom") && (bt.h = Math.max(
          xr,
          Math.min(1 - ot.y, ot.h + zt)
        )), b(bt);
      }, pt = () => {
        Y.removeEventListener("pointermove", ft), Y.removeEventListener("pointerup", pt);
      };
      Y.addEventListener("pointermove", ft), Y.addEventListener("pointerup", pt);
    },
    [g, x, n]
  ), D = it(
    (j) => {
      if (j.stopPropagation(), j.preventDefault(), !x) return;
      const q = j.currentTarget.ownerDocument, Y = j.clientX, $ = j.clientY, nt = { ...g }, ot = (pt) => {
        const rt = (pt.clientX - Y) / n / x.w, It = (pt.clientY - $) / n / x.h;
        b({
          ...nt,
          x: Math.max(0, Math.min(1 - nt.w, nt.x + rt)),
          y: Math.max(0, Math.min(1 - nt.h, nt.y + It))
        });
      }, ft = () => {
        q.removeEventListener("pointermove", ot), q.removeEventListener("pointerup", ft);
      };
      q.addEventListener("pointermove", ot), q.addEventListener("pointerup", ft);
    },
    [g, x, n]
  ), T = it(
    (j) => {
      if (p.current) return;
      const q = j.currentTarget.ownerDocument;
      if (j.altKey) return;
      if (!o.selection.has(t.id) && o.selection.size > 0) {
        const { x: Tt, y: Rt } = o.screenToCanvas(
          j.clientX,
          j.clientY
        );
        for (const Qt of o.selection) {
          const Jt = o.getNode(Qt);
          if (!Jt) continue;
          const Zt = Jt.h === "auto" ? 100 : Jt.h;
          if (Tt >= Jt.x && Tt <= Jt.x + Jt.w && Rt >= Jt.y && Rt <= Jt.y + Zt)
            return;
        }
      }
      j.stopPropagation(), j.preventDefault(), j.shiftKey ? o.toggleSelect(t.id) : o.selection.has(t.id) || o.select(t.id);
      const Y = j.clientX, $ = j.clientY, nt = Array.from(o.selection), ot = nt.map((Tt) => {
        const Rt = o.getNode(Tt);
        return { id: Tt, x: Rt.x, y: Rt.y };
      });
      let ft = !1, pt = null, rt = Y, It = $, zt = !1;
      const bt = () => {
        pt = null;
        const Tt = (rt - Y) / o.viewport.zoom, Rt = (It - $) / o.viewport.zoom, { finalDx: Qt, finalDy: Jt } = o.computeDragSnap(
          ot,
          nt,
          Tt,
          Rt,
          zt
        ), Zt = ot.map(($t) => ({
          id: $t.id,
          patch: { x: $t.x + Qt, y: $t.y + Jt }
        }));
        o.updateMany(Zt);
      }, ht = (Tt) => {
        const Rt = (Tt.clientX - Y) / o.viewport.zoom, Qt = (Tt.clientY - $) / o.viewport.zoom;
        if (!ft)
          if (Math.abs(Rt) > 2 || Math.abs(Qt) > 2)
            ft = !0, o.pushHistorySnapshot();
          else
            return;
        rt = Tt.clientX, It = Tt.clientY, zt = Tt.metaKey || Tt.ctrlKey, pt === null && (pt = requestAnimationFrame(bt));
      }, Wt = () => {
        pt !== null && (cancelAnimationFrame(pt), bt()), o.clearAlignGuides(), q.removeEventListener("pointermove", ht), q.removeEventListener("pointerup", Wt);
      };
      q.addEventListener("pointermove", ht), q.addEventListener("pointerup", Wt);
    },
    [o, t.id]
  ), P = [
    { pos: "nw", cx: 0, cy: 0 },
    { pos: "n", cx: 0.5, cy: 0 },
    { pos: "ne", cx: 1, cy: 0 },
    { pos: "e", cx: 1, cy: 0.5 },
    { pos: "se", cx: 1, cy: 1 },
    { pos: "s", cx: 0.5, cy: 1 },
    { pos: "sw", cx: 0, cy: 1 },
    { pos: "w", cx: 0, cy: 0.5 }
  ], G = 8 / n, K = G / 2, at = 25 / n, B = e && s && !i, et = it(
    (j) => {
      const q = j.currentTarget.ownerDocument;
      j.stopPropagation(), j.preventDefault();
      const Y = t.x + t.w / 2, $ = t.y + c / 2, nt = t.rotation || 0, { x: ot, y: ft } = o.screenToCanvas(
        j.clientX,
        j.clientY
      ), pt = Math.atan2(ft - $, ot - Y);
      o.pushHistorySnapshot();
      const rt = (zt) => {
        const { x: bt, y: ht } = o.screenToCanvas(
          zt.clientX,
          zt.clientY
        ), Wt = Math.atan2(ht - $, bt - Y);
        let Tt = nt + (Wt - pt) * (180 / Math.PI);
        (zt.shiftKey || o.snapToGrid) && !(zt.metaKey || zt.ctrlKey) && (Tt = Math.round(Tt / 15) * 15), o.updateNode(t.id, { rotation: Tt });
      }, It = () => {
        q.removeEventListener("pointermove", rt), q.removeEventListener("pointerup", It);
      };
      q.addEventListener("pointermove", rt), q.addEventListener("pointerup", It);
    },
    [o, t.id, t.x, t.y, t.w, c, t.rotation]
  ), Z = i && x ? {
    left: x.x + g.x * x.w,
    top: x.y + g.y * x.h,
    width: g.w * x.w,
    height: g.h * x.h
  } : null, Q = i ? void 0 : [t.data.flipH ? "scaleX(-1)" : "", t.data.flipV ? "scaleY(-1)" : ""].filter(Boolean).join(" ") || void 0, L = {
    width: "100%",
    height: "100%",
    objectFit: "contain",
    display: "block",
    pointerEvents: "none",
    opacity: t.data.opacity ?? 1,
    transform: Q
  };
  if (!i && h) {
    const j = h.y * 100, q = (1 - h.x - h.w) * 100, Y = (1 - h.y - h.h) * 100, $ = h.x * 100;
    L.objectViewBox = `inset(${j}% ${q}% ${Y}% ${$}%)`;
  }
  const X = 8 / n, _ = X / 2;
  return /* @__PURE__ */ k(
    "div",
    {
      onPointerDown: T,
      onDoubleClick: !i && r ? (j) => {
        j.stopPropagation(), l == null || l();
      } : void 0,
      style: {
        position: "absolute",
        left: t.x + t.w / 2,
        top: t.y + c / 2,
        width: t.w,
        height: c,
        marginLeft: -t.w / 2,
        marginTop: -c / 2,
        zIndex: t.z,
        border: e ? "2px dashed #3b82f6" : "none",
        borderRadius: 6,
        overflow: "visible",
        pointerEvents: r ? "auto" : "none",
        cursor: i ? "default" : "move",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        boxSizing: "border-box"
      },
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              overflow: "hidden",
              borderRadius: 4,
              border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : "none",
              boxSizing: "border-box"
            },
            children: [
              /* @__PURE__ */ u(
                "img",
                {
                  ref: d,
                  src: t.data.src,
                  alt: t.data.alt ?? "",
                  onLoad: y,
                  style: L,
                  draggable: !1
                }
              ),
              i && Z && /* @__PURE__ */ u(
                "div",
                {
                  onPointerDown: D,
                  style: {
                    position: "absolute",
                    left: Z.left,
                    top: Z.top,
                    width: Z.width,
                    height: Z.height,
                    boxShadow: `0 0 0 ${Math.max(t.w, c) * 2}px rgba(0,0,0,0.45)`,
                    border: `${1.5 / n}px dashed rgba(255,255,255,0.8)`,
                    boxSizing: "border-box",
                    cursor: "move",
                    zIndex: 10
                  }
                }
              )
            ]
          }
        ),
        i && Z && ad.map(({ pos: j, edges: q, cx: Y, cy: $, cursor: nt }) => /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: (ot) => M(q, ot),
            style: {
              position: "absolute",
              left: Z.left + Y * Z.width - _,
              top: Z.top + $ * Z.height - _,
              width: X,
              height: X,
              background: "white",
              border: `${1.5 / n}px solid #3b82f6`,
              borderRadius: 2,
              cursor: nt,
              zIndex: 11
            }
          },
          j
        )),
        e && !i && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                left: "50%",
                top: -at,
                width: 1,
                height: at,
                background: "#3b82f6",
                marginLeft: -0.5,
                pointerEvents: "none"
              }
            }
          ),
          /* @__PURE__ */ u(
            "div",
            {
              onPointerDown: et,
              style: {
                position: "absolute",
                left: "50%",
                top: -(at + G / 2),
                width: G,
                height: G,
                marginLeft: -G / 2,
                borderRadius: "50%",
                background: "white",
                border: "1.5px solid #3b82f6",
                cursor: "grab"
              }
            }
          )
        ] }),
        B && P.map(({ pos: j, cx: q, cy: Y }) => /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: ($) => {
              $.stopPropagation(), s == null || s(t.id, j, $);
            },
            style: {
              position: "absolute",
              left: `calc(${q * 100}% - ${K}px)`,
              top: `calc(${Y * 100}% - ${K}px)`,
              width: G,
              height: G,
              background: "white",
              border: "1.5px solid #3b82f6",
              borderRadius: 2,
              cursor: Br(j, t.rotation || 0)
            }
          },
          j
        ))
      ]
    }
  );
}
const ji = de(ld);
function cd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    ji,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      onResizeHandleDown: t.callbacks.onResizeHandleDown,
      cropping: t.editing,
      onCropStart: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditStart) == null ? void 0 : r.call(o, e.id);
      },
      onCropEnd: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      }
    }
  );
}
const dd = {
  type: "image",
  component: cd,
  handlesOwnLayout: !0,
  onFlip: (t, e) => {
    const o = t.data;
    return e === "h" ? { flipH: !o.flipH } : { flipV: !o.flipV };
  },
  getClipboardText: (t) => t.data.src || null
};
function hd({
  node: t,
  engine: e,
  editing: o,
  editClickPos: r,
  onStopEdit: n,
  onMeasuredHeight: s
}) {
  const i = lt(null), [l, a] = tt(t.data.text), c = lt(!1), h = lt(t.data.text), p = lt(null), d = lt(e);
  d.current = e;
  const f = lt(t);
  f.current = t, mt(() => {
    o || a(t.data.text);
  }, [t.data.text]), wi(() => {
    var M, D;
    if (o && i.current) {
      i.current.innerText = t.data.text, i.current.focus();
      const T = i.current.ownerDocument;
      let P = !1;
      if (r) {
        const G = T.caretRangeFromPoint(r.clientX, r.clientY);
        if (G && i.current.contains(G.startContainer)) {
          const K = (M = T.defaultView) == null ? void 0 : M.getSelection();
          K == null || K.removeAllRanges(), K == null || K.addRange(G), P = !0;
        }
      }
      if (!P) {
        const G = T.createRange(), K = (D = T.defaultView) == null ? void 0 : D.getSelection();
        i.current.childNodes.length > 0 && (G.selectNodeContents(i.current), G.collapse(!1)), K == null || K.removeAllRanges(), K == null || K.addRange(G);
      }
      h.current = t.data.text, c.current = !1;
    }
  }, [o]), mt(() => {
    if (o)
      return () => {
        if (c.current) return;
        c.current = !0;
        const M = h.current, D = e.getNode(t.id);
        if (D && D.type === "text") {
          const T = D.data;
          M !== T.text && e.updateNodeWithHistory(t.id, {
            data: { ...T, text: M }
          });
        }
      };
  }, [o, e, t.id]), mt(() => {
    if (!i.current || !s) return;
    const M = new ResizeObserver(() => {
      var T;
      const D = ((T = i.current) == null ? void 0 : T.offsetHeight) ?? 0;
      D > 0 && s(t.id, D);
    });
    return M.observe(i.current), () => M.disconnect();
  }, [t.id, s, o]);
  const m = it(() => {
    var D;
    if (c.current) return;
    c.current = !0, p.current && (clearTimeout(p.current), p.current = null);
    const M = ((D = i.current) == null ? void 0 : D.innerText) ?? "";
    a(M), h.current = M, M !== t.data.text && e.updateNodeWithHistory(t.id, {
      data: { ...t.data, text: M }
    }), n();
  }, [e, t, n]), y = it(
    (M) => {
      var D;
      M.key === "Escape" && (M.preventDefault(), m(), (D = i.current) == null || D.blur()), M.stopPropagation();
    },
    [m]
  ), g = it(() => {
    m();
  }, [m]), b = it(() => {
    if (i.current) {
      const M = i.current.innerText;
      a(M), h.current = M, p.current && clearTimeout(p.current), p.current = setTimeout(() => {
        const D = f.current;
        M !== D.data.text && d.current.updateNode(D.id, {
          data: { ...D.data, text: M }
        });
      }, 0);
    }
  }, []), x = t.h === "auto" ? void 0 : t.h, C = t.data.opacity ?? 1, I = {
    fontFamily: Ue(t.data.fontFamily),
    fontSize: t.data.fontSize,
    color: t.data.color,
    textAlign: t.data.align,
    opacity: C,
    lineHeight: 1,
    outline: "none",
    whiteSpace: "pre-wrap",
    wordBreak: "break-word",
    border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : void 0,
    boxSizing: t.data.borderColor ? "border-box" : void 0,
    borderRadius: t.data.borderColor ? 4 : void 0,
    padding: t.data.borderColor ? 6 : void 0
  };
  return /* @__PURE__ */ u(
    "div",
    {
      "data-node-id": t.id,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: x,
        zIndex: t.z,
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        pointerEvents: o ? "auto" : "none"
      },
      children: o ? /* @__PURE__ */ u(
        "div",
        {
          ref: i,
          contentEditable: !0,
          suppressContentEditableWarning: !0,
          onKeyDown: y,
          onBlur: g,
          onInput: b,
          onPointerDown: (M) => M.stopPropagation(),
          style: { ...I, minHeight: t.data.fontSize, cursor: "text" }
        }
      ) : /* @__PURE__ */ u("div", { ref: i, style: I, children: l || " " })
    }
  );
}
const Vi = de(hd);
function ud(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Vi,
    {
      node: e,
      engine: t.engine,
      editing: t.editing,
      editClickPos: t.editClickPos,
      onStopEdit: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      },
      onMeasuredHeight: t.callbacks.onMeasuredHeight
    }
  );
}
const pd = {
  type: "text",
  component: ud,
  handlesOwnLayout: !0,
  onResize: (t, e, o) => ({ fontSize: t.data.fontSize * e }),
  getClipboardText: (t) => t.data.text || null
};
function fd(t) {
  const e = t.node, o = e.h === "auto" ? 100 : e.h, r = it(
    (s) => {
      var l, a;
      const i = s.currentTarget.value.trim();
      t.engine.updateNodeWithHistory(e.id, {
        data: { ...e.data, label: i || void 0 }
      }), (a = (l = t.callbacks).onEditEnd) == null || a.call(l);
    },
    [e.id, e.data, t.engine, t.callbacks]
  ), n = it(
    (s) => {
      (s.key === "Enter" || s.key === "Escape") && s.currentTarget.blur(), s.stopPropagation();
    },
    []
  );
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x,
        top: e.y,
        width: e.w,
        height: o,
        zIndex: e.z,
        background: e.data.backgroundColor || "rgba(0,0,0,0.02)",
        border: `${e.data.borderWidth || 1}px ${e.data.borderStyle || "dashed"} ${e.data.borderColor || "#ccc"}`,
        boxSizing: "border-box",
        borderRadius: 8,
        opacity: e.data.opacity ?? 1,
        pointerEvents: "none",
        overflow: "visible",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: t.editing ? /* @__PURE__ */ u(
        "input",
        {
          autoFocus: !0,
          defaultValue: e.data.label ?? "",
          placeholder: "Frame label...",
          onBlur: r,
          onKeyDown: n,
          onPointerDown: (s) => s.stopPropagation(),
          style: {
            position: "absolute",
            top: -24,
            left: 0,
            fontSize: 12,
            color: e.data.borderColor || "#999",
            fontWeight: 500,
            background: "rgba(255,255,255,0.95)",
            border: "1px solid #3b82f6",
            borderRadius: 4,
            padding: "1px 4px",
            outline: "none",
            pointerEvents: "auto",
            minWidth: 80
          }
        }
      ) : e.data.label ? /* @__PURE__ */ u(
        "div",
        {
          onDoubleClick: (s) => {
            var i, l;
            s.stopPropagation(), t.engine.select(e.id), (l = (i = t.callbacks).onEditStart) == null || l.call(i, e.id);
          },
          style: {
            position: "absolute",
            top: -20,
            left: 4,
            fontSize: 12,
            color: e.data.borderColor || "#999",
            fontWeight: 500,
            whiteSpace: "nowrap",
            userSelect: "none",
            pointerEvents: "auto",
            cursor: "default"
          },
          children: e.data.label
        }
      ) : null
    }
  );
}
const yd = {
  type: "frame",
  component: fd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.label || null
}, md = 100;
function gd({
  node: t,
  isSelected: e,
  engine: o,
  interactive: r,
  zoom: n,
  editing: s,
  onEditStart: i,
  onEditEnd: l
}) {
  const a = lt(null), c = lt(null), h = lt(""), p = lt(null), d = lt(null), f = lt(t);
  f.current = t;
  const m = lt(o);
  m.current = o, mt(() => {
    var I;
    if (s && c.current) {
      const M = c.current;
      M.innerText = t.data.text || "", h.current = t.data.text || "", M.focus();
      const D = M.ownerDocument, T = (I = D.defaultView) == null ? void 0 : I.getSelection(), P = p.current;
      p.current = null;
      let G = !1;
      if (P && T && D.caretRangeFromPoint) {
        const K = D.caretRangeFromPoint(P.x, P.y);
        K && M.contains(K.startContainer) && (T.removeAllRanges(), T.addRange(K), G = !0);
      }
      if (!G && T) {
        const K = D.createRange();
        M.childNodes.length > 0 && (K.selectNodeContents(M), K.collapse(!1)), T.removeAllRanges(), T.addRange(K);
      }
    }
  }, [s]), mt(() => {
    if (s)
      return () => {
        const I = f.current, M = h.current;
        M !== I.data.text && m.current.updateNodeWithHistory(I.id, {
          data: { ...I.data, text: M }
        });
      };
  }, [s]);
  const y = it(() => {
    d.current && (clearTimeout(d.current), d.current = null), c.current && (h.current = c.current.innerText), l();
  }, [l]), g = it(
    (I) => {
      const M = I.currentTarget.ownerDocument;
      if (I.altKey) return;
      if (!o.selection.has(t.id) && o.selection.size > 0) {
        const { x: _, y: j } = o.screenToCanvas(I.clientX, I.clientY);
        for (const q of o.selection) {
          const Y = o.getNode(q);
          if (!Y) continue;
          const $ = Y.h === "auto" ? 100 : Y.h;
          if (_ >= Y.x && _ <= Y.x + Y.w && j >= Y.y && j <= Y.y + $)
            return;
        }
      }
      if (I.stopPropagation(), s) return;
      I.currentTarget.setPointerCapture(I.pointerId), I.shiftKey ? o.toggleSelect(t.id) : o.selection.has(t.id) || o.select(t.id);
      const D = I.clientX, T = I.clientY, P = Array.from(o.selection), G = [];
      for (const _ of P) {
        const j = o.getNode(_);
        j && G.push({ id: _, x: j.x, y: j.y });
      }
      if (G.length === 0) return;
      let K = !1, at = null, B = D, et = T, Z = !1;
      const Q = () => {
        at = null;
        const _ = (B - D) / o.viewport.zoom, j = (et - T) / o.viewport.zoom, { finalDx: q, finalDy: Y } = o.computeDragSnap(
          G,
          P,
          _,
          j,
          Z
        ), $ = G.map((nt) => ({
          id: nt.id,
          patch: { x: nt.x + q, y: nt.y + Y }
        }));
        o.updateMany($);
      }, L = (_) => {
        const j = (_.clientX - D) / o.viewport.zoom, q = (_.clientY - T) / o.viewport.zoom;
        if (!K)
          if (Math.abs(j) > 2 || Math.abs(q) > 2)
            K = !0, o.pushHistorySnapshot();
          else
            return;
        B = _.clientX, et = _.clientY, Z = _.metaKey || _.ctrlKey, at === null && (at = requestAnimationFrame(Q));
      }, X = () => {
        at !== null && (cancelAnimationFrame(at), Q()), o.clearAlignGuides(), M.removeEventListener("pointermove", L), M.removeEventListener("pointerup", X);
      };
      M.addEventListener("pointermove", L), M.addEventListener("pointerup", X);
    },
    [o, t.id, s]
  ), b = it(
    (I) => {
      if (r) {
        if (I.stopPropagation(), t.groupId) {
          const M = [];
          let D = t.groupId;
          for (; D; )
            M.push(D), D = o.groupParent.get(D);
          if (!o.activeGroupId) {
            o.enterGroup(M[M.length - 1]), o.select(t.id);
            return;
          }
          const T = M.indexOf(o.activeGroupId);
          if (T > 0) {
            o.enterGroup(M[T - 1]), o.select(t.id);
            return;
          }
        }
        s || (p.current = { x: I.clientX, y: I.clientY }, o.select(t.id), i(t.id));
      }
    },
    [r, s, o, t.id, t.groupId, i]
  ), x = t.data.fontSize ?? 16, C = t.h === "auto" ? md : t.h;
  return /* @__PURE__ */ u(
    "div",
    {
      ref: a,
      "data-node-id": t.id,
      className: r ? void 0 : "sb-block-inert",
      onPointerDown: r ? g : void 0,
      onDoubleClick: b,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: C,
        zIndex: t.z,
        background: t.data.color,
        borderRadius: t.data.edgeStyle === "round" ? 12 : 2,
        boxShadow: "0 2px 8px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.08)",
        opacity: t.data.opacity ?? 1,
        pointerEvents: r ? "auto" : "none",
        outline: "none",
        overflow: "hidden",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "div",
        {
          style: {
            padding: 12,
            height: "100%",
            overflow: "auto",
            touchAction: s ? "pan-y" : "none",
            cursor: s ? "text" : "move",
            userSelect: s ? "text" : "none"
          },
          children: s ? /* @__PURE__ */ u(
            "div",
            {
              ref: c,
              contentEditable: !0,
              suppressContentEditableWarning: !0,
              onBlur: y,
              onInput: () => {
                c.current && (h.current = c.current.innerText, d.current && clearTimeout(d.current), d.current = setTimeout(() => {
                  const I = f.current, M = h.current;
                  M !== I.data.text && m.current.updateNode(I.id, {
                    data: { ...I.data, text: M }
                  });
                }, 0));
              },
              onKeyDown: (I) => {
                I.key === "Escape" && (I.stopPropagation(), y()), I.stopPropagation();
              },
              onPointerDown: (I) => I.stopPropagation(),
              style: {
                fontSize: x,
                fontFamily: Ue(Ze),
                lineHeight: 1.5,
                color: "#1e1e2e",
                wordWrap: "break-word",
                whiteSpace: "pre-wrap",
                outline: "none",
                minHeight: "100%"
              }
            }
          ) : /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: x,
                fontFamily: Ue(Ze),
                lineHeight: 1.5,
                color: "#1e1e2e",
                wordWrap: "break-word",
                whiteSpace: "pre-wrap"
              },
              children: t.data.text || ""
            }
          )
        }
      )
    }
  );
}
const Ki = de(gd);
function bd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Ki,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      editing: t.editing,
      onEditStart: (o) => {
        var r, n;
        return (n = (r = t.callbacks).onEditStart) == null ? void 0 : n.call(r, o);
      },
      onEditEnd: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      }
    }
  );
}
const xd = {
  type: "sticky",
  component: bd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.text || null
}, qi = /(?:youtube\.com\/(?:watch\?.*v=|embed\/|shorts\/|live\/)|youtu\.be\/)([\w-]{11})/;
function wd(t) {
  const e = t.match(qi);
  return e ? e[1] : null;
}
function kd(t) {
  return qi.test(t);
}
function vd(t) {
  return `https://www.youtube.com/embed/${t}`;
}
function Sd(t) {
  return `https://img.youtube.com/vi/${t}/hqdefault.jpg`;
}
function Md({
  node: t,
  isSelected: e,
  engine: o,
  interactive: r,
  zoom: n,
  editing: s,
  onResizeHandleDown: i,
  onEditStart: l
}) {
  const a = t.h, { data: c } = t, h = (m) => {
    if (r && s) {
      m.stopPropagation();
      return;
    }
  }, p = c.borderColor ? `${c.borderWidth ?? 1}px ${c.borderStyle ?? "solid"} ${c.borderColor}` : "none", d = Math.max(6, 8 / n), f = [
    { key: "nw", x: "0%", y: "0%", cursor: "nwse-resize" },
    { key: "ne", x: "100%", y: "0%", cursor: "nesw-resize" },
    { key: "se", x: "100%", y: "100%", cursor: "nwse-resize" },
    { key: "sw", x: "0%", y: "100%", cursor: "nesw-resize" },
    { key: "n", x: "50%", y: "0%", cursor: "ns-resize" },
    { key: "s", x: "50%", y: "100%", cursor: "ns-resize" },
    { key: "e", x: "100%", y: "50%", cursor: "ew-resize" },
    { key: "w", x: "0%", y: "50%", cursor: "ew-resize" }
  ];
  return /* @__PURE__ */ k(
    "div",
    {
      onPointerDown: h,
      onDoubleClick: !s && r ? (m) => {
        m.stopPropagation(), l == null || l();
      } : void 0,
      style: {
        position: "absolute",
        left: t.x + t.w / 2,
        top: t.y + a / 2,
        width: t.w,
        height: a,
        marginLeft: -t.w / 2,
        marginTop: -a / 2,
        zIndex: t.z,
        border: e ? "2px dashed #3b82f6" : "none",
        borderRadius: 6,
        overflow: "visible",
        pointerEvents: r ? "auto" : "none",
        cursor: s ? "default" : "move",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        boxSizing: "border-box"
      },
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              overflow: "hidden",
              borderRadius: 4,
              border: p,
              boxSizing: "border-box",
              opacity: c.opacity ?? 1
            },
            children: [
              /* @__PURE__ */ u(
                "iframe",
                {
                  src: vd(c.videoId),
                  title: "YouTube video",
                  allow: "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture",
                  allowFullScreen: !0,
                  style: {
                    width: "100%",
                    height: "100%",
                    border: "none",
                    display: "block",
                    pointerEvents: s ? "auto" : "none"
                  }
                }
              ),
              !s && /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    position: "absolute",
                    inset: 0,
                    cursor: "move"
                  }
                }
              )
            ]
          }
        ),
        e && r && !s && f.map((m) => /* @__PURE__ */ u(
          "div",
          {
            "data-handle": m.key,
            onPointerDown: (y) => {
              y.stopPropagation(), i == null || i(t.id, m.key, y);
            },
            style: {
              position: "absolute",
              left: m.x,
              top: m.y,
              width: d,
              height: d,
              marginLeft: -d / 2,
              marginTop: -d / 2,
              background: "#fff",
              border: "1px solid #3b82f6",
              borderRadius: 2,
              cursor: m.cursor,
              zIndex: 1
            }
          },
          m.key
        ))
      ]
    }
  );
}
const Cd = de(Md);
function zd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Cd,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      editing: t.editing,
      onResizeHandleDown: t.callbacks.onResizeHandleDown,
      onEditStart: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditStart) == null ? void 0 : r.call(o, e.id);
      },
      onEditEnd: () => {
        var o, r;
        return (r = (o = t.callbacks).onEditEnd) == null ? void 0 : r.call(o);
      }
    }
  );
}
const Id = {
  type: "youtube",
  component: zd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.url || null
}, Td = [
  nc,
  od,
  nd,
  id,
  dd,
  pd,
  yd,
  xd,
  Id
];
function ao(t, e) {
  return `${t}:${e}`;
}
class Pd {
  constructor(e, o) {
    St(this, "spatial");
    St(this, "registry");
    /** Current resolved port values. */
    St(this, "values", /* @__PURE__ */ new Map());
    /** Node IDs that need recomputation. */
    St(this, "dirty", /* @__PURE__ */ new Set());
    /** Whether a microtask flush is already scheduled. */
    St(this, "scheduled", !1);
    /** Generation counter for canceling stale async results. */
    St(this, "generation", 0);
    /** Change subscribers. */
    St(this, "listeners", /* @__PURE__ */ new Set());
    /** Node IDs that are part of a cycle (updated after each topoSort). */
    St(this, "_cycleNodeIds", /* @__PURE__ */ new Set());
    this.spatial = e, this.registry = o;
  }
  // ── Public API ─────────────────────────────────────────────
  /** Node IDs that are part of a dependency cycle (read-only). */
  get cycleNodeIds() {
    return this._cycleNodeIds;
  }
  /** Subscribe to port value changes. Returns unsubscribe function. */
  onChange(e) {
    return this.listeners.add(e), () => this.listeners.delete(e);
  }
  /** Get current value of a specific port. */
  getPortValue(e, o) {
    return this.values.get(ao(e, o)) ?? null;
  }
  /** Get all input values for a node, resolved from connected edges. */
  getInputs(e) {
    var s;
    const o = this.registry.get(
      ((s = this.spatial.nodes.get(e)) == null ? void 0 : s.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const r = {}, n = o.ports.filter((i) => i.direction === "input");
    for (const i of n) {
      const l = this.spatial.getEdgesForNode(e);
      let a = !1;
      for (const c of l) {
        const h = c.data;
        if (h.toId === e && h.targetPort === i.id) {
          const p = this.values.get(
            ao(h.fromId, h.sourcePort ?? "")
          );
          r[i.id] = p ?? i.defaultValue ?? null, a = !0;
          break;
        }
      }
      a || (r[i.id] = i.defaultValue ?? null);
    }
    return r;
  }
  /** Get all output values for a node. */
  getOutputs(e) {
    var n;
    const o = this.registry.get(
      ((n = this.spatial.nodes.get(e)) == null ? void 0 : n.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const r = {};
    for (const s of o.ports)
      s.direction === "output" && (r[s.id] = this.values.get(ao(e, s.id)) ?? null);
    return r;
  }
  /** Get all port values (inputs + outputs) for a node. */
  getAllPortValues(e) {
    var n;
    const o = this.registry.get(
      ((n = this.spatial.nodes.get(e)) == null ? void 0 : n.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const r = {};
    for (const s of o.ports)
      if (s.direction === "input") {
        const i = this.spatial.getEdgesForNode(e);
        let l = !1;
        for (const a of i) {
          const c = a.data;
          if (c.toId === e && c.targetPort === s.id) {
            r[s.id] = this.values.get(ao(c.fromId, c.sourcePort ?? "")) ?? s.defaultValue ?? null, l = !0;
            break;
          }
        }
        l || (r[s.id] = s.defaultValue ?? null);
      } else
        r[s.id] = this.values.get(ao(e, s.id)) ?? null;
    return r;
  }
  /** Mark a node as dirty and schedule recomputation. */
  markDirty(e) {
    this.dirty.add(e), this.scheduleFlush();
  }
  /** Wire up SpatialEngine event listeners. Returns cleanup function. */
  connect() {
    const e = (n) => {
      const s = this.registry.get(n.type);
      s != null && s.ports && this.markDirty(n.id);
    }, o = (n) => {
      if (n.type === "edge") {
        const s = n.data;
        s.targetPort && this.markDirty(s.toId);
      } else {
        const s = this.registry.get(n.type);
        s != null && s.ports && s.compute && this.markDirty(n.id);
      }
    }, r = (n) => {
      if (n.type === "edge") {
        const s = n.data;
        s.targetPort && this.markDirty(s.toId);
      } else {
        const s = this.registry.get(n.type);
        if (s != null && s.ports) {
          for (const i of s.ports)
            this.values.delete(ao(n.id, i.id));
          this.markDownstream(n.id);
        }
      }
    };
    return this.spatial.on("node:data", e), this.spatial.on("node:create", o), this.spatial.on("node:delete", r), this.initializeAll(), () => {
      this.spatial.off("node:data", e), this.spatial.off("node:create", o), this.spatial.off("node:delete", r);
    };
  }
  /** Dispose and clean up. */
  dispose() {
    this.values.clear(), this.dirty.clear(), this.listeners.clear(), this.scheduled = !1;
  }
  // ── Private implementation ─────────────────────────────────
  /** Initialize all nodes with ports. */
  initializeAll() {
    for (const e of this.spatial.nodes.values()) {
      const o = this.registry.get(e.type);
      o != null && o.ports && o.compute && this.dirty.add(e.id);
    }
    this.dirty.size > 0 && this.scheduleFlush();
  }
  /** Schedule a microtask flush if not already scheduled. */
  scheduleFlush() {
    this.scheduled || (this.scheduled = !0, queueMicrotask(() => {
      this.scheduled = !1, this.flush();
    }));
  }
  /** Mark all downstream nodes (nodes that depend on outputs of nodeId) as dirty. */
  markDownstream(e) {
    const o = this.spatial.getEdgesForNode(e);
    for (const r of o) {
      const n = r.data;
      n.fromId === e && n.targetPort && this.dirty.add(n.toId);
    }
  }
  /** Topological sort of dirty nodes + their downstream dependents. */
  topoSort() {
    const e = /* @__PURE__ */ new Set();
    for (const m of this.spatial.nodes.values()) {
      const y = this.registry.get(m.type);
      y != null && y.ports && y.compute && e.add(m.id);
    }
    if (e.size === 0) {
      const m = this._cycleNodeIds.size > 0;
      return m && (this._cycleNodeIds = /* @__PURE__ */ new Set()), { sorted: [], cyclesChanged: m };
    }
    const o = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
    for (const m of e)
      o.set(m, /* @__PURE__ */ new Set()), r.set(m, 0);
    const n = this.spatial.getAllEdges();
    for (const m of n) {
      const y = m.data;
      y.sourcePort && y.targetPort && e.has(y.fromId) && e.has(y.toId) && (o.get(y.fromId).add(y.toId), r.set(y.toId, (r.get(y.toId) ?? 0) + 1));
    }
    const s = new Set(this.dirty), i = /* @__PURE__ */ new Set(), l = (m) => {
      if (i.has(m)) return;
      i.add(m);
      const y = o.get(m);
      if (y)
        for (const g of y)
          s.add(g), l(g);
    };
    for (const m of [...this.dirty])
      l(m);
    const a = /* @__PURE__ */ new Map();
    for (const m of s)
      a.set(m, 0);
    for (const m of n) {
      const y = m.data;
      y.sourcePort && y.targetPort && s.has(y.fromId) && s.has(y.toId) && a.set(
        y.toId,
        (a.get(y.toId) ?? 0) + 1
      );
    }
    const c = [];
    for (const [m, y] of a)
      y === 0 && c.push(m);
    const h = [];
    for (; c.length > 0; ) {
      const m = c.shift();
      h.push(m);
      const y = o.get(m);
      if (y)
        for (const g of y) {
          if (!s.has(g)) continue;
          const b = (a.get(g) ?? 1) - 1;
          a.set(g, b), b === 0 && c.push(g);
        }
    }
    const p = new Set(h), d = /* @__PURE__ */ new Set();
    for (const m of s)
      p.has(m) || d.add(m);
    let f = !1;
    return (d.size !== this._cycleNodeIds.size || [...d].some((m) => !this._cycleNodeIds.has(m))) && (this._cycleNodeIds = d, f = !0), { sorted: h, cyclesChanged: f };
  }
  /** Full graph recompute of dirty nodes. */
  flush() {
    if (this.dirty.size === 0) return;
    const { sorted: e, cyclesChanged: o } = this.topoSort();
    this.dirty.clear();
    let r = !1;
    for (const n of e)
      this.executeNode(n) && (r = !0);
    (r || o) && this.notifyListeners();
  }
  /** Execute a single node's compute function. Returns true if outputs changed. */
  executeNode(e) {
    const o = this.spatial.nodes.get(e);
    if (!o) return !1;
    const r = this.registry.get(o.type);
    if (!(r != null && r.compute) || !r.ports) return !1;
    const n = this.getInputs(e), s = r.compute(n, o.data);
    if (s instanceof Promise) {
      const i = ++this.generation;
      return s.then((l) => {
        if (i !== this.generation) return;
        this.applyOutputs(e, r.ports, l) && (this.markDownstream(e), this.notifyListeners(), this.dirty.size > 0 && this.scheduleFlush());
      }), !1;
    }
    return this.applyOutputs(e, r.ports, s);
  }
  /** Apply computed outputs to the values map. Returns true if any value changed. */
  applyOutputs(e, o, r) {
    let n = !1;
    for (const s of o) {
      if (s.direction !== "output") continue;
      const i = ao(e, s.id), l = r[s.id] ?? null, a = this.values.get(i) ?? null;
      Rd(a, l) || (this.values.set(i, l), n = !0);
    }
    return n && this.markDownstream(e), n;
  }
  /** Notify all change listeners. */
  notifyListeners() {
    for (const e of this.listeners)
      e();
  }
}
function Rd(t, e) {
  if (t === e) return !0;
  if (t == null || e == null || typeof t != typeof e) return !1;
  if (typeof t == "object" && typeof e == "object") {
    const o = Object.keys(t), r = Object.keys(e);
    if (o.length !== r.length) return !1;
    for (const n of o)
      if (t[n] !== e[n])
        return !1;
    return !0;
  }
  return !1;
}
const No = [
  // Light
  { key: "plain-white", label: "White", group: "light", canvasBg: "#ffffff", swatchColor: "#ffffff" },
  { key: "dot-grid", label: "Cream", group: "light", canvasBg: "#f8f7f5", swatchColor: "#f8f7f5" },
  { key: "engineering", label: "Warm", group: "light", canvasBg: "#fdf6e3", swatchColor: "#fdf6e3" },
  // Dark
  { key: "blueprint", label: "Blueprint", group: "dark", canvasBg: "#1e3a5f", swatchColor: "#1e3a5f" },
  { key: "dark-grid", label: "Night", group: "dark", canvasBg: "#1a1a2e", swatchColor: "#1a1a2e" },
  // Textured
  { key: "japanese-stationery", label: "Japanese Stationery", group: "textured", canvasBg: "#f5f0e8", swatchColor: "#f5f0e8" },
  { key: "kraft", label: "Kraft Paper", group: "textured", canvasBg: "#d4b896", swatchColor: "#d4b896" }
];
function sr(t) {
  return No.find((e) => e.key === t) ?? No[1];
}
function Ad() {
  return {
    staticDefs: /* @__PURE__ */ k("filter", { id: "paper-texture", x: "-20%", y: "-20%", width: "140%", height: "140%", children: [
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.08", numOctaves: 4, seed: 12, stitchTiles: "stitch", result: "noise" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "noise", type: "saturate", values: "0", result: "bump" }),
      /* @__PURE__ */ u("feDiffuseLighting", { in: "bump", lightingColor: "#f7f4ee", surfaceScale: "1.2", diffuseConstant: "1", result: "lit", children: /* @__PURE__ */ u("feDistantLight", { azimuth: "225", elevation: "50" }) }),
      /* @__PURE__ */ u("feComposite", { in: "lit", in2: "bump", operator: "in", result: "lit-masked" }),
      /* @__PURE__ */ u("feFlood", { floodColor: "#f5f0e8", result: "base" }),
      /* @__PURE__ */ u("feBlend", { in: "base", in2: "lit-masked", mode: "overlay", result: "paper" }),
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.6", numOctaves: 3, seed: 7, stitchTiles: "stitch", result: "grain" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "grain", type: "saturate", values: "0", result: "grain-gray" }),
      /* @__PURE__ */ k("feComponentTransfer", { in: "grain-gray", result: "grain-subtle", children: [
        /* @__PURE__ */ u("feFuncR", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncG", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncB", { type: "linear", slope: "0.06", intercept: "0.47" })
      ] }),
      /* @__PURE__ */ u("feBlend", { in: "paper", in2: "grain-subtle", mode: "overlay", result: "paper-final" })
    ] }),
    staticLayers: [
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "#f5f0e8", filter: "url(#paper-texture)" }, "texture")
    ]
  };
}
function Ld() {
  return {
    staticDefs: /* @__PURE__ */ k("filter", { id: "kraft-texture", x: "-20%", y: "-20%", width: "140%", height: "140%", children: [
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.04", numOctaves: 5, seed: 42, stitchTiles: "stitch", result: "noise" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "noise", type: "saturate", values: "0", result: "bump" }),
      /* @__PURE__ */ u("feDiffuseLighting", { in: "bump", lightingColor: "#e0c9a6", surfaceScale: "1.4", diffuseConstant: "0.95", result: "lit", children: /* @__PURE__ */ u("feDistantLight", { azimuth: "200", elevation: "50" }) }),
      /* @__PURE__ */ u("feComposite", { in: "lit", in2: "bump", operator: "in", result: "lit-masked" }),
      /* @__PURE__ */ u("feFlood", { floodColor: "#d4b896", result: "base" }),
      /* @__PURE__ */ u("feBlend", { in: "base", in2: "lit-masked", mode: "overlay", result: "kraft" }),
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.35", numOctaves: 2, seed: 99, stitchTiles: "stitch", result: "fiber" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "fiber", type: "saturate", values: "0", result: "fiber-gray" }),
      /* @__PURE__ */ k("feComponentTransfer", { in: "fiber-gray", result: "fiber-subtle", children: [
        /* @__PURE__ */ u("feFuncR", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncG", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncB", { type: "linear", slope: "0.06", intercept: "0.47" })
      ] }),
      /* @__PURE__ */ u("feBlend", { in: "kraft", in2: "fiber-subtle", mode: "overlay", result: "kraft-final" })
    ] }),
    staticLayers: [
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "#d4b896", filter: "url(#kraft-texture)" }, "texture")
    ]
  };
}
const dn = {
  "japanese-stationery": Ad,
  kraft: Ld
};
function Dd(t) {
  var e;
  return ((e = dn[t]) == null ? void 0 : e.call(dn)) ?? {};
}
const Zi = {
  position: "absolute",
  inset: 0,
  width: "100%",
  height: "100%",
  pointerEvents: "none"
}, Ed = {
  ...Zi,
  willChange: "transform"
}, Wd = de(function({
  background: e
}) {
  const o = sr(e), { staticDefs: r, staticLayers: n } = Dd(e);
  return /* @__PURE__ */ k("svg", { style: Ed, children: [
    r && /* @__PURE__ */ u("defs", { children: r }),
    /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: o.canvasBg }),
    n
  ] });
});
function Fd({
  viewport: t,
  gridSize: e = 20,
  background: o = "dot-grid",
  gridVisible: r = !0
}) {
  const n = e * t.zoom, s = t.x % n, i = t.y % n, a = sr(o).group === "dark" ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.15)";
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(Wd, { background: o }),
    r && /* @__PURE__ */ k("svg", { style: Zi, children: [
      /* @__PURE__ */ u("defs", { children: /* @__PURE__ */ u(
        "pattern",
        {
          id: "grid-dots",
          x: s,
          y: i,
          width: n,
          height: n,
          patternUnits: "userSpaceOnUse",
          children: /* @__PURE__ */ u("circle", { cx: n / 2, cy: n / 2, r: 1.5, fill: a })
        }
      ) }),
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "url(#grid-dots)" })
    ] })
  ] });
}
const Ui = "sb-excalib-index", Jn = "sb-excalib-";
function Hr() {
  try {
    const t = localStorage.getItem(Ui);
    return t ? JSON.parse(t) : [];
  } catch {
    return [];
  }
}
function Qi(t) {
  localStorage.setItem(Ui, JSON.stringify(t));
}
function Bd(t) {
  try {
    const e = localStorage.getItem(Jn + t);
    return e ? $n(JSON.parse(e)) : null;
  } catch {
    return null;
  }
}
function $n(t) {
  if (t.libraryItems)
    return t;
  const o = (t.library ?? []).map((r, n) => ({
    id: gt(10),
    name: `Item ${n + 1}`,
    status: "published",
    created: Date.now(),
    elements: r
  }));
  return {
    type: "excalidrawlib",
    version: 2,
    source: t.source,
    libraryItems: o
  };
}
function Ji() {
  return Hr();
}
function _n(t) {
  const e = Bd(t);
  return (e == null ? void 0 : e.libraryItems) ?? [];
}
function ts(t, e) {
  const o = $n(t), r = gt(10), n = o.libraryItems.map((l) => l.name || "Untitled"), s = {
    id: r,
    name: (e == null ? void 0 : e.name) || "Imported Library",
    source: (e == null ? void 0 : e.source) || "local-import",
    installedAt: Date.now(),
    itemCount: o.libraryItems.length,
    itemNames: n
  };
  localStorage.setItem(Jn + r, JSON.stringify(o));
  const i = Hr();
  return i.push(s), Qi(i), s;
}
function Nd(t) {
  localStorage.removeItem(Jn + t);
  const e = Hr().filter((o) => o.id !== t);
  Qi(e);
}
function Hd(t) {
  if (!t.trim()) return [];
  const e = t.toLowerCase(), o = [], r = Hr();
  for (const n of r) {
    if (!n.itemNames.some((l) => l.toLowerCase().includes(e)) && !n.name.toLowerCase().includes(e)) continue;
    const i = _n(n.id);
    for (const l of i)
      ((l.name || "").toLowerCase().includes(e) || n.name.toLowerCase().includes(e)) && o.push({ library: n, item: l });
  }
  return o;
}
async function Od(t, e) {
  const o = await fetch(t);
  if (!o.ok) throw new Error(`Failed to fetch library: ${o.status}`);
  const r = await o.json();
  if (r.type !== "excalidrawlib")
    throw new Error("Invalid file: not an Excalidraw library");
  const n = $n(r);
  return ts(n, { name: e, source: t });
}
const Tn = {
  toolbarBg: "#1e1e2e",
  panelBg: "#1e1e2e",
  panelShadow: "0 4px 24px rgba(0,0,0,0.4)",
  panelBorderRadius: 12,
  controlBg: "#2a2a3e",
  controlBgActive: "#3b82f6",
  controlBorderRadius: 6,
  text: "white",
  textMuted: "#999",
  textSecondary: "#888",
  textFaint: "#666",
  textDisabled: "#555",
  border: "#333",
  separator: "#444",
  swatchBorderActive: "white",
  error: "#e74c3c",
  accentColor: "#3b82f6"
}, $i = ki(Tn);
function qt() {
  return Wr($i);
}
function ir(t) {
  const e = Math.round(t) / 100;
  return e < 1 ? e : void 0;
}
function mo(t) {
  if (t)
    return t * (180 / Math.PI);
}
function _i(t) {
  if (!(!t || t === "transparent"))
    return t;
}
function ta(t) {
  if (t === "solid") return "solid";
  if (t === "cross-hatch") return "cross-hatch";
  if (t === "hachure") return "hachure";
}
function ea(t) {
  if (t === "dashed") return "dashed";
  if (t === "dotted") return "dotted";
}
function oa(t) {
  switch (t) {
    case 2:
      return "sans-serif";
    case 3:
      return "monospace";
    default:
      return "Excalifont";
  }
}
function ra(t) {
  return t === "right" ? "right" : t === "center" ? "center" : "left";
}
function Xd(t) {
  if (t.roundness || t.strokeSharpness === "round") return "round";
}
function hn(t, e) {
  return {
    id: gt(10),
    type: "shape",
    x: t.x,
    y: t.y,
    w: t.width,
    h: t.height,
    z: 0,
    rotation: mo(t.angle),
    locked: t.locked || void 0,
    data: {
      shape: e,
      stroke: t.strokeColor || "#1e1e2e",
      fill: _i(t.backgroundColor),
      fillStyle: ta(t.fillStyle),
      strokeWidth: t.strokeWidth || 2,
      strokeStyle: ea(t.strokeStyle),
      roughness: Math.min(t.roughness ?? 1, 2),
      opacity: ir(t.opacity ?? 100),
      edgeStyle: e === "rect" || e === "diamond" ? Xd(t) : void 0
    }
  };
}
function qs(t, e) {
  const o = t.points ?? [[0, 0]];
  if (o.length < 2) return [];
  const r = {
    stroke: t.strokeColor || "#1e1e2e",
    fill: void 0,
    fillStyle: void 0,
    strokeWidth: t.strokeWidth || 2,
    strokeStyle: ea(t.strokeStyle),
    roughness: Math.min(t.roughness ?? 1, 2),
    opacity: ir(t.opacity ?? 100)
  };
  if (o.length === 2) {
    const [l, a] = o, c = Math.min(l[0], a[0]), h = Math.min(l[1], a[1]), p = Math.max(l[0], a[0]), d = Math.max(l[1], a[1]), f = Math.max(p - c, 1), m = Math.max(d - h, 1);
    return [
      {
        id: gt(10),
        type: "shape",
        x: t.x + c,
        y: t.y + h,
        w: f,
        h: m,
        z: 0,
        rotation: mo(t.angle),
        locked: t.locked || void 0,
        data: {
          ...r,
          shape: e ? "arrow" : "line",
          startPoint: [l[0] - c, l[1] - h],
          endPoint: [a[0] - c, a[1] - h]
        }
      }
    ];
  }
  if (t.backgroundColor && t.backgroundColor !== "transparent") {
    const l = Yd(t);
    if (l) return [l];
  }
  const s = gt(10), i = [];
  for (let l = 0; l < o.length - 1; l++) {
    const a = o[l], c = o[l + 1], h = Math.min(a[0], c[0]), p = Math.min(a[1], c[1]), d = Math.max(a[0], c[0]), f = Math.max(a[1], c[1]), m = Math.max(d - h, 1), y = Math.max(f - p, 1), g = l === o.length - 2;
    i.push({
      id: gt(10),
      type: "shape",
      x: t.x + h,
      y: t.y + p,
      w: m,
      h: y,
      z: 0,
      rotation: mo(t.angle),
      locked: t.locked || void 0,
      groupId: s,
      data: {
        ...r,
        shape: e && g ? "arrow" : "line",
        startPoint: [a[0] - h, a[1] - p],
        endPoint: [c[0] - h, c[1] - p]
      }
    });
  }
  return i;
}
function Yd(t) {
  const e = t.points ?? [];
  if (e.length < 3) return null;
  let o = 1 / 0, r = 1 / 0, n = -1 / 0, s = -1 / 0;
  for (const [l, a] of e)
    l < o && (o = l), a < r && (r = a), l > n && (n = l), a > s && (s = a);
  if (!isFinite(o)) return null;
  const i = e.map(([l, a]) => [
    l - o,
    a - r,
    0.5
  ]);
  return {
    id: gt(10),
    type: "draw",
    x: t.x + o,
    y: t.y + r,
    w: Math.max(n - o, 1),
    h: Math.max(s - r, 1),
    z: 0,
    rotation: mo(t.angle),
    locked: t.locked || void 0,
    data: {
      tool: "vector",
      points: i,
      color: t.strokeColor || "#1e1e2e",
      strokeWidth: t.strokeWidth || 2,
      opacity: ir(t.opacity ?? 100),
      fill: _i(t.backgroundColor),
      fillStyle: ta(t.fillStyle)
    }
  };
}
function Gd(t) {
  const e = t.points;
  if (!e || e.length === 0) return null;
  const o = t.pressures, r = t.simulatePressure !== !1, n = e.map((h, p) => {
    const d = !r && o && p < o.length ? o[p] : 0.5;
    return [h[0], h[1], d];
  });
  let s = 1 / 0, i = 1 / 0, l = -1 / 0, a = -1 / 0;
  for (const [h, p] of n)
    h < s && (s = h), p < i && (i = p), h > l && (l = h), p > a && (a = p);
  isFinite(s) || (s = 0, i = 0, l = 0, a = 0);
  const c = n.map(
    ([h, p, d]) => [h - s, p - i, d]
  );
  return {
    id: gt(10),
    type: "draw",
    x: t.x + s,
    y: t.y + i,
    w: Math.max(l - s, 1),
    h: Math.max(a - i, 1),
    z: 0,
    rotation: mo(t.angle),
    locked: t.locked || void 0,
    data: {
      tool: "pen",
      points: c,
      color: t.strokeColor || "#1e1e2e",
      strokeWidth: t.strokeWidth || 2,
      opacity: ir(t.opacity ?? 100)
    }
  };
}
function jd(t) {
  return {
    id: gt(10),
    type: "text",
    x: t.x,
    y: t.y,
    w: Math.ceil((t.width || 200) * 1.2),
    h: "auto",
    z: 0,
    rotation: mo(t.angle),
    locked: t.locked || void 0,
    data: {
      text: t.originalText || t.text || "",
      fontSize: t.fontSize || 20,
      fontFamily: oa(t.fontFamily),
      color: t.strokeColor || "#1e1e2e",
      align: ra(t.textAlign),
      opacity: ir(t.opacity ?? 100)
    }
  };
}
function Vd(t) {
  return {
    id: gt(10),
    type: "frame",
    x: t.x,
    y: t.y,
    w: t.width || 400,
    h: t.height || 300,
    z: 0,
    rotation: mo(t.angle),
    locked: t.locked || void 0,
    data: {
      label: t.name || void 0
    }
  };
}
function na(t) {
  return Kd(t.elements);
}
function Kd(t) {
  const e = [], o = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map();
  for (const s of t)
    s.isDeleted || s.type === "text" && s.containerId && n.set(s.containerId, s);
  for (const s of t) {
    if (s.isDeleted || s.type === "text" && s.containerId) continue;
    let i = [];
    switch (s.type) {
      case "rectangle":
        i = [hn(s, "rect")];
        break;
      case "ellipse":
        i = [hn(s, "ellipse")];
        break;
      case "diamond":
        i = [hn(s, "diamond")];
        break;
      case "arrow":
        i = qs(s, !0);
        break;
      case "line":
        i = qs(s, !1);
        break;
      case "freedraw": {
        const l = Gd(s);
        l && (i = [l]);
        break;
      }
      case "text":
        i = [jd(s)];
        break;
      case "frame":
      case "magicframe":
        i = [Vd(s)];
        break;
      case "image":
        continue;
      default:
        continue;
    }
    i.length > 0 && o.set(s.id, i[0].id), e.push(...i);
  }
  for (const [s, i] of n) {
    const l = o.get(s);
    if (!l) continue;
    const a = e.find((h) => h.id === l);
    if (!a || a.type !== "shape") continue;
    const c = a.data;
    c.label = i.originalText || i.text || "", c.labelFontSize = i.fontSize || 20, c.labelFontFamily = oa(i.fontFamily), c.labelAlign = ra(i.textAlign);
  }
  return qd(t, e, o, r), Zd(e), { nodes: e, groupParent: r };
}
function qd(t, e, o, r) {
  var s;
  const n = /* @__PURE__ */ new Map();
  for (const i of t) {
    if (i.isDeleted || !((s = i.groupIds) != null && s.length)) continue;
    for (let a = 0; a < i.groupIds.length - 1; a++) {
      const c = i.groupIds[a], h = i.groupIds[a + 1];
      n.has(c) || n.set(c, h);
    }
    const l = o.get(i.id);
    if (l) {
      const a = e.find((c) => c.id === l);
      a && (a.groupId = i.groupIds[0]);
    }
  }
  for (const [i, l] of n)
    r.set(i, l);
}
function Zd(t) {
  if (t.length === 0) return;
  let e = 1 / 0, o = 1 / 0;
  for (const r of t)
    r.x < e && (e = r.x), r.y < o && (o = r.y);
  if (isFinite(e))
    for (const r of t)
      r.x -= e, r.y -= o;
}
function es(t, e = 60) {
  if (t.length === 0)
    return `<svg width="${e}" height="${e}" xmlns="http://www.w3.org/2000/svg"/>`;
  let o = 1 / 0, r = 1 / 0, n = -1 / 0, s = -1 / 0;
  for (const p of t) {
    const d = p.h === "auto" ? 40 : p.h;
    o = Math.min(o, p.x), r = Math.min(r, p.y), n = Math.max(n, p.x + p.w), s = Math.max(s, p.y + d);
  }
  const i = n - o || 1, l = s - r || 1, a = 4, c = `${o - a} ${r - a} ${i + a * 2} ${l + a * 2}`, h = [];
  for (const p of t)
    switch (p.type) {
      case "shape":
        h.push(Ud(p));
        break;
      case "draw":
        h.push(Qd(p));
        break;
      case "text":
        h.push(Jd(p));
        break;
    }
  return `<svg width="${e}" height="${e}" viewBox="${c}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">${h.join("")}</svg>`;
}
function sa(t) {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function Ud(t) {
  var d, f, m, y;
  const e = t.data, o = t.h === "auto" ? 100 : t.h, r = {
    stroke: e.stroke,
    fill: e.fill,
    fillStyle: e.fillStyle,
    roughness: Math.min(e.roughness, 1),
    // cap roughness for speed
    strokeWidth: e.strokeWidth,
    strokeLineDash: _e(e.strokeStyle),
    seed: t.id
  }, n = ((d = e.startPoint) == null ? void 0 : d[0]) ?? 0, s = ((f = e.startPoint) == null ? void 0 : f[1]) ?? o / 2, i = ((m = e.endPoint) == null ? void 0 : m[0]) ?? t.w, l = ((y = e.endPoint) == null ? void 0 : y[1]) ?? o / 2;
  let a;
  switch (e.shape) {
    case "rect":
      a = Lr(t.x, t.y, t.w, o, r, e.edgeStyle === "round");
      break;
    case "ellipse":
      a = qn(t.x + t.w / 2, t.y + o / 2, t.w, o, r);
      break;
    case "diamond":
      a = Zn(t.x, t.y, t.w, o, r, e.edgeStyle === "round");
      break;
    case "line":
      a = Bo(t.x + n, t.y + s, t.x + i, t.y + l, r);
      break;
    case "arrow":
      a = Un(t.x + n, t.y + s, t.x + i, t.y + l, r);
      break;
    default:
      return "";
  }
  const c = e.opacity ?? 1, h = c < 1 ? `<g opacity="${c}">` : "<g>", p = a.map(
    (g) => `<path d="${sa(g.d)}" fill="${g.fill || "none"}" stroke="${g.stroke}" stroke-width="${g.strokeWidth}"${g.strokeDasharray ? ` stroke-dasharray="${g.strokeDasharray}"` : ""} stroke-linecap="round" stroke-linejoin="round"/>`
  );
  return `${h}${p.join("")}</g>`;
}
function Qd(t) {
  const e = t.data;
  if (!e.points.length) return "";
  const o = e.points.map(([s, i]) => `${(t.x + s).toFixed(1)},${(t.y + i).toFixed(1)}`).join(" "), r = e.opacity ?? 1, n = e.tool === "vector" && e.fill ? e.fill : "none";
  return `<polygon points="${o}" fill="${n}" stroke="${e.color}" stroke-width="${e.strokeWidth}" stroke-linecap="round" stroke-linejoin="round"${r < 1 ? ` opacity="${r}"` : ""}/>`;
}
function Jd(t) {
  const e = t.data, o = Math.max(e.fontSize, 8), r = e.opacity ?? 1, n = e.text.split(`
`)[0] || "";
  return `<text x="${t.x}" y="${t.y + o}" fill="${e.color}" font-size="${o}" font-family="sans-serif"${r < 1 ? ` opacity="${r}"` : ""}>${sa(n)}</text>`;
}
const ia = "sb-personal-library";
function os() {
  try {
    const t = localStorage.getItem(ia);
    return t ? JSON.parse(t) : [];
  } catch {
    return [];
  }
}
function aa(t) {
  localStorage.setItem(ia, JSON.stringify(t));
}
function la() {
  return os();
}
function $d(t, e, o) {
  const r = structuredClone(e);
  if (r.length > 0) {
    let a = 1 / 0, c = 1 / 0;
    for (const h of r)
      h.x < a && (a = h.x), h.y < c && (c = h.y);
    if (isFinite(a))
      for (const h of r)
        h.x -= a, h.y -= c;
  }
  const n = new Set(
    r.map((a) => a.groupId).filter(Boolean)
  ), s = [];
  for (const [a, c] of o)
    n.has(a) && s.push([a, c]);
  const i = {
    id: gt(10),
    name: t.trim() || "Untitled",
    nodes: r,
    groupParent: s,
    createdAt: Date.now()
  }, l = os();
  return l.unshift(i), aa(l), i;
}
function _d(t) {
  const e = os().filter((o) => o.id !== t);
  aa(e);
}
function ca(t, e, o, r) {
  const { nodes: n, groupParent: s } = na(e);
  if (n.length === 0) return;
  const i = structuredClone(n), l = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map();
  for (const x of i) {
    const C = gt(10);
    l.set(x.id, C), x.id = C;
  }
  for (const x of i)
    x.groupId && (a.has(x.groupId) || a.set(x.groupId, gt(10)), x.groupId = a.get(x.groupId));
  let c = 1 / 0, h = 1 / 0, p = -1 / 0, d = -1 / 0;
  for (const x of i) {
    const C = x.h === "auto" ? 100 : x.h;
    c = Math.min(c, x.x), h = Math.min(h, x.y), p = Math.max(p, x.x + x.w), d = Math.max(d, x.y + C);
  }
  const f = o ?? window.innerWidth / 2, m = r ?? window.innerHeight / 2, y = t.screenToCanvas(f, m), g = y.x - (c + p) / 2, b = y.y - (h + d) / 2;
  for (const x of i)
    x.x += g, x.y += b, x.z = t.nextZ();
  t.addNodes(i);
  for (const [x, C] of s) {
    const I = a.get(x) ?? x, M = a.get(C) ?? C;
    t.groupParent.set(I, M);
  }
  t.selectMultiple(i.map((x) => x.id));
}
const Pn = "application/x-spatialboard-library-item", Rn = "application/x-spatialboard-personal-item";
function da(t, e, o, r) {
  if (e.nodes.length === 0) return;
  const n = structuredClone(e.nodes), s = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Map();
  for (const g of n) {
    const b = gt(10);
    s.set(g.id, b), g.id = b;
  }
  for (const g of n)
    g.groupId && (i.has(g.groupId) || i.set(g.groupId, gt(10)), g.groupId = i.get(g.groupId));
  for (const g of n)
    if (g.type === "edge") {
      const b = g.data;
      b.fromId && s.has(b.fromId) && (b.fromId = s.get(b.fromId)), b.toId && s.has(b.toId) && (b.toId = s.get(b.toId));
    }
  let l = 1 / 0, a = 1 / 0, c = -1 / 0, h = -1 / 0;
  for (const g of n) {
    const b = g.h === "auto" ? 100 : g.h;
    l = Math.min(l, g.x), a = Math.min(a, g.y), c = Math.max(c, g.x + g.w), h = Math.max(h, g.y + b);
  }
  const p = o ?? window.innerWidth / 2, d = r ?? window.innerHeight / 2, f = t.screenToCanvas(p, d), m = f.x - (l + c) / 2, y = f.y - (a + h) / 2;
  for (const g of n)
    g.x += m, g.y += y, g.z = t.nextZ();
  t.addNodes(n);
  for (const [g, b] of e.groupParent) {
    const x = i.get(g) ?? g, C = i.get(b) ?? b;
    t.groupParent.set(x, C);
  }
  t.selectMultiple(n.map((g) => g.id));
}
const Oo = /* @__PURE__ */ new Map();
function th({ item: t }) {
  const e = Kt(() => {
    const o = Oo.get(t.id);
    if (o) return o;
    const { nodes: r } = na(t), n = es(r, 56);
    return Oo.set(t.id, n), n;
  }, [t.id]);
  return /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: e } });
}
function ha({
  item: t,
  libId: e,
  onClick: o,
  theme: r
}) {
  const n = it(
    (s) => {
      s.dataTransfer.setData(
        Pn,
        JSON.stringify({ libraryId: e, itemId: t.id })
      ), s.dataTransfer.effectAllowed = "copy";
    },
    [e, t.id]
  );
  return /* @__PURE__ */ u(
    "button",
    {
      title: t.name || "Untitled",
      onClick: o,
      draggable: !0,
      onDragStart: n,
      style: {
        border: `1px solid ${r.border}`,
        borderRadius: 4,
        background: r.controlBg,
        cursor: "grab",
        padding: 2,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        aspectRatio: "1"
      },
      children: /* @__PURE__ */ u(th, { item: t })
    }
  );
}
function eh({ nodes: t }) {
  const e = Kt(() => {
    const o = "personal-" + t.map((s) => s.id).join(","), r = Oo.get(o);
    if (r) return r;
    const n = es(t, 56);
    return Oo.set(o, n), n;
  }, [t]);
  return /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: e } });
}
function ua({
  item: t,
  onClick: e,
  onRemove: o,
  theme: r
}) {
  const [n, s] = tt(!1), i = it(
    (l) => {
      l.dataTransfer.setData(
        Rn,
        JSON.stringify({ itemId: t.id })
      ), l.dataTransfer.effectAllowed = "copy";
    },
    [t.id]
  );
  return /* @__PURE__ */ k(
    "div",
    {
      style: { position: "relative", aspectRatio: "1" },
      onMouseEnter: () => s(!0),
      onMouseLeave: () => s(!1),
      children: [
        /* @__PURE__ */ u(
          "button",
          {
            title: t.name,
            onClick: e,
            draggable: !0,
            onDragStart: i,
            style: {
              border: `1px solid ${r.border}`,
              borderRadius: 4,
              background: r.controlBg,
              cursor: "grab",
              padding: 2,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              width: "100%",
              height: "100%"
            },
            children: /* @__PURE__ */ u(eh, { nodes: t.nodes })
          }
        ),
        n && /* @__PURE__ */ u(
          "button",
          {
            title: "Remove from Personal Library",
            onClick: (l) => {
              l.stopPropagation(), o();
            },
            style: {
              position: "absolute",
              top: -4,
              right: -4,
              width: 16,
              height: 16,
              borderRadius: "50%",
              border: "none",
              background: "#ef4444",
              color: "#fff",
              fontSize: 10,
              lineHeight: "16px",
              textAlign: "center",
              cursor: "pointer",
              padding: 0,
              display: "flex",
              alignItems: "center",
              justifyContent: "center"
            },
            children: "×"
          }
        )
      ]
    }
  );
}
function oh({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: r,
  onBrowseDirectory: n
}) {
  const s = qt(), i = lt(null), l = lt(null), [a, c] = tt([]), [h, p] = tt([]), [d, f] = tt(""), [m, y] = tt(/* @__PURE__ */ new Set()), g = it(() => {
    c(Ji()), p(la());
  }, []);
  mt(() => {
    e && g();
  }, [e, g]), mt(() => {
    if (!e) return;
    const P = (G) => {
      i.current && !i.current.contains(G.target) && o();
    };
    return document.addEventListener("pointerdown", P), () => document.removeEventListener("pointerdown", P);
  }, [e, o]);
  const b = it(
    (P) => {
      var at;
      const G = (at = P.target.files) == null ? void 0 : at[0];
      if (!G) return;
      const K = new FileReader();
      K.onload = () => {
        try {
          const B = JSON.parse(K.result);
          if (B.type !== "excalidrawlib") {
            console.warn("Not an Excalidraw library file");
            return;
          }
          const et = G.name.replace(/\.excalidrawlib$/, "");
          ts(B, { name: et }), g();
        } catch (B) {
          console.error("Failed to parse library file:", B);
        }
      }, K.readAsText(G), P.target.value = "";
    },
    [g]
  ), x = it(
    (P) => {
      Nd(P), Oo.clear(), g();
    },
    [g]
  ), C = it(
    (P) => {
      ca(t, P);
    },
    [t]
  ), I = it(
    (P) => {
      da(t, P);
    },
    [t]
  ), M = it(
    (P) => {
      _d(P), Oo.clear(), g();
    },
    [g]
  ), D = it((P) => {
    y((G) => {
      const K = new Set(G);
      return K.has(P) ? K.delete(P) : K.add(P), K;
    });
  }, []), T = Kt(() => {
    if (!d.trim()) return null;
    const P = d.toLowerCase(), G = Hd(d), K = h.filter(
      (at) => at.name.toLowerCase().includes(P)
    );
    return { excalidraw: G, personal: K };
  }, [d, h]);
  return !e || !r ? null : Ne(
    /* @__PURE__ */ k(
      "div",
      {
        ref: i,
        style: {
          position: "fixed",
          left: r.right + 8,
          top: r.top,
          background: s.panelBg,
          border: `1px solid ${s.border}`,
          borderRadius: s.panelBorderRadius,
          padding: 0,
          zIndex: 99999,
          boxShadow: s.panelShadow,
          width: 280,
          maxHeight: `calc(100vh - ${r.top + 20}px)`,
          display: "flex",
          flexDirection: "column",
          overflow: "hidden"
        },
        onPointerDown: (P) => P.stopPropagation(),
        children: [
          /* @__PURE__ */ k("div", { style: { padding: "10px 12px 6px", flexShrink: 0 }, children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 11,
                  fontWeight: 600,
                  color: s.text,
                  marginBottom: 8
                },
                children: "Libraries"
              }
            ),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                placeholder: "Search library...",
                value: d,
                onChange: (P) => f(P.target.value),
                style: {
                  width: "100%",
                  padding: "5px 8px",
                  border: `1px solid ${s.border}`,
                  borderRadius: s.controlBorderRadius,
                  background: s.controlBg,
                  color: s.text,
                  fontSize: 11,
                  outline: "none",
                  boxSizing: "border-box"
                }
              }
            )
          ] }),
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                flex: 1,
                overflowY: "auto",
                overflowX: "hidden",
                padding: "4px 12px"
              },
              children: T !== null ? T.excalidraw.length === 0 && T.personal.length === 0 ? /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    color: s.textDisabled,
                    fontSize: 11,
                    textAlign: "center",
                    padding: 20
                  },
                  children: "No matching items"
                }
              ) : /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    display: "grid",
                    gridTemplateColumns: "repeat(4, 1fr)",
                    gap: 4
                  },
                  children: [
                    T.personal.map((P) => /* @__PURE__ */ u(
                      ua,
                      {
                        item: P,
                        onClick: () => I(P),
                        onRemove: () => M(P.id),
                        theme: s
                      },
                      P.id
                    )),
                    T.excalidraw.map(({ library: P, item: G }) => /* @__PURE__ */ u(
                      ha,
                      {
                        item: G,
                        libId: P.id,
                        onClick: () => C(G),
                        theme: s
                      },
                      G.id
                    ))
                  ]
                }
              ) : /* @__PURE__ */ k(ut, { children: [
                h.length > 0 && /* @__PURE__ */ u(
                  nh,
                  {
                    items: h,
                    onPlace: I,
                    onRemove: M,
                    theme: s
                  }
                ),
                a.length === 0 && h.length === 0 ? /* @__PURE__ */ k(
                  "div",
                  {
                    style: {
                      color: s.textDisabled,
                      fontSize: 11,
                      textAlign: "center",
                      padding: "20px 10px"
                    },
                    children: [
                      "No libraries installed.",
                      /* @__PURE__ */ u("br", {}),
                      "Import an .excalidrawlib file",
                      /* @__PURE__ */ u("br", {}),
                      "or browse the community directory."
                    ]
                  }
                ) : a.map((P) => {
                  const G = m.has(P.id);
                  return /* @__PURE__ */ u(
                    rh,
                    {
                      lib: P,
                      expanded: G,
                      onToggle: () => D(P.id),
                      onPlace: C,
                      onUninstall: () => x(P.id),
                      theme: s
                    },
                    P.id
                  );
                })
              ] })
            }
          ),
          /* @__PURE__ */ k(
            "div",
            {
              style: {
                borderTop: `1px solid ${s.border}`,
                padding: "8px 12px",
                display: "flex",
                gap: 6,
                flexShrink: 0
              },
              children: [
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: () => {
                      var P;
                      return (P = l.current) == null ? void 0 : P.click();
                    },
                    style: {
                      flex: 1,
                      padding: "5px 8px",
                      border: `1px solid ${s.border}`,
                      borderRadius: s.controlBorderRadius,
                      background: s.controlBg,
                      color: s.text,
                      cursor: "pointer",
                      fontSize: 10,
                      fontWeight: 500
                    },
                    children: "Import file"
                  }
                ),
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: n,
                    style: {
                      flex: 1,
                      padding: "5px 8px",
                      border: "none",
                      borderRadius: s.controlBorderRadius,
                      background: s.accentColor,
                      color: "#fff",
                      cursor: "pointer",
                      fontSize: 10,
                      fontWeight: 500
                    },
                    children: "Browse libraries"
                  }
                )
              ]
            }
          ),
          /* @__PURE__ */ u(
            "input",
            {
              ref: l,
              type: "file",
              accept: ".excalidrawlib,.json",
              style: { display: "none" },
              onChange: b
            }
          )
        ]
      }
    ),
    document.body
  );
}
function rh({
  lib: t,
  expanded: e,
  onToggle: o,
  onPlace: r,
  onUninstall: n,
  theme: s
}) {
  const [i, l] = tt(null);
  return mt(() => {
    e && i === null && l(_n(t.id));
  }, [e, i, t.id]), /* @__PURE__ */ k("div", { style: { marginBottom: 4 }, children: [
    /* @__PURE__ */ k(
      "div",
      {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
          padding: "4px 0",
          cursor: "pointer",
          userSelect: "none"
        },
        onClick: o,
        children: [
          /* @__PURE__ */ u(
            "svg",
            {
              width: 12,
              height: 12,
              viewBox: "0 0 12 12",
              fill: "none",
              style: {
                transform: e ? "rotate(90deg)" : "rotate(0deg)",
                transition: "transform 0.15s"
              },
              children: /* @__PURE__ */ u(
                "path",
                {
                  d: "M4 2l4 4-4 4",
                  stroke: s.textMuted,
                  strokeWidth: 1.5,
                  strokeLinecap: "round"
                }
              )
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flex: 1,
                fontSize: 10,
                fontWeight: 600,
                color: s.textMuted,
                textTransform: "uppercase",
                letterSpacing: "0.03em",
                overflow: "hidden",
                textOverflow: "ellipsis",
                whiteSpace: "nowrap"
              },
              children: t.name
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                fontSize: 9,
                color: s.textDisabled
              },
              children: t.itemCount
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Uninstall library",
              onClick: (a) => {
                a.stopPropagation(), n();
              },
              style: {
                border: "none",
                background: "transparent",
                color: s.textDisabled,
                cursor: "pointer",
                padding: "2px 4px",
                fontSize: 12,
                lineHeight: 1
              },
              children: "×"
            }
          )
        ]
      }
    ),
    e && i && /* @__PURE__ */ u(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: "repeat(4, 1fr)",
          gap: 4,
          padding: "2px 0 6px"
        },
        children: i.map((a) => /* @__PURE__ */ u(
          ha,
          {
            item: a,
            libId: t.id,
            onClick: () => r(a),
            theme: s
          },
          a.id
        ))
      }
    )
  ] });
}
function nh({
  items: t,
  onPlace: e,
  onRemove: o,
  theme: r
}) {
  const [n, s] = tt(!0);
  return /* @__PURE__ */ k("div", { style: { marginBottom: 4 }, children: [
    /* @__PURE__ */ k(
      "div",
      {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
          padding: "4px 0",
          cursor: "pointer",
          userSelect: "none"
        },
        onClick: () => s((i) => !i),
        children: [
          /* @__PURE__ */ u(
            "svg",
            {
              width: 12,
              height: 12,
              viewBox: "0 0 12 12",
              fill: "none",
              style: {
                transform: n ? "rotate(90deg)" : "rotate(0deg)",
                transition: "transform 0.15s"
              },
              children: /* @__PURE__ */ u(
                "path",
                {
                  d: "M4 2l4 4-4 4",
                  stroke: r.textMuted,
                  strokeWidth: 1.5,
                  strokeLinecap: "round"
                }
              )
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flex: 1,
                fontSize: 10,
                fontWeight: 600,
                color: r.textMuted,
                textTransform: "uppercase",
                letterSpacing: "0.03em"
              },
              children: "Personal"
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                fontSize: 9,
                color: r.textDisabled
              },
              children: t.length
            }
          )
        ]
      }
    ),
    n && /* @__PURE__ */ u(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: "repeat(4, 1fr)",
          gap: 4,
          padding: "2px 0 6px"
        },
        children: t.map((i) => /* @__PURE__ */ u(
          ua,
          {
            item: i,
            onClick: () => e(i),
            onRemove: () => o(i.id),
            theme: r
          },
          i.id
        ))
      }
    )
  ] });
}
async function sh(t, e, o = 1, r = 20, n) {
  const s = `${t}/search?q=${encodeURIComponent(e)}&page=${o}&per_page=${r}`;
  return (await fetch(s, { signal: n, credentials: "include" })).json();
}
async function Zs(t, e = 1, o = 20, r) {
  const n = `${t}/trending?page=${e}&per_page=${o}`;
  return (await fetch(n, { signal: r, credentials: "include" })).json();
}
const An = "application/x-spatialboard-gif-item";
function pa(t, e, o, r) {
  const n = e.file.hd.gif, s = 400, i = 300;
  let l = n.width, a = n.height;
  const c = Math.min(1, s / l, i / a);
  l = Math.round(l * c), a = Math.round(a * c);
  const h = o ?? window.innerWidth / 2, p = r ?? window.innerHeight / 2, d = t.screenToCanvas(h, p), f = {
    id: gt(10),
    type: "image",
    x: d.x - l / 2,
    y: d.y - a / 2,
    w: l,
    h: a,
    z: t.nextZ(),
    data: { src: n.url }
  };
  t.addNode(f), t.select(f.id);
}
function ih({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: r,
  baseUrl: n
}) {
  const s = qt(), i = lt(null), l = lt(null), [a, c] = tt(""), [h, p] = tt([]), [d, f] = tt(!1), [m, y] = tt(1), [g, b] = tt(!1), x = lt();
  mt(() => {
    if (!e) return;
    const T = (P) => {
      i.current && !i.current.contains(P.target) && o();
    };
    return document.addEventListener("pointerdown", T), () => document.removeEventListener("pointerdown", T);
  }, [e, o]), mt(() => {
    if (!e || a.trim()) return;
    const T = new AbortController();
    return f(!0), Zs(n, 1, 30, T.signal).then((P) => {
      p(P.data.data.filter((G) => G.type !== "ad")), y(1), b(P.data.has_next);
    }).catch(() => {
    }).finally(() => f(!1)), () => T.abort();
  }, [e, n, a]);
  const C = it(
    (T, P, G) => {
      if (!T.trim()) return;
      const K = new AbortController();
      return f(!0), sh(n, T, P, 30, K.signal).then((at) => {
        const B = at.data.data.filter((et) => et.type !== "ad");
        p((et) => G ? [...et, ...B] : B), y(P), b(at.data.has_next);
      }).catch(() => {
      }).finally(() => f(!1)), K;
    },
    [n]
  ), I = it(
    (T) => {
      if (c(T), x.current && clearTimeout(x.current), !T.trim()) {
        p([]), y(1), b(!1);
        return;
      }
      x.current = setTimeout(() => {
        C(T, 1, !1);
      }, 350);
    },
    [C]
  ), M = it(() => {
    const T = l.current;
    !T || d || !g || T.scrollTop + T.clientHeight >= T.scrollHeight - 100 && (a.trim() ? C(a, m + 1, !0) : (f(!0), Zs(n, m + 1, 30).then((P) => {
      const G = P.data.data.filter((K) => K.type !== "ad");
      p((K) => [...K, ...G]), y(m + 1), b(P.data.has_next);
    }).catch(() => {
    }).finally(() => f(!1))));
  }, [d, g, a, m, C, n]), D = it(
    (T) => {
      pa(t, T);
    },
    [t]
  );
  return !e || !r ? null : Ne(
    /* @__PURE__ */ k(
      "div",
      {
        ref: i,
        style: {
          position: "fixed",
          left: r.right + 8,
          top: r.top,
          background: s.panelBg,
          border: `1px solid ${s.border}`,
          borderRadius: s.panelBorderRadius,
          padding: 0,
          zIndex: 99999,
          boxShadow: s.panelShadow,
          width: 300,
          maxHeight: `calc(100vh - ${r.top + 20}px)`,
          display: "flex",
          flexDirection: "column",
          overflow: "hidden"
        },
        onPointerDown: (T) => T.stopPropagation(),
        children: [
          /* @__PURE__ */ k("div", { style: { padding: "10px 12px 6px", flexShrink: 0 }, children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 11,
                  fontWeight: 600,
                  color: s.text,
                  marginBottom: 8
                },
                children: "GIFs"
              }
            ),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                placeholder: "Search KLIPY",
                value: a,
                onChange: (T) => I(T.target.value),
                style: {
                  width: "100%",
                  padding: "5px 8px",
                  border: `1px solid ${s.border}`,
                  borderRadius: s.controlBorderRadius,
                  background: s.controlBg,
                  color: s.text,
                  fontSize: 11,
                  outline: "none",
                  boxSizing: "border-box"
                }
              }
            )
          ] }),
          /* @__PURE__ */ k(
            "div",
            {
              ref: l,
              onScroll: M,
              style: {
                flex: 1,
                overflowY: "auto",
                overflowX: "hidden",
                padding: "4px 12px",
                minHeight: 200
              },
              children: [
                h.length === 0 && !d ? /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      color: s.textDisabled,
                      fontSize: 11,
                      textAlign: "center",
                      padding: 20
                    },
                    children: a.trim() ? "No results" : "Loading..."
                  }
                ) : /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      display: "grid",
                      gridTemplateColumns: "repeat(3, 1fr)",
                      gap: 4
                    },
                    children: h.map((T) => /* @__PURE__ */ u(
                      ah,
                      {
                        item: T,
                        onClick: () => D(T),
                        engine: t,
                        theme: s
                      },
                      T.id
                    ))
                  }
                ),
                d && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      color: s.textMuted,
                      fontSize: 10,
                      textAlign: "center",
                      padding: 12
                    },
                    children: "Loading..."
                  }
                )
              ]
            }
          ),
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                borderTop: `1px solid ${s.border}`,
                padding: "6px 12px",
                fontSize: 9,
                color: s.textMuted,
                textAlign: "center",
                flexShrink: 0
              },
              children: "Powered by KLIPY"
            }
          )
        ]
      }
    ),
    document.body
  );
}
function ah({
  item: t,
  onClick: e,
  engine: o,
  theme: r
}) {
  const n = t.file.sm.webp, s = n.width / n.height, i = it(
    (l) => {
      l.dataTransfer.setData(An, JSON.stringify(t)), l.dataTransfer.effectAllowed = "copy";
    },
    [t]
  );
  return /* @__PURE__ */ u(
    "button",
    {
      title: t.title,
      onClick: e,
      draggable: !0,
      onDragStart: i,
      style: {
        border: `1px solid ${r.border}`,
        borderRadius: r.controlBorderRadius,
        background: r.controlBg,
        cursor: "grab",
        padding: 0,
        overflow: "hidden",
        aspectRatio: s > 1.5 ? "16/9" : s < 0.7 ? "3/4" : "1"
      },
      children: /* @__PURE__ */ u(
        "img",
        {
          src: n.url,
          alt: t.title,
          loading: "lazy",
          style: {
            width: "100%",
            height: "100%",
            objectFit: "cover",
            display: "block"
          }
        }
      )
    }
  );
}
function lh({
  nodes: t,
  onSave: e,
  onCancel: o
}) {
  const [r, n] = tt(""), s = lt(null), i = lt(null);
  mt(() => {
    var p;
    (p = s.current) == null || p.focus();
  }, []);
  const l = Kt(() => es(t, 56), [t]), a = it(() => {
    e(r.trim() || "Untitled");
  }, [r, e]), c = it(
    (p) => {
      p.key === "Enter" ? (p.preventDefault(), a()) : p.key === "Escape" && (p.preventDefault(), o());
    },
    [a, o]
  ), h = it(
    (p) => {
      i.current && !i.current.contains(p.target) && o();
    },
    [o]
  );
  return Ne(
    /* @__PURE__ */ u(
      "div",
      {
        onClick: h,
        style: {
          position: "fixed",
          inset: 0,
          zIndex: 99999,
          background: "rgba(0,0,0,0.4)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center"
        },
        children: /* @__PURE__ */ k(
          "div",
          {
            ref: i,
            onPointerDown: (p) => p.stopPropagation(),
            style: {
              background: "#1e1e2e",
              borderRadius: 8,
              border: "1px solid #333",
              boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
              padding: 16,
              width: 280,
              color: "#e0e0e0",
              fontSize: 13,
              fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
            },
            children: [
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    fontSize: 13,
                    fontWeight: 600,
                    marginBottom: 12
                  },
                  children: "Add to Personal Library"
                }
              ),
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    display: "flex",
                    justifyContent: "center",
                    marginBottom: 12,
                    padding: 8,
                    background: "rgba(255,255,255,0.05)",
                    borderRadius: 6,
                    border: "1px solid #333"
                  },
                  children: /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: l } })
                }
              ),
              /* @__PURE__ */ u(
                "input",
                {
                  ref: s,
                  type: "text",
                  value: r,
                  onChange: (p) => n(p.target.value),
                  onKeyDown: c,
                  placeholder: "Item name",
                  style: {
                    width: "100%",
                    padding: "6px 8px",
                    border: "1px solid #333",
                    borderRadius: 4,
                    background: "rgba(255,255,255,0.06)",
                    color: "#e0e0e0",
                    fontSize: 13,
                    outline: "none",
                    boxSizing: "border-box",
                    marginBottom: 12
                  }
                }
              ),
              /* @__PURE__ */ k("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end" }, children: [
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: o,
                    style: {
                      padding: "5px 12px",
                      border: "1px solid #333",
                      borderRadius: 4,
                      background: "transparent",
                      color: "#e0e0e0",
                      cursor: "pointer",
                      fontSize: 12
                    },
                    children: "Cancel"
                  }
                ),
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: a,
                    style: {
                      padding: "5px 12px",
                      border: "none",
                      borderRadius: 4,
                      background: "#6366f1",
                      color: "#fff",
                      cursor: "pointer",
                      fontSize: 12,
                      fontWeight: 500
                    },
                    children: "Save"
                  }
                )
              ] })
            ]
          }
        )
      }
    ),
    document.body
  );
}
function Ln(t) {
  const e = t.trim();
  if (!e.includes("<svg")) return null;
  const o = e.match(/<svg[\s\S]*?<\/svg>/i);
  return o ? o[0] : null;
}
function ch(t) {
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(t)}`;
}
function fa(t, e, o, r) {
  return new Promise((n) => {
    const s = ch(t), i = new Image();
    i.onload = () => {
      let c = i.naturalWidth || 200, h = i.naturalHeight || 200;
      if (c <= 1 || h <= 1) {
        const p = t.match(/viewBox=["']([^"']+)["']/i);
        if (p) {
          const d = p[1].trim().split(/[\s,]+/).map(Number);
          d.length === 4 && d[2] > 0 && d[3] > 0 && (c = d[2], h = d[3]);
        }
      }
      if (c > 400 || h > 400) {
        const p = Math.min(400 / c, 400 / h);
        c = Math.round(c * p), h = Math.round(h * p);
      }
      n({
        id: gt(10),
        type: "image",
        x: e,
        y: o,
        w: c,
        h,
        z: r,
        data: { src: s }
      });
    }, i.onerror = () => n(null), i.src = s;
  });
}
async function dh(t, e, o, r) {
  const { x: n, y: s } = t.screenToCanvas(o, r), i = await fa(e, n, s, t.nextZ());
  i && (t.addNode(i), t.select(i.id));
}
const Us = {
  number: "#3b82f6",
  // blue
  string: "#10b981",
  // green
  boolean: "#f59e0b",
  // amber
  object: "#8b5cf6",
  // purple
  any: "#6b7280",
  // gray
  signal: "#ef4444"
  // red
}, hh = de(function({
  node: e,
  zoom: o,
  showHandles: r = !0,
  measuredHeights: n,
  onHandlePointerDown: s,
  onRotateStart: i
}) {
  const l = e.h === "auto" ? (n == null ? void 0 : n[e.id]) ?? 100 : e.h, a = e.rotation || 0, c = e.x + e.w / 2, h = e.y + l / 2, p = 8 / o, d = p / 2, f = 25 / o, m = !!e.locked, y = [
    { pos: "nw", cx: e.x, cy: e.y },
    { pos: "n", cx: e.x + e.w / 2, cy: e.y },
    { pos: "ne", cx: e.x + e.w, cy: e.y },
    { pos: "e", cx: e.x + e.w, cy: e.y + l / 2 },
    { pos: "se", cx: e.x + e.w, cy: e.y + l },
    { pos: "s", cx: e.x + e.w / 2, cy: e.y + l },
    { pos: "sw", cx: e.x, cy: e.y + l },
    { pos: "w", cx: e.x, cy: e.y + l / 2 }
  ];
  return /* @__PURE__ */ k("g", { transform: `rotate(${a}, ${c}, ${h})`, children: [
    /* @__PURE__ */ u(
      "rect",
      {
        x: e.x,
        y: e.y,
        width: e.w,
        height: l,
        fill: "none",
        stroke: m ? "#f59e0b" : "#3b82f6",
        strokeWidth: 1.5 / o,
        strokeDasharray: `${4 / o} ${3 / o}`
      }
    ),
    m && (() => {
      const g = 16 / o, b = e.x + e.w - g - 4 / o, x = e.y - g - 4 / o;
      return /* @__PURE__ */ k("g", { transform: `translate(${b}, ${x})`, children: [
        /* @__PURE__ */ u(
          "rect",
          {
            x: 0,
            y: 0,
            width: g,
            height: g,
            rx: 3 / o,
            fill: "#f59e0b"
          }
        ),
        /* @__PURE__ */ k("g", { transform: `scale(${g / 24})`, children: [
          /* @__PURE__ */ u("rect", { x: "6", y: "11", width: "12", height: "9", rx: "1", fill: "white" }),
          /* @__PURE__ */ u("path", { d: "M8 11V7a4 4 0 0 1 8 0v4", fill: "none", stroke: "white", strokeWidth: "2", strokeLinecap: "round" })
        ] })
      ] });
    })(),
    r && !m && y.map(({ pos: g, cx: b, cy: x }) => /* @__PURE__ */ u(
      "rect",
      {
        x: b - d,
        y: x - d,
        width: p,
        height: p,
        fill: "white",
        stroke: "#3b82f6",
        strokeWidth: 1.5 / o,
        style: {
          cursor: Br(g, a),
          pointerEvents: "auto"
        },
        onPointerDown: (C) => {
          C.stopPropagation(), s == null || s(e.id, g, C);
        }
      },
      g
    )),
    r && !m && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u(
        "line",
        {
          x1: e.x + e.w / 2,
          y1: e.y,
          x2: e.x + e.w / 2,
          y2: e.y - f,
          stroke: "#3b82f6",
          strokeWidth: 1.5 / o
        }
      ),
      /* @__PURE__ */ u(
        "rect",
        {
          x: e.x + e.w / 2 - d,
          y: e.y - f - d,
          width: p,
          height: p,
          rx: 1.5 / o,
          transform: `rotate(45, ${e.x + e.w / 2}, ${e.y - f})`,
          fill: "white",
          stroke: "#3b82f6",
          strokeWidth: 1.5 / o,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (g) => {
            g.stopPropagation(), i == null || i(e.id, g);
          }
        }
      )
    ] })
  ] });
}), uh = de(function({
  edge: e,
  fromNode: o,
  toNode: r,
  viewport: n,
  selection: s,
  measuredHeights: i,
  registry: l,
  onEdgeEndpointDown: a,
  onKinkHandleDown: c,
  edgeReconnect: h,
  eraserMarkedIds: p,
  cycleNodeIds: d
}) {
  const f = e.data.edgeType || "bezier";
  let m, y;
  if (l && e.data.sourcePort) {
    const ht = l.get(o.type);
    ht != null && ht.ports && (m = or(o, ht.ports, e.data.sourcePort, n.zoom, i) ?? void 0);
  }
  if (l && e.data.targetPort) {
    const ht = l.get(r.type);
    ht != null && ht.ports && (y = or(r, ht.ports, e.data.targetPort, n.zoom, i) ?? void 0);
  }
  const g = Ke(
    o,
    r,
    f,
    i,
    e.data.sourceHandle,
    e.data.targetHandle,
    e.data.midpointOffset,
    e.data.curveOffset,
    m,
    y
  ), { path: b, x1: x, y1: C, x2: I, y2: M, labelX: D, labelY: T, arrowAngle: P, tailAngle: G, kinkHandle: K } = g, at = s.has(e.id), B = e.data.strokeWidth, et = e.data.style === "dashed" ? `${8 * B},${4 * B}` : e.data.style === "dotted" ? `${2 * B},${3 * B}` : void 0, Z = Math.max(8, B * 3), Q = e.data.arrowHeadSize ?? Z, L = e.data.arrowTailSize ?? Z, X = e.data.animated, _ = p == null ? void 0 : p.has(e.id), j = (h == null ? void 0 : h.edgeId) === e.id, q = !!(d && d.size > 0 && e.data.sourcePort && e.data.targetPort && d.has(e.data.fromId) && d.has(e.data.toId)), Y = q ? "#ef4444" : e.data.color, $ = e.data.roughness ?? 0, nt = Kt(() => $ <= 0 ? null : {
    stroke: Y,
    roughness: $,
    strokeWidth: B,
    strokeLineDash: e.data.style === "dashed" ? [8, 4] : e.data.style === "dotted" ? [2, 2] : void 0,
    seed: e.id
  }, [Y, $, B, e.data.style, e.id]);
  let ot = null, ft = null, pt = null;
  nt && (ot = cn(b, nt), e.data.arrowHead === "arrow" && (ft = cn(Eo(I, M, P, Q), { ...nt, strokeLineDash: void 0 })), e.data.arrowTail === "arrow" && (pt = cn(Eo(x, C, G, L), { ...nt, strokeLineDash: void 0 })));
  const rt = Kt(
    () => ({ animation: "edge-cycle-pulse 1.5s ease-in-out infinite" }),
    []
  ), It = Kt(() => {
    if (!X) return;
    const ht = e.data.animatedDirection === "reverse" ? "edge-flow-reverse" : e.data.animatedDirection === "both" ? "edge-flow-both" : "edge-flow", Wt = e.data.animatedDirection === "both" ? "2s" : "1s";
    return { animation: `${ht} ${Wt} linear infinite` };
  }, [X, e.data.animatedDirection]), zt = Kt(
    () => ({ animation: "edge-cycle-pulse 1.5s ease-in-out infinite, edge-flow 1s linear infinite" }),
    []
  ), bt = Kt(
    () => _ ? { filter: "saturate(0)" } : void 0,
    [_]
  );
  return /* @__PURE__ */ k("g", { opacity: j ? 0.15 : _ ? 0.25 : void 0, style: bt, children: [
    q && /* @__PURE__ */ u(
      "path",
      {
        d: b,
        stroke: "#ef4444",
        strokeWidth: B + 6 / n.zoom,
        strokeLinecap: "round",
        fill: "none",
        opacity: 0.25,
        style: rt
      }
    ),
    at && /* @__PURE__ */ u(
      "path",
      {
        d: b,
        stroke: "#3b82f6",
        strokeWidth: B + 6 / n.zoom,
        strokeLinecap: "round",
        fill: "none",
        opacity: 0.3
      }
    ),
    ot ? ot.map((ht, Wt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeDasharray: ht.strokeDasharray,
        strokeLinecap: "round",
        fill: ht.fill ?? "none",
        style: X ? It : void 0
      },
      Wt
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: b,
        stroke: Y,
        strokeWidth: B,
        strokeDasharray: X ? "12,8" : q ? `${6 * B},${4 * B}` : et,
        strokeLinecap: "round",
        fill: "none",
        style: q ? zt : It
      }
    ),
    e.data.arrowHead === "arrow" && (ft ? ft.map((ht, Wt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeLinecap: "round",
        strokeLinejoin: "round",
        fill: ht.fill ?? "none"
      },
      `ah${Wt}`
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: Eo(I, M, P, Q),
        fill: "none",
        stroke: Y,
        strokeWidth: B,
        strokeLinecap: "round",
        strokeLinejoin: "round"
      }
    )),
    e.data.arrowHead === "filled" && /* @__PURE__ */ u(
      "path",
      {
        d: Ir(I, M, P, Q),
        fill: Y,
        stroke: "none"
      }
    ),
    e.data.arrowHead === "dot" && /* @__PURE__ */ u(
      "circle",
      {
        cx: I,
        cy: M,
        r: Q * 0.25,
        fill: Y
      }
    ),
    e.data.arrowTail === "arrow" && (pt ? pt.map((ht, Wt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeLinecap: "round",
        strokeLinejoin: "round",
        fill: ht.fill ?? "none"
      },
      `at${Wt}`
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: Eo(x, C, G, L),
        fill: "none",
        stroke: Y,
        strokeWidth: B,
        strokeLinecap: "round",
        strokeLinejoin: "round"
      }
    )),
    e.data.arrowTail === "filled" && /* @__PURE__ */ u(
      "path",
      {
        d: Ir(x, C, G, L),
        fill: Y,
        stroke: "none"
      }
    ),
    e.data.arrowTail === "dot" && /* @__PURE__ */ u(
      "circle",
      {
        cx: x,
        cy: C,
        r: L * 0.25,
        fill: Y
      }
    ),
    e.data.label && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u(
        "rect",
        {
          x: D - (e.data.label.length * 3.5 + 6) / n.zoom,
          y: T - 8 / n.zoom,
          width: (e.data.label.length * 7 + 12) / n.zoom,
          height: 16 / n.zoom,
          fill: "white",
          rx: 4 / n.zoom,
          opacity: 0.9
        }
      ),
      /* @__PURE__ */ u(
        "text",
        {
          x: D,
          y: T + 4 / n.zoom,
          fill: Y,
          fontSize: 12 / n.zoom,
          textAnchor: "middle",
          style: { pointerEvents: "none" },
          children: e.data.label
        }
      )
    ] }),
    at && !j && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u(
        "circle",
        {
          cx: x,
          cy: C,
          r: 5 / n.zoom,
          fill: "#3b82f6",
          stroke: "white",
          strokeWidth: 1.5 / n.zoom,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (ht) => {
            ht.stopPropagation(), a == null || a(e.id, "source", ht);
          }
        }
      ),
      /* @__PURE__ */ u(
        "circle",
        {
          cx: I,
          cy: M,
          r: 5 / n.zoom,
          fill: "#3b82f6",
          stroke: "white",
          strokeWidth: 1.5 / n.zoom,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (ht) => {
            ht.stopPropagation(), a == null || a(e.id, "target", ht);
          }
        }
      )
    ] }),
    at && !j && K && /* @__PURE__ */ u(
      "circle",
      {
        cx: K.x,
        cy: K.y,
        r: 5 / n.zoom,
        fill: "white",
        stroke: "#3b82f6",
        strokeWidth: 1.5 / n.zoom,
        style: {
          cursor: K.axis === "xy" ? "move" : K.axis === "x" ? "ew-resize" : "ns-resize",
          pointerEvents: "auto"
        },
        onPointerDown: (ht) => {
          ht.stopPropagation(), c == null || c(e.id, K.axis, K.min, K.max, ht);
        }
      }
    )
  ] });
});
function ph({
  nodes: t,
  viewport: e,
  selection: o,
  measuredHeights: r,
  activeStroke: n,
  shapePreview: s,
  shapePreviewStyle: i,
  onResizeHandleDown: l,
  onRotateStart: a,
  onConnectionHandleDown: c,
  onEdgeEndpointDown: h,
  onKinkHandleDown: p,
  edgePreview: d,
  edgeReconnect: f,
  eraserMarkedIds: m,
  eraserTrail: y,
  laserTrail: g,
  mode: b,
  hoveredNodeId: x,
  registry: C,
  onPortHandleDown: I,
  cycleNodeIds: M,
  containerTypes: D,
  alignGuides: T
}) {
  const P = `translate(${e.x}, ${e.y}) scale(${e.zoom})`, G = t.filter(
    (B) => B.type !== "edge" && B.type !== "content" && B.type !== "image"
  ), K = t.filter((B) => B.type === "edge").sort((B, et) => B.z - et.z), at = Kt(() => new Map(t.map((B) => [B.id, B])), [t]);
  return /* @__PURE__ */ u(
    "svg",
    {
      "data-sb-overlay": !0,
      style: {
        position: "absolute",
        inset: 0,
        width: "100%",
        height: "100%",
        overflow: "visible",
        pointerEvents: "none"
      },
      children: /* @__PURE__ */ k("g", { transform: P, children: [
        K.map((B) => {
          const et = at.get(B.data.fromId), Z = at.get(B.data.toId);
          return !et || !Z ? null : /* @__PURE__ */ u(
            uh,
            {
              edge: B,
              fromNode: et,
              toNode: Z,
              viewport: e,
              selection: o,
              measuredHeights: r,
              registry: C,
              onEdgeEndpointDown: h,
              onKinkHandleDown: p,
              edgeReconnect: f,
              eraserMarkedIds: m,
              cycleNodeIds: M
            },
            B.id
          );
        }),
        (() => {
          var j, q;
          const B = !!d || !!f, et = (d == null ? void 0 : d.cursorX) ?? (f == null ? void 0 : f.cursorX) ?? 0, Z = (d == null ? void 0 : d.cursorY) ?? (f == null ? void 0 : f.cursorY) ?? 0, Q = (d == null ? void 0 : d.fromNode.id) ?? (f == null ? void 0 : f.anchorNodeId) ?? null;
          let L = null, X = null;
          const _ = /* @__PURE__ */ new Set();
          if (B) {
            let Y = 1 / 0, $ = !1;
            const nt = 50 / e.zoom;
            for (const ot of t) {
              if (ot.type === "edge" || ot.id === Q || (q = (j = C == null ? void 0 : C.get(ot.type)) == null ? void 0 : j.ports) != null && q.length) continue;
              const ft = ot.h === "auto" ? (r == null ? void 0 : r[ot.id]) ?? 100 : ot.h, pt = ot.w * 0.2, rt = ft * 0.2;
              et >= ot.x - pt && et <= ot.x + ot.w + pt && Z >= ot.y - rt && Z <= ot.y + ft + rt && _.add(ot.id);
              const It = Fo(ot, r), zt = D ? D.has(ot.type) : ot.type === "frame";
              for (const bt of It) {
                const ht = Math.hypot(bt.x - et, bt.y - Z);
                ht >= nt || zt && !$ && L || (!zt && $ || ht < Y) && (Y = ht, $ = zt, L = ot.id, X = bt.side);
              }
            }
          }
          return t.filter((Y) => {
            var $, nt;
            return Y.type === "edge" || (nt = ($ = C == null ? void 0 : C.get(Y.type)) == null ? void 0 : $.ports) != null && nt.length ? !1 : o.size <= 1 && o.has(Y.id) || B && (Y.id === Q || _.has(Y.id));
          }).map((Y) => {
            const $ = Fo(Y, r), nt = 4 / e.zoom, ot = 26 / e.zoom, ft = Y.rotation || 0, pt = Y.h === "auto" ? (r == null ? void 0 : r[Y.id]) ?? 100 : Y.h, rt = Y.x + Y.w / 2, It = Y.y + pt / 2, zt = d && d.fromNode.id === Y.id || f && f.anchorNodeId === Y.id, bt = o.has(Y.id) && !B;
            return /* @__PURE__ */ u("g", { transform: ft ? `rotate(${ft}, ${rt}, ${It})` : void 0, children: $.map(({ side: ht }) => {
              const Wt = {
                top: [Y.x + Y.w / 2, Y.y],
                bottom: [Y.x + Y.w / 2, Y.y + pt],
                left: [Y.x, Y.y + pt / 2],
                right: [Y.x + Y.w, Y.y + pt / 2]
              }, [Tt, Rt] = Wt[ht], Qt = ht === "top" && o.has(Y.id) ? 42 / e.zoom : ot;
              let Jt = Tt, Zt = Rt;
              switch (ht) {
                case "top":
                  Zt = Rt - Qt;
                  break;
                case "bottom":
                  Zt = Rt + Qt;
                  break;
                case "left":
                  Jt = Tt - Qt;
                  break;
                case "right":
                  Jt = Tt + Qt;
                  break;
              }
              const $t = B && L === Y.id && X === ht;
              return /* @__PURE__ */ u(
                "circle",
                {
                  cx: Jt,
                  cy: Zt,
                  r: $t ? 5 / e.zoom : nt,
                  fill: zt || $t ? "#3b82f6" : "white",
                  stroke: $t ? "white" : B && !zt ? "#3b82f6" : "#94a3b8",
                  strokeWidth: 1.5 / e.zoom,
                  opacity: $t || B && !zt ? 1 : 0.8,
                  style: {
                    cursor: bt ? "crosshair" : "default",
                    pointerEvents: bt ? "auto" : "none"
                  },
                  onPointerDown: bt ? (he) => {
                    he.stopPropagation(), c == null || c(Y.id, ht, he);
                  } : void 0
                },
                `ch-${Y.id}-${ht}`
              );
            }) }, `conn-${Y.id}`);
          });
        })(),
        C && (() => {
          var j;
          const B = !!d || !!f, et = (d == null ? void 0 : d.cursorX) ?? (f == null ? void 0 : f.cursorX) ?? 0, Z = (d == null ? void 0 : d.cursorY) ?? (f == null ? void 0 : f.cursorY) ?? 0, Q = (d == null ? void 0 : d.fromNode.id) ?? null, L = (d == null ? void 0 : d.sourceDirection) === "output" ? "input" : (d == null ? void 0 : d.sourceDirection) === "input" ? "output" : null;
          let X = null, _ = null;
          if (B && L) {
            let q = 40 / e.zoom;
            for (const Y of t) {
              if (Y.type === "edge" || Y.id === Q) continue;
              const $ = C.get(Y.type);
              if (!((j = $ == null ? void 0 : $.ports) != null && j.length)) continue;
              const nt = Y.h === "auto" ? (r == null ? void 0 : r[Y.id]) ?? 100 : Y.h, ot = 14 / e.zoom, ft = $.ports.filter((pt) => pt.direction === L);
              for (let pt = 0; pt < ft.length; pt++) {
                const rt = ft[pt], It = Y.y + nt / (ft.length + 1) * (pt + 1), zt = rt.direction === "input" ? Y.x - ot : Y.x + Y.w + ot, bt = Math.hypot(zt - et, It - Z);
                bt < q && (q = bt, X = Y.id, _ = rt.id);
              }
            }
          }
          return t.filter((q) => {
            var $;
            if (q.type === "edge") return !1;
            const Y = C.get(q.type);
            return !!(($ = Y == null ? void 0 : Y.ports) != null && $.length);
          }).map((q) => {
            const $ = C.get(q.type).ports, nt = q.h === "auto" ? (r == null ? void 0 : r[q.id]) ?? 100 : q.h, ot = q.rotation || 0, ft = q.x + q.w / 2, pt = q.y + nt / 2, rt = 6 / e.zoom, It = 14 / e.zoom, zt = $.filter((Rt) => Rt.direction === "input"), bt = $.filter((Rt) => Rt.direction === "output"), ht = !B, Wt = (Rt, Qt, Jt, Zt) => {
              const $t = q.y + nt / (Jt.length + 1) * (Qt + 1), he = Zt === "input" ? q.x - It : q.x + q.w + It, Ae = Us[Rt.dataType] || Us.any, _t = X === q.id && _ === Rt.id, Le = _t ? 8 / e.zoom : rt, eo = Zt === "input" ? q.x : q.x + q.w, xo = Zt === "input" ? he - rt - 4 / e.zoom : he + rt + 4 / e.zoom;
              return /* @__PURE__ */ k("g", { children: [
                /* @__PURE__ */ u(
                  "line",
                  {
                    x1: he,
                    y1: $t,
                    x2: eo,
                    y2: $t,
                    stroke: Ae,
                    strokeWidth: 1.5 / e.zoom,
                    opacity: 0.4,
                    style: { pointerEvents: "none" }
                  }
                ),
                _t && /* @__PURE__ */ u(
                  "circle",
                  {
                    cx: he,
                    cy: $t,
                    r: 12 / e.zoom,
                    fill: "none",
                    stroke: "white",
                    strokeWidth: 1.5 / e.zoom,
                    opacity: 0.3,
                    style: { pointerEvents: "none" }
                  }
                ),
                /* @__PURE__ */ u(
                  "circle",
                  {
                    cx: he,
                    cy: $t,
                    r: Le,
                    fill: _t ? "white" : Ae,
                    stroke: _t ? Ae : "#1a1a2e",
                    strokeWidth: 2 / e.zoom,
                    style: {
                      cursor: ht ? "crosshair" : "default",
                      pointerEvents: ht ? "auto" : "none",
                      transition: "r 0.1s, fill 0.1s"
                    },
                    onPointerDown: ht ? (ue) => {
                      ue.stopPropagation(), I == null || I(q.id, Rt.id, Zt, ue);
                    } : void 0
                  }
                ),
                (() => {
                  const ue = Rt.label || Rt.id, Me = 9 / e.zoom, oo = 5 / e.zoom, ro = 2.5 / e.zoom, pe = ue.length * Me * 0.62 + oo * 2, Ce = Me + ro * 2, be = Zt === "input" ? xo - pe : xo, He = $t - Ce / 2, v = Ce / 2, st = _t ? Ae : "#1a1a2e", Yt = _t ? Ae : "#2a2a40", ne = _t ? "#fff" : "#94a3b8";
                  return /* @__PURE__ */ k("g", { style: { pointerEvents: "none" }, children: [
                    /* @__PURE__ */ u(
                      "rect",
                      {
                        x: be,
                        y: He,
                        width: pe,
                        height: Ce,
                        rx: v,
                        ry: v,
                        fill: st,
                        fillOpacity: _t ? 0.9 : 0.85,
                        stroke: Yt,
                        strokeWidth: 1 / e.zoom
                      }
                    ),
                    /* @__PURE__ */ u(
                      "text",
                      {
                        x: be + pe / 2,
                        y: $t + Me * 0.35,
                        fill: ne,
                        fontSize: Me,
                        fontWeight: 600,
                        fontFamily: "'Inter', system-ui, sans-serif",
                        textAnchor: "middle",
                        style: { userSelect: "none" },
                        children: ue
                      }
                    )
                  ] });
                })()
              ] }, `port-${q.id}-${Rt.id}`);
            }, Tt = M == null ? void 0 : M.has(q.id);
            return /* @__PURE__ */ k("g", { transform: ot ? `rotate(${ot}, ${ft}, ${pt})` : void 0, children: [
              zt.map((Rt, Qt) => Wt(Rt, Qt, zt, "input")),
              bt.map((Rt, Qt) => Wt(Rt, Qt, bt, "output")),
              Tt && (() => {
                const Rt = 10 / e.zoom, Qt = q.x + q.w + Rt * 0.3, Jt = q.y - Rt * 0.3;
                return /* @__PURE__ */ k("g", { style: { pointerEvents: "none" }, children: [
                  /* @__PURE__ */ u(
                    "circle",
                    {
                      cx: Qt,
                      cy: Jt,
                      r: Rt,
                      fill: "#ef4444",
                      stroke: "#1a1a2e",
                      strokeWidth: 2 / e.zoom
                    }
                  ),
                  /* @__PURE__ */ u(
                    "text",
                    {
                      x: Qt,
                      y: Jt + 4 / e.zoom,
                      fill: "white",
                      fontSize: 12 / e.zoom,
                      fontWeight: 800,
                      textAnchor: "middle",
                      style: { pointerEvents: "none", userSelect: "none" },
                      children: "!"
                    }
                  )
                ] });
              })()
            ] }, `ports-${q.id}`);
          });
        })(),
        d && (() => {
          let B, et;
          if (d.sourcePort && C) {
            const Z = d.fromNode, Q = C.get(Z.type), L = Q != null && Q.ports ? or(Z, Q.ports, d.sourcePort, e.zoom, r) : null;
            if (L)
              B = L.x, et = L.y;
            else {
              const X = _r(Z, d.cursorX, d.cursorY, r);
              B = X.x, et = X.y;
            }
          } else if (d.sourceHandle) {
            const Z = d.fromNode, Q = Z.h === "auto" ? (r == null ? void 0 : r[Z.id]) ?? 100 : Z.h, L = {
              top: [Z.x + Z.w / 2, Z.y],
              bottom: [Z.x + Z.w / 2, Z.y + Q],
              left: [Z.x, Z.y + Q / 2],
              right: [Z.x + Z.w, Z.y + Q / 2]
            }, X = d.sourceHandle, _ = X === "top" ? 42 / e.zoom : 26 / e.zoom, [j, q] = L[X];
            let Y = j, $ = q;
            switch (X) {
              case "top":
                $ = q - _;
                break;
              case "bottom":
                $ = q + _;
                break;
              case "left":
                Y = j - _;
                break;
              case "right":
                Y = j + _;
                break;
            }
            if (Z.rotation) {
              const nt = Z.x + Z.w / 2, ot = Z.y + Q / 2, ft = Z.rotation * Math.PI / 180, pt = Math.cos(ft), rt = Math.sin(ft), It = Y - nt, zt = $ - ot;
              B = nt + It * pt - zt * rt, et = ot + It * rt + zt * pt;
            } else
              B = Y, et = $;
          } else {
            const Z = _r(d.fromNode, d.cursorX, d.cursorY, r);
            B = Z.x, et = Z.y;
          }
          return /* @__PURE__ */ u(
            "line",
            {
              x1: B,
              y1: et,
              x2: d.cursorX,
              y2: d.cursorY,
              stroke: "#3b82f6",
              strokeWidth: 2 / e.zoom,
              strokeDasharray: `${4 / e.zoom}`,
              strokeLinecap: "round"
            }
          );
        })(),
        f && (() => {
          const B = at.get(f.anchorNodeId);
          if (!B) return null;
          let et, Z;
          if (f.anchorHandle) {
            const Q = B.h === "auto" ? (r == null ? void 0 : r[B.id]) ?? 100 : B.h, L = {
              top: [B.x + B.w / 2, B.y],
              bottom: [B.x + B.w / 2, B.y + Q],
              left: [B.x, B.y + Q / 2],
              right: [B.x + B.w, B.y + Q / 2]
            }, X = f.anchorHandle, _ = X === "top" ? 42 / e.zoom : 26 / e.zoom, [j, q] = L[X];
            let Y = j, $ = q;
            switch (X) {
              case "top":
                $ = q - _;
                break;
              case "bottom":
                $ = q + _;
                break;
              case "left":
                Y = j - _;
                break;
              case "right":
                Y = j + _;
                break;
            }
            if (B.rotation) {
              const nt = B.x + B.w / 2, ot = B.y + Q / 2, ft = B.rotation * Math.PI / 180, pt = Math.cos(ft), rt = Math.sin(ft), It = Y - nt, zt = $ - ot;
              et = nt + It * pt - zt * rt, Z = ot + It * rt + zt * pt;
            } else
              et = Y, Z = $;
          } else {
            const Q = _r(B, f.cursorX, f.cursorY, r);
            et = Q.x, Z = Q.y;
          }
          return /* @__PURE__ */ u(
            "line",
            {
              x1: et,
              y1: Z,
              x2: f.cursorX,
              y2: f.cursorY,
              stroke: "#3b82f6",
              strokeWidth: 2 / e.zoom,
              strokeDasharray: `${4 / e.zoom}`,
              strokeLinecap: "round"
            }
          );
        })(),
        o.size === 1 && G.filter((B) => o.has(B.id)).map((B) => /* @__PURE__ */ u(
          hh,
          {
            node: B,
            zoom: e.zoom,
            showHandles: o.size === 1,
            measuredHeights: r,
            onHandlePointerDown: l,
            onRotateStart: a
          },
          `sel-${B.id}`
        )),
        n && n.points.length > 1 && (() => {
          if (n.strokeStyle === "dashed" || n.strokeStyle === "dotted") {
            const et = n.points, Z = ["M", et[0][0], et[0][1]];
            for (let X = 1; X < et.length; X++) {
              const [_, j] = et[X], [q, Y] = et[X - 1];
              Z.push("Q", q, Y, (q + _) / 2, (Y + j) / 2);
            }
            const Q = et[et.length - 1];
            Z.push("L", Q[0], Q[1]);
            const L = _e(n.strokeStyle);
            return /* @__PURE__ */ u(
              "path",
              {
                d: Z.join(" "),
                fill: "none",
                stroke: n.color,
                strokeWidth: n.width,
                strokeDasharray: L == null ? void 0 : L.map((X) => X * Math.max(n.width, 1)).join(" "),
                strokeLinecap: "round",
                strokeLinejoin: "round"
              }
            );
          }
          return /* @__PURE__ */ u(
            "path",
            {
              d: jn(n.points, {
                size: n.width
              }),
              fill: n.color
            }
          );
        })(),
        s && i && (() => {
          const B = Math.min(s.startX, s.endX), et = Math.min(s.startY, s.endY), Z = Math.abs(s.endX - s.startX), Q = Math.abs(s.endY - s.startY);
          if (Z < 2 && Q < 2) return null;
          const L = i, X = L.shapeType || "rect";
          if (X === "ellipse")
            return /* @__PURE__ */ u(
              "ellipse",
              {
                cx: B + Z / 2,
                cy: et + Q / 2,
                rx: Z / 2,
                ry: Q / 2,
                stroke: L.stroke,
                strokeWidth: L.strokeWidth,
                fill: "none",
                strokeDasharray: "4"
              }
            );
          if (X === "diamond")
            return /* @__PURE__ */ u(
              "polygon",
              {
                points: `${B + Z / 2},${et} ${B + Z},${et + Q / 2} ${B + Z / 2},${et + Q} ${B},${et + Q / 2}`,
                stroke: L.stroke,
                strokeWidth: L.strokeWidth,
                fill: "none",
                strokeDasharray: "4"
              }
            );
          if (X === "line" || X === "arrow") {
            const _ = s.startX, j = s.startY, q = s.endX, Y = s.endY;
            return /* @__PURE__ */ k(ut, { children: [
              /* @__PURE__ */ u(
                "line",
                {
                  x1: _,
                  y1: j,
                  x2: q,
                  y2: Y,
                  stroke: L.stroke,
                  strokeWidth: L.strokeWidth,
                  strokeDasharray: "4"
                }
              ),
              X === "arrow" && (() => {
                const $ = Math.atan2(Y - j, q - _), nt = Math.max(12, L.strokeWidth * 4), ot = Math.PI / 6, ft = q - nt * Math.cos($ - ot), pt = Y - nt * Math.sin($ - ot), rt = q - nt * Math.cos($ + ot), It = Y - nt * Math.sin($ + ot);
                return /* @__PURE__ */ u(
                  "polyline",
                  {
                    points: `${ft},${pt} ${q},${Y} ${rt},${It}`,
                    stroke: L.stroke,
                    strokeWidth: L.strokeWidth,
                    fill: "none",
                    strokeDasharray: "4"
                  }
                );
              })()
            ] });
          }
          return /* @__PURE__ */ u(
            "rect",
            {
              x: B,
              y: et,
              width: Z,
              height: Q,
              stroke: L.stroke,
              strokeWidth: L.strokeWidth,
              fill: "none",
              strokeDasharray: "4"
            }
          );
        })(),
        y && y.length > 1 && (() => {
          const B = performance.now(), et = 400, Z = 6 / e.zoom, Q = [`M${y[0][0]},${y[0][1]}`];
          if (y.length === 2)
            Q.push(`L${y[1][0]},${y[1][1]}`);
          else {
            for (let nt = 0; nt < y.length - 1; nt++) {
              const ot = (y[nt][0] + y[nt + 1][0]) / 2, ft = (y[nt][1] + y[nt + 1][1]) / 2;
              Q.push(`Q${y[nt][0]},${y[nt][1]},${ot},${ft}`);
            }
            const $ = y[y.length - 1];
            Q.push(`L${$[0]},${$[1]}`);
          }
          const L = Q.join(" "), X = (B - y[y.length - 1][2]) / et, _ = (B - y[0][2]) / et, j = Math.max(0, 0.85 * (1 - X)), q = Math.max(0, 0.85 * (1 - _)), Y = (j + q) / 2;
          return Y <= 0 ? null : /* @__PURE__ */ k(ut, { children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: L,
                fill: "none",
                stroke: "#9ca3af",
                strokeWidth: Z * 3,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: Y * 0.35
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: L,
                fill: "none",
                stroke: "#d1d5db",
                strokeWidth: Z,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: Y
              }
            )
          ] });
        })(),
        g && g.length > 1 && (() => {
          const B = performance.now(), et = 1560, Z = 6 / e.zoom, Q = [];
          let L = !1, X = !1;
          for (let ft = 0; ft < g.length; ft++) {
            const pt = g[ft];
            if (isNaN(pt[0])) {
              L = !1, X = !1;
              continue;
            }
            if (!L)
              Q.push(`M${pt[0]},${pt[1]}`), L = !0, X = !0;
            else if (X) {
              const rt = ft + 1 < g.length && !isNaN(g[ft + 1][0]) ? g[ft + 1] : null;
              if (rt) {
                const It = (pt[0] + rt[0]) / 2, zt = (pt[1] + rt[1]) / 2;
                Q.push(`Q${pt[0]},${pt[1]},${It},${zt}`);
              } else
                Q.push(`L${pt[0]},${pt[1]}`);
            }
          }
          if (Q.length === 0) return null;
          const _ = Q.join(" "), j = g.filter((ft) => !isNaN(ft[0]));
          if (j.length === 0) return null;
          const q = (B - j[j.length - 1][2]) / et, Y = (B - j[0][2]) / et, $ = Math.max(0, 0.85 * (1 - q)), nt = Math.max(0, 0.85 * (1 - Y)), ot = ($ + nt) / 2;
          return ot <= 0 ? null : /* @__PURE__ */ k(ut, { children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: _,
                fill: "none",
                stroke: "#ef4444",
                strokeWidth: Z * 3,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: ot * 0.35
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: _,
                fill: "none",
                stroke: "#ff6b6b",
                strokeWidth: Z,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: ot
              }
            )
          ] });
        })(),
        T && T.length > 0 && T.map((B, et) => /* @__PURE__ */ u(
          "line",
          {
            x1: B.axis === "x" ? B.position : B.start,
            y1: B.axis === "x" ? B.start : B.position,
            x2: B.axis === "x" ? B.position : B.end,
            y2: B.axis === "x" ? B.end : B.position,
            stroke: "#f472b6",
            strokeWidth: 1 / e.zoom,
            strokeDasharray: `${3 / e.zoom} ${2 / e.zoom}`,
            opacity: 0.8
          },
          `guide-${et}`
        ))
      ] })
    }
  );
}
function fh({
  x: t,
  y: e,
  sections: o,
  onClose: r
}) {
  const n = lt(null);
  mt(() => {
    var m;
    const p = (y) => {
      n.current && !n.current.contains(y.target) && r();
    }, d = (y) => {
      y.key === "Escape" && r();
    }, f = ((m = n.current) == null ? void 0 : m.ownerDocument) ?? document;
    return f.addEventListener("pointerdown", p, !0), f.addEventListener("keydown", d), () => {
      f.removeEventListener("pointerdown", p, !0), f.removeEventListener("keydown", d);
    };
  }, [r]), mt(() => {
    const p = n.current;
    if (!p) return;
    const d = p.getBoundingClientRect(), f = p.ownerDocument.defaultView ?? window;
    let m = t, y = e;
    d.right > f.innerWidth && (m = t - d.width), d.bottom > f.innerHeight && (y = e - d.height), m = Math.max(0, m), y = Math.max(0, y), p.style.left = `${m}px`, p.style.top = `${y}px`;
  }, [t, e]);
  const s = it(
    (p) => {
      p.disabled || (p.action(), r());
    },
    [r]
  ), i = navigator.platform.includes("Mac"), l = i ? "⌘" : "Ctrl+", a = i ? "⌥" : "Alt+", c = i ? "⇧" : "Shift+", h = (p) => p.replace("Mod+", l).replace("Alt+", a).replace("Shift+", c);
  return /* @__PURE__ */ u(
    "div",
    {
      "data-sb-context-menu": !0,
      ref: n,
      onPointerDown: (p) => p.stopPropagation(),
      onContextMenu: (p) => p.preventDefault(),
      style: {
        position: "fixed",
        left: t,
        top: e,
        zIndex: 9999,
        minWidth: 200,
        background: "#1e1e2e",
        borderRadius: 8,
        border: "1px solid #333",
        boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
        padding: "4px 0",
        color: "#e0e0e0",
        fontSize: 13,
        fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
        userSelect: "none"
      },
      children: o.map((p, d) => /* @__PURE__ */ k("div", { children: [
        d > 0 && /* @__PURE__ */ u(
          "div",
          {
            style: {
              height: 1,
              background: "#333",
              margin: "4px 0"
            }
          }
        ),
        p.items.map((f, m) => /* @__PURE__ */ k(
          "div",
          {
            onClick: () => s(f),
            style: {
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              padding: "6px 16px",
              cursor: f.disabled ? "default" : "pointer",
              opacity: f.disabled ? 0.4 : 1,
              color: f.danger ? "#f87171" : "#e0e0e0",
              transition: "background 0.1s"
            },
            onMouseEnter: (y) => {
              f.disabled || (y.currentTarget.style.background = "rgba(255,255,255,0.08)");
            },
            onMouseLeave: (y) => {
              y.currentTarget.style.background = "transparent";
            },
            children: [
              /* @__PURE__ */ k("span", { children: [
                f.checked !== void 0 && /* @__PURE__ */ u("span", { style: { display: "inline-block", width: 16, marginRight: 4 }, children: f.checked ? "✓" : "" }),
                f.label
              ] }),
              f.shortcut && /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    marginLeft: 32,
                    fontSize: 12,
                    color: "#888"
                  },
                  children: h(f.shortcut)
                }
              )
            ]
          },
          m
        ))
      ] }, d))
    }
  );
}
const ya = "sbd-clipboard", yh = "sbd-nodes:";
function ma(t) {
  const e = JSON.stringify(t), o = new TextEncoder().encode(e);
  let r = "";
  for (let n = 0; n < o.length; n++) r += String.fromCharCode(o[n]);
  return btoa(r);
}
function Qs(t) {
  try {
    const e = atob(t), o = new Uint8Array(e.length);
    for (let n = 0; n < e.length; n++) o[n] = e.charCodeAt(n);
    const r = new TextDecoder().decode(o);
    return JSON.parse(r);
  } catch {
    return null;
  }
}
function ga(t) {
  const e = t.match(/data-sbd-nodes="([A-Za-z0-9+/=]+)"/);
  if (e) return Qs(e[1]);
  const o = t.match(
    new RegExp(`<!--${yh}([A-Za-z0-9+/=]+)-->`)
  );
  return o ? Qs(o[1]) : null;
}
function wr(t) {
  return !!t.closest(".bn-editor") || t.isContentEditable || t.tagName === "INPUT" || t.tagName === "TEXTAREA";
}
function ba(t) {
  return t.map((e) => {
    var n;
    const o = (e.content || []).filter((s) => s.type === "text").map((s) => s.text ?? "").join(""), r = (n = e.children) != null && n.length ? `
` + ba(e.children) : "";
    return o + r;
  }).filter(Boolean).join(`
`);
}
function mh(t) {
  var o;
  const e = [];
  for (const r of t)
    switch (r.type) {
      case "content": {
        const n = r.data;
        (o = n.blocks) != null && o.length ? e.push(ba(n.blocks)) : n.markdown && e.push(n.markdown);
        break;
      }
      case "image": {
        const n = r.data;
        n.src.startsWith("http") ? e.push(n.src) : e.push(n.alt || "[Image]");
        break;
      }
      case "shape": {
        const n = r.data;
        n.label && e.push(n.label);
        break;
      }
      case "text": {
        const n = r.data;
        n.text && e.push(n.text);
        break;
      }
      case "sticky": {
        const n = r.data;
        n.text && e.push(n.text);
        break;
      }
      case "draw":
        break;
      case "edge": {
        const n = r.data;
        n.label && e.push(n.label);
        break;
      }
    }
  return e.join(`

`);
}
function Js(t, e) {
  const o = mh(e), r = o.split(`
`).filter(Boolean).map((s) => `<p>${s}</p>`).join(""), n = ma(e);
  return t.setData(
    "text/html",
    `<!--${ya}--><div data-sbd-nodes="${n}">${r || "<p></p>"}</div>`
  ), t.setData("text/plain", o), o;
}
function gh(t, e) {
  let o = (e == null ? void 0 : e.ownerDocument) ?? document, r = o.defaultView ?? window, n = r.innerWidth / 2, s = r.innerHeight / 2, i = null;
  const l = (y) => {
    n = y.clientX, s = y.clientY;
  }, a = (y) => {
    wr(y.target) || t.selection.size !== 0 && (y.preventDefault(), t.copySelected(), i = Js(
      y.clipboardData,
      t.getClipboardNodes()
    ));
  }, c = (y) => {
    wr(y.target) || t.selection.size !== 0 && (y.preventDefault(), t.copySelected(), i = Js(
      y.clipboardData,
      t.getClipboardNodes()
    ), t.deleteSelected());
  }, h = async (y) => {
    var P, G, K;
    if (wr(y.target)) return;
    const { x: g, y: b } = t.screenToCanvas(n, s), x = ((P = y.clipboardData) == null ? void 0 : P.getData("text/html")) || "", C = ((G = y.clipboardData) == null ? void 0 : G.getData("text/plain")) || "";
    if (x.includes(ya) || x.includes("data-sbd-nodes=") || i !== null && C === i) {
      if (i !== null && C === i && t.hasClipboard()) {
        y.preventDefault(), t.pasteClipboard(g, b);
        return;
      }
      const B = ga(x);
      if (B) {
        y.preventDefault(), t.setClipboard(B), t.pasteClipboard(g, b);
        return;
      }
    }
    const M = (K = y.clipboardData) == null ? void 0 : K.items;
    if (M) {
      for (const at of Array.from(M))
        if (at.type.startsWith("image/")) {
          y.preventDefault();
          const B = at.getAsFile();
          if (!B) continue;
          const et = new FileReader();
          et.onload = () => {
            const Z = et.result, Q = new Image();
            Q.onload = () => {
              const L = t.screenToCanvas(n, s), X = 400, _ = 300, j = Q.naturalWidth / Q.naturalHeight, q = Math.min(Q.naturalWidth, X), Y = Math.min(Q.naturalHeight, _), $ = j >= 1 ? q : Y * j, nt = j >= 1 ? q / j : Y;
              let ot = Z;
              if (x) {
                const pt = x.match(/<img[^>]+src=["']([^"']+)["']/i);
                pt && /\.(gif|webp|apng)(\?|#|$)/i.test(pt[1]) && (ot = pt[1].replace(/&amp;/g, "&"));
              }
              const ft = {
                id: gt(10),
                type: "image",
                x: L.x,
                y: L.y,
                w: $,
                h: nt,
                z: t.nextZ(),
                data: { src: ot }
              };
              t.addNode(ft), t.select(ft.id);
            }, Q.src = Z;
          }, et.readAsDataURL(B);
          return;
        }
    }
    const D = Ln(C) ?? Ln(x);
    if (D) {
      y.preventDefault();
      const at = t.screenToCanvas(n, s), B = await fa(
        D,
        at.x,
        at.y,
        t.nextZ()
      );
      B && (t.addNode(B), t.select(B.id));
      return;
    }
    if (kd(C)) {
      const at = wd(C);
      if (at) {
        y.preventDefault();
        const B = {
          id: gt(10),
          type: "youtube",
          x: g,
          y: b,
          w: 560,
          h: 315,
          z: t.nextZ(),
          data: { videoId: at, url: C.trim() }
        };
        t.addNode(B), t.select(B.id);
        return;
      }
    }
    const T = x.replace(/^<meta[^>]*>/i, "").replace(/<!--StartFragment-->|<!--EndFragment-->/g, "").trim();
    if (T)
      try {
        const at = Ii(T);
        if (at.length > 0) {
          y.preventDefault();
          const B = {
            id: gt(10),
            type: "content",
            x: g,
            y: b,
            w: 300,
            h: "auto",
            z: t.nextZ(),
            data: { blocks: at, markdown: C, borderColor: "#1e1e2e" }
          };
          t.addNode(B), t.select(B.id);
          return;
        }
      } catch {
      }
    if (C.trim()) {
      y.preventDefault();
      const at = await On(C), B = {
        id: gt(10),
        type: "content",
        x: g,
        y: b,
        w: 300,
        h: "auto",
        z: t.nextZ(),
        data: { blocks: at, markdown: C, borderColor: "#1e1e2e" }
      };
      t.addNode(B), t.select(B.id);
      return;
    }
    t.hasClipboard() && (y.preventDefault(), t.pasteClipboard(g, b));
  }, p = (y) => {
    const g = y.target;
    if (wr(g)) return;
    if (t.presentationMode) {
      if (y.key === "ArrowRight" || y.key === " ") {
        y.preventDefault(), t.presentationNext();
        return;
      }
      if (y.key === "ArrowLeft") {
        y.preventDefault(), t.presentationPrev();
        return;
      }
      if (y.key === "Escape") {
        y.preventDefault(), t.exitPresentation();
        return;
      }
      return;
    }
    const b = y.ctrlKey || y.metaKey;
    if (b && y.key === "c") {
      t.copySelected();
      return;
    }
    if (b && y.key === "x") {
      t.copySelected();
      return;
    }
    if (b && y.key === "d") {
      y.preventDefault(), t.duplicateSelected();
      return;
    }
    if (b && y.key === "g") {
      y.preventDefault(), y.shiftKey ? t.ungroupSelected() : t.groupSelected();
      return;
    }
    if (y.shiftKey && !b && y.key === "H") {
      y.preventDefault(), t.flipSelectedHorizontal();
      return;
    }
    if (y.shiftKey && !b && y.key === "V") {
      y.preventDefault(), t.flipSelectedVertical();
      return;
    }
    if (b && y.key === "]") {
      y.preventDefault();
      const x = Array.from(t.selection);
      y.altKey ? t.bringToFront(x) : t.bringForward(x);
      return;
    }
    if (b && y.key === "[") {
      y.preventDefault();
      const x = Array.from(t.selection);
      y.altKey ? t.sendToBack(x) : t.sendBackward(x);
      return;
    }
    if (!b && !y.altKey && !y.shiftKey) {
      if (y.key === "s") {
        t.setMode("select");
        return;
      }
      if (y.key === "p") {
        t.setMode("hand");
        return;
      }
      if (y.key === "d") {
        t.setMode("draw");
        return;
      }
      if (y.key === "g") {
        t.setMode("shape");
        return;
      }
      if (y.key === "t") {
        t.setMode("text");
        return;
      }
      if (y.key === "b") {
        t.setMode("note");
        return;
      }
      if (y.key === "y") {
        t.setMode("sticky");
        return;
      }
      if (y.key === "f") {
        t.setMode("frame");
        return;
      }
      if (y.key === "e") {
        t.setMode("erase");
        return;
      }
      if (y.key === "l") {
        t.toggleLassoSelect();
        return;
      }
      if (y.key === "z") {
        t.setMode("laser");
        return;
      }
    }
    if (y.key === "Delete" || y.key === "Backspace") {
      t.deleteSelected();
      return;
    }
    if (b && y.key === "z") {
      y.preventDefault(), y.shiftKey ? t.redo() : t.undo();
      return;
    }
    if (b && y.key === "a") {
      y.preventDefault(), t.selectMultiple(t.getAllNodes().map((x) => x.id));
      return;
    }
    if (y.key === "Escape") {
      if (t.activeGroupId) {
        t.exitGroup();
        return;
      }
      t.deselectAll(), t.setMode("select");
      return;
    }
    if (b && (y.key === "=" || y.key === "+")) {
      y.preventDefault(), t.zoomIn();
      return;
    }
    if (b && y.key === "-") {
      y.preventDefault(), t.zoomOut();
      return;
    }
    if (b && y.key === "0") {
      y.preventDefault(), t.fitToContent();
      return;
    }
  };
  function d(y, g) {
    y.addEventListener("pointermove", l), y.addEventListener("copy", a), y.addEventListener("cut", c), y.addEventListener("paste", h), g.addEventListener("keydown", p);
  }
  function f(y, g) {
    y.removeEventListener("pointermove", l), y.removeEventListener("copy", a), y.removeEventListener("cut", c), y.removeEventListener("paste", h), g.removeEventListener("keydown", p);
  }
  d(o, r);
  const m = setInterval(() => {
    if (!e) return;
    const y = e.ownerDocument;
    y !== o && (f(o, r), o = y, r = y.defaultView ?? window, n = r.innerWidth / 2, s = r.innerHeight / 2, d(o, r));
  }, 500);
  return () => {
    clearInterval(m), f(o, r);
  };
}
async function $s(t, e) {
  const o = t.getAllNodes();
  if (o.length === 0) return;
  const r = t.measuredHeights, n = bh(o, r, t), s = e.padding ?? 40, i = e.background !== !1, l = e.format === "png", a = n.w + s * 2, c = n.h + s * 2, h = n.x - s, p = n.y - s, d = await xa(o, t, r, h, p, l), f = i ? sr(t.boardBackground).canvasBg : "transparent", m = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${a}" height="${c}" viewBox="0 0 ${a} ${c}">`,
    `<rect width="${a}" height="${c}" fill="${f}"/>`,
    ...d,
    "</svg>"
  ].join(`
`);
  if (e.format === "svg")
    _s(new Blob([m], { type: "image/svg+xml" }), "board.svg");
  else {
    const y = e.scale ?? 4, g = await Ah(m, a, c, y);
    _s(g, "board.png");
  }
}
function bh(t, e, o) {
  let r = 1 / 0, n = 1 / 0, s = -1 / 0, i = -1 / 0;
  for (const a of t) {
    if (a.type === "edge") continue;
    const c = o.resolveHeight(a);
    r = Math.min(r, a.x), n = Math.min(n, a.y), s = Math.max(s, a.x + a.w), i = Math.max(i, a.y + c);
  }
  const l = new Map(t.map((a) => [a.id, a]));
  for (const a of t) {
    if (a.type !== "edge") continue;
    const c = a, h = l.get(c.data.fromId), p = l.get(c.data.toId);
    if (!h || !p) continue;
    const d = Ke(
      h,
      p,
      c.data.edgeType,
      e,
      c.data.sourceHandle,
      c.data.targetHandle,
      c.data.midpointOffset,
      c.data.curveOffset
    );
    r = Math.min(r, d.bounds.x), n = Math.min(n, d.bounds.y), s = Math.max(s, d.bounds.x + d.bounds.w), i = Math.max(i, d.bounds.y + d.bounds.h);
  }
  return isFinite(r) ? { x: r, y: n, w: s - r, h: i - n } : { x: 0, y: 0, w: 100, h: 100 };
}
async function xa(t, e, o, r, n, s) {
  const i = new Map(t.map((c) => [c.id, c])), l = [...t].sort((c, h) => c.z - h.z), a = [];
  for (const c of l) {
    const h = c.x - r, p = c.y - n, d = e.resolveHeight(c);
    switch (c.type) {
      case "frame":
        a.push(xh(c, h, p, d));
        break;
      case "content":
        a.push(wh(c, h, p, c.w, d));
        break;
      case "draw":
        a.push(kh(c, r, n));
        break;
      case "shape":
        a.push(Sh(c, h, p, c.w, d));
        break;
      case "text":
        a.push(Mh(c, h, p, c.w, d));
        break;
      case "sticky":
        a.push(Ch(c, h, p, c.w, d));
        break;
      case "image":
        a.push(await zh(c, h, p, c.w, d, s));
        break;
      case "youtube":
        a.push(await Ih(c, h, p, c.w, d, s));
        break;
      case "edge": {
        const f = c, m = i.get(f.data.fromId), y = i.get(f.data.toId);
        m && y && a.push(Ph(f, m, y, o, r, n));
        break;
      }
    }
  }
  return a;
}
function to(t, e, o, r, n, s, i) {
  const l = [];
  if (s) {
    const a = e + r / 2, c = o + n / 2;
    l.push(`transform="rotate(${s}, ${a}, ${c})"`);
  }
  return i !== void 0 && i !== 1 && l.push(`opacity="${i}"`), `<g ${l.join(" ")}>${t}</g>`;
}
function xh(t, e, o, r) {
  const n = t.data, s = n.backgroundColor || "rgba(0,0,0,0.02)", i = n.borderColor || "#d1d5db", l = n.borderWidth ?? 1, a = Or(n.borderStyle, l), c = n.label ? Xo(n.label) : "";
  let h = `<rect x="${e}" y="${o}" width="${t.w}" height="${r}" rx="4" fill="${s}" stroke="${i}" stroke-width="${l}"` + (a ? ` stroke-dasharray="${a}"` : "") + "/>";
  return c && (h += `<text x="${e + 8}" y="${o - 6}" font-size="12" fill="#6b7280" font-family="sans-serif">${c}</text>`), to(h, e, o, t.w, r, t.rotation, n.opacity);
}
function wh(t, e, o, r, n) {
  var p;
  const s = t.data, i = ((p = s.markdown) == null ? void 0 : p.trim()) || "", l = s.borderColor, a = s.borderWidth ?? 0, c = Or(s.borderStyle, a);
  let h = "";
  return l && a > 0 ? h += `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="4" fill="white" stroke="${l}" stroke-width="${a}"` + (c ? ` stroke-dasharray="${c}"` : "") + "/>" : h += `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="4" fill="white"/>`, i && (h += rs(i, e + 12, o + 20, r - 24, 14, 1.6, "#374151", "left", "sans-serif")), to(h, e, o, r, n, t.rotation, s.opacity);
}
function kh(t, e, o) {
  const r = t.data, n = r.points.map(
    ([l, a, c]) => [l + t.x - e, a + t.y - o, c]
  );
  if (n.length === 0) return "";
  if (r.tool === "vector")
    return vh(n, r, t);
  const s = _e(r.strokeStyle);
  let i = "";
  if (r.fill) {
    const l = n.map(([a, c]) => [a, c]);
    if (l.length > 2) {
      const a = l.map((c, h) => `${h === 0 ? "M" : "L"}${c[0]},${c[1]}`).join(" ") + " Z";
      i += `<path d="${a}" fill="${r.fill}" fill-opacity="0.4" stroke="none"/>`;
    }
  }
  if (s) {
    const l = n.map((c, h) => `${h === 0 ? "M" : "L"}${c[0].toFixed(1)},${c[1].toFixed(1)}`).join(" "), a = s.map((c) => c * Math.max(r.strokeWidth, 1)).join(" ");
    i += `<path d="${l}" fill="none" stroke="${r.color}" stroke-width="${r.strokeWidth}" stroke-dasharray="${a}" stroke-linecap="round" stroke-linejoin="round"/>`;
  } else {
    const l = jn(n, { size: r.strokeWidth });
    l && (i += `<path d="${l}" fill="${r.color}" stroke="none"/>`);
  }
  return r.opacity !== void 0 && r.opacity !== 1 ? `<g opacity="${r.opacity}">${i}</g>` : i;
}
function vh(t, e, o) {
  const r = t.map((a, c) => `${c === 0 ? "M" : "L"}${a[0].toFixed(2)},${a[1].toFixed(2)}`).join(" ") + " Z", n = _e(e.strokeStyle), s = n ? ` stroke-dasharray="${n.map((a) => a * Math.max(e.strokeWidth, 1)).join(" ")}"` : "", i = `<path d="${r}" fill="${e.fill || "none"}" stroke="${e.color}" stroke-width="${e.strokeWidth}" stroke-linecap="round" stroke-linejoin="round"${s}/>`, l = o.h === "auto" ? 0 : o.h;
  return to(i, o.x, o.y, o.w, l, o.rotation, e.opacity);
}
function Sh(t, e, o, r, n) {
  const s = t.data, i = {
    stroke: s.stroke,
    fill: s.fill,
    fillStyle: s.fillStyle,
    roughness: s.roughness,
    strokeWidth: s.strokeWidth,
    strokeLineDash: _e(s.strokeStyle),
    seed: t.id
  };
  let l;
  const a = s.edgeStyle === "round";
  switch (s.shape) {
    case "rect":
      l = Lr(e, o, r, n, i, a);
      break;
    case "ellipse":
      l = qn(e + r / 2, o + n / 2, r, n, i);
      break;
    case "diamond":
      l = Zn(e, o, r, n, i, a);
      break;
    case "line": {
      const h = s.startPoint ?? [0, 0], p = s.endPoint ?? [r, n];
      l = Bo(e + h[0], o + h[1], e + p[0], o + p[1], i);
      break;
    }
    case "arrow": {
      const h = s.startPoint ?? [0, 0], p = s.endPoint ?? [r, n];
      l = Un(e + h[0], o + h[1], e + p[0], o + p[1], i);
      break;
    }
    default:
      l = Lr(e, o, r, n, i);
  }
  const c = l.map(
    (h) => `<path d="${h.d}" fill="${h.fill || "none"}" stroke="${h.stroke}" stroke-width="${h.strokeWidth}"` + (h.strokeDasharray ? ` stroke-dasharray="${h.strokeDasharray}"` : "") + "/>"
  ).join(`
`);
  return to(c, e, o, r, n, t.rotation, s.opacity);
}
function Mh(t, e, o, r, n) {
  const s = t.data, i = n || s.text.split(`
`).length * s.fontSize * 1, l = Ue(s.fontFamily), a = !!s.borderColor, c = a ? 6 : 0;
  let h = "";
  if (a) {
    const d = s.borderWidth ?? 1, f = Or(s.borderStyle, d);
    h += `<rect x="${e}" y="${o}" width="${r}" height="${i}" rx="4" fill="none" stroke="${s.borderColor}" stroke-width="${d}"` + (f ? ` stroke-dasharray="${f}"` : "") + "/>";
  }
  const p = s.align === "center" ? e + r / 2 : s.align === "right" ? e + r - c : e + c;
  return h += rs(
    s.text,
    p,
    o + c + s.fontSize,
    r - c * 2,
    s.fontSize,
    1,
    s.color,
    s.align,
    l
  ), to(h, e, o, r, i, t.rotation, s.opacity);
}
function Ch(t, e, o, r, n) {
  const s = t.data, i = s.fontSize ?? 16, l = `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="2" fill="${s.color}"/>` + rs(s.text, e + 12, o + 12 + i, r - 24, i, 1.4, "#1e1e2e", "left", "sans-serif");
  return to(l, e, o, r, n, t.rotation, s.opacity);
}
async function zh(t, e, o, r, n, s) {
  const i = t.data;
  let l = i.src;
  if (s && l && !l.startsWith("data:"))
    try {
      l = await Er(l);
    } catch {
    }
  const a = i.borderColor, c = i.borderWidth ?? 0, h = Or(i.borderStyle, c);
  let p = `<image href="${Xo(l)}" x="${e}" y="${o}" width="${r}" height="${n}" preserveAspectRatio="xMidYMid slice"/>`;
  return a && c > 0 && (p += `<rect x="${e}" y="${o}" width="${r}" height="${n}" fill="none" stroke="${a}" stroke-width="${c}"` + (h ? ` stroke-dasharray="${h}"` : "") + "/>"), to(p, e, o, r, n, t.rotation, i.opacity);
}
async function Ih(t, e, o, r, n, s) {
  const i = t.data;
  let l = Sd(i.videoId);
  if (s)
    try {
      l = await Er(l);
    } catch {
    }
  let a = `<rect x="${e}" y="${o}" width="${r}" height="${n}" rx="4" fill="#1a1a1a"/><image href="${Xo(l)}" x="${e}" y="${o}" width="${r}" height="${n}" preserveAspectRatio="xMidYMid slice"/>`;
  const c = e + r / 2, h = o + n / 2, p = Math.min(r, n) * 0.12;
  return a += `<circle cx="${c}" cy="${h}" r="${p}" fill="rgba(0,0,0,0.6)"/><path d="${Th(c, h, p * 0.5)}" fill="white"/>`, to(a, e, o, r, n, t.rotation, i.opacity);
}
function Th(t, e, o) {
  const r = o * 0.15, n = t - o * 0.7 + r, s = e - o, i = t + o + r, l = e, a = n, c = e + o;
  return `M${n},${s} L${i},${l} L${a},${c} Z`;
}
function Ph(t, e, o, r, n, s) {
  const i = t.data, l = Ke(
    e,
    o,
    i.edgeType,
    r,
    i.sourceHandle,
    i.targetHandle,
    i.midpointOffset,
    i.curveOffset
  ), a = `translate(${-n}, ${-s})`, c = i.style === "dashed" ? "8 4" : i.style === "dotted" ? "2 3" : void 0, h = i.strokeWidth;
  let p = `<path d="${l.path}" fill="none" stroke="${i.color}" stroke-width="${h}"` + (c ? ` stroke-dasharray="${c}"` : "") + ' stroke-linecap="round" stroke-linejoin="round"/>';
  const d = i.arrowHeadSize ?? Math.max(8, h * 3), f = i.arrowTailSize ?? Math.max(8, h * 3);
  if (i.arrowHead && i.arrowHead !== "none") {
    if (i.arrowHead === "arrow")
      p += `<path d="${Eo(l.x2, l.y2, l.arrowAngle, d)}" fill="none" stroke="${i.color}" stroke-width="${h}" stroke-linecap="round" stroke-linejoin="round"/>`;
    else if (i.arrowHead === "filled")
      p += `<path d="${Ir(l.x2, l.y2, l.arrowAngle, d)}" fill="${i.color}" stroke="none"/>`;
    else if (i.arrowHead === "dot") {
      const m = d / 3;
      p += `<circle cx="${l.x2}" cy="${l.y2}" r="${m}" fill="${i.color}"/>`;
    }
  }
  if (i.arrowTail && i.arrowTail !== "none") {
    if (i.arrowTail === "arrow")
      p += `<path d="${Eo(l.x1, l.y1, l.tailAngle, f)}" fill="none" stroke="${i.color}" stroke-width="${h}" stroke-linecap="round" stroke-linejoin="round"/>`;
    else if (i.arrowTail === "filled")
      p += `<path d="${Ir(l.x1, l.y1, l.tailAngle, f)}" fill="${i.color}" stroke="none"/>`;
    else if (i.arrowTail === "dot") {
      const m = f / 3;
      p += `<circle cx="${l.x1}" cy="${l.y1}" r="${m}" fill="${i.color}"/>`;
    }
  }
  return i.label && (p += `<text x="${l.labelX}" y="${l.labelY}" font-size="12" fill="${i.color}" text-anchor="middle" dominant-baseline="central" font-family="sans-serif">${Xo(i.label)}</text>`), `<g transform="${a}">${p}</g>`;
}
function rs(t, e, o, r, n, s, i, l, a) {
  if (!t) return "";
  const c = l === "center" ? "middle" : l === "right" ? "end" : "start", h = Rh(t, r, n), p = n * s, d = h.map(
    (f, m) => `<tspan x="${e}" dy="${m === 0 ? 0 : p}">${Xo(f)}</tspan>`
  ).join("");
  return `<text x="${e}" y="${o}" font-size="${n}" fill="${i}" font-family="${Xo(a)}" text-anchor="${c}">${d}</text>`;
}
function Rh(t, e, o) {
  const r = o * 0.55, n = Math.max(1, Math.floor(e / r)), s = [];
  for (const i of t.split(`
`)) {
    if (!i.trim()) {
      s.push("");
      continue;
    }
    const l = i.split(/\s+/);
    let a = "";
    for (const c of l) {
      const h = a ? a + " " + c : c;
      h.length > n && a ? (s.push(a), a = c) : a = h;
    }
    a && s.push(a);
  }
  return s;
}
function Or(t, e) {
  const o = e ?? 1;
  if (t === "dashed") return `${8 * o} ${4 * o}`;
  if (t === "dotted") return `${2 * o} ${2 * o}`;
}
function Xo(t) {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
}
async function Er(t) {
  const o = await (await fetch(t)).blob();
  return new Promise((r, n) => {
    const s = new FileReader();
    s.onloadend = () => r(s.result), s.onerror = n, s.readAsDataURL(o);
  });
}
function Ah(t, e, o, r) {
  return new Promise((n, s) => {
    const i = new Image(), l = new Blob([t], { type: "image/svg+xml;charset=utf-8" }), a = URL.createObjectURL(l);
    i.onload = () => {
      const c = document.createElement("canvas");
      c.width = e * r, c.height = o * r;
      const h = c.getContext("2d");
      h.scale(r, r), h.drawImage(i, 0, 0, e, o), URL.revokeObjectURL(a), c.toBlob((p) => {
        p ? n(p) : s(new Error("Canvas toBlob failed"));
      }, "image/png");
    }, i.onerror = () => {
      URL.revokeObjectURL(a), s(new Error("Failed to load SVG as image"));
    }, i.src = a;
  });
}
const Lh = /* @__PURE__ */ new Set(["sans-serif", "serif", "monospace"]), zo = /* @__PURE__ */ new Map(), Dh = 12;
function Eh(t) {
  const e = /* @__PURE__ */ new Set();
  for (const o of t)
    if (o.type === "text") {
      const r = o.data.fontFamily;
      r && !Lh.has(r) && e.add(r);
    }
  return [...e];
}
async function Wh(t) {
  if (t.length === 0) return "";
  const e = [];
  for (const o of t) {
    if (zo.has(o)) {
      e.push(zo.get(o));
      continue;
    }
    try {
      let r;
      if (o === "Excalifont")
        r = await Er(Ti);
      else {
        const l = (await (await fetch(
          `https://fonts.googleapis.com/css2?family=${encodeURIComponent(o)}&display=swap`
        )).text()).match(/url\((https:\/\/[^)]+\.woff2)\)/);
        if (!l) continue;
        r = await Er(l[1]);
      }
      const n = `@font-face { font-family: '${o}'; src: url('${r}') format('woff2'); }`;
      if (zo.size >= Dh) {
        const s = zo.keys().next().value;
        s !== void 0 && zo.delete(s);
      }
      zo.set(o, n), e.push(n);
    } catch {
    }
  }
  return e.join(`
`);
}
async function Fh(t, e) {
  const o = t.getNode(e);
  if (!o || o.type !== "frame") return "";
  const r = t.resolveHeight(o), n = 0, s = o.w + n * 2, i = r + n * 2, l = o.x - n, a = o.y - n, c = [o], h = /* @__PURE__ */ new Set([e]), p = (x) => {
    h.has(x.id) || x.type === "edge" || (h.add(x.id), c.push(x));
  };
  for (const x of t.getNodesInRect({ x: o.x, y: o.y, w: o.w, h: r }))
    p(x);
  for (const x of t.getFrameChildren(e))
    p(x);
  for (const x of t.getAllNodes())
    if (x.type === "edge") {
      const C = x;
      h.has(C.data.fromId) && h.has(C.data.toId) && c.push(x);
    }
  const d = t.measuredHeights, f = await xa(c, t, d, l, a, !0), m = Eh(c), y = await Wh(m), g = sr(t.boardBackground).canvasBg, b = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${i}" viewBox="0 0 ${s} ${i}">`,
    y ? `<defs><style>${y}</style></defs>` : "",
    `<rect width="${s}" height="${i}" fill="${g}"/>`,
    ...f,
    "</svg>"
  ].join(`
`);
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(b)}`;
}
function _s(t, e) {
  const o = URL.createObjectURL(t), r = document.createElement("a");
  r.href = o, r.download = e, document.body.appendChild(r), r.click(), document.body.removeChild(r), URL.revokeObjectURL(o);
}
const ti = [
  { key: "iphone-se", label: "iPhone SE", w: 375, h: 667, group: "phone" },
  { key: "iphone-xr", label: "iPhone XR", w: 414, h: 896, group: "phone" },
  { key: "iphone-12-pro", label: "iPhone 12 Pro", w: 390, h: 844, group: "phone" },
  { key: "iphone-14-pro-max", label: "iPhone 14 Pro Max", w: 430, h: 932, group: "phone" },
  { key: "iphone-16-pro", label: "iPhone 16 Pro", w: 402, h: 874, group: "phone" },
  { key: "pixel-7", label: "Pixel 7", w: 412, h: 915, group: "phone" },
  { key: "galaxy-s8", label: "Galaxy S8+", w: 360, h: 740, group: "phone" },
  { key: "galaxy-s20-ultra", label: "Galaxy S20 Ultra", w: 412, h: 915, group: "phone" },
  { key: "galaxy-z-fold-5", label: "Galaxy Z Fold 5", w: 344, h: 882, group: "phone" },
  { key: "galaxy-a51", label: "Galaxy A51/71", w: 412, h: 914, group: "phone" }
], ei = [
  { key: "ipad-mini", label: "iPad Mini", w: 768, h: 1024, group: "tablet" },
  { key: "ipad-air", label: "iPad Air", w: 820, h: 1180, group: "tablet" },
  { key: "ipad-pro", label: "iPad Pro", w: 1024, h: 1366, group: "tablet" },
  { key: "surface-pro-7", label: "Surface Pro 7", w: 912, h: 1368, group: "tablet" },
  { key: "surface-duo", label: "Surface Duo", w: 540, h: 720, group: "tablet" },
  { key: "zenbook-fold", label: "Asus Zenbook Fold", w: 853, h: 1280, group: "tablet" }
];
function oi(t, e) {
  return t.map((o) => ({
    key: `${o.key}-landscape`,
    label: `${o.label} ↔`,
    w: o.h,
    h: o.w,
    group: e
  }));
}
const wa = [
  ...ti,
  ...oi(ti, "phone-landscape"),
  ...ei,
  ...oi(ei, "tablet-landscape"),
  // Other devices (already landscape-oriented)
  { key: "nest-hub", label: "Nest Hub", w: 1024, h: 600, group: "other" },
  { key: "nest-hub-max", label: "Nest Hub Max", w: 1280, h: 800, group: "other" },
  // Standard ratios
  { key: "16-9", label: "16:9", w: 1920, h: 1080, group: "standard" },
  { key: "9-16", label: "9:16", w: 1080, h: 1920, group: "standard" },
  { key: "4-3", label: "4:3", w: 1024, h: 768, group: "standard" },
  { key: "3-4", label: "3:4", w: 768, h: 1024, group: "standard" },
  { key: "1-1", label: "1:1", w: 1e3, h: 1e3, group: "standard" },
  { key: "21-9", label: "21:9", w: 2560, h: 1080, group: "standard" }
], Bh = new Map(wa.map((t) => [t.key, t]));
function Dn(t) {
  return Bh.get(t);
}
function ka(t) {
  return t.w / t.h;
}
const Nh = {
  phone: "Phones",
  "phone-landscape": "Phones (Landscape)",
  tablet: "Tablets",
  "tablet-landscape": "Tablets (Landscape)",
  other: "Devices",
  standard: "Standard"
};
function Hh() {
  const t = /* @__PURE__ */ new Map();
  for (const e of wa) {
    const o = t.get(e.group);
    o ? o.push(e) : t.set(e.group, [e]);
  }
  return Array.from(t.entries()).map(([e, o]) => ({
    label: Nh[e] ?? e,
    presets: o
  }));
}
function Oh(t) {
  const e = t.replace("#", ""), o = parseInt(e.substring(0, 2), 16) || 0, r = parseInt(e.substring(2, 4), 16) || 0, n = parseInt(e.substring(4, 6), 16) || 0;
  return (0.299 * o + 0.587 * r + 0.114 * n) / 255 > 0.5 ? "#1e1e2e" : "#ffffff";
}
function un(t, e, o) {
  let r = !1;
  for (let n = 0, s = o.length - 1; n < o.length; s = n++) {
    const [i, l] = o[n], [a, c] = o[s];
    l > e != c > e && t < (a - i) * (e - l) / (c - l) + i && (r = !r);
  }
  return r;
}
async function Xh(t, e, o) {
  try {
    const r = await navigator.clipboard.read();
    let n = null;
    for (const i of r)
      if (i.types.includes("text/html")) {
        const l = await (await i.getType("text/html")).text();
        if (l.includes("sbd-clipboard") || l.includes("data-sbd-nodes=")) {
          const a = ga(l);
          if (a) {
            t.setClipboard(a), t.pasteClipboard(e, o);
            return;
          }
          if (t.hasClipboard()) {
            t.pasteClipboard(e, o);
            return;
          }
        }
        n = l;
      }
    for (const i of r) {
      const l = i.types.find((a) => a.startsWith("image/"));
      if (l) {
        const a = await i.getType(l), c = await new Promise((x) => {
          const C = new FileReader();
          C.onload = () => x(C.result), C.readAsDataURL(a);
        }), h = new Image();
        await new Promise((x) => {
          h.onload = () => x(), h.src = c;
        });
        const p = h.naturalWidth / h.naturalHeight, d = Math.min(h.naturalWidth, 400), f = Math.min(h.naturalHeight, 300), m = p >= 1 ? d : f * p, y = p >= 1 ? d / p : f;
        let g = c;
        if (n) {
          const x = n.match(/<img[^>]+src=["']([^"']+)["']/i);
          x && /\.(gif|webp|apng)(\?|#|$)/i.test(x[1]) && (g = x[1].replace(/&amp;/g, "&"));
        }
        const b = {
          id: gt(10),
          type: "image",
          x: e,
          y: o,
          w: m,
          h: y,
          z: t.nextZ(),
          data: { src: g }
        };
        t.addNode(b), t.select(b.id);
        return;
      }
    }
    const s = await navigator.clipboard.readText();
    if (n) {
      const i = n.replace(/^<meta[^>]*>/i, "").replace(/<!--StartFragment-->|<!--EndFragment-->/g, "").trim();
      try {
        const l = Ii(i);
        if (l.length > 0) {
          const a = {
            id: gt(10),
            type: "content",
            x: e,
            y: o,
            w: 300,
            h: "auto",
            z: t.nextZ(),
            data: { blocks: l, markdown: s || "", borderColor: "#1e1e2e" }
          };
          t.addNode(a), t.select(a.id);
          return;
        }
      } catch {
      }
    }
    if (s != null && s.trim()) {
      const i = await On(s), l = {
        id: gt(10),
        type: "content",
        x: e,
        y: o,
        w: 300,
        h: "auto",
        z: t.nextZ(),
        data: { blocks: i, markdown: s, borderColor: "#1e1e2e" }
      };
      t.addNode(l), t.select(l.id);
      return;
    }
  } catch {
  }
  t.pasteClipboard(e, o);
}
async function ri(t) {
  const e = t.getClipboardNodes();
  if (e.length === 0) return;
  const o = [];
  for (const l of e)
    if (l.type === "content") {
      const a = l.data;
      a.markdown && o.push(a.markdown);
    } else if (l.type === "text") {
      const a = l.data;
      a.text && o.push(a.text);
    } else if (l.type === "image") {
      const a = l.data;
      o.push(a.src.startsWith("http") ? a.src : a.alt || "[Image]");
    } else if (l.type === "shape") {
      const a = l.data;
      a.label && o.push(a.label);
    } else if (l.type === "sticky") {
      const a = l.data;
      a.text && o.push(a.text);
    } else if (l.type === "edge") {
      const a = l.data;
      a.label && o.push(a.label);
    }
  const r = o.join(`

`), n = r.split(`
`).filter(Boolean).map((l) => `<p>${l}</p>`).join(""), i = `<!--sbd-clipboard--><div data-sbd-nodes="${ma(e)}">${n || "<p></p>"}</div>`;
  try {
    await navigator.clipboard.write([
      new ClipboardItem({
        "text/plain": new Blob([r], { type: "text/plain" }),
        "text/html": new Blob([i], { type: "text/html" })
      })
    ]);
  } catch {
    try {
      await navigator.clipboard.writeText(r);
    } catch {
    }
  }
}
function kr(t) {
  switch (t) {
    case "select":
      return "default";
    case "text":
      return "text";
    case "note":
      return "text";
    case "sticky":
      return "crosshair";
    case "draw":
      return "crosshair";
    case "shape":
      return "crosshair";
    case "edge":
      return "crosshair";
    case "frame":
      return "crosshair";
    case "erase":
      return `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='20' height='20'><circle cx='10' cy='10' r='9' fill='none' stroke='%239ca3af' stroke-width='1.5'/></svg>") ${20 / 2} ${20 / 2}, crosshair`;
    case "laser":
      return `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16'><circle cx='8' cy='8' r='3' fill='%23ef4444'/><circle cx='8' cy='8' r='7' fill='none' stroke='%23ef4444' stroke-width='1' opacity='0.4'/></svg>") ${16 / 2} ${16 / 2}, crosshair`;
    case "hand":
      return "grab";
    default:
      return "default";
  }
}
function ni(t, e) {
  const o = e.x - t.x, r = e.y - t.y;
  return { dist: Math.sqrt(o * o + r * r), mx: (t.x + e.x) / 2, my: (t.y + e.y) / 2 };
}
const Io = `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' fill='none' stroke='%23e0e0e0' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'><path d='M12 3a7 7 0 0 1 4.5 12.4l-2 2'/><path d='M14.5 15.4q.5 1.5.5 2.6c0 2-1.5 3-3 3s-2-1-2-2c0-.8.5-1.5 1-2'/><circle cx='12' cy='3' r='1.5' fill='%23e0e0e0' stroke='none'/></svg>") 12 3, crosshair`;
function Yh({
  node: t,
  isInteractive: e,
  measuredH: o,
  onMeasuredHeight: r,
  observeElement: n,
  unobserveElement: s,
  isContainer: i,
  children: l
}) {
  const a = lt(null);
  mt(() => {
    if (t.h !== "auto") return;
    const p = a.current;
    if (!p) return;
    const d = p.offsetHeight;
    return d > 0 && r(t.id, d), n(p, () => {
      const f = p.offsetHeight;
      f > 0 && r(t.id, f);
    }), () => s(p);
  }, [t.id, t.h, r, n, s]);
  const c = t.h === "auto" ? o ?? "auto" : t.h, h = Kt(() => ({
    position: "absolute",
    left: t.x,
    top: t.y,
    width: t.w,
    height: c,
    zIndex: t.z,
    pointerEvents: i ? "none" : e ? "auto" : "none",
    transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
    transformOrigin: "center center"
  }), [t.x, t.y, t.w, c, t.z, t.rotation, i, e]);
  return /* @__PURE__ */ u(
    "div",
    {
      ref: a,
      "data-node-id": t.id,
      className: e ? void 0 : "sb-block-inert",
      style: h,
      children: l
    }
  );
}
function Gh({
  node: t,
  engine: e,
  onDone: o
}) {
  const r = lt(null), n = lt(t.data.label ?? ""), s = lt(t);
  s.current = t;
  const i = lt(t.data.label ?? "");
  mt(() => () => {
    const h = s.current, p = n.current.trim();
    if (p !== i.current) {
      const f = { data: { ...h.data, label: p || void 0 } }, m = r.current;
      if (m && p) {
        const g = h.h === "auto" ? 100 : h.h, b = m.scrollHeight + 24;
        b > g && (f.h = b);
      }
      e.updateNodeWithHistory(h.id, f);
    }
  }, []);
  const l = t.h === "auto" ? 100 : t.h, a = t.data.labelFontSize ?? 14, c = t.data.fill && t.data.fillStyle === "solid" ? Oh(t.data.fill) : t.data.stroke;
  return /* @__PURE__ */ u(
    "div",
    {
      "data-node-id": t.id,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: l,
        zIndex: 999998,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        pointerEvents: "auto",
        padding: "8px 12px",
        boxSizing: "border-box"
      },
      children: /* @__PURE__ */ u(
        "textarea",
        {
          ref: r,
          autoFocus: !0,
          defaultValue: t.data.label ?? "",
          placeholder: "",
          rows: 1,
          onBlur: () => o(),
          onKeyDown: (h) => {
            h.key === "Escape" && h.currentTarget.blur(), h.stopPropagation();
          },
          onInput: (h) => {
            const p = h.currentTarget;
            n.current = p.value;
            const d = s.current;
            e.updateNode(d.id, {
              data: { ...d.data, label: p.value || void 0 }
            }), p.style.height = "auto", p.style.height = p.scrollHeight + "px";
            const m = p.scrollHeight + 24;
            m > l && e.updateNode(t.id, { h: m });
          },
          onPointerDown: (h) => h.stopPropagation(),
          style: {
            textAlign: t.data.labelAlign ?? "center",
            fontSize: a,
            fontFamily: Ue(t.data.labelFontFamily ?? Ze),
            color: c,
            background: "transparent",
            border: "none",
            outline: "none",
            resize: "none",
            overflow: "hidden",
            width: "100%",
            padding: 0,
            margin: 0,
            lineHeight: 1.3,
            wordBreak: "break-word",
            whiteSpace: "pre-wrap"
          }
        }
      )
    }
  );
}
function jh({
  engine: t,
  schema: e,
  registry: o,
  dataFlow: r
}) {
  const n = lt(null), s = () => {
    var w;
    return ((w = n.current) == null ? void 0 : w.ownerDocument) ?? document;
  }, [i, l] = tt({ w: 0, h: 0 }), [a, c] = tt({ ...t.viewport }), [h, p] = tt(t.getAllNodes()), [d, f] = tt(
    new Set(t.selection)
  ), [m, y] = tt(t.mode), [g, b] = tt(t.activeGroupId), [x, C] = tt(t.snapToGrid), [I, M] = tt(t.gridSize), [D, T] = tt(t.smartGuides), [P, G] = tt([]), [K, at] = tt(t.boardBackground), B = lt(!1), et = lt(!1), Z = lt(/* @__PURE__ */ new Map()), Q = lt(!1), L = lt(!1), X = lt(null), _ = lt(null);
  mt(() => {
    const w = (z) => {
      var N, S;
      if (z.key === " " && !z.repeat && !B.current) {
        const F = (N = z.target) == null ? void 0 : N.tagName;
        if (F === "INPUT" || F === "TEXTAREA" || (S = z.target) != null && S.isContentEditable) return;
        B.current = !0;
        const R = n.current;
        R && (R.style.cursor = "grab"), z.preventDefault();
      }
    }, E = (z) => {
      if (z.key === " ") {
        B.current = !1, et.current = !1;
        const N = n.current;
        N && (N.style.cursor = t.lassoSelect ? Io : kr(t.mode));
      }
    };
    return window.addEventListener("keydown", w), window.addEventListener("keyup", E), () => {
      window.removeEventListener("keydown", w), window.removeEventListener("keyup", E);
    };
  }, []), mt(() => {
    const w = (z) => {
      Z.current.delete(z.pointerId), z.pointerType === "pen" && (L.current = !1), X.current && (clearTimeout(X.current), X.current = null, _.current = null);
    }, E = s();
    return E.addEventListener("pointerup", w), E.addEventListener("pointercancel", w), () => {
      E.removeEventListener("pointerup", w), E.removeEventListener("pointercancel", w);
    };
  }, []);
  const [j, q] = tt(null), [Y, $] = tt(null), [nt, ot] = tt(null), [ft, pt] = tt(null);
  mt(() => {
    const w = n.current;
    if (!w) return;
    t.setContainer(w);
    const E = () => {
      const N = w.getBoundingClientRect();
      t.containerOffset = { x: N.left, y: N.top };
    };
    E();
    const z = new ResizeObserver((N) => {
      var R;
      const { width: S, height: F } = ((R = N[0]) == null ? void 0 : R.contentRect) ?? { width: 0, height: 0 };
      l((H) => H.w === S && H.h === F ? H : { w: S, h: F }), t.setContainerSize(S, F), E();
    });
    return z.observe(w), () => z.disconnect();
  }, [t]);
  const [rt, It] = tt({}), zt = it((w, E) => {
    It(
      (z) => z[w] === E ? z : { ...z, [w]: E }
    ), t.updateMeasuredHeight(w, E);
  }, [t]), bt = lt(null), ht = lt(/* @__PURE__ */ new Map());
  function Wt() {
    return bt.current || (bt.current = new ResizeObserver((w) => {
      var E;
      for (const z of w)
        (E = ht.current.get(z.target)) == null || E(z);
    })), bt.current;
  }
  const Tt = it((w, E) => {
    ht.current.set(w, E), Wt().observe(w);
  }, []), Rt = it((w) => {
    var E;
    ht.current.delete(w), (E = bt.current) == null || E.unobserve(w);
  }, []);
  mt(() => () => {
    var w;
    (w = bt.current) == null || w.disconnect(), bt.current = null, ht.current.clear();
  }, []);
  const Qt = Kt(() => new Set(h.map((w) => w.id)), [h]);
  mt(() => {
    It((w) => {
      let E = !1;
      const z = {};
      for (const [N, S] of Object.entries(w))
        Qt.has(N) ? z[N] = S : E = !0;
      return E ? z : w;
    });
  }, [Qt]);
  const Jt = it(
    (w, E, z) => {
      let N, S;
      if (o && w.data.sourcePort) {
        const F = o.get(E.type);
        F != null && F.ports && (N = or(E, F.ports, w.data.sourcePort, a.zoom, rt) ?? void 0);
      }
      if (o && w.data.targetPort) {
        const F = o.get(z.type);
        F != null && F.ports && (S = or(z, F.ports, w.data.targetPort, a.zoom, rt) ?? void 0);
      }
      return { sourcePortPos: N, targetPortPos: S };
    },
    [o, a.zoom, rt]
  );
  it(
    (w) => t.zoomToNode(w),
    [t]
  );
  const Zt = it(
    (w, E) => {
      if (!w.rotation)
        return { minX: w.x, minY: w.y, maxX: w.x + w.w, maxY: w.y + E };
      const z = w.x + w.w / 2, N = w.y + E / 2, S = w.rotation * Math.PI / 180, F = Math.cos(S), R = Math.sin(S), H = [
        [w.w / 2, E / 2],
        [-w.w / 2, E / 2],
        [-w.w / 2, -E / 2],
        [w.w / 2, -E / 2]
      ];
      let W = 1 / 0, O = 1 / 0, A = -1 / 0, V = -1 / 0;
      for (const [U, J] of H) {
        const dt = z + U * F - J * R, ct = N + U * R + J * F;
        W = Math.min(W, dt), O = Math.min(O, ct), A = Math.max(A, dt), V = Math.max(V, ct);
      }
      return { minX: W, minY: O, maxX: A, maxY: V };
    },
    []
  ), $t = 8, he = it(
    (w, E) => E.filter((z) => {
      if (z.type === "edge") {
        const F = z.data, R = t.getNode(F.fromId), H = t.getNode(F.toId);
        if (!R || !H) return !1;
        const { x1: W, y1: O, x2: A, y2: V } = $r(R, H, rt);
        return W >= w.x && W <= w.x + w.w && O >= w.y && O <= w.y + w.h && A >= w.x && A <= w.x + w.w && V >= w.y && V <= w.y + w.h;
      }
      const N = z.h === "auto" ? rt[z.id] ?? 100 : z.h, S = Zt(z, N);
      return S.minX >= w.x && S.maxX <= w.x + w.w && S.minY >= w.y && S.maxY <= w.y + w.h;
    }),
    [Zt, rt]
  ), Ae = it(
    (w, E) => w.length < 3 ? [] : E.filter((z) => {
      if (z.type === "edge") {
        const R = z, H = t.getNode(R.data.fromId), W = t.getNode(R.data.toId);
        if (!H || !W) return !1;
        const { x1: O, y1: A, x2: V, y2: U } = $r(H, W, rt);
        return un(O, A, w) && un(V, U, w);
      }
      const N = z.h === "auto" ? rt[z.id] ?? 100 : z.h, S = z.x + z.w / 2, F = z.y + N / 2;
      return un(S, F, w);
    }),
    [t, rt]
  ), _t = Kt(() => {
    if (d.size < 2) return null;
    let w = 1 / 0, E = 1 / 0, z = -1 / 0, N = -1 / 0;
    for (const S of d) {
      const F = h.find((W) => W.id === S);
      if (!F || F.type === "edge") continue;
      const R = F.h === "auto" ? rt[F.id] ?? 100 : F.h, H = Zt(F, R);
      w = Math.min(w, H.minX), E = Math.min(E, H.minY), z = Math.max(z, H.maxX), N = Math.max(N, H.maxY);
    }
    return w === 1 / 0 ? null : {
      x: w - $t,
      y: E - $t,
      w: z - w + $t * 2,
      h: N - E + $t * 2
    };
  }, [d, h, rt, Zt]), Le = Kt(() => {
    if (!g) return null;
    const w = t.getAllGroupDescendantNodes(g);
    if (w.length === 0) return null;
    let E = 1 / 0, z = 1 / 0, N = -1 / 0, S = -1 / 0;
    for (const R of w) {
      if (R.type === "edge") continue;
      const H = R.h === "auto" ? rt[R.id] ?? 100 : R.h, W = Zt(R, H);
      E = Math.min(E, W.minX), z = Math.min(z, W.minY), N = Math.max(N, W.maxX), S = Math.max(S, W.maxY);
    }
    if (E === 1 / 0) return null;
    const F = 8;
    return { x: E - F, y: z - F, w: N - E + F * 2, h: S - z + F * 2 };
  }, [g, h, rt, Zt, t]), eo = Kt(() => {
    const w = h.filter(
      (O) => {
        if (o) {
          const A = o.get(O.type);
          return A && !A.isSVGOnly;
        }
        return O.type === "content" || O.type === "draw" || O.type === "shape" || O.type === "image" || O.type === "text" || O.type === "frame" || O.type === "sticky";
      }
    ), E = a.zoom < 0.5 ? 15 : a.zoom < 1 ? 25 : 30;
    if (i.w <= 0 || i.h <= 0 || w.length < E)
      return null;
    const { zoom: z, x: N, y: S } = a, F = 500, R = {
      x: -N / z - F,
      y: -S / z - F,
      w: i.w / z + F * 2,
      h: i.h / z + F * 2
    }, H = t.getNodesInRect(R), W = /* @__PURE__ */ new Map();
    for (const O of H) {
      const A = t.getNode(O.id);
      A && W.set(O.id, A);
    }
    for (const O of d) {
      const A = t.getNode(O);
      A && W.set(A.id, A);
    }
    for (const O of h) {
      if (O.type !== "edge" || W.has(O.id)) continue;
      const A = O.data, V = t.getNode(A.fromId), U = t.getNode(A.toId);
      if (!V || !U) continue;
      let J = W.has(A.fromId) || W.has(A.toId);
      if (!J) {
        const { x1: dt, y1: ct, x2: kt, y2: Mt } = $r(V, U, rt);
        J = jl(dt, ct, kt, Mt, R);
      }
      J && (W.set(O.id, O), W.has(V.id) || W.set(V.id, V), W.has(U.id) || W.set(U.id, U));
    }
    return Array.from(W.values());
  }, [a, i, h, d, t]), xo = Kt(() => eo || h, [h, eo]);
  mt(() => {
    let w = null;
    const E = () => {
      w === null && (w = requestAnimationFrame(() => {
        w = null, p([...t.getAllNodes()]);
      }));
    };
    let z = null;
    const N = () => {
      z === null && (z = requestAnimationFrame(() => {
        z = null, c({ ...t.viewport });
      }));
    }, S = () => {
      f((V) => {
        const U = new Set(t.selection);
        return V.size !== U.size || [...V].some((J) => !U.has(J)) ? (ge((J) => J && !U.has(J) ? null : J), Ee((J) => J && !U.has(J) ? null : J), so((J) => J && !U.has(J) ? null : J), wo((J) => J && !U.has(J) ? null : J), ko((J) => J && !U.has(J) ? null : J), He(null), U) : V;
      });
    }, F = () => {
      y(t.mode), t.mode === "text" && (Zo.current = !1);
    }, R = () => at(t.boardBackground), H = () => {
      G([...t.alignGuides]), C(t.snapToGrid), M(t.gridSize), T(t.smartGuides);
    };
    t.on("change", E), t.on("viewport", N), t.on("selection", S), t.on("mode", F), t.on("background", R), t.on("guides", H);
    const W = (V) => b(V), O = () => b(null), A = () => {
      const V = n.current;
      V && (V.style.cursor = t.lassoSelect ? Io : kr(t.mode));
    };
    return t.on("group:enter", W), t.on("group:exit", O), t.on("lassoToggle", A), () => {
      w !== null && cancelAnimationFrame(w), z !== null && cancelAnimationFrame(z), t.off("change", E), t.off("viewport", N), t.off("selection", S), t.off("mode", F), t.off("background", R), t.off("guides", H), t.off("group:enter", W), t.off("group:exit", O), t.off("lassoToggle", A);
    };
  }, [t]), mt(() => {
    const w = n.current;
    if (!w) return;
    const E = (z) => {
      if (!z.ctrlKey && !z.metaKey) {
        const S = z.target.closest(".sb-editor-wrap");
        if (S && S.scrollHeight > S.clientHeight) {
          const F = S.scrollTop <= 0 && z.deltaY < 0, R = S.scrollTop + S.clientHeight >= S.scrollHeight && z.deltaY > 0;
          if (!F && !R) return;
        }
      }
      z.preventDefault(), z.ctrlKey || z.metaKey ? t.zoomByWheel(z.deltaY, z.clientX, z.clientY) : t.pan(-z.deltaX, -z.deltaY);
    };
    return w.addEventListener("wheel", E, { passive: !1 }), () => w.removeEventListener("wheel", E);
  }, [t]);
  const [ue, Me] = tt(null), [oo, ro] = tt(null), [pe, Ce] = tt(null), [be, He] = tt(null), v = lt({
    x: 0,
    y: 0,
    index: -1
  }), [st, Yt] = tt(null), [ne, xe] = tt(null), De = lt(null), [no, ge] = tt(null), Vo = lt(null), [cr, Ee] = tt(null), [ds, so] = tt(null), [Ko, wo] = tt(null), [hs, ko] = tt(null), [Ma, us] = tt(null);
  mt(() => {
    const w = (E) => {
      tl(() => ko(E));
    };
    return t.on("image:cropRequest", w), () => t.off("image:cropRequest", w);
  }, [t]);
  const ps = no || ds || cr || Ko || hs || Ma, Gr = lt(null), fs = lt(null), [jr, Vr] = tt(/* @__PURE__ */ new Set()), io = lt(/* @__PURE__ */ new Set()), [ys, qo] = tt([]), [dr, Kr] = tt(null), ze = lt([]), Oe = lt(null), [ms, hr] = tt([]), ae = lt([]), vo = lt(null), Zo = lt(!1), gs = it(
    (w, E, z, N = "auto") => {
      const S = gt(10);
      fs.current = S, t.addNode({
        id: S,
        type: "content",
        x: w,
        y: E,
        w: z,
        h: N,
        z: t.nextZ(),
        data: { blocks: [], borderColor: "#1e1e2e" }
      });
    },
    [t]
  ), ur = it(
    (w, E, z) => {
      const { x: N, y: S } = t.screenToCanvas(w, E);
      if (z) {
        const O = t.hitTestAll(N, S, rt);
        if (O.length > 0) {
          const A = v.current, V = Math.abs(N - A.x) + Math.abs(S - A.y);
          let U = 0;
          V < 5 && (U = (A.index + 1) % O.length), v.current = { x: N, y: S, index: U }, t.select(O[U].id);
        } else
          t.deselectAll();
      } else {
        let O = !1;
        for (const A of t.selection) {
          const V = t.getNode(A);
          if (!V) continue;
          const U = V.h === "auto" ? 100 : V.h;
          if (N >= V.x && N <= V.x + V.w && S >= V.y && S <= V.y + U) {
            O = !0;
            break;
          }
        }
        if (!O && t.selection.size >= 2) {
          let A = 1 / 0, V = 1 / 0, U = -1 / 0, J = -1 / 0;
          for (const dt of t.selection) {
            const ct = t.getNode(dt);
            if (!ct || ct.type === "edge") continue;
            const kt = ct.h === "auto" ? 100 : ct.h;
            A = Math.min(A, ct.x), V = Math.min(V, ct.y), U = Math.max(U, ct.x + ct.w), J = Math.max(J, ct.y + kt);
          }
          A !== 1 / 0 && N >= A && N <= U && S >= V && S <= J && (O = !0);
        }
        if (!O) {
          const A = t.hitTest(N, S, rt);
          A ? t.select(A.id) : t.deselectAll();
        }
      }
      const F = Array.from(t.selection), R = F.length > 0, H = [];
      if (H.push({
        items: [
          {
            label: "Cut",
            shortcut: "Mod+X",
            disabled: !R,
            action: () => {
              t.cutSelected(), ri(t);
            }
          },
          {
            label: "Copy",
            shortcut: "Mod+C",
            disabled: !R,
            action: () => {
              t.copySelected(), ri(t);
            }
          },
          {
            label: "Paste",
            shortcut: "Mod+V",
            disabled: !1,
            action: () => {
              Xh(t, N, S);
            }
          }
        ]
      }), H.push({
        items: [
          {
            label: "Duplicate",
            shortcut: "Mod+D",
            disabled: !R,
            action: () => t.duplicateSelected()
          }
        ]
      }), R && H.push({
        items: [
          {
            label: "Add to Personal Library",
            action: () => {
              const O = F.map((U) => t.getNode(U)).filter((U) => !!U).map((U) => structuredClone(U)), A = new Set(
                O.map((U) => U.groupId).filter(Boolean)
              ), V = /* @__PURE__ */ new Map();
              for (const [U, J] of t.groupParent)
                A.has(U) && V.set(U, J);
              Kr({
                nodes: O,
                groupParent: V
              });
            }
          }
        ]
      }), F.length >= 2 || R && t.selectionHasGroup()) {
        const O = [];
        F.length >= 2 && O.push({
          label: "Group selection",
          shortcut: "Mod+G",
          action: () => t.groupSelected()
        }), t.selectionHasGroup() && O.push({
          label: "Ungroup selection",
          shortcut: "Mod+Shift+G",
          action: () => t.ungroupSelected()
        }), H.push({ items: O });
      }
      if (R && F.every((A) => {
        const V = t.getNode(A);
        return V && (V.type === "draw" || V.type === "shape");
      }) && H.push({
        items: [
          {
            label: "Flip horizontal",
            shortcut: "Shift+H",
            action: () => t.flipSelectedHorizontal()
          },
          {
            label: "Flip vertical",
            shortcut: "Shift+V",
            action: () => t.flipSelectedVertical()
          }
        ]
      }), R && H.push({
        items: [
          {
            label: "Bring forward",
            shortcut: "Mod+]",
            action: () => t.bringForward(F)
          },
          {
            label: "Send backward",
            shortcut: "Mod+[",
            action: () => t.sendBackward(F)
          },
          {
            label: "Bring to front",
            shortcut: "Mod+Alt+]",
            action: () => t.bringToFront(F)
          },
          {
            label: "Send to back",
            shortcut: "Mod+Alt+[",
            action: () => t.sendToBack(F)
          }
        ]
      }), R) {
        const O = F.some((U) => {
          var J;
          return (J = t.getNode(U)) == null ? void 0 : J.locked;
        }), A = F.some((U) => {
          var J;
          return !((J = t.getNode(U)) != null && J.locked);
        }), V = [];
        A && V.push({
          label: "Lock",
          action: () => {
            for (const U of F) t.updateNode(U, { locked: !0 });
          }
        }), O && V.push({
          label: "Unlock",
          action: () => {
            for (const U of F) t.updateNode(U, { locked: void 0 });
          }
        }), H.push({ items: V });
      }
      R && H.push({
        items: [
          {
            label: "Delete",
            shortcut: "Delete",
            danger: !0,
            action: () => t.deleteSelected()
          }
        ]
      });
      const W = [10, 20, 40, 80];
      return H.push({
        items: [
          {
            label: "Toggle Grid",
            checked: t.snapToGrid,
            action: () => {
              t.toggleSnapToGrid(), C(t.snapToGrid);
            }
          },
          {
            label: "Smart Guides",
            checked: t.smartGuides,
            action: () => {
              t.toggleSmartGuides(), T(t.smartGuides);
            }
          },
          ...W.map((O) => ({
            label: `${O}px`,
            checked: t.gridSize === O,
            action: () => {
              t.setGridSize(O);
            }
          }))
        ]
      }), H.push({
        items: [
          {
            label: "Export as PNG",
            action: () => $s(t, { format: "png" })
          },
          {
            label: "Export as SVG",
            action: () => $s(t, { format: "svg" })
          }
        ]
      }), H;
    },
    [t]
  ), Ca = it(
    (w) => {
      if (w.preventDefault(), t.presentationMode) return;
      const E = ur(w.clientX, w.clientY, w.altKey);
      Ce({ x: w.clientX, y: w.clientY, sections: E });
    },
    [t, ur]
  ), Uo = it(
    (w, E, z) => {
      const N = gt(10);
      t.addNode({
        id: N,
        type: "text",
        x: w,
        y: E,
        w: z,
        h: "auto",
        z: t.nextZ(),
        data: {
          text: "",
          fontSize: t.activeTool.fontSize ?? 20,
          fontFamily: t.activeTool.fontFamily ?? Ze,
          color: t.activeTool.color,
          align: t.activeTool.textAlign ?? "left",
          opacity: t.activeTool.opacity
        }
      }), t.select(N), Gr.current = N, ge(N);
    },
    [t]
  ), za = it(
    (w) => {
      if (t.presentationMode) return;
      if (t.mode === "text" && Zo.current) {
        Zo.current = !1, n.current && (n.current.style.cursor = "text"), t.deselectAll();
        const { x: F, y: R } = t.screenToCanvas(w.clientX, w.clientY);
        Uo(F, R, 300);
        return;
      }
      if (t.mode !== "select") return;
      const { x: E, y: z } = t.screenToCanvas(w.clientX, w.clientY), N = t.hitTestAll(E, z, rt), S = N.find((F) => !t.isContainerType(F.type)) ?? N[0] ?? null;
      if (S != null && S.groupId) {
        const F = [];
        let R = S.groupId;
        for (; R; )
          F.push(R), R = t.groupParent.get(R);
        if (!t.activeGroupId) {
          t.enterGroup(F[F.length - 1]), t.select(S.id);
          return;
        }
        const H = F.indexOf(t.activeGroupId);
        if (H > 0) {
          t.enterGroup(F[H - 1]), t.select(S.id);
          return;
        }
      }
      if (S && S.type === "text") {
        t.select(S.id), Vo.current = { clientX: w.clientX, clientY: w.clientY }, ge(S.id);
        return;
      }
      if (S && S.type === "sticky") {
        t.select(S.id), so(S.id);
        return;
      }
      if (S && S.type === "frame") {
        t.select(S.id), Ee(S.id);
        return;
      }
      if (S && S.type === "shape") {
        const F = S.data, R = F.shape === "line" || F.shape === "arrow";
        t.select(S.id), R || wo(S.id);
        return;
      }
      if (S && S.type === "draw") {
        t.select(S.id);
        return;
      }
      if (!S || S.type === "draw") {
        const R = t.getAllNodes().filter((H) => H.type === "shape").sort((H, W) => W.z - H.z).find((H) => !(H.data.shape === "line" || H.data.shape === "arrow") && Fr(H, E, z, t.viewport.zoom, !0));
        if (R) {
          t.select(R.id), wo(R.id);
          return;
        }
      }
      S || (t.deselectAll(), Uo(E, z, 300));
    },
    [t, rt, Uo]
  ), Ia = it(
    (w) => {
      if (Z.current.set(w.pointerId, { x: w.clientX, y: w.clientY }), w.pointerType === "pen" && (L.current = !0), w.pointerType === "touch" && (Z.current.size >= 2 || L.current)) {
        Q.current = !0, X.current && (clearTimeout(X.current), X.current = null, _.current = null);
        const S = new Map(Z.current), F = [...Z.current.keys()].find((A) => A !== w.pointerId);
        F !== void 0 && s().dispatchEvent(
          new PointerEvent("pointerup", {
            pointerId: F,
            bubbles: !0,
            clientX: w.clientX,
            clientY: w.clientY
          })
        );
        const R = [...S.values()];
        let H = ni(R[0], R[1] ?? R[0]);
        const W = (A) => {
          if (!S.has(A.pointerId)) return;
          S.set(A.pointerId, { x: A.clientX, y: A.clientY });
          const V = [...S.values()];
          if (V.length < 2) return;
          const U = ni(V[0], V[1]);
          if (t.pan(U.mx - H.mx, U.my - H.my), H.dist > 1) {
            const J = Math.min(Math.max(U.dist / H.dist, 0.9), 1.1);
            t.zoomByFactor(J, U.mx, U.my);
          }
          H = U;
        }, O = (A) => {
          Z.current.delete(A.pointerId), S.delete(A.pointerId), A.pointerType === "pen" && (L.current = !1), S.size < 2 && !L.current && (Q.current = !1, s().removeEventListener("pointermove", W), s().removeEventListener("pointerup", O), s().removeEventListener("pointercancel", O));
        };
        s().addEventListener("pointermove", W), s().addEventListener("pointerup", O), s().addEventListener("pointercancel", O);
        return;
      }
      if (Q.current || t.presentationMode && !(w.button === 1 || w.button === 0 && B.current))
        return;
      if (pe && Ce(null), w.pointerType === "touch") {
        const S = w.clientX, F = w.clientY, R = w.pointerId;
        _.current = { clientX: S, clientY: F }, X.current = setTimeout(() => {
          if (X.current = null, !_.current || Q.current) return;
          const H = ur(S, F, !1);
          Ce({ x: S, y: F, sections: H }), s().dispatchEvent(
            new PointerEvent("pointerup", {
              pointerId: R,
              bubbles: !0,
              clientX: S,
              clientY: F
            })
          ), _.current = null;
        }, 500);
      }
      if (w.button === 1 || w.button === 0 && B.current) {
        w.preventDefault(), et.current = !0;
        const S = t.viewport.x, F = t.viewport.y, R = w.clientX, H = w.clientY, W = n.current;
        W && (W.style.cursor = "grabbing");
        const O = (V) => {
          t.viewport.x = S + (V.clientX - R), t.viewport.y = F + (V.clientY - H), c({ ...t.viewport });
        }, A = () => {
          et.current = !1, W && (W.style.cursor = B.current ? "grab" : t.lassoSelect ? Io : ""), s().removeEventListener("pointermove", O), s().removeEventListener("pointerup", A);
        };
        s().addEventListener("pointermove", O), s().addEventListener("pointerup", A);
        return;
      }
      const { x: z, y: N } = t.screenToCanvas(w.clientX, w.clientY);
      if (w.pointerType === "touch" && X.current && t.hitTest(z, N, rt) && (clearTimeout(X.current), X.current = null, _.current = null), t.mode === "select") {
        if (w.button !== 0) return;
        if (w.altKey) {
          const R = t.hitTestAll(z, N, rt);
          if (R.length > 0) {
            const H = v.current, W = Math.abs(z - H.x) + Math.abs(N - H.y);
            let O = 0;
            W < 5 && (O = (H.index + 1) % R.length), v.current = { x: z, y: N, index: O }, t.select(R[O].id);
          }
          return;
        }
        let S = !1;
        !t.lassoSelect && t.selection.size >= 2 && _t && z >= _t.x && z <= _t.x + _t.w && N >= _t.y && N <= _t.y + _t.h && (S = !0);
        let F = null;
        if (!t.lassoSelect) {
          const R = t.hitTestAll(z, N, rt);
          F = R.find((H) => t.selection.has(H.id) && !t.isContainerType(H.type)) ?? R.find((H) => !t.isContainerType(H.type)) ?? R[0] ?? null, !F && !S && (F = ql(t.nodes, z, N, t.viewport.zoom, rt, Jt));
        }
        if (F || S) {
          F && (t.activeGroupId && !t.isNodeInActiveGroup(F.id) && t.exitAllGroups(), w.shiftKey ? t.toggleSelect(F.id) : t.selection.has(F.id) || t.select(F.id));
          const R = Array.from(t.selection).filter(
            (vt) => {
              var Ct;
              return !((Ct = t.getNode(vt)) != null && Ct.locked);
            }
          );
          if (R.length === 0) return;
          const H = w.clientX, W = w.clientY, O = /* @__PURE__ */ new Set(), A = /* @__PURE__ */ new Set();
          for (const vt of R) {
            const Ct = t.getNode(vt);
            if (Ct && t.isContainerType(Ct.type)) {
              A.add(vt);
              for (const Ht of t.getFrameDescendantIds(vt))
                t.selection.has(Ht) || O.add(Ht);
            }
          }
          const V = [...R, ...O], U = V.map((vt) => {
            const Ct = t.getNode(vt);
            return { id: vt, x: Ct.x, y: Ct.y };
          }), J = t.selectionGroupId(), dt = J ? t.groupRotations.get(J) : null, ct = dt == null ? void 0 : dt.cx, kt = dt == null ? void 0 : dt.cy;
          He(null);
          let Mt = !1, wt = null, Pt = H, Xt = W, Ot = !1;
          const yt = new Set(V), Et = () => {
            wt = null;
            const vt = (Pt - H) / t.viewport.zoom, Ct = (Xt - W) / t.viewport.zoom, { finalDx: Ht, finalDy: jt } = t.computeDragSnap(
              U,
              yt,
              vt,
              Ct,
              Ot
            ), oe = U.map((re) => ({
              id: re.id,
              patch: { x: re.x + Ht, y: re.y + jt }
            }));
            t.updateMany(oe), dt && J && t.groupRotations.set(J, {
              angle: dt.angle,
              cx: ct + Ht,
              cy: kt + jt
            });
          }, xt = (vt) => {
            const Ct = (vt.clientX - H) / t.viewport.zoom, Ht = (vt.clientY - W) / t.viewport.zoom;
            if (!Mt)
              if (Math.abs(Ct) > 2 || Math.abs(Ht) > 2)
                Mt = !0, t.pushHistorySnapshot();
              else
                return;
            Pt = vt.clientX, Xt = vt.clientY, Ot = vt.metaKey || vt.ctrlKey, wt === null && (wt = requestAnimationFrame(Et));
          }, Gt = () => {
            if (wt !== null && (cancelAnimationFrame(wt), Et()), t.clearAlignGuides(), s().removeEventListener("pointermove", xt), s().removeEventListener("pointerup", Gt), Mt) {
              const vt = R.filter(
                (Ct) => !O.has(Ct)
              );
              vt.length > 0 && t.updateFrameMembership(vt);
            }
          };
          s().addEventListener("pointermove", xt), s().addEventListener("pointerup", Gt);
        } else {
          if (t.activeGroupId) {
            t.exitGroup();
            return;
          }
          w.shiftKey || t.deselectAll();
          const R = new Set(t.selection);
          if (t.lassoSelect) {
            const H = [[z, N]];
            ro([...H]);
            let W = null, O = 0;
            const A = (J = !1) => {
              W = null;
              const dt = J || O % 2 === 0;
              if (O++, dt && H.length >= 3) {
                const kt = Ae(H, t.getAllNodes()).map((wt) => wt.id), Mt = w.shiftKey ? [.../* @__PURE__ */ new Set([...R, ...kt])] : kt;
                (Mt.length !== t.selection.size || Mt.some((wt) => !t.selection.has(wt))) && t.selectMultiple(Mt);
              }
              ro([...H]);
            }, V = (J) => {
              const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY);
              H.push([dt, ct]), W === null && (W = requestAnimationFrame(() => A(!1)));
            }, U = () => {
              W !== null && cancelAnimationFrame(W), A(!0), s().removeEventListener("pointermove", V), s().removeEventListener("pointerup", U), ro(null), t.toggleLassoSelect();
            };
            s().addEventListener("pointermove", V), s().addEventListener("pointerup", U);
          } else {
            const H = { startX: z, startY: N, endX: z, endY: N };
            Me(H);
            let W = null, O = 0;
            const A = (J = !1, dt = !1) => {
              W = null;
              const ct = Math.min(H.startX, H.endX), kt = Math.min(H.startY, H.endY), Mt = Math.abs(H.endX - H.startX), wt = Math.abs(H.endY - H.startY), Pt = dt || J || O % 2 === 0;
              if (O++, Pt) {
                const Ot = he(
                  { x: ct, y: kt, w: Mt, h: wt },
                  t.getAllNodes()
                ).map((Et) => Et.id), yt = w.shiftKey ? [.../* @__PURE__ */ new Set([...R, ...Ot])] : Ot;
                (yt.length !== t.selection.size || yt.some((Et) => !t.selection.has(Et))) && t.selectMultiple(yt);
              }
              Me({ ...H });
            }, V = (J) => {
              const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY);
              H.endX = dt, H.endY = ct, W === null && (W = requestAnimationFrame(() => A(!1)));
            }, U = () => {
              W !== null && cancelAnimationFrame(W), A(!0), s().removeEventListener("pointermove", V), s().removeEventListener("pointerup", U), Me(null);
            };
            s().addEventListener("pointermove", V), s().addEventListener("pointerup", U);
          }
        }
      } else if (t.mode === "text") {
        if (Zo.current) return;
        t.deselectAll();
        const S = z, F = N, R = {
          startX: z,
          startY: N,
          endX: z,
          endY: N
        };
        let H = !1;
        Yt(R);
        const W = (A) => {
          const { x: V, y: U } = t.screenToCanvas(A.clientX, A.clientY);
          R.endX = V, R.endY = U;
          const J = Math.abs(R.endX - R.startX), dt = Math.abs(R.endY - R.startY);
          (J > 10 || dt > 10) && (H = !0), Yt({ ...R });
        }, O = () => {
          s().removeEventListener("pointermove", W), s().removeEventListener("pointerup", O), Yt(null);
          const A = H ? Math.max(Math.abs(R.endX - R.startX), 60) : 300, V = H ? Math.min(R.startX, R.endX) : S, U = H ? Math.min(R.startY, R.endY) : F;
          Uo(V, U, A), Zo.current = !0, n.current && (n.current.style.cursor = "crosshair");
        };
        s().addEventListener("pointermove", W), s().addEventListener("pointerup", O);
      } else if (t.mode === "note") {
        t.deselectAll();
        const S = z, F = N, R = {
          startX: z,
          startY: N,
          endX: z,
          endY: N
        };
        let H = !1;
        Yt(R);
        const W = (A) => {
          const { x: V, y: U } = t.screenToCanvas(A.clientX, A.clientY);
          R.endX = V, R.endY = U;
          const J = Math.abs(R.endX - R.startX), dt = Math.abs(R.endY - R.startY);
          (J > 10 || dt > 10) && (H = !0), Yt({ ...R });
        }, O = () => {
          s().removeEventListener("pointermove", W), s().removeEventListener("pointerup", O), Yt(null);
          const A = H ? Math.max(Math.abs(R.endX - R.startX), 100) : 300, V = H ? Math.max(Math.abs(R.endY - R.startY), 40) : "auto", U = H ? Math.min(R.startX, R.endX) : S, J = H ? Math.min(R.startY, R.endY) : F;
          gs(U, J, A, V), t.setMode("select");
        };
        s().addEventListener("pointermove", W), s().addEventListener("pointerup", O);
      } else if (t.mode === "sticky") {
        t.deselectAll();
        const S = z, F = N, R = {
          startX: z,
          startY: N,
          endX: z,
          endY: N
        };
        let H = !1;
        Yt(R);
        const W = (A) => {
          const { x: V, y: U } = t.screenToCanvas(A.clientX, A.clientY);
          R.endX = V, R.endY = U, Math.abs(R.endX - R.startX) > 10 && (H = !0), Yt({ ...R });
        }, O = () => {
          s().removeEventListener("pointermove", W), s().removeEventListener("pointerup", O), Yt(null);
          const A = H ? Math.max(Math.abs(R.endX - R.startX), 100) : 200, V = H ? Math.min(R.startX, R.endX) : S, U = H ? Math.min(R.startY, R.endY) : F, J = gt(10), dt = H ? Math.max(Math.abs(R.endY - R.startY), 100) : 150;
          t.addNode({
            id: J,
            type: "sticky",
            x: V,
            y: U,
            w: A,
            h: dt,
            z: t.nextZ(),
            data: { text: "", color: "#FEF3C7" }
          }), t.select(J), so(J), t.setMode("select");
        };
        s().addEventListener("pointermove", W), s().addEventListener("pointerup", O);
      } else if (t.mode === "draw") {
        const S = w.pressure || 0.5, F = {
          points: [[z, N, S]],
          color: t.activeTool.color,
          width: t.activeTool.width,
          strokeStyle: t.activeTool.strokeStyle
        };
        q(F), t.notifyDrawProgress(F);
        const R = (W) => {
          const { x: O, y: A } = t.screenToCanvas(W.clientX, W.clientY), V = W.pressure || 0.5;
          F.points.push([O, A, V]), q({ ...F, points: [...F.points] }), t.notifyDrawProgress({ ...F, points: [...F.points] });
        }, H = () => {
          if (s().removeEventListener("pointermove", R), s().removeEventListener("pointerup", H), t.notifyDrawEnd(), F.points.length < 2) {
            q(null);
            return;
          }
          let W = 1 / 0, O = 1 / 0, A = -1 / 0, V = -1 / 0;
          for (const [J, dt] of F.points)
            J < W && (W = J), dt < O && (O = dt), J > A && (A = J), dt > V && (V = dt);
          const U = F.points.map(
            ([J, dt, ct]) => [J - W, dt - O, ct]
          );
          t.addNode({
            id: gt(10),
            type: "draw",
            x: W,
            y: O,
            w: A - W,
            h: V - O,
            z: t.nextZ(),
            data: {
              tool: "pen",
              points: U,
              color: F.color,
              strokeWidth: F.width,
              opacity: t.activeTool.opacity,
              fill: t.activeTool.fillColor || void 0,
              fillStyle: t.activeTool.fillStyle || void 0,
              strokeStyle: t.activeTool.strokeStyle || void 0
            }
          }), requestAnimationFrame(() => q(null));
        };
        s().addEventListener("pointermove", R), s().addEventListener("pointerup", H);
      } else if (t.mode === "shape") {
        const S = {
          startX: z,
          startY: N,
          endX: z,
          endY: N
        };
        $(S);
        const F = {
          shapeType: t.activeTool.shapeType || "rect",
          stroke: t.activeTool.color,
          strokeWidth: t.activeTool.width
        }, R = (W) => {
          const { x: O, y: A } = t.screenToCanvas(W.clientX, W.clientY);
          S.endX = O, S.endY = A, $({ ...S }), t.notifyShapeProgress({ ...S, ...F });
        }, H = () => {
          s().removeEventListener("pointermove", R), s().removeEventListener("pointerup", H), t.notifyShapeEnd();
          const W = t.activeTool.shapeType || "rect", O = W === "line" || W === "arrow", A = Math.min(S.startX, S.endX);
          let V = Math.min(S.startY, S.endY);
          const U = Math.abs(S.endX - S.startX), J = Math.abs(S.endY - S.startY);
          let dt;
          if (O) {
            const Mt = t.activeTool.width * 2;
            dt = Math.max(J, Mt), J < Mt && (V -= (Mt - J) / 2);
          } else
            dt = J;
          if (U < 5 && (O ? U < 5 && Math.abs(S.endY - S.startY) < 5 : dt < 5)) {
            $(null);
            return;
          }
          const ct = {};
          O && (ct.startPoint = [
            S.startX - A,
            S.startY - V
          ], ct.endPoint = [
            S.endX - A,
            S.endY - V
          ]);
          const kt = gt(10);
          t.addNode({
            id: kt,
            type: "shape",
            x: A,
            y: V,
            w: U,
            h: dt,
            z: t.nextZ(),
            data: {
              shape: W,
              stroke: t.activeTool.color,
              fill: t.activeTool.fillColor || void 0,
              fillStyle: t.activeTool.fillStyle,
              strokeWidth: t.activeTool.width,
              strokeStyle: t.activeTool.strokeStyle,
              roughness: t.activeTool.roughness ?? 1,
              opacity: t.activeTool.opacity ?? 1,
              ...ct
            }
          }), $(null), t.setMode("select"), t.select(kt);
        };
        s().addEventListener("pointermove", R), s().addEventListener("pointerup", H);
      } else if (t.mode === "edge") {
        const S = t.hitTest(z, N, rt);
        if (!S || S.type === "edge") return;
        ot({ fromNode: S, cursorX: z, cursorY: N });
        const F = (H) => {
          const { x: W, y: O } = t.screenToCanvas(H.clientX, H.clientY);
          ot(
            (A) => A ? { ...A, cursorX: W, cursorY: O } : null
          );
        }, R = (H) => {
          s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", R), ot(null);
          const { x: W, y: O } = t.screenToCanvas(H.clientX, H.clientY);
          let A = t.hitTest(W, O, rt);
          if (!A || A.type === "edge" || t.isContainerType(A.type)) {
            const ct = 50 / t.viewport.zoom;
            let kt = 1 / 0, Mt = !1, wt = null;
            for (const Pt of t.getAllNodes()) {
              if (Pt.type === "edge" || Pt.id === S.id) continue;
              const Xt = t.isContainerType(Pt.type), Ot = Fo(Pt, rt);
              for (const yt of Ot) {
                const Et = Math.hypot(yt.x - W, yt.y - O);
                Et >= ct || Xt && !Mt && wt || (!Xt && Mt || Et < kt) && (kt = Et, Mt = Xt, wt = Pt);
              }
            }
            wt && (A = wt);
          }
          if (!A || A.type === "edge" || A.id === S.id || t.getAllNodes().some(
            (ct) => ct.type === "edge" && (ct.data.fromId === S.id && ct.data.toId === A.id || ct.data.fromId === A.id && ct.data.toId === S.id)
          )) return;
          const U = yr(S, z, N, rt), J = yr(A, W, O, rt), dt = {
            id: gt(10),
            type: "edge",
            x: 0,
            y: 0,
            w: 0,
            h: 0,
            z: t.nextZ(),
            data: {
              fromId: S.id,
              toId: A.id,
              style: "solid",
              color: t.activeTool.color,
              strokeWidth: 2,
              arrowHead: "arrow",
              arrowTail: "none",
              edgeType: "bezier",
              sourceHandle: U,
              targetHandle: J
            }
          };
          t.addNode(dt), t.select(dt.id);
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", R);
      } else if (t.mode === "frame") {
        const S = {
          startX: z,
          startY: N,
          endX: z,
          endY: N
        };
        $(S);
        const F = (H) => {
          const { x: W, y: O } = t.screenToCanvas(H.clientX, H.clientY);
          S.endX = W, S.endY = O, $({ ...S });
        }, R = () => {
          s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", R);
          const H = Math.min(S.startX, S.endX), W = Math.min(S.startY, S.endY), O = Math.abs(S.endX - S.startX), A = Math.abs(S.endY - S.startY);
          if (O < 20 || A < 20) {
            $(null);
            return;
          }
          const V = gt(10);
          t.addNode({
            id: V,
            type: "frame",
            x: H,
            y: W,
            w: O,
            h: A,
            z: t.nextZ(),
            data: {
              label: "Frame",
              backgroundColor: "rgba(0, 0, 0, 0.02)",
              borderColor: "#1e1e2e",
              borderWidth: 1,
              borderStyle: "dashed"
            }
          }), t.adoptNodesIntoNewFrame(V), $(null), t.select(V), t.setMode("select");
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", R);
      } else if (t.mode === "erase") {
        if (w.button !== 0) return;
        const S = (ct, kt) => {
          const Mt = t.hitTestAll(ct, kt, rt), wt = Kl(
            t.nodes,
            ct,
            kt,
            t.viewport.zoom,
            rt,
            Jt
          );
          let Pt = !1;
          for (const Xt of [...Mt, ...wt])
            io.current.has(Xt.id) || (io.current.add(Xt.id), Pt = !0);
          Pt && Vr(new Set(io.current));
        }, F = 400;
        io.current = /* @__PURE__ */ new Set();
        const R = performance.now();
        ze.current = [[z, N, R]], qo([[z, N, R]]), S(z, N);
        let H = z, W = N;
        const O = () => {
          const ct = performance.now(), kt = ze.current.length;
          ze.current = ze.current.filter(
            (Mt) => ct - Mt[2] < F
          ), ze.current.length !== kt && qo([...ze.current]), Oe.current = requestAnimationFrame(O);
        };
        Oe.current = requestAnimationFrame(O);
        const A = (ct) => {
          const { x: kt, y: Mt } = t.screenToCanvas(ct.clientX, ct.clientY);
          H = kt, W = Mt;
          const wt = performance.now();
          ze.current.push([H, W, wt]), qo([...ze.current]), S(H, W);
        }, V = () => {
          Oe.current !== null && (cancelAnimationFrame(Oe.current), Oe.current = null), io.current = /* @__PURE__ */ new Set(), Vr(/* @__PURE__ */ new Set()), ze.current = [], qo([]);
        }, U = () => {
          dt();
          const ct = Array.from(io.current);
          V(), ct.length > 0 && t.deleteNodes(ct);
        }, J = (ct) => {
          ct.key === "Escape" && (dt(), V());
        }, dt = () => {
          s().removeEventListener("pointermove", A), s().removeEventListener("pointerup", U), s().removeEventListener("keydown", J);
        };
        s().addEventListener("pointermove", A), s().addEventListener("pointerup", U), s().addEventListener("keydown", J);
      } else if (t.mode === "laser") {
        if (w.button !== 0) return;
        const S = 1560;
        vo.current !== null && (cancelAnimationFrame(vo.current), vo.current = null);
        const F = performance.now();
        ae.current.length > 0 && ae.current.push([NaN, NaN, F]), ae.current.push([z, N, F]), hr([...ae.current]), t.notifyLaserProgress([[z, N]]);
        let R = F;
        const H = () => {
          const A = performance.now(), V = ae.current.length;
          ae.current = ae.current.filter(
            (U) => A - U[2] < S
          ), (ae.current.length !== V || ae.current.length > 0) && hr([...ae.current]), A - R >= 60 && (R = A, ae.current.length > 0 && t.notifyLaserProgress(
            ae.current.map((U) => [U[0], U[1]])
          )), ae.current.length > 0 ? vo.current = requestAnimationFrame(H) : (vo.current = null, hr([]), t.notifyLaserEnd());
        };
        vo.current = requestAnimationFrame(H);
        const W = (A) => {
          const { x: V, y: U } = t.screenToCanvas(A.clientX, A.clientY), J = performance.now();
          ae.current.push([V, U, J]), hr([...ae.current]), t.notifyLaserProgress(
            ae.current.map((dt) => [dt[0], dt[1]])
          );
        }, O = () => {
          s().removeEventListener("pointermove", W), s().removeEventListener("pointerup", O);
        };
        s().addEventListener("pointermove", W), s().addEventListener("pointerup", O);
      } else if (t.mode === "hand") {
        if (w.button !== 0) return;
        w.preventDefault();
        const S = t.viewport.x, F = t.viewport.y, R = w.clientX, H = w.clientY, W = n.current;
        W && (W.style.cursor = "grabbing");
        const O = (V) => {
          t.viewport.x = S + (V.clientX - R), t.viewport.y = F + (V.clientY - H), c({ ...t.viewport });
        }, A = () => {
          W && (W.style.cursor = t.lassoSelect ? Io : kr(t.mode)), s().removeEventListener("pointermove", O), s().removeEventListener("pointerup", A);
        };
        s().addEventListener("pointermove", O), s().addEventListener("pointerup", A);
      }
    },
    [
      t,
      gs,
      Uo,
      pe,
      ur,
      _t,
      rt,
      Zt,
      he
    ]
  ), qr = it(
    (w, E, z) => {
      if (z.preventDefault(), t.presentationMode) return;
      const N = t.getNode(w);
      if (!N || N.locked) return;
      const S = z.clientX, F = z.clientY, R = N.x, H = N.y, W = N.w, O = N.h === "auto", A = O ? rt[w] ?? 100 : N.h, V = N.type === "draw" ? N.data.points.map(
        (Mt) => [...Mt]
      ) : null, U = N.type === "shape" ? N.data.startPoint : void 0, J = N.type === "shape" ? N.data.endPoint : void 0, dt = N.type === "text" ? N.data.fontSize : 0;
      t.pushHistorySnapshot();
      const ct = (Mt) => {
        const wt = (Mt.clientX - S) / t.viewport.zoom, Pt = (Mt.clientY - F) / t.viewport.zoom;
        let Xt = R, Ot = H, yt = W, Et = A;
        if ((E === "nw" || E === "w" || E === "sw") && (Xt = R + wt, yt = W - wt), (E === "ne" || E === "e" || E === "se") && (yt = W + wt), (E === "nw" || E === "n" || E === "ne") && (Ot = H + Pt, Et = A - Pt), (E === "sw" || E === "s" || E === "se") && (Et = A + Pt), t.snapToGrid && !(Mt.metaKey || Mt.ctrlKey)) {
          const vt = t.gridSize, Ct = (Ht) => Math.round(Ht / vt) * vt;
          (E === "nw" || E === "w" || E === "sw") && (Xt = Ct(Xt), yt = R + W - Xt), (E === "ne" || E === "e" || E === "se") && (yt = Ct(Xt + yt) - Xt), (E === "nw" || E === "n" || E === "ne") && (Ot = Ct(Ot), Et = H + A - Ot), (E === "sw" || E === "s" || E === "se") && (Et = Ct(Ot + Et) - Ot);
        }
        const xt = 10;
        if (yt < xt && (yt = xt, (E === "nw" || E === "w" || E === "sw") && (Xt = R + W - xt)), Et < xt && (Et = xt, (E === "nw" || E === "n" || E === "ne") && (Ot = H + A - xt)), N.type === "frame") {
          const vt = N.data.devicePreset;
          if (vt) {
            const Ct = Dn(vt);
            if (Ct) {
              const Ht = ka(Ct);
              if (E === "nw" || E === "ne" || E === "sw" || E === "se" || (E === "e" || E === "w")) {
                const re = Math.round(yt / Ht);
                (E === "nw" || E === "ne") && (Ot = H + A - re), Et = re;
              } else
                yt = Math.round(Et * Ht);
            }
          }
        }
        const Gt = {
          x: Xt,
          y: Ot,
          w: yt,
          h: O ? "auto" : Et
        };
        if (V && N.type === "draw") {
          const vt = W > 0 ? yt / W : 1, Ct = A > 0 ? Et / A : 1, Ht = V.map(
            ([jt, oe, re]) => [jt * vt, oe * Ct, re]
          );
          Gt.data = { ...N.data, points: Ht };
        }
        if (N.type === "shape" && (U || J)) {
          const vt = W > 0 ? yt / W : 1, Ct = A > 0 ? Et / A : 1, Ht = { ...N.data };
          U && (Ht.startPoint = [
            U[0] * vt,
            U[1] * Ct
          ]), J && (Ht.endPoint = [
            J[0] * vt,
            J[1] * Ct
          ]), Gt.data = Ht;
        }
        if (N.type === "text" && dt > 0 && E !== "e" && E !== "w") {
          const vt = E === "n" || E === "s" ? A > 0 ? Et / A : 1 : W > 0 ? yt / W : 1, Ct = Math.max(8, Math.round(dt * vt));
          Gt.data = { ...N.data, fontSize: Ct };
        }
        t.updateNode(w, Gt);
      }, kt = () => {
        s().removeEventListener("pointermove", ct), s().removeEventListener("pointerup", kt), t.isContainerType(N.type) && t.syncFrameChildrenAfterResize(w);
      };
      s().addEventListener("pointermove", ct), s().addEventListener("pointerup", kt);
    },
    [t, rt]
  ), Ta = it(
    (w, E) => {
      E.stopPropagation(), E.preventDefault();
      const z = t.getNode(w);
      if (!z || z.locked) return;
      const N = z.h === "auto" ? rt[w] ?? 100 : z.h, S = z.x + z.w / 2, F = z.y + N / 2, R = z.rotation || 0, { x: H, y: W } = t.screenToCanvas(
        E.clientX,
        E.clientY
      ), O = Math.atan2(W - F, H - S);
      t.pushHistorySnapshot();
      const A = (U) => {
        const { x: J, y: dt } = t.screenToCanvas(U.clientX, U.clientY), ct = Math.atan2(dt - F, J - S);
        let kt = R + (ct - O) * (180 / Math.PI);
        (U.shiftKey || t.snapToGrid) && !(U.metaKey || U.ctrlKey) && (kt = Math.round(kt / 15) * 15), t.updateNode(w, { rotation: kt });
      }, V = () => {
        s().removeEventListener("pointermove", A), s().removeEventListener("pointerup", V);
      };
      s().addEventListener("pointermove", A), s().addEventListener("pointerup", V);
    },
    [t, rt]
  ), bs = it(
    (w, E, z) => {
      z.stopPropagation(), z.preventDefault();
      const N = t.getNode(w);
      if (!N) return;
      const { x: S, y: F } = t.screenToCanvas(z.clientX, z.clientY);
      ot({ fromNode: N, cursorX: S, cursorY: F, sourceHandle: E });
      const R = (W) => {
        const { x: O, y: A } = t.screenToCanvas(W.clientX, W.clientY);
        ot(
          (V) => V ? { ...V, cursorX: O, cursorY: A } : null
        );
      }, H = (W) => {
        s().removeEventListener("pointermove", R), s().removeEventListener("pointerup", H), ot(null);
        const { x: O, y: A } = t.screenToCanvas(W.clientX, W.clientY);
        let V = t.hitTest(O, A, rt);
        if (!V || V.type === "edge" || t.isContainerType(V.type)) {
          const ct = 50 / t.viewport.zoom;
          let kt = 1 / 0, Mt = !1, wt = null;
          for (const Pt of t.getAllNodes()) {
            if (Pt.type === "edge" || Pt.id === N.id) continue;
            const Xt = t.isContainerType(Pt.type), Ot = Fo(Pt, rt);
            for (const yt of Ot) {
              const Et = Math.hypot(yt.x - O, yt.y - A);
              Et >= ct || Xt && !Mt && wt || (!Xt && Mt || Et < kt) && (kt = Et, Mt = Xt, wt = Pt);
            }
          }
          wt && (V = wt);
        }
        if (!V || V.type === "edge" || V.id === N.id || t.getAllNodes().some(
          (ct) => ct.type === "edge" && (ct.data.fromId === N.id && ct.data.toId === V.id || ct.data.fromId === V.id && ct.data.toId === N.id)
        )) return;
        const J = yr(V, O, A, rt), dt = {
          id: gt(10),
          type: "edge",
          x: 0,
          y: 0,
          w: 0,
          h: 0,
          z: t.nextZ(),
          data: {
            fromId: N.id,
            toId: V.id,
            style: "solid",
            color: t.activeTool.color,
            strokeWidth: 2,
            arrowHead: "arrow",
            arrowTail: "none",
            edgeType: "bezier",
            sourceHandle: E,
            targetHandle: J
          }
        };
        t.addNode(dt), t.select(dt.id);
      };
      s().addEventListener("pointermove", R), s().addEventListener("pointerup", H);
    },
    [t, rt]
  ), Pa = it(
    (w) => {
      let E = null, z = w === "top" || w === "left" ? 1 / 0 : -1 / 0;
      for (const N of t.selection) {
        const S = t.getNode(N);
        if (!S || S.type === "edge") continue;
        const F = S.h === "auto" ? rt[S.id] ?? 100 : S.h;
        let R;
        switch (w) {
          case "top":
            R = S.y;
            break;
          case "bottom":
            R = S.y + F;
            break;
          case "left":
            R = S.x;
            break;
          case "right":
            R = S.x + S.w;
            break;
        }
        (w === "top" || w === "left" ? R < z : R > z) && (z = R, E = N);
      }
      return E;
    },
    [t, rt]
  ), Ra = it(
    (w, E, z, N) => {
      var U;
      N.stopPropagation(), N.preventDefault();
      const S = t.getNode(w);
      if (!S || !o) return;
      const F = o.get(S.type), R = (U = F == null ? void 0 : F.ports) == null ? void 0 : U.find((J) => J.id === E);
      if (!R) return;
      const H = z === "input" ? "left" : "right", { x: W, y: O } = t.screenToCanvas(N.clientX, N.clientY);
      ot({
        fromNode: S,
        cursorX: W,
        cursorY: O,
        sourceHandle: H,
        sourcePort: E,
        sourceDirection: z
      });
      const A = (J) => {
        const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY);
        ot(
          (kt) => kt ? { ...kt, cursorX: dt, cursorY: ct } : null
        );
      }, V = (J) => {
        var We;
        s().removeEventListener("pointermove", A), s().removeEventListener("pointerup", V), ot(null);
        const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY), kt = z === "output" ? "input" : "output", Mt = 40 / t.viewport.zoom;
        let wt = null, Pt = null, Xt = 1 / 0;
        for (const ee of t.getAllNodes()) {
          if (ee.type === "edge" || ee.id === S.id) continue;
          const se = o.get(ee.type);
          if (!((We = se == null ? void 0 : se.ports) != null && We.length)) continue;
          const So = ee.h === "auto" ? t.measuredHeights[ee.id] ?? 100 : ee.h;
          for (const we of se.ports) {
            if (we.direction !== kt || R.dataType !== "any" && we.dataType !== "any" && R.dataType !== we.dataType) continue;
            const Mo = se.ports.filter((Ya) => Ya.direction === we.direction), Ur = Mo.indexOf(we), pr = 14 / t.viewport.zoom, Oa = ee.y + So / (Mo.length + 1) * (Ur + 1), Xa = we.direction === "input" ? ee.x - pr : ee.x + ee.w + pr, Qr = Math.hypot(Xa - dt, Oa - ct);
            Qr < Mt && Qr < Xt && (Xt = Qr, wt = ee, Pt = we);
          }
        }
        if (!wt || !Pt) return;
        const Ot = Pt.id, yt = z === "output" ? wt.id : S.id, Et = z === "output" ? Ot : E;
        if (t.getAllNodes().some((ee) => {
          if (ee.type !== "edge") return !1;
          const se = ee.data;
          return se.toId === yt && se.targetPort === Et;
        })) return;
        const Gt = z === "output" ? S.id : wt.id, vt = z === "output" ? wt.id : S.id, Ct = z === "output" ? E : Ot, Ht = z === "output" ? Ot : E, re = {
          id: gt(10),
          type: "edge",
          x: 0,
          y: 0,
          w: 0,
          h: 0,
          z: t.nextZ(),
          data: {
            fromId: Gt,
            toId: vt,
            style: "solid",
            color: "#6b7280",
            strokeWidth: 2,
            arrowHead: "filled",
            arrowTail: "none",
            edgeType: "bezier",
            sourceHandle: "right",
            targetHandle: "left",
            sourcePort: Ct,
            targetPort: Ht
          }
        };
        t.addNode(re), t.select(re.id);
      };
      s().addEventListener("pointermove", A), s().addEventListener("pointerup", V);
    },
    [t, o, rt]
  ), [xs, Aa] = tt(0);
  mt(() => {
    if (r)
      return r.onChange(() => Aa((w) => w + 1));
  }, [r]);
  const La = it(
    (w, E, z, N, S) => {
      S.stopPropagation(), S.preventDefault();
      const F = t.getNode(w);
      if (!F || F.type !== "edge") return;
      t.pushHistorySnapshot();
      const R = (W) => {
        const O = t.screenToCanvas(W.clientX, W.clientY), A = t.getNode(w);
        if (!A) return;
        const V = t.getNode(A.data.fromId), U = t.getNode(A.data.toId);
        if (!(!V || !U))
          if (E === "xy") {
            const J = Ke(
              V,
              U,
              A.data.edgeType || "bezier",
              rt,
              A.data.sourceHandle,
              A.data.targetHandle,
              void 0,
              void 0
              // no offsets → natural midpoint
            );
            if (!J.kinkHandle) return;
            const dt = O.x - J.kinkHandle.x, ct = O.y - J.kinkHandle.y;
            t.updateNode(w, {
              data: { ...A.data, curveOffset: [dt, ct] }
            });
          } else {
            const J = E === "x" ? O.x : O.y, dt = Ke(
              V,
              U,
              A.data.edgeType || "bezier",
              rt,
              A.data.sourceHandle,
              A.data.targetHandle,
              0.5
              // default to get range
            );
            if (!dt.kinkHandle) return;
            const ct = dt.kinkHandle.min, kt = dt.kinkHandle.max, Mt = kt - ct;
            if (Mt === 0) return;
            const Pt = (Math.max(ct, Math.min(kt, J)) - ct) / Mt;
            t.updateNode(w, {
              data: { ...A.data, midpointOffset: Pt }
            });
          }
      }, H = () => {
        s().removeEventListener("pointermove", R), s().removeEventListener("pointerup", H);
      };
      s().addEventListener("pointermove", R), s().addEventListener("pointerup", H);
    },
    [t, rt]
  ), Da = it(
    (w, E, z) => {
      z.stopPropagation(), z.preventDefault();
      const N = t.getNode(w);
      if (!N || N.type !== "edge") return;
      const { fromId: S, toId: F, sourceHandle: R, targetHandle: H } = N.data, W = E === "source" ? F : S, O = E === "source" ? H : R, A = t.getNode(S), V = t.getNode(F);
      if (!A || !V) return;
      const U = Ke(
        A,
        V,
        N.data.edgeType || "bezier",
        rt,
        R,
        H
      ), J = E === "source" ? { x: U.x1, y: U.y1 } : { x: U.x2, y: U.y2 };
      pt({
        edgeId: w,
        endpoint: E,
        anchorNodeId: W,
        anchorHandle: O,
        cursorX: J.x,
        cursorY: J.y
      });
      const dt = (kt) => {
        const { x: Mt, y: wt } = t.screenToCanvas(kt.clientX, kt.clientY);
        pt(
          (Pt) => Pt ? { ...Pt, cursorX: Mt, cursorY: wt } : null
        );
      }, ct = (kt) => {
        s().removeEventListener("pointermove", dt), s().removeEventListener("pointerup", ct), pt(null);
        const { x: Mt, y: wt } = t.screenToCanvas(kt.clientX, kt.clientY);
        let Pt = t.hitTest(Mt, wt, rt);
        if (!Pt || Pt.type === "edge" || t.isContainerType(Pt.type)) {
          const vt = 50 / t.viewport.zoom;
          let Ct = 1 / 0, Ht = !1, jt = null;
          for (const oe of t.getAllNodes()) {
            if (oe.type === "edge") continue;
            const re = t.isContainerType(oe.type), We = Fo(oe, rt);
            for (const ee of We) {
              const se = Math.hypot(ee.x - Mt, ee.y - wt);
              se >= vt || re && !Ht && jt || (!re && Ht || se < Ct) && (Ct = se, Ht = re, jt = oe);
            }
          }
          jt && (Pt = jt);
        }
        if (!Pt || Pt.type === "edge") return;
        const Xt = E === "source" ? Pt.id : S, Ot = E === "target" ? Pt.id : F;
        if (Xt === Ot) return;
        const yt = E === "source" ? S : F;
        if (Pt.id === yt || t.getAllNodes().some((vt) => {
          if (vt.type !== "edge" || vt.id === w) return !1;
          const Ct = vt.data;
          return Ct.fromId === Xt && Ct.toId === Ot || Ct.fromId === Ot && Ct.toId === Xt;
        })) return;
        const xt = yr(Pt, Mt, wt, rt), Gt = E === "source" ? { fromId: Pt.id, sourceHandle: xt } : { toId: Pt.id, targetHandle: xt };
        t.updateNodeWithHistory(w, { data: Gt });
      };
      s().addEventListener("pointermove", dt), s().addEventListener("pointerup", ct);
    },
    [t, rt]
  ), Ea = it(
    (w) => {
      if (w.stopPropagation(), w.preventDefault(), t.presentationMode) return;
      const E = Array.from(t.selection).map((xt) => t.getNode(xt)).filter(Boolean);
      if (E.length < 2) return;
      const N = t.selectionIsSingleGroup() ? t.selectionGroupId() ?? null : null, S = N ? t.groupRotations.get(N) : null;
      let F, R;
      if (S)
        F = S.cx, R = S.cy;
      else {
        let xt = 1 / 0, Gt = 1 / 0, vt = -1 / 0, Ct = -1 / 0;
        for (const Ht of E) {
          const jt = Ht.h === "auto" ? rt[Ht.id] ?? 100 : Ht.h, oe = Zt(Ht, jt);
          xt = Math.min(xt, oe.minX), Gt = Math.min(Gt, oe.minY), vt = Math.max(vt, oe.maxX), Ct = Math.max(Ct, oe.maxY);
        }
        F = (xt + vt) / 2, R = (Gt + Ct) / 2;
      }
      const H = (S == null ? void 0 : S.angle) ?? 0, O = E.filter((xt) => !xt.locked).map((xt) => {
        const Gt = xt.h === "auto" ? rt[xt.id] ?? 100 : xt.h;
        return {
          id: xt.id,
          cx: xt.x + xt.w / 2,
          cy: xt.y + Gt / 2,
          w: xt.w,
          h: Gt,
          rotation: xt.rotation || 0
        };
      }), A = -H * Math.PI / 180, V = Math.cos(A), U = Math.sin(A);
      let J = 1 / 0, dt = 1 / 0, ct = -1 / 0, kt = -1 / 0;
      for (const xt of O) {
        const Gt = xt.cx - F, vt = xt.cy - R, Ct = F + Gt * V - vt * U, Ht = R + Gt * U + vt * V;
        J = Math.min(J, Ct - xt.w / 2), dt = Math.min(dt, Ht - xt.h / 2), ct = Math.max(ct, Ct + xt.w / 2), kt = Math.max(kt, Ht + xt.h / 2);
      }
      const Mt = {
        x: J - $t,
        y: dt - $t,
        w: ct - J + $t * 2,
        h: kt - dt + $t * 2
      }, { x: wt, y: Pt } = t.screenToCanvas(w.clientX, w.clientY), Xt = Math.atan2(Pt - R, wt - F);
      t.pushHistorySnapshot();
      let Ot = H;
      const yt = (xt) => {
        const { x: Gt, y: vt } = t.screenToCanvas(xt.clientX, xt.clientY);
        let Ht = (Math.atan2(vt - R, Gt - F) - Xt) * (180 / Math.PI);
        (xt.shiftKey || t.snapToGrid) && !(xt.metaKey || xt.ctrlKey) && (Ht = Math.round(Ht / 15) * 15), Ot = H + Ht, He({ angle: Ot, cx: F, cy: R, bounds: Mt });
        const jt = Ht * Math.PI / 180, oe = Math.cos(jt), re = Math.sin(jt), We = O.map((ee) => {
          const se = ee.cx - F, So = ee.cy - R, we = F + se * oe - So * re, Mo = R + se * re + So * oe;
          return {
            id: ee.id,
            patch: {
              x: we - ee.w / 2,
              y: Mo - ee.h / 2,
              rotation: Ot
            }
          };
        });
        t.updateMany(We);
      }, Et = () => {
        N && t.groupRotations.set(N, { angle: Ot, cx: F, cy: R }), He({ angle: Ot, cx: F, cy: R, bounds: Mt }), s().removeEventListener("pointermove", yt), s().removeEventListener("pointerup", Et);
      };
      s().addEventListener("pointermove", yt), s().addEventListener("pointerup", Et);
    },
    [t, rt, Zt]
  ), Wa = it(
    (w, E) => {
      if (E.stopPropagation(), E.preventDefault(), t.presentationMode) return;
      const z = Array.from(t.selection).map((yt) => t.getNode(yt)).filter(Boolean);
      if (z.length < 2) return;
      const N = (yt) => yt.h === "auto" ? rt[yt.id] ?? 100 : yt.h;
      let S = 1 / 0, F = 1 / 0, R = -1 / 0, H = -1 / 0;
      for (const yt of z) {
        const Et = N(yt), xt = Zt(yt, Et);
        S = Math.min(S, xt.minX), F = Math.min(F, xt.minY), R = Math.max(R, xt.maxX), H = Math.max(H, xt.maxY);
      }
      const W = { x: S, y: F, w: R - S, h: H - F }, O = W.w || 1, A = W.h || 1, U = z.filter((yt) => !yt.locked).map((yt) => {
        const Et = N(yt);
        return {
          id: yt.id,
          type: yt.type,
          isAutoH: yt.h === "auto",
          relX: (yt.x - W.x) / O,
          relY: (yt.y - W.y) / A,
          relW: yt.w / O,
          relH: Et / A,
          origW: yt.w,
          origH: Et,
          origPoints: yt.type === "draw" ? yt.data.points.map((xt) => [...xt]) : null,
          drawData: yt.type === "draw" ? { ...yt.data } : null
        };
      }), J = E.clientX, dt = E.clientY;
      t.pushHistorySnapshot();
      let ct = null, kt = J, Mt = dt, wt = !1;
      const Pt = () => {
        ct = null;
        const yt = (kt - J) / t.viewport.zoom, Et = (Mt - dt) / t.viewport.zoom;
        let xt = W.x, Gt = W.y, vt = W.w, Ct = W.h;
        if ((w === "nw" || w === "w" || w === "sw") && (xt = W.x + yt, vt = W.w - yt), (w === "ne" || w === "e" || w === "se") && (vt = W.w + yt), (w === "nw" || w === "n" || w === "ne") && (Gt = W.y + Et, Ct = W.h - Et), (w === "sw" || w === "s" || w === "se") && (Ct = W.h + Et), t.snapToGrid && !wt) {
          const jt = t.gridSize, oe = (re) => Math.round(re / jt) * jt;
          (w === "nw" || w === "w" || w === "sw") && (xt = oe(xt), vt = W.x + W.w - xt), (w === "ne" || w === "e" || w === "se") && (vt = oe(xt + vt) - xt), (w === "nw" || w === "n" || w === "ne") && (Gt = oe(Gt), Ct = W.y + W.h - Gt), (w === "sw" || w === "s" || w === "se") && (Ct = oe(Gt + Ct) - Gt);
        }
        vt < 20 && (vt = 20, (w === "nw" || w === "w" || w === "sw") && (xt = W.x + W.w - 20)), Ct < 20 && (Ct = 20, (w === "nw" || w === "n" || w === "ne") && (Gt = W.y + W.h - 20));
        const Ht = U.map((jt) => {
          const oe = xt + jt.relX * vt, re = Gt + jt.relY * Ct, We = jt.relW * vt, ee = jt.relH * Ct, se = {
            x: oe,
            y: re,
            w: We,
            h: jt.isAutoH ? "auto" : ee
          };
          if (jt.origPoints && jt.drawData) {
            const So = jt.origW > 0 ? We / jt.origW : 1, we = jt.origH > 0 ? ee / jt.origH : 1;
            se.data = {
              ...jt.drawData,
              points: jt.origPoints.map(
                ([Mo, Ur, pr]) => [Mo * So, Ur * we, pr]
              )
            };
          }
          return { id: jt.id, patch: se };
        });
        t.updateMany(Ht);
      }, Xt = (yt) => {
        kt = yt.clientX, Mt = yt.clientY, wt = yt.metaKey || yt.ctrlKey, ct === null && (ct = requestAnimationFrame(Pt));
      }, Ot = () => {
        ct !== null && (cancelAnimationFrame(ct), Pt()), s().removeEventListener("pointermove", Xt), s().removeEventListener("pointerup", Ot);
        for (const yt of z)
          t.isContainerType(yt.type) && t.syncFrameChildrenAfterResize(yt.id);
      };
      s().addEventListener("pointermove", Xt), s().addEventListener("pointerup", Ot);
    },
    [t, rt, Zt]
  );
  mt(() => {
    n.current && (n.current.style.cursor = t.lassoSelect ? Io : kr(m)), m !== "select" && m !== "edge" && (De.current = null, xe(null)), m !== "erase" && (Oe.current !== null && (cancelAnimationFrame(Oe.current), Oe.current = null), io.current = /* @__PURE__ */ new Set(), Vr(/* @__PURE__ */ new Set()), ze.current = [], qo([]));
  }, [m]);
  const Zr = lt(null), ws = lt(null), Fa = it(
    (w) => {
      if (X.current && w.pointerType === "touch" && _.current) {
        const E = w.clientX - _.current.clientX, z = w.clientY - _.current.clientY;
        Math.sqrt(E * E + z * z) > 8 && (clearTimeout(X.current), X.current = null, _.current = null);
      }
      t.mode !== "select" && t.mode !== "edge" || (ws.current = { clientX: w.clientX, clientY: w.clientY }, Zr.current === null && (Zr.current = requestAnimationFrame(() => {
        Zr.current = null;
        const E = n.current, z = ws.current;
        if (!E || !z) return;
        const { x: N, y: S } = t.screenToCanvas(z.clientX, z.clientY);
        if (t.lassoSelect) {
          E.style.cursor = Io;
          return;
        }
        if (t.mode === "edge") {
          const H = t.hitTest(N, S, rt), W = H && H.type !== "edge" ? H.id : null;
          W !== De.current && (De.current = W, xe(W));
          return;
        }
        if (t.selection.size >= 2 && _t && N >= _t.x && N <= _t.x + _t.w && S >= _t.y && S <= _t.y + _t.h) {
          E.style.cursor = "move";
          return;
        }
        const F = t.hitTest(N, S, rt), R = F ? F.id : null;
        if (R !== De.current && (De.current = R, xe(R)), F) {
          E.style.cursor = "move";
          return;
        }
        E.style.cursor = "default";
      })));
    },
    [t, _t, rt, Zt]
  ), Ba = it((w) => {
    (w.dataTransfer.types.includes("Files") || w.dataTransfer.types.includes(Pn) || w.dataTransfer.types.includes(Rn) || w.dataTransfer.types.includes(An)) && (w.preventDefault(), w.dataTransfer.dropEffect = "copy");
  }, []), Na = it(
    (w) => {
      if (w.preventDefault(), t.presentationMode) return;
      const E = w.dataTransfer.getData(An);
      if (E) {
        try {
          const W = JSON.parse(E);
          pa(t, W, w.clientX, w.clientY);
        } catch (W) {
          console.error("Failed to place GIF:", W);
        }
        return;
      }
      const z = w.dataTransfer.getData(Rn);
      if (z) {
        try {
          const { itemId: W } = JSON.parse(z), A = la().find((V) => V.id === W);
          A && da(t, A, w.clientX, w.clientY);
        } catch (W) {
          console.error("Failed to place personal library item:", W);
        }
        return;
      }
      const N = w.dataTransfer.getData(Pn);
      if (N) {
        try {
          const { libraryId: W, itemId: O } = JSON.parse(N), V = _n(W).find((U) => U.id === O);
          V && ca(t, V, w.clientX, w.clientY);
        } catch (W) {
          console.error("Failed to place library item:", W);
        }
        return;
      }
      const S = w.dataTransfer.files[0];
      if (!S) return;
      if (S.name.endsWith(".excalidrawlib") || S.name.endsWith(".excalidrawlib.json")) {
        const W = new FileReader();
        W.onload = () => {
          try {
            const O = JSON.parse(W.result);
            if (O.type === "excalidrawlib") {
              const A = S.name.replace(/\.excalidrawlib(\.json)?$/, "");
              ts(O, { name: A });
            }
          } catch (O) {
            console.error("Failed to import library:", O);
          }
        }, W.readAsText(S);
        return;
      }
      if (S.type === "image/svg+xml" || S.name.endsWith(".svg")) {
        const W = new FileReader();
        W.onload = () => {
          const O = W.result, A = Ln(O);
          A && dh(t, A, w.clientX, w.clientY);
        }, W.readAsText(S);
        return;
      }
      if (!S.type.startsWith("image/")) return;
      const { x: F, y: R } = t.screenToCanvas(w.clientX, w.clientY), H = new FileReader();
      H.onload = () => {
        const W = H.result, O = new Image();
        O.onload = () => {
          const A = Math.min(O.naturalWidth, 400), V = Math.min(O.naturalHeight, 300), U = O.naturalWidth / O.naturalHeight, J = U >= 1 ? A : V * U, dt = U >= 1 ? A / U : V;
          t.addNode({
            id: gt(10),
            type: "image",
            x: F,
            y: R,
            w: J,
            h: dt,
            z: t.nextZ(),
            data: { src: W }
          });
        }, O.src = W;
      }, H.readAsDataURL(S);
    },
    [t]
  ), Ha = `translate(${a.x}px, ${a.y}px) scale(${a.zoom})`;
  return /* @__PURE__ */ k(
    "div",
    {
      ref: n,
      "data-sb-canvas": !0,
      style: {
        width: "100%",
        height: "100%",
        overflow: "hidden",
        position: "relative",
        userSelect: "none",
        touchAction: "none",
        background: sr(K).canvasBg
      },
      onPointerDown: Ia,
      onPointerMove: Fa,
      onDoubleClick: za,
      onContextMenu: Ca,
      onDragOver: Ba,
      onDrop: Na,
      children: [
        /* @__PURE__ */ u(Fd, { viewport: a, gridSize: I, background: K, gridVisible: x }),
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              transform: Ha,
              transformOrigin: "0 0",
              pointerEvents: "none"
            },
            children: [
              (eo || h).filter((w) => {
                if (o) {
                  const E = o.get(w.type);
                  return E && !E.isSVGOnly;
                }
                return w.type === "content" || w.type === "draw" || w.type === "shape" || w.type === "image" || w.type === "text" || w.type === "frame" || w.type === "sticky";
              }).sort((w, E) => w.z - E.z).map((w) => {
                var N;
                const E = jr.has(w.id);
                let z;
                if (o) {
                  const S = o.get(w.type);
                  if (S) {
                    const F = S.component, R = d.has(w.id), H = m === "select" || m === "text" || m === "note" || m === "sticky", W = /* @__PURE__ */ u(
                      F,
                      {
                        node: w,
                        data: w.data,
                        isSelected: R,
                        multiSelected: d.size > 1 && R && !t.selectionIsSingleGroup(),
                        engine: t,
                        interactive: H,
                        zoom: a.zoom,
                        editing: ps === w.id,
                        editClickPos: ps === w.id ? Vo.current : null,
                        callbacks: {
                          onMeasuredHeight: zt,
                          onResizeHandleDown: qr,
                          onEditStart: (O) => {
                            const A = t.getNode(O);
                            A && (A.type === "text" ? ge(O) : A.type === "sticky" ? so(O) : A.type === "frame" ? Ee(O) : A.type === "shape" ? wo(O) : A.type === "image" ? ko(O) : A.type === "youtube" && us(O));
                          },
                          onEditEnd: () => {
                            ge(null), so(null), Ee(null), wo(null), ko(null), us(null);
                          }
                        },
                        portValues: r && ((N = S.ports) != null && N.length) && xs >= 0 ? r.getAllPortValues(w.id) : void 0,
                        updateData: (O) => {
                          t.updateNodeWithHistory(w.id, {
                            data: { ...w.data, ...O }
                          });
                        }
                      },
                      S.handlesOwnLayout ? w.id : void 0
                    );
                    S.handlesOwnLayout ? z = W : z = /* @__PURE__ */ u(
                      Yh,
                      {
                        node: w,
                        isInteractive: H,
                        measuredH: rt[w.id],
                        onMeasuredHeight: zt,
                        observeElement: Tt,
                        unobserveElement: Rt,
                        isContainer: S.isContainer,
                        children: W
                      },
                      w.id
                    );
                  }
                } else if (w.type === "content") {
                  const S = w;
                  z = /* @__PURE__ */ u(
                    Li,
                    {
                      node: S,
                      isSelected: d.has(w.id),
                      multiSelected: d.size > 1 && d.has(w.id) && !t.selectionIsSingleGroup(),
                      engine: t,
                      schema: e,
                      interactive: m === "select" || m === "text" || m === "note",
                      zoom: a.zoom,
                      onMeasuredHeight: zt,
                      autoEdit: fs.current === S.id
                    },
                    w.id
                  );
                } else if (w.type === "text")
                  z = /* @__PURE__ */ u(
                    Vi,
                    {
                      node: w,
                      engine: t,
                      editing: no === w.id,
                      editClickPos: no === w.id ? Vo.current : null,
                      onStopEdit: () => {
                        if (Gr.current === w.id) {
                          Gr.current = null;
                          const S = t.getNode(w.id);
                          if (!S || !S.data.text.trim()) {
                            t.deleteNode(w.id), ge(null);
                            return;
                          }
                          t.pushHistorySnapshot();
                        }
                        ge(null);
                      },
                      onMeasuredHeight: zt
                    },
                    w.id
                  );
                else if (w.type === "image")
                  z = /* @__PURE__ */ u(
                    ji,
                    {
                      node: w,
                      isSelected: d.has(w.id),
                      engine: t,
                      interactive: m === "select",
                      zoom: a.zoom,
                      onResizeHandleDown: qr,
                      cropping: hs === w.id,
                      onCropStart: () => ko(w.id),
                      onCropEnd: () => ko(null)
                    },
                    w.id
                  );
                else if (w.type === "sticky")
                  z = /* @__PURE__ */ u(
                    Ki,
                    {
                      node: w,
                      isSelected: d.has(w.id),
                      engine: t,
                      interactive: m === "select" || m === "sticky",
                      zoom: a.zoom,
                      editing: ds === w.id,
                      onEditStart: so,
                      onEditEnd: () => so(null)
                    },
                    w.id
                  );
                else if (w.type === "frame") {
                  const S = w, F = S.h === "auto" ? 100 : S.h;
                  z = /* @__PURE__ */ u(
                    "div",
                    {
                      style: {
                        position: "absolute",
                        left: S.x,
                        top: S.y,
                        width: S.w,
                        height: F,
                        zIndex: S.z,
                        background: S.data.backgroundColor || "rgba(0,0,0,0.02)",
                        border: `${S.data.borderWidth || 1}px ${S.data.borderStyle || "dashed"} ${S.data.borderColor || "#ccc"}`,
                        boxSizing: "border-box",
                        borderRadius: 8,
                        opacity: S.data.opacity ?? 1,
                        pointerEvents: "none",
                        overflow: "visible",
                        transform: S.rotation ? `rotate(${S.rotation}deg)` : void 0,
                        transformOrigin: "center center"
                      },
                      children: cr === w.id ? /* @__PURE__ */ u(
                        "input",
                        {
                          autoFocus: !0,
                          defaultValue: S.data.label ?? "",
                          placeholder: "Frame label...",
                          onBlur: (R) => {
                            const H = R.currentTarget.value.trim();
                            t.updateNodeWithHistory(w.id, {
                              data: { ...S.data, label: H || void 0 }
                            }), Ee(null);
                          },
                          onKeyDown: (R) => {
                            (R.key === "Enter" || R.key === "Escape") && R.currentTarget.blur(), R.stopPropagation();
                          },
                          onPointerDown: (R) => R.stopPropagation(),
                          style: {
                            position: "absolute",
                            top: -24,
                            left: 0,
                            fontSize: 12,
                            color: S.data.borderColor || "#999",
                            fontWeight: 500,
                            background: "rgba(255,255,255,0.95)",
                            border: "1px solid #3b82f6",
                            borderRadius: 4,
                            padding: "1px 4px",
                            outline: "none",
                            pointerEvents: "auto",
                            minWidth: 80
                          }
                        }
                      ) : S.data.label ? /* @__PURE__ */ u(
                        "div",
                        {
                          onDoubleClick: (R) => {
                            R.stopPropagation(), t.select(w.id), Ee(w.id);
                          },
                          style: {
                            position: "absolute",
                            top: -20,
                            left: 4,
                            fontSize: 12,
                            color: S.data.borderColor || "#999",
                            fontWeight: 500,
                            whiteSpace: "nowrap",
                            userSelect: "none",
                            pointerEvents: "auto",
                            cursor: "default"
                          },
                          children: S.data.label
                        }
                      ) : null
                    },
                    w.id
                  );
                } else {
                  const S = w;
                  S.type === "draw" ? z = /* @__PURE__ */ u(Dr, { node: S }, w.id) : z = /* @__PURE__ */ u(Dr, { node: S, editingLabel: Ko === w.id }, w.id);
                }
                return E ? /* @__PURE__ */ u("div", { style: { opacity: 0.25, filter: "saturate(0)" }, children: z }, w.id) : z;
              }),
              Ko && (() => {
                const w = t.getNode(Ko);
                if (!w || w.type !== "shape") return null;
                const E = w.data;
                return E.shape === "line" || E.shape === "arrow" ? null : /* @__PURE__ */ u(
                  Gh,
                  {
                    node: w,
                    engine: t,
                    onDone: () => wo(null)
                  },
                  Ko
                );
              })()
            ]
          }
        ),
        /* @__PURE__ */ u(
          ph,
          {
            nodes: xo,
            viewport: a,
            selection: d,
            measuredHeights: rt,
            activeStroke: j,
            shapePreview: Y,
            shapePreviewStyle: Y ? {
              stroke: t.mode === "frame" ? "#1e1e2e" : t.activeTool.color,
              strokeWidth: t.mode === "frame" ? 1 : t.activeTool.width,
              roughness: t.mode === "frame" ? 0 : t.activeTool.roughness ?? 1,
              shapeType: t.mode === "frame" ? "rect" : t.activeTool.shapeType || "rect"
            } : null,
            onResizeHandleDown: qr,
            onRotateStart: Ta,
            onConnectionHandleDown: bs,
            onEdgeEndpointDown: Da,
            onKinkHandleDown: La,
            edgePreview: nt,
            edgeReconnect: ft,
            eraserMarkedIds: jr.size > 0 ? jr : void 0,
            eraserTrail: ys.length > 1 ? ys : void 0,
            laserTrail: ms.length > 1 ? ms : void 0,
            mode: m,
            hoveredNodeId: ne,
            registry: o,
            onPortHandleDown: Ra,
            cycleNodeIds: r && xs >= 0 ? r.cycleNodeIds : void 0,
            containerTypes: t.containerTypes,
            alignGuides: P
          }
        ),
        _t && (() => {
          const w = t.selectionGroupId(), E = w ? t.groupRotations.get(w) : void 0;
          let z, N, S, F;
          if (be)
            z = be.bounds, N = be.angle, S = be.cx, F = be.cy;
          else if (E && E.angle !== 0) {
            const A = -E.angle * Math.PI / 180, V = Math.cos(A), U = Math.sin(A);
            let J = 1 / 0, dt = 1 / 0, ct = -1 / 0, kt = -1 / 0;
            for (const Mt of t.selection) {
              const wt = t.getNode(Mt);
              if (!wt || wt.type === "edge") continue;
              const Pt = wt.h === "auto" ? rt[wt.id] ?? 100 : wt.h, Xt = wt.x + wt.w / 2, Ot = wt.y + Pt / 2, yt = Xt - E.cx, Et = Ot - E.cy, xt = E.cx + yt * V - Et * U, Gt = E.cy + yt * U + Et * V;
              J = Math.min(J, xt - wt.w / 2), dt = Math.min(dt, Gt - Pt / 2), ct = Math.max(ct, xt + wt.w / 2), kt = Math.max(kt, Gt + Pt / 2);
            }
            z = {
              x: J - $t,
              y: dt - $t,
              w: ct - J + $t * 2,
              h: kt - dt + $t * 2
            }, N = E.angle, S = E.cx, F = E.cy;
          } else
            z = _t, N = 0, S = 0, F = 0;
          const R = 8 / a.zoom, H = R / 2, W = [
            { pos: "nw", cx: z.x, cy: z.y },
            { pos: "n", cx: z.x + z.w / 2, cy: z.y },
            { pos: "ne", cx: z.x + z.w, cy: z.y },
            { pos: "e", cx: z.x + z.w, cy: z.y + z.h / 2 },
            { pos: "se", cx: z.x + z.w, cy: z.y + z.h },
            { pos: "s", cx: z.x + z.w / 2, cy: z.y + z.h },
            { pos: "sw", cx: z.x, cy: z.y + z.h },
            { pos: "w", cx: z.x, cy: z.y + z.h / 2 }
          ], O = N !== 0 ? ` rotate(${N}, ${S}, ${F})` : "";
          return /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ k("g", { transform: O, children: [
                /* @__PURE__ */ u(
                  "rect",
                  {
                    x: z.x,
                    y: z.y,
                    width: z.w,
                    height: z.h,
                    fill: "none",
                    stroke: "#3b82f6",
                    strokeWidth: 1.5 / a.zoom
                  }
                ),
                N === 0 && W.map(({ pos: A, cx: V, cy: U }) => /* @__PURE__ */ u(
                  "rect",
                  {
                    x: V - H,
                    y: U - H,
                    width: R,
                    height: R,
                    fill: "white",
                    stroke: "#3b82f6",
                    strokeWidth: 1.5 / a.zoom,
                    style: { cursor: Br(A, N), pointerEvents: "auto" },
                    onPointerDown: (J) => {
                      J.stopPropagation(), Wa(A, J);
                    }
                  },
                  A
                )),
                (() => {
                  const A = 25 / a.zoom, V = z.x + z.w / 2, U = z.y;
                  return /* @__PURE__ */ k(ut, { children: [
                    /* @__PURE__ */ u(
                      "line",
                      {
                        x1: V,
                        y1: U,
                        x2: V,
                        y2: U - A,
                        stroke: "#3b82f6",
                        strokeWidth: 1.5 / a.zoom,
                        style: { pointerEvents: "none" }
                      }
                    ),
                    (() => {
                      const J = 8 / a.zoom, dt = J / 2;
                      return /* @__PURE__ */ u(
                        "rect",
                        {
                          x: V - dt,
                          y: U - A - dt,
                          width: J,
                          height: J,
                          rx: 1.5 / a.zoom,
                          transform: `rotate(45, ${V}, ${U - A})`,
                          fill: "white",
                          stroke: "#3b82f6",
                          strokeWidth: 1.5 / a.zoom,
                          style: { cursor: "grab", pointerEvents: "auto" },
                          onPointerDown: (ct) => Ea(ct)
                        }
                      );
                    })()
                  ] });
                })(),
                (() => {
                  const A = 26 / a.zoom, V = 42 / a.zoom, U = 4 / a.zoom;
                  return [
                    { side: "top", cx: z.x + z.w / 2, cy: z.y - V },
                    { side: "right", cx: z.x + z.w + A, cy: z.y + z.h / 2 },
                    { side: "bottom", cx: z.x + z.w / 2, cy: z.y + z.h + A },
                    { side: "left", cx: z.x - A, cy: z.y + z.h / 2 }
                  ].map(({ side: dt, cx: ct, cy: kt }) => /* @__PURE__ */ u(
                    "circle",
                    {
                      cx: ct,
                      cy: kt,
                      r: U,
                      fill: "white",
                      stroke: "#94a3b8",
                      strokeWidth: 1.5 / a.zoom,
                      opacity: 0.8,
                      style: { cursor: "crosshair", pointerEvents: "auto" },
                      onPointerDown: (Mt) => {
                        Mt.stopPropagation();
                        const wt = Pa(dt);
                        wt && bs(wt, dt, Mt);
                      }
                    },
                    `conn-${dt}`
                  ));
                })()
              ] }) })
            }
          );
        })(),
        Le && /* @__PURE__ */ u(
          "svg",
          {
            "data-sb-overlay": !0,
            style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
            children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ u(
              "rect",
              {
                x: Le.x,
                y: Le.y,
                width: Le.w,
                height: Le.h,
                fill: "none",
                stroke: "#6366f1",
                strokeWidth: 1.5 / a.zoom,
                strokeDasharray: `${5 / a.zoom} ${3 / a.zoom}`,
                rx: 4 / a.zoom,
                opacity: 0.5
              }
            ) })
          }
        ),
        ue && (() => {
          const w = t.canvasToScreen(ue.startX, ue.startY), E = t.canvasToScreen(ue.endX, ue.endY), z = Math.min(w.x, E.x), N = Math.min(w.y, E.y), S = Math.abs(E.x - w.x), F = Math.abs(E.y - w.y);
          return S < 2 && F < 2 ? null : /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u(
                "rect",
                {
                  x: z,
                  y: N,
                  width: S,
                  height: F,
                  fill: "rgba(59,130,246,0.08)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5,
                  strokeDasharray: "4"
                }
              )
            }
          );
        })(),
        oo && oo.length > 2 && (() => {
          const E = oo.map(([z, N]) => t.canvasToScreen(z, N)).map((z) => `${z.x},${z.y}`).join(" ");
          return /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u(
                "polygon",
                {
                  points: E,
                  fill: "rgba(59,130,246,0.08)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5,
                  strokeDasharray: "4"
                }
              )
            }
          );
        })(),
        st && (() => {
          const w = Math.min(st.startX, st.endX), E = Math.min(st.startY, st.endY), z = Math.abs(st.endX - st.startX), N = Math.abs(st.endY - st.startY);
          return z < 2 && N < 2 ? null : /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ u(
                "rect",
                {
                  x: w,
                  y: E,
                  width: z,
                  height: N,
                  fill: "rgba(59,130,246,0.06)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5 / a.zoom,
                  strokeDasharray: `${4 / a.zoom}`,
                  rx: 8 / a.zoom
                }
              ) })
            }
          );
        })(),
        pe && /* @__PURE__ */ u(
          fh,
          {
            x: pe.x,
            y: pe.y,
            sections: pe.sections,
            onClose: () => Ce(null)
          }
        ),
        dr && /* @__PURE__ */ u(
          lh,
          {
            nodes: dr.nodes,
            onSave: (w) => {
              $d(w, dr.nodes, dr.groupParent), Kr(null);
            },
            onCancel: () => Kr(null)
          }
        )
      ]
    }
  );
}
const ar = 52, _o = 300, Op = ar + _o, Vh = [
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], ns = [
  { key: "hachure", label: "Hachure" },
  { key: "cross-hatch", label: "Cross-hatch" },
  { key: "solid", label: "Solid" }
], Kh = [
  { key: "solid", label: "Solid", dash: "" },
  { key: "dashed", label: "Dashed", dash: "6,3" },
  { key: "dotted", label: "Dotted", dash: "2,2" }
], ss = [
  { value: 0, label: "Architect" },
  { value: 1, label: "Artist" },
  { value: 2, label: "Cartoonist" }
], qh = [1, 2.5, 5, 10, 20], va = [14, 20, 28, 36], is = [
  { key: "left", label: "←" },
  { key: "center", label: "↔" },
  { key: "right", label: "→" }
], Zh = [
  "#FEF3C7",
  "#FCE7F3",
  "#DBEAFE",
  "#D1FAE5",
  "#EDE9FE",
  "#FFEDD5"
], ve = [
  { name: "Standard", colors: Vh },
  { name: "Pastel", colors: ["#F8B4B4", "#FDBA74", "#FDE68A", "#86EFAC", "#93C5FD", "#C4B5FD"] },
  { name: "Earth", colors: ["#78350F", "#92400E", "#6B7280", "#065F46", "#1E3A5F", "#7C2D12"] },
  { name: "Neon", colors: ["#FF1493", "#39FF14", "#00FFFF", "#FF6600", "#FFFF00", "#BF00FF"] },
  { name: "Crayon", colors: ["#EF4444", "#F97316", "#EAB308", "#22C55E", "#3B82F6", "#A855F7"] },
  { name: "Mono", colors: ["#000000", "#404040", "#737373", "#A3A3A3", "#D4D4D4", "#FFFFFF"] }
], as = ve, Uh = [
  { name: "Standard", colors: Zh },
  { name: "Bright", colors: ["#FDE047", "#FB923C", "#F87171", "#4ADE80", "#60A5FA", "#C084FC"] },
  { name: "Earth", colors: ["#D6CFC7", "#E8D5B7", "#C4B5A0", "#B8C5A3", "#A3B5C4", "#C7B8A8"] },
  { name: "Cool", colors: ["#BFDBFE", "#A5F3FC", "#C7D2FE", "#DDD6FE", "#BAE6FD", "#E0E7FF"] }
], Lt = {
  display: "flex",
  alignItems: "center",
  gap: 6
}, Dt = {
  width: 64,
  fontSize: 10,
  flexShrink: 0
}, Ut = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0,
  flexShrink: 0
}, Qh = "https://libraries.excalidraw.com/libraries.json", En = "https://libraries.excalidraw.com/libraries";
function Jh({
  onClose: t,
  onInstalled: e
}) {
  const o = qt(), [r, n] = tt([]), [s, i] = tt(!0), [l, a] = tt(null), [c, h] = tt(""), [p, d] = tt(null), [f, m] = tt(/* @__PURE__ */ new Set()), y = it(() => {
    const x = Ji(), C = new Set(x.map((I) => I.source));
    m(C);
  }, []);
  mt(() => {
    let x = !1;
    return (async () => {
      try {
        const C = await fetch(Qh);
        if (!C.ok) throw new Error(`HTTP ${C.status}`);
        const I = await C.json();
        x || (n(I), i(!1));
      } catch (C) {
        x || (a(String(C)), i(!1));
      }
    })(), y(), () => {
      x = !0;
    };
  }, [y]);
  const g = Kt(() => {
    if (!c.trim()) return r;
    const x = c.toLowerCase();
    return r.filter(
      (C) => {
        var I, M;
        return C.name.toLowerCase().includes(x) || ((I = C.description) == null ? void 0 : I.toLowerCase().includes(x)) || ((M = C.itemNames) == null ? void 0 : M.some((D) => D.toLowerCase().includes(x)));
      }
    );
  }, [r, c]), b = it(
    async (x) => {
      d(x.id);
      try {
        const C = `${En}/${x.source}`;
        await Od(C, x.name), y(), e();
      } catch (C) {
        console.error("Failed to install library:", C);
      } finally {
        d(null);
      }
    },
    [e, y]
  );
  return Ne(
    /* @__PURE__ */ u(
      "div",
      {
        style: {
          position: "fixed",
          inset: 0,
          zIndex: 1e5,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          background: "rgba(0,0,0,0.4)",
          backdropFilter: "blur(4px)"
        },
        onPointerDown: (x) => {
          x.target === x.currentTarget && t();
        },
        children: /* @__PURE__ */ k(
          "div",
          {
            style: {
              width: 620,
              maxWidth: "90vw",
              maxHeight: "80vh",
              background: o.panelBg,
              border: `1px solid ${o.border}`,
              borderRadius: 12,
              boxShadow: "0 8px 32px rgba(0,0,0,0.25)",
              display: "flex",
              flexDirection: "column",
              overflow: "hidden"
            },
            onPointerDown: (x) => x.stopPropagation(),
            children: [
              /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    padding: "16px 20px 12px",
                    borderBottom: `1px solid ${o.border}`,
                    flexShrink: 0
                  },
                  children: [
                    /* @__PURE__ */ k(
                      "div",
                      {
                        style: {
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "center",
                          marginBottom: 12
                        },
                        children: [
                          /* @__PURE__ */ u(
                            "span",
                            {
                              style: {
                                fontSize: 14,
                                fontWeight: 600,
                                color: o.text
                              },
                              children: "Excalidraw Libraries"
                            }
                          ),
                          /* @__PURE__ */ u(
                            "button",
                            {
                              onClick: t,
                              style: {
                                border: "none",
                                background: "transparent",
                                color: o.textMuted,
                                cursor: "pointer",
                                fontSize: 18,
                                lineHeight: 1,
                                padding: "2px 6px"
                              },
                              children: "×"
                            }
                          )
                        ]
                      }
                    ),
                    /* @__PURE__ */ u(
                      "input",
                      {
                        type: "text",
                        placeholder: "Search libraries...",
                        value: c,
                        onChange: (x) => h(x.target.value),
                        autoFocus: !0,
                        style: {
                          width: "100%",
                          padding: "7px 10px",
                          border: `1px solid ${o.border}`,
                          borderRadius: 6,
                          background: o.controlBg,
                          color: o.text,
                          fontSize: 12,
                          outline: "none",
                          boxSizing: "border-box"
                        }
                      }
                    )
                  ]
                }
              ),
              /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    flex: 1,
                    overflowY: "auto",
                    padding: "8px 20px"
                  },
                  children: [
                    s && /* @__PURE__ */ u(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: o.textMuted,
                          fontSize: 12
                        },
                        children: "Loading libraries..."
                      }
                    ),
                    l && /* @__PURE__ */ k(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: "#ef4444",
                          fontSize: 12
                        },
                        children: [
                          "Failed to load directory: ",
                          l
                        ]
                      }
                    ),
                    !s && !l && g.length === 0 && /* @__PURE__ */ u(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: o.textDisabled,
                          fontSize: 12
                        },
                        children: "No libraries match your search."
                      }
                    ),
                    g.map((x, C) => {
                      const I = f.has(
                        `${En}/${x.source}`
                      ), M = p === x.id;
                      return /* @__PURE__ */ u(
                        $h,
                        {
                          entry: x,
                          isInstalled: I,
                          isInstalling: M,
                          onInstall: () => b(x),
                          theme: o
                        },
                        x.id || `dir-${C}`
                      );
                    })
                  ]
                }
              ),
              /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    padding: "8px 20px",
                    borderTop: `1px solid ${o.border}`,
                    color: o.textDisabled,
                    fontSize: 10,
                    textAlign: "center",
                    flexShrink: 0
                  },
                  children: [
                    g.length,
                    " libraries • Powered by Excalidraw Libraries"
                  ]
                }
              )
            ]
          }
        )
      }
    ),
    document.body
  );
}
function $h({
  entry: t,
  isInstalled: e,
  isInstalling: o,
  onInstall: r,
  theme: n
}) {
  var i;
  const s = t.preview ? `${En}/${t.preview}` : null;
  return /* @__PURE__ */ k(
    "div",
    {
      style: {
        display: "flex",
        gap: 12,
        padding: "10px 0",
        borderBottom: `1px solid ${n.border}`,
        alignItems: "flex-start"
      },
      children: [
        s && /* @__PURE__ */ u(
          "img",
          {
            src: s,
            alt: t.name,
            loading: "lazy",
            style: {
              width: 64,
              height: 64,
              objectFit: "cover",
              borderRadius: 6,
              border: `1px solid ${n.border}`,
              flexShrink: 0,
              background: "#fff"
            }
          }
        ),
        /* @__PURE__ */ k("div", { style: { flex: 1, minWidth: 0 }, children: [
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: 12,
                fontWeight: 600,
                color: n.text,
                marginBottom: 2
              },
              children: t.name
            }
          ),
          ((i = t.authors) == null ? void 0 : i.length) > 0 && /* @__PURE__ */ k(
            "div",
            {
              style: {
                fontSize: 10,
                color: n.textMuted,
                marginBottom: 4
              },
              children: [
                "by ",
                t.authors.map((l) => l.name).join(", ")
              ]
            }
          ),
          t.description && /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: 10,
                color: n.textSecondary,
                lineHeight: 1.4,
                overflow: "hidden",
                display: "-webkit-box",
                WebkitLineClamp: 2,
                WebkitBoxOrient: "vertical"
              },
              children: t.description
            }
          )
        ] }),
        /* @__PURE__ */ u(
          "button",
          {
            onClick: r,
            disabled: e || o,
            style: {
              flexShrink: 0,
              padding: "5px 10px",
              border: e ? `1px solid ${n.border}` : "none",
              borderRadius: 4,
              background: e ? "transparent" : o ? n.controlBgActive : n.accentColor,
              color: e ? n.textMuted : "#fff",
              cursor: e || o ? "default" : "pointer",
              fontSize: 10,
              fontWeight: 500,
              opacity: o ? 0.7 : 1
            },
            children: e ? "Installed" : o ? "Installing..." : "Install"
          }
        )
      ]
    }
  );
}
const _h = /^[A-Za-z][A-Za-z0-9_:-]*$/, si = /^[A-Za-z][A-Za-z0-9_]*$/;
function tu(t) {
  const e = t.trim();
  return e.startsWith('"') && e.endsWith('"') || e.startsWith("'") && e.endsWith("'") ? e.slice(1, -1).trim() : e;
}
function ke(t) {
  return tu(t).replace(/<br\s*\/?>/gi, `
`).replace(/\\n/g, `
`);
}
function pn(t, e) {
  const o = t.nodes.get(e.key);
  return o ? (o.label === o.key && e.label !== e.key && (o.label = e.label), o.shape === "rect" && e.shape !== "rect" && (o.shape = e.shape), o) : (t.nodes.set(e.key, e), e);
}
function uo(t) {
  const e = t.trim();
  if (!e) return null;
  let o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\(\(([\s\S]+)\)\)$/);
  return o ? { key: o[1], label: ke(o[2]), shape: "circle" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\(([\s\S]+)\)$/), o ? { key: o[1], label: ke(o[2]), shape: "round" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\{([\s\S]+)\}$/), o ? { key: o[1], label: ke(o[2]), shape: "diamond" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\[([\s\S]+)\]$/), o ? { key: o[1], label: ke(o[2]), shape: "rect" } : _h.test(e) ? { key: e, label: e, shape: "rect" } : null)));
}
function eu(t) {
  let e = t.match(/^(.*?)\s*--\s*\|([^|]+)\|\s*-->\s*(.*?)$/);
  if (e) {
    const o = uo(e[1]), r = uo(e[3]);
    return !o || !r ? null : { from: o, to: r, label: ke(e[2]) };
  }
  if (e = t.match(/^(.*?)\s*--\s*([^>-][\s\S]*?)\s*-->\s*(.*?)$/), e) {
    const o = uo(e[1]), r = uo(e[3]);
    return !o || !r ? null : { from: o, to: r, label: ke(e[2]) };
  }
  if (e = t.match(/^(.*?)\s*(?:-->|==>|-\.->|---)\s*(.*?)$/), e) {
    const o = uo(e[1]), r = uo(e[2]);
    return !o || !r ? null : { from: o, to: r };
  }
  return null;
}
function ou(t) {
  const e = t.match(/\b(TB|TD|BT|LR|RL)\b/i);
  if (!e) return "TB";
  const o = e[1].toUpperCase();
  return o === "TD" ? "TB" : o === "TB" || o === "BT" || o === "LR" || o === "RL" ? o : "TB";
}
function ru(t) {
  const e = t.match(/^subgraph(?:\s+([\s\S]+))?$/i);
  if (!e) return null;
  const o = (e[1] ?? "").trim();
  if (!o) return {};
  const r = o.match(/^[A-Za-z][A-Za-z0-9_:-]*\s*\[([\s\S]+)\]$/);
  return r ? { label: ke(r[1]) } : { label: ke(o) };
}
function nu(t) {
  const o = { direction: "TB", nodes: /* @__PURE__ */ new Map(), edges: [], groups: [] }, r = t.replace(/\r\n/g, `
`).split(`
`).map((a) => a.replace(/%%.*$/, "").trim()).filter(Boolean);
  if (r.length === 0)
    throw new Error("Paste a Mermaid flowchart first.");
  const n = r[0];
  /^(flowchart|graph)\b/i.test(n) && (o.direction = ou(n), r.shift());
  const i = [], l = (a) => {
    for (const c of i) c.nodeKeys.add(a);
  };
  for (const a of r) {
    const c = a.split(";").map((h) => h.trim()).filter(Boolean);
    for (const h of c) {
      const p = ru(h);
      if (p) {
        i.push({ label: p.label, nodeKeys: /* @__PURE__ */ new Set() });
        continue;
      }
      if (/^end\b/i.test(h)) {
        const m = i.pop();
        m && o.groups.push({
          label: m.label,
          nodeKeys: Array.from(m.nodeKeys)
        });
        continue;
      }
      const d = eu(h);
      if (d) {
        const m = pn(o, d.from), y = pn(o, d.to);
        l(m.key), l(y.key), o.edges.push({ fromKey: m.key, toKey: y.key, label: d.label });
        continue;
      }
      const f = uo(h);
      if (f) {
        const m = pn(o, f);
        l(m.key);
      }
    }
  }
  for (; i.length > 0; ) {
    const a = i.pop();
    o.groups.push({
      label: a.label,
      nodeKeys: Array.from(a.nodeKeys)
    });
  }
  if (o.nodes.size === 0)
    throw new Error("Could not parse Mermaid nodes. Try simple flowchart syntax like A-->B.");
  return o;
}
function su(t) {
  const e = t.indexOf(":");
  if (e < 0) return null;
  const o = t.slice(0, e).trim(), r = t.slice(e + 1).trim();
  if (!o || !r) return null;
  const n = [
    "-->>",
    "->>",
    "--x",
    "-x",
    "-->",
    "->",
    "--)",
    "-)",
    "-.->",
    "==>",
    "---"
  ];
  for (const s of n) {
    const i = o.indexOf(s);
    if (i < 0) continue;
    const l = o.slice(0, i).trim(), a = o.slice(i + s.length).trim();
    if (!(!si.test(l) || !si.test(a)))
      return {
        from: l,
        arrow: s,
        to: a,
        label: ke(r)
      };
  }
  return null;
}
function iu(t) {
  const e = t.match(/^Note\s+(left|right|over)\s+of\s+([A-Za-z][A-Za-z0-9_:-]*)\s*:\s*([\s\S]+)$/i);
  return e ? {
    side: e[1].toLowerCase(),
    of: e[2],
    text: ke(e[3])
  } : null;
}
function au(t) {
  return t ? /^#[0-9a-f]{3,8}$/i.test(t) || /^(rgb|rgba|hsl|hsla)\(/i.test(t) ? !0 : (/* @__PURE__ */ new Set([
    "red",
    "green",
    "blue",
    "purple",
    "pink",
    "orange",
    "yellow",
    "brown",
    "gray",
    "grey",
    "black",
    "white",
    "teal",
    "cyan",
    "magenta",
    "indigo",
    "violet",
    "gold",
    "silver",
    "maroon",
    "navy",
    "olive",
    "lime",
    "aqua",
    "fuchsia",
    "rebeccapurple"
  ])).has(t.toLowerCase()) : !1;
}
function lu(t) {
  const e = t.match(/^box(?:\s+(.+))?$/i);
  if (!e) return null;
  const o = (e[1] ?? "").trim();
  if (!o) return {};
  const r = o.indexOf(" "), n = r >= 0 ? o.slice(0, r) : o, s = r >= 0 ? o.slice(r + 1).trim() : "";
  return au(n) ? { color: n, label: s || void 0 } : { label: o };
}
function cu(t) {
  const e = t.replace(/\r\n/g, `
`).split(`
`).map((d) => d.replace(/%%.*$/, "").trim()).filter(Boolean);
  if (e.length === 0)
    throw new Error("Paste Mermaid sequenceDiagram text first.");
  if (!/^sequenceDiagram\b/i.test(e[0]))
    throw new Error("Not a Mermaid sequence diagram.");
  const o = /* @__PURE__ */ new Set(), r = [], n = [], s = [], i = [], l = [], a = [];
  let c = 0;
  const h = (d) => {
    o.has(d) || (o.add(d), r.push(d));
    for (const f of a) f.participants.add(d);
  };
  for (let d = 1; d < e.length; d++) {
    const f = e[d];
    if (/^autonumber\b/i.test(f)) continue;
    const m = lu(f);
    if (m) {
      a.push({ type: "box", label: m.label, color: m.color, participants: /* @__PURE__ */ new Set() });
      continue;
    }
    const y = f.match(/^loop(?:\s+([\s\S]+))?$/i);
    if (y) {
      a.push({
        type: "loop",
        label: y[1] ? ke(y[1]) : void 0,
        startStep: c,
        participants: /* @__PURE__ */ new Set()
      });
      continue;
    }
    if (/^end\b/i.test(f)) {
      const C = a.pop();
      (C == null ? void 0 : C.type) === "box" ? l.push(C) : (C == null ? void 0 : C.type) === "loop" && i.push({
        label: C.label,
        startStep: C.startStep,
        endStep: c,
        participants: C.participants
      });
      continue;
    }
    const g = f.match(/^participant\s+([A-Za-z][A-Za-z0-9_]*)(?:\s+as\s+[\s\S]+)?$/i);
    if (g) {
      h(g[1]);
      continue;
    }
    const b = iu(f);
    if (b) {
      h(b.of), s.push({ step: c, note: b });
      continue;
    }
    const x = su(f);
    if (x) {
      h(x.from), h(x.to), n.push(x), c += 1;
      continue;
    }
  }
  for (; a.length > 0; ) {
    const d = a.pop();
    d.type === "box" ? l.push(d) : i.push({
      label: d.label,
      startStep: d.startStep,
      endStep: c,
      participants: d.participants
    });
  }
  const p = r;
  if (p.length === 0)
    throw new Error("No participants found in sequenceDiagram.");
  if (n.length === 0 && s.length === 0)
    throw new Error("No messages/notes found in sequenceDiagram.");
  return {
    participants: p,
    messages: n,
    notes: s,
    loops: i.map((d) => ({
      label: d.label,
      startStep: d.startStep,
      endStep: d.endStep,
      participants: Array.from(d.participants)
    })).filter((d) => d.endStep >= d.startStep),
    groups: l.map((d) => ({
      label: d.label,
      color: d.color,
      participants: Array.from(d.participants)
    })).filter((d) => d.participants.length > 0)
  };
}
function vr(t) {
  return t === "diamond" ? { w: 200, h: 120 } : t === "circle" ? { w: 140, h: 140 } : { w: 200, h: 96 };
}
function du(t) {
  const e = Array.from(t.nodes.keys()).sort(), o = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
  for (const c of e)
    o.set(c, 0), r.set(c, []);
  for (const c of t.edges)
    !o.has(c.fromKey) || !o.has(c.toKey) || (r.get(c.fromKey).push(c.toKey), o.set(c.toKey, (o.get(c.toKey) ?? 0) + 1));
  const n = e.filter((c) => (o.get(c) ?? 0) === 0), s = /* @__PURE__ */ new Map();
  for (const c of n) s.set(c, 0);
  const i = [...n];
  for (; i.length > 0; ) {
    const c = i.shift(), h = s.get(c) ?? 0;
    for (const p of r.get(c) ?? []) {
      const d = Math.max(s.get(p) ?? 0, h + 1);
      s.set(p, d), o.set(p, (o.get(p) ?? 0) - 1), (o.get(p) ?? 0) <= 0 && i.push(p);
    }
  }
  let l = 0;
  for (const c of s.values()) l = Math.max(l, c);
  for (const c of e)
    s.has(c) || (l += 1, s.set(c, l));
  const a = /* @__PURE__ */ new Map();
  for (const c of e) {
    const h = s.get(c) ?? 0;
    a.has(h) || a.set(h, []), a.get(h).push(c);
  }
  return Array.from(a.entries()).sort((c, h) => c[0] - h[0]).map(([, c]) => c.sort());
}
function hu(t, e, o, r) {
  const n = cu(t), s = [], i = [], l = 6, a = "#94a3b8", c = 3, h = "#475569", p = 180, d = 64, f = 270, m = o - 140, y = m + d + 8, g = 88, b = Math.max(1, n.messages.length), x = y + b * g + 40, C = x + 12, I = C + d, M = /* @__PURE__ */ new Map();
  for (const D of n.groups) {
    const T = D.participants.map((et) => M.get(et)).filter((et) => typeof et == "number");
    if (T.length === 0)
      for (const et of D.participants) {
        const Z = n.participants.indexOf(et);
        Z >= 0 && T.push(e + (Z - (n.participants.length - 1) / 2) * f);
      }
    if (T.length === 0) continue;
    const P = Math.min(...T) - p / 2 - 24, G = Math.max(...T) + p / 2 + 24, K = m - 22, at = I - K + 18, B = {
      id: gt(10),
      type: "shape",
      x: P,
      y: K,
      w: G - P,
      h: at,
      z: r(),
      data: {
        shape: "rect",
        stroke: D.color ? D.color : "#475569",
        strokeWidth: 1.5,
        strokeStyle: "solid",
        roughness: 0,
        fill: D.color ? D.color : "#334155",
        fillStyle: "solid",
        opacity: D.color ? 0.2 : 0.08,
        edgeStyle: "sharp"
      }
    };
    if (s.push(B), i.push(B.id), D.label) {
      const et = {
        id: gt(10),
        type: "text",
        x: P + 10,
        y: K + 8,
        w: Math.max(120, G - P - 20),
        h: "auto",
        z: r(),
        data: {
          text: D.label,
          fontSize: 14,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "left"
        }
      };
      s.push(et);
    }
  }
  for (let D = 0; D < n.participants.length; D++) {
    const T = n.participants[D], P = e + (D - (n.participants.length - 1) / 2) * f;
    M.set(T, P);
    const G = {
      id: gt(10),
      type: "shape",
      x: P - p / 2,
      y: m,
      w: p,
      h: d,
      z: r(),
      data: {
        shape: "rect",
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        edgeStyle: "round",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        label: T,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    s.push(G), i.push(G.id);
    const K = {
      id: gt(10),
      type: "shape",
      x: P - l / 2,
      y,
      w: l,
      h: x - y,
      z: r(),
      data: {
        shape: "rect",
        stroke: a,
        strokeWidth: 1,
        strokeStyle: "solid",
        roughness: 0,
        fill: a,
        fillStyle: "solid",
        opacity: 0.3,
        edgeStyle: "round"
      }
    };
    s.push(K);
    const at = {
      id: gt(10),
      type: "shape",
      x: P - p / 2,
      y: C,
      w: p,
      h: d,
      z: r(),
      data: {
        shape: "rect",
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        edgeStyle: "round",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        label: T,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    s.push(at), i.push(at.id);
  }
  for (const D of n.loops) {
    const T = D.participants.map((X) => M.get(X)).filter((X) => typeof X == "number");
    if (T.length === 0) continue;
    const P = Math.min(...T) - 130, G = Math.max(...T) + 130, K = D.startStep + 1, at = Math.max(K, D.endStep), B = y + (K - 1) * g + 16, et = y + at * g + 34, Z = {
      id: gt(10),
      type: "shape",
      x: P,
      y: B,
      w: G - P,
      h: Math.max(90, et - B),
      z: r(),
      data: {
        shape: "rect",
        stroke: "#c4b5fd",
        strokeWidth: 1.5,
        strokeStyle: "dotted",
        roughness: 0,
        fill: "#64748b",
        fillStyle: "solid",
        opacity: 0.08,
        edgeStyle: "sharp"
      }
    };
    s.push(Z);
    const Q = `loop${D.label ? ` [${D.label}]` : ""}`, L = {
      id: gt(10),
      type: "text",
      x: P + 10,
      y: B + 8,
      w: G - P - 20,
      h: "auto",
      z: r(),
      data: {
        text: Q,
        fontSize: 14,
        fontFamily: "sans-serif",
        color: "#1f2937",
        align: "left"
      }
    };
    s.push(L);
  }
  for (let D = 0; D < n.messages.length; D++) {
    const T = n.messages[D], P = y + (D + 1) * g, G = M.get(T.from), K = M.get(T.to);
    if (G == null || K == null) continue;
    const at = G === K, B = Math.min(G, K), et = Math.max(G, K), Z = Math.max(et - B, 40), Q = G <= K ? 0 : Z, L = G <= K ? Z : 0, X = T.arrow.includes("--") || T.arrow === "-.->", _ = T.arrow.toLowerCase().includes("x"), j = T.arrow.includes(">") || T.arrow.includes(")");
    if (at) {
      const nt = G + 6, ot = P - 16, ft = 92, pt = 48, rt = X ? "dashed" : "solid", It = {
        id: gt(10),
        type: "shape",
        x: nt,
        y: ot,
        w: ft,
        h: c,
        z: r(),
        data: {
          shape: "rect",
          stroke: h,
          strokeWidth: 0,
          strokeStyle: "solid",
          roughness: 0,
          fill: h,
          fillStyle: "solid",
          opacity: 1,
          edgeStyle: "sharp"
        }
      }, zt = {
        id: gt(10),
        type: "shape",
        x: nt + ft - c,
        y: ot,
        w: c,
        h: pt,
        z: r(),
        data: {
          shape: "rect",
          stroke: h,
          strokeWidth: 0,
          strokeStyle: "solid",
          roughness: 0,
          fill: h,
          fillStyle: "solid",
          opacity: 1,
          edgeStyle: "sharp"
        }
      }, bt = {
        id: gt(10),
        type: "shape",
        x: nt,
        y: ot + pt - c,
        w: ft,
        h: c,
        z: r(),
        data: {
          shape: j ? "arrow" : "line",
          stroke: h,
          strokeWidth: c,
          strokeStyle: rt,
          roughness: 0,
          startPoint: [ft, c / 2],
          endPoint: [8, c / 2]
        }
      };
      s.push(It, zt, bt);
    } else {
      const nt = {
        id: gt(10),
        type: "shape",
        x: B,
        y: P - 14,
        w: Z,
        h: 28,
        z: r(),
        data: {
          shape: j ? "arrow" : "line",
          stroke: h,
          strokeWidth: c,
          strokeStyle: X ? "dashed" : "solid",
          roughness: 0,
          startPoint: [Q, 14],
          endPoint: [L, 14]
        }
      };
      s.push(nt);
    }
    const q = at ? G + 18 : B, Y = at ? 170 : Z, $ = {
      id: gt(10),
      type: "text",
      x: q,
      y: P - 46,
      w: Y,
      h: "auto",
      z: r(),
      data: {
        text: T.label,
        fontSize: 14,
        fontFamily: "sans-serif",
        color: "#0f172a",
        align: "center"
      }
    };
    if (s.push($), _) {
      const nt = G <= K ? B + Z - 14 : B + 8, ot = {
        id: gt(10),
        type: "text",
        x: nt,
        y: P - 20,
        w: 20,
        h: "auto",
        z: r(),
        data: {
          text: "×",
          fontSize: 16,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "center"
        }
      };
      s.push(ot);
    }
  }
  for (const D of n.notes) {
    const T = y + (D.step + 1) * g, P = M.get(D.note.of);
    if (P == null) continue;
    let G = P;
    D.note.side === "right" && (G += 130), D.note.side === "left" && (G -= 300), D.note.side === "over" && (G -= 110);
    const K = {
      id: gt(10),
      type: "text",
      x: G,
      y: T - 8,
      w: 260,
      h: "auto",
      z: r(),
      data: {
        text: D.note.text,
        fontSize: 13,
        fontFamily: "sans-serif",
        color: "#0f172a",
        align: "left"
      }
    };
    s.push(K);
  }
  return { nodes: s, shapeNodeIds: i };
}
function uu(t, e, o, r) {
  const n = t.trimStart();
  if (/^sequenceDiagram\b/i.test(n))
    return hu(t, e, o, r);
  const s = nu(t), i = du(s), l = Array.from(s.nodes.values()).map((g) => vr(g.shape)), a = l.length > 0 ? Math.max(...l.map((g) => g.h)) : 96, c = Math.max(a + 130, 260), h = /* @__PURE__ */ new Map(), p = i.length;
  for (let g = 0; g < i.length; g++) {
    const b = i[g], x = b.length, C = (g - (p - 1) / 2) * c, I = b.length > 0 ? Math.max(
      ...b.map((D) => {
        const T = s.nodes.get(D);
        return T ? vr(T.shape).w : 200;
      })
    ) : 200, M = Math.max(I + 90, 260);
    for (let D = 0; D < b.length; D++) {
      const T = b[D], P = (D - (x - 1) / 2) * M;
      if (s.direction === "LR" || s.direction === "RL") {
        const G = s.direction === "LR" ? e + C : e - C, K = o + P;
        h.set(T, { x: G, y: K });
      } else {
        const G = e + P, K = s.direction === "TB" ? o + C : o - C;
        h.set(T, { x: G, y: K });
      }
    }
  }
  const d = /* @__PURE__ */ new Map(), f = [], m = [], y = /* @__PURE__ */ new Map();
  for (const g of s.groups) {
    if (!g.nodeKeys.length) continue;
    const b = g.nodeKeys.map((T) => {
      const P = s.nodes.get(T), G = h.get(T);
      if (!P || !G) return null;
      const K = vr(P.shape);
      return { x: G.x - K.w / 2, y: G.y - K.h / 2, w: K.w, h: K.h };
    }).filter((T) => !!T);
    if (!b.length) continue;
    const x = Math.min(...b.map((T) => T.x)) - 30, C = Math.max(...b.map((T) => T.x + T.w)) + 30, I = Math.min(...b.map((T) => T.y)) - 34, M = Math.max(...b.map((T) => T.y + T.h)) + 24, D = {
      id: gt(10),
      type: "shape",
      x,
      y: I,
      w: C - x,
      h: M - I,
      z: r(),
      data: {
        shape: "rect",
        stroke: "#475569",
        strokeWidth: 1.5,
        strokeStyle: "solid",
        roughness: 0,
        fill: "#334155",
        fillStyle: "solid",
        opacity: 0.12,
        edgeStyle: "sharp"
      }
    };
    if (f.push(D), m.push(D.id), g.label) {
      const T = {
        id: gt(10),
        type: "text",
        x: x + 10,
        y: I + 8,
        w: Math.max(120, C - x - 20),
        h: "auto",
        z: r(),
        data: {
          text: g.label,
          fontSize: 14,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "left"
        }
      };
      f.push(T);
    }
  }
  for (const [g, b] of s.nodes) {
    const x = h.get(g) ?? { x: e, y: o }, C = vr(b.shape), I = {
      id: gt(10),
      type: "shape",
      x: x.x - C.w / 2,
      y: x.y - C.h / 2,
      w: C.w,
      h: C.h,
      z: r(),
      data: {
        shape: b.shape === "diamond" ? "diamond" : b.shape === "circle" ? "ellipse" : (b.shape === "round", "rect"),
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        edgeStyle: b.shape === "round" ? "round" : "sharp",
        label: b.label,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    f.push(I), m.push(I.id), d.set(g, I.id), y.set(g, { x: I.x, y: I.y, w: C.w, h: C.h });
  }
  for (const g of s.edges) {
    const b = d.get(g.fromKey), x = d.get(g.toKey);
    if (!b || !x || b === x) continue;
    const C = {
      id: gt(10),
      type: "edge",
      x: 0,
      y: 0,
      w: 0,
      h: 0,
      z: r(),
      data: {
        fromId: b,
        toId: x,
        label: g.label,
        style: "solid",
        color: "#64748b",
        strokeWidth: 2,
        arrowHead: "arrow",
        edgeType: "bezier"
      }
    };
    f.push(C);
  }
  return { nodes: f, shapeNodeIds: m };
}
const ii = `flowchart LR
  A[Idea] --> B{Decision}
  B -- Yes --> C[Ship]
  B -- No --> D[Refine]
  D --> B`;
function pu({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: r
}) {
  const n = qt(), s = lt(null), [i, l] = tt(ii), [a, c] = tt(null), [h, p] = tt(null);
  mt(() => {
    if (!e) return;
    const m = (y) => {
      s.current && !s.current.contains(y.target) && o();
    };
    return document.addEventListener("pointerdown", m), () => document.removeEventListener("pointerdown", m);
  }, [e, o]);
  const d = Kt(
    () => "Supported: flowchart/graph (TB/BT/LR/RL) and sequenceDiagram. Flowchart nodes: A[Text], A{Decision}, A((Start)). Edges: A-->B, A -- label --> B.",
    []
  ), f = it(() => {
    try {
      const m = window.innerWidth / 2, y = window.innerHeight / 2, g = t.screenToCanvas(m, y), { nodes: b, shapeNodeIds: x } = uu(i, g.x, g.y, () => t.nextZ());
      if (b.length === 0)
        throw new Error("No nodes were parsed.");
      t.addNodes(b), x.length > 0 && t.selectMultiple(x), c(null), p(`Inserted ${x.length} nodes and ${b.length - x.length} edges.`);
    } catch (m) {
      p(null), c(m instanceof Error ? m.message : "Failed to parse Mermaid graph.");
    }
  }, [t, i]);
  return !e || !r ? null : Ne(
    /* @__PURE__ */ k(
      "div",
      {
        ref: s,
        style: {
          position: "fixed",
          left: r.right + 8,
          top: r.top,
          background: n.panelBg,
          border: `1px solid ${n.border}`,
          borderRadius: n.panelBorderRadius,
          boxShadow: n.panelShadow,
          width: 340,
          maxHeight: `calc(100vh - ${r.top + 20}px)`,
          overflow: "hidden",
          display: "flex",
          flexDirection: "column",
          zIndex: 99999
        },
        onPointerDown: (m) => m.stopPropagation(),
        children: [
          /* @__PURE__ */ k("div", { style: { padding: "10px 12px 8px", borderBottom: `1px solid ${n.border}` }, children: [
            /* @__PURE__ */ u("div", { style: { fontSize: 12, fontWeight: 700, color: n.text }, children: "Mermaid Sketch" }),
            /* @__PURE__ */ u("div", { style: { marginTop: 4, fontSize: 10, color: n.textMuted, lineHeight: 1.45 }, children: d })
          ] }),
          /* @__PURE__ */ k("div", { style: { padding: 12, display: "flex", flexDirection: "column", gap: 8 }, children: [
            /* @__PURE__ */ u(
              "textarea",
              {
                value: i,
                onChange: (m) => l(m.target.value),
                spellCheck: !1,
                style: {
                  width: "100%",
                  minHeight: 180,
                  resize: "vertical",
                  padding: "8px 10px",
                  borderRadius: n.controlBorderRadius,
                  border: `1px solid ${n.border}`,
                  background: n.controlBg,
                  color: n.text,
                  fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace",
                  fontSize: 11,
                  lineHeight: 1.4,
                  boxSizing: "border-box"
                }
              }
            ),
            a && /* @__PURE__ */ u("div", { style: { fontSize: 10, color: "#ef4444" }, children: a }),
            h && /* @__PURE__ */ u("div", { style: { fontSize: 10, color: "#16a34a" }, children: h }),
            /* @__PURE__ */ k("div", { style: { display: "flex", justifyContent: "space-between", gap: 8 }, children: [
              /* @__PURE__ */ u(
                "button",
                {
                  onClick: () => l(ii),
                  style: {
                    border: `1px solid ${n.border}`,
                    background: "transparent",
                    color: n.text,
                    borderRadius: n.controlBorderRadius,
                    padding: "6px 10px",
                    fontSize: 11,
                    cursor: "pointer"
                  },
                  children: "Reset Example"
                }
              ),
              /* @__PURE__ */ u(
                "button",
                {
                  onClick: f,
                  style: {
                    border: `1px solid ${n.border}`,
                    background: n.controlBgActive,
                    color: n.text,
                    borderRadius: n.controlBorderRadius,
                    padding: "6px 10px",
                    fontSize: 11,
                    fontWeight: 600,
                    cursor: "pointer"
                  },
                  children: "Insert Diagram"
                }
              )
            ] })
          ] })
        ]
      }
    ),
    document.body
  );
}
const fu = [
  { key: "select", label: "Select", shortcut: "S", num: "" },
  { key: "hand", label: "Hand", shortcut: "P", num: "" },
  { key: "draw", label: "Draw", shortcut: "D", num: "" },
  { key: "shape", label: "Shape", shortcut: "G", num: "" },
  { key: "text", label: "Text", shortcut: "T", num: "" },
  { key: "note", label: "Note", shortcut: "B", num: "" },
  { key: "sticky", label: "Sticky", shortcut: "Y", num: "" },
  { key: "frame", label: "Frame", shortcut: "F", num: "" },
  { key: "erase", label: "Eraser", shortcut: "E", num: "" },
  { key: "laser", label: "Laser", shortcut: "Z", num: "" }
], go = {
  border: "none",
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, Ft = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function bo({ name: t, size: e = 18 }) {
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "select" && /* @__PURE__ */ u("path", { d: "M6 2v17l4-4.5L13.5 21l2.5-1.5L12.5 13H18z", fill: "currentColor" }),
    t === "draw" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M17 3l4 4L7.5 20.5 2 22l1.5-5.5z", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M15 5l4 4", ...Ft })
    ] }),
    t === "shape" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...Ft }),
    t === "text" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M7 4h10", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M12 4v16", ...Ft })
    ] }),
    t === "note" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 3h16v14l-4 4H4z", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M16 17v4l4-4z", fill: "currentColor", opacity: 0.4 })
    ] }),
    t === "sticky" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "1", fill: "currentColor", opacity: 0.15, ...Ft }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "9", x2: "17", y2: "9", ...Ft, opacity: 0.5 }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "13", x2: "14", y2: "13", ...Ft, opacity: 0.5 })
    ] }),
    t === "frame" && /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2", ...Ft, strokeDasharray: "4,2" }),
    t === "hand" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M8 13V5.5a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M11 5.5v-2a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M14 5.5a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M17 7.5a1.5 1.5 0 0 1 3 0V16a6 6 0 0 1-6 6h-2a6 6 0 0 1-6-6V9.5a1.5 1.5 0 0 1 3 0", ...Ft })
    ] }),
    t === "erase" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 20H9L3 14l9.5-9.5 8 8L16 17", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M12.5 4.5l8 8", ...Ft })
    ] }),
    t === "laser" && /* @__PURE__ */ u("circle", { cx: "12", cy: "12", r: "4", fill: "currentColor", opacity: 0.9 }),
    t === "lasso" && /* @__PURE__ */ u("path", { d: "M18 4c-3 0-5 2-8 5S5 14 4 16c-1 3 1 4 3 4s4-2 6-4 4-4 6-5 3-2 3-4-1-3-3-3z", ...Ft, strokeDasharray: "3,2" }),
    t === "undo" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...Ft, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...Ft, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...Ft, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...Ft, fill: "none" })
    ] }),
    t === "print" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M6 9V3h12v6", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M6 18H4a1 1 0 0 1-1-1v-5a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-2", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "6", y: "14", width: "12", height: "7", rx: "1", ...Ft })
    ] }),
    t === "fit" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...Ft })
    ] }),
    t === "paper" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "4", y: "2", width: "16", height: "20", rx: "1", ...Ft }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "7", x2: "16", y2: "7", ...Ft, opacity: 0.4 }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "11", x2: "16", y2: "11", ...Ft, opacity: 0.4 }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "15", x2: "13", y2: "15", ...Ft, opacity: 0.4 })
    ] }),
    t === "template" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "13", y: "3", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "3", y: "13", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "13", y: "13", width: "8", height: "8", rx: "1", ...Ft })
    ] }),
    t === "library" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M8 7h6", ...Ft, opacity: 0.5 }),
      /* @__PURE__ */ u("path", { d: "M8 11h4", ...Ft, opacity: 0.5 })
    ] }),
    t === "gif" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "4", width: "20", height: "16", rx: "2", ...Ft }),
      /* @__PURE__ */ u("text", { x: "12", y: "14.5", textAnchor: "middle", fontSize: "7", fontWeight: "700", fill: "currentColor", stroke: "none", children: "GIF" })
    ] }),
    t === "mermaid" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "4", width: "18", height: "14", rx: "2", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M6 8l2.6 3 2.1-2 2.2 3 2.2-2.5L18 13", ...Ft }),
      /* @__PURE__ */ u("circle", { cx: "6", cy: "8", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "10.7", cy: "9", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "14.9", cy: "9.5", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "18", cy: "13", r: "1.1", fill: "currentColor", stroke: "none" })
    ] })
  ] });
}
const yu = {
  light: "Light",
  dark: "Dark",
  textured: "Textured"
};
function mu({
  engine: t,
  background: e
}) {
  const o = qt(), [r, n] = tt(!1), s = lt(null), i = lt(null);
  mt(() => {
    if (!r) return;
    const c = (h) => {
      i.current && !i.current.contains(h.target) && s.current && !s.current.contains(h.target) && n(!1);
    };
    return document.addEventListener("pointerdown", c), () => document.removeEventListener("pointerdown", c);
  }, [r]);
  const l = No.find((c) => c.key === e) ?? No[1], a = r && s.current ? (() => {
    const c = s.current.getBoundingClientRect();
    return Ne(
      /* @__PURE__ */ u(
        "div",
        {
          ref: i,
          style: {
            position: "fixed",
            left: c.right + 8,
            top: c.top,
            background: o.panelBg,
            border: `1px solid ${o.border}`,
            borderRadius: o.panelBorderRadius,
            padding: 8,
            zIndex: 99999,
            boxShadow: o.panelShadow,
            width: 180,
            maxHeight: 400,
            overflowY: "auto"
          },
          onPointerDown: (h) => h.stopPropagation(),
          children: ["light", "dark", "textured"].map((h) => {
            const p = No.filter((d) => d.group === h);
            return p.length === 0 ? null : /* @__PURE__ */ k("div", { style: { marginBottom: 6 }, children: [
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    fontSize: 10,
                    fontWeight: 600,
                    color: o.textMuted,
                    textTransform: "uppercase",
                    letterSpacing: "0.05em",
                    padding: "4px 6px 2px"
                  },
                  children: yu[h]
                }
              ),
              p.map((d) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    t.setBoardBackground(d.key), n(!1);
                  },
                  style: {
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                    width: "100%",
                    padding: "5px 6px",
                    border: "none",
                    borderRadius: o.controlBorderRadius,
                    background: e === d.key ? o.controlBgActive : "transparent",
                    color: o.text,
                    cursor: "pointer",
                    fontSize: 12,
                    textAlign: "left"
                  },
                  children: [
                    /* @__PURE__ */ u(
                      "span",
                      {
                        style: {
                          width: 18,
                          height: 18,
                          borderRadius: 3,
                          background: d.swatchColor,
                          border: `1.5px solid ${o.border}`,
                          flexShrink: 0
                        }
                      }
                    ),
                    d.label
                  ]
                },
                d.key
              ))
            ] }, h);
          })
        }
      ),
      document.body
    );
  })() : null;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ k(
      "button",
      {
        ref: s,
        title: "Paper type",
        onClick: () => n((c) => !c),
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: o.controlBorderRadius,
          background: r ? o.controlBgActive : "transparent",
          color: o.text,
          position: "relative"
        },
        children: [
          /* @__PURE__ */ u(bo, { name: "paper" }),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                position: "absolute",
                bottom: 4,
                right: 4,
                width: 8,
                height: 8,
                borderRadius: "50%",
                background: l.swatchColor,
                border: `1px solid ${o.border}`
              }
            }
          )
        ]
      }
    ),
    a
  ] });
}
function gu({ engine: t }) {
  const e = qt(), [o, r] = tt(!1), n = lt(null), s = lt(null);
  mt(() => {
    if (!o) return;
    const l = (a) => {
      s.current && !s.current.contains(a.target) && n.current && !n.current.contains(a.target) && r(!1);
    };
    return document.addEventListener("pointerdown", l), () => document.removeEventListener("pointerdown", l);
  }, [o]);
  const i = o && n.current ? (() => {
    const l = n.current.getBoundingClientRect();
    return Ne(
      /* @__PURE__ */ k(
        "div",
        {
          ref: s,
          style: {
            position: "fixed",
            left: l.right + 8,
            top: l.top,
            background: e.panelBg,
            border: `1px solid ${e.border}`,
            borderRadius: 8,
            padding: 8,
            zIndex: 99999,
            boxShadow: "0 4px 16px rgba(0,0,0,0.15)",
            width: 180
          },
          onPointerDown: (a) => a.stopPropagation(),
          children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 10,
                  fontWeight: 600,
                  color: e.textMuted,
                  textTransform: "uppercase",
                  letterSpacing: "0.05em",
                  padding: "4px 6px 2px"
                },
                children: "Templates"
              }
            ),
            vi.map((a) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => {
                  const c = typeof window < "u" ? window : void 0;
                  if (!c) return;
                  const h = c.innerWidth / 2, p = c.innerHeight / 2, d = Wo(t.viewport, h, p);
                  t.applyTemplate(a.id, d.x, d.y), r(!1);
                },
                style: {
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  width: "100%",
                  padding: "6px 6px",
                  border: "none",
                  borderRadius: e.controlBorderRadius,
                  background: "transparent",
                  color: e.text,
                  cursor: "pointer",
                  fontSize: 12,
                  textAlign: "left"
                },
                onMouseEnter: (c) => {
                  c.currentTarget.style.background = e.controlBgActive;
                },
                onMouseLeave: (c) => {
                  c.currentTarget.style.background = "transparent";
                },
                children: a.label
              },
              a.id
            ))
          ]
        }
      ),
      document.body
    );
  })() : null;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: n,
        title: "Templates",
        onClick: () => r((l) => !l),
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(bo, { name: "template" })
      }
    ),
    i
  ] });
}
function bu({ engine: t }) {
  const e = qt(), [o, r] = tt(!1), [n, s] = tt(!1), i = lt(null), [l, a] = tt(null), c = it(() => {
    r((d) => (!d && i.current && a(i.current.getBoundingClientRect()), !d));
  }, []), h = it(() => r(!1), []), p = it(() => {
    s(!0);
  }, []);
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: i,
        title: "Libraries",
        onClick: c,
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(bo, { name: "library" })
      }
    ),
    /* @__PURE__ */ u(
      oh,
      {
        engine: t,
        open: o,
        onClose: h,
        triggerRect: l,
        onBrowseDirectory: p
      }
    ),
    n && /* @__PURE__ */ u(
      Jh,
      {
        onClose: () => s(!1),
        onInstalled: () => {
          r(!1), setTimeout(() => {
            i.current && a(i.current.getBoundingClientRect()), r(!0);
          }, 100);
        }
      }
    )
  ] });
}
function xu({ engine: t, baseUrl: e }) {
  const o = qt(), [r, n] = tt(!1), s = lt(null), [i, l] = tt(null), a = it(() => {
    n((h) => (!h && s.current && l(s.current.getBoundingClientRect()), !h));
  }, []), c = it(() => n(!1), []);
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: s,
        title: "GIF Search",
        onClick: a,
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: o.controlBorderRadius,
          background: r ? o.controlBgActive : "transparent",
          color: o.text
        },
        children: /* @__PURE__ */ u(bo, { name: "gif" })
      }
    ),
    /* @__PURE__ */ u(
      ih,
      {
        engine: t,
        open: r,
        onClose: c,
        triggerRect: i,
        baseUrl: e
      }
    )
  ] });
}
function wu({ engine: t }) {
  const e = qt(), [o, r] = tt(!1), n = lt(null), [s, i] = tt(null), l = it(() => {
    r((c) => (!c && n.current && i(n.current.getBoundingClientRect()), !c));
  }, []), a = it(() => r(!1), []);
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: n,
        title: "Mermaid Sketch",
        onClick: l,
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(bo, { name: "mermaid" })
      }
    ),
    /* @__PURE__ */ u(
      pu,
      {
        engine: t,
        open: o,
        onClose: a,
        triggerRect: s
      }
    )
  ] });
}
function ku({ engine: t, gifApiBaseUrl: e }) {
  const o = qt(), [r, n] = tt(t.mode), [s, i] = tt(t.boardBackground), [l, a] = tt(t.lassoSelect);
  return mt(() => {
    const c = () => n(t.mode), h = () => i(t.boardBackground), p = () => a(t.lassoSelect);
    return t.on("mode", c), t.on("background", h), t.on("lassoToggle", p), () => {
      t.off("mode", c), t.off("background", h), t.off("lassoToggle", p);
    };
  }, [t]), /* @__PURE__ */ k(
    "div",
    {
      "data-sb-toolbar": !0,
      style: {
        width: ar,
        height: "100%",
        flexShrink: 0,
        background: o.toolbarBg,
        borderRight: `1px solid ${o.border}`,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        padding: "8px 0",
        gap: 4
      },
      children: [
        fu.map((c) => {
          const h = r === c.key && !(c.key === "select" && l);
          return /* @__PURE__ */ k(
            "button",
            {
              title: `${c.label} (${c.shortcut}${c.num ? ` / ${c.num}` : ""})`,
              onClick: () => {
                l && (t.toggleLassoSelect(), a(!1)), t.setMode(c.key);
              },
              style: {
                ...go,
                width: 40,
                height: 40,
                borderRadius: o.controlBorderRadius,
                background: h ? o.controlBgActive : "transparent",
                color: o.text,
                position: "relative"
              },
              children: [
                /* @__PURE__ */ u(bo, { name: c.key }),
                /* @__PURE__ */ u(
                  "span",
                  {
                    style: {
                      position: "absolute",
                      bottom: 2,
                      right: 2,
                      fontSize: 8,
                      lineHeight: 1,
                      color: o.textMuted,
                      fontWeight: 500,
                      pointerEvents: "none"
                    },
                    children: c.num || c.shortcut
                  }
                )
              ]
            },
            c.key
          );
        }),
        /* @__PURE__ */ u("div", { style: { width: 28, height: 1, background: o.separator, margin: "8px 0" } }),
        /* @__PURE__ */ k(
          "button",
          {
            title: "Lasso Select (L)",
            onClick: () => {
              l ? (t.toggleLassoSelect(), a(!1)) : (t.setMode("select"), t.lassoSelect || t.toggleLassoSelect(), a(!0));
            },
            style: {
              ...go,
              width: 40,
              height: 40,
              borderRadius: o.controlBorderRadius,
              background: l ? o.controlBgActive : "transparent",
              color: o.text,
              position: "relative"
            },
            children: [
              /* @__PURE__ */ u(bo, { name: "lasso" }),
              /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    position: "absolute",
                    bottom: 2,
                    right: 2,
                    fontSize: 8,
                    lineHeight: 1,
                    color: o.textMuted,
                    fontWeight: 500,
                    pointerEvents: "none"
                  },
                  children: "L"
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ u("div", { style: { width: 28, height: 1, background: o.separator, margin: "8px 0" } }),
        /* @__PURE__ */ u(mu, { engine: t, background: s }),
        /* @__PURE__ */ u(gu, { engine: t }),
        /* @__PURE__ */ u(bu, { engine: t }),
        /* @__PURE__ */ u(wu, { engine: t }),
        e && /* @__PURE__ */ u(xu, { engine: t, baseUrl: e })
      ]
    }
  );
}
const vu = /* @__PURE__ */ new Set(["shape", "draw", "text", "image", "content", "frame", "sticky"]), Su = /* @__PURE__ */ new Set(["text", "image", "content", "frame"]);
function ai(t) {
  return t.data.opacity ?? 1;
}
function To(t, e) {
  return t.data[e];
}
function Mu(t) {
  const e = {}, o = t.filter((n) => vu.has(n.type));
  if (o.length > 0) {
    const n = ai(o[0]), s = o.every((i) => ai(i) === n);
    e.opacity = s ? n : "mixed";
  }
  const r = t.filter((n) => Su.has(n.type));
  if (r.length > 0) {
    const n = To(r[0], "borderColor"), s = r.every(
      (h) => To(h, "borderColor") === n
    );
    e.borderColor = s ? n ?? null : "mixed";
    const i = To(r[0], "borderWidth") ?? 1, l = r.every(
      (h) => (To(h, "borderWidth") ?? 1) === i
    );
    e.borderWidth = l ? i : "mixed";
    const a = To(r[0], "borderStyle") ?? "solid", c = r.every(
      (h) => (To(h, "borderStyle") ?? "solid") === a
    );
    e.borderStyle = c ? a : "mixed";
  }
  return e;
}
function Cu(t) {
  const [e, o] = tt(t.mode), [r, n] = tt(new Set(t.selection)), [, s] = tt(0);
  if (mt(() => {
    const h = () => o(t.mode), p = () => {
      n(new Set(t.selection)), s((f) => f + 1);
    }, d = () => s((f) => f + 1);
    return t.on("mode", h), t.on("selection", p), t.on("change", d), () => {
      t.off("mode", h), t.off("selection", p), t.off("change", d);
    };
  }, [t]), r.size === 0)
    return e === "draw" || e === "shape" || e === "text" ? { target: { kind: "tool", mode: e }, commonProps: {} } : { target: { kind: "none" }, commonProps: {} };
  const i = [];
  for (const h of r) {
    const p = t.getNode(h);
    p && i.push(p);
  }
  if (i.length === 0)
    return { target: { kind: "none" }, commonProps: {} };
  if (i.length === 1)
    return { target: { kind: "single", node: i[0] }, commonProps: {} };
  const l = /* @__PURE__ */ new Map();
  for (const h of i) {
    const p = l.get(h.type);
    p ? p.push(h) : l.set(h.type, [h]);
  }
  const a = [];
  for (const [h, p] of l)
    a.push({ type: h, nodes: p });
  const c = Mu(i);
  return {
    target: { kind: "multi", nodes: i, typeGroups: a },
    commonProps: c
  };
}
const Xr = ki(null);
function Re(t, e) {
  const o = Wr(Xr);
  return it(
    (r) => {
      if (o && o.length > 1) {
        const n = o.map((s) => ({
          id: s.id,
          patch: {
            data: { ...s.data, ...r }
          }
        }));
        t.batchUpdateWithHistory(n);
      } else
        t.updateNodeWithHistory(e.id, {
          data: { ...e.data, ...r }
        });
    },
    [t, e, o]
  );
}
function Se({
  value: t,
  onChange: e,
  mixed: o
}) {
  const r = qt(), n = o || t === void 0 ? 100 : Math.round(t * 100);
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Opacity" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "range",
        min: 0,
        max: 100,
        value: n,
        onChange: (s) => e(parseInt(s.target.value) / 100),
        style: { flex: 1, accentColor: r.accentColor }
      }
    ),
    /* @__PURE__ */ u("span", { style: { width: 28, textAlign: "right", fontSize: 10, color: o ? r.textFaint : r.text }, children: o ? "--" : n })
  ] });
}
const zu = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
function me({
  label: t,
  palettes: e,
  value: o,
  onChange: r,
  allowNull: n,
  mixed: s
}) {
  const i = qt(), [l, a] = tt(""), [c, h] = tt(0), [p, d] = tt(!1), f = lt(null), m = lt(null), [y, g] = tt(null), [b, x] = tt("bottom"), C = e[c] ?? e[0], I = o == null ? void 0 : o.toLowerCase();
  mt(() => {
    if (!p) return;
    const T = (P) => {
      f.current && !f.current.contains(P.target) && d(!1);
    };
    return document.addEventListener("mousedown", T), () => document.removeEventListener("mousedown", T);
  }, [p]), mt(() => {
    if (!p) return;
    const T = () => {
      const P = m.current;
      if (!P) return;
      const G = P.getBoundingClientRect(), at = e.length * 30 + 10, B = window.innerHeight - G.bottom, et = G.top, Z = B < at && et > B;
      x(Z ? "top" : "bottom"), g({
        top: Z ? G.top - 4 : G.bottom + 4,
        left: G.right
      });
    };
    return T(), window.addEventListener("resize", T), window.addEventListener("scroll", T, !0), () => {
      window.removeEventListener("resize", T), window.removeEventListener("scroll", T, !0);
    };
  }, [p]);
  const M = () => {
    const T = l.trim();
    if (!T) return;
    const P = T.startsWith("#") ? T : `#${T}`;
    zu.test(P) && (r(P), a(""));
  }, D = e.some(
    (T) => T.colors.some((P) => P.toLowerCase() === I)
  );
  return /* @__PURE__ */ k("div", { style: { display: "flex", flexDirection: "column", gap: 3 }, children: [
    /* @__PURE__ */ k("div", { style: { ...Lt, margin: 0, flexWrap: "wrap", gap: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: i.textMuted }, children: t }),
      n && /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r(null),
          title: "None",
          style: {
            ...Ut,
            width: 20,
            height: 20,
            background: "transparent",
            border: !s && o == null ? `2px solid ${i.swatchBorderActive}` : `2px solid ${i.textDisabled}`,
            borderRadius: "50%",
            position: "relative",
            overflow: "hidden"
          },
          children: /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                width: "140%",
                height: 2,
                background: i.error,
                transform: "rotate(-45deg)",
                top: "50%",
                left: "-20%"
              }
            }
          )
        }
      ),
      C.colors.map((T) => {
        const P = !s && I === T.toLowerCase();
        return /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r(T),
            style: {
              ...Ut,
              width: 20,
              height: 20,
              background: T,
              border: P ? `2px solid ${i.swatchBorderActive}` : "2px solid transparent",
              borderRadius: "50%"
            }
          },
          T
        );
      }),
      o && !D && !s && /* @__PURE__ */ u(
        "div",
        {
          style: {
            width: 20,
            height: 20,
            borderRadius: "50%",
            background: o,
            border: `2px solid ${i.swatchBorderActive}`,
            flexShrink: 0
          }
        }
      ),
      s && /* @__PURE__ */ u("span", { style: { fontSize: 9, color: i.textMuted, fontStyle: "italic" }, children: "Mixed" }),
      e.length > 1 && /* @__PURE__ */ k("div", { ref: m, style: { position: "relative", marginLeft: "auto" }, children: [
        /* @__PURE__ */ k(
          "button",
          {
            onClick: () => d((T) => !T),
            title: "Switch palette",
            style: {
              ...Ut,
              height: 20,
              padding: "0 5px",
              background: i.controlBg,
              color: i.textMuted,
              fontSize: 9,
              borderRadius: i.controlBorderRadius,
              display: "flex",
              alignItems: "center",
              gap: 2
            },
            children: [
              C.name,
              /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: p ? "▲" : "▼" })
            ]
          }
        ),
        p && y && Ne(
          /* @__PURE__ */ u(
            "div",
            {
              ref: f,
              style: {
                position: "fixed",
                top: y.top,
                left: y.left,
                transform: b === "top" ? "translate(-100%, -100%)" : "translateX(-100%)",
                background: i.panelBg,
                border: `1px solid ${i.border}`,
                borderRadius: i.panelBorderRadius,
                padding: 4,
                zIndex: 2e4,
                display: "flex",
                flexDirection: "column",
                gap: 2,
                minWidth: 120,
                boxShadow: i.panelShadow
              },
              children: e.map((T, P) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    h(P), d(!1);
                  },
                  style: {
                    ...Ut,
                    height: 28,
                    padding: "0 8px",
                    background: P === c ? i.controlBgActive : "transparent",
                    color: i.text,
                    fontSize: 10,
                    borderRadius: i.controlBorderRadius,
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    width: "100%",
                    justifyContent: "flex-start"
                  },
                  children: [
                    /* @__PURE__ */ u("span", { style: { display: "flex", gap: 2 }, children: T.colors.slice(0, 6).map((G) => /* @__PURE__ */ u(
                      "span",
                      {
                        style: {
                          width: 10,
                          height: 10,
                          borderRadius: "50%",
                          background: G,
                          display: "inline-block"
                        }
                      },
                      G
                    )) }),
                    /* @__PURE__ */ u("span", { children: T.name })
                  ]
                },
                T.name
              ))
            }
          ),
          document.body
        )
      ] })
    ] }),
    /* @__PURE__ */ u("div", { style: { display: "flex", alignItems: "center", gap: 4, paddingLeft: 52 }, children: /* @__PURE__ */ u(
      "input",
      {
        type: "text",
        value: l,
        onChange: (T) => a(T.target.value),
        onKeyDown: (T) => {
          T.key === "Enter" && M();
        },
        onBlur: M,
        placeholder: o ?? "#000000",
        style: {
          width: 72,
          height: 20,
          background: i.controlBg,
          border: `1px solid ${i.border}`,
          borderRadius: i.controlBorderRadius,
          color: i.text,
          fontSize: 10,
          fontFamily: "monospace",
          padding: "0 6px",
          outline: "none"
        }
      }
    ) })
  ] });
}
function Go({
  label: t,
  value: e,
  onChange: o,
  mixed: r
}) {
  const n = qt();
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Dt, color: n.textMuted }, children: t }),
    Kh.map((s) => /* @__PURE__ */ u(
      "button",
      {
        title: s.label,
        onClick: () => o(s.key),
        style: {
          ...Ut,
          width: 36,
          height: 28,
          background: !r && e === s.key ? n.controlBgActive : n.controlBg,
          borderRadius: n.controlBorderRadius
        },
        children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
          "line",
          {
            x1: 2,
            y1: 6,
            x2: 22,
            y2: 6,
            stroke: n.text,
            strokeWidth: 2,
            strokeDasharray: s.dash
          }
        ) })
      },
      s.key
    ))
  ] });
}
function jo({
  label: t,
  widths: e = qh,
  value: o,
  onChange: r,
  mixed: n
}) {
  const s = qt();
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Dt, color: s.textMuted }, children: t }),
    /* @__PURE__ */ u("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", flex: 1, minWidth: 0 }, children: e.map((i) => /* @__PURE__ */ u(
      "button",
      {
        title: `${i}px`,
        onClick: () => r(i),
        style: {
          ...Ut,
          width: 30,
          height: 24,
          background: !n && o === i ? s.controlBgActive : s.controlBg,
          borderRadius: s.controlBorderRadius
        },
        children: /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 16,
              height: Math.max(i, 1),
              background: s.text,
              borderRadius: i / 2
            }
          }
        )
      },
      i
    )) })
  ] });
}
function lr({
  borderColor: t,
  borderStyle: e,
  borderWidth: o,
  mixed: r,
  onChange: n
}) {
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      me,
      {
        label: "Border",
        palettes: ve,
        value: t,
        onChange: (s) => n("borderColor", s ?? void 0),
        allowNull: !0,
        mixed: r == null ? void 0 : r.color
      }
    ),
    (t || (r == null ? void 0 : r.color)) && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u(
        Go,
        {
          label: "Style",
          value: e ?? "solid",
          onChange: (s) => n("borderStyle", s),
          mixed: r == null ? void 0 : r.style
        }
      ),
      /* @__PURE__ */ u(
        jo,
        {
          label: "Width",
          value: o ?? 1,
          onChange: (s) => n("borderWidth", s),
          mixed: r == null ? void 0 : r.width
        }
      )
    ] })
  ] });
}
const fn = /* @__PURE__ */ new Map();
function ye({
  title: t,
  defaultOpen: e = !0,
  variant: o = "sub",
  open: r,
  onToggle: n,
  persistKey: s,
  children: i
}) {
  const l = qt(), [a, c] = tt(() => s && fn.has(s) ? !!fn.get(s) : e), h = r ?? a, p = o === "group";
  return mt(() => {
    !s || r !== void 0 || fn.set(s, h);
  }, [s, r, h]), /* @__PURE__ */ k(
    "section",
    {
      style: {
        border: `1px solid ${l.border}`,
        borderRadius: l.controlBorderRadius,
        background: p ? l.panelBg : l.controlBg,
        overflow: "visible",
        flexShrink: 0,
        alignSelf: "stretch"
      },
      children: [
        /* @__PURE__ */ k(
          "button",
          {
            type: "button",
            onClick: () => {
              n ? n() : c((d) => !d);
            },
            style: {
              width: "100%",
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              background: "transparent",
              border: "none",
              color: p ? l.textMuted : l.textSecondary,
              fontSize: 10,
              fontWeight: 700,
              letterSpacing: "0.03em",
              textTransform: "uppercase",
              padding: "8px 10px",
              cursor: "pointer"
            },
            children: [
              /* @__PURE__ */ u("span", { children: t }),
              /* @__PURE__ */ u("span", { style: { color: l.textMuted }, children: h ? "−" : "+" })
            ]
          }
        ),
        h && /* @__PURE__ */ u(
          "div",
          {
            style: {
              padding: "8px 10px 10px",
              borderTop: `1px solid ${l.border}`,
              display: "flex",
              flexDirection: "column",
              gap: 8,
              background: p ? "transparent" : l.controlBg
            },
            children: i
          }
        )
      ]
    }
  );
}
function ls({ style: t }) {
  const e = qt();
  return t === "hachure" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: e.text, strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: e.text, strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: e.text, strokeWidth: 1.5 })
  ] }) : t === "cross-hatch" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 2, y1: 2, x2: 8, y2: 14, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 2, x2: 14, y2: 14, stroke: e.text, strokeWidth: 1.2 })
  ] }) : /* @__PURE__ */ u("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: /* @__PURE__ */ u("rect", { x: 2, y: 2, width: 16, height: 12, fill: e.text, rx: 2 }) });
}
const Iu = /* @__PURE__ */ k("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
  /* @__PURE__ */ u("circle", { cx: "11", cy: "11", r: "8" }),
  /* @__PURE__ */ u("path", { d: "m21 21-4.35-4.35" })
] });
function Yr({
  value: t,
  onChange: e,
  fontsInScene: o,
  triggerStyle: r
}) {
  var x, C;
  const n = qt(), [s, i] = tt(!1), [l, a] = tt(""), c = lt(null), h = lt(null), [p, d] = tt(null), f = l.trim().toLowerCase(), m = Kt(
    () => o.filter((I) => I.toLowerCase().includes(f)),
    [o, f]
  ), y = Kt(
    () => Sr.filter(
      (I) => !o.includes(I.key) && (I.key.toLowerCase().includes(f) || I.label.toLowerCase().includes(f))
    ),
    [o, f]
  );
  mt(() => {
    if (!s || !h.current) return;
    const I = h.current.getBoundingClientRect(), M = 260, D = 16;
    let T = I.left;
    T + M > window.innerWidth - D && (T = window.innerWidth - M - D), T < D && (T = D), d({ top: I.bottom + 4, left: T });
  }, [s]), mt(() => {
    var D;
    if (!s) return;
    const I = (T) => {
      var at, B;
      const P = T.target;
      if ((at = c.current) != null && at.contains(P)) return;
      const K = (((B = c.current) == null ? void 0 : B.ownerDocument) ?? document).getElementById("font-picker-popover");
      K != null && K.contains(P) || i(!1);
    }, M = ((D = c.current) == null ? void 0 : D.ownerDocument) ?? document;
    return M.addEventListener("mousedown", I), () => M.removeEventListener("mousedown", I);
  }, [s]);
  const g = (I) => {
    e(I), i(!1), a("");
  }, b = (I, M) => {
    const D = (M == null ? void 0 : M.label) ?? I, T = M == null ? void 0 : M.category, P = t === I;
    return /* @__PURE__ */ k(
      "button",
      {
        type: "button",
        onClick: () => g(I),
        style: {
          display: "flex",
          alignItems: "center",
          gap: 10,
          width: "100%",
          padding: "8px 12px",
          border: "none",
          background: P ? "rgba(139, 92, 246, 0.15)" : "transparent",
          color: "#1e1e2e",
          cursor: "pointer",
          textAlign: "left",
          fontFamily: Ue(I),
          fontSize: 14,
          borderRadius: 6
        },
        onMouseEnter: (G) => {
          P || (G.currentTarget.style.background = "rgba(0,0,0,0.05)");
        },
        onMouseLeave: (G) => {
          P || (G.currentTarget.style.background = "transparent");
        },
        children: [
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                width: 24,
                flexShrink: 0,
                fontSize: 12,
                color: "#64748b",
                fontFamily: "sans-serif"
              },
              children: Tl(T)
            }
          ),
          /* @__PURE__ */ u("span", { style: { flex: 1, overflow: "hidden", textOverflow: "ellipsis" }, children: D })
        ]
      },
      I
    );
  };
  return /* @__PURE__ */ k("div", { ref: c, style: { position: "relative", flex: 1, minWidth: 0 }, children: [
    /* @__PURE__ */ k(
      "button",
      {
        ref: h,
        type: "button",
        onClick: () => i((I) => !I),
        style: {
          display: "flex",
          alignItems: "center",
          width: "100%",
          height: 28,
          padding: "0 8px",
          background: n.controlBg,
          color: n.text,
          border: `1px solid ${n.separator}`,
          borderRadius: n.controlBorderRadius,
          fontSize: 11,
          fontFamily: Ue(t),
          cursor: "pointer",
          textAlign: "left",
          justifyContent: "space-between",
          ...r
        },
        children: [
          /* @__PURE__ */ u("span", { style: { overflow: "hidden", textOverflow: "ellipsis" }, children: ((x = Sr.find((I) => I.key === t)) == null ? void 0 : x.label) ?? t }),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flexShrink: 0,
                opacity: 0.7,
                transform: s ? "rotate(180deg)" : "none",
                transition: "transform 0.15s"
              },
              children: "▼"
            }
          )
        ]
      }
    ),
    s && p && Ne(
      /* @__PURE__ */ k(
        "div",
        {
          id: "font-picker-popover",
          style: {
            position: "fixed",
            top: p.top,
            left: p.left,
            width: 260,
            maxHeight: Math.min(320, window.innerHeight - p.top - 16),
            background: "#fff",
            borderRadius: 12,
            boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
            zIndex: 1e4,
            overflow: "hidden",
            display: "flex",
            flexDirection: "column"
          },
          children: [
            /* @__PURE__ */ k(
              "div",
              {
                style: {
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  padding: "10px 12px",
                  borderBottom: "1px solid #eee",
                  background: "#fafafa"
                },
                children: [
                  /* @__PURE__ */ u("span", { style: { color: "#64748b", display: "flex" }, children: Iu }),
                  /* @__PURE__ */ u(
                    "input",
                    {
                      type: "text",
                      placeholder: "Quick search",
                      value: l,
                      onChange: (I) => a(I.target.value),
                      autoFocus: !0,
                      style: {
                        flex: 1,
                        border: "none",
                        background: "transparent",
                        fontSize: 13,
                        outline: "none",
                        color: "#1e1e2e"
                      }
                    }
                  )
                ]
              }
            ),
            /* @__PURE__ */ k("div", { style: { overflowY: "auto", padding: 8, flex: 1 }, children: [
              m.length > 0 && /* @__PURE__ */ k("div", { style: { marginBottom: 12 }, children: [
                /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      fontSize: 10,
                      fontWeight: 600,
                      color: "#64748b",
                      textTransform: "uppercase",
                      letterSpacing: "0.05em",
                      marginBottom: 6,
                      paddingLeft: 4
                    },
                    children: "In this scene"
                  }
                ),
                m.map((I) => b(I, Sr.find((M) => M.key === I)))
              ] }),
              /* @__PURE__ */ k("div", { children: [
                /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      fontSize: 10,
                      fontWeight: 600,
                      color: "#64748b",
                      textTransform: "uppercase",
                      letterSpacing: "0.05em",
                      marginBottom: 6,
                      paddingLeft: 4
                    },
                    children: "Available fonts"
                  }
                ),
                y.length > 0 ? y.map((I) => b(I.key, I)) : /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      padding: "12px",
                      fontSize: 12,
                      color: "#94a3b8"
                    },
                    children: l ? "No fonts match your search" : "All fonts are in use"
                  }
                )
              ] })
            ] })
          ]
        }
      ),
      (((C = c.current) == null ? void 0 : C.ownerDocument) ?? document).body
    )
  ] });
}
function cs({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round",
    fill: "none"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "sharp" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", ...o }),
    t === "round" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "4", ...o })
  ] });
}
const Tu = [
  { label: "S", size: 14 },
  { label: "M", size: 20 },
  { label: "L", size: 28 },
  { label: "XL", size: 36 }
], Pu = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
];
function Ru({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
function lo(t, e) {
  if (t.length < 2) return !1;
  const o = e(t[0]);
  return !t.every((r) => e(r) === o);
}
function Au({ engine: t, node: e, fontsInScene: o }) {
  const r = qt(), n = Re(t, e), s = Wr(Xr) ?? [e], { data: i } = e, l = i.fill ?? null, a = i.fillStyle ?? "hachure", c = i.strokeStyle ?? "solid", h = lo(s, (b) => b.data.stroke), p = lo(s, (b) => b.data.fill ?? null), d = lo(s, (b) => b.data.fillStyle ?? "hachure"), f = lo(s, (b) => b.data.strokeStyle ?? "solid"), m = lo(s, (b) => b.data.strokeWidth), y = lo(s, (b) => b.data.roughness), g = lo(s, (b) => b.data.opacity ?? 1);
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ k(ye, { title: "Structure", persistKey: "shape.structure", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Shape" }),
        Pu.map((b) => /* @__PURE__ */ u(
          "button",
          {
            title: b.label,
            onClick: () => n({ shape: b.key }),
            style: {
              ...Ut,
              width: 28,
              height: 28,
              background: i.shape === b.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              borderRadius: r.controlBorderRadius
            },
            children: /* @__PURE__ */ u(Ru, { name: b.key })
          },
          b.key
        ))
      ] }),
      (i.shape === "rect" || i.shape === "diamond") && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Edges" }),
        [
          { key: "sharp", label: "Sharp" },
          { key: "round", label: "Round" }
        ].map((b) => /* @__PURE__ */ u(
          "button",
          {
            title: b.label,
            onClick: () => n({ edgeStyle: b.key === "sharp" ? void 0 : b.key }),
            style: {
              ...Ut,
              width: 28,
              height: 28,
              background: (i.edgeStyle ?? "sharp") === b.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              borderRadius: r.controlBorderRadius
            },
            children: /* @__PURE__ */ u(cs, { name: b.key })
          },
          b.key
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Label" }),
        /* @__PURE__ */ u(
          "input",
          {
            type: "text",
            value: i.label ?? "",
            placeholder: "Add label...",
            onChange: (b) => n({ label: b.target.value || void 0 }),
            style: {
              flex: 1,
              fontSize: 12,
              padding: "4px 6px",
              background: r.controlBg,
              color: r.text,
              border: `1px solid ${r.border}`,
              borderRadius: r.controlBorderRadius,
              outline: "none"
            }
          }
        )
      ] })
    ] }),
    i.label && /* @__PURE__ */ k(ye, { title: "Typography", defaultOpen: !1, persistKey: "shape.typography", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Yr,
          {
            value: i.labelFontFamily ?? "Excalifont",
            onChange: (b) => n({ labelFontFamily: b === "Excalifont" ? void 0 : b }),
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Size" }),
        Tu.map((b) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n({ labelFontSize: b.size === 14 ? void 0 : b.size }),
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: (i.labelFontSize ?? 14) === b.size ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 10,
              fontWeight: 600,
              borderRadius: r.controlBorderRadius
            },
            children: b.label
          },
          b.size
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Align" }),
        is.map((b) => /* @__PURE__ */ u(
          "button",
          {
            title: b.key,
            onClick: () => n({ labelAlign: b.key === "center" ? void 0 : b.key }),
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: (i.labelAlign ?? "center") === b.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 12,
              borderRadius: r.controlBorderRadius
            },
            children: b.label
          },
          b.key
        ))
      ] })
    ] }),
    /* @__PURE__ */ k(ye, { title: "Appearance", persistKey: "shape.appearance", children: [
      /* @__PURE__ */ u(
        me,
        {
          label: "Stroke",
          palettes: ve,
          value: h ? void 0 : i.stroke,
          mixed: h,
          onChange: (b) => n({ stroke: b })
        }
      ),
      /* @__PURE__ */ u(
        me,
        {
          label: "Fill",
          palettes: as,
          value: p ? void 0 : l,
          mixed: p,
          onChange: (b) => n({ fill: b ?? void 0 }),
          allowNull: !0
        }
      ),
      l && !p && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Fill pattern" }),
        ns.map((b) => /* @__PURE__ */ u(
          "button",
          {
            title: b.label,
            onClick: () => n({ fillStyle: b.key }),
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: !d && a === b.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 9,
              borderRadius: r.controlBorderRadius
            },
            children: /* @__PURE__ */ u(ls, { style: b.key })
          },
          b.key
        ))
      ] }),
      /* @__PURE__ */ u(
        Go,
        {
          label: "Stroke style",
          value: c,
          mixed: f,
          onChange: (b) => n({ strokeStyle: b })
        }
      ),
      /* @__PURE__ */ u(
        jo,
        {
          label: "Stroke width",
          value: i.strokeWidth,
          mixed: m,
          onChange: (b) => n({ strokeWidth: b })
        }
      ),
      /* @__PURE__ */ u(
        Se,
        {
          value: i.opacity ?? 1,
          mixed: g,
          onChange: (b) => n({ opacity: b })
        }
      )
    ] }),
    /* @__PURE__ */ u(ye, { title: "Sketch", defaultOpen: !1, persistKey: "shape.sketch", children: /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Roughness" }),
      ss.map((b) => /* @__PURE__ */ u(
        "button",
        {
          title: b.label,
          onClick: () => n({ roughness: b.value }),
          style: {
            ...Ut,
            height: 28,
            padding: "0 8px",
            background: !y && i.roughness === b.value ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: b.label
        },
        b.value
      ))
    ] }) })
  ] });
}
function Po(t, e) {
  if (t.length < 2) return !1;
  const o = e(t[0]);
  return !t.every((r) => e(r) === o);
}
function Lu({ engine: t, node: e }) {
  const o = qt(), r = Re(t, e), n = Wr(Xr) ?? [e], { data: s } = e, i = s.fill ?? null, l = s.fillStyle ?? "hachure", a = s.strokeStyle ?? "solid", c = Po(n, (y) => y.data.color), h = Po(n, (y) => y.data.fill ?? null), p = Po(n, (y) => y.data.fillStyle ?? "hachure"), d = Po(n, (y) => y.data.strokeStyle ?? "solid"), f = Po(n, (y) => y.data.strokeWidth), m = Po(n, (y) => y.data.opacity ?? 1);
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      me,
      {
        label: "Stroke",
        palettes: ve,
        value: c ? void 0 : s.color,
        mixed: c,
        onChange: (y) => r({ color: y })
      }
    ),
    /* @__PURE__ */ u(
      me,
      {
        label: "Fill",
        palettes: as,
        value: h ? void 0 : i,
        mixed: h,
        onChange: (y) => r({ fill: y ?? void 0 }),
        allowNull: !0
      }
    ),
    i && !h && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Fill pattern" }),
      ns.map((y) => /* @__PURE__ */ u(
        "button",
        {
          title: y.label,
          onClick: () => r({ fillStyle: y.key }),
          style: {
            ...Ut,
            width: 36,
            height: 28,
            background: !p && l === y.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 9,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(ls, { style: y.key })
        },
        y.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Go,
      {
        label: "Stroke style",
        value: a,
        mixed: d,
        onChange: (y) => r({ strokeStyle: y })
      }
    ),
    /* @__PURE__ */ u(
      jo,
      {
        label: "Stroke width",
        value: s.strokeWidth,
        mixed: f,
        onChange: (y) => r({ strokeWidth: y })
      }
    ),
    /* @__PURE__ */ u(
      Se,
      {
        value: s.opacity ?? 1,
        mixed: m,
        onChange: (y) => r({ opacity: y })
      }
    )
  ] });
}
function Du({ engine: t, node: e, fontsInScene: o }) {
  const r = qt(), n = Re(t, e), { data: s } = e;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ k(ye, { title: "Typography", persistKey: "text.typography", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Yr,
          {
            value: s.fontFamily,
            onChange: (i) => n({ fontFamily: i }),
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Size" }),
        va.map((i) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n({ fontSize: i }),
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: s.fontSize === i ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 10,
              borderRadius: r.controlBorderRadius
            },
            children: i
          },
          i
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Align" }),
        is.map((i) => /* @__PURE__ */ u(
          "button",
          {
            title: i.key,
            onClick: () => n({ align: i.key }),
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: s.align === i.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 12,
              borderRadius: r.controlBorderRadius
            },
            children: i.label
          },
          i.key
        ))
      ] })
    ] }),
    /* @__PURE__ */ k(ye, { title: "Appearance", persistKey: "text.appearance", children: [
      /* @__PURE__ */ u(
        me,
        {
          label: "Color",
          palettes: ve,
          value: s.color,
          onChange: (i) => n({ color: i })
        }
      ),
      /* @__PURE__ */ u(
        lr,
        {
          borderColor: s.borderColor ?? null,
          borderStyle: s.borderStyle,
          borderWidth: s.borderWidth,
          onChange: (i, l) => n({ [i]: l })
        }
      ),
      /* @__PURE__ */ u(
        Se,
        {
          value: s.opacity ?? 1,
          onChange: (i) => n({ opacity: i })
        }
      )
    ] })
  ] });
}
function Eu({ engine: t, node: e }) {
  const o = qt(), r = Re(t, e), { data: n } = e;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ k(ye, { title: "Line", persistKey: "edge.line", children: [
      /* @__PURE__ */ u(
        me,
        {
          label: "Color",
          palettes: ve,
          value: n.color,
          onChange: (s) => r({ color: s })
        }
      ),
      /* @__PURE__ */ u(
        Go,
        {
          label: "Style",
          value: n.style,
          onChange: (s) => r({ style: s })
        }
      ),
      /* @__PURE__ */ u(
        jo,
        {
          label: "Width",
          value: n.strokeWidth,
          onChange: (s) => r({ strokeWidth: s })
        }
      )
    ] }),
    /* @__PURE__ */ k(ye, { title: "Arrows", persistKey: "edge.arrows", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Head" }),
        ["none", "arrow", "filled", "dot"].map((s) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r({ arrowHead: s }),
            style: {
              ...Ut,
              height: 28,
              padding: "0 6px",
              background: (n.arrowHead ?? "none") === s ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 11,
              borderRadius: o.controlBorderRadius
            },
            children: s === "none" ? "None" : s === "arrow" ? "▷" : s === "filled" ? "▶" : "●"
          },
          s
        ))
      ] }),
      (n.arrowHead ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Head size" }),
        /* @__PURE__ */ u(
          "input",
          {
            type: "range",
            min: 4,
            max: 40,
            step: 1,
            value: n.arrowHeadSize ?? Math.max(8, n.strokeWidth * 3),
            onChange: (s) => r({ arrowHeadSize: Number(s.target.value) }),
            style: { flex: 1, accentColor: o.accentColor }
          }
        ),
        /* @__PURE__ */ u("span", { style: { color: o.textMuted, fontSize: 11, minWidth: 24, textAlign: "right" }, children: n.arrowHeadSize ?? Math.max(8, n.strokeWidth * 3) })
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Tail" }),
        ["none", "arrow", "filled", "dot"].map((s) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r({ arrowTail: s }),
            style: {
              ...Ut,
              height: 28,
              padding: "0 6px",
              background: (n.arrowTail ?? "none") === s ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 11,
              borderRadius: o.controlBorderRadius
            },
            children: s === "none" ? "None" : s === "arrow" ? "◁" : s === "filled" ? "◀" : "●"
          },
          s
        ))
      ] }),
      (n.arrowTail ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Tail size" }),
        /* @__PURE__ */ u(
          "input",
          {
            type: "range",
            min: 4,
            max: 40,
            step: 1,
            value: n.arrowTailSize ?? Math.max(8, n.strokeWidth * 3),
            onChange: (s) => r({ arrowTailSize: Number(s.target.value) }),
            style: { flex: 1, accentColor: o.accentColor }
          }
        ),
        /* @__PURE__ */ u("span", { style: { color: o.textMuted, fontSize: 11, minWidth: 24, textAlign: "right" }, children: n.arrowTailSize ?? Math.max(8, n.strokeWidth * 3) })
      ] })
    ] }),
    /* @__PURE__ */ k(ye, { title: "Path & Motion", defaultOpen: !1, persistKey: "edge.path-motion", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Path" }),
        [
          { key: "bezier", label: "Bezier" },
          { key: "straight", label: "Straight" },
          { key: "smoothstep", label: "Smooth" },
          { key: "step", label: "Step" }
        ].map((s) => /* @__PURE__ */ u(
          "button",
          {
            title: s.label,
            onClick: () => r({ edgeType: s.key }),
            style: {
              ...Ut,
              height: 28,
              padding: "0 6px",
              background: (n.edgeType ?? "bezier") === s.key ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 9,
              borderRadius: o.controlBorderRadius
            },
            children: s.label
          },
          s.key
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Animate" }),
        /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r({ animated: !n.animated }),
            style: {
              ...Ut,
              height: 28,
              padding: "0 12px",
              background: n.animated ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 11,
              borderRadius: o.controlBorderRadius
            },
            children: n.animated ? "On" : "Off"
          }
        )
      ] }),
      n.animated && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Direction" }),
        ["forward", "reverse", "both"].map((s) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r({ animatedDirection: s }),
            style: {
              ...Ut,
              height: 28,
              padding: "0 6px",
              background: (n.animatedDirection ?? "forward") === s ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 10,
              borderRadius: o.controlBorderRadius
            },
            children: s === "forward" ? "→" : s === "reverse" ? "←" : "⇆"
          },
          s
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Roughness" }),
        ss.map((s) => /* @__PURE__ */ u(
          "button",
          {
            title: s.label,
            onClick: () => r({ roughness: s.value }),
            style: {
              ...Ut,
              height: 28,
              padding: "0 8px",
              background: (n.roughness ?? 0) === s.value ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 9,
              borderRadius: o.controlBorderRadius
            },
            children: s.label
          },
          s.value
        ))
      ] })
    ] }),
    /* @__PURE__ */ u(ye, { title: "Label", defaultOpen: !1, persistKey: "edge.label", children: /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Text" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: n.label ?? "",
          onChange: (s) => r({ label: s.target.value || void 0 }),
          placeholder: "Edge label...",
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 11,
            outline: "none"
          }
        }
      )
    ] }) })
  ] });
}
function Wu({ engine: t, node: e }) {
  const o = qt(), [r, n] = tt("idle"), s = Re(t, e), { data: i } = e, l = !!i.crop;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      lr,
      {
        borderColor: i.borderColor ?? null,
        borderStyle: i.borderStyle,
        borderWidth: i.borderWidth,
        onChange: (a, c) => s({ [a]: c })
      }
    ),
    /* @__PURE__ */ k("div", { style: { ...Lt, marginTop: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Crop" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.requestImageCrop(e.id),
          style: {
            ...Ut,
            height: 28,
            padding: "0 10px",
            background: o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: "Crop"
        }
      ),
      l && /* @__PURE__ */ u(
        "button",
        {
          onClick: () => s({ crop: void 0 }),
          style: {
            ...Ut,
            height: 28,
            padding: "0 10px",
            background: o.controlBg,
            color: o.textMuted,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: "Reset"
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: { ...Lt, marginTop: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Background" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: async () => {
            if (r !== "loading") {
              n("loading");
              try {
                const { removeBackground: a } = await import("@imgly/background-removal"), h = await (await fetch(i.src)).blob(), p = await a(h), d = new FileReader(), f = await new Promise((m, y) => {
                  d.onload = () => m(d.result), d.onerror = y, d.readAsDataURL(p);
                });
                s({ src: f }), n("idle");
              } catch (a) {
                console.error("Background removal failed:", a), n("error"), setTimeout(() => n("idle"), 3e3);
              }
            }
          },
          disabled: r === "loading",
          style: {
            ...Ut,
            height: 28,
            padding: "0 10px",
            background: r === "error" ? o.error : o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius,
            gap: 4,
            opacity: r === "loading" ? 0.6 : 1
          },
          children: r === "loading" ? "Removing..." : r === "error" ? "Failed" : "Remove BG"
        }
      )
    ] }),
    /* @__PURE__ */ u(
      Se,
      {
        value: i.opacity ?? 1,
        onChange: (a) => s({ opacity: a })
      }
    )
  ] });
}
function Fu({ engine: t, node: e }) {
  const o = qt(), r = Re(t, e), { data: n } = e;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      lr,
      {
        borderColor: n.borderColor ?? null,
        borderStyle: n.borderStyle,
        borderWidth: n.borderWidth,
        onChange: (s, i) => r({ [s]: i })
      }
    ),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => r({ edgeStyle: s.key === "sharp" ? void 0 : s.key }),
          style: {
            ...Ut,
            width: 28,
            height: 28,
            background: (n.edgeStyle ?? "sharp") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(cs, { name: s.key })
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Se,
      {
        value: n.opacity ?? 1,
        onChange: (s) => r({ opacity: s })
      }
    )
  ] });
}
const tr = {
  pan: 400,
  fade: 500,
  dissolve: 400,
  zoom: 600,
  fold: 700,
  cube: 1200,
  none: 0
}, Bu = Hh();
function Nu({
  value: t,
  onChange: e,
  theme: o
}) {
  const [r, n] = tt(String(t));
  mt(() => n(String(t)), [t]);
  const s = () => {
    const i = parseInt(r, 10);
    !isNaN(i) && i >= 100 && i <= 5e3 ? e(i) : n(String(t));
  };
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Duration" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "number",
        min: 100,
        max: 5e3,
        step: 50,
        value: r,
        onChange: (i) => n(i.target.value),
        onBlur: s,
        onKeyDown: (i) => {
          i.key === "Enter" && s();
        },
        style: {
          width: 64,
          background: o.controlBg,
          color: o.text,
          border: "none",
          borderRadius: o.controlBorderRadius,
          padding: "4px 6px",
          fontSize: 11,
          outline: "none"
        }
      }
    ),
    /* @__PURE__ */ u("span", { style: { fontSize: 10, color: o.textMuted }, children: "ms" })
  ] });
}
function Hu({ engine: t, node: e }) {
  const o = qt(), r = Re(t, e), { data: n } = e, s = it(
    (l) => {
      var d;
      if (!l) {
        r({ devicePreset: void 0 });
        return;
      }
      const a = Dn(l);
      if (!a) return;
      const c = ka(a), h = Math.round(e.w / c), p = { devicePreset: l };
      (!n.label || ((d = Dn(n.devicePreset ?? "")) == null ? void 0 : d.label) === n.label) && (p.label = a.label), r(p), t.updateNodeWithHistory(e.id, { h });
    },
    [t, e, n.label, n.devicePreset, r]
  ), i = Kt(() => {
    const l = t.getAllNodes().filter((d) => d.type === "frame"), a = l.length, c = /* @__PURE__ */ new Set();
    for (const d of l)
      d.id !== e.id && d.data.slideOrder != null && c.add(d.data.slideOrder);
    const h = [];
    for (let d = 1; d <= a; d++)
      c.has(d) || h.push(d);
    const p = e.data.slideOrder;
    return p != null && !h.includes(p) && (h.push(p), h.sort((d, f) => d - f)), h;
  }, [t, e]);
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Label" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: n.label ?? "",
          onChange: (l) => r({ label: l.target.value || void 0 }),
          placeholder: "Frame label...",
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 11,
            outline: "none"
          }
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Device" }),
      /* @__PURE__ */ k(
        "select",
        {
          value: n.devicePreset ?? "",
          onChange: (l) => s(l.target.value),
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "", children: "Freeform" }),
            Bu.map((l) => /* @__PURE__ */ u("optgroup", { label: l.label, children: l.presets.map((a) => /* @__PURE__ */ k("option", { value: a.key, children: [
              a.label,
              " (",
              a.w,
              "×",
              a.h,
              ")"
            ] }, a.key)) }, l.label))
          ]
        }
      )
    ] }),
    /* @__PURE__ */ u(
      me,
      {
        label: "Background",
        palettes: ve,
        value: (() => {
          const l = n.backgroundColor;
          if (!l) return null;
          for (const a of ve) {
            const c = a.colors.find((h) => l === `${h}15`);
            if (c) return c;
          }
          return l.length === 9 && l.endsWith("15") ? l.slice(0, 7) : null;
        })(),
        onChange: (l) => r({ backgroundColor: l ? `${l}15` : void 0 }),
        allowNull: !0
      }
    ),
    /* @__PURE__ */ u(
      me,
      {
        label: "Border",
        palettes: ve,
        value: n.borderColor,
        onChange: (l) => r({ borderColor: l })
      }
    ),
    /* @__PURE__ */ u(
      Go,
      {
        label: "Style",
        value: n.borderStyle ?? "dashed",
        onChange: (l) => r({ borderStyle: l })
      }
    ),
    /* @__PURE__ */ u(
      jo,
      {
        label: "Width",
        value: n.borderWidth ?? 1,
        onChange: (l) => r({ borderWidth: l })
      }
    ),
    /* @__PURE__ */ u(
      Se,
      {
        value: n.opacity ?? 1,
        onChange: (l) => r({ opacity: l })
      }
    ),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Slide #" }),
      /* @__PURE__ */ k(
        "select",
        {
          value: n.slideOrder ?? "",
          onChange: (l) => {
            const a = l.target.value;
            r({ slideOrder: a ? parseInt(a, 10) : void 0 });
          },
          style: {
            width: 72,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "", children: "Auto" }),
            i.map((l) => /* @__PURE__ */ u("option", { value: l, children: l }, l))
          ]
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Transition" }),
      /* @__PURE__ */ k(
        "select",
        {
          value: n.transition ?? "pan",
          onChange: (l) => {
            const a = l.target.value;
            r({ transition: a === "pan" ? void 0 : a, transitionDuration: void 0 });
          },
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "pan", children: "Pan" }),
            /* @__PURE__ */ u("option", { value: "fade", children: "Fade to Black" }),
            /* @__PURE__ */ u("option", { value: "dissolve", children: "Dissolve" }),
            /* @__PURE__ */ u("option", { value: "zoom", children: "Zoom" }),
            /* @__PURE__ */ u("option", { value: "fold", children: "Fold" }),
            /* @__PURE__ */ u("option", { value: "cube", children: "Cube" }),
            /* @__PURE__ */ u("option", { value: "none", children: "None (instant)" })
          ]
        }
      )
    ] }),
    (n.transition ?? "pan") !== "none" && /* @__PURE__ */ u(
      Nu,
      {
        value: n.transitionDuration ?? tr[n.transition ?? "pan"],
        onChange: (l) => r({ transitionDuration: l === tr[n.transition ?? "pan"] ? void 0 : l }),
        theme: o
      }
    )
  ] });
}
function Ou({ engine: t, node: e }) {
  const o = qt(), r = Re(t, e), { data: n } = e;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      me,
      {
        label: "Color",
        palettes: Uh,
        value: n.color,
        onChange: (s) => {
          s && r({ color: s });
        }
      }
    ),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Size" }),
      [12, 14, 16, 20, 24].map((s) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => r({ fontSize: s }),
          style: {
            ...Ut,
            width: 32,
            height: 24,
            background: (n.fontSize ?? 16) === s ? o.controlBgActive : o.controlBg,
            borderRadius: o.controlBorderRadius,
            fontSize: 10,
            color: o.text
          },
          children: s
        },
        s
      ))
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => r({ edgeStyle: s.key === "sharp" ? void 0 : s.key }),
          style: {
            ...Ut,
            width: 28,
            height: 28,
            background: (n.edgeStyle ?? "sharp") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(cs, { name: s.key })
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Se,
      {
        value: n.opacity ?? 1,
        onChange: (s) => r({ opacity: s })
      }
    )
  ] });
}
function Xu({ engine: t, node: e }) {
  const o = qt(), r = Re(t, e), { data: n } = e;
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "URL" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: n.url,
          readOnly: !0,
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.textMuted,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 10,
            outline: "none",
            cursor: "default"
          },
          onClick: (s) => s.target.select()
        }
      )
    ] }),
    /* @__PURE__ */ u(
      lr,
      {
        borderColor: n.borderColor ?? null,
        borderStyle: n.borderStyle,
        borderWidth: n.borderWidth,
        onChange: (s, i) => r({ [s]: i })
      }
    ),
    /* @__PURE__ */ u(
      Se,
      {
        value: n.opacity ?? 1,
        onChange: (s) => r({ opacity: s })
      }
    )
  ] });
}
function Yu({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
const Gu = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
];
function ju({ engine: t, mode: e, fontsInScene: o }) {
  const r = qt(), [, n] = tt(0), s = it(() => n((m) => m + 1), []), i = t.activeTool;
  if (e === "text") {
    const m = i.fontFamily ?? Ze, y = i.fontSize ?? 20, g = i.textAlign ?? "left", b = i.color;
    return /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Yr,
          {
            value: m,
            onChange: (x) => {
              i.fontFamily = x, s();
            },
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Size" }),
        va.map((x) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => {
              i.fontSize = x, s();
            },
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: y === x ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 10,
              borderRadius: r.controlBorderRadius
            },
            children: x
          },
          x
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Align" }),
        is.map((x) => /* @__PURE__ */ u(
          "button",
          {
            title: x.key,
            onClick: () => {
              i.textAlign = x.key, s();
            },
            style: {
              ...Ut,
              width: 36,
              height: 28,
              background: g === x.key ? r.controlBgActive : r.controlBg,
              color: r.text,
              fontSize: 12,
              borderRadius: r.controlBorderRadius
            },
            children: x.label
          },
          x.key
        ))
      ] }),
      /* @__PURE__ */ u(
        me,
        {
          label: "Color",
          palettes: ve,
          value: b,
          onChange: (x) => {
            i.color = x, s();
          }
        }
      ),
      /* @__PURE__ */ u(
        Se,
        {
          value: i.opacity ?? 1,
          onChange: (x) => {
            i.opacity = x, s();
          }
        }
      )
    ] });
  }
  const l = e === "shape", a = i.color, c = i.fillColor ?? null, h = i.fillStyle ?? "hachure", p = i.strokeStyle ?? "solid", d = i.width, f = i.roughness ?? 1;
  return /* @__PURE__ */ k(ut, { children: [
    l && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Shape" }),
      Gu.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => {
            i.shapeType = m.key, s();
          },
          style: {
            ...Ut,
            width: 28,
            height: 28,
            background: (i.shapeType ?? "rect") === m.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(Yu, { name: m.key })
        },
        m.key
      ))
    ] }),
    /* @__PURE__ */ u(
      me,
      {
        label: "Stroke",
        palettes: ve,
        value: a,
        onChange: (m) => {
          i.color = m, s();
        }
      }
    ),
    /* @__PURE__ */ u(
      me,
      {
        label: "Fill",
        palettes: as,
        value: c,
        onChange: (m) => {
          i.fillColor = m ?? void 0, s();
        },
        allowNull: !0
      }
    ),
    c && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Fill pattern" }),
      ns.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => {
            i.fillStyle = m.key, s();
          },
          style: {
            ...Ut,
            width: 36,
            height: 28,
            background: h === m.key ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: /* @__PURE__ */ u(ls, { style: m.key })
        },
        m.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Go,
      {
        label: "Stroke style",
        value: p,
        onChange: (m) => {
          i.strokeStyle = m, s();
        }
      }
    ),
    /* @__PURE__ */ u(
      jo,
      {
        label: "Stroke width",
        value: d,
        onChange: (m) => {
          i.width = m, s();
        }
      }
    ),
    l && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Roughness" }),
      ss.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => {
            i.roughness = m.value, s();
          },
          style: {
            ...Ut,
            height: 28,
            padding: "0 8px",
            background: f === m.value ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 9,
            borderRadius: r.controlBorderRadius
          },
          children: m.label
        },
        m.value
      ))
    ] }),
    /* @__PURE__ */ u(
      Se,
      {
        value: i.opacity ?? 1,
        onChange: (m) => {
          i.opacity = m, s();
        }
      }
    )
  ] });
}
function Vu({ engine: t, node: e, PanelComponent: o }) {
  const r = Re(t, e);
  return /* @__PURE__ */ u(o, { node: e, data: e.data, engine: t, updateData: r });
}
const Wn = {
  shape: "Shape",
  draw: "Drawing",
  text: "Text",
  edge: "Edge",
  image: "Image",
  content: "Content",
  frame: "Frame",
  sticky: "Sticky Note",
  youtube: "YouTube"
}, Ku = /* @__PURE__ */ new Set(["shape", "draw", "text", "image", "content", "frame", "sticky", "youtube"]), qu = /* @__PURE__ */ new Set(["text", "image", "content", "frame", "youtube"]);
function Zu(t) {
  const e = /* @__PURE__ */ new Set(), o = [];
  for (const r of t.getAllNodes()) {
    let n;
    r.type === "text" ? n = r.data.fontFamily : r.type === "shape" && (n = r.data.labelFontFamily), n && !e.has(n) && (e.add(n), o.push(n));
  }
  return o;
}
function Uu({ label: t }) {
  const e = qt();
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        fontSize: 10,
        fontWeight: 600,
        color: e.textSecondary,
        padding: "2px 0 6px",
        borderBottom: `1px solid ${e.border}`,
        marginBottom: 2
      },
      children: t
    }
  );
}
function Qu({
  engine: t,
  open: e,
  onToggle: o
}) {
  const r = qt(), [n, s] = tt(t.snapToGrid), [i, l] = tt(t.gridSize), [a, c] = tt(t.smartGuides), [h, p] = tt(t.boardBackground);
  mt(() => {
    const f = () => {
      s(t.snapToGrid), l(t.gridSize), c(t.smartGuides);
    }, m = () => p(t.boardBackground);
    return t.on("guides", f), t.on("background", m), () => {
      t.off("guides", f), t.off("background", m);
    };
  }, [t]);
  const d = [10, 20, 40, 80];
  return /* @__PURE__ */ k(ye, { title: "Canvas", defaultOpen: !1, variant: "group", open: e, onToggle: o, children: [
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Grid" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.toggleSnapToGrid(),
          style: {
            border: "none",
            borderRadius: r.controlBorderRadius,
            background: n ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 10,
            padding: "4px 10px",
            cursor: "pointer"
          },
          children: n ? "On" : "Off"
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Grid size" }),
      /* @__PURE__ */ u("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", flex: 1 }, children: d.map((f) => /* @__PURE__ */ k(
        "button",
        {
          onClick: () => t.setGridSize(f),
          style: {
            border: "none",
            borderRadius: r.controlBorderRadius,
            background: i === f ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 10,
            padding: "4px 8px",
            cursor: "pointer"
          },
          children: [
            f,
            "px"
          ]
        },
        f
      )) })
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Guides" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.toggleSmartGuides(),
          style: {
            border: "none",
            borderRadius: r.controlBorderRadius,
            background: a ? r.controlBgActive : r.controlBg,
            color: r.text,
            fontSize: 10,
            padding: "4px 10px",
            cursor: "pointer"
          },
          children: a ? "On" : "Off"
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Dt, color: r.textMuted }, children: "Paper" }),
      /* @__PURE__ */ u(
        "select",
        {
          value: h,
          onChange: (f) => t.setBoardBackground(f.target.value),
          style: {
            flex: 1,
            height: 28,
            border: `1px solid ${r.border}`,
            borderRadius: r.controlBorderRadius,
            background: r.controlBg,
            color: r.text,
            fontSize: 11,
            padding: "0 8px",
            outline: "none"
          },
          children: No.map((f) => /* @__PURE__ */ u("option", { value: f.key, children: f.label }, f.key))
        }
      )
    ] })
  ] });
}
function Sa({
  engine: t,
  node: e,
  registry: o,
  fontsInScene: r
}) {
  switch (e.type) {
    case "shape":
      return /* @__PURE__ */ u(Au, { engine: t, node: e, fontsInScene: r });
    case "draw":
      return /* @__PURE__ */ u(Lu, { engine: t, node: e });
    case "text":
      return /* @__PURE__ */ u(Du, { engine: t, node: e, fontsInScene: r });
    case "edge":
      return /* @__PURE__ */ u(Eu, { engine: t, node: e });
    case "image":
      return /* @__PURE__ */ u(Wu, { engine: t, node: e });
    case "content":
      return /* @__PURE__ */ u(Fu, { engine: t, node: e });
    case "frame":
      return /* @__PURE__ */ u(Hu, { engine: t, node: e });
    case "sticky":
      return /* @__PURE__ */ u(Ou, { engine: t, node: e });
    case "youtube":
      return /* @__PURE__ */ u(Xu, { engine: t, node: e });
    default: {
      const n = o == null ? void 0 : o.get(e.type);
      return n != null && n.propertiesPanel ? /* @__PURE__ */ u(Vu, { engine: t, node: e, PanelComponent: n.propertiesPanel }) : null;
    }
  }
}
function li({
  engine: t,
  nodes: e
}) {
  const o = qt(), r = Math.round(e[0].rotation ?? 0), s = e.every(
    (c) => Math.round(c.rotation ?? 0) === r
  ) ? r : null, [i, l] = tt(null), a = it(
    (c) => {
      l(null);
      const h = parseFloat(c);
      if (isNaN(h)) return;
      const p = Math.max(-360, Math.min(360, h)), d = e.map((f) => ({
        id: f.id,
        patch: { rotation: p }
      }));
      t.batchUpdateWithHistory(d);
    },
    [t, e]
  );
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Dt, color: o.textMuted }, children: "Rotation" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "number",
        min: -360,
        max: 360,
        value: i ?? (s !== null ? String(s) : ""),
        placeholder: s === null ? "Mixed" : void 0,
        onChange: (c) => l(c.target.value),
        onBlur: (c) => a(c.target.value),
        onKeyDown: (c) => {
          c.key === "Enter" && a(c.target.value), c.key === "Escape" && l(null);
        },
        style: {
          width: 52,
          height: 24,
          border: `1px solid ${o.border}`,
          borderRadius: o.controlBorderRadius,
          background: o.controlBg,
          color: o.text,
          fontSize: 10,
          textAlign: "center",
          outline: "none",
          padding: "0 2px"
        }
      }
    ),
    /* @__PURE__ */ u("span", { style: { fontSize: 10, color: o.textMuted }, children: "°" })
  ] });
}
function Ju({
  engine: t,
  nodes: e,
  commonProps: o
}) {
  const r = it(
    (n, s) => {
      const i = n === "opacity" ? Ku : qu, l = e.filter((a) => i.has(a.type)).map((a) => ({
        id: a.id,
        patch: {
          data: { ...a.data, [n]: s }
        }
      }));
      t.batchUpdateWithHistory(l);
    },
    [t, e]
  );
  return /* @__PURE__ */ k(ut, { children: [
    o.opacity !== void 0 && /* @__PURE__ */ u(
      Se,
      {
        value: o.opacity === "mixed" ? void 0 : o.opacity,
        mixed: o.opacity === "mixed",
        onChange: (n) => r("opacity", n)
      }
    ),
    o.borderColor !== void 0 && /* @__PURE__ */ u(
      lr,
      {
        borderColor: o.borderColor === "mixed" ? void 0 : o.borderColor,
        borderStyle: o.borderStyle === "mixed" ? void 0 : o.borderStyle,
        borderWidth: o.borderWidth === "mixed" ? void 0 : o.borderWidth,
        mixed: {
          color: o.borderColor === "mixed",
          style: o.borderStyle === "mixed",
          width: o.borderWidth === "mixed"
        },
        onChange: (n, s) => r(n, s)
      }
    )
  ] });
}
function $u({
  engine: t,
  group: e,
  registry: o,
  fontsInScene: r,
  open: n,
  onToggle: s
}) {
  const i = Wn[e.type] ?? e.type, l = e.nodes.length, a = e.nodes[0], c = `${i} (${l})`;
  return /* @__PURE__ */ u(ye, { title: c, defaultOpen: !1, variant: "group", open: n, onToggle: s, children: /* @__PURE__ */ u(Xr.Provider, { value: e.nodes, children: /* @__PURE__ */ u(
    Sa,
    {
      engine: t,
      node: a,
      registry: o,
      fontsInScene: r
    }
  ) }) });
}
function _u(t) {
  switch (t.kind) {
    case "none":
      return "No selection";
    case "tool":
      return `${t.mode.charAt(0).toUpperCase() + t.mode.slice(1)} tool`;
    case "single":
      return Wn[t.node.type] ?? t.node.type;
    case "multi":
      return t.typeGroups.map(
        (o) => `${o.nodes.length} ${(Wn[o.type] ?? o.type).toLowerCase()}${o.nodes.length > 1 ? "s" : ""}`
      ).join(", ");
  }
}
function ci({
  engine: t,
  registry: e,
  target: o,
  commonProps: r
}) {
  const n = Kt(() => Zu(t), [t, o]), s = _u(o), [i, l] = tt("shared");
  return mt(() => {
    if (o.kind !== "multi") {
      l("shared");
      return;
    }
    (/* @__PURE__ */ new Set(["canvas", "shared", ...o.typeGroups.map((c) => c.type)])).has(i) || l("shared");
  }, [o, i]), /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(Uu, { label: s }),
    /* @__PURE__ */ u(
      Qu,
      {
        engine: t,
        open: o.kind === "multi" ? i === "canvas" : void 0,
        onToggle: o.kind === "multi" ? () => l((a) => a === "canvas" ? "" : "canvas") : void 0
      }
    ),
    o.kind === "tool" && /* @__PURE__ */ u(ju, { engine: t, mode: o.mode, fontsInScene: n }),
    o.kind === "single" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u(
        Sa,
        {
          engine: t,
          node: o.node,
          registry: e,
          fontsInScene: n
        }
      ),
      /* @__PURE__ */ u(li, { engine: t, nodes: [o.node] })
    ] }),
    o.kind === "multi" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ k(
        ye,
        {
          title: "Shared",
          defaultOpen: !0,
          variant: "group",
          open: i === "shared",
          onToggle: () => l((a) => a === "shared" ? "" : "shared"),
          children: [
            /* @__PURE__ */ u(Ju, { engine: t, nodes: o.nodes, commonProps: r }),
            /* @__PURE__ */ u(li, { engine: t, nodes: o.nodes })
          ]
        }
      ),
      o.typeGroups.map((a) => /* @__PURE__ */ u(
        $u,
        {
          engine: t,
          group: a,
          registry: e,
          fontsInScene: n,
          open: i === a.type,
          onToggle: () => l((c) => c === a.type ? "" : a.type)
        },
        a.type
      ))
    ] })
  ] });
}
function tp({ engine: t, registry: e }) {
  const o = qt(), { target: r, commonProps: n } = Cu(t), s = r.kind !== "none", [i, l] = tt(!1), a = lt(null), [c, h] = tt(null), p = lt(null), [d, f] = tt(!1), m = it(() => {
    var T, P;
    const M = (T = a.current) == null ? void 0 : T.offsetParent;
    if (M) return { width: M.clientWidth, height: M.clientHeight };
    const D = ((P = a.current) == null ? void 0 : P.ownerDocument.defaultView) ?? window;
    return { width: D.innerWidth, height: D.innerHeight };
  }, []), y = it(() => {
    const { width: M } = m();
    return { x: M - _o - 16, y: 12 };
  }, [m]), g = c ?? y(), b = lt(!1);
  wi(() => {
    if (!b.current && a.current && !c) {
      b.current = !0;
      const M = a.current.offsetParent;
      M && h({ x: M.clientWidth - _o - 16, y: 12 });
    }
  }), mt(() => {
    var T, P;
    const M = ((T = a.current) == null ? void 0 : T.offsetParent) ?? ((P = a.current) == null ? void 0 : P.ownerDocument.body);
    if (!M) return;
    const D = new ResizeObserver((G) => {
      var at;
      const K = ((at = G[0]) == null ? void 0 : at.contentRect.width) ?? M.clientWidth;
      l(K < 600);
    });
    return D.observe(M), l(M.clientWidth < 600), () => D.disconnect();
  }, []);
  const x = it(
    (M) => {
      M.stopPropagation(), f(!0);
      const D = c ? c.x : y().x, T = c ? c.y : y().y;
      p.current = {
        startX: M.clientX,
        startY: M.clientY,
        startLeft: D,
        startTop: T
      }, M.currentTarget.setPointerCapture(M.pointerId);
    },
    [c, y]
  ), C = it(
    (M) => {
      if (!p.current) return;
      M.stopPropagation();
      const D = M.clientX - p.current.startX, T = M.clientY - p.current.startY, { width: P, height: G } = m(), K = Math.max(
        ar,
        Math.min(P - _o - 8, p.current.startLeft + D)
      ), at = Math.max(
        8,
        Math.min(G - 100, p.current.startTop + T)
      );
      h({ x: K, y: at });
    },
    [m]
  ), I = it(() => {
    p.current = null, f(!1);
  }, []);
  return s ? i ? /* @__PURE__ */ k(
    "div",
    {
      ref: a,
      "data-sb-props-panel": !0,
      onPointerDown: (M) => M.stopPropagation(),
      style: {
        position: "fixed",
        bottom: 0,
        left: 0,
        right: 0,
        height: "45vh",
        minHeight: 200,
        background: o.panelBg,
        borderRadius: "12px 12px 0 0",
        boxShadow: "0 -4px 24px rgba(0,0,0,0.25)",
        zIndex: 200,
        display: "flex",
        flexDirection: "column",
        color: o.text,
        fontSize: 12
      },
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              height: 36,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              flexShrink: 0
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                style: {
                  width: 36,
                  height: 4,
                  borderRadius: 2,
                  background: o.border
                }
              }
            )
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              overflowY: "auto",
              padding: "0 16px 24px",
              flex: 1,
              touchAction: "pan-y"
            },
            children: /* @__PURE__ */ u(
              ci,
              {
                engine: t,
                registry: e,
                target: r,
                commonProps: n
              }
            )
          }
        )
      ]
    }
  ) : /* @__PURE__ */ k(
    "div",
    {
      ref: a,
      "data-sb-props-panel": !0,
      style: {
        position: "absolute",
        left: g.x,
        top: g.y,
        width: _o,
        background: o.panelBg,
        borderRadius: o.panelBorderRadius,
        padding: "0 0 12px",
        display: "flex",
        flexDirection: "column",
        zIndex: 99,
        color: o.text,
        fontSize: 11,
        maxHeight: "calc(100% - 40px)",
        boxShadow: o.panelShadow
      },
      onPointerDown: (M) => M.stopPropagation(),
      onPointerMove: C,
      onPointerUp: I,
      onPointerCancel: I,
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: x,
            style: {
              cursor: d ? "grabbing" : "grab",
              padding: "8px 16px",
              userSelect: "none",
              touchAction: "none",
              display: "flex",
              alignItems: "center",
              gap: 6,
              borderBottom: `1px solid ${o.border}`,
              color: o.textMuted,
              fontSize: 10,
              flexShrink: 0
            },
            children: /* @__PURE__ */ u("span", { style: { fontWeight: 600, letterSpacing: "0.02em" }, children: "Inspector" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              overflowY: "auto",
              overflowX: "hidden",
              padding: "8px 12px",
              display: "flex",
              flexDirection: "column",
              gap: 8,
              touchAction: "pan-y"
            },
            children: /* @__PURE__ */ u(
              ci,
              {
                engine: t,
                registry: e,
                target: r,
                commonProps: n
              }
            )
          }
        )
      ]
    }
  ) : null;
}
function ep({ engine: t, registry: e, gifApiBaseUrl: o }) {
  return /* @__PURE__ */ k(ut, { children: [
    /* @__PURE__ */ u(
      "div",
      {
        "data-sb-sidebar": !0,
        style: {
          position: "absolute",
          left: 0,
          top: 0,
          bottom: 0,
          width: ar,
          zIndex: 100
        },
        onPointerDown: (r) => r.stopPropagation(),
        children: /* @__PURE__ */ u(ku, { engine: t, gifApiBaseUrl: o })
      }
    ),
    /* @__PURE__ */ u(tp, { engine: t, registry: e })
  ] });
}
const er = [0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 3, 4];
function op(t) {
  const e = t.viewport.zoom, o = er.find((r) => r > e + 1e-3) ?? er[er.length - 1];
  t.viewport.zoom = o, t.pan(0, 0);
}
function rp(t) {
  const e = t.viewport.zoom, o = [...er].reverse().find((r) => r < e - 1e-3) ?? er[0];
  t.viewport.zoom = o, t.pan(0, 0);
}
const np = {
  display: "flex",
  alignItems: "center",
  overflow: "hidden"
}, Ie = {
  border: "none",
  background: "transparent",
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, le = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function Fe({ name: t, size: e = 16 }) {
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "minus" && /* @__PURE__ */ u("path", { d: "M5 12h14", ...le }),
    t === "plus" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M12 5v14", ...le }),
      /* @__PURE__ */ u("path", { d: "M5 12h14", ...le })
    ] }),
    t === "undo" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...le, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...le, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...le, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...le, fill: "none" })
    ] }),
    t === "fit" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...le }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...le })
    ] }),
    t === "play" && /* @__PURE__ */ u("path", { d: "M6 4l14 8-14 8z", fill: "currentColor" }),
    t === "slides" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "6", width: "20", height: "12", rx: "1", ...le }),
      /* @__PURE__ */ u("path", { d: "M6 6V18M18 6V18", ...le }),
      /* @__PURE__ */ u("path", { d: "M2 9h4M2 12h4M2 15h4M18 9h4M18 12h4M18 15h4", ...le })
    ] }),
    t === "home" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M3 12l9-8 9 8", ...le, fill: "none" }),
      /* @__PURE__ */ u("path", { d: "M5 12v7a1 1 0 001 1h12a1 1 0 001-1v-7", ...le, fill: "none" })
    ] }),
    t === "bookmark" && /* @__PURE__ */ u("path", { d: "M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z", ...le, fill: "none" }),
    t === "bookmark-fill" && /* @__PURE__ */ u("path", { d: "M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", fill: "currentColor" })
  ] });
}
function sp({ engine: t, framesPanelOpen: e, onToggleFramesPanel: o }) {
  const r = qt(), [n, s] = tt(t.viewport.zoom), [i, l] = tt(!1), [a, c] = tt(!1), [h, p] = tt(() => t.originView != null), [d, f] = tt(
    () => t.getAllNodes().filter((x) => x.type === "frame").length
  );
  mt(() => {
    const x = () => s(t.viewport.zoom), C = () => {
      l(t.canUndo()), c(t.canRedo());
    }, I = () => {
      f(t.getAllNodes().filter((M) => M.type === "frame").length), p(t.originView != null);
    };
    return t.on("viewport", x), t.on("history", C), t.on("change", I), t.on("node:create", I), t.on("node:delete", I), () => {
      t.off("viewport", x), t.off("history", C), t.off("change", I), t.off("node:create", I), t.off("node:delete", I);
    };
  }, [t]);
  const m = r.panelBg, y = `1px solid ${r.border}`, g = {
    ...np,
    borderRadius: r.panelBorderRadius
  }, b = {
    width: 1,
    height: 20,
    background: r.separator,
    flexShrink: 0
  };
  return /* @__PURE__ */ k(
    "div",
    {
      "data-sb-bottombar": !0,
      style: {
        position: "absolute",
        bottom: 12,
        left: "50%",
        transform: "translateX(-50%)",
        display: "flex",
        gap: 8,
        zIndex: 9999,
        pointerEvents: "auto"
      },
      onPointerDown: (x) => x.stopPropagation(),
      children: [
        /* @__PURE__ */ k("div", { style: { ...g, background: m, border: y, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Zoom out",
              onClick: () => rp(t),
              style: { ...Ie, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(Fe, { name: "minus" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ k(
            "button",
            {
              title: "Reset zoom to 100%",
              onClick: () => {
                t.viewport.zoom = 1, t.pan(0, 0);
              },
              style: {
                ...Ie,
                minWidth: 48,
                height: 32,
                color: r.text,
                fontSize: 11,
                fontWeight: 600,
                fontFamily: "inherit",
                padding: "0 4px"
              },
              children: [
                Math.round(n * 100),
                "%"
              ]
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Zoom in",
              onClick: () => op(t),
              style: { ...Ie, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(Fe, { name: "plus" })
            }
          )
        ] }),
        /* @__PURE__ */ k("div", { style: { ...g, background: m, border: y, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Fit to content (Ctrl+0)",
              onClick: () => t.fitToContent(),
              style: { ...Ie, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(Fe, { name: "fit" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: h ? "Clear saved view" : "Save current view as origin",
              onClick: () => {
                h ? (t.clearOriginView(), p(!1)) : (t.setOriginView(), p(!0));
              },
              style: { ...Ie, width: 32, height: 32, color: h ? r.accentColor : r.textFaint },
              children: /* @__PURE__ */ u(Fe, { name: h ? "bookmark-fill" : "bookmark" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Go to saved view",
              onClick: () => {
                h && t.goToOriginView();
              },
              disabled: !h,
              style: { ...Ie, width: 32, height: 32, color: h ? r.text : r.textFaint },
              children: /* @__PURE__ */ u(Fe, { name: "home" })
            }
          )
        ] }),
        /* @__PURE__ */ k("div", { style: { ...g, overflow: "visible", background: m, border: y, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Present (frames as slides)",
              onClick: () => t.enterPresentation(),
              style: { ...Ie, width: 32, height: 32, color: r.text },
              children: /* @__PURE__ */ u(Fe, { name: "play" })
            }
          ),
          o && /* @__PURE__ */ k(ut, { children: [
            /* @__PURE__ */ u("div", { style: b }),
            /* @__PURE__ */ k(
              "button",
              {
                title: "Toggle slides panel",
                onClick: o,
                style: {
                  ...Ie,
                  width: 32,
                  height: 32,
                  color: e ? r.text : r.textMuted,
                  position: "relative"
                },
                children: [
                  /* @__PURE__ */ u(Fe, { name: "slides" }),
                  d > 0 && /* @__PURE__ */ u(
                    "span",
                    {
                      style: {
                        position: "absolute",
                        top: -4,
                        right: -4,
                        minWidth: 14,
                        height: 14,
                        borderRadius: 7,
                        background: r.accentColor,
                        color: "#fff",
                        fontSize: 9,
                        fontWeight: 700,
                        lineHeight: "14px",
                        textAlign: "center",
                        padding: "0 3px",
                        pointerEvents: "none"
                      },
                      children: d
                    }
                  )
                ]
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ k("div", { style: { ...g, background: m, border: y, boxShadow: r.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Undo (Ctrl+Z)",
              onClick: () => t.undo(),
              disabled: !i,
              style: { ...Ie, width: 32, height: 32, color: i ? r.text : r.textFaint },
              children: /* @__PURE__ */ u(Fe, { name: "undo" })
            }
          ),
          /* @__PURE__ */ u("div", { style: b }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Redo (Ctrl+Shift+Z)",
              onClick: () => t.redo(),
              disabled: !a,
              style: { ...Ie, width: 32, height: 32, color: a ? r.text : r.textFaint },
              children: /* @__PURE__ */ u(Fe, { name: "redo" })
            }
          )
        ] })
      ]
    }
  );
}
const di = 240, hi = 6;
function yn(t) {
  const o = t.getAllNodes().filter((h) => h.type === "frame");
  if (o.length === 0) return [];
  const r = o.map((h) => ({
    id: h.id,
    x: h.x,
    y: h.y,
    slideOrder: h.data.slideOrder,
    label: h.data.label || "",
    borderColor: h.data.borderColor,
    transition: h.data.transition,
    transitionDuration: h.data.transitionDuration
  })), n = r.filter((h) => h.slideOrder != null).sort((h, p) => h.slideOrder - p.slideOrder), s = r.filter((h) => h.slideOrder == null), i = 100;
  s.sort((h, p) => h.y - p.y);
  const l = [];
  for (const h of s) {
    const p = l[l.length - 1];
    p && Math.abs(h.y - p[0].y) < i ? p.push(h) : l.push([h]);
  }
  const a = l.flatMap((h) => h.sort((p, d) => p.x - d.x));
  return [...n, ...a].map((h, p) => ({
    id: h.id,
    label: h.label || `Frame ${p + 1}`,
    order: p + 1,
    slideOrder: h.slideOrder,
    borderColor: h.borderColor,
    transition: h.transition,
    transitionDuration: h.transitionDuration
  }));
}
const ip = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function ap() {
  return /* @__PURE__ */ u("svg", { width: 14, height: 14, viewBox: "0 0 24 24", fill: "none", children: /* @__PURE__ */ u("path", { d: "M18 6L6 18M6 6l12 12", ...ip }) });
}
function lp(t, e, o) {
  const [r, n] = tt("");
  return mt(() => {
    let s = !1;
    return Fh(t, e).then((i) => {
      s || n(i);
    }), () => {
      s = !0;
    };
  }, [t, e, o]), r;
}
function cp({ engine: t, frameId: e, tick: o }) {
  const r = lp(t, e, o);
  return r ? /* @__PURE__ */ u(
    "img",
    {
      src: r,
      alt: "",
      draggable: !1,
      style: {
        width: "100%",
        display: "block",
        borderRadius: "4px 4px 0 0",
        pointerEvents: "none"
      }
    }
  ) : /* @__PURE__ */ u(
    "div",
    {
      style: {
        width: "100%",
        aspectRatio: "16/10",
        background: "rgba(128,128,128,0.06)",
        borderRadius: "4px 4px 0 0"
      }
    }
  );
}
const ui = [
  { key: "pan", label: "Pan" },
  { key: "fade", label: "Fade" },
  { key: "dissolve", label: "Dissolve" },
  { key: "zoom", label: "Zoom" },
  { key: "fold", label: "Fold" },
  { key: "cube", label: "Cube" },
  { key: "none", label: "Cut" }
];
function pi({ type: t, size: e = 12 }) {
  const o = { stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", fill: "none" };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 16 16", fill: "none", children: [
    t === "pan" && /* @__PURE__ */ u("path", { d: "M3 8h10M10 5l3 3-3 3", ...o }),
    t === "fade" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "3", width: "5", height: "10", rx: "1", ...o, opacity: 0.4 }),
      /* @__PURE__ */ u("rect", { x: "9", y: "3", width: "5", height: "10", rx: "1", ...o })
    ] }),
    t === "dissolve" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "3", width: "5", height: "10", rx: "1", ...o, strokeDasharray: "2,1" }),
      /* @__PURE__ */ u("rect", { x: "9", y: "3", width: "5", height: "10", rx: "1", ...o })
    ] }),
    t === "zoom" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("circle", { cx: "8", cy: "8", r: "3", ...o }),
      /* @__PURE__ */ u("path", { d: "M5 3L3 1M11 3l2-2M5 13l-2 2M11 13l2 2", ...o })
    ] }),
    t === "fold" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M2 3v10l6-5z", ...o }),
      /* @__PURE__ */ u("path", { d: "M14 3v10l-6-5z", ...o })
    ] }),
    t === "cube" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M3 4h7v8H3z", ...o }),
      /* @__PURE__ */ u("path", { d: "M10 4l3-2v8l-3 2", ...o }),
      /* @__PURE__ */ u("path", { d: "M3 4l3-2h7l-3 2", ...o })
    ] }),
    t === "none" && /* @__PURE__ */ u("path", { d: "M4 4l8 8M12 4l-8 8", ...o })
  ] });
}
const dp = [200, 300, 400, 500, 600, 800, 1e3, 1500, 2e3];
function hp({
  value: t,
  durationMs: e,
  onChange: o,
  onDurationChange: r,
  theme: n
}) {
  var m;
  const [s, i] = tt(!1), [l, a] = tt(!1), c = lt(null), h = lt(null), p = t !== "none", d = e ?? tr[t];
  mt(() => {
    if (!s && !l) return;
    const y = (g) => {
      s && c.current && !c.current.contains(g.target) && i(!1), l && h.current && !h.current.contains(g.target) && a(!1);
    };
    return document.addEventListener("mousedown", y), () => document.removeEventListener("mousedown", y);
  }, [s, l]);
  const f = {
    border: `1px solid ${n.border}`,
    background: n.panelBg,
    borderRadius: 10,
    height: 20,
    padding: "0 6px",
    display: "flex",
    alignItems: "center",
    gap: 3,
    cursor: "pointer",
    color: n.textMuted,
    fontSize: 9,
    position: "relative",
    zIndex: 1
  };
  return /* @__PURE__ */ k(
    "div",
    {
      style: {
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        height: 22,
        flexShrink: 0,
        position: "relative",
        gap: 4,
        zIndex: s || l ? 50 : void 0
      },
      children: [
        /* @__PURE__ */ u("div", { style: { position: "absolute", left: 0, right: 0, top: "50%", height: 1, background: n.border } }),
        /* @__PURE__ */ k("div", { ref: c, style: { position: "relative", zIndex: 1 }, children: [
          /* @__PURE__ */ k("button", { onClick: () => {
            i((y) => !y), a(!1);
          }, style: f, children: [
            /* @__PURE__ */ u(pi, { type: t }),
            /* @__PURE__ */ u("span", { children: ((m = ui.find((y) => y.key === t)) == null ? void 0 : m.label) ?? "Pan" }),
            /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: s ? "▲" : "▼" })
          ] }),
          s && /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                top: "100%",
                left: "50%",
                transform: "translateX(-50%)",
                marginTop: 2,
                background: n.panelBg,
                border: `1px solid ${n.border}`,
                borderRadius: 6,
                padding: 3,
                zIndex: 100,
                display: "flex",
                flexDirection: "column",
                gap: 1,
                minWidth: 100,
                boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
              },
              children: ui.map((y) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    o(y.key), i(!1);
                  },
                  style: {
                    border: "none",
                    background: y.key === t ? n.controlBgActive : "transparent",
                    color: n.text,
                    borderRadius: 4,
                    padding: "4px 8px",
                    fontSize: 10,
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    width: "100%"
                  },
                  children: [
                    /* @__PURE__ */ u(pi, { type: y.key }),
                    y.label
                  ]
                },
                y.key
              ))
            }
          )
        ] }),
        p && /* @__PURE__ */ k("div", { ref: h, style: { position: "relative", zIndex: 1 }, children: [
          /* @__PURE__ */ k("button", { onClick: () => {
            a((y) => !y), i(!1);
          }, style: f, children: [
            /* @__PURE__ */ k("span", { children: [
              d,
              "ms"
            ] }),
            /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: l ? "▲" : "▼" })
          ] }),
          l && /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                top: "100%",
                left: "50%",
                transform: "translateX(-50%)",
                marginTop: 2,
                background: n.panelBg,
                border: `1px solid ${n.border}`,
                borderRadius: 6,
                padding: 3,
                zIndex: 100,
                display: "flex",
                flexDirection: "column",
                gap: 1,
                minWidth: 64,
                boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
              },
              children: dp.map((y) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    r(y === tr[t] ? void 0 : y), a(!1);
                  },
                  style: {
                    border: "none",
                    background: y === d ? n.controlBgActive : "transparent",
                    color: n.text,
                    borderRadius: 4,
                    padding: "4px 8px",
                    fontSize: 10,
                    cursor: "pointer",
                    textAlign: "center",
                    width: "100%"
                  },
                  children: [
                    y,
                    "ms",
                    y === tr[t] ? " •" : ""
                  ]
                },
                y
              ))
            }
          )
        ] })
      ]
    }
  );
}
function up({ engine: t, open: e, onClose: o }) {
  const r = qt(), [n, s] = tt(() => yn(t)), [i, l] = tt(() => new Set(t.selection)), [a, c] = tt(0), h = lt(null), p = lt(null), d = lt(0), f = lt(!1), m = lt(n);
  m.current = n;
  const y = lt(!1), g = lt(!1), [b, x] = tt(null), [C, I] = tt(null), [M, D] = tt(0), T = lt([]), P = lt(null), G = it(() => {
    if (y.current) return;
    const Q = yn(t);
    s(Q);
  }, [t]), K = it(() => {
    l(new Set(t.selection));
  }, [t]), at = lt(null), B = it(() => {
    at.current && clearTimeout(at.current), at.current = setTimeout(() => c((Q) => Q + 1), 500);
  }, []);
  mt(() => {
    G(), K();
    const Q = setTimeout(() => c((X) => X + 1), 200), L = () => {
      G(), B();
    };
    return t.on("change", L), t.on("node:create", L), t.on("node:delete", L), t.on("node:data", L), t.on("selection", K), t.on("history", L), () => {
      clearTimeout(Q), t.off("change", L), t.off("node:create", L), t.off("node:delete", L), t.off("node:data", L), t.off("selection", K), t.off("history", L), at.current && clearTimeout(at.current);
    };
  }, [t, G, K, B]), mt(() => {
    if (!P.current) return;
    const Q = P.current.querySelectorAll("[data-frame-card]");
    T.current = Array.from(Q).map((L) => L.offsetHeight + hi);
  }, [n]);
  const et = it(
    (Q) => {
      t.select(Q), t.zoomToNode(Q, 0.8);
    },
    [t]
  ), Z = it(
    (Q, L) => {
      Q.preventDefault(), Q.stopPropagation(), d.current = Q.clientY, h.current = L, p.current = L, f.current = !1;
    },
    []
  );
  return mt(() => {
    const Q = (X) => {
      if (h.current === null) return;
      const _ = X.clientY - d.current;
      if (!f.current) {
        if (Math.abs(_) < 4) return;
        f.current = !0, x(h.current), I(h.current);
      }
      D(_);
      const j = T.current, q = h.current;
      let Y = q;
      if (_ > 0) {
        let $ = 0;
        for (let nt = q + 1; nt < m.current.length && ($ += j[nt] || 0, _ > $ - (j[nt] || 0) / 2); nt++)
          Y = nt;
      } else if (_ < 0) {
        let $ = 0;
        for (let nt = q - 1; nt >= 0 && ($ -= j[nt] || 0, _ < $ + (j[nt] || 0) / 2); nt--)
          Y = nt;
      }
      p.current = Y, I(Y);
    }, L = () => {
      const X = h.current, _ = p.current;
      if (X !== null && _ !== null && X !== _) {
        y.current = !0;
        const j = [...m.current], [q] = j.splice(X, 1);
        j.splice(_, 0, q);
        let Y = !0;
        for (let $ = 0; $ < j.length; $++) {
          const nt = j[$], ot = t.getNode(nt.id);
          ot && (Y ? (t.updateNodeWithHistory(nt.id, {
            data: { ...ot.data, slideOrder: $ + 1 }
          }), Y = !1) : t.updateNode(nt.id, {
            data: { ...ot.data, slideOrder: $ + 1 }
          }));
        }
        y.current = !1, g.current = !0, s(yn(t)), c(($) => $ + 1);
      }
      h.current = null, p.current = null, f.current = !1, x(null), I(null), D(0), g.current && requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          g.current = !1;
        });
      });
    };
    return document.addEventListener("pointermove", Q), document.addEventListener("pointerup", L), document.addEventListener("pointercancel", L), () => {
      document.removeEventListener("pointermove", Q), document.removeEventListener("pointerup", L), document.removeEventListener("pointercancel", L);
    };
  }, [t]), /* @__PURE__ */ k(
    "div",
    {
      "data-sb-frames-panel": !0,
      style: {
        position: "absolute",
        top: 0,
        right: 0,
        bottom: 0,
        width: di,
        background: r.panelBg,
        borderLeft: `1px solid ${r.border}`,
        zIndex: 98,
        display: "flex",
        flexDirection: "column",
        transform: e ? "translateX(0)" : `translateX(${di}px)`,
        transition: "transform 0.2s ease-in-out",
        pointerEvents: e ? "auto" : "none"
      },
      onPointerDown: (Q) => Q.stopPropagation(),
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              padding: "10px 12px",
              borderBottom: `1px solid ${r.border}`,
              flexShrink: 0
            },
            children: [
              /* @__PURE__ */ k("span", { style: { fontSize: 12, fontWeight: 600, color: r.text, letterSpacing: "0.02em" }, children: [
                "Slides (",
                n.length,
                ")"
              ] }),
              /* @__PURE__ */ u(
                "button",
                {
                  title: "Close slides panel",
                  onClick: o,
                  style: {
                    border: "none",
                    background: "transparent",
                    cursor: "pointer",
                    color: r.textMuted,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    width: 24,
                    height: 24,
                    borderRadius: 4,
                    padding: 0
                  },
                  children: /* @__PURE__ */ u(ap, {})
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ k(
          "div",
          {
            ref: P,
            style: {
              flex: 1,
              overflowY: "auto",
              overflowX: "hidden",
              padding: "8px 12px",
              display: "flex",
              flexDirection: "column",
              gap: hi
            },
            children: [
              n.length === 0 && /* @__PURE__ */ u("div", { style: { padding: "20px 8px", textAlign: "center", color: r.textMuted, fontSize: 11 }, children: "No frames yet. Use the Frame tool (F) to create slides." }),
              n.map((Q, L) => {
                const X = i.has(Q.id), _ = b === L;
                let j = 0;
                if (_)
                  j = M;
                else if (b !== null && C !== null) {
                  const $ = T.current;
                  b < C ? L > b && L <= C && (j = -($[b] || 0)) : b > C && L >= C && L < b && (j = $[b] || 0);
                }
                const q = ($) => {
                  t.updateNodeWithHistory(Q.id, {
                    data: { transition: $ === "pan" ? void 0 : $, transitionDuration: void 0 }
                  });
                }, Y = ($) => {
                  t.updateNodeWithHistory(Q.id, {
                    data: { transitionDuration: $ }
                  });
                };
                return /* @__PURE__ */ k(Za.Fragment, { children: [
                  b === null && /* @__PURE__ */ u(
                    hp,
                    {
                      value: Q.transition ?? "pan",
                      durationMs: Q.transitionDuration,
                      onChange: q,
                      onDurationChange: Y,
                      theme: r
                    }
                  ),
                  /* @__PURE__ */ u(
                    "div",
                    {
                      "data-frame-card": !0,
                      onPointerDown: ($) => Z($, L),
                      onDoubleClick: () => et(Q.id),
                      style: {
                        borderRadius: 6,
                        border: X ? `2px solid ${Q.borderColor || r.text}` : `1px solid ${r.border}`,
                        background: X ? r.controlBgActive : "transparent",
                        cursor: _ ? "grabbing" : "grab",
                        userSelect: "none",
                        touchAction: "none",
                        transition: _ || g.current ? "none" : "transform 0.15s ease, border-color 0.1s ease",
                        transform: `translateY(${j}px)`,
                        zIndex: _ ? 10 : 1,
                        opacity: _ ? 0.92 : 1,
                        boxShadow: _ ? "0 4px 12px rgba(0,0,0,0.18)" : "none",
                        overflow: "hidden",
                        position: "relative",
                        flexShrink: 0
                      },
                      children: /* @__PURE__ */ u(cp, { engine: t, frameId: Q.id, tick: a })
                    }
                  )
                ] }, Q.id);
              })
            ]
          }
        )
      ]
    }
  );
}
const co = 50, mn = 30, pp = `
attribute vec2 aUV;
uniform float uLayPos;
uniform float uRadius;
uniform float uSide;

varying float vLight;
varying float vSpec;
varying float vShadow;
varying float vY;

void main() {
  float PI = 3.14159265;
  float d = aUV.x; // 0 = fold edge (center), 1 = far edge
  float y = aUV.y; // -1 to 1

  float x, z, theta;

  // Slight height-based radius modulation for organic curl
  float R = uRadius * (1.0 + 0.04 * sin(y * PI));

  if (d >= uLayPos) {
    // Flat — already laid on screen
    x = d;
    z = 0.0;
    theta = 0.0;
  } else {
    float arc = uLayPos - d;
    theta = arc / R;
    if (theta < PI) {
      // On the curl cylinder
      x = uLayPos - R * sin(theta);
      z = R * (1.0 - cos(theta));
    } else {
      // Back side — past the full curl, extending away
      float back = (theta - PI) * R;
      x = uLayPos + back;
      z = 2.0 * R;
      theta = PI;
    }
  }

  // Mirror for left/right half
  x *= uSide;

  // Perspective
  float eye = 2.5;
  float s = eye / (eye + z);

  // Diffuse lighting (front-facing light)
  float diffuse = 0.2 + 0.8 * max(0.0, cos(theta));
  diffuse *= (1.0 - z * 0.25); // depth darkening

  // Specular highlight on curl crest (~40 degrees)
  float specTarget = 0.7;
  vSpec = pow(max(0.0, 1.0 - abs(theta - specTarget) * 2.5), 6.0) * 0.5;

  // Shadow on flat surface near curl
  if (d >= uLayPos) {
    float distToCurl = d - uLayPos;
    vShadow = 1.0 - 0.4 * exp(-distToCurl * 25.0);
  } else {
    vShadow = 1.0;
  }

  vLight = diffuse;
  vY = y * 0.5 + 0.5; // normalized 0..1

  gl_Position = vec4(x * s * 2.0, y * s, -z * 0.01, 1.0);
}
`, fp = `
precision mediump float;
varying float vLight;
varying float vSpec;
varying float vShadow;
varying float vY;
uniform vec3 uColor;

void main() {
  // Height gradient for subtle sheen (Video Toaster aesthetic)
  float gradient = 0.88 + 0.12 * vY;
  vec3 c = uColor * vLight * gradient * vShadow + vec3(vSpec);
  gl_FragColor = vec4(c, 1.0);
}
`;
function fi(t, e, o) {
  const r = t.createShader(e);
  return r ? (t.shaderSource(r, o), t.compileShader(r), t.getShaderParameter(r, t.COMPILE_STATUS) ? r : (t.deleteShader(r), null)) : null;
}
function yp(t, e, o) {
  const r = fi(t, t.VERTEX_SHADER, e), n = fi(t, t.FRAGMENT_SHADER, o);
  if (!r || !n) return null;
  const s = t.createProgram();
  return t.attachShader(s, r), t.attachShader(s, n), t.linkProgram(s), t.getProgramParameter(s, t.LINK_STATUS) ? s : null;
}
function mp() {
  const t = [], e = [];
  for (let o = 0; o <= mn; o++)
    for (let r = 0; r <= co; r++)
      t.push(r / co, o / mn * 2 - 1);
  for (let o = 0; o < mn; o++)
    for (let r = 0; r < co; r++) {
      const n = o * (co + 1) + r;
      e.push(n, n + co + 1, n + 1, n + 1, n + co + 1, n + co + 2);
    }
  return { vertices: new Float32Array(t), indices: new Uint16Array(e) };
}
function gp({ phase: t, progress: e }) {
  const o = lt(null), r = lt(null);
  return mt(() => {
    const n = o.current;
    if (!n) return;
    const s = window.devicePixelRatio || 1;
    n.width = n.clientWidth * s, n.height = n.clientHeight * s;
    const i = n.getContext("webgl", { alpha: !0, premultipliedAlpha: !1, antialias: !0 });
    if (!i) return;
    const l = yp(i, pp, fp);
    if (!l) return;
    i.useProgram(l);
    const { vertices: a, indices: c } = mp(), h = i.createBuffer();
    i.bindBuffer(i.ARRAY_BUFFER, h), i.bufferData(i.ARRAY_BUFFER, a, i.STATIC_DRAW);
    const p = i.createBuffer();
    i.bindBuffer(i.ELEMENT_ARRAY_BUFFER, p), i.bufferData(i.ELEMENT_ARRAY_BUFFER, c, i.STATIC_DRAW);
    const d = i.getAttribLocation(l, "aUV");
    i.enableVertexAttribArray(d), i.vertexAttribPointer(d, 2, i.FLOAT, !1, 0, 0), i.enable(i.DEPTH_TEST), i.clearColor(0, 0, 0, 0);
    const f = (m) => i.getUniformLocation(l, m);
    return r.current = {
      gl: i,
      locs: { uLayPos: f("uLayPos"), uRadius: f("uRadius"), uSide: f("uSide"), uColor: f("uColor") },
      count: c.length
    }, () => {
      i.deleteProgram(l), i.deleteBuffer(h), i.deleteBuffer(p), r.current = null;
    };
  }, []), mt(() => {
    const n = r.current;
    if (!n) return;
    const { gl: s, locs: i, count: l } = n, a = t === "out" ? 1 - Math.pow(1 - e, 3) : Math.pow(e, 3), c = t === "out" ? 1 - a : a, h = 0.07 + 0.16 * c;
    s.viewport(0, 0, s.canvas.width, s.canvas.height), s.clear(s.COLOR_BUFFER_BIT | s.DEPTH_BUFFER_BIT), s.uniform1f(i.uLayPos, c), s.uniform1f(i.uRadius, h), s.uniform1f(i.uSide, 1), s.uniform3f(i.uColor, 0.09, 0.09, 0.17), s.drawElements(s.TRIANGLES, l, s.UNSIGNED_SHORT, 0), s.uniform1f(i.uSide, -1), s.uniform3f(i.uColor, 0.09, 0.09, 0.17), s.drawElements(s.TRIANGLES, l, s.UNSIGNED_SHORT, 0);
  }, [t, e]), /* @__PURE__ */ u(
    "canvas",
    {
      ref: o,
      style: {
        position: "absolute",
        inset: 0,
        width: "100%",
        height: "100%",
        pointerEvents: "none",
        zIndex: 9999
      }
    }
  );
}
const bp = {
  position: "absolute",
  bottom: 0,
  left: 0,
  right: 0,
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  gap: 12,
  padding: "10px 20px",
  background: "rgba(0,0,0,0.7)",
  backdropFilter: "blur(8px)",
  pointerEvents: "auto"
}, gn = {
  border: "none",
  background: "rgba(255,255,255,0.1)",
  color: "white",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  width: 32,
  height: 32,
  padding: 0
}, Fn = {
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function yi({ dir: t }) {
  return /* @__PURE__ */ k("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", children: [
    t === "left" && /* @__PURE__ */ u("polyline", { points: "15,18 9,12 15,6", ...Fn }),
    t === "right" && /* @__PURE__ */ u("polyline", { points: "9,6 15,12 9,18", ...Fn })
  ] });
}
function xp() {
  return /* @__PURE__ */ u("svg", { width: 14, height: 14, viewBox: "0 0 24 24", fill: "none", children: /* @__PURE__ */ u("path", { d: "M18 6L6 18M6 6l12 12", ...Fn }) });
}
function mi(t) {
  return 1 - Math.pow(1 - t, 3);
}
function gi(t) {
  return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
}
function bi(t, e) {
  let r;
  t <= 0.2 ? r = 1 + (0.55 - 1) * mi(t / 0.2) : t >= 0.8 ? r = 0.55 + (1 - 0.55) * mi((t - 0.8) / 0.2) : r = 0.55;
  let n;
  return t <= 0.1 ? n = 0 : t <= 0.5 ? n = -e * 90 * gi((t - 0.1) / 0.4) : t <= 0.9 ? n = e * 90 * (1 - gi((t - 0.5) / 0.4)) : n = 0, { zoom: r, angle: n };
}
function wp(t, e, o, r) {
  t.style.transform = `perspective(1200px) scale(${o}) rotateY(${r}deg)`, t.style.transformOrigin = "50% 50%", t.style.backfaceVisibility = "hidden", t.style.overflow = "visible", e.style.background = "#0a0a15";
}
function xi(t, e) {
  t.style.transform = "", t.style.transformOrigin = "", t.style.backfaceVisibility = "", t.style.overflow = "", e.style.background = "";
}
function kp({ engine: t }) {
  const [e, o] = tt(t.presentationMode), [r, n] = tt(t.presentationIndex), [s, i] = tt(t.presentationSlides.length), [l, a] = tt(""), [c, h] = tt(t.transitionOverlay), p = lt(null), d = lt(null);
  if (mt(() => {
    const m = document.querySelector("[data-sb-canvas]");
    p.current = m, d.current = (m == null ? void 0 : m.parentElement) ?? null;
    const y = () => {
      var C;
      if (o(t.presentationMode), n(t.presentationIndex), i(t.presentationSlides.length), h(t.transitionOverlay), t.presentationMode && t.presentationSlides.length > 0) {
        const I = t.presentationSlides[t.presentationIndex], M = t.getNode(I);
        a(((C = M == null ? void 0 : M.data) == null ? void 0 : C.label) || "");
      } else
        a("");
      const g = t.transitionOverlay, b = p.current, x = d.current;
      if (b && x && g && g.type === "cube" && g.t != null) {
        const I = g.direction ?? 1, { zoom: M, angle: D } = bi(g.t, I);
        wp(b, x, M, D);
      } else b && x && xi(b, x);
    };
    return t.on("presentation", y), () => {
      t.off("presentation", y);
      const g = p.current, b = d.current;
      g && b && xi(g, b);
    };
  }, [t]), !e || s === 0) return null;
  const f = c && c.type === "cube" && c.t != null ? (() => {
    const m = c.direction ?? 1, { angle: y } = bi(c.t, m);
    return Math.abs(y) / 90 * 0.4;
  })() : 0;
  return /* @__PURE__ */ k(
    "div",
    {
      "data-sb-presentation": !0,
      style: {
        position: "absolute",
        inset: 0,
        zIndex: 1e4,
        pointerEvents: "none"
      },
      children: [
        c && c.type !== "fold" && c.type !== "cube" && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              backgroundColor: "black",
              opacity: c.phase === "out" ? c.progress : 1 - c.progress,
              pointerEvents: "none",
              zIndex: 9999
            }
          }
        ),
        c && c.type === "fold" && /* @__PURE__ */ u(gp, { phase: c.phase, progress: c.progress }),
        f > 0.01 && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              backgroundColor: "black",
              opacity: f,
              pointerEvents: "none",
              zIndex: 9999
            }
          }
        ),
        /* @__PURE__ */ k("div", { style: bp, onPointerDown: (m) => m.stopPropagation(), children: [
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...gn, position: "absolute", right: 16 },
              title: "Exit presentation (Esc)",
              onClick: () => t.exitPresentation(),
              children: /* @__PURE__ */ u(xp, {})
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...gn, opacity: r <= 0 ? 0.3 : 1 },
              title: "Previous slide (←)",
              onClick: () => t.presentationPrev(),
              disabled: r <= 0,
              children: /* @__PURE__ */ u(yi, { dir: "left" })
            }
          ),
          /* @__PURE__ */ k(
            "span",
            {
              style: {
                color: "white",
                fontSize: 13,
                fontWeight: 500,
                fontFamily: "inherit",
                minWidth: 80,
                textAlign: "center",
                userSelect: "none"
              },
              children: [
                r + 1,
                " / ",
                s,
                l && /* @__PURE__ */ k("span", { style: { opacity: 0.6, marginLeft: 8 }, children: [
                  "— ",
                  l
                ] })
              ]
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...gn, opacity: r >= s - 1 ? 0.3 : 1 },
              title: "Next slide (→)",
              onClick: () => t.presentationNext(),
              disabled: r >= s - 1,
              children: /* @__PURE__ */ u(yi, { dir: "right" })
            }
          )
        ] })
      ]
    }
  );
}
const vp = Ja(() => import("./DebugPanel-bEV0ypIy.js"));
function Xp({
  nodeTypes: t = Td,
  engine: e,
  keyboardShortcuts: o = !0,
  style: r,
  initialData: n,
  toolbar: s = !0,
  debugPanel: i = !1,
  debugBoards: l,
  theme: a,
  onPresentationChange: c,
  gifApiBaseUrl: h
}) {
  const p = Kt(
    () => e ?? new Ul(),
    [e]
  ), d = Kt(() => new Ql(t), [t]);
  mt(() => Pl(), []), mt(() => {
    p.setRegistry(d);
  }, [p, d]), mt(() => {
    for (const M of t)
      M.isContainer && p.registerContainerType(M.type);
  }, [p, t]);
  const f = lt(!1);
  mt(() => {
    n && !f.current && (f.current = !0, p.fromSBD(n));
  }, [p, n]);
  const m = lt(null);
  mt(() => {
    if (o)
      return gh(p, m.current);
  }, [p, o]);
  const y = Kt(() => t.some((D) => {
    var T;
    return (T = D.ports) == null ? void 0 : T.length;
  }) ? new Pd(p, d) : null, [p, d, t]);
  mt(() => {
    if (y)
      return y.connect();
  }, [y]);
  const g = Kt(
    () => a ? { ...Tn, ...a } : Tn,
    [a]
  ), [b, x] = tt(!1), [C, I] = tt(!1);
  return mt(() => {
    const M = () => {
      const D = p.presentationMode;
      x(D), c == null || c(D);
    };
    return p.on("presentation", M), () => p.off("presentation", M);
  }, [p, c]), /* @__PURE__ */ u($i.Provider, { value: g, children: /* @__PURE__ */ k(
    "div",
    {
      ref: m,
      style: {
        width: "100%",
        height: "100%",
        position: "relative",
        ...r
      },
      children: [
        s && !b && /* @__PURE__ */ u(ep, { engine: p, registry: d, gifApiBaseUrl: h }),
        i && /* @__PURE__ */ u(Qa, { fallback: null, children: /* @__PURE__ */ u(vp, { engine: p, extraBoards: l }) }),
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              left: s && !b ? ar : 0,
              top: 0,
              right: 0,
              bottom: 0,
              overflow: "hidden"
            },
            children: [
              /* @__PURE__ */ u(jh, { engine: p, schema: Nn, registry: d, dataFlow: y }),
              !b && /* @__PURE__ */ u(
                sp,
                {
                  engine: p,
                  framesPanelOpen: C,
                  onToggleFramesPanel: () => I((M) => !M)
                }
              ),
              !b && /* @__PURE__ */ u(
                up,
                {
                  engine: p,
                  open: C,
                  onClose: () => I(!1)
                }
              ),
              /* @__PURE__ */ u(kp, { engine: p })
            ]
          }
        )
      ]
    }
  ) });
}
const Sp = [
  { key: "select", label: "Select", shortcut: "V" },
  { key: "draw", label: "Draw", shortcut: "D" },
  { key: "shape", label: "Shape", shortcut: "S" },
  { key: "text", label: "Text", shortcut: "T" },
  { key: "note", label: "Note", shortcut: "N" },
  { key: "sticky", label: "Sticky", shortcut: "P" },
  { key: "frame", label: "Frame", shortcut: "F" },
  { key: "erase", label: "Eraser", shortcut: "X" }
], Ro = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, te = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function $o({ name: t, size: e = 18 }) {
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "select" && /* @__PURE__ */ u("path", { d: "M6 2v17l4-4.5L13.5 21l2.5-1.5L12.5 13H18z", fill: "currentColor" }),
    t === "draw" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M17 3l4 4L7.5 20.5 2 22l1.5-5.5z", ...te }),
      /* @__PURE__ */ u("path", { d: "M15 5l4 4", ...te })
    ] }),
    t === "shape" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...te }),
    t === "text" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M7 4h10", ...te }),
      /* @__PURE__ */ u("path", { d: "M12 4v16", ...te })
    ] }),
    t === "note" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 3h16v14l-4 4H4z", ...te }),
      /* @__PURE__ */ u("path", { d: "M16 17v4l4-4z", fill: "currentColor", opacity: 0.4 })
    ] }),
    t === "sticky" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "1", fill: "currentColor", opacity: 0.15, ...te }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "9", x2: "17", y2: "9", ...te, opacity: 0.5 }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "13", x2: "14", y2: "13", ...te, opacity: 0.5 })
    ] }),
    t === "frame" && /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2", ...te, strokeDasharray: "4,2" }),
    t === "erase" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 20H9L3 14l9.5-9.5 8 8L16 17", ...te }),
      /* @__PURE__ */ u("path", { d: "M12.5 4.5l8 8", ...te })
    ] }),
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...te }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...te }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...te }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...te }),
    t === "arrow" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...te }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...te, fill: "none" })
    ] }),
    t === "undo" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...te, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...te, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...te, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...te, fill: "none" })
    ] }),
    t === "print" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M6 9V3h12v6", ...te }),
      /* @__PURE__ */ u("path", { d: "M6 18H4a1 1 0 0 1-1-1v-5a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-2", ...te }),
      /* @__PURE__ */ u("rect", { x: "6", y: "14", width: "12", height: "7", rx: "1", ...te })
    ] }),
    t === "fit" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...te }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...te })
    ] })
  ] });
}
function Yp({ engine: t }) {
  const [e, o] = tt(t.mode), [r, n] = tt(!1), [s, i] = tt(!1), [l, a] = tt(t.boardBackground);
  return mt(() => {
    const c = () => o(t.mode), h = () => {
      n(t.canUndo()), i(t.canRedo());
    }, p = () => a(t.boardBackground);
    return t.on("mode", c), t.on("history", h), t.on("background", p), () => {
      t.off("mode", c), t.off("history", h), t.off("background", p);
    };
  }, [t]), /* @__PURE__ */ k(
    "div",
    {
      "data-sb-toolbar": !0,
      style: {
        position: "absolute",
        left: 0,
        top: 0,
        bottom: 0,
        width: 48,
        background: "#1e1e2e",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        padding: "8px 0",
        gap: 4,
        zIndex: 100
      },
      children: [
        Sp.map((c) => /* @__PURE__ */ u(
          "button",
          {
            title: `${c.label} (${c.shortcut})`,
            onClick: () => t.setMode(c.key),
            style: {
              ...Ro,
              width: 36,
              height: 36,
              background: e === c.key ? "#3b82f6" : "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u($o, { name: c.key })
          },
          c.key
        )),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "8px 0"
            }
          }
        ),
        [
          { key: "dot-grid", color: "#f8f7f5", label: "Dot Grid" },
          { key: "blueprint", color: "#1e3a5f", label: "Blueprint" },
          { key: "japanese-stationery", color: "#f5f0e8", label: "Japanese Stationery" }
        ].map((c) => /* @__PURE__ */ u(
          "button",
          {
            title: c.label,
            onClick: () => t.setBoardBackground(c.key),
            style: {
              ...Ro,
              width: 20,
              height: 20,
              background: c.color,
              border: l === c.key ? "2px solid white" : "2px solid transparent",
              borderRadius: 4,
              boxShadow: c.key === "japanese-stationery" ? "inset 0 1px 0 rgba(255,255,255,0.5), inset 0 -1px 0 rgba(160,140,110,0.15)" : void 0
            }
          },
          c.key
        )),
        /* @__PURE__ */ u("div", { style: { flex: 1 } }),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Print (landscape)",
            onClick: () => {
              const c = { ...t.viewport };
              t.fitToContent(), requestAnimationFrame(() => {
                window.print(), t.viewport.x = c.x, t.viewport.y = c.y, t.viewport.zoom = c.zoom, t.pan(0, 0);
              });
            },
            style: {
              ...Ro,
              width: 36,
              height: 36,
              background: "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u($o, { name: "print" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "4px 0"
            }
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Undo (Ctrl+Z)",
            onClick: () => t.undo(),
            disabled: !r,
            style: {
              ...Ro,
              width: 36,
              height: 36,
              background: "transparent",
              color: r ? "white" : "#666"
            },
            children: /* @__PURE__ */ u($o, { name: "undo" })
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Redo (Ctrl+Shift+Z)",
            onClick: () => t.redo(),
            disabled: !s,
            style: {
              ...Ro,
              width: 36,
              height: 36,
              background: "transparent",
              color: s ? "white" : "#666"
            },
            children: /* @__PURE__ */ u($o, { name: "redo" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "4px 0"
            }
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Fit to content (Ctrl+0)",
            onClick: () => t.fitToContent(),
            style: {
              ...Ro,
              width: 36,
              height: 36,
              background: "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u($o, { name: "fit" })
          }
        )
      ]
    }
  );
}
const je = [
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], Mp = [
  null,
  // no fill
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], Cp = [
  { key: "hachure", label: "Hachure" },
  { key: "cross-hatch", label: "Cross-hatch" },
  { key: "solid", label: "Solid" }
], Ao = [
  { key: "solid", label: "Solid", dash: "" },
  { key: "dashed", label: "Dashed", dash: "6,3" },
  { key: "dotted", label: "Dotted", dash: "2,2" }
], zp = [
  { value: 0, label: "Architect" },
  { value: 1, label: "Artist" },
  { value: 2, label: "Cartoonist" }
], Lo = [1, 2.5, 5, 10, 20], Ip = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
], Tp = [14, 20, 28, 36], Pp = [
  { key: "left", label: "←" },
  { key: "center", label: "↔" },
  { key: "right", label: "→" }
], bn = 300, Bt = {
  display: "flex",
  alignItems: "center",
  gap: 4
}, Nt = {
  width: 64,
  fontSize: 10,
  color: "#999",
  flexShrink: 0
}, Vt = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0,
  flexShrink: 0
};
function Gp({
  engine: t,
  registry: e
}) {
  const [o, r] = tt(t.mode), [n, s] = tt(t.selection), [, i] = tt(0), [l, a] = tt(null), c = lt(null), h = lt(null), [p, d] = tt(!1), f = it(() => {
    var st;
    return { x: (((st = c.current) == null ? void 0 : st.ownerDocument.defaultView) ?? window).innerWidth - bn - 12, y: 12 };
  }, []), m = l ?? f();
  mt(() => {
    const v = () => r(t.mode), st = () => {
      s(new Set(t.selection)), i((ne) => ne + 1);
    }, Yt = () => i((ne) => ne + 1);
    return t.on("mode", v), t.on("selection", st), t.on("change", Yt), () => {
      t.off("mode", v), t.off("selection", st), t.off("change", Yt);
    };
  }, [t]);
  const y = it((v) => {
    v.stopPropagation(), d(!0);
    const st = l ? l.x : f().x, Yt = l ? l.y : f().y;
    h.current = { startX: v.clientX, startY: v.clientY, startLeft: st, startTop: Yt }, v.currentTarget.setPointerCapture(v.pointerId);
  }, [l, f]);
  mt(() => {
    var ne;
    const v = (xe) => {
      var Ee;
      if (!h.current) return;
      const De = xe.clientX - h.current.startX, no = xe.clientY - h.current.startY, ge = ((Ee = c.current) == null ? void 0 : Ee.ownerDocument.defaultView) ?? window, Vo = Math.max(48, Math.min(ge.innerWidth - bn - 8, h.current.startLeft + De)), cr = Math.max(8, Math.min(ge.innerHeight - 100, h.current.startTop + no));
      a({ x: Vo, y: cr });
    }, st = () => {
      h.current = null, d(!1);
    }, Yt = ((ne = c.current) == null ? void 0 : ne.ownerDocument) ?? document;
    return Yt.addEventListener("pointermove", v), Yt.addEventListener("pointerup", st), Yt.addEventListener("pointercancel", st), () => {
      Yt.removeEventListener("pointermove", v), Yt.removeEventListener("pointerup", st), Yt.removeEventListener("pointercancel", st);
    };
  }, []);
  const g = (() => {
    if (n.size === 1) {
      const v = Array.from(n)[0], st = t.getNode(v);
      if ((st == null ? void 0 : st.type) === "shape") return { kind: "shape", node: st };
      if ((st == null ? void 0 : st.type) === "draw") return { kind: "draw", node: st };
      if ((st == null ? void 0 : st.type) === "text") return { kind: "text", node: st };
      if ((st == null ? void 0 : st.type) === "edge") return { kind: "edge", node: st };
      if ((st == null ? void 0 : st.type) === "image") return { kind: "image", node: st };
      if ((st == null ? void 0 : st.type) === "content") return { kind: "content", node: st };
      if ((st == null ? void 0 : st.type) === "frame") return { kind: "frame", node: st };
      if ((st == null ? void 0 : st.type) === "sticky") return { kind: "sticky", node: st };
      if (st && e) {
        const Yt = e.get(st.type);
        if (Yt != null && Yt.propertiesPanel)
          return { kind: "custom", node: st, PanelComponent: Yt.propertiesPanel };
      }
    }
    return o === "draw" || o === "shape" || o === "text" ? { kind: "tool" } : null;
  })(), b = it(
    (v) => {
      !g || g.kind !== "shape" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), x = it(
    (v) => {
      !g || g.kind !== "draw" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), C = it(
    (v) => {
      !g || g.kind !== "text" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), I = it(
    (v) => {
      !g || g.kind !== "edge" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), M = it(
    (v) => {
      !g || g.kind !== "image" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), D = it(
    (v) => {
      !g || g.kind !== "content" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), T = it(
    (v) => {
      !g || g.kind !== "frame" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), P = it(
    (v) => {
      !g || g.kind !== "sticky" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), G = it(
    (v) => {
      !g || g.kind !== "custom" || t.updateNodeWithHistory(g.node.id, {
        data: { ...g.node.data, ...v }
      });
    },
    [t, g]
  ), [K, at] = tt("idle");
  if (!g) return null;
  const B = g.kind === "custom", et = g.kind === "shape", Z = g.kind === "draw", Q = g.kind === "text", L = g.kind === "edge", X = g.kind === "image", _ = g.kind === "content", j = g.kind === "frame", q = g.kind === "sticky", Y = g.kind === "tool", $ = Y && o === "shape", nt = Y && o === "text", ot = Q ? g.node.data.fontFamily : t.activeTool.fontFamily ?? Ze, ft = Q ? g.node.data.fontSize : t.activeTool.fontSize ?? 20, pt = Q ? g.node.data.align : t.activeTool.textAlign ?? "left", rt = Q ? g.node.data.color : t.activeTool.color, It = et ? g.node.data.stroke : Z ? g.node.data.color : t.activeTool.color, zt = et || Z ? g.node.data.fill ?? null : t.activeTool.fillColor ?? null, bt = et || Z ? g.node.data.fillStyle ?? "hachure" : t.activeTool.fillStyle ?? "hachure", ht = et || Z ? g.node.data.strokeStyle ?? "solid" : t.activeTool.strokeStyle ?? "solid", Wt = et || Z ? g.node.data.strokeWidth : t.activeTool.width, Tt = et ? g.node.data.roughness : t.activeTool.roughness ?? 1, Rt = et || Z || Q || X || _ || j || q ? g.node.data.opacity ?? 1 : t.activeTool.opacity ?? 1, Qt = (() => {
    const v = /* @__PURE__ */ new Set(), st = [];
    for (const Yt of t.getAllNodes())
      if (Yt.type === "text") {
        const ne = Yt.data.fontFamily;
        ne && !v.has(ne) && (v.add(ne), st.push(ne));
      }
    return st;
  })(), Jt = !Q && !nt && !L && !X && !_ && !j && !q && !B, Zt = Jt, $t = Jt, he = et || $, Ae = Q || nt, _t = (v) => {
    et ? b({ stroke: v }) : Z ? x({ color: v }) : (t.activeTool.color = v, i((st) => st + 1));
  }, Le = (v) => {
    et ? b({ fill: v ?? void 0 }) : Z ? x({ fill: v ?? void 0 }) : (t.activeTool.fillColor = v ?? void 0, i((st) => st + 1));
  }, eo = (v) => {
    et ? b({ fillStyle: v }) : Z ? x({ fillStyle: v }) : (t.activeTool.fillStyle = v, i((st) => st + 1));
  }, xo = (v) => {
    et ? b({ strokeStyle: v }) : Z ? x({ strokeStyle: v }) : (t.activeTool.strokeStyle = v, i((st) => st + 1));
  }, ue = (v) => {
    et ? b({ strokeWidth: v }) : Z ? x({ strokeWidth: v }) : (t.activeTool.width = v, i((st) => st + 1));
  }, Me = (v) => {
    et ? b({ roughness: v }) : (t.activeTool.roughness = v, i((st) => st + 1));
  }, oo = (v) => {
    et ? b({ opacity: v }) : Z ? x({ opacity: v }) : Q ? C({ opacity: v }) : X ? M({ opacity: v }) : _ ? D({ opacity: v }) : j ? T({ opacity: v }) : q ? P({ opacity: v }) : (t.activeTool.opacity = v, i((st) => st + 1));
  }, ro = (v) => {
    Q ? C({ fontFamily: v }) : (t.activeTool.fontFamily = v, i((st) => st + 1));
  }, pe = (v) => {
    Q ? C({ fontSize: v }) : (t.activeTool.fontSize = v, i((st) => st + 1));
  }, Ce = (v) => {
    Q ? C({ align: v }) : (t.activeTool.textAlign = v, i((st) => st + 1));
  }, be = (v) => {
    Q ? C({ color: v }) : (t.activeTool.color = v, i((st) => st + 1));
  }, He = {
    position: "fixed",
    left: m.x,
    top: m.y,
    width: bn,
    background: "#1e1e2e",
    borderRadius: 12,
    padding: "12px 16px",
    display: "flex",
    flexDirection: "column",
    gap: 10,
    zIndex: 99,
    color: "white",
    fontSize: 11,
    maxHeight: "calc(100vh - 40px)",
    overflowY: "auto",
    boxShadow: "0 4px 24px rgba(0,0,0,0.4)"
  };
  return /* @__PURE__ */ k(
    "div",
    {
      ref: c,
      "data-sb-props-panel": !0,
      style: He,
      onPointerDown: (v) => v.stopPropagation(),
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: y,
            style: {
              cursor: p ? "grabbing" : "grab",
              margin: "-12px -16px 8px -16px",
              padding: "8px 16px",
              userSelect: "none",
              display: "flex",
              alignItems: "center",
              gap: 6,
              borderBottom: "1px solid #333",
              color: "#999",
              fontSize: 10
            },
            children: /* @__PURE__ */ u("span", { style: { fontWeight: 600, letterSpacing: "0.02em", color: "white" }, children: "Inspector" })
          }
        ),
        Ae && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Font" }),
            /* @__PURE__ */ u(
              Yr,
              {
                value: ot,
                onChange: ro,
                fontsInScene: Qt
              }
            )
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Size" }),
            Tp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => pe(v),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: ft === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: v
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Align" }),
            Pp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.key,
                onClick: () => Ce(v.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: pt === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 12,
                  borderRadius: 6
                },
                children: v.label
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            je.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => be(v),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: rt === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          Q && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...je].map((v, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => C({ borderColor: v ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (g.node.data.borderColor ?? null) === v ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: 2,
                      height: 24,
                      background: "#e74c3c",
                      transform: "rotate(45deg)"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          Q && g.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ao.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => C({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "solid") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          Q && g.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Lo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => C({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] })
        ] }),
        Jt && /* @__PURE__ */ k(ut, { children: [
          $ && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Shape" }),
            Ip.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => {
                  t.activeTool.shapeType = v.key, i((st) => st + 1);
                },
                style: {
                  ...Vt,
                  width: 28,
                  height: 28,
                  background: (t.activeTool.shapeType ?? "rect") === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(Rp, { name: v.key })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke" }),
            je.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => _t(v),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: It === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          Zt && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Fill" }),
            Mp.map((v, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => Le(v),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: zt === v ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          Zt && zt && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Fill pattern" }),
            Cp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => eo(v.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: bt === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 9,
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(Ap, { style: v.key })
              },
              v.key
            ))
          ] }),
          $t && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke style" }),
            Ao.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => xo(v.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: ht === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke width" }),
            Lo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => ue(v),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: Wt === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] }),
          he && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Roughness" }),
            zp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => Me(v.value),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: Tt === v.value ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 9,
                  borderRadius: 6
                },
                children: v.label
              },
              v.value
            ))
          ] })
        ] }),
        L && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            je.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ color: v }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: g.node.data.color === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ao.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => I({ style: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: g.node.data.style === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Lo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => I({ strokeWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: g.node.data.strokeWidth === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Head" }),
            ["none", "arrow", "filled", "dot"].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ arrowHead: v }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.arrowHead ?? "none") === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: v === "none" ? "None" : v === "arrow" ? "▷" : v === "filled" ? "▶" : "●"
              },
              v
            ))
          ] }),
          (g.node.data.arrowHead ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Head size" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "range",
                min: 4,
                max: 40,
                step: 1,
                value: g.node.data.arrowHeadSize ?? Math.max(8, g.node.data.strokeWidth * 3),
                onChange: (v) => I({ arrowHeadSize: Number(v.target.value) }),
                style: { flex: 1 }
              }
            ),
            /* @__PURE__ */ u("span", { style: { color: "#999", fontSize: 11, minWidth: 24, textAlign: "right" }, children: g.node.data.arrowHeadSize ?? Math.max(8, g.node.data.strokeWidth * 3) })
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Tail" }),
            ["none", "arrow", "filled", "dot"].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ arrowTail: v }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.arrowTail ?? "none") === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: v === "none" ? "None" : v === "arrow" ? "◁" : v === "filled" ? "◀" : "●"
              },
              v
            ))
          ] }),
          (g.node.data.arrowTail ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Tail size" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "range",
                min: 4,
                max: 40,
                step: 1,
                value: g.node.data.arrowTailSize ?? Math.max(8, g.node.data.strokeWidth * 3),
                onChange: (v) => I({ arrowTailSize: Number(v.target.value) }),
                style: { flex: 1 }
              }
            ),
            /* @__PURE__ */ u("span", { style: { color: "#999", fontSize: 11, minWidth: 24, textAlign: "right" }, children: g.node.data.arrowTailSize ?? Math.max(8, g.node.data.strokeWidth * 3) })
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Label" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                value: g.node.data.label ?? "",
                onChange: (v) => I({ label: v.target.value || void 0 }),
                placeholder: "Edge label...",
                style: {
                  flex: 1,
                  background: "#2a2a3e",
                  color: "white",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 8px",
                  fontSize: 11,
                  outline: "none"
                }
              }
            )
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Path" }),
            [
              { key: "bezier", label: "Bezier" },
              { key: "straight", label: "Straight" },
              { key: "smoothstep", label: "Smooth" },
              { key: "step", label: "Step" }
            ].map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => I({ edgeType: v.key }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.edgeType ?? "bezier") === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: v.label
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Animate" }),
            /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ animated: !g.node.data.animated }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 12px",
                  background: g.node.data.animated ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: g.node.data.animated ? "On" : "Off"
              }
            )
          ] }),
          g.node.data.animated && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Direction" }),
            ["forward", "reverse", "both"].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ animatedDirection: v }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (g.node.data.animatedDirection ?? "forward") === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: v === "forward" ? "→" : v === "reverse" ? "←" : "⇆"
              },
              v
            ))
          ] })
        ] }),
        X && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...je].map((v, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => M({ borderColor: v ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (g.node.data.borderColor ?? null) === v ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ao.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => M({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "solid") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Lo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => M({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: { ...Bt, marginTop: 4 }, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Background" }),
            /* @__PURE__ */ u(
              "button",
              {
                onClick: async () => {
                  if (!(K === "loading" || g.kind !== "image")) {
                    at("loading");
                    try {
                      const { removeBackground: v } = await import("@imgly/background-removal"), Yt = await (await fetch(g.node.data.src)).blob(), ne = await v(Yt), xe = new FileReader(), De = await new Promise((no, ge) => {
                        xe.onload = () => no(xe.result), xe.onerror = ge, xe.readAsDataURL(ne);
                      });
                      M({ src: De }), at("idle");
                    } catch (v) {
                      console.error("Background removal failed:", v), at("error"), setTimeout(() => at("idle"), 3e3);
                    }
                  }
                },
                disabled: K === "loading",
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 10px",
                  background: K === "error" ? "#e74c3c" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6,
                  gap: 4,
                  opacity: K === "loading" ? 0.6 : 1
                },
                children: K === "loading" ? "Removing..." : K === "error" ? "Failed" : "Remove BG"
              }
            )
          ] })
        ] }),
        _ && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...je].map((v, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => D({ borderColor: v ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (g.node.data.borderColor ?? null) === v ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ao.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => D({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "solid") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          g.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Lo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => D({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] })
        ] }),
        j && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Label" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                value: g.node.data.label ?? "",
                onChange: (v) => T({ label: v.target.value || void 0 }),
                placeholder: "Frame label...",
                style: {
                  flex: 1,
                  background: "#2a2a3e",
                  color: "white",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 8px",
                  fontSize: 11,
                  outline: "none"
                }
              }
            )
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Background" }),
            [null, ...je].map((v, st) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => T({ backgroundColor: v ? `${v}15` : void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (() => {
                    const Yt = g.node.data.backgroundColor;
                    return (v === null ? !Yt : Yt === `${v}15`) ? "2px solid white" : `2px solid ${st === 0 ? "#555" : "transparent"}`;
                  })(),
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: st === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            je.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => T({ borderColor: v }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: g.node.data.borderColor === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Ao.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => T({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (g.node.data.borderStyle ?? "dashed") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Lo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => T({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (g.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] })
        ] }),
        q && /* @__PURE__ */ k(ut, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            [
              "#FEF3C7",
              "#FCE7F3",
              "#DBEAFE",
              "#D1FAE5",
              "#EDE9FE",
              "#FFEDD5"
            ].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => P({ color: v }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: g.node.data.color === v ? "2px solid #1e1e2e" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Size" }),
            [12, 14, 16, 20, 24].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => P({ fontSize: v }),
                style: {
                  ...Vt,
                  width: 32,
                  height: 24,
                  background: (g.node.data.fontSize ?? 16) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6,
                  fontSize: 10,
                  color: "white"
                },
                children: v
              },
              v
            ))
          ] })
        ] }),
        B && (() => {
          const { node: v, PanelComponent: st } = g;
          return /* @__PURE__ */ u(st, { node: v, data: v.data, engine: t, updateData: G });
        })(),
        !L && !B && /* @__PURE__ */ k("div", { style: Bt, children: [
          /* @__PURE__ */ u("span", { style: Nt, children: "Opacity" }),
          /* @__PURE__ */ u(
            "input",
            {
              type: "range",
              min: 0,
              max: 100,
              value: Math.round(Rt * 100),
              onChange: (v) => oo(parseInt(v.target.value) / 100),
              style: { flex: 1, accentColor: "#3b82f6" }
            }
          ),
          /* @__PURE__ */ u("span", { style: { width: 28, textAlign: "right", fontSize: 10 }, children: Math.round(Rt * 100) })
        ] })
      ]
    }
  );
}
function Rp({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ k(ut, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
function Ap({ style: t }) {
  return t === "hachure" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: "white", strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: "white", strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: "white", strokeWidth: 1.5 })
  ] }) : t === "cross-hatch" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 2, y1: 2, x2: 8, y2: 14, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 2, x2: 14, y2: 14, stroke: "white", strokeWidth: 1.2 })
  ] }) : /* @__PURE__ */ u("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: /* @__PURE__ */ u("rect", { x: 2, y: 2, width: 16, height: 12, fill: "white", rx: 2 }) });
}
export {
  Ze as D,
  Ql as N,
  No as P,
  Op as S,
  Yp as T,
  Tn as a,
  Pd as b,
  Gp as c,
  ep as d,
  Xp as e,
  jh as f,
  Ul as g,
  Td as h,
  nc as i,
  od as j,
  id as k,
  yd as l,
  sr as m,
  gt as n,
  jn as o,
  dd as p,
  On as q,
  Al as r,
  ao as s,
  Cl as t,
  gh as u,
  nd as v,
  xd as w,
  pd as x,
  qt as y
};
