var Ua = Object.defineProperty;
var Qa = (t, e, o) => e in t ? Ua(t, e, { enumerable: !0, configurable: !0, writable: !0, value: o }) : t[e] = o;
var Ct = (t, e, o) => Qa(t, typeof e != "symbol" ? e + "" : e, o);
import { BlockNoteSchema as Ja, defaultBlockSpecs as $a, BlockNoteEditor as _a } from "@blocknote/core";
import { jsxs as k, jsx as u, Fragment as ft } from "react/jsx-runtime";
import tl, { memo as ue, useRef as at, useState as et, useEffect as yt, useCallback as st, Component as el, useMemo as Kt, useLayoutEffect as Hr, useContext as Nn, createContext as Ii, Suspense as ol, lazy as nl } from "react";
import { useCreateBlockNote as rl } from "@blocknote/react";
import { BlockNoteView as sl } from "@blocknote/mantine";
import "@blocknote/mantine/style.css";
import { createPortal as Oe, flushSync as il } from "react-dom";
const al = "useandom-26T198340PX75pxJACKVERYMINDBUSHWOLF_GQZbfghjklqvwyzrict";
let bt = (t = 21) => {
  let e = "", o = crypto.getRandomValues(new Uint8Array(t |= 0));
  for (; t--; )
    e += al[o[t] & 63];
  return e;
};
const ll = {
  id: "kanban",
  label: "Kanban Board",
  nodes: [
    // Column frames
    { id: "kf1", type: "frame", x: 0, y: 0, w: 300, h: 500, z: 0, data: { label: "To Do", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "kf2", type: "frame", x: 330, y: 0, w: 300, h: 500, z: 0, data: { label: "In Progress", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    { id: "kf3", type: "frame", x: 660, y: 0, w: 300, h: 500, z: 0, data: { label: "Done", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    // To Do stickies
    { id: "ks1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 1", color: "#fecaca", fontSize: 14 } },
    { id: "ks2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Task 2", color: "#fecaca", fontSize: 14 } },
    { id: "ks3", type: "sticky", x: 30, y: 240, w: 240, h: 80, z: 1, data: { text: "Task 3", color: "#fecaca", fontSize: 14 } },
    // In Progress stickies
    { id: "ks4", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 4", color: "#fed7aa", fontSize: 14 } },
    { id: "ks5", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Task 5", color: "#fed7aa", fontSize: 14 } },
    // Done stickies
    { id: "ks6", type: "sticky", x: 690, y: 40, w: 240, h: 80, z: 1, data: { text: "Task 6", color: "#bbf7d0", fontSize: 14 } }
  ]
}, cl = {
  id: "mind-map",
  label: "Mind Map",
  nodes: [
    // Central topic
    { id: "c0", type: "shape", x: 250, y: 200, w: 200, h: 80, z: 0, data: { shape: "ellipse", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "Main Topic" } },
    // Branch topics
    { id: "c1", type: "shape", x: 0, y: 0, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#ec4899", strokeWidth: 2, roughness: 0, fill: "#fdf2f8", fillStyle: "solid", label: "Idea 1" } },
    { id: "c2", type: "shape", x: 540, y: 0, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Idea 2" } },
    { id: "c3", type: "shape", x: 0, y: 400, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Idea 3" } },
    { id: "c4", type: "shape", x: 540, y: 400, w: 160, h: 60, z: 1, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Idea 4" } },
    // Edges from center to branches
    { id: "e1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c1", style: "solid", color: "#ec4899", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c2", style: "solid", color: "#f59e0b", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c3", style: "solid", color: "#22c55e", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } },
    { id: "e4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 2, data: { fromId: "c0", toId: "c4", style: "solid", color: "#3b82f6", strokeWidth: 2, arrowHead: "filled", edgeType: "bezier" } }
  ]
}, dl = {
  id: "flowchart",
  label: "Flowchart",
  nodes: [
    // Start (ellipse)
    { id: "fc0", type: "shape", x: 200, y: 0, w: 160, h: 60, z: 0, data: { shape: "ellipse", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Start" } },
    // Process 1
    { id: "fc1", type: "shape", x: 200, y: 120, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Process" } },
    // Decision (diamond)
    { id: "fc2", type: "shape", x: 180, y: 250, w: 200, h: 100, z: 0, data: { shape: "diamond", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Decision?" } },
    // Yes branch
    { id: "fc3", type: "shape", x: 200, y: 420, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Action A" } },
    // No branch
    { id: "fc4", type: "shape", x: 480, y: 270, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Action B" } },
    // End
    { id: "fc5", type: "shape", x: 200, y: 540, w: 160, h: 60, z: 0, data: { shape: "ellipse", stroke: "#ef4444", strokeWidth: 2, roughness: 0, fill: "#fef2f2", fillStyle: "solid", label: "End" } },
    // Edges
    { id: "fe1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc0", toId: "fc1", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc1", toId: "fc2", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc2", toId: "fc3", style: "solid", color: "#22c55e", strokeWidth: 2, arrowHead: "filled", label: "Yes", sourceHandle: "bottom" } },
    { id: "fe4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc2", toId: "fc4", style: "solid", color: "#ef4444", strokeWidth: 2, arrowHead: "filled", label: "No", sourceHandle: "right" } },
    { id: "fe5", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc3", toId: "fc5", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } },
    { id: "fe6", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "fc4", toId: "fc5", style: "solid", color: "#666666", strokeWidth: 2, arrowHead: "filled" } }
  ]
}, hl = {
  id: "swot",
  label: "SWOT Analysis",
  nodes: [
    // Quadrant frames
    { id: "sf1", type: "frame", x: 0, y: 0, w: 300, h: 300, z: 0, data: { label: "Strengths", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf2", type: "frame", x: 330, y: 0, w: 300, h: 300, z: 0, data: { label: "Weaknesses", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf3", type: "frame", x: 0, y: 330, w: 300, h: 300, z: 0, data: { label: "Opportunities", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    { id: "sf4", type: "frame", x: 330, y: 330, w: 300, h: 300, z: 0, data: { label: "Threats", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "ss1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Add strength...", color: "#bbf7d0", fontSize: 14 } },
    { id: "ss2", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Add weakness...", color: "#fecaca", fontSize: 14 } },
    { id: "ss3", type: "sticky", x: 30, y: 370, w: 240, h: 80, z: 1, data: { text: "Add opportunity...", color: "#bfdbfe", fontSize: 14 } },
    { id: "ss4", type: "sticky", x: 360, y: 370, w: 240, h: 80, z: 1, data: { text: "Add threat...", color: "#fed7aa", fontSize: 14 } }
  ]
}, ul = {
  id: "retrospective",
  label: "Retrospective",
  nodes: [
    // Column frames
    { id: "rf1", type: "frame", x: 0, y: 0, w: 300, h: 500, z: 0, data: { label: "What went well", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "rf2", type: "frame", x: 330, y: 0, w: 300, h: 500, z: 0, data: { label: "What didn't go well", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "rf3", type: "frame", x: 660, y: 0, w: 300, h: 500, z: 0, data: { label: "Action items", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "rs1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Good thing 1", color: "#bbf7d0", fontSize: 14 } },
    { id: "rs2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Good thing 2", color: "#bbf7d0", fontSize: 14 } },
    { id: "rs3", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Issue 1", color: "#fecaca", fontSize: 14 } },
    { id: "rs4", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Issue 2", color: "#fecaca", fontSize: 14 } },
    { id: "rs5", type: "sticky", x: 690, y: 40, w: 240, h: 80, z: 1, data: { text: "Action 1", color: "#bfdbfe", fontSize: 14 } },
    { id: "rs6", type: "sticky", x: 690, y: 140, w: 240, h: 80, z: 1, data: { text: "Action 2", color: "#bfdbfe", fontSize: 14 } }
  ]
}, pl = {
  id: "weekly-planner",
  label: "Weekly Planner",
  nodes: [
    // Day frames
    { id: "wf1", type: "frame", x: 0, y: 0, w: 170, h: 400, z: 0, data: { label: "Monday", backgroundColor: "#6366f115", borderColor: "#6366f1", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf2", type: "frame", x: 180, y: 0, w: 170, h: 400, z: 0, data: { label: "Tuesday", backgroundColor: "#8b5cf615", borderColor: "#8b5cf6", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf3", type: "frame", x: 360, y: 0, w: 170, h: 400, z: 0, data: { label: "Wednesday", backgroundColor: "#ec489915", borderColor: "#ec4899", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf4", type: "frame", x: 540, y: 0, w: 170, h: 400, z: 0, data: { label: "Thursday", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf5", type: "frame", x: 720, y: 0, w: 170, h: 400, z: 0, data: { label: "Friday", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf6", type: "frame", x: 900, y: 0, w: 170, h: 400, z: 0, data: { label: "Saturday", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 1, borderStyle: "solid" } },
    { id: "wf7", type: "frame", x: 1080, y: 0, w: 170, h: 400, z: 0, data: { label: "Sunday", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 1, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "ws1", type: "sticky", x: 10, y: 40, w: 150, h: 60, z: 1, data: { text: "Task", color: "#e0e7ff", fontSize: 12 } },
    { id: "ws2", type: "sticky", x: 190, y: 40, w: 150, h: 60, z: 1, data: { text: "Task", color: "#ede9fe", fontSize: 12 } }
  ]
}, fl = {
  id: "pros-cons",
  label: "Pros & Cons",
  nodes: [
    // Column frames
    { id: "pf1", type: "frame", x: 0, y: 0, w: 300, h: 450, z: 0, data: { label: "Pros", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 2, borderStyle: "solid" } },
    { id: "pf2", type: "frame", x: 330, y: 0, w: 300, h: 450, z: 0, data: { label: "Cons", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "ps1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Pro 1", color: "#bbf7d0", fontSize: 14 } },
    { id: "ps2", type: "sticky", x: 30, y: 140, w: 240, h: 80, z: 1, data: { text: "Pro 2", color: "#bbf7d0", fontSize: 14 } },
    { id: "ps3", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Con 1", color: "#fecaca", fontSize: 14 } },
    { id: "ps4", type: "sticky", x: 360, y: 140, w: 240, h: 80, z: 1, data: { text: "Con 2", color: "#fecaca", fontSize: 14 } }
  ]
}, yl = {
  id: "eisenhower",
  label: "Eisenhower Matrix",
  nodes: [
    // Quadrant frames
    { id: "ef1", type: "frame", x: 0, y: 0, w: 300, h: 300, z: 0, data: { label: "Do First (Urgent + Important)", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef2", type: "frame", x: 330, y: 0, w: 300, h: 300, z: 0, data: { label: "Schedule (Important)", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef3", type: "frame", x: 0, y: 330, w: 300, h: 300, z: 0, data: { label: "Delegate (Urgent)", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 2, borderStyle: "solid" } },
    { id: "ef4", type: "frame", x: 330, y: 330, w: 300, h: 300, z: 0, data: { label: "Eliminate", backgroundColor: "#6b728015", borderColor: "#6b7280", borderWidth: 2, borderStyle: "solid" } },
    // Stickies
    { id: "es1", type: "sticky", x: 30, y: 40, w: 240, h: 80, z: 1, data: { text: "Critical task...", color: "#fecaca", fontSize: 14 } },
    { id: "es2", type: "sticky", x: 360, y: 40, w: 240, h: 80, z: 1, data: { text: "Plan for later...", color: "#bfdbfe", fontSize: 14 } },
    { id: "es3", type: "sticky", x: 30, y: 370, w: 240, h: 80, z: 1, data: { text: "Hand off...", color: "#fed7aa", fontSize: 14 } },
    { id: "es4", type: "sticky", x: 360, y: 370, w: 240, h: 80, z: 1, data: { text: "Drop this...", color: "#e5e7eb", fontSize: 14 } }
  ]
}, ml = {
  id: "org-chart",
  label: "Org Chart",
  nodes: [
    // Top level
    { id: "oc0", type: "shape", x: 250, y: 0, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "CEO" } },
    // Second level
    { id: "oc1", type: "shape", x: 0, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Engineering" } },
    { id: "oc2", type: "shape", x: 250, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Product" } },
    { id: "oc3", type: "shape", x: 500, y: 140, w: 160, h: 60, z: 0, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Marketing" } },
    // Third level
    { id: "oc4", type: "shape", x: -60, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Frontend" } },
    { id: "oc5", type: "shape", x: 80, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Backend" } },
    { id: "oc6", type: "shape", x: 200, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Design" } },
    { id: "oc7", type: "shape", x: 360, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Research" } },
    { id: "oc8", type: "shape", x: 450, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Content" } },
    { id: "oc9", type: "shape", x: 590, y: 280, w: 140, h: 50, z: 0, data: { shape: "rect", stroke: "#94a3b8", strokeWidth: 1, roughness: 0, fill: "#f8fafc", fillStyle: "solid", label: "Growth" } },
    // Edges - top to second
    { id: "oe1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc1", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc2", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc0", toId: "oc3", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    // Edges - second to third
    { id: "oe4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc1", toId: "oc4", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe5", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc1", toId: "oc5", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe6", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc2", toId: "oc6", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe7", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc2", toId: "oc7", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe8", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc3", toId: "oc8", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } },
    { id: "oe9", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "oc3", toId: "oc9", style: "solid", color: "#cbd5e1", strokeWidth: 1, arrowHead: "none", sourceHandle: "bottom", targetHandle: "top" } }
  ]
}, bl = {
  id: "timeline",
  label: "Project Timeline",
  nodes: [
    // Phase shapes
    { id: "tl0", type: "shape", x: 0, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#eef2ff", fillStyle: "solid", label: "Discovery" } },
    { id: "tl1", type: "shape", x: 230, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#eff6ff", fillStyle: "solid", label: "Design" } },
    { id: "tl2", type: "shape", x: 460, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#f0fdf4", fillStyle: "solid", label: "Development" } },
    { id: "tl3", type: "shape", x: 690, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#fffbeb", fillStyle: "solid", label: "Testing" } },
    { id: "tl4", type: "shape", x: 920, y: 0, w: 180, h: 70, z: 0, data: { shape: "rect", stroke: "#ec4899", strokeWidth: 2, roughness: 0, fill: "#fdf2f8", fillStyle: "solid", label: "Launch" } },
    // Milestone diamonds below
    { id: "tl5", type: "shape", x: 185, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#6366f1", strokeWidth: 2, roughness: 0, fill: "#6366f1", fillStyle: "solid" } },
    { id: "tl6", type: "shape", x: 415, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#3b82f6", strokeWidth: 2, roughness: 0, fill: "#3b82f6", fillStyle: "solid" } },
    { id: "tl7", type: "shape", x: 645, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#22c55e", strokeWidth: 2, roughness: 0, fill: "#22c55e", fillStyle: "solid" } },
    { id: "tl8", type: "shape", x: 875, y: 120, w: 40, h: 40, z: 0, data: { shape: "diamond", stroke: "#f59e0b", strokeWidth: 2, roughness: 0, fill: "#f59e0b", fillStyle: "solid" } },
    // Phase arrows
    { id: "te1", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl0", toId: "tl1", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te2", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl1", toId: "tl2", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te3", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl2", toId: "tl3", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    { id: "te4", type: "edge", x: 0, y: 0, w: 0, h: 0, z: 1, data: { fromId: "tl3", toId: "tl4", style: "solid", color: "#94a3b8", strokeWidth: 2, arrowHead: "filled", sourceHandle: "right", targetHandle: "left" } },
    // Milestone labels
    { id: "tt1", type: "text", x: 155, y: 170, w: 100, h: "auto", z: 1, data: { text: "Kickoff", fontSize: 12, fontFamily: "sans-serif", color: "#6366f1", align: "center" } },
    { id: "tt2", type: "text", x: 385, y: 170, w: 100, h: "auto", z: 1, data: { text: "Specs Done", fontSize: 12, fontFamily: "sans-serif", color: "#3b82f6", align: "center" } },
    { id: "tt3", type: "text", x: 615, y: 170, w: 100, h: "auto", z: 1, data: { text: "Code Freeze", fontSize: 12, fontFamily: "sans-serif", color: "#22c55e", align: "center" } },
    { id: "tt4", type: "text", x: 845, y: 170, w: 100, h: "auto", z: 1, data: { text: "Sign-off", fontSize: 12, fontFamily: "sans-serif", color: "#f59e0b", align: "center" } }
  ]
}, gl = {
  id: "bmc",
  label: "Business Model Canvas",
  nodes: [
    // Top row: 5 sections
    { id: "bf1", type: "frame", x: 0, y: 0, w: 220, h: 320, z: 0, data: { label: "Key Partners", backgroundColor: "#8b5cf615", borderColor: "#8b5cf6", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf2", type: "frame", x: 230, y: 0, w: 220, h: 150, z: 0, data: { label: "Key Activities", backgroundColor: "#6366f115", borderColor: "#6366f1", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf3", type: "frame", x: 230, y: 160, w: 220, h: 160, z: 0, data: { label: "Key Resources", backgroundColor: "#3b82f615", borderColor: "#3b82f6", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf4", type: "frame", x: 460, y: 0, w: 220, h: 320, z: 0, data: { label: "Value Propositions", backgroundColor: "#22c55e15", borderColor: "#22c55e", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf5", type: "frame", x: 690, y: 0, w: 220, h: 150, z: 0, data: { label: "Customer Relationships", backgroundColor: "#f59e0b15", borderColor: "#f59e0b", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf6", type: "frame", x: 690, y: 160, w: 220, h: 160, z: 0, data: { label: "Channels", backgroundColor: "#ec489915", borderColor: "#ec4899", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf7", type: "frame", x: 920, y: 0, w: 220, h: 320, z: 0, data: { label: "Customer Segments", backgroundColor: "#ef444415", borderColor: "#ef4444", borderWidth: 1, borderStyle: "solid" } },
    // Bottom row: 2 wide sections
    { id: "bf8", type: "frame", x: 0, y: 340, w: 565, h: 160, z: 0, data: { label: "Cost Structure", backgroundColor: "#6b728015", borderColor: "#6b7280", borderWidth: 1, borderStyle: "solid" } },
    { id: "bf9", type: "frame", x: 575, y: 340, w: 565, h: 160, z: 0, data: { label: "Revenue Streams", backgroundColor: "#14b8a615", borderColor: "#14b8a6", borderWidth: 1, borderStyle: "solid" } },
    // Placeholder stickies
    { id: "bs1", type: "sticky", x: 15, y: 40, w: 190, h: 60, z: 1, data: { text: "Partner...", color: "#ede9fe", fontSize: 12 } },
    { id: "bs2", type: "sticky", x: 475, y: 40, w: 190, h: 60, z: 1, data: { text: "Value prop...", color: "#bbf7d0", fontSize: 12 } },
    { id: "bs3", type: "sticky", x: 935, y: 40, w: 190, h: 60, z: 1, data: { text: "Segment...", color: "#fecaca", fontSize: 12 } }
  ]
}, xl = {
  id: "storyboard",
  label: "Storyboard",
  nodes: [
    // Row 1
    { id: "sb1", type: "frame", x: 0, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 1", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb2", type: "frame", x: 320, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 2", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb3", type: "frame", x: 640, y: 0, w: 300, h: 250, z: 0, data: { label: "Scene 3", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    // Row 2
    { id: "sb4", type: "frame", x: 0, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 4", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb5", type: "frame", x: 320, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 5", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    { id: "sb6", type: "frame", x: 640, y: 280, w: 300, h: 250, z: 0, data: { label: "Scene 6", backgroundColor: "#f8fafc", borderColor: "#94a3b8", borderWidth: 1, borderStyle: "solid" } },
    // Caption text below each frame
    { id: "st1", type: "text", x: 0, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st2", type: "text", x: 320, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st3", type: "text", x: 640, y: 200, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st4", type: "text", x: 0, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st5", type: "text", x: 320, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } },
    { id: "st6", type: "text", x: 640, y: 480, w: 300, h: "auto", z: 1, data: { text: "Caption...", fontSize: 12, fontFamily: "sans-serif", color: "#64748b", align: "center" } }
  ]
}, Ti = [
  ll,
  cl,
  dl,
  hl,
  ul,
  pl,
  fl,
  yl,
  ml,
  bl,
  gl,
  xl
];
class wl {
  constructor() {
    Ct(this, "undoStack", []);
    Ct(this, "redoStack", []);
    Ct(this, "maxSize", 50);
  }
  pushSnapshot(e, o) {
    const n = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (n.groupParent = Array.from(o.entries()));
    const r = JSON.stringify(n);
    this.undoStack.push(r), this.undoStack.length > this.maxSize && this.undoStack.shift(), this.redoStack = [];
  }
  undo(e, o) {
    if (this.undoStack.length === 0) return null;
    const n = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (n.groupParent = Array.from(o.entries())), this.redoStack.push(JSON.stringify(n));
    const r = JSON.parse(this.undoStack.pop());
    return {
      nodes: new Map(r.nodes),
      groupParent: new Map(r.groupParent ?? [])
    };
  }
  redo(e, o) {
    if (this.redoStack.length === 0) return null;
    const n = { nodes: Array.from(e.entries()) };
    o && o.size > 0 && (n.groupParent = Array.from(o.entries())), this.undoStack.push(JSON.stringify(n));
    const r = JSON.parse(this.redoStack.pop());
    return {
      nodes: new Map(r.nodes),
      groupParent: new Map(r.groupParent ?? [])
    };
  }
  clear() {
    this.undoStack = [], this.redoStack = [];
  }
  canUndo() {
    return this.undoStack.length > 0;
  }
  canRedo() {
    return this.redoStack.length > 0;
  }
}
const Pi = 4, kl = 8, vl = 24;
function fo(t, e, o, n) {
  if (!t.rotation) return [e, o];
  const r = t.x + t.w / 2, s = t.y + n / 2, i = -t.rotation * Math.PI / 180, l = Math.cos(i), a = Math.sin(i), c = e - r, h = o - s;
  return [r + c * l - h * a, s + c * a + h * l];
}
function Tn(t, e) {
  return t.h !== "auto" ? t.h : (e == null ? void 0 : e[t.id]) ?? 100;
}
function Sl(t, e, o, n = 1, r, s) {
  const i = Array.from(t.values()).filter((c) => c.type !== "edge").sort((c, h) => h.z - c.z);
  let l = null, a = null;
  for (const c of i)
    if (c.type === "draw") {
      if (Or(c, e, o, n))
        return c;
    } else if (c.type === "shape") {
      if (Hn(c, e, o, n)) return c;
      if (!a && c.data.label) {
        const h = c.h === "auto" ? 100 : c.h, [p, d] = fo(c, e, o, h), f = Li(c, h);
        f && p >= f.lx && p <= f.rx && d >= f.ly && d <= f.ry && (a = c);
      }
    } else if (s && s.has(c.type)) {
      const h = Tn(c, r);
      Ri(c, e, o, n, h) && (l || (l = c));
    } else {
      const h = Tn(c, r), p = Pi / n, [d, f] = fo(c, e, o, h);
      d >= c.x - p && d <= c.x + c.w + p && f >= c.y - p && f <= c.y + h + p && (a || (a = c));
    }
  return a ?? l;
}
function Ri(t, e, o, n, r) {
  const s = r ?? (t.h === "auto" ? 100 : t.h), [i, l] = fo(t, e, o, s), a = kl / n;
  if (t.data.label && i >= t.x && i <= t.x + t.w && l >= t.y - vl && l <= t.y)
    return !0;
  if (i < t.x - a || i > t.x + t.w + a || l < t.y - a || l > t.y + s + a)
    return !1;
  const h = Math.abs(i - t.x), p = Math.abs(i - (t.x + t.w)), d = Math.abs(l - t.y), f = Math.abs(l - (t.y + s)), m = i >= t.x - a && i <= t.x + t.w + a;
  return l >= t.y - a && l <= t.y + s + a && (h <= a || p <= a) || m && (d <= a || f <= a);
}
function Ai(t, e, o, n, r, s) {
  const i = r - o, l = s - n, a = i * i + l * l;
  if (a === 0) return (t - o) ** 2 + (e - n) ** 2;
  const c = Math.max(0, Math.min(1, ((t - o) * i + (e - n) * l) / a)), h = o + c * i, p = n + c * l;
  return (t - h) ** 2 + (e - p) ** 2;
}
function Li(t, e) {
  const o = t.data;
  if (!o.label) return null;
  const n = o.labelFontSize ?? 14, r = n * 1.3, s = n * 0.55, l = t.w - 12 * 2, a = o.label.split(`
`);
  let c = 0;
  for (const m of a) {
    const y = m.length * s;
    c += Math.max(1, Math.ceil(y / Math.max(l, 1)));
  }
  const h = c * r, p = Math.min(l, Math.max(...a.map((m) => m.length)) * s), d = t.x + t.w / 2, f = t.y + e / 2;
  return {
    lx: d - p / 2 - 4,
    ly: f - h / 2 - 4,
    rx: d + p / 2 + 4,
    ry: f + h / 2 + 4
  };
}
function Hn(t, e, o, n, r) {
  const s = t.h === "auto" ? 100 : t.h, [i, l] = fo(t, e, o, s), a = t.data, h = (a.strokeWidth ?? 2) / 2 / n, p = !!a.fill || !!r;
  switch (a.shape) {
    case "rect": {
      if (p)
        return i >= t.x - h && i <= t.x + t.w + h && l >= t.y - h && l <= t.y + s + h;
      const d = Math.abs(i - t.x), f = Math.abs(i - (t.x + t.w)), m = Math.abs(l - t.y), y = Math.abs(l - (t.y + s)), b = i >= t.x - h && i <= t.x + t.w + h;
      return l >= t.y - h && l <= t.y + s + h && (d <= h || f <= h) || b && (m <= h || y <= h);
    }
    case "ellipse": {
      const d = t.x + t.w / 2, f = t.y + s / 2, m = t.w / 2, y = s / 2;
      if (m === 0 || y === 0) return !1;
      const b = (i - d) / m, g = (l - f) / y, x = b * b + g * g;
      if (p) {
        const M = ((m + h) / m) ** 2;
        return x <= M;
      }
      const S = h / Math.min(m, y);
      return Math.abs(Math.sqrt(x) - 1) <= S;
    }
    case "diamond": {
      const d = t.x + t.w / 2, f = t.y + s / 2, m = t.w / 2, y = s / 2;
      if (m === 0 || y === 0) return !1;
      const b = Math.abs(i - d) / m, g = Math.abs(l - f) / y, x = b + g;
      if (p) {
        const M = h / Math.min(m, y);
        return x <= 1 + M;
      }
      const S = h / Math.min(m, y);
      return Math.abs(x - 1) <= S;
    }
    case "line":
    case "arrow": {
      const d = a.startPoint ?? [0, 0], f = a.endPoint ?? [t.w, s], m = t.x + d[0], y = t.y + d[1], b = t.x + f[0], g = t.y + f[1];
      return Ai(i, l, m, y, b, g) <= h * h;
    }
    default:
      return i >= t.x - h && i <= t.x + t.w + h && l >= t.y - h && l <= t.y + s + h;
  }
}
function Ml(t, e, o) {
  let n = !1;
  for (let r = 0, s = o.length - 1; r < o.length; s = r++) {
    const i = o[r][0], l = o[r][1], a = o[s][0], c = o[s][1];
    l > e != c > e && t < (a - i) * (e - l) / (c - l) + i && (n = !n);
  }
  return n;
}
function Or(t, e, o, n) {
  const s = t.data.strokeWidth / 2 / n, i = s * s, l = t.h === "auto" ? 100 : t.h, [a, c] = fo(t, e, o, l);
  if (a < t.x - s || a > t.x + t.w + s || c < t.y - s || c > t.y + l + s)
    return !1;
  const h = t.data.points;
  if (!h || h.length === 0) return !1;
  const p = a - t.x, d = c - t.y;
  if (h.length === 1) {
    const f = p - h[0][0], m = d - h[0][1];
    return f * f + m * m <= i;
  }
  if (t.data.fill && h.length >= 3 && Ml(p, d, h))
    return !0;
  for (let f = 0; f < h.length - 1; f++)
    if (Ai(p, d, h[f][0], h[f][1], h[f + 1][0], h[f + 1][1]) <= i)
      return !0;
  return !1;
}
function Cl(t, e, o, n = 1, r, s) {
  const i = Array.from(t.values()).filter((c) => c.type !== "edge").sort((c, h) => h.z - c.z), l = [], a = [];
  for (const c of i)
    if (c.type === "draw")
      Or(c, e, o, n) && l.push(c);
    else if (c.type === "shape") {
      if (Hn(c, e, o, n))
        l.push(c);
      else if (c.data.label) {
        const h = c.h === "auto" ? 100 : c.h, [p, d] = fo(c, e, o, h), f = Li(c, h);
        f && p >= f.lx && p <= f.rx && d >= f.ly && d <= f.ry && a.push(c);
      }
    } else if (s && s.has(c.type)) {
      const h = Tn(c, r);
      Ri(c, e, o, n, h) && a.push(c);
    } else {
      const h = Tn(c, r), p = Pi / n, [d, f] = fo(c, e, o, h);
      d >= c.x - p && d <= c.x + c.w + p && f >= c.y - p && f <= c.y + h + p && a.push(c);
    }
  return [...l, ...a];
}
function mn(t, e) {
  if (!t.rotation) return { x: t.x, y: t.y, w: t.w, h: e };
  const o = t.x + t.w / 2, n = t.y + e / 2, r = t.w / 2, s = e / 2, i = t.rotation * Math.PI / 180, l = Math.abs(Math.cos(i)), a = Math.abs(Math.sin(i)), c = r * l + s * a, h = r * a + s * l;
  return {
    x: o - c,
    y: n - h,
    w: c * 2,
    h: h * 2
  };
}
const Ie = class Ie {
  constructor(e, o = 0, n) {
    // Increased depth for potentially large boards
    Ct(this, "level");
    Ct(this, "bounds");
    Ct(this, "objects");
    Ct(this, "nodes");
    /** Shared across all levels — maps node ID → measured height for auto-height nodes */
    Ct(this, "heightMap");
    this.bounds = e, this.level = o, this.objects = [], this.nodes = [], this.heightMap = n ?? /* @__PURE__ */ new Map();
  }
  /** Resolve the effective height for a node (uses measured height for auto-height nodes) */
  resolveH(e) {
    return typeof e.h == "number" ? e.h : this.heightMap.get(e.id) ?? 100;
  }
  /** Store a measured height for an auto-height node */
  setMeasuredHeight(e, o) {
    this.heightMap.set(e, o);
  }
  // Clear the quadtree
  clear() {
    this.objects = [];
    for (let e = 0; e < this.nodes.length; e++)
      this.nodes[e].clear();
    this.nodes = [];
  }
  // Split the node into 4 subnodes
  split() {
    const e = this.bounds.w / 2, o = this.bounds.h / 2, n = this.bounds.x, r = this.bounds.y;
    this.nodes[0] = new Ie({ x: n + e, y: r, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[1] = new Ie({ x: n, y: r, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[2] = new Ie({ x: n, y: r + o, w: e, h: o }, this.level + 1, this.heightMap), this.nodes[3] = new Ie({ x: n + e, y: r + o, w: e, h: o }, this.level + 1, this.heightMap);
  }
  // Determine which quadrant the object belongs to
  getIndex(e) {
    let o = -1;
    const n = this.bounds.x + this.bounds.w / 2, r = this.bounds.y + this.bounds.h / 2, s = e.y < r && e.y + e.h < r, i = e.y > r;
    return e.x < n && e.x + e.w < n ? s ? o = 1 : i && (o = 2) : e.x > n && (s ? o = 0 : i && (o = 3)), o;
  }
  // Insert the object into the quadtree
  insert(e, o) {
    const n = o ?? this.resolveH(e);
    o != null && e.h === "auto" && this.heightMap.set(e.id, o);
    const r = mn(e, n);
    if (this.nodes.length) {
      const s = this.getIndex(r);
      if (s !== -1) {
        this.nodes[s].insert(e, n);
        return;
      }
    }
    if (this.objects.push(e), this.objects.length > Ie.MAX_OBJECTS && this.level < Ie.MAX_LEVELS) {
      this.nodes.length || this.split();
      let s = 0;
      for (; s < this.objects.length; ) {
        const i = this.objects[s], l = this.resolveH(i), a = mn(i, l), c = this.getIndex(a);
        c !== -1 ? (this.nodes[c].insert(i, l), this.objects.splice(s, 1)) : s++;
      }
    }
  }
  // Remove an object. Requires the node (with its coordinates) to find it efficiently.
  remove(e) {
    const o = this.objects.findIndex((n) => n.id === e.id);
    if (o !== -1)
      return this.objects.splice(o, 1), !0;
    if (this.nodes.length) {
      const n = this.resolveH(e), r = this.getIndex(mn(e, n));
      if (r !== -1 && this.nodes[r].remove(e))
        return !0;
      for (let s = 0; s < this.nodes.length; s++)
        if (s !== r && this.nodes[s].remove(e)) return !0;
    }
    return !1;
  }
  // Return all objects that could collide with the given rect
  retrieve(e, o) {
    const n = this.getIndex(o);
    for (const r of this.objects) {
      const s = this.resolveH(r), i = mn(r, s);
      i.x < o.x + o.w && i.x + i.w > o.x && i.y < o.y + o.h && i.y + i.h > o.y && e.push(r);
    }
    if (this.nodes.length)
      if (n !== -1)
        this.nodes[n].retrieve(e, o);
      else
        for (const r of this.nodes)
          r.bounds.x < o.x + o.w && r.bounds.x + r.bounds.w > o.x && r.bounds.y < o.y + o.h && r.bounds.y + r.bounds.h > o.y && r.retrieve(e, o);
    return e;
  }
};
// Max number of objects per node before splitting
Ct(Ie, "MAX_OBJECTS", 10), // Max depth of the tree
Ct(Ie, "MAX_LEVELS", 8);
let kr = Ie;
function Fo(t, e, o) {
  return Math.min(Math.max(t, e), o);
}
function No(t, e, o) {
  return {
    x: (e - t.x) / t.zoom,
    y: (o - t.y) / t.zoom
  };
}
function zl(t, e, o) {
  return {
    x: e * t.zoom + t.x,
    y: o * t.zoom + t.y
  };
}
function Il(t, e, o, n) {
  const r = e > 0 ? 0.95 : 1.05, s = Fo(t.zoom * r, 0.1, 5), i = No(t, o, n);
  return {
    x: o - i.x * s,
    y: n - i.y * s,
    zoom: s
  };
}
function Tl(t, e, o, n) {
  const r = Fo(t.zoom * e, 0.1, 5), s = No(t, o, n);
  return {
    x: o - s.x * r,
    y: n - s.y * r,
    zoom: r
  };
}
const Xr = Ja.create({
  blockSpecs: {
    ...$a
    // Future custom blocks:
    // callout: CalloutBlock,
    // aiPrompt: AIPromptBlock,
  }
});
let tr = null;
function Yr() {
  return tr || (tr = _a.create({ schema: Xr })), tr;
}
async function Pl(t) {
  return await Yr().blocksToMarkdownLossy(t);
}
async function Gr(t) {
  return await Yr().tryParseMarkdownToBlocks(t);
}
function Ei(t) {
  return Yr().tryParseHTMLToBlocks(t);
}
function Rl(t, e, o) {
  const [n, r] = t, [s, i] = e, [l, a] = o, c = l - s, h = a - i, p = c * c + h * h;
  if (p === 0)
    return (n - s) ** 2 + (r - i) ** 2;
  let d = ((n - s) * c + (r - i) * h) / p;
  d = Math.max(0, Math.min(1, d));
  const f = s + d * c, m = i + d * h;
  return (n - f) ** 2 + (r - m) ** 2;
}
function vr(t, e = 1) {
  if (t.length <= 2) return t;
  let o = 0, n = 0;
  const r = t[0], s = t[t.length - 1];
  for (let a = 1; a < t.length - 1; a++) {
    const c = Rl(t[a], r, s);
    c > o && (o = c, n = a);
  }
  if (o <= e)
    return [r, s];
  const i = vr(t.slice(0, n + 1), e), l = vr(t.slice(n), e);
  return [...i.slice(0, -1), ...l];
}
async function Al(t, e) {
  const o = [], n = ['canvas_w="2000"', 'canvas_h="1500"', 'grid="20"', 'snap="false"'];
  if (e != null && e.background && e.background !== "dot-grid" && n.push(`background="${e.background}"`), e != null && e.originView) {
    const d = e.originView;
    n.push(`originView="${d.x},${d.y},${d.zoom}"`);
  }
  o.push(`<!--@meta ${n.join(" ")} -->`), o.push("");
  const r = t.filter((d) => d.type === "frame").sort((d, f) => d.z - f.z || d.y - f.y || d.x - f.x);
  for (const d of r) {
    const f = d.h === "auto" ? "auto" : Math.round(d.h), m = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${f}"`,
      `z="${d.z}"`
    ];
    d.data.label && m.push(`label="${d.data.label.replace(/"/g, "&quot;")}"`), d.data.backgroundColor && m.push(`backgroundColor="${d.data.backgroundColor}"`), d.data.borderColor && m.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && m.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && m.push(`borderStyle="${d.data.borderStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && m.push(`opacity="${d.data.opacity}"`), d.data.slideOrder != null && m.push(`slideOrder="${d.data.slideOrder}"`), d.data.transition && d.data.transition !== "pan" && m.push(`transition="${d.data.transition}"`), d.data.transitionDuration != null && m.push(`transitionDuration="${d.data.transitionDuration}"`), d.rotation && m.push(`rotation="${d.rotation}"`), d.locked && m.push('locked="true"'), d.groupId && m.push(`group="${d.groupId}"`), o.push(`<!--@frame ${m.join(" ")} -->`), o.push("");
  }
  const s = t.filter((d) => d.type === "content").sort((d, f) => d.z - f.z || d.y - f.y || d.x - f.x);
  for (const d of s) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${d.h}"`,
      `z="${d.z}"`
    ];
    d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), d.data.borderColor && f.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && f.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && f.push(`borderStyle="${d.data.borderStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), o.push(`<!--@block ${f.join(" ")} -->`);
    const m = d.data.blocks.length > 0 ? await Pl(d.data.blocks) : "";
    o.push(m), o.push("");
  }
  const i = t.filter((d) => d.type === "draw");
  for (const d of i) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `z="${d.z}"`,
      `tool="${d.data.tool}"`,
      `color="${d.data.color}"`,
      `width="${d.data.strokeWidth}"`
    ];
    d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.data.fill && f.push(`fill="${d.data.fill}"`), d.data.fillStyle && d.data.fillStyle !== "hachure" && f.push(`fillStyle="${d.data.fillStyle}"`), d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@draw ${f.join(" ")} -->`);
    const y = vr([...d.data.points], 1).map(
      ([b, g, x]) => `${(b + d.x).toFixed(1)},${(g + d.y).toFixed(1)},${x.toFixed(2)}`
    ).join(" ");
    o.push(y), o.push("");
  }
  const l = t.filter((d) => d.type === "shape");
  for (const d of l) {
    const f = d.h === "auto" ? "auto" : Math.round(d.h), m = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${f}"`,
      `z="${d.z}"`,
      'tool="shape"',
      `shape="${d.data.shape}"`,
      `color="${d.data.stroke}"`,
      `stroke="${d.data.strokeWidth}"`,
      `roughness="${d.data.roughness}"`
    ];
    d.data.fill && m.push(`fill="${d.data.fill}"`), d.data.fillStyle && d.data.fillStyle !== "hachure" && m.push(`fillStyle="${d.data.fillStyle}"`), d.data.strokeStyle && d.data.strokeStyle !== "solid" && m.push(`strokeStyle="${d.data.strokeStyle}"`), d.data.edgeStyle && d.data.edgeStyle !== "sharp" && m.push(`edgeStyle="${d.data.edgeStyle}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && m.push(`opacity="${d.data.opacity}"`), d.data.startPoint && m.push(`startPt="${d.data.startPoint[0].toFixed(1)},${d.data.startPoint[1].toFixed(1)}"`), d.data.endPoint && m.push(`endPt="${d.data.endPoint[0].toFixed(1)},${d.data.endPoint[1].toFixed(1)}"`), d.data.label && m.push(`label="${d.data.label.replace(/"/g, "&quot;")}"`), d.data.labelFontSize && m.push(`labelFontSize="${d.data.labelFontSize}"`), d.data.labelFontFamily && d.data.labelFontFamily !== "Excalifont" && m.push(`labelFontFamily="${d.data.labelFontFamily}"`), d.data.labelAlign && d.data.labelAlign !== "center" && m.push(`labelAlign="${d.data.labelAlign}"`), d.rotation && m.push(`rotation="${d.rotation}"`), d.locked && m.push('locked="true"'), d.groupId && m.push(`group="${d.groupId}"`), o.push(`<!--@draw ${m.join(" ")} -->`), o.push("");
  }
  const a = t.filter((d) => d.type === "text");
  for (const d of a) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `z="${d.z}"`,
      `fontSize="${d.data.fontSize}"`,
      `fontFamily="${d.data.fontFamily}"`,
      `color="${d.data.color}"`,
      `align="${d.data.align}"`
    ];
    d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@text ${f.join(" ")} -->`), o.push(d.data.text), o.push("");
  }
  const c = t.filter((d) => d.type === "image");
  for (const d of c) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${Math.round(d.h)}"`,
      `z="${d.z}"`,
      `src="${d.data.src.replace(/"/g, "&quot;")}"`
    ];
    d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), d.data.alt && f.push(`alt="${d.data.alt.replace(/"/g, "&quot;")}"`), d.data.opacity != null && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.data.borderColor && f.push(`borderColor="${d.data.borderColor}"`), d.data.borderWidth != null && f.push(`borderWidth="${d.data.borderWidth}"`), d.data.borderStyle && d.data.borderStyle !== "solid" && f.push(`borderStyle="${d.data.borderStyle}"`), o.push(`<!--@image ${f.join(" ")} -->`), o.push("");
  }
  const h = t.filter((d) => d.type === "edge");
  for (const d of h) {
    const f = [
      `id="${d.id}"`,
      `from="${d.data.fromId}"`,
      `to="${d.data.toId}"`,
      `style="${d.data.style}"`,
      `color="${d.data.color}"`
    ];
    d.data.label && f.push(`label="${d.data.label}"`), d.data.strokeWidth && d.data.strokeWidth !== 1 && f.push(`strokeWidth="${d.data.strokeWidth}"`), d.data.arrowHead && d.data.arrowHead !== "none" && f.push(`arrowHead="${d.data.arrowHead}"`), d.data.arrowTail && d.data.arrowTail !== "none" && f.push(`arrowTail="${d.data.arrowTail}"`), d.data.arrowHeadSize && f.push(`arrowHeadSize="${d.data.arrowHeadSize}"`), d.data.arrowTailSize && f.push(`arrowTailSize="${d.data.arrowTailSize}"`), d.data.edgeType && d.data.edgeType !== "bezier" && f.push(`edgeType="${d.data.edgeType}"`), d.data.animated && f.push('animated="true"'), d.data.animatedDirection && d.data.animatedDirection !== "forward" && f.push(`animatedDirection="${d.data.animatedDirection}"`), d.data.sourceHandle && f.push(`sourceHandle="${d.data.sourceHandle}"`), d.data.targetHandle && f.push(`targetHandle="${d.data.targetHandle}"`), d.data.midpointOffset != null && d.data.midpointOffset !== 0.5 && f.push(`midpointOffset="${d.data.midpointOffset}"`), d.data.curveOffset && (d.data.curveOffset[0] !== 0 || d.data.curveOffset[1] !== 0) && f.push(`curveOffset="${d.data.curveOffset[0]},${d.data.curveOffset[1]}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@edge ${f.join(" ")} -->`), o.push("");
  }
  const p = t.filter((d) => d.type === "sticky");
  for (const d of p) {
    const f = [
      `id="${d.id}"`,
      `x="${Math.round(d.x)}"`,
      `y="${Math.round(d.y)}"`,
      `w="${Math.round(d.w)}"`,
      `h="${d.h}"`,
      `z="${d.z}"`,
      `color="${d.data.color}"`
    ];
    d.data.fontSize && d.data.fontSize !== 16 && f.push(`fontSize="${d.data.fontSize}"`), d.data.opacity !== void 0 && d.data.opacity !== 1 && f.push(`opacity="${d.data.opacity}"`), d.rotation && f.push(`rotation="${d.rotation}"`), d.locked && f.push('locked="true"'), d.groupId && f.push(`group="${d.groupId}"`), o.push(`<!--@sticky ${f.join(" ")} -->`), o.push(d.data.text), o.push("");
  }
  return o.join(`
`);
}
const Di = "data:font/woff2;base64,", Qe = "Excalifont", Cn = [
  // Bundled — hand-drawn default
  { key: "Excalifont", label: "Excalifont", category: "hand" },
  // System
  { key: "sans-serif", label: "Sans (system)", category: "system" },
  { key: "serif", label: "Serif (system)", category: "system" },
  { key: "monospace", label: "Mono (system)", category: "system" },
  // Google — Professional
  { key: "Inter", label: "Inter", category: "sans" },
  { key: "Roboto", label: "Roboto", category: "sans" },
  { key: "Open Sans", label: "Open Sans", category: "sans" },
  { key: "Lora", label: "Lora", category: "serif" },
  { key: "Playfair Display", label: "Playfair Display", category: "serif" },
  { key: "Merriweather", label: "Merriweather", category: "serif" },
  // Google — Mono
  { key: "JetBrains Mono", label: "JetBrains Mono", category: "mono" },
  { key: "Fira Code", label: "Fira Code", category: "mono" },
  { key: "Source Code Pro", label: "Source Code Pro", category: "mono" },
  // Google — Handwritten / Casual
  { key: "Caveat", label: "Caveat", category: "hand" },
  { key: "Shadows Into Light", label: "Shadows Into Light", category: "hand" },
  { key: "Dancing Script", label: "Dancing Script", category: "hand" },
  { key: "Amatic SC", label: "Amatic SC", category: "hand" },
  // Google — Display / Funky
  { key: "Pacifico", label: "Pacifico", category: "display" },
  { key: "Lobster", label: "Lobster", category: "display" },
  { key: "Permanent Marker", label: "Permanent Marker", category: "display" },
  { key: "Bangers", label: "Bangers", category: "display" },
  { key: "Righteous", label: "Righteous", category: "display" },
  { key: "Satisfy", label: "Satisfy", category: "display" },
  { key: "Kaushan Script", label: "Kaushan Script", category: "display" },
  { key: "Fredericka the Great", label: "Fredericka the Great", category: "display" },
  { key: "Comfortaa", label: "Comfortaa", category: "display" }
], Wi = /* @__PURE__ */ new Set(["sans-serif", "serif", "monospace"]), Ll = /* @__PURE__ */ new Set(["Excalifont"]), El = /* @__PURE__ */ new Set([...Wi, ...Ll]);
function Dl(t) {
  switch (t) {
    case "serif":
    case "display":
      return "H";
    case "mono":
      return "</>";
    case "hand":
      return "✏";
    default:
      return "A";
  }
}
function Je(t) {
  return Wi.has(t) ? t : `'${t}', sans-serif`;
}
let Is = !1;
function Wl(t = document) {
  if (Is) return;
  Is = !0;
  const e = t.createElement("style");
  e.textContent = `
@font-face {
  font-family: 'Excalifont';
  src: url('${Di}') format('woff2');
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}`, t.head.appendChild(e);
  const o = Cn.filter((r) => !El.has(r.key)).map((r) => "family=" + r.key.replace(/ /g, "+")).join("&"), n = t.createElement("link");
  n.rel = "stylesheet", n.href = `https://fonts.googleapis.com/css2?${o}&display=swap`, t.head.appendChild(n);
}
function Ge(t) {
  const e = {}, o = /(\w+)="([^"]*)"/g;
  let n;
  for (; (n = o.exec(t)) !== null; )
    e[n[1]] = n[2];
  return e;
}
const Fl = {
  default: "dot-grid",
  "cutting-board": "blueprint",
  // Removed grid-as-paper types → nearest color equivalent
  "graph-paper": "plain-white",
  "college-ruled": "plain-white",
  isometric: "plain-white"
};
async function Bl(t) {
  var s, i;
  const e = [], o = {}, n = t.split(`
`);
  let r = 0;
  for (; r < n.length; ) {
    const l = n[r].trim();
    if (l.startsWith("<!--@meta")) {
      const a = Ge(l);
      if (a.background) {
        const c = Fl[a.background] ?? a.background;
        o.background = c;
      }
      if (a.originView) {
        const c = a.originView.split(",").map(Number);
        c.length === 3 && c.every((h) => !isNaN(h)) && (o.originView = { x: c[0], y: c[1], zoom: c[2] });
      }
      r++;
      continue;
    }
    if (l.startsWith("<!--@frame")) {
      const a = Ge(l);
      for (r++; r < n.length && n[r].trim() === ""; ) r++;
      e.push({
        id: a.id || bt(10),
        type: "frame",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "400"),
        h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          label: ((s = a.label) == null ? void 0 : s.replace(/&quot;/g, '"')) || void 0,
          backgroundColor: a.backgroundColor || void 0,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0,
          slideOrder: a.slideOrder ? parseInt(a.slideOrder, 10) : void 0,
          transition: a.transition || void 0,
          transitionDuration: a.transitionDuration ? parseInt(a.transitionDuration, 10) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@block")) {
      const a = Ge(l);
      r++;
      const c = [];
      for (; r < n.length && !n[r].trim().startsWith("<!--@"); )
        c.push(n[r]), r++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      const h = c.join(`
`), p = h.trim().length > 0 ? await Gr(h) : [];
      e.push({
        id: a.id || bt(10),
        type: "content",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "300"),
        h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
        z: parseInt(a.z || "1"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          blocks: p,
          markdown: h,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@draw")) {
      const a = Ge(l);
      if (r++, a.tool === "shape")
        for (e.push({
          id: a.id || bt(10),
          type: "shape",
          x: parseFloat(a.x || "0"),
          y: parseFloat(a.y || "0"),
          w: parseFloat(a.w || "100"),
          h: a.h === "auto" || !a.h ? "auto" : parseFloat(a.h),
          z: parseInt(a.z || "0"),
          rotation: a.rotation ? parseFloat(a.rotation) : void 0,
          locked: a.locked === "true" || void 0,
          groupId: a.group || void 0,
          data: {
            shape: a.shape || "rect",
            stroke: a.color || "#1e1e2e",
            fill: a.fill || void 0,
            fillStyle: a.fillStyle || void 0,
            strokeWidth: parseFloat(a.stroke || "2"),
            strokeStyle: a.strokeStyle || void 0,
            edgeStyle: a.edgeStyle || void 0,
            roughness: parseFloat(a.roughness || "1"),
            opacity: a.opacity ? parseFloat(a.opacity) : void 0,
            startPoint: a.startPt ? a.startPt.split(",").map(Number) : void 0,
            endPoint: a.endPt ? a.endPt.split(",").map(Number) : void 0,
            label: ((i = a.label) == null ? void 0 : i.replace(/&quot;/g, '"')) || void 0,
            labelFontSize: a.labelFontSize ? parseFloat(a.labelFontSize) : void 0,
            labelFontFamily: a.labelFontFamily || void 0,
            labelAlign: a.labelAlign || void 0
          }
        }); r < n.length && n[r].trim() === ""; ) r++;
      else {
        let c = "";
        r < n.length && !n[r].trim().startsWith("<!--@") && (c = n[r].trim(), r++);
        const h = c ? c.split(" ").filter(Boolean).map((b) => {
          const g = b.split(",").map(Number);
          return [
            g[0] || 0,
            g[1] || 0,
            g[2] || 0.5
          ];
        }) : [];
        let p = 1 / 0, d = 1 / 0, f = -1 / 0, m = -1 / 0;
        for (const [b, g] of h)
          b < p && (p = b), g < d && (d = g), b > f && (f = b), g > m && (m = g);
        isFinite(p) || (p = parseFloat(a.x || "0"), d = parseFloat(a.y || "0"), f = p, m = d);
        const y = h.map(
          ([b, g, x]) => [b - p, g - d, x]
        );
        for (e.push({
          id: a.id || bt(10),
          type: "draw",
          x: p,
          y: d,
          w: f - p,
          h: m - d,
          z: parseInt(a.z || "0"),
          rotation: a.rotation ? parseFloat(a.rotation) : void 0,
          locked: a.locked === "true" || void 0,
          groupId: a.group || void 0,
          data: {
            tool: a.tool || "pen",
            points: y,
            color: a.color || "#1e1e2e",
            strokeWidth: parseFloat(a.width || "2"),
            opacity: a.opacity ? parseFloat(a.opacity) : void 0,
            fill: a.fill || void 0,
            fillStyle: a.fillStyle || void 0
          }
        }); r < n.length && n[r].trim() === ""; ) r++;
      }
      continue;
    }
    if (l.startsWith("<!--@image")) {
      const a = Ge(l);
      r++, e.push({
        id: a.id || bt(10),
        type: "image",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: parseFloat(a.h || "150"),
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          src: a.src || "",
          alt: a.alt,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0,
          borderColor: a.borderColor || void 0,
          borderWidth: a.borderWidth ? parseFloat(a.borderWidth) : void 0,
          borderStyle: a.borderStyle || void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@edge")) {
      const a = Ge(l);
      for (r++, e.push({
        id: a.id || bt(10),
        type: "edge",
        x: 0,
        y: 0,
        w: 0,
        h: 0,
        z: 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          fromId: a.from || "",
          toId: a.to || "",
          label: a.label,
          style: a.style || "solid",
          color: a.color || "#666",
          strokeWidth: a.strokeWidth ? parseFloat(a.strokeWidth) : 1,
          arrowHead: a.arrowHead || void 0,
          arrowTail: a.arrowTail || void 0,
          arrowHeadSize: a.arrowHeadSize ? parseFloat(a.arrowHeadSize) : void 0,
          arrowTailSize: a.arrowTailSize ? parseFloat(a.arrowTailSize) : void 0,
          edgeType: a.edgeType || void 0,
          animated: a.animated === "true" || void 0,
          animatedDirection: a.animatedDirection || void 0,
          sourceHandle: a.sourceHandle || void 0,
          targetHandle: a.targetHandle || void 0,
          midpointOffset: a.midpointOffset ? parseFloat(a.midpointOffset) : void 0,
          curveOffset: a.curveOffset ? a.curveOffset.split(",").map(Number) : void 0
        }
      }); r < n.length && n[r].trim() === ""; ) r++;
      continue;
    }
    if (l.startsWith("<!--@text")) {
      const a = Ge(l);
      r++;
      const c = [];
      for (; r < n.length && !n[r].trim().startsWith("<!--@"); )
        c.push(n[r]), r++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      e.push({
        id: a.id || bt(10),
        type: "text",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: "auto",
        z: parseInt(a.z || "0"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          text: c.join(`
`),
          fontSize: parseFloat(a.fontSize || "20"),
          fontFamily: a.fontFamily || Qe,
          color: a.color || "#1e1e2e",
          align: a.align || "left",
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    if (l.startsWith("<!--@sticky")) {
      const a = Ge(l);
      r++;
      const c = [];
      for (; r < n.length && !n[r].trim().startsWith("<!--@"); )
        c.push(n[r]), r++;
      for (; c.length > 0 && c[c.length - 1].trim() === ""; )
        c.pop();
      e.push({
        id: a.id || bt(10),
        type: "sticky",
        x: parseFloat(a.x || "0"),
        y: parseFloat(a.y || "0"),
        w: parseFloat(a.w || "200"),
        h: parseFloat(a.h || "150"),
        z: parseInt(a.z || "1"),
        rotation: a.rotation ? parseFloat(a.rotation) : void 0,
        locked: a.locked === "true" || void 0,
        groupId: a.group || void 0,
        data: {
          text: c.join(`
`),
          color: a.color || "#FEF3C7",
          fontSize: a.fontSize ? parseFloat(a.fontSize) : void 0,
          opacity: a.opacity ? parseFloat(a.opacity) : void 0
        }
      });
      continue;
    }
    r++;
  }
  return { nodes: e, meta: o };
}
function yo(t, e) {
  return t.h !== "auto" ? t.h : (e == null ? void 0 : e[t.id]) ?? 100;
}
const Nl = 14;
function rn(t, e, o, n, r) {
  const s = e.find((d) => d.id === o);
  if (!s) return null;
  const i = yo(t, r), l = Nl / n, a = e.filter((d) => d.direction === s.direction), c = a.indexOf(s);
  if (c < 0) return null;
  const h = t.y + i / (a.length + 1) * (c + 1), p = s.direction === "input" ? t.x - l : t.x + t.w + l;
  if (t.rotation) {
    const d = t.x + t.w / 2, f = t.y + i / 2, m = t.rotation * Math.PI / 180, y = Math.cos(m), b = Math.sin(m), g = p - d, x = h - f;
    return { x: d + g * y - x * b, y: f + g * b + x * y };
  }
  return { x: p, y: h };
}
function Ts(t, e, o, n, r, s, i, l) {
  const a = i - r, c = l - s;
  if (a === 0 && c === 0) return { x: r, y: s, side: "right" };
  let h = 1 / 0, p = r, d = s, f = "right";
  if (a !== 0) {
    const m = (t + o - r) / a;
    if (m > 0 && m < h) {
      const y = s + m * c;
      y >= e && y <= e + n && (h = m, p = t + o, d = y, f = "right");
    }
  }
  if (a !== 0) {
    const m = (t - r) / a;
    if (m > 0 && m < h) {
      const y = s + m * c;
      y >= e && y <= e + n && (h = m, p = t, d = y, f = "left");
    }
  }
  if (c !== 0) {
    const m = (e + n - s) / c;
    if (m > 0 && m < h) {
      const y = r + m * a;
      y >= t && y <= t + o && (h = m, p = y, d = e + n, f = "bottom");
    }
  }
  if (c !== 0) {
    const m = (e - s) / c;
    if (m > 0 && m < h) {
      const y = r + m * a;
      y >= t && y <= t + o && (h = m, p = y, d = e, f = "top");
    }
  }
  return { x: p, y: d, side: f };
}
function Te(t, e, o, n, r) {
  const s = Math.cos(r), i = Math.sin(r), l = t - o, a = e - n;
  return [o + l * s - a * i, n + l * i + a * s];
}
function Sr(t, e, o, n) {
  const r = t.x + t.w / 2, s = t.y + e / 2;
  if (!t.rotation)
    return Ts(t.x, t.y, t.w, e, r, s, o, n);
  const i = -t.rotation * Math.PI / 180, [l, a] = Te(o, n, r, s, i), c = Ts(t.x, t.y, t.w, e, r, s, l, a), [h, p] = Te(c.x, c.y, r, s, -i);
  return { x: h, y: p, side: c.side };
}
function jr(t, e, o, n) {
  return Math.abs(t) / o >= Math.abs(e) / n ? t >= 0 ? "right" : "left" : e >= 0 ? "bottom" : "top";
}
function Hl(t, e, o, n) {
  const r = t.x + t.w / 2, s = t.y + e / 2, i = t.w / 2, l = e / 2, a = t.rotation ? -t.rotation * Math.PI / 180 : 0, [c, h] = t.rotation ? Te(o, n, r, s, a) : [o, n], p = c - r, d = h - s;
  if (p === 0 && d === 0)
    return { x: r + i, y: s, side: "right" };
  const f = 1 / Math.sqrt((p / i) ** 2 + (d / l) ** 2);
  let m = r + p * f, y = s + d * f;
  const b = jr(p, d, i, l);
  return t.rotation && ([m, y] = Te(m, y, r, s, -a)), { x: m, y, side: b };
}
function Ol(t, e, o, n) {
  const r = t.x + t.w / 2, s = t.y + e / 2, i = t.w / 2, l = e / 2, a = t.rotation ? -t.rotation * Math.PI / 180 : 0, [c, h] = t.rotation ? Te(o, n, r, s, a) : [o, n], p = c - r, d = h - s;
  if (p === 0 && d === 0)
    return { x: r + i, y: s, side: "right" };
  const f = 1 / (Math.abs(p) / i + Math.abs(d) / l);
  let m = r + p * f, y = s + d * f;
  const b = jr(p, d, i, l);
  return t.rotation && ([m, y] = Te(m, y, r, s, -a)), { x: m, y, side: b };
}
function Xl(t, e, o, n) {
  const r = t.data.points;
  if (!r || r.length === 0)
    return Sr(t, e, o, n);
  const s = t.x + t.w / 2, i = t.y + e / 2, l = t.rotation ? -t.rotation * Math.PI / 180 : 0, [a, c] = t.rotation ? Te(o, n, s, i, l) : [o, n], h = a - s, p = c - i, d = Math.hypot(h, p);
  if (d === 0)
    return Sr(t, e, o, n);
  const f = h / d, m = p / d;
  let y = t.x + r[0][0], b = t.y + r[0][1], g = (y - s) * f + (b - i) * m;
  for (let W = 1; W < r.length; W++) {
    const I = t.x + r[W][0], R = t.y + r[W][1], K = (I - s) * f + (R - i) * m;
    K > g && (g = K, y = I, b = R);
  }
  const x = Math.max(0.5, (t.data.strokeWidth ?? 1) / 2);
  let S = y + f * x, M = b + m * x;
  const z = jr(h, p, t.w / 2, e / 2);
  return t.rotation && ([S, M] = Te(S, M, s, i, -l)), { x: S, y: M, side: z };
}
function Ps(t, e, o) {
  const n = t.data.points;
  if (!n || n.length === 0)
    return Pn(t, e, o);
  const r = t.x + t.w / 2, s = t.y + e / 2, i = mo(o);
  let l = t.x + n[0][0], a = t.y + n[0][1], c = (l - r) * i.dx + (a - s) * i.dy;
  for (let f = 1; f < n.length; f++) {
    const m = t.x + n[f][0], y = t.y + n[f][1], b = (m - r) * i.dx + (y - s) * i.dy;
    b > c && (c = b, l = m, a = y);
  }
  const h = Math.max(0.5, (t.data.strokeWidth ?? 1) / 2);
  let p = l + i.dx * h, d = a + i.dy * h;
  if (t.rotation) {
    const f = t.rotation * Math.PI / 180;
    [p, d] = Te(p, d, r, s, f);
  }
  return { x: p, y: d };
}
function Mr(t, e, o, n) {
  var r;
  if (t.type === "draw")
    return Xl(t, e, o, n);
  if (t.type === "shape") {
    const s = (r = t.data) == null ? void 0 : r.shape;
    if (s === "ellipse") return Hl(t, e, o, n);
    if (s === "diamond") return Ol(t, e, o, n);
  }
  return Sr(t, e, o, n);
}
function Cr(t, e, o, n) {
  const r = Mr(t, e, o, n);
  return { x: r.x, y: r.y };
}
function Pn(t, e, o) {
  const n = t.x + t.w / 2, r = t.y + e / 2;
  let s, i;
  switch (o) {
    case "top":
      s = n, i = t.y;
      break;
    case "bottom":
      s = n, i = t.y + e;
      break;
    case "left":
      s = t.x, i = r;
      break;
    case "right":
      s = t.x + t.w, i = r;
      break;
  }
  if (!t.rotation) return { x: s, y: i };
  const l = t.rotation * Math.PI / 180, [a, c] = Te(s, i, n, r, l);
  return { x: a, y: c };
}
function mo(t) {
  switch (t) {
    case "top":
      return { dx: 0, dy: -1 };
    case "bottom":
      return { dx: 0, dy: 1 };
    case "left":
      return { dx: -1, dy: 0 };
    case "right":
      return { dx: 1, dy: 0 };
  }
}
function Rs(t) {
  var o;
  if (t.type !== "shape") return !1;
  const e = (o = t.data) == null ? void 0 : o.shape;
  return e === "ellipse" || e === "diamond";
}
function Ze(t, e, o = "bezier", n, r, s, i, l, a, c) {
  const h = yo(t, n), p = yo(e, n), d = t.x + t.w / 2, f = t.y + h / 2, m = e.x + e.w / 2, y = e.y + p / 2;
  let b, g, x, S;
  if (a)
    b = a.x, g = a.y, x = r ?? "right";
  else if (r) {
    const R = t.type === "draw" ? Ps(t, h, r) : Pn(t, h, r);
    b = R.x, g = R.y, x = r;
  } else {
    const R = Mr(t, h, m, y);
    if (b = R.x, g = R.y, x = R.side, Rs(t)) {
      const K = Math.hypot(m - d, y - f);
      K > 0 && (S = { dx: (m - d) / K, dy: (y - f) / K });
    }
  }
  let M, z, W, I;
  if (c)
    M = c.x, z = c.y, W = s ?? "left";
  else if (s) {
    const R = e.type === "draw" ? Ps(e, p, s) : Pn(e, p, s);
    M = R.x, z = R.y, W = s;
  } else {
    const R = Mr(e, p, d, f);
    if (M = R.x, z = R.y, W = R.side, Rs(e)) {
      const K = Math.hypot(d - m, f - y);
      K > 0 && (I = { dx: (d - m) / K, dy: (f - y) / K });
    }
  }
  switch (o) {
    case "straight":
      return Yl(b, g, M, z, x, W);
    case "bezier":
      return Gl(b, g, M, z, x, W, l, S, I);
    case "smoothstep":
      return jl(b, g, M, z, x, W, i);
    case "step":
      return Vl(b, g, M, z, x, W, i);
  }
}
function Yl(t, e, o, n, r, s) {
  const i = Math.min(t, o), l = Math.min(e, n), a = Math.abs(o - t), c = Math.abs(n - e);
  return {
    path: `M${t},${e} L${o},${n}`,
    labelX: (t + o) / 2,
    labelY: (e + n) / 2,
    x1: t,
    y1: e,
    x2: o,
    y2: n,
    arrowAngle: Math.atan2(n - e, o - t),
    tailAngle: Math.atan2(e - n, t - o),
    sourceSide: r,
    targetSide: s,
    bounds: { x: i, y: l, w: a, h: c }
  };
}
function Gl(t, e, o, n, r, s, i, l, a) {
  const c = Math.hypot(o - t, n - e), h = Math.min(c * 0.5, Math.max(50, c * 0.25)), p = l ?? mo(r), d = a ?? mo(s), f = i ? i[0] * (4 / 3) : 0, m = i ? i[1] * (4 / 3) : 0, y = t + p.dx * h + f, b = e + p.dy * h + m, g = o + d.dx * h + f, x = n + d.dy * h + m, S = 0.125 * t + 0.375 * y + 0.375 * g + 0.125 * o, M = 0.125 * e + 0.375 * b + 0.375 * x + 0.125 * n, z = Math.atan2(n - x, o - g), W = Math.atan2(e - b, t - y), I = {
    x: S,
    y: M,
    axis: "xy",
    min: 0,
    max: 0
  }, R = Math.min(t, o, y, g), K = Math.min(e, n, b, x), Z = Math.max(t, o, y, g), lt = Math.max(e, n, b, x);
  return {
    path: `M${t},${e} C${y},${b} ${g},${x} ${o},${n}`,
    labelX: S,
    labelY: M,
    x1: t,
    y1: e,
    x2: o,
    y2: n,
    arrowAngle: z,
    tailAngle: W,
    sourceSide: r,
    targetSide: s,
    kinkHandle: I,
    bounds: { x: R, y: K, w: Z - R, h: lt - K }
  };
}
function jl(t, e, o, n, r, s, i) {
  const { points: c, kinkHandle: h } = Vr(t, e, o, n, r, s, 20, i), p = Kl(c, 8), d = Math.floor(c.length / 2), f = (c[d - 1][0] + c[d][0]) / 2, m = (c[d - 1][1] + c[d][1]) / 2, y = c[c.length - 1], b = c[c.length - 2], g = Math.atan2(y[1] - b[1], y[0] - b[0]), x = c[0], S = c[1], M = Math.atan2(x[1] - S[1], x[0] - S[0]);
  let z = 1 / 0, W = 1 / 0, I = -1 / 0, R = -1 / 0;
  for (const [K, Z] of c)
    K < z && (z = K), Z < W && (W = Z), K > I && (I = K), Z > R && (R = Z);
  return {
    path: p,
    labelX: f,
    labelY: m,
    x1: t,
    y1: e,
    x2: o,
    y2: n,
    arrowAngle: g,
    tailAngle: M,
    sourceSide: r,
    targetSide: s,
    kinkHandle: h,
    bounds: { x: z, y: W, w: I - z, h: R - W }
  };
}
function Vl(t, e, o, n, r, s, i) {
  const { points: a, kinkHandle: c } = Vr(t, e, o, n, r, s, 20, i), h = [`M${a[0][0]},${a[0][1]}`];
  for (let R = 1; R < a.length; R++)
    h.push(`L${a[R][0]},${a[R][1]}`);
  const p = Math.floor(a.length / 2), d = (a[p - 1][0] + a[p][0]) / 2, f = (a[p - 1][1] + a[p][1]) / 2, m = a[a.length - 1], y = a[a.length - 2], b = Math.atan2(m[1] - y[1], m[0] - y[0]), g = a[0], x = a[1], S = Math.atan2(g[1] - x[1], g[0] - x[0]);
  let M = 1 / 0, z = 1 / 0, W = -1 / 0, I = -1 / 0;
  for (const [R, K] of a)
    R < M && (M = R), K < z && (z = K), R > W && (W = R), K > I && (I = K);
  return {
    path: h.join(" "),
    labelX: d,
    labelY: f,
    x1: t,
    y1: e,
    x2: o,
    y2: n,
    arrowAngle: b,
    tailAngle: S,
    sourceSide: r,
    targetSide: s,
    kinkHandle: c,
    bounds: { x: M, y: z, w: W - M, h: I - z }
  };
}
function Vr(t, e, o, n, r, s, i, l) {
  const a = mo(r), c = mo(s), h = t + a.dx * i, p = e + a.dy * i, d = o + c.dx * i, f = n + c.dy * i, m = r === "left" || r === "right", y = s === "left" || s === "right", b = [[t, e], [h, p]], g = l ?? 0.5;
  let x;
  if (m && y) {
    const S = h + (d - h) * g;
    b.push([S, p], [S, f]);
    const M = Math.min(h, d), z = Math.max(h, d);
    x = { x: S, y: (p + f) / 2, axis: "x", min: M, max: z };
  } else if (!m && !y) {
    const S = p + (f - p) * g;
    b.push([h, S], [d, S]);
    const M = Math.min(p, f), z = Math.max(p, f);
    x = { x: (h + d) / 2, y: S, axis: "y", min: M, max: z };
  } else m && !y ? b.push([d, p]) : b.push([h, f]);
  return b.push([d, f], [o, n]), { points: b, kinkHandle: x };
}
function Kl(t, e) {
  if (t.length < 2) return "";
  if (t.length === 2) return `M${t[0][0]},${t[0][1]} L${t[1][0]},${t[1][1]}`;
  const o = [`M${t[0][0]},${t[0][1]}`];
  for (let r = 1; r < t.length - 1; r++) {
    const s = t[r - 1], i = t[r], l = t[r + 1], a = i[0] - s[0], c = i[1] - s[1], h = l[0] - i[0], p = l[1] - i[1], d = Math.hypot(a, c), f = Math.hypot(h, p);
    if (d === 0 || f === 0) {
      o.push(`L${i[0]},${i[1]}`);
      continue;
    }
    const m = Math.min(e, d / 2, f / 2), y = i[0] - a / d * m, b = i[1] - c / d * m, g = i[0] + h / f * m, x = i[1] + p / f * m;
    o.push(`L${y},${b}`), o.push(`Q${i[0]},${i[1]} ${g},${x}`);
  }
  const n = t[t.length - 1];
  return o.push(`L${n[0]},${n[1]}`), o.join(" ");
}
function ql(t, e, o, n, r, s, i, l, a) {
  const c = 1 - a, h = c * c, p = h * c, d = a * a, f = d * a;
  return [
    p * t + 3 * h * a * o + 3 * c * d * r + f * i,
    p * e + 3 * h * a * n + 3 * c * d * s + f * l
  ];
}
function Zl(t, e, o, n, r, s, i, l, a, c, h = 24) {
  let p = 1 / 0, d = o, f = n;
  for (let m = 1; m <= h; m++) {
    const y = m / h, [b, g] = ql(o, n, r, s, i, l, a, c, y), x = Kr(t, e, d, f, b, g);
    x < p && (p = x), d = b, f = g;
  }
  return p;
}
function Ul(t, e, o) {
  let n = 1 / 0;
  for (let r = 1; r < o.length; r++) {
    const s = Kr(t, e, o[r - 1][0], o[r - 1][1], o[r][0], o[r][1]);
    s < n && (n = s);
  }
  return n;
}
function Fi(t, e, o, n, r, s, i, l) {
  const a = r.data.edgeType || "bezier", c = Ze(
    o,
    n,
    a,
    s,
    r.data.sourceHandle,
    r.data.targetHandle,
    r.data.midpointOffset,
    r.data.curveOffset,
    i,
    l
  ), { x1: h, y1: p, x2: d, y2: f } = c;
  if (a === "straight")
    return Kr(t, e, h, p, d, f);
  if (a === "bezier") {
    const b = Math.hypot(d - h, f - p), g = Math.min(b * 0.5, Math.max(50, b * 0.25)), x = mo(c.sourceSide), S = mo(c.targetSide), M = r.data.curveOffset ? r.data.curveOffset[0] * (4 / 3) : 0, z = r.data.curveOffset ? r.data.curveOffset[1] * (4 / 3) : 0, W = h + x.dx * g + M, I = p + x.dy * g + z, R = d + S.dx * g + M, K = f + S.dy * g + z;
    return Zl(t, e, h, p, W, I, R, K, d, f);
  }
  const m = 20, { points: y } = Vr(h, p, d, f, c.sourceSide, c.targetSide, m, r.data.midpointOffset);
  return Ul(t, e, y);
}
function er(t, e, o) {
  const n = yo(t, o), r = yo(e, o), s = t.x + t.w / 2, i = t.y + n / 2, l = e.x + e.w / 2, a = e.y + r / 2, c = Cr(t, n, l, a), h = Cr(e, r, s, i);
  return { x1: c.x, y1: c.y, x2: h.x, y2: h.y };
}
function or(t, e, o, n) {
  const r = yo(t, n);
  return Cr(t, r, e, o);
}
function Kr(t, e, o, n, r, s) {
  const i = r - o, l = s - n, a = i * i + l * l;
  if (a === 0) return Math.hypot(t - o, e - n);
  const c = Math.max(0, Math.min(1, ((t - o) * i + (e - n) * l) / a)), h = o + c * i, p = n + c * l;
  return Math.hypot(t - h, e - p);
}
function Ql(t, e, o, n, r) {
  const s = (l, a) => l >= r.x && l <= r.x + r.w && a >= r.y && a <= r.y + r.h;
  if (s(t, e) || s(o, n)) return !0;
  const i = [
    [r.x, r.y, r.x + r.w, r.y],
    // top
    [r.x, r.y + r.h, r.x + r.w, r.y + r.h],
    // bottom
    [r.x, r.y, r.x, r.y + r.h],
    // left
    [r.x + r.w, r.y, r.x + r.w, r.y + r.h]
    // right
  ];
  for (const [l, a, c, h] of i)
    if (Jl(t, e, o, n, l, a, c, h)) return !0;
  return !1;
}
function Jl(t, e, o, n, r, s, i, l) {
  const a = o - t, c = n - e, h = i - r, p = l - s, d = a * p - c * h;
  if (Math.abs(d) < 1e-10) return !1;
  const f = r - t, m = s - e, y = (f * p - m * h) / d, b = (f * c - m * a) / d;
  return y >= 0 && y <= 1 && b >= 0 && b <= 1;
}
function Bo(t, e, o, n) {
  const r = Math.cos(o), s = Math.sin(o), i = -s, l = r, a = n / 2, c = t + r * a, h = e + s * a, p = t - r * a, d = e - s * a, f = n * 0.4;
  return `M${p + i * f},${d + l * f} L${c},${h} L${p - i * f},${d - l * f}`;
}
function Rn(t, e, o, n) {
  const r = Math.cos(o), s = Math.sin(o), i = -s, l = r, a = n / 2, c = t + r * a, h = e + s * a, p = t - r * a, d = e - s * a, f = n * 0.4;
  return `M${c},${h} L${p + i * f},${d + l * f} L${p - i * f},${d - l * f} Z`;
}
function Ho(t, e) {
  const o = yo(t, e);
  return ["top", "right", "bottom", "left"].map((r) => {
    const s = Pn(t, o, r);
    return { side: r, x: s.x, y: s.y };
  });
}
function bn(t, e, o, n) {
  const r = Ho(t, n);
  let s = r[0], i = 1 / 0;
  for (const l of r) {
    const a = Math.hypot(l.x - e, l.y - o);
    a < i && (i = a, s = l);
  }
  return s.side;
}
function $l(t, e, o, n, r, s) {
  const i = 8 / n, l = [];
  for (const a of t.values()) {
    if (a.type !== "edge") continue;
    const c = a, h = t.get(c.data.fromId), p = t.get(c.data.toId);
    if (!h || !p) continue;
    const d = s == null ? void 0 : s(c, h, p);
    Fi(e, o, h, p, c, r, d == null ? void 0 : d.sourcePortPos, d == null ? void 0 : d.targetPortPos) < i && l.push(a);
  }
  return l;
}
function _l(t, e, o, n, r, s) {
  const i = 8 / n;
  let l = null, a = i;
  for (const c of t.values()) {
    if (c.type !== "edge") continue;
    const h = c, p = t.get(h.data.fromId), d = t.get(h.data.toId);
    if (!p || !d) continue;
    const f = s == null ? void 0 : s(h, p, d), m = Fi(e, o, p, d, h, r, f == null ? void 0 : f.sourcePortPos, f == null ? void 0 : f.targetPortPos);
    m < a && (a = m, l = c);
  }
  return l;
}
function tc(t, e, o) {
  const n = t.x, r = t.x + t.w / 2, s = t.x + t.w, i = t.y, l = t.y + t.h / 2, a = t.y + t.h, c = [n, r, s], h = [i, l, a];
  let p = 1 / 0, d = 1 / 0;
  const f = [];
  for (const y of e) {
    const b = y.x, g = y.x + y.w / 2, x = y.x + y.w, S = y.y, M = y.y + y.h / 2, z = y.y + y.h, W = [b, g, x], I = [S, M, z];
    for (const R of c)
      for (const K of W) {
        const Z = K - R;
        Math.abs(Z) <= o && (Math.abs(Z) < Math.abs(p) && (p = Z), f.push({
          axis: "x",
          position: K,
          start: Math.min(t.y, t.y + t.h, y.y, y.y + y.h),
          end: Math.max(t.y, t.y + t.h, y.y, y.y + y.h)
        }));
      }
    for (const R of h)
      for (const K of I) {
        const Z = K - R;
        Math.abs(Z) <= o && (Math.abs(Z) < Math.abs(d) && (d = Z), f.push({
          axis: "y",
          position: K,
          start: Math.min(t.x, t.x + t.w, y.x, y.x + y.w),
          end: Math.max(t.x, t.x + t.w, y.x, y.x + y.w)
        }));
      }
  }
  const m = /* @__PURE__ */ new Map();
  for (const y of f) {
    const b = `${y.axis}:${y.position.toFixed(1)}`, g = m.get(b);
    g ? (g.start = Math.min(g.start, y.start), g.end = Math.max(g.end, y.end)) : m.set(b, { ...y });
  }
  return {
    guides: Array.from(m.values()),
    snapDx: Math.abs(p) <= o ? p : 0,
    snapDy: Math.abs(d) <= o ? d : 0
  };
}
class ec {
  constructor() {
    Ct(this, "nodes", /* @__PURE__ */ new Map());
    Ct(this, "viewport", { x: 0, y: 0, zoom: 1 });
    Ct(this, "selection", /* @__PURE__ */ new Set());
    Ct(this, "activeGroupId", null);
    Ct(this, "groupRotations", /* @__PURE__ */ new Map());
    /** Maps child groupId → parent groupId for nested groups. */
    Ct(this, "groupParent", /* @__PURE__ */ new Map());
    /** Reverse index: parent groupId → set of child groupIds. Maintained alongside groupParent. */
    Ct(this, "groupChildren", /* @__PURE__ */ new Map());
    Ct(this, "mode", "select");
    Ct(this, "activeTool", {
      tool: "pen",
      color: "#1e1e2e",
      width: 3,
      shapeType: "rect",
      strokeStyle: "solid",
      roughness: 1,
      opacity: 1
    });
    Ct(this, "containerOffset", { x: 0, y: 0 });
    /** DOM element that hosts the canvas — used to derive the correct window in pop-out scenarios. */
    Ct(this, "_container", null);
    Ct(this, "snapToGrid", !1);
    Ct(this, "smartGuides", !0);
    Ct(this, "lassoSelect", !1);
    Ct(this, "presentationMode", !1);
    Ct(this, "presentationSlides", []);
    Ct(this, "presentationIndex", 0);
    Ct(this, "_presentationAnimId", null);
    /** Transition overlay state — consumed by PresentationOverlay for visual effects. */
    Ct(this, "_transitionOverlay", null);
    Ct(this, "gridSize", 20);
    Ct(this, "boardBackground", "dot-grid");
    /** Saved "origin" viewport position restored on next load. */
    Ct(this, "originView", null);
    /** Current alignment guides (set during drag). */
    Ct(this, "alignGuides", []);
    /** Container dimensions for viewport bounds computation. */
    Ct(this, "_containerWidth", 2e3);
    Ct(this, "_containerHeight", 1500);
    Ct(this, "history", new wl());
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    Ct(this, "listeners", {});
    Ct(this, "_suppressEvents", !1);
    Ct(this, "_collabMode", !1);
    Ct(this, "clipboard", []);
    Ct(this, "pasteCount", 0);
    Ct(this, "nextZValue", 1);
    Ct(this, "_minZ", 0);
    Ct(this, "quadTree", new kr({ x: -1e5, y: -1e5, w: 2e5, h: 2e5 }));
    Ct(this, "adjacency", /* @__PURE__ */ new Map());
    /** Explicit frame→children tracking. Only nodes intentionally placed inside a frame are tracked. */
    Ct(this, "frameChildren", /* @__PURE__ */ new Map());
    /** Node types that act as containers (frame-like behavior). "frame" is always included. */
    Ct(this, "_containerTypes", /* @__PURE__ */ new Set(["frame"]));
    Ct(this, "registry");
    /** Measured heights for auto-height nodes (canvas-coordinate units). */
    Ct(this, "_measuredHeights", {});
  }
  get transitionOverlay() {
    return this._transitionOverlay;
  }
  /** Set the node type registry for lifecycle hooks. */
  setRegistry(e) {
    this.registry = e;
  }
  /** Enable collaborative mode. Disables local snapshot history. */
  setCollabMode(e) {
    this._collabMode = e, e && this.history.clear();
  }
  /** Whether the engine is in collaborative mode. */
  get isCollabMode() {
    return this._collabMode;
  }
  /** Register a node type as a container (frame-like behavior). */
  registerContainerType(e) {
    this._containerTypes.add(e);
  }
  /** Check whether a node type behaves as a container. */
  isContainerType(e) {
    return this._containerTypes.has(e);
  }
  /** The set of container type strings (read-only). */
  get containerTypes() {
    return this._containerTypes;
  }
  /** Update the measured height for an auto-height node. */
  setMeasuredHeight(e, o) {
    this._measuredHeights[e] = o;
  }
  /** Get the resolved height for a node (measured or explicit). */
  resolveHeight(e) {
    return e.h !== "auto" ? e.h : this._measuredHeights[e.id] ?? 100;
  }
  /** Get all measured heights (for canvas rendering). */
  get measuredHeights() {
    return this._measuredHeights;
  }
  // --- Edge helpers (data-flow) ---
  /** Get all edge nodes connected to a given node. */
  getEdgesForNode(e) {
    const o = this.adjacency.get(e);
    if (!o) return [];
    const n = [];
    for (const r of o) {
      const s = this.nodes.get(r);
      s && s.type === "edge" && n.push(s);
    }
    return n;
  }
  /** Get all edge nodes in the board. */
  getAllEdges() {
    const e = [];
    for (const o of this.nodes.values())
      o.type === "edge" && e.push(o);
    return e;
  }
  /** Set the container element (used by SpatialCanvas on mount). */
  setContainer(e) {
    this._container = e;
  }
  /** Get the window object for the container (supports pop-out windows). */
  getWindow() {
    var e;
    return ((e = this._container) == null ? void 0 : e.ownerDocument.defaultView) ?? window;
  }
  // --- Event Emitter ---
  on(e, o) {
    this.listeners[e] || (this.listeners[e] = /* @__PURE__ */ new Set()), this.listeners[e].add(o);
  }
  off(e, o) {
    var n;
    (n = this.listeners[e]) == null || n.delete(o);
  }
  emit(e, ...o) {
    var n;
    this._suppressEvents || (n = this.listeners[e]) == null || n.forEach((r) => r(...o));
  }
  /** Request entering image crop mode (handled by the canvas component). */
  requestImageCrop(e) {
    this.emit("image:cropRequest", e);
  }
  // --- Grid Snapping ---
  toggleSnapToGrid() {
    this.snapToGrid = !this.snapToGrid, this.emit("guides");
  }
  toggleSmartGuides() {
    this.smartGuides = !this.smartGuides, this.emit("guides");
  }
  setGridSize(e) {
    const o = Math.max(1, Math.round(e));
    this.gridSize !== o && (this.gridSize = o, this.emit("guides"));
  }
  toggleLassoSelect() {
    this.lassoSelect = !this.lassoSelect, this.emit("lassoToggle");
  }
  // ── Presentation mode ─────────────────────────────────────────
  enterPresentation() {
    const e = [];
    for (const a of this.nodes.values())
      if (a.type === "frame") {
        const c = a.data;
        e.push({ id: a.id, x: a.x, y: a.y, order: c.slideOrder });
      }
    if (e.length === 0) return;
    const o = e.filter((a) => a.order != null).sort((a, c) => a.order - c.order), n = e.filter((a) => a.order == null), r = 100;
    n.sort((a, c) => a.y - c.y);
    const s = [];
    for (const a of n) {
      const c = s[s.length - 1];
      c && Math.abs(a.y - c[0].y) < r ? c.push(a) : s.push([a]);
    }
    const i = s.flatMap((a) => a.sort((c, h) => c.x - h.x)), l = [...o, ...i];
    this.presentationSlides = l.map((a) => a.id), this.presentationIndex = 0, this.presentationMode = !0, this.selection.size > 0 && (this.selection.clear(), this.emit("selection")), this.emit("presentation"), this.presentationGoTo(0);
  }
  exitPresentation() {
    this._presentationAnimId != null && (cancelAnimationFrame(this._presentationAnimId), this._presentationAnimId = null), this._transitionOverlay = null, this.presentationMode = !1, this.presentationSlides = [], this.presentationIndex = 0, this.emit("presentation");
  }
  presentationNext() {
    this.presentationIndex < this.presentationSlides.length - 1 && this.presentationGoTo(this.presentationIndex + 1);
  }
  presentationPrev() {
    this.presentationIndex > 0 && this.presentationGoTo(this.presentationIndex - 1);
  }
  presentationGoTo(e) {
    if (e < 0 || e >= this.presentationSlides.length) return;
    const o = this.presentationSlides[e], n = this.nodes.get(o);
    if (!n) {
      this.exitPresentation();
      return;
    }
    const r = this.presentationIndex;
    this.presentationIndex = e, this.emit("presentation"), this._presentationAnimId != null && (cancelAnimationFrame(this._presentationAnimId), this._presentationAnimId = null), this._transitionOverlay = null;
    const s = this._computeSlideViewport(n), i = n.data, l = i.transition ?? "pan", a = i.transitionDuration, c = e >= r ? 1 : -1;
    switch (l) {
      case "none":
        this._transitionNone(s);
        break;
      case "fade":
        this._transitionFade(s, a);
        break;
      case "dissolve":
        this._transitionDissolve(s, a);
        break;
      case "zoom":
        this._transitionZoom(s, a);
        break;
      case "fold":
        this._transitionFold(s, a);
        break;
      case "cube":
        this._transitionCube(s, a, c);
        break;
      case "pan":
      default:
        this._transitionPan(s, a);
        break;
    }
  }
  _computeSlideViewport(e) {
    const o = this.resolveHeight(e), n = 40, r = e.x - n, s = e.y - n, i = e.w + n * 2, l = o + n * 2, a = this._containerWidth, c = this._containerHeight, h = Fo(Math.min(a / i, c / l), 0.1, 5);
    return {
      x: (a - i * h) / 2 - r * h,
      y: (c - l * h) / 2 - s * h,
      zoom: h
    };
  }
  /** Pan transition: smooth viewport interpolation (default). */
  _transitionPan(e, o) {
    const n = o ?? 400, r = performance.now(), s = { ...this.viewport }, i = (l) => {
      const a = Math.min((l - r) / n, 1), c = 1 - Math.pow(1 - a, 3);
      this.viewport.x = s.x + (e.x - s.x) * c, this.viewport.y = s.y + (e.y - s.y) * c, this.viewport.zoom = s.zoom + (e.zoom - s.zoom) * c, this.emit("viewport"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : this._presentationAnimId = null;
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** None transition: instant viewport snap. */
  _transitionNone(e) {
    this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport");
  }
  /** Fade transition: fade to black, snap viewport, fade from black. */
  _transitionFade(e, o) {
    const n = (o ?? 500) / 2, r = performance.now(), s = (i) => {
      const l = Math.min((i - r) / n, 1);
      if (this._transitionOverlay = { type: "fade", phase: "out", progress: l }, this.emit("presentation"), l < 1)
        this._presentationAnimId = requestAnimationFrame(s);
      else {
        this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport");
        const a = performance.now(), c = (h) => {
          const p = Math.min((h - a) / n, 1);
          this._transitionOverlay = { type: "fade", phase: "in", progress: p }, this.emit("presentation"), p < 1 ? this._presentationAnimId = requestAnimationFrame(c) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
        };
        this._presentationAnimId = requestAnimationFrame(c);
      }
    };
    this._presentationAnimId = requestAnimationFrame(s);
  }
  /** Dissolve transition: quick overlay fade, snap viewport at midpoint. */
  _transitionDissolve(e, o) {
    const n = o ?? 400, r = performance.now();
    let s = !1;
    const i = (l) => {
      const a = Math.min((l - r) / n, 1);
      a < 0.5 ? this._transitionOverlay = { type: "dissolve", phase: "out", progress: a * 2 } : (s || (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), s = !0), this._transitionOverlay = { type: "dissolve", phase: "in", progress: (a - 0.5) * 2 }), this.emit("presentation"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** Zoom transition: zoom out from current, zoom into target. */
  _transitionZoom(e, o) {
    const n = o ?? 600, r = performance.now(), s = { ...this.viewport }, i = Math.max(0.1, Math.min(s.zoom, e.zoom) * 0.35), l = (s.x + e.x) / 2, a = (s.y + e.y) / 2, c = (h) => {
      const p = Math.min((h - r) / n, 1);
      if (p < 0.5) {
        const d = p * 2, f = 1 - Math.pow(1 - d, 3);
        this.viewport.x = s.x + (l - s.x) * f, this.viewport.y = s.y + (a - s.y) * f, this.viewport.zoom = s.zoom + (i - s.zoom) * f;
      } else {
        const d = (p - 0.5) * 2, f = 1 - Math.pow(1 - d, 3);
        this.viewport.x = l + (e.x - l) * f, this.viewport.y = a + (e.y - a) * f, this.viewport.zoom = i + (e.zoom - i) * f;
      }
      this.emit("viewport"), p < 1 ? this._presentationAnimId = requestAnimationFrame(c) : this._presentationAnimId = null;
    };
    this._presentationAnimId = requestAnimationFrame(c);
  }
  /** Fold transition: two halves fold shut like a book, snap viewport, unfold to reveal. */
  _transitionFold(e, o) {
    const n = o ?? 700, r = performance.now();
    let s = !1;
    const i = (l) => {
      const a = Math.min((l - r) / n, 1);
      a < 0.5 ? this._transitionOverlay = { type: "fold", phase: "out", progress: a * 2 } : (s || (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), s = !0), this._transitionOverlay = { type: "fold", phase: "in", progress: (a - 0.5) * 2 }), this.emit("presentation"), a < 1 ? this._presentationAnimId = requestAnimationFrame(i) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(i);
  }
  /** Cube transition: zoom out → 3D rotate → zoom in, snap viewport at midpoint. */
  _transitionCube(e, o, n = 1) {
    const r = o ?? 1200, s = performance.now();
    let i = !1;
    const l = (a) => {
      const c = Math.min((a - s) / r, 1);
      c >= 0.5 && !i && (this.viewport.x = e.x, this.viewport.y = e.y, this.viewport.zoom = e.zoom, this.emit("viewport"), i = !0), this._transitionOverlay = {
        type: "cube",
        phase: c < 0.5 ? "out" : "in",
        progress: c < 0.5 ? c * 2 : (c - 0.5) * 2,
        direction: n,
        t: c
      }, this.emit("presentation"), c < 1 ? this._presentationAnimId = requestAnimationFrame(l) : (this._transitionOverlay = null, this._presentationAnimId = null, this.emit("presentation"));
    };
    this._presentationAnimId = requestAnimationFrame(l);
  }
  snap(e, o) {
    return this.snapToGrid ? {
      x: Math.round(e / this.gridSize) * this.gridSize,
      y: Math.round(o / this.gridSize) * this.gridSize
    } : { x: e, y: o };
  }
  /** Update the container dimensions (called from canvas resize observer). */
  setContainerSize(e, o) {
    const n = this._containerWidth, r = this._containerHeight;
    this._containerWidth = e, this._containerHeight = o, this.presentationMode && this.presentationSlides.length > 0 ? this.presentationGoTo(this.presentationIndex) : n > 0 && r > 0 && (this.viewport.x += (e - n) / 2, this.viewport.y += (o - r) / 2, this.emit("viewport"));
  }
  /**
   * Compute smart guide alignment + grid snap for a drag operation.
   * Sets `this.alignGuides` and emits `guides` event.
   * Returns the adjusted delta to apply.
   */
  computeDragSnap(e, o, n, r, s) {
    const i = this.snapToGrid && !s, l = this.smartGuides && !s;
    let a = n, c = r, h = [];
    const p = o instanceof Set ? o : new Set(o);
    if (l) {
      let d = 1 / 0, f = 1 / 0, m = -1 / 0, y = -1 / 0;
      for (const R of e) {
        const K = this.getNode(R.id);
        if (!K) continue;
        const Z = R.x + n, lt = R.y + r, O = this.resolveHeight(K);
        d = Math.min(d, Z), f = Math.min(f, lt), m = Math.max(m, Z + K.w), y = Math.max(y, lt + O);
      }
      const b = { x: d, y: f, w: m - d, h: y - f }, g = -this.viewport.x / this.viewport.zoom, x = -this.viewport.y / this.viewport.zoom, S = this._containerWidth / this.viewport.zoom, M = this._containerHeight / this.viewport.zoom, z = [], W = this.quadTree.retrieve([], { x: g, y: x, w: S, h: M });
      for (const R of W) {
        if (R.type === "edge" || p.has(R.id)) continue;
        const K = this.resolveHeight(R);
        z.push({ x: R.x, y: R.y, w: R.w, h: K });
      }
      const I = tc(b, z, 5);
      if (h = I.guides, i) {
        const R = e[0].x + n, K = e[0].y + r, Z = this.snap(R, K), lt = Z.x - R, O = Z.y - K, ot = I.snapDx !== 0 && Math.abs(I.snapDx) <= Math.abs(lt), Q = I.snapDy !== 0 && Math.abs(I.snapDy) <= Math.abs(O);
        a = n + (ot ? I.snapDx : lt), c = r + (Q ? I.snapDy : O), ot || (h = h.filter(($) => $.axis !== "x")), Q || (h = h.filter(($) => $.axis !== "y"));
      } else
        a = n + I.snapDx, c = r + I.snapDy;
    } else if (i) {
      const d = this.snap(e[0].x + n, e[0].y + r);
      a = d.x - e[0].x, c = d.y - e[0].y;
    }
    return this.alignGuides = h, this.emit("guides"), { finalDx: a, finalDy: c };
  }
  /** Clear alignment guides (call on drag end). */
  clearAlignGuides() {
    this.alignGuides.length !== 0 && (this.alignGuides = [], this.emit("guides"));
  }
  // --- Board Background ---
  setBoardBackground(e) {
    this.boardBackground !== e && (this.boardBackground = e, this.emit("background"));
  }
  // --- Viewport ---
  pan(e, o) {
    this.viewport.x += e, this.viewport.y += o, this.emit("viewport");
  }
  zoomByWheel(e, o, n) {
    this.viewport = Il(
      this.viewport,
      e,
      o - this.containerOffset.x,
      n - this.containerOffset.y
    ), this.emit("viewport");
  }
  zoomByFactor(e, o, n) {
    this.viewport = Tl(
      this.viewport,
      e,
      o - this.containerOffset.x,
      n - this.containerOffset.y
    ), this.emit("viewport");
  }
  zoomTo(e, o) {
    const n = Fo(e, 0.1, 5);
    if (o) {
      const r = o.x - this.containerOffset.x, s = o.y - this.containerOffset.y, i = No(this.viewport, r, s);
      this.viewport = {
        x: r - i.x * n,
        y: s - i.y * n,
        zoom: n
      };
    } else
      this.viewport.zoom = n;
    this.emit("viewport");
  }
  zoomIn() {
    this.zoomTo(this.viewport.zoom * 1.2);
  }
  zoomOut() {
    this.zoomTo(this.viewport.zoom / 1.2);
  }
  /** Zoom and pan to center a node for editing (e.g. after double-click on placeholder) */
  zoomToNode(e, o = 1) {
    const n = this.nodes.get(e);
    if (!n) return;
    const r = n.h === "auto" ? 100 : n.h, s = n.x + n.w / 2, i = n.y + r / 2, l = this.getWindow(), a = l.innerWidth, c = l.innerHeight, h = Fo(o, 0.2, 5);
    this.viewport = {
      x: a / 2 - s * h,
      y: c / 2 - i * h,
      zoom: h
    }, this.emit("viewport");
  }
  fitToContent() {
    if (this.nodes.size === 0) return;
    let e = 1 / 0, o = 1 / 0, n = -1 / 0, r = -1 / 0;
    for (const p of this.nodes.values()) {
      const d = p.h === "auto" ? 100 : p.h;
      p.x < e && (e = p.x), p.y < o && (o = p.y), p.x + p.w > n && (n = p.x + p.w), p.y + d > r && (r = p.y + d);
    }
    const s = 50;
    e -= s, o -= s, n += s, r += s;
    const i = n - e, l = r - o, a = this._containerWidth, c = this._containerHeight, h = Fo(
      Math.min(a / i, c / l),
      0.1,
      5
    );
    this.viewport = {
      x: (a - i * h) / 2 - e * h,
      y: (c - l * h) / 2 - o * h,
      zoom: h
    }, this.emit("viewport");
  }
  /** Save the current viewport as the origin view (restored on next load). */
  setOriginView() {
    this.originView = { ...this.viewport }, this.emit("background");
  }
  /** Clear the saved origin view. */
  clearOriginView() {
    this.originView = null, this.emit("background");
  }
  /** Jump to the saved origin view, or fit-to-content if none is saved. */
  goToOriginView() {
    this.originView ? (this.viewport = { ...this.originView }, this.emit("viewport")) : this.fitToContent();
  }
  screenToCanvas(e, o) {
    return No(
      this.viewport,
      e - this.containerOffset.x,
      o - this.containerOffset.y
    );
  }
  canvasToScreen(e, o) {
    return zl(this.viewport, e, o);
  }
  // --- Node CRUD ---
  addNode(e) {
    var o, n, r;
    if (this.history.pushSnapshot(this.nodes, this.groupParent), this.nodes.set(e.id, e), this.quadTree.insert(e), e.z < this._minZ && (this._minZ = e.z), e.type === "edge") {
      const s = e, { fromId: i, toId: l } = s.data;
      this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.has(l) || this.adjacency.set(l, /* @__PURE__ */ new Set()), this.adjacency.get(i).add(e.id), this.adjacency.get(l).add(e.id);
    }
    e.type !== "edge" && this.updateFrameMembership([e.id]), (r = (n = (o = this.registry) == null ? void 0 : o.get(e.type)) == null ? void 0 : n.onCreate) == null || r.call(n, e, this), this.emit("node:create", e), this.emit("change"), this.emit("history");
  }
  addNodes(e) {
    if (e.length === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    for (const n of e)
      if (this.nodes.set(n.id, n), this.quadTree.insert(n), n.type === "edge") {
        const r = n, { fromId: s, toId: i } = r.data;
        this.adjacency.has(s) || this.adjacency.set(s, /* @__PURE__ */ new Set()), this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.get(s).add(n.id), this.adjacency.get(i).add(n.id);
      }
    const o = e.filter((n) => n.type !== "edge").map((n) => n.id);
    o.length > 0 && this.updateFrameMembership(o), this.emit("change"), this.emit("history");
  }
  updateNode(e, o) {
    var s, i, l, a, c, h, p, d, f;
    const n = this.nodes.get(e);
    if (!n) return;
    const r = { ...n, ...o };
    if (o.data && typeof o.data == "object" && n.data && typeof n.data == "object" && (r.data = {
      ...n.data,
      ...o.data
    }), this.nodes.set(e, r), (n.x !== r.x || n.y !== r.y || n.w !== r.w || n.h !== r.h || (n.rotation ?? 0) !== (r.rotation ?? 0)) && (this.quadTree.remove(n), this.quadTree.insert(r), this.updateConnectedEdges(e)), n.x !== r.x || n.y !== r.y) {
      const m = r.x - n.x, y = r.y - n.y;
      (l = (i = (s = this.registry) == null ? void 0 : s.get(r.type)) == null ? void 0 : i.onMove) == null || l.call(i, r, m, y, this), this.emit("node:move", r, m, y);
    }
    if (n.w !== r.w || n.h !== r.h) {
      const m = n.w !== 0 ? r.w / n.w : 1, y = n.h === "auto" ? 0 : n.h, b = r.h === "auto" ? 0 : r.h, g = y !== 0 ? b / y : 1;
      this.emit("node:resize", r, m, g);
    }
    (n.rotation ?? 0) !== (r.rotation ?? 0) && ((h = (c = (a = this.registry) == null ? void 0 : a.get(r.type)) == null ? void 0 : c.onRotate) == null || h.call(c, r, r.rotation ?? 0, this), this.emit("node:rotate", r, r.rotation ?? 0)), o.data && n.data !== r.data && ((f = (d = (p = this.registry) == null ? void 0 : p.get(r.type)) == null ? void 0 : d.onDataChange) == null || f.call(d, r, n.data, r.data, this), this.emit("node:data", r, n.data, r.data)), this.emit("change");
  }
  /**
   * Batch update multiple nodes with a single change emit.
   * Use during drag/resize to avoid N re-renders per frame.
   */
  updateMany(e) {
    let o = !1;
    for (const { id: n, patch: r } of e) {
      const s = this.nodes.get(n);
      if (!s) continue;
      const i = { ...s, ...r };
      r.data && typeof r.data == "object" && s.data && typeof s.data == "object" && (i.data = {
        ...s.data,
        ...r.data
      }), this.nodes.set(n, i), (s.x !== i.x || s.y !== i.y || s.w !== i.w || s.h !== i.h || (s.rotation ?? 0) !== (i.rotation ?? 0)) && (this.quadTree.remove(s), this.quadTree.insert(i), this.updateConnectedEdges(n)), o = !0;
    }
    o && this.emit("change");
  }
  updateConnectedEdges(e) {
    const o = this.adjacency.get(e);
    if (o)
      for (const n of o) {
        const r = this.nodes.get(n);
        if (!r || r.type !== "edge") continue;
        const s = r, i = this.nodes.get(s.data.fromId), l = this.nodes.get(s.data.toId);
        if (i && l) {
          const a = Ze(
            i,
            l,
            s.data.edgeType,
            void 0,
            s.data.sourceHandle,
            s.data.targetHandle,
            s.data.midpointOffset,
            s.data.curveOffset
          ), c = { ...s, ...a.bounds };
          this.nodes.set(n, c), this.quadTree.remove(s), this.quadTree.insert(c);
        }
      }
  }
  updateNodeWithHistory(e, o) {
    this.history.pushSnapshot(this.nodes, this.groupParent), this.updateNode(e, o), this.emit("history");
  }
  /** Update multiple nodes in a single undo step. */
  batchUpdateWithHistory(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const { id: o, patch: n } of e)
        this.updateNode(o, n);
      this.emit("history");
    }
  }
  deleteNode(e) {
    var n, r, s, i, l;
    if (!this.nodes.has(e) || (n = this.nodes.get(e)) != null && n.locked) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = this.nodes.get(e);
    o && ((i = (s = (r = this.registry) == null ? void 0 : r.get(o.type)) == null ? void 0 : s.onDelete) == null || i.call(s, o, this), this.emit("node:delete", o), this.quadTree.remove(o)), this.nodes.delete(e), this.selection.delete(e), this.adjacency.delete(e), this.frameChildren.delete(e);
    for (const a of this.frameChildren.values()) a.delete(e);
    for (const [a, c] of this.nodes)
      if (c.type === "edge") {
        const h = c.data;
        if (h.fromId === e || h.toId === e) {
          const p = this.nodes.get(a);
          p && this.quadTree.remove(p), this.nodes.delete(a), this.selection.delete(a);
          const d = h.fromId === e ? h.toId : h.fromId;
          (l = this.adjacency.get(d)) == null || l.delete(a);
        }
      }
    this.emit("change"), this.emit("selection"), this.emit("history");
  }
  getNode(e) {
    return this.nodes.get(e);
  }
  getAllNodes() {
    return Array.from(this.nodes.values());
  }
  /** Returns a read-only iterable of all nodes (no copy). */
  iterNodes() {
    return this.nodes.values();
  }
  getNodesByType(e) {
    const o = [];
    for (const n of this.nodes.values())
      n.type === e && o.push(n);
    return o;
  }
  /** Returns all non-edge nodes fully contained within a frame's bounds (including nested frames). */
  getNodesInsideFrame(e) {
    const o = this.nodes.get(e);
    if (!o || !this._containerTypes.has(o.type)) return [];
    const n = this.resolveHeight(o), r = [];
    for (const s of this.nodes.values()) {
      if (s.id === e || s.type === "edge") continue;
      const i = this.resolveHeight(s);
      s.x >= o.x && s.y >= o.y && s.x + s.w <= o.x + o.w && s.y + i <= o.y + n && r.push(s);
    }
    return r;
  }
  /** Returns tracked frame children (nodes explicitly added to the frame). */
  getFrameChildren(e) {
    const o = this.frameChildren.get(e);
    if (!o) return [];
    const n = [];
    for (const r of o) {
      const s = this.nodes.get(r);
      s && n.push(s);
    }
    return n;
  }
  /** Returns IDs of all descendants of a frame (children, grandchildren, etc.). */
  getFrameDescendantIds(e) {
    const o = /* @__PURE__ */ new Set(), n = (r) => {
      const s = this.frameChildren.get(r);
      if (s)
        for (const i of s) {
          if (o.has(i)) continue;
          o.add(i);
          const l = this.nodes.get(i);
          l && this._containerTypes.has(l.type) && n(i);
        }
    };
    return n(e), o;
  }
  /** Rebuild frameChildren from spatial containment. Called on load/undo/redo.
   *  Each node is assigned only to its smallest containing frame. */
  rebuildFrameChildren() {
    this.frameChildren.clear();
    const e = [];
    for (const n of this.nodes.values()) {
      if (!this._containerTypes.has(n.type)) continue;
      const r = this.resolveHeight(n);
      e.push({ node: n, area: n.w * r });
    }
    e.sort((n, r) => n.area - r.area);
    const o = /* @__PURE__ */ new Set();
    for (const { node: n } of e) {
      const s = this.getNodesInsideFrame(n.id).filter((i) => !o.has(i.id));
      if (s.length > 0) {
        const i = /* @__PURE__ */ new Set();
        for (const l of s)
          i.add(l.id), o.add(l.id);
        this.frameChildren.set(n.id, i);
      }
    }
  }
  /** After nodes are moved, update which frames they belong to.
   *  Each node is assigned only to its smallest containing frame.
   *  Frames can be nested inside other frames (but not inside themselves or their descendants). */
  updateFrameMembership(e) {
    for (const o of e) {
      const n = this.nodes.get(o);
      if (!n || n.type === "edge") continue;
      const r = this.resolveHeight(n);
      for (const [c, h] of this.frameChildren) {
        if (!h.has(o)) continue;
        const p = this.nodes.get(c);
        if (!p) {
          h.delete(o);
          continue;
        }
        const d = this.resolveHeight(p);
        n.x >= p.x && n.y >= p.y && n.x + n.w <= p.x + p.w && n.y + r <= p.y + d || h.delete(o);
      }
      let s;
      this._containerTypes.has(n.type) && (s = this.getFrameDescendantIds(o));
      let i = null, l = 1 / 0;
      const a = this.quadTree.retrieve([], { x: n.x, y: n.y, w: n.w, h: r });
      for (const c of a) {
        if (!this._containerTypes.has(c.type) || c.id === o || s != null && s.has(c.id)) continue;
        const h = this.resolveHeight(c);
        if (n.x >= c.x && n.y >= c.y && n.x + n.w <= c.x + c.w && n.y + r <= c.y + h) {
          const d = c.w * h;
          d < l && (l = d, i = c);
        }
      }
      for (const [, c] of this.frameChildren)
        c.delete(o);
      i && (this.frameChildren.has(i.id) || this.frameChildren.set(i.id, /* @__PURE__ */ new Set()), this.frameChildren.get(i.id).add(o));
    }
  }
  /** Sync frame children after resize: remove nodes no longer inside, add newly contained nodes. */
  syncFrameChildrenAfterResize(e) {
    const o = this.nodes.get(e);
    if (!o || !this._containerTypes.has(o.type)) return;
    const n = this.getNodesInsideFrame(e);
    n.length > 0 ? this.frameChildren.set(e, new Set(n.map((r) => r.id))) : this.frameChildren.delete(e);
  }
  /** Adopt all existing nodes that are spatially inside a newly created frame. */
  adoptNodesIntoNewFrame(e) {
    const o = this.getNodesInsideFrame(e);
    if (o.length > 0) {
      const n = /* @__PURE__ */ new Set();
      for (const r of o) n.add(r.id);
      this.frameChildren.set(e, n);
    }
  }
  nextZ() {
    return this.nextZValue++;
  }
  setNextZ(e) {
    this.nextZValue = e;
  }
  // --- Z-ordering ---
  bringToFront(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const n = this.nodes.get(o);
        n && !n.locked && this.nodes.set(o, { ...n, z: this.nextZValue++ });
      }
      this.emit("change"), this.emit("history");
    }
  }
  sendToBack(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (let o = e.length - 1; o >= 0; o--) {
        const n = this.nodes.get(e[o]);
        n && !n.locked && this.nodes.set(e[o], { ...n, z: --this._minZ });
      }
      this.emit("change"), this.emit("history");
    }
  }
  /** AABB overlap test between two nodes. */
  _nodesOverlap(e, o) {
    const n = this.resolveHeight(e), r = this.resolveHeight(o);
    return e.x < o.x + o.w && e.x + e.w > o.x && e.y < o.y + r && e.y + n > o.y;
  }
  bringForward(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const n = this.nodes.get(o);
        if (!n || n.locked) continue;
        const r = n.type === "edge", s = [];
        for (const h of this.nodes.values())
          h.id !== o && (r ? h.type === "edge" : h.type !== "edge") && h.z >= n.z && this._nodesOverlap(n, h) && s.push(h);
        if (s.length === 0) continue;
        s.sort((h, p) => h.z - p.z);
        const i = s[0], l = this.nodes.get(i.id), a = n.z, c = l.z;
        a === c ? this.nodes.set(o, { ...n, z: c + 1 }) : (this.nodes.set(o, { ...n, z: c }), this.nodes.set(i.id, { ...l, z: a }));
      }
      this.emit("change"), this.emit("history");
    }
  }
  sendBackward(e) {
    if (e.length !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const n = this.nodes.get(o);
        if (!n || n.locked) continue;
        const r = n.type === "edge", s = [];
        for (const h of this.nodes.values())
          h.id !== o && (r ? h.type === "edge" : h.type !== "edge") && h.z <= n.z && this._nodesOverlap(n, h) && s.push(h);
        if (s.length === 0) continue;
        s.sort((h, p) => p.z - h.z);
        const i = s[0], l = this.nodes.get(i.id), a = n.z, c = l.z;
        a === c ? this.nodes.set(o, { ...n, z: c - 1 }) : (this.nodes.set(o, { ...n, z: c }), this.nodes.set(i.id, { ...l, z: a }));
      }
      this.emit("change"), this.emit("history");
    }
  }
  /** Update the QuadTree bounds for an auto-height node when its measured height changes. */
  updateMeasuredHeight(e, o) {
    const n = this.nodes.get(e);
    !n || n.h !== "auto" || (this._measuredHeights[e] = o, this.quadTree.remove(n), this.quadTree.insert(n, o));
  }
  // --- Spatial Queries ---
  hitTest(e, o, n) {
    const s = this.quadTree.retrieve([], {
      x: e - 50,
      y: o - 50,
      w: 100,
      h: 100
    }), i = /* @__PURE__ */ new Map();
    for (const l of s) i.set(l.id, l);
    return Sl(i, e, o, this.viewport.zoom, n, this._containerTypes);
  }
  /** Returns all nodes at a point, sorted highest-z first */
  hitTestAll(e, o, n) {
    const s = this.quadTree.retrieve([], {
      x: e - 50,
      y: o - 50,
      w: 100,
      h: 100
    }), i = /* @__PURE__ */ new Map();
    for (const l of s) i.set(l.id, l);
    return Cl(i, e, o, this.viewport.zoom, n, this._containerTypes);
  }
  getNodesInRect(e) {
    return this.quadTree.retrieve([], e);
  }
  // --- Selection ---
  /** Expand selection to include all group siblings, walking up the group
   *  hierarchy until the active group (or root) is reached. */
  expandSelectionToGroups() {
    const e = /* @__PURE__ */ new Set();
    for (const r of this.selection) {
      const s = this.nodes.get(r);
      if (!(s != null && s.groupId) || this.activeGroupId && s.groupId === this.activeGroupId) continue;
      let i = s.groupId;
      for (; ; ) {
        const l = this.groupParent.get(i);
        if (!l || this.activeGroupId && l === this.activeGroupId) break;
        i = l;
      }
      e.add(i);
    }
    if (e.size === 0) return;
    const o = new Set(e), n = (r) => {
      const s = this.groupChildren.get(r);
      if (s)
        for (const i of s)
          o.has(i) || (o.add(i), n(i));
    };
    for (const r of e)
      n(r);
    for (const r of this.nodes.values())
      r.groupId && o.has(r.groupId) && this.selection.add(r.id);
  }
  select(e) {
    var o, n, r, s, i, l;
    for (const a of this.selection) {
      const c = this.nodes.get(a);
      c && ((r = (n = (o = this.registry) == null ? void 0 : o.get(c.type)) == null ? void 0 : n.onDeselect) == null || r.call(n, c, this), this.emit("node:deselect", c));
    }
    this.selection.clear(), this.selection.add(e), this.expandSelectionToGroups();
    for (const a of this.selection) {
      const c = this.nodes.get(a);
      c && ((l = (i = (s = this.registry) == null ? void 0 : s.get(c.type)) == null ? void 0 : i.onSelect) == null || l.call(i, c, this), this.emit("node:select", c));
    }
    this.emit("selection");
  }
  toggleSelect(e) {
    const o = this.nodes.get(e);
    if (this.selection.has(e))
      if (o != null && o.groupId)
        for (const n of this.nodes.values())
          n.groupId === o.groupId && this.selection.delete(n.id);
      else
        this.selection.delete(e);
    else
      this.selection.add(e), this.expandSelectionToGroups();
    this.emit("selection");
  }
  selectMultiple(e) {
    this.selection = new Set(e), this.expandSelectionToGroups(), this.emit("selection");
  }
  deselectAll() {
    var e, o, n;
    if (!(this.selection.size === 0 && !this.activeGroupId)) {
      for (const r of this.selection) {
        const s = this.nodes.get(r);
        s && ((n = (o = (e = this.registry) == null ? void 0 : e.get(s.type)) == null ? void 0 : o.onDeselect) == null || n.call(o, s, this), this.emit("node:deselect", s));
      }
      this.selection.clear(), this.activeGroupId && (this.activeGroupId = null, this.emit("group:exit")), this.emit("selection");
    }
  }
  deleteSelected() {
    var n, r, s;
    if (this.selection.size === 0) return;
    const e = new Set(
      Array.from(this.selection).filter((i) => {
        var l;
        return !((l = this.nodes.get(i)) != null && l.locked);
      })
    );
    if (e.size === 0) return;
    this.activeGroupId && this.getGroupMembers(this.activeGroupId).filter((l) => !e.has(l.id)).length === 0 && (this.activeGroupId = null, this.emit("group:exit")), this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = e;
    for (const i of e) {
      const l = this.nodes.get(i);
      l && ((s = (r = (n = this.registry) == null ? void 0 : n.get(l.type)) == null ? void 0 : r.onDelete) == null || s.call(r, l, this), this.emit("node:delete", l), this.quadTree.remove(l), this.nodes.delete(i));
    }
    for (const [i, l] of this.nodes)
      if (l.type === "edge") {
        const a = l.data;
        if (o.has(a.fromId) || o.has(a.toId)) {
          const c = this.nodes.get(i);
          c && this.quadTree.remove(c), this.nodes.delete(i);
        }
      }
    this.cleanupEmptyGroups();
    for (const i of e) this.selection.delete(i);
    this.emit("change"), this.emit("selection"), this.emit("history");
  }
  /** Remove groupParent entries for groups that no longer have any members. */
  cleanupEmptyGroups() {
    const e = /* @__PURE__ */ new Set();
    for (const o of this.nodes.values())
      o.groupId && e.add(o.groupId);
    for (const [o] of this.groupParent)
      e.has(o) || this.unlinkGroupParent(o);
  }
  /** Set a groupParent entry and keep groupChildren in sync. */
  linkGroupParent(e, o) {
    var s;
    const n = this.groupParent.get(e);
    n && ((s = this.groupChildren.get(n)) == null || s.delete(e)), this.groupParent.set(e, o);
    let r = this.groupChildren.get(o);
    r || (r = /* @__PURE__ */ new Set(), this.groupChildren.set(o, r)), r.add(e);
  }
  /** Remove a groupParent entry and keep groupChildren in sync. */
  unlinkGroupParent(e) {
    const o = this.groupParent.get(e);
    if (o) {
      const n = this.groupChildren.get(o);
      n && (n.delete(e), n.size === 0 && this.groupChildren.delete(o));
    }
    this.groupParent.delete(e);
  }
  /** Rebuild the groupChildren reverse index from groupParent. */
  rebuildGroupChildren() {
    this.groupChildren.clear();
    for (const [e, o] of this.groupParent) {
      let n = this.groupChildren.get(o);
      n || (n = /* @__PURE__ */ new Set(), this.groupChildren.set(o, n)), n.add(e);
    }
  }
  deleteNodes(e) {
    var n, r, s;
    if (e.length === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const o = new Set(e);
    for (const i of e) {
      const l = this.nodes.get(i);
      if (l) {
        (s = (r = (n = this.registry) == null ? void 0 : n.get(l.type)) == null ? void 0 : r.onDelete) == null || s.call(r, l, this), this.emit("node:delete", l), this.quadTree.remove(l), this.nodes.delete(i), this.frameChildren.delete(i);
        for (const a of this.frameChildren.values()) a.delete(i);
      }
    }
    for (const [i, l] of this.nodes)
      if (l.type === "edge") {
        const a = l.data;
        if (o.has(a.fromId) || o.has(a.toId)) {
          const c = this.nodes.get(i);
          c && this.quadTree.remove(c), this.nodes.delete(i);
        }
      }
    this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  // --- Flip ---
  flipSelected(e) {
    var o;
    if (this.selection.size !== 0) {
      this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const n of this.selection) {
        const r = this.nodes.get(n);
        if (!r) continue;
        this.quadTree.remove(r);
        let s = null;
        const i = (o = this.registry) == null ? void 0 : o.get(r.type);
        if (i != null && i.onFlip) {
          const l = i.onFlip(r, e, this);
          l && Object.keys(l).length > 0 && (s = {
            ...r,
            data: { ...r.data, ...l }
          });
        } else if (r.type === "draw") {
          const l = r;
          if (e === "h") {
            const a = l.data.points.map(
              ([c, h, p]) => [l.w - c, h, p]
            );
            s = { ...l, data: { ...l.data, points: a } };
          } else {
            const a = l.h === "auto" ? 0 : l.h, c = l.data.points.map(
              ([h, p, d]) => [h, a - p, d]
            );
            s = { ...l, data: { ...l.data, points: c } };
          }
        } else if (r.type === "shape") {
          const l = r;
          if (l.data.shape === "arrow" || l.data.shape === "line")
            if (l.data.startPoint && l.data.endPoint)
              if (e === "h") {
                const a = [l.w - l.data.startPoint[0], l.data.startPoint[1]], c = [l.w - l.data.endPoint[0], l.data.endPoint[1]];
                s = { ...l, data: { ...l.data, startPoint: a, endPoint: c } };
              } else {
                const a = l.h === "auto" ? 0 : l.h, c = [l.data.startPoint[0], a - l.data.startPoint[1]], h = [l.data.endPoint[0], a - l.data.endPoint[1]];
                s = { ...l, data: { ...l.data, startPoint: c, endPoint: h } };
              }
            else
              s = e === "h" ? { ...l, rotation: -(l.rotation || 0) + 180 } : { ...l, rotation: -(l.rotation || 0) };
        } else if (r.type === "image") {
          const l = r;
          s = e === "h" ? { ...l, data: { ...l.data, flipH: !l.data.flipH } } : { ...l, data: { ...l.data, flipV: !l.data.flipV } };
        }
        s ? (this.nodes.set(n, s), this.quadTree.insert(s), this.emit("node:flip", s, e)) : this.quadTree.insert(r);
      }
      this.emit("change"), this.emit("history");
    }
  }
  flipSelectedHorizontal() {
    this.flipSelected("h");
  }
  flipSelectedVertical() {
    this.flipSelected("v");
  }
  // --- Grouping ---
  groupSelected() {
    if (this.selection.size < 2 || this.activeGroupId) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const e = bt(10), o = /* @__PURE__ */ new Set();
    for (const n of this.selection) {
      const r = this.nodes.get(n);
      if (r != null && r.groupId) {
        let s = r.groupId;
        for (; this.groupParent.has(s); ) s = this.groupParent.get(s);
        o.add(s);
      }
    }
    if (o.size > 0) {
      for (const n of o)
        this.linkGroupParent(n, e);
      for (const n of this.selection) {
        const r = this.nodes.get(n);
        r && !r.groupId && this.nodes.set(n, { ...r, groupId: e });
      }
    } else
      for (const n of this.selection) {
        const r = this.nodes.get(n);
        r && this.nodes.set(n, { ...r, groupId: e });
      }
    this.emit("change"), this.emit("history");
  }
  ungroupSelected() {
    if (this.selection.size === 0) return;
    const e = /* @__PURE__ */ new Set();
    for (const o of this.selection) {
      const n = this.nodes.get(o);
      if (n != null && n.groupId) {
        let r = n.groupId;
        for (; this.groupParent.has(r); ) {
          const s = this.groupParent.get(r);
          if (s === this.activeGroupId) break;
          r = s;
        }
        e.add(r);
      }
    }
    if (e.size !== 0) {
      this.activeGroupId && e.has(this.activeGroupId) && (this.activeGroupId = null, this.emit("group:exit")), this.history.pushSnapshot(this.nodes, this.groupParent);
      for (const o of e) {
        const n = this.groupParent.get(o);
        for (const s of this.nodes.values())
          if (s.groupId === o)
            if (n)
              this.nodes.set(s.id, { ...s, groupId: n });
            else {
              const { groupId: i, ...l } = s;
              this.nodes.set(s.id, l);
            }
        const r = this.groupChildren.get(o);
        if (r)
          for (const s of [...r])
            n ? this.linkGroupParent(s, n) : this.unlinkGroupParent(s);
        this.unlinkGroupParent(o), this.groupChildren.delete(o), this.groupRotations.delete(o);
      }
      this.emit("change"), this.emit("history");
    }
  }
  selectionHasGroup() {
    var e;
    for (const o of this.selection)
      if ((e = this.nodes.get(o)) != null && e.groupId) return !0;
    return !1;
  }
  /** Returns the outermost groupId if all selected nodes belong to the same group tree, else undefined. */
  selectionGroupId() {
    if (this.selection.size < 2) return;
    let e;
    for (const o of this.selection) {
      const n = this.nodes.get(o);
      if (!(n != null && n.groupId)) return;
      let r = n.groupId;
      for (; this.groupParent.has(r); ) r = this.groupParent.get(r);
      if (!e) e = r;
      else if (r !== e) return;
    }
    return e;
  }
  /** True if all selected nodes belong to exactly one group (possibly nested). */
  selectionIsSingleGroup() {
    if (this.selection.size < 2) return !1;
    let e;
    for (const o of this.selection) {
      const n = this.nodes.get(o);
      if (!(n != null && n.groupId)) return !1;
      let r = n.groupId;
      for (; this.groupParent.has(r); ) r = this.groupParent.get(r);
      if (!e) e = r;
      else if (r !== e) return !1;
    }
    return !0;
  }
  getGroupMembers(e) {
    const o = [];
    for (const n of this.nodes.values())
      n.groupId === e && o.push(n);
    return o;
  }
  /** Enter a group for drill-down selection of individual children. */
  enterGroup(e) {
    this.activeGroupId !== e && (this.activeGroupId = e, this.emit("group:enter", e));
  }
  /** Fully exit all group levels and deselect. */
  exitAllGroups() {
    this.activeGroupId && (this.activeGroupId = null, this.emit("group:exit"));
  }
  /** Exit the active group — go up one level for nested groups, or exit entirely. */
  exitGroup() {
    if (!this.activeGroupId) return;
    const e = this.activeGroupId, o = this.groupParent.get(e);
    o ? (this.activeGroupId = o, this.emit("group:enter", o)) : (this.activeGroupId = null, this.emit("group:exit"));
    const n = this.getGroupMembers(e);
    n.length > 0 && (this.selection = /* @__PURE__ */ new Set([n[0].id]), this.expandSelectionToGroups(), this.emit("selection"));
  }
  /** Check if a node belongs to the currently active (entered) group or any of its descendants. */
  isNodeInActiveGroup(e) {
    if (!this.activeGroupId) return !1;
    const o = this.nodes.get(e);
    if (!(o != null && o.groupId)) return !1;
    let n = o.groupId;
    for (; n; ) {
      if (n === this.activeGroupId) return !0;
      n = this.groupParent.get(n);
    }
    return !1;
  }
  /** Get the outermost group of a node (stopping at activeGroupId boundary). */
  getNodeOutermostGroup(e) {
    const o = this.nodes.get(e);
    if (!(o != null && o.groupId)) return;
    let n = o.groupId;
    for (; ; ) {
      const r = this.groupParent.get(n);
      if (!r || this.activeGroupId && r === this.activeGroupId) break;
      n = r;
    }
    return n;
  }
  /** Get all nodes that are descendants of a group (direct + nested sub-groups). */
  getAllGroupDescendantNodes(e) {
    const o = /* @__PURE__ */ new Set([e]), n = (s) => {
      const i = this.groupChildren.get(s);
      if (i)
        for (const l of i)
          o.has(l) || (o.add(l), n(l));
    };
    n(e);
    const r = [];
    for (const s of this.nodes.values())
      s.groupId && o.has(s.groupId) && r.push(s);
    return r;
  }
  duplicateSelected() {
    if (this.selection.size === 0) return;
    this.history.pushSnapshot(this.nodes, this.groupParent);
    const e = 20, o = /* @__PURE__ */ new Map(), n = [];
    for (const s of this.selection) {
      const i = this.nodes.get(s);
      if (!i) continue;
      const l = bt();
      o.set(i.id, l), n.push({
        ...JSON.parse(JSON.stringify(i)),
        id: l,
        x: i.x + e,
        y: i.y + e,
        z: this.nextZValue++,
        locked: void 0
      });
    }
    for (const s of n)
      if (s.type === "edge" && s.data) {
        const i = s.data;
        o.has(i.fromId) && (i.fromId = o.get(i.fromId)), o.has(i.toId) && (i.toId = o.get(i.toId));
      }
    const r = /* @__PURE__ */ new Map();
    for (const s of n)
      s.groupId && (r.has(s.groupId) || r.set(s.groupId, bt(10)), s.groupId = r.get(s.groupId));
    for (const [s, i] of this.groupParent)
      r.has(s) && r.has(i) && this.linkGroupParent(r.get(s), r.get(i));
    this.addNodes(n), this.selection = new Set(n.map((s) => s.id)), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  // --- Mode ---
  // --- Clipboard ---
  copySelected() {
    this.selection.size !== 0 && (this.clipboard = Array.from(this.selection).map((e) => {
      const o = this.nodes.get(e);
      return JSON.parse(JSON.stringify(o));
    }), this.pasteCount = 0);
  }
  cutSelected() {
    this.copySelected(), this.deleteSelected();
  }
  /**
   * Paste clipboard contents centered at a canvas position.
   * If no position given, uses viewport center.
   */
  pasteClipboard(e, o) {
    if (this.clipboard.length === 0) return;
    this.pasteCount++;
    let n = 1 / 0, r = 1 / 0, s = -1 / 0, i = -1 / 0;
    for (const g of this.clipboard) {
      const x = g.h === "auto" ? 100 : g.h;
      g.x < n && (n = g.x), g.y < r && (r = g.y), g.x + g.w > s && (s = g.x + g.w), g.y + x > i && (i = g.y + x);
    }
    const l = (n + s) / 2, a = (r + i) / 2;
    let c, h;
    if (e !== void 0 && o !== void 0)
      c = e, h = o;
    else {
      const g = this.getWindow(), x = g.innerWidth / 2, S = g.innerHeight / 2, M = No(this.viewport, x, S);
      c = M.x, h = M.y;
    }
    const p = this.pasteCount * 20, d = c - l + p, f = h - a + p, m = /* @__PURE__ */ new Map(), y = this.clipboard.map((g) => {
      const x = bt();
      return m.set(g.id, x), {
        ...structuredClone(g),
        id: x,
        x: g.x + d,
        y: g.y + f,
        z: this.nextZValue++,
        locked: void 0
      };
    });
    for (const g of y)
      if (g.type === "edge" && g.data) {
        const x = g.data;
        m.has(x.fromId) && (x.fromId = m.get(x.fromId)), m.has(x.toId) && (x.toId = m.get(x.toId));
      }
    const b = /* @__PURE__ */ new Map();
    for (const g of y)
      g.groupId && (b.has(g.groupId) || b.set(g.groupId, bt(10)), g.groupId = b.get(g.groupId));
    for (const [g, x] of this.groupParent)
      b.has(g) && b.has(x) && this.linkGroupParent(b.get(g), b.get(x));
    this.addNodes(y), this.selectMultiple(y.map((g) => g.id));
  }
  /**
   * Insert a pre-built template centered at (cx, cy) in canvas coordinates.
   */
  applyTemplate(e, o, n) {
    const r = Ti.find((f) => f.id === e);
    if (!r) return;
    const s = structuredClone(r.nodes), i = /* @__PURE__ */ new Map();
    for (const f of s) {
      const m = bt(10);
      i.set(f.id, m), f.id = m;
    }
    for (const f of s) {
      if (f.type === "edge" && f.data) {
        const m = f.data;
        i.has(m.fromId) && (m.fromId = i.get(m.fromId)), i.has(m.toId) && (m.toId = i.get(m.toId));
      }
      f.groupId && i.has(f.groupId) && (f.groupId = i.get(f.groupId));
    }
    let l = 1 / 0, a = 1 / 0, c = -1 / 0, h = -1 / 0;
    for (const f of s) {
      if (f.type === "edge") continue;
      const m = f.h === "auto" ? 100 : f.h;
      l = Math.min(l, f.x), a = Math.min(a, f.y), c = Math.max(c, f.x + f.w), h = Math.max(h, f.y + m);
    }
    const p = o - (l + c) / 2, d = n - (a + h) / 2;
    for (const f of s)
      f.type !== "edge" && (f.x += p, f.y += d), f.z = this.nextZValue++;
    this.addNodes(s), this.selectMultiple(s.map((f) => f.id));
  }
  hasClipboard() {
    return this.clipboard.length > 0;
  }
  getClipboardNodes() {
    return this.clipboard.map((e) => structuredClone(e));
  }
  setClipboard(e) {
    this.clipboard = e.map((o) => structuredClone(o)), this.pasteCount = 0;
  }
  // --- Mode ---
  setMode(e) {
    this.mode !== e && (this.mode = e, this.selection.size > 0 && (this.selection.clear(), this.emit("selection")), this.emit("mode"));
  }
  // --- History ---
  pushHistorySnapshot() {
    this.history.pushSnapshot(this.nodes, this.groupParent), this.emit("history");
  }
  /*private*/
  rebuildQuadTree() {
    this.quadTree.clear(), this.adjacency.clear();
    let e = 0, o = 0;
    for (const n of this.nodes.values())
      if (this.quadTree.insert(n), n.z < e && (e = n.z), n.z > o && (o = n.z), n.type === "edge") {
        const r = n, { fromId: s, toId: i } = r.data;
        this.adjacency.has(s) || this.adjacency.set(s, /* @__PURE__ */ new Set()), this.adjacency.has(i) || this.adjacency.set(i, /* @__PURE__ */ new Set()), this.adjacency.get(s).add(n.id), this.adjacency.get(i).add(n.id);
      }
    this._minZ = e, this.nextZValue = o + 1;
  }
  undo() {
    const e = this.history.undo(this.nodes, this.groupParent);
    e && (this.nodes = e.nodes, this.groupParent = e.groupParent, this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history"));
  }
  redo() {
    const e = this.history.redo(this.nodes, this.groupParent);
    e && (this.nodes = e.nodes, this.groupParent = e.groupParent, this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), this.selection.clear(), this.emit("change"), this.emit("selection"), this.emit("history"));
  }
  canUndo() {
    return this.history.canUndo();
  }
  canRedo() {
    return this.history.canRedo();
  }
  // --- Remote Collaboration ---
  /** Add a remote node without emitting events or pushing history. */
  addRemoteNode(e) {
    if (this._suppressEvents = !0, this.nodes.set(e.id, e), this.quadTree.insert(e), e.type === "edge") {
      const o = e, { fromId: n, toId: r } = o.data;
      this.adjacency.has(n) || this.adjacency.set(n, /* @__PURE__ */ new Set()), this.adjacency.has(r) || this.adjacency.set(r, /* @__PURE__ */ new Set()), this.adjacency.get(n).add(e.id), this.adjacency.get(r).add(e.id);
    }
    e.z >= this.nextZValue && (this.nextZValue = e.z + 1), e.z < this._minZ && (this._minZ = e.z), this._suppressEvents = !1;
  }
  /** Delete a remote node without emitting events or pushing history. */
  deleteRemoteNode(e) {
    var n;
    this._suppressEvents = !0;
    const o = this.nodes.get(e);
    if (o) {
      this.quadTree.remove(o), this.nodes.delete(e), this.selection.delete(e), this.adjacency.delete(e), this.frameChildren.delete(e);
      for (const r of this.frameChildren.values()) r.delete(e);
      for (const [r, s] of this.nodes)
        if (s.type === "edge") {
          const i = s.data;
          if (i.fromId === e || i.toId === e) {
            const l = this.nodes.get(r);
            l && this.quadTree.remove(l), this.nodes.delete(r), this.selection.delete(r);
            const a = i.fromId === e ? i.toId : i.fromId;
            (n = this.adjacency.get(a)) == null || n.delete(r);
          }
        }
    }
    this._suppressEvents = !1;
  }
  /** Apply a remote node update without emitting events or pushing history. */
  applyRemoteNodeUpdate(e, o) {
    this._suppressEvents = !0;
    const n = this.nodes.get(e);
    if (n) {
      const r = { ...n, ...o };
      o.data && typeof o.data == "object" && n.data && typeof n.data == "object" && (r.data = {
        ...n.data,
        ...o.data
      }), this.nodes.set(e, r), (n.x !== r.x || n.y !== r.y || n.w !== r.w || n.h !== r.h) && (this.quadTree.remove(n), this.quadTree.insert(r), this.updateConnectedEdges(e)), r.z >= this.nextZValue && (this.nextZValue = r.z + 1);
    }
    this._suppressEvents = !1;
  }
  /** Trigger a re-render without pushing history. Used after remote updates. */
  notifyChange() {
    var e;
    (e = this.listeners.change) == null || e.forEach((o) => o());
  }
  /** Emit draw progress for collab live stroke preview. */
  notifyDrawProgress(e) {
    this.emit("draw:progress", e);
  }
  /** Emit draw end when a stroke is completed or cancelled. */
  notifyDrawEnd() {
    this.emit("draw:end");
  }
  /** Emit shape progress for collab live shape preview. */
  notifyShapeProgress(e) {
    this.emit("shape:progress", e);
  }
  /** Emit shape end when shape creation is completed or cancelled. */
  notifyShapeEnd() {
    this.emit("shape:end");
  }
  /** Emit laser pointer progress for collab trail preview. */
  notifyLaserProgress(e) {
    this.emit("laser:progress", e);
  }
  /** Emit laser pointer end when trail has fully faded. */
  notifyLaserEnd() {
    this.emit("laser:end");
  }
  // --- Serialization ---
  async toSBD() {
    return Al(this.getAllNodes(), {
      background: this.boardBackground,
      originView: this.originView ?? void 0
    });
  }
  async fromSBD(e) {
    this.history.clear(), this.nodes.clear(), this.groupParent.clear(), this.groupChildren.clear();
    const { nodes: o, meta: n } = await Bl(e);
    n.background && (this.boardBackground = n.background, this.emit("background")), n.originView ? this.originView = n.originView : this.originView = null;
    let r = 0, s = 0;
    for (const i of o)
      this.nodes.set(i.id, i), i.z > r && (r = i.z), i.z < s && (s = i.z);
    this.rebuildQuadTree(), this.rebuildFrameChildren(), this.nextZValue = r + 1, this._minZ = s, this.selection.clear(), this.goToOriginView(), this.emit("change"), this.emit("selection"), this.emit("history");
  }
  toJSON() {
    const e = {
      nodes: Array.from(this.nodes.entries()),
      viewport: this.viewport
    };
    return this.groupParent.size > 0 && (e.groupParent = Array.from(this.groupParent.entries())), e;
  }
  fromJSON(e) {
    this.history.clear(), this.nodes = new Map(e.nodes), this.groupParent = new Map(e.groupParent ?? []), this.rebuildGroupChildren(), this.rebuildQuadTree(), this.rebuildFrameChildren(), e.viewport && (this.viewport = e.viewport), this.selection.clear(), this.emit("change"), this.emit("viewport"), this.emit("selection"), this.emit("history");
  }
}
class oc {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor(e) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    Ct(this, "types", /* @__PURE__ */ new Map());
    if (e)
      for (const o of e) this.register(o);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  register(e) {
    this.types.set(e.type, e);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  get(e) {
    return this.types.get(e);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getAll() {
    return Array.from(this.types.values());
  }
  /** Returns type strings for nodes that render in the DOM layer (not SVG-only). */
  getDOMTypes() {
    const e = /* @__PURE__ */ new Set();
    for (const o of this.types.values())
      o.isSVGOnly || e.add(o.type);
    return e;
  }
  has(e) {
    return this.types.has(e);
  }
}
const As = ["n", "ne", "e", "se", "s", "sw", "w", "nw"], nc = {
  n: "ns-resize",
  ne: "nesw-resize",
  e: "ew-resize",
  se: "nwse-resize",
  s: "ns-resize",
  sw: "nesw-resize",
  w: "ew-resize",
  nw: "nwse-resize"
};
function On(t, e) {
  const o = As.indexOf(t);
  if (o === -1) return "default";
  const n = (e % 360 + 360) % 360, r = Math.round(n / 45) % 8, s = (o + r) % 8;
  return nc[As[s]];
}
class rc extends el {
  constructor() {
    super(...arguments);
    Ct(this, "state", { hasError: !1 });
  }
  static getDerivedStateFromError() {
    return { hasError: !0 };
  }
  componentDidCatch(o, n) {
    console.error("[ContentBlock] Editor mount failed, showing markdown fallback:", o, n);
  }
  render() {
    return this.state.hasError ? this.props.fallback : this.props.children;
  }
}
function Ls({ markdown: t }) {
  return /* @__PURE__ */ u(
    "div",
    {
      className: "sb-markdown-fallback",
      style: {
        padding: "8px 12px",
        fontSize: 14,
        lineHeight: 1.6,
        whiteSpace: "pre-wrap",
        wordBreak: "break-word",
        color: "#374151",
        opacity: 0.85
      },
      children: t || " "
    }
  );
}
const sc = 0, ic = [
  { pos: "nw", top: 0, left: 0 },
  { pos: "n", top: 0, left: "50%" },
  { pos: "ne", top: 0, left: "100%" },
  { pos: "e", top: "50%", left: "100%" },
  { pos: "se", top: "100%", left: "100%" },
  { pos: "s", top: "100%", left: "50%" },
  { pos: "sw", top: "100%", left: 0 },
  { pos: "w", top: "50%", left: 0 }
];
function ac(t) {
  return t.type !== "paragraph" ? !1 : !t.content || t.content.length === 0 ? !0 : t.content.every(
    (e) => e.type === "text" && (!e.text || e.text === "")
  );
}
function lc({
  node: t,
  isSelected: e,
  multiSelected: o,
  engine: n,
  schema: r,
  interactive: s,
  zoom: i,
  onMeasuredHeight: l,
  autoEdit: a
}) {
  const c = at(null), h = at(a === !0), p = at(!1), d = at(!1), f = at(!1), m = at(!1), y = at(t.data.blocks), [b, g] = et(!1), [x, S] = et(!1), M = at(null), z = rl({ schema: r }), W = at(
    t.data.blocks.length > 0 ? t.data.blocks : null
  );
  yt(() => {
    const P = W.current;
    if (!P) return;
    W.current = null;
    const G = requestAnimationFrame(() => {
      try {
        z.replaceBlocks(z.document, P);
        return;
      } catch {
      }
      try {
        const X = z.blocksToHTMLLossy(P);
        z._tiptapEditor.commands.setContent(X);
        return;
      } catch {
      }
      console.warn("[ContentBlock] All block rendering strategies failed, using markdown fallback"), S(!0);
    });
    return () => cancelAnimationFrame(G);
  }, [z]), yt(() => {
    (!e || o) && g(!1);
  }, [e, o]), yt(() => {
    h.current && (h.current = !1, p.current = !0, g(!0));
  }, [z]), yt(() => {
    if (!b || !p.current && !M.current) return;
    const P = M.current;
    M.current = null, p.current = !1;
    const G = requestAnimationFrame(() => {
      if (z.focus(), P)
        try {
          const X = z._tiptapEditor, j = X.view.posAtCoords({ left: P.x, top: P.y });
          j && X.commands.setTextSelection(j.pos);
        } catch {
        }
    });
    return () => cancelAnimationFrame(G);
  }, [b, z]);
  const I = st(() => {
    if (d.current || f.current) return;
    const P = n.getNode(t.id), G = z.document;
    y.current = G, n.updateNode(t.id, {
      data: { ...P == null ? void 0 : P.data, blocks: G }
    });
  }, [z, n, t.id]);
  yt(() => {
    if (!z) return;
    const P = () => {
      var tt, nt;
      if (d.current || f.current || m.current) return;
      const N = z.document.length, j = n.getNode(t.id), H = ((nt = (tt = j == null ? void 0 : j.data) == null ? void 0 : tt.blocks) == null ? void 0 : nt.length) ?? 0;
      if (N < H) return;
      const _ = setTimeout(I, 100);
      return () => clearTimeout(_);
    };
    let G;
    const X = z.onChange(() => {
      G == null || G(), G = P();
    });
    return () => {
      X == null || X(), G == null || G();
    };
  }, [z, I]), yt(() => {
    const P = c.current;
    if (!P) return;
    const G = (X) => {
      const N = X.relatedTarget;
      N && P.contains(N) || I();
    };
    return P.addEventListener("focusout", G), () => P.removeEventListener("focusout", G);
  }, [I]), yt(() => {
    if (b || t.data.blocks === y.current) return;
    const P = JSON.stringify(t.data.blocks), G = JSON.stringify(y.current);
    if (P !== G) {
      if (t.data.blocks.length > 0 && z.document.length > 0) {
        m.current = !0;
        try {
          z.replaceBlocks(z.document, t.data.blocks);
        } catch {
          try {
            const X = z.blocksToHTMLLossy(t.data.blocks);
            z._tiptapEditor.commands.setContent(X);
          } catch {
          }
        }
        m.current = !1;
      }
      y.current = t.data.blocks;
    }
  }, [t.data.blocks, b, z]), yt(() => {
    if (t.h !== "auto" || !l) return;
    const P = c.current;
    if (!P) return;
    const G = () => {
      const N = P.offsetHeight;
      N > 0 && l(t.id, N);
    };
    G();
    const X = new ResizeObserver(G);
    return X.observe(P), () => X.disconnect();
  }, [t.id, t.h, l]);
  const R = st(() => {
    const P = n.getNode(t.id);
    if (!P || P.h === "auto" || !z || !c.current)
      return;
    const G = P.h - sc, X = c.current.querySelector(".bn-editor");
    if (!X) return;
    const N = z.document;
    if (N.length === 0) return;
    let j = 0;
    for (let nt = N.length - 1; nt >= 1 && ac(N[nt]); nt--)
      j++;
    const H = X.scrollHeight, _ = N.length > 0 ? H / N.length : 36;
    if (d.current = !0, H < G) {
      const nt = G - H, ut = Math.max(0, Math.floor(nt / _));
      if (ut > 0) {
        const pt = N[N.length - 1];
        z.insertBlocks(
          Array.from({ length: ut }, () => ({
            type: "paragraph",
            content: []
          })),
          pt,
          "after"
        );
      }
    } else if (H > G && j > 0) {
      const nt = H - G, ut = Math.min(j, Math.ceil(nt / _));
      if (ut > 0) {
        const pt = N.slice(N.length - ut);
        z.removeBlocks(pt);
      }
    }
    const tt = n.getNode(t.id);
    tt && n.updateNode(t.id, {
      data: { ...tt.data, blocks: z.document }
    }), d.current = !1;
  }, [z, n, t.id]), K = at(R);
  K.current = R, yt(() => {
    if (t.h === "auto") return;
    const P = setTimeout(() => K.current(), 60);
    return () => clearTimeout(P);
  }, []);
  const Z = st(
    (P) => {
      const G = P.currentTarget.ownerDocument;
      if (P.altKey) return;
      if (!n.selection.has(t.id) && n.selection.size > 0) {
        const { x: xt, y: ht } = n.screenToCanvas(P.clientX, P.clientY);
        for (const Dt of n.selection) {
          const Tt = n.getNode(Dt);
          if (!Tt) continue;
          const Rt = Tt.h === "auto" ? 100 : Tt.h;
          if (xt >= Tt.x && xt <= Tt.x + Tt.w && ht >= Tt.y && ht <= Tt.y + Rt)
            return;
        }
      }
      P.stopPropagation(), P.preventDefault(), P.currentTarget.setPointerCapture(P.pointerId), P.shiftKey ? n.toggleSelect(t.id) : n.selection.has(t.id) || n.select(t.id);
      const X = P.clientX, N = P.clientY, j = Array.from(n.selection), H = j.map((xt) => {
        const ht = n.getNode(xt);
        return { id: xt, x: ht.x, y: ht.y };
      });
      let _ = !1, tt = null, nt = X, ut = N, pt = !1;
      const vt = () => {
        tt = null;
        const xt = (nt - X) / n.viewport.zoom, ht = (ut - N) / n.viewport.zoom, { finalDx: Dt, finalDy: Tt } = n.computeDragSnap(
          H,
          j,
          xt,
          ht,
          pt
        ), Rt = H.map((Qt) => ({
          id: Qt.id,
          patch: { x: Qt.x + Dt, y: Qt.y + Tt }
        }));
        n.updateMany(Rt);
      }, rt = (xt) => {
        const ht = (xt.clientX - X) / n.viewport.zoom, Dt = (xt.clientY - N) / n.viewport.zoom;
        if (!_)
          if (Math.abs(ht) > 2 || Math.abs(Dt) > 2)
            _ = !0, f.current = !0, n.pushHistorySnapshot();
          else
            return;
        nt = xt.clientX, ut = xt.clientY, pt = xt.metaKey || xt.ctrlKey, tt === null && (tt = requestAnimationFrame(vt));
      }, gt = () => {
        f.current = !1, tt !== null && (cancelAnimationFrame(tt), vt()), n.clearAlignGuides(), G.removeEventListener("pointermove", rt), G.removeEventListener("pointerup", gt);
      };
      G.addEventListener("pointermove", rt), G.addEventListener("pointerup", gt);
    },
    [n, t.id]
  ), lt = st(
    (P) => {
      var vt;
      const G = P.currentTarget.ownerDocument;
      P.stopPropagation(), P.preventDefault();
      const X = t.h === "auto" ? (((vt = c.current) == null ? void 0 : vt.getBoundingClientRect().height) ?? 60) / n.viewport.zoom : t.h, N = t.x + t.w / 2, j = t.y + X / 2, H = t.rotation || 0, { x: _, y: tt } = n.screenToCanvas(
        P.clientX,
        P.clientY
      ), nt = Math.atan2(tt - j, _ - N);
      n.pushHistorySnapshot();
      const ut = (rt) => {
        const { x: gt, y: xt } = n.screenToCanvas(rt.clientX, rt.clientY), ht = Math.atan2(xt - j, gt - N);
        let Dt = H + (ht - nt) * (180 / Math.PI);
        (rt.shiftKey || n.snapToGrid) && !(rt.metaKey || rt.ctrlKey) && (Dt = Math.round(Dt / 15) * 15), n.updateNode(t.id, { rotation: Dt });
      }, pt = () => {
        G.removeEventListener("pointermove", ut), G.removeEventListener("pointerup", pt);
      };
      G.addEventListener("pointermove", ut), G.addEventListener("pointerup", pt);
    },
    [n, t.id, t.x, t.y, t.w, t.h, t.rotation]
  ), O = st(
    (P, G) => {
      var vt;
      const X = G.currentTarget.ownerDocument;
      G.stopPropagation(), G.preventDefault();
      const N = G.clientX, j = G.clientY, H = t.x, _ = t.y, tt = t.w, nt = t.h === "auto" ? (((vt = c.current) == null ? void 0 : vt.getBoundingClientRect().height) ?? 60) / n.viewport.zoom : t.h;
      n.pushHistorySnapshot();
      const ut = (rt) => {
        const gt = (rt.clientX - N) / n.viewport.zoom, xt = (rt.clientY - j) / n.viewport.zoom;
        let ht = H, Dt = _, Tt = tt, Rt = nt;
        if ((P === "nw" || P === "w" || P === "sw") && (ht = H + gt, Tt = tt - gt), (P === "ne" || P === "e" || P === "se") && (Tt = tt + gt), (P === "nw" || P === "n" || P === "ne") && (Dt = _ + xt, Rt = nt - xt), (P === "sw" || P === "s" || P === "se") && (Rt = nt + xt), n.snapToGrid && !(rt.metaKey || rt.ctrlKey)) {
          const Qt = n.gridSize, Ut = (re) => Math.round(re / Qt) * Qt;
          (P === "nw" || P === "w" || P === "sw") && (ht = Ut(ht), Tt = H + tt - ht), (P === "ne" || P === "e" || P === "se") && (Tt = Ut(ht + Tt) - ht), (P === "nw" || P === "n" || P === "ne") && (Dt = Ut(Dt), Rt = _ + nt - Dt), (P === "sw" || P === "s" || P === "se") && (Rt = Ut(Dt + Rt) - Dt);
        }
        Tt < 100 && (Tt = 100, (P === "nw" || P === "w" || P === "sw") && (ht = H + tt - 100)), Rt < 60 && (Rt = 60, (P === "nw" || P === "n" || P === "ne") && (Dt = _ + nt - 60)), n.updateNode(t.id, { x: ht, y: Dt, w: Tt, h: Rt });
      }, pt = () => {
        X.removeEventListener("pointermove", ut), X.removeEventListener("pointerup", pt), requestAnimationFrame(() => K.current());
      };
      X.addEventListener("pointermove", ut), X.addEventListener("pointerup", pt);
    },
    [n, t.id, t.x, t.y, t.w, t.h]
  ), ot = st(
    (P) => {
      if (!P.altKey) {
        if (b) {
          P.stopPropagation();
          return;
        }
        if (e) {
          Z(P);
          return;
        }
        Z(P);
      }
    },
    [b, e, Z, n, t.id]
  ), Q = st(
    (P) => {
      if (P.stopPropagation(), !b) {
        if (t.groupId) {
          const G = [];
          let X = t.groupId;
          for (; X; )
            G.push(X), X = n.groupParent.get(X);
          if (!n.activeGroupId) {
            n.enterGroup(G[G.length - 1]), n.select(t.id);
            return;
          }
          const N = G.indexOf(n.activeGroupId);
          if (N > 0) {
            n.enterGroup(G[N - 1]), n.select(t.id);
            return;
          }
        }
        n.select(t.id), M.current = { x: P.clientX, y: P.clientY }, g(!0);
      }
    },
    [b, n, t.id, t.groupId, z]
  ), $ = e && !o;
  return /* @__PURE__ */ k(
    "div",
    {
      ref: c,
      "data-node-id": t.id,
      className: s ? void 0 : "sb-block-inert",
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        zIndex: t.z,
        height: t.h === "auto" ? void 0 : t.h,
        minHeight: 20,
        border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : "none",
        boxSizing: t.data.borderColor ? "border-box" : void 0,
        outline: e ? `${1.5 / i}px dashed #3b82f6` : "none",
        outlineOffset: t.data.borderColor ? 2 / i : 0,
        borderRadius: t.data.edgeStyle === "round" ? 12 : 0,
        background: "transparent",
        boxShadow: "none",
        overflow: "visible",
        pointerEvents: s ? "auto" : "none",
        display: "flex",
        flexDirection: "column",
        opacity: t.data.opacity ?? 1,
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0
      },
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            onDoubleClick: Q,
            style: {
              overflow: "hidden",
              height: "100%",
              display: "flex",
              flexDirection: "column",
              borderRadius: t.data.borderColor || e ? 7 : 0
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                className: "sb-editor-wrap",
                onPointerDown: ot,
                onKeyDown: b ? (P) => {
                  P.key === "Escape" && (P.stopPropagation(), g(!1));
                } : void 0,
                style: b ? { cursor: "text", userSelect: "text" } : { cursor: "move", userSelect: "none" },
                children: x ? /* @__PURE__ */ u(Ls, { markdown: t.data.markdown ?? "" }) : /* @__PURE__ */ u(rc, { fallback: /* @__PURE__ */ u(Ls, { markdown: t.data.markdown ?? "" }), children: /* @__PURE__ */ u(
                  sl,
                  {
                    editor: z,
                    theme: "light",
                    editable: s && b
                  }
                ) })
              }
            )
          }
        ),
        $ && ic.map(({ pos: P, top: G, left: X }) => {
          const N = 8 / i;
          return /* @__PURE__ */ u(
            "div",
            {
              onPointerDown: (j) => O(P, j),
              style: {
                position: "absolute",
                top: G,
                left: X,
                width: N,
                height: N,
                transform: "translate(-50%, -50%)",
                background: "white",
                border: `${1.5 / i}px solid #3b82f6`,
                cursor: On(P, t.rotation || 0),
                zIndex: 10,
                pointerEvents: "auto"
              }
            },
            P
          );
        }),
        $ && (() => {
          const P = 25 / i, G = 10 / i;
          return /* @__PURE__ */ k(ft, { children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  position: "absolute",
                  top: -P,
                  left: "50%",
                  width: 1.5 / i,
                  height: P,
                  transform: "translateX(-50%)",
                  background: "#3b82f6",
                  pointerEvents: "none"
                }
              }
            ),
            /* @__PURE__ */ u(
              "div",
              {
                onPointerDown: lt,
                style: {
                  position: "absolute",
                  top: -(P + G / 2),
                  left: "50%",
                  width: G,
                  height: G,
                  transform: "translateX(-50%) rotate(45deg)",
                  borderRadius: 1.5 / i,
                  background: "white",
                  border: `${1.5 / i}px solid #3b82f6`,
                  cursor: "grab",
                  zIndex: 10,
                  pointerEvents: "auto"
                }
              }
            )
          ] });
        })()
      ]
    }
  );
}
const Bi = ue(lc);
function cc(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Bi,
    {
      node: e,
      isSelected: t.isSelected,
      multiSelected: t.multiSelected,
      engine: t.engine,
      schema: Xr,
      interactive: t.interactive,
      zoom: t.zoom,
      onMeasuredHeight: t.callbacks.onMeasuredHeight
    }
  );
}
const dc = {
  type: "content",
  component: cc,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.markdown || null
}, { PI: hc } = Math, sn = hc + 1e-4, Es = 0.5, Ds = [1, 1];
function Ws(t, e, o, n = (r) => r) {
  return t * n(0.5 - e * (0.5 - o));
}
const { min: nr } = Math;
function Ni(t, e, o) {
  let n = nr(1, e / o);
  return nr(1, t + (nr(1, 1 - n) - t) * (n * 0.275));
}
function uc(t) {
  return [-t[0], -t[1]];
}
function Pe(t, e) {
  return [t[0] + e[0], t[1] + e[1]];
}
function Fs(t, e, o) {
  return t[0] = e[0] + o[0], t[1] = e[1] + o[1], t;
}
function Ue(t, e) {
  return [t[0] - e[0], t[1] - e[1]];
}
function zr(t, e, o) {
  return t[0] = e[0] - o[0], t[1] = e[1] - o[1], t;
}
function qe(t, e) {
  return [t[0] * e, t[1] * e];
}
function rr(t, e, o) {
  return t[0] = e[0] * o, t[1] = e[1] * o, t;
}
function pc(t, e) {
  return [t[0] / e, t[1] / e];
}
function Hi(t) {
  return [t[1], -t[0]];
}
function sr(t, e) {
  let o = e[0];
  return t[0] = e[1], t[1] = -o, t;
}
function Bs(t, e) {
  return t[0] * e[0] + t[1] * e[1];
}
function fc(t, e) {
  return t[0] === e[0] && t[1] === e[1];
}
function yc(t) {
  return Math.hypot(t[0], t[1]);
}
function Ns(t, e) {
  let o = t[0] - e[0], n = t[1] - e[1];
  return o * o + n * n;
}
function Oi(t) {
  return pc(t, yc(t));
}
function mc(t, e) {
  return Math.hypot(t[1] - e[1], t[0] - e[0]);
}
function qr(t, e, o) {
  let n = Math.sin(o), r = Math.cos(o), s = t[0] - e[0], i = t[1] - e[1], l = s * r - i * n, a = s * n + i * r;
  return [l + e[0], a + e[1]];
}
function Hs(t, e, o, n) {
  let r = Math.sin(n), s = Math.cos(n), i = e[0] - o[0], l = e[1] - o[1], a = i * s - l * r, c = i * r + l * s;
  return t[0] = a + o[0], t[1] = c + o[1], t;
}
function Os(t, e, o) {
  return Pe(t, qe(Ue(e, t), o));
}
function bc(t, e, o, n) {
  let r = o[0] - e[0], s = o[1] - e[1];
  return t[0] = e[0] + r * n, t[1] = e[1] + s * n, t;
}
function Xi(t, e, o) {
  return Pe(t, qe(e, o));
}
const ae = [0, 0], je = [0, 0], Ve = [0, 0];
function gc(t, e) {
  let o = Xi(t, Oi(Hi(Ue(t, Pe(t, [1, 1])))), -e), n = [], r = 1 / 13;
  for (let s = r; s <= 1; s += r) n.push(qr(o, t, sn * 2 * s));
  return n;
}
function xc(t, e, o) {
  let n = [], r = 1 / o;
  for (let s = r; s <= 1; s += r) n.push(qr(e, t, sn * s));
  return n;
}
function wc(t, e, o) {
  let n = Ue(e, o), r = qe(n, 0.5), s = qe(n, 0.51);
  return [Ue(t, r), Ue(t, s), Pe(t, s), Pe(t, r)];
}
function kc(t, e, o, n) {
  let r = [], s = Xi(t, e, o), i = 1 / n;
  for (let l = i; l < 1; l += i) r.push(qr(s, t, sn * 3 * l));
  return r;
}
function vc(t, e, o) {
  return [Pe(t, qe(e, o)), Pe(t, qe(e, o * 0.99)), Ue(t, qe(e, o * 0.99)), Ue(t, qe(e, o))];
}
function Xs(t, e, o) {
  return t === !1 || t === void 0 ? 0 : t === !0 ? Math.max(e, o) : t;
}
function Sc(t, e, o) {
  return t.slice(0, 10).reduce((n, r) => {
    let s = r.pressure;
    return e && (s = Ni(n, r.distance, o)), (n + s) / 2;
  }, t[0].pressure);
}
function Mc(t, e = {}) {
  let { size: o = 16, smoothing: n = 0.5, thinning: r = 0.5, simulatePressure: s = !0, easing: i = (G) => G, start: l = {}, end: a = {}, last: c = !1 } = e, { cap: h = !0, easing: p = (G) => G * (2 - G) } = l, { cap: d = !0, easing: f = (G) => --G * G * G + 1 } = a;
  if (t.length === 0 || o <= 0) return [];
  let m = t[t.length - 1].runningLength, y = Xs(l.taper, o, m), b = Xs(a.taper, o, m), g = (o * n) ** 2, x = [], S = [], M = Sc(t, s, o), z = Ws(o, r, t[t.length - 1].pressure, i), W, I = t[0].vector, R = t[0].point, K = R, Z = R, lt = K, O = !1;
  for (let G = 0; G < t.length; G++) {
    let { pressure: X } = t[G], { point: N, vector: j, distance: H, runningLength: _ } = t[G], tt = G === t.length - 1;
    if (!tt && m - _ < 3) continue;
    r ? (s && (X = Ni(M, H, o)), z = Ws(o, r, X, i)) : z = o / 2, W === void 0 && (W = z);
    let nt = _ < y ? p(_ / y) : 1, ut = m - _ < b ? f((m - _) / b) : 1;
    z = Math.max(0.01, z * Math.min(nt, ut));
    let pt = (tt ? t[G] : t[G + 1]).vector, vt = tt ? 1 : Bs(j, pt), rt = Bs(j, I) < 0 && !O, gt = vt !== null && vt < 0;
    if (rt || gt) {
      sr(ae, I), rr(ae, ae, z);
      for (let xt = 0; xt <= 1; xt += 0.07692307692307693) zr(je, N, ae), Hs(je, je, N, sn * xt), Z = [je[0], je[1]], x.push(Z), Fs(Ve, N, ae), Hs(Ve, Ve, N, sn * -xt), lt = [Ve[0], Ve[1]], S.push(lt);
      R = Z, K = lt, gt && (O = !0);
      continue;
    }
    if (O = !1, tt) {
      sr(ae, j), rr(ae, ae, z), x.push(Ue(N, ae)), S.push(Pe(N, ae));
      continue;
    }
    bc(ae, pt, j, vt), sr(ae, ae), rr(ae, ae, z), zr(je, N, ae), Z = [je[0], je[1]], (G <= 1 || Ns(R, Z) > g) && (x.push(Z), R = Z), Fs(Ve, N, ae), lt = [Ve[0], Ve[1]], (G <= 1 || Ns(K, lt) > g) && (S.push(lt), K = lt), M = X, I = j;
  }
  let ot = [t[0].point[0], t[0].point[1]], Q = t.length > 1 ? [t[t.length - 1].point[0], t[t.length - 1].point[1]] : Pe(t[0].point, [1, 1]), $ = [], P = [];
  if (t.length === 1) {
    if (!(y || b) || c) return gc(ot, W || z);
  } else {
    y || b && t.length === 1 || (h ? $.push(...xc(ot, S[0], 13)) : $.push(...wc(ot, x[0], S[0])));
    let G = Hi(uc(t[t.length - 1].vector));
    b || y && t.length === 1 ? P.push(Q) : d ? P.push(...kc(Q, G, z, 29)) : P.push(...vc(Q, G, z));
  }
  return x.concat(P, S.reverse(), $);
}
const Ys = [0, 0];
function Gs(t) {
  return t != null && t >= 0;
}
function Cc(t, e = {}) {
  var d;
  let { streamline: o = 0.5, size: n = 16, last: r = !1 } = e;
  if (t.length === 0) return [];
  let s = 0.15 + (1 - o) * 0.85, i = Array.isArray(t[0]) ? t : t.map(({ x: f, y: m, pressure: y = Es }) => [f, m, y]);
  if (i.length === 2) {
    let f = i[1];
    i = i.slice(0, -1);
    for (let m = 1; m < 5; m++) i.push(Os(i[0], f, m / 4));
  }
  i.length === 1 && (i = [...i, [...Pe(i[0], Ds), ...i[0].slice(2)]]);
  let l = [{ point: [i[0][0], i[0][1]], pressure: Gs(i[0][2]) ? i[0][2] : 0.25, vector: [...Ds], distance: 0, runningLength: 0 }], a = !1, c = 0, h = l[0], p = i.length - 1;
  for (let f = 1; f < i.length; f++) {
    let m = r && f === p ? [i[f][0], i[f][1]] : Os(h.point, i[f], s);
    if (fc(h.point, m)) continue;
    let y = mc(m, h.point);
    if (c += y, f < p && !a) {
      if (c < n) continue;
      a = !0;
    }
    zr(Ys, h.point, m), h = { point: m, pressure: Gs(i[f][2]) ? i[f][2] : Es, vector: Oi(Ys), distance: y, runningLength: c }, l.push(h);
  }
  return l[0].vector = ((d = l[1]) == null ? void 0 : d.vector) || [0, 0], l;
}
function zc(t, e = {}) {
  return Mc(Cc(t, e), e);
}
var Ic = zc;
function Zr(t, e = {}) {
  const o = Ic(t, {
    size: e.size || 4,
    thinning: e.thinning ?? 0.5,
    smoothing: e.smoothing ?? 0.5,
    streamline: e.streamline ?? 0.5
  });
  return Tc(o);
}
function Tc(t) {
  if (!t.length) return "";
  const e = [], [o, n] = t[0];
  e.push("M", o, n);
  for (let r = 0; r < t.length; r++) {
    const [s, i] = t[r], [l, a] = t[(r + 1) % t.length];
    e.push("Q", s, i, (s + l) / 2, (i + a) / 2);
  }
  return e.push("Z"), e.join(" ");
}
function Yi(t, e = 0.5) {
  if (t.length < 2) return t;
  const o = 0.15 + (1 - e) * 0.85, n = [[t[0][0], t[0][1]]];
  for (let r = 1; r < t.length; r++) {
    const s = n[r - 1];
    n.push([
      s[0] + (t[r][0] - s[0]) * o,
      s[1] + (t[r][1] - s[1]) * o
    ]);
  }
  return n;
}
function Pc(t, e = 0.5) {
  if (t.length < 2) return "";
  const o = Yi(t, e), n = o.length, r = [];
  r.push("M", o[0][0], o[0][1]);
  for (let s = 0; s < n; s++) {
    const [i, l] = o[s], [a, c] = o[(s + 1) % n];
    r.push("Q", i, l, (i + a) / 2, (l + c) / 2);
  }
  return r.push("Z"), r.join(" ");
}
function Rc(t, e, o, n) {
  const r = e[0] - t[0], s = e[1] - t[1], i = n[0] - o[0], l = n[1] - o[1], a = r * l - s * i;
  if (Math.abs(a) < 1e-10) return null;
  const c = ((o[0] - t[0]) * l - (o[1] - t[1]) * i) / a, h = ((o[0] - t[0]) * s - (o[1] - t[1]) * r) / a;
  return c <= 0 || c >= 1 || h <= 0 || h >= 1 ? null : [t[0] + c * r, t[1] + c * s];
}
function Ac(t) {
  if (t.length < 2) return "";
  let e = `M ${t[0][0]},${t[0][1]}`;
  for (let o = 1; o < t.length; o++)
    e += ` L ${t[o][0]},${t[o][1]}`;
  return e + " Z";
}
function js(t) {
  let e = 0;
  for (let o = 0, n = t.length - 1; o < t.length; n = o++)
    e += (t[n][0] + t[o][0]) * (t[n][1] - t[o][1]);
  return Math.abs(e) / 2;
}
function Lc(t) {
  if (t.length < 4) return [];
  const e = t.length, o = [];
  for (let i = 0; i < e - 1; i++)
    for (let l = i + 2; l < e - 1; l++) {
      const a = Rc(
        t[i],
        t[i + 1],
        t[l],
        t[l + 1]
      );
      if (!a) continue;
      const c = [a];
      for (let h = i + 1; h <= l; h++)
        c.push(t[h]);
      js(c) < 100 || o.push({
        pathD: Ac(c),
        points: c.map((h) => [h[0], h[1]])
      });
    }
  if (o.length === 0) return [];
  const n = o.map((i) => js(i.points)), s = Math.max(...n) * 0.05;
  return o.filter((i, l) => n[l] >= s);
}
function ir(t, e, o) {
  if (t && t.length) {
    const [n, r] = e, s = Math.PI / 180 * o, i = Math.cos(s), l = Math.sin(s);
    for (const a of t) {
      const [c, h] = a;
      a[0] = (c - n) * i - (h - r) * l + n, a[1] = (c - n) * l + (h - r) * i + r;
    }
  }
}
function Ec(t, e) {
  return t[0] === e[0] && t[1] === e[1];
}
function Dc(t, e, o, n = 1) {
  const r = o, s = Math.max(e, 0.1), i = t[0] && t[0][0] && typeof t[0][0] == "number" ? [t] : t, l = [0, 0];
  if (r) for (const c of i) ir(c, l, r);
  const a = function(c, h, p) {
    const d = [];
    for (const x of c) {
      const S = [...x];
      Ec(S[0], S[S.length - 1]) || S.push([S[0][0], S[0][1]]), S.length > 2 && d.push(S);
    }
    const f = [];
    h = Math.max(h, 0.1);
    const m = [];
    for (const x of d) for (let S = 0; S < x.length - 1; S++) {
      const M = x[S], z = x[S + 1];
      if (M[1] !== z[1]) {
        const W = Math.min(M[1], z[1]);
        m.push({ ymin: W, ymax: Math.max(M[1], z[1]), x: W === M[1] ? M[0] : z[0], islope: (z[0] - M[0]) / (z[1] - M[1]) });
      }
    }
    if (m.sort((x, S) => x.ymin < S.ymin ? -1 : x.ymin > S.ymin ? 1 : x.x < S.x ? -1 : x.x > S.x ? 1 : x.ymax === S.ymax ? 0 : (x.ymax - S.ymax) / Math.abs(x.ymax - S.ymax)), !m.length) return f;
    let y = [], b = m[0].ymin, g = 0;
    for (; y.length || m.length; ) {
      if (m.length) {
        let x = -1;
        for (let S = 0; S < m.length && !(m[S].ymin > b); S++) x = S;
        m.splice(0, x + 1).forEach((S) => {
          y.push({ s: b, edge: S });
        });
      }
      if (y = y.filter((x) => !(x.edge.ymax <= b)), y.sort((x, S) => x.edge.x === S.edge.x ? 0 : (x.edge.x - S.edge.x) / Math.abs(x.edge.x - S.edge.x)), (p !== 1 || g % h == 0) && y.length > 1) for (let x = 0; x < y.length; x += 2) {
        const S = x + 1;
        if (S >= y.length) break;
        const M = y[x].edge, z = y[S].edge;
        f.push([[Math.round(M.x), b], [Math.round(z.x), b]]);
      }
      b += p, y.forEach((x) => {
        x.edge.x = x.edge.x + p * x.edge.islope;
      }), g++;
    }
    return f;
  }(i, s, n);
  if (r) {
    for (const c of i) ir(c, l, -r);
    (function(c, h, p) {
      const d = [];
      c.forEach((f) => d.push(...f)), ir(d, h, p);
    })(a, l, -r);
  }
  return a;
}
function an(t, e) {
  var o;
  const n = e.hachureAngle + 90;
  let r = e.hachureGap;
  r < 0 && (r = 4 * e.strokeWidth), r = Math.round(Math.max(r, 0.1));
  let s = 1;
  return e.roughness >= 1 && (((o = e.randomizer) === null || o === void 0 ? void 0 : o.next()) || Math.random()) > 0.7 && (s = r), Dc(t, r, n, s || 1);
}
class Ur {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    return this._fillPolygons(e, o);
  }
  _fillPolygons(e, o) {
    const n = an(e, o);
    return { type: "fillSketch", ops: this.renderLines(n, o) };
  }
  renderLines(e, o) {
    const n = [];
    for (const r of e) n.push(...this.helper.doubleLineOps(r[0][0], r[0][1], r[1][0], r[1][1], o));
    return n;
  }
}
function Xn(t) {
  const e = t[0], o = t[1];
  return Math.sqrt(Math.pow(e[0] - o[0], 2) + Math.pow(e[1] - o[1], 2));
}
class Wc extends Ur {
  fillPolygons(e, o) {
    let n = o.hachureGap;
    n < 0 && (n = 4 * o.strokeWidth), n = Math.max(n, 0.1);
    const r = an(e, Object.assign({}, o, { hachureGap: n })), s = Math.PI / 180 * o.hachureAngle, i = [], l = 0.5 * n * Math.cos(s), a = 0.5 * n * Math.sin(s);
    for (const [c, h] of r) Xn([c, h]) && i.push([[c[0] - l, c[1] + a], [...h]], [[c[0] + l, c[1] - a], [...h]]);
    return { type: "fillSketch", ops: this.renderLines(i, o) };
  }
}
class Fc extends Ur {
  fillPolygons(e, o) {
    const n = this._fillPolygons(e, o), r = Object.assign({}, o, { hachureAngle: o.hachureAngle + 90 }), s = this._fillPolygons(e, r);
    return n.ops = n.ops.concat(s.ops), n;
  }
}
class Bc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const n = an(e, o = Object.assign({}, o, { hachureAngle: 0 }));
    return this.dotsOnLines(n, o);
  }
  dotsOnLines(e, o) {
    const n = [];
    let r = o.hachureGap;
    r < 0 && (r = 4 * o.strokeWidth), r = Math.max(r, 0.1);
    let s = o.fillWeight;
    s < 0 && (s = o.strokeWidth / 2);
    const i = r / 4;
    for (const l of e) {
      const a = Xn(l), c = a / r, h = Math.ceil(c) - 1, p = a - h * r, d = (l[0][0] + l[1][0]) / 2 - r / 4, f = Math.min(l[0][1], l[1][1]);
      for (let m = 0; m < h; m++) {
        const y = f + p + m * r, b = d - i + 2 * Math.random() * i, g = y - i + 2 * Math.random() * i, x = this.helper.ellipse(b, g, s, s, o);
        n.push(...x.ops);
      }
    }
    return { type: "fillSketch", ops: n };
  }
}
class Nc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const n = an(e, o);
    return { type: "fillSketch", ops: this.dashedLine(n, o) };
  }
  dashedLine(e, o) {
    const n = o.dashOffset < 0 ? o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap : o.dashOffset, r = o.dashGap < 0 ? o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap : o.dashGap, s = [];
    return e.forEach((i) => {
      const l = Xn(i), a = Math.floor(l / (n + r)), c = (l + r - a * (n + r)) / 2;
      let h = i[0], p = i[1];
      h[0] > p[0] && (h = i[1], p = i[0]);
      const d = Math.atan((p[1] - h[1]) / (p[0] - h[0]));
      for (let f = 0; f < a; f++) {
        const m = f * (n + r), y = m + n, b = [h[0] + m * Math.cos(d) + c * Math.cos(d), h[1] + m * Math.sin(d) + c * Math.sin(d)], g = [h[0] + y * Math.cos(d) + c * Math.cos(d), h[1] + y * Math.sin(d) + c * Math.sin(d)];
        s.push(...this.helper.doubleLineOps(b[0], b[1], g[0], g[1], o));
      }
    }), s;
  }
}
class Hc {
  constructor(e) {
    this.helper = e;
  }
  fillPolygons(e, o) {
    const n = o.hachureGap < 0 ? 4 * o.strokeWidth : o.hachureGap, r = o.zigzagOffset < 0 ? n : o.zigzagOffset, s = an(e, o = Object.assign({}, o, { hachureGap: n + r }));
    return { type: "fillSketch", ops: this.zigzagLines(s, r, o) };
  }
  zigzagLines(e, o, n) {
    const r = [];
    return e.forEach((s) => {
      const i = Xn(s), l = Math.round(i / (2 * o));
      let a = s[0], c = s[1];
      a[0] > c[0] && (a = s[1], c = s[0]);
      const h = Math.atan((c[1] - a[1]) / (c[0] - a[0]));
      for (let p = 0; p < l; p++) {
        const d = 2 * p * o, f = 2 * (p + 1) * o, m = Math.sqrt(2 * Math.pow(o, 2)), y = [a[0] + d * Math.cos(h), a[1] + d * Math.sin(h)], b = [a[0] + f * Math.cos(h), a[1] + f * Math.sin(h)], g = [y[0] + m * Math.cos(h + Math.PI / 4), y[1] + m * Math.sin(h + Math.PI / 4)];
        r.push(...this.helper.doubleLineOps(y[0], y[1], g[0], g[1], n), ...this.helper.doubleLineOps(g[0], g[1], b[0], b[1], n));
      }
    }), r;
  }
}
const de = {};
class Oc {
  constructor(e) {
    this.seed = e;
  }
  next() {
    return this.seed ? (2 ** 31 - 1 & (this.seed = Math.imul(48271, this.seed))) / 2 ** 31 : Math.random();
  }
}
const Xc = 0, ar = 1, Vs = 2, gn = { A: 7, a: 7, C: 6, c: 6, H: 1, h: 1, L: 2, l: 2, M: 2, m: 2, Q: 4, q: 4, S: 4, s: 4, T: 2, t: 2, V: 1, v: 1, Z: 0, z: 0 };
function lr(t, e) {
  return t.type === e;
}
function Qr(t) {
  const e = [], o = function(i) {
    const l = new Array();
    for (; i !== ""; ) if (i.match(/^([ \t\r\n,]+)/)) i = i.substr(RegExp.$1.length);
    else if (i.match(/^([aAcChHlLmMqQsStTvVzZ])/)) l[l.length] = { type: Xc, text: RegExp.$1 }, i = i.substr(RegExp.$1.length);
    else {
      if (!i.match(/^(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)/)) return [];
      l[l.length] = { type: ar, text: `${parseFloat(RegExp.$1)}` }, i = i.substr(RegExp.$1.length);
    }
    return l[l.length] = { type: Vs, text: "" }, l;
  }(t);
  let n = "BOD", r = 0, s = o[r];
  for (; !lr(s, Vs); ) {
    let i = 0;
    const l = [];
    if (n === "BOD") {
      if (s.text !== "M" && s.text !== "m") return Qr("M0,0" + t);
      r++, i = gn[s.text], n = s.text;
    } else lr(s, ar) ? i = gn[n] : (r++, i = gn[s.text], n = s.text);
    if (!(r + i < o.length)) throw new Error("Path data ended short");
    for (let a = r; a < r + i; a++) {
      const c = o[a];
      if (!lr(c, ar)) throw new Error("Param not a number: " + n + "," + c.text);
      l[l.length] = +c.text;
    }
    if (typeof gn[n] != "number") throw new Error("Bad segment: " + n);
    {
      const a = { key: n, data: l };
      e.push(a), r += i, s = o[r], n === "M" && (n = "L"), n === "m" && (n = "l");
    }
  }
  return e;
}
function Gi(t) {
  let e = 0, o = 0, n = 0, r = 0;
  const s = [];
  for (const { key: i, data: l } of t) switch (i) {
    case "M":
      s.push({ key: "M", data: [...l] }), [e, o] = l, [n, r] = l;
      break;
    case "m":
      e += l[0], o += l[1], s.push({ key: "M", data: [e, o] }), n = e, r = o;
      break;
    case "L":
      s.push({ key: "L", data: [...l] }), [e, o] = l;
      break;
    case "l":
      e += l[0], o += l[1], s.push({ key: "L", data: [e, o] });
      break;
    case "C":
      s.push({ key: "C", data: [...l] }), e = l[4], o = l[5];
      break;
    case "c": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "C", data: a }), e = a[4], o = a[5];
      break;
    }
    case "Q":
      s.push({ key: "Q", data: [...l] }), e = l[2], o = l[3];
      break;
    case "q": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "Q", data: a }), e = a[2], o = a[3];
      break;
    }
    case "A":
      s.push({ key: "A", data: [...l] }), e = l[5], o = l[6];
      break;
    case "a":
      e += l[5], o += l[6], s.push({ key: "A", data: [l[0], l[1], l[2], l[3], l[4], e, o] });
      break;
    case "H":
      s.push({ key: "H", data: [...l] }), e = l[0];
      break;
    case "h":
      e += l[0], s.push({ key: "H", data: [e] });
      break;
    case "V":
      s.push({ key: "V", data: [...l] }), o = l[0];
      break;
    case "v":
      o += l[0], s.push({ key: "V", data: [o] });
      break;
    case "S":
      s.push({ key: "S", data: [...l] }), e = l[2], o = l[3];
      break;
    case "s": {
      const a = l.map((c, h) => h % 2 ? c + o : c + e);
      s.push({ key: "S", data: a }), e = a[2], o = a[3];
      break;
    }
    case "T":
      s.push({ key: "T", data: [...l] }), e = l[0], o = l[1];
      break;
    case "t":
      e += l[0], o += l[1], s.push({ key: "T", data: [e, o] });
      break;
    case "Z":
    case "z":
      s.push({ key: "Z", data: [] }), e = n, o = r;
  }
  return s;
}
function ji(t) {
  const e = [];
  let o = "", n = 0, r = 0, s = 0, i = 0, l = 0, a = 0;
  for (const { key: c, data: h } of t) {
    switch (c) {
      case "M":
        e.push({ key: "M", data: [...h] }), [n, r] = h, [s, i] = h;
        break;
      case "C":
        e.push({ key: "C", data: [...h] }), n = h[4], r = h[5], l = h[2], a = h[3];
        break;
      case "L":
        e.push({ key: "L", data: [...h] }), [n, r] = h;
        break;
      case "H":
        n = h[0], e.push({ key: "L", data: [n, r] });
        break;
      case "V":
        r = h[0], e.push({ key: "L", data: [n, r] });
        break;
      case "S": {
        let p = 0, d = 0;
        o === "C" || o === "S" ? (p = n + (n - l), d = r + (r - a)) : (p = n, d = r), e.push({ key: "C", data: [p, d, ...h] }), l = h[0], a = h[1], n = h[2], r = h[3];
        break;
      }
      case "T": {
        const [p, d] = h;
        let f = 0, m = 0;
        o === "Q" || o === "T" ? (f = n + (n - l), m = r + (r - a)) : (f = n, m = r);
        const y = n + 2 * (f - n) / 3, b = r + 2 * (m - r) / 3, g = p + 2 * (f - p) / 3, x = d + 2 * (m - d) / 3;
        e.push({ key: "C", data: [y, b, g, x, p, d] }), l = f, a = m, n = p, r = d;
        break;
      }
      case "Q": {
        const [p, d, f, m] = h, y = n + 2 * (p - n) / 3, b = r + 2 * (d - r) / 3, g = f + 2 * (p - f) / 3, x = m + 2 * (d - m) / 3;
        e.push({ key: "C", data: [y, b, g, x, f, m] }), l = p, a = d, n = f, r = m;
        break;
      }
      case "A": {
        const p = Math.abs(h[0]), d = Math.abs(h[1]), f = h[2], m = h[3], y = h[4], b = h[5], g = h[6];
        p === 0 || d === 0 ? (e.push({ key: "C", data: [n, r, b, g, b, g] }), n = b, r = g) : (n !== b || r !== g) && (Vi(n, r, b, g, p, d, f, m, y).forEach(function(x) {
          e.push({ key: "C", data: x });
        }), n = b, r = g);
        break;
      }
      case "Z":
        e.push({ key: "Z", data: [] }), n = s, r = i;
    }
    o = c;
  }
  return e;
}
function $o(t, e, o) {
  return [t * Math.cos(o) - e * Math.sin(o), t * Math.sin(o) + e * Math.cos(o)];
}
function Vi(t, e, o, n, r, s, i, l, a, c) {
  const h = (p = i, Math.PI * p / 180);
  var p;
  let d = [], f = 0, m = 0, y = 0, b = 0;
  if (c) [f, m, y, b] = c;
  else {
    [t, e] = $o(t, e, -h), [o, n] = $o(o, n, -h);
    const ot = (t - o) / 2, Q = (e - n) / 2;
    let $ = ot * ot / (r * r) + Q * Q / (s * s);
    $ > 1 && ($ = Math.sqrt($), r *= $, s *= $);
    const P = r * r, G = s * s, X = P * G - P * Q * Q - G * ot * ot, N = P * Q * Q + G * ot * ot, j = (l === a ? -1 : 1) * Math.sqrt(Math.abs(X / N));
    y = j * r * Q / s + (t + o) / 2, b = j * -s * ot / r + (e + n) / 2, f = Math.asin(parseFloat(((e - b) / s).toFixed(9))), m = Math.asin(parseFloat(((n - b) / s).toFixed(9))), t < y && (f = Math.PI - f), o < y && (m = Math.PI - m), f < 0 && (f = 2 * Math.PI + f), m < 0 && (m = 2 * Math.PI + m), a && f > m && (f -= 2 * Math.PI), !a && m > f && (m -= 2 * Math.PI);
  }
  let g = m - f;
  if (Math.abs(g) > 120 * Math.PI / 180) {
    const ot = m, Q = o, $ = n;
    m = a && m > f ? f + 120 * Math.PI / 180 * 1 : f + 120 * Math.PI / 180 * -1, d = Vi(o = y + r * Math.cos(m), n = b + s * Math.sin(m), Q, $, r, s, i, 0, a, [m, ot, y, b]);
  }
  g = m - f;
  const x = Math.cos(f), S = Math.sin(f), M = Math.cos(m), z = Math.sin(m), W = Math.tan(g / 4), I = 4 / 3 * r * W, R = 4 / 3 * s * W, K = [t, e], Z = [t + I * S, e - R * x], lt = [o + I * z, n - R * M], O = [o, n];
  if (Z[0] = 2 * K[0] - Z[0], Z[1] = 2 * K[1] - Z[1], c) return [Z, lt, O].concat(d);
  {
    d = [Z, lt, O].concat(d);
    const ot = [];
    for (let Q = 0; Q < d.length; Q += 3) {
      const $ = $o(d[Q][0], d[Q][1], h), P = $o(d[Q + 1][0], d[Q + 1][1], h), G = $o(d[Q + 2][0], d[Q + 2][1], h);
      ot.push([$[0], $[1], P[0], P[1], G[0], G[1]]);
    }
    return ot;
  }
}
const Yc = { randOffset: function(t, e) {
  return At(t, e);
}, randOffsetWithRange: function(t, e, o) {
  return An(t, e, o);
}, ellipse: function(t, e, o, n, r) {
  const s = qi(o, n, r);
  return Ir(t, e, r, s).opset;
}, doubleLineOps: function(t, e, o, n, r) {
  return $e(t, e, o, n, r, !0);
} };
function Ki(t, e, o, n, r) {
  return { type: "path", ops: $e(t, e, o, n, r) };
}
function zn(t, e, o) {
  const n = (t || []).length;
  if (n > 2) {
    const r = [];
    for (let s = 0; s < n - 1; s++) r.push(...$e(t[s][0], t[s][1], t[s + 1][0], t[s + 1][1], o));
    return e && r.push(...$e(t[n - 1][0], t[n - 1][1], t[0][0], t[0][1], o)), { type: "path", ops: r };
  }
  return n === 2 ? Ki(t[0][0], t[0][1], t[1][0], t[1][1], o) : { type: "path", ops: [] };
}
function Gc(t, e, o, n, r) {
  return function(s, i) {
    return zn(s, !0, i);
  }([[t, e], [t + o, e], [t + o, e + n], [t, e + n]], r);
}
function Ks(t, e) {
  if (t.length) {
    const o = typeof t[0][0] == "number" ? [t] : t, n = xn(o[0], 1 * (1 + 0.2 * e.roughness), e), r = e.disableMultiStroke ? [] : xn(o[0], 1.5 * (1 + 0.22 * e.roughness), Us(e));
    for (let s = 1; s < o.length; s++) {
      const i = o[s];
      if (i.length) {
        const l = xn(i, 1 * (1 + 0.2 * e.roughness), e), a = e.disableMultiStroke ? [] : xn(i, 1.5 * (1 + 0.22 * e.roughness), Us(e));
        for (const c of l) c.op !== "move" && n.push(c);
        for (const c of a) c.op !== "move" && r.push(c);
      }
    }
    return { type: "path", ops: n.concat(r) };
  }
  return { type: "path", ops: [] };
}
function qi(t, e, o) {
  const n = Math.sqrt(2 * Math.PI * Math.sqrt((Math.pow(t / 2, 2) + Math.pow(e / 2, 2)) / 2)), r = Math.ceil(Math.max(o.curveStepCount, o.curveStepCount / Math.sqrt(200) * n)), s = 2 * Math.PI / r;
  let i = Math.abs(t / 2), l = Math.abs(e / 2);
  const a = 1 - o.curveFitting;
  return i += At(i * a, o), l += At(l * a, o), { increment: s, rx: i, ry: l };
}
function Ir(t, e, o, n) {
  const [r, s] = Qs(n.increment, t, e, n.rx, n.ry, 1, n.increment * An(0.1, An(0.4, 1, o), o), o);
  let i = Ln(r, null, o);
  if (!o.disableMultiStroke && o.roughness !== 0) {
    const [l] = Qs(n.increment, t, e, n.rx, n.ry, 1.5, 0, o), a = Ln(l, null, o);
    i = i.concat(a);
  }
  return { estimatedPoints: s, opset: { type: "path", ops: i } };
}
function qs(t, e, o, n, r, s, i, l, a) {
  const c = t, h = e;
  let p = Math.abs(o / 2), d = Math.abs(n / 2);
  p += At(0.01 * p, a), d += At(0.01 * d, a);
  let f = r, m = s;
  for (; f < 0; ) f += 2 * Math.PI, m += 2 * Math.PI;
  m - f > 2 * Math.PI && (f = 0, m = 2 * Math.PI);
  const y = 2 * Math.PI / a.curveStepCount, b = Math.min(y / 2, (m - f) / 2), g = Js(b, c, h, p, d, f, m, 1, a);
  if (!a.disableMultiStroke) {
    const x = Js(b, c, h, p, d, f, m, 1.5, a);
    g.push(...x);
  }
  return i && (l ? g.push(...$e(c, h, c + p * Math.cos(f), h + d * Math.sin(f), a), ...$e(c, h, c + p * Math.cos(m), h + d * Math.sin(m), a)) : g.push({ op: "lineTo", data: [c, h] }, { op: "lineTo", data: [c + p * Math.cos(f), h + d * Math.sin(f)] })), { type: "path", ops: g };
}
function Zs(t, e) {
  const o = ji(Gi(Qr(t))), n = [];
  let r = [0, 0], s = [0, 0];
  for (const { key: i, data: l } of o) switch (i) {
    case "M":
      s = [l[0], l[1]], r = [l[0], l[1]];
      break;
    case "L":
      n.push(...$e(s[0], s[1], l[0], l[1], e)), s = [l[0], l[1]];
      break;
    case "C": {
      const [a, c, h, p, d, f] = l;
      n.push(...jc(a, c, h, p, d, f, s, e)), s = [d, f];
      break;
    }
    case "Z":
      n.push(...$e(s[0], s[1], r[0], r[1], e)), s = [r[0], r[1]];
  }
  return { type: "path", ops: n };
}
function cr(t, e) {
  const o = [];
  for (const n of t) if (n.length) {
    const r = e.maxRandomnessOffset || 0, s = n.length;
    if (s > 2) {
      o.push({ op: "move", data: [n[0][0] + At(r, e), n[0][1] + At(r, e)] });
      for (let i = 1; i < s; i++) o.push({ op: "lineTo", data: [n[i][0] + At(r, e), n[i][1] + At(r, e)] });
    }
  }
  return { type: "fillPath", ops: o };
}
function To(t, e) {
  return function(o, n) {
    let r = o.fillStyle || "hachure";
    if (!de[r]) switch (r) {
      case "zigzag":
        de[r] || (de[r] = new Wc(n));
        break;
      case "cross-hatch":
        de[r] || (de[r] = new Fc(n));
        break;
      case "dots":
        de[r] || (de[r] = new Bc(n));
        break;
      case "dashed":
        de[r] || (de[r] = new Nc(n));
        break;
      case "zigzag-line":
        de[r] || (de[r] = new Hc(n));
        break;
      default:
        r = "hachure", de[r] || (de[r] = new Ur(n));
    }
    return de[r];
  }(e, Yc).fillPolygons(t, e);
}
function Us(t) {
  const e = Object.assign({}, t);
  return e.randomizer = void 0, t.seed && (e.seed = t.seed + 1), e;
}
function Zi(t) {
  return t.randomizer || (t.randomizer = new Oc(t.seed || 0)), t.randomizer.next();
}
function An(t, e, o, n = 1) {
  return o.roughness * n * (Zi(o) * (e - t) + t);
}
function At(t, e, o = 1) {
  return An(-t, t, e, o);
}
function $e(t, e, o, n, r, s = !1) {
  const i = s ? r.disableMultiStrokeFill : r.disableMultiStroke, l = Tr(t, e, o, n, r, !0, !1);
  if (i) return l;
  const a = Tr(t, e, o, n, r, !0, !0);
  return l.concat(a);
}
function Tr(t, e, o, n, r, s, i) {
  const l = Math.pow(t - o, 2) + Math.pow(e - n, 2), a = Math.sqrt(l);
  let c = 1;
  c = a < 200 ? 1 : a > 500 ? 0.4 : -16668e-7 * a + 1.233334;
  let h = r.maxRandomnessOffset || 0;
  h * h * 100 > l && (h = a / 10);
  const p = h / 2, d = 0.2 + 0.2 * Zi(r);
  let f = r.bowing * r.maxRandomnessOffset * (n - e) / 200, m = r.bowing * r.maxRandomnessOffset * (t - o) / 200;
  f = At(f, r, c), m = At(m, r, c);
  const y = [], b = () => At(p, r, c), g = () => At(h, r, c), x = r.preserveVertices;
  return i ? y.push({ op: "move", data: [t + (x ? 0 : b()), e + (x ? 0 : b())] }) : y.push({ op: "move", data: [t + (x ? 0 : At(h, r, c)), e + (x ? 0 : At(h, r, c))] }), i ? y.push({ op: "bcurveTo", data: [f + t + (o - t) * d + b(), m + e + (n - e) * d + b(), f + t + 2 * (o - t) * d + b(), m + e + 2 * (n - e) * d + b(), o + (x ? 0 : b()), n + (x ? 0 : b())] }) : y.push({ op: "bcurveTo", data: [f + t + (o - t) * d + g(), m + e + (n - e) * d + g(), f + t + 2 * (o - t) * d + g(), m + e + 2 * (n - e) * d + g(), o + (x ? 0 : g()), n + (x ? 0 : g())] }), y;
}
function xn(t, e, o) {
  if (!t.length) return [];
  const n = [];
  n.push([t[0][0] + At(e, o), t[0][1] + At(e, o)]), n.push([t[0][0] + At(e, o), t[0][1] + At(e, o)]);
  for (let r = 1; r < t.length; r++) n.push([t[r][0] + At(e, o), t[r][1] + At(e, o)]), r === t.length - 1 && n.push([t[r][0] + At(e, o), t[r][1] + At(e, o)]);
  return Ln(n, null, o);
}
function Ln(t, e, o) {
  const n = t.length, r = [];
  if (n > 3) {
    const s = [], i = 1 - o.curveTightness;
    r.push({ op: "move", data: [t[1][0], t[1][1]] });
    for (let l = 1; l + 2 < n; l++) {
      const a = t[l];
      s[0] = [a[0], a[1]], s[1] = [a[0] + (i * t[l + 1][0] - i * t[l - 1][0]) / 6, a[1] + (i * t[l + 1][1] - i * t[l - 1][1]) / 6], s[2] = [t[l + 1][0] + (i * t[l][0] - i * t[l + 2][0]) / 6, t[l + 1][1] + (i * t[l][1] - i * t[l + 2][1]) / 6], s[3] = [t[l + 1][0], t[l + 1][1]], r.push({ op: "bcurveTo", data: [s[1][0], s[1][1], s[2][0], s[2][1], s[3][0], s[3][1]] });
    }
  } else n === 3 ? (r.push({ op: "move", data: [t[1][0], t[1][1]] }), r.push({ op: "bcurveTo", data: [t[1][0], t[1][1], t[2][0], t[2][1], t[2][0], t[2][1]] })) : n === 2 && r.push(...Tr(t[0][0], t[0][1], t[1][0], t[1][1], o, !0, !0));
  return r;
}
function Qs(t, e, o, n, r, s, i, l) {
  const a = [], c = [];
  if (l.roughness === 0) {
    t /= 4, c.push([e + n * Math.cos(-t), o + r * Math.sin(-t)]);
    for (let h = 0; h <= 2 * Math.PI; h += t) {
      const p = [e + n * Math.cos(h), o + r * Math.sin(h)];
      a.push(p), c.push(p);
    }
    c.push([e + n * Math.cos(0), o + r * Math.sin(0)]), c.push([e + n * Math.cos(t), o + r * Math.sin(t)]);
  } else {
    const h = At(0.5, l) - Math.PI / 2;
    c.push([At(s, l) + e + 0.9 * n * Math.cos(h - t), At(s, l) + o + 0.9 * r * Math.sin(h - t)]);
    const p = 2 * Math.PI + h - 0.01;
    for (let d = h; d < p; d += t) {
      const f = [At(s, l) + e + n * Math.cos(d), At(s, l) + o + r * Math.sin(d)];
      a.push(f), c.push(f);
    }
    c.push([At(s, l) + e + n * Math.cos(h + 2 * Math.PI + 0.5 * i), At(s, l) + o + r * Math.sin(h + 2 * Math.PI + 0.5 * i)]), c.push([At(s, l) + e + 0.98 * n * Math.cos(h + i), At(s, l) + o + 0.98 * r * Math.sin(h + i)]), c.push([At(s, l) + e + 0.9 * n * Math.cos(h + 0.5 * i), At(s, l) + o + 0.9 * r * Math.sin(h + 0.5 * i)]);
  }
  return [c, a];
}
function Js(t, e, o, n, r, s, i, l, a) {
  const c = s + At(0.1, a), h = [];
  h.push([At(l, a) + e + 0.9 * n * Math.cos(c - t), At(l, a) + o + 0.9 * r * Math.sin(c - t)]);
  for (let p = c; p <= i; p += t) h.push([At(l, a) + e + n * Math.cos(p), At(l, a) + o + r * Math.sin(p)]);
  return h.push([e + n * Math.cos(i), o + r * Math.sin(i)]), h.push([e + n * Math.cos(i), o + r * Math.sin(i)]), Ln(h, null, a);
}
function jc(t, e, o, n, r, s, i, l) {
  const a = [], c = [l.maxRandomnessOffset || 1, (l.maxRandomnessOffset || 1) + 0.3];
  let h = [0, 0];
  const p = l.disableMultiStroke ? 1 : 2, d = l.preserveVertices;
  for (let f = 0; f < p; f++) f === 0 ? a.push({ op: "move", data: [i[0], i[1]] }) : a.push({ op: "move", data: [i[0] + (d ? 0 : At(c[0], l)), i[1] + (d ? 0 : At(c[0], l))] }), h = d ? [r, s] : [r + At(c[f], l), s + At(c[f], l)], a.push({ op: "bcurveTo", data: [t + At(c[f], l), e + At(c[f], l), o + At(c[f], l), n + At(c[f], l), h[0], h[1]] });
  return a;
}
function _o(t) {
  return [...t];
}
function $s(t, e = 0) {
  const o = t.length;
  if (o < 3) throw new Error("A curve must have at least three points.");
  const n = [];
  if (o === 3) n.push(_o(t[0]), _o(t[1]), _o(t[2]), _o(t[2]));
  else {
    const r = [];
    r.push(t[0], t[0]);
    for (let l = 1; l < t.length; l++) r.push(t[l]), l === t.length - 1 && r.push(t[l]);
    const s = [], i = 1 - e;
    n.push(_o(r[0]));
    for (let l = 1; l + 2 < r.length; l++) {
      const a = r[l];
      s[0] = [a[0], a[1]], s[1] = [a[0] + (i * r[l + 1][0] - i * r[l - 1][0]) / 6, a[1] + (i * r[l + 1][1] - i * r[l - 1][1]) / 6], s[2] = [r[l + 1][0] + (i * r[l][0] - i * r[l + 2][0]) / 6, r[l + 1][1] + (i * r[l][1] - i * r[l + 2][1]) / 6], s[3] = [r[l + 1][0], r[l + 1][1]], n.push(s[1], s[2], s[3]);
    }
  }
  return n;
}
function In(t, e) {
  return Math.pow(t[0] - e[0], 2) + Math.pow(t[1] - e[1], 2);
}
function Vc(t, e, o) {
  const n = In(e, o);
  if (n === 0) return In(t, e);
  let r = ((t[0] - e[0]) * (o[0] - e[0]) + (t[1] - e[1]) * (o[1] - e[1])) / n;
  return r = Math.max(0, Math.min(1, r)), In(t, uo(e, o, r));
}
function uo(t, e, o) {
  return [t[0] + (e[0] - t[0]) * o, t[1] + (e[1] - t[1]) * o];
}
function Pr(t, e, o, n) {
  const r = n || [];
  if (function(l, a) {
    const c = l[a + 0], h = l[a + 1], p = l[a + 2], d = l[a + 3];
    let f = 3 * h[0] - 2 * c[0] - d[0];
    f *= f;
    let m = 3 * h[1] - 2 * c[1] - d[1];
    m *= m;
    let y = 3 * p[0] - 2 * d[0] - c[0];
    y *= y;
    let b = 3 * p[1] - 2 * d[1] - c[1];
    return b *= b, f < y && (f = y), m < b && (m = b), f + m;
  }(t, e) < o) {
    const l = t[e + 0];
    r.length ? (s = r[r.length - 1], i = l, Math.sqrt(In(s, i)) > 1 && r.push(l)) : r.push(l), r.push(t[e + 3]);
  } else {
    const a = t[e + 0], c = t[e + 1], h = t[e + 2], p = t[e + 3], d = uo(a, c, 0.5), f = uo(c, h, 0.5), m = uo(h, p, 0.5), y = uo(d, f, 0.5), b = uo(f, m, 0.5), g = uo(y, b, 0.5);
    Pr([a, d, y, g], 0, o, r), Pr([g, b, m, p], 0, o, r);
  }
  var s, i;
  return r;
}
function Kc(t, e) {
  return En(t, 0, t.length, e);
}
function En(t, e, o, n, r) {
  const s = r || [], i = t[e], l = t[o - 1];
  let a = 0, c = 1;
  for (let h = e + 1; h < o - 1; ++h) {
    const p = Vc(t[h], i, l);
    p > a && (a = p, c = h);
  }
  return Math.sqrt(a) > n ? (En(t, e, c + 1, n, s), En(t, c, o, n, s)) : (s.length || s.push(i), s.push(l)), s;
}
function dr(t, e = 0.15, o) {
  const n = [], r = (t.length - 1) / 3;
  for (let s = 0; s < r; s++)
    Pr(t, 3 * s, e, n);
  return o && o > 0 ? En(n, 0, n.length, o) : n;
}
const pe = "none";
class Dn {
  constructor(e) {
    this.defaultOptions = { maxRandomnessOffset: 2, roughness: 1, bowing: 1, stroke: "#000", strokeWidth: 1, curveTightness: 0, curveFitting: 0.95, curveStepCount: 9, fillStyle: "hachure", fillWeight: -1, hachureAngle: -41, hachureGap: -1, dashOffset: -1, dashGap: -1, zigzagOffset: -1, seed: 0, disableMultiStroke: !1, disableMultiStrokeFill: !1, preserveVertices: !1, fillShapeRoughnessGain: 0.8 }, this.config = e || {}, this.config.options && (this.defaultOptions = this._o(this.config.options));
  }
  static newSeed() {
    return Math.floor(Math.random() * 2 ** 31);
  }
  _o(e) {
    return e ? Object.assign({}, this.defaultOptions, e) : this.defaultOptions;
  }
  _d(e, o, n) {
    return { shape: e, sets: o || [], options: n || this.defaultOptions };
  }
  line(e, o, n, r, s) {
    const i = this._o(s);
    return this._d("line", [Ki(e, o, n, r, i)], i);
  }
  rectangle(e, o, n, r, s) {
    const i = this._o(s), l = [], a = Gc(e, o, n, r, i);
    if (i.fill) {
      const c = [[e, o], [e + n, o], [e + n, o + r], [e, o + r]];
      i.fillStyle === "solid" ? l.push(cr([c], i)) : l.push(To([c], i));
    }
    return i.stroke !== pe && l.push(a), this._d("rectangle", l, i);
  }
  ellipse(e, o, n, r, s) {
    const i = this._o(s), l = [], a = qi(n, r, i), c = Ir(e, o, i, a);
    if (i.fill) if (i.fillStyle === "solid") {
      const h = Ir(e, o, i, a).opset;
      h.type = "fillPath", l.push(h);
    } else l.push(To([c.estimatedPoints], i));
    return i.stroke !== pe && l.push(c.opset), this._d("ellipse", l, i);
  }
  circle(e, o, n, r) {
    const s = this.ellipse(e, o, n, n, r);
    return s.shape = "circle", s;
  }
  linearPath(e, o) {
    const n = this._o(o);
    return this._d("linearPath", [zn(e, !1, n)], n);
  }
  arc(e, o, n, r, s, i, l = !1, a) {
    const c = this._o(a), h = [], p = qs(e, o, n, r, s, i, l, !0, c);
    if (l && c.fill) if (c.fillStyle === "solid") {
      const d = Object.assign({}, c);
      d.disableMultiStroke = !0;
      const f = qs(e, o, n, r, s, i, !0, !1, d);
      f.type = "fillPath", h.push(f);
    } else h.push(function(d, f, m, y, b, g, x) {
      const S = d, M = f;
      let z = Math.abs(m / 2), W = Math.abs(y / 2);
      z += At(0.01 * z, x), W += At(0.01 * W, x);
      let I = b, R = g;
      for (; I < 0; ) I += 2 * Math.PI, R += 2 * Math.PI;
      R - I > 2 * Math.PI && (I = 0, R = 2 * Math.PI);
      const K = (R - I) / x.curveStepCount, Z = [];
      for (let lt = I; lt <= R; lt += K) Z.push([S + z * Math.cos(lt), M + W * Math.sin(lt)]);
      return Z.push([S + z * Math.cos(R), M + W * Math.sin(R)]), Z.push([S, M]), To([Z], x);
    }(e, o, n, r, s, i, c));
    return c.stroke !== pe && h.push(p), this._d("arc", h, c);
  }
  curve(e, o) {
    const n = this._o(o), r = [], s = Ks(e, n);
    if (n.fill && n.fill !== pe) if (n.fillStyle === "solid") {
      const i = Ks(e, Object.assign(Object.assign({}, n), { disableMultiStroke: !0, roughness: n.roughness ? n.roughness + n.fillShapeRoughnessGain : 0 }));
      r.push({ type: "fillPath", ops: this._mergedShape(i.ops) });
    } else {
      const i = [], l = e;
      if (l.length) {
        const a = typeof l[0][0] == "number" ? [l] : l;
        for (const c of a) c.length < 3 ? i.push(...c) : c.length === 3 ? i.push(...dr($s([c[0], c[0], c[1], c[2]]), 10, (1 + n.roughness) / 2)) : i.push(...dr($s(c), 10, (1 + n.roughness) / 2));
      }
      i.length && r.push(To([i], n));
    }
    return n.stroke !== pe && r.push(s), this._d("curve", r, n);
  }
  polygon(e, o) {
    const n = this._o(o), r = [], s = zn(e, !0, n);
    return n.fill && (n.fillStyle === "solid" ? r.push(cr([e], n)) : r.push(To([e], n))), n.stroke !== pe && r.push(s), this._d("polygon", r, n);
  }
  path(e, o) {
    const n = this._o(o), r = [];
    if (!e) return this._d("path", r, n);
    e = (e || "").replace(/\n/g, " ").replace(/(-\s)/g, "-").replace("/(ss)/g", " ");
    const s = n.fill && n.fill !== "transparent" && n.fill !== pe, i = n.stroke !== pe, l = !!(n.simplification && n.simplification < 1), a = function(h, p, d) {
      const f = ji(Gi(Qr(h))), m = [];
      let y = [], b = [0, 0], g = [];
      const x = () => {
        g.length >= 4 && y.push(...dr(g, p)), g = [];
      }, S = () => {
        x(), y.length && (m.push(y), y = []);
      };
      for (const { key: z, data: W } of f) switch (z) {
        case "M":
          S(), b = [W[0], W[1]], y.push(b);
          break;
        case "L":
          x(), y.push([W[0], W[1]]);
          break;
        case "C":
          if (!g.length) {
            const I = y.length ? y[y.length - 1] : b;
            g.push([I[0], I[1]]);
          }
          g.push([W[0], W[1]]), g.push([W[2], W[3]]), g.push([W[4], W[5]]);
          break;
        case "Z":
          x(), y.push([b[0], b[1]]);
      }
      if (S(), !d) return m;
      const M = [];
      for (const z of m) {
        const W = Kc(z, d);
        W.length && M.push(W);
      }
      return M;
    }(e, 1, l ? 4 - 4 * (n.simplification || 1) : (1 + n.roughness) / 2), c = Zs(e, n);
    if (s) if (n.fillStyle === "solid") if (a.length === 1) {
      const h = Zs(e, Object.assign(Object.assign({}, n), { disableMultiStroke: !0, roughness: n.roughness ? n.roughness + n.fillShapeRoughnessGain : 0 }));
      r.push({ type: "fillPath", ops: this._mergedShape(h.ops) });
    } else r.push(cr(a, n));
    else r.push(To(a, n));
    return i && (l ? a.forEach((h) => {
      r.push(zn(h, !1, n));
    }) : r.push(c)), this._d("path", r, n);
  }
  opsToPath(e, o) {
    let n = "";
    for (const r of e.ops) {
      const s = typeof o == "number" && o >= 0 ? r.data.map((i) => +i.toFixed(o)) : r.data;
      switch (r.op) {
        case "move":
          n += `M${s[0]} ${s[1]} `;
          break;
        case "bcurveTo":
          n += `C${s[0]} ${s[1]}, ${s[2]} ${s[3]}, ${s[4]} ${s[5]} `;
          break;
        case "lineTo":
          n += `L${s[0]} ${s[1]} `;
      }
    }
    return n.trim();
  }
  toPaths(e) {
    const o = e.sets || [], n = e.options || this.defaultOptions, r = [];
    for (const s of o) {
      let i = null;
      switch (s.type) {
        case "path":
          i = { d: this.opsToPath(s), stroke: n.stroke, strokeWidth: n.strokeWidth, fill: pe };
          break;
        case "fillPath":
          i = { d: this.opsToPath(s), stroke: pe, strokeWidth: 0, fill: n.fill || pe };
          break;
        case "fillSketch":
          i = this.fillSketch(s, n);
      }
      i && r.push(i);
    }
    return r;
  }
  fillSketch(e, o) {
    let n = o.fillWeight;
    return n < 0 && (n = o.strokeWidth / 2), { d: this.opsToPath(e), stroke: o.fill || pe, strokeWidth: n, fill: pe };
  }
  _mergedShape(e) {
    return e.filter((o, n) => n === 0 || o.op !== "move");
  }
}
class qc {
  constructor(e, o) {
    this.canvas = e, this.ctx = this.canvas.getContext("2d"), this.gen = new Dn(o);
  }
  draw(e) {
    const o = e.sets || [], n = e.options || this.getDefaultOptions(), r = this.ctx, s = e.options.fixedDecimalPlaceDigits;
    for (const i of o) switch (i.type) {
      case "path":
        r.save(), r.strokeStyle = n.stroke === "none" ? "transparent" : n.stroke, r.lineWidth = n.strokeWidth, n.strokeLineDash && r.setLineDash(n.strokeLineDash), n.strokeLineDashOffset && (r.lineDashOffset = n.strokeLineDashOffset), this._drawToContext(r, i, s), r.restore();
        break;
      case "fillPath": {
        r.save(), r.fillStyle = n.fill || "";
        const l = e.shape === "curve" || e.shape === "polygon" || e.shape === "path" ? "evenodd" : "nonzero";
        this._drawToContext(r, i, s, l), r.restore();
        break;
      }
      case "fillSketch":
        this.fillSketch(r, i, n);
    }
  }
  fillSketch(e, o, n) {
    let r = n.fillWeight;
    r < 0 && (r = n.strokeWidth / 2), e.save(), n.fillLineDash && e.setLineDash(n.fillLineDash), n.fillLineDashOffset && (e.lineDashOffset = n.fillLineDashOffset), e.strokeStyle = n.fill || "", e.lineWidth = r, this._drawToContext(e, o, n.fixedDecimalPlaceDigits), e.restore();
  }
  _drawToContext(e, o, n, r = "nonzero") {
    e.beginPath();
    for (const s of o.ops) {
      const i = typeof n == "number" && n >= 0 ? s.data.map((l) => +l.toFixed(n)) : s.data;
      switch (s.op) {
        case "move":
          e.moveTo(i[0], i[1]);
          break;
        case "bcurveTo":
          e.bezierCurveTo(i[0], i[1], i[2], i[3], i[4], i[5]);
          break;
        case "lineTo":
          e.lineTo(i[0], i[1]);
      }
    }
    o.type === "fillPath" ? e.fill(r) : e.stroke();
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  line(e, o, n, r, s) {
    const i = this.gen.line(e, o, n, r, s);
    return this.draw(i), i;
  }
  rectangle(e, o, n, r, s) {
    const i = this.gen.rectangle(e, o, n, r, s);
    return this.draw(i), i;
  }
  ellipse(e, o, n, r, s) {
    const i = this.gen.ellipse(e, o, n, r, s);
    return this.draw(i), i;
  }
  circle(e, o, n, r) {
    const s = this.gen.circle(e, o, n, r);
    return this.draw(s), s;
  }
  linearPath(e, o) {
    const n = this.gen.linearPath(e, o);
    return this.draw(n), n;
  }
  polygon(e, o) {
    const n = this.gen.polygon(e, o);
    return this.draw(n), n;
  }
  arc(e, o, n, r, s, i, l = !1, a) {
    const c = this.gen.arc(e, o, n, r, s, i, l, a);
    return this.draw(c), c;
  }
  curve(e, o) {
    const n = this.gen.curve(e, o);
    return this.draw(n), n;
  }
  path(e, o) {
    const n = this.gen.path(e, o);
    return this.draw(n), n;
  }
}
const wn = "http://www.w3.org/2000/svg";
class Zc {
  constructor(e, o) {
    this.svg = e, this.gen = new Dn(o);
  }
  draw(e) {
    const o = e.sets || [], n = e.options || this.getDefaultOptions(), r = this.svg.ownerDocument || window.document, s = r.createElementNS(wn, "g"), i = e.options.fixedDecimalPlaceDigits;
    for (const l of o) {
      let a = null;
      switch (l.type) {
        case "path":
          a = r.createElementNS(wn, "path"), a.setAttribute("d", this.opsToPath(l, i)), a.setAttribute("stroke", n.stroke), a.setAttribute("stroke-width", n.strokeWidth + ""), a.setAttribute("fill", "none"), n.strokeLineDash && a.setAttribute("stroke-dasharray", n.strokeLineDash.join(" ").trim()), n.strokeLineDashOffset && a.setAttribute("stroke-dashoffset", `${n.strokeLineDashOffset}`);
          break;
        case "fillPath":
          a = r.createElementNS(wn, "path"), a.setAttribute("d", this.opsToPath(l, i)), a.setAttribute("stroke", "none"), a.setAttribute("stroke-width", "0"), a.setAttribute("fill", n.fill || ""), e.shape !== "curve" && e.shape !== "polygon" || a.setAttribute("fill-rule", "evenodd");
          break;
        case "fillSketch":
          a = this.fillSketch(r, l, n);
      }
      a && s.appendChild(a);
    }
    return s;
  }
  fillSketch(e, o, n) {
    let r = n.fillWeight;
    r < 0 && (r = n.strokeWidth / 2);
    const s = e.createElementNS(wn, "path");
    return s.setAttribute("d", this.opsToPath(o, n.fixedDecimalPlaceDigits)), s.setAttribute("stroke", n.fill || ""), s.setAttribute("stroke-width", r + ""), s.setAttribute("fill", "none"), n.fillLineDash && s.setAttribute("stroke-dasharray", n.fillLineDash.join(" ").trim()), n.fillLineDashOffset && s.setAttribute("stroke-dashoffset", `${n.fillLineDashOffset}`), s;
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  opsToPath(e, o) {
    return this.gen.opsToPath(e, o);
  }
  line(e, o, n, r, s) {
    const i = this.gen.line(e, o, n, r, s);
    return this.draw(i);
  }
  rectangle(e, o, n, r, s) {
    const i = this.gen.rectangle(e, o, n, r, s);
    return this.draw(i);
  }
  ellipse(e, o, n, r, s) {
    const i = this.gen.ellipse(e, o, n, r, s);
    return this.draw(i);
  }
  circle(e, o, n, r) {
    const s = this.gen.circle(e, o, n, r);
    return this.draw(s);
  }
  linearPath(e, o) {
    const n = this.gen.linearPath(e, o);
    return this.draw(n);
  }
  polygon(e, o) {
    const n = this.gen.polygon(e, o);
    return this.draw(n);
  }
  arc(e, o, n, r, s, i, l = !1, a) {
    const c = this.gen.arc(e, o, n, r, s, i, l, a);
    return this.draw(c);
  }
  curve(e, o) {
    const n = this.gen.curve(e, o);
    return this.draw(n);
  }
  path(e, o) {
    const n = this.gen.path(e, o);
    return this.draw(n);
  }
}
var Uc = { canvas: (t, e) => new qc(t, e), svg: (t, e) => new Zc(t, e), generator: (t) => new Dn(t), newSeed: () => Dn.newSeed() };
const He = Uc.generator();
function Qc(t) {
  let e = 0;
  for (let o = 0; o < t.length; o++) {
    const n = t.charCodeAt(o);
    e = (e << 5) - e + n, e |= 0;
  }
  return Math.abs(e);
}
function _e(t) {
  return {
    stroke: t.stroke,
    fill: t.fill || "none",
    fillStyle: t.fill ? t.fillStyle || "hachure" : void 0,
    roughness: t.roughness,
    strokeWidth: t.strokeWidth,
    strokeLineDash: t.strokeLineDash,
    seed: t.seed ? Qc(t.seed) : void 0,
    fillWeight: t.strokeWidth / 2,
    hachureGap: Math.max(t.strokeWidth * 4, 4)
  };
}
function to(t) {
  var n;
  const e = t.options, o = (n = e == null ? void 0 : e.strokeLineDash) != null && n.length ? e.strokeLineDash.join(" ") : void 0;
  return He.toPaths(t).map((r) => ({
    d: r.d,
    stroke: r.stroke,
    strokeWidth: r.strokeWidth,
    fill: r.fill,
    // Apply dash only to stroke paths, not fill paths
    strokeDasharray: r.stroke !== "none" && r.strokeWidth > 0 ? o : void 0
  }));
}
function jo(t, e) {
  return Math.min(t, e) * 0.25;
}
function Jc(t, e, o, n, r) {
  const s = Math.min(r, o / 2, n / 2);
  return [
    `M${t + s},${e}`,
    `L${t + o - s},${e}`,
    `A${s},${s} 0 0 1 ${t + o},${e + s}`,
    `L${t + o},${e + n - s}`,
    `A${s},${s} 0 0 1 ${t + o - s},${e + n}`,
    `L${t + s},${e + n}`,
    `A${s},${s} 0 0 1 ${t},${e + n - s}`,
    `L${t},${e + s}`,
    `A${s},${s} 0 0 1 ${t + s},${e}`,
    "Z"
  ].join(" ");
}
function Wn(t, e, o, n, r, s) {
  if (s) {
    const i = jo(o, n);
    return to(He.path(Jc(t, e, o, n, i), _e(r)));
  }
  return to(He.rectangle(t, e, o, n, _e(r)));
}
function Jr(t, e, o, n, r) {
  return to(He.ellipse(t, e, o, n, _e(r)));
}
function $c(t, e, o, n, r) {
  const s = t + o / 2, i = e + n / 2, l = [s, e], a = [t + o, i], c = [s, e + n], h = [t, i], p = Math.hypot(o / 2, n / 2), d = Math.min(r, p / 2) / p, f = (W, I, R) => [
    W[0] + R * (I[0] - W[0]),
    W[1] + R * (I[1] - W[1])
  ], m = f(h, l, 1 - d), y = f(l, a, d), b = f(l, a, 1 - d), g = f(a, c, d), x = f(a, c, 1 - d), S = f(c, h, d), M = f(c, h, 1 - d), z = f(h, l, d);
  return [
    `M${y[0]},${y[1]}`,
    `L${b[0]},${b[1]}`,
    `Q${a[0]},${a[1]} ${g[0]},${g[1]}`,
    `L${x[0]},${x[1]}`,
    `Q${c[0]},${c[1]} ${S[0]},${S[1]}`,
    `L${M[0]},${M[1]}`,
    `Q${h[0]},${h[1]} ${z[0]},${z[1]}`,
    `L${m[0]},${m[1]}`,
    `Q${l[0]},${l[1]} ${y[0]},${y[1]}`,
    "Z"
  ].join(" ");
}
function $r(t, e, o, n, r, s) {
  if (s) {
    const l = jo(o, n);
    return to(He.path($c(t, e, o, n, l), _e(r)));
  }
  const i = [
    [t + o / 2, e],
    [t + o, e + n / 2],
    [t + o / 2, e + n],
    [t, e + n / 2]
  ];
  return to(He.polygon(i, _e(r)));
}
function Oo(t, e, o, n, r) {
  return to(He.line(t, e, o, n, _e(r)));
}
function _r(t, e, o, n, r) {
  const s = Oo(t, e, o, n, r), i = Math.atan2(n - e, o - t), l = Math.max(12, r.strokeWidth * 4), a = Math.PI / 6, c = o - l * Math.cos(i - a), h = n - l * Math.sin(i - a), p = o - l * Math.cos(i + a), d = n - l * Math.sin(i + a), f = Oo(o, n, c, h, r), m = Oo(o, n, p, d, r);
  return [...s, ...f, ...m];
}
function _s(t, e) {
  const o = {
    ..._e(e),
    stroke: "none"
  };
  return to(He.polygon(t, o));
}
function hr(t, e) {
  return to(He.path(t, _e(e)));
}
function eo(t) {
  if (t === "dashed") return [8, 4];
  if (t === "dotted") return [2, 2];
}
function _c(t) {
  const e = t.replace("#", ""), o = parseInt(e.substring(0, 2), 16) || 0, n = parseInt(e.substring(2, 4), 16) || 0, r = parseInt(e.substring(4, 6), 16) || 0;
  return (0.299 * o + 0.587 * n + 0.114 * r) / 255 > 0.5 ? "#1e1e2e" : "#ffffff";
}
function td({ node: t, editingLabel: e }) {
  if (t.type === "draw") {
    const o = t;
    return o.data.tool === "vector" ? /* @__PURE__ */ u(od, { node: o }) : /* @__PURE__ */ u(ed, { node: o });
  }
  return /* @__PURE__ */ u(nd, { node: t, editingLabel: e });
}
const Fn = ue(td), ed = ue(function({ node: e }) {
  const o = e.data.strokeStyle === "dashed" || e.data.strokeStyle === "dotted", n = eo(e.data.strokeStyle), r = Kt(
    () => o ? null : Zr(e.data.points, { size: e.data.strokeWidth }),
    [e.data.points, e.data.strokeWidth, o]
  ), s = Kt(() => {
    const h = e.data.points;
    if (!h || h.length === 0) return "";
    if (h.length === 1) return `M${h[0][0]},${h[0][1]}L${h[0][0]},${h[0][1]}`;
    const p = [`M${h[0][0]},${h[0][1]}`];
    for (let d = 1; d < h.length; d++)
      p.push(`L${h[d][0]},${h[d][1]}`);
    return p.join("");
  }, [e.data.points]), i = Kt(() => {
    if (!o) return null;
    const h = e.data.points;
    if (h.length < 2) return "";
    const p = ["M", h[0][0], h[0][1]];
    for (let f = 1; f < h.length; f++) {
      const [m, y] = h[f], [b, g] = h[f - 1];
      p.push("Q", b, g, (b + m) / 2, (g + y) / 2);
    }
    const d = h[h.length - 1];
    return p.push("L", d[0], d[1]), p.join(" ");
  }, [e.data.points, o]), l = Kt(() => {
    if (!e.data.fill || e.data.points.length < 3) return null;
    const h = e.data.points.map((M) => [M[0], M[1]]), p = Yi(h), d = p[0], f = p[p.length - 1], m = Math.hypot(d[0] - f[0], d[1] - f[1]);
    let y = 0;
    for (let M = 1; M < p.length; M++)
      y += Math.hypot(p[M][0] - p[M - 1][0], p[M][1] - p[M - 1][1]);
    const b = y >= 1 && m <= Math.max(e.data.strokeWidth * 4, 20) && m <= y * 0.1, g = e.data.fillStyle || "solid";
    if (b) {
      const M = Pc(p, 0);
      return g === "solid" ? { kind: "solid", d: M, fill: e.data.fill } : { kind: "rough", paths: _s(p, {
        stroke: "none",
        fill: e.data.fill,
        fillStyle: g,
        roughness: 1,
        strokeWidth: e.data.strokeWidth
      }) };
    }
    const x = Lc(p);
    if (x.length === 0) return null;
    if (g === "solid")
      return {
        kind: "solid",
        d: "",
        fill: e.data.fill,
        regions: x
      };
    const S = [];
    for (const { points: M } of x)
      M.length >= 3 && S.push(
        ..._s(M, {
          stroke: "none",
          fill: e.data.fill,
          fillStyle: g,
          roughness: 1,
          strokeWidth: e.data.strokeWidth
        })
      );
    return { kind: "rough", paths: S, regions: x };
  }, [e.data.fill, e.data.fillStyle, e.data.points, e.data.strokeWidth]), a = e.h === "auto" ? 0 : e.h, c = e.data.strokeWidth * 4;
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x - c,
        top: e.y - c,
        width: e.w + c * 2,
        height: a + c * 2,
        zIndex: e.z,
        pointerEvents: "none",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "svg",
        {
          width: e.w + c * 2,
          height: a + c * 2,
          style: { overflow: "visible" },
          children: /* @__PURE__ */ k("g", { transform: `translate(${c}, ${c})`, opacity: e.data.opacity ?? 1, children: [
            (l == null ? void 0 : l.kind) === "solid" && (l.regions ? l.regions.map((h, p) => /* @__PURE__ */ u(
              "path",
              {
                d: h.pathD,
                fill: l.fill,
                stroke: "none"
              },
              p
            )) : /* @__PURE__ */ u("path", { d: l.d, fill: l.fill, stroke: "none" })),
            (l == null ? void 0 : l.kind) === "rough" && l.paths.map((h, p) => /* @__PURE__ */ u(
              "path",
              {
                d: h.d,
                stroke: h.stroke,
                strokeWidth: h.strokeWidth,
                fill: h.fill,
                strokeLinecap: "round",
                strokeLinejoin: "round"
              },
              p
            )),
            o ? /* @__PURE__ */ u(
              "path",
              {
                d: i,
                fill: "none",
                stroke: e.data.color,
                strokeWidth: e.data.strokeWidth,
                strokeDasharray: n == null ? void 0 : n.map((h) => h * Math.max(e.data.strokeWidth, 1)).join(" "),
                strokeLinecap: "round",
                strokeLinejoin: "round"
              }
            ) : /* @__PURE__ */ u(
              "path",
              {
                d: r,
                fill: e.data.color
              }
            ),
            s && /* @__PURE__ */ u(
              "path",
              {
                d: s,
                fill: "none",
                stroke: "transparent",
                strokeWidth: e.data.strokeWidth,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                pointerEvents: "stroke"
              }
            )
          ] })
        }
      )
    }
  );
}), od = ue(function({ node: e }) {
  const o = e.h === "auto" ? 0 : e.h, n = e.data.strokeWidth * 2, r = Kt(() => {
    const l = e.data.points;
    if (!l || l.length === 0) return "";
    const a = [`M${l[0][0]},${l[0][1]}`];
    for (let c = 1; c < l.length; c++)
      a.push(`L${l[c][0]},${l[c][1]}`);
    return a.push("Z"), a.join("");
  }, [e.data.points]), s = eo(e.data.strokeStyle), i = s == null ? void 0 : s.map((l) => l * Math.max(e.data.strokeWidth, 1)).join(" ");
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x - n,
        top: e.y - n,
        width: e.w + n * 2,
        height: o + n * 2,
        zIndex: e.z,
        pointerEvents: "none",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "svg",
        {
          width: e.w + n * 2,
          height: o + n * 2,
          style: { overflow: "visible" },
          children: /* @__PURE__ */ k("g", { transform: `translate(${n}, ${n})`, opacity: e.data.opacity ?? 1, children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: r,
                fill: e.data.fill || "none",
                stroke: e.data.color,
                strokeWidth: e.data.strokeWidth,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                strokeDasharray: i
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: r,
                fill: e.data.fill ? "transparent" : "none",
                stroke: "transparent",
                strokeWidth: Math.max(e.data.strokeWidth, 8),
                pointerEvents: e.data.fill ? "painted" : "stroke"
              }
            )
          ] })
        }
      )
    }
  );
}), nd = ue(function({ node: e, editingLabel: o }) {
  var b, g, x, S;
  const n = e.h === "auto" ? 100 : e.h, r = e.data.strokeWidth * 2, s = eo(e.data.strokeStyle), i = ((b = e.data.startPoint) == null ? void 0 : b[0]) ?? 0, l = ((g = e.data.startPoint) == null ? void 0 : g[1]) ?? n / 2, a = ((x = e.data.endPoint) == null ? void 0 : x[0]) ?? e.w, c = ((S = e.data.endPoint) == null ? void 0 : S[1]) ?? n / 2, h = Kt(() => {
    if (e.data.roughness === 0) return null;
    const M = {
      stroke: e.data.stroke,
      fill: e.data.fill,
      fillStyle: e.data.fillStyle,
      roughness: e.data.roughness,
      strokeWidth: e.data.strokeWidth,
      strokeLineDash: s,
      seed: e.id
    }, z = e.data.edgeStyle === "round";
    switch (e.data.shape) {
      case "rect":
        return Wn(0, 0, e.w, n, M, z);
      case "ellipse":
        return Jr(e.w / 2, n / 2, e.w, n, M);
      case "diamond":
        return $r(0, 0, e.w, n, M, z);
      case "line":
        return Oo(i, l, a, c, M);
      case "arrow":
        return _r(i, l, a, c, M);
      default:
        return null;
    }
  }, [e, s, i, l, a, c, n]), p = e.data.fill && e.data.fillStyle === "solid" && e.data.roughness > 0, d = e.data.opacity ?? 1, f = e.data.shape === "line" || e.data.shape === "arrow", m = e.data.label, y = e.data.labelFontSize ?? 14;
  return /* @__PURE__ */ k(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x,
        top: e.y,
        width: e.w,
        height: n,
        zIndex: e.z,
        pointerEvents: "none",
        overflow: "visible",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: [
        /* @__PURE__ */ u(
          "svg",
          {
            width: e.w + r * 2,
            height: n + r * 2,
            style: { overflow: "visible", marginLeft: -r, marginTop: -r },
            children: /* @__PURE__ */ k("g", { transform: `translate(${r}, ${r})`, opacity: d, children: [
              p && /* @__PURE__ */ u(
                id,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: n,
                  fill: e.data.fill,
                  rounded: e.data.edgeStyle === "round"
                }
              ),
              h ? h.map((M, z) => p && M.fill && M.fill !== "none" ? null : /* @__PURE__ */ u(
                "path",
                {
                  d: M.d,
                  stroke: M.stroke,
                  strokeWidth: M.strokeWidth,
                  fill: M.fill,
                  strokeDasharray: M.strokeDasharray,
                  strokeLinecap: "round",
                  strokeLinejoin: "round"
                },
                z
              )) : /* @__PURE__ */ u(
                rd,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: n,
                  x1: i,
                  y1: l,
                  x2: a,
                  y2: c,
                  stroke: e.data.stroke,
                  fill: e.data.fill,
                  strokeWidth: e.data.strokeWidth,
                  dashArray: s,
                  rounded: e.data.edgeStyle === "round"
                }
              ),
              /* @__PURE__ */ u(
                sd,
                {
                  shape: e.data.shape,
                  w: e.w,
                  h: n,
                  x1: i,
                  y1: l,
                  x2: a,
                  y2: c,
                  hasFill: !!e.data.fill,
                  strokeWidth: e.data.strokeWidth,
                  rounded: e.data.edgeStyle === "round"
                }
              )
            ] })
          }
        ),
        !f && m && !o && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              pointerEvents: "none",
              padding: "4px 8px"
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                style: {
                  textAlign: e.data.labelAlign ?? "center",
                  fontFamily: Je(e.data.labelFontFamily ?? Qe),
                  fontSize: y,
                  color: e.data.fill && e.data.fillStyle === "solid" ? _c(e.data.fill) : e.data.stroke,
                  lineHeight: 1.3,
                  wordBreak: "break-word",
                  whiteSpace: "pre-wrap",
                  width: "100%"
                },
                children: m
              }
            )
          }
        )
      ]
    }
  );
});
function ts(t, e) {
  const o = jo(t, e), n = t / 2, r = e / 2, s = [n, 0], i = [t, r], l = [n, e], a = [0, r], c = Math.hypot(t / 2, e / 2), h = Math.min(o, c / 2) / c, p = (M, z, W) => [
    M[0] + W * (z[0] - M[0]),
    M[1] + W * (z[1] - M[1])
  ], d = p(s, i, h), f = p(s, i, 1 - h), m = p(i, l, h), y = p(i, l, 1 - h), b = p(l, a, h), g = p(l, a, 1 - h), x = p(a, s, h), S = p(a, s, 1 - h);
  return [
    `M${d[0]},${d[1]}`,
    `L${f[0]},${f[1]}`,
    `Q${i[0]},${i[1]} ${m[0]},${m[1]}`,
    `L${y[0]},${y[1]}`,
    `Q${l[0]},${l[1]} ${b[0]},${b[1]}`,
    `L${g[0]},${g[1]}`,
    `Q${a[0]},${a[1]} ${x[0]},${x[1]}`,
    `L${S[0]},${S[1]}`,
    `Q${s[0]},${s[1]} ${d[0]},${d[1]}`,
    "Z"
  ].join(" ");
}
function rd({
  shape: t,
  w: e,
  h: o,
  x1: n,
  y1: r,
  x2: s,
  y2: i,
  stroke: l,
  fill: a,
  strokeWidth: c,
  dashArray: h,
  rounded: p
}) {
  const d = h == null ? void 0 : h.join(",");
  switch (t) {
    case "rect": {
      const f = !!a && a !== "none", m = o <= Math.max(c * 2, 4), y = e <= Math.max(c * 2, 4);
      if (!f && (m || y))
        return m && e >= o ? /* @__PURE__ */ u(
          "line",
          {
            x1: 0,
            y1: o / 2,
            x2: e,
            y2: o / 2,
            stroke: l,
            strokeWidth: Math.max(c, o),
            strokeDasharray: d
          }
        ) : /* @__PURE__ */ u(
          "line",
          {
            x1: e / 2,
            y1: 0,
            x2: e / 2,
            y2: o,
            stroke: l,
            strokeWidth: Math.max(c, e),
            strokeDasharray: d
          }
        );
      const b = p ? jo(e, o) : 0;
      return /* @__PURE__ */ u(
        "rect",
        {
          x: 0,
          y: 0,
          width: e,
          height: o,
          rx: b || void 0,
          ry: b || void 0,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    }
    case "ellipse":
      return /* @__PURE__ */ u(
        "ellipse",
        {
          cx: e / 2,
          cy: o / 2,
          rx: e / 2,
          ry: o / 2,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "diamond":
      return p ? /* @__PURE__ */ u(
        "path",
        {
          d: ts(e, o),
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          stroke: l,
          fill: a || "none",
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "line":
      return /* @__PURE__ */ u(
        "line",
        {
          x1: n,
          y1: r,
          x2: s,
          y2: i,
          stroke: l,
          strokeWidth: c,
          strokeDasharray: d
        }
      );
    case "arrow": {
      const f = Math.atan2(i - r, s - n), m = Math.max(12, c * 4), y = Math.PI / 6, b = s - m * Math.cos(f - y), g = i - m * Math.sin(f - y), x = s - m * Math.cos(f + y), S = i - m * Math.sin(f + y);
      return /* @__PURE__ */ k(ft, { children: [
        /* @__PURE__ */ u(
          "line",
          {
            x1: n,
            y1: r,
            x2: s,
            y2: i,
            stroke: l,
            strokeWidth: c,
            strokeDasharray: d
          }
        ),
        /* @__PURE__ */ u(
          "polyline",
          {
            points: `${b},${g} ${s},${i} ${x},${S}`,
            stroke: l,
            strokeWidth: c,
            fill: "none"
          }
        )
      ] });
    }
    default:
      return null;
  }
}
function sd({
  shape: t,
  w: e,
  h: o,
  x1: n,
  y1: r,
  x2: s,
  y2: i,
  hasFill: l,
  strokeWidth: a,
  rounded: c
}) {
  const h = l ? "painted" : "stroke", p = l ? "transparent" : "none";
  switch (t) {
    case "rect": {
      const d = c ? jo(e, o) : 0;
      return /* @__PURE__ */ u(
        "rect",
        {
          x: 0,
          y: 0,
          width: e,
          height: o,
          rx: d || void 0,
          ry: d || void 0,
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    }
    case "ellipse":
      return /* @__PURE__ */ u(
        "ellipse",
        {
          cx: e / 2,
          cy: o / 2,
          rx: e / 2,
          ry: o / 2,
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    case "diamond":
      return c ? /* @__PURE__ */ u(
        "path",
        {
          d: ts(e, o),
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          fill: p,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: h
        }
      );
    case "line":
    case "arrow":
      return /* @__PURE__ */ u(
        "line",
        {
          x1: n,
          y1: r,
          x2: s,
          y2: i,
          stroke: "transparent",
          strokeWidth: a,
          pointerEvents: "stroke"
        }
      );
    default:
      return null;
  }
}
function id({
  shape: t,
  w: e,
  h: o,
  fill: n,
  rounded: r
}) {
  switch (t) {
    case "rect": {
      const s = r ? jo(e, o) : 0;
      return /* @__PURE__ */ u("rect", { x: 0, y: 0, width: e, height: o, rx: s || void 0, ry: s || void 0, fill: n, stroke: "none" });
    }
    case "ellipse":
      return /* @__PURE__ */ u("ellipse", { cx: e / 2, cy: o / 2, rx: e / 2, ry: o / 2, fill: n, stroke: "none" });
    case "diamond":
      return r ? /* @__PURE__ */ u(
        "path",
        {
          d: ts(e, o),
          fill: n,
          stroke: "none"
        }
      ) : /* @__PURE__ */ u(
        "polygon",
        {
          points: `${e / 2},0 ${e},${o / 2} ${e / 2},${o} 0,${o / 2}`,
          fill: n,
          stroke: "none"
        }
      );
    default:
      return null;
  }
}
const ad = ue(function(e) {
  return /* @__PURE__ */ u(Fn, { node: e.node });
}), ld = {
  type: "draw",
  component: ad,
  handlesOwnLayout: !0,
  hitTest: (t, e, o, n) => Or(t, e, o, n),
  getHitPadding: (t) => {
    const e = t.data;
    return Math.max(20, e.strokeWidth * 4);
  },
  onResize: (t, e, o) => ({
    points: t.data.points.map(
      ([r, s, i]) => [r * e, s * o, i]
    )
  }),
  onFlip: (t, e) => {
    const o = t.data;
    if (e === "h")
      return {
        points: o.points.map(
          ([r, s, i]) => [t.w - r, s, i]
        )
      };
    const n = t.h === "auto" ? 0 : t.h;
    return {
      points: o.points.map(
        ([r, s, i]) => [r, n - s, i]
      )
    };
  },
  getClipboardText: () => null
}, cd = ue(function(e) {
  const o = e.node;
  return /* @__PURE__ */ u(Fn, { node: o, editingLabel: e.editing });
}), dd = {
  type: "shape",
  component: cd,
  handlesOwnLayout: !0,
  hitTest: (t, e, o, n) => Hn(t, e, o, n),
  onResize: (t, e, o) => {
    const n = t.data, r = {};
    return n.startPoint && (r.startPoint = [n.startPoint[0] * e, n.startPoint[1] * o]), n.endPoint && (r.endPoint = [n.endPoint[0] * e, n.endPoint[1] * o]), Object.keys(r).length > 0 ? r : null;
  },
  onFlip: (t, e) => {
    const o = t.data;
    if (o.shape !== "arrow" && o.shape !== "line")
      return {};
    if (e === "h")
      return o.startPoint && o.endPoint ? {
        startPoint: [t.w - o.startPoint[0], o.startPoint[1]],
        endPoint: [t.w - o.endPoint[0], o.endPoint[1]]
      } : {};
    const n = t.h === "auto" ? 0 : t.h;
    return o.startPoint && o.endPoint ? {
      startPoint: [o.startPoint[0], n - o.startPoint[1]],
      endPoint: [o.endPoint[0], n - o.endPoint[1]]
    } : {};
  },
  getClipboardText: (t) => t.data.label || null
};
function hd(t) {
  return null;
}
const ud = {
  type: "edge",
  component: hd,
  isSVGOnly: !0,
  handlesOwnLayout: !0,
  getClipboardText: () => null
}, kn = 0.05, pd = [
  { pos: "nw", edges: ["left", "top"], cx: 0, cy: 0, cursor: "nwse-resize" },
  { pos: "n", edges: ["top"], cx: 0.5, cy: 0, cursor: "ns-resize" },
  { pos: "ne", edges: ["right", "top"], cx: 1, cy: 0, cursor: "nesw-resize" },
  { pos: "e", edges: ["right"], cx: 1, cy: 0.5, cursor: "ew-resize" },
  { pos: "se", edges: ["right", "bottom"], cx: 1, cy: 1, cursor: "nwse-resize" },
  { pos: "s", edges: ["bottom"], cx: 0.5, cy: 1, cursor: "ns-resize" },
  { pos: "sw", edges: ["left", "bottom"], cx: 0, cy: 1, cursor: "nesw-resize" },
  { pos: "w", edges: ["left"], cx: 0, cy: 0.5, cursor: "ew-resize" }
];
function fd({
  node: t,
  isSelected: e,
  engine: o,
  interactive: n,
  zoom: r,
  onResizeHandleDown: s,
  cropping: i,
  onCropStart: l,
  onCropEnd: a
}) {
  const c = t.h, h = t.data.crop, p = at(!1);
  p.current = !!i;
  const d = at(null), [f, m] = et(null), y = st(() => {
    d.current && d.current.naturalWidth > 0 && m({ w: d.current.naturalWidth, h: d.current.naturalHeight });
  }, []);
  yt(() => {
    d.current && d.current.naturalWidth > 0 && m({ w: d.current.naturalWidth, h: d.current.naturalHeight });
  }, [t.data.src]);
  const [b, g] = et({ x: 0, y: 0, w: 1, h: 1 });
  yt(() => {
    i && (g(h ?? { x: 0, y: 0, w: 1, h: 1 }), !f && d.current && d.current.naturalWidth > 0 && m({ w: d.current.naturalWidth, h: d.current.naturalHeight }));
  }, [i]);
  const x = Kt(() => {
    if (!f) return null;
    const N = f.w / f.h, j = t.w / c;
    let H, _;
    return N > j ? (H = t.w, _ = t.w / N) : (_ = c, H = c * N), { x: (t.w - H) / 2, y: (c - _) / 2, w: H, h: _ };
  }, [f, t.w, c]), S = st(() => {
    const N = b.x < 1e-3 && b.y < 1e-3 && b.w > 0.999 && b.h > 0.999;
    o.updateNodeWithHistory(t.id, {
      data: {
        ...t.data,
        crop: N ? void 0 : { x: b.x, y: b.y, w: b.w, h: b.h }
      }
    }), a == null || a();
  }, [o, t, b, a]), M = st(() => {
    a == null || a();
  }, [a]);
  yt(() => {
    if (!i) return;
    const N = (j) => {
      j.key === "Enter" ? (S(), j.preventDefault(), j.stopPropagation()) : j.key === "Escape" && (M(), j.preventDefault(), j.stopPropagation());
    };
    return document.addEventListener("keydown", N, !0), () => document.removeEventListener("keydown", N, !0);
  }, [i, S, M]);
  const z = st(
    (N, j) => {
      if (j.stopPropagation(), j.preventDefault(), !x) return;
      const H = j.currentTarget.ownerDocument, _ = j.clientX, tt = j.clientY, nt = { ...b }, ut = (vt) => {
        const rt = (vt.clientX - _) / r / x.w, gt = (vt.clientY - tt) / r / x.h, xt = { ...nt }, ht = nt.x + nt.w, Dt = nt.y + nt.h;
        if (N.includes("left")) {
          const Tt = Math.max(0, Math.min(ht - kn, nt.x + rt));
          xt.x = Tt, xt.w = ht - Tt;
        }
        if (N.includes("right") && (xt.w = Math.max(
          kn,
          Math.min(1 - nt.x, nt.w + rt)
        )), N.includes("top")) {
          const Tt = Math.max(0, Math.min(Dt - kn, nt.y + gt));
          xt.y = Tt, xt.h = Dt - Tt;
        }
        N.includes("bottom") && (xt.h = Math.max(
          kn,
          Math.min(1 - nt.y, nt.h + gt)
        )), g(xt);
      }, pt = () => {
        H.removeEventListener("pointermove", ut), H.removeEventListener("pointerup", pt);
      };
      H.addEventListener("pointermove", ut), H.addEventListener("pointerup", pt);
    },
    [b, x, r]
  ), W = st(
    (N) => {
      if (N.stopPropagation(), N.preventDefault(), !x) return;
      const j = N.currentTarget.ownerDocument, H = N.clientX, _ = N.clientY, tt = { ...b }, nt = (pt) => {
        const vt = (pt.clientX - H) / r / x.w, rt = (pt.clientY - _) / r / x.h;
        g({
          ...tt,
          x: Math.max(0, Math.min(1 - tt.w, tt.x + vt)),
          y: Math.max(0, Math.min(1 - tt.h, tt.y + rt))
        });
      }, ut = () => {
        j.removeEventListener("pointermove", nt), j.removeEventListener("pointerup", ut);
      };
      j.addEventListener("pointermove", nt), j.addEventListener("pointerup", ut);
    },
    [b, x, r]
  ), I = st(
    (N) => {
      if (p.current) return;
      const j = N.currentTarget.ownerDocument;
      if (N.altKey) return;
      if (!o.selection.has(t.id) && o.selection.size > 0) {
        const { x: Tt, y: Rt } = o.screenToCanvas(
          N.clientX,
          N.clientY
        );
        for (const Qt of o.selection) {
          const Ut = o.getNode(Qt);
          if (!Ut) continue;
          const re = Ut.h === "auto" ? 100 : Ut.h;
          if (Tt >= Ut.x && Tt <= Ut.x + Ut.w && Rt >= Ut.y && Rt <= Ut.y + re)
            return;
        }
      }
      N.stopPropagation(), N.preventDefault(), N.shiftKey ? o.toggleSelect(t.id) : o.selection.has(t.id) || o.select(t.id);
      const H = N.clientX, _ = N.clientY, tt = Array.from(o.selection), nt = tt.map((Tt) => {
        const Rt = o.getNode(Tt);
        return { id: Tt, x: Rt.x, y: Rt.y };
      });
      let ut = !1, pt = null, vt = H, rt = _, gt = !1;
      const xt = () => {
        pt = null;
        const Tt = (vt - H) / o.viewport.zoom, Rt = (rt - _) / o.viewport.zoom, { finalDx: Qt, finalDy: Ut } = o.computeDragSnap(
          nt,
          tt,
          Tt,
          Rt,
          gt
        ), re = nt.map((Jt) => ({
          id: Jt.id,
          patch: { x: Jt.x + Qt, y: Jt.y + Ut }
        }));
        o.updateMany(re);
      }, ht = (Tt) => {
        const Rt = (Tt.clientX - H) / o.viewport.zoom, Qt = (Tt.clientY - _) / o.viewport.zoom;
        if (!ut)
          if (Math.abs(Rt) > 2 || Math.abs(Qt) > 2)
            ut = !0, o.pushHistorySnapshot();
          else
            return;
        vt = Tt.clientX, rt = Tt.clientY, gt = Tt.metaKey || Tt.ctrlKey, pt === null && (pt = requestAnimationFrame(xt));
      }, Dt = () => {
        pt !== null && (cancelAnimationFrame(pt), xt()), o.clearAlignGuides(), j.removeEventListener("pointermove", ht), j.removeEventListener("pointerup", Dt);
      };
      j.addEventListener("pointermove", ht), j.addEventListener("pointerup", Dt);
    },
    [o, t.id]
  ), R = [
    { pos: "nw", cx: 0, cy: 0 },
    { pos: "n", cx: 0.5, cy: 0 },
    { pos: "ne", cx: 1, cy: 0 },
    { pos: "e", cx: 1, cy: 0.5 },
    { pos: "se", cx: 1, cy: 1 },
    { pos: "s", cx: 0.5, cy: 1 },
    { pos: "sw", cx: 0, cy: 1 },
    { pos: "w", cx: 0, cy: 0.5 }
  ], K = 8 / r, Z = K / 2, lt = 25 / r, O = e && s && !i, ot = st(
    (N) => {
      const j = N.currentTarget.ownerDocument;
      N.stopPropagation(), N.preventDefault();
      const H = t.x + t.w / 2, _ = t.y + c / 2, tt = t.rotation || 0, { x: nt, y: ut } = o.screenToCanvas(
        N.clientX,
        N.clientY
      ), pt = Math.atan2(ut - _, nt - H);
      o.pushHistorySnapshot();
      const vt = (gt) => {
        const { x: xt, y: ht } = o.screenToCanvas(
          gt.clientX,
          gt.clientY
        ), Dt = Math.atan2(ht - _, xt - H);
        let Tt = tt + (Dt - pt) * (180 / Math.PI);
        (gt.shiftKey || o.snapToGrid) && !(gt.metaKey || gt.ctrlKey) && (Tt = Math.round(Tt / 15) * 15), o.updateNode(t.id, { rotation: Tt });
      }, rt = () => {
        j.removeEventListener("pointermove", vt), j.removeEventListener("pointerup", rt);
      };
      j.addEventListener("pointermove", vt), j.addEventListener("pointerup", rt);
    },
    [o, t.id, t.x, t.y, t.w, c, t.rotation]
  ), Q = i && x ? {
    left: x.x + b.x * x.w,
    top: x.y + b.y * x.h,
    width: b.w * x.w,
    height: b.h * x.h
  } : null, $ = i ? void 0 : [t.data.flipH ? "scaleX(-1)" : "", t.data.flipV ? "scaleY(-1)" : ""].filter(Boolean).join(" ") || void 0, P = {
    width: "100%",
    height: "100%",
    objectFit: "contain",
    display: "block",
    pointerEvents: "none",
    opacity: t.data.opacity ?? 1,
    transform: $
  };
  if (!i && h) {
    const N = h.y * 100, j = (1 - h.x - h.w) * 100, H = (1 - h.y - h.h) * 100, _ = h.x * 100;
    P.objectViewBox = `inset(${N}% ${j}% ${H}% ${_}%)`;
  }
  const G = 8 / r, X = G / 2;
  return /* @__PURE__ */ k(
    "div",
    {
      onPointerDown: I,
      onDoubleClick: !i && n ? (N) => {
        N.stopPropagation(), l == null || l();
      } : void 0,
      style: {
        position: "absolute",
        left: t.x + t.w / 2,
        top: t.y + c / 2,
        width: t.w,
        height: c,
        marginLeft: -t.w / 2,
        marginTop: -c / 2,
        zIndex: t.z,
        border: e ? "2px dashed #3b82f6" : "none",
        borderRadius: 6,
        overflow: "visible",
        pointerEvents: n ? "auto" : "none",
        cursor: i ? "default" : "move",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        boxSizing: "border-box"
      },
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              overflow: "hidden",
              borderRadius: 4,
              border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : "none",
              boxSizing: "border-box"
            },
            children: [
              /* @__PURE__ */ u(
                "img",
                {
                  ref: d,
                  src: t.data.src,
                  alt: t.data.alt ?? "",
                  onLoad: y,
                  style: P,
                  draggable: !1
                }
              ),
              i && Q && /* @__PURE__ */ u(
                "div",
                {
                  onPointerDown: W,
                  style: {
                    position: "absolute",
                    left: Q.left,
                    top: Q.top,
                    width: Q.width,
                    height: Q.height,
                    boxShadow: `0 0 0 ${Math.max(t.w, c) * 2}px rgba(0,0,0,0.45)`,
                    border: `${1.5 / r}px dashed rgba(255,255,255,0.8)`,
                    boxSizing: "border-box",
                    cursor: "move",
                    zIndex: 10
                  }
                }
              )
            ]
          }
        ),
        i && Q && pd.map(({ pos: N, edges: j, cx: H, cy: _, cursor: tt }) => /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: (nt) => z(j, nt),
            style: {
              position: "absolute",
              left: Q.left + H * Q.width - X,
              top: Q.top + _ * Q.height - X,
              width: G,
              height: G,
              background: "white",
              border: `${1.5 / r}px solid #3b82f6`,
              borderRadius: 2,
              cursor: tt,
              zIndex: 11
            }
          },
          N
        )),
        e && !i && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                left: "50%",
                top: -lt,
                width: 1,
                height: lt,
                background: "#3b82f6",
                marginLeft: -0.5,
                pointerEvents: "none"
              }
            }
          ),
          /* @__PURE__ */ u(
            "div",
            {
              onPointerDown: ot,
              style: {
                position: "absolute",
                left: "50%",
                top: -(lt + K / 2),
                width: K,
                height: K,
                marginLeft: -K / 2,
                borderRadius: "50%",
                background: "white",
                border: "1.5px solid #3b82f6",
                cursor: "grab"
              }
            }
          )
        ] }),
        O && R.map(({ pos: N, cx: j, cy: H }) => /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: (_) => {
              _.stopPropagation(), s == null || s(t.id, N, _);
            },
            style: {
              position: "absolute",
              left: `calc(${j * 100}% - ${Z}px)`,
              top: `calc(${H * 100}% - ${Z}px)`,
              width: K,
              height: K,
              background: "white",
              border: "1.5px solid #3b82f6",
              borderRadius: 2,
              cursor: On(N, t.rotation || 0)
            }
          },
          N
        ))
      ]
    }
  );
}
const Ui = ue(fd);
function yd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Ui,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      onResizeHandleDown: t.callbacks.onResizeHandleDown,
      cropping: t.editing,
      onCropStart: () => {
        var o, n;
        return (n = (o = t.callbacks).onEditStart) == null ? void 0 : n.call(o, e.id);
      },
      onCropEnd: () => {
        var o, n;
        return (n = (o = t.callbacks).onEditEnd) == null ? void 0 : n.call(o);
      }
    }
  );
}
const md = {
  type: "image",
  component: yd,
  handlesOwnLayout: !0,
  onFlip: (t, e) => {
    const o = t.data;
    return e === "h" ? { flipH: !o.flipH } : { flipV: !o.flipV };
  },
  getClipboardText: (t) => t.data.src || null
};
function bd({
  node: t,
  engine: e,
  editing: o,
  editClickPos: n,
  onStopEdit: r,
  onMeasuredHeight: s
}) {
  const i = at(null), [l, a] = et(t.data.text), c = at(!1), h = at(t.data.text), p = at(null), d = at(e);
  d.current = e;
  const f = at(t);
  f.current = t, yt(() => {
    o || a(t.data.text);
  }, [t.data.text]), Hr(() => {
    var z, W;
    if (o && i.current) {
      i.current.innerText = t.data.text, i.current.focus();
      const I = i.current.ownerDocument;
      let R = !1;
      if (n) {
        const K = I.caretRangeFromPoint(n.clientX, n.clientY);
        if (K && i.current.contains(K.startContainer)) {
          const Z = (z = I.defaultView) == null ? void 0 : z.getSelection();
          Z == null || Z.removeAllRanges(), Z == null || Z.addRange(K), R = !0;
        }
      }
      if (!R) {
        const K = I.createRange(), Z = (W = I.defaultView) == null ? void 0 : W.getSelection();
        i.current.childNodes.length > 0 && (K.selectNodeContents(i.current), K.collapse(!1)), Z == null || Z.removeAllRanges(), Z == null || Z.addRange(K);
      }
      h.current = t.data.text, c.current = !1;
    }
  }, [o]), yt(() => {
    if (o)
      return () => {
        if (c.current) return;
        c.current = !0;
        const z = h.current, W = e.getNode(t.id);
        if (W && W.type === "text") {
          const I = W.data;
          z !== I.text && e.updateNodeWithHistory(t.id, {
            data: { ...I, text: z }
          });
        }
      };
  }, [o, e, t.id]), yt(() => {
    if (!i.current || !s) return;
    const z = new ResizeObserver(() => {
      var I;
      const W = ((I = i.current) == null ? void 0 : I.offsetHeight) ?? 0;
      W > 0 && s(t.id, W);
    });
    return z.observe(i.current), () => z.disconnect();
  }, [t.id, s, o]);
  const m = st(() => {
    var W;
    if (c.current) return;
    c.current = !0, p.current && (clearTimeout(p.current), p.current = null);
    const z = ((W = i.current) == null ? void 0 : W.innerText) ?? "";
    a(z), h.current = z, z !== t.data.text && e.updateNodeWithHistory(t.id, {
      data: { ...t.data, text: z }
    }), r();
  }, [e, t, r]), y = st(
    (z) => {
      var W;
      z.key === "Escape" && (z.preventDefault(), m(), (W = i.current) == null || W.blur()), z.stopPropagation();
    },
    [m]
  ), b = st(() => {
    m();
  }, [m]), g = st(() => {
    if (i.current) {
      const z = i.current.innerText;
      a(z), h.current = z, p.current && clearTimeout(p.current), p.current = setTimeout(() => {
        const W = f.current;
        z !== W.data.text && d.current.updateNode(W.id, {
          data: { ...W.data, text: z }
        });
      }, 0);
    }
  }, []), x = t.h === "auto" ? void 0 : t.h, S = t.data.opacity ?? 1, M = {
    fontFamily: Je(t.data.fontFamily),
    fontSize: t.data.fontSize,
    color: t.data.color,
    textAlign: t.data.align,
    opacity: S,
    lineHeight: 1,
    outline: "none",
    whiteSpace: "pre-wrap",
    wordBreak: "break-word",
    border: t.data.borderColor ? `${t.data.borderWidth ?? 1}px ${t.data.borderStyle ?? "solid"} ${t.data.borderColor}` : void 0,
    boxSizing: t.data.borderColor ? "border-box" : void 0,
    borderRadius: t.data.borderColor ? 4 : void 0,
    padding: t.data.borderColor ? 6 : void 0
  };
  return /* @__PURE__ */ u(
    "div",
    {
      "data-node-id": t.id,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: x,
        zIndex: t.z,
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        pointerEvents: o ? "auto" : "none"
      },
      children: o ? /* @__PURE__ */ u(
        "div",
        {
          ref: i,
          contentEditable: !0,
          suppressContentEditableWarning: !0,
          onKeyDown: y,
          onBlur: b,
          onInput: g,
          onPointerDown: (z) => z.stopPropagation(),
          style: { ...M, minHeight: t.data.fontSize, cursor: "text" }
        }
      ) : /* @__PURE__ */ u("div", { ref: i, style: M, children: l || " " })
    }
  );
}
const Qi = ue(bd);
function gd(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Qi,
    {
      node: e,
      engine: t.engine,
      editing: t.editing,
      editClickPos: t.editClickPos,
      onStopEdit: () => {
        var o, n;
        return (n = (o = t.callbacks).onEditEnd) == null ? void 0 : n.call(o);
      },
      onMeasuredHeight: t.callbacks.onMeasuredHeight
    }
  );
}
const xd = {
  type: "text",
  component: gd,
  handlesOwnLayout: !0,
  onResize: (t, e, o) => ({ fontSize: t.data.fontSize * e }),
  getClipboardText: (t) => t.data.text || null
};
function wd(t) {
  const e = t.node, o = e.h === "auto" ? 100 : e.h, n = st(
    (s) => {
      var l, a;
      const i = s.currentTarget.value.trim();
      t.engine.updateNodeWithHistory(e.id, {
        data: { ...e.data, label: i || void 0 }
      }), (a = (l = t.callbacks).onEditEnd) == null || a.call(l);
    },
    [e.id, e.data, t.engine, t.callbacks]
  ), r = st(
    (s) => {
      (s.key === "Enter" || s.key === "Escape") && s.currentTarget.blur(), s.stopPropagation();
    },
    []
  );
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        position: "absolute",
        left: e.x,
        top: e.y,
        width: e.w,
        height: o,
        zIndex: e.z,
        background: e.data.backgroundColor || "rgba(0,0,0,0.02)",
        border: `${e.data.borderWidth || 1}px ${e.data.borderStyle || "dashed"} ${e.data.borderColor || "#ccc"}`,
        boxSizing: "border-box",
        borderRadius: 8,
        opacity: e.data.opacity ?? 1,
        pointerEvents: "none",
        overflow: "visible",
        transform: e.rotation ? `rotate(${e.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: t.editing ? /* @__PURE__ */ u(
        "input",
        {
          autoFocus: !0,
          defaultValue: e.data.label ?? "",
          placeholder: "Frame label...",
          onBlur: n,
          onKeyDown: r,
          onPointerDown: (s) => s.stopPropagation(),
          style: {
            position: "absolute",
            top: -24,
            left: 0,
            fontSize: 12,
            color: e.data.borderColor || "#999",
            fontWeight: 500,
            background: "rgba(255,255,255,0.95)",
            border: "1px solid #3b82f6",
            borderRadius: 4,
            padding: "1px 4px",
            outline: "none",
            pointerEvents: "auto",
            minWidth: 80
          }
        }
      ) : e.data.label ? /* @__PURE__ */ u(
        "div",
        {
          onDoubleClick: (s) => {
            var i, l;
            s.stopPropagation(), t.engine.select(e.id), (l = (i = t.callbacks).onEditStart) == null || l.call(i, e.id);
          },
          style: {
            position: "absolute",
            top: -20,
            left: 4,
            fontSize: 12,
            color: e.data.borderColor || "#999",
            fontWeight: 500,
            whiteSpace: "nowrap",
            userSelect: "none",
            pointerEvents: "auto",
            cursor: "default"
          },
          children: e.data.label
        }
      ) : null
    }
  );
}
const kd = {
  type: "frame",
  component: wd,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.label || null
}, vd = 100;
function Sd({
  node: t,
  isSelected: e,
  engine: o,
  interactive: n,
  zoom: r,
  editing: s,
  onEditStart: i,
  onEditEnd: l
}) {
  const a = at(null), c = at(null), h = at(""), p = at(null), d = at(null), f = at(t);
  f.current = t;
  const m = at(o);
  m.current = o, yt(() => {
    var M;
    if (s && c.current) {
      const z = c.current;
      z.innerText = t.data.text || "", h.current = t.data.text || "", z.focus();
      const W = z.ownerDocument, I = (M = W.defaultView) == null ? void 0 : M.getSelection(), R = p.current;
      p.current = null;
      let K = !1;
      if (R && I && W.caretRangeFromPoint) {
        const Z = W.caretRangeFromPoint(R.x, R.y);
        Z && z.contains(Z.startContainer) && (I.removeAllRanges(), I.addRange(Z), K = !0);
      }
      if (!K && I) {
        const Z = W.createRange();
        z.childNodes.length > 0 && (Z.selectNodeContents(z), Z.collapse(!1)), I.removeAllRanges(), I.addRange(Z);
      }
    }
  }, [s]), yt(() => {
    if (s)
      return () => {
        const M = f.current, z = h.current;
        z !== M.data.text && m.current.updateNodeWithHistory(M.id, {
          data: { ...M.data, text: z }
        });
      };
  }, [s]);
  const y = st(() => {
    d.current && (clearTimeout(d.current), d.current = null), c.current && (h.current = c.current.innerText), l();
  }, [l]), b = st(
    (M) => {
      const z = M.currentTarget.ownerDocument;
      if (M.altKey) return;
      if (!o.selection.has(t.id) && o.selection.size > 0) {
        const { x: X, y: N } = o.screenToCanvas(M.clientX, M.clientY);
        for (const j of o.selection) {
          const H = o.getNode(j);
          if (!H) continue;
          const _ = H.h === "auto" ? 100 : H.h;
          if (X >= H.x && X <= H.x + H.w && N >= H.y && N <= H.y + _)
            return;
        }
      }
      if (M.stopPropagation(), s) return;
      M.currentTarget.setPointerCapture(M.pointerId), M.shiftKey ? o.toggleSelect(t.id) : o.selection.has(t.id) || o.select(t.id);
      const W = M.clientX, I = M.clientY, R = Array.from(o.selection), K = [];
      for (const X of R) {
        const N = o.getNode(X);
        N && K.push({ id: X, x: N.x, y: N.y });
      }
      if (K.length === 0) return;
      let Z = !1, lt = null, O = W, ot = I, Q = !1;
      const $ = () => {
        lt = null;
        const X = (O - W) / o.viewport.zoom, N = (ot - I) / o.viewport.zoom, { finalDx: j, finalDy: H } = o.computeDragSnap(
          K,
          R,
          X,
          N,
          Q
        ), _ = K.map((tt) => ({
          id: tt.id,
          patch: { x: tt.x + j, y: tt.y + H }
        }));
        o.updateMany(_);
      }, P = (X) => {
        const N = (X.clientX - W) / o.viewport.zoom, j = (X.clientY - I) / o.viewport.zoom;
        if (!Z)
          if (Math.abs(N) > 2 || Math.abs(j) > 2)
            Z = !0, o.pushHistorySnapshot();
          else
            return;
        O = X.clientX, ot = X.clientY, Q = X.metaKey || X.ctrlKey, lt === null && (lt = requestAnimationFrame($));
      }, G = () => {
        lt !== null && (cancelAnimationFrame(lt), $()), o.clearAlignGuides(), z.removeEventListener("pointermove", P), z.removeEventListener("pointerup", G);
      };
      z.addEventListener("pointermove", P), z.addEventListener("pointerup", G);
    },
    [o, t.id, s]
  ), g = st(
    (M) => {
      if (n) {
        if (M.stopPropagation(), t.groupId) {
          const z = [];
          let W = t.groupId;
          for (; W; )
            z.push(W), W = o.groupParent.get(W);
          if (!o.activeGroupId) {
            o.enterGroup(z[z.length - 1]), o.select(t.id);
            return;
          }
          const I = z.indexOf(o.activeGroupId);
          if (I > 0) {
            o.enterGroup(z[I - 1]), o.select(t.id);
            return;
          }
        }
        s || (p.current = { x: M.clientX, y: M.clientY }, o.select(t.id), i(t.id));
      }
    },
    [n, s, o, t.id, t.groupId, i]
  ), x = t.data.fontSize ?? 16, S = t.h === "auto" ? vd : t.h;
  return /* @__PURE__ */ u(
    "div",
    {
      ref: a,
      "data-node-id": t.id,
      className: n ? void 0 : "sb-block-inert",
      onPointerDown: n ? b : void 0,
      onDoubleClick: g,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: S,
        zIndex: t.z,
        background: t.data.color,
        borderRadius: t.data.edgeStyle === "round" ? 12 : 2,
        boxShadow: "0 2px 8px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.08)",
        opacity: t.data.opacity ?? 1,
        pointerEvents: n ? "auto" : "none",
        outline: "none",
        overflow: "hidden",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center"
      },
      children: /* @__PURE__ */ u(
        "div",
        {
          style: {
            padding: 12,
            height: "100%",
            overflow: "auto",
            touchAction: s ? "pan-y" : "none",
            cursor: s ? "text" : "move",
            userSelect: s ? "text" : "none"
          },
          children: s ? /* @__PURE__ */ u(
            "div",
            {
              ref: c,
              contentEditable: !0,
              suppressContentEditableWarning: !0,
              onBlur: y,
              onInput: () => {
                c.current && (h.current = c.current.innerText, d.current && clearTimeout(d.current), d.current = setTimeout(() => {
                  const M = f.current, z = h.current;
                  z !== M.data.text && m.current.updateNode(M.id, {
                    data: { ...M.data, text: z }
                  });
                }, 0));
              },
              onKeyDown: (M) => {
                M.key === "Escape" && (M.stopPropagation(), y()), M.stopPropagation();
              },
              onPointerDown: (M) => M.stopPropagation(),
              style: {
                fontSize: x,
                fontFamily: Je(Qe),
                lineHeight: 1.5,
                color: "#1e1e2e",
                wordWrap: "break-word",
                whiteSpace: "pre-wrap",
                outline: "none",
                minHeight: "100%"
              }
            }
          ) : /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: x,
                fontFamily: Je(Qe),
                lineHeight: 1.5,
                color: "#1e1e2e",
                wordWrap: "break-word",
                whiteSpace: "pre-wrap"
              },
              children: t.data.text || ""
            }
          )
        }
      )
    }
  );
}
const Ji = ue(Sd);
function Md(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Ji,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      editing: t.editing,
      onEditStart: (o) => {
        var n, r;
        return (r = (n = t.callbacks).onEditStart) == null ? void 0 : r.call(n, o);
      },
      onEditEnd: () => {
        var o, n;
        return (n = (o = t.callbacks).onEditEnd) == null ? void 0 : n.call(o);
      }
    }
  );
}
const Cd = {
  type: "sticky",
  component: Md,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.text || null
}, $i = /(?:youtube\.com\/(?:watch\?.*v=|embed\/|shorts\/|live\/)|youtu\.be\/)([\w-]{11})/;
function zd(t) {
  const e = t.match($i);
  return e ? e[1] : null;
}
function Id(t) {
  return $i.test(t);
}
function Td(t) {
  return `https://www.youtube.com/embed/${t}`;
}
function Pd(t) {
  return `https://img.youtube.com/vi/${t}/hqdefault.jpg`;
}
function Rd({
  node: t,
  isSelected: e,
  engine: o,
  interactive: n,
  zoom: r,
  editing: s,
  onResizeHandleDown: i,
  onEditStart: l
}) {
  const a = t.h, { data: c } = t, h = (m) => {
    if (n && s) {
      m.stopPropagation();
      return;
    }
  }, p = c.borderColor ? `${c.borderWidth ?? 1}px ${c.borderStyle ?? "solid"} ${c.borderColor}` : "none", d = Math.max(6, 8 / r), f = [
    { key: "nw", x: "0%", y: "0%", cursor: "nwse-resize" },
    { key: "ne", x: "100%", y: "0%", cursor: "nesw-resize" },
    { key: "se", x: "100%", y: "100%", cursor: "nwse-resize" },
    { key: "sw", x: "0%", y: "100%", cursor: "nesw-resize" },
    { key: "n", x: "50%", y: "0%", cursor: "ns-resize" },
    { key: "s", x: "50%", y: "100%", cursor: "ns-resize" },
    { key: "e", x: "100%", y: "50%", cursor: "ew-resize" },
    { key: "w", x: "0%", y: "50%", cursor: "ew-resize" }
  ];
  return /* @__PURE__ */ k(
    "div",
    {
      onPointerDown: h,
      onDoubleClick: !s && n ? (m) => {
        m.stopPropagation(), l == null || l();
      } : void 0,
      style: {
        position: "absolute",
        left: t.x + t.w / 2,
        top: t.y + a / 2,
        width: t.w,
        height: a,
        marginLeft: -t.w / 2,
        marginTop: -a / 2,
        zIndex: t.z,
        border: e ? "2px dashed #3b82f6" : "none",
        borderRadius: 6,
        overflow: "visible",
        pointerEvents: n ? "auto" : "none",
        cursor: s ? "default" : "move",
        transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
        transformOrigin: "center center",
        boxSizing: "border-box"
      },
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              overflow: "hidden",
              borderRadius: 4,
              border: p,
              boxSizing: "border-box",
              opacity: c.opacity ?? 1
            },
            children: [
              /* @__PURE__ */ u(
                "iframe",
                {
                  src: Td(c.videoId),
                  title: "YouTube video",
                  allow: "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture",
                  allowFullScreen: !0,
                  style: {
                    width: "100%",
                    height: "100%",
                    border: "none",
                    display: "block",
                    pointerEvents: s ? "auto" : "none"
                  }
                }
              ),
              !s && /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    position: "absolute",
                    inset: 0,
                    cursor: "move"
                  }
                }
              )
            ]
          }
        ),
        e && n && !s && f.map((m) => /* @__PURE__ */ u(
          "div",
          {
            "data-handle": m.key,
            onPointerDown: (y) => {
              y.stopPropagation(), i == null || i(t.id, m.key, y);
            },
            style: {
              position: "absolute",
              left: m.x,
              top: m.y,
              width: d,
              height: d,
              marginLeft: -d / 2,
              marginTop: -d / 2,
              background: "#fff",
              border: "1px solid #3b82f6",
              borderRadius: 2,
              cursor: m.cursor,
              zIndex: 1
            }
          },
          m.key
        ))
      ]
    }
  );
}
const Ad = ue(Rd);
function Ld(t) {
  const e = t.node;
  return /* @__PURE__ */ u(
    Ad,
    {
      node: e,
      isSelected: t.isSelected,
      engine: t.engine,
      interactive: t.interactive,
      zoom: t.zoom,
      editing: t.editing,
      onResizeHandleDown: t.callbacks.onResizeHandleDown,
      onEditStart: () => {
        var o, n;
        return (n = (o = t.callbacks).onEditStart) == null ? void 0 : n.call(o, e.id);
      },
      onEditEnd: () => {
        var o, n;
        return (n = (o = t.callbacks).onEditEnd) == null ? void 0 : n.call(o);
      }
    }
  );
}
const Ed = {
  type: "youtube",
  component: Ld,
  handlesOwnLayout: !0,
  getClipboardText: (t) => t.data.url || null
}, Dd = [
  dc,
  ld,
  dd,
  ud,
  md,
  xd,
  kd,
  Cd,
  Ed
];
function lo(t, e) {
  return `${t}:${e}`;
}
class Wd {
  constructor(e, o) {
    Ct(this, "spatial");
    Ct(this, "registry");
    /** Current resolved port values. */
    Ct(this, "values", /* @__PURE__ */ new Map());
    /** Node IDs that need recomputation. */
    Ct(this, "dirty", /* @__PURE__ */ new Set());
    /** Whether a microtask flush is already scheduled. */
    Ct(this, "scheduled", !1);
    /** Generation counter for canceling stale async results. */
    Ct(this, "generation", 0);
    /** Change subscribers. */
    Ct(this, "listeners", /* @__PURE__ */ new Set());
    /** Node IDs that are part of a cycle (updated after each topoSort). */
    Ct(this, "_cycleNodeIds", /* @__PURE__ */ new Set());
    this.spatial = e, this.registry = o;
  }
  // ── Public API ─────────────────────────────────────────────
  /** Node IDs that are part of a dependency cycle (read-only). */
  get cycleNodeIds() {
    return this._cycleNodeIds;
  }
  /** Subscribe to port value changes. Returns unsubscribe function. */
  onChange(e) {
    return this.listeners.add(e), () => this.listeners.delete(e);
  }
  /** Get current value of a specific port. */
  getPortValue(e, o) {
    return this.values.get(lo(e, o)) ?? null;
  }
  /** Get all input values for a node, resolved from connected edges. */
  getInputs(e) {
    var s;
    const o = this.registry.get(
      ((s = this.spatial.nodes.get(e)) == null ? void 0 : s.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const n = {}, r = o.ports.filter((i) => i.direction === "input");
    for (const i of r) {
      const l = this.spatial.getEdgesForNode(e);
      let a = !1;
      for (const c of l) {
        const h = c.data;
        if (h.toId === e && h.targetPort === i.id) {
          const p = this.values.get(
            lo(h.fromId, h.sourcePort ?? "")
          );
          n[i.id] = p ?? i.defaultValue ?? null, a = !0;
          break;
        }
      }
      a || (n[i.id] = i.defaultValue ?? null);
    }
    return n;
  }
  /** Get all output values for a node. */
  getOutputs(e) {
    var r;
    const o = this.registry.get(
      ((r = this.spatial.nodes.get(e)) == null ? void 0 : r.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const n = {};
    for (const s of o.ports)
      s.direction === "output" && (n[s.id] = this.values.get(lo(e, s.id)) ?? null);
    return n;
  }
  /** Get all port values (inputs + outputs) for a node. */
  getAllPortValues(e) {
    var r;
    const o = this.registry.get(
      ((r = this.spatial.nodes.get(e)) == null ? void 0 : r.type) ?? ""
    );
    if (!(o != null && o.ports)) return {};
    const n = {};
    for (const s of o.ports)
      if (s.direction === "input") {
        const i = this.spatial.getEdgesForNode(e);
        let l = !1;
        for (const a of i) {
          const c = a.data;
          if (c.toId === e && c.targetPort === s.id) {
            n[s.id] = this.values.get(lo(c.fromId, c.sourcePort ?? "")) ?? s.defaultValue ?? null, l = !0;
            break;
          }
        }
        l || (n[s.id] = s.defaultValue ?? null);
      } else
        n[s.id] = this.values.get(lo(e, s.id)) ?? null;
    return n;
  }
  /** Mark a node as dirty and schedule recomputation. */
  markDirty(e) {
    this.dirty.add(e), this.scheduleFlush();
  }
  /** Wire up SpatialEngine event listeners. Returns cleanup function. */
  connect() {
    const e = (r) => {
      const s = this.registry.get(r.type);
      s != null && s.ports && this.markDirty(r.id);
    }, o = (r) => {
      if (r.type === "edge") {
        const s = r.data;
        s.targetPort && this.markDirty(s.toId);
      } else {
        const s = this.registry.get(r.type);
        s != null && s.ports && s.compute && this.markDirty(r.id);
      }
    }, n = (r) => {
      if (r.type === "edge") {
        const s = r.data;
        s.targetPort && this.markDirty(s.toId);
      } else {
        const s = this.registry.get(r.type);
        if (s != null && s.ports) {
          for (const i of s.ports)
            this.values.delete(lo(r.id, i.id));
          this.markDownstream(r.id);
        }
      }
    };
    return this.spatial.on("node:data", e), this.spatial.on("node:create", o), this.spatial.on("node:delete", n), this.initializeAll(), () => {
      this.spatial.off("node:data", e), this.spatial.off("node:create", o), this.spatial.off("node:delete", n);
    };
  }
  /** Dispose and clean up. */
  dispose() {
    this.values.clear(), this.dirty.clear(), this.listeners.clear(), this.scheduled = !1;
  }
  // ── Private implementation ─────────────────────────────────
  /** Initialize all nodes with ports. */
  initializeAll() {
    for (const e of this.spatial.nodes.values()) {
      const o = this.registry.get(e.type);
      o != null && o.ports && o.compute && this.dirty.add(e.id);
    }
    this.dirty.size > 0 && this.scheduleFlush();
  }
  /** Schedule a microtask flush if not already scheduled. */
  scheduleFlush() {
    this.scheduled || (this.scheduled = !0, queueMicrotask(() => {
      this.scheduled = !1, this.flush();
    }));
  }
  /** Mark all downstream nodes (nodes that depend on outputs of nodeId) as dirty. */
  markDownstream(e) {
    const o = this.spatial.getEdgesForNode(e);
    for (const n of o) {
      const r = n.data;
      r.fromId === e && r.targetPort && this.dirty.add(r.toId);
    }
  }
  /** Topological sort of dirty nodes + their downstream dependents. */
  topoSort() {
    const e = /* @__PURE__ */ new Set();
    for (const m of this.spatial.nodes.values()) {
      const y = this.registry.get(m.type);
      y != null && y.ports && y.compute && e.add(m.id);
    }
    if (e.size === 0) {
      const m = this._cycleNodeIds.size > 0;
      return m && (this._cycleNodeIds = /* @__PURE__ */ new Set()), { sorted: [], cyclesChanged: m };
    }
    const o = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map();
    for (const m of e)
      o.set(m, /* @__PURE__ */ new Set()), n.set(m, 0);
    const r = this.spatial.getAllEdges();
    for (const m of r) {
      const y = m.data;
      y.sourcePort && y.targetPort && e.has(y.fromId) && e.has(y.toId) && (o.get(y.fromId).add(y.toId), n.set(y.toId, (n.get(y.toId) ?? 0) + 1));
    }
    const s = new Set(this.dirty), i = /* @__PURE__ */ new Set(), l = (m) => {
      if (i.has(m)) return;
      i.add(m);
      const y = o.get(m);
      if (y)
        for (const b of y)
          s.add(b), l(b);
    };
    for (const m of [...this.dirty])
      l(m);
    const a = /* @__PURE__ */ new Map();
    for (const m of s)
      a.set(m, 0);
    for (const m of r) {
      const y = m.data;
      y.sourcePort && y.targetPort && s.has(y.fromId) && s.has(y.toId) && a.set(
        y.toId,
        (a.get(y.toId) ?? 0) + 1
      );
    }
    const c = [];
    for (const [m, y] of a)
      y === 0 && c.push(m);
    const h = [];
    for (; c.length > 0; ) {
      const m = c.shift();
      h.push(m);
      const y = o.get(m);
      if (y)
        for (const b of y) {
          if (!s.has(b)) continue;
          const g = (a.get(b) ?? 1) - 1;
          a.set(b, g), g === 0 && c.push(b);
        }
    }
    const p = new Set(h), d = /* @__PURE__ */ new Set();
    for (const m of s)
      p.has(m) || d.add(m);
    let f = !1;
    return (d.size !== this._cycleNodeIds.size || [...d].some((m) => !this._cycleNodeIds.has(m))) && (this._cycleNodeIds = d, f = !0), { sorted: h, cyclesChanged: f };
  }
  /** Full graph recompute of dirty nodes. */
  flush() {
    if (this.dirty.size === 0) return;
    const { sorted: e, cyclesChanged: o } = this.topoSort();
    this.dirty.clear();
    let n = !1;
    for (const r of e)
      this.executeNode(r) && (n = !0);
    (n || o) && this.notifyListeners();
  }
  /** Execute a single node's compute function. Returns true if outputs changed. */
  executeNode(e) {
    const o = this.spatial.nodes.get(e);
    if (!o) return !1;
    const n = this.registry.get(o.type);
    if (!(n != null && n.compute) || !n.ports) return !1;
    const r = this.getInputs(e), s = n.compute(r, o.data);
    if (s instanceof Promise) {
      const i = ++this.generation;
      return s.then((l) => {
        if (i !== this.generation) return;
        this.applyOutputs(e, n.ports, l) && (this.markDownstream(e), this.notifyListeners(), this.dirty.size > 0 && this.scheduleFlush());
      }), !1;
    }
    return this.applyOutputs(e, n.ports, s);
  }
  /** Apply computed outputs to the values map. Returns true if any value changed. */
  applyOutputs(e, o, n) {
    let r = !1;
    for (const s of o) {
      if (s.direction !== "output") continue;
      const i = lo(e, s.id), l = n[s.id] ?? null, a = this.values.get(i) ?? null;
      Fd(a, l) || (this.values.set(i, l), r = !0);
    }
    return r && this.markDownstream(e), r;
  }
  /** Notify all change listeners. */
  notifyListeners() {
    for (const e of this.listeners)
      e();
  }
}
function Fd(t, e) {
  if (t === e) return !0;
  if (t == null || e == null || typeof t != typeof e) return !1;
  if (typeof t == "object" && typeof e == "object") {
    const o = Object.keys(t), n = Object.keys(e);
    if (o.length !== n.length) return !1;
    for (const r of o)
      if (t[r] !== e[r])
        return !1;
    return !0;
  }
  return !1;
}
const Xo = [
  // Light
  { key: "plain-white", label: "White", group: "light", canvasBg: "#ffffff", swatchColor: "#ffffff" },
  { key: "dot-grid", label: "Cream", group: "light", canvasBg: "#f8f7f5", swatchColor: "#f8f7f5" },
  { key: "engineering", label: "Warm", group: "light", canvasBg: "#fdf6e3", swatchColor: "#fdf6e3" },
  // Dark
  { key: "blueprint", label: "Blueprint", group: "dark", canvasBg: "#1e3a5f", swatchColor: "#1e3a5f" },
  { key: "dark-grid", label: "Night", group: "dark", canvasBg: "#1a1a2e", swatchColor: "#1a1a2e" },
  // Textured
  { key: "japanese-stationery", label: "Japanese Stationery", group: "textured", canvasBg: "#f5f0e8", swatchColor: "#f5f0e8" },
  { key: "kraft", label: "Kraft Paper", group: "textured", canvasBg: "#d4b896", swatchColor: "#d4b896" }
];
function ln(t) {
  return Xo.find((e) => e.key === t) ?? Xo[1];
}
function Bd() {
  return {
    staticDefs: /* @__PURE__ */ k("filter", { id: "paper-texture", x: "-20%", y: "-20%", width: "140%", height: "140%", children: [
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.08", numOctaves: 4, seed: 12, stitchTiles: "stitch", result: "noise" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "noise", type: "saturate", values: "0", result: "bump" }),
      /* @__PURE__ */ u("feDiffuseLighting", { in: "bump", lightingColor: "#f7f4ee", surfaceScale: "1.2", diffuseConstant: "1", result: "lit", children: /* @__PURE__ */ u("feDistantLight", { azimuth: "225", elevation: "50" }) }),
      /* @__PURE__ */ u("feComposite", { in: "lit", in2: "bump", operator: "in", result: "lit-masked" }),
      /* @__PURE__ */ u("feFlood", { floodColor: "#f5f0e8", result: "base" }),
      /* @__PURE__ */ u("feBlend", { in: "base", in2: "lit-masked", mode: "overlay", result: "paper" }),
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.6", numOctaves: 3, seed: 7, stitchTiles: "stitch", result: "grain" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "grain", type: "saturate", values: "0", result: "grain-gray" }),
      /* @__PURE__ */ k("feComponentTransfer", { in: "grain-gray", result: "grain-subtle", children: [
        /* @__PURE__ */ u("feFuncR", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncG", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncB", { type: "linear", slope: "0.06", intercept: "0.47" })
      ] }),
      /* @__PURE__ */ u("feBlend", { in: "paper", in2: "grain-subtle", mode: "overlay", result: "paper-final" })
    ] }),
    staticLayers: [
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "#f5f0e8", filter: "url(#paper-texture)" }, "texture")
    ]
  };
}
function Nd() {
  return {
    staticDefs: /* @__PURE__ */ k("filter", { id: "kraft-texture", x: "-20%", y: "-20%", width: "140%", height: "140%", children: [
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.04", numOctaves: 5, seed: 42, stitchTiles: "stitch", result: "noise" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "noise", type: "saturate", values: "0", result: "bump" }),
      /* @__PURE__ */ u("feDiffuseLighting", { in: "bump", lightingColor: "#e0c9a6", surfaceScale: "1.4", diffuseConstant: "0.95", result: "lit", children: /* @__PURE__ */ u("feDistantLight", { azimuth: "200", elevation: "50" }) }),
      /* @__PURE__ */ u("feComposite", { in: "lit", in2: "bump", operator: "in", result: "lit-masked" }),
      /* @__PURE__ */ u("feFlood", { floodColor: "#d4b896", result: "base" }),
      /* @__PURE__ */ u("feBlend", { in: "base", in2: "lit-masked", mode: "overlay", result: "kraft" }),
      /* @__PURE__ */ u("feTurbulence", { type: "fractalNoise", baseFrequency: "0.35", numOctaves: 2, seed: 99, stitchTiles: "stitch", result: "fiber" }),
      /* @__PURE__ */ u("feColorMatrix", { in: "fiber", type: "saturate", values: "0", result: "fiber-gray" }),
      /* @__PURE__ */ k("feComponentTransfer", { in: "fiber-gray", result: "fiber-subtle", children: [
        /* @__PURE__ */ u("feFuncR", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncG", { type: "linear", slope: "0.06", intercept: "0.47" }),
        /* @__PURE__ */ u("feFuncB", { type: "linear", slope: "0.06", intercept: "0.47" })
      ] }),
      /* @__PURE__ */ u("feBlend", { in: "kraft", in2: "fiber-subtle", mode: "overlay", result: "kraft-final" })
    ] }),
    staticLayers: [
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "#d4b896", filter: "url(#kraft-texture)" }, "texture")
    ]
  };
}
const ur = {
  "japanese-stationery": Bd,
  kraft: Nd
};
function Hd(t) {
  var e;
  return ((e = ur[t]) == null ? void 0 : e.call(ur)) ?? {};
}
const _i = {
  position: "absolute",
  inset: 0,
  width: "100%",
  height: "100%",
  pointerEvents: "none"
}, Od = {
  ..._i,
  willChange: "transform"
}, Xd = ue(function({
  background: e
}) {
  const o = ln(e), { staticDefs: n, staticLayers: r } = Hd(e);
  return /* @__PURE__ */ k("svg", { style: Od, children: [
    n && /* @__PURE__ */ u("defs", { children: n }),
    /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: o.canvasBg }),
    r
  ] });
});
function Yd({
  viewport: t,
  gridSize: e = 20,
  background: o = "dot-grid",
  gridVisible: n = !0
}) {
  const r = e * t.zoom, s = t.x % r, i = t.y % r, a = ln(o).group === "dark" ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.15)";
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(Xd, { background: o }),
    n && /* @__PURE__ */ k("svg", { style: _i, children: [
      /* @__PURE__ */ u("defs", { children: /* @__PURE__ */ u(
        "pattern",
        {
          id: "grid-dots",
          x: s,
          y: i,
          width: r,
          height: r,
          patternUnits: "userSpaceOnUse",
          children: /* @__PURE__ */ u("circle", { cx: r / 2, cy: r / 2, r: 1.5, fill: a })
        }
      ) }),
      /* @__PURE__ */ u("rect", { width: "100%", height: "100%", fill: "url(#grid-dots)" })
    ] })
  ] });
}
const ta = "sb-excalib-index", es = "sb-excalib-";
function Yn() {
  try {
    const t = localStorage.getItem(ta);
    return t ? JSON.parse(t) : [];
  } catch {
    return [];
  }
}
function ea(t) {
  localStorage.setItem(ta, JSON.stringify(t));
}
function Gd(t) {
  try {
    const e = localStorage.getItem(es + t);
    return e ? os(JSON.parse(e)) : null;
  } catch {
    return null;
  }
}
function os(t) {
  if (t.libraryItems)
    return t;
  const o = (t.library ?? []).map((n, r) => ({
    id: bt(10),
    name: `Item ${r + 1}`,
    status: "published",
    created: Date.now(),
    elements: n
  }));
  return {
    type: "excalidrawlib",
    version: 2,
    source: t.source,
    libraryItems: o
  };
}
function oa() {
  return Yn();
}
function ns(t) {
  const e = Gd(t);
  return (e == null ? void 0 : e.libraryItems) ?? [];
}
function rs(t, e) {
  const o = os(t), n = bt(10), r = o.libraryItems.map((l) => l.name || "Untitled"), s = {
    id: n,
    name: (e == null ? void 0 : e.name) || "Imported Library",
    source: (e == null ? void 0 : e.source) || "local-import",
    installedAt: Date.now(),
    itemCount: o.libraryItems.length,
    itemNames: r
  };
  localStorage.setItem(es + n, JSON.stringify(o));
  const i = Yn();
  return i.push(s), ea(i), s;
}
function jd(t) {
  localStorage.removeItem(es + t);
  const e = Yn().filter((o) => o.id !== t);
  ea(e);
}
function Vd(t) {
  if (!t.trim()) return [];
  const e = t.toLowerCase(), o = [], n = Yn();
  for (const r of n) {
    if (!r.itemNames.some((l) => l.toLowerCase().includes(e)) && !r.name.toLowerCase().includes(e)) continue;
    const i = ns(r.id);
    for (const l of i)
      ((l.name || "").toLowerCase().includes(e) || r.name.toLowerCase().includes(e)) && o.push({ library: r, item: l });
  }
  return o;
}
async function Kd(t, e) {
  const o = await fetch(t);
  if (!o.ok) throw new Error(`Failed to fetch library: ${o.status}`);
  const n = await o.json();
  if (n.type !== "excalidrawlib")
    throw new Error("Invalid file: not an Excalidraw library");
  const r = os(n);
  return rs(r, { name: e, source: t });
}
const Rr = {
  toolbarBg: "#1e1e2e",
  panelBg: "#1e1e2e",
  panelShadow: "0 4px 24px rgba(0,0,0,0.4)",
  panelBorderRadius: 12,
  controlBg: "#2a2a3e",
  controlBgActive: "#3b82f6",
  controlBorderRadius: 6,
  text: "white",
  textMuted: "#999",
  textSecondary: "#888",
  textFaint: "#666",
  textDisabled: "#555",
  border: "#333",
  separator: "#444",
  swatchBorderActive: "white",
  error: "#e74c3c",
  accentColor: "#3b82f6"
}, na = Ii(Rr);
function qt() {
  return Nn(na);
}
function cn(t) {
  const e = Math.round(t) / 100;
  return e < 1 ? e : void 0;
}
function bo(t) {
  if (t)
    return t * (180 / Math.PI);
}
function ra(t) {
  if (!(!t || t === "transparent"))
    return t;
}
function sa(t) {
  if (t === "solid") return "solid";
  if (t === "cross-hatch") return "cross-hatch";
  if (t === "hachure") return "hachure";
}
function ia(t) {
  if (t === "dashed") return "dashed";
  if (t === "dotted") return "dotted";
}
function aa(t) {
  switch (t) {
    case 2:
      return "sans-serif";
    case 3:
      return "monospace";
    default:
      return "Excalifont";
  }
}
function la(t) {
  return t === "right" ? "right" : t === "center" ? "center" : "left";
}
function qd(t) {
  if (t.roundness || t.strokeSharpness === "round") return "round";
}
function pr(t, e) {
  return {
    id: bt(10),
    type: "shape",
    x: t.x,
    y: t.y,
    w: t.width,
    h: t.height,
    z: 0,
    rotation: bo(t.angle),
    locked: t.locked || void 0,
    data: {
      shape: e,
      stroke: t.strokeColor || "#1e1e2e",
      fill: ra(t.backgroundColor),
      fillStyle: sa(t.fillStyle),
      strokeWidth: t.strokeWidth || 2,
      strokeStyle: ia(t.strokeStyle),
      roughness: Math.min(t.roughness ?? 1, 2),
      opacity: cn(t.opacity ?? 100),
      edgeStyle: e === "rect" || e === "diamond" ? qd(t) : void 0
    }
  };
}
function ti(t, e) {
  const o = t.points ?? [[0, 0]];
  if (o.length < 2) return [];
  const n = {
    stroke: t.strokeColor || "#1e1e2e",
    fill: void 0,
    fillStyle: void 0,
    strokeWidth: t.strokeWidth || 2,
    strokeStyle: ia(t.strokeStyle),
    roughness: Math.min(t.roughness ?? 1, 2),
    opacity: cn(t.opacity ?? 100)
  };
  if (o.length === 2) {
    const [l, a] = o, c = Math.min(l[0], a[0]), h = Math.min(l[1], a[1]), p = Math.max(l[0], a[0]), d = Math.max(l[1], a[1]), f = Math.max(p - c, 1), m = Math.max(d - h, 1);
    return [
      {
        id: bt(10),
        type: "shape",
        x: t.x + c,
        y: t.y + h,
        w: f,
        h: m,
        z: 0,
        rotation: bo(t.angle),
        locked: t.locked || void 0,
        data: {
          ...n,
          shape: e ? "arrow" : "line",
          startPoint: [l[0] - c, l[1] - h],
          endPoint: [a[0] - c, a[1] - h]
        }
      }
    ];
  }
  if (t.backgroundColor && t.backgroundColor !== "transparent") {
    const l = Zd(t);
    if (l) return [l];
  }
  const s = bt(10), i = [];
  for (let l = 0; l < o.length - 1; l++) {
    const a = o[l], c = o[l + 1], h = Math.min(a[0], c[0]), p = Math.min(a[1], c[1]), d = Math.max(a[0], c[0]), f = Math.max(a[1], c[1]), m = Math.max(d - h, 1), y = Math.max(f - p, 1), b = l === o.length - 2;
    i.push({
      id: bt(10),
      type: "shape",
      x: t.x + h,
      y: t.y + p,
      w: m,
      h: y,
      z: 0,
      rotation: bo(t.angle),
      locked: t.locked || void 0,
      groupId: s,
      data: {
        ...n,
        shape: e && b ? "arrow" : "line",
        startPoint: [a[0] - h, a[1] - p],
        endPoint: [c[0] - h, c[1] - p]
      }
    });
  }
  return i;
}
function Zd(t) {
  const e = t.points ?? [];
  if (e.length < 3) return null;
  let o = 1 / 0, n = 1 / 0, r = -1 / 0, s = -1 / 0;
  for (const [l, a] of e)
    l < o && (o = l), a < n && (n = a), l > r && (r = l), a > s && (s = a);
  if (!isFinite(o)) return null;
  const i = e.map(([l, a]) => [
    l - o,
    a - n,
    0.5
  ]);
  return {
    id: bt(10),
    type: "draw",
    x: t.x + o,
    y: t.y + n,
    w: Math.max(r - o, 1),
    h: Math.max(s - n, 1),
    z: 0,
    rotation: bo(t.angle),
    locked: t.locked || void 0,
    data: {
      tool: "vector",
      points: i,
      color: t.strokeColor || "#1e1e2e",
      strokeWidth: t.strokeWidth || 2,
      opacity: cn(t.opacity ?? 100),
      fill: ra(t.backgroundColor),
      fillStyle: sa(t.fillStyle)
    }
  };
}
function Ud(t) {
  const e = t.points;
  if (!e || e.length === 0) return null;
  const o = t.pressures, n = t.simulatePressure !== !1, r = e.map((h, p) => {
    const d = !n && o && p < o.length ? o[p] : 0.5;
    return [h[0], h[1], d];
  });
  let s = 1 / 0, i = 1 / 0, l = -1 / 0, a = -1 / 0;
  for (const [h, p] of r)
    h < s && (s = h), p < i && (i = p), h > l && (l = h), p > a && (a = p);
  isFinite(s) || (s = 0, i = 0, l = 0, a = 0);
  const c = r.map(
    ([h, p, d]) => [h - s, p - i, d]
  );
  return {
    id: bt(10),
    type: "draw",
    x: t.x + s,
    y: t.y + i,
    w: Math.max(l - s, 1),
    h: Math.max(a - i, 1),
    z: 0,
    rotation: bo(t.angle),
    locked: t.locked || void 0,
    data: {
      tool: "pen",
      points: c,
      color: t.strokeColor || "#1e1e2e",
      strokeWidth: t.strokeWidth || 2,
      opacity: cn(t.opacity ?? 100)
    }
  };
}
function Qd(t) {
  return {
    id: bt(10),
    type: "text",
    x: t.x,
    y: t.y,
    w: Math.ceil((t.width || 200) * 1.2),
    h: "auto",
    z: 0,
    rotation: bo(t.angle),
    locked: t.locked || void 0,
    data: {
      text: t.originalText || t.text || "",
      fontSize: t.fontSize || 20,
      fontFamily: aa(t.fontFamily),
      color: t.strokeColor || "#1e1e2e",
      align: la(t.textAlign),
      opacity: cn(t.opacity ?? 100)
    }
  };
}
function Jd(t) {
  return {
    id: bt(10),
    type: "frame",
    x: t.x,
    y: t.y,
    w: t.width || 400,
    h: t.height || 300,
    z: 0,
    rotation: bo(t.angle),
    locked: t.locked || void 0,
    data: {
      label: t.name || void 0
    }
  };
}
function ca(t) {
  return $d(t.elements);
}
function $d(t) {
  const e = [], o = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
  for (const s of t)
    s.isDeleted || s.type === "text" && s.containerId && r.set(s.containerId, s);
  for (const s of t) {
    if (s.isDeleted || s.type === "text" && s.containerId) continue;
    let i = [];
    switch (s.type) {
      case "rectangle":
        i = [pr(s, "rect")];
        break;
      case "ellipse":
        i = [pr(s, "ellipse")];
        break;
      case "diamond":
        i = [pr(s, "diamond")];
        break;
      case "arrow":
        i = ti(s, !0);
        break;
      case "line":
        i = ti(s, !1);
        break;
      case "freedraw": {
        const l = Ud(s);
        l && (i = [l]);
        break;
      }
      case "text":
        i = [Qd(s)];
        break;
      case "frame":
      case "magicframe":
        i = [Jd(s)];
        break;
      case "image":
        continue;
      default:
        continue;
    }
    i.length > 0 && o.set(s.id, i[0].id), e.push(...i);
  }
  for (const [s, i] of r) {
    const l = o.get(s);
    if (!l) continue;
    const a = e.find((h) => h.id === l);
    if (!a || a.type !== "shape") continue;
    const c = a.data;
    c.label = i.originalText || i.text || "", c.labelFontSize = i.fontSize || 20, c.labelFontFamily = aa(i.fontFamily), c.labelAlign = la(i.textAlign);
  }
  return _d(t, e, o, n), th(e), { nodes: e, groupParent: n };
}
function _d(t, e, o, n) {
  var s;
  const r = /* @__PURE__ */ new Map();
  for (const i of t) {
    if (i.isDeleted || !((s = i.groupIds) != null && s.length)) continue;
    for (let a = 0; a < i.groupIds.length - 1; a++) {
      const c = i.groupIds[a], h = i.groupIds[a + 1];
      r.has(c) || r.set(c, h);
    }
    const l = o.get(i.id);
    if (l) {
      const a = e.find((c) => c.id === l);
      a && (a.groupId = i.groupIds[0]);
    }
  }
  for (const [i, l] of r)
    n.set(i, l);
}
function th(t) {
  if (t.length === 0) return;
  let e = 1 / 0, o = 1 / 0;
  for (const n of t)
    n.x < e && (e = n.x), n.y < o && (o = n.y);
  if (isFinite(e))
    for (const n of t)
      n.x -= e, n.y -= o;
}
function ss(t, e = 60) {
  if (t.length === 0)
    return `<svg width="${e}" height="${e}" xmlns="http://www.w3.org/2000/svg"/>`;
  let o = 1 / 0, n = 1 / 0, r = -1 / 0, s = -1 / 0;
  for (const p of t) {
    const d = p.h === "auto" ? 40 : p.h;
    o = Math.min(o, p.x), n = Math.min(n, p.y), r = Math.max(r, p.x + p.w), s = Math.max(s, p.y + d);
  }
  const i = r - o || 1, l = s - n || 1, a = 4, c = `${o - a} ${n - a} ${i + a * 2} ${l + a * 2}`, h = [];
  for (const p of t)
    switch (p.type) {
      case "shape":
        h.push(eh(p));
        break;
      case "draw":
        h.push(oh(p));
        break;
      case "text":
        h.push(nh(p));
        break;
    }
  return `<svg width="${e}" height="${e}" viewBox="${c}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">${h.join("")}</svg>`;
}
function da(t) {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function eh(t) {
  var d, f, m, y;
  const e = t.data, o = t.h === "auto" ? 100 : t.h, n = {
    stroke: e.stroke,
    fill: e.fill,
    fillStyle: e.fillStyle,
    roughness: Math.min(e.roughness, 1),
    // cap roughness for speed
    strokeWidth: e.strokeWidth,
    strokeLineDash: eo(e.strokeStyle),
    seed: t.id
  }, r = ((d = e.startPoint) == null ? void 0 : d[0]) ?? 0, s = ((f = e.startPoint) == null ? void 0 : f[1]) ?? o / 2, i = ((m = e.endPoint) == null ? void 0 : m[0]) ?? t.w, l = ((y = e.endPoint) == null ? void 0 : y[1]) ?? o / 2;
  let a;
  switch (e.shape) {
    case "rect":
      a = Wn(t.x, t.y, t.w, o, n, e.edgeStyle === "round");
      break;
    case "ellipse":
      a = Jr(t.x + t.w / 2, t.y + o / 2, t.w, o, n);
      break;
    case "diamond":
      a = $r(t.x, t.y, t.w, o, n, e.edgeStyle === "round");
      break;
    case "line":
      a = Oo(t.x + r, t.y + s, t.x + i, t.y + l, n);
      break;
    case "arrow":
      a = _r(t.x + r, t.y + s, t.x + i, t.y + l, n);
      break;
    default:
      return "";
  }
  const c = e.opacity ?? 1, h = c < 1 ? `<g opacity="${c}">` : "<g>", p = a.map(
    (b) => `<path d="${da(b.d)}" fill="${b.fill || "none"}" stroke="${b.stroke}" stroke-width="${b.strokeWidth}"${b.strokeDasharray ? ` stroke-dasharray="${b.strokeDasharray}"` : ""} stroke-linecap="round" stroke-linejoin="round"/>`
  );
  return `${h}${p.join("")}</g>`;
}
function oh(t) {
  const e = t.data;
  if (!e.points.length) return "";
  const o = e.points.map(([s, i]) => `${(t.x + s).toFixed(1)},${(t.y + i).toFixed(1)}`).join(" "), n = e.opacity ?? 1, r = e.tool === "vector" && e.fill ? e.fill : "none";
  return `<polygon points="${o}" fill="${r}" stroke="${e.color}" stroke-width="${e.strokeWidth}" stroke-linecap="round" stroke-linejoin="round"${n < 1 ? ` opacity="${n}"` : ""}/>`;
}
function nh(t) {
  const e = t.data, o = Math.max(e.fontSize, 8), n = e.opacity ?? 1, r = e.text.split(`
`)[0] || "";
  return `<text x="${t.x}" y="${t.y + o}" fill="${e.color}" font-size="${o}" font-family="sans-serif"${n < 1 ? ` opacity="${n}"` : ""}>${da(r)}</text>`;
}
const ha = "sb-personal-library";
function is() {
  try {
    const t = localStorage.getItem(ha);
    return t ? JSON.parse(t) : [];
  } catch {
    return [];
  }
}
function ua(t) {
  localStorage.setItem(ha, JSON.stringify(t));
}
function pa() {
  return is();
}
function rh(t, e, o) {
  const n = structuredClone(e);
  if (n.length > 0) {
    let a = 1 / 0, c = 1 / 0;
    for (const h of n)
      h.x < a && (a = h.x), h.y < c && (c = h.y);
    if (isFinite(a))
      for (const h of n)
        h.x -= a, h.y -= c;
  }
  const r = new Set(
    n.map((a) => a.groupId).filter(Boolean)
  ), s = [];
  for (const [a, c] of o)
    r.has(a) && s.push([a, c]);
  const i = {
    id: bt(10),
    name: t.trim() || "Untitled",
    nodes: n,
    groupParent: s,
    createdAt: Date.now()
  }, l = is();
  return l.unshift(i), ua(l), i;
}
function sh(t) {
  const e = is().filter((o) => o.id !== t);
  ua(e);
}
function fa(t, e, o, n) {
  const { nodes: r, groupParent: s } = ca(e);
  if (r.length === 0) return;
  const i = structuredClone(r), l = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map();
  for (const x of i) {
    const S = bt(10);
    l.set(x.id, S), x.id = S;
  }
  for (const x of i)
    x.groupId && (a.has(x.groupId) || a.set(x.groupId, bt(10)), x.groupId = a.get(x.groupId));
  let c = 1 / 0, h = 1 / 0, p = -1 / 0, d = -1 / 0;
  for (const x of i) {
    const S = x.h === "auto" ? 100 : x.h;
    c = Math.min(c, x.x), h = Math.min(h, x.y), p = Math.max(p, x.x + x.w), d = Math.max(d, x.y + S);
  }
  const f = o ?? window.innerWidth / 2, m = n ?? window.innerHeight / 2, y = t.screenToCanvas(f, m), b = y.x - (c + p) / 2, g = y.y - (h + d) / 2;
  for (const x of i)
    x.x += b, x.y += g, x.z = t.nextZ();
  t.addNodes(i);
  for (const [x, S] of s) {
    const M = a.get(x) ?? x, z = a.get(S) ?? S;
    t.groupParent.set(M, z);
  }
  t.selectMultiple(i.map((x) => x.id));
}
const Ar = "application/x-spatialboard-library-item", Lr = "application/x-spatialboard-personal-item";
function ya(t, e, o, n) {
  if (e.nodes.length === 0) return;
  const r = structuredClone(e.nodes), s = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Map();
  for (const b of r) {
    const g = bt(10);
    s.set(b.id, g), b.id = g;
  }
  for (const b of r)
    b.groupId && (i.has(b.groupId) || i.set(b.groupId, bt(10)), b.groupId = i.get(b.groupId));
  for (const b of r)
    if (b.type === "edge") {
      const g = b.data;
      g.fromId && s.has(g.fromId) && (g.fromId = s.get(g.fromId)), g.toId && s.has(g.toId) && (g.toId = s.get(g.toId));
    }
  let l = 1 / 0, a = 1 / 0, c = -1 / 0, h = -1 / 0;
  for (const b of r) {
    const g = b.h === "auto" ? 100 : b.h;
    l = Math.min(l, b.x), a = Math.min(a, b.y), c = Math.max(c, b.x + b.w), h = Math.max(h, b.y + g);
  }
  const p = o ?? window.innerWidth / 2, d = n ?? window.innerHeight / 2, f = t.screenToCanvas(p, d), m = f.x - (l + c) / 2, y = f.y - (a + h) / 2;
  for (const b of r)
    b.x += m, b.y += y, b.z = t.nextZ();
  t.addNodes(r);
  for (const [b, g] of e.groupParent) {
    const x = i.get(b) ?? b, S = i.get(g) ?? g;
    t.groupParent.set(x, S);
  }
  t.selectMultiple(r.map((b) => b.id));
}
const Yo = /* @__PURE__ */ new Map();
function ih({ item: t }) {
  const e = Kt(() => {
    const o = Yo.get(t.id);
    if (o) return o;
    const { nodes: n } = ca(t), r = ss(n, 56);
    return Yo.set(t.id, r), r;
  }, [t.id]);
  return /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: e } });
}
function ma({
  item: t,
  libId: e,
  onClick: o,
  theme: n
}) {
  const r = st(
    (s) => {
      s.dataTransfer.setData(
        Ar,
        JSON.stringify({ libraryId: e, itemId: t.id })
      ), s.dataTransfer.effectAllowed = "copy";
    },
    [e, t.id]
  );
  return /* @__PURE__ */ u(
    "button",
    {
      title: t.name || "Untitled",
      onClick: o,
      draggable: !0,
      onDragStart: r,
      style: {
        border: `1px solid ${n.border}`,
        borderRadius: 4,
        background: n.controlBg,
        cursor: "grab",
        padding: 2,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        aspectRatio: "1"
      },
      children: /* @__PURE__ */ u(ih, { item: t })
    }
  );
}
function ah({ nodes: t }) {
  const e = Kt(() => {
    const o = "personal-" + t.map((s) => s.id).join(","), n = Yo.get(o);
    if (n) return n;
    const r = ss(t, 56);
    return Yo.set(o, r), r;
  }, [t]);
  return /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: e } });
}
function ba({
  item: t,
  onClick: e,
  onRemove: o,
  theme: n
}) {
  const [r, s] = et(!1), i = st(
    (l) => {
      l.dataTransfer.setData(
        Lr,
        JSON.stringify({ itemId: t.id })
      ), l.dataTransfer.effectAllowed = "copy";
    },
    [t.id]
  );
  return /* @__PURE__ */ k(
    "div",
    {
      style: { position: "relative", aspectRatio: "1" },
      onMouseEnter: () => s(!0),
      onMouseLeave: () => s(!1),
      children: [
        /* @__PURE__ */ u(
          "button",
          {
            title: t.name,
            onClick: e,
            draggable: !0,
            onDragStart: i,
            style: {
              border: `1px solid ${n.border}`,
              borderRadius: 4,
              background: n.controlBg,
              cursor: "grab",
              padding: 2,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              width: "100%",
              height: "100%"
            },
            children: /* @__PURE__ */ u(ah, { nodes: t.nodes })
          }
        ),
        r && /* @__PURE__ */ u(
          "button",
          {
            title: "Remove from Personal Library",
            onClick: (l) => {
              l.stopPropagation(), o();
            },
            style: {
              position: "absolute",
              top: -4,
              right: -4,
              width: 16,
              height: 16,
              borderRadius: "50%",
              border: "none",
              background: "#ef4444",
              color: "#fff",
              fontSize: 10,
              lineHeight: "16px",
              textAlign: "center",
              cursor: "pointer",
              padding: 0,
              display: "flex",
              alignItems: "center",
              justifyContent: "center"
            },
            children: "×"
          }
        )
      ]
    }
  );
}
function lh({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: n,
  onBrowseDirectory: r
}) {
  const s = qt(), i = at(null), l = at(null), [a, c] = et([]), [h, p] = et([]), [d, f] = et(""), [m, y] = et(/* @__PURE__ */ new Set()), b = st(() => {
    c(oa()), p(pa());
  }, []);
  yt(() => {
    e && b();
  }, [e, b]), yt(() => {
    if (!e) return;
    const R = (K) => {
      i.current && !i.current.contains(K.target) && o();
    };
    return document.addEventListener("pointerdown", R), () => document.removeEventListener("pointerdown", R);
  }, [e, o]);
  const g = st(
    (R) => {
      var lt;
      const K = (lt = R.target.files) == null ? void 0 : lt[0];
      if (!K) return;
      const Z = new FileReader();
      Z.onload = () => {
        try {
          const O = JSON.parse(Z.result);
          if (O.type !== "excalidrawlib") {
            console.warn("Not an Excalidraw library file");
            return;
          }
          const ot = K.name.replace(/\.excalidrawlib$/, "");
          rs(O, { name: ot }), b();
        } catch (O) {
          console.error("Failed to parse library file:", O);
        }
      }, Z.readAsText(K), R.target.value = "";
    },
    [b]
  ), x = st(
    (R) => {
      jd(R), Yo.clear(), b();
    },
    [b]
  ), S = st(
    (R) => {
      fa(t, R);
    },
    [t]
  ), M = st(
    (R) => {
      ya(t, R);
    },
    [t]
  ), z = st(
    (R) => {
      sh(R), Yo.clear(), b();
    },
    [b]
  ), W = st((R) => {
    y((K) => {
      const Z = new Set(K);
      return Z.has(R) ? Z.delete(R) : Z.add(R), Z;
    });
  }, []), I = Kt(() => {
    if (!d.trim()) return null;
    const R = d.toLowerCase(), K = Vd(d), Z = h.filter(
      (lt) => lt.name.toLowerCase().includes(R)
    );
    return { excalidraw: K, personal: Z };
  }, [d, h]);
  return !e || !n ? null : Oe(
    /* @__PURE__ */ k(
      "div",
      {
        ref: i,
        style: {
          position: "fixed",
          left: n.right + 8,
          top: n.top,
          background: s.panelBg,
          border: `1px solid ${s.border}`,
          borderRadius: s.panelBorderRadius,
          padding: 0,
          zIndex: 99999,
          boxShadow: s.panelShadow,
          width: 280,
          maxHeight: `calc(100vh - ${n.top + 20}px)`,
          display: "flex",
          flexDirection: "column",
          overflow: "hidden"
        },
        onPointerDown: (R) => R.stopPropagation(),
        children: [
          /* @__PURE__ */ k("div", { style: { padding: "10px 12px 6px", flexShrink: 0 }, children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 11,
                  fontWeight: 600,
                  color: s.text,
                  marginBottom: 8
                },
                children: "Libraries"
              }
            ),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                placeholder: "Search library...",
                value: d,
                onChange: (R) => f(R.target.value),
                style: {
                  width: "100%",
                  padding: "5px 8px",
                  border: `1px solid ${s.border}`,
                  borderRadius: s.controlBorderRadius,
                  background: s.controlBg,
                  color: s.text,
                  fontSize: 11,
                  outline: "none",
                  boxSizing: "border-box"
                }
              }
            )
          ] }),
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                flex: 1,
                overflowY: "auto",
                overflowX: "hidden",
                padding: "4px 12px"
              },
              children: I !== null ? I.excalidraw.length === 0 && I.personal.length === 0 ? /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    color: s.textDisabled,
                    fontSize: 11,
                    textAlign: "center",
                    padding: 20
                  },
                  children: "No matching items"
                }
              ) : /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    display: "grid",
                    gridTemplateColumns: "repeat(4, 1fr)",
                    gap: 4
                  },
                  children: [
                    I.personal.map((R) => /* @__PURE__ */ u(
                      ba,
                      {
                        item: R,
                        onClick: () => M(R),
                        onRemove: () => z(R.id),
                        theme: s
                      },
                      R.id
                    )),
                    I.excalidraw.map(({ library: R, item: K }) => /* @__PURE__ */ u(
                      ma,
                      {
                        item: K,
                        libId: R.id,
                        onClick: () => S(K),
                        theme: s
                      },
                      K.id
                    ))
                  ]
                }
              ) : /* @__PURE__ */ k(ft, { children: [
                h.length > 0 && /* @__PURE__ */ u(
                  dh,
                  {
                    items: h,
                    onPlace: M,
                    onRemove: z,
                    theme: s
                  }
                ),
                a.length === 0 && h.length === 0 ? /* @__PURE__ */ k(
                  "div",
                  {
                    style: {
                      color: s.textDisabled,
                      fontSize: 11,
                      textAlign: "center",
                      padding: "20px 10px"
                    },
                    children: [
                      "No libraries installed.",
                      /* @__PURE__ */ u("br", {}),
                      "Import an .excalidrawlib file",
                      /* @__PURE__ */ u("br", {}),
                      "or browse the community directory."
                    ]
                  }
                ) : a.map((R) => {
                  const K = m.has(R.id);
                  return /* @__PURE__ */ u(
                    ch,
                    {
                      lib: R,
                      expanded: K,
                      onToggle: () => W(R.id),
                      onPlace: S,
                      onUninstall: () => x(R.id),
                      theme: s
                    },
                    R.id
                  );
                })
              ] })
            }
          ),
          /* @__PURE__ */ k(
            "div",
            {
              style: {
                borderTop: `1px solid ${s.border}`,
                padding: "8px 12px",
                display: "flex",
                gap: 6,
                flexShrink: 0
              },
              children: [
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: () => {
                      var R;
                      return (R = l.current) == null ? void 0 : R.click();
                    },
                    style: {
                      flex: 1,
                      padding: "5px 8px",
                      border: `1px solid ${s.border}`,
                      borderRadius: s.controlBorderRadius,
                      background: s.controlBg,
                      color: s.text,
                      cursor: "pointer",
                      fontSize: 10,
                      fontWeight: 500
                    },
                    children: "Import file"
                  }
                ),
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: r,
                    style: {
                      flex: 1,
                      padding: "5px 8px",
                      border: "none",
                      borderRadius: s.controlBorderRadius,
                      background: s.accentColor,
                      color: "#fff",
                      cursor: "pointer",
                      fontSize: 10,
                      fontWeight: 500
                    },
                    children: "Browse libraries"
                  }
                )
              ]
            }
          ),
          /* @__PURE__ */ u(
            "input",
            {
              ref: l,
              type: "file",
              accept: ".excalidrawlib,.json",
              style: { display: "none" },
              onChange: g
            }
          )
        ]
      }
    ),
    document.body
  );
}
function ch({
  lib: t,
  expanded: e,
  onToggle: o,
  onPlace: n,
  onUninstall: r,
  theme: s
}) {
  const [i, l] = et(null);
  return yt(() => {
    e && i === null && l(ns(t.id));
  }, [e, i, t.id]), /* @__PURE__ */ k("div", { style: { marginBottom: 4 }, children: [
    /* @__PURE__ */ k(
      "div",
      {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
          padding: "4px 0",
          cursor: "pointer",
          userSelect: "none"
        },
        onClick: o,
        children: [
          /* @__PURE__ */ u(
            "svg",
            {
              width: 12,
              height: 12,
              viewBox: "0 0 12 12",
              fill: "none",
              style: {
                transform: e ? "rotate(90deg)" : "rotate(0deg)",
                transition: "transform 0.15s"
              },
              children: /* @__PURE__ */ u(
                "path",
                {
                  d: "M4 2l4 4-4 4",
                  stroke: s.textMuted,
                  strokeWidth: 1.5,
                  strokeLinecap: "round"
                }
              )
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flex: 1,
                fontSize: 10,
                fontWeight: 600,
                color: s.textMuted,
                textTransform: "uppercase",
                letterSpacing: "0.03em",
                overflow: "hidden",
                textOverflow: "ellipsis",
                whiteSpace: "nowrap"
              },
              children: t.name
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                fontSize: 9,
                color: s.textDisabled
              },
              children: t.itemCount
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Uninstall library",
              onClick: (a) => {
                a.stopPropagation(), r();
              },
              style: {
                border: "none",
                background: "transparent",
                color: s.textDisabled,
                cursor: "pointer",
                padding: "2px 4px",
                fontSize: 12,
                lineHeight: 1
              },
              children: "×"
            }
          )
        ]
      }
    ),
    e && i && /* @__PURE__ */ u(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: "repeat(4, 1fr)",
          gap: 4,
          padding: "2px 0 6px"
        },
        children: i.map((a) => /* @__PURE__ */ u(
          ma,
          {
            item: a,
            libId: t.id,
            onClick: () => n(a),
            theme: s
          },
          a.id
        ))
      }
    )
  ] });
}
function dh({
  items: t,
  onPlace: e,
  onRemove: o,
  theme: n
}) {
  const [r, s] = et(!0);
  return /* @__PURE__ */ k("div", { style: { marginBottom: 4 }, children: [
    /* @__PURE__ */ k(
      "div",
      {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
          padding: "4px 0",
          cursor: "pointer",
          userSelect: "none"
        },
        onClick: () => s((i) => !i),
        children: [
          /* @__PURE__ */ u(
            "svg",
            {
              width: 12,
              height: 12,
              viewBox: "0 0 12 12",
              fill: "none",
              style: {
                transform: r ? "rotate(90deg)" : "rotate(0deg)",
                transition: "transform 0.15s"
              },
              children: /* @__PURE__ */ u(
                "path",
                {
                  d: "M4 2l4 4-4 4",
                  stroke: n.textMuted,
                  strokeWidth: 1.5,
                  strokeLinecap: "round"
                }
              )
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flex: 1,
                fontSize: 10,
                fontWeight: 600,
                color: n.textMuted,
                textTransform: "uppercase",
                letterSpacing: "0.03em"
              },
              children: "Personal"
            }
          ),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                fontSize: 9,
                color: n.textDisabled
              },
              children: t.length
            }
          )
        ]
      }
    ),
    r && /* @__PURE__ */ u(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: "repeat(4, 1fr)",
          gap: 4,
          padding: "2px 0 6px"
        },
        children: t.map((i) => /* @__PURE__ */ u(
          ba,
          {
            item: i,
            onClick: () => e(i),
            onRemove: () => o(i.id),
            theme: n
          },
          i.id
        ))
      }
    )
  ] });
}
async function hh(t, e, o = 1, n = 20, r) {
  const s = `${t}/search?q=${encodeURIComponent(e)}&page=${o}&per_page=${n}`;
  return (await fetch(s, { signal: r, credentials: "include" })).json();
}
async function ei(t, e = 1, o = 20, n) {
  const r = `${t}/trending?page=${e}&per_page=${o}`;
  return (await fetch(r, { signal: n, credentials: "include" })).json();
}
const Er = "application/x-spatialboard-gif-item";
function ga(t, e, o, n) {
  const r = e.file.hd.gif, s = 400, i = 300;
  let l = r.width, a = r.height;
  const c = Math.min(1, s / l, i / a);
  l = Math.round(l * c), a = Math.round(a * c);
  const h = o ?? window.innerWidth / 2, p = n ?? window.innerHeight / 2, d = t.screenToCanvas(h, p), f = {
    id: bt(10),
    type: "image",
    x: d.x - l / 2,
    y: d.y - a / 2,
    w: l,
    h: a,
    z: t.nextZ(),
    data: { src: r.url }
  };
  t.addNode(f), t.select(f.id);
}
function uh({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: n,
  baseUrl: r
}) {
  const s = qt(), i = at(null), l = at(null), [a, c] = et(""), [h, p] = et([]), [d, f] = et(!1), [m, y] = et(1), [b, g] = et(!1), x = at();
  yt(() => {
    if (!e) return;
    const I = (R) => {
      i.current && !i.current.contains(R.target) && o();
    };
    return document.addEventListener("pointerdown", I), () => document.removeEventListener("pointerdown", I);
  }, [e, o]), yt(() => {
    if (!e || a.trim()) return;
    const I = new AbortController();
    return f(!0), ei(r, 1, 30, I.signal).then((R) => {
      p(R.data.data.filter((K) => K.type !== "ad")), y(1), g(R.data.has_next);
    }).catch(() => {
    }).finally(() => f(!1)), () => I.abort();
  }, [e, r, a]);
  const S = st(
    (I, R, K) => {
      if (!I.trim()) return;
      const Z = new AbortController();
      return f(!0), hh(r, I, R, 30, Z.signal).then((lt) => {
        const O = lt.data.data.filter((ot) => ot.type !== "ad");
        p((ot) => K ? [...ot, ...O] : O), y(R), g(lt.data.has_next);
      }).catch(() => {
      }).finally(() => f(!1)), Z;
    },
    [r]
  ), M = st(
    (I) => {
      if (c(I), x.current && clearTimeout(x.current), !I.trim()) {
        p([]), y(1), g(!1);
        return;
      }
      x.current = setTimeout(() => {
        S(I, 1, !1);
      }, 350);
    },
    [S]
  ), z = st(() => {
    const I = l.current;
    !I || d || !b || I.scrollTop + I.clientHeight >= I.scrollHeight - 100 && (a.trim() ? S(a, m + 1, !0) : (f(!0), ei(r, m + 1, 30).then((R) => {
      const K = R.data.data.filter((Z) => Z.type !== "ad");
      p((Z) => [...Z, ...K]), y(m + 1), g(R.data.has_next);
    }).catch(() => {
    }).finally(() => f(!1))));
  }, [d, b, a, m, S, r]), W = st(
    (I) => {
      ga(t, I);
    },
    [t]
  );
  return !e || !n ? null : Oe(
    /* @__PURE__ */ k(
      "div",
      {
        ref: i,
        style: {
          position: "fixed",
          left: n.right + 8,
          top: n.top,
          background: s.panelBg,
          border: `1px solid ${s.border}`,
          borderRadius: s.panelBorderRadius,
          padding: 0,
          zIndex: 99999,
          boxShadow: s.panelShadow,
          width: 300,
          maxHeight: `calc(100vh - ${n.top + 20}px)`,
          display: "flex",
          flexDirection: "column",
          overflow: "hidden"
        },
        onPointerDown: (I) => I.stopPropagation(),
        children: [
          /* @__PURE__ */ k("div", { style: { padding: "10px 12px 6px", flexShrink: 0 }, children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 11,
                  fontWeight: 600,
                  color: s.text,
                  marginBottom: 8
                },
                children: "GIFs"
              }
            ),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                placeholder: "Search KLIPY",
                value: a,
                onChange: (I) => M(I.target.value),
                style: {
                  width: "100%",
                  padding: "5px 8px",
                  border: `1px solid ${s.border}`,
                  borderRadius: s.controlBorderRadius,
                  background: s.controlBg,
                  color: s.text,
                  fontSize: 11,
                  outline: "none",
                  boxSizing: "border-box"
                }
              }
            )
          ] }),
          /* @__PURE__ */ k(
            "div",
            {
              ref: l,
              onScroll: z,
              style: {
                flex: 1,
                overflowY: "auto",
                overflowX: "hidden",
                padding: "4px 12px",
                minHeight: 200
              },
              children: [
                h.length === 0 && !d ? /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      color: s.textDisabled,
                      fontSize: 11,
                      textAlign: "center",
                      padding: 20
                    },
                    children: a.trim() ? "No results" : "Loading..."
                  }
                ) : /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      display: "grid",
                      gridTemplateColumns: "repeat(3, 1fr)",
                      gap: 4
                    },
                    children: h.map((I) => /* @__PURE__ */ u(
                      ph,
                      {
                        item: I,
                        onClick: () => W(I),
                        engine: t,
                        theme: s
                      },
                      I.id
                    ))
                  }
                ),
                d && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      color: s.textMuted,
                      fontSize: 10,
                      textAlign: "center",
                      padding: 12
                    },
                    children: "Loading..."
                  }
                )
              ]
            }
          ),
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                borderTop: `1px solid ${s.border}`,
                padding: "6px 12px",
                fontSize: 9,
                color: s.textMuted,
                textAlign: "center",
                flexShrink: 0
              },
              children: "Powered by KLIPY"
            }
          )
        ]
      }
    ),
    document.body
  );
}
function ph({
  item: t,
  onClick: e,
  engine: o,
  theme: n
}) {
  const r = t.file.sm.webp, s = r.width / r.height, i = st(
    (l) => {
      l.dataTransfer.setData(Er, JSON.stringify(t)), l.dataTransfer.effectAllowed = "copy";
    },
    [t]
  );
  return /* @__PURE__ */ u(
    "button",
    {
      title: t.title,
      onClick: e,
      draggable: !0,
      onDragStart: i,
      style: {
        border: `1px solid ${n.border}`,
        borderRadius: n.controlBorderRadius,
        background: n.controlBg,
        cursor: "grab",
        padding: 0,
        overflow: "hidden",
        aspectRatio: s > 1.5 ? "16/9" : s < 0.7 ? "3/4" : "1"
      },
      children: /* @__PURE__ */ u(
        "img",
        {
          src: r.url,
          alt: t.title,
          loading: "lazy",
          style: {
            width: "100%",
            height: "100%",
            objectFit: "cover",
            display: "block"
          }
        }
      )
    }
  );
}
function fh({
  nodes: t,
  onSave: e,
  onCancel: o
}) {
  const [n, r] = et(""), s = at(null), i = at(null);
  yt(() => {
    var p;
    (p = s.current) == null || p.focus();
  }, []);
  const l = Kt(() => ss(t, 56), [t]), a = st(() => {
    e(n.trim() || "Untitled");
  }, [n, e]), c = st(
    (p) => {
      p.key === "Enter" ? (p.preventDefault(), a()) : p.key === "Escape" && (p.preventDefault(), o());
    },
    [a, o]
  ), h = st(
    (p) => {
      i.current && !i.current.contains(p.target) && o();
    },
    [o]
  );
  return Oe(
    /* @__PURE__ */ u(
      "div",
      {
        onClick: h,
        style: {
          position: "fixed",
          inset: 0,
          zIndex: 99999,
          background: "rgba(0,0,0,0.4)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center"
        },
        children: /* @__PURE__ */ k(
          "div",
          {
            ref: i,
            onPointerDown: (p) => p.stopPropagation(),
            style: {
              background: "#1e1e2e",
              borderRadius: 8,
              border: "1px solid #333",
              boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
              padding: 16,
              width: 280,
              color: "#e0e0e0",
              fontSize: 13,
              fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
            },
            children: [
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    fontSize: 13,
                    fontWeight: 600,
                    marginBottom: 12
                  },
                  children: "Add to Personal Library"
                }
              ),
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    display: "flex",
                    justifyContent: "center",
                    marginBottom: 12,
                    padding: 8,
                    background: "rgba(255,255,255,0.05)",
                    borderRadius: 6,
                    border: "1px solid #333"
                  },
                  children: /* @__PURE__ */ u("div", { dangerouslySetInnerHTML: { __html: l } })
                }
              ),
              /* @__PURE__ */ u(
                "input",
                {
                  ref: s,
                  type: "text",
                  value: n,
                  onChange: (p) => r(p.target.value),
                  onKeyDown: c,
                  placeholder: "Item name",
                  style: {
                    width: "100%",
                    padding: "6px 8px",
                    border: "1px solid #333",
                    borderRadius: 4,
                    background: "rgba(255,255,255,0.06)",
                    color: "#e0e0e0",
                    fontSize: 13,
                    outline: "none",
                    boxSizing: "border-box",
                    marginBottom: 12
                  }
                }
              ),
              /* @__PURE__ */ k("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end" }, children: [
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: o,
                    style: {
                      padding: "5px 12px",
                      border: "1px solid #333",
                      borderRadius: 4,
                      background: "transparent",
                      color: "#e0e0e0",
                      cursor: "pointer",
                      fontSize: 12
                    },
                    children: "Cancel"
                  }
                ),
                /* @__PURE__ */ u(
                  "button",
                  {
                    onClick: a,
                    style: {
                      padding: "5px 12px",
                      border: "none",
                      borderRadius: 4,
                      background: "#6366f1",
                      color: "#fff",
                      cursor: "pointer",
                      fontSize: 12,
                      fontWeight: 500
                    },
                    children: "Save"
                  }
                )
              ] })
            ]
          }
        )
      }
    ),
    document.body
  );
}
function Dr(t) {
  const e = t.trim();
  if (!e.includes("<svg")) return null;
  const o = e.match(/<svg[\s\S]*?<\/svg>/i);
  return o ? o[0] : null;
}
function yh(t) {
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(t)}`;
}
function xa(t, e, o, n) {
  return new Promise((r) => {
    const s = yh(t), i = new Image();
    i.onload = () => {
      let c = i.naturalWidth || 200, h = i.naturalHeight || 200;
      if (c <= 1 || h <= 1) {
        const p = t.match(/viewBox=["']([^"']+)["']/i);
        if (p) {
          const d = p[1].trim().split(/[\s,]+/).map(Number);
          d.length === 4 && d[2] > 0 && d[3] > 0 && (c = d[2], h = d[3]);
        }
      }
      if (c > 400 || h > 400) {
        const p = Math.min(400 / c, 400 / h);
        c = Math.round(c * p), h = Math.round(h * p);
      }
      r({
        id: bt(10),
        type: "image",
        x: e,
        y: o,
        w: c,
        h,
        z: n,
        data: { src: s }
      });
    }, i.onerror = () => r(null), i.src = s;
  });
}
async function mh(t, e, o, n) {
  const { x: r, y: s } = t.screenToCanvas(o, n), i = await xa(e, r, s, t.nextZ());
  i && (t.addNode(i), t.select(i.id));
}
const oi = {
  number: "#3b82f6",
  // blue
  string: "#10b981",
  // green
  boolean: "#f59e0b",
  // amber
  object: "#8b5cf6",
  // purple
  any: "#6b7280",
  // gray
  signal: "#ef4444"
  // red
}, bh = ue(function({
  node: e,
  zoom: o,
  showHandles: n = !0,
  measuredHeights: r,
  onHandlePointerDown: s,
  onRotateStart: i
}) {
  const l = e.h === "auto" ? (r == null ? void 0 : r[e.id]) ?? 100 : e.h, a = e.rotation || 0, c = e.x + e.w / 2, h = e.y + l / 2, p = 8 / o, d = p / 2, f = 25 / o, m = !!e.locked, y = [
    { pos: "nw", cx: e.x, cy: e.y },
    { pos: "n", cx: e.x + e.w / 2, cy: e.y },
    { pos: "ne", cx: e.x + e.w, cy: e.y },
    { pos: "e", cx: e.x + e.w, cy: e.y + l / 2 },
    { pos: "se", cx: e.x + e.w, cy: e.y + l },
    { pos: "s", cx: e.x + e.w / 2, cy: e.y + l },
    { pos: "sw", cx: e.x, cy: e.y + l },
    { pos: "w", cx: e.x, cy: e.y + l / 2 }
  ];
  return /* @__PURE__ */ k("g", { transform: `rotate(${a}, ${c}, ${h})`, children: [
    /* @__PURE__ */ u(
      "rect",
      {
        x: e.x,
        y: e.y,
        width: e.w,
        height: l,
        fill: "none",
        stroke: m ? "#f59e0b" : "#3b82f6",
        strokeWidth: 1.5 / o,
        strokeDasharray: `${4 / o} ${3 / o}`
      }
    ),
    m && (() => {
      const b = 16 / o, g = e.x + e.w - b - 4 / o, x = e.y - b - 4 / o;
      return /* @__PURE__ */ k("g", { transform: `translate(${g}, ${x})`, children: [
        /* @__PURE__ */ u(
          "rect",
          {
            x: 0,
            y: 0,
            width: b,
            height: b,
            rx: 3 / o,
            fill: "#f59e0b"
          }
        ),
        /* @__PURE__ */ k("g", { transform: `scale(${b / 24})`, children: [
          /* @__PURE__ */ u("rect", { x: "6", y: "11", width: "12", height: "9", rx: "1", fill: "white" }),
          /* @__PURE__ */ u("path", { d: "M8 11V7a4 4 0 0 1 8 0v4", fill: "none", stroke: "white", strokeWidth: "2", strokeLinecap: "round" })
        ] })
      ] });
    })(),
    n && !m && y.map(({ pos: b, cx: g, cy: x }) => /* @__PURE__ */ u(
      "rect",
      {
        x: g - d,
        y: x - d,
        width: p,
        height: p,
        fill: "white",
        stroke: "#3b82f6",
        strokeWidth: 1.5 / o,
        style: {
          cursor: On(b, a),
          pointerEvents: "auto"
        },
        onPointerDown: (S) => {
          S.stopPropagation(), s == null || s(e.id, b, S);
        }
      },
      b
    )),
    n && !m && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u(
        "line",
        {
          x1: e.x + e.w / 2,
          y1: e.y,
          x2: e.x + e.w / 2,
          y2: e.y - f,
          stroke: "#3b82f6",
          strokeWidth: 1.5 / o
        }
      ),
      /* @__PURE__ */ u(
        "rect",
        {
          x: e.x + e.w / 2 - d,
          y: e.y - f - d,
          width: p,
          height: p,
          rx: 1.5 / o,
          transform: `rotate(45, ${e.x + e.w / 2}, ${e.y - f})`,
          fill: "white",
          stroke: "#3b82f6",
          strokeWidth: 1.5 / o,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (b) => {
            b.stopPropagation(), i == null || i(e.id, b);
          }
        }
      )
    ] })
  ] });
}), gh = ue(function({
  edge: e,
  fromNode: o,
  toNode: n,
  viewport: r,
  selection: s,
  measuredHeights: i,
  registry: l,
  onEdgeEndpointDown: a,
  onKinkHandleDown: c,
  edgeReconnect: h,
  eraserMarkedIds: p,
  cycleNodeIds: d
}) {
  const f = e.data.edgeType || "bezier";
  let m, y;
  if (l && e.data.sourcePort) {
    const ht = l.get(o.type);
    ht != null && ht.ports && (m = rn(o, ht.ports, e.data.sourcePort, r.zoom, i) ?? void 0);
  }
  if (l && e.data.targetPort) {
    const ht = l.get(n.type);
    ht != null && ht.ports && (y = rn(n, ht.ports, e.data.targetPort, r.zoom, i) ?? void 0);
  }
  const b = Ze(
    o,
    n,
    f,
    i,
    e.data.sourceHandle,
    e.data.targetHandle,
    e.data.midpointOffset,
    e.data.curveOffset,
    m,
    y
  ), { path: g, x1: x, y1: S, x2: M, y2: z, labelX: W, labelY: I, arrowAngle: R, tailAngle: K, kinkHandle: Z } = b, lt = s.has(e.id), O = e.data.strokeWidth, ot = e.data.style === "dashed" ? `${8 * O},${4 * O}` : e.data.style === "dotted" ? `${2 * O},${3 * O}` : void 0, Q = Math.max(8, O * 3), $ = e.data.arrowHeadSize ?? Q, P = e.data.arrowTailSize ?? Q, G = e.data.animated, X = p == null ? void 0 : p.has(e.id), N = (h == null ? void 0 : h.edgeId) === e.id, j = !!(d && d.size > 0 && e.data.sourcePort && e.data.targetPort && d.has(e.data.fromId) && d.has(e.data.toId)), H = j ? "#ef4444" : e.data.color, _ = e.data.roughness ?? 0, tt = Kt(() => _ <= 0 ? null : {
    stroke: H,
    roughness: _,
    strokeWidth: O,
    strokeLineDash: e.data.style === "dashed" ? [8, 4] : e.data.style === "dotted" ? [2, 2] : void 0,
    seed: e.id
  }, [H, _, O, e.data.style, e.id]);
  let nt = null, ut = null, pt = null;
  tt && (nt = hr(g, tt), e.data.arrowHead === "arrow" && (ut = hr(Bo(M, z, R, $), { ...tt, strokeLineDash: void 0 })), e.data.arrowTail === "arrow" && (pt = hr(Bo(x, S, K, P), { ...tt, strokeLineDash: void 0 })));
  const vt = Kt(
    () => ({ animation: "edge-cycle-pulse 1.5s ease-in-out infinite" }),
    []
  ), rt = Kt(() => {
    if (!G) return;
    const ht = e.data.animatedDirection === "reverse" ? "edge-flow-reverse" : e.data.animatedDirection === "both" ? "edge-flow-both" : e.data.animatedDirection === "bop" ? "edge-flow-bop" : "edge-flow", Dt = e.data.animatedDirection === "both" ? "2s" : e.data.animatedDirection === "bop" ? "3.4s" : "1s", Tt = e.data.animatedDirection === "bop" ? "ease-in-out" : "linear";
    return { animation: `${ht} ${Dt} ${Tt} infinite` };
  }, [G, e.data.animatedDirection]), gt = Kt(
    () => ({
      animation: e.data.animatedDirection === "bop" ? "edge-cycle-pulse 1.5s ease-in-out infinite, edge-flow-bop 3.4s ease-in-out infinite" : "edge-cycle-pulse 1.5s ease-in-out infinite, edge-flow 1s linear infinite"
    }),
    [e.data.animatedDirection]
  ), xt = Kt(
    () => X ? { filter: "saturate(0)" } : void 0,
    [X]
  );
  return /* @__PURE__ */ k("g", { opacity: N ? 0.15 : X ? 0.25 : void 0, style: xt, children: [
    j && /* @__PURE__ */ u(
      "path",
      {
        d: g,
        stroke: "#ef4444",
        strokeWidth: O + 6 / r.zoom,
        strokeLinecap: "round",
        fill: "none",
        opacity: 0.25,
        style: vt
      }
    ),
    lt && /* @__PURE__ */ u(
      "path",
      {
        d: g,
        stroke: "#3b82f6",
        strokeWidth: O + 6 / r.zoom,
        strokeLinecap: "round",
        fill: "none",
        opacity: 0.3
      }
    ),
    nt ? nt.map((ht, Dt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeDasharray: ht.strokeDasharray,
        strokeLinecap: "round",
        fill: ht.fill ?? "none",
        style: G ? rt : void 0
      },
      Dt
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: g,
        stroke: H,
        strokeWidth: O,
        strokeDasharray: G ? "12,8" : j ? `${6 * O},${4 * O}` : ot,
        strokeLinecap: "round",
        fill: "none",
        style: j ? gt : rt
      }
    ),
    e.data.arrowHead === "arrow" && (ut ? ut.map((ht, Dt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeLinecap: "round",
        strokeLinejoin: "round",
        fill: ht.fill ?? "none"
      },
      `ah${Dt}`
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: Bo(M, z, R, $),
        fill: "none",
        stroke: H,
        strokeWidth: O,
        strokeLinecap: "round",
        strokeLinejoin: "round"
      }
    )),
    e.data.arrowHead === "filled" && /* @__PURE__ */ u(
      "path",
      {
        d: Rn(M, z, R, $),
        fill: H,
        stroke: "none"
      }
    ),
    e.data.arrowHead === "dot" && /* @__PURE__ */ u(
      "circle",
      {
        cx: M,
        cy: z,
        r: $ * 0.25,
        fill: H
      }
    ),
    e.data.arrowTail === "arrow" && (pt ? pt.map((ht, Dt) => /* @__PURE__ */ u(
      "path",
      {
        d: ht.d,
        stroke: ht.stroke,
        strokeWidth: ht.strokeWidth,
        strokeLinecap: "round",
        strokeLinejoin: "round",
        fill: ht.fill ?? "none"
      },
      `at${Dt}`
    )) : /* @__PURE__ */ u(
      "path",
      {
        d: Bo(x, S, K, P),
        fill: "none",
        stroke: H,
        strokeWidth: O,
        strokeLinecap: "round",
        strokeLinejoin: "round"
      }
    )),
    e.data.arrowTail === "filled" && /* @__PURE__ */ u(
      "path",
      {
        d: Rn(x, S, K, P),
        fill: H,
        stroke: "none"
      }
    ),
    e.data.arrowTail === "dot" && /* @__PURE__ */ u(
      "circle",
      {
        cx: x,
        cy: S,
        r: P * 0.25,
        fill: H
      }
    ),
    e.data.label && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u(
        "rect",
        {
          x: W - (e.data.label.length * 3.5 + 6) / r.zoom,
          y: I - 8 / r.zoom,
          width: (e.data.label.length * 7 + 12) / r.zoom,
          height: 16 / r.zoom,
          fill: "white",
          rx: 4 / r.zoom,
          opacity: 0.9
        }
      ),
      /* @__PURE__ */ u(
        "text",
        {
          x: W,
          y: I + 4 / r.zoom,
          fill: H,
          fontSize: 12 / r.zoom,
          textAnchor: "middle",
          style: { pointerEvents: "none" },
          children: e.data.label
        }
      )
    ] }),
    lt && !N && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u(
        "circle",
        {
          cx: x,
          cy: S,
          r: 5 / r.zoom,
          fill: "#3b82f6",
          stroke: "white",
          strokeWidth: 1.5 / r.zoom,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (ht) => {
            ht.stopPropagation(), a == null || a(e.id, "source", ht);
          }
        }
      ),
      /* @__PURE__ */ u(
        "circle",
        {
          cx: M,
          cy: z,
          r: 5 / r.zoom,
          fill: "#3b82f6",
          stroke: "white",
          strokeWidth: 1.5 / r.zoom,
          style: { cursor: "grab", pointerEvents: "auto" },
          onPointerDown: (ht) => {
            ht.stopPropagation(), a == null || a(e.id, "target", ht);
          }
        }
      )
    ] }),
    lt && !N && Z && /* @__PURE__ */ u(
      "circle",
      {
        cx: Z.x,
        cy: Z.y,
        r: 5 / r.zoom,
        fill: "white",
        stroke: "#3b82f6",
        strokeWidth: 1.5 / r.zoom,
        style: {
          cursor: Z.axis === "xy" ? "move" : Z.axis === "x" ? "ew-resize" : "ns-resize",
          pointerEvents: "auto"
        },
        onPointerDown: (ht) => {
          ht.stopPropagation(), c == null || c(e.id, Z.axis, Z.min, Z.max, ht);
        }
      }
    )
  ] });
});
function xh({
  nodes: t,
  viewport: e,
  selection: o,
  measuredHeights: n,
  activeStroke: r,
  shapePreview: s,
  shapePreviewStyle: i,
  onResizeHandleDown: l,
  onRotateStart: a,
  onConnectionHandleDown: c,
  onEdgeEndpointDown: h,
  onKinkHandleDown: p,
  edgePreview: d,
  edgeReconnect: f,
  eraserMarkedIds: m,
  eraserTrail: y,
  laserTrail: b,
  mode: g,
  hoveredNodeId: x,
  registry: S,
  onPortHandleDown: M,
  cycleNodeIds: z,
  containerTypes: W,
  alignGuides: I
}) {
  const R = `translate(${e.x}, ${e.y}) scale(${e.zoom})`, K = t.filter(
    (O) => O.type !== "edge" && O.type !== "content" && O.type !== "image"
  ), Z = t.filter((O) => O.type === "edge").sort((O, ot) => O.z - ot.z), lt = Kt(() => new Map(t.map((O) => [O.id, O])), [t]);
  return /* @__PURE__ */ u(
    "svg",
    {
      "data-sb-overlay": !0,
      style: {
        position: "absolute",
        inset: 0,
        width: "100%",
        height: "100%",
        overflow: "visible",
        pointerEvents: "none"
      },
      children: /* @__PURE__ */ k("g", { transform: R, children: [
        Z.map((O) => {
          const ot = lt.get(O.data.fromId), Q = lt.get(O.data.toId);
          return !ot || !Q ? null : /* @__PURE__ */ u(
            gh,
            {
              edge: O,
              fromNode: ot,
              toNode: Q,
              viewport: e,
              selection: o,
              measuredHeights: n,
              registry: S,
              onEdgeEndpointDown: h,
              onKinkHandleDown: p,
              edgeReconnect: f,
              eraserMarkedIds: m,
              cycleNodeIds: z
            },
            O.id
          );
        }),
        (() => {
          var N, j;
          const O = !!d || !!f, ot = (d == null ? void 0 : d.cursorX) ?? (f == null ? void 0 : f.cursorX) ?? 0, Q = (d == null ? void 0 : d.cursorY) ?? (f == null ? void 0 : f.cursorY) ?? 0, $ = (d == null ? void 0 : d.fromNode.id) ?? (f == null ? void 0 : f.anchorNodeId) ?? null;
          let P = null, G = null;
          const X = /* @__PURE__ */ new Set();
          if (O) {
            let H = 1 / 0, _ = !1;
            const tt = 50 / e.zoom;
            for (const nt of t) {
              if (nt.type === "edge" || nt.id === $ || (j = (N = S == null ? void 0 : S.get(nt.type)) == null ? void 0 : N.ports) != null && j.length) continue;
              const ut = nt.h === "auto" ? (n == null ? void 0 : n[nt.id]) ?? 100 : nt.h, pt = nt.w * 0.2, vt = ut * 0.2;
              ot >= nt.x - pt && ot <= nt.x + nt.w + pt && Q >= nt.y - vt && Q <= nt.y + ut + vt && X.add(nt.id);
              const rt = Ho(nt, n), gt = W ? W.has(nt.type) : nt.type === "frame";
              for (const xt of rt) {
                const ht = Math.hypot(xt.x - ot, xt.y - Q);
                ht >= tt || gt && !_ && P || (!gt && _ || ht < H) && (H = ht, _ = gt, P = nt.id, G = xt.side);
              }
            }
          }
          return t.filter((H) => {
            var _, tt;
            return H.type === "edge" || (tt = (_ = S == null ? void 0 : S.get(H.type)) == null ? void 0 : _.ports) != null && tt.length ? !1 : o.size <= 1 && o.has(H.id) || O && (H.id === $ || X.has(H.id));
          }).map((H) => {
            const _ = Ho(H, n), tt = 4 / e.zoom, nt = 26 / e.zoom, ut = H.rotation || 0, pt = H.h === "auto" ? (n == null ? void 0 : n[H.id]) ?? 100 : H.h, vt = H.x + H.w / 2, rt = H.y + pt / 2, gt = d && d.fromNode.id === H.id || f && f.anchorNodeId === H.id, xt = o.has(H.id) && !O;
            return /* @__PURE__ */ u("g", { transform: ut ? `rotate(${ut}, ${vt}, ${rt})` : void 0, children: _.map(({ side: ht }) => {
              const Dt = {
                top: [H.x + H.w / 2, H.y],
                bottom: [H.x + H.w / 2, H.y + pt],
                left: [H.x, H.y + pt / 2],
                right: [H.x + H.w, H.y + pt / 2]
              }, [Tt, Rt] = Dt[ht], Qt = ht === "top" && o.has(H.id) ? 42 / e.zoom : nt;
              let Ut = Tt, re = Rt;
              switch (ht) {
                case "top":
                  re = Rt - Qt;
                  break;
                case "bottom":
                  re = Rt + Qt;
                  break;
                case "left":
                  Ut = Tt - Qt;
                  break;
                case "right":
                  Ut = Tt + Qt;
                  break;
              }
              const Jt = O && P === H.id && G === ht;
              return /* @__PURE__ */ u(
                "circle",
                {
                  cx: Ut,
                  cy: re,
                  r: Jt ? 5 / e.zoom : tt,
                  fill: gt || Jt ? "#3b82f6" : "white",
                  stroke: Jt ? "white" : O && !gt ? "#3b82f6" : "#94a3b8",
                  strokeWidth: 1.5 / e.zoom,
                  opacity: Jt || O && !gt ? 1 : 0.8,
                  style: {
                    cursor: xt ? "crosshair" : "default",
                    pointerEvents: xt ? "auto" : "none"
                  },
                  onPointerDown: xt ? (oe) => {
                    oe.stopPropagation(), c == null || c(H.id, ht, oe);
                  } : void 0
                },
                `ch-${H.id}-${ht}`
              );
            }) }, `conn-${H.id}`);
          });
        })(),
        S && (() => {
          var N;
          const O = !!d || !!f, ot = (d == null ? void 0 : d.cursorX) ?? (f == null ? void 0 : f.cursorX) ?? 0, Q = (d == null ? void 0 : d.cursorY) ?? (f == null ? void 0 : f.cursorY) ?? 0, $ = (d == null ? void 0 : d.fromNode.id) ?? null, P = (d == null ? void 0 : d.sourceDirection) === "output" ? "input" : (d == null ? void 0 : d.sourceDirection) === "input" ? "output" : null;
          let G = null, X = null;
          if (O && P) {
            let j = 40 / e.zoom;
            for (const H of t) {
              if (H.type === "edge" || H.id === $) continue;
              const _ = S.get(H.type);
              if (!((N = _ == null ? void 0 : _.ports) != null && N.length)) continue;
              const tt = H.h === "auto" ? (n == null ? void 0 : n[H.id]) ?? 100 : H.h, nt = 14 / e.zoom, ut = _.ports.filter((pt) => pt.direction === P);
              for (let pt = 0; pt < ut.length; pt++) {
                const vt = ut[pt], rt = H.y + tt / (ut.length + 1) * (pt + 1), gt = vt.direction === "input" ? H.x - nt : H.x + H.w + nt, xt = Math.hypot(gt - ot, rt - Q);
                xt < j && (j = xt, G = H.id, X = vt.id);
              }
            }
          }
          return t.filter((j) => {
            var _;
            if (j.type === "edge") return !1;
            const H = S.get(j.type);
            return !!((_ = H == null ? void 0 : H.ports) != null && _.length);
          }).map((j) => {
            const _ = S.get(j.type).ports, tt = j.h === "auto" ? (n == null ? void 0 : n[j.id]) ?? 100 : j.h, nt = j.rotation || 0, ut = j.x + j.w / 2, pt = j.y + tt / 2, vt = 6 / e.zoom, rt = 14 / e.zoom, gt = _.filter((Rt) => Rt.direction === "input"), xt = _.filter((Rt) => Rt.direction === "output"), ht = !O, Dt = (Rt, Qt, Ut, re) => {
              const Jt = j.y + tt / (Ut.length + 1) * (Qt + 1), oe = re === "input" ? j.x - rt : j.x + j.w + rt, ve = oi[Rt.dataType] || oi.any, ye = G === j.id && X === Rt.id, se = ye ? 8 / e.zoom : vt, Ae = re === "input" ? j.x : j.x + j.w, Xe = re === "input" ? oe - vt - 4 / e.zoom : oe + vt + 4 / e.zoom;
              return /* @__PURE__ */ k("g", { children: [
                /* @__PURE__ */ u(
                  "line",
                  {
                    x1: oe,
                    y1: Jt,
                    x2: Ae,
                    y2: Jt,
                    stroke: ve,
                    strokeWidth: 1.5 / e.zoom,
                    opacity: 0.4,
                    style: { pointerEvents: "none" }
                  }
                ),
                ye && /* @__PURE__ */ u(
                  "circle",
                  {
                    cx: oe,
                    cy: Jt,
                    r: 12 / e.zoom,
                    fill: "none",
                    stroke: "white",
                    strokeWidth: 1.5 / e.zoom,
                    opacity: 0.3,
                    style: { pointerEvents: "none" }
                  }
                ),
                /* @__PURE__ */ u(
                  "circle",
                  {
                    cx: oe,
                    cy: Jt,
                    r: se,
                    fill: ye ? "white" : ve,
                    stroke: ye ? ve : "#1a1a2e",
                    strokeWidth: 2 / e.zoom,
                    style: {
                      cursor: ht ? "crosshair" : "default",
                      pointerEvents: ht ? "auto" : "none",
                      transition: "r 0.1s, fill 0.1s"
                    },
                    onPointerDown: ht ? (Le) => {
                      Le.stopPropagation(), M == null || M(j.id, Rt.id, re, Le);
                    } : void 0
                  }
                ),
                (() => {
                  const Le = Rt.label || Rt.id, me = 9 / e.zoom, no = 5 / e.zoom, ro = 2.5 / e.zoom, Se = Le.length * me * 0.62 + no * 2, be = me + ro * 2, Me = re === "input" ? Xe - Se : Xe, Ee = Jt - be / 2, v = be / 2, it = ye ? ve : "#1a1a2e", Yt = ye ? ve : "#2a2a40", _t = ye ? "#fff" : "#94a3b8";
                  return /* @__PURE__ */ k("g", { style: { pointerEvents: "none" }, children: [
                    /* @__PURE__ */ u(
                      "rect",
                      {
                        x: Me,
                        y: Ee,
                        width: Se,
                        height: be,
                        rx: v,
                        ry: v,
                        fill: it,
                        fillOpacity: ye ? 0.9 : 0.85,
                        stroke: Yt,
                        strokeWidth: 1 / e.zoom
                      }
                    ),
                    /* @__PURE__ */ u(
                      "text",
                      {
                        x: Me + Se / 2,
                        y: Jt + me * 0.35,
                        fill: _t,
                        fontSize: me,
                        fontWeight: 600,
                        fontFamily: "'Inter', system-ui, sans-serif",
                        textAnchor: "middle",
                        style: { userSelect: "none" },
                        children: Le
                      }
                    )
                  ] });
                })()
              ] }, `port-${j.id}-${Rt.id}`);
            }, Tt = z == null ? void 0 : z.has(j.id);
            return /* @__PURE__ */ k("g", { transform: nt ? `rotate(${nt}, ${ut}, ${pt})` : void 0, children: [
              gt.map((Rt, Qt) => Dt(Rt, Qt, gt, "input")),
              xt.map((Rt, Qt) => Dt(Rt, Qt, xt, "output")),
              Tt && (() => {
                const Rt = 10 / e.zoom, Qt = j.x + j.w + Rt * 0.3, Ut = j.y - Rt * 0.3;
                return /* @__PURE__ */ k("g", { style: { pointerEvents: "none" }, children: [
                  /* @__PURE__ */ u(
                    "circle",
                    {
                      cx: Qt,
                      cy: Ut,
                      r: Rt,
                      fill: "#ef4444",
                      stroke: "#1a1a2e",
                      strokeWidth: 2 / e.zoom
                    }
                  ),
                  /* @__PURE__ */ u(
                    "text",
                    {
                      x: Qt,
                      y: Ut + 4 / e.zoom,
                      fill: "white",
                      fontSize: 12 / e.zoom,
                      fontWeight: 800,
                      textAnchor: "middle",
                      style: { pointerEvents: "none", userSelect: "none" },
                      children: "!"
                    }
                  )
                ] });
              })()
            ] }, `ports-${j.id}`);
          });
        })(),
        d && (() => {
          let O, ot;
          if (d.sourcePort && S) {
            const Q = d.fromNode, $ = S.get(Q.type), P = $ != null && $.ports ? rn(Q, $.ports, d.sourcePort, e.zoom, n) : null;
            if (P)
              O = P.x, ot = P.y;
            else {
              const G = or(Q, d.cursorX, d.cursorY, n);
              O = G.x, ot = G.y;
            }
          } else if (d.sourceHandle) {
            const Q = d.fromNode, $ = Q.h === "auto" ? (n == null ? void 0 : n[Q.id]) ?? 100 : Q.h, P = {
              top: [Q.x + Q.w / 2, Q.y],
              bottom: [Q.x + Q.w / 2, Q.y + $],
              left: [Q.x, Q.y + $ / 2],
              right: [Q.x + Q.w, Q.y + $ / 2]
            }, G = d.sourceHandle, X = G === "top" ? 42 / e.zoom : 26 / e.zoom, [N, j] = P[G];
            let H = N, _ = j;
            switch (G) {
              case "top":
                _ = j - X;
                break;
              case "bottom":
                _ = j + X;
                break;
              case "left":
                H = N - X;
                break;
              case "right":
                H = N + X;
                break;
            }
            if (Q.rotation) {
              const tt = Q.x + Q.w / 2, nt = Q.y + $ / 2, ut = Q.rotation * Math.PI / 180, pt = Math.cos(ut), vt = Math.sin(ut), rt = H - tt, gt = _ - nt;
              O = tt + rt * pt - gt * vt, ot = nt + rt * vt + gt * pt;
            } else
              O = H, ot = _;
          } else {
            const Q = or(d.fromNode, d.cursorX, d.cursorY, n);
            O = Q.x, ot = Q.y;
          }
          return /* @__PURE__ */ u(
            "line",
            {
              x1: O,
              y1: ot,
              x2: d.cursorX,
              y2: d.cursorY,
              stroke: "#3b82f6",
              strokeWidth: 2 / e.zoom,
              strokeDasharray: `${4 / e.zoom}`,
              strokeLinecap: "round"
            }
          );
        })(),
        f && (() => {
          const O = lt.get(f.anchorNodeId);
          if (!O) return null;
          let ot, Q;
          if (f.anchorHandle) {
            const $ = O.h === "auto" ? (n == null ? void 0 : n[O.id]) ?? 100 : O.h, P = {
              top: [O.x + O.w / 2, O.y],
              bottom: [O.x + O.w / 2, O.y + $],
              left: [O.x, O.y + $ / 2],
              right: [O.x + O.w, O.y + $ / 2]
            }, G = f.anchorHandle, X = G === "top" ? 42 / e.zoom : 26 / e.zoom, [N, j] = P[G];
            let H = N, _ = j;
            switch (G) {
              case "top":
                _ = j - X;
                break;
              case "bottom":
                _ = j + X;
                break;
              case "left":
                H = N - X;
                break;
              case "right":
                H = N + X;
                break;
            }
            if (O.rotation) {
              const tt = O.x + O.w / 2, nt = O.y + $ / 2, ut = O.rotation * Math.PI / 180, pt = Math.cos(ut), vt = Math.sin(ut), rt = H - tt, gt = _ - nt;
              ot = tt + rt * pt - gt * vt, Q = nt + rt * vt + gt * pt;
            } else
              ot = H, Q = _;
          } else {
            const $ = or(O, f.cursorX, f.cursorY, n);
            ot = $.x, Q = $.y;
          }
          return /* @__PURE__ */ u(
            "line",
            {
              x1: ot,
              y1: Q,
              x2: f.cursorX,
              y2: f.cursorY,
              stroke: "#3b82f6",
              strokeWidth: 2 / e.zoom,
              strokeDasharray: `${4 / e.zoom}`,
              strokeLinecap: "round"
            }
          );
        })(),
        o.size === 1 && K.filter((O) => o.has(O.id)).map((O) => /* @__PURE__ */ u(
          bh,
          {
            node: O,
            zoom: e.zoom,
            showHandles: o.size === 1,
            measuredHeights: n,
            onHandlePointerDown: l,
            onRotateStart: a
          },
          `sel-${O.id}`
        )),
        r && r.points.length > 1 && (() => {
          if (r.strokeStyle === "dashed" || r.strokeStyle === "dotted") {
            const ot = r.points, Q = ["M", ot[0][0], ot[0][1]];
            for (let G = 1; G < ot.length; G++) {
              const [X, N] = ot[G], [j, H] = ot[G - 1];
              Q.push("Q", j, H, (j + X) / 2, (H + N) / 2);
            }
            const $ = ot[ot.length - 1];
            Q.push("L", $[0], $[1]);
            const P = eo(r.strokeStyle);
            return /* @__PURE__ */ u(
              "path",
              {
                d: Q.join(" "),
                fill: "none",
                stroke: r.color,
                strokeWidth: r.width,
                strokeDasharray: P == null ? void 0 : P.map((G) => G * Math.max(r.width, 1)).join(" "),
                strokeLinecap: "round",
                strokeLinejoin: "round"
              }
            );
          }
          return /* @__PURE__ */ u(
            "path",
            {
              d: Zr(r.points, {
                size: r.width
              }),
              fill: r.color
            }
          );
        })(),
        s && i && (() => {
          const O = Math.min(s.startX, s.endX), ot = Math.min(s.startY, s.endY), Q = Math.abs(s.endX - s.startX), $ = Math.abs(s.endY - s.startY);
          if (Q < 2 && $ < 2) return null;
          const P = i, G = P.shapeType || "rect";
          if (G === "ellipse")
            return /* @__PURE__ */ u(
              "ellipse",
              {
                cx: O + Q / 2,
                cy: ot + $ / 2,
                rx: Q / 2,
                ry: $ / 2,
                stroke: P.stroke,
                strokeWidth: P.strokeWidth,
                fill: "none",
                strokeDasharray: "4"
              }
            );
          if (G === "diamond")
            return /* @__PURE__ */ u(
              "polygon",
              {
                points: `${O + Q / 2},${ot} ${O + Q},${ot + $ / 2} ${O + Q / 2},${ot + $} ${O},${ot + $ / 2}`,
                stroke: P.stroke,
                strokeWidth: P.strokeWidth,
                fill: "none",
                strokeDasharray: "4"
              }
            );
          if (G === "line" || G === "arrow") {
            const X = s.startX, N = s.startY, j = s.endX, H = s.endY;
            return /* @__PURE__ */ k(ft, { children: [
              /* @__PURE__ */ u(
                "line",
                {
                  x1: X,
                  y1: N,
                  x2: j,
                  y2: H,
                  stroke: P.stroke,
                  strokeWidth: P.strokeWidth,
                  strokeDasharray: "4"
                }
              ),
              G === "arrow" && (() => {
                const _ = Math.atan2(H - N, j - X), tt = Math.max(12, P.strokeWidth * 4), nt = Math.PI / 6, ut = j - tt * Math.cos(_ - nt), pt = H - tt * Math.sin(_ - nt), vt = j - tt * Math.cos(_ + nt), rt = H - tt * Math.sin(_ + nt);
                return /* @__PURE__ */ u(
                  "polyline",
                  {
                    points: `${ut},${pt} ${j},${H} ${vt},${rt}`,
                    stroke: P.stroke,
                    strokeWidth: P.strokeWidth,
                    fill: "none",
                    strokeDasharray: "4"
                  }
                );
              })()
            ] });
          }
          return /* @__PURE__ */ u(
            "rect",
            {
              x: O,
              y: ot,
              width: Q,
              height: $,
              stroke: P.stroke,
              strokeWidth: P.strokeWidth,
              fill: "none",
              strokeDasharray: "4"
            }
          );
        })(),
        y && y.length > 1 && (() => {
          const O = performance.now(), ot = 400, Q = 6 / e.zoom, $ = [`M${y[0][0]},${y[0][1]}`];
          if (y.length === 2)
            $.push(`L${y[1][0]},${y[1][1]}`);
          else {
            for (let tt = 0; tt < y.length - 1; tt++) {
              const nt = (y[tt][0] + y[tt + 1][0]) / 2, ut = (y[tt][1] + y[tt + 1][1]) / 2;
              $.push(`Q${y[tt][0]},${y[tt][1]},${nt},${ut}`);
            }
            const _ = y[y.length - 1];
            $.push(`L${_[0]},${_[1]}`);
          }
          const P = $.join(" "), G = (O - y[y.length - 1][2]) / ot, X = (O - y[0][2]) / ot, N = Math.max(0, 0.85 * (1 - G)), j = Math.max(0, 0.85 * (1 - X)), H = (N + j) / 2;
          return H <= 0 ? null : /* @__PURE__ */ k(ft, { children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: P,
                fill: "none",
                stroke: "#9ca3af",
                strokeWidth: Q * 3,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: H * 0.35
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: P,
                fill: "none",
                stroke: "#d1d5db",
                strokeWidth: Q,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: H
              }
            )
          ] });
        })(),
        b && b.length > 1 && (() => {
          const O = performance.now(), ot = 1560, Q = 6 / e.zoom, $ = [];
          let P = !1, G = !1;
          for (let ut = 0; ut < b.length; ut++) {
            const pt = b[ut];
            if (isNaN(pt[0])) {
              P = !1, G = !1;
              continue;
            }
            if (!P)
              $.push(`M${pt[0]},${pt[1]}`), P = !0, G = !0;
            else if (G) {
              const vt = ut + 1 < b.length && !isNaN(b[ut + 1][0]) ? b[ut + 1] : null;
              if (vt) {
                const rt = (pt[0] + vt[0]) / 2, gt = (pt[1] + vt[1]) / 2;
                $.push(`Q${pt[0]},${pt[1]},${rt},${gt}`);
              } else
                $.push(`L${pt[0]},${pt[1]}`);
            }
          }
          if ($.length === 0) return null;
          const X = $.join(" "), N = b.filter((ut) => !isNaN(ut[0]));
          if (N.length === 0) return null;
          const j = (O - N[N.length - 1][2]) / ot, H = (O - N[0][2]) / ot, _ = Math.max(0, 0.85 * (1 - j)), tt = Math.max(0, 0.85 * (1 - H)), nt = (_ + tt) / 2;
          return nt <= 0 ? null : /* @__PURE__ */ k(ft, { children: [
            /* @__PURE__ */ u(
              "path",
              {
                d: X,
                fill: "none",
                stroke: "#ef4444",
                strokeWidth: Q * 3,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: nt * 0.35
              }
            ),
            /* @__PURE__ */ u(
              "path",
              {
                d: X,
                fill: "none",
                stroke: "#ff6b6b",
                strokeWidth: Q,
                strokeLinecap: "round",
                strokeLinejoin: "round",
                opacity: nt
              }
            )
          ] });
        })(),
        I && I.length > 0 && I.map((O, ot) => /* @__PURE__ */ u(
          "line",
          {
            x1: O.axis === "x" ? O.position : O.start,
            y1: O.axis === "x" ? O.start : O.position,
            x2: O.axis === "x" ? O.position : O.end,
            y2: O.axis === "x" ? O.end : O.position,
            stroke: "#f472b6",
            strokeWidth: 1 / e.zoom,
            strokeDasharray: `${3 / e.zoom} ${2 / e.zoom}`,
            opacity: 0.8
          },
          `guide-${ot}`
        ))
      ] })
    }
  );
}
function wh({
  x: t,
  y: e,
  sections: o,
  onClose: n
}) {
  const r = at(null);
  yt(() => {
    var m;
    const p = (y) => {
      r.current && !r.current.contains(y.target) && n();
    }, d = (y) => {
      y.key === "Escape" && n();
    }, f = ((m = r.current) == null ? void 0 : m.ownerDocument) ?? document;
    return f.addEventListener("pointerdown", p, !0), f.addEventListener("keydown", d), () => {
      f.removeEventListener("pointerdown", p, !0), f.removeEventListener("keydown", d);
    };
  }, [n]), yt(() => {
    const p = r.current;
    if (!p) return;
    const d = p.getBoundingClientRect(), f = p.ownerDocument.defaultView ?? window;
    let m = t, y = e;
    d.right > f.innerWidth && (m = t - d.width), d.bottom > f.innerHeight && (y = e - d.height), m = Math.max(0, m), y = Math.max(0, y), p.style.left = `${m}px`, p.style.top = `${y}px`;
  }, [t, e]);
  const s = st(
    (p) => {
      p.disabled || (p.action(), n());
    },
    [n]
  ), i = navigator.platform.includes("Mac"), l = i ? "⌘" : "Ctrl+", a = i ? "⌥" : "Alt+", c = i ? "⇧" : "Shift+", h = (p) => p.replace("Mod+", l).replace("Alt+", a).replace("Shift+", c);
  return /* @__PURE__ */ u(
    "div",
    {
      "data-sb-context-menu": !0,
      ref: r,
      onPointerDown: (p) => p.stopPropagation(),
      onContextMenu: (p) => p.preventDefault(),
      style: {
        position: "fixed",
        left: t,
        top: e,
        zIndex: 9999,
        minWidth: 200,
        background: "#1e1e2e",
        borderRadius: 8,
        border: "1px solid #333",
        boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
        padding: "4px 0",
        color: "#e0e0e0",
        fontSize: 13,
        fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
        userSelect: "none"
      },
      children: o.map((p, d) => /* @__PURE__ */ k("div", { children: [
        d > 0 && /* @__PURE__ */ u(
          "div",
          {
            style: {
              height: 1,
              background: "#333",
              margin: "4px 0"
            }
          }
        ),
        p.items.map((f, m) => /* @__PURE__ */ k(
          "div",
          {
            onClick: () => s(f),
            style: {
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              padding: "6px 16px",
              cursor: f.disabled ? "default" : "pointer",
              opacity: f.disabled ? 0.4 : 1,
              color: f.danger ? "#f87171" : "#e0e0e0",
              transition: "background 0.1s"
            },
            onMouseEnter: (y) => {
              f.disabled || (y.currentTarget.style.background = "rgba(255,255,255,0.08)");
            },
            onMouseLeave: (y) => {
              y.currentTarget.style.background = "transparent";
            },
            children: [
              /* @__PURE__ */ k("span", { children: [
                f.checked !== void 0 && /* @__PURE__ */ u("span", { style: { display: "inline-block", width: 16, marginRight: 4 }, children: f.checked ? "✓" : "" }),
                f.label
              ] }),
              f.shortcut && /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    marginLeft: 32,
                    fontSize: 12,
                    color: "#888"
                  },
                  children: h(f.shortcut)
                }
              )
            ]
          },
          m
        ))
      ] }, d))
    }
  );
}
const wa = "sbd-clipboard", kh = "sbd-nodes:";
function ka(t) {
  const e = JSON.stringify(t), o = new TextEncoder().encode(e);
  let n = "";
  for (let r = 0; r < o.length; r++) n += String.fromCharCode(o[r]);
  return btoa(n);
}
function ni(t) {
  try {
    const e = atob(t), o = new Uint8Array(e.length);
    for (let r = 0; r < e.length; r++) o[r] = e.charCodeAt(r);
    const n = new TextDecoder().decode(o);
    return JSON.parse(n);
  } catch {
    return null;
  }
}
function va(t) {
  const e = t.match(/data-sbd-nodes="([A-Za-z0-9+/=]+)"/);
  if (e) return ni(e[1]);
  const o = t.match(
    new RegExp(`<!--${kh}([A-Za-z0-9+/=]+)-->`)
  );
  return o ? ni(o[1]) : null;
}
function vn(t) {
  return !!t.closest(".bn-editor") || t.isContentEditable || t.tagName === "INPUT" || t.tagName === "TEXTAREA";
}
function Sa(t) {
  return t.map((e) => {
    var r;
    const o = (e.content || []).filter((s) => s.type === "text").map((s) => s.text ?? "").join(""), n = (r = e.children) != null && r.length ? `
` + Sa(e.children) : "";
    return o + n;
  }).filter(Boolean).join(`
`);
}
function vh(t) {
  var o;
  const e = [];
  for (const n of t)
    switch (n.type) {
      case "content": {
        const r = n.data;
        (o = r.blocks) != null && o.length ? e.push(Sa(r.blocks)) : r.markdown && e.push(r.markdown);
        break;
      }
      case "image": {
        const r = n.data;
        r.src.startsWith("http") ? e.push(r.src) : e.push(r.alt || "[Image]");
        break;
      }
      case "shape": {
        const r = n.data;
        r.label && e.push(r.label);
        break;
      }
      case "text": {
        const r = n.data;
        r.text && e.push(r.text);
        break;
      }
      case "sticky": {
        const r = n.data;
        r.text && e.push(r.text);
        break;
      }
      case "draw":
        break;
      case "edge": {
        const r = n.data;
        r.label && e.push(r.label);
        break;
      }
    }
  return e.join(`

`);
}
function ri(t, e) {
  const o = vh(e), n = o.split(`
`).filter(Boolean).map((s) => `<p>${s}</p>`).join(""), r = ka(e);
  return t.setData(
    "text/html",
    `<!--${wa}--><div data-sbd-nodes="${r}">${n || "<p></p>"}</div>`
  ), t.setData("text/plain", o), o;
}
function Sh(t, e) {
  let o = (e == null ? void 0 : e.ownerDocument) ?? document, n = o.defaultView ?? window, r = n.innerWidth / 2, s = n.innerHeight / 2, i = null;
  const l = (y) => {
    r = y.clientX, s = y.clientY;
  }, a = (y) => {
    vn(y.target) || t.selection.size !== 0 && (y.preventDefault(), t.copySelected(), i = ri(
      y.clipboardData,
      t.getClipboardNodes()
    ));
  }, c = (y) => {
    vn(y.target) || t.selection.size !== 0 && (y.preventDefault(), t.copySelected(), i = ri(
      y.clipboardData,
      t.getClipboardNodes()
    ), t.deleteSelected());
  }, h = async (y) => {
    var R, K, Z;
    if (vn(y.target)) return;
    const { x: b, y: g } = t.screenToCanvas(r, s), x = ((R = y.clipboardData) == null ? void 0 : R.getData("text/html")) || "", S = ((K = y.clipboardData) == null ? void 0 : K.getData("text/plain")) || "";
    if (x.includes(wa) || x.includes("data-sbd-nodes=") || i !== null && S === i) {
      if (i !== null && S === i && t.hasClipboard()) {
        y.preventDefault(), t.pasteClipboard(b, g);
        return;
      }
      const O = va(x);
      if (O) {
        y.preventDefault(), t.setClipboard(O), t.pasteClipboard(b, g);
        return;
      }
    }
    const z = (Z = y.clipboardData) == null ? void 0 : Z.items;
    if (z) {
      for (const lt of Array.from(z))
        if (lt.type.startsWith("image/")) {
          y.preventDefault();
          const O = lt.getAsFile();
          if (!O) continue;
          const ot = new FileReader();
          ot.onload = () => {
            const Q = ot.result, $ = new Image();
            $.onload = () => {
              const P = t.screenToCanvas(r, s), G = 400, X = 300, N = $.naturalWidth / $.naturalHeight, j = Math.min($.naturalWidth, G), H = Math.min($.naturalHeight, X), _ = N >= 1 ? j : H * N, tt = N >= 1 ? j / N : H;
              let nt = Q;
              if (x) {
                const pt = x.match(/<img[^>]+src=["']([^"']+)["']/i);
                pt && /\.(gif|webp|apng)(\?|#|$)/i.test(pt[1]) && (nt = pt[1].replace(/&amp;/g, "&"));
              }
              const ut = {
                id: bt(10),
                type: "image",
                x: P.x,
                y: P.y,
                w: _,
                h: tt,
                z: t.nextZ(),
                data: { src: nt }
              };
              t.addNode(ut), t.select(ut.id);
            }, $.src = Q;
          }, ot.readAsDataURL(O);
          return;
        }
    }
    const W = Dr(S) ?? Dr(x);
    if (W) {
      y.preventDefault();
      const lt = t.screenToCanvas(r, s), O = await xa(
        W,
        lt.x,
        lt.y,
        t.nextZ()
      );
      O && (t.addNode(O), t.select(O.id));
      return;
    }
    if (Id(S)) {
      const lt = zd(S);
      if (lt) {
        y.preventDefault();
        const O = {
          id: bt(10),
          type: "youtube",
          x: b,
          y: g,
          w: 560,
          h: 315,
          z: t.nextZ(),
          data: { videoId: lt, url: S.trim() }
        };
        t.addNode(O), t.select(O.id);
        return;
      }
    }
    const I = x.replace(/^<meta[^>]*>/i, "").replace(/<!--StartFragment-->|<!--EndFragment-->/g, "").trim();
    if (I)
      try {
        const lt = Ei(I);
        if (lt.length > 0) {
          y.preventDefault();
          const O = {
            id: bt(10),
            type: "content",
            x: b,
            y: g,
            w: 300,
            h: "auto",
            z: t.nextZ(),
            data: { blocks: lt, markdown: S, borderColor: "#1e1e2e" }
          };
          t.addNode(O), t.select(O.id);
          return;
        }
      } catch {
      }
    if (S.trim()) {
      y.preventDefault();
      const lt = await Gr(S), O = {
        id: bt(10),
        type: "content",
        x: b,
        y: g,
        w: 300,
        h: "auto",
        z: t.nextZ(),
        data: { blocks: lt, markdown: S, borderColor: "#1e1e2e" }
      };
      t.addNode(O), t.select(O.id);
      return;
    }
    t.hasClipboard() && (y.preventDefault(), t.pasteClipboard(b, g));
  }, p = (y) => {
    const b = y.target;
    if (vn(b)) return;
    if (t.presentationMode) {
      if (y.key === "ArrowRight" || y.key === " ") {
        y.preventDefault(), t.presentationNext();
        return;
      }
      if (y.key === "ArrowLeft") {
        y.preventDefault(), t.presentationPrev();
        return;
      }
      if (y.key === "Escape") {
        y.preventDefault(), t.exitPresentation();
        return;
      }
      return;
    }
    const g = y.ctrlKey || y.metaKey;
    if (g && y.key === "c") {
      t.copySelected();
      return;
    }
    if (g && y.key === "x") {
      t.copySelected();
      return;
    }
    if (g && y.key === "d") {
      y.preventDefault(), t.duplicateSelected();
      return;
    }
    if (g && y.key === "g") {
      y.preventDefault(), y.shiftKey ? t.ungroupSelected() : t.groupSelected();
      return;
    }
    if (y.shiftKey && !g && y.key === "H") {
      y.preventDefault(), t.flipSelectedHorizontal();
      return;
    }
    if (y.shiftKey && !g && y.key === "V") {
      y.preventDefault(), t.flipSelectedVertical();
      return;
    }
    if (g && y.key === "]") {
      y.preventDefault();
      const x = Array.from(t.selection);
      y.altKey ? t.bringToFront(x) : t.bringForward(x);
      return;
    }
    if (g && y.key === "[") {
      y.preventDefault();
      const x = Array.from(t.selection);
      y.altKey ? t.sendToBack(x) : t.sendBackward(x);
      return;
    }
    if (!g && !y.altKey && !y.shiftKey) {
      if (y.key === "s") {
        t.setMode("select");
        return;
      }
      if (y.key === "p") {
        t.setMode("hand");
        return;
      }
      if (y.key === "d") {
        t.setMode("draw");
        return;
      }
      if (y.key === "g") {
        t.setMode("shape");
        return;
      }
      if (y.key === "t") {
        t.setMode("text");
        return;
      }
      if (y.key === "b") {
        t.setMode("note");
        return;
      }
      if (y.key === "y") {
        t.setMode("sticky");
        return;
      }
      if (y.key === "f") {
        t.setMode("frame");
        return;
      }
      if (y.key === "e") {
        t.setMode("erase");
        return;
      }
      if (y.key === "l") {
        t.toggleLassoSelect();
        return;
      }
      if (y.key === "z") {
        t.setMode("laser");
        return;
      }
    }
    if (y.key === "Delete" || y.key === "Backspace") {
      t.deleteSelected();
      return;
    }
    if (g && y.key === "z") {
      y.preventDefault(), y.shiftKey ? t.redo() : t.undo();
      return;
    }
    if (g && y.key === "a") {
      y.preventDefault(), t.selectMultiple(t.getAllNodes().map((x) => x.id));
      return;
    }
    if (y.key === "Escape") {
      if (t.activeGroupId) {
        t.exitGroup();
        return;
      }
      t.deselectAll(), t.setMode("select");
      return;
    }
    if (g && (y.key === "=" || y.key === "+")) {
      y.preventDefault(), t.zoomIn();
      return;
    }
    if (g && y.key === "-") {
      y.preventDefault(), t.zoomOut();
      return;
    }
    if (g && y.key === "0") {
      y.preventDefault(), t.fitToContent();
      return;
    }
  };
  function d(y, b) {
    y.addEventListener("pointermove", l), y.addEventListener("copy", a), y.addEventListener("cut", c), y.addEventListener("paste", h), b.addEventListener("keydown", p);
  }
  function f(y, b) {
    y.removeEventListener("pointermove", l), y.removeEventListener("copy", a), y.removeEventListener("cut", c), y.removeEventListener("paste", h), b.removeEventListener("keydown", p);
  }
  d(o, n);
  const m = setInterval(() => {
    if (!e) return;
    const y = e.ownerDocument;
    y !== o && (f(o, n), o = y, n = y.defaultView ?? window, r = n.innerWidth / 2, s = n.innerHeight / 2, d(o, n));
  }, 500);
  return () => {
    clearInterval(m), f(o, n);
  };
}
async function si(t, e) {
  const o = t.getAllNodes();
  if (o.length === 0) return;
  const n = t.measuredHeights, r = Mh(o, n, t), s = e.padding ?? 40, i = e.background !== !1, l = e.format === "png", a = r.w + s * 2, c = r.h + s * 2, h = r.x - s, p = r.y - s, d = await Ma(o, t, n, h, p, l), f = i ? ln(t.boardBackground).canvasBg : "transparent", m = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${a}" height="${c}" viewBox="0 0 ${a} ${c}">`,
    `<rect width="${a}" height="${c}" fill="${f}"/>`,
    ...d,
    "</svg>"
  ].join(`
`);
  if (e.format === "svg")
    ii(new Blob([m], { type: "image/svg+xml" }), "board.svg");
  else {
    const y = e.scale ?? 4, b = await Bh(m, a, c, y);
    ii(b, "board.png");
  }
}
function Mh(t, e, o) {
  let n = 1 / 0, r = 1 / 0, s = -1 / 0, i = -1 / 0;
  for (const a of t) {
    if (a.type === "edge") continue;
    const c = o.resolveHeight(a);
    n = Math.min(n, a.x), r = Math.min(r, a.y), s = Math.max(s, a.x + a.w), i = Math.max(i, a.y + c);
  }
  const l = new Map(t.map((a) => [a.id, a]));
  for (const a of t) {
    if (a.type !== "edge") continue;
    const c = a, h = l.get(c.data.fromId), p = l.get(c.data.toId);
    if (!h || !p) continue;
    const d = Ze(
      h,
      p,
      c.data.edgeType,
      e,
      c.data.sourceHandle,
      c.data.targetHandle,
      c.data.midpointOffset,
      c.data.curveOffset
    );
    n = Math.min(n, d.bounds.x), r = Math.min(r, d.bounds.y), s = Math.max(s, d.bounds.x + d.bounds.w), i = Math.max(i, d.bounds.y + d.bounds.h);
  }
  return isFinite(n) ? { x: n, y: r, w: s - n, h: i - r } : { x: 0, y: 0, w: 100, h: 100 };
}
async function Ma(t, e, o, n, r, s) {
  const i = new Map(t.map((c) => [c.id, c])), l = [...t].sort((c, h) => c.z - h.z), a = [];
  for (const c of l) {
    const h = c.x - n, p = c.y - r, d = e.resolveHeight(c);
    switch (c.type) {
      case "frame":
        a.push(Ch(c, h, p, d));
        break;
      case "content":
        a.push(zh(c, h, p, c.w, d));
        break;
      case "draw":
        a.push(Ih(c, n, r));
        break;
      case "shape":
        a.push(Ph(c, h, p, c.w, d));
        break;
      case "text":
        a.push(Rh(c, h, p, c.w, d));
        break;
      case "sticky":
        a.push(Ah(c, h, p, c.w, d));
        break;
      case "image":
        a.push(await Lh(c, h, p, c.w, d, s));
        break;
      case "youtube":
        a.push(await Eh(c, h, p, c.w, d, s));
        break;
      case "edge": {
        const f = c, m = i.get(f.data.fromId), y = i.get(f.data.toId);
        m && y && a.push(Wh(f, m, y, o, n, r));
        break;
      }
    }
  }
  return a;
}
function oo(t, e, o, n, r, s, i) {
  const l = [];
  if (s) {
    const a = e + n / 2, c = o + r / 2;
    l.push(`transform="rotate(${s}, ${a}, ${c})"`);
  }
  return i !== void 0 && i !== 1 && l.push(`opacity="${i}"`), `<g ${l.join(" ")}>${t}</g>`;
}
function Ch(t, e, o, n) {
  const r = t.data, s = r.backgroundColor || "rgba(0,0,0,0.02)", i = r.borderColor || "#d1d5db", l = r.borderWidth ?? 1, a = Gn(r.borderStyle, l), c = r.label ? Go(r.label) : "";
  let h = `<rect x="${e}" y="${o}" width="${t.w}" height="${n}" rx="4" fill="${s}" stroke="${i}" stroke-width="${l}"` + (a ? ` stroke-dasharray="${a}"` : "") + "/>";
  return c && (h += `<text x="${e + 8}" y="${o - 6}" font-size="12" fill="#6b7280" font-family="sans-serif">${c}</text>`), oo(h, e, o, t.w, n, t.rotation, r.opacity);
}
function zh(t, e, o, n, r) {
  var p;
  const s = t.data, i = ((p = s.markdown) == null ? void 0 : p.trim()) || "", l = s.borderColor, a = s.borderWidth ?? 0, c = Gn(s.borderStyle, a);
  let h = "";
  return l && a > 0 ? h += `<rect x="${e}" y="${o}" width="${n}" height="${r}" rx="4" fill="white" stroke="${l}" stroke-width="${a}"` + (c ? ` stroke-dasharray="${c}"` : "") + "/>" : h += `<rect x="${e}" y="${o}" width="${n}" height="${r}" rx="4" fill="white"/>`, i && (h += as(i, e + 12, o + 20, n - 24, 14, 1.6, "#374151", "left", "sans-serif")), oo(h, e, o, n, r, t.rotation, s.opacity);
}
function Ih(t, e, o) {
  const n = t.data, r = n.points.map(
    ([l, a, c]) => [l + t.x - e, a + t.y - o, c]
  );
  if (r.length === 0) return "";
  if (n.tool === "vector")
    return Th(r, n, t);
  const s = eo(n.strokeStyle);
  let i = "";
  if (n.fill) {
    const l = r.map(([a, c]) => [a, c]);
    if (l.length > 2) {
      const a = l.map((c, h) => `${h === 0 ? "M" : "L"}${c[0]},${c[1]}`).join(" ") + " Z";
      i += `<path d="${a}" fill="${n.fill}" fill-opacity="0.4" stroke="none"/>`;
    }
  }
  if (s) {
    const l = r.map((c, h) => `${h === 0 ? "M" : "L"}${c[0].toFixed(1)},${c[1].toFixed(1)}`).join(" "), a = s.map((c) => c * Math.max(n.strokeWidth, 1)).join(" ");
    i += `<path d="${l}" fill="none" stroke="${n.color}" stroke-width="${n.strokeWidth}" stroke-dasharray="${a}" stroke-linecap="round" stroke-linejoin="round"/>`;
  } else {
    const l = Zr(r, { size: n.strokeWidth });
    l && (i += `<path d="${l}" fill="${n.color}" stroke="none"/>`);
  }
  return n.opacity !== void 0 && n.opacity !== 1 ? `<g opacity="${n.opacity}">${i}</g>` : i;
}
function Th(t, e, o) {
  const n = t.map((a, c) => `${c === 0 ? "M" : "L"}${a[0].toFixed(2)},${a[1].toFixed(2)}`).join(" ") + " Z", r = eo(e.strokeStyle), s = r ? ` stroke-dasharray="${r.map((a) => a * Math.max(e.strokeWidth, 1)).join(" ")}"` : "", i = `<path d="${n}" fill="${e.fill || "none"}" stroke="${e.color}" stroke-width="${e.strokeWidth}" stroke-linecap="round" stroke-linejoin="round"${s}/>`, l = o.h === "auto" ? 0 : o.h;
  return oo(i, o.x, o.y, o.w, l, o.rotation, e.opacity);
}
function Ph(t, e, o, n, r) {
  const s = t.data, i = {
    stroke: s.stroke,
    fill: s.fill,
    fillStyle: s.fillStyle,
    roughness: s.roughness,
    strokeWidth: s.strokeWidth,
    strokeLineDash: eo(s.strokeStyle),
    seed: t.id
  };
  let l;
  const a = s.edgeStyle === "round";
  switch (s.shape) {
    case "rect":
      l = Wn(e, o, n, r, i, a);
      break;
    case "ellipse":
      l = Jr(e + n / 2, o + r / 2, n, r, i);
      break;
    case "diamond":
      l = $r(e, o, n, r, i, a);
      break;
    case "line": {
      const h = s.startPoint ?? [0, 0], p = s.endPoint ?? [n, r];
      l = Oo(e + h[0], o + h[1], e + p[0], o + p[1], i);
      break;
    }
    case "arrow": {
      const h = s.startPoint ?? [0, 0], p = s.endPoint ?? [n, r];
      l = _r(e + h[0], o + h[1], e + p[0], o + p[1], i);
      break;
    }
    default:
      l = Wn(e, o, n, r, i);
  }
  const c = l.map(
    (h) => `<path d="${h.d}" fill="${h.fill || "none"}" stroke="${h.stroke}" stroke-width="${h.strokeWidth}"` + (h.strokeDasharray ? ` stroke-dasharray="${h.strokeDasharray}"` : "") + "/>"
  ).join(`
`);
  return oo(c, e, o, n, r, t.rotation, s.opacity);
}
function Rh(t, e, o, n, r) {
  const s = t.data, i = r || s.text.split(`
`).length * s.fontSize * 1, l = Je(s.fontFamily), a = !!s.borderColor, c = a ? 6 : 0;
  let h = "";
  if (a) {
    const d = s.borderWidth ?? 1, f = Gn(s.borderStyle, d);
    h += `<rect x="${e}" y="${o}" width="${n}" height="${i}" rx="4" fill="none" stroke="${s.borderColor}" stroke-width="${d}"` + (f ? ` stroke-dasharray="${f}"` : "") + "/>";
  }
  const p = s.align === "center" ? e + n / 2 : s.align === "right" ? e + n - c : e + c;
  return h += as(
    s.text,
    p,
    o + c + s.fontSize,
    n - c * 2,
    s.fontSize,
    1,
    s.color,
    s.align,
    l
  ), oo(h, e, o, n, i, t.rotation, s.opacity);
}
function Ah(t, e, o, n, r) {
  const s = t.data, i = s.fontSize ?? 16, l = `<rect x="${e}" y="${o}" width="${n}" height="${r}" rx="2" fill="${s.color}"/>` + as(s.text, e + 12, o + 12 + i, n - 24, i, 1.4, "#1e1e2e", "left", "sans-serif");
  return oo(l, e, o, n, r, t.rotation, s.opacity);
}
async function Lh(t, e, o, n, r, s) {
  const i = t.data;
  let l = i.src;
  if (s && l && !l.startsWith("data:"))
    try {
      l = await Bn(l);
    } catch {
    }
  const a = i.borderColor, c = i.borderWidth ?? 0, h = Gn(i.borderStyle, c);
  let p = `<image href="${Go(l)}" x="${e}" y="${o}" width="${n}" height="${r}" preserveAspectRatio="xMidYMid slice"/>`;
  return a && c > 0 && (p += `<rect x="${e}" y="${o}" width="${n}" height="${r}" fill="none" stroke="${a}" stroke-width="${c}"` + (h ? ` stroke-dasharray="${h}"` : "") + "/>"), oo(p, e, o, n, r, t.rotation, i.opacity);
}
async function Eh(t, e, o, n, r, s) {
  const i = t.data;
  let l = Pd(i.videoId);
  if (s)
    try {
      l = await Bn(l);
    } catch {
    }
  let a = `<rect x="${e}" y="${o}" width="${n}" height="${r}" rx="4" fill="#1a1a1a"/><image href="${Go(l)}" x="${e}" y="${o}" width="${n}" height="${r}" preserveAspectRatio="xMidYMid slice"/>`;
  const c = e + n / 2, h = o + r / 2, p = Math.min(n, r) * 0.12;
  return a += `<circle cx="${c}" cy="${h}" r="${p}" fill="rgba(0,0,0,0.6)"/><path d="${Dh(c, h, p * 0.5)}" fill="white"/>`, oo(a, e, o, n, r, t.rotation, i.opacity);
}
function Dh(t, e, o) {
  const n = o * 0.15, r = t - o * 0.7 + n, s = e - o, i = t + o + n, l = e, a = r, c = e + o;
  return `M${r},${s} L${i},${l} L${a},${c} Z`;
}
function Wh(t, e, o, n, r, s) {
  const i = t.data, l = Ze(
    e,
    o,
    i.edgeType,
    n,
    i.sourceHandle,
    i.targetHandle,
    i.midpointOffset,
    i.curveOffset
  ), a = `translate(${-r}, ${-s})`, c = i.style === "dashed" ? "8 4" : i.style === "dotted" ? "2 3" : void 0, h = i.strokeWidth;
  let p = `<path d="${l.path}" fill="none" stroke="${i.color}" stroke-width="${h}"` + (c ? ` stroke-dasharray="${c}"` : "") + ' stroke-linecap="round" stroke-linejoin="round"/>';
  const d = i.arrowHeadSize ?? Math.max(8, h * 3), f = i.arrowTailSize ?? Math.max(8, h * 3);
  if (i.arrowHead && i.arrowHead !== "none") {
    if (i.arrowHead === "arrow")
      p += `<path d="${Bo(l.x2, l.y2, l.arrowAngle, d)}" fill="none" stroke="${i.color}" stroke-width="${h}" stroke-linecap="round" stroke-linejoin="round"/>`;
    else if (i.arrowHead === "filled")
      p += `<path d="${Rn(l.x2, l.y2, l.arrowAngle, d)}" fill="${i.color}" stroke="none"/>`;
    else if (i.arrowHead === "dot") {
      const m = d / 3;
      p += `<circle cx="${l.x2}" cy="${l.y2}" r="${m}" fill="${i.color}"/>`;
    }
  }
  if (i.arrowTail && i.arrowTail !== "none") {
    if (i.arrowTail === "arrow")
      p += `<path d="${Bo(l.x1, l.y1, l.tailAngle, f)}" fill="none" stroke="${i.color}" stroke-width="${h}" stroke-linecap="round" stroke-linejoin="round"/>`;
    else if (i.arrowTail === "filled")
      p += `<path d="${Rn(l.x1, l.y1, l.tailAngle, f)}" fill="${i.color}" stroke="none"/>`;
    else if (i.arrowTail === "dot") {
      const m = f / 3;
      p += `<circle cx="${l.x1}" cy="${l.y1}" r="${m}" fill="${i.color}"/>`;
    }
  }
  return i.label && (p += `<text x="${l.labelX}" y="${l.labelY}" font-size="12" fill="${i.color}" text-anchor="middle" dominant-baseline="central" font-family="sans-serif">${Go(i.label)}</text>`), `<g transform="${a}">${p}</g>`;
}
function as(t, e, o, n, r, s, i, l, a) {
  if (!t) return "";
  const c = l === "center" ? "middle" : l === "right" ? "end" : "start", h = Fh(t, n, r), p = r * s, d = h.map(
    (f, m) => `<tspan x="${e}" dy="${m === 0 ? 0 : p}">${Go(f)}</tspan>`
  ).join("");
  return `<text x="${e}" y="${o}" font-size="${r}" fill="${i}" font-family="${Go(a)}" text-anchor="${c}">${d}</text>`;
}
function Fh(t, e, o) {
  const n = o * 0.55, r = Math.max(1, Math.floor(e / n)), s = [];
  for (const i of t.split(`
`)) {
    if (!i.trim()) {
      s.push("");
      continue;
    }
    const l = i.split(/\s+/);
    let a = "";
    for (const c of l) {
      const h = a ? a + " " + c : c;
      h.length > r && a ? (s.push(a), a = c) : a = h;
    }
    a && s.push(a);
  }
  return s;
}
function Gn(t, e) {
  const o = e ?? 1;
  if (t === "dashed") return `${8 * o} ${4 * o}`;
  if (t === "dotted") return `${2 * o} ${2 * o}`;
}
function Go(t) {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
}
async function Bn(t) {
  const o = await (await fetch(t)).blob();
  return new Promise((n, r) => {
    const s = new FileReader();
    s.onloadend = () => n(s.result), s.onerror = r, s.readAsDataURL(o);
  });
}
function Bh(t, e, o, n) {
  return new Promise((r, s) => {
    const i = new Image(), l = new Blob([t], { type: "image/svg+xml;charset=utf-8" }), a = URL.createObjectURL(l);
    i.onload = () => {
      const c = document.createElement("canvas");
      c.width = e * n, c.height = o * n;
      const h = c.getContext("2d");
      h.scale(n, n), h.drawImage(i, 0, 0, e, o), URL.revokeObjectURL(a), c.toBlob((p) => {
        p ? r(p) : s(new Error("Canvas toBlob failed"));
      }, "image/png");
    }, i.onerror = () => {
      URL.revokeObjectURL(a), s(new Error("Failed to load SVG as image"));
    }, i.src = a;
  });
}
const Nh = /* @__PURE__ */ new Set(["sans-serif", "serif", "monospace"]), Po = /* @__PURE__ */ new Map(), Hh = 12;
function Oh(t) {
  const e = /* @__PURE__ */ new Set();
  for (const o of t)
    if (o.type === "text") {
      const n = o.data.fontFamily;
      n && !Nh.has(n) && e.add(n);
    }
  return [...e];
}
async function Xh(t) {
  if (t.length === 0) return "";
  const e = [];
  for (const o of t) {
    if (Po.has(o)) {
      e.push(Po.get(o));
      continue;
    }
    try {
      let n;
      if (o === "Excalifont")
        n = await Bn(Di);
      else {
        const l = (await (await fetch(
          `https://fonts.googleapis.com/css2?family=${encodeURIComponent(o)}&display=swap`
        )).text()).match(/url\((https:\/\/[^)]+\.woff2)\)/);
        if (!l) continue;
        n = await Bn(l[1]);
      }
      const r = `@font-face { font-family: '${o}'; src: url('${n}') format('woff2'); }`;
      if (Po.size >= Hh) {
        const s = Po.keys().next().value;
        s !== void 0 && Po.delete(s);
      }
      Po.set(o, r), e.push(r);
    } catch {
    }
  }
  return e.join(`
`);
}
async function Yh(t, e) {
  const o = t.getNode(e);
  if (!o || o.type !== "frame") return "";
  const n = t.resolveHeight(o), r = 0, s = o.w + r * 2, i = n + r * 2, l = o.x - r, a = o.y - r, c = [o], h = /* @__PURE__ */ new Set([e]), p = (x) => {
    h.has(x.id) || x.type === "edge" || (h.add(x.id), c.push(x));
  };
  for (const x of t.getNodesInRect({ x: o.x, y: o.y, w: o.w, h: n }))
    p(x);
  for (const x of t.getFrameChildren(e))
    p(x);
  for (const x of t.getAllNodes())
    if (x.type === "edge") {
      const S = x;
      h.has(S.data.fromId) && h.has(S.data.toId) && c.push(x);
    }
  const d = t.measuredHeights, f = await Ma(c, t, d, l, a, !0), m = Oh(c), y = await Xh(m), b = ln(t.boardBackground).canvasBg, g = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${i}" viewBox="0 0 ${s} ${i}">`,
    y ? `<defs><style>${y}</style></defs>` : "",
    `<rect width="${s}" height="${i}" fill="${b}"/>`,
    ...f,
    "</svg>"
  ].join(`
`);
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(g)}`;
}
function ii(t, e) {
  const o = URL.createObjectURL(t), n = document.createElement("a");
  n.href = o, n.download = e, document.body.appendChild(n), n.click(), document.body.removeChild(n), URL.revokeObjectURL(o);
}
const ai = [
  { key: "iphone-se", label: "iPhone SE", w: 375, h: 667, group: "phone" },
  { key: "iphone-xr", label: "iPhone XR", w: 414, h: 896, group: "phone" },
  { key: "iphone-12-pro", label: "iPhone 12 Pro", w: 390, h: 844, group: "phone" },
  { key: "iphone-14-pro-max", label: "iPhone 14 Pro Max", w: 430, h: 932, group: "phone" },
  { key: "iphone-16-pro", label: "iPhone 16 Pro", w: 402, h: 874, group: "phone" },
  { key: "pixel-7", label: "Pixel 7", w: 412, h: 915, group: "phone" },
  { key: "galaxy-s8", label: "Galaxy S8+", w: 360, h: 740, group: "phone" },
  { key: "galaxy-s20-ultra", label: "Galaxy S20 Ultra", w: 412, h: 915, group: "phone" },
  { key: "galaxy-z-fold-5", label: "Galaxy Z Fold 5", w: 344, h: 882, group: "phone" },
  { key: "galaxy-a51", label: "Galaxy A51/71", w: 412, h: 914, group: "phone" }
], li = [
  { key: "ipad-mini", label: "iPad Mini", w: 768, h: 1024, group: "tablet" },
  { key: "ipad-air", label: "iPad Air", w: 820, h: 1180, group: "tablet" },
  { key: "ipad-pro", label: "iPad Pro", w: 1024, h: 1366, group: "tablet" },
  { key: "surface-pro-7", label: "Surface Pro 7", w: 912, h: 1368, group: "tablet" },
  { key: "surface-duo", label: "Surface Duo", w: 540, h: 720, group: "tablet" },
  { key: "zenbook-fold", label: "Asus Zenbook Fold", w: 853, h: 1280, group: "tablet" }
];
function ci(t, e) {
  return t.map((o) => ({
    key: `${o.key}-landscape`,
    label: `${o.label} ↔`,
    w: o.h,
    h: o.w,
    group: e
  }));
}
const Ca = [
  ...ai,
  ...ci(ai, "phone-landscape"),
  ...li,
  ...ci(li, "tablet-landscape"),
  // Other devices (already landscape-oriented)
  { key: "nest-hub", label: "Nest Hub", w: 1024, h: 600, group: "other" },
  { key: "nest-hub-max", label: "Nest Hub Max", w: 1280, h: 800, group: "other" },
  // Standard ratios
  { key: "16-9", label: "16:9", w: 1920, h: 1080, group: "standard" },
  { key: "9-16", label: "9:16", w: 1080, h: 1920, group: "standard" },
  { key: "4-3", label: "4:3", w: 1024, h: 768, group: "standard" },
  { key: "3-4", label: "3:4", w: 768, h: 1024, group: "standard" },
  { key: "1-1", label: "1:1", w: 1e3, h: 1e3, group: "standard" },
  { key: "21-9", label: "21:9", w: 2560, h: 1080, group: "standard" }
], Gh = new Map(Ca.map((t) => [t.key, t]));
function Wr(t) {
  return Gh.get(t);
}
function za(t) {
  return t.w / t.h;
}
const jh = {
  phone: "Phones",
  "phone-landscape": "Phones (Landscape)",
  tablet: "Tablets",
  "tablet-landscape": "Tablets (Landscape)",
  other: "Devices",
  standard: "Standard"
};
function Vh() {
  const t = /* @__PURE__ */ new Map();
  for (const e of Ca) {
    const o = t.get(e.group);
    o ? o.push(e) : t.set(e.group, [e]);
  }
  return Array.from(t.entries()).map(([e, o]) => ({
    label: jh[e] ?? e,
    presets: o
  }));
}
function Kh(t) {
  const e = t.replace("#", ""), o = parseInt(e.substring(0, 2), 16) || 0, n = parseInt(e.substring(2, 4), 16) || 0, r = parseInt(e.substring(4, 6), 16) || 0;
  return (0.299 * o + 0.587 * n + 0.114 * r) / 255 > 0.5 ? "#1e1e2e" : "#ffffff";
}
function fr(t, e, o) {
  let n = !1;
  for (let r = 0, s = o.length - 1; r < o.length; s = r++) {
    const [i, l] = o[r], [a, c] = o[s];
    l > e != c > e && t < (a - i) * (e - l) / (c - l) + i && (n = !n);
  }
  return n;
}
async function qh(t, e, o) {
  try {
    const n = await navigator.clipboard.read();
    let r = null;
    for (const i of n)
      if (i.types.includes("text/html")) {
        const l = await (await i.getType("text/html")).text();
        if (l.includes("sbd-clipboard") || l.includes("data-sbd-nodes=")) {
          const a = va(l);
          if (a) {
            t.setClipboard(a), t.pasteClipboard(e, o);
            return;
          }
          if (t.hasClipboard()) {
            t.pasteClipboard(e, o);
            return;
          }
        }
        r = l;
      }
    for (const i of n) {
      const l = i.types.find((a) => a.startsWith("image/"));
      if (l) {
        const a = await i.getType(l), c = await new Promise((x) => {
          const S = new FileReader();
          S.onload = () => x(S.result), S.readAsDataURL(a);
        }), h = new Image();
        await new Promise((x) => {
          h.onload = () => x(), h.src = c;
        });
        const p = h.naturalWidth / h.naturalHeight, d = Math.min(h.naturalWidth, 400), f = Math.min(h.naturalHeight, 300), m = p >= 1 ? d : f * p, y = p >= 1 ? d / p : f;
        let b = c;
        if (r) {
          const x = r.match(/<img[^>]+src=["']([^"']+)["']/i);
          x && /\.(gif|webp|apng)(\?|#|$)/i.test(x[1]) && (b = x[1].replace(/&amp;/g, "&"));
        }
        const g = {
          id: bt(10),
          type: "image",
          x: e,
          y: o,
          w: m,
          h: y,
          z: t.nextZ(),
          data: { src: b }
        };
        t.addNode(g), t.select(g.id);
        return;
      }
    }
    const s = await navigator.clipboard.readText();
    if (r) {
      const i = r.replace(/^<meta[^>]*>/i, "").replace(/<!--StartFragment-->|<!--EndFragment-->/g, "").trim();
      try {
        const l = Ei(i);
        if (l.length > 0) {
          const a = {
            id: bt(10),
            type: "content",
            x: e,
            y: o,
            w: 300,
            h: "auto",
            z: t.nextZ(),
            data: { blocks: l, markdown: s || "", borderColor: "#1e1e2e" }
          };
          t.addNode(a), t.select(a.id);
          return;
        }
      } catch {
      }
    }
    if (s != null && s.trim()) {
      const i = await Gr(s), l = {
        id: bt(10),
        type: "content",
        x: e,
        y: o,
        w: 300,
        h: "auto",
        z: t.nextZ(),
        data: { blocks: i, markdown: s, borderColor: "#1e1e2e" }
      };
      t.addNode(l), t.select(l.id);
      return;
    }
  } catch {
  }
  t.pasteClipboard(e, o);
}
async function di(t) {
  const e = t.getClipboardNodes();
  if (e.length === 0) return;
  const o = [];
  for (const l of e)
    if (l.type === "content") {
      const a = l.data;
      a.markdown && o.push(a.markdown);
    } else if (l.type === "text") {
      const a = l.data;
      a.text && o.push(a.text);
    } else if (l.type === "image") {
      const a = l.data;
      o.push(a.src.startsWith("http") ? a.src : a.alt || "[Image]");
    } else if (l.type === "shape") {
      const a = l.data;
      a.label && o.push(a.label);
    } else if (l.type === "sticky") {
      const a = l.data;
      a.text && o.push(a.text);
    } else if (l.type === "edge") {
      const a = l.data;
      a.label && o.push(a.label);
    }
  const n = o.join(`

`), r = n.split(`
`).filter(Boolean).map((l) => `<p>${l}</p>`).join(""), i = `<!--sbd-clipboard--><div data-sbd-nodes="${ka(e)}">${r || "<p></p>"}</div>`;
  try {
    await navigator.clipboard.write([
      new ClipboardItem({
        "text/plain": new Blob([n], { type: "text/plain" }),
        "text/html": new Blob([i], { type: "text/html" })
      })
    ]);
  } catch {
    try {
      await navigator.clipboard.writeText(n);
    } catch {
    }
  }
}
function Sn(t) {
  switch (t) {
    case "select":
      return "default";
    case "text":
      return "text";
    case "note":
      return "text";
    case "sticky":
      return "crosshair";
    case "draw":
      return "crosshair";
    case "shape":
      return "crosshair";
    case "edge":
      return "crosshair";
    case "frame":
      return "crosshair";
    case "erase":
      return `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='20' height='20'><circle cx='10' cy='10' r='9' fill='none' stroke='%239ca3af' stroke-width='1.5'/></svg>") ${20 / 2} ${20 / 2}, crosshair`;
    case "laser":
      return `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16'><circle cx='8' cy='8' r='3' fill='%23ef4444'/><circle cx='8' cy='8' r='7' fill='none' stroke='%23ef4444' stroke-width='1' opacity='0.4'/></svg>") ${16 / 2} ${16 / 2}, crosshair`;
    case "hand":
      return "grab";
    default:
      return "default";
  }
}
function hi(t, e) {
  const o = e.x - t.x, n = e.y - t.y;
  return { dist: Math.sqrt(o * o + n * n), mx: (t.x + e.x) / 2, my: (t.y + e.y) / 2 };
}
const Ro = `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' fill='none' stroke='%23e0e0e0' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'><path d='M12 3a7 7 0 0 1 4.5 12.4l-2 2'/><path d='M14.5 15.4q.5 1.5.5 2.6c0 2-1.5 3-3 3s-2-1-2-2c0-.8.5-1.5 1-2'/><circle cx='12' cy='3' r='1.5' fill='%23e0e0e0' stroke='none'/></svg>") 12 3, crosshair`;
function Zh({
  node: t,
  isInteractive: e,
  measuredH: o,
  onMeasuredHeight: n,
  observeElement: r,
  unobserveElement: s,
  isContainer: i,
  children: l
}) {
  const a = at(null);
  yt(() => {
    if (t.h !== "auto") return;
    const p = a.current;
    if (!p) return;
    const d = p.offsetHeight;
    return d > 0 && n(t.id, d), r(p, () => {
      const f = p.offsetHeight;
      f > 0 && n(t.id, f);
    }), () => s(p);
  }, [t.id, t.h, n, r, s]);
  const c = t.h === "auto" ? o ?? "auto" : t.h, h = Kt(() => ({
    position: "absolute",
    left: t.x,
    top: t.y,
    width: t.w,
    height: c,
    zIndex: t.z,
    pointerEvents: i ? "none" : e ? "auto" : "none",
    transform: t.rotation ? `rotate(${t.rotation}deg)` : void 0,
    transformOrigin: "center center"
  }), [t.x, t.y, t.w, c, t.z, t.rotation, i, e]);
  return /* @__PURE__ */ u(
    "div",
    {
      ref: a,
      "data-node-id": t.id,
      className: e ? void 0 : "sb-block-inert",
      style: h,
      children: l
    }
  );
}
function Uh({
  node: t,
  engine: e,
  onDone: o
}) {
  const n = at(null), r = at(t.data.label ?? ""), s = at(t);
  s.current = t;
  const i = at(t.data.label ?? "");
  yt(() => () => {
    const h = s.current, p = r.current.trim();
    if (p !== i.current) {
      const f = { data: { ...h.data, label: p || void 0 } }, m = n.current;
      if (m && p) {
        const b = h.h === "auto" ? 100 : h.h, g = m.scrollHeight + 24;
        g > b && (f.h = g);
      }
      e.updateNodeWithHistory(h.id, f);
    }
  }, []);
  const l = t.h === "auto" ? 100 : t.h, a = t.data.labelFontSize ?? 14, c = t.data.fill && t.data.fillStyle === "solid" ? Kh(t.data.fill) : t.data.stroke;
  return /* @__PURE__ */ u(
    "div",
    {
      "data-node-id": t.id,
      style: {
        position: "absolute",
        left: t.x,
        top: t.y,
        width: t.w,
        height: l,
        zIndex: 999998,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        pointerEvents: "auto",
        padding: "8px 12px",
        boxSizing: "border-box"
      },
      children: /* @__PURE__ */ u(
        "textarea",
        {
          ref: n,
          autoFocus: !0,
          defaultValue: t.data.label ?? "",
          placeholder: "",
          rows: 1,
          onBlur: () => o(),
          onKeyDown: (h) => {
            h.key === "Escape" && h.currentTarget.blur(), h.stopPropagation();
          },
          onInput: (h) => {
            const p = h.currentTarget;
            r.current = p.value;
            const d = s.current;
            e.updateNode(d.id, {
              data: { ...d.data, label: p.value || void 0 }
            }), p.style.height = "auto", p.style.height = p.scrollHeight + "px";
            const m = p.scrollHeight + 24;
            m > l && e.updateNode(t.id, { h: m });
          },
          onPointerDown: (h) => h.stopPropagation(),
          style: {
            textAlign: t.data.labelAlign ?? "center",
            fontSize: a,
            fontFamily: Je(t.data.labelFontFamily ?? Qe),
            color: c,
            background: "transparent",
            border: "none",
            outline: "none",
            resize: "none",
            overflow: "hidden",
            width: "100%",
            padding: 0,
            margin: 0,
            lineHeight: 1.3,
            wordBreak: "break-word",
            whiteSpace: "pre-wrap"
          }
        }
      )
    }
  );
}
function Qh({
  engine: t,
  schema: e,
  registry: o,
  dataFlow: n
}) {
  const r = at(null), s = () => {
    var w;
    return ((w = r.current) == null ? void 0 : w.ownerDocument) ?? document;
  }, [i, l] = et({ w: 0, h: 0 }), [a, c] = et({ ...t.viewport }), [h, p] = et(t.getAllNodes()), [d, f] = et(
    new Set(t.selection)
  ), [m, y] = et(t.mode), [b, g] = et(t.activeGroupId), [x, S] = et(t.snapToGrid), [M, z] = et(t.gridSize), [W, I] = et(t.smartGuides), [R, K] = et([]), [Z, lt] = et(t.boardBackground), O = at(!1), ot = at(!1), Q = at(/* @__PURE__ */ new Map()), $ = at(!1), P = at(!1), G = at(null), X = at(null), N = st((w) => {
    s().dispatchEvent(new CustomEvent("sb:canvas-interaction", { detail: { active: w } }));
  }, []);
  yt(() => {
    const w = (C) => {
      var V, T;
      if (C.key === " " && !C.repeat && !O.current) {
        const B = (V = C.target) == null ? void 0 : V.tagName;
        if (B === "INPUT" || B === "TEXTAREA" || (T = C.target) != null && T.isContentEditable) return;
        O.current = !0;
        const D = r.current;
        D && (D.style.cursor = "grab"), C.preventDefault();
      }
    }, E = (C) => {
      if (C.key === " ") {
        O.current = !1, ot.current = !1;
        const V = r.current;
        V && (V.style.cursor = t.lassoSelect ? Ro : Sn(t.mode));
      }
    };
    return window.addEventListener("keydown", w), window.addEventListener("keyup", E), () => {
      window.removeEventListener("keydown", w), window.removeEventListener("keyup", E);
    };
  }, []), yt(() => {
    const w = (C) => {
      Q.current.delete(C.pointerId), C.pointerType === "pen" && (P.current = !1), Q.current.size === 0 && N(!1), G.current && (clearTimeout(G.current), G.current = null, X.current = null);
    }, E = s();
    return E.addEventListener("pointerup", w), E.addEventListener("pointercancel", w), () => {
      E.removeEventListener("pointerup", w), E.removeEventListener("pointercancel", w);
    };
  }, [N]);
  const [j, H] = et(null), [_, tt] = et(null), [nt, ut] = et(null), [pt, vt] = et(null);
  yt(() => {
    const w = r.current;
    if (!w) return;
    t.setContainer(w);
    const E = () => {
      const V = w.getBoundingClientRect();
      t.containerOffset = { x: V.left, y: V.top };
    };
    E();
    const C = new ResizeObserver((V) => {
      var D;
      const { width: T, height: B } = ((D = V[0]) == null ? void 0 : D.contentRect) ?? { width: 0, height: 0 };
      l((A) => A.w === T && A.h === B ? A : { w: T, h: B }), t.setContainerSize(T, B), E();
    });
    return C.observe(w), () => C.disconnect();
  }, [t]);
  const [rt, gt] = et({}), xt = st((w, E) => {
    gt(
      (C) => C[w] === E ? C : { ...C, [w]: E }
    ), t.updateMeasuredHeight(w, E);
  }, [t]), ht = at(null), Dt = at(/* @__PURE__ */ new Map());
  function Tt() {
    return ht.current || (ht.current = new ResizeObserver((w) => {
      var E;
      for (const C of w)
        (E = Dt.current.get(C.target)) == null || E(C);
    })), ht.current;
  }
  const Rt = st((w, E) => {
    Dt.current.set(w, E), Tt().observe(w);
  }, []), Qt = st((w) => {
    var E;
    Dt.current.delete(w), (E = ht.current) == null || E.unobserve(w);
  }, []);
  yt(() => () => {
    var w;
    (w = ht.current) == null || w.disconnect(), ht.current = null, Dt.current.clear();
  }, []);
  const Ut = Kt(() => new Set(h.map((w) => w.id)), [h]);
  yt(() => {
    gt((w) => {
      let E = !1;
      const C = {};
      for (const [V, T] of Object.entries(w))
        Ut.has(V) ? C[V] = T : E = !0;
      return E ? C : w;
    });
  }, [Ut]);
  const re = st(
    (w, E, C) => {
      let V, T;
      if (o && w.data.sourcePort) {
        const B = o.get(E.type);
        B != null && B.ports && (V = rn(E, B.ports, w.data.sourcePort, a.zoom, rt) ?? void 0);
      }
      if (o && w.data.targetPort) {
        const B = o.get(C.type);
        B != null && B.ports && (T = rn(C, B.ports, w.data.targetPort, a.zoom, rt) ?? void 0);
      }
      return { sourcePortPos: V, targetPortPos: T };
    },
    [o, a.zoom, rt]
  );
  st(
    (w) => t.zoomToNode(w),
    [t]
  );
  const Jt = st(
    (w, E) => {
      if (!w.rotation)
        return { minX: w.x, minY: w.y, maxX: w.x + w.w, maxY: w.y + E };
      const C = w.x + w.w / 2, V = w.y + E / 2, T = w.rotation * Math.PI / 180, B = Math.cos(T), D = Math.sin(T), A = [
        [w.w / 2, E / 2],
        [-w.w / 2, E / 2],
        [-w.w / 2, -E / 2],
        [w.w / 2, -E / 2]
      ];
      let F = 1 / 0, Y = 1 / 0, L = -1 / 0, q = -1 / 0;
      for (const [U, J] of A) {
        const dt = C + U * B - J * D, ct = V + U * D + J * B;
        F = Math.min(F, dt), Y = Math.min(Y, ct), L = Math.max(L, dt), q = Math.max(q, ct);
      }
      return { minX: F, minY: Y, maxX: L, maxY: q };
    },
    []
  ), oe = 8, ve = st(
    (w, E) => E.filter((C) => {
      if (C.type === "edge") {
        const B = C.data, D = t.getNode(B.fromId), A = t.getNode(B.toId);
        if (!D || !A) return !1;
        const { x1: F, y1: Y, x2: L, y2: q } = er(D, A, rt);
        return F >= w.x && F <= w.x + w.w && Y >= w.y && Y <= w.y + w.h && L >= w.x && L <= w.x + w.w && q >= w.y && q <= w.y + w.h;
      }
      const V = C.h === "auto" ? rt[C.id] ?? 100 : C.h, T = Jt(C, V);
      return T.minX >= w.x && T.maxX <= w.x + w.w && T.minY >= w.y && T.maxY <= w.y + w.h;
    }),
    [Jt, rt]
  ), ye = st(
    (w, E) => w.length < 3 ? [] : E.filter((C) => {
      if (C.type === "edge") {
        const D = C, A = t.getNode(D.data.fromId), F = t.getNode(D.data.toId);
        if (!A || !F) return !1;
        const { x1: Y, y1: L, x2: q, y2: U } = er(A, F, rt);
        return fr(Y, L, w) && fr(q, U, w);
      }
      const V = C.h === "auto" ? rt[C.id] ?? 100 : C.h, T = C.x + C.w / 2, B = C.y + V / 2;
      return fr(T, B, w);
    }),
    [t, rt]
  ), se = Kt(() => {
    if (d.size < 2) return null;
    let w = 1 / 0, E = 1 / 0, C = -1 / 0, V = -1 / 0;
    for (const T of d) {
      const B = h.find((F) => F.id === T);
      if (!B || B.type === "edge") continue;
      const D = B.h === "auto" ? rt[B.id] ?? 100 : B.h, A = Jt(B, D);
      w = Math.min(w, A.minX), E = Math.min(E, A.minY), C = Math.max(C, A.maxX), V = Math.max(V, A.maxY);
    }
    return w === 1 / 0 ? null : {
      x: w - oe,
      y: E - oe,
      w: C - w + oe * 2,
      h: V - E + oe * 2
    };
  }, [d, h, rt, Jt]), Ae = Kt(() => {
    if (!b) return null;
    const w = t.getAllGroupDescendantNodes(b);
    if (w.length === 0) return null;
    let E = 1 / 0, C = 1 / 0, V = -1 / 0, T = -1 / 0;
    for (const D of w) {
      if (D.type === "edge") continue;
      const A = D.h === "auto" ? rt[D.id] ?? 100 : D.h, F = Jt(D, A);
      E = Math.min(E, F.minX), C = Math.min(C, F.minY), V = Math.max(V, F.maxX), T = Math.max(T, F.maxY);
    }
    if (E === 1 / 0) return null;
    const B = 8;
    return { x: E - B, y: C - B, w: V - E + B * 2, h: T - C + B * 2 };
  }, [b, h, rt, Jt, t]), Xe = Kt(() => {
    const w = h.filter(
      (Y) => {
        if (o) {
          const L = o.get(Y.type);
          return L && !L.isSVGOnly;
        }
        return Y.type === "content" || Y.type === "draw" || Y.type === "shape" || Y.type === "image" || Y.type === "text" || Y.type === "frame" || Y.type === "sticky";
      }
    ), E = a.zoom < 0.5 ? 15 : a.zoom < 1 ? 25 : 30;
    if (i.w <= 0 || i.h <= 0 || w.length < E)
      return null;
    const { zoom: C, x: V, y: T } = a, B = 500, D = {
      x: -V / C - B,
      y: -T / C - B,
      w: i.w / C + B * 2,
      h: i.h / C + B * 2
    }, A = t.getNodesInRect(D), F = /* @__PURE__ */ new Map();
    for (const Y of A) {
      const L = t.getNode(Y.id);
      L && F.set(Y.id, L);
    }
    for (const Y of d) {
      const L = t.getNode(Y);
      L && F.set(L.id, L);
    }
    for (const Y of h) {
      if (Y.type !== "edge" || F.has(Y.id)) continue;
      const L = Y.data, q = t.getNode(L.fromId), U = t.getNode(L.toId);
      if (!q || !U) continue;
      let J = F.has(L.fromId) || F.has(L.toId);
      if (!J) {
        const { x1: dt, y1: ct, x2: St, y2: zt } = er(q, U, rt);
        J = Ql(dt, ct, St, zt, D);
      }
      J && (F.set(Y.id, Y), F.has(q.id) || F.set(q.id, q), F.has(U.id) || F.set(U.id, U));
    }
    return Array.from(F.values());
  }, [a, i, h, d, t]), Le = Kt(() => Xe || h, [h, Xe]);
  yt(() => {
    let w = null;
    const E = () => {
      w === null && (w = requestAnimationFrame(() => {
        w = null, p([...t.getAllNodes()]);
      }));
    };
    let C = null;
    const V = () => {
      C === null && (C = requestAnimationFrame(() => {
        C = null, c({ ...t.viewport });
      }));
    }, T = () => {
      f((q) => {
        const U = new Set(t.selection);
        return q.size !== U.size || [...q].some((J) => !U.has(J)) ? (Fe((J) => J && !U.has(J) ? null : J), vo((J) => J && !U.has(J) ? null : J), io((J) => J && !U.has(J) ? null : J), So((J) => J && !U.has(J) ? null : J), Mo((J) => J && !U.has(J) ? null : J), v(null), U) : q;
      });
    }, B = () => {
      y(t.mode), t.mode === "text" && (Qo.current = !1);
    }, D = () => lt(t.boardBackground), A = () => {
      K([...t.alignGuides]), S(t.snapToGrid), z(t.gridSize), I(t.smartGuides);
    };
    t.on("change", E), t.on("viewport", V), t.on("selection", T), t.on("mode", B), t.on("background", D), t.on("guides", A);
    const F = (q) => g(q), Y = () => g(null), L = () => {
      const q = r.current;
      q && (q.style.cursor = t.lassoSelect ? Ro : Sn(t.mode));
    };
    return t.on("group:enter", F), t.on("group:exit", Y), t.on("lassoToggle", L), () => {
      w !== null && cancelAnimationFrame(w), C !== null && cancelAnimationFrame(C), t.off("change", E), t.off("viewport", V), t.off("selection", T), t.off("mode", B), t.off("background", D), t.off("guides", A), t.off("group:enter", F), t.off("group:exit", Y), t.off("lassoToggle", L);
    };
  }, [t]), yt(() => {
    const w = r.current;
    if (!w) return;
    const E = (C) => {
      if (!C.ctrlKey && !C.metaKey) {
        const T = C.target.closest(".sb-editor-wrap");
        if (T && T.scrollHeight > T.clientHeight) {
          const B = T.scrollTop <= 0 && C.deltaY < 0, D = T.scrollTop + T.clientHeight >= T.scrollHeight && C.deltaY > 0;
          if (!B && !D) return;
        }
      }
      C.preventDefault(), C.ctrlKey || C.metaKey ? t.zoomByWheel(C.deltaY, C.clientX, C.clientY) : t.pan(-C.deltaX, -C.deltaY);
    };
    return w.addEventListener("wheel", E, { passive: !1 }), () => w.removeEventListener("wheel", E);
  }, [t]);
  const [me, no] = et(null), [ro, Se] = et(null), [be, Me] = et(null), [Ee, v] = et(null), it = at({
    x: 0,
    y: 0,
    index: -1
  }), [Yt, _t] = et(null), [De, so] = et(null), We = at(null), wo = Kt(() => {
    const w = /* @__PURE__ */ new Set();
    for (const E of h) {
      if (E.type !== "edge") continue;
      const C = E;
      C.data.animated && C.data.animatedDirection === "bop" && (w.add(C.data.fromId), w.add(C.data.toId));
    }
    return w;
  }, [h]), [qo, Fe] = et(null), ko = at(null), [ys, vo] = et(null), [ms, io] = et(null), [Zo, So] = et(null), [bs, Mo] = et(null), [Ra, gs] = et(null);
  yt(() => {
    const w = (E) => {
      il(() => Mo(E));
    };
    return t.on("image:cropRequest", w), () => t.off("image:cropRequest", w);
  }, [t]);
  const xs = qo || ms || ys || Zo || bs || Ra, Kn = at(null), ws = at(null), [qn, Zn] = et(/* @__PURE__ */ new Set()), ao = at(/* @__PURE__ */ new Set()), [ks, Uo] = et([]), [un, Un] = et(null), Ce = at([]), Ye = at(null), [vs, pn] = et([]), le = at([]), Co = at(null), Qo = at(!1), Ss = st(
    (w, E, C, V = "auto") => {
      const T = bt(10);
      ws.current = T, t.addNode({
        id: T,
        type: "content",
        x: w,
        y: E,
        w: C,
        h: V,
        z: t.nextZ(),
        data: { blocks: [], borderColor: "#1e1e2e" }
      });
    },
    [t]
  ), fn = st(
    (w, E, C) => {
      const { x: V, y: T } = t.screenToCanvas(w, E);
      if (C) {
        const Y = t.hitTestAll(V, T, rt);
        if (Y.length > 0) {
          const L = it.current, q = Math.abs(V - L.x) + Math.abs(T - L.y);
          let U = 0;
          q < 5 && (U = (L.index + 1) % Y.length), it.current = { x: V, y: T, index: U }, t.select(Y[U].id);
        } else
          t.deselectAll();
      } else {
        let Y = !1;
        for (const L of t.selection) {
          const q = t.getNode(L);
          if (!q) continue;
          const U = q.h === "auto" ? 100 : q.h;
          if (V >= q.x && V <= q.x + q.w && T >= q.y && T <= q.y + U) {
            Y = !0;
            break;
          }
        }
        if (!Y && t.selection.size >= 2) {
          let L = 1 / 0, q = 1 / 0, U = -1 / 0, J = -1 / 0;
          for (const dt of t.selection) {
            const ct = t.getNode(dt);
            if (!ct || ct.type === "edge") continue;
            const St = ct.h === "auto" ? 100 : ct.h;
            L = Math.min(L, ct.x), q = Math.min(q, ct.y), U = Math.max(U, ct.x + ct.w), J = Math.max(J, ct.y + St);
          }
          L !== 1 / 0 && V >= L && V <= U && T >= q && T <= J && (Y = !0);
        }
        if (!Y) {
          const L = t.hitTest(V, T, rt);
          L ? t.select(L.id) : t.deselectAll();
        }
      }
      const B = Array.from(t.selection), D = B.length > 0, A = [];
      if (A.push({
        items: [
          {
            label: "Cut",
            shortcut: "Mod+X",
            disabled: !D,
            action: () => {
              t.cutSelected(), di(t);
            }
          },
          {
            label: "Copy",
            shortcut: "Mod+C",
            disabled: !D,
            action: () => {
              t.copySelected(), di(t);
            }
          },
          {
            label: "Paste",
            shortcut: "Mod+V",
            disabled: !1,
            action: () => {
              qh(t, V, T);
            }
          }
        ]
      }), A.push({
        items: [
          {
            label: "Duplicate",
            shortcut: "Mod+D",
            disabled: !D,
            action: () => t.duplicateSelected()
          }
        ]
      }), D && A.push({
        items: [
          {
            label: "Add to Personal Library",
            action: () => {
              const Y = B.map((U) => t.getNode(U)).filter((U) => !!U).map((U) => structuredClone(U)), L = new Set(
                Y.map((U) => U.groupId).filter(Boolean)
              ), q = /* @__PURE__ */ new Map();
              for (const [U, J] of t.groupParent)
                L.has(U) && q.set(U, J);
              Un({
                nodes: Y,
                groupParent: q
              });
            }
          }
        ]
      }), B.length >= 2 || D && t.selectionHasGroup()) {
        const Y = [];
        B.length >= 2 && Y.push({
          label: "Group selection",
          shortcut: "Mod+G",
          action: () => t.groupSelected()
        }), t.selectionHasGroup() && Y.push({
          label: "Ungroup selection",
          shortcut: "Mod+Shift+G",
          action: () => t.ungroupSelected()
        }), A.push({ items: Y });
      }
      if (D && B.every((L) => {
        const q = t.getNode(L);
        return q && (q.type === "draw" || q.type === "shape");
      }) && A.push({
        items: [
          {
            label: "Flip horizontal",
            shortcut: "Shift+H",
            action: () => t.flipSelectedHorizontal()
          },
          {
            label: "Flip vertical",
            shortcut: "Shift+V",
            action: () => t.flipSelectedVertical()
          }
        ]
      }), D && A.push({
        items: [
          {
            label: "Bring forward",
            shortcut: "Mod+]",
            action: () => t.bringForward(B)
          },
          {
            label: "Send backward",
            shortcut: "Mod+[",
            action: () => t.sendBackward(B)
          },
          {
            label: "Bring to front",
            shortcut: "Mod+Alt+]",
            action: () => t.bringToFront(B)
          },
          {
            label: "Send to back",
            shortcut: "Mod+Alt+[",
            action: () => t.sendToBack(B)
          }
        ]
      }), D) {
        const Y = B.some((U) => {
          var J;
          return (J = t.getNode(U)) == null ? void 0 : J.locked;
        }), L = B.some((U) => {
          var J;
          return !((J = t.getNode(U)) != null && J.locked);
        }), q = [];
        L && q.push({
          label: "Lock",
          action: () => {
            for (const U of B) t.updateNode(U, { locked: !0 });
          }
        }), Y && q.push({
          label: "Unlock",
          action: () => {
            for (const U of B) t.updateNode(U, { locked: void 0 });
          }
        }), A.push({ items: q });
      }
      D && A.push({
        items: [
          {
            label: "Delete",
            shortcut: "Delete",
            danger: !0,
            action: () => t.deleteSelected()
          }
        ]
      });
      const F = [10, 20, 40, 80];
      return A.push({
        items: [
          {
            label: "Toggle Grid",
            checked: t.snapToGrid,
            action: () => {
              t.toggleSnapToGrid(), S(t.snapToGrid);
            }
          },
          {
            label: "Smart Guides",
            checked: t.smartGuides,
            action: () => {
              t.toggleSmartGuides(), I(t.smartGuides);
            }
          },
          ...F.map((Y) => ({
            label: `${Y}px`,
            checked: t.gridSize === Y,
            action: () => {
              t.setGridSize(Y);
            }
          }))
        ]
      }), A.push({
        items: [
          {
            label: "Export as PNG",
            action: () => si(t, { format: "png" })
          },
          {
            label: "Export as SVG",
            action: () => si(t, { format: "svg" })
          }
        ]
      }), A;
    },
    [t]
  ), Aa = st(
    (w) => {
      if (w.preventDefault(), t.presentationMode) return;
      const E = fn(w.clientX, w.clientY, w.altKey);
      Me({ x: w.clientX, y: w.clientY, sections: E });
    },
    [t, fn]
  ), Jo = st(
    (w, E, C) => {
      const V = bt(10);
      t.addNode({
        id: V,
        type: "text",
        x: w,
        y: E,
        w: C,
        h: "auto",
        z: t.nextZ(),
        data: {
          text: "",
          fontSize: t.activeTool.fontSize ?? 20,
          fontFamily: t.activeTool.fontFamily ?? Qe,
          color: t.activeTool.color,
          align: t.activeTool.textAlign ?? "left",
          opacity: t.activeTool.opacity
        }
      }), t.select(V), Kn.current = V, Fe(V);
    },
    [t]
  ), La = st(
    (w) => {
      if (t.presentationMode) return;
      if (t.mode === "text" && Qo.current) {
        Qo.current = !1, r.current && (r.current.style.cursor = "text"), t.deselectAll();
        const { x: B, y: D } = t.screenToCanvas(w.clientX, w.clientY);
        Jo(B, D, 300);
        return;
      }
      if (t.mode !== "select") return;
      const { x: E, y: C } = t.screenToCanvas(w.clientX, w.clientY), V = t.hitTestAll(E, C, rt), T = V.find((B) => !t.isContainerType(B.type)) ?? V[0] ?? null;
      if (T != null && T.groupId) {
        const B = [];
        let D = T.groupId;
        for (; D; )
          B.push(D), D = t.groupParent.get(D);
        if (!t.activeGroupId) {
          t.enterGroup(B[B.length - 1]), t.select(T.id);
          return;
        }
        const A = B.indexOf(t.activeGroupId);
        if (A > 0) {
          t.enterGroup(B[A - 1]), t.select(T.id);
          return;
        }
      }
      if (T && T.type === "text") {
        t.select(T.id), ko.current = { clientX: w.clientX, clientY: w.clientY }, Fe(T.id);
        return;
      }
      if (T && T.type === "sticky") {
        t.select(T.id), io(T.id);
        return;
      }
      if (T && T.type === "frame") {
        t.select(T.id), vo(T.id);
        return;
      }
      if (T && T.type === "shape") {
        const B = T.data, D = B.shape === "line" || B.shape === "arrow";
        t.select(T.id), D || So(T.id);
        return;
      }
      if (T && T.type === "draw") {
        t.select(T.id);
        return;
      }
      if (!T || T.type === "draw") {
        const D = t.getAllNodes().filter((A) => A.type === "shape").sort((A, F) => F.z - A.z).find((A) => !(A.data.shape === "line" || A.data.shape === "arrow") && Hn(A, E, C, t.viewport.zoom, !0));
        if (D) {
          t.select(D.id), So(D.id);
          return;
        }
      }
      T || (t.deselectAll(), Jo(E, C, 300));
    },
    [t, rt, Jo]
  ), Ea = st(
    (w) => {
      if (Q.current.set(w.pointerId, { x: w.clientX, y: w.clientY }), w.pointerType === "pen" && (P.current = !0), w.button !== 2 && N(!0), w.pointerType === "touch" && (Q.current.size >= 2 || P.current)) {
        $.current = !0, G.current && (clearTimeout(G.current), G.current = null, X.current = null);
        const T = new Map(Q.current), B = [...Q.current.keys()].find((L) => L !== w.pointerId);
        B !== void 0 && s().dispatchEvent(
          new PointerEvent("pointerup", {
            pointerId: B,
            bubbles: !0,
            clientX: w.clientX,
            clientY: w.clientY
          })
        );
        const D = [...T.values()];
        let A = hi(D[0], D[1] ?? D[0]);
        const F = (L) => {
          if (!T.has(L.pointerId)) return;
          T.set(L.pointerId, { x: L.clientX, y: L.clientY });
          const q = [...T.values()];
          if (q.length < 2) return;
          const U = hi(q[0], q[1]);
          if (t.pan(U.mx - A.mx, U.my - A.my), A.dist > 1) {
            const J = Math.min(Math.max(U.dist / A.dist, 0.9), 1.1);
            t.zoomByFactor(J, U.mx, U.my);
          }
          A = U;
        }, Y = (L) => {
          Q.current.delete(L.pointerId), T.delete(L.pointerId), L.pointerType === "pen" && (P.current = !1), T.size < 2 && !P.current && ($.current = !1, s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", Y), s().removeEventListener("pointercancel", Y));
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", Y), s().addEventListener("pointercancel", Y);
        return;
      }
      if ($.current || t.presentationMode && !(w.button === 1 || w.button === 0 && O.current))
        return;
      if (be && Me(null), w.pointerType === "touch") {
        const T = w.clientX, B = w.clientY, D = w.pointerId;
        X.current = { clientX: T, clientY: B }, G.current = setTimeout(() => {
          if (G.current = null, !X.current || $.current) return;
          const A = fn(T, B, !1);
          Me({ x: T, y: B, sections: A }), s().dispatchEvent(
            new PointerEvent("pointerup", {
              pointerId: D,
              bubbles: !0,
              clientX: T,
              clientY: B
            })
          ), X.current = null;
        }, 500);
      }
      if (w.button === 1 || w.button === 0 && O.current) {
        w.preventDefault(), ot.current = !0;
        const T = t.viewport.x, B = t.viewport.y, D = w.clientX, A = w.clientY, F = r.current;
        F && (F.style.cursor = "grabbing");
        const Y = (q) => {
          t.viewport.x = T + (q.clientX - D), t.viewport.y = B + (q.clientY - A), c({ ...t.viewport });
        }, L = () => {
          ot.current = !1, F && (F.style.cursor = O.current ? "grab" : t.lassoSelect ? Ro : ""), s().removeEventListener("pointermove", Y), s().removeEventListener("pointerup", L);
        };
        s().addEventListener("pointermove", Y), s().addEventListener("pointerup", L);
        return;
      }
      const { x: C, y: V } = t.screenToCanvas(w.clientX, w.clientY);
      if (w.pointerType === "touch" && G.current && t.hitTest(C, V, rt) && (clearTimeout(G.current), G.current = null, X.current = null), t.mode === "select") {
        if (w.button !== 0) return;
        if (w.altKey) {
          const D = t.hitTestAll(C, V, rt);
          if (D.length > 0) {
            const A = it.current, F = Math.abs(C - A.x) + Math.abs(V - A.y);
            let Y = 0;
            F < 5 && (Y = (A.index + 1) % D.length), it.current = { x: C, y: V, index: Y }, t.select(D[Y].id);
          }
          return;
        }
        let T = !1;
        !t.lassoSelect && t.selection.size >= 2 && se && C >= se.x && C <= se.x + se.w && V >= se.y && V <= se.y + se.h && (T = !0);
        let B = null;
        if (!t.lassoSelect) {
          const D = t.hitTestAll(C, V, rt);
          B = D.find((A) => t.selection.has(A.id) && !t.isContainerType(A.type)) ?? D.find((A) => !t.isContainerType(A.type)) ?? D[0] ?? null, !B && !T && (B = _l(t.nodes, C, V, t.viewport.zoom, rt, re));
        }
        if (B || T) {
          B && (t.activeGroupId && !t.isNodeInActiveGroup(B.id) && t.exitAllGroups(), w.shiftKey ? t.toggleSelect(B.id) : t.selection.has(B.id) || t.select(B.id));
          const D = Array.from(t.selection).filter(
            (Mt) => {
              var It;
              return !((It = t.getNode(Mt)) != null && It.locked);
            }
          );
          if (D.length === 0) return;
          const A = w.clientX, F = w.clientY, Y = /* @__PURE__ */ new Set(), L = /* @__PURE__ */ new Set();
          for (const Mt of D) {
            const It = t.getNode(Mt);
            if (It && t.isContainerType(It.type)) {
              L.add(Mt);
              for (const Ht of t.getFrameDescendantIds(Mt))
                t.selection.has(Ht) || Y.add(Ht);
            }
          }
          const q = [...D, ...Y], U = q.map((Mt) => {
            const It = t.getNode(Mt);
            return { id: Mt, x: It.x, y: It.y };
          }), J = t.selectionGroupId(), dt = J ? t.groupRotations.get(J) : null, ct = dt == null ? void 0 : dt.cx, St = dt == null ? void 0 : dt.cy;
          v(null);
          let zt = !1, kt = null, Pt = A, Xt = F, Ot = !1;
          const mt = new Set(q), Wt = () => {
            kt = null;
            const Mt = (Pt - A) / t.viewport.zoom, It = (Xt - F) / t.viewport.zoom, { finalDx: Ht, finalDy: jt } = t.computeDragSnap(
              U,
              mt,
              Mt,
              It,
              Ot
            ), ee = U.map((ne) => ({
              id: ne.id,
              patch: { x: ne.x + Ht, y: ne.y + jt }
            }));
            t.updateMany(ee), dt && J && t.groupRotations.set(J, {
              angle: dt.angle,
              cx: ct + Ht,
              cy: St + jt
            });
          }, wt = (Mt) => {
            const It = (Mt.clientX - A) / t.viewport.zoom, Ht = (Mt.clientY - F) / t.viewport.zoom;
            if (!zt)
              if (Math.abs(It) > 2 || Math.abs(Ht) > 2)
                zt = !0, t.pushHistorySnapshot();
              else
                return;
            Pt = Mt.clientX, Xt = Mt.clientY, Ot = Mt.metaKey || Mt.ctrlKey, kt === null && (kt = requestAnimationFrame(Wt));
          }, Gt = () => {
            if (kt !== null && (cancelAnimationFrame(kt), Wt()), t.clearAlignGuides(), s().removeEventListener("pointermove", wt), s().removeEventListener("pointerup", Gt), zt) {
              const Mt = D.filter(
                (It) => !Y.has(It)
              );
              Mt.length > 0 && t.updateFrameMembership(Mt);
            }
          };
          s().addEventListener("pointermove", wt), s().addEventListener("pointerup", Gt);
        } else {
          if (t.activeGroupId) {
            t.exitGroup();
            return;
          }
          w.shiftKey || t.deselectAll();
          const D = new Set(t.selection);
          if (t.lassoSelect) {
            const A = [[C, V]];
            Se([...A]);
            let F = null, Y = 0;
            const L = (J = !1) => {
              F = null;
              const dt = J || Y % 2 === 0;
              if (Y++, dt && A.length >= 3) {
                const St = ye(A, t.getAllNodes()).map((kt) => kt.id), zt = w.shiftKey ? [.../* @__PURE__ */ new Set([...D, ...St])] : St;
                (zt.length !== t.selection.size || zt.some((kt) => !t.selection.has(kt))) && t.selectMultiple(zt);
              }
              Se([...A]);
            }, q = (J) => {
              const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY);
              A.push([dt, ct]), F === null && (F = requestAnimationFrame(() => L(!1)));
            }, U = () => {
              F !== null && cancelAnimationFrame(F), L(!0), s().removeEventListener("pointermove", q), s().removeEventListener("pointerup", U), Se(null), t.toggleLassoSelect();
            };
            s().addEventListener("pointermove", q), s().addEventListener("pointerup", U);
          } else {
            const A = { startX: C, startY: V, endX: C, endY: V };
            no(A);
            let F = null, Y = 0;
            const L = (J = !1, dt = !1) => {
              F = null;
              const ct = Math.min(A.startX, A.endX), St = Math.min(A.startY, A.endY), zt = Math.abs(A.endX - A.startX), kt = Math.abs(A.endY - A.startY), Pt = dt || J || Y % 2 === 0;
              if (Y++, Pt) {
                const Ot = ve(
                  { x: ct, y: St, w: zt, h: kt },
                  t.getAllNodes()
                ).map((Wt) => Wt.id), mt = w.shiftKey ? [.../* @__PURE__ */ new Set([...D, ...Ot])] : Ot;
                (mt.length !== t.selection.size || mt.some((Wt) => !t.selection.has(Wt))) && t.selectMultiple(mt);
              }
              no({ ...A });
            }, q = (J) => {
              const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY);
              A.endX = dt, A.endY = ct, F === null && (F = requestAnimationFrame(() => L(!1)));
            }, U = () => {
              F !== null && cancelAnimationFrame(F), L(!0), s().removeEventListener("pointermove", q), s().removeEventListener("pointerup", U), no(null);
            };
            s().addEventListener("pointermove", q), s().addEventListener("pointerup", U);
          }
        }
      } else if (t.mode === "text") {
        if (Qo.current) return;
        t.deselectAll();
        const T = C, B = V, D = {
          startX: C,
          startY: V,
          endX: C,
          endY: V
        };
        let A = !1;
        _t(D);
        const F = (L) => {
          const { x: q, y: U } = t.screenToCanvas(L.clientX, L.clientY);
          D.endX = q, D.endY = U;
          const J = Math.abs(D.endX - D.startX), dt = Math.abs(D.endY - D.startY);
          (J > 10 || dt > 10) && (A = !0), _t({ ...D });
        }, Y = () => {
          s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", Y), _t(null);
          const L = A ? Math.max(Math.abs(D.endX - D.startX), 60) : 300, q = A ? Math.min(D.startX, D.endX) : T, U = A ? Math.min(D.startY, D.endY) : B;
          Jo(q, U, L), Qo.current = !0, r.current && (r.current.style.cursor = "crosshair");
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", Y);
      } else if (t.mode === "note") {
        t.deselectAll();
        const T = C, B = V, D = {
          startX: C,
          startY: V,
          endX: C,
          endY: V
        };
        let A = !1;
        _t(D);
        const F = (L) => {
          const { x: q, y: U } = t.screenToCanvas(L.clientX, L.clientY);
          D.endX = q, D.endY = U;
          const J = Math.abs(D.endX - D.startX), dt = Math.abs(D.endY - D.startY);
          (J > 10 || dt > 10) && (A = !0), _t({ ...D });
        }, Y = () => {
          s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", Y), _t(null);
          const L = A ? Math.max(Math.abs(D.endX - D.startX), 100) : 300, q = A ? Math.max(Math.abs(D.endY - D.startY), 40) : "auto", U = A ? Math.min(D.startX, D.endX) : T, J = A ? Math.min(D.startY, D.endY) : B;
          Ss(U, J, L, q), t.setMode("select");
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", Y);
      } else if (t.mode === "sticky") {
        t.deselectAll();
        const T = C, B = V, D = {
          startX: C,
          startY: V,
          endX: C,
          endY: V
        };
        let A = !1;
        _t(D);
        const F = (L) => {
          const { x: q, y: U } = t.screenToCanvas(L.clientX, L.clientY);
          D.endX = q, D.endY = U, Math.abs(D.endX - D.startX) > 10 && (A = !0), _t({ ...D });
        }, Y = () => {
          s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", Y), _t(null);
          const L = A ? Math.max(Math.abs(D.endX - D.startX), 100) : 200, q = A ? Math.min(D.startX, D.endX) : T, U = A ? Math.min(D.startY, D.endY) : B, J = bt(10), dt = A ? Math.max(Math.abs(D.endY - D.startY), 100) : 150;
          t.addNode({
            id: J,
            type: "sticky",
            x: q,
            y: U,
            w: L,
            h: dt,
            z: t.nextZ(),
            data: { text: "", color: "#FEF3C7" }
          }), t.select(J), io(J), t.setMode("select");
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", Y);
      } else if (t.mode === "draw") {
        const T = w.pressure || 0.5, B = {
          points: [[C, V, T]],
          color: t.activeTool.color,
          width: t.activeTool.width,
          strokeStyle: t.activeTool.strokeStyle
        };
        H(B), t.notifyDrawProgress(B);
        const D = (F) => {
          const { x: Y, y: L } = t.screenToCanvas(F.clientX, F.clientY), q = F.pressure || 0.5;
          B.points.push([Y, L, q]), H({ ...B, points: [...B.points] }), t.notifyDrawProgress({ ...B, points: [...B.points] });
        }, A = () => {
          if (s().removeEventListener("pointermove", D), s().removeEventListener("pointerup", A), t.notifyDrawEnd(), B.points.length < 2) {
            H(null);
            return;
          }
          let F = 1 / 0, Y = 1 / 0, L = -1 / 0, q = -1 / 0;
          for (const [J, dt] of B.points)
            J < F && (F = J), dt < Y && (Y = dt), J > L && (L = J), dt > q && (q = dt);
          const U = B.points.map(
            ([J, dt, ct]) => [J - F, dt - Y, ct]
          );
          t.addNode({
            id: bt(10),
            type: "draw",
            x: F,
            y: Y,
            w: L - F,
            h: q - Y,
            z: t.nextZ(),
            data: {
              tool: "pen",
              points: U,
              color: B.color,
              strokeWidth: B.width,
              opacity: t.activeTool.opacity,
              fill: t.activeTool.fillColor || void 0,
              fillStyle: t.activeTool.fillStyle || void 0,
              strokeStyle: t.activeTool.strokeStyle || void 0
            }
          }), requestAnimationFrame(() => H(null));
        };
        s().addEventListener("pointermove", D), s().addEventListener("pointerup", A);
      } else if (t.mode === "shape") {
        const T = {
          startX: C,
          startY: V,
          endX: C,
          endY: V
        };
        tt(T);
        const B = {
          shapeType: t.activeTool.shapeType || "rect",
          stroke: t.activeTool.color,
          strokeWidth: t.activeTool.width
        }, D = (F) => {
          const { x: Y, y: L } = t.screenToCanvas(F.clientX, F.clientY);
          T.endX = Y, T.endY = L, tt({ ...T }), t.notifyShapeProgress({ ...T, ...B });
        }, A = () => {
          s().removeEventListener("pointermove", D), s().removeEventListener("pointerup", A), t.notifyShapeEnd();
          const F = t.activeTool.shapeType || "rect", Y = F === "line" || F === "arrow", L = Math.min(T.startX, T.endX);
          let q = Math.min(T.startY, T.endY);
          const U = Math.abs(T.endX - T.startX), J = Math.abs(T.endY - T.startY);
          let dt;
          if (Y) {
            const zt = t.activeTool.width * 2;
            dt = Math.max(J, zt), J < zt && (q -= (zt - J) / 2);
          } else
            dt = J;
          if (U < 5 && (Y ? U < 5 && Math.abs(T.endY - T.startY) < 5 : dt < 5)) {
            tt(null);
            return;
          }
          const ct = {};
          Y && (ct.startPoint = [
            T.startX - L,
            T.startY - q
          ], ct.endPoint = [
            T.endX - L,
            T.endY - q
          ]);
          const St = bt(10);
          t.addNode({
            id: St,
            type: "shape",
            x: L,
            y: q,
            w: U,
            h: dt,
            z: t.nextZ(),
            data: {
              shape: F,
              stroke: t.activeTool.color,
              fill: t.activeTool.fillColor || void 0,
              fillStyle: t.activeTool.fillStyle,
              strokeWidth: t.activeTool.width,
              strokeStyle: t.activeTool.strokeStyle,
              roughness: t.activeTool.roughness ?? 1,
              opacity: t.activeTool.opacity ?? 1,
              ...ct
            }
          }), tt(null), t.setMode("select"), t.select(St);
        };
        s().addEventListener("pointermove", D), s().addEventListener("pointerup", A);
      } else if (t.mode === "edge") {
        const T = t.hitTest(C, V, rt);
        if (!T || T.type === "edge") return;
        ut({ fromNode: T, cursorX: C, cursorY: V });
        const B = (A) => {
          const { x: F, y: Y } = t.screenToCanvas(A.clientX, A.clientY);
          ut(
            (L) => L ? { ...L, cursorX: F, cursorY: Y } : null
          );
        }, D = (A) => {
          s().removeEventListener("pointermove", B), s().removeEventListener("pointerup", D), ut(null);
          const { x: F, y: Y } = t.screenToCanvas(A.clientX, A.clientY);
          let L = t.hitTest(F, Y, rt);
          if (!L || L.type === "edge" || t.isContainerType(L.type)) {
            const ct = 50 / t.viewport.zoom;
            let St = 1 / 0, zt = !1, kt = null;
            for (const Pt of t.getAllNodes()) {
              if (Pt.type === "edge" || Pt.id === T.id) continue;
              const Xt = t.isContainerType(Pt.type), Ot = Ho(Pt, rt);
              for (const mt of Ot) {
                const Wt = Math.hypot(mt.x - F, mt.y - Y);
                Wt >= ct || Xt && !zt && kt || (!Xt && zt || Wt < St) && (St = Wt, zt = Xt, kt = Pt);
              }
            }
            kt && (L = kt);
          }
          if (!L || L.type === "edge" || L.id === T.id || t.getAllNodes().some(
            (ct) => ct.type === "edge" && (ct.data.fromId === T.id && ct.data.toId === L.id || ct.data.fromId === L.id && ct.data.toId === T.id)
          )) return;
          const U = bn(T, C, V, rt), J = bn(L, F, Y, rt), dt = {
            id: bt(10),
            type: "edge",
            x: 0,
            y: 0,
            w: 0,
            h: 0,
            z: t.nextZ(),
            data: {
              fromId: T.id,
              toId: L.id,
              style: "solid",
              color: t.activeTool.color,
              strokeWidth: 2,
              arrowHead: "arrow",
              arrowTail: "none",
              edgeType: "bezier",
              sourceHandle: U,
              targetHandle: J
            }
          };
          t.addNode(dt), t.select(dt.id);
        };
        s().addEventListener("pointermove", B), s().addEventListener("pointerup", D);
      } else if (t.mode === "frame") {
        const T = {
          startX: C,
          startY: V,
          endX: C,
          endY: V
        };
        tt(T);
        const B = (A) => {
          const { x: F, y: Y } = t.screenToCanvas(A.clientX, A.clientY);
          T.endX = F, T.endY = Y, tt({ ...T });
        }, D = () => {
          s().removeEventListener("pointermove", B), s().removeEventListener("pointerup", D);
          const A = Math.min(T.startX, T.endX), F = Math.min(T.startY, T.endY), Y = Math.abs(T.endX - T.startX), L = Math.abs(T.endY - T.startY);
          if (Y < 20 || L < 20) {
            tt(null);
            return;
          }
          const q = bt(10);
          t.addNode({
            id: q,
            type: "frame",
            x: A,
            y: F,
            w: Y,
            h: L,
            z: t.nextZ(),
            data: {
              label: "Frame",
              backgroundColor: "rgba(0, 0, 0, 0.02)",
              borderColor: "#1e1e2e",
              borderWidth: 1,
              borderStyle: "dashed"
            }
          }), t.adoptNodesIntoNewFrame(q), tt(null), t.select(q), t.setMode("select");
        };
        s().addEventListener("pointermove", B), s().addEventListener("pointerup", D);
      } else if (t.mode === "erase") {
        if (w.button !== 0) return;
        const T = (ct, St) => {
          const zt = t.hitTestAll(ct, St, rt), kt = $l(
            t.nodes,
            ct,
            St,
            t.viewport.zoom,
            rt,
            re
          );
          let Pt = !1;
          for (const Xt of [...zt, ...kt])
            ao.current.has(Xt.id) || (ao.current.add(Xt.id), Pt = !0);
          Pt && Zn(new Set(ao.current));
        }, B = 400;
        ao.current = /* @__PURE__ */ new Set();
        const D = performance.now();
        Ce.current = [[C, V, D]], Uo([[C, V, D]]), T(C, V);
        let A = C, F = V;
        const Y = () => {
          const ct = performance.now(), St = Ce.current.length;
          Ce.current = Ce.current.filter(
            (zt) => ct - zt[2] < B
          ), Ce.current.length !== St && Uo([...Ce.current]), Ye.current = requestAnimationFrame(Y);
        };
        Ye.current = requestAnimationFrame(Y);
        const L = (ct) => {
          const { x: St, y: zt } = t.screenToCanvas(ct.clientX, ct.clientY);
          A = St, F = zt;
          const kt = performance.now();
          Ce.current.push([A, F, kt]), Uo([...Ce.current]), T(A, F);
        }, q = () => {
          Ye.current !== null && (cancelAnimationFrame(Ye.current), Ye.current = null), ao.current = /* @__PURE__ */ new Set(), Zn(/* @__PURE__ */ new Set()), Ce.current = [], Uo([]);
        }, U = () => {
          dt();
          const ct = Array.from(ao.current);
          q(), ct.length > 0 && t.deleteNodes(ct);
        }, J = (ct) => {
          ct.key === "Escape" && (dt(), q());
        }, dt = () => {
          s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", U), s().removeEventListener("keydown", J);
        };
        s().addEventListener("pointermove", L), s().addEventListener("pointerup", U), s().addEventListener("keydown", J);
      } else if (t.mode === "laser") {
        if (w.button !== 0) return;
        const T = 1560;
        Co.current !== null && (cancelAnimationFrame(Co.current), Co.current = null);
        const B = performance.now();
        le.current.length > 0 && le.current.push([NaN, NaN, B]), le.current.push([C, V, B]), pn([...le.current]), t.notifyLaserProgress([[C, V]]);
        let D = B;
        const A = () => {
          const L = performance.now(), q = le.current.length;
          le.current = le.current.filter(
            (U) => L - U[2] < T
          ), (le.current.length !== q || le.current.length > 0) && pn([...le.current]), L - D >= 60 && (D = L, le.current.length > 0 && t.notifyLaserProgress(
            le.current.map((U) => [U[0], U[1]])
          )), le.current.length > 0 ? Co.current = requestAnimationFrame(A) : (Co.current = null, pn([]), t.notifyLaserEnd());
        };
        Co.current = requestAnimationFrame(A);
        const F = (L) => {
          const { x: q, y: U } = t.screenToCanvas(L.clientX, L.clientY), J = performance.now();
          le.current.push([q, U, J]), pn([...le.current]), t.notifyLaserProgress(
            le.current.map((dt) => [dt[0], dt[1]])
          );
        }, Y = () => {
          s().removeEventListener("pointermove", F), s().removeEventListener("pointerup", Y);
        };
        s().addEventListener("pointermove", F), s().addEventListener("pointerup", Y);
      } else if (t.mode === "hand") {
        if (w.button !== 0) return;
        w.preventDefault();
        const T = t.viewport.x, B = t.viewport.y, D = w.clientX, A = w.clientY, F = r.current;
        F && (F.style.cursor = "grabbing");
        const Y = (q) => {
          t.viewport.x = T + (q.clientX - D), t.viewport.y = B + (q.clientY - A), c({ ...t.viewport });
        }, L = () => {
          F && (F.style.cursor = t.lassoSelect ? Ro : Sn(t.mode)), s().removeEventListener("pointermove", Y), s().removeEventListener("pointerup", L);
        };
        s().addEventListener("pointermove", Y), s().addEventListener("pointerup", L);
      }
    },
    [
      t,
      Ss,
      Jo,
      be,
      fn,
      se,
      rt,
      Jt,
      ve,
      N
    ]
  ), Qn = st(
    (w, E, C) => {
      if (C.preventDefault(), t.presentationMode) return;
      const V = t.getNode(w);
      if (!V || V.locked) return;
      const T = C.clientX, B = C.clientY, D = V.x, A = V.y, F = V.w, Y = V.h === "auto", L = Y ? rt[w] ?? 100 : V.h, q = V.type === "draw" ? V.data.points.map(
        (zt) => [...zt]
      ) : null, U = V.type === "shape" ? V.data.startPoint : void 0, J = V.type === "shape" ? V.data.endPoint : void 0, dt = V.type === "text" ? V.data.fontSize : 0;
      t.pushHistorySnapshot();
      const ct = (zt) => {
        const kt = (zt.clientX - T) / t.viewport.zoom, Pt = (zt.clientY - B) / t.viewport.zoom;
        let Xt = D, Ot = A, mt = F, Wt = L;
        if ((E === "nw" || E === "w" || E === "sw") && (Xt = D + kt, mt = F - kt), (E === "ne" || E === "e" || E === "se") && (mt = F + kt), (E === "nw" || E === "n" || E === "ne") && (Ot = A + Pt, Wt = L - Pt), (E === "sw" || E === "s" || E === "se") && (Wt = L + Pt), t.snapToGrid && !(zt.metaKey || zt.ctrlKey)) {
          const Mt = t.gridSize, It = (Ht) => Math.round(Ht / Mt) * Mt;
          (E === "nw" || E === "w" || E === "sw") && (Xt = It(Xt), mt = D + F - Xt), (E === "ne" || E === "e" || E === "se") && (mt = It(Xt + mt) - Xt), (E === "nw" || E === "n" || E === "ne") && (Ot = It(Ot), Wt = A + L - Ot), (E === "sw" || E === "s" || E === "se") && (Wt = It(Ot + Wt) - Ot);
        }
        const wt = 10;
        if (mt < wt && (mt = wt, (E === "nw" || E === "w" || E === "sw") && (Xt = D + F - wt)), Wt < wt && (Wt = wt, (E === "nw" || E === "n" || E === "ne") && (Ot = A + L - wt)), V.type === "frame") {
          const Mt = V.data.devicePreset;
          if (Mt) {
            const It = Wr(Mt);
            if (It) {
              const Ht = za(It);
              if (E === "nw" || E === "ne" || E === "sw" || E === "se" || (E === "e" || E === "w")) {
                const ne = Math.round(mt / Ht);
                (E === "nw" || E === "ne") && (Ot = A + L - ne), Wt = ne;
              } else
                mt = Math.round(Wt * Ht);
            }
          }
        }
        const Gt = {
          x: Xt,
          y: Ot,
          w: mt,
          h: Y ? "auto" : Wt
        };
        if (q && V.type === "draw") {
          const Mt = F > 0 ? mt / F : 1, It = L > 0 ? Wt / L : 1, Ht = q.map(
            ([jt, ee, ne]) => [jt * Mt, ee * It, ne]
          );
          Gt.data = { ...V.data, points: Ht };
        }
        if (V.type === "shape" && (U || J)) {
          const Mt = F > 0 ? mt / F : 1, It = L > 0 ? Wt / L : 1, Ht = { ...V.data };
          U && (Ht.startPoint = [
            U[0] * Mt,
            U[1] * It
          ]), J && (Ht.endPoint = [
            J[0] * Mt,
            J[1] * It
          ]), Gt.data = Ht;
        }
        if (V.type === "text" && dt > 0 && E !== "e" && E !== "w") {
          const Mt = E === "n" || E === "s" ? L > 0 ? Wt / L : 1 : F > 0 ? mt / F : 1, It = Math.max(8, Math.round(dt * Mt));
          Gt.data = { ...V.data, fontSize: It };
        }
        t.updateNode(w, Gt);
      }, St = () => {
        s().removeEventListener("pointermove", ct), s().removeEventListener("pointerup", St), t.isContainerType(V.type) && t.syncFrameChildrenAfterResize(w);
      };
      s().addEventListener("pointermove", ct), s().addEventListener("pointerup", St);
    },
    [t, rt]
  ), Da = st(
    (w, E) => {
      E.stopPropagation(), E.preventDefault();
      const C = t.getNode(w);
      if (!C || C.locked) return;
      const V = C.h === "auto" ? rt[w] ?? 100 : C.h, T = C.x + C.w / 2, B = C.y + V / 2, D = C.rotation || 0, { x: A, y: F } = t.screenToCanvas(
        E.clientX,
        E.clientY
      ), Y = Math.atan2(F - B, A - T);
      t.pushHistorySnapshot();
      const L = (U) => {
        const { x: J, y: dt } = t.screenToCanvas(U.clientX, U.clientY), ct = Math.atan2(dt - B, J - T);
        let St = D + (ct - Y) * (180 / Math.PI);
        (U.shiftKey || t.snapToGrid) && !(U.metaKey || U.ctrlKey) && (St = Math.round(St / 15) * 15), t.updateNode(w, { rotation: St });
      }, q = () => {
        s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", q);
      };
      s().addEventListener("pointermove", L), s().addEventListener("pointerup", q);
    },
    [t, rt]
  ), Ms = st(
    (w, E, C) => {
      C.stopPropagation(), C.preventDefault();
      const V = t.getNode(w);
      if (!V) return;
      const { x: T, y: B } = t.screenToCanvas(C.clientX, C.clientY);
      ut({ fromNode: V, cursorX: T, cursorY: B, sourceHandle: E });
      const D = (F) => {
        const { x: Y, y: L } = t.screenToCanvas(F.clientX, F.clientY);
        ut(
          (q) => q ? { ...q, cursorX: Y, cursorY: L } : null
        );
      }, A = (F) => {
        s().removeEventListener("pointermove", D), s().removeEventListener("pointerup", A), ut(null);
        const { x: Y, y: L } = t.screenToCanvas(F.clientX, F.clientY);
        let q = t.hitTest(Y, L, rt);
        if (!q || q.type === "edge" || t.isContainerType(q.type)) {
          const ct = 50 / t.viewport.zoom;
          let St = 1 / 0, zt = !1, kt = null;
          for (const Pt of t.getAllNodes()) {
            if (Pt.type === "edge" || Pt.id === V.id) continue;
            const Xt = t.isContainerType(Pt.type), Ot = Ho(Pt, rt);
            for (const mt of Ot) {
              const Wt = Math.hypot(mt.x - Y, mt.y - L);
              Wt >= ct || Xt && !zt && kt || (!Xt && zt || Wt < St) && (St = Wt, zt = Xt, kt = Pt);
            }
          }
          kt && (q = kt);
        }
        if (!q || q.type === "edge" || q.id === V.id || t.getAllNodes().some(
          (ct) => ct.type === "edge" && (ct.data.fromId === V.id && ct.data.toId === q.id || ct.data.fromId === q.id && ct.data.toId === V.id)
        )) return;
        const J = bn(q, Y, L, rt), dt = {
          id: bt(10),
          type: "edge",
          x: 0,
          y: 0,
          w: 0,
          h: 0,
          z: t.nextZ(),
          data: {
            fromId: V.id,
            toId: q.id,
            style: "solid",
            color: t.activeTool.color,
            strokeWidth: 2,
            arrowHead: "arrow",
            arrowTail: "none",
            edgeType: "bezier",
            sourceHandle: E,
            targetHandle: J
          }
        };
        t.addNode(dt), t.select(dt.id);
      };
      s().addEventListener("pointermove", D), s().addEventListener("pointerup", A);
    },
    [t, rt]
  ), Wa = st(
    (w) => {
      let E = null, C = w === "top" || w === "left" ? 1 / 0 : -1 / 0;
      for (const V of t.selection) {
        const T = t.getNode(V);
        if (!T || T.type === "edge") continue;
        const B = T.h === "auto" ? rt[T.id] ?? 100 : T.h;
        let D;
        switch (w) {
          case "top":
            D = T.y;
            break;
          case "bottom":
            D = T.y + B;
            break;
          case "left":
            D = T.x;
            break;
          case "right":
            D = T.x + T.w;
            break;
        }
        (w === "top" || w === "left" ? D < C : D > C) && (C = D, E = V);
      }
      return E;
    },
    [t, rt]
  ), Fa = st(
    (w, E, C, V) => {
      var U;
      V.stopPropagation(), V.preventDefault();
      const T = t.getNode(w);
      if (!T || !o) return;
      const B = o.get(T.type), D = (U = B == null ? void 0 : B.ports) == null ? void 0 : U.find((J) => J.id === E);
      if (!D) return;
      const A = C === "input" ? "left" : "right", { x: F, y: Y } = t.screenToCanvas(V.clientX, V.clientY);
      ut({
        fromNode: T,
        cursorX: F,
        cursorY: Y,
        sourceHandle: A,
        sourcePort: E,
        sourceDirection: C
      });
      const L = (J) => {
        const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY);
        ut(
          (St) => St ? { ...St, cursorX: dt, cursorY: ct } : null
        );
      }, q = (J) => {
        var Be;
        s().removeEventListener("pointermove", L), s().removeEventListener("pointerup", q), ut(null);
        const { x: dt, y: ct } = t.screenToCanvas(J.clientX, J.clientY), St = C === "output" ? "input" : "output", zt = 40 / t.viewport.zoom;
        let kt = null, Pt = null, Xt = 1 / 0;
        for (const te of t.getAllNodes()) {
          if (te.type === "edge" || te.id === T.id) continue;
          const ie = o.get(te.type);
          if (!((Be = ie == null ? void 0 : ie.ports) != null && Be.length)) continue;
          const zo = te.h === "auto" ? t.measuredHeights[te.id] ?? 100 : te.h;
          for (const ge of ie.ports) {
            if (ge.direction !== St || D.dataType !== "any" && ge.dataType !== "any" && D.dataType !== ge.dataType) continue;
            const Io = ie.ports.filter((Za) => Za.direction === ge.direction), $n = Io.indexOf(ge), yn = 14 / t.viewport.zoom, Ka = te.y + zo / (Io.length + 1) * ($n + 1), qa = ge.direction === "input" ? te.x - yn : te.x + te.w + yn, _n = Math.hypot(qa - dt, Ka - ct);
            _n < zt && _n < Xt && (Xt = _n, kt = te, Pt = ge);
          }
        }
        if (!kt || !Pt) return;
        const Ot = Pt.id, mt = C === "output" ? kt.id : T.id, Wt = C === "output" ? Ot : E;
        if (t.getAllNodes().some((te) => {
          if (te.type !== "edge") return !1;
          const ie = te.data;
          return ie.toId === mt && ie.targetPort === Wt;
        })) return;
        const Gt = C === "output" ? T.id : kt.id, Mt = C === "output" ? kt.id : T.id, It = C === "output" ? E : Ot, Ht = C === "output" ? Ot : E, ne = {
          id: bt(10),
          type: "edge",
          x: 0,
          y: 0,
          w: 0,
          h: 0,
          z: t.nextZ(),
          data: {
            fromId: Gt,
            toId: Mt,
            style: "solid",
            color: "#6b7280",
            strokeWidth: 2,
            arrowHead: "filled",
            arrowTail: "none",
            edgeType: "bezier",
            sourceHandle: "right",
            targetHandle: "left",
            sourcePort: It,
            targetPort: Ht
          }
        };
        t.addNode(ne), t.select(ne.id);
      };
      s().addEventListener("pointermove", L), s().addEventListener("pointerup", q);
    },
    [t, o, rt]
  ), [Cs, Ba] = et(0);
  yt(() => {
    if (n)
      return n.onChange(() => Ba((w) => w + 1));
  }, [n]);
  const Na = st(
    (w, E, C, V, T) => {
      T.stopPropagation(), T.preventDefault();
      const B = t.getNode(w);
      if (!B || B.type !== "edge") return;
      t.pushHistorySnapshot();
      const D = (F) => {
        const Y = t.screenToCanvas(F.clientX, F.clientY), L = t.getNode(w);
        if (!L) return;
        const q = t.getNode(L.data.fromId), U = t.getNode(L.data.toId);
        if (!(!q || !U))
          if (E === "xy") {
            const J = Ze(
              q,
              U,
              L.data.edgeType || "bezier",
              rt,
              L.data.sourceHandle,
              L.data.targetHandle,
              void 0,
              void 0
              // no offsets → natural midpoint
            );
            if (!J.kinkHandle) return;
            const dt = Y.x - J.kinkHandle.x, ct = Y.y - J.kinkHandle.y;
            t.updateNode(w, {
              data: { ...L.data, curveOffset: [dt, ct] }
            });
          } else {
            const J = E === "x" ? Y.x : Y.y, dt = Ze(
              q,
              U,
              L.data.edgeType || "bezier",
              rt,
              L.data.sourceHandle,
              L.data.targetHandle,
              0.5
              // default to get range
            );
            if (!dt.kinkHandle) return;
            const ct = dt.kinkHandle.min, St = dt.kinkHandle.max, zt = St - ct;
            if (zt === 0) return;
            const Pt = (Math.max(ct, Math.min(St, J)) - ct) / zt;
            t.updateNode(w, {
              data: { ...L.data, midpointOffset: Pt }
            });
          }
      }, A = () => {
        s().removeEventListener("pointermove", D), s().removeEventListener("pointerup", A);
      };
      s().addEventListener("pointermove", D), s().addEventListener("pointerup", A);
    },
    [t, rt]
  ), Ha = st(
    (w, E, C) => {
      C.stopPropagation(), C.preventDefault();
      const V = t.getNode(w);
      if (!V || V.type !== "edge") return;
      const { fromId: T, toId: B, sourceHandle: D, targetHandle: A } = V.data, F = E === "source" ? B : T, Y = E === "source" ? A : D, L = t.getNode(T), q = t.getNode(B);
      if (!L || !q) return;
      const U = Ze(
        L,
        q,
        V.data.edgeType || "bezier",
        rt,
        D,
        A
      ), J = E === "source" ? { x: U.x1, y: U.y1 } : { x: U.x2, y: U.y2 };
      vt({
        edgeId: w,
        endpoint: E,
        anchorNodeId: F,
        anchorHandle: Y,
        cursorX: J.x,
        cursorY: J.y
      });
      const dt = (St) => {
        const { x: zt, y: kt } = t.screenToCanvas(St.clientX, St.clientY);
        vt(
          (Pt) => Pt ? { ...Pt, cursorX: zt, cursorY: kt } : null
        );
      }, ct = (St) => {
        s().removeEventListener("pointermove", dt), s().removeEventListener("pointerup", ct), vt(null);
        const { x: zt, y: kt } = t.screenToCanvas(St.clientX, St.clientY);
        let Pt = t.hitTest(zt, kt, rt);
        if (!Pt || Pt.type === "edge" || t.isContainerType(Pt.type)) {
          const Mt = 50 / t.viewport.zoom;
          let It = 1 / 0, Ht = !1, jt = null;
          for (const ee of t.getAllNodes()) {
            if (ee.type === "edge") continue;
            const ne = t.isContainerType(ee.type), Be = Ho(ee, rt);
            for (const te of Be) {
              const ie = Math.hypot(te.x - zt, te.y - kt);
              ie >= Mt || ne && !Ht && jt || (!ne && Ht || ie < It) && (It = ie, Ht = ne, jt = ee);
            }
          }
          jt && (Pt = jt);
        }
        if (!Pt || Pt.type === "edge") return;
        const Xt = E === "source" ? Pt.id : T, Ot = E === "target" ? Pt.id : B;
        if (Xt === Ot) return;
        const mt = E === "source" ? T : B;
        if (Pt.id === mt || t.getAllNodes().some((Mt) => {
          if (Mt.type !== "edge" || Mt.id === w) return !1;
          const It = Mt.data;
          return It.fromId === Xt && It.toId === Ot || It.fromId === Ot && It.toId === Xt;
        })) return;
        const wt = bn(Pt, zt, kt, rt), Gt = E === "source" ? { fromId: Pt.id, sourceHandle: wt } : { toId: Pt.id, targetHandle: wt };
        t.updateNodeWithHistory(w, { data: Gt });
      };
      s().addEventListener("pointermove", dt), s().addEventListener("pointerup", ct);
    },
    [t, rt]
  ), Oa = st(
    (w) => {
      if (w.stopPropagation(), w.preventDefault(), t.presentationMode) return;
      const E = Array.from(t.selection).map((wt) => t.getNode(wt)).filter(Boolean);
      if (E.length < 2) return;
      const V = t.selectionIsSingleGroup() ? t.selectionGroupId() ?? null : null, T = V ? t.groupRotations.get(V) : null;
      let B, D;
      if (T)
        B = T.cx, D = T.cy;
      else {
        let wt = 1 / 0, Gt = 1 / 0, Mt = -1 / 0, It = -1 / 0;
        for (const Ht of E) {
          const jt = Ht.h === "auto" ? rt[Ht.id] ?? 100 : Ht.h, ee = Jt(Ht, jt);
          wt = Math.min(wt, ee.minX), Gt = Math.min(Gt, ee.minY), Mt = Math.max(Mt, ee.maxX), It = Math.max(It, ee.maxY);
        }
        B = (wt + Mt) / 2, D = (Gt + It) / 2;
      }
      const A = (T == null ? void 0 : T.angle) ?? 0, Y = E.filter((wt) => !wt.locked).map((wt) => {
        const Gt = wt.h === "auto" ? rt[wt.id] ?? 100 : wt.h;
        return {
          id: wt.id,
          cx: wt.x + wt.w / 2,
          cy: wt.y + Gt / 2,
          w: wt.w,
          h: Gt,
          rotation: wt.rotation || 0
        };
      }), L = -A * Math.PI / 180, q = Math.cos(L), U = Math.sin(L);
      let J = 1 / 0, dt = 1 / 0, ct = -1 / 0, St = -1 / 0;
      for (const wt of Y) {
        const Gt = wt.cx - B, Mt = wt.cy - D, It = B + Gt * q - Mt * U, Ht = D + Gt * U + Mt * q;
        J = Math.min(J, It - wt.w / 2), dt = Math.min(dt, Ht - wt.h / 2), ct = Math.max(ct, It + wt.w / 2), St = Math.max(St, Ht + wt.h / 2);
      }
      const zt = {
        x: J - oe,
        y: dt - oe,
        w: ct - J + oe * 2,
        h: St - dt + oe * 2
      }, { x: kt, y: Pt } = t.screenToCanvas(w.clientX, w.clientY), Xt = Math.atan2(Pt - D, kt - B);
      t.pushHistorySnapshot();
      let Ot = A;
      const mt = (wt) => {
        const { x: Gt, y: Mt } = t.screenToCanvas(wt.clientX, wt.clientY);
        let Ht = (Math.atan2(Mt - D, Gt - B) - Xt) * (180 / Math.PI);
        (wt.shiftKey || t.snapToGrid) && !(wt.metaKey || wt.ctrlKey) && (Ht = Math.round(Ht / 15) * 15), Ot = A + Ht, v({ angle: Ot, cx: B, cy: D, bounds: zt });
        const jt = Ht * Math.PI / 180, ee = Math.cos(jt), ne = Math.sin(jt), Be = Y.map((te) => {
          const ie = te.cx - B, zo = te.cy - D, ge = B + ie * ee - zo * ne, Io = D + ie * ne + zo * ee;
          return {
            id: te.id,
            patch: {
              x: ge - te.w / 2,
              y: Io - te.h / 2,
              rotation: Ot
            }
          };
        });
        t.updateMany(Be);
      }, Wt = () => {
        V && t.groupRotations.set(V, { angle: Ot, cx: B, cy: D }), v({ angle: Ot, cx: B, cy: D, bounds: zt }), s().removeEventListener("pointermove", mt), s().removeEventListener("pointerup", Wt);
      };
      s().addEventListener("pointermove", mt), s().addEventListener("pointerup", Wt);
    },
    [t, rt, Jt]
  ), Xa = st(
    (w, E) => {
      if (E.stopPropagation(), E.preventDefault(), t.presentationMode) return;
      const C = Array.from(t.selection).map((mt) => t.getNode(mt)).filter(Boolean);
      if (C.length < 2) return;
      const V = (mt) => mt.h === "auto" ? rt[mt.id] ?? 100 : mt.h;
      let T = 1 / 0, B = 1 / 0, D = -1 / 0, A = -1 / 0;
      for (const mt of C) {
        const Wt = V(mt), wt = Jt(mt, Wt);
        T = Math.min(T, wt.minX), B = Math.min(B, wt.minY), D = Math.max(D, wt.maxX), A = Math.max(A, wt.maxY);
      }
      const F = { x: T, y: B, w: D - T, h: A - B }, Y = F.w || 1, L = F.h || 1, U = C.filter((mt) => !mt.locked).map((mt) => {
        const Wt = V(mt);
        return {
          id: mt.id,
          type: mt.type,
          isAutoH: mt.h === "auto",
          relX: (mt.x - F.x) / Y,
          relY: (mt.y - F.y) / L,
          relW: mt.w / Y,
          relH: Wt / L,
          origW: mt.w,
          origH: Wt,
          origPoints: mt.type === "draw" ? mt.data.points.map((wt) => [...wt]) : null,
          drawData: mt.type === "draw" ? { ...mt.data } : null
        };
      }), J = E.clientX, dt = E.clientY;
      t.pushHistorySnapshot();
      let ct = null, St = J, zt = dt, kt = !1;
      const Pt = () => {
        ct = null;
        const mt = (St - J) / t.viewport.zoom, Wt = (zt - dt) / t.viewport.zoom;
        let wt = F.x, Gt = F.y, Mt = F.w, It = F.h;
        if ((w === "nw" || w === "w" || w === "sw") && (wt = F.x + mt, Mt = F.w - mt), (w === "ne" || w === "e" || w === "se") && (Mt = F.w + mt), (w === "nw" || w === "n" || w === "ne") && (Gt = F.y + Wt, It = F.h - Wt), (w === "sw" || w === "s" || w === "se") && (It = F.h + Wt), t.snapToGrid && !kt) {
          const jt = t.gridSize, ee = (ne) => Math.round(ne / jt) * jt;
          (w === "nw" || w === "w" || w === "sw") && (wt = ee(wt), Mt = F.x + F.w - wt), (w === "ne" || w === "e" || w === "se") && (Mt = ee(wt + Mt) - wt), (w === "nw" || w === "n" || w === "ne") && (Gt = ee(Gt), It = F.y + F.h - Gt), (w === "sw" || w === "s" || w === "se") && (It = ee(Gt + It) - Gt);
        }
        Mt < 20 && (Mt = 20, (w === "nw" || w === "w" || w === "sw") && (wt = F.x + F.w - 20)), It < 20 && (It = 20, (w === "nw" || w === "n" || w === "ne") && (Gt = F.y + F.h - 20));
        const Ht = U.map((jt) => {
          const ee = wt + jt.relX * Mt, ne = Gt + jt.relY * It, Be = jt.relW * Mt, te = jt.relH * It, ie = {
            x: ee,
            y: ne,
            w: Be,
            h: jt.isAutoH ? "auto" : te
          };
          if (jt.origPoints && jt.drawData) {
            const zo = jt.origW > 0 ? Be / jt.origW : 1, ge = jt.origH > 0 ? te / jt.origH : 1;
            ie.data = {
              ...jt.drawData,
              points: jt.origPoints.map(
                ([Io, $n, yn]) => [Io * zo, $n * ge, yn]
              )
            };
          }
          return { id: jt.id, patch: ie };
        });
        t.updateMany(Ht);
      }, Xt = (mt) => {
        St = mt.clientX, zt = mt.clientY, kt = mt.metaKey || mt.ctrlKey, ct === null && (ct = requestAnimationFrame(Pt));
      }, Ot = () => {
        ct !== null && (cancelAnimationFrame(ct), Pt()), s().removeEventListener("pointermove", Xt), s().removeEventListener("pointerup", Ot);
        for (const mt of C)
          t.isContainerType(mt.type) && t.syncFrameChildrenAfterResize(mt.id);
      };
      s().addEventListener("pointermove", Xt), s().addEventListener("pointerup", Ot);
    },
    [t, rt, Jt]
  );
  yt(() => {
    r.current && (r.current.style.cursor = t.lassoSelect ? Ro : Sn(m)), m !== "select" && m !== "edge" && (We.current = null, so(null)), m !== "erase" && (Ye.current !== null && (cancelAnimationFrame(Ye.current), Ye.current = null), ao.current = /* @__PURE__ */ new Set(), Zn(/* @__PURE__ */ new Set()), Ce.current = [], Uo([]));
  }, [m]);
  const Jn = at(null), zs = at(null), Ya = st(
    (w) => {
      if (G.current && w.pointerType === "touch" && X.current) {
        const E = w.clientX - X.current.clientX, C = w.clientY - X.current.clientY;
        Math.sqrt(E * E + C * C) > 8 && (clearTimeout(G.current), G.current = null, X.current = null);
      }
      t.mode !== "select" && t.mode !== "edge" || (zs.current = { clientX: w.clientX, clientY: w.clientY }, Jn.current === null && (Jn.current = requestAnimationFrame(() => {
        Jn.current = null;
        const E = r.current, C = zs.current;
        if (!E || !C) return;
        const { x: V, y: T } = t.screenToCanvas(C.clientX, C.clientY);
        if (t.lassoSelect) {
          E.style.cursor = Ro;
          return;
        }
        if (t.mode === "edge") {
          const A = t.hitTest(V, T, rt), F = A && A.type !== "edge" ? A.id : null;
          F !== We.current && (We.current = F, so(F));
          return;
        }
        if (t.selection.size >= 2 && se && V >= se.x && V <= se.x + se.w && T >= se.y && T <= se.y + se.h) {
          E.style.cursor = "move";
          return;
        }
        const B = t.hitTest(V, T, rt), D = B ? B.id : null;
        if (D !== We.current && (We.current = D, so(D)), B) {
          E.style.cursor = "move";
          return;
        }
        E.style.cursor = "default";
      })));
    },
    [t, se, rt, Jt]
  ), Ga = st((w) => {
    (w.dataTransfer.types.includes("Files") || w.dataTransfer.types.includes(Ar) || w.dataTransfer.types.includes(Lr) || w.dataTransfer.types.includes(Er)) && (w.preventDefault(), w.dataTransfer.dropEffect = "copy");
  }, []), ja = st(
    (w) => {
      if (w.preventDefault(), t.presentationMode) return;
      const E = w.dataTransfer.getData(Er);
      if (E) {
        try {
          const F = JSON.parse(E);
          ga(t, F, w.clientX, w.clientY);
        } catch (F) {
          console.error("Failed to place GIF:", F);
        }
        return;
      }
      const C = w.dataTransfer.getData(Lr);
      if (C) {
        try {
          const { itemId: F } = JSON.parse(C), L = pa().find((q) => q.id === F);
          L && ya(t, L, w.clientX, w.clientY);
        } catch (F) {
          console.error("Failed to place personal library item:", F);
        }
        return;
      }
      const V = w.dataTransfer.getData(Ar);
      if (V) {
        try {
          const { libraryId: F, itemId: Y } = JSON.parse(V), q = ns(F).find((U) => U.id === Y);
          q && fa(t, q, w.clientX, w.clientY);
        } catch (F) {
          console.error("Failed to place library item:", F);
        }
        return;
      }
      const T = w.dataTransfer.files[0];
      if (!T) return;
      if (T.name.endsWith(".excalidrawlib") || T.name.endsWith(".excalidrawlib.json")) {
        const F = new FileReader();
        F.onload = () => {
          try {
            const Y = JSON.parse(F.result);
            if (Y.type === "excalidrawlib") {
              const L = T.name.replace(/\.excalidrawlib(\.json)?$/, "");
              rs(Y, { name: L });
            }
          } catch (Y) {
            console.error("Failed to import library:", Y);
          }
        }, F.readAsText(T);
        return;
      }
      if (T.type === "image/svg+xml" || T.name.endsWith(".svg")) {
        const F = new FileReader();
        F.onload = () => {
          const Y = F.result, L = Dr(Y);
          L && mh(t, L, w.clientX, w.clientY);
        }, F.readAsText(T);
        return;
      }
      if (!T.type.startsWith("image/")) return;
      const { x: B, y: D } = t.screenToCanvas(w.clientX, w.clientY), A = new FileReader();
      A.onload = () => {
        const F = A.result, Y = new Image();
        Y.onload = () => {
          const L = Math.min(Y.naturalWidth, 400), q = Math.min(Y.naturalHeight, 300), U = Y.naturalWidth / Y.naturalHeight, J = U >= 1 ? L : q * U, dt = U >= 1 ? L / U : q;
          t.addNode({
            id: bt(10),
            type: "image",
            x: B,
            y: D,
            w: J,
            h: dt,
            z: t.nextZ(),
            data: { src: F }
          });
        }, Y.src = F;
      }, A.readAsDataURL(T);
    },
    [t]
  ), Va = `translate(${a.x}px, ${a.y}px) scale(${a.zoom})`;
  return /* @__PURE__ */ k(
    "div",
    {
      ref: r,
      "data-sb-canvas": !0,
      style: {
        width: "100%",
        height: "100%",
        overflow: "hidden",
        position: "relative",
        userSelect: "none",
        touchAction: "none",
        background: ln(Z).canvasBg
      },
      onPointerDown: Ea,
      onPointerMove: Ya,
      onDoubleClick: La,
      onContextMenu: Aa,
      onDragOver: Ga,
      onDrop: ja,
      children: [
        /* @__PURE__ */ u(Yd, { viewport: a, gridSize: M, background: Z, gridVisible: x }),
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              transform: Va,
              transformOrigin: "0 0",
              pointerEvents: "none"
            },
            children: [
              (Xe || h).filter((w) => {
                if (o) {
                  const E = o.get(w.type);
                  return E && !E.isSVGOnly;
                }
                return w.type === "content" || w.type === "draw" || w.type === "shape" || w.type === "image" || w.type === "text" || w.type === "frame" || w.type === "sticky";
              }).sort((w, E) => w.z - E.z).map((w) => {
                var D;
                const E = qn.has(w.id), C = wo.has(w.id), T = -(w.id.split("").reduce((A, F) => A + F.charCodeAt(0), 0) % 240 / 100);
                let B;
                if (o) {
                  const A = o.get(w.type);
                  if (A) {
                    const F = A.component, Y = d.has(w.id), L = m === "select" || m === "text" || m === "note" || m === "sticky", q = /* @__PURE__ */ u(
                      F,
                      {
                        node: w,
                        data: w.data,
                        isSelected: Y,
                        multiSelected: d.size > 1 && Y && !t.selectionIsSingleGroup(),
                        engine: t,
                        interactive: L,
                        zoom: a.zoom,
                        editing: xs === w.id,
                        editClickPos: xs === w.id ? ko.current : null,
                        callbacks: {
                          onMeasuredHeight: xt,
                          onResizeHandleDown: Qn,
                          onEditStart: (U) => {
                            const J = t.getNode(U);
                            J && (J.type === "text" ? Fe(U) : J.type === "sticky" ? io(U) : J.type === "frame" ? vo(U) : J.type === "shape" ? So(U) : J.type === "image" ? Mo(U) : J.type === "youtube" && gs(U));
                          },
                          onEditEnd: () => {
                            Fe(null), io(null), vo(null), So(null), Mo(null), gs(null);
                          }
                        },
                        portValues: n && ((D = A.ports) != null && D.length) && Cs >= 0 ? n.getAllPortValues(w.id) : void 0,
                        updateData: (U) => {
                          t.updateNodeWithHistory(w.id, {
                            data: { ...w.data, ...U }
                          });
                        }
                      },
                      A.handlesOwnLayout ? w.id : void 0
                    );
                    A.handlesOwnLayout ? B = q : B = /* @__PURE__ */ u(
                      Zh,
                      {
                        node: w,
                        isInteractive: L,
                        measuredH: rt[w.id],
                        onMeasuredHeight: xt,
                        observeElement: Rt,
                        unobserveElement: Qt,
                        isContainer: A.isContainer,
                        children: q
                      },
                      w.id
                    );
                  }
                } else if (w.type === "content") {
                  const A = w;
                  B = /* @__PURE__ */ u(
                    Bi,
                    {
                      node: A,
                      isSelected: d.has(w.id),
                      multiSelected: d.size > 1 && d.has(w.id) && !t.selectionIsSingleGroup(),
                      engine: t,
                      schema: e,
                      interactive: m === "select" || m === "text" || m === "note",
                      zoom: a.zoom,
                      onMeasuredHeight: xt,
                      autoEdit: ws.current === A.id
                    },
                    w.id
                  );
                } else if (w.type === "text")
                  B = /* @__PURE__ */ u(
                    Qi,
                    {
                      node: w,
                      engine: t,
                      editing: qo === w.id,
                      editClickPos: qo === w.id ? ko.current : null,
                      onStopEdit: () => {
                        if (Kn.current === w.id) {
                          Kn.current = null;
                          const A = t.getNode(w.id);
                          if (!A || !A.data.text.trim()) {
                            t.deleteNode(w.id), Fe(null);
                            return;
                          }
                          t.pushHistorySnapshot();
                        }
                        Fe(null);
                      },
                      onMeasuredHeight: xt
                    },
                    w.id
                  );
                else if (w.type === "image")
                  B = /* @__PURE__ */ u(
                    Ui,
                    {
                      node: w,
                      isSelected: d.has(w.id),
                      engine: t,
                      interactive: m === "select",
                      zoom: a.zoom,
                      onResizeHandleDown: Qn,
                      cropping: bs === w.id,
                      onCropStart: () => Mo(w.id),
                      onCropEnd: () => Mo(null)
                    },
                    w.id
                  );
                else if (w.type === "sticky")
                  B = /* @__PURE__ */ u(
                    Ji,
                    {
                      node: w,
                      isSelected: d.has(w.id),
                      engine: t,
                      interactive: m === "select" || m === "sticky",
                      zoom: a.zoom,
                      editing: ms === w.id,
                      onEditStart: io,
                      onEditEnd: () => io(null)
                    },
                    w.id
                  );
                else if (w.type === "frame") {
                  const A = w, F = A.h === "auto" ? 100 : A.h;
                  B = /* @__PURE__ */ u(
                    "div",
                    {
                      style: {
                        position: "absolute",
                        left: A.x,
                        top: A.y,
                        width: A.w,
                        height: F,
                        zIndex: A.z,
                        background: A.data.backgroundColor || "rgba(0,0,0,0.02)",
                        border: `${A.data.borderWidth || 1}px ${A.data.borderStyle || "dashed"} ${A.data.borderColor || "#ccc"}`,
                        boxSizing: "border-box",
                        borderRadius: 8,
                        opacity: A.data.opacity ?? 1,
                        pointerEvents: "none",
                        overflow: "visible",
                        transform: A.rotation ? `rotate(${A.rotation}deg)` : void 0,
                        transformOrigin: "center center"
                      },
                      children: ys === w.id ? /* @__PURE__ */ u(
                        "input",
                        {
                          autoFocus: !0,
                          defaultValue: A.data.label ?? "",
                          placeholder: "Frame label...",
                          onBlur: (Y) => {
                            const L = Y.currentTarget.value.trim();
                            t.updateNodeWithHistory(w.id, {
                              data: { ...A.data, label: L || void 0 }
                            }), vo(null);
                          },
                          onKeyDown: (Y) => {
                            (Y.key === "Enter" || Y.key === "Escape") && Y.currentTarget.blur(), Y.stopPropagation();
                          },
                          onPointerDown: (Y) => Y.stopPropagation(),
                          style: {
                            position: "absolute",
                            top: -24,
                            left: 0,
                            fontSize: 12,
                            color: A.data.borderColor || "#999",
                            fontWeight: 500,
                            background: "rgba(255,255,255,0.95)",
                            border: "1px solid #3b82f6",
                            borderRadius: 4,
                            padding: "1px 4px",
                            outline: "none",
                            pointerEvents: "auto",
                            minWidth: 80
                          }
                        }
                      ) : A.data.label ? /* @__PURE__ */ u(
                        "div",
                        {
                          onDoubleClick: (Y) => {
                            Y.stopPropagation(), t.select(w.id), vo(w.id);
                          },
                          style: {
                            position: "absolute",
                            top: -20,
                            left: 4,
                            fontSize: 12,
                            color: A.data.borderColor || "#999",
                            fontWeight: 500,
                            whiteSpace: "nowrap",
                            userSelect: "none",
                            pointerEvents: "auto",
                            cursor: "default"
                          },
                          children: A.data.label
                        }
                      ) : null
                    },
                    w.id
                  );
                } else {
                  const A = w;
                  A.type === "draw" ? B = /* @__PURE__ */ u(Fn, { node: A }, w.id) : B = /* @__PURE__ */ u(Fn, { node: A, editingLabel: Zo === w.id }, w.id);
                }
                return E || C ? /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      opacity: E ? 0.25 : void 0,
                      filter: E ? "saturate(0)" : void 0,
                      animation: C ? "sb-node-bop 3.4s ease-in-out infinite" : void 0,
                      animationDelay: C ? `${T}s` : void 0,
                      transformOrigin: "center center",
                      willChange: C ? "transform" : void 0
                    },
                    children: B
                  },
                  w.id
                ) : B;
              }),
              Zo && (() => {
                const w = t.getNode(Zo);
                if (!w || w.type !== "shape") return null;
                const E = w.data;
                return E.shape === "line" || E.shape === "arrow" ? null : /* @__PURE__ */ u(
                  Uh,
                  {
                    node: w,
                    engine: t,
                    onDone: () => So(null)
                  },
                  Zo
                );
              })()
            ]
          }
        ),
        /* @__PURE__ */ u(
          xh,
          {
            nodes: Le,
            viewport: a,
            selection: d,
            measuredHeights: rt,
            activeStroke: j,
            shapePreview: _,
            shapePreviewStyle: _ ? {
              stroke: t.mode === "frame" ? "#1e1e2e" : t.activeTool.color,
              strokeWidth: t.mode === "frame" ? 1 : t.activeTool.width,
              roughness: t.mode === "frame" ? 0 : t.activeTool.roughness ?? 1,
              shapeType: t.mode === "frame" ? "rect" : t.activeTool.shapeType || "rect"
            } : null,
            onResizeHandleDown: Qn,
            onRotateStart: Da,
            onConnectionHandleDown: Ms,
            onEdgeEndpointDown: Ha,
            onKinkHandleDown: Na,
            edgePreview: nt,
            edgeReconnect: pt,
            eraserMarkedIds: qn.size > 0 ? qn : void 0,
            eraserTrail: ks.length > 1 ? ks : void 0,
            laserTrail: vs.length > 1 ? vs : void 0,
            mode: m,
            hoveredNodeId: De,
            registry: o,
            onPortHandleDown: Fa,
            cycleNodeIds: n && Cs >= 0 ? n.cycleNodeIds : void 0,
            containerTypes: t.containerTypes,
            alignGuides: R
          }
        ),
        se && (() => {
          const w = t.selectionGroupId(), E = w ? t.groupRotations.get(w) : void 0;
          let C, V, T, B;
          if (Ee)
            C = Ee.bounds, V = Ee.angle, T = Ee.cx, B = Ee.cy;
          else if (E && E.angle !== 0) {
            const L = -E.angle * Math.PI / 180, q = Math.cos(L), U = Math.sin(L);
            let J = 1 / 0, dt = 1 / 0, ct = -1 / 0, St = -1 / 0;
            for (const zt of t.selection) {
              const kt = t.getNode(zt);
              if (!kt || kt.type === "edge") continue;
              const Pt = kt.h === "auto" ? rt[kt.id] ?? 100 : kt.h, Xt = kt.x + kt.w / 2, Ot = kt.y + Pt / 2, mt = Xt - E.cx, Wt = Ot - E.cy, wt = E.cx + mt * q - Wt * U, Gt = E.cy + mt * U + Wt * q;
              J = Math.min(J, wt - kt.w / 2), dt = Math.min(dt, Gt - Pt / 2), ct = Math.max(ct, wt + kt.w / 2), St = Math.max(St, Gt + Pt / 2);
            }
            C = {
              x: J - oe,
              y: dt - oe,
              w: ct - J + oe * 2,
              h: St - dt + oe * 2
            }, V = E.angle, T = E.cx, B = E.cy;
          } else
            C = se, V = 0, T = 0, B = 0;
          const D = 8 / a.zoom, A = D / 2, F = [
            { pos: "nw", cx: C.x, cy: C.y },
            { pos: "n", cx: C.x + C.w / 2, cy: C.y },
            { pos: "ne", cx: C.x + C.w, cy: C.y },
            { pos: "e", cx: C.x + C.w, cy: C.y + C.h / 2 },
            { pos: "se", cx: C.x + C.w, cy: C.y + C.h },
            { pos: "s", cx: C.x + C.w / 2, cy: C.y + C.h },
            { pos: "sw", cx: C.x, cy: C.y + C.h },
            { pos: "w", cx: C.x, cy: C.y + C.h / 2 }
          ], Y = V !== 0 ? ` rotate(${V}, ${T}, ${B})` : "";
          return /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ k("g", { transform: Y, children: [
                /* @__PURE__ */ u(
                  "rect",
                  {
                    x: C.x,
                    y: C.y,
                    width: C.w,
                    height: C.h,
                    fill: "none",
                    stroke: "#3b82f6",
                    strokeWidth: 1.5 / a.zoom
                  }
                ),
                V === 0 && F.map(({ pos: L, cx: q, cy: U }) => /* @__PURE__ */ u(
                  "rect",
                  {
                    x: q - A,
                    y: U - A,
                    width: D,
                    height: D,
                    fill: "white",
                    stroke: "#3b82f6",
                    strokeWidth: 1.5 / a.zoom,
                    style: { cursor: On(L, V), pointerEvents: "auto" },
                    onPointerDown: (J) => {
                      J.stopPropagation(), Xa(L, J);
                    }
                  },
                  L
                )),
                (() => {
                  const L = 25 / a.zoom, q = C.x + C.w / 2, U = C.y;
                  return /* @__PURE__ */ k(ft, { children: [
                    /* @__PURE__ */ u(
                      "line",
                      {
                        x1: q,
                        y1: U,
                        x2: q,
                        y2: U - L,
                        stroke: "#3b82f6",
                        strokeWidth: 1.5 / a.zoom,
                        style: { pointerEvents: "none" }
                      }
                    ),
                    (() => {
                      const J = 8 / a.zoom, dt = J / 2;
                      return /* @__PURE__ */ u(
                        "rect",
                        {
                          x: q - dt,
                          y: U - L - dt,
                          width: J,
                          height: J,
                          rx: 1.5 / a.zoom,
                          transform: `rotate(45, ${q}, ${U - L})`,
                          fill: "white",
                          stroke: "#3b82f6",
                          strokeWidth: 1.5 / a.zoom,
                          style: { cursor: "grab", pointerEvents: "auto" },
                          onPointerDown: (ct) => Oa(ct)
                        }
                      );
                    })()
                  ] });
                })(),
                (() => {
                  const L = 26 / a.zoom, q = 42 / a.zoom, U = 4 / a.zoom;
                  return [
                    { side: "top", cx: C.x + C.w / 2, cy: C.y - q },
                    { side: "right", cx: C.x + C.w + L, cy: C.y + C.h / 2 },
                    { side: "bottom", cx: C.x + C.w / 2, cy: C.y + C.h + L },
                    { side: "left", cx: C.x - L, cy: C.y + C.h / 2 }
                  ].map(({ side: dt, cx: ct, cy: St }) => /* @__PURE__ */ u(
                    "circle",
                    {
                      cx: ct,
                      cy: St,
                      r: U,
                      fill: "white",
                      stroke: "#94a3b8",
                      strokeWidth: 1.5 / a.zoom,
                      opacity: 0.8,
                      style: { cursor: "crosshair", pointerEvents: "auto" },
                      onPointerDown: (zt) => {
                        zt.stopPropagation();
                        const kt = Wa(dt);
                        kt && Ms(kt, dt, zt);
                      }
                    },
                    `conn-${dt}`
                  ));
                })()
              ] }) })
            }
          );
        })(),
        Ae && /* @__PURE__ */ u(
          "svg",
          {
            "data-sb-overlay": !0,
            style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
            children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ u(
              "rect",
              {
                x: Ae.x,
                y: Ae.y,
                width: Ae.w,
                height: Ae.h,
                fill: "none",
                stroke: "#6366f1",
                strokeWidth: 1.5 / a.zoom,
                strokeDasharray: `${5 / a.zoom} ${3 / a.zoom}`,
                rx: 4 / a.zoom,
                opacity: 0.5
              }
            ) })
          }
        ),
        me && (() => {
          const w = t.canvasToScreen(me.startX, me.startY), E = t.canvasToScreen(me.endX, me.endY), C = Math.min(w.x, E.x), V = Math.min(w.y, E.y), T = Math.abs(E.x - w.x), B = Math.abs(E.y - w.y);
          return T < 2 && B < 2 ? null : /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u(
                "rect",
                {
                  x: C,
                  y: V,
                  width: T,
                  height: B,
                  fill: "rgba(59,130,246,0.08)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5,
                  strokeDasharray: "4"
                }
              )
            }
          );
        })(),
        ro && ro.length > 2 && (() => {
          const E = ro.map(([C, V]) => t.canvasToScreen(C, V)).map((C) => `${C.x},${C.y}`).join(" ");
          return /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u(
                "polygon",
                {
                  points: E,
                  fill: "rgba(59,130,246,0.08)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5,
                  strokeDasharray: "4"
                }
              )
            }
          );
        })(),
        Yt && (() => {
          const w = Math.min(Yt.startX, Yt.endX), E = Math.min(Yt.startY, Yt.endY), C = Math.abs(Yt.endX - Yt.startX), V = Math.abs(Yt.endY - Yt.startY);
          return C < 2 && V < 2 ? null : /* @__PURE__ */ u(
            "svg",
            {
              "data-sb-overlay": !0,
              style: { position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" },
              children: /* @__PURE__ */ u("g", { transform: `translate(${a.x}, ${a.y}) scale(${a.zoom})`, children: /* @__PURE__ */ u(
                "rect",
                {
                  x: w,
                  y: E,
                  width: C,
                  height: V,
                  fill: "rgba(59,130,246,0.06)",
                  stroke: "#3b82f6",
                  strokeWidth: 1.5 / a.zoom,
                  strokeDasharray: `${4 / a.zoom}`,
                  rx: 8 / a.zoom
                }
              ) })
            }
          );
        })(),
        be && /* @__PURE__ */ u(
          wh,
          {
            x: be.x,
            y: be.y,
            sections: be.sections,
            onClose: () => Me(null)
          }
        ),
        un && /* @__PURE__ */ u(
          fh,
          {
            nodes: un.nodes,
            onSave: (w) => {
              rh(w, un.nodes, un.groupParent), Un(null);
            },
            onCancel: () => Un(null)
          }
        )
      ]
    }
  );
}
const dn = 52, en = 300, Zp = dn + en, Jh = [
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], ls = [
  { key: "hachure", label: "Hachure" },
  { key: "cross-hatch", label: "Cross-hatch" },
  { key: "solid", label: "Solid" }
], $h = [
  { key: "solid", label: "Solid", dash: "" },
  { key: "dashed", label: "Dashed", dash: "6,3" },
  { key: "dotted", label: "Dotted", dash: "2,2" }
], cs = [
  { value: 0, label: "Architect" },
  { value: 1, label: "Artist" },
  { value: 2, label: "Cartoonist" }
], Ia = [1, 2, 3, 5, 8, 12], ds = [1, 2, 3, 4, 6, 8], _h = [1, 2, 3, 4, 6], tu = ds, Ta = [14, 20, 28, 36], hs = [
  { key: "left", label: "←" },
  { key: "center", label: "↔" },
  { key: "right", label: "→" }
], eu = [
  "#FEF3C7",
  "#FCE7F3",
  "#DBEAFE",
  "#D1FAE5",
  "#EDE9FE",
  "#FFEDD5"
], we = [
  { name: "Standard", colors: Jh },
  { name: "Pastel", colors: ["#F8B4B4", "#FDBA74", "#FDE68A", "#86EFAC", "#93C5FD", "#C4B5FD"] },
  { name: "Earth", colors: ["#78350F", "#92400E", "#6B7280", "#065F46", "#1E3A5F", "#7C2D12"] },
  { name: "Neon", colors: ["#FF1493", "#39FF14", "#00FFFF", "#FF6600", "#FFFF00", "#BF00FF"] },
  { name: "Crayon", colors: ["#EF4444", "#F97316", "#EAB308", "#22C55E", "#3B82F6", "#A855F7"] },
  { name: "Mono", colors: ["#000000", "#404040", "#737373", "#A3A3A3", "#D4D4D4", "#FFFFFF"] }
], us = we, ou = [
  { name: "Standard", colors: eu },
  { name: "Bright", colors: ["#FDE047", "#FB923C", "#F87171", "#4ADE80", "#60A5FA", "#C084FC"] },
  { name: "Earth", colors: ["#D6CFC7", "#E8D5B7", "#C4B5A0", "#B8C5A3", "#A3B5C4", "#C7B8A8"] },
  { name: "Cool", colors: ["#BFDBFE", "#A5F3FC", "#C7D2FE", "#DDD6FE", "#BAE6FD", "#E0E7FF"] }
], Lt = {
  display: "flex",
  alignItems: "center",
  gap: 6
}, Et = {
  width: 64,
  fontSize: 10,
  flexShrink: 0
}, Zt = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0,
  flexShrink: 0
}, nu = "https://libraries.excalidraw.com/libraries.json", Fr = "https://libraries.excalidraw.com/libraries";
function ru({
  onClose: t,
  onInstalled: e
}) {
  const o = qt(), [n, r] = et([]), [s, i] = et(!0), [l, a] = et(null), [c, h] = et(""), [p, d] = et(null), [f, m] = et(/* @__PURE__ */ new Set()), y = st(() => {
    const x = oa(), S = new Set(x.map((M) => M.source));
    m(S);
  }, []);
  yt(() => {
    let x = !1;
    return (async () => {
      try {
        const S = await fetch(nu);
        if (!S.ok) throw new Error(`HTTP ${S.status}`);
        const M = await S.json();
        x || (r(M), i(!1));
      } catch (S) {
        x || (a(String(S)), i(!1));
      }
    })(), y(), () => {
      x = !0;
    };
  }, [y]);
  const b = Kt(() => {
    if (!c.trim()) return n;
    const x = c.toLowerCase();
    return n.filter(
      (S) => {
        var M, z;
        return S.name.toLowerCase().includes(x) || ((M = S.description) == null ? void 0 : M.toLowerCase().includes(x)) || ((z = S.itemNames) == null ? void 0 : z.some((W) => W.toLowerCase().includes(x)));
      }
    );
  }, [n, c]), g = st(
    async (x) => {
      d(x.id);
      try {
        const S = `${Fr}/${x.source}`;
        await Kd(S, x.name), y(), e();
      } catch (S) {
        console.error("Failed to install library:", S);
      } finally {
        d(null);
      }
    },
    [e, y]
  );
  return Oe(
    /* @__PURE__ */ u(
      "div",
      {
        style: {
          position: "fixed",
          inset: 0,
          zIndex: 1e5,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          background: "rgba(0,0,0,0.4)",
          backdropFilter: "blur(4px)"
        },
        onPointerDown: (x) => {
          x.target === x.currentTarget && t();
        },
        children: /* @__PURE__ */ k(
          "div",
          {
            style: {
              width: 620,
              maxWidth: "90vw",
              maxHeight: "80vh",
              background: o.panelBg,
              border: `1px solid ${o.border}`,
              borderRadius: 12,
              boxShadow: "0 8px 32px rgba(0,0,0,0.25)",
              display: "flex",
              flexDirection: "column",
              overflow: "hidden"
            },
            onPointerDown: (x) => x.stopPropagation(),
            children: [
              /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    padding: "16px 20px 12px",
                    borderBottom: `1px solid ${o.border}`,
                    flexShrink: 0
                  },
                  children: [
                    /* @__PURE__ */ k(
                      "div",
                      {
                        style: {
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "center",
                          marginBottom: 12
                        },
                        children: [
                          /* @__PURE__ */ u(
                            "span",
                            {
                              style: {
                                fontSize: 14,
                                fontWeight: 600,
                                color: o.text
                              },
                              children: "Excalidraw Libraries"
                            }
                          ),
                          /* @__PURE__ */ u(
                            "button",
                            {
                              onClick: t,
                              style: {
                                border: "none",
                                background: "transparent",
                                color: o.textMuted,
                                cursor: "pointer",
                                fontSize: 18,
                                lineHeight: 1,
                                padding: "2px 6px"
                              },
                              children: "×"
                            }
                          )
                        ]
                      }
                    ),
                    /* @__PURE__ */ u(
                      "input",
                      {
                        type: "text",
                        placeholder: "Search libraries...",
                        value: c,
                        onChange: (x) => h(x.target.value),
                        autoFocus: !0,
                        style: {
                          width: "100%",
                          padding: "7px 10px",
                          border: `1px solid ${o.border}`,
                          borderRadius: 6,
                          background: o.controlBg,
                          color: o.text,
                          fontSize: 12,
                          outline: "none",
                          boxSizing: "border-box"
                        }
                      }
                    )
                  ]
                }
              ),
              /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    flex: 1,
                    overflowY: "auto",
                    padding: "8px 20px"
                  },
                  children: [
                    s && /* @__PURE__ */ u(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: o.textMuted,
                          fontSize: 12
                        },
                        children: "Loading libraries..."
                      }
                    ),
                    l && /* @__PURE__ */ k(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: "#ef4444",
                          fontSize: 12
                        },
                        children: [
                          "Failed to load directory: ",
                          l
                        ]
                      }
                    ),
                    !s && !l && b.length === 0 && /* @__PURE__ */ u(
                      "div",
                      {
                        style: {
                          textAlign: "center",
                          padding: 40,
                          color: o.textDisabled,
                          fontSize: 12
                        },
                        children: "No libraries match your search."
                      }
                    ),
                    b.map((x, S) => {
                      const M = f.has(
                        `${Fr}/${x.source}`
                      ), z = p === x.id;
                      return /* @__PURE__ */ u(
                        su,
                        {
                          entry: x,
                          isInstalled: M,
                          isInstalling: z,
                          onInstall: () => g(x),
                          theme: o
                        },
                        x.id || `dir-${S}`
                      );
                    })
                  ]
                }
              ),
              /* @__PURE__ */ k(
                "div",
                {
                  style: {
                    padding: "8px 20px",
                    borderTop: `1px solid ${o.border}`,
                    color: o.textDisabled,
                    fontSize: 10,
                    textAlign: "center",
                    flexShrink: 0
                  },
                  children: [
                    b.length,
                    " libraries • Powered by Excalidraw Libraries"
                  ]
                }
              )
            ]
          }
        )
      }
    ),
    document.body
  );
}
function su({
  entry: t,
  isInstalled: e,
  isInstalling: o,
  onInstall: n,
  theme: r
}) {
  var i;
  const s = t.preview ? `${Fr}/${t.preview}` : null;
  return /* @__PURE__ */ k(
    "div",
    {
      style: {
        display: "flex",
        gap: 12,
        padding: "10px 0",
        borderBottom: `1px solid ${r.border}`,
        alignItems: "flex-start"
      },
      children: [
        s && /* @__PURE__ */ u(
          "img",
          {
            src: s,
            alt: t.name,
            loading: "lazy",
            style: {
              width: 64,
              height: 64,
              objectFit: "cover",
              borderRadius: 6,
              border: `1px solid ${r.border}`,
              flexShrink: 0,
              background: "#fff"
            }
          }
        ),
        /* @__PURE__ */ k("div", { style: { flex: 1, minWidth: 0 }, children: [
          /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: 12,
                fontWeight: 600,
                color: r.text,
                marginBottom: 2
              },
              children: t.name
            }
          ),
          ((i = t.authors) == null ? void 0 : i.length) > 0 && /* @__PURE__ */ k(
            "div",
            {
              style: {
                fontSize: 10,
                color: r.textMuted,
                marginBottom: 4
              },
              children: [
                "by ",
                t.authors.map((l) => l.name).join(", ")
              ]
            }
          ),
          t.description && /* @__PURE__ */ u(
            "div",
            {
              style: {
                fontSize: 10,
                color: r.textSecondary,
                lineHeight: 1.4,
                overflow: "hidden",
                display: "-webkit-box",
                WebkitLineClamp: 2,
                WebkitBoxOrient: "vertical"
              },
              children: t.description
            }
          )
        ] }),
        /* @__PURE__ */ u(
          "button",
          {
            onClick: n,
            disabled: e || o,
            style: {
              flexShrink: 0,
              padding: "5px 10px",
              border: e ? `1px solid ${r.border}` : "none",
              borderRadius: 4,
              background: e ? "transparent" : o ? r.controlBgActive : r.accentColor,
              color: e ? r.textMuted : "#fff",
              cursor: e || o ? "default" : "pointer",
              fontSize: 10,
              fontWeight: 500,
              opacity: o ? 0.7 : 1
            },
            children: e ? "Installed" : o ? "Installing..." : "Install"
          }
        )
      ]
    }
  );
}
const iu = /^[A-Za-z][A-Za-z0-9_:-]*$/, ui = /^[A-Za-z][A-Za-z0-9_]*$/;
function au(t) {
  const e = t.trim();
  return e.startsWith('"') && e.endsWith('"') || e.startsWith("'") && e.endsWith("'") ? e.slice(1, -1).trim() : e;
}
function xe(t) {
  return au(t).replace(/<br\s*\/?>/gi, `
`).replace(/\\n/g, `
`);
}
function yr(t, e) {
  const o = t.nodes.get(e.key);
  return o ? (o.label === o.key && e.label !== e.key && (o.label = e.label), o.shape === "rect" && e.shape !== "rect" && (o.shape = e.shape), o) : (t.nodes.set(e.key, e), e);
}
function po(t) {
  const e = t.trim();
  if (!e) return null;
  let o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\(\(([\s\S]+)\)\)$/);
  return o ? { key: o[1], label: xe(o[2]), shape: "circle" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\(([\s\S]+)\)$/), o ? { key: o[1], label: xe(o[2]), shape: "round" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\{([\s\S]+)\}$/), o ? { key: o[1], label: xe(o[2]), shape: "diamond" } : (o = e.match(/^([A-Za-z][A-Za-z0-9_:-]*)\s*\[([\s\S]+)\]$/), o ? { key: o[1], label: xe(o[2]), shape: "rect" } : iu.test(e) ? { key: e, label: e, shape: "rect" } : null)));
}
function lu(t) {
  let e = t.match(/^(.*?)\s*--\s*\|([^|]+)\|\s*-->\s*(.*?)$/);
  if (e) {
    const o = po(e[1]), n = po(e[3]);
    return !o || !n ? null : { from: o, to: n, label: xe(e[2]) };
  }
  if (e = t.match(/^(.*?)\s*--\s*([^>-][\s\S]*?)\s*-->\s*(.*?)$/), e) {
    const o = po(e[1]), n = po(e[3]);
    return !o || !n ? null : { from: o, to: n, label: xe(e[2]) };
  }
  if (e = t.match(/^(.*?)\s*(?:-->|==>|-\.->|---)\s*(.*?)$/), e) {
    const o = po(e[1]), n = po(e[2]);
    return !o || !n ? null : { from: o, to: n };
  }
  return null;
}
function cu(t) {
  const e = t.match(/\b(TB|TD|BT|LR|RL)\b/i);
  if (!e) return "TB";
  const o = e[1].toUpperCase();
  return o === "TD" ? "TB" : o === "TB" || o === "BT" || o === "LR" || o === "RL" ? o : "TB";
}
function du(t) {
  const e = t.match(/^subgraph(?:\s+([\s\S]+))?$/i);
  if (!e) return null;
  const o = (e[1] ?? "").trim();
  if (!o) return {};
  const n = o.match(/^[A-Za-z][A-Za-z0-9_:-]*\s*\[([\s\S]+)\]$/);
  return n ? { label: xe(n[1]) } : { label: xe(o) };
}
function hu(t) {
  const o = { direction: "TB", nodes: /* @__PURE__ */ new Map(), edges: [], groups: [] }, n = t.replace(/\r\n/g, `
`).split(`
`).map((a) => a.replace(/%%.*$/, "").trim()).filter(Boolean);
  if (n.length === 0)
    throw new Error("Paste a Mermaid flowchart first.");
  const r = n[0];
  /^(flowchart|graph)\b/i.test(r) && (o.direction = cu(r), n.shift());
  const i = [], l = (a) => {
    for (const c of i) c.nodeKeys.add(a);
  };
  for (const a of n) {
    const c = a.split(";").map((h) => h.trim()).filter(Boolean);
    for (const h of c) {
      const p = du(h);
      if (p) {
        i.push({ label: p.label, nodeKeys: /* @__PURE__ */ new Set() });
        continue;
      }
      if (/^end\b/i.test(h)) {
        const m = i.pop();
        m && o.groups.push({
          label: m.label,
          nodeKeys: Array.from(m.nodeKeys)
        });
        continue;
      }
      const d = lu(h);
      if (d) {
        const m = yr(o, d.from), y = yr(o, d.to);
        l(m.key), l(y.key), o.edges.push({ fromKey: m.key, toKey: y.key, label: d.label });
        continue;
      }
      const f = po(h);
      if (f) {
        const m = yr(o, f);
        l(m.key);
      }
    }
  }
  for (; i.length > 0; ) {
    const a = i.pop();
    o.groups.push({
      label: a.label,
      nodeKeys: Array.from(a.nodeKeys)
    });
  }
  if (o.nodes.size === 0)
    throw new Error("Could not parse Mermaid nodes. Try simple flowchart syntax like A-->B.");
  return o;
}
function uu(t) {
  const e = t.indexOf(":");
  if (e < 0) return null;
  const o = t.slice(0, e).trim(), n = t.slice(e + 1).trim();
  if (!o || !n) return null;
  const r = [
    "-->>",
    "->>",
    "--x",
    "-x",
    "-->",
    "->",
    "--)",
    "-)",
    "-.->",
    "==>",
    "---"
  ];
  for (const s of r) {
    const i = o.indexOf(s);
    if (i < 0) continue;
    const l = o.slice(0, i).trim(), a = o.slice(i + s.length).trim();
    if (!(!ui.test(l) || !ui.test(a)))
      return {
        from: l,
        arrow: s,
        to: a,
        label: xe(n)
      };
  }
  return null;
}
function pu(t) {
  const e = t.match(/^Note\s+(left|right|over)\s+of\s+([A-Za-z][A-Za-z0-9_:-]*)\s*:\s*([\s\S]+)$/i);
  return e ? {
    side: e[1].toLowerCase(),
    of: e[2],
    text: xe(e[3])
  } : null;
}
function fu(t) {
  return t ? /^#[0-9a-f]{3,8}$/i.test(t) || /^(rgb|rgba|hsl|hsla)\(/i.test(t) ? !0 : (/* @__PURE__ */ new Set([
    "red",
    "green",
    "blue",
    "purple",
    "pink",
    "orange",
    "yellow",
    "brown",
    "gray",
    "grey",
    "black",
    "white",
    "teal",
    "cyan",
    "magenta",
    "indigo",
    "violet",
    "gold",
    "silver",
    "maroon",
    "navy",
    "olive",
    "lime",
    "aqua",
    "fuchsia",
    "rebeccapurple"
  ])).has(t.toLowerCase()) : !1;
}
function yu(t) {
  const e = t.match(/^box(?:\s+(.+))?$/i);
  if (!e) return null;
  const o = (e[1] ?? "").trim();
  if (!o) return {};
  const n = o.indexOf(" "), r = n >= 0 ? o.slice(0, n) : o, s = n >= 0 ? o.slice(n + 1).trim() : "";
  return fu(r) ? { color: r, label: s || void 0 } : { label: o };
}
function mu(t) {
  const e = t.replace(/\r\n/g, `
`).split(`
`).map((d) => d.replace(/%%.*$/, "").trim()).filter(Boolean);
  if (e.length === 0)
    throw new Error("Paste Mermaid sequenceDiagram text first.");
  if (!/^sequenceDiagram\b/i.test(e[0]))
    throw new Error("Not a Mermaid sequence diagram.");
  const o = /* @__PURE__ */ new Set(), n = [], r = [], s = [], i = [], l = [], a = [];
  let c = 0;
  const h = (d) => {
    o.has(d) || (o.add(d), n.push(d));
    for (const f of a) f.participants.add(d);
  };
  for (let d = 1; d < e.length; d++) {
    const f = e[d];
    if (/^autonumber\b/i.test(f)) continue;
    const m = yu(f);
    if (m) {
      a.push({ type: "box", label: m.label, color: m.color, participants: /* @__PURE__ */ new Set() });
      continue;
    }
    const y = f.match(/^loop(?:\s+([\s\S]+))?$/i);
    if (y) {
      a.push({
        type: "loop",
        label: y[1] ? xe(y[1]) : void 0,
        startStep: c,
        participants: /* @__PURE__ */ new Set()
      });
      continue;
    }
    if (/^end\b/i.test(f)) {
      const S = a.pop();
      (S == null ? void 0 : S.type) === "box" ? l.push(S) : (S == null ? void 0 : S.type) === "loop" && i.push({
        label: S.label,
        startStep: S.startStep,
        endStep: c,
        participants: S.participants
      });
      continue;
    }
    const b = f.match(/^participant\s+([A-Za-z][A-Za-z0-9_]*)(?:\s+as\s+[\s\S]+)?$/i);
    if (b) {
      h(b[1]);
      continue;
    }
    const g = pu(f);
    if (g) {
      h(g.of), s.push({ step: c, note: g });
      continue;
    }
    const x = uu(f);
    if (x) {
      h(x.from), h(x.to), r.push(x), c += 1;
      continue;
    }
  }
  for (; a.length > 0; ) {
    const d = a.pop();
    d.type === "box" ? l.push(d) : i.push({
      label: d.label,
      startStep: d.startStep,
      endStep: c,
      participants: d.participants
    });
  }
  const p = n;
  if (p.length === 0)
    throw new Error("No participants found in sequenceDiagram.");
  if (r.length === 0 && s.length === 0)
    throw new Error("No messages/notes found in sequenceDiagram.");
  return {
    participants: p,
    messages: r,
    notes: s,
    loops: i.map((d) => ({
      label: d.label,
      startStep: d.startStep,
      endStep: d.endStep,
      participants: Array.from(d.participants)
    })).filter((d) => d.endStep >= d.startStep),
    groups: l.map((d) => ({
      label: d.label,
      color: d.color,
      participants: Array.from(d.participants)
    })).filter((d) => d.participants.length > 0)
  };
}
function Mn(t) {
  return t === "diamond" ? { w: 200, h: 120 } : t === "circle" ? { w: 140, h: 140 } : { w: 200, h: 96 };
}
function bu(t) {
  const e = Array.from(t.nodes.keys()).sort(), o = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map();
  for (const c of e)
    o.set(c, 0), n.set(c, []);
  for (const c of t.edges)
    !o.has(c.fromKey) || !o.has(c.toKey) || (n.get(c.fromKey).push(c.toKey), o.set(c.toKey, (o.get(c.toKey) ?? 0) + 1));
  const r = e.filter((c) => (o.get(c) ?? 0) === 0), s = /* @__PURE__ */ new Map();
  for (const c of r) s.set(c, 0);
  const i = [...r];
  for (; i.length > 0; ) {
    const c = i.shift(), h = s.get(c) ?? 0;
    for (const p of n.get(c) ?? []) {
      const d = Math.max(s.get(p) ?? 0, h + 1);
      s.set(p, d), o.set(p, (o.get(p) ?? 0) - 1), (o.get(p) ?? 0) <= 0 && i.push(p);
    }
  }
  let l = 0;
  for (const c of s.values()) l = Math.max(l, c);
  for (const c of e)
    s.has(c) || (l += 1, s.set(c, l));
  const a = /* @__PURE__ */ new Map();
  for (const c of e) {
    const h = s.get(c) ?? 0;
    a.has(h) || a.set(h, []), a.get(h).push(c);
  }
  return Array.from(a.entries()).sort((c, h) => c[0] - h[0]).map(([, c]) => c.sort());
}
function gu(t, e, o, n) {
  const r = mu(t), s = [], i = [], l = 6, a = "#94a3b8", c = 3, h = "#475569", p = 180, d = 64, f = 270, m = o - 140, y = m + d + 8, b = 88, g = Math.max(1, r.messages.length), x = y + g * b + 40, S = x + 12, M = S + d, z = /* @__PURE__ */ new Map();
  for (const W of r.groups) {
    const I = W.participants.map((ot) => z.get(ot)).filter((ot) => typeof ot == "number");
    if (I.length === 0)
      for (const ot of W.participants) {
        const Q = r.participants.indexOf(ot);
        Q >= 0 && I.push(e + (Q - (r.participants.length - 1) / 2) * f);
      }
    if (I.length === 0) continue;
    const R = Math.min(...I) - p / 2 - 24, K = Math.max(...I) + p / 2 + 24, Z = m - 22, lt = M - Z + 18, O = {
      id: bt(10),
      type: "shape",
      x: R,
      y: Z,
      w: K - R,
      h: lt,
      z: n(),
      data: {
        shape: "rect",
        stroke: W.color ? W.color : "#475569",
        strokeWidth: 1.5,
        strokeStyle: "solid",
        roughness: 0,
        fill: W.color ? W.color : "#334155",
        fillStyle: "solid",
        opacity: W.color ? 0.2 : 0.08,
        edgeStyle: "sharp"
      }
    };
    if (s.push(O), i.push(O.id), W.label) {
      const ot = {
        id: bt(10),
        type: "text",
        x: R + 10,
        y: Z + 8,
        w: Math.max(120, K - R - 20),
        h: "auto",
        z: n(),
        data: {
          text: W.label,
          fontSize: 14,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "left"
        }
      };
      s.push(ot);
    }
  }
  for (let W = 0; W < r.participants.length; W++) {
    const I = r.participants[W], R = e + (W - (r.participants.length - 1) / 2) * f;
    z.set(I, R);
    const K = {
      id: bt(10),
      type: "shape",
      x: R - p / 2,
      y: m,
      w: p,
      h: d,
      z: n(),
      data: {
        shape: "rect",
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        edgeStyle: "round",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        label: I,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    s.push(K), i.push(K.id);
    const Z = {
      id: bt(10),
      type: "shape",
      x: R - l / 2,
      y,
      w: l,
      h: x - y,
      z: n(),
      data: {
        shape: "rect",
        stroke: a,
        strokeWidth: 1,
        strokeStyle: "solid",
        roughness: 0,
        fill: a,
        fillStyle: "solid",
        opacity: 0.3,
        edgeStyle: "round"
      }
    };
    s.push(Z);
    const lt = {
      id: bt(10),
      type: "shape",
      x: R - p / 2,
      y: S,
      w: p,
      h: d,
      z: n(),
      data: {
        shape: "rect",
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        edgeStyle: "round",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        label: I,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    s.push(lt), i.push(lt.id);
  }
  for (const W of r.loops) {
    const I = W.participants.map((G) => z.get(G)).filter((G) => typeof G == "number");
    if (I.length === 0) continue;
    const R = Math.min(...I) - 130, K = Math.max(...I) + 130, Z = W.startStep + 1, lt = Math.max(Z, W.endStep), O = y + (Z - 1) * b + 16, ot = y + lt * b + 34, Q = {
      id: bt(10),
      type: "shape",
      x: R,
      y: O,
      w: K - R,
      h: Math.max(90, ot - O),
      z: n(),
      data: {
        shape: "rect",
        stroke: "#c4b5fd",
        strokeWidth: 1.5,
        strokeStyle: "dotted",
        roughness: 0,
        fill: "#64748b",
        fillStyle: "solid",
        opacity: 0.08,
        edgeStyle: "sharp"
      }
    };
    s.push(Q);
    const $ = `loop${W.label ? ` [${W.label}]` : ""}`, P = {
      id: bt(10),
      type: "text",
      x: R + 10,
      y: O + 8,
      w: K - R - 20,
      h: "auto",
      z: n(),
      data: {
        text: $,
        fontSize: 14,
        fontFamily: "sans-serif",
        color: "#1f2937",
        align: "left"
      }
    };
    s.push(P);
  }
  for (let W = 0; W < r.messages.length; W++) {
    const I = r.messages[W], R = y + (W + 1) * b, K = z.get(I.from), Z = z.get(I.to);
    if (K == null || Z == null) continue;
    const lt = K === Z, O = Math.min(K, Z), ot = Math.max(K, Z), Q = Math.max(ot - O, 40), $ = K <= Z ? 0 : Q, P = K <= Z ? Q : 0, G = I.arrow.includes("--") || I.arrow === "-.->", X = I.arrow.toLowerCase().includes("x"), N = I.arrow.includes(">") || I.arrow.includes(")");
    if (lt) {
      const tt = K + 6, nt = R - 16, ut = 92, pt = 48, vt = G ? "dashed" : "solid", rt = {
        id: bt(10),
        type: "shape",
        x: tt,
        y: nt,
        w: ut,
        h: c,
        z: n(),
        data: {
          shape: "rect",
          stroke: h,
          strokeWidth: 0,
          strokeStyle: "solid",
          roughness: 0,
          fill: h,
          fillStyle: "solid",
          opacity: 1,
          edgeStyle: "sharp"
        }
      }, gt = {
        id: bt(10),
        type: "shape",
        x: tt + ut - c,
        y: nt,
        w: c,
        h: pt,
        z: n(),
        data: {
          shape: "rect",
          stroke: h,
          strokeWidth: 0,
          strokeStyle: "solid",
          roughness: 0,
          fill: h,
          fillStyle: "solid",
          opacity: 1,
          edgeStyle: "sharp"
        }
      }, xt = {
        id: bt(10),
        type: "shape",
        x: tt,
        y: nt + pt - c,
        w: ut,
        h: c,
        z: n(),
        data: {
          shape: N ? "arrow" : "line",
          stroke: h,
          strokeWidth: c,
          strokeStyle: vt,
          roughness: 0,
          startPoint: [ut, c / 2],
          endPoint: [8, c / 2]
        }
      };
      s.push(rt, gt, xt);
    } else {
      const tt = {
        id: bt(10),
        type: "shape",
        x: O,
        y: R - 14,
        w: Q,
        h: 28,
        z: n(),
        data: {
          shape: N ? "arrow" : "line",
          stroke: h,
          strokeWidth: c,
          strokeStyle: G ? "dashed" : "solid",
          roughness: 0,
          startPoint: [$, 14],
          endPoint: [P, 14]
        }
      };
      s.push(tt);
    }
    const j = lt ? K + 18 : O, H = lt ? 170 : Q, _ = {
      id: bt(10),
      type: "text",
      x: j,
      y: R - 46,
      w: H,
      h: "auto",
      z: n(),
      data: {
        text: I.label,
        fontSize: 14,
        fontFamily: "sans-serif",
        color: "#0f172a",
        align: "center"
      }
    };
    if (s.push(_), X) {
      const tt = K <= Z ? O + Q - 14 : O + 8, nt = {
        id: bt(10),
        type: "text",
        x: tt,
        y: R - 20,
        w: 20,
        h: "auto",
        z: n(),
        data: {
          text: "×",
          fontSize: 16,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "center"
        }
      };
      s.push(nt);
    }
  }
  for (const W of r.notes) {
    const I = y + (W.step + 1) * b, R = z.get(W.note.of);
    if (R == null) continue;
    let K = R;
    W.note.side === "right" && (K += 130), W.note.side === "left" && (K -= 300), W.note.side === "over" && (K -= 110);
    const Z = {
      id: bt(10),
      type: "text",
      x: K,
      y: I - 8,
      w: 260,
      h: "auto",
      z: n(),
      data: {
        text: W.note.text,
        fontSize: 13,
        fontFamily: "sans-serif",
        color: "#0f172a",
        align: "left"
      }
    };
    s.push(Z);
  }
  return { nodes: s, shapeNodeIds: i };
}
function xu(t, e, o, n) {
  const r = t.trimStart();
  if (/^sequenceDiagram\b/i.test(r))
    return gu(t, e, o, n);
  const s = hu(t), i = bu(s), l = Array.from(s.nodes.values()).map((b) => Mn(b.shape)), a = l.length > 0 ? Math.max(...l.map((b) => b.h)) : 96, c = Math.max(a + 130, 260), h = /* @__PURE__ */ new Map(), p = i.length;
  for (let b = 0; b < i.length; b++) {
    const g = i[b], x = g.length, S = (b - (p - 1) / 2) * c, M = g.length > 0 ? Math.max(
      ...g.map((W) => {
        const I = s.nodes.get(W);
        return I ? Mn(I.shape).w : 200;
      })
    ) : 200, z = Math.max(M + 90, 260);
    for (let W = 0; W < g.length; W++) {
      const I = g[W], R = (W - (x - 1) / 2) * z;
      if (s.direction === "LR" || s.direction === "RL") {
        const K = s.direction === "LR" ? e + S : e - S, Z = o + R;
        h.set(I, { x: K, y: Z });
      } else {
        const K = e + R, Z = s.direction === "TB" ? o + S : o - S;
        h.set(I, { x: K, y: Z });
      }
    }
  }
  const d = /* @__PURE__ */ new Map(), f = [], m = [], y = /* @__PURE__ */ new Map();
  for (const b of s.groups) {
    if (!b.nodeKeys.length) continue;
    const g = b.nodeKeys.map((I) => {
      const R = s.nodes.get(I), K = h.get(I);
      if (!R || !K) return null;
      const Z = Mn(R.shape);
      return { x: K.x - Z.w / 2, y: K.y - Z.h / 2, w: Z.w, h: Z.h };
    }).filter((I) => !!I);
    if (!g.length) continue;
    const x = Math.min(...g.map((I) => I.x)) - 30, S = Math.max(...g.map((I) => I.x + I.w)) + 30, M = Math.min(...g.map((I) => I.y)) - 34, z = Math.max(...g.map((I) => I.y + I.h)) + 24, W = {
      id: bt(10),
      type: "shape",
      x,
      y: M,
      w: S - x,
      h: z - M,
      z: n(),
      data: {
        shape: "rect",
        stroke: "#475569",
        strokeWidth: 1.5,
        strokeStyle: "solid",
        roughness: 0,
        fill: "#334155",
        fillStyle: "solid",
        opacity: 0.12,
        edgeStyle: "sharp"
      }
    };
    if (f.push(W), m.push(W.id), b.label) {
      const I = {
        id: bt(10),
        type: "text",
        x: x + 10,
        y: M + 8,
        w: Math.max(120, S - x - 20),
        h: "auto",
        z: n(),
        data: {
          text: b.label,
          fontSize: 14,
          fontFamily: "sans-serif",
          color: "#475569",
          align: "left"
        }
      };
      f.push(I);
    }
  }
  for (const [b, g] of s.nodes) {
    const x = h.get(b) ?? { x: e, y: o }, S = Mn(g.shape), M = {
      id: bt(10),
      type: "shape",
      x: x.x - S.w / 2,
      y: x.y - S.h / 2,
      w: S.w,
      h: S.h,
      z: n(),
      data: {
        shape: g.shape === "diamond" ? "diamond" : g.shape === "circle" ? "ellipse" : (g.shape === "round", "rect"),
        stroke: "#334155",
        strokeWidth: 2,
        strokeStyle: "solid",
        roughness: 1,
        fill: "#f8fafc",
        fillStyle: "solid",
        edgeStyle: g.shape === "round" ? "round" : "sharp",
        label: g.label,
        labelAlign: "center",
        labelFontSize: 14
      }
    };
    f.push(M), m.push(M.id), d.set(b, M.id), y.set(b, { x: M.x, y: M.y, w: S.w, h: S.h });
  }
  for (const b of s.edges) {
    const g = d.get(b.fromKey), x = d.get(b.toKey);
    if (!g || !x || g === x) continue;
    const S = {
      id: bt(10),
      type: "edge",
      x: 0,
      y: 0,
      w: 0,
      h: 0,
      z: n(),
      data: {
        fromId: g,
        toId: x,
        label: b.label,
        style: "solid",
        color: "#64748b",
        strokeWidth: 2,
        arrowHead: "arrow",
        edgeType: "bezier"
      }
    };
    f.push(S);
  }
  return { nodes: f, shapeNodeIds: m };
}
const pi = `flowchart LR
  A[Idea] --> B{Decision}
  B -- Yes --> C[Ship]
  B -- No --> D[Refine]
  D --> B`;
function wu({
  engine: t,
  open: e,
  onClose: o,
  triggerRect: n
}) {
  const r = qt(), s = at(null), [i, l] = et(pi), [a, c] = et(null), [h, p] = et(null);
  yt(() => {
    if (!e) return;
    const m = (y) => {
      s.current && !s.current.contains(y.target) && o();
    };
    return document.addEventListener("pointerdown", m), () => document.removeEventListener("pointerdown", m);
  }, [e, o]);
  const d = Kt(
    () => "Supported: flowchart/graph (TB/BT/LR/RL) and sequenceDiagram. Flowchart nodes: A[Text], A{Decision}, A((Start)). Edges: A-->B, A -- label --> B.",
    []
  ), f = st(() => {
    try {
      const m = window.innerWidth / 2, y = window.innerHeight / 2, b = t.screenToCanvas(m, y), { nodes: g, shapeNodeIds: x } = xu(i, b.x, b.y, () => t.nextZ());
      if (g.length === 0)
        throw new Error("No nodes were parsed.");
      t.addNodes(g), x.length > 0 && t.selectMultiple(x), c(null), p(`Inserted ${x.length} nodes and ${g.length - x.length} edges.`);
    } catch (m) {
      p(null), c(m instanceof Error ? m.message : "Failed to parse Mermaid graph.");
    }
  }, [t, i]);
  return !e || !n ? null : Oe(
    /* @__PURE__ */ k(
      "div",
      {
        ref: s,
        style: {
          position: "fixed",
          left: n.right + 8,
          top: n.top,
          background: r.panelBg,
          border: `1px solid ${r.border}`,
          borderRadius: r.panelBorderRadius,
          boxShadow: r.panelShadow,
          width: 340,
          maxHeight: `calc(100vh - ${n.top + 20}px)`,
          overflow: "hidden",
          display: "flex",
          flexDirection: "column",
          zIndex: 99999
        },
        onPointerDown: (m) => m.stopPropagation(),
        children: [
          /* @__PURE__ */ k("div", { style: { padding: "10px 12px 8px", borderBottom: `1px solid ${r.border}` }, children: [
            /* @__PURE__ */ u("div", { style: { fontSize: 12, fontWeight: 700, color: r.text }, children: "Mermaid Sketch" }),
            /* @__PURE__ */ u("div", { style: { marginTop: 4, fontSize: 10, color: r.textMuted, lineHeight: 1.45 }, children: d })
          ] }),
          /* @__PURE__ */ k("div", { style: { padding: 12, display: "flex", flexDirection: "column", gap: 8 }, children: [
            /* @__PURE__ */ u(
              "textarea",
              {
                value: i,
                onChange: (m) => l(m.target.value),
                spellCheck: !1,
                style: {
                  width: "100%",
                  minHeight: 180,
                  resize: "vertical",
                  padding: "8px 10px",
                  borderRadius: r.controlBorderRadius,
                  border: `1px solid ${r.border}`,
                  background: r.controlBg,
                  color: r.text,
                  fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace",
                  fontSize: 11,
                  lineHeight: 1.4,
                  boxSizing: "border-box"
                }
              }
            ),
            a && /* @__PURE__ */ u("div", { style: { fontSize: 10, color: "#ef4444" }, children: a }),
            h && /* @__PURE__ */ u("div", { style: { fontSize: 10, color: "#16a34a" }, children: h }),
            /* @__PURE__ */ k("div", { style: { display: "flex", justifyContent: "space-between", gap: 8 }, children: [
              /* @__PURE__ */ u(
                "button",
                {
                  onClick: () => l(pi),
                  style: {
                    border: `1px solid ${r.border}`,
                    background: "transparent",
                    color: r.text,
                    borderRadius: r.controlBorderRadius,
                    padding: "6px 10px",
                    fontSize: 11,
                    cursor: "pointer"
                  },
                  children: "Reset Example"
                }
              ),
              /* @__PURE__ */ u(
                "button",
                {
                  onClick: f,
                  style: {
                    border: `1px solid ${r.border}`,
                    background: r.controlBgActive,
                    color: r.text,
                    borderRadius: r.controlBorderRadius,
                    padding: "6px 10px",
                    fontSize: 11,
                    fontWeight: 600,
                    cursor: "pointer"
                  },
                  children: "Insert Diagram"
                }
              )
            ] })
          ] })
        ]
      }
    ),
    document.body
  );
}
const ku = [
  { key: "select", label: "Select", shortcut: "S", num: "" },
  { key: "hand", label: "Hand", shortcut: "P", num: "" },
  { key: "draw", label: "Draw", shortcut: "D", num: "" },
  { key: "shape", label: "Shape", shortcut: "G", num: "" },
  { key: "text", label: "Text", shortcut: "T", num: "" },
  { key: "note", label: "Note", shortcut: "B", num: "" },
  { key: "sticky", label: "Sticky", shortcut: "Y", num: "" },
  { key: "frame", label: "Frame", shortcut: "F", num: "" },
  { key: "erase", label: "Eraser", shortcut: "E", num: "" },
  { key: "laser", label: "Laser", shortcut: "Z", num: "" }
], go = {
  border: "none",
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, Ft = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function xo({ name: t, size: e = 18 }) {
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "select" && /* @__PURE__ */ u("path", { d: "M6 2v17l4-4.5L13.5 21l2.5-1.5L12.5 13H18z", fill: "currentColor" }),
    t === "draw" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M17 3l4 4L7.5 20.5 2 22l1.5-5.5z", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M15 5l4 4", ...Ft })
    ] }),
    t === "shape" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...Ft }),
    t === "text" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M7 4h10", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M12 4v16", ...Ft })
    ] }),
    t === "note" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M4 3h16v14l-4 4H4z", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M16 17v4l4-4z", fill: "currentColor", opacity: 0.4 })
    ] }),
    t === "sticky" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "1", fill: "currentColor", opacity: 0.15, ...Ft }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "9", x2: "17", y2: "9", ...Ft, opacity: 0.5 }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "13", x2: "14", y2: "13", ...Ft, opacity: 0.5 })
    ] }),
    t === "frame" && /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2", ...Ft, strokeDasharray: "4,2" }),
    t === "hand" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M8 13V5.5a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M11 5.5v-2a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M14 5.5a1.5 1.5 0 0 1 3 0V12", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M17 7.5a1.5 1.5 0 0 1 3 0V16a6 6 0 0 1-6 6h-2a6 6 0 0 1-6-6V9.5a1.5 1.5 0 0 1 3 0", ...Ft })
    ] }),
    t === "erase" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M20 20H9L3 14l9.5-9.5 8 8L16 17", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M12.5 4.5l8 8", ...Ft })
    ] }),
    t === "laser" && /* @__PURE__ */ u("circle", { cx: "12", cy: "12", r: "4", fill: "currentColor", opacity: 0.9 }),
    t === "lasso" && /* @__PURE__ */ u("path", { d: "M18 4c-3 0-5 2-8 5S5 14 4 16c-1 3 1 4 3 4s4-2 6-4 4-4 6-5 3-2 3-4-1-3-3-3z", ...Ft, strokeDasharray: "3,2" }),
    t === "undo" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...Ft, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...Ft, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...Ft, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...Ft, fill: "none" })
    ] }),
    t === "print" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M6 9V3h12v6", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M6 18H4a1 1 0 0 1-1-1v-5a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-2", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "6", y: "14", width: "12", height: "7", rx: "1", ...Ft })
    ] }),
    t === "fit" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...Ft })
    ] }),
    t === "paper" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "4", y: "2", width: "16", height: "20", rx: "1", ...Ft }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "7", x2: "16", y2: "7", ...Ft, opacity: 0.4 }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "11", x2: "16", y2: "11", ...Ft, opacity: 0.4 }),
      /* @__PURE__ */ u("line", { x1: "8", y1: "15", x2: "13", y2: "15", ...Ft, opacity: 0.4 })
    ] }),
    t === "template" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "13", y: "3", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "3", y: "13", width: "8", height: "8", rx: "1", ...Ft }),
      /* @__PURE__ */ u("rect", { x: "13", y: "13", width: "8", height: "8", rx: "1", ...Ft })
    ] }),
    t === "library" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M8 7h6", ...Ft, opacity: 0.5 }),
      /* @__PURE__ */ u("path", { d: "M8 11h4", ...Ft, opacity: 0.5 })
    ] }),
    t === "gif" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "4", width: "20", height: "16", rx: "2", ...Ft }),
      /* @__PURE__ */ u("text", { x: "12", y: "14.5", textAnchor: "middle", fontSize: "7", fontWeight: "700", fill: "currentColor", stroke: "none", children: "GIF" })
    ] }),
    t === "mermaid" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "4", width: "18", height: "14", rx: "2", ...Ft }),
      /* @__PURE__ */ u("path", { d: "M6 8l2.6 3 2.1-2 2.2 3 2.2-2.5L18 13", ...Ft }),
      /* @__PURE__ */ u("circle", { cx: "6", cy: "8", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "10.7", cy: "9", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "14.9", cy: "9.5", r: "1.1", fill: "currentColor", stroke: "none" }),
      /* @__PURE__ */ u("circle", { cx: "18", cy: "13", r: "1.1", fill: "currentColor", stroke: "none" })
    ] })
  ] });
}
const vu = {
  light: "Light",
  dark: "Dark",
  textured: "Textured"
};
function Su({
  engine: t,
  background: e
}) {
  const o = qt(), [n, r] = et(!1), s = at(null), i = at(null);
  yt(() => {
    if (!n) return;
    const c = (h) => {
      i.current && !i.current.contains(h.target) && s.current && !s.current.contains(h.target) && r(!1);
    };
    return document.addEventListener("pointerdown", c), () => document.removeEventListener("pointerdown", c);
  }, [n]);
  const l = Xo.find((c) => c.key === e) ?? Xo[1], a = n && s.current ? (() => {
    const c = s.current.getBoundingClientRect();
    return Oe(
      /* @__PURE__ */ u(
        "div",
        {
          ref: i,
          style: {
            position: "fixed",
            left: c.right + 8,
            top: c.top,
            background: o.panelBg,
            border: `1px solid ${o.border}`,
            borderRadius: o.panelBorderRadius,
            padding: 8,
            zIndex: 99999,
            boxShadow: o.panelShadow,
            width: 180,
            maxHeight: 400,
            overflowY: "auto"
          },
          onPointerDown: (h) => h.stopPropagation(),
          children: ["light", "dark", "textured"].map((h) => {
            const p = Xo.filter((d) => d.group === h);
            return p.length === 0 ? null : /* @__PURE__ */ k("div", { style: { marginBottom: 6 }, children: [
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    fontSize: 10,
                    fontWeight: 600,
                    color: o.textMuted,
                    textTransform: "uppercase",
                    letterSpacing: "0.05em",
                    padding: "4px 6px 2px"
                  },
                  children: vu[h]
                }
              ),
              p.map((d) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    t.setBoardBackground(d.key), r(!1);
                  },
                  style: {
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                    width: "100%",
                    padding: "5px 6px",
                    border: "none",
                    borderRadius: o.controlBorderRadius,
                    background: e === d.key ? o.controlBgActive : "transparent",
                    color: o.text,
                    cursor: "pointer",
                    fontSize: 12,
                    textAlign: "left"
                  },
                  children: [
                    /* @__PURE__ */ u(
                      "span",
                      {
                        style: {
                          width: 18,
                          height: 18,
                          borderRadius: 3,
                          background: d.swatchColor,
                          border: `1.5px solid ${o.border}`,
                          flexShrink: 0
                        }
                      }
                    ),
                    d.label
                  ]
                },
                d.key
              ))
            ] }, h);
          })
        }
      ),
      document.body
    );
  })() : null;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ k(
      "button",
      {
        ref: s,
        title: "Paper type",
        onClick: () => r((c) => !c),
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: o.controlBorderRadius,
          background: n ? o.controlBgActive : "transparent",
          color: o.text,
          position: "relative"
        },
        children: [
          /* @__PURE__ */ u(xo, { name: "paper" }),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                position: "absolute",
                bottom: 4,
                right: 4,
                width: 8,
                height: 8,
                borderRadius: "50%",
                background: l.swatchColor,
                border: `1px solid ${o.border}`
              }
            }
          )
        ]
      }
    ),
    a
  ] });
}
function Mu({ engine: t }) {
  const e = qt(), [o, n] = et(!1), r = at(null), s = at(null);
  yt(() => {
    if (!o) return;
    const l = (a) => {
      s.current && !s.current.contains(a.target) && r.current && !r.current.contains(a.target) && n(!1);
    };
    return document.addEventListener("pointerdown", l), () => document.removeEventListener("pointerdown", l);
  }, [o]);
  const i = o && r.current ? (() => {
    const l = r.current.getBoundingClientRect();
    return Oe(
      /* @__PURE__ */ k(
        "div",
        {
          ref: s,
          style: {
            position: "fixed",
            left: l.right + 8,
            top: l.top,
            background: e.panelBg,
            border: `1px solid ${e.border}`,
            borderRadius: 8,
            padding: 8,
            zIndex: 99999,
            boxShadow: "0 4px 16px rgba(0,0,0,0.15)",
            width: 180
          },
          onPointerDown: (a) => a.stopPropagation(),
          children: [
            /* @__PURE__ */ u(
              "div",
              {
                style: {
                  fontSize: 10,
                  fontWeight: 600,
                  color: e.textMuted,
                  textTransform: "uppercase",
                  letterSpacing: "0.05em",
                  padding: "4px 6px 2px"
                },
                children: "Templates"
              }
            ),
            Ti.map((a) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => {
                  const c = typeof window < "u" ? window : void 0;
                  if (!c) return;
                  const h = c.innerWidth / 2, p = c.innerHeight / 2, d = No(t.viewport, h, p);
                  t.applyTemplate(a.id, d.x, d.y), n(!1);
                },
                style: {
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  width: "100%",
                  padding: "6px 6px",
                  border: "none",
                  borderRadius: e.controlBorderRadius,
                  background: "transparent",
                  color: e.text,
                  cursor: "pointer",
                  fontSize: 12,
                  textAlign: "left"
                },
                onMouseEnter: (c) => {
                  c.currentTarget.style.background = e.controlBgActive;
                },
                onMouseLeave: (c) => {
                  c.currentTarget.style.background = "transparent";
                },
                children: a.label
              },
              a.id
            ))
          ]
        }
      ),
      document.body
    );
  })() : null;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: r,
        title: "Templates",
        onClick: () => n((l) => !l),
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(xo, { name: "template" })
      }
    ),
    i
  ] });
}
function Cu({ engine: t }) {
  const e = qt(), [o, n] = et(!1), [r, s] = et(!1), i = at(null), [l, a] = et(null), c = st(() => {
    n((d) => (!d && i.current && a(i.current.getBoundingClientRect()), !d));
  }, []), h = st(() => n(!1), []), p = st(() => {
    s(!0);
  }, []);
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: i,
        title: "Libraries",
        onClick: c,
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(xo, { name: "library" })
      }
    ),
    /* @__PURE__ */ u(
      lh,
      {
        engine: t,
        open: o,
        onClose: h,
        triggerRect: l,
        onBrowseDirectory: p
      }
    ),
    r && /* @__PURE__ */ u(
      ru,
      {
        onClose: () => s(!1),
        onInstalled: () => {
          n(!1), setTimeout(() => {
            i.current && a(i.current.getBoundingClientRect()), n(!0);
          }, 100);
        }
      }
    )
  ] });
}
function zu({ engine: t, baseUrl: e }) {
  const o = qt(), [n, r] = et(!1), s = at(null), [i, l] = et(null), a = st(() => {
    r((h) => (!h && s.current && l(s.current.getBoundingClientRect()), !h));
  }, []), c = st(() => r(!1), []);
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: s,
        title: "GIF Search",
        onClick: a,
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: o.controlBorderRadius,
          background: n ? o.controlBgActive : "transparent",
          color: o.text
        },
        children: /* @__PURE__ */ u(xo, { name: "gif" })
      }
    ),
    /* @__PURE__ */ u(
      uh,
      {
        engine: t,
        open: n,
        onClose: c,
        triggerRect: i,
        baseUrl: e
      }
    )
  ] });
}
function Iu({ engine: t }) {
  const e = qt(), [o, n] = et(!1), r = at(null), [s, i] = et(null), l = st(() => {
    n((c) => (!c && r.current && i(r.current.getBoundingClientRect()), !c));
  }, []), a = st(() => n(!1), []);
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      "button",
      {
        ref: r,
        title: "Mermaid Sketch",
        onClick: l,
        style: {
          ...go,
          width: 40,
          height: 40,
          borderRadius: e.controlBorderRadius,
          background: o ? e.controlBgActive : "transparent",
          color: e.text
        },
        children: /* @__PURE__ */ u(xo, { name: "mermaid" })
      }
    ),
    /* @__PURE__ */ u(
      wu,
      {
        engine: t,
        open: o,
        onClose: a,
        triggerRect: s
      }
    )
  ] });
}
function Tu({ engine: t, gifApiBaseUrl: e }) {
  const o = qt(), [n, r] = et(t.mode), [s, i] = et(t.boardBackground), [l, a] = et(t.lassoSelect);
  return yt(() => {
    const c = () => r(t.mode), h = () => i(t.boardBackground), p = () => a(t.lassoSelect);
    return t.on("mode", c), t.on("background", h), t.on("lassoToggle", p), () => {
      t.off("mode", c), t.off("background", h), t.off("lassoToggle", p);
    };
  }, [t]), /* @__PURE__ */ k(
    "div",
    {
      "data-sb-toolbar": !0,
      style: {
        width: dn,
        height: "100%",
        flexShrink: 0,
        background: o.toolbarBg,
        borderRight: `1px solid ${o.border}`,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        padding: "8px 0",
        gap: 4
      },
      children: [
        ku.map((c) => {
          const h = n === c.key && !(c.key === "select" && l);
          return /* @__PURE__ */ k(
            "button",
            {
              title: `${c.label} (${c.shortcut}${c.num ? ` / ${c.num}` : ""})`,
              onClick: () => {
                l && (t.toggleLassoSelect(), a(!1)), t.setMode(c.key);
              },
              style: {
                ...go,
                width: 40,
                height: 40,
                borderRadius: o.controlBorderRadius,
                background: h ? o.controlBgActive : "transparent",
                color: o.text,
                position: "relative"
              },
              children: [
                /* @__PURE__ */ u(xo, { name: c.key }),
                /* @__PURE__ */ u(
                  "span",
                  {
                    style: {
                      position: "absolute",
                      bottom: 2,
                      right: 2,
                      fontSize: 8,
                      lineHeight: 1,
                      color: o.textMuted,
                      fontWeight: 500,
                      pointerEvents: "none"
                    },
                    children: c.num || c.shortcut
                  }
                )
              ]
            },
            c.key
          );
        }),
        /* @__PURE__ */ u("div", { style: { width: 28, height: 1, background: o.separator, margin: "8px 0" } }),
        /* @__PURE__ */ k(
          "button",
          {
            title: "Lasso Select (L)",
            onClick: () => {
              l ? (t.toggleLassoSelect(), a(!1)) : (t.setMode("select"), t.lassoSelect || t.toggleLassoSelect(), a(!0));
            },
            style: {
              ...go,
              width: 40,
              height: 40,
              borderRadius: o.controlBorderRadius,
              background: l ? o.controlBgActive : "transparent",
              color: o.text,
              position: "relative"
            },
            children: [
              /* @__PURE__ */ u(xo, { name: "lasso" }),
              /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    position: "absolute",
                    bottom: 2,
                    right: 2,
                    fontSize: 8,
                    lineHeight: 1,
                    color: o.textMuted,
                    fontWeight: 500,
                    pointerEvents: "none"
                  },
                  children: "L"
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ u("div", { style: { width: 28, height: 1, background: o.separator, margin: "8px 0" } }),
        /* @__PURE__ */ u(Su, { engine: t, background: s }),
        /* @__PURE__ */ u(Mu, { engine: t }),
        /* @__PURE__ */ u(Cu, { engine: t }),
        /* @__PURE__ */ u(Iu, { engine: t }),
        e && /* @__PURE__ */ u(zu, { engine: t, baseUrl: e })
      ]
    }
  );
}
const Pu = /* @__PURE__ */ new Set(["shape", "draw", "text", "image", "content", "frame", "sticky"]), Ru = /* @__PURE__ */ new Set(["text", "image", "content", "frame"]);
function fi(t) {
  return t.data.opacity ?? 1;
}
function Ao(t, e) {
  return t.data[e];
}
function Au(t) {
  const e = {}, o = t.filter((r) => Pu.has(r.type));
  if (o.length > 0) {
    const r = fi(o[0]), s = o.every((i) => fi(i) === r);
    e.opacity = s ? r : "mixed";
  }
  const n = t.filter((r) => Ru.has(r.type));
  if (n.length > 0) {
    const r = Ao(n[0], "borderColor"), s = n.every(
      (h) => Ao(h, "borderColor") === r
    );
    e.borderColor = s ? r ?? null : "mixed";
    const i = Ao(n[0], "borderWidth") ?? 1, l = n.every(
      (h) => (Ao(h, "borderWidth") ?? 1) === i
    );
    e.borderWidth = l ? i : "mixed";
    const a = Ao(n[0], "borderStyle") ?? "solid", c = n.every(
      (h) => (Ao(h, "borderStyle") ?? "solid") === a
    );
    e.borderStyle = c ? a : "mixed";
  }
  return e;
}
function Lu(t) {
  const [e, o] = et(t.mode), [n, r] = et(new Set(t.selection)), [, s] = et(0);
  if (yt(() => {
    const h = () => o(t.mode), p = () => {
      r(new Set(t.selection)), s((f) => f + 1);
    }, d = () => s((f) => f + 1);
    return t.on("mode", h), t.on("selection", p), t.on("change", d), () => {
      t.off("mode", h), t.off("selection", p), t.off("change", d);
    };
  }, [t]), n.size === 0)
    return e === "draw" || e === "shape" || e === "text" ? { target: { kind: "tool", mode: e }, commonProps: {} } : { target: { kind: "none" }, commonProps: {} };
  const i = [];
  for (const h of n) {
    const p = t.getNode(h);
    p && i.push(p);
  }
  if (i.length === 0)
    return { target: { kind: "none" }, commonProps: {} };
  if (i.length === 1)
    return { target: { kind: "single", node: i[0] }, commonProps: {} };
  const l = /* @__PURE__ */ new Map();
  for (const h of i) {
    const p = l.get(h.type);
    p ? p.push(h) : l.set(h.type, [h]);
  }
  const a = [];
  for (const [h, p] of l)
    a.push({ type: h, nodes: p });
  const c = Au(i);
  return {
    target: { kind: "multi", nodes: i, typeGroups: a },
    commonProps: c
  };
}
const jn = Ii(null);
function Re(t, e) {
  const o = Nn(jn);
  return st(
    (n) => {
      if (o && o.length > 1) {
        const r = o.map((s) => ({
          id: s.id,
          patch: {
            data: { ...s.data, ...n }
          }
        }));
        t.batchUpdateWithHistory(r);
      } else
        t.updateNodeWithHistory(e.id, {
          data: { ...e.data, ...n }
        });
    },
    [t, e, o]
  );
}
function ke({
  value: t,
  onChange: e,
  mixed: o
}) {
  const n = qt(), r = o || t === void 0 ? 100 : Math.round(t * 100);
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Opacity" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "range",
        min: 0,
        max: 100,
        value: r,
        onChange: (s) => e(parseInt(s.target.value) / 100),
        style: { flex: 1, accentColor: n.accentColor }
      }
    ),
    /* @__PURE__ */ u("span", { style: { width: 28, textAlign: "right", fontSize: 10, color: o ? n.textFaint : n.text }, children: o ? "--" : r })
  ] });
}
const Eu = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
function fe({
  label: t,
  palettes: e,
  value: o,
  onChange: n,
  allowNull: r,
  mixed: s
}) {
  const i = qt(), [l, a] = et(""), [c, h] = et(0), [p, d] = et(!1), f = at(null), m = at(null), [y, b] = et(null), [g, x] = et("bottom"), S = e[c] ?? e[0], M = o == null ? void 0 : o.toLowerCase();
  yt(() => {
    if (!p) return;
    const I = (R) => {
      f.current && !f.current.contains(R.target) && d(!1);
    };
    return document.addEventListener("mousedown", I), () => document.removeEventListener("mousedown", I);
  }, [p]), yt(() => {
    if (!p) return;
    const I = () => {
      const R = m.current;
      if (!R) return;
      const K = R.getBoundingClientRect(), lt = e.length * 30 + 10, O = window.innerHeight - K.bottom, ot = K.top, Q = O < lt && ot > O;
      x(Q ? "top" : "bottom"), b({
        top: Q ? K.top - 4 : K.bottom + 4,
        left: K.right
      });
    };
    return I(), window.addEventListener("resize", I), window.addEventListener("scroll", I, !0), () => {
      window.removeEventListener("resize", I), window.removeEventListener("scroll", I, !0);
    };
  }, [p]);
  const z = () => {
    const I = l.trim();
    if (!I) return;
    const R = I.startsWith("#") ? I : `#${I}`;
    Eu.test(R) && (n(R), a(""));
  }, W = e.some(
    (I) => I.colors.some((R) => R.toLowerCase() === M)
  );
  return /* @__PURE__ */ k("div", { style: { display: "flex", flexDirection: "column", gap: 3 }, children: [
    /* @__PURE__ */ k("div", { style: { ...Lt, margin: 0, flexWrap: "wrap", gap: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: i.textMuted }, children: t }),
      r && /* @__PURE__ */ u(
        "button",
        {
          onClick: () => n(null),
          title: "None",
          style: {
            ...Zt,
            width: 20,
            height: 20,
            background: "transparent",
            border: !s && o == null ? `2px solid ${i.swatchBorderActive}` : `2px solid ${i.textDisabled}`,
            borderRadius: "50%",
            position: "relative",
            overflow: "hidden"
          },
          children: /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                width: "140%",
                height: 2,
                background: i.error,
                transform: "rotate(-45deg)",
                top: "50%",
                left: "-20%"
              }
            }
          )
        }
      ),
      S.colors.map((I) => {
        const R = !s && M === I.toLowerCase();
        return /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n(I),
            style: {
              ...Zt,
              width: 20,
              height: 20,
              background: I,
              border: R ? `2px solid ${i.swatchBorderActive}` : "2px solid transparent",
              borderRadius: "50%"
            }
          },
          I
        );
      }),
      o && !W && !s && /* @__PURE__ */ u(
        "div",
        {
          style: {
            width: 20,
            height: 20,
            borderRadius: "50%",
            background: o,
            border: `2px solid ${i.swatchBorderActive}`,
            flexShrink: 0
          }
        }
      ),
      s && /* @__PURE__ */ u("span", { style: { fontSize: 9, color: i.textMuted, fontStyle: "italic" }, children: "Mixed" }),
      e.length > 1 && /* @__PURE__ */ k("div", { ref: m, style: { position: "relative", marginLeft: "auto" }, children: [
        /* @__PURE__ */ k(
          "button",
          {
            onClick: () => d((I) => !I),
            title: "Switch palette",
            style: {
              ...Zt,
              height: 20,
              padding: "0 5px",
              background: i.controlBg,
              color: i.textMuted,
              fontSize: 9,
              borderRadius: i.controlBorderRadius,
              display: "flex",
              alignItems: "center",
              gap: 2
            },
            children: [
              S.name,
              /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: p ? "▲" : "▼" })
            ]
          }
        ),
        p && y && Oe(
          /* @__PURE__ */ u(
            "div",
            {
              ref: f,
              style: {
                position: "fixed",
                top: y.top,
                left: y.left,
                transform: g === "top" ? "translate(-100%, -100%)" : "translateX(-100%)",
                background: i.panelBg,
                border: `1px solid ${i.border}`,
                borderRadius: i.panelBorderRadius,
                padding: 4,
                zIndex: 2e4,
                display: "flex",
                flexDirection: "column",
                gap: 2,
                minWidth: 120,
                boxShadow: i.panelShadow
              },
              children: e.map((I, R) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    h(R), d(!1);
                  },
                  style: {
                    ...Zt,
                    height: 28,
                    padding: "0 8px",
                    background: R === c ? i.controlBgActive : "transparent",
                    color: i.text,
                    fontSize: 10,
                    borderRadius: i.controlBorderRadius,
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    width: "100%",
                    justifyContent: "flex-start"
                  },
                  children: [
                    /* @__PURE__ */ u("span", { style: { display: "flex", gap: 2 }, children: I.colors.slice(0, 6).map((K) => /* @__PURE__ */ u(
                      "span",
                      {
                        style: {
                          width: 10,
                          height: 10,
                          borderRadius: "50%",
                          background: K,
                          display: "inline-block"
                        }
                      },
                      K
                    )) }),
                    /* @__PURE__ */ u("span", { children: I.name })
                  ]
                },
                I.name
              ))
            }
          ),
          document.body
        )
      ] })
    ] }),
    /* @__PURE__ */ u("div", { style: { display: "flex", alignItems: "center", gap: 4, paddingLeft: 52 }, children: /* @__PURE__ */ u(
      "input",
      {
        type: "text",
        value: l,
        onChange: (I) => a(I.target.value),
        onKeyDown: (I) => {
          I.key === "Enter" && z();
        },
        onBlur: z,
        placeholder: o ?? "#000000",
        style: {
          width: 72,
          height: 20,
          background: i.controlBg,
          border: `1px solid ${i.border}`,
          borderRadius: i.controlBorderRadius,
          color: i.text,
          fontSize: 10,
          fontFamily: "monospace",
          padding: "0 6px",
          outline: "none"
        }
      }
    ) })
  ] });
}
function Vo({
  label: t,
  value: e,
  onChange: o,
  mixed: n
}) {
  const r = qt();
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Et, color: r.textMuted }, children: t }),
    $h.map((s) => /* @__PURE__ */ u(
      "button",
      {
        title: s.label,
        onClick: () => o(s.key),
        style: {
          ...Zt,
          width: 36,
          height: 28,
          background: !n && e === s.key ? r.controlBgActive : r.controlBg,
          borderRadius: r.controlBorderRadius
        },
        children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
          "line",
          {
            x1: 2,
            y1: 6,
            x2: 22,
            y2: 6,
            stroke: r.text,
            strokeWidth: 2,
            strokeDasharray: s.dash
          }
        ) })
      },
      s.key
    ))
  ] });
}
function Ko({
  label: t,
  widths: e = tu,
  value: o,
  onChange: n,
  mixed: r
}) {
  const s = qt();
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Et, color: s.textMuted }, children: t }),
    /* @__PURE__ */ u("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", flex: 1, minWidth: 0 }, children: e.map((i) => /* @__PURE__ */ u(
      "button",
      {
        title: `${i}px`,
        onClick: () => n(i),
        style: {
          ...Zt,
          width: 30,
          height: 24,
          background: !r && o === i ? s.controlBgActive : s.controlBg,
          borderRadius: s.controlBorderRadius
        },
        children: /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 16,
              height: Math.max(i, 1),
              background: s.text,
              borderRadius: i / 2
            }
          }
        )
      },
      i
    )) })
  ] });
}
function hn({
  borderColor: t,
  borderStyle: e,
  borderWidth: o,
  mixed: n,
  onChange: r
}) {
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      fe,
      {
        label: "Border",
        palettes: we,
        value: t,
        onChange: (s) => r("borderColor", s ?? void 0),
        allowNull: !0,
        mixed: n == null ? void 0 : n.color
      }
    ),
    (t || (n == null ? void 0 : n.color)) && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u(
        Vo,
        {
          label: "Style",
          value: e ?? "solid",
          onChange: (s) => r("borderStyle", s),
          mixed: n == null ? void 0 : n.style
        }
      ),
      /* @__PURE__ */ u(
        Ko,
        {
          label: "Width",
          value: o ?? 1,
          onChange: (s) => r("borderWidth", s),
          mixed: n == null ? void 0 : n.width
        }
      )
    ] })
  ] });
}
const mr = /* @__PURE__ */ new Map();
function he({
  title: t,
  defaultOpen: e = !0,
  variant: o = "sub",
  open: n,
  onToggle: r,
  persistKey: s,
  children: i
}) {
  const l = qt(), [a, c] = et(() => s && mr.has(s) ? !!mr.get(s) : e), h = n ?? a, p = o === "group", d = at(null), [f, m] = et(0);
  return yt(() => {
    !s || n !== void 0 || mr.set(s, h);
  }, [s, n, h]), Hr(() => {
    const y = d.current;
    if (!y) return;
    const b = () => m(y.scrollHeight);
    b();
    const g = new ResizeObserver(() => b());
    return g.observe(y), () => g.disconnect();
  }, [i]), /* @__PURE__ */ k(
    "section",
    {
      style: {
        border: `1px solid ${l.border}`,
        borderRadius: l.controlBorderRadius,
        background: p ? l.panelBg : l.controlBg,
        overflow: "hidden",
        flexShrink: 0,
        alignSelf: "stretch"
      },
      children: [
        /* @__PURE__ */ k(
          "button",
          {
            type: "button",
            onClick: () => {
              r ? r() : c((y) => !y);
            },
            style: {
              width: "100%",
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              background: "transparent",
              border: "none",
              color: p ? l.textMuted : l.textSecondary,
              fontSize: 10,
              fontWeight: 700,
              letterSpacing: "0.03em",
              textTransform: "uppercase",
              padding: "8px 10px",
              cursor: "pointer"
            },
            children: [
              /* @__PURE__ */ u("span", { children: t }),
              /* @__PURE__ */ u(
                "span",
                {
                  style: {
                    color: l.textMuted,
                    display: "inline-block",
                    transform: h ? "rotate(90deg)" : "rotate(0deg)",
                    transition: "transform 170ms ease",
                    lineHeight: 1
                  },
                  children: "▸"
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              maxHeight: h ? f : 0,
              opacity: h ? 1 : 0,
              transition: "max-height 200ms ease, opacity 140ms ease",
              overflow: "hidden",
              pointerEvents: h ? "auto" : "none"
            },
            children: /* @__PURE__ */ u(
              "div",
              {
                ref: d,
                style: {
                  padding: "8px 10px 10px",
                  borderTop: `1px solid ${l.border}`,
                  display: "flex",
                  flexDirection: "column",
                  gap: 8,
                  background: p ? "transparent" : l.controlBg
                },
                children: i
              }
            )
          }
        )
      ]
    }
  );
}
function ps({ style: t }) {
  const e = qt();
  return t === "hachure" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: e.text, strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: e.text, strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: e.text, strokeWidth: 1.5 })
  ] }) : t === "cross-hatch" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 2, y1: 2, x2: 8, y2: 14, stroke: e.text, strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 2, x2: 14, y2: 14, stroke: e.text, strokeWidth: 1.2 })
  ] }) : /* @__PURE__ */ u("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: /* @__PURE__ */ u("rect", { x: 2, y: 2, width: 16, height: 12, fill: e.text, rx: 2 }) });
}
const Du = /* @__PURE__ */ k("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [
  /* @__PURE__ */ u("circle", { cx: "11", cy: "11", r: "8" }),
  /* @__PURE__ */ u("path", { d: "m21 21-4.35-4.35" })
] });
function Vn({
  value: t,
  onChange: e,
  fontsInScene: o,
  triggerStyle: n
}) {
  var x, S;
  const r = qt(), [s, i] = et(!1), [l, a] = et(""), c = at(null), h = at(null), [p, d] = et(null), f = l.trim().toLowerCase(), m = Kt(
    () => o.filter((M) => M.toLowerCase().includes(f)),
    [o, f]
  ), y = Kt(
    () => Cn.filter(
      (M) => !o.includes(M.key) && (M.key.toLowerCase().includes(f) || M.label.toLowerCase().includes(f))
    ),
    [o, f]
  );
  yt(() => {
    if (!s || !h.current) return;
    const M = h.current.getBoundingClientRect(), z = 260, W = 16;
    let I = M.left;
    I + z > window.innerWidth - W && (I = window.innerWidth - z - W), I < W && (I = W), d({ top: M.bottom + 4, left: I });
  }, [s]), yt(() => {
    var W;
    if (!s) return;
    const M = (I) => {
      var lt, O;
      const R = I.target;
      if ((lt = c.current) != null && lt.contains(R)) return;
      const Z = (((O = c.current) == null ? void 0 : O.ownerDocument) ?? document).getElementById("font-picker-popover");
      Z != null && Z.contains(R) || i(!1);
    }, z = ((W = c.current) == null ? void 0 : W.ownerDocument) ?? document;
    return z.addEventListener("mousedown", M), () => z.removeEventListener("mousedown", M);
  }, [s]);
  const b = (M) => {
    e(M), i(!1), a("");
  }, g = (M, z) => {
    const W = (z == null ? void 0 : z.label) ?? M, I = z == null ? void 0 : z.category, R = t === M;
    return /* @__PURE__ */ k(
      "button",
      {
        type: "button",
        onClick: () => b(M),
        style: {
          display: "flex",
          alignItems: "center",
          gap: 10,
          width: "100%",
          padding: "8px 12px",
          border: "none",
          background: R ? "rgba(139, 92, 246, 0.15)" : "transparent",
          color: "#1e1e2e",
          cursor: "pointer",
          textAlign: "left",
          fontFamily: Je(M),
          fontSize: 14,
          borderRadius: 6
        },
        onMouseEnter: (K) => {
          R || (K.currentTarget.style.background = "rgba(0,0,0,0.05)");
        },
        onMouseLeave: (K) => {
          R || (K.currentTarget.style.background = "transparent");
        },
        children: [
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                width: 24,
                flexShrink: 0,
                fontSize: 12,
                color: "#64748b",
                fontFamily: "sans-serif"
              },
              children: Dl(I)
            }
          ),
          /* @__PURE__ */ u("span", { style: { flex: 1, overflow: "hidden", textOverflow: "ellipsis" }, children: W })
        ]
      },
      M
    );
  };
  return /* @__PURE__ */ k("div", { ref: c, style: { position: "relative", flex: 1, minWidth: 0 }, children: [
    /* @__PURE__ */ k(
      "button",
      {
        ref: h,
        type: "button",
        onClick: () => i((M) => !M),
        style: {
          display: "flex",
          alignItems: "center",
          width: "100%",
          height: 28,
          padding: "0 8px",
          background: r.controlBg,
          color: r.text,
          border: `1px solid ${r.separator}`,
          borderRadius: r.controlBorderRadius,
          fontSize: 11,
          fontFamily: Je(t),
          cursor: "pointer",
          textAlign: "left",
          justifyContent: "space-between",
          ...n
        },
        children: [
          /* @__PURE__ */ u("span", { style: { overflow: "hidden", textOverflow: "ellipsis" }, children: ((x = Cn.find((M) => M.key === t)) == null ? void 0 : x.label) ?? t }),
          /* @__PURE__ */ u(
            "span",
            {
              style: {
                flexShrink: 0,
                opacity: 0.7,
                transform: s ? "rotate(180deg)" : "none",
                transition: "transform 0.15s"
              },
              children: "▼"
            }
          )
        ]
      }
    ),
    s && p && Oe(
      /* @__PURE__ */ k(
        "div",
        {
          id: "font-picker-popover",
          style: {
            position: "fixed",
            top: p.top,
            left: p.left,
            width: 260,
            maxHeight: Math.min(320, window.innerHeight - p.top - 16),
            background: "#fff",
            borderRadius: 12,
            boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
            zIndex: 1e4,
            overflow: "hidden",
            display: "flex",
            flexDirection: "column"
          },
          children: [
            /* @__PURE__ */ k(
              "div",
              {
                style: {
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  padding: "10px 12px",
                  borderBottom: "1px solid #eee",
                  background: "#fafafa"
                },
                children: [
                  /* @__PURE__ */ u("span", { style: { color: "#64748b", display: "flex" }, children: Du }),
                  /* @__PURE__ */ u(
                    "input",
                    {
                      type: "text",
                      placeholder: "Quick search",
                      value: l,
                      onChange: (M) => a(M.target.value),
                      autoFocus: !0,
                      style: {
                        flex: 1,
                        border: "none",
                        background: "transparent",
                        fontSize: 13,
                        outline: "none",
                        color: "#1e1e2e"
                      }
                    }
                  )
                ]
              }
            ),
            /* @__PURE__ */ k("div", { style: { overflowY: "auto", padding: 8, flex: 1 }, children: [
              m.length > 0 && /* @__PURE__ */ k("div", { style: { marginBottom: 12 }, children: [
                /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      fontSize: 10,
                      fontWeight: 600,
                      color: "#64748b",
                      textTransform: "uppercase",
                      letterSpacing: "0.05em",
                      marginBottom: 6,
                      paddingLeft: 4
                    },
                    children: "In this scene"
                  }
                ),
                m.map((M) => g(M, Cn.find((z) => z.key === M)))
              ] }),
              /* @__PURE__ */ k("div", { children: [
                /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      fontSize: 10,
                      fontWeight: 600,
                      color: "#64748b",
                      textTransform: "uppercase",
                      letterSpacing: "0.05em",
                      marginBottom: 6,
                      paddingLeft: 4
                    },
                    children: "Available fonts"
                  }
                ),
                y.length > 0 ? y.map((M) => g(M.key, M)) : /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      padding: "12px",
                      fontSize: 12,
                      color: "#94a3b8"
                    },
                    children: l ? "No fonts match your search" : "All fonts are in use"
                  }
                )
              ] })
            ] })
          ]
        }
      ),
      (((S = c.current) == null ? void 0 : S.ownerDocument) ?? document).body
    )
  ] });
}
function fs({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round",
    fill: "none"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "sharp" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", ...o }),
    t === "round" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "4", ...o })
  ] });
}
const Wu = [
  { label: "S", size: 14 },
  { label: "M", size: 20 },
  { label: "L", size: 28 },
  { label: "XL", size: 36 }
], Fu = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
];
function Bu({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
function co(t, e) {
  if (t.length < 2) return !1;
  const o = e(t[0]);
  return !t.every((n) => e(n) === o);
}
function Nu({ engine: t, node: e, fontsInScene: o }) {
  const n = qt(), r = Re(t, e), s = Nn(jn) ?? [e], { data: i } = e, l = i.fill ?? null, a = i.fillStyle ?? "hachure", c = i.strokeStyle ?? "solid", h = co(s, (g) => g.data.stroke), p = co(s, (g) => g.data.fill ?? null), d = co(s, (g) => g.data.fillStyle ?? "hachure"), f = co(s, (g) => g.data.strokeStyle ?? "solid"), m = co(s, (g) => g.data.strokeWidth), y = co(s, (g) => g.data.roughness), b = co(s, (g) => g.data.opacity ?? 1);
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ k(he, { title: "Structure", persistKey: "shape.structure", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Shape" }),
        Fu.map((g) => /* @__PURE__ */ u(
          "button",
          {
            title: g.label,
            onClick: () => r({ shape: g.key }),
            style: {
              ...Zt,
              width: 28,
              height: 28,
              background: i.shape === g.key ? n.controlBgActive : n.controlBg,
              color: n.text,
              borderRadius: n.controlBorderRadius
            },
            children: /* @__PURE__ */ u(Bu, { name: g.key })
          },
          g.key
        ))
      ] }),
      (i.shape === "rect" || i.shape === "diamond") && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Edges" }),
        [
          { key: "sharp", label: "Sharp" },
          { key: "round", label: "Round" }
        ].map((g) => /* @__PURE__ */ u(
          "button",
          {
            title: g.label,
            onClick: () => r({ edgeStyle: g.key === "sharp" ? void 0 : g.key }),
            style: {
              ...Zt,
              width: 28,
              height: 28,
              background: (i.edgeStyle ?? "sharp") === g.key ? n.controlBgActive : n.controlBg,
              color: n.text,
              borderRadius: n.controlBorderRadius
            },
            children: /* @__PURE__ */ u(fs, { name: g.key })
          },
          g.key
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Label" }),
        /* @__PURE__ */ u(
          "input",
          {
            type: "text",
            value: i.label ?? "",
            placeholder: "Add label...",
            onChange: (g) => r({ label: g.target.value || void 0 }),
            style: {
              flex: 1,
              fontSize: 12,
              padding: "4px 6px",
              background: n.controlBg,
              color: n.text,
              border: `1px solid ${n.border}`,
              borderRadius: n.controlBorderRadius,
              outline: "none"
            }
          }
        )
      ] })
    ] }),
    i.label && /* @__PURE__ */ k(he, { title: "Typography", defaultOpen: !1, persistKey: "shape.typography", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Vn,
          {
            value: i.labelFontFamily ?? "Excalifont",
            onChange: (g) => r({ labelFontFamily: g === "Excalifont" ? void 0 : g }),
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Size" }),
        Wu.map((g) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r({ labelFontSize: g.size === 14 ? void 0 : g.size }),
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: (i.labelFontSize ?? 14) === g.size ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 10,
              fontWeight: 600,
              borderRadius: n.controlBorderRadius
            },
            children: g.label
          },
          g.size
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Align" }),
        hs.map((g) => /* @__PURE__ */ u(
          "button",
          {
            title: g.key,
            onClick: () => r({ labelAlign: g.key === "center" ? void 0 : g.key }),
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: (i.labelAlign ?? "center") === g.key ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 12,
              borderRadius: n.controlBorderRadius
            },
            children: g.label
          },
          g.key
        ))
      ] })
    ] }),
    /* @__PURE__ */ k(he, { title: "Appearance", persistKey: "shape.appearance", children: [
      /* @__PURE__ */ u(
        fe,
        {
          label: "Stroke",
          palettes: we,
          value: h ? void 0 : i.stroke,
          mixed: h,
          onChange: (g) => r({ stroke: g })
        }
      ),
      /* @__PURE__ */ u(
        fe,
        {
          label: "Fill",
          palettes: us,
          value: p ? void 0 : l,
          mixed: p,
          onChange: (g) => r({ fill: g ?? void 0 }),
          allowNull: !0
        }
      ),
      l && !p && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Fill pattern" }),
        ls.map((g) => /* @__PURE__ */ u(
          "button",
          {
            title: g.label,
            onClick: () => r({ fillStyle: g.key }),
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: !d && a === g.key ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 9,
              borderRadius: n.controlBorderRadius
            },
            children: /* @__PURE__ */ u(ps, { style: g.key })
          },
          g.key
        ))
      ] }),
      /* @__PURE__ */ u(
        Vo,
        {
          label: "Stroke style",
          value: c,
          mixed: f,
          onChange: (g) => r({ strokeStyle: g })
        }
      ),
      /* @__PURE__ */ u(
        Ko,
        {
          label: "Stroke width",
          widths: ds,
          value: i.strokeWidth,
          mixed: m,
          onChange: (g) => r({ strokeWidth: g })
        }
      ),
      /* @__PURE__ */ u(
        ke,
        {
          value: i.opacity ?? 1,
          mixed: b,
          onChange: (g) => r({ opacity: g })
        }
      )
    ] }),
    /* @__PURE__ */ u(he, { title: "Sketch", defaultOpen: !1, persistKey: "shape.sketch", children: /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Roughness" }),
      cs.map((g) => /* @__PURE__ */ u(
        "button",
        {
          title: g.label,
          onClick: () => r({ roughness: g.value }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 8px",
            background: !y && i.roughness === g.value ? n.controlBgActive : n.controlBg,
            color: n.text,
            fontSize: 9,
            borderRadius: n.controlBorderRadius
          },
          children: g.label
        },
        g.value
      ))
    ] }) })
  ] });
}
function Lo(t, e) {
  if (t.length < 2) return !1;
  const o = e(t[0]);
  return !t.every((n) => e(n) === o);
}
function Hu({ engine: t, node: e }) {
  const o = qt(), n = Re(t, e), r = Nn(jn) ?? [e], { data: s } = e, i = s.fill ?? null, l = s.fillStyle ?? "hachure", a = s.strokeStyle ?? "solid", c = Lo(r, (y) => y.data.color), h = Lo(r, (y) => y.data.fill ?? null), p = Lo(r, (y) => y.data.fillStyle ?? "hachure"), d = Lo(r, (y) => y.data.strokeStyle ?? "solid"), f = Lo(r, (y) => y.data.strokeWidth), m = Lo(r, (y) => y.data.opacity ?? 1);
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      fe,
      {
        label: "Stroke",
        palettes: we,
        value: c ? void 0 : s.color,
        mixed: c,
        onChange: (y) => n({ color: y })
      }
    ),
    /* @__PURE__ */ u(
      fe,
      {
        label: "Fill",
        palettes: us,
        value: h ? void 0 : i,
        mixed: h,
        onChange: (y) => n({ fill: y ?? void 0 }),
        allowNull: !0
      }
    ),
    i && !h && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Fill pattern" }),
      ls.map((y) => /* @__PURE__ */ u(
        "button",
        {
          title: y.label,
          onClick: () => n({ fillStyle: y.key }),
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: !p && l === y.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            fontSize: 9,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(ps, { style: y.key })
        },
        y.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Vo,
      {
        label: "Stroke style",
        value: a,
        mixed: d,
        onChange: (y) => n({ strokeStyle: y })
      }
    ),
    /* @__PURE__ */ u(
      Ko,
      {
        label: "Stroke width",
        widths: Ia,
        value: s.strokeWidth,
        mixed: f,
        onChange: (y) => n({ strokeWidth: y })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: s.opacity ?? 1,
        mixed: m,
        onChange: (y) => n({ opacity: y })
      }
    )
  ] });
}
function Ou({ engine: t, node: e, fontsInScene: o }) {
  const n = qt(), r = Re(t, e), { data: s } = e;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ k(he, { title: "Typography", persistKey: "text.typography", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Vn,
          {
            value: s.fontFamily,
            onChange: (i) => r({ fontFamily: i }),
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Size" }),
        Ta.map((i) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => r({ fontSize: i }),
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: s.fontSize === i ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 10,
              borderRadius: n.controlBorderRadius
            },
            children: i
          },
          i
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Align" }),
        hs.map((i) => /* @__PURE__ */ u(
          "button",
          {
            title: i.key,
            onClick: () => r({ align: i.key }),
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: s.align === i.key ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 12,
              borderRadius: n.controlBorderRadius
            },
            children: i.label
          },
          i.key
        ))
      ] })
    ] }),
    /* @__PURE__ */ k(he, { title: "Appearance", persistKey: "text.appearance", children: [
      /* @__PURE__ */ u(
        fe,
        {
          label: "Color",
          palettes: we,
          value: s.color,
          onChange: (i) => r({ color: i })
        }
      ),
      /* @__PURE__ */ u(
        hn,
        {
          borderColor: s.borderColor ?? null,
          borderStyle: s.borderStyle,
          borderWidth: s.borderWidth,
          onChange: (i, l) => r({ [i]: l })
        }
      ),
      /* @__PURE__ */ u(
        ke,
        {
          value: s.opacity ?? 1,
          onChange: (i) => r({ opacity: i })
        }
      )
    ] })
  ] });
}
function Xu({ engine: t, node: e }) {
  const o = qt(), n = Re(t, e), { data: r } = e;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ k(he, { title: "Line", persistKey: "edge.line", children: [
      /* @__PURE__ */ u(
        fe,
        {
          label: "Color",
          palettes: we,
          value: r.color,
          onChange: (s) => n({ color: s })
        }
      ),
      /* @__PURE__ */ u(
        Vo,
        {
          label: "Style",
          value: r.style,
          onChange: (s) => n({ style: s })
        }
      ),
      /* @__PURE__ */ u(
        Ko,
        {
          label: "Width",
          widths: _h,
          value: r.strokeWidth,
          onChange: (s) => n({ strokeWidth: s })
        }
      )
    ] }),
    /* @__PURE__ */ k(he, { title: "Arrows", persistKey: "edge.arrows", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Head" }),
        ["none", "arrow", "filled", "dot"].map((s) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n({ arrowHead: s }),
            style: {
              ...Zt,
              height: 28,
              padding: "0 6px",
              background: (r.arrowHead ?? "none") === s ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 11,
              borderRadius: o.controlBorderRadius
            },
            children: s === "none" ? "None" : s === "arrow" ? "▷" : s === "filled" ? "▶" : "●"
          },
          s
        ))
      ] }),
      (r.arrowHead ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Head size" }),
        /* @__PURE__ */ u(
          "input",
          {
            type: "range",
            min: 4,
            max: 40,
            step: 1,
            value: r.arrowHeadSize ?? Math.max(8, r.strokeWidth * 3),
            onChange: (s) => n({ arrowHeadSize: Number(s.target.value) }),
            style: { flex: 1, accentColor: o.accentColor }
          }
        ),
        /* @__PURE__ */ u("span", { style: { color: o.textMuted, fontSize: 11, minWidth: 24, textAlign: "right" }, children: r.arrowHeadSize ?? Math.max(8, r.strokeWidth * 3) })
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Tail" }),
        ["none", "arrow", "filled", "dot"].map((s) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n({ arrowTail: s }),
            style: {
              ...Zt,
              height: 28,
              padding: "0 6px",
              background: (r.arrowTail ?? "none") === s ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 11,
              borderRadius: o.controlBorderRadius
            },
            children: s === "none" ? "None" : s === "arrow" ? "◁" : s === "filled" ? "◀" : "●"
          },
          s
        ))
      ] }),
      (r.arrowTail ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Tail size" }),
        /* @__PURE__ */ u(
          "input",
          {
            type: "range",
            min: 4,
            max: 40,
            step: 1,
            value: r.arrowTailSize ?? Math.max(8, r.strokeWidth * 3),
            onChange: (s) => n({ arrowTailSize: Number(s.target.value) }),
            style: { flex: 1, accentColor: o.accentColor }
          }
        ),
        /* @__PURE__ */ u("span", { style: { color: o.textMuted, fontSize: 11, minWidth: 24, textAlign: "right" }, children: r.arrowTailSize ?? Math.max(8, r.strokeWidth * 3) })
      ] })
    ] }),
    /* @__PURE__ */ k(he, { title: "Path & Motion", defaultOpen: !1, persistKey: "edge.path-motion", children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Path" }),
        [
          { key: "bezier", label: "Bezier" },
          { key: "straight", label: "Straight" },
          { key: "smoothstep", label: "Smooth" },
          { key: "step", label: "Step" }
        ].map((s) => /* @__PURE__ */ u(
          "button",
          {
            title: s.label,
            onClick: () => n({ edgeType: s.key }),
            style: {
              ...Zt,
              height: 28,
              padding: "0 6px",
              background: (r.edgeType ?? "bezier") === s.key ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 9,
              borderRadius: o.controlBorderRadius
            },
            children: s.label
          },
          s.key
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Animate" }),
        /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n({ animated: !r.animated }),
            style: {
              ...Zt,
              height: 28,
              padding: "0 12px",
              background: r.animated ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 11,
              borderRadius: o.controlBorderRadius
            },
            children: r.animated ? "On" : "Off"
          }
        )
      ] }),
      r.animated && /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Direction" }),
        ["forward", "reverse", "both", "bop"].map((s) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => n({ animatedDirection: s }),
            style: {
              ...Zt,
              height: 28,
              padding: "0 6px",
              background: (r.animatedDirection ?? "forward") === s ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 10,
              borderRadius: o.controlBorderRadius
            },
            children: s === "forward" ? "→" : s === "reverse" ? "←" : s === "both" ? "⇆" : "~"
          },
          s
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Roughness" }),
        cs.map((s) => /* @__PURE__ */ u(
          "button",
          {
            title: s.label,
            onClick: () => n({ roughness: s.value }),
            style: {
              ...Zt,
              height: 28,
              padding: "0 8px",
              background: (r.roughness ?? 0) === s.value ? o.controlBgActive : o.controlBg,
              color: o.text,
              fontSize: 9,
              borderRadius: o.controlBorderRadius
            },
            children: s.label
          },
          s.value
        ))
      ] })
    ] }),
    /* @__PURE__ */ u(he, { title: "Label", defaultOpen: !1, persistKey: "edge.label", children: /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Text" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: r.label ?? "",
          onChange: (s) => n({ label: s.target.value || void 0 }),
          placeholder: "Edge label...",
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 11,
            outline: "none"
          }
        }
      )
    ] }) })
  ] });
}
function Yu({ engine: t, node: e }) {
  const o = qt(), [n, r] = et("idle"), s = Re(t, e), { data: i } = e, l = !!i.crop;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      hn,
      {
        borderColor: i.borderColor ?? null,
        borderStyle: i.borderStyle,
        borderWidth: i.borderWidth,
        onChange: (a, c) => s({ [a]: c })
      }
    ),
    /* @__PURE__ */ k("div", { style: { ...Lt, marginTop: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Crop" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.requestImageCrop(e.id),
          style: {
            ...Zt,
            height: 28,
            padding: "0 10px",
            background: o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: "Crop"
        }
      ),
      l && /* @__PURE__ */ u(
        "button",
        {
          onClick: () => s({ crop: void 0 }),
          style: {
            ...Zt,
            height: 28,
            padding: "0 10px",
            background: o.controlBg,
            color: o.textMuted,
            fontSize: 10,
            borderRadius: o.controlBorderRadius
          },
          children: "Reset"
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: { ...Lt, marginTop: 4 }, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Background" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: async () => {
            if (n !== "loading") {
              r("loading");
              try {
                const { removeBackground: a } = await import("@imgly/background-removal"), h = await (await fetch(i.src)).blob(), p = await a(h), d = new FileReader(), f = await new Promise((m, y) => {
                  d.onload = () => m(d.result), d.onerror = y, d.readAsDataURL(p);
                });
                s({ src: f }), r("idle");
              } catch (a) {
                console.error("Background removal failed:", a), r("error"), setTimeout(() => r("idle"), 3e3);
              }
            }
          },
          disabled: n === "loading",
          style: {
            ...Zt,
            height: 28,
            padding: "0 10px",
            background: n === "error" ? o.error : o.controlBg,
            color: o.text,
            fontSize: 10,
            borderRadius: o.controlBorderRadius,
            gap: 4,
            opacity: n === "loading" ? 0.6 : 1
          },
          children: n === "loading" ? "Removing..." : n === "error" ? "Failed" : "Remove BG"
        }
      )
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: i.opacity ?? 1,
        onChange: (a) => s({ opacity: a })
      }
    )
  ] });
}
function Gu({ engine: t, node: e }) {
  const o = qt(), n = Re(t, e), { data: r } = e;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      hn,
      {
        borderColor: r.borderColor ?? null,
        borderStyle: r.borderStyle,
        borderWidth: r.borderWidth,
        onChange: (s, i) => n({ [s]: i })
      }
    ),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => n({ edgeStyle: s.key === "sharp" ? void 0 : s.key }),
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (r.edgeStyle ?? "sharp") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(fs, { name: s.key })
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: r.opacity ?? 1,
        onChange: (s) => n({ opacity: s })
      }
    )
  ] });
}
const on = {
  pan: 400,
  fade: 500,
  dissolve: 400,
  zoom: 600,
  fold: 700,
  cube: 1200,
  none: 0
}, ju = Vh();
function Vu({
  value: t,
  onChange: e,
  theme: o
}) {
  const [n, r] = et(String(t));
  yt(() => r(String(t)), [t]);
  const s = () => {
    const i = parseInt(n, 10);
    !isNaN(i) && i >= 100 && i <= 5e3 ? e(i) : r(String(t));
  };
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Duration" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "number",
        min: 100,
        max: 5e3,
        step: 50,
        value: n,
        onChange: (i) => r(i.target.value),
        onBlur: s,
        onKeyDown: (i) => {
          i.key === "Enter" && s();
        },
        style: {
          width: 64,
          background: o.controlBg,
          color: o.text,
          border: "none",
          borderRadius: o.controlBorderRadius,
          padding: "4px 6px",
          fontSize: 11,
          outline: "none"
        }
      }
    ),
    /* @__PURE__ */ u("span", { style: { fontSize: 10, color: o.textMuted }, children: "ms" })
  ] });
}
function Ku({ engine: t, node: e }) {
  const o = qt(), n = Re(t, e), { data: r } = e, s = st(
    (l) => {
      var d;
      if (!l) {
        n({ devicePreset: void 0 });
        return;
      }
      const a = Wr(l);
      if (!a) return;
      const c = za(a), h = Math.round(e.w / c), p = { devicePreset: l };
      (!r.label || ((d = Wr(r.devicePreset ?? "")) == null ? void 0 : d.label) === r.label) && (p.label = a.label), n(p), t.updateNodeWithHistory(e.id, { h });
    },
    [t, e, r.label, r.devicePreset, n]
  ), i = Kt(() => {
    const l = t.getAllNodes().filter((d) => d.type === "frame"), a = l.length, c = /* @__PURE__ */ new Set();
    for (const d of l)
      d.id !== e.id && d.data.slideOrder != null && c.add(d.data.slideOrder);
    const h = [];
    for (let d = 1; d <= a; d++)
      c.has(d) || h.push(d);
    const p = e.data.slideOrder;
    return p != null && !h.includes(p) && (h.push(p), h.sort((d, f) => d - f)), h;
  }, [t, e]);
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Label" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: r.label ?? "",
          onChange: (l) => n({ label: l.target.value || void 0 }),
          placeholder: "Frame label...",
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 11,
            outline: "none"
          }
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Device" }),
      /* @__PURE__ */ k(
        "select",
        {
          value: r.devicePreset ?? "",
          onChange: (l) => s(l.target.value),
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "", children: "Freeform" }),
            ju.map((l) => /* @__PURE__ */ u("optgroup", { label: l.label, children: l.presets.map((a) => /* @__PURE__ */ k("option", { value: a.key, children: [
              a.label,
              " (",
              a.w,
              "×",
              a.h,
              ")"
            ] }, a.key)) }, l.label))
          ]
        }
      )
    ] }),
    /* @__PURE__ */ u(
      fe,
      {
        label: "Background",
        palettes: we,
        value: (() => {
          const l = r.backgroundColor;
          if (!l) return null;
          for (const a of we) {
            const c = a.colors.find((h) => l === `${h}15`);
            if (c) return c;
          }
          return l.length === 9 && l.endsWith("15") ? l.slice(0, 7) : null;
        })(),
        onChange: (l) => n({ backgroundColor: l ? `${l}15` : void 0 }),
        allowNull: !0
      }
    ),
    /* @__PURE__ */ u(
      fe,
      {
        label: "Border",
        palettes: we,
        value: r.borderColor,
        onChange: (l) => n({ borderColor: l })
      }
    ),
    /* @__PURE__ */ u(
      Vo,
      {
        label: "Style",
        value: r.borderStyle ?? "dashed",
        onChange: (l) => n({ borderStyle: l })
      }
    ),
    /* @__PURE__ */ u(
      Ko,
      {
        label: "Width",
        value: r.borderWidth ?? 1,
        onChange: (l) => n({ borderWidth: l })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: r.opacity ?? 1,
        onChange: (l) => n({ opacity: l })
      }
    ),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Slide #" }),
      /* @__PURE__ */ k(
        "select",
        {
          value: r.slideOrder ?? "",
          onChange: (l) => {
            const a = l.target.value;
            n({ slideOrder: a ? parseInt(a, 10) : void 0 });
          },
          style: {
            width: 72,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "", children: "Auto" }),
            i.map((l) => /* @__PURE__ */ u("option", { value: l, children: l }, l))
          ]
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Transition" }),
      /* @__PURE__ */ k(
        "select",
        {
          value: r.transition ?? "pan",
          onChange: (l) => {
            const a = l.target.value;
            n({ transition: a === "pan" ? void 0 : a, transitionDuration: void 0 });
          },
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.text,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 6px",
            fontSize: 11,
            outline: "none",
            cursor: "pointer"
          },
          children: [
            /* @__PURE__ */ u("option", { value: "pan", children: "Pan" }),
            /* @__PURE__ */ u("option", { value: "fade", children: "Fade to Black" }),
            /* @__PURE__ */ u("option", { value: "dissolve", children: "Dissolve" }),
            /* @__PURE__ */ u("option", { value: "zoom", children: "Zoom" }),
            /* @__PURE__ */ u("option", { value: "fold", children: "Fold" }),
            /* @__PURE__ */ u("option", { value: "cube", children: "Cube" }),
            /* @__PURE__ */ u("option", { value: "none", children: "None (instant)" })
          ]
        }
      )
    ] }),
    (r.transition ?? "pan") !== "none" && /* @__PURE__ */ u(
      Vu,
      {
        value: r.transitionDuration ?? on[r.transition ?? "pan"],
        onChange: (l) => n({ transitionDuration: l === on[r.transition ?? "pan"] ? void 0 : l }),
        theme: o
      }
    )
  ] });
}
function qu({ engine: t, node: e }) {
  const o = qt(), n = Re(t, e), { data: r } = e;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      fe,
      {
        label: "Color",
        palettes: ou,
        value: r.color,
        onChange: (s) => {
          s && n({ color: s });
        }
      }
    ),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Size" }),
      [12, 14, 16, 20, 24].map((s) => /* @__PURE__ */ u(
        "button",
        {
          onClick: () => n({ fontSize: s }),
          style: {
            ...Zt,
            width: 32,
            height: 24,
            background: (r.fontSize ?? 16) === s ? o.controlBgActive : o.controlBg,
            borderRadius: o.controlBorderRadius,
            fontSize: 10,
            color: o.text
          },
          children: s
        },
        s
      ))
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Edges" }),
      [
        { key: "sharp", label: "Sharp" },
        { key: "round", label: "Round" }
      ].map((s) => /* @__PURE__ */ u(
        "button",
        {
          title: s.label,
          onClick: () => n({ edgeStyle: s.key === "sharp" ? void 0 : s.key }),
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (r.edgeStyle ?? "sharp") === s.key ? o.controlBgActive : o.controlBg,
            color: o.text,
            borderRadius: o.controlBorderRadius
          },
          children: /* @__PURE__ */ u(fs, { name: s.key })
        },
        s.key
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: r.opacity ?? 1,
        onChange: (s) => n({ opacity: s })
      }
    )
  ] });
}
function Zu({ engine: t, node: e }) {
  const o = qt(), n = Re(t, e), { data: r } = e;
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "URL" }),
      /* @__PURE__ */ u(
        "input",
        {
          type: "text",
          value: r.url,
          readOnly: !0,
          style: {
            flex: 1,
            background: o.controlBg,
            color: o.textMuted,
            border: "none",
            borderRadius: o.controlBorderRadius,
            padding: "4px 8px",
            fontSize: 10,
            outline: "none",
            cursor: "default"
          },
          onClick: (s) => s.target.select()
        }
      )
    ] }),
    /* @__PURE__ */ u(
      hn,
      {
        borderColor: r.borderColor ?? null,
        borderStyle: r.borderStyle,
        borderWidth: r.borderWidth,
        onChange: (s, i) => n({ [s]: i })
      }
    ),
    /* @__PURE__ */ u(
      ke,
      {
        value: r.opacity ?? 1,
        onChange: (s) => n({ opacity: s })
      }
    )
  ] });
}
function Uu({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
const Qu = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
];
function Ju({ engine: t, mode: e, fontsInScene: o }) {
  const n = qt(), [, r] = et(0), s = st(() => r((m) => m + 1), []), i = t.activeTool;
  if (e === "text") {
    const m = i.fontFamily ?? Qe, y = i.fontSize ?? 20, b = i.textAlign ?? "left", g = i.color;
    return /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Font" }),
        /* @__PURE__ */ u(
          Vn,
          {
            value: m,
            onChange: (x) => {
              i.fontFamily = x, s();
            },
            fontsInScene: o
          }
        )
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Size" }),
        Ta.map((x) => /* @__PURE__ */ u(
          "button",
          {
            onClick: () => {
              i.fontSize = x, s();
            },
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: y === x ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 10,
              borderRadius: n.controlBorderRadius
            },
            children: x
          },
          x
        ))
      ] }),
      /* @__PURE__ */ k("div", { style: Lt, children: [
        /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Align" }),
        hs.map((x) => /* @__PURE__ */ u(
          "button",
          {
            title: x.key,
            onClick: () => {
              i.textAlign = x.key, s();
            },
            style: {
              ...Zt,
              width: 36,
              height: 28,
              background: b === x.key ? n.controlBgActive : n.controlBg,
              color: n.text,
              fontSize: 12,
              borderRadius: n.controlBorderRadius
            },
            children: x.label
          },
          x.key
        ))
      ] }),
      /* @__PURE__ */ u(
        fe,
        {
          label: "Color",
          palettes: we,
          value: g,
          onChange: (x) => {
            i.color = x, s();
          }
        }
      ),
      /* @__PURE__ */ u(
        ke,
        {
          value: i.opacity ?? 1,
          onChange: (x) => {
            i.opacity = x, s();
          }
        }
      )
    ] });
  }
  const l = e === "shape", a = i.color, c = i.fillColor ?? null, h = i.fillStyle ?? "hachure", p = i.strokeStyle ?? "solid", d = i.width, f = i.roughness ?? 1;
  return /* @__PURE__ */ k(ft, { children: [
    l && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Shape" }),
      Qu.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => {
            i.shapeType = m.key, s();
          },
          style: {
            ...Zt,
            width: 28,
            height: 28,
            background: (i.shapeType ?? "rect") === m.key ? n.controlBgActive : n.controlBg,
            color: n.text,
            borderRadius: n.controlBorderRadius
          },
          children: /* @__PURE__ */ u(Uu, { name: m.key })
        },
        m.key
      ))
    ] }),
    /* @__PURE__ */ u(
      fe,
      {
        label: "Stroke",
        palettes: we,
        value: a,
        onChange: (m) => {
          i.color = m, s();
        }
      }
    ),
    /* @__PURE__ */ u(
      fe,
      {
        label: "Fill",
        palettes: us,
        value: c,
        onChange: (m) => {
          i.fillColor = m ?? void 0, s();
        },
        allowNull: !0
      }
    ),
    c && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Fill pattern" }),
      ls.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => {
            i.fillStyle = m.key, s();
          },
          style: {
            ...Zt,
            width: 36,
            height: 28,
            background: h === m.key ? n.controlBgActive : n.controlBg,
            color: n.text,
            fontSize: 9,
            borderRadius: n.controlBorderRadius
          },
          children: /* @__PURE__ */ u(ps, { style: m.key })
        },
        m.key
      ))
    ] }),
    /* @__PURE__ */ u(
      Vo,
      {
        label: "Stroke style",
        value: p,
        onChange: (m) => {
          i.strokeStyle = m, s();
        }
      }
    ),
    /* @__PURE__ */ u(
      Ko,
      {
        label: "Stroke width",
        widths: l ? ds : Ia,
        value: d,
        onChange: (m) => {
          i.width = m, s();
        }
      }
    ),
    l && /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Roughness" }),
      cs.map((m) => /* @__PURE__ */ u(
        "button",
        {
          title: m.label,
          onClick: () => {
            i.roughness = m.value, s();
          },
          style: {
            ...Zt,
            height: 28,
            padding: "0 8px",
            background: f === m.value ? n.controlBgActive : n.controlBg,
            color: n.text,
            fontSize: 9,
            borderRadius: n.controlBorderRadius
          },
          children: m.label
        },
        m.value
      ))
    ] }),
    /* @__PURE__ */ u(
      ke,
      {
        value: i.opacity ?? 1,
        onChange: (m) => {
          i.opacity = m, s();
        }
      }
    )
  ] });
}
function $u({ engine: t, node: e, PanelComponent: o }) {
  const n = Re(t, e);
  return /* @__PURE__ */ u(o, { node: e, data: e.data, engine: t, updateData: n });
}
const Br = {
  shape: "Shape",
  draw: "Drawing",
  text: "Text",
  edge: "Edge",
  image: "Image",
  content: "Content",
  frame: "Frame",
  sticky: "Sticky Note",
  youtube: "YouTube"
}, _u = /* @__PURE__ */ new Set(["shape", "draw", "text", "image", "content", "frame", "sticky", "youtube"]), tp = /* @__PURE__ */ new Set(["text", "image", "content", "frame", "youtube"]);
function ep(t) {
  const e = /* @__PURE__ */ new Set(), o = [];
  for (const n of t.getAllNodes()) {
    let r;
    n.type === "text" ? r = n.data.fontFamily : n.type === "shape" && (r = n.data.labelFontFamily), r && !e.has(r) && (e.add(r), o.push(r));
  }
  return o;
}
function op({ label: t }) {
  const e = qt();
  return /* @__PURE__ */ u(
    "div",
    {
      style: {
        fontSize: 10,
        fontWeight: 600,
        color: e.textSecondary,
        padding: "2px 0 6px",
        borderBottom: `1px solid ${e.border}`,
        marginBottom: 2
      },
      children: t
    }
  );
}
function np({
  engine: t,
  open: e,
  onToggle: o
}) {
  const n = qt(), [r, s] = et(t.snapToGrid), [i, l] = et(t.gridSize), [a, c] = et(t.smartGuides), [h, p] = et(t.boardBackground);
  yt(() => {
    const f = () => {
      s(t.snapToGrid), l(t.gridSize), c(t.smartGuides);
    }, m = () => p(t.boardBackground);
    return t.on("guides", f), t.on("background", m), () => {
      t.off("guides", f), t.off("background", m);
    };
  }, [t]);
  const d = [10, 20, 40, 80];
  return /* @__PURE__ */ k(he, { title: "Canvas", defaultOpen: !1, variant: "group", open: e, onToggle: o, children: [
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Grid" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.toggleSnapToGrid(),
          style: {
            border: "none",
            borderRadius: n.controlBorderRadius,
            background: r ? n.controlBgActive : n.controlBg,
            color: n.text,
            fontSize: 10,
            padding: "4px 10px",
            cursor: "pointer"
          },
          children: r ? "On" : "Off"
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Grid size" }),
      /* @__PURE__ */ u("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", flex: 1 }, children: d.map((f) => /* @__PURE__ */ k(
        "button",
        {
          onClick: () => t.setGridSize(f),
          style: {
            border: "none",
            borderRadius: n.controlBorderRadius,
            background: i === f ? n.controlBgActive : n.controlBg,
            color: n.text,
            fontSize: 10,
            padding: "4px 8px",
            cursor: "pointer"
          },
          children: [
            f,
            "px"
          ]
        },
        f
      )) })
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Guides" }),
      /* @__PURE__ */ u(
        "button",
        {
          onClick: () => t.toggleSmartGuides(),
          style: {
            border: "none",
            borderRadius: n.controlBorderRadius,
            background: a ? n.controlBgActive : n.controlBg,
            color: n.text,
            fontSize: 10,
            padding: "4px 10px",
            cursor: "pointer"
          },
          children: a ? "On" : "Off"
        }
      )
    ] }),
    /* @__PURE__ */ k("div", { style: Lt, children: [
      /* @__PURE__ */ u("span", { style: { ...Et, color: n.textMuted }, children: "Paper" }),
      /* @__PURE__ */ u(
        "select",
        {
          value: h,
          onChange: (f) => t.setBoardBackground(f.target.value),
          style: {
            flex: 1,
            height: 28,
            border: `1px solid ${n.border}`,
            borderRadius: n.controlBorderRadius,
            background: n.controlBg,
            color: n.text,
            fontSize: 11,
            padding: "0 8px",
            outline: "none"
          },
          children: Xo.map((f) => /* @__PURE__ */ u("option", { value: f.key, children: f.label }, f.key))
        }
      )
    ] })
  ] });
}
function Pa({
  engine: t,
  node: e,
  registry: o,
  fontsInScene: n
}) {
  switch (e.type) {
    case "shape":
      return /* @__PURE__ */ u(Nu, { engine: t, node: e, fontsInScene: n });
    case "draw":
      return /* @__PURE__ */ u(Hu, { engine: t, node: e });
    case "text":
      return /* @__PURE__ */ u(Ou, { engine: t, node: e, fontsInScene: n });
    case "edge":
      return /* @__PURE__ */ u(Xu, { engine: t, node: e });
    case "image":
      return /* @__PURE__ */ u(Yu, { engine: t, node: e });
    case "content":
      return /* @__PURE__ */ u(Gu, { engine: t, node: e });
    case "frame":
      return /* @__PURE__ */ u(Ku, { engine: t, node: e });
    case "sticky":
      return /* @__PURE__ */ u(qu, { engine: t, node: e });
    case "youtube":
      return /* @__PURE__ */ u(Zu, { engine: t, node: e });
    default: {
      const r = o == null ? void 0 : o.get(e.type);
      return r != null && r.propertiesPanel ? /* @__PURE__ */ u($u, { engine: t, node: e, PanelComponent: r.propertiesPanel }) : null;
    }
  }
}
function yi({
  engine: t,
  nodes: e
}) {
  const o = qt(), n = Math.round(e[0].rotation ?? 0), s = e.every(
    (c) => Math.round(c.rotation ?? 0) === n
  ) ? n : null, [i, l] = et(null), a = st(
    (c) => {
      l(null);
      const h = parseFloat(c);
      if (isNaN(h)) return;
      const p = Math.max(-360, Math.min(360, h)), d = e.map((f) => ({
        id: f.id,
        patch: { rotation: p }
      }));
      t.batchUpdateWithHistory(d);
    },
    [t, e]
  );
  return /* @__PURE__ */ k("div", { style: Lt, children: [
    /* @__PURE__ */ u("span", { style: { ...Et, color: o.textMuted }, children: "Rotation" }),
    /* @__PURE__ */ u(
      "input",
      {
        type: "number",
        min: -360,
        max: 360,
        value: i ?? (s !== null ? String(s) : ""),
        placeholder: s === null ? "Mixed" : void 0,
        onChange: (c) => l(c.target.value),
        onBlur: (c) => a(c.target.value),
        onKeyDown: (c) => {
          c.key === "Enter" && a(c.target.value), c.key === "Escape" && l(null);
        },
        style: {
          width: 52,
          height: 24,
          border: `1px solid ${o.border}`,
          borderRadius: o.controlBorderRadius,
          background: o.controlBg,
          color: o.text,
          fontSize: 10,
          textAlign: "center",
          outline: "none",
          padding: "0 2px"
        }
      }
    ),
    /* @__PURE__ */ u("span", { style: { fontSize: 10, color: o.textMuted }, children: "°" })
  ] });
}
function rp({
  engine: t,
  nodes: e,
  commonProps: o
}) {
  const n = st(
    (r, s) => {
      const i = r === "opacity" ? _u : tp, l = e.filter((a) => i.has(a.type)).map((a) => ({
        id: a.id,
        patch: {
          data: { ...a.data, [r]: s }
        }
      }));
      t.batchUpdateWithHistory(l);
    },
    [t, e]
  );
  return /* @__PURE__ */ k(ft, { children: [
    o.opacity !== void 0 && /* @__PURE__ */ u(
      ke,
      {
        value: o.opacity === "mixed" ? void 0 : o.opacity,
        mixed: o.opacity === "mixed",
        onChange: (r) => n("opacity", r)
      }
    ),
    o.borderColor !== void 0 && /* @__PURE__ */ u(
      hn,
      {
        borderColor: o.borderColor === "mixed" ? void 0 : o.borderColor,
        borderStyle: o.borderStyle === "mixed" ? void 0 : o.borderStyle,
        borderWidth: o.borderWidth === "mixed" ? void 0 : o.borderWidth,
        mixed: {
          color: o.borderColor === "mixed",
          style: o.borderStyle === "mixed",
          width: o.borderWidth === "mixed"
        },
        onChange: (r, s) => n(r, s)
      }
    )
  ] });
}
function sp({
  engine: t,
  target: e
}) {
  const o = qt();
  if (e.kind !== "single" && e.kind !== "multi") return null;
  const n = Array.from(t.selection), r = n.length > 0, s = n.length >= 2 || t.selectionHasGroup(), i = n.some((c) => {
    var h;
    return (h = t.getNode(c)) == null ? void 0 : h.locked;
  }), l = n.some((c) => {
    var h;
    return !((h = t.getNode(c)) != null && h.locked);
  }), a = [
    {
      label: "Cut",
      disabled: !r,
      action: () => t.cutSelected()
    },
    {
      label: "Copy",
      disabled: !r,
      action: () => t.copySelected()
    },
    {
      label: "Paste",
      disabled: !t.hasClipboard(),
      action: () => t.pasteClipboard()
    },
    {
      label: "Duplicate",
      disabled: !r,
      action: () => t.duplicateSelected()
    },
    {
      label: "Group",
      disabled: !s || n.length < 2,
      action: () => t.groupSelected()
    },
    {
      label: "Ungroup",
      disabled: !s || !t.selectionHasGroup(),
      action: () => t.ungroupSelected()
    },
    {
      label: "Lock",
      disabled: !l,
      action: () => {
        for (const c of n) t.updateNode(c, { locked: !0 });
      }
    },
    {
      label: "Unlock",
      disabled: !i,
      action: () => {
        for (const c of n) t.updateNode(c, { locked: void 0 });
      }
    },
    {
      label: "Delete",
      disabled: !r,
      danger: !0,
      action: () => t.deleteSelected()
    }
  ];
  return /* @__PURE__ */ u(he, { title: "Actions", defaultOpen: !0, variant: "group", persistKey: "touch-actions", children: /* @__PURE__ */ u("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 }, children: a.map((c) => /* @__PURE__ */ u(
    "button",
    {
      type: "button",
      disabled: c.disabled,
      onClick: c.action,
      style: {
        border: `1px solid ${o.border}`,
        borderRadius: 999,
        background: c.disabled ? o.controlBg : o.controlBgActive,
        color: c.danger ? "#fecaca" : o.text,
        opacity: c.disabled ? 0.45 : 0.95,
        padding: "5px 10px",
        fontSize: 10,
        fontWeight: 700,
        letterSpacing: "0.01em",
        cursor: c.disabled ? "default" : "pointer",
        whiteSpace: "nowrap"
      },
      children: c.label
    },
    c.label
  )) }) });
}
function ip({
  engine: t,
  group: e,
  registry: o,
  fontsInScene: n,
  open: r,
  onToggle: s
}) {
  const i = Br[e.type] ?? e.type, l = e.nodes.length, a = e.nodes[0], c = `${i} (${l})`;
  return /* @__PURE__ */ u(he, { title: c, defaultOpen: !1, variant: "group", open: r, onToggle: s, children: /* @__PURE__ */ u(jn.Provider, { value: e.nodes, children: /* @__PURE__ */ u(
    Pa,
    {
      engine: t,
      node: a,
      registry: o,
      fontsInScene: n
    }
  ) }) });
}
function ap(t) {
  switch (t.kind) {
    case "none":
      return "No selection";
    case "tool":
      return `${t.mode.charAt(0).toUpperCase() + t.mode.slice(1)} tool`;
    case "single":
      return Br[t.node.type] ?? t.node.type;
    case "multi":
      return t.typeGroups.map(
        (o) => `${o.nodes.length} ${(Br[o.type] ?? o.type).toLowerCase()}${o.nodes.length > 1 ? "s" : ""}`
      ).join(", ");
  }
}
function mi({
  engine: t,
  registry: e,
  target: o,
  commonProps: n
}) {
  const r = Kt(() => ep(t), [t, o]), s = ap(o), [i, l] = et("shared"), [a, c] = et(!1);
  return yt(() => {
    const h = () => {
      c(
        window.matchMedia("(pointer: coarse)").matches || window.matchMedia("(hover: none)").matches || navigator.maxTouchPoints > 0
      );
    };
    return h(), window.addEventListener("resize", h), () => window.removeEventListener("resize", h);
  }, []), yt(() => {
    if (o.kind !== "multi") {
      l("shared");
      return;
    }
    (/* @__PURE__ */ new Set(["canvas", "shared", ...o.typeGroups.map((p) => p.type)])).has(i) || l("shared");
  }, [o, i]), /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(op, { label: s }),
    /* @__PURE__ */ u(
      np,
      {
        engine: t,
        open: o.kind === "multi" ? i === "canvas" : void 0,
        onToggle: o.kind === "multi" ? () => l((h) => h === "canvas" ? "" : "canvas") : void 0
      }
    ),
    a && /* @__PURE__ */ u(sp, { engine: t, target: o }),
    o.kind === "tool" && /* @__PURE__ */ u(Ju, { engine: t, mode: o.mode, fontsInScene: r }),
    o.kind === "single" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u(
        Pa,
        {
          engine: t,
          node: o.node,
          registry: e,
          fontsInScene: r
        }
      ),
      /* @__PURE__ */ u(yi, { engine: t, nodes: [o.node] })
    ] }),
    o.kind === "multi" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ k(
        he,
        {
          title: "Shared",
          defaultOpen: !0,
          variant: "group",
          open: i === "shared",
          onToggle: () => l((h) => h === "shared" ? "" : "shared"),
          children: [
            /* @__PURE__ */ u(rp, { engine: t, nodes: o.nodes, commonProps: n }),
            /* @__PURE__ */ u(yi, { engine: t, nodes: o.nodes })
          ]
        }
      ),
      o.typeGroups.map((h) => /* @__PURE__ */ u(
        ip,
        {
          engine: t,
          group: h,
          registry: e,
          fontsInScene: r,
          open: i === h.type,
          onToggle: () => l((p) => p === h.type ? "" : h.type)
        },
        h.type
      ))
    ] })
  ] });
}
function lp({ engine: t, registry: e }) {
  const o = qt(), { target: n, commonProps: r } = Lu(t), s = n.kind !== "none", i = st((X, N) => {
    const j = X.trim();
    if (j.startsWith("#")) {
      const H = j.slice(1), _ = H.length === 3 ? H.split("").map((tt) => tt + tt).join("") : H;
      if (_.length === 6) {
        const tt = parseInt(_.slice(0, 2), 16), nt = parseInt(_.slice(2, 4), 16), ut = parseInt(_.slice(4, 6), 16);
        return `rgba(${tt}, ${nt}, ${ut}, ${N})`;
      }
    }
    return j.startsWith("rgb(") ? `rgba(${j.slice(4, -1)}, ${N})` : (j.startsWith("rgba("), j);
  }, []), [l, a] = et(!1), [c, h] = et(!1), [p, d] = et(!1), [f, m] = et(!1), y = at(null), b = at(!1), g = st(() => typeof window > "u" ? !1 : window.matchMedia("(pointer: coarse)").matches || window.matchMedia("(hover: none)").matches || navigator.maxTouchPoints > 0, []), x = st(
    (X) => {
      const N = g() ? 1366 : 1024;
      return X <= N;
    },
    [g]
  ), S = at(null), [M, z] = et(null), W = at(null), [I, R] = et(!1), K = st(() => {
    var j, H;
    const X = (j = S.current) == null ? void 0 : j.offsetParent;
    if (X) return { width: X.clientWidth, height: X.clientHeight };
    const N = ((H = S.current) == null ? void 0 : H.ownerDocument.defaultView) ?? window;
    return { width: N.innerWidth, height: N.innerHeight };
  }, []), Z = st(() => {
    const { width: X } = K();
    return { x: X - en - 16, y: 12 };
  }, [K]), lt = M ?? Z(), O = at(!1);
  Hr(() => {
    if (!O.current && S.current && !M) {
      O.current = !0;
      const X = S.current.offsetParent;
      X && z({ x: X.clientWidth - en - 16, y: 12 });
    }
  }), yt(() => {
    var H, _;
    const X = ((H = S.current) == null ? void 0 : H.offsetParent) ?? ((_ = S.current) == null ? void 0 : _.ownerDocument.body);
    if (!X) return;
    const N = new ResizeObserver((tt) => {
      var pt;
      const nt = ((pt = tt[0]) == null ? void 0 : pt.contentRect.width) ?? X.clientWidth;
      a(nt < 600);
      const ut = x(nt);
      h(ut), b.current || (m(ut), b.current = !0);
    });
    N.observe(X), a(X.clientWidth < 600);
    const j = x(X.clientWidth);
    return h(j), b.current || (m(j), b.current = !0), () => N.disconnect();
  }, [x]), yt(() => {
    var rt;
    const X = ((rt = S.current) == null ? void 0 : rt.ownerDocument) ?? document, N = () => {
      y.current !== null && window.clearTimeout(y.current), y.current = window.setTimeout(() => {
        d(!1), y.current = null;
      }, 200);
    }, j = () => {
      y.current !== null && (window.clearTimeout(y.current), y.current = null), d(!0);
    }, H = (gt) => !!(gt instanceof Element && gt.closest("[data-sb-canvas]")), _ = (gt) => {
      gt.button !== 2 && H(gt.target) && j();
    }, tt = () => N(), nt = () => N(), ut = (gt) => {
      H(gt.target) && j();
    }, pt = () => N(), vt = (gt) => {
      var ht;
      ((ht = gt.detail) == null ? void 0 : ht.active) ? j() : N();
    };
    return X.addEventListener("pointerdown", _, !0), X.addEventListener("pointerup", tt, !0), X.addEventListener("pointercancel", nt, !0), X.addEventListener("focusin", ut, !0), X.addEventListener("focusout", pt, !0), X.addEventListener("sb:canvas-interaction", vt), () => {
      X.removeEventListener("pointerdown", _, !0), X.removeEventListener("pointerup", tt, !0), X.removeEventListener("pointercancel", nt, !0), X.removeEventListener("focusin", ut, !0), X.removeEventListener("focusout", pt, !0), X.removeEventListener("sb:canvas-interaction", vt), y.current !== null && (window.clearTimeout(y.current), y.current = null);
    };
  }, []);
  const ot = st(
    (X) => {
      X.stopPropagation(), R(!0);
      const N = M ? M.x : Z().x, j = M ? M.y : Z().y;
      W.current = {
        startX: X.clientX,
        startY: X.clientY,
        startLeft: N,
        startTop: j
      }, X.currentTarget.setPointerCapture(X.pointerId);
    },
    [M, Z]
  ), Q = st(
    (X) => {
      if (!W.current) return;
      X.stopPropagation();
      const N = X.clientX - W.current.startX, j = X.clientY - W.current.startY, { width: H, height: _ } = K(), tt = Math.max(
        dn,
        Math.min(H - en - 8, W.current.startLeft + N)
      ), nt = Math.max(
        8,
        Math.min(_ - 100, W.current.startTop + j)
      );
      z({ x: tt, y: nt });
    },
    [K]
  ), $ = st(() => {
    W.current = null, R(!1);
  }, []), P = f && p, G = i(o.panelBg, 0.9);
  return s ? l ? /* @__PURE__ */ k(
    "div",
    {
      ref: S,
      "data-sb-props-panel": !0,
      onPointerDown: (X) => X.stopPropagation(),
      style: {
        position: "fixed",
        bottom: 0,
        left: 0,
        right: 0,
        height: "45vh",
        minHeight: 200,
        background: G,
        borderRadius: "12px 12px 0 0",
        boxShadow: "0 -4px 24px rgba(0,0,0,0.25)",
        zIndex: 200,
        display: "flex",
        flexDirection: "column",
        color: o.text,
        fontSize: 12,
        backdropFilter: "blur(8px) saturate(120%)",
        WebkitBackdropFilter: "blur(8px) saturate(120%)",
        opacity: P ? 0 : 1,
        transform: P ? "translateY(8px)" : "translateY(0)",
        transition: "opacity 140ms ease, transform 160ms ease",
        pointerEvents: P ? "none" : "auto"
      },
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              height: 36,
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              flexShrink: 0,
              padding: "0 12px"
            },
            children: [
              /* @__PURE__ */ k(
                "label",
                {
                  style: {
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                    color: o.textMuted,
                    fontSize: 11,
                    userSelect: "none"
                  },
                  onPointerDown: (X) => X.stopPropagation(),
                  children: [
                    /* @__PURE__ */ u("span", { children: "Auto-hide" }),
                    /* @__PURE__ */ u(
                      "input",
                      {
                        type: "checkbox",
                        checked: f,
                        onChange: (X) => m(X.target.checked),
                        style: { accentColor: o.accentColor }
                      }
                    )
                  ]
                }
              ),
              /* @__PURE__ */ u(
                "div",
                {
                  style: {
                    width: 36,
                    height: 4,
                    borderRadius: 2,
                    background: o.border
                  }
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              overflowY: "auto",
              padding: "0 16px 24px",
              flex: 1,
              touchAction: "pan-y"
            },
            children: /* @__PURE__ */ u(
              mi,
              {
                engine: t,
                registry: e,
                target: n,
                commonProps: r
              }
            )
          }
        )
      ]
    }
  ) : /* @__PURE__ */ k(
    "div",
    {
      ref: S,
      "data-sb-props-panel": !0,
      style: {
        position: "absolute",
        left: lt.x,
        top: lt.y,
        width: en,
        background: G,
        borderRadius: o.panelBorderRadius,
        padding: "0 0 12px",
        display: "flex",
        flexDirection: "column",
        zIndex: 99,
        color: o.text,
        fontSize: 11,
        maxHeight: "calc(100% - 40px)",
        boxShadow: o.panelShadow,
        backdropFilter: "blur(8px) saturate(120%)",
        WebkitBackdropFilter: "blur(8px) saturate(120%)",
        opacity: P ? 0 : 1,
        transform: P ? "translateY(-4px) scale(0.995)" : "translateY(0) scale(1)",
        transformOrigin: "top right",
        transition: "opacity 140ms ease, transform 160ms ease",
        pointerEvents: P ? "none" : "auto"
      },
      onPointerDown: (X) => X.stopPropagation(),
      onPointerMove: Q,
      onPointerUp: $,
      onPointerCancel: $,
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            onPointerDown: ot,
            style: {
              cursor: I ? "grabbing" : "grab",
              padding: "8px 16px",
              userSelect: "none",
              touchAction: "none",
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              borderBottom: `1px solid ${o.border}`,
              color: o.textMuted,
              fontSize: 10,
              flexShrink: 0
            },
            children: [
              /* @__PURE__ */ u("span", { style: { fontWeight: 600, letterSpacing: "0.02em" }, children: "Inspector" }),
              /* @__PURE__ */ k(
                "label",
                {
                  style: {
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    color: o.textMuted,
                    fontSize: 10,
                    userSelect: "none",
                    cursor: "default"
                  },
                  onPointerDown: (X) => X.stopPropagation(),
                  children: [
                    /* @__PURE__ */ u("span", { children: "Auto-hide" }),
                    /* @__PURE__ */ u(
                      "input",
                      {
                        type: "checkbox",
                        checked: f,
                        onChange: (X) => m(X.target.checked),
                        style: { accentColor: o.accentColor }
                      }
                    )
                  ]
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              overflowY: "auto",
              overflowX: "hidden",
              padding: "8px 12px",
              display: "flex",
              flexDirection: "column",
              gap: 8,
              touchAction: "pan-y"
            },
            children: /* @__PURE__ */ u(
              mi,
              {
                engine: t,
                registry: e,
                target: n,
                commonProps: r
              }
            )
          }
        )
      ]
    }
  ) : null;
}
function cp({ engine: t, registry: e, gifApiBaseUrl: o }) {
  return /* @__PURE__ */ k(ft, { children: [
    /* @__PURE__ */ u(
      "div",
      {
        "data-sb-sidebar": !0,
        style: {
          position: "absolute",
          left: 0,
          top: 0,
          bottom: 0,
          width: dn,
          zIndex: 100
        },
        onPointerDown: (n) => n.stopPropagation(),
        children: /* @__PURE__ */ u(Tu, { engine: t, gifApiBaseUrl: o })
      }
    ),
    /* @__PURE__ */ u(lp, { engine: t, registry: e })
  ] });
}
const nn = [0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 3, 4];
function dp(t) {
  const e = t.viewport.zoom, o = nn.find((n) => n > e + 1e-3) ?? nn[nn.length - 1];
  t.viewport.zoom = o, t.pan(0, 0);
}
function hp(t) {
  const e = t.viewport.zoom, o = [...nn].reverse().find((n) => n < e - 1e-3) ?? nn[0];
  t.viewport.zoom = o, t.pan(0, 0);
}
const up = {
  display: "flex",
  alignItems: "center",
  overflow: "hidden"
}, ze = {
  border: "none",
  background: "transparent",
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, ce = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function Ne({ name: t, size: e = 16 }) {
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "minus" && /* @__PURE__ */ u("path", { d: "M5 12h14", ...ce }),
    t === "plus" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M12 5v14", ...ce }),
      /* @__PURE__ */ u("path", { d: "M5 12h14", ...ce })
    ] }),
    t === "undo" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...ce, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...ce, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...ce, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...ce, fill: "none" })
    ] }),
    t === "fit" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...ce }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...ce })
    ] }),
    t === "play" && /* @__PURE__ */ u("path", { d: "M6 4l14 8-14 8z", fill: "currentColor" }),
    t === "slides" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "6", width: "20", height: "12", rx: "1", ...ce }),
      /* @__PURE__ */ u("path", { d: "M6 6V18M18 6V18", ...ce }),
      /* @__PURE__ */ u("path", { d: "M2 9h4M2 12h4M2 15h4M18 9h4M18 12h4M18 15h4", ...ce })
    ] }),
    t === "home" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M3 12l9-8 9 8", ...ce, fill: "none" }),
      /* @__PURE__ */ u("path", { d: "M5 12v7a1 1 0 001 1h12a1 1 0 001-1v-7", ...ce, fill: "none" })
    ] }),
    t === "bookmark" && /* @__PURE__ */ u("path", { d: "M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z", ...ce, fill: "none" }),
    t === "bookmark-fill" && /* @__PURE__ */ u("path", { d: "M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", fill: "currentColor" })
  ] });
}
function pp({ engine: t, framesPanelOpen: e, onToggleFramesPanel: o }) {
  const n = qt(), [r, s] = et(t.viewport.zoom), [i, l] = et(!1), [a, c] = et(!1), [h, p] = et(() => t.originView != null), [d, f] = et(
    () => t.getAllNodes().filter((x) => x.type === "frame").length
  );
  yt(() => {
    const x = () => s(t.viewport.zoom), S = () => {
      l(t.canUndo()), c(t.canRedo());
    }, M = () => {
      f(t.getAllNodes().filter((z) => z.type === "frame").length), p(t.originView != null);
    };
    return t.on("viewport", x), t.on("history", S), t.on("change", M), t.on("node:create", M), t.on("node:delete", M), () => {
      t.off("viewport", x), t.off("history", S), t.off("change", M), t.off("node:create", M), t.off("node:delete", M);
    };
  }, [t]);
  const m = n.panelBg, y = `1px solid ${n.border}`, b = {
    ...up,
    borderRadius: n.panelBorderRadius
  }, g = {
    width: 1,
    height: 20,
    background: n.separator,
    flexShrink: 0
  };
  return /* @__PURE__ */ k(
    "div",
    {
      "data-sb-bottombar": !0,
      style: {
        position: "absolute",
        bottom: 12,
        left: "50%",
        transform: "translateX(-50%)",
        display: "flex",
        gap: 8,
        zIndex: 9999,
        pointerEvents: "auto"
      },
      onPointerDown: (x) => x.stopPropagation(),
      children: [
        /* @__PURE__ */ k("div", { style: { ...b, background: m, border: y, boxShadow: n.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Zoom out",
              onClick: () => hp(t),
              style: { ...ze, width: 32, height: 32, color: n.text },
              children: /* @__PURE__ */ u(Ne, { name: "minus" })
            }
          ),
          /* @__PURE__ */ u("div", { style: g }),
          /* @__PURE__ */ k(
            "button",
            {
              title: "Reset zoom to 100%",
              onClick: () => {
                t.viewport.zoom = 1, t.pan(0, 0);
              },
              style: {
                ...ze,
                minWidth: 48,
                height: 32,
                color: n.text,
                fontSize: 11,
                fontWeight: 600,
                fontFamily: "inherit",
                padding: "0 4px"
              },
              children: [
                Math.round(r * 100),
                "%"
              ]
            }
          ),
          /* @__PURE__ */ u("div", { style: g }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Zoom in",
              onClick: () => dp(t),
              style: { ...ze, width: 32, height: 32, color: n.text },
              children: /* @__PURE__ */ u(Ne, { name: "plus" })
            }
          )
        ] }),
        /* @__PURE__ */ k("div", { style: { ...b, background: m, border: y, boxShadow: n.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Fit to content (Ctrl+0)",
              onClick: () => t.fitToContent(),
              style: { ...ze, width: 32, height: 32, color: n.text },
              children: /* @__PURE__ */ u(Ne, { name: "fit" })
            }
          ),
          /* @__PURE__ */ u("div", { style: g }),
          /* @__PURE__ */ u(
            "button",
            {
              title: h ? "Clear saved view" : "Save current view as origin",
              onClick: () => {
                h ? (t.clearOriginView(), p(!1)) : (t.setOriginView(), p(!0));
              },
              style: { ...ze, width: 32, height: 32, color: h ? n.accentColor : n.textFaint },
              children: /* @__PURE__ */ u(Ne, { name: h ? "bookmark-fill" : "bookmark" })
            }
          ),
          /* @__PURE__ */ u("div", { style: g }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Go to saved view",
              onClick: () => {
                h && t.goToOriginView();
              },
              disabled: !h,
              style: { ...ze, width: 32, height: 32, color: h ? n.text : n.textFaint },
              children: /* @__PURE__ */ u(Ne, { name: "home" })
            }
          )
        ] }),
        /* @__PURE__ */ k("div", { style: { ...b, overflow: "visible", background: m, border: y, boxShadow: n.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Present (frames as slides)",
              onClick: () => t.enterPresentation(),
              style: { ...ze, width: 32, height: 32, color: n.text },
              children: /* @__PURE__ */ u(Ne, { name: "play" })
            }
          ),
          o && /* @__PURE__ */ k(ft, { children: [
            /* @__PURE__ */ u("div", { style: g }),
            /* @__PURE__ */ k(
              "button",
              {
                title: "Toggle slides panel",
                onClick: o,
                style: {
                  ...ze,
                  width: 32,
                  height: 32,
                  color: e ? n.text : n.textMuted,
                  position: "relative"
                },
                children: [
                  /* @__PURE__ */ u(Ne, { name: "slides" }),
                  d > 0 && /* @__PURE__ */ u(
                    "span",
                    {
                      style: {
                        position: "absolute",
                        top: -4,
                        right: -4,
                        minWidth: 14,
                        height: 14,
                        borderRadius: 7,
                        background: n.accentColor,
                        color: "#fff",
                        fontSize: 9,
                        fontWeight: 700,
                        lineHeight: "14px",
                        textAlign: "center",
                        padding: "0 3px",
                        pointerEvents: "none"
                      },
                      children: d
                    }
                  )
                ]
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ k("div", { style: { ...b, background: m, border: y, boxShadow: n.panelShadow }, children: [
          /* @__PURE__ */ u(
            "button",
            {
              title: "Undo (Ctrl+Z)",
              onClick: () => t.undo(),
              disabled: !i,
              style: { ...ze, width: 32, height: 32, color: i ? n.text : n.textFaint },
              children: /* @__PURE__ */ u(Ne, { name: "undo" })
            }
          ),
          /* @__PURE__ */ u("div", { style: g }),
          /* @__PURE__ */ u(
            "button",
            {
              title: "Redo (Ctrl+Shift+Z)",
              onClick: () => t.redo(),
              disabled: !a,
              style: { ...ze, width: 32, height: 32, color: a ? n.text : n.textFaint },
              children: /* @__PURE__ */ u(Ne, { name: "redo" })
            }
          )
        ] })
      ]
    }
  );
}
const bi = 240, gi = 6;
function br(t) {
  const o = t.getAllNodes().filter((h) => h.type === "frame");
  if (o.length === 0) return [];
  const n = o.map((h) => ({
    id: h.id,
    x: h.x,
    y: h.y,
    slideOrder: h.data.slideOrder,
    label: h.data.label || "",
    borderColor: h.data.borderColor,
    transition: h.data.transition,
    transitionDuration: h.data.transitionDuration
  })), r = n.filter((h) => h.slideOrder != null).sort((h, p) => h.slideOrder - p.slideOrder), s = n.filter((h) => h.slideOrder == null), i = 100;
  s.sort((h, p) => h.y - p.y);
  const l = [];
  for (const h of s) {
    const p = l[l.length - 1];
    p && Math.abs(h.y - p[0].y) < i ? p.push(h) : l.push([h]);
  }
  const a = l.flatMap((h) => h.sort((p, d) => p.x - d.x));
  return [...r, ...a].map((h, p) => ({
    id: h.id,
    label: h.label || `Frame ${p + 1}`,
    order: p + 1,
    slideOrder: h.slideOrder,
    borderColor: h.borderColor,
    transition: h.transition,
    transitionDuration: h.transitionDuration
  }));
}
const fp = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function yp() {
  return /* @__PURE__ */ u("svg", { width: 14, height: 14, viewBox: "0 0 24 24", fill: "none", children: /* @__PURE__ */ u("path", { d: "M18 6L6 18M6 6l12 12", ...fp }) });
}
function mp(t, e, o) {
  const [n, r] = et("");
  return yt(() => {
    let s = !1;
    return Yh(t, e).then((i) => {
      s || r(i);
    }), () => {
      s = !0;
    };
  }, [t, e, o]), n;
}
function bp({ engine: t, frameId: e, tick: o }) {
  const n = mp(t, e, o);
  return n ? /* @__PURE__ */ u(
    "img",
    {
      src: n,
      alt: "",
      draggable: !1,
      style: {
        width: "100%",
        display: "block",
        borderRadius: "4px 4px 0 0",
        pointerEvents: "none"
      }
    }
  ) : /* @__PURE__ */ u(
    "div",
    {
      style: {
        width: "100%",
        aspectRatio: "16/10",
        background: "rgba(128,128,128,0.06)",
        borderRadius: "4px 4px 0 0"
      }
    }
  );
}
const xi = [
  { key: "pan", label: "Pan" },
  { key: "fade", label: "Fade" },
  { key: "dissolve", label: "Dissolve" },
  { key: "zoom", label: "Zoom" },
  { key: "fold", label: "Fold" },
  { key: "cube", label: "Cube" },
  { key: "none", label: "Cut" }
];
function wi({ type: t, size: e = 12 }) {
  const o = { stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", fill: "none" };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 16 16", fill: "none", children: [
    t === "pan" && /* @__PURE__ */ u("path", { d: "M3 8h10M10 5l3 3-3 3", ...o }),
    t === "fade" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "3", width: "5", height: "10", rx: "1", ...o, opacity: 0.4 }),
      /* @__PURE__ */ u("rect", { x: "9", y: "3", width: "5", height: "10", rx: "1", ...o })
    ] }),
    t === "dissolve" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "2", y: "3", width: "5", height: "10", rx: "1", ...o, strokeDasharray: "2,1" }),
      /* @__PURE__ */ u("rect", { x: "9", y: "3", width: "5", height: "10", rx: "1", ...o })
    ] }),
    t === "zoom" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("circle", { cx: "8", cy: "8", r: "3", ...o }),
      /* @__PURE__ */ u("path", { d: "M5 3L3 1M11 3l2-2M5 13l-2 2M11 13l2 2", ...o })
    ] }),
    t === "fold" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M2 3v10l6-5z", ...o }),
      /* @__PURE__ */ u("path", { d: "M14 3v10l-6-5z", ...o })
    ] }),
    t === "cube" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M3 4h7v8H3z", ...o }),
      /* @__PURE__ */ u("path", { d: "M10 4l3-2v8l-3 2", ...o }),
      /* @__PURE__ */ u("path", { d: "M3 4l3-2h7l-3 2", ...o })
    ] }),
    t === "none" && /* @__PURE__ */ u("path", { d: "M4 4l8 8M12 4l-8 8", ...o })
  ] });
}
const gp = [200, 300, 400, 500, 600, 800, 1e3, 1500, 2e3];
function xp({
  value: t,
  durationMs: e,
  onChange: o,
  onDurationChange: n,
  theme: r
}) {
  var m;
  const [s, i] = et(!1), [l, a] = et(!1), c = at(null), h = at(null), p = t !== "none", d = e ?? on[t];
  yt(() => {
    if (!s && !l) return;
    const y = (b) => {
      s && c.current && !c.current.contains(b.target) && i(!1), l && h.current && !h.current.contains(b.target) && a(!1);
    };
    return document.addEventListener("mousedown", y), () => document.removeEventListener("mousedown", y);
  }, [s, l]);
  const f = {
    border: `1px solid ${r.border}`,
    background: r.panelBg,
    borderRadius: 10,
    height: 20,
    padding: "0 6px",
    display: "flex",
    alignItems: "center",
    gap: 3,
    cursor: "pointer",
    color: r.textMuted,
    fontSize: 9,
    position: "relative",
    zIndex: 1
  };
  return /* @__PURE__ */ k(
    "div",
    {
      style: {
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        height: 22,
        flexShrink: 0,
        position: "relative",
        gap: 4,
        zIndex: s || l ? 50 : void 0
      },
      children: [
        /* @__PURE__ */ u("div", { style: { position: "absolute", left: 0, right: 0, top: "50%", height: 1, background: r.border } }),
        /* @__PURE__ */ k("div", { ref: c, style: { position: "relative", zIndex: 1 }, children: [
          /* @__PURE__ */ k("button", { onClick: () => {
            i((y) => !y), a(!1);
          }, style: f, children: [
            /* @__PURE__ */ u(wi, { type: t }),
            /* @__PURE__ */ u("span", { children: ((m = xi.find((y) => y.key === t)) == null ? void 0 : m.label) ?? "Pan" }),
            /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: s ? "▲" : "▼" })
          ] }),
          s && /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                top: "100%",
                left: "50%",
                transform: "translateX(-50%)",
                marginTop: 2,
                background: r.panelBg,
                border: `1px solid ${r.border}`,
                borderRadius: 6,
                padding: 3,
                zIndex: 100,
                display: "flex",
                flexDirection: "column",
                gap: 1,
                minWidth: 100,
                boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
              },
              children: xi.map((y) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    o(y.key), i(!1);
                  },
                  style: {
                    border: "none",
                    background: y.key === t ? r.controlBgActive : "transparent",
                    color: r.text,
                    borderRadius: 4,
                    padding: "4px 8px",
                    fontSize: 10,
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    width: "100%"
                  },
                  children: [
                    /* @__PURE__ */ u(wi, { type: y.key }),
                    y.label
                  ]
                },
                y.key
              ))
            }
          )
        ] }),
        p && /* @__PURE__ */ k("div", { ref: h, style: { position: "relative", zIndex: 1 }, children: [
          /* @__PURE__ */ k("button", { onClick: () => {
            a((y) => !y), i(!1);
          }, style: f, children: [
            /* @__PURE__ */ k("span", { children: [
              d,
              "ms"
            ] }),
            /* @__PURE__ */ u("span", { style: { fontSize: 7 }, children: l ? "▲" : "▼" })
          ] }),
          l && /* @__PURE__ */ u(
            "div",
            {
              style: {
                position: "absolute",
                top: "100%",
                left: "50%",
                transform: "translateX(-50%)",
                marginTop: 2,
                background: r.panelBg,
                border: `1px solid ${r.border}`,
                borderRadius: 6,
                padding: 3,
                zIndex: 100,
                display: "flex",
                flexDirection: "column",
                gap: 1,
                minWidth: 64,
                boxShadow: "0 4px 12px rgba(0,0,0,0.25)"
              },
              children: gp.map((y) => /* @__PURE__ */ k(
                "button",
                {
                  onClick: () => {
                    n(y === on[t] ? void 0 : y), a(!1);
                  },
                  style: {
                    border: "none",
                    background: y === d ? r.controlBgActive : "transparent",
                    color: r.text,
                    borderRadius: 4,
                    padding: "4px 8px",
                    fontSize: 10,
                    cursor: "pointer",
                    textAlign: "center",
                    width: "100%"
                  },
                  children: [
                    y,
                    "ms",
                    y === on[t] ? " •" : ""
                  ]
                },
                y
              ))
            }
          )
        ] })
      ]
    }
  );
}
function wp({ engine: t, open: e, onClose: o }) {
  const n = qt(), [r, s] = et(() => br(t)), [i, l] = et(() => new Set(t.selection)), [a, c] = et(0), h = at(null), p = at(null), d = at(0), f = at(!1), m = at(r);
  m.current = r;
  const y = at(!1), b = at(!1), [g, x] = et(null), [S, M] = et(null), [z, W] = et(0), I = at([]), R = at(null), K = st(() => {
    if (y.current) return;
    const $ = br(t);
    s($);
  }, [t]), Z = st(() => {
    l(new Set(t.selection));
  }, [t]), lt = at(null), O = st(() => {
    lt.current && clearTimeout(lt.current), lt.current = setTimeout(() => c(($) => $ + 1), 500);
  }, []);
  yt(() => {
    K(), Z();
    const $ = setTimeout(() => c((G) => G + 1), 200), P = () => {
      K(), O();
    };
    return t.on("change", P), t.on("node:create", P), t.on("node:delete", P), t.on("node:data", P), t.on("selection", Z), t.on("history", P), () => {
      clearTimeout($), t.off("change", P), t.off("node:create", P), t.off("node:delete", P), t.off("node:data", P), t.off("selection", Z), t.off("history", P), lt.current && clearTimeout(lt.current);
    };
  }, [t, K, Z, O]), yt(() => {
    if (!R.current) return;
    const $ = R.current.querySelectorAll("[data-frame-card]");
    I.current = Array.from($).map((P) => P.offsetHeight + gi);
  }, [r]);
  const ot = st(
    ($) => {
      t.select($), t.zoomToNode($, 0.8);
    },
    [t]
  ), Q = st(
    ($, P) => {
      $.preventDefault(), $.stopPropagation(), d.current = $.clientY, h.current = P, p.current = P, f.current = !1;
    },
    []
  );
  return yt(() => {
    const $ = (G) => {
      if (h.current === null) return;
      const X = G.clientY - d.current;
      if (!f.current) {
        if (Math.abs(X) < 4) return;
        f.current = !0, x(h.current), M(h.current);
      }
      W(X);
      const N = I.current, j = h.current;
      let H = j;
      if (X > 0) {
        let _ = 0;
        for (let tt = j + 1; tt < m.current.length && (_ += N[tt] || 0, X > _ - (N[tt] || 0) / 2); tt++)
          H = tt;
      } else if (X < 0) {
        let _ = 0;
        for (let tt = j - 1; tt >= 0 && (_ -= N[tt] || 0, X < _ + (N[tt] || 0) / 2); tt--)
          H = tt;
      }
      p.current = H, M(H);
    }, P = () => {
      const G = h.current, X = p.current;
      if (G !== null && X !== null && G !== X) {
        y.current = !0;
        const N = [...m.current], [j] = N.splice(G, 1);
        N.splice(X, 0, j);
        let H = !0;
        for (let _ = 0; _ < N.length; _++) {
          const tt = N[_], nt = t.getNode(tt.id);
          nt && (H ? (t.updateNodeWithHistory(tt.id, {
            data: { ...nt.data, slideOrder: _ + 1 }
          }), H = !1) : t.updateNode(tt.id, {
            data: { ...nt.data, slideOrder: _ + 1 }
          }));
        }
        y.current = !1, b.current = !0, s(br(t)), c((_) => _ + 1);
      }
      h.current = null, p.current = null, f.current = !1, x(null), M(null), W(0), b.current && requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          b.current = !1;
        });
      });
    };
    return document.addEventListener("pointermove", $), document.addEventListener("pointerup", P), document.addEventListener("pointercancel", P), () => {
      document.removeEventListener("pointermove", $), document.removeEventListener("pointerup", P), document.removeEventListener("pointercancel", P);
    };
  }, [t]), /* @__PURE__ */ k(
    "div",
    {
      "data-sb-frames-panel": !0,
      style: {
        position: "absolute",
        top: 0,
        right: 0,
        bottom: 0,
        width: bi,
        background: n.panelBg,
        borderLeft: `1px solid ${n.border}`,
        zIndex: 98,
        display: "flex",
        flexDirection: "column",
        transform: e ? "translateX(0)" : `translateX(${bi}px)`,
        transition: "transform 0.2s ease-in-out",
        pointerEvents: e ? "auto" : "none"
      },
      onPointerDown: ($) => $.stopPropagation(),
      children: [
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              padding: "10px 12px",
              borderBottom: `1px solid ${n.border}`,
              flexShrink: 0
            },
            children: [
              /* @__PURE__ */ k("span", { style: { fontSize: 12, fontWeight: 600, color: n.text, letterSpacing: "0.02em" }, children: [
                "Slides (",
                r.length,
                ")"
              ] }),
              /* @__PURE__ */ u(
                "button",
                {
                  title: "Close slides panel",
                  onClick: o,
                  style: {
                    border: "none",
                    background: "transparent",
                    cursor: "pointer",
                    color: n.textMuted,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    width: 24,
                    height: 24,
                    borderRadius: 4,
                    padding: 0
                  },
                  children: /* @__PURE__ */ u(yp, {})
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ k(
          "div",
          {
            ref: R,
            style: {
              flex: 1,
              overflowY: "auto",
              overflowX: "hidden",
              padding: "8px 12px",
              display: "flex",
              flexDirection: "column",
              gap: gi
            },
            children: [
              r.length === 0 && /* @__PURE__ */ u("div", { style: { padding: "20px 8px", textAlign: "center", color: n.textMuted, fontSize: 11 }, children: "No frames yet. Use the Frame tool (F) to create slides." }),
              r.map(($, P) => {
                const G = i.has($.id), X = g === P;
                let N = 0;
                if (X)
                  N = z;
                else if (g !== null && S !== null) {
                  const _ = I.current;
                  g < S ? P > g && P <= S && (N = -(_[g] || 0)) : g > S && P >= S && P < g && (N = _[g] || 0);
                }
                const j = (_) => {
                  t.updateNodeWithHistory($.id, {
                    data: { transition: _ === "pan" ? void 0 : _, transitionDuration: void 0 }
                  });
                }, H = (_) => {
                  t.updateNodeWithHistory($.id, {
                    data: { transitionDuration: _ }
                  });
                };
                return /* @__PURE__ */ k(tl.Fragment, { children: [
                  g === null && /* @__PURE__ */ u(
                    xp,
                    {
                      value: $.transition ?? "pan",
                      durationMs: $.transitionDuration,
                      onChange: j,
                      onDurationChange: H,
                      theme: n
                    }
                  ),
                  /* @__PURE__ */ u(
                    "div",
                    {
                      "data-frame-card": !0,
                      onPointerDown: (_) => Q(_, P),
                      onDoubleClick: () => ot($.id),
                      style: {
                        borderRadius: 6,
                        border: G ? `2px solid ${$.borderColor || n.text}` : `1px solid ${n.border}`,
                        background: G ? n.controlBgActive : "transparent",
                        cursor: X ? "grabbing" : "grab",
                        userSelect: "none",
                        touchAction: "none",
                        transition: X || b.current ? "none" : "transform 0.15s ease, border-color 0.1s ease",
                        transform: `translateY(${N}px)`,
                        zIndex: X ? 10 : 1,
                        opacity: X ? 0.92 : 1,
                        boxShadow: X ? "0 4px 12px rgba(0,0,0,0.18)" : "none",
                        overflow: "hidden",
                        position: "relative",
                        flexShrink: 0
                      },
                      children: /* @__PURE__ */ u(bp, { engine: t, frameId: $.id, tick: a })
                    }
                  )
                ] }, $.id);
              })
            ]
          }
        )
      ]
    }
  );
}
const ho = 50, gr = 30, kp = `
attribute vec2 aUV;
uniform float uLayPos;
uniform float uRadius;
uniform float uSide;

varying float vLight;
varying float vSpec;
varying float vShadow;
varying float vY;

void main() {
  float PI = 3.14159265;
  float d = aUV.x; // 0 = fold edge (center), 1 = far edge
  float y = aUV.y; // -1 to 1

  float x, z, theta;

  // Slight height-based radius modulation for organic curl
  float R = uRadius * (1.0 + 0.04 * sin(y * PI));

  if (d >= uLayPos) {
    // Flat — already laid on screen
    x = d;
    z = 0.0;
    theta = 0.0;
  } else {
    float arc = uLayPos - d;
    theta = arc / R;
    if (theta < PI) {
      // On the curl cylinder
      x = uLayPos - R * sin(theta);
      z = R * (1.0 - cos(theta));
    } else {
      // Back side — past the full curl, extending away
      float back = (theta - PI) * R;
      x = uLayPos + back;
      z = 2.0 * R;
      theta = PI;
    }
  }

  // Mirror for left/right half
  x *= uSide;

  // Perspective
  float eye = 2.5;
  float s = eye / (eye + z);

  // Diffuse lighting (front-facing light)
  float diffuse = 0.2 + 0.8 * max(0.0, cos(theta));
  diffuse *= (1.0 - z * 0.25); // depth darkening

  // Specular highlight on curl crest (~40 degrees)
  float specTarget = 0.7;
  vSpec = pow(max(0.0, 1.0 - abs(theta - specTarget) * 2.5), 6.0) * 0.5;

  // Shadow on flat surface near curl
  if (d >= uLayPos) {
    float distToCurl = d - uLayPos;
    vShadow = 1.0 - 0.4 * exp(-distToCurl * 25.0);
  } else {
    vShadow = 1.0;
  }

  vLight = diffuse;
  vY = y * 0.5 + 0.5; // normalized 0..1

  gl_Position = vec4(x * s * 2.0, y * s, -z * 0.01, 1.0);
}
`, vp = `
precision mediump float;
varying float vLight;
varying float vSpec;
varying float vShadow;
varying float vY;
uniform vec3 uColor;

void main() {
  // Height gradient for subtle sheen (Video Toaster aesthetic)
  float gradient = 0.88 + 0.12 * vY;
  vec3 c = uColor * vLight * gradient * vShadow + vec3(vSpec);
  gl_FragColor = vec4(c, 1.0);
}
`;
function ki(t, e, o) {
  const n = t.createShader(e);
  return n ? (t.shaderSource(n, o), t.compileShader(n), t.getShaderParameter(n, t.COMPILE_STATUS) ? n : (t.deleteShader(n), null)) : null;
}
function Sp(t, e, o) {
  const n = ki(t, t.VERTEX_SHADER, e), r = ki(t, t.FRAGMENT_SHADER, o);
  if (!n || !r) return null;
  const s = t.createProgram();
  return t.attachShader(s, n), t.attachShader(s, r), t.linkProgram(s), t.getProgramParameter(s, t.LINK_STATUS) ? s : null;
}
function Mp() {
  const t = [], e = [];
  for (let o = 0; o <= gr; o++)
    for (let n = 0; n <= ho; n++)
      t.push(n / ho, o / gr * 2 - 1);
  for (let o = 0; o < gr; o++)
    for (let n = 0; n < ho; n++) {
      const r = o * (ho + 1) + n;
      e.push(r, r + ho + 1, r + 1, r + 1, r + ho + 1, r + ho + 2);
    }
  return { vertices: new Float32Array(t), indices: new Uint16Array(e) };
}
function Cp({ phase: t, progress: e }) {
  const o = at(null), n = at(null);
  return yt(() => {
    const r = o.current;
    if (!r) return;
    const s = window.devicePixelRatio || 1;
    r.width = r.clientWidth * s, r.height = r.clientHeight * s;
    const i = r.getContext("webgl", { alpha: !0, premultipliedAlpha: !1, antialias: !0 });
    if (!i) return;
    const l = Sp(i, kp, vp);
    if (!l) return;
    i.useProgram(l);
    const { vertices: a, indices: c } = Mp(), h = i.createBuffer();
    i.bindBuffer(i.ARRAY_BUFFER, h), i.bufferData(i.ARRAY_BUFFER, a, i.STATIC_DRAW);
    const p = i.createBuffer();
    i.bindBuffer(i.ELEMENT_ARRAY_BUFFER, p), i.bufferData(i.ELEMENT_ARRAY_BUFFER, c, i.STATIC_DRAW);
    const d = i.getAttribLocation(l, "aUV");
    i.enableVertexAttribArray(d), i.vertexAttribPointer(d, 2, i.FLOAT, !1, 0, 0), i.enable(i.DEPTH_TEST), i.clearColor(0, 0, 0, 0);
    const f = (m) => i.getUniformLocation(l, m);
    return n.current = {
      gl: i,
      locs: { uLayPos: f("uLayPos"), uRadius: f("uRadius"), uSide: f("uSide"), uColor: f("uColor") },
      count: c.length
    }, () => {
      i.deleteProgram(l), i.deleteBuffer(h), i.deleteBuffer(p), n.current = null;
    };
  }, []), yt(() => {
    const r = n.current;
    if (!r) return;
    const { gl: s, locs: i, count: l } = r, a = t === "out" ? 1 - Math.pow(1 - e, 3) : Math.pow(e, 3), c = t === "out" ? 1 - a : a, h = 0.07 + 0.16 * c;
    s.viewport(0, 0, s.canvas.width, s.canvas.height), s.clear(s.COLOR_BUFFER_BIT | s.DEPTH_BUFFER_BIT), s.uniform1f(i.uLayPos, c), s.uniform1f(i.uRadius, h), s.uniform1f(i.uSide, 1), s.uniform3f(i.uColor, 0.09, 0.09, 0.17), s.drawElements(s.TRIANGLES, l, s.UNSIGNED_SHORT, 0), s.uniform1f(i.uSide, -1), s.uniform3f(i.uColor, 0.09, 0.09, 0.17), s.drawElements(s.TRIANGLES, l, s.UNSIGNED_SHORT, 0);
  }, [t, e]), /* @__PURE__ */ u(
    "canvas",
    {
      ref: o,
      style: {
        position: "absolute",
        inset: 0,
        width: "100%",
        height: "100%",
        pointerEvents: "none",
        zIndex: 9999
      }
    }
  );
}
const zp = {
  position: "absolute",
  bottom: 0,
  left: 0,
  right: 0,
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  gap: 12,
  padding: "10px 20px",
  background: "rgba(0,0,0,0.7)",
  backdropFilter: "blur(8px)",
  pointerEvents: "auto"
}, xr = {
  border: "none",
  background: "rgba(255,255,255,0.1)",
  color: "white",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  width: 32,
  height: 32,
  padding: 0
}, Nr = {
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function vi({ dir: t }) {
  return /* @__PURE__ */ k("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", children: [
    t === "left" && /* @__PURE__ */ u("polyline", { points: "15,18 9,12 15,6", ...Nr }),
    t === "right" && /* @__PURE__ */ u("polyline", { points: "9,6 15,12 9,18", ...Nr })
  ] });
}
function Ip() {
  return /* @__PURE__ */ u("svg", { width: 14, height: 14, viewBox: "0 0 24 24", fill: "none", children: /* @__PURE__ */ u("path", { d: "M18 6L6 18M6 6l12 12", ...Nr }) });
}
function Si(t) {
  return 1 - Math.pow(1 - t, 3);
}
function Mi(t) {
  return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
}
function Ci(t, e) {
  let n;
  t <= 0.2 ? n = 1 + (0.55 - 1) * Si(t / 0.2) : t >= 0.8 ? n = 0.55 + (1 - 0.55) * Si((t - 0.8) / 0.2) : n = 0.55;
  let r;
  return t <= 0.1 ? r = 0 : t <= 0.5 ? r = -e * 90 * Mi((t - 0.1) / 0.4) : t <= 0.9 ? r = e * 90 * (1 - Mi((t - 0.5) / 0.4)) : r = 0, { zoom: n, angle: r };
}
function Tp(t, e, o, n) {
  t.style.transform = `perspective(1200px) scale(${o}) rotateY(${n}deg)`, t.style.transformOrigin = "50% 50%", t.style.backfaceVisibility = "hidden", t.style.overflow = "visible", e.style.background = "#0a0a15";
}
function zi(t, e) {
  t.style.transform = "", t.style.transformOrigin = "", t.style.backfaceVisibility = "", t.style.overflow = "", e.style.background = "";
}
function Pp({ engine: t }) {
  const [e, o] = et(t.presentationMode), [n, r] = et(t.presentationIndex), [s, i] = et(t.presentationSlides.length), [l, a] = et(""), [c, h] = et(t.transitionOverlay), p = at(null), d = at(null);
  if (yt(() => {
    const m = document.querySelector("[data-sb-canvas]");
    p.current = m, d.current = (m == null ? void 0 : m.parentElement) ?? null;
    const y = () => {
      var S;
      if (o(t.presentationMode), r(t.presentationIndex), i(t.presentationSlides.length), h(t.transitionOverlay), t.presentationMode && t.presentationSlides.length > 0) {
        const M = t.presentationSlides[t.presentationIndex], z = t.getNode(M);
        a(((S = z == null ? void 0 : z.data) == null ? void 0 : S.label) || "");
      } else
        a("");
      const b = t.transitionOverlay, g = p.current, x = d.current;
      if (g && x && b && b.type === "cube" && b.t != null) {
        const M = b.direction ?? 1, { zoom: z, angle: W } = Ci(b.t, M);
        Tp(g, x, z, W);
      } else g && x && zi(g, x);
    };
    return t.on("presentation", y), () => {
      t.off("presentation", y);
      const b = p.current, g = d.current;
      b && g && zi(b, g);
    };
  }, [t]), !e || s === 0) return null;
  const f = c && c.type === "cube" && c.t != null ? (() => {
    const m = c.direction ?? 1, { angle: y } = Ci(c.t, m);
    return Math.abs(y) / 90 * 0.4;
  })() : 0;
  return /* @__PURE__ */ k(
    "div",
    {
      "data-sb-presentation": !0,
      style: {
        position: "absolute",
        inset: 0,
        zIndex: 1e4,
        pointerEvents: "none"
      },
      children: [
        c && c.type !== "fold" && c.type !== "cube" && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              backgroundColor: "black",
              opacity: c.phase === "out" ? c.progress : 1 - c.progress,
              pointerEvents: "none",
              zIndex: 9999
            }
          }
        ),
        c && c.type === "fold" && /* @__PURE__ */ u(Cp, { phase: c.phase, progress: c.progress }),
        f > 0.01 && /* @__PURE__ */ u(
          "div",
          {
            style: {
              position: "absolute",
              inset: 0,
              backgroundColor: "black",
              opacity: f,
              pointerEvents: "none",
              zIndex: 9999
            }
          }
        ),
        /* @__PURE__ */ k("div", { style: zp, onPointerDown: (m) => m.stopPropagation(), children: [
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...xr, position: "absolute", right: 16 },
              title: "Exit presentation (Esc)",
              onClick: () => t.exitPresentation(),
              children: /* @__PURE__ */ u(Ip, {})
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...xr, opacity: n <= 0 ? 0.3 : 1 },
              title: "Previous slide (←)",
              onClick: () => t.presentationPrev(),
              disabled: n <= 0,
              children: /* @__PURE__ */ u(vi, { dir: "left" })
            }
          ),
          /* @__PURE__ */ k(
            "span",
            {
              style: {
                color: "white",
                fontSize: 13,
                fontWeight: 500,
                fontFamily: "inherit",
                minWidth: 80,
                textAlign: "center",
                userSelect: "none"
              },
              children: [
                n + 1,
                " / ",
                s,
                l && /* @__PURE__ */ k("span", { style: { opacity: 0.6, marginLeft: 8 }, children: [
                  "— ",
                  l
                ] })
              ]
            }
          ),
          /* @__PURE__ */ u(
            "button",
            {
              style: { ...xr, opacity: n >= s - 1 ? 0.3 : 1 },
              title: "Next slide (→)",
              onClick: () => t.presentationNext(),
              disabled: n >= s - 1,
              children: /* @__PURE__ */ u(vi, { dir: "right" })
            }
          )
        ] })
      ]
    }
  );
}
const Rp = nl(() => import("./DebugPanel-tGeazws4.js"));
function Up({
  nodeTypes: t = Dd,
  engine: e,
  keyboardShortcuts: o = !0,
  style: n,
  initialData: r,
  toolbar: s = !0,
  debugPanel: i = !1,
  debugBoards: l,
  theme: a,
  onPresentationChange: c,
  gifApiBaseUrl: h
}) {
  const p = Kt(
    () => e ?? new ec(),
    [e]
  ), d = Kt(() => new oc(t), [t]);
  yt(() => Wl(), []), yt(() => {
    p.setRegistry(d);
  }, [p, d]), yt(() => {
    for (const z of t)
      z.isContainer && p.registerContainerType(z.type);
  }, [p, t]);
  const f = at(!1);
  yt(() => {
    r && !f.current && (f.current = !0, p.fromSBD(r));
  }, [p, r]);
  const m = at(null);
  yt(() => {
    if (o)
      return Sh(p, m.current);
  }, [p, o]);
  const y = Kt(() => t.some((W) => {
    var I;
    return (I = W.ports) == null ? void 0 : I.length;
  }) ? new Wd(p, d) : null, [p, d, t]);
  yt(() => {
    if (y)
      return y.connect();
  }, [y]);
  const b = Kt(
    () => a ? { ...Rr, ...a } : Rr,
    [a]
  ), [g, x] = et(!1), [S, M] = et(!1);
  return yt(() => {
    const z = () => {
      const W = p.presentationMode;
      x(W), c == null || c(W);
    };
    return p.on("presentation", z), () => p.off("presentation", z);
  }, [p, c]), /* @__PURE__ */ u(na.Provider, { value: b, children: /* @__PURE__ */ k(
    "div",
    {
      ref: m,
      style: {
        width: "100%",
        height: "100%",
        position: "relative",
        ...n
      },
      children: [
        s && !g && /* @__PURE__ */ u(cp, { engine: p, registry: d, gifApiBaseUrl: h }),
        i && /* @__PURE__ */ u(ol, { fallback: null, children: /* @__PURE__ */ u(Rp, { engine: p, extraBoards: l }) }),
        /* @__PURE__ */ k(
          "div",
          {
            style: {
              position: "absolute",
              left: s && !g ? dn : 0,
              top: 0,
              right: 0,
              bottom: 0,
              overflow: "hidden"
            },
            children: [
              /* @__PURE__ */ u(Qh, { engine: p, schema: Xr, registry: d, dataFlow: y }),
              !g && /* @__PURE__ */ u(
                pp,
                {
                  engine: p,
                  framesPanelOpen: S,
                  onToggleFramesPanel: () => M((z) => !z)
                }
              ),
              !g && /* @__PURE__ */ u(
                wp,
                {
                  engine: p,
                  open: S,
                  onClose: () => M(!1)
                }
              ),
              /* @__PURE__ */ u(Pp, { engine: p })
            ]
          }
        )
      ]
    }
  ) });
}
const Ap = [
  { key: "select", label: "Select", shortcut: "V" },
  { key: "draw", label: "Draw", shortcut: "D" },
  { key: "shape", label: "Shape", shortcut: "S" },
  { key: "text", label: "Text", shortcut: "T" },
  { key: "note", label: "Note", shortcut: "N" },
  { key: "sticky", label: "Sticky", shortcut: "P" },
  { key: "frame", label: "Frame", shortcut: "F" },
  { key: "erase", label: "Eraser", shortcut: "X" }
], Eo = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0
}, $t = {
  stroke: "currentColor",
  strokeWidth: 1.5,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
function tn({ name: t, size: e = 18 }) {
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "select" && /* @__PURE__ */ u("path", { d: "M6 2v17l4-4.5L13.5 21l2.5-1.5L12.5 13H18z", fill: "currentColor" }),
    t === "draw" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M17 3l4 4L7.5 20.5 2 22l1.5-5.5z", ...$t }),
      /* @__PURE__ */ u("path", { d: "M15 5l4 4", ...$t })
    ] }),
    t === "shape" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...$t }),
    t === "text" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M7 4h10", ...$t }),
      /* @__PURE__ */ u("path", { d: "M12 4v16", ...$t })
    ] }),
    t === "note" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M4 3h16v14l-4 4H4z", ...$t }),
      /* @__PURE__ */ u("path", { d: "M16 17v4l4-4z", fill: "currentColor", opacity: 0.4 })
    ] }),
    t === "sticky" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "1", fill: "currentColor", opacity: 0.15, ...$t }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "9", x2: "17", y2: "9", ...$t, opacity: 0.5 }),
      /* @__PURE__ */ u("line", { x1: "7", y1: "13", x2: "14", y2: "13", ...$t, opacity: 0.5 })
    ] }),
    t === "frame" && /* @__PURE__ */ u("rect", { x: "3", y: "3", width: "18", height: "18", rx: "2", ...$t, strokeDasharray: "4,2" }),
    t === "erase" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M20 20H9L3 14l9.5-9.5 8 8L16 17", ...$t }),
      /* @__PURE__ */ u("path", { d: "M12.5 4.5l8 8", ...$t })
    ] }),
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...$t }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...$t }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...$t }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...$t }),
    t === "arrow" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...$t }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...$t, fill: "none" })
    ] }),
    t === "undo" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M4 9h11a4 4 0 0 1 0 8h-4", ...$t, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "8,5 4,9 8,13", ...$t, fill: "none" })
    ] }),
    t === "redo" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M20 9H9a4 4 0 0 0 0 8h4", ...$t, fill: "none" }),
      /* @__PURE__ */ u("polyline", { points: "16,5 20,9 16,13", ...$t, fill: "none" })
    ] }),
    t === "print" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M6 9V3h12v6", ...$t }),
      /* @__PURE__ */ u("path", { d: "M6 18H4a1 1 0 0 1-1-1v-5a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-2", ...$t }),
      /* @__PURE__ */ u("rect", { x: "6", y: "14", width: "12", height: "7", rx: "1", ...$t })
    ] }),
    t === "fit" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("path", { d: "M15 3h6v6M9 21H3v-6", ...$t }),
      /* @__PURE__ */ u("path", { d: "M21 3l-7 7M3 21l7-7", ...$t })
    ] })
  ] });
}
function Qp({ engine: t }) {
  const [e, o] = et(t.mode), [n, r] = et(!1), [s, i] = et(!1), [l, a] = et(t.boardBackground);
  return yt(() => {
    const c = () => o(t.mode), h = () => {
      r(t.canUndo()), i(t.canRedo());
    }, p = () => a(t.boardBackground);
    return t.on("mode", c), t.on("history", h), t.on("background", p), () => {
      t.off("mode", c), t.off("history", h), t.off("background", p);
    };
  }, [t]), /* @__PURE__ */ k(
    "div",
    {
      "data-sb-toolbar": !0,
      style: {
        position: "absolute",
        left: 0,
        top: 0,
        bottom: 0,
        width: 48,
        background: "#1e1e2e",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        padding: "8px 0",
        gap: 4,
        zIndex: 100
      },
      children: [
        Ap.map((c) => /* @__PURE__ */ u(
          "button",
          {
            title: `${c.label} (${c.shortcut})`,
            onClick: () => t.setMode(c.key),
            style: {
              ...Eo,
              width: 36,
              height: 36,
              background: e === c.key ? "#3b82f6" : "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u(tn, { name: c.key })
          },
          c.key
        )),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "8px 0"
            }
          }
        ),
        [
          { key: "dot-grid", color: "#f8f7f5", label: "Dot Grid" },
          { key: "blueprint", color: "#1e3a5f", label: "Blueprint" },
          { key: "japanese-stationery", color: "#f5f0e8", label: "Japanese Stationery" }
        ].map((c) => /* @__PURE__ */ u(
          "button",
          {
            title: c.label,
            onClick: () => t.setBoardBackground(c.key),
            style: {
              ...Eo,
              width: 20,
              height: 20,
              background: c.color,
              border: l === c.key ? "2px solid white" : "2px solid transparent",
              borderRadius: 4,
              boxShadow: c.key === "japanese-stationery" ? "inset 0 1px 0 rgba(255,255,255,0.5), inset 0 -1px 0 rgba(160,140,110,0.15)" : void 0
            }
          },
          c.key
        )),
        /* @__PURE__ */ u("div", { style: { flex: 1 } }),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Print (landscape)",
            onClick: () => {
              const c = { ...t.viewport };
              t.fitToContent(), requestAnimationFrame(() => {
                window.print(), t.viewport.x = c.x, t.viewport.y = c.y, t.viewport.zoom = c.zoom, t.pan(0, 0);
              });
            },
            style: {
              ...Eo,
              width: 36,
              height: 36,
              background: "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u(tn, { name: "print" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "4px 0"
            }
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Undo (Ctrl+Z)",
            onClick: () => t.undo(),
            disabled: !n,
            style: {
              ...Eo,
              width: 36,
              height: 36,
              background: "transparent",
              color: n ? "white" : "#666"
            },
            children: /* @__PURE__ */ u(tn, { name: "undo" })
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Redo (Ctrl+Shift+Z)",
            onClick: () => t.redo(),
            disabled: !s,
            style: {
              ...Eo,
              width: 36,
              height: 36,
              background: "transparent",
              color: s ? "white" : "#666"
            },
            children: /* @__PURE__ */ u(tn, { name: "redo" })
          }
        ),
        /* @__PURE__ */ u(
          "div",
          {
            style: {
              width: 28,
              height: 1,
              background: "#444",
              margin: "4px 0"
            }
          }
        ),
        /* @__PURE__ */ u(
          "button",
          {
            title: "Fit to content (Ctrl+0)",
            onClick: () => t.fitToContent(),
            style: {
              ...Eo,
              width: 36,
              height: 36,
              background: "transparent",
              color: "white"
            },
            children: /* @__PURE__ */ u(tn, { name: "fit" })
          }
        )
      ]
    }
  );
}
const Ke = [
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], Lp = [
  null,
  // no fill
  "#1e1e2e",
  "#e74c3c",
  "#2ecc71",
  "#3498db",
  "#f39c12",
  "#9b59b6"
], Ep = [
  { key: "hachure", label: "Hachure" },
  { key: "cross-hatch", label: "Cross-hatch" },
  { key: "solid", label: "Solid" }
], Do = [
  { key: "solid", label: "Solid", dash: "" },
  { key: "dashed", label: "Dashed", dash: "6,3" },
  { key: "dotted", label: "Dotted", dash: "2,2" }
], Dp = [
  { value: 0, label: "Architect" },
  { value: 1, label: "Artist" },
  { value: 2, label: "Cartoonist" }
], Wo = [1, 2.5, 5, 10, 20], Wp = [
  { key: "rect", label: "Rectangle" },
  { key: "ellipse", label: "Ellipse" },
  { key: "diamond", label: "Diamond" },
  { key: "line", label: "Line" },
  { key: "arrow", label: "Arrow" }
], Fp = [14, 20, 28, 36], Bp = [
  { key: "left", label: "←" },
  { key: "center", label: "↔" },
  { key: "right", label: "→" }
], wr = 300, Bt = {
  display: "flex",
  alignItems: "center",
  gap: 4
}, Nt = {
  width: 64,
  fontSize: 10,
  color: "#999",
  flexShrink: 0
}, Vt = {
  border: "none",
  borderRadius: 6,
  cursor: "pointer",
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  padding: 0,
  flexShrink: 0
};
function Jp({
  engine: t,
  registry: e
}) {
  const [o, n] = et(t.mode), [r, s] = et(t.selection), [, i] = et(0), [l, a] = et(null), c = at(null), h = at(null), [p, d] = et(!1), f = st(() => {
    var it;
    return { x: (((it = c.current) == null ? void 0 : it.ownerDocument.defaultView) ?? window).innerWidth - wr - 12, y: 12 };
  }, []), m = l ?? f();
  yt(() => {
    const v = () => n(t.mode), it = () => {
      s(new Set(t.selection)), i((_t) => _t + 1);
    }, Yt = () => i((_t) => _t + 1);
    return t.on("mode", v), t.on("selection", it), t.on("change", Yt), () => {
      t.off("mode", v), t.off("selection", it), t.off("change", Yt);
    };
  }, [t]);
  const y = st((v) => {
    v.stopPropagation(), d(!0);
    const it = l ? l.x : f().x, Yt = l ? l.y : f().y;
    h.current = { startX: v.clientX, startY: v.clientY, startLeft: it, startTop: Yt }, v.currentTarget.setPointerCapture(v.pointerId);
  }, [l, f]);
  yt(() => {
    var _t;
    const v = (De) => {
      var ko;
      if (!h.current) return;
      const so = De.clientX - h.current.startX, We = De.clientY - h.current.startY, wo = ((ko = c.current) == null ? void 0 : ko.ownerDocument.defaultView) ?? window, qo = Math.max(48, Math.min(wo.innerWidth - wr - 8, h.current.startLeft + so)), Fe = Math.max(8, Math.min(wo.innerHeight - 100, h.current.startTop + We));
      a({ x: qo, y: Fe });
    }, it = () => {
      h.current = null, d(!1);
    }, Yt = ((_t = c.current) == null ? void 0 : _t.ownerDocument) ?? document;
    return Yt.addEventListener("pointermove", v), Yt.addEventListener("pointerup", it), Yt.addEventListener("pointercancel", it), () => {
      Yt.removeEventListener("pointermove", v), Yt.removeEventListener("pointerup", it), Yt.removeEventListener("pointercancel", it);
    };
  }, []);
  const b = (() => {
    if (r.size === 1) {
      const v = Array.from(r)[0], it = t.getNode(v);
      if ((it == null ? void 0 : it.type) === "shape") return { kind: "shape", node: it };
      if ((it == null ? void 0 : it.type) === "draw") return { kind: "draw", node: it };
      if ((it == null ? void 0 : it.type) === "text") return { kind: "text", node: it };
      if ((it == null ? void 0 : it.type) === "edge") return { kind: "edge", node: it };
      if ((it == null ? void 0 : it.type) === "image") return { kind: "image", node: it };
      if ((it == null ? void 0 : it.type) === "content") return { kind: "content", node: it };
      if ((it == null ? void 0 : it.type) === "frame") return { kind: "frame", node: it };
      if ((it == null ? void 0 : it.type) === "sticky") return { kind: "sticky", node: it };
      if (it && e) {
        const Yt = e.get(it.type);
        if (Yt != null && Yt.propertiesPanel)
          return { kind: "custom", node: it, PanelComponent: Yt.propertiesPanel };
      }
    }
    return o === "draw" || o === "shape" || o === "text" ? { kind: "tool" } : null;
  })(), g = st(
    (v) => {
      !b || b.kind !== "shape" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), x = st(
    (v) => {
      !b || b.kind !== "draw" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), S = st(
    (v) => {
      !b || b.kind !== "text" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), M = st(
    (v) => {
      !b || b.kind !== "edge" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), z = st(
    (v) => {
      !b || b.kind !== "image" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), W = st(
    (v) => {
      !b || b.kind !== "content" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), I = st(
    (v) => {
      !b || b.kind !== "frame" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), R = st(
    (v) => {
      !b || b.kind !== "sticky" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), K = st(
    (v) => {
      !b || b.kind !== "custom" || t.updateNodeWithHistory(b.node.id, {
        data: { ...b.node.data, ...v }
      });
    },
    [t, b]
  ), [Z, lt] = et("idle");
  if (!b) return null;
  const O = b.kind === "custom", ot = b.kind === "shape", Q = b.kind === "draw", $ = b.kind === "text", P = b.kind === "edge", G = b.kind === "image", X = b.kind === "content", N = b.kind === "frame", j = b.kind === "sticky", H = b.kind === "tool", _ = H && o === "shape", tt = H && o === "text", nt = $ ? b.node.data.fontFamily : t.activeTool.fontFamily ?? Qe, ut = $ ? b.node.data.fontSize : t.activeTool.fontSize ?? 20, pt = $ ? b.node.data.align : t.activeTool.textAlign ?? "left", vt = $ ? b.node.data.color : t.activeTool.color, rt = ot ? b.node.data.stroke : Q ? b.node.data.color : t.activeTool.color, gt = ot || Q ? b.node.data.fill ?? null : t.activeTool.fillColor ?? null, xt = ot || Q ? b.node.data.fillStyle ?? "hachure" : t.activeTool.fillStyle ?? "hachure", ht = ot || Q ? b.node.data.strokeStyle ?? "solid" : t.activeTool.strokeStyle ?? "solid", Dt = ot || Q ? b.node.data.strokeWidth : t.activeTool.width, Tt = ot ? b.node.data.roughness : t.activeTool.roughness ?? 1, Rt = ot || Q || $ || G || X || N || j ? b.node.data.opacity ?? 1 : t.activeTool.opacity ?? 1, Qt = (() => {
    const v = /* @__PURE__ */ new Set(), it = [];
    for (const Yt of t.getAllNodes())
      if (Yt.type === "text") {
        const _t = Yt.data.fontFamily;
        _t && !v.has(_t) && (v.add(_t), it.push(_t));
      }
    return it;
  })(), Ut = !$ && !tt && !P && !G && !X && !N && !j && !O, re = Ut, Jt = Ut, oe = ot || _, ve = $ || tt, ye = (v) => {
    ot ? g({ stroke: v }) : Q ? x({ color: v }) : (t.activeTool.color = v, i((it) => it + 1));
  }, se = (v) => {
    ot ? g({ fill: v ?? void 0 }) : Q ? x({ fill: v ?? void 0 }) : (t.activeTool.fillColor = v ?? void 0, i((it) => it + 1));
  }, Ae = (v) => {
    ot ? g({ fillStyle: v }) : Q ? x({ fillStyle: v }) : (t.activeTool.fillStyle = v, i((it) => it + 1));
  }, Xe = (v) => {
    ot ? g({ strokeStyle: v }) : Q ? x({ strokeStyle: v }) : (t.activeTool.strokeStyle = v, i((it) => it + 1));
  }, Le = (v) => {
    ot ? g({ strokeWidth: v }) : Q ? x({ strokeWidth: v }) : (t.activeTool.width = v, i((it) => it + 1));
  }, me = (v) => {
    ot ? g({ roughness: v }) : (t.activeTool.roughness = v, i((it) => it + 1));
  }, no = (v) => {
    ot ? g({ opacity: v }) : Q ? x({ opacity: v }) : $ ? S({ opacity: v }) : G ? z({ opacity: v }) : X ? W({ opacity: v }) : N ? I({ opacity: v }) : j ? R({ opacity: v }) : (t.activeTool.opacity = v, i((it) => it + 1));
  }, ro = (v) => {
    $ ? S({ fontFamily: v }) : (t.activeTool.fontFamily = v, i((it) => it + 1));
  }, Se = (v) => {
    $ ? S({ fontSize: v }) : (t.activeTool.fontSize = v, i((it) => it + 1));
  }, be = (v) => {
    $ ? S({ align: v }) : (t.activeTool.textAlign = v, i((it) => it + 1));
  }, Me = (v) => {
    $ ? S({ color: v }) : (t.activeTool.color = v, i((it) => it + 1));
  }, Ee = {
    position: "fixed",
    left: m.x,
    top: m.y,
    width: wr,
    background: "#1e1e2e",
    borderRadius: 12,
    padding: "12px 16px",
    display: "flex",
    flexDirection: "column",
    gap: 10,
    zIndex: 99,
    color: "white",
    fontSize: 11,
    maxHeight: "calc(100vh - 40px)",
    overflowY: "auto",
    boxShadow: "0 4px 24px rgba(0,0,0,0.4)"
  };
  return /* @__PURE__ */ k(
    "div",
    {
      ref: c,
      "data-sb-props-panel": !0,
      style: Ee,
      onPointerDown: (v) => v.stopPropagation(),
      children: [
        /* @__PURE__ */ u(
          "div",
          {
            onPointerDown: y,
            style: {
              cursor: p ? "grabbing" : "grab",
              margin: "-12px -16px 8px -16px",
              padding: "8px 16px",
              userSelect: "none",
              display: "flex",
              alignItems: "center",
              gap: 6,
              borderBottom: "1px solid #333",
              color: "#999",
              fontSize: 10
            },
            children: /* @__PURE__ */ u("span", { style: { fontWeight: 600, letterSpacing: "0.02em", color: "white" }, children: "Inspector" })
          }
        ),
        ve && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Font" }),
            /* @__PURE__ */ u(
              Vn,
              {
                value: nt,
                onChange: ro,
                fontsInScene: Qt
              }
            )
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Size" }),
            Fp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => Se(v),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: ut === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: v
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Align" }),
            Bp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.key,
                onClick: () => be(v.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: pt === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 12,
                  borderRadius: 6
                },
                children: v.label
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            Ke.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => Me(v),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: vt === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          $ && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...Ke].map((v, it) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => S({ borderColor: v ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (b.node.data.borderColor ?? null) === v ? "2px solid white" : `2px solid ${it === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: it === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: 2,
                      height: 24,
                      background: "#e74c3c",
                      transform: "rotate(45deg)"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          $ && b.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Do.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => S({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (b.node.data.borderStyle ?? "solid") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          $ && b.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Wo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => S({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (b.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] })
        ] }),
        Ut && /* @__PURE__ */ k(ft, { children: [
          _ && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Shape" }),
            Wp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => {
                  t.activeTool.shapeType = v.key, i((it) => it + 1);
                },
                style: {
                  ...Vt,
                  width: 28,
                  height: 28,
                  background: (t.activeTool.shapeType ?? "rect") === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(Np, { name: v.key })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke" }),
            Ke.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => ye(v),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: rt === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          re && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Fill" }),
            Lp.map((v, it) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => se(v),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: gt === v ? "2px solid white" : `2px solid ${it === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: it === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          re && gt && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Fill pattern" }),
            Ep.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => Ae(v.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: xt === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 9,
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(Hp, { style: v.key })
              },
              v.key
            ))
          ] }),
          Jt && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke style" }),
            Do.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => Xe(v.key),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: ht === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Stroke width" }),
            Wo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => Le(v),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: Dt === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] }),
          oe && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Roughness" }),
            Dp.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => me(v.value),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: Tt === v.value ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 9,
                  borderRadius: 6
                },
                children: v.label
              },
              v.value
            ))
          ] })
        ] }),
        P && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            Ke.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => M({ color: v }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: b.node.data.color === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Do.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => M({ style: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: b.node.data.style === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Wo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => M({ strokeWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: b.node.data.strokeWidth === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Head" }),
            ["none", "arrow", "filled", "dot"].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => M({ arrowHead: v }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (b.node.data.arrowHead ?? "none") === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: v === "none" ? "None" : v === "arrow" ? "▷" : v === "filled" ? "▶" : "●"
              },
              v
            ))
          ] }),
          (b.node.data.arrowHead ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Head size" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "range",
                min: 4,
                max: 40,
                step: 1,
                value: b.node.data.arrowHeadSize ?? Math.max(8, b.node.data.strokeWidth * 3),
                onChange: (v) => M({ arrowHeadSize: Number(v.target.value) }),
                style: { flex: 1 }
              }
            ),
            /* @__PURE__ */ u("span", { style: { color: "#999", fontSize: 11, minWidth: 24, textAlign: "right" }, children: b.node.data.arrowHeadSize ?? Math.max(8, b.node.data.strokeWidth * 3) })
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Tail" }),
            ["none", "arrow", "filled", "dot"].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => M({ arrowTail: v }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (b.node.data.arrowTail ?? "none") === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: v === "none" ? "None" : v === "arrow" ? "◁" : v === "filled" ? "◀" : "●"
              },
              v
            ))
          ] }),
          (b.node.data.arrowTail ?? "none") !== "none" && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Tail size" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "range",
                min: 4,
                max: 40,
                step: 1,
                value: b.node.data.arrowTailSize ?? Math.max(8, b.node.data.strokeWidth * 3),
                onChange: (v) => M({ arrowTailSize: Number(v.target.value) }),
                style: { flex: 1 }
              }
            ),
            /* @__PURE__ */ u("span", { style: { color: "#999", fontSize: 11, minWidth: 24, textAlign: "right" }, children: b.node.data.arrowTailSize ?? Math.max(8, b.node.data.strokeWidth * 3) })
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Label" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                value: b.node.data.label ?? "",
                onChange: (v) => M({ label: v.target.value || void 0 }),
                placeholder: "Edge label...",
                style: {
                  flex: 1,
                  background: "#2a2a3e",
                  color: "white",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 8px",
                  fontSize: 11,
                  outline: "none"
                }
              }
            )
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Path" }),
            [
              { key: "bezier", label: "Bezier" },
              { key: "straight", label: "Straight" },
              { key: "smoothstep", label: "Smooth" },
              { key: "step", label: "Step" }
            ].map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => M({ edgeType: v.key }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (b.node.data.edgeType ?? "bezier") === v.key ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: v.label
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Animate" }),
            /* @__PURE__ */ u(
              "button",
              {
                onClick: () => M({ animated: !b.node.data.animated }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 12px",
                  background: b.node.data.animated ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 11,
                  borderRadius: 6
                },
                children: b.node.data.animated ? "On" : "Off"
              }
            )
          ] }),
          b.node.data.animated && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Direction" }),
            ["forward", "reverse", "both"].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => M({ animatedDirection: v }),
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 8px",
                  background: (b.node.data.animatedDirection ?? "forward") === v ? "#3b82f6" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6
                },
                children: v === "forward" ? "→" : v === "reverse" ? "←" : "⇆"
              },
              v
            ))
          ] })
        ] }),
        G && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...Ke].map((v, it) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => z({ borderColor: v ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (b.node.data.borderColor ?? null) === v ? "2px solid white" : `2px solid ${it === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: it === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          b.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Do.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => z({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (b.node.data.borderStyle ?? "solid") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          b.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Wo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => z({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (b.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: { ...Bt, marginTop: 4 }, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Background" }),
            /* @__PURE__ */ u(
              "button",
              {
                onClick: async () => {
                  if (!(Z === "loading" || b.kind !== "image")) {
                    lt("loading");
                    try {
                      const { removeBackground: v } = await import("@imgly/background-removal"), Yt = await (await fetch(b.node.data.src)).blob(), _t = await v(Yt), De = new FileReader(), so = await new Promise((We, wo) => {
                        De.onload = () => We(De.result), De.onerror = wo, De.readAsDataURL(_t);
                      });
                      z({ src: so }), lt("idle");
                    } catch (v) {
                      console.error("Background removal failed:", v), lt("error"), setTimeout(() => lt("idle"), 3e3);
                    }
                  }
                },
                disabled: Z === "loading",
                style: {
                  ...Vt,
                  height: 28,
                  padding: "0 10px",
                  background: Z === "error" ? "#e74c3c" : "#2a2a3e",
                  color: "white",
                  fontSize: 10,
                  borderRadius: 6,
                  gap: 4,
                  opacity: Z === "loading" ? 0.6 : 1
                },
                children: Z === "loading" ? "Removing..." : Z === "error" ? "Failed" : "Remove BG"
              }
            )
          ] })
        ] }),
        X && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            [null, ...Ke].map((v, it) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => W({ borderColor: v ?? void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (b.node.data.borderColor ?? null) === v ? "2px solid white" : `2px solid ${it === 0 ? "#555" : "transparent"}`,
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: it === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          b.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Do.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => W({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (b.node.data.borderStyle ?? "solid") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          b.node.data.borderColor && /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Wo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => W({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (b.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] })
        ] }),
        N && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Label" }),
            /* @__PURE__ */ u(
              "input",
              {
                type: "text",
                value: b.node.data.label ?? "",
                onChange: (v) => I({ label: v.target.value || void 0 }),
                placeholder: "Frame label...",
                style: {
                  flex: 1,
                  background: "#2a2a3e",
                  color: "white",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 8px",
                  fontSize: 11,
                  outline: "none"
                }
              }
            )
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Background" }),
            [null, ...Ke].map((v, it) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ backgroundColor: v ? `${v}15` : void 0 }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v ?? "transparent",
                  border: (() => {
                    const Yt = b.node.data.backgroundColor;
                    return (v === null ? !Yt : Yt === `${v}15`) ? "2px solid white" : `2px solid ${it === 0 ? "#555" : "transparent"}`;
                  })(),
                  borderRadius: "50%",
                  position: "relative",
                  overflow: "hidden"
                },
                children: it === 0 && /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      position: "absolute",
                      width: "140%",
                      height: 2,
                      background: "#e74c3c",
                      transform: "rotate(-45deg)",
                      top: "50%",
                      left: "-20%"
                    }
                  }
                )
              },
              v ?? "none"
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Border" }),
            Ke.map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => I({ borderColor: v }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: b.node.data.borderColor === v ? "2px solid white" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Style" }),
            Do.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: v.label,
                onClick: () => I({ borderStyle: v.key }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 28,
                  background: (b.node.data.borderStyle ?? "dashed") === v.key ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u("svg", { width: 24, height: 12, children: /* @__PURE__ */ u(
                  "line",
                  {
                    x1: 2,
                    y1: 6,
                    x2: 22,
                    y2: 6,
                    stroke: "white",
                    strokeWidth: 2,
                    strokeDasharray: v.dash
                  }
                ) })
              },
              v.key
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Width" }),
            Wo.map((v) => /* @__PURE__ */ u(
              "button",
              {
                title: `${v}px`,
                onClick: () => I({ borderWidth: v }),
                style: {
                  ...Vt,
                  width: 36,
                  height: 24,
                  background: (b.node.data.borderWidth ?? 1) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6
                },
                children: /* @__PURE__ */ u(
                  "div",
                  {
                    style: {
                      width: 20,
                      height: Math.max(v, 1),
                      background: "white",
                      borderRadius: v / 2
                    }
                  }
                )
              },
              v
            ))
          ] })
        ] }),
        j && /* @__PURE__ */ k(ft, { children: [
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Color" }),
            [
              "#FEF3C7",
              "#FCE7F3",
              "#DBEAFE",
              "#D1FAE5",
              "#EDE9FE",
              "#FFEDD5"
            ].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => R({ color: v }),
                style: {
                  ...Vt,
                  width: 20,
                  height: 20,
                  background: v,
                  border: b.node.data.color === v ? "2px solid #1e1e2e" : "2px solid transparent",
                  borderRadius: "50%"
                }
              },
              v
            ))
          ] }),
          /* @__PURE__ */ k("div", { style: Bt, children: [
            /* @__PURE__ */ u("span", { style: Nt, children: "Size" }),
            [12, 14, 16, 20, 24].map((v) => /* @__PURE__ */ u(
              "button",
              {
                onClick: () => R({ fontSize: v }),
                style: {
                  ...Vt,
                  width: 32,
                  height: 24,
                  background: (b.node.data.fontSize ?? 16) === v ? "#3b82f6" : "#2a2a3e",
                  borderRadius: 6,
                  fontSize: 10,
                  color: "white"
                },
                children: v
              },
              v
            ))
          ] })
        ] }),
        O && (() => {
          const { node: v, PanelComponent: it } = b;
          return /* @__PURE__ */ u(it, { node: v, data: v.data, engine: t, updateData: K });
        })(),
        !P && !O && /* @__PURE__ */ k("div", { style: Bt, children: [
          /* @__PURE__ */ u("span", { style: Nt, children: "Opacity" }),
          /* @__PURE__ */ u(
            "input",
            {
              type: "range",
              min: 0,
              max: 100,
              value: Math.round(Rt * 100),
              onChange: (v) => no(parseInt(v.target.value) / 100),
              style: { flex: 1, accentColor: "#3b82f6" }
            }
          ),
          /* @__PURE__ */ u("span", { style: { width: 28, textAlign: "right", fontSize: 10 }, children: Math.round(Rt * 100) })
        ] })
      ]
    }
  );
}
function Np({ name: t, size: e = 16 }) {
  const o = {
    stroke: "currentColor",
    strokeWidth: 1.5,
    strokeLinecap: "round",
    strokeLinejoin: "round"
  };
  return /* @__PURE__ */ k("svg", { width: e, height: e, viewBox: "0 0 24 24", fill: "none", children: [
    t === "rect" && /* @__PURE__ */ u("rect", { x: "4", y: "4", width: "16", height: "16", rx: "2", ...o }),
    t === "ellipse" && /* @__PURE__ */ u("ellipse", { cx: "12", cy: "12", rx: "9", ry: "8", ...o }),
    t === "diamond" && /* @__PURE__ */ u("path", { d: "M12 3l9 9-9 9-9-9z", ...o }),
    t === "line" && /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
    t === "arrow" && /* @__PURE__ */ k(ft, { children: [
      /* @__PURE__ */ u("line", { x1: "5", y1: "19", x2: "19", y2: "5", ...o }),
      /* @__PURE__ */ u("polyline", { points: "12,5 19,5 19,12", ...o, fill: "none" })
    ] })
  ] });
}
function Hp({ style: t }) {
  return t === "hachure" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: "white", strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: "white", strokeWidth: 1.5 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: "white", strokeWidth: 1.5 })
  ] }) : t === "cross-hatch" ? /* @__PURE__ */ k("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: [
    /* @__PURE__ */ u("line", { x1: 2, y1: 14, x2: 8, y2: 2, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 14, x2: 14, y2: 2, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 14, y1: 14, x2: 18, y2: 6, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 2, y1: 2, x2: 8, y2: 14, stroke: "white", strokeWidth: 1.2 }),
    /* @__PURE__ */ u("line", { x1: 8, y1: 2, x2: 14, y2: 14, stroke: "white", strokeWidth: 1.2 })
  ] }) : /* @__PURE__ */ u("svg", { width: 20, height: 16, viewBox: "0 0 20 16", children: /* @__PURE__ */ u("rect", { x: 2, y: 2, width: 16, height: 12, fill: "white", rx: 2 }) });
}
export {
  Qe as D,
  oc as N,
  Xo as P,
  Zp as S,
  Qp as T,
  Rr as a,
  Wd as b,
  Jp as c,
  cp as d,
  Up as e,
  Qh as f,
  ec as g,
  Dd as h,
  dc as i,
  ld as j,
  ud as k,
  kd as l,
  ln as m,
  bt as n,
  Zr as o,
  md as p,
  Gr as q,
  Bl as r,
  lo as s,
  Al as t,
  Sh as u,
  dd as v,
  Cd as w,
  xd as x,
  qt as y
};
